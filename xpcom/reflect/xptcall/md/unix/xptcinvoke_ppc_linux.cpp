                     ...
                     double* fpregs)
{
  ...
  for (uint32_t i = 0; i < paramCount; i++, s++) {
    ...
    if (!s->IsPtrData() && s->type == nsXPTType::T_DOUBLE) {
#ifndef __NO_FPRS__
      if (fpr < FPR_COUNT) fpregs[fpr++] = s->val.d;
#else
      if (gpr & 1) gpr++;
      if ((gpr + 1) < GPR_COUNT) {
      ...
      if (fpr < FPR_COUNT)
        fpregs[fpr++] =
            s->val.f;  // if passed in registers, floats are promoted to doubles
#else
      if (gpr < GPR_COUNT) *((float*)&gpregs[gpr++]) = s->val.f;
#endif
      else
        *((float*)d++) = s->val.f;
    } else if (!s->IsPtrData() &&
               (s->type == nsXPTType::T_I64 || s->type == nsXPTType::T_U64)) {
      if (gpr & 1)
...
extern "C" EXPORT_XPCOM_API(nsresult) NS_InvokeByIndex(nsISupports* that,
