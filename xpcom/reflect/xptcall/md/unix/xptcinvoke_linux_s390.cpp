...
static void
invoke_copy_to_stack(uint32_t paramCount,
                     nsXPTCVariant* s,
                     uint32_t* d_ov,
                     uint32_t overflow)
{
...
typedef nsresult (*vtable_func)(
    nsISupports*, uint32_t, uint32_t, uint32_t, uint32_t, double, double);
                 ...
                 nsXPTCVariant* params)
{
  ...
  uint32_t* stack_space = reinterpret_cast<uint32_t*>(
      __builtin_alloca((overflow + 8 /* 4 32-bits gpr + 2 64-bits fpr */) * 4));
  ...
  return method(
      that, d_gpr[0], d_gpr[1], d_gpr[2], d_gpr[3], d_fpr[0], d_fpr[1]);
}
