...
static inline void
alloc_float(double*& freg_args, uint64_t*& stack_args, double* end, float data)
{
  if (freg_args < end) {
...
extern "C" void
invoke_copy_to_stack(uint64_t* stk,
                     uint64_t* end,
                     uint32_t paramCount,
                     nsXPTCVariant* s)
{
...
extern "C" nsresult
_NS_InvokeByIndex(nsISupports* that,
