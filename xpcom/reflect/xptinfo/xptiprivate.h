...
#define LOG_AUTOREG(x)                      \
  do {                                      \
    printf x;                               \
    xptiInterfaceInfoManager::WriteToLog x; \
  } while (0)
#else
...
class xptiTypelibGuts
{
 ...
 private:
  explicit xptiTypelibGuts(XPTHeader* aHeader) : mHeader(aHeader) {}
  ~xptiTypelibGuts();
...
class xptiInterfaceEntry
{
 public:
  ...
  nsresult GetMethodInfoForName(const char* methodName,
                                uint16_t* index,
                                const nsXPTMethodInfo** info);
  nsresult GetConstant(uint16_t index, JS::MutableHandleValue, char** constant);
  nsresult GetInfoForParam(uint16_t methodIndex,
                           const nsXPTParamInfo* param,
                           nsIInterfaceInfo** _retval);
  nsresult GetIIDForParam(uint16_t methodIndex,
                          const nsXPTParamInfo* param,
                          nsIID** _retval);
  nsresult GetTypeForParam(uint16_t methodIndex,
                           const nsXPTParamInfo* param,
                           uint16_t dimension,
                           nsXPTType* _retval);
...
class xptiInterfaceInfo final : public nsIInterfaceInfo
{
 public:
  ...
  NS_IMETHOD GetMethodInfoForName(const char* methodName,
                                  uint16_t* index,
                                  const nsXPTMethodInfo** info) override
  {
    return !mEntry ? NS_ERROR_UNEXPECTED
  ...
  NS_IMETHOD GetConstant(uint16_t index,
  ...
  NS_IMETHOD GetInfoForParam(uint16_t methodIndex,
                             const nsXPTParamInfo* param,
  ...
  NS_IMETHOD GetIIDForParam(uint16_t methodIndex,
                            const nsXPTParamInfo* param,
  ...
  NS_IMETHOD GetTypeForParam(uint16_t methodIndex,
                             const nsXPTParamInfo* param,
                             uint16_t dimension,
                             nsXPTType* _retval) override
  {
    return !mEntry ? NS_ERROR_UNEXPECTED
                   : mEntry->GetTypeForParam(
                         methodIndex, param, dimension, _retval);
  }
  ...
  {
    return !mEntry ? NS_ERROR_UNEXPECTED
                   : mEntry->GetSizeIsArgNumberForParam(
                         methodIndex, param, dimension, _retval);
  }
  ...
  {
    return !mEntry ? NS_ERROR_UNEXPECTED
                   : mEntry->GetInterfaceIsArgNumberForParam(
                         methodIndex, param, _retval);
  }
