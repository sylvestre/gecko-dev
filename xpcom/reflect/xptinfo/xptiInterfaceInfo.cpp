...
xptiInterfaceEntry::Create(const char* name,
                           const nsID& iid,
...
xptiInterfaceEntry::ResolveLocked()
{
  ...
  if (resolvedState == FULLY_RESOLVED) return true;
  if (resolvedState == RESOLVE_FAILED) return false;
  ...
  if (parent_index) {
    ...
    mMethodBaseIndex =
        parent->mMethodBaseIndex + parent->mDescriptor->num_methods;

    mConstantBaseIndex =
        parent->mConstantBaseIndex + parent->mDescriptor->num_constants;
  }
...
{
  if (!EnsureResolved()) return NS_ERROR_UNEXPECTED;
...
xptiInterfaceEntry::GetMethodCount(uint16_t* count)
{
  ...
  *count = mMethodBaseIndex + mDescriptor->num_methods;
  return NS_OK;
}
...
xptiInterfaceEntry::GetConstantCount(uint16_t* count)
{
  ...
  *count = mConstantBaseIndex + mDescriptor->num_constants;
  return NS_OK;
}
...
xptiInterfaceEntry::GetMethodInfo(uint16_t index, const nsXPTMethodInfo** info)
{
  ...
  if (index < mMethodBaseIndex) return mParent->GetMethodInfo(index, info);

  if (index >= mMethodBaseIndex + mDescriptor->num_methods) {
    NS_ERROR("bad param");
    *info = nullptr;
                                         ...
                                         const nsXPTMethodInfo** result)
{
  ...
  for (uint16_t i = 0; i < mDescriptor->num_methods; ++i) {
    ...
    info =
        reinterpret_cast<nsXPTMethodInfo*>(&mDescriptor->method_descriptors[i]);
...
nsresult
xptiInterfaceEntry::GetConstant(uint16_t index,
                                ...
                                char** name)
{
  ...
  if (index >= mConstantBaseIndex + mDescriptor->num_constants) {
                                              ...
                                              uint16_t* interfaceIndex)
{
  ...
  if (methodIndex < mMethodBaseIndex)
    return mParent->GetInterfaceIndexForParam(
        methodIndex, param, interfaceIndex);

  if (methodIndex >= mMethodBaseIndex + mDescriptor->num_methods) {
    NS_ERROR("bad param");
    return NS_ERROR_INVALID_ARG;
  }
                                     ...
                                     xptiInterfaceEntry** entry)
{
  ...
  nsresult rv = GetInterfaceIndexForParam(methodIndex, param, &interfaceIndex);
  if (NS_FAILED(rv)) {
    return rv;
  }
                                   ...
                                   const XPTTypeDescriptor** type)
{
  ...
  const XPTTypeDescriptor* td = &param->type;
  const XPTTypeDescriptor* additional_types = mDescriptor->additional_types;
                                               ...
                                               uint8_t* argnum)
{
  ...
  if (methodIndex < mMethodBaseIndex)
    return mParent->GetSizeIsArgNumberForParam(
        methodIndex, param, dimension, argnum);
                                                    ...
                                                    uint8_t* argnum)
{
  ...
  if (methodIndex < mMethodBaseIndex)
    return mParent->GetInterfaceIsArgNumberForParam(methodIndex, param, argnum);
...
xptiInterfaceEntry::HasAncestor(const nsIID* iid, bool* _retval)
{
  *_retval = false;
  ...
  for (xptiInterfaceEntry* current = this; current;
       current = current->mParent) {
    if (current->mIID.Equals(*iid)) {
      *_retval = true;
      break;
    }
    if (!current->EnsureResolved()) return NS_ERROR_UNEXPECTED;
  }
...
xptiInterfaceInfo::BuildParent()
{
  ...
  NS_ASSERTION(
      mEntry && mEntry->IsFullyResolved() && !mParent && mEntry->Parent(),
...
xptiInterfaceInfo::xptiInterfaceInfo(xptiInterfaceEntry* entry) : mEntry(entry)
{
}
...
xptiInterfaceInfo::Release(void)
{
  ...
  if (!cnt) {
    ...
    if (entry && !entry->InterfaceInfoEquals(this)) return 0;
