...
static mach_error_t
allocateBranchIsland(BranchIsland** island, void* originalFunctionAddress);

mach_error_t
freeBranchIsland(BranchIsland* island);

#if defined(__ppc__) || defined(__POWERPC__)
mach_error_t
setBranchIslandTarget(BranchIsland* island,
                      const void* branchTo,
                      long instruction);
#endif
...
mach_error_t
setBranchIslandTarget_i386(BranchIsland* island,
                           const void* branchTo,
                           char* instructions);
void
atomic_mov64(uint64_t* targetAddress, uint64_t value);

static Boolean
eatKnownInstructions(unsigned char* code,
...
static void
fixupInstructions(uint32_t offset,
                  ...
                  void** originalFunctionReentryIsland)
{
  ...
  for (;;) {
    if (*(uint16_t*)originalFunctionAddress ==
        0x25FF)  // jmp qword near [rip+0x????????]
      originalFunctionAddress =
          *(void**)((char*)originalFunctionAddress + 6 +
  ...
  for (;;) {
    if (*(uint16_t*)originalFunctionAddress == 0x25FF)  // jmp *0x????????
      originalFunctionAddress =
          **(void***)((uint16_t*)originalFunctionAddress + 1);
    else
      break;
  }
#endif
...
#define kMFCTRInstruction 0x7c0903a6
  ...
  if (!err) err = allocateBranchIsland(&escapeIsland, originalFunctionAddress);
  if (err) fprintf(stderr, "err = %x %s:%d\n", err, __FILE__, __LINE__);
  ...
  if (!err) {
    uint32_t addressOffset =
        ((char*)escapeIsland - (char*)originalFunctionPtr - 5);
  ...
  if (!err && originalFunctionReentryIsland) {
    ...
    if (!err) *originalFunctionReentryIsland = reentryIsland;
  }
  ...
  if (!err) {
    ...
    if (reentryIsland)
      err = setBranchIslandTarget_i386(
          reentryIsland,
          (void*)((char*)originalFunctionPtr + eatenCount),
          originalInstructions);
      // try making islands executable before planting the jmp
#if defined(__x86_64__) || defined(__i386__)
    if (!err) err = makeIslandExecutable(escapeIsland);
    if (!err && reentryIsland) err = makeIslandExecutable(reentryIsland);
#endif
    if (!err)
  ...
  //	Clean up on error.
  if (err) {
    if (reentryIsland) freeBranchIsland(reentryIsland);
    if (escapeIsland) freeBranchIsland(escapeIsland);
  }
...
static bool
jump_in_range(intptr_t from, intptr_t to)
{
...
static mach_error_t
allocateBranchIslandAux(BranchIsland** island,
...
#if __WORDSIZE == 64
    if (!jump_in_range(original_address, new_address)) break;
#endif
    address = new_address;
...
freeBranchIsland(BranchIsland* island)
{
  ...
  assert(sizeof(BranchIsland) <= kPageSize);
  return vm_deallocate(mach_task_self(), (vm_address_t)island, kPageSize);
}
                           ...
                           char* instructions)
{
  ...
  if (instructions) {
    bcopy(instructions,
          island->instructions + kInstructions,
          kOriginalInstructionsSize);
  }

  // Fill in the address.
  int32_t addressOffset =
      (char*)branchTo - (island->instructions + kJumpAddress + 4);
...
static AsmInstructionMatch possibleInstructions[] = {
    {0x5,
     ...
     {0xE9, 0x00, 0x00, 0x00, 0x00}},  // jmp 0x????????
    {0x5,
     {0xFF, 0xFF, 0xFF, 0xFF, 0xFF},
     {0x55, 0x89, 0xe5, 0xc9, 0xc3}},   // push %ebp; mov %esp,%ebp; leave; ret
    {0x1, {0xFF}, {0x90}},              // nop
    ...
    {0x6,
     ...
     {0x81,
      ...
      0x00,
      0x00,
      0x00}},                           // sub 0x??, %esp with 32bit immediate
    ...
    {0x4,
     {0xFF, 0xFF, 0xFF, 0x00},
     {0x8B, 0x4C, 0x24, 0x00}},  // mov $imm(%esp), %ecx
    {0x5,
     {0xFF, 0x00, 0x00, 0x00, 0x00},
     {0xB8, 0x00, 0x00, 0x00, 0x00}},  // mov $imm, %eax
    {0x6,
     {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF},
     {0xE8, 0x00, 0x00, 0x00, 0x00, 0x58}},  // call $imm; pop %eax
    {0x0}};
#elif defined(__x86_64__)
static AsmInstructionMatch possibleInstructions[] = {
    {0x5,
    ...
    {0x4,
     {0xFF, 0xFF, 0xFF, 0x00},
     {0x48, 0x83, 0xEC, 0x00}},  // sub 0x??, %rsp
    {0x4, {0xFB, 0xFF, 0x00, 0x00}, {0x48, 0x89, 0x00, 0x00}},  // move onto rbp
    {0x4,
     {0xFF, 0xFF, 0xFF, 0xFF},
     {0x40, 0x0f, 0xbe, 0xce}},         // movsbl %sil, %ecx
    {0x2, {0xFF, 0x00}, {0x41, 0x00}},  // push %rXX
    {0x2, {0xFF, 0x00}, {0x85, 0x00}},  // test %rX,%rX
    {0x5,
     {0xF8, 0x00, 0x00, 0x00, 0x00},
     {0xB8, 0x00, 0x00, 0x00, 0x00}},               // mov $imm, %reg
...
static Boolean
codeMatchesInstruction(unsigned char* code, AsmInstructionMatch* instruction)
{
  Boolean match = true;
                     ...
                     uint8_t* originalInstructionSizes)
{
  ...
  while (remainsToEat > 0) {
    ...
    if (!curInstructionKnown) {
      ...
      fprintf(stderr,
              "mach_override: some instructions unknown! Need to update "
              "mach_override.c\n");
      break;
    }
  ...
  if (originalInstructions) {
    Boolean enoughSpaceForOriginalInstructions =
        (totalEaten < kOriginalInstructionsSize);

    if (enoughSpaceForOriginalInstructions) {
      memset(originalInstructions,
             0x90 /* NOP */,
  ...
  if (allInstructionsKnown) {
    ...
    *newInstruction |=
        (currentFirst64BitsOfCode & 0x0000000000FFFFFFLL);  // set last 3 bytes
  }
                  ...
                  uint8_t* instructionSizes)
{
  ...
  for (index = 0; index < instructionCount; index += 1) {
    ...
    instructionsToFix =
        (void*)((uintptr_t)instructionsToFix + instructionSizes[index]);
  }
}
#endif
