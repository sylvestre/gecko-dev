...
static bool
DoInterfaceDirectoryEntry(XPTArena* arena,
                          NotNull<XPTCursor*> cursor,
                          XPTInterfaceDirectoryEntry* ide);

static bool
DoConstDescriptor(XPTArena* arena,
                  NotNull<XPTCursor*> cursor,
                  XPTConstDescriptor* cd,
                  XPTInterfaceDescriptor* id);

static bool
DoMethodDescriptor(XPTArena* arena,
                   NotNull<XPTCursor*> cursor,
...
static bool
DoInterfaceDescriptor(XPTArena* arena,
                      NotNull<XPTCursor*> outer,
                      XPTInterfaceDescriptor** idp);

static bool
DoTypeDescriptorPrefix(XPTArena* arena,
                       NotNull<XPTCursor*> cursor,
                       XPTTypeDescriptorPrefix* tdp);

static bool
DoTypeDescriptor(XPTArena* arena,
                 NotNull<XPTCursor*> cursor,
                 XPTTypeDescriptor* td,
                 XPTInterfaceDescriptor* id);

static bool
DoParamDescriptor(XPTArena* arena,
                  NotNull<XPTCursor*> cursor,
...
XPT_DoHeader(XPTArena* arena, NotNull<XPTCursor*> cursor, XPTHeader** headerp)
{
  ...
  *headerp = header;
  ...
  if (header->num_interfaces) {
    ...
    if (!header->interface_directory) return false;
  }
  ...
  for (i = 0; i < header->num_interfaces; i++) {
    if (!DoInterfaceDirectoryEntry(
            arena, cursor, &header->interface_directory[i]))
      return false;
  }
...
static bool
InterfaceDescriptorAddTypes(XPTArena* arena,
                            XPTInterfaceDescriptor* id,
                            uint16_t num)
{
  ...
  if (num + uint16_t(id->num_additional_types) > 256) return false;
                      ...
                      XPTInterfaceDescriptor** idp)
{
  ...
  *idp = id;
  ...
  if (id->num_methods) {
    ...
    if (!id->method_descriptors) return false;
  }
  ...
  if (id->num_constants) {
    ...
    if (!id->const_descriptors) return false;
  }
                   ...
                   XPTInterfaceDescriptor* id)
{
  ...
  if (md->num_args) {
    ...
    if (!md->params) return false;
  }

  for (i = 0; i < md->num_args; i++) {
    if (!DoParamDescriptor(arena, cursor, &md->params[i], id)) return false;
  }

  if (!DoParamDescriptor(arena, cursor, &md->result, id)) return false;
                 ...
                 XPTInterfaceDescriptor* id)
{
  ...
  switch (XPT_TDP_TAG(td->prefix)) {
    ...
    case TD_INTERFACE_IS_TYPE:
      if (!XPT_Do8(cursor, &td->u.interface_is.argnum)) return false;
      break;
    case TD_ARRAY: {
      // argnum2 appears in the on-disk format but it isn't used.
      uint8_t argnum2 = 0;
      if (!XPT_Do8(cursor, &td->u.array.argnum) || !XPT_Do8(cursor, &argnum2))
        return false;

      if (!InterfaceDescriptorAddTypes(arena, id, 1)) return false;
      td->u.array.additional_type = id->num_additional_types - 1;

      if (!DoTypeDescriptor(arena,
                            cursor,
