...
XPT_NewArena(size_t block_size8, size_t block_size1)
{
  ...
  if (arena) {
    if (block_size8 < XPT_MIN_BLOCK_SIZE) block_size8 = XPT_MIN_BLOCK_SIZE;
    arena->subarena8.block_size = ALIGN_RND(block_size8, 8);

    if (block_size1 < XPT_MIN_BLOCK_SIZE) block_size1 = XPT_MIN_BLOCK_SIZE;
...
XPT_ArenaCalloc(XPTArena* arena, size_t size, size_t alignment)
{
  ...
  if (bytes > subarena->space) {
    ...
    new_block = static_cast<BLK_HDR*>(calloc(new_space / alignment, alignment));
    if (!new_block) {
      subarena->next = NULL;
      subarena->space = 0;
      return NULL;
    ...
    subarena->next = reinterpret_cast<uint8_t*>(new_block) + block_header_size;
