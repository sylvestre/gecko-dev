...
extern XPT_PUBLIC_API(bool) XPT_SkipStringInline(NotNull<XPTCursor*> cursor);

extern XPT_PUBLIC_API(bool) XPT_DoCString(XPTArena* arena,
                                          NotNull<XPTCursor*> cursor,
                                          char** strp,
                                          bool ignore = false);

extern XPT_PUBLIC_API(bool) XPT_DoIID(NotNull<XPTCursor*> cursor, nsID* iidp);

extern XPT_PUBLIC_API(bool) XPT_Do64(NotNull<XPTCursor*> cursor, int64_t* u64p);
...
extern XPT_PUBLIC_API(bool) XPT_Do8(NotNull<XPTCursor*> cursor, uint8_t* u8p);

extern XPT_PUBLIC_API(bool) XPT_DoHeader(XPTArena* arena,
                                         NotNull<XPTCursor*> cursor,
                                         XPTHeader** headerp);

typedef enum { XPT_HEADER = 0, XPT_DATA = 1 } XPTPool;
...
extern XPT_PUBLIC_API(bool) XPT_MakeCursor(XPTState* state,
                                           XPTPool pool,
                                           uint32_t len,
                                           NotNull<XPTCursor*> cursor);
