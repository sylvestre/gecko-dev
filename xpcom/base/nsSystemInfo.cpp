           ...
           nsAutoCString& aRevision)
{
  ...
  ScopedHandle handle(::CreateFileW(volumeMountPoint,
                                    0,
...
GetInstallYear(uint32_t& aYear)
{
  ...
  status = RegQueryValueExW(
      hKey, L"InstallDate", nullptr, &type, (LPBYTE)&raw_time, &time_size);
...
nsresult
GetCountryCode(nsAString& aCountryCode)
{
  ...
  numChars = GetGeoInfoW(geoid,
                         GEO_ISO2,
                         char16ptr_t(aCountryCode.BeginWriting()),
                         aCountryCode.Length(),
                         0);
  if (!numChars) {
    return NS_ERROR_FAILURE;
...
#ifdef XP_MACOSX
static nsresult
GetAppleModelId(nsAutoCString& aModelId)
{
  size_t numChars = 0;
...
static void
GetProcessorInformation(int* physical_cpus,
                        int* cache_size_L2,
                        int* cache_size_L3)
{
  ...
  *cache_size_L3 = 0;  // This will be in kbytes
  ...
  for (size_t i = 0;
       i < return_length / sizeof(SYSTEM_LOGICAL_PROCESSOR_INFORMATION);
       ++i) {
...
nsSystemInfo::Init()
{
  ...
  if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, keyName, 0, KEY_QUERY_VALUE, &key) ==
    ...
    if (RegQueryValueEx(
            key, L"~Mhz", 0, 0, reinterpret_cast<LPBYTE>(&data), &len) ==
        ERROR_SUCCESS) {
  ...
  if (gtkver_len <= 0) {
    gtkver_len = SprintfLiteral(gtkver,
                                "GTK %u.%u.%u",
                                gtk_major_version,
                                gtk_minor_version,
                                gtk_micro_version);
...
nsSystemInfo::GetAndroidSystemInfo(AndroidSystemInfo* aInfo)
{
  ...
  if (mozilla::AndroidBridge::Bridge()->GetStaticStringField(
    ...
    int minor_version;
    int bugfix_version;
    int num_read = sscanf(NS_ConvertUTF16toUTF8(str).get(),
                          "%d.%d.%d",
                          &major_version,
                          &minor_version,
                          &bugfix_version);
...
#if defined(XP_WIN)
NS_IMETHODIMP
nsSystemInfo::Observe(nsISupports* aSubject,
                      const char* aTopic,
