 ...
 public:
  ...
  NS_IMETHOD Run() override
  {
    ...
    dumper->DumpMemoryInfoToTempDir(
        mIdentifier, mAnonymize, mMinimizeMemoryUsage);
    return NS_OK;
  }
 ...
 public:
  ...
  NS_IMETHOD Run() override
  {
    ...
    dumper->DumpGCAndCCLogsToFile(
        mIdentifier, mDumpAllTraces, mDumpChildProcesses, this);
    return NS_OK;
  }
...
NS_IMPL_ISUPPORTS_INHERITED(GCAndCCLogDumpRunnable,
                            Runnable,
...
void
doMemoryReport(const uint8_t aRecvSig)
{
...
void
doGCCCDump(const uint8_t aRecvSig)
{
...
OnFifoEnabledChange(const char* /*unused*/, void* /*unused*/)
{
  ...
  if (SetupFifo()) {
    Preferences::UnregisterCallback(
        OnFifoEnabledChange, FifoWatcher::kPrefName, nullptr);
  }
}
...
// have gone away.
class nsDumpGCAndCCLogsCallbackHolder final : public nsIDumpGCAndCCLogsCallback
{
 public:
  NS_DECL_ISUPPORTS
                                          ...
                                          nsIDumpGCAndCCLogsCallback* aCallback)
{
  ...
  if (aDumpChildProcesses) {
    ...
    for (uint32_t i = 0; i < children.Length(); i++) {
      ...
      Unused << cp->CycleCollectWithLogs(
          aDumpAllTraces, logSink, callbackHolder);
    }
  }
...
static void
MakeFilename(const char* aPrefix,
...
static nsresult
DumpMemoryInfoToFile(nsIFile* aReportsFile,
                     nsIFinishDumpingCallback* aFinishDumping,
                     nsISupports* aFinishDumpingData,
                     bool aAnonymize,
                     bool aMinimizeMemoryUsage,
                     nsAString& aDMDIdentifier)
{
  ...
  rv = mgr->GetReportsExtended(handleReportAndFinishReporting,
                               nullptr,
                               handleReportAndFinishReporting,
                               nullptr,
                               aAnonymize,
    ...
    bool aAnonymize)
{
  ...
  return DumpMemoryInfoToFile(reportsFile,
                              aFinishDumping,
                              aFinishDumpingData,
                              aAnonymize,
                                            ...
                                            bool aMinimizeMemoryUsage)
{
  ...
  MakeFilename("unified-memory-report",
               identifier,
               getpid(),
               "json.gz",
  ...
  return DumpMemoryInfoToFile(reportsTmpFile,
                              finishDumping,
                              nullptr,
                              aAnonymize,
                              aMinimizeMemoryUsage,
...
nsresult
nsMemoryInfoDumper::OpenDMDFile(const nsAString& aIdentifier,
                                int aPid,
                                FILE** aOutFile)
{
  if (!dmd::IsRunning()) {
