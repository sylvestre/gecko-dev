...
NS_warn_if_impl(bool aCondition,
                const char* aExpr,
                const char* aFile,
                int32_t aLine)
{
...
#ifdef DEBUG
inline void
MOZ_PretendNoReturn() MOZ_PRETEND_NORETURN_FOR_STATIC_ANALYSIS
{
}
...
#define NS_NOTYETIMPLEMENTED(str)                                          \
  do {                                                                     \
    NS_DebugBreak(                                                         \
        NS_DEBUG_ASSERTION, str, "NotYetImplemented", __FILE__, __LINE__); \
    MOZ_PretendNoReturn();                                                 \
  } while (0)
#else
#define NS_NOTYETIMPLEMENTED(str) \
 ...
 */
void
printf_stderr(const char* aFmt, ...) MOZ_FORMAT_PRINTF(1, 2);

/**
 * Same as printf_stderr, but taking va_list instead of varargs
 */
void
vprintf_stderr(const char* aFmt, va_list aArgs) MOZ_FORMAT_PRINTF(1, 0);
 ...
 */
void
fprintf_stderr(FILE* aFile, const char* aFmt, ...) MOZ_FORMAT_PRINTF(2, 3);
