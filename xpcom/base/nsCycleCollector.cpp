...
void
SuspectUsingNurseryPurpleBuffer(void* aPtr,
...
static PLDHashTableOps PtrNodeOps = {PLDHashTable::HashVoidPtrKeyStub,
                                     PtrToNodeMatchEntry,
...
{
  nsPurpleBufferEntry(void* aObject,
                      nsCycleCollectingAutoRefCnt* aRefCnt,
...
struct nsPurpleBuffer
{
 private:
  ...
  typedef SegmentedVector<nsPurpleBufferEntry,
                          kSegmentSize,
 ...
 public:
  ...
  MOZ_ALWAYS_INLINE void Put(void* aObject,
                             nsCycleCollectionParticipant* aCp,
                             nsCycleCollectingAutoRefCnt* aRefCnt)
  {
...
static bool
AddPurpleRoot(CCGraphBuilder& aBuilder,
              void* aRoot,
              nsCycleCollectionParticipant* aParti);

struct SelectPointersVisitor
{
  explicit SelectPointersVisitor(CCGraphBuilder& aBuilder) : mBuilder(aBuilder)
  {
  }
...
class nsCycleCollector : public nsIMemoryReporter
{
 ...
 public:
  ...
  void Suspect(void* aPtr,
               nsCycleCollectionParticipant* aCp,
               nsCycleCollectingAutoRefCnt* aRefCnt);
  void SuspectNurseryEntries();
  uint32_t SuspectedCount();
  void ForgetSkippable(js::SliceBudget& aBudget,
                       bool aRemoveChildlessNodes,
                       bool aAsyncSnowWhiteFreeing);
...
class nsCycleCollectorLogger final : public nsICycleCollectorListener
{
 ...
 public:
  ...
  void NoteRefCountedObject(uint64_t aAddress,
                            uint32_t aRefCount,
  ...
  void NoteWeakMapEntry(uint64_t aMap,
                        uint64_t aKey,
      ...
      switch (d->mType) {
        ...
        case CCGraphDescriber::eGCMarkedObject:
          aHandler->NoteGCedObject(
              d->mAddress,
              d->mType == CCGraphDescriber::eGCMarkedObject,
              d->mName,
                             ...
                             public nsCycleCollectionNoteRootCallback
{
 ...
 public:
  ...
  NS_IMETHOD_(void)
  NoteXPCOMRoot(nsISupports* aRoot, nsCycleCollectionParticipant* aParticipant);
  NS_IMETHOD_(void) NoteJSRoot(JSObject* aRoot);
  NS_IMETHOD_(void)
  NoteNativeRoot(void* aRoot, nsCycleCollectionParticipant* aParticipant);
  NS_IMETHOD_(void)
  NoteWeakMapping(JSObject* aMap,
                  JS::GCCellPtr aKey,
                  JSObject* aKdelegate,
                  JS::GCCellPtr aVal);
  ...
  NS_IMETHOD_(void)
  NoteNativeChild(void* aChild, nsCycleCollectionParticipant* aParticipant);
...
CCGraphBuilder::DescribeRefCountedNode(nsrefcnt aRefCount, const char* aObjName)
{
  ...
  if (mLogger) {
    mLogger->NoteRefCountedObject(
        (uint64_t)mCurrPi->mPointer, aRefCount, aObjName);
  }
...
NS_IMETHODIMP_(void)
CCGraphBuilder::DescribeGCedNode(bool aIsMarked,
                                 const char* aObjName,
                                 uint64_t aCompartmentAddress)
{
  ...
  if (mLogger) {
    mLogger->NoteGCedObject(
        (uint64_t)mCurrPi->mPointer, aIsMarked, aObjName, aCompartmentAddress);
  }
...
NS_IMETHODIMP_(void)
CCGraphBuilder::NoteWeakMapping(JSObject* aMap,
                                JS::GCCellPtr aKey,
                                JSObject* aKdelegate,
                                JS::GCCellPtr aVal)
{
  ...
  if (mLogger) {
    mLogger->NoteWeakMapEntry((uint64_t)aMap,
                              aKey ? aKey.unsafeAsInteger() : 0,
                              (uint64_t)aKdelegate,
...
class SnowWhiteKiller : public TraceCallbacks
{
 ...
 public:
  ...
  virtual void Trace(JS::Heap<JS::Value>* aValue,
                     const char* aName,
  ...
  virtual void Trace(JS::Heap<jsid>* aId,
                     const char* aName,
                     void* aClosure) const override
  {
  }
  ...
  virtual void Trace(JS::Heap<JSObject*>* aObject,
                     const char* aName,
                     void* aClosure) const override
  {
  ...
  virtual void Trace(JSObject** aObject,
                     const char* aName,
                     void* aClosure) const override
  {
  ...
  virtual void Trace(JS::TenuredHeap<JSObject*>* aObject,
                     const char* aName,
                     void* aClosure) const override
  {
  ...
  virtual void Trace(JS::Heap<JSString*>* aString,
                     const char* aName,
                     void* aClosure) const override
  {
  }

  virtual void Trace(JS::Heap<JSScript*>* aScript,
                     const char* aName,
                     void* aClosure) const override
  {
  }

  virtual void Trace(JS::Heap<JSFunction*>* aFunction,
                     const char* aName,
                     void* aClosure) const override
  {
  }
...
nsPurpleBuffer::RemoveSkippable(nsCycleCollector* aCollector,
                                ...
                                CC_ForgetSkippableCallback aCb)
{
  RemoveSkippableVisitor visitor(
      aCollector, aBudget, aRemoveChildlessNodes, aAsyncSnowWhiteFreeing, aCb);
  VisitEntries(visitor);
}
                                  ...
                                  bool aAsyncSnowWhiteFreeing)
{
  ...
  mPurpleBuf.RemoveSkippable(this,
                             aBudget,
...
{
 public:
  PurpleScanBlackVisitor(CCGraph& aGraph,
                         nsCycleCollectorLogger* aLogger,
                         uint32_t& aCount,
                         bool& aFailed)
...
nsCycleCollector::ScanIncrementalRoots()
{
  ...
  // if they are garbage.
  bool failed = false;
  PurpleScanBlackVisitor purpleScanBlackVisitor(
      mGraph, mLogger, mWhiteNodeCount, failed);
                                 ...
                                 bool aAnonymize)
{
  ...
  SizeOfIncludingThis(
      CycleCollectorMallocSizeOf, &objectSize, &graphSize, &purpleBufferSize);

  if (objectSize > 0) {
    MOZ_COLLECT_REPORT("explicit/cycle-collector/collector-object",
                       KIND_HEAP,
                       UNITS_BYTES,
                       objectSize,
  ...
  if (graphSize > 0) {
    MOZ_COLLECT_REPORT(
        "explicit/cycle-collector/graph",
        KIND_HEAP,
        UNITS_BYTES,
        graphSize,
  ...
  if (purpleBufferSize > 0) {
    MOZ_COLLECT_REPORT("explicit/cycle-collector/purple-buffer",
                       KIND_HEAP,
                       UNITS_BYTES,
                       purpleBufferSize,
...
nsCycleCollector::Suspect(void* aPtr,
                          nsCycleCollectionParticipant* aParti,
                          nsCycleCollectingAutoRefCnt* aRefCnt)
{
  CheckThreadSafety();
...
SuspectAfterShutdown(void* aPtr,
                     nsCycleCollectionParticipant* aCp,
                     nsCycleCollectingAutoRefCnt* aRefCnt,
                     bool* aShouldDelete)
{
...
void
NS_CycleCollectorSuspect3(void* aPtr,
                          nsCycleCollectionParticipant* aCp,
                          nsCycleCollectingAutoRefCnt* aRefCnt,
                          bool* aShouldDelete)
{
                                 ...
                                 bool aAsyncSnowWhiteFreeing)
{
  ...
  TimeLog timeLog;
  data->mCollector->ForgetSkippable(
      aBudget, aRemoveChildlessNodes, aAsyncSnowWhiteFreeing);
...
void
nsCycleCollector_collectSlice(SliceBudget& budget, bool aPreferShorterSlices)
{
  CollectorData* data = sCollectorData.get();
