...
NS_IMPL_ISUPPORTS(BaseStringEnumerator,
                  nsISimpleEnumerator,
...
int
BaseStringEnumerator::SortCallback(const void* aE1,
                                   const void* aE2,
                                   void* /*unused*/)
{
...
nsresult
CategoryNode::GetLeaf(const char* aEntryName, char** aResult)
{
...
NS_IMPL_QUERY_INTERFACE(nsCategoryManager,
                        nsICategoryManager,
                        nsIMemoryReporter)
...
nsCategoryManager::CollectReports(nsIHandleReportCallback* aHandleReport,
                                  ...
                                  bool aAnonymize)
{
  MOZ_COLLECT_REPORT("explicit/xpcom/category-manager",
                     KIND_HEAP,
                     UNITS_BYTES,
                                    ...
                                    char** aOldValue)
{
  ...
  if (NS_SUCCEEDED(rv)) {
    ...
    NotifyObservers(
        NS_XPCOM_CATEGORY_ENTRY_ADDED_OBSERVER_ID, aCategoryName, aEntryName);
                                       ...
                                       bool aDontPersist)
{
  ...
  */
  ...
  if (category) {
    ...
    NotifyObservers(
        NS_XPCOM_CATEGORY_ENTRY_REMOVED_OBSERVER_ID, aCategoryName, aEntryName);
  }
...
nsCategoryManager::DeleteCategory(const char* aCategoryName)
{
  ...
  if (category) {
    category->Clear();
    NotifyObservers(
        NS_XPCOM_CATEGORY_CLEARED_OBSERVER_ID, aCategoryName, nullptr);
  }
                              ...
                              const char16_t* aObserverData)
{
  ...
  while (NS_SUCCEEDED(senumerator->HasMore(&hasMore)) && hasMore) {
    ...
    rv = categoryManager->GetCategoryEntry(
        aCategory, entryString.get(), getter_Copies(contractID));
    if (NS_FAILED(rv)) {
      continue;
    }
    ...
    if (aObserverTopic) {
      ...
      if (observer) {
        observer->Observe(
            aOrigin, aObserverTopic, aObserverData ? aObserverData : u"");
      } else {
        LogMessage(
