...
TimerObserverRunnable::Run()
{
  ...
  if (observerService) {
    ...
    observerService->AddObserver(
        mObserver, "suspend_process_notification", false);
    observerService->AddObserver(
        mObserver, "resume_process_notification", false);
  }
  return NS_OK;
}
...
TimerThread::Run()
{
  ...
  BinarySearchIf(MicrosecondsToInterval(),
                 0,
                 usForPosInterval,
                 IntervalComparator(),
...
NS_IMETHODIMP
TimerThread::Observe(nsISupports* /* aSubject */,
                     const char* aTopic,
