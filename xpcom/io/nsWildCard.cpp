              ...
              unsigned int aLevel)
{
  ...
  for (sx = 1;; ++sx) {
    ...
    nsCharTraits<T>::copy(
        e2 + count, aExpr + cp, nsCharTraits<T>::length(aExpr + cp) + 1);
  ...
  for (x = 0, y = 0; aExpr[y]; ++y, ++x) {
    ...
    switch (aExpr[y]) {
      ...
      case '*':
        ...
        while (aStr[x]) {
          ret = ::_shexp_match(
              &aStr[x++], &aExpr[y], aCaseInsensitive, aLevel + 1);
          switch (ret) {
            case NOMATCH:
              continue;
            case ABORTED:
      ...
      case '[': {
        ...
        if (::alphanumeric(end) && aExpr[i] == ']') {
          ...
          if (aCaseInsensitive && ::alpha(val)) {
            val = ::_is_char_in_range(
                (unsigned char)start, (unsigned char)end, (unsigned char)val);
            if (neg == val) {
              return NOMATCH;
            }
