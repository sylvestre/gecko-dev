...
TEST(Timers, TargetedTimers)
{
  ...
  nsresult rv = NS_NewTimerWithCallback(
      getter_AddRefs(timer), callback, 2000, nsITimer::TYPE_ONE_SHOT, target);
...
TEST(Timers, TimerWithStoppedTarget)
{
  ...
  nsCOMPtr<nsITimer> timer;
  nsresult rv = NS_NewTimerWithCallback(
      getter_AddRefs(timer), callback, 100, nsITimer::TYPE_ONE_SHOT, target);
  ASSERT_TRUE(NS_SUCCEEDED(rv));
...
class FindExpirationTimeState final
{
 public:
  ...
  void InitTimers(uint32_t aNumDifferingTimers,
                  uint32_t aType,
