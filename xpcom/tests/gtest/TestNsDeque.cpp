...
static bool
VerifyContents(const nsDeque& aDeque, const int* aContents, size_t aLength)
{
...
TEST(NsDeque, TestRangeFor)
{
  ...
  Test tests[] = {
      ...
      {3,
       [](nsDeque& d) { d.PopFront(); },
       1 + 2 + 3,
       "PopFront after 3rd loop"},

      {1,
       [](nsDeque& d) { d.Push(new int(4)); },
       1 + 2 + 3 + 4,
       "Push after 1st loop"},
      {2,
       [](nsDeque& d) { d.Push(new int(4)); },
      ...
      {1,
       [](nsDeque& d) { d.PushFront(new int(4)); },
       1 + 1 + 2 + 3,
      ...
      {2,
       ...
       1 + 2 + 6,
       "Erase after 2nd loop, Push 4,5,6"}};
