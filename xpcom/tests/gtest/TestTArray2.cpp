...
template<class T>
inline bool
operator<(const nsCOMPtr<T>& lhs, const nsCOMPtr<T>& rhs)
{
...
static bool
test_basic_array(ElementType* data, size_t dataLen, const ElementType& extra)
{
...
TEST(TArray, test_move_array)
{
  ...
  nsTArray<Countable> copyCountableArray2(
      Move(differentAllocatorCountableArray));
  // with auto
  ...
  FallibleTArray<Moveable> differentAllocatorMoveableArray2(
      Move(copyMoveableArray2));
  AutoTArray<Moveable, 4> autoMoveableArray2(
      Move(differentAllocatorMoveableArray2));
...
static bool
is_heap(const Array& ary, size_t len)
{
  size_t index = 1;
  while (index < len) {
