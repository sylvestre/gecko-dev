...
static Chunk kTest2Chunks[] = {
    Chunk(3, "Hel"), Chunk(3, "lo "), Chunk(3, "sir"), Chunk(0, nullptr)};
...
static Chunk kTest5Chunks[] = {
    Chunk(1, "B"), Chunk(2, "ob"), Chunk(0, nullptr)};

static Chunk kTest6Chunks[] = {
    Chunk(2, "Bo"), Chunk(1, "b"), Chunk(0, nullptr)};
                              ...
                              uint32_t* aRead)
{
  *aRead = 0;
  ...
  while (mChunk->mLength) {
    ...
    nsresult rv = (*aWriter)(
        this, aClosure, mChunk->mData, *aRead, mChunk->mLength, &written);
...
TEST(Base64, RFC4648DecodingRawPointers)
{
  for (auto& testcase : sRFC4648TestCases) {
    ...
    nsresult rv = mozilla::Base64Decode(
        testcase.mOutput, outputLength, &buffer, &binaryLength);
    ASSERT_TRUE(NS_SUCCEEDED(rv));
