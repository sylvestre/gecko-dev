...
static bool
VerifyResult(nsresult aRV, const char* aMsg)
{
...
static bool
TestInvalidFileName(nsIFile* aBase, const char* aName)
{
...
static bool
TestCreate(nsIFile* aBase, const char* aName, int32_t aType, int32_t aPerm)
{
  nsCOMPtr<nsIFile> file = NewFile(aBase);
  if (!file) return false;
...
static bool
TestCreateUnique(nsIFile* aBase,
                 const char* aName,
                 int32_t aType,
                 int32_t aPerm)
{
...
static bool
TestDeleteOnClose(nsIFile* aBase,
                  const char* aName,
                  int32_t aFlags,
                  int32_t aPerm)
{
  ...
  rv = file->OpenNSPRFileDesc(
      aFlags | nsIFile::DELETE_ON_CLOSE, aPerm, &fileDesc);
...
static bool
TestRemove(nsIFile* aBase, const char* aName, bool aRecursive)
{
  nsCOMPtr<nsIFile> file = NewFile(aBase);
  if (!file) return false;
...
static bool
TestParent(nsIFile* aBase, nsIFile* aStart)
{
...
static bool
TestNormalizeNativePath(nsIFile* aBase, nsIFile* aStart)
{
  nsCOMPtr<nsIFile> file = NewFile(aStart);
  if (!file) return false;
  ...
  EXPECT_TRUE(path.Equals(basePath))
      << "Incorrect normalization: " << path.get() << " - " << basePath.get();
