...
TEST(Pipes, ChainedPipes)
{
  ...
  rv = NS_NewNamedThread(
      "ChainedPipeRecv", getter_AddRefs(receiverThread), receiver);
  if (NS_FAILED(rv)) return;
...
// manual read loop.
static void
TestPipe2(uint32_t aNumBytes, uint32_t aSegmentSize = DEFAULT_SEGMENT_SIZE)
{
  ...
  nsresult rv = NS_NewPipe(
      getter_AddRefs(reader), getter_AddRefs(writer), aSegmentSize, maxSize);
...
static void
TestPipeClone(uint32_t aTotalBytes,
              uint32_t aNumWrites,
              ...
              uint32_t aSegmentSize = DEFAULT_SEGMENT_SIZE)
{
  ...
  nsresult rv = NS_NewPipe(getter_AddRefs(reader),
                           ...
                           maxSize,
                           true,
                           false);  // non-blocking - reader, writer
  ASSERT_TRUE(NS_SUCCEEDED(rv));
...
TestCloseDuringRead(uint32_t aSegmentSize, uint32_t aDataSize)
{
  ...
  uint32_t numRead = 0;
  rv = reader->ReadSegments(
      CloseDuringReadFunc, &outputData, inputData.Length(), &numRead);
  ASSERT_TRUE(NS_SUCCEEDED(rv));
