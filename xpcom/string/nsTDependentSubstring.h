...
class nsTDependentSubstring : public nsTSubstring<T>
{
 ...
 public:
  void Rebind(const substring_type&,
              uint32_t aStartPos,
  ...
  nsTDependentSubstring(const substring_type& aStr,
                        uint32_t aStartPos,
                        uint32_t aLength = size_type(-1))
      : substring_type()
  {
  ...
  nsTDependentSubstring(const char_type* aData, size_type aLength)
      : substring_type(
            const_cast<char_type*>(aData), aLength, DataFlags(0), ClassFlags(0))
  {
  }
...
Substring(const nsTSubstring<T>& aStr,
          uint32_t aStartPos,
...
template<typename T>
inline const nsTDependentSubstring<T>
Substring(const nsTLiteralString<T>& aStr,
          uint32_t aStartPos,
          uint32_t aLength = uint32_t(-1))
{
  return nsTDependentSubstring<T>(aStr, aStartPos, aLength);
...
template<typename T>
inline const nsTDependentSubstring<T>
Substring(const nsReadingIterator<T>& aStart, const nsReadingIterator<T>& aEnd)
{
