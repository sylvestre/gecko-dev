...
nsChromeRegistry::LogMessageWithContext(
    nsIURI* aURL, uint32_t aLineNumber, uint32_t flags, const char* aMsg, ...)
{
  nsresult rv;
...
static void
FlushSkinBindingsForWindow(nsPIDOMWindowOuter* aWindow)
{
  // Get the document.
...
nsresult
nsChromeRegistry::RefreshWindow(nsPIDOMWindowOuter* aWindow)
{
  ...
  for (StyleSheet* sheet : oldSheets) {
    MOZ_ASSERT(sheet,
               "GetStyleSheetAt shouldn't return nullptr for "
...
nsChromeRegistry::FlushAllCaches()
{
  ...
  obsSvc->NotifyObservers(
      (nsIChromeRegistry*)this, NS_CHROME_FLUSH_TOPIC, nullptr);
}
