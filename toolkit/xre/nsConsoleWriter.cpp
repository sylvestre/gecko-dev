...
WriteConsoleLog()
{
  ...
  PRFileDesc* file;
  rv = lfile->OpenNSPRFileDesc(
      PR_WRONLY | PR_APPEND | PR_CREATE_FILE, 0660, &file);
  if (NS_FAILED(rv)) return;
  ...
  if (mcount) {
    ...
    PR_FormatTimeUSEnglish(
        datetime, sizeof(datetime), "%Y-%m-%d %H:%M:%S", &etime);

    PR_fprintf(
        file, NS_LINEBREAK "*** Console log: %s ***" NS_LINEBREAK, datetime);
  }
