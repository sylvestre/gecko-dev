...
extern "C" {
static void
my_glib_log_func(const gchar* log_domain,
                 ...
                 gpointer user_data)
{
  if (log_level &
    ...
    NS_DebugBreak(
        NS_DEBUG_ASSERTION, message, "glib assertion", __FILE__, __LINE__);
  } else if (log_level & (G_LOG_LEVEL_CRITICAL | G_LOG_LEVEL_WARNING)) {
    NS_DebugBreak(
        NS_DEBUG_WARNING, message, "glib warning", __FILE__, __LINE__);
  }
...
#ifdef SA_SIGINFO
static void
fpehandler(int signum, siginfo_t* si, void* context)
{
  ...
  if (si->si_code == FPE_INTDIV || si->si_code == FPE_INTOVF) {
    NS_DebugBreak(
        NS_DEBUG_ABORT, "Divide by zero", nullptr, __FILE__, __LINE__);
  }
...
void
InstallSignalHandlers(const char* aProgname)
{
#if defined(CRAWL_STACK_ON_SIGSEGV)
