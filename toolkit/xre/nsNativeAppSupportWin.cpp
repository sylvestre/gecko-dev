...
class nsNativeAppSupportWin : public nsNativeAppSupportBase, public nsIObserver
{
 public:
  NS_DECL_NSIOBSERVER
...
struct MessageWindow
{
  ...
  // Window proc.
  static LRESULT CALLBACK WindowProc(HWND msgWindow,
                                     UINT msg,
                                     WPARAM wp,
                                     LPARAM lp)
  {
    if (msg == WM_COPYDATA) {
      ...
      if (1 >= cds->dwData) {
        ...
        NS_NewLocalFile(
            NS_ConvertUTF8toUTF16(wdpath), false, getter_AddRefs(workingDir));
      }
...
nsNativeAppSupportWin::Start(bool* aResult)
{
  ...
  *aResult = false;
  ...
  ::_snwprintf(
      ...
      sizeof mMutexName / sizeof(char16_t),
      L"%s%s%s",
      MOZ_MUTEX_NAMESPACE,
...
NS_IMETHODIMP
nsNativeAppSupportWin::Observe(nsISupports* aSubject,
                               const char* aTopic,
...
static nsCString
hszValue(DWORD instance, HSZ hsz)
{
...
static void
escapeQuotes(nsAString& aString)
{
  int32_t offset = -1;
  while (1) {
    // Find next '"'.
...
#endif
          // Now handle it.
          HandleCommandLine(NS_ConvertUTF16toUTF8(url).get(),
                            nullptr,
        ...
        case topicGetWindowInfo: {
          ...
          do {
            ...
            nsCOMPtr<nsPIDOMWindowOuter> internalContent =
                nsGlobalWindow::Cast(piNavWin)->GetContent();
            if (!internalContent) {
              break;
            }
            // Get location.
...
// of the arg.
static int32_t
advanceToEndOfQuotedArg(const WCHAR* p, int32_t offset, int32_t len)
{
...
void
nsNativeAppSupportWin::ParseDDEArg(const WCHAR* args,
                                   int index,
                                   nsString& aString)
{
  if (args) {
...
void
nsNativeAppSupportWin::ParseDDEArg(HSZ args, int index, nsString& aString)
{
...
HDDEDATA
nsNativeAppSupportWin::CreateDDEData(LPBYTE value, DWORD len)
{
  HDDEDATA result =
...
void
nsNativeAppSupportWin::ActivateLastWindow()
{
  nsCOMPtr<mozIDOMWindowProxy> navWin;
  GetMostRecentWindow(u"navigator:browser", getter_AddRefs(navWin));
  if (navWin) {
...
HWND
hwndForDOMWindow(mozIDOMWindowProxy* window)
{
  if (!window) {
    return 0;
  }
