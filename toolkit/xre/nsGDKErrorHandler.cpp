 ...
 */
static void
GdkErrorHandler(const gchar* log_domain,
                ...
                gpointer user_data)
{
  if (strstr(message, "X Window System error")) {
    ...
    start = buffer.Find(minorCodeString,
                        /* aIgnoreCase = */ false,
                        endptr - buffer.BeginReading());
    if (!start) NS_RUNTIMEABORT(message);
