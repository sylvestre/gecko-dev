...
UpdateUninstallerVersionString(LPWSTR versionString)
{
  ...
  LONG rv = RegSetValueExW(uninstallKey,
                           L"DisplayVersion",
                           0,
                           REG_SZ,
 ...
 */
static BOOL
GetVersionNumberFromPath(LPWSTR path, DWORD& A, DWORD& B, DWORD& C, DWORD& D)
{
  ...
  if (!GetFileVersionInfoW(
          path, 0, fileVersionInfoSize, fileVersionInfo.get())) {
    LOG_WARN(
...
UpdateServiceDescription(SC_HANDLE serviceHandle)
{
  ...
  if (!MultiByteToWideChar(
          CP_UTF8,
          0,
          serviceStrings.serviceDescription,
          -1,
          serviceDescription,
  ...
  if (!ChangeServiceConfig2W(
          serviceHandle, SERVICE_CONFIG_DESCRIPTION, &descriptionConfig)) {
...
SvcInstall(SvcInstallAction action)
{
  ...
  if (schService) {
    ...
    BOOL obtainedExistingVersionInfo =
        GetVersionNumberFromPath(serviceConfig.lpBinaryPathName,
                                 ...
                                 existingC,
                                 existingD);
    if (!GetVersionNumberFromPath(
            newServiceBinaryPath, newA, newB, newC, newD)) {
    ...
    if (ForceInstallSvc == action || !obtainedExistingVersionInfo ||
      ...
      if (!CopyFileW(
              newServiceBinaryPath, serviceConfig.lpBinaryPathName, FALSE)) {
        LOG_WARN(
        ...
        if (len > 3) {
          ...
          wcsncpy(
              oldServiceBinaryTempPath, serviceConfig.lpBinaryPathName, len);
      ...
      if (MoveFileExW(
              newServiceBinaryPath, nullptr, MOVEFILE_DELAY_UNTIL_REBOOT)) {
  ...
  schService.own(CreateServiceW(schSCManager,
                                SVC_NAME,
                                SVC_DISPLAY_NAME,
                                SERVICE_ALL_ACCESS,
                         ...
                         PSECURITY_DESCRIPTOR psd)
{
  ...
  if (!QueryServiceObjectSecurity(
          hService, DACL_SECURITY_INFORMATION, &psd, 0, &needed)) {
    ...
    if (!QueryServiceObjectSecurity(
            hService, DACL_SECURITY_INFORMATION, psd, size, &needed)) {
      LOG_WARN(
          ("Could not allocate security descriptor.  (%d)", GetLastError()));
      return GetLastError();
    }
  ...
  EXPLICIT_ACCESS ea;
  BuildExplicitAccessWithNameW(&ea,
                               accountName,
