...
#define NOTIFY_RESULT_OBSERVERS_RET(_result, _method, _ret)        \
  ...
  if (!_result->mSuppressNotifications) {                          \
    ENUMERATE_WEAKARRAY(                                           \
        _result->mObservers, nsINavHistoryResultObserver, _method) \
  }                                                                \
  PR_END_MACRO
...
inline int32_t
ComparePRTime(PRTime a, PRTime b)
{
  if (a < b)
    return -1;
  else if (a > b)
...
inline int32_t
CompareIntegers(uint32_t a, uint32_t b)
{
  return a - b;
}
...
nsNavHistoryResultNode::GetGeneratingOptions()
{
  if (!mParent) {
    ...
    // generating options.
    if (IsContainer()) return GetAsContainer()->mOptions;

    NS_NOTREACHED(
        "Can't find a generating node for this container, perhaps FillStats "
        "has not been called on this tree yet?");
    return nullptr;
  }
  ...
  while (cur) {
    if (cur->IsContainer()) return cur->GetAsContainer()->mOptions;
    cur = cur->mParent;
  }

  // We should always find a container node as an ancestor.
  NS_NOTREACHED(
      "Can't find a generating node for this container, the tree seemes "
      "corrupted.");
  return nullptr;
}
    ...
    if (shouldNotify) {
      NOTIFY_RESULT_OBSERVERS(
          ...
          NodeHistoryDetailsChanged(
              TO_ICONTAINER(mParent), oldTime, oldAccessCount));
    }
...
nsresult
nsNavHistoryContainerResultNode::Refresh()
{
  NS_WARNING(
 ...
 */
void
nsNavHistoryContainerResultNode::RecursiveSort(const char* aData,
...
nsNavHistoryContainerResultNode::DoesChildNeedResorting(
    uint32_t aIndex, SortComparator aComparator, const char* aData)
{
...
/* static */
int32_t
nsNavHistoryContainerResultNode::SortComparison_StringLess(const nsAString& a,
                                                           const nsAString& b)
{
  nsNavHistory* history = nsNavHistory::GetHistoryService();
 ...
 */
int32_t
nsNavHistoryContainerResultNode::SortComparison_TitleLess(
    nsNavHistoryResultNode* a, nsNavHistoryResultNode* b, void* closure)
{
  uint32_t aType;
  a->GetType(&aType);
...
int32_t
nsNavHistoryContainerResultNode::SortComparison_TitleGreater(
    nsNavHistoryResultNode* a, nsNavHistoryResultNode* b, void* closure)
{
 ...
 */
int32_t
nsNavHistoryContainerResultNode::SortComparison_DateLess(
    nsNavHistoryResultNode* a, nsNavHistoryResultNode* b, void* closure)
{
...
int32_t
nsNavHistoryContainerResultNode::SortComparison_DateGreater(
    nsNavHistoryResultNode* a, nsNavHistoryResultNode* b, void* closure)
{
  return -nsNavHistoryContainerResultNode::SortComparison_DateLess(
      a, b, closure);
}

int32_t
nsNavHistoryContainerResultNode::SortComparison_DateAddedLess(
    nsNavHistoryResultNode* a, nsNavHistoryResultNode* b, void* closure)
{
...
int32_t
nsNavHistoryContainerResultNode::SortComparison_DateAddedGreater(
    nsNavHistoryResultNode* a, nsNavHistoryResultNode* b, void* closure)
{
...
int32_t
nsNavHistoryContainerResultNode::SortComparison_LastModifiedLess(
    nsNavHistoryResultNode* a, nsNavHistoryResultNode* b, void* closure)
{
...
int32_t
nsNavHistoryContainerResultNode::SortComparison_LastModifiedGreater(
    nsNavHistoryResultNode* a, nsNavHistoryResultNode* b, void* closure)
{
 ...
 */
int32_t
nsNavHistoryContainerResultNode::SortComparison_URILess(
    nsNavHistoryResultNode* a, nsNavHistoryResultNode* b, void* closure)
{
  int32_t value;
  if (a->IsURI() && b->IsURI()) {
...
int32_t
nsNavHistoryContainerResultNode::SortComparison_URIGreater(
    nsNavHistoryResultNode* a, nsNavHistoryResultNode* b, void* closure)
{
...
int32_t
nsNavHistoryContainerResultNode::SortComparison_KeywordLess(
    nsNavHistoryResultNode* a, nsNavHistoryResultNode* b, void* closure)
{
  int32_t value = 0;
...
int32_t
nsNavHistoryContainerResultNode::SortComparison_KeywordGreater(
    nsNavHistoryResultNode* a, nsNavHistoryResultNode* b, void* closure)
{
...
int32_t
nsNavHistoryContainerResultNode::SortComparison_AnnotationLess(
    nsNavHistoryResultNode* a, nsNavHistoryResultNode* b, void* closure)
{
  ...
  if (a_itemAnno) {
    NS_ENSURE_SUCCESS(
        annosvc->ItemHasAnnotation(a->mItemId, annoName, &a_hasAnno), 0);
  } else {
    NS_ENSURE_SUCCESS(annosvc->PageHasAnnotation(a_uri, annoName, &a_hasAnno),
                      0);
  }
  if (b_itemAnno) {
    NS_ENSURE_SUCCESS(
        annosvc->ItemHasAnnotation(b->mItemId, annoName, &b_hasAnno), 0);
  } else {
    NS_ENSURE_SUCCESS(annosvc->PageHasAnnotation(b_uri, annoName, &b_hasAnno),
                      0);
  }
  ...
  if (a_hasAnno || b_hasAnno) {
    ...
    if (a_hasAnno) {
      if (a_itemAnno) {
        NS_ENSURE_SUCCESS(
            annosvc->GetItemAnnotationType(a->mItemId, annoName, &annoType), 0);
      } else {
        NS_ENSURE_SUCCESS(
            annosvc->GetPageAnnotationType(a_uri, annoName, &annoType), 0);
      }
    }
    if (b_hasAnno) {
      uint16_t b_type;
      if (b_itemAnno) {
        NS_ENSURE_SUCCESS(
            annosvc->GetItemAnnotationType(b->mItemId, annoName, &b_type), 0);
      } else {
        NS_ENSURE_SUCCESS(
            annosvc->GetPageAnnotationType(b_uri, annoName, &b_type), 0);
      }
      // We better make the API not support this state, really
      // XXXmano: this is actually wrong for double<->int and int64_t<->int32_t
      if (a_hasAnno && b_type != annoType) return 0;
      annoType = b_type;
    }
...
#define GET_ANNOTATIONS_VALUES(METHOD_ITEM, METHOD_PAGE, A_VAL, B_VAL)         \
  if (a_hasAnno) {                                                             \
    if (a_itemAnno) {                                                          \
      NS_ENSURE_SUCCESS(annosvc->METHOD_ITEM(a->mItemId, annoName, A_VAL), 0); \
    } else {                                                                   \
      NS_ENSURE_SUCCESS(annosvc->METHOD_PAGE(a_uri, annoName, A_VAL), 0);      \
    }                                                                          \
  }                                                                            \
  if (b_hasAnno) {                                                             \
    if (b_itemAnno) {                                                          \
      NS_ENSURE_SUCCESS(annosvc->METHOD_ITEM(b->mItemId, annoName, B_VAL), 0); \
    } else {                                                                   \
      NS_ENSURE_SUCCESS(annosvc->METHOD_PAGE(b_uri, annoName, B_VAL), 0);      \
    }                                                                          \
  }
  ...
  // type
  if (value == 0) return SortComparison_TitleLess(a, b, nullptr);
 ...
 */
int32_t
nsNavHistoryContainerResultNode::SortComparison_VisitCountLess(
    nsNavHistoryResultNode* a, nsNavHistoryResultNode* b, void* closure)
{
...
int32_t
nsNavHistoryContainerResultNode::SortComparison_VisitCountGreater(
    nsNavHistoryResultNode* a, nsNavHistoryResultNode* b, void* closure)
{
...
int32_t
nsNavHistoryContainerResultNode::SortComparison_TagsLess(
    nsNavHistoryResultNode* a, nsNavHistoryResultNode* b, void* closure)
{
  int32_t value = 0;
...
int32_t
nsNavHistoryContainerResultNode::SortComparison_TagsGreater(
    nsNavHistoryResultNode* a, nsNavHistoryResultNode* b, void* closure)
{
...
nsNavHistoryContainerResultNode::SortComparison_FrecencyGreater(
    nsNavHistoryResultNode* a, nsNavHistoryResultNode* b, void* closure)
{
  return -nsNavHistoryContainerResultNode::SortComparison_FrecencyLess(
      a, b, closure);
}
                                               ...
                                               int32_t aIndex)
{
  ...
  if (!mParent || mParent->AreChildrenVisible()) {
    NOTIFY_RESULT_OBSERVERS(result,
                            NodeHistoryDetailsChanged(
                                TO_ICONTAINER(this), oldTime, oldAccessCount));
  }
    ...
    nsNavHistoryResultNode* aNode, bool aIgnoreDuplicates)
{
  ...
  if (comparator) {
    ...
    bool itemExists;
    uint32_t position = FindInsertionPoint(
        aNode, comparator, sortingAnnotation.get(), &itemExists);
...
nsNavHistoryContainerResultNode::EnsureItemPosition(uint32_t aIndex)
{
  ...
  if (!DoesChildNeedResorting(aIndex, comparator, sortAnno.get())) return false;

  RefPtr<nsNavHistoryResultNode> node(mChildren[aIndex]);
  mChildren.RemoveObjectAt(aIndex);

  uint32_t newIndex =
      FindInsertionPoint(node, comparator, sortAnno.get(), nullptr);
...
nsNavHistoryContainerResultNode::UpdateURIs(
    ...
    bool aUpdateSort,
    const nsCString& aSpec,
    nsresult (*aCallback)(nsNavHistoryResultNode*,
                          const void*,
 ...
 */
static nsresult
setTitleCallback(nsNavHistoryResultNode* aNode,
                 const void* aClosure,
...
nsNavHistoryQueryResultNode::nsNavHistoryQueryResultNode(
    ...
    : nsNavHistoryContainerResultNode(aQueryURI,
                                      aTitle,
      ...
      mTransitions(mQueries[0]->Transitions())
{
  ...
  if (history) {
    mLiveUpdate =
        history->GetUpdateRequirements(mQueries, mOptions, &mHasSearchTerms);
  }
...
{
  if (IsContainersQuery()) return true;
  ...
  nsNavHistoryQueryOptions* options = GetGeneratingOptions();
  if (options) {
    if (options->ExcludeItems()) return false;
    if (options->ExpandQueries()) return true;
  }

  if (mResult && mResult->mRootNode == this) return true;
...
nsNavHistoryQueryResultNode::VerifyQueriesParsed()
{
  ...
  nsresult rv = history->QueryStringToQueryArray(
      mURI, &mQueries, getter_AddRefs(mOptions));
  NS_ENSURE_SUCCESS(rv, rv);
...
nsNavHistoryQueryResultNode::VerifyQueriesSerialized()
{
  ...
  for (int32_t i = 0; i < mQueries.Count(); ++i)
    flatQueries.AppendElement(
        static_cast<nsINavHistoryQuery*>(mQueries.ObjectAt(i)));

  nsNavHistory* history = nsNavHistory::GetHistoryService();
  NS_ENSURE_TRUE(history, NS_ERROR_OUT_OF_MEMORY);

  nsresult rv = history->QueriesToQueryString(
      flatQueries.Elements(), flatQueries.Length(), mOptions, mURI);
  NS_ENSURE_SUCCESS(rv, rv);
  ...
  if (mResult && mResult->mNeedsToApplySortingMode) {
    ...
    if (comparator) {
      ...
      if (IsContainersQuery() && sortType == mOptions->SortingMode() &&
          (sortType == nsINavHistoryQueryOptions::SORT_BY_TITLE_ASCENDING ||
           sortType == nsINavHistoryQueryOptions::SORT_BY_TITLE_DESCENDING))
        nsNavHistoryContainerResultNode::RecursiveSort(sortingAnnotation.get(),
                                                       comparator);
      else
...
nsNavHistoryQueryResultNode::Refresh()
{
  ...
  if (!mExpanded || (mParent && mParent->IsQuery() &&
...
void
nsNavHistoryQueryResultNode::RecursiveSort(const char* aData,
                                           SortComparator aComparator)
{
  void* data = const_cast<void*>(static_cast<const void*>(aData));

  if (!IsContainersQuery()) mChildren.Sort(aComparator, data);
...
static nsresult
setHistoryDetailsCallback(nsNavHistoryResultNode* aNode,
                          const void* aClosure,
                          const nsNavHistoryResult* aResult)
{
 ...
 */
NS_IMETHODIMP
nsNavHistoryQueryResultNode::OnVisit(nsIURI* aURI,
                                     int64_t aVisitId,
                                     PRTime aTime,
                                     int64_t aSessionId,
                                     ...
                                     uint32_t* aAdded)
{
  ...
  switch (mLiveUpdate) {
    case QUERYUPDATE_HOST: {
      ...
      if (!hasDomain) return NS_OK;

      nsAutoCString host;
      if (NS_FAILED(aURI->GetAsciiHost(host))) return NS_OK;

      if (!query->Domain().Equals(host)) return NS_OK;
    ...
    case QUERYUPDATE_TIME: {
      ...
      if (hasIt) {
        PRTime endTime =
            history->NormalizeTime(query->EndTimeReference(), query->EndTime());
    ...
    case QUERYUPDATE_SIMPLE: {
      ...
      nsresult rv = history->VisitIdToResultNode(
          aVisitId, mOptions, getter_AddRefs(addition));
      NS_ENSURE_SUCCESS(rv, rv);
      if (!addition) {
...
static nsresult
setFaviconCallback(nsNavHistoryResultNode* aNode,
                   const void* aClosure,
                   const nsNavHistoryResult* aResult)
{
  if (aResult && (!aNode->mParent || aNode->mParent->AreChildrenVisible()))
                                           ...
                                           uint16_t aSource)
{
  ...
  return nsNavHistoryResultNode::OnItemChanged(aItemId,
                                               aProperty,
                                         ...
                                         nsINavHistoryQuery*** queries)
{
  ...
  *queries = static_cast<nsINavHistoryQuery**>(
  ...
  if (!*queries) return NS_ERROR_OUT_OF_MEMORY;
...
nsNavHistoryFolderResultNode::FillChildrenAsync()
{
  ...
  nsresult rv = bmSvc->QueryFolderChildrenAsync(
      this, mTargetFolderItemId, getter_AddRefs(mAsyncPendingStmt));
  NS_ENSURE_SUCCESS(rv, rv);
...
nsNavHistoryFolderResultNode::HandleResult(mozIStorageResultSet* aResultSet)
{
  ...
  while (NS_SUCCEEDED(aResultSet->GetNextRow(getter_AddRefs(row))) && row) {
    nsresult rv = bmSvc->ProcessFolderNodeRow(
        row, mOptions, &mChildren, mAsyncBookmarkIndex);
    if (NS_FAILED(rv)) {
      CancelAsyncOpen(false);
      return rv;
    }
                                          ...
                                          uint16_t aSource)
{
  ...
  bool excludeItems =
      (mResult && mResult->mRootNode->mOptions->ExcludeItems()) ||
  ...
  if (aItemType == nsINavBookmarksService::TYPE_BOOKMARK) {
    nsNavHistory* history = nsNavHistory::GetHistoryService();
    NS_ENSURE_TRUE(history, NS_ERROR_OUT_OF_MEMORY);
    rv = history->BookmarkIdToResultNode(
        aItemId, mOptions, getter_AddRefs(node));
    NS_ENSURE_SUCCESS(rv, rv);
  } else if (aItemType == nsINavBookmarksService::TYPE_FOLDER) {
    nsNavBookmarks* bookmarks = nsNavBookmarks::GetBookmarksService();
    NS_ENSURE_TRUE(bookmarks, NS_ERROR_OUT_OF_MEMORY);
    rv = bookmarks->ResultNodeForContainer(
        aItemId, mOptions, getter_AddRefs(node));
    NS_ENSURE_SUCCESS(rv, rv);
                                            ...
                                            const nsACString& aParentGUID)
{
  ...
  rv = history->VisitIdToResultNode(
      aVisitId, mOptions, getter_AddRefs(visitNode));
  NS_ENSURE_SUCCESS(rv, rv);
  if (!visitNode) {
    // Certain result types manage the nodes by themselves.
    return NS_OK;
                         ...
                         nsNavHistoryQueryOptions* aOptions)
{
  ...
  for (uint32_t i = 0; i < aQueryCount; ++i) {
    ...
    NS_ENSURE_SUCCESS(rv, rv);
    if (!mQueries.AppendObject(queryClone)) return NS_ERROR_OUT_OF_MEMORY;
  }
...
#define ENUMERATE_BOOKMARK_FOLDER_OBSERVERS(_folderId, _functionCall)        \
  ...
  PR_END_MACRO
#define ENUMERATE_LIST_OBSERVERS(                                    \
    _listType, _functionCall, _observersList, _conditionCall)        \
  PR_BEGIN_MACRO                                                     \
  ...
  PR_END_MACRO
#define ENUMERATE_QUERY_OBSERVERS(                 \
    _functionCall, _observersList, _conditionCall) \
  ENUMERATE_LIST_OBSERVERS(                        \
      QueryObserverList, _functionCall, _observersList, _conditionCall)
...
#define NOTIFY_REFRESH_PARTICIPANTS()                                         \
  PR_BEGIN_MACRO                                                              \
  ENUMERATE_LIST_OBSERVERS(                                                   \
      ContainerObserverList, Refresh(), mRefreshParticipants, IsContainer()); \
                                ...
                                uint16_t aSource)
{
  ...
  ENUMERATE_BOOKMARK_FOLDER_OBSERVERS(aParentId,
                                      OnItemAdded(aItemId,
                                                  ...
                                                  aParentGUID,
                                                  aSource));
  ENUMERATE_HISTORY_OBSERVERS(OnItemAdded(aItemId,
                                          aParentId,
                                          aIndex,
                                          aItemType,
                                          aURI,
                                          ...
                                          aParentGUID,
                                          aSource));
  ENUMERATE_ALL_BOOKMARKS_OBSERVERS(OnItemAdded(aItemId,
                                                aParentId,
                                                aIndex,
                                                aItemType,
                                                aURI,
                                  ...
                                  uint16_t aSource)
{
  ...
  ENUMERATE_BOOKMARK_FOLDER_OBSERVERS(aParentId,
                                      OnItemRemoved(aItemId,
                                                    ...
                                                    aParentGUID,
                                                    aSource));
  ENUMERATE_ALL_BOOKMARKS_OBSERVERS(OnItemRemoved(aItemId,
                                                  aParentId,
                                                  aIndex,
                                                  aItemType,
                                                  aURI,
                                                  ...
                                                  aParentGUID,
                                                  aSource));
  ENUMERATE_HISTORY_OBSERVERS(OnItemRemoved(aItemId,
                                            aParentId,
                                            aIndex,
                                            aItemType,
                                            aURI,
...
nsNavHistoryResult::OnItemChanged(int64_t aItemId,
                                  ...
                                  uint16_t aSource)
{
  ENUMERATE_ALL_BOOKMARKS_OBSERVERS(OnItemChanged(aItemId,
                                                  aProperty,
                                                  aIsAnnotationProperty,
                                                  aNewValue,
                                                  aLastModified,
                                  ...
                                  const nsACString& aParentGUID)
{
  ...
  ENUMERATE_BOOKMARK_FOLDER_OBSERVERS(aParentId,
                                      OnItemVisited(aItemId,
                                                    ...
                                                    aGUID,
                                                    aParentGUID));
  ENUMERATE_ALL_BOOKMARKS_OBSERVERS(OnItemVisited(aItemId,
                                                  aVisitId,
                                                  aVisitTime,
                                                  aTransitionType,
                                                  aURI,
                                ...
                                uint16_t aSource)
{
  ...
  ENUMERATE_ALL_BOOKMARKS_OBSERVERS(OnItemMoved(aItemId,
                                                aOldParent,
                                                aOldIndex,
                                                aNewParent,
                                                aNewIndex,
                                                ...
                                                aNewParentGUID,
                                                aSource));
  ENUMERATE_HISTORY_OBSERVERS(OnItemMoved(aItemId,
                                          aOldParent,
                                          aOldIndex,
                                          aNewParent,
                                          aNewIndex,
...
NS_IMETHODIMP
nsNavHistoryResult::OnVisit(nsIURI* aURI,
                            int64_t aVisitId,
                            PRTime aTime,
                            int64_t aSessionId,
                            int64_t aReferringId,
                            ...
                            const nsAString& aLastKnownTitle)
{
  ...
  ENUMERATE_HISTORY_OBSERVERS(OnVisit(aURI,
                                      aVisitId,
                                      aTime,
                                      aSessionId,
                                      aReferringId,
  ...
  if (!added || todayIsMissing) {
    ...
    ENUMERATE_QUERY_OBSERVERS(
        Refresh(), mHistoryObservers, IsContainersQuery());
  }
