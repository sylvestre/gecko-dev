...
/**
   ...
   */
static MOZ_ALWAYS_INLINE const_char_iterator
            ...
            const_char_iterator aSourceEnd)
{
  ...
  while (tokenCur < aTokenEnd) {
    ...
    bool error;
    if (!CaseInsensitiveUTF8CharsEqual(sourceCur,
                                       tokenCur,
                                       aSourceEnd,
                                       aTokenEnd,
                                       &sourceCur,
                                        ...
                                        nsACString& aSpecBuf)
{
  ...
  bool unescaped = NS_UnescapeURL(
      aURISpec.BeginReading(), aURISpec.Length(), esc_SkipControl, aSpecBuf);
                                         ...
                                         const nsACString& aSourceString)
{
  ...
  bool dummy;
  while (sourceStart < sourceEnd && CaseInsensitiveUTF8CharsEqual(sourceStart,
                                                                  tokenStart,
                                                                  sourceEnd,
                                                                  tokenEnd,
                                                                  &sourceStart,
                                             ...
                                             nsIVariant** _result)
{
  ...
  navHistory->DispatchFrecencyChangedNotification(
      spec, newFrecency, guid, hidden, lastVisitDate);
                             ...
                             nsIVariant** _result)
{
  ...
  const uint32_t maxLenToHash =
      std::min(static_cast<uint32_t>(str.Length()), MAX_CHARS_TO_HASH);
  RefPtr<nsVariant> result = new nsVariant();
  if (mode.IsEmpty()) {
    ...
    // Keep only 16 bits.
    uint64_t hash =
        static_cast<uint64_t>(HashString(str.get(), maxLenToHash) & 0x0000FFFF)
        << 32;
    result->SetAsInt64(hash);
