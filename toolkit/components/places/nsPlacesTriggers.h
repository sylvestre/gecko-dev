...
#define CREATE_PLACES_AFTERINSERT_TRIGGER                                     \
  NS_LITERAL_CSTRING(                                                         \
      ...
      "SELECT "                                                               \
      "(SELECT id FROM moz_hosts WHERE host = "                               \
      "fixup_url(get_unreversed_host(NEW.rev_host))), "                       \
      "fixup_url(get_unreversed_host(NEW.rev_host)), "                        \
      "MAX(IFNULL((SELECT frecency FROM moz_hosts WHERE host = "              \
      "fixup_url(get_unreversed_host(NEW.rev_host))), -1), NEW.frecency), "   \
      "MAX(IFNULL((SELECT typed FROM moz_hosts WHERE host = "                 \
      "fixup_url(get_unreversed_host(NEW.rev_host))), 0), NEW.typed), "       \
...
#define CREATE_UPDATEHOSTS_AFTERDELETE_TRIGGER                           \
  NS_LITERAL_CSTRING(                                                    \
