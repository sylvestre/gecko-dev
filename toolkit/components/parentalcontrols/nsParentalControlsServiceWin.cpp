    ...
    : mEnabled(false), mProvider(0), mPC(nullptr)
{
  ...
  hr = CoCreateInstance(CLSID_WinParentalControls,
                        nullptr,
                        CLSCTX_INPROC,
    ...
    nsIURI* aTarget, nsIInterfaceRequestor* aWindowContext, bool* _retval)
{
  *_retval = false;
  ...
  RefPtr<IWPCWebSettings> wpcws;
  if (SUCCEEDED(mPC->GetWebSettings(nullptr, getter_AddRefs(wpcws)))) {
    wpcws->RequestURLOverride(
        hWnd, NS_ConvertUTF8toUTF16(spec).get(), 0, nullptr, &ret);
    *_retval = ret;
  }
    ...
    nsIArray* aTargets, nsIInterfaceRequestor* aWindowContext, bool* _retval)
{
  *_retval = false;
  ...
  RefPtr<IWPCWebSettings> wpcws;
  if (SUCCEEDED(mPC->GetWebSettings(nullptr, getter_AddRefs(wpcws)))) {
    wpcws->RequestURLOverride(hWnd,
                              NS_ConvertUTF8toUTF16(rootSpec).get(),
                              uriIdx,
                              (LPCWSTR*)arrUrls.get(),
                              &ret);
    *_retval = ret;
...
void
nsParentalControlsService::LogFileDownload(bool blocked,
                                           nsIURI* aSource,
                                           nsIFile* aTarget)
{
  nsAutoCString curi;
  ...
  gEventWrite(
      mProvider, &WPCEVENT_WEB_FILEDOWNLOAD, ARRAYSIZE(eventData), eventData);
}
