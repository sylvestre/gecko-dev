                ...
                JS::Handle<JSObject*> obj)
{
  ...
  if (stat->successCount &&
      !JS_DefineProperty(
          cx, statsObj, "successCount", stat->successCount, JSPROP_ENUMERATE)) {
    return false;
  }
  if (stat->failureCount &&
      !JS_DefineProperty(
          cx, statsObj, "failureCount", stat->failureCount, JSPROP_ENUMERATE)) {
    return false;
  }
  return true;
}
...
bool
ReflectIceWebrtc(WebrtcTelemetry::WebrtcIceCandidateType* entry,
                 JSContext* cx,
                 JS::Handle<JSObject*> obj)
{
...
WebrtcTelemetry::GetWebrtcStats(JSContext* cx, JS::MutableHandle<JS::Value> ret)
{
  ...
  JS_DefineProperty(
      cx, root_obj, "IceCandidatesStats", ice_obj, JSPROP_ENUMERATE);
