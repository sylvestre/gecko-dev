...
CreateJSStackObject(JSContext* cx, const CombinedStacks& stacks)
{
  ...
  for (size_t moduleIndex = 0; moduleIndex < moduleCount; ++moduleIndex) {
    ...
    if (!JS_DefineElement(
            cx, moduleArray, moduleIndex, moduleInfoArray, JSPROP_ENUMERATE)) {
      return nullptr;
    }
    ...
    if (!str || !JS_DefineElement(
                    cx, moduleInfoArray, index++, str, JSPROP_ENUMERATE)) {
      return nullptr;
    }
  ...
  for (size_t i = 0; i < length; ++i) {
    ...
    for (size_t pcIndex = 0; pcIndex < pcCount; ++pcIndex) {
      ...
      if (!JS_DefineElement(
              cx, pcArray, pcIndex, framePair, JSPROP_ENUMERATE)) {
        return nullptr;
      }
    }
  }
