...
{
  DynamicEventInfo(const nsACString& category,
...
nsCString
UniqueEventName(const nsACString& category,
                const nsACString& method,
                const nsACString& object)
{
  nsCString name;
...
UniqueEventName(const EventInfo& info)
{
  return UniqueEventName(
      info.common_info.category(), info.method(), info.object());
}
...
IsExpiredDate(uint32_t expires_days_since_epoch)
{
  ...
  const uint32_t days_since_epoch =
      PR_Now() / (PRTime(PR_USEC_PER_SEC) * 24 * 60 * 60);
...
bool
CanRecordEvent(const StaticMutexAutoLock& lock,
               ...
               ProcessID process)
{
  ...
  if (!CanRecordDataset(
          GetDataset(lock, eventKey), gCanRecordBase, gCanRecordExtended)) {
    return false;
  }
...
EventRecordArray*
GetEventRecordsForProcess(const StaticMutexAutoLock& lock,
...
EventKey*
GetEventKey(const StaticMutexAutoLock& lock,
...
RecordEventResult
RecordEvent(const StaticMutexAutoLock& lock,
            ProcessID processType,
            double timestamp,
            const nsACString& category,
            const nsACString& method,
...
RecordEventResult
ShouldRecordChildEvent(const StaticMutexAutoLock& lock,
                       const nsACString& category,
                       const nsACString& method,
                       const nsACString& object)
{
...
RegisterEventResult
RegisterEvents(const StaticMutexAutoLock& lock,
               const nsACString& category,
               ...
               const nsTArray<bool>& eventExpired)
{
  ...
  for (uint32_t i = 0, len = eventInfos.Length(); i < len; ++i) {
    ...
    gEventNameIDMap.Put(UniqueEventName(eventInfos[i]),
                        new EventKey{eventId, true});
  }
    ...
    if (!record.Extra().IsEmpty()) {
      ...
      for (uint32_t i = 0; i < extra.Length(); ++i) {
        ...
        if (!JS_DefineProperty(
                cx, obj, extra[i].key.get(), value, JSPROP_ENUMERATE)) {
          return NS_ERROR_FAILURE;
        }
      }
      val.setObject(*obj);
...
void
TelemetryEvent::InitializeGlobalState(bool aCanRecordBase,
...
nsresult
TelemetryEvent::RecordChildEvents(
...
nsresult
TelemetryEvent::RecordEvent(const nsACString& aCategory,
...
static bool
GetArrayPropertyValues(JSContext* cx,
                       JS::HandleObject obj,
                       const char* property,
                       nsTArray<nsCString>* results)
{
  ...
  if (!JS_GetProperty(cx, obj, property, &value)) {
    JS_ReportErrorASCII(
        cx, R"(Missing required property "%s" for event)", property);
    return false;
  }
  ...
  if (!JS_IsArrayObject(cx, value, &isArray) || !isArray) {
    JS_ReportErrorASCII(
        cx, R"(Property "%s" for event should be an array)", property);
    return false;
  }
  ...
  for (uint32_t arrayIdx = 0; arrayIdx < arrayLength; ++arrayIdx) {
    ...
    if (!element.isString()) {
      JS_ReportErrorASCII(
          cx,
          R"(Array entries for event property "%s" should be strings)",
          property);
      return false;
    }
                               ...
                               JSContext* cx)
{
  ...
  for (size_t i = 0, n = eventPropertyIds.length(); i < n; i++) {
    ...
    for (auto& method : methods) {
      if (!IsValidIdentifierString(
              method, kMaxMethodNameByteLength, false, false)) {
        JS_ReportErrorASCII(
    ...
    for (auto& object : objects) {
      if (!IsValidIdentifierString(
              object, kMaxObjectNameByteLength, false, true)) {
        JS_ReportErrorASCII(
    ...
    for (auto& key : extra_keys) {
      if (!IsValidIdentifierString(
              key, kMaxExtraKeyNameByteLength, false, false)) {
        JS_ReportErrorASCII(
    ...
    for (auto& method : methods) {
      for (auto& object : objects) {
        ...
        DynamicEventInfo info{nsCString(aCategory),
                              method,
                              object,
                              nsTArray<nsCString>(extra_keys),
                              recordOnRelease};
  ...
  switch (res) {
    case RegisterEventResult::AlreadyRegistered:
      JS_ReportErrorASCII(
          cx, "Attempt to register event that is already registered.");
      return NS_ERROR_INVALID_ARG;
    default:
      break;
  }
...
nsresult
TelemetryEvent::CreateSnapshots(uint32_t aDataset,
                                bool aClear,
                                JSContext* cx,
                                uint8_t optional_argc,
                                JS::MutableHandleValue aResult)
{
  ...
  for (uint32_t i = 0; i < processLength; ++i) {
    ...
    if (NS_FAILED(SerializeEventsArray(
            processEvents[i].second(), cx, &eventsArray, aDataset))) {
      return NS_ERROR_FAILURE;
    }

    if (!JS_DefineProperty(cx,
                           rootObj,
                           processEvents[i].first(),
                           eventsArray,
                           JSPROP_ENUMERATE)) {
      return NS_ERROR_FAILURE;
    }
...
void
TelemetryEvent::SetEventRecordingEnabled(const nsACString& category,
                                         bool enabled)
{
  StaticMutexAutoLock locker(gTelemetryEventsMutex);
