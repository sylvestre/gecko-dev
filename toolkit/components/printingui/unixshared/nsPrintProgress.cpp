...
NS_IMETHODIMP
nsPrintProgress::OpenProgressDialog(mozIDOMWindowProxy* parent,
  ...
  *notifyOnOpen = true;
  ...
  if (parent) {
    ...
    rv = piOwnerWindow->OpenDialog(
        NS_ConvertASCIItoUTF16(dialogURL),
...
NS_IMETHODIMP
nsPrintProgress::CloseProgressDialog(bool forceClose)
{
  m_closeProgress = true;
  // XXX Invalid cast of bool to nsresult (bug 778106)
  return OnStateChange(nullptr,
                       nullptr,
...
NS_IMETHODIMP
nsPrintProgress::GetPrompter(nsIPrompt** _retval)
{
...
NS_IMETHODIMP
nsPrintProgress::GetProcessCanceledByUser(bool* aProcessCanceledByUser)
{
...
NS_IMETHODIMP
nsPrintProgress::SetProcessCanceledByUser(bool aProcessCanceledByUser)
{
...
NS_IMETHODIMP
nsPrintProgress::RegisterListener(nsIWebProgressListener* listener)
{
  ...
  if (m_closeProgress || m_processCanceled)
    listener->OnStateChange(
        nullptr, nullptr, nsIWebProgressListener::STATE_STOP, NS_OK);
  else {
    listener->OnStatusChange(nullptr, nullptr, NS_OK, m_pendingStatus.get());
    if (m_pendingStateFlags != -1)
      listener->OnStateChange(
          nullptr, nullptr, m_pendingStateFlags, m_pendingStateValue);
  }
...
NS_IMETHODIMP
nsPrintProgress::UnregisterListener(nsIWebProgressListener* listener)
{
...
NS_IMETHODIMP
nsPrintProgress::OnStateChange(nsIWebProgress* aWebProgress,
                               nsIRequest* aRequest,
...
NS_IMETHODIMP
nsPrintProgress::OnProgressChange(nsIWebProgress* aWebProgress,
                                  nsIRequest* aRequest,
                                  ...
                                  int32_t aMaxTotalProgress)
{
  ...
  for (uint32_t i = count - 1; i < count; i--) {
    nsCOMPtr<nsIWebProgressListener> progressListener =
        m_listenerList.SafeObjectAt(i);
    if (progressListener)
      progressListener->OnProgressChange(aWebProgress,
                                         aRequest,
                                         aCurSelfProgress,
                                         aMaxSelfProgress,
                                         aCurTotalProgress,
...
NS_IMETHODIMP
nsPrintProgress::OnLocationChange(nsIWebProgress* aWebProgress,
                                  nsIRequest* aRequest,
                                  nsIURI* location,
                                  uint32_t aFlags)
{
...
NS_IMETHODIMP
nsPrintProgress::OnStatusChange(nsIWebProgress* aWebProgress,
                                nsIRequest* aRequest,
                                nsresult aStatus,
...
NS_IMETHODIMP
nsPrintProgress::OnSecurityChange(nsIWebProgress* aWebProgress,
                                  nsIRequest* aRequest,
                                  uint32_t state)
{
  return NS_OK;
...
nsresult
nsPrintProgress::ReleaseListeners()
{
...
NS_IMETHODIMP
nsPrintProgress::ShowStatusString(const char16_t* status)
{
  return OnStatusChange(nullptr, nullptr, NS_OK, status);
}

NS_IMETHODIMP
nsPrintProgress::StartMeteors()
{
  return NS_ERROR_NOT_IMPLEMENTED;
}
...
NS_IMETHODIMP
nsPrintProgress::ShowProgress(int32_t percent)
{
  return NS_ERROR_NOT_IMPLEMENTED;
}

NS_IMETHODIMP
nsPrintProgress::SetDocShell(nsIDocShell* shell, mozIDOMWindowProxy* window)
{
  return NS_ERROR_NOT_IMPLEMENTED;
}

NS_IMETHODIMP
nsPrintProgress::CloseWindow()
{
  return NS_ERROR_NOT_IMPLEMENTED;
}
