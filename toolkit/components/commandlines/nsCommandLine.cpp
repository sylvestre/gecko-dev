...
class nsCommandLine final : public nsICommandLineRunner
{
 ...
 protected:
  ...
  typedef nsresult (*EnumerateValidatorsCallback)(
      nsICommandLineValidator* aValidator,
      nsICommandLine* aThis,
      void* aClosure);
...
NS_IMETHODIMP
nsCommandLine::FindFlag(const nsAString& aFlag,
                        bool aCaseSensitive,
                        int32_t* aResult)
{
...
NS_IMETHODIMP
nsCommandLine::HandleFlag(const nsAString& aFlag,
                          bool aCaseSensitive,
                          bool* aResult)
{
  nsresult rv;
...
NS_IMETHODIMP
nsCommandLine::HandleFlagWithParam(const nsAString& aFlag,
                                   bool aCaseSensitive,
                                   nsAString& aResult)
{
  nsresult rv;
...
nsCommandLine::ResolveURI(const nsAString& aArgument, nsIURI** aResult)
{
  ...
  return io->NewURI(
      NS_ConvertUTF16toUTF8(aArgument), nullptr, workingDirURI, aResult);
}
...
NS_IMETHODIMP
nsCommandLine::Init(int32_t argc,
                                 ...
                                 void* aClosure)
{
  ...
  while (NS_SUCCEEDED(strenum->HasMore(&hasMore)) && hasMore) {
    ...
    rv = catman->GetCategoryEntry(
        "command-line-handler", entry.get(), getter_Copies(contractID));
...
nsresult
nsCommandLine::EnumerateValidators(EnumerateValidatorsCallback aCallback,
                                   void* aClosure)
{
  nsresult rv;
  ...
  while (NS_SUCCEEDED(strenum->HasMore(&hasMore)) && hasMore) {
    ...
    nsCString contractID;
    rv = catman->GetCategoryEntry(
        "command-line-validator", entry.get(), getter_Copies(contractID));
