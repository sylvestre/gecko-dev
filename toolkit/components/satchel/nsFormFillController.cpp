...
NS_IMETHODIMP
nsFormFillController::SetMinResultsForPopup(uint32_t aMinResultsForPopup)
{
...
NS_IMETHODIMP
nsFormFillController::SetShowImageColumn(bool aShowImageColumn)
{
...
NS_IMETHODIMP
nsFormFillController::SetShowCommentColumn(bool aShowCommentColumn)
{
  return NS_ERROR_NOT_IMPLEMENTED;
}
...
nsFormFillController::SelectTextRange(int32_t aStartIndex, int32_t aEndIndex)
{
  ...
  ErrorResult rv;
  HTMLInputElement::FromContent(content)->SetSelectionRange(
      aStartIndex, aEndIndex, Optional<nsAString>(), rv);
  return rv.StealNSResult();
}
                                  ...
                                  nsIAutoCompleteObserver* aListener)
{
  ...
  if (mFocusedInputNode &&
    ...
    rv = mLoginManager->AutoCompleteSearchAsync(
        aSearchString, aPreviousResult, mFocusedInput, this);
...
nsresult
nsFormFillController::PerformInputListAutoComplete(
    const nsAString& aSearch, nsIAutoCompleteResult** aResult)
{
  ...
  NS_ENSURE_SUCCESS(rv, rv);
  rv = inputListAutoComplete->AutoCompleteSearch(
      aSearch, mFocusedInput, aResult);
  NS_ENSURE_SUCCESS(rv, rv);
...
{
  MOZ_LOG(
      sLogger, LogLevel::Debug, ("AddWindowListeners for window %p", aWindow));
  if (!aWindow) {
    return;
  }
  ...
  target->AddEventListener(NS_LITERAL_STRING("focus"), this, true, false);
  target->AddEventListener(NS_LITERAL_STRING("blur"), this, true, false);
  target->AddEventListener(NS_LITERAL_STRING("pagehide"), this, true, false);
  target->AddEventListener(NS_LITERAL_STRING("mousedown"), this, true, false);
  target->AddEventListener(NS_LITERAL_STRING("input"), this, true, false);
  target->AddEventListener(NS_LITERAL_STRING("keypress"), this, true, false);
  target->AddEventListener(
      NS_LITERAL_STRING("compositionstart"), this, true, false);
  target->AddEventListener(
      NS_LITERAL_STRING("compositionend"), this, true, false);
  target->AddEventListener(NS_LITERAL_STRING("contextmenu"), this, true, false);
...
nsFormFillController::RemoveWindowListeners(nsPIDOMWindowOuter* aWindow)
{
  ...
  target->RemoveEventListener(NS_LITERAL_STRING("compositionend"), this, true);
...
static const mozilla::Module kSatchelModule = {
    mozilla::Module::kVersion, kSatchelCIDs, kSatchelContracts};
