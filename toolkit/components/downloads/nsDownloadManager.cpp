...
#define DOWNLOAD_MANAGER_BUNDLE \
...
nsDownloadManager::GetDefaultDownloadsDirectory(nsIFile** aResult)
{
  ...
  if (version < 6) {  // XP/2K
    ...
    rv = dirService->Get(
        NS_WIN_PERSONAL_DIR, NS_GET_IID(nsIFile), getter_AddRefs(downloadDir));
    NS_ENSURE_SUCCESS(rv, rv);
  ...
  if (downloadDirPath) {
    rv = NS_NewNativeLocalFile(
        nsDependentCString(downloadDirPath), true, getter_AddRefs(downloadDir));
    NS_ENSURE_SUCCESS(rv, rv);
  } else {
    rv = NS_ERROR_FAILURE;
  }
  ...
  if (NS_FAILED(rv)) {
    rv = dirService->Get(
        NS_UNIX_HOME_DIR, NS_GET_IID(nsIFile), getter_AddRefs(downloadDir));
    NS_ENSURE_SUCCESS(rv, rv);
    rv = downloadDir->Append(folderName);
    NS_ENSURE_SUCCESS(rv, rv);
  }
...
#else
  rv = dirService->Get(
      NS_OS_HOME_DIR, NS_GET_IID(nsIFile), getter_AddRefs(downloadDir));
  NS_ENSURE_SUCCESS(rv, rv);
  rv = downloadDir->Append(folderName);
  NS_ENSURE_SUCCESS(rv, rv);
#endif
...
nsDownloadManager::GetUserDownloadsDirectory(nsIFile** aResult)
{
  ...
  rv = prefService->GetBranch(NS_BRANCH_DOWNLOAD, getter_AddRefs(prefBranch));
  NS_ENSURE_SUCCESS(rv, rv);
  ...
  switch (val) {
    case 0:  // Desktop
    {
      nsCOMPtr<nsIFile> downloadDir;
      rv = dirService->Get(
          NS_OS_DESKTOP_DIR, NS_GET_IID(nsIFile), getter_AddRefs(downloadDir));
      NS_ENSURE_SUCCESS(rv, rv);
      downloadDir.forget(aResult);
      return NS_OK;
    } break;
    ...
    case 2:  // Custom
    {
      ...
      prefBranch->GetComplexValue(
          NS_PREF_DIR, NS_GET_IID(nsIFile), getter_AddRefs(customDirectory));
      if (customDirectory) {
        bool exists = false;
      ...
      if (NS_SUCCEEDED(rv)) {
        (void)prefBranch->SetComplexValue(
            NS_PREF_DIR, NS_GET_IID(nsIFile), *aResult);
      }
      return rv;
    } break;
  }
...
NS_IMETHODIMP
nsDownloadManager::CancelDownload(uint32_t aID) { return NS_ERROR_UNEXPECTED; }

NS_IMETHODIMP
nsDownloadManager::RetryDownload(uint32_t aID) { return NS_ERROR_UNEXPECTED; }

NS_IMETHODIMP
nsDownloadManager::RemoveDownload(uint32_t aID) { return NS_ERROR_UNEXPECTED; }
...
NS_IMETHODIMP
nsDownloadManager::CleanUp() { return NS_ERROR_UNEXPECTED; }

NS_IMETHODIMP
nsDownloadManager::CleanUpPrivate() { return NS_ERROR_UNEXPECTED; }

NS_IMETHODIMP
nsDownloadManager::GetCanCleanUp(bool* aResult) { return NS_ERROR_UNEXPECTED; }
...
NS_IMETHODIMP
nsDownloadManager::PauseDownload(uint32_t aID) { return NS_ERROR_UNEXPECTED; }

NS_IMETHODIMP
nsDownloadManager::ResumeDownload(uint32_t aID) { return NS_ERROR_UNEXPECTED; }
...
NS_IMETHODIMP
nsDownloadManager::AddPrivacyAwareListener(
    nsIDownloadProgressListener* aListener)
{
  return NS_ERROR_UNEXPECTED;
}
