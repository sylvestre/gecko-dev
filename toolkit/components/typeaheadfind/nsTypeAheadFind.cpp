...
NS_IMPL_CYCLE_COLLECTION(nsTypeAheadFind,
                         mFoundLink,
                         mFoundEditable,
                         mCurrentWindow,
                         mStartFindRange,
...
NS_IMETHODIMP
nsTypeAheadFind::Observe(nsISupports* aSubject,
                         const char* aTopic,
                           ...
                           uint16_t* aResult)
{
  *aResult = FIND_NOTFOUND;
  ...
  if (NS_FAILED(
          GetSearchContainers(currentContainer,
  ...
  mStartPointRange->CompareBoundaryPoints(
      nsIDOMRange::START_TO_START, mSearchRange, &rangeCompareResult);
  ...
  while (true) {    // ----- Outer while loop: go through all docs -----
    while (true) {  // === Inner while loop: go through a single doc ===
      mFind->Find(mTypeAheadBuffer.get(),
                  mSearchRange,
                  mStartPointRange,
                  mEndPointRange,
      ...
      if (aIsLinksOnly) {
        ...
        RangeStartsInsideLink(
            returnRange, presShell, &isInsideLink, &isStartingLink);
      }

      bool usesIndependentSelection;
      if (!IsRangeVisible(presShell,
                          presContext,
                          returnRange,
                          aIsFirstVisiblePreferred,
                          false,
  ...
  while (true) {
    ...
    if (startContent->IsHTMLElement()) {
      ...
      *aIsInsideLink = startContent->HasAttr(kNameSpaceID_XLink, hrefAtom);
      if (*aIsInsideLink) {
        if (!startContent->AttrValueIs(kNameSpaceID_XLink,
                                       typeAtom,
...
NS_IMETHODIMP
nsTypeAheadFind::FindAgain(bool aFindBackwards,
                           bool aLinksOnly,
                           uint16_t* aResult)
...
NS_IMETHODIMP
nsTypeAheadFind::Find(const nsAString& aSearchString,
                      bool aLinksOnly,
                      uint16_t* aResult)
{
  *aResult = FIND_NOTFOUND;
  ...
  if (bufferLength == 0) {
    ...
    if (isFirstVisiblePreferred) {
      ...
      if (fm) {
        ...
        nsCOMPtr<mozIDOMWindowProxy> focusedWindow;
        fm->GetFocusedElementForWindow(window,
                                       false,
                                ...
                                bool* aResult)
{
  ...
  *aResult = IsRangeVisible(presShell,
                            presContext,
                            aRange,
                            aMustBeInViewPort,
                            false,
