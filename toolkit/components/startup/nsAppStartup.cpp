    ...
    if (!mAttemptingQuit) {
      ...
      if (obsService)
        obsService->NotifyObservers(
            nullptr, "quit-application-granted", nullptr);
    }
                                 ...
                                 nsIWebBrowserChrome** _retval)
{
  ...
  return CreateChromeWindow2(
      aParent, aChromeFlags, nullptr, nullptr, 0, &cancel, _retval);
}
                                  ...
                                  nsIWebBrowserChrome** _retval)
{
  ...
  *_retval = 0;
  ...
  if (aParent) {
    ...
    if (xulParent)
      xulParent->CreateNewWindow(aChromeFlags,
                                 aOpeningTab,
                                 aOpener,
                                 aNextTabParentId,
    ...
    appShell->CreateTopLevelWindow(0,
                                   0,
                                   aChromeFlags,
  ...
  for (int i = StartupTimeline::PROCESS_CREATION;
    ...
    if (!stamp.IsNull()) {
      if (stamp >= procTime) {
        ...
        JS_DefineProperty(
            aCx, obj, StartupTimeline::Describe(ev), date, JSPROP_ENUMERATE);
      } else {
...
nsAppStartup::TrackStartupCrashBegin(bool* aIsSafeModeNecessary)
{
  ...
  rv =
      static_cast<Preferences*>(prefs.get())
          ->SavePrefFileBlocking();  // flush prefs to disk since we are tracking crashes
  NS_ENSURE_SUCCESS(rv, rv);
...
nsAppStartup::TrackStartupCrashEnd()
{
  ...
  if (inSafeMode && mIsSafeModeNecessary) {
    ...
    int32_t prefType;
    rv = Preferences::GetDefaultRootBranch()->GetPrefType(
        kPrefMaxResumedCrashes, &prefType);
    NS_ENSURE_SUCCESS(rv, rv);
