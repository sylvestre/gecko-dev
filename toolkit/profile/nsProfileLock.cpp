...
void
nsProfileLock::RemovePidLockFiles(bool aFatalSignal)
{
...
nsresult
nsProfileLock::LockWithFcntl(nsIFile* aLockFile)
{
  nsresult rv = NS_OK;
...
static bool
IsSymlinkStaleLock(struct in_addr* aAddr,
...
nsresult
nsProfileLock::LockWithSymlink(nsIFile* aLockFile, bool aHaveFcntlLock)
{
  nsresult rv;
  nsAutoCString lockFilePath;
  rv = aLockFile->GetNativePath(lockFilePath);
...
nsresult
nsProfileLock::GetReplacedLockTime(PRTime* aResult)
{
...
nsresult
nsProfileLock::Lock(nsIFile* aProfileDir, nsIProfileUnlocker** aUnlocker)
{
#if defined(XP_MACOSX)
