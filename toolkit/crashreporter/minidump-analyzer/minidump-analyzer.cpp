...
static wstring
UTF8ToWide(const string& aUtf8Str, bool* aSuccess = nullptr)
{
  wchar_t* buffer = nullptr;
  int buffer_size =
      MultiByteToWideChar(CP_UTF8, 0, aUtf8Str.c_str(), -1, nullptr, 0);
  if (buffer_size == 0) {
    if (aSuccess) {
      *aSuccess = false;
    }
  ...
  MultiByteToWideChar(CP_UTF8, 0, aUtf8Str.c_str(), -1, buffer, buffer_size);
  wstring str = buffer;
  delete[] buffer;
