...
static wstring
UTF8ToWide(const string& utf8, bool* success = 0);
static DWORD WINAPI
...
static bool
GetBoolValue(HKEY hRegKey, LPCTSTR valueName, DWORD* value)
{
  DWORD type, dataSize;
  dataSize = sizeof(DWORD);
  if (RegQueryValueEx(
          hRegKey, valueName, nullptr, &type, (LPBYTE)value, &dataSize) ==
          ERROR_SUCCESS &&
      type == REG_DWORD)
    return true;
...
static void
RemoveUnusedValues(const wchar_t* key, LPCTSTR valueName)
{
  HKEY hRegKey;
...
static bool
CheckBoolKey(const wchar_t* key, const wchar_t* valueName, bool* enabled)
{
  /*
...
static void
SetBoolKey(const wchar_t* key, const wchar_t* value, bool enabled)
{
  /*
...
static bool
GetStringValue(HKEY hRegKey, LPCTSTR valueName, wstring& value)
{
  DWORD type, dataSize;
  wchar_t buf[2048] = {};
  dataSize = sizeof(buf) - 1;
  if (RegQueryValueEx(
          hRegKey, valueName, nullptr, &type, (LPBYTE)buf, &dataSize) ==
          ERROR_SUCCESS &&
      type == REG_SZ) {
    value = buf;
    return true;
...
static bool
GetStringKey(const wchar_t* key, const wchar_t* valueName, wstring& value)
{
  value = L"";
  bool found = false;
  HKEY hRegKey;
...
static void
SetStringKey(const wchar_t* key, const wchar_t* valueName, const wstring& value)
{
  HKEY hRegKey;
  if (RegCreateKey(HKEY_CURRENT_USER, key, &hRegKey) == ERROR_SUCCESS) {
...
static void
GetRelativeRect(HWND hwnd, HWND hwndParent, RECT* r)
{
...
static void
SetDlgItemVisible(HWND hwndDlg, UINT item, bool visible)
{
...
static void
SetDlgItemDisabled(HWND hwndDlg, UINT item, bool disabled)
{
  HWND hwnd = GetDlgItem(hwndDlg, item);
...
static void
StretchDialog(HWND hwndDlg, int ydiff)
{
  RECT r;
...
static void
ReflowDialog(HWND hwndDlg, int ydiff)
{
...
static void
EndCrashReporterDialog(HWND hwndDlg, int code)
{
  // Save the current values to the registry
  wchar_t email[MAX_EMAIL_LENGTH];
  GetDlgItemTextW(
      hwndDlg, IDC_EMAILTEXT, email, sizeof(email) / sizeof(email[0]));
  SetStringKey(gCrashReporterKey.c_str(), EMAIL_VALUE, email);

  SetBoolKey(gCrashReporterKey.c_str(),
             INCLUDE_URL_VALUE,
...
static void
MaybeResizeProgressText(HWND hwndDlg)
{
...
static void
UpdateComment(HWND hwndDlg)
{
  wchar_t comment[MAX_COMMENT_LENGTH + 1];
  GetDlgItemTextW(
      hwndDlg, IDC_COMMENTTEXT, comment, sizeof(comment) / sizeof(comment[0]));
  if (wcslen(comment) > 0)
...
ViewReportDialogProc(HWND hwndDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
  switch (message) {
    case WM_INITDIALOG: {
      ...
      SendDlgItemMessage(
          hwndDlg, IDC_VIEWREPORTTEXT, EM_SETTARGETDEVICE, (WPARAM) nullptr, 0);
...
// with |insert|.
static int
NewTextLength(HWND hwndEdit, wchar_t* insert)
{
...
static void
StretchControlsToFit(HWND hwndDlg)
{
  int controls[] = {IDC_DESCRIPTIONTEXT,
                    IDC_SUBMITREPORTCHECK,
                    IDC_COMMENTTEXT,
                    IDC_INCLUDEURLCHECK,
                    IDC_EMAILMECHECK,
  ...
  for (int i = 0; i < sizeof(controls) / sizeof(controls[0]); i++) {
    ...
    if (r.right + 6 != dlgRect.right) {
      ...
      MoveWindow(
          hwndControl, r.left, r.top, r.right - r.left, r.bottom - r.top, TRUE);
    }
  }
}

static void
SubmitReportChecked(HWND hwndDlg)
{
...
static INT_PTR
DialogBoxParamMaybeRTL(UINT idd, HWND hwndParent, DLGPROC dlgProc, LPARAM param)
{
  INT_PTR rv = 0;
  if (gRTLlayout) {
    ...
    rv = DialogBoxIndirectParam(
        nullptr, (LPCDLGTEMPLATE)pMyDlgTemplate, hwndParent, dlgProc, param);
    GlobalUnlock(hMyDlgTemplate);
    GlobalFree(hMyDlgTemplate);
  } else {
    rv = DialogBoxParam(
        nullptr, MAKEINTRESOURCE(idd), hwndParent, dlgProc, param);
  }
                        ...
                        LPARAM lParam)
{
  ...
  switch (message) {
    case WM_INITDIALOG: {
      ...
      GetRelativeRect(hwnd, hwndDlg, &rect);
      long maxdiff = ResizeControl(
          hwnd, rect, Str(ST_CHECKSUBMIT), false, gCheckboxPadding);
      SetDlgItemText(
          hwndDlg, IDC_SUBMITREPORTCHECK, Str(ST_CHECKSUBMIT).c_str());

      if (!CheckBoolKey(
              gCrashReporterKey.c_str(), SUBMIT_REPORT_VALUE, &enabled))
        enabled = true;
      ...
      if (CheckBoolKey(
              gCrashReporterKey.c_str(), INCLUDE_URL_VALUE, &enabled) &&
          !enabled) {
      ...
      GetRelativeRect(hwnd, hwndDlg, &rect);
      diff = ResizeControl(
          hwnd, rect, Str(ST_CHECKEMAIL), false, gCheckboxPadding);
      maxdiff = std::max(diff, maxdiff);
      ...
      OldWndProc = (WNDPROC)SetWindowLongPtr(
          hwndEmail, GWLP_WNDPROC, (LONG_PTR)EditSubclassProc);
      ...
      SetDlgItemText(
          hwndDlg, IDC_PROGRESSTEXT, Str(ST_REPORTPRESUBMIT).c_str());
      ...
      if (maxdiff > 0) {
        ...
        r.right += maxdiff;
        MoveWindow(
            hwndDlg, r.left, r.top, r.right - r.left, r.bottom - r.top, TRUE);
    ...
    case WM_NOTIFY: {
      ...
      if (notification->code == EN_REQUESTRESIZE) {
        ...
        MoveWindow(notification->hwndFrom,
                   newSize.left,
                   newSize.top,
      ...
      if (HIWORD(wParam) == BN_CLICKED) {
        switch (LOWORD(wParam)) {
          case IDC_VIEWREPORTBUTTON:
            DialogBoxParamMaybeRTL(IDD_VIEWREPORTDIALOG,
                                   hwndDlg,
                                   (DLGPROC)ViewReportDialogProc,
                                   0);
            break;
...
static wstring
UTF8ToWide(const string& utf8, bool* success)
{
...
static string
WideToMBCP(const wstring& wide, unsigned int cp, bool* success = nullptr)
{
  char* buffer = nullptr;
  int buffer_size = WideCharToMultiByte(
      cp, 0, wide.c_str(), -1, nullptr, 0, nullptr, nullptr);
  if (buffer_size == 0) {
    if (success) *success = false;
    return "";
  }
  ...
  WideCharToMultiByte(
      cp, 0, wide.c_str(), -1, buffer, buffer_size, nullptr, nullptr);
  string mb = buffer;
  delete[] buffer;
...
string
WideToUTF8(const wstring& wide, bool* success)
{
...
{
  MessageBox(nullptr,
             Str(ST_CRASHREPORTERDEFAULT).c_str(),
             L"Crash Reporter",
             MB_OK | MB_ICONSTOP);
}
...
bool
UIShowCrashUI(const StringTable& files,
              ...
              const vector<string>& restartArgs)
{
  ...
  if (!CanUseMainCrashReportServer() &&
    ...
    gSendData.sendURL.replace(
        0, ARRAYSIZE(SENDURL_ORIGINAL) - 1, SENDURL_XPSP2);
  }
  ...
  return 1 == DialogBoxParamMaybeRTL(
                  IDD_SENDDIALOG, nullptr, (DLGPROC)CrashReporterDialogProc, 0);
}

void
UIError_impl(const string& message)
{
  wstring title = Str(ST_CRASHREPORTERTITLE);
  if (title.empty()) title = L"Crash Reporter Error";

  MessageBox(
      nullptr, UTF8ToWide(message).c_str(), title.c_str(), MB_OK | MB_ICONSTOP);
}

bool
UIGetIniPath(string& path)
{
...
bool
UIMoveFile(const string& oldfile, const string& newfile)
{
...
bool
UIDeleteFile(const string& oldfile)
{
...
bool
UIRunProgram(const string& exename,
