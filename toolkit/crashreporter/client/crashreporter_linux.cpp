...
SaveSettings()
{
  ...
  WriteStringsToFile(
      gSettingsPath + "/" + kIniFile, "Crash Reporter", settings, true);
}
...
static void
ShowReportInfo(GtkTextView* viewReportTextView)
{
  ...
  gtk_text_buffer_insert(
      buffer, &end, gStrings[ST_EXTRAREPORTINFO].c_str(), -1);
}
...
static void
ViewReportClicked(GtkButton* button, gpointer userData)
{
  GtkDialog* dialog = GTK_DIALOG(
      gtk_dialog_new_with_buttons(gStrings[ST_VIEWREPORTTITLE].c_str(),
                                  GTK_WINDOW(gWindow),
                                  GTK_DIALOG_MODAL,
                                  GTK_STOCK_OK,
                                  GTK_RESPONSE_OK,
  ...
  gtk_scrolled_window_set_policy(
      GTK_SCROLLED_WINDOW(scrolled), GTK_POLICY_AUTOMATIC, GTK_POLICY_ALWAYS);
...
static void
CommentChanged(GtkTextBuffer* buffer, gpointer userData)
{
  GtkTextIter start, end;
  gtk_text_buffer_get_start_iter(buffer, &start);
  gtk_text_buffer_get_end_iter(buffer, &end);
...
static void
CommentInsert(GtkTextBuffer* buffer,
...
static void
UpdateHintText(GtkWidget* widget,
               gboolean gainedFocus,
               bool* hintShowing,
               const char* hintText)
{
...
static gboolean
CommentFocusChange(GtkWidget* widget, GdkEventFocus* event, gpointer userData)
{
  UpdateHintText(widget,
                 event->in,
                 &gCommentFieldHint,
...
static void
EmailMeClicked(GtkButton* sender, gpointer userData)
{
  UpdateEmail();
}

static void
EmailChanged(GtkEditable* editable, gpointer userData)
{
  UpdateEmail();
}

static gboolean
EmailFocusChange(GtkWidget* widget, GdkEventFocus* event, gpointer userData)
{
  UpdateHintText(
      widget, event->in, &gEmailFieldHint, gStrings[ST_EMAILGRAYTEXT].c_str());
...
TryInitGnome()
{
  ...
  if (gnome_program_init && libgnomeui_module_info_get) {
    gnome_program_init("crashreporter",
                       "1.0",
              ...
              const vector<string>& restartArgs)
{
  ...
  g_signal_connect(
      gWindow, "key_press_event", G_CALLBACK(check_escape), nullptr);
  ...
  g_signal_connect(
      gSubmitReportCheck, "clicked", G_CALLBACK(SubmitReportChecked), 0);
  ...
  gtk_box_pack_start(
      GTK_BOX(viewReportButtonBox), gViewReportButton, FALSE, FALSE, 0);
  g_signal_connect(
      gViewReportButton, "clicked", G_CALLBACK(ViewReportClicked), 0);
  ...
  g_signal_connect(
      gCommentText, "focus-in-event", G_CALLBACK(CommentFocusChange), 0);
  g_signal_connect(
      gCommentText, "focus-out-event", G_CALLBACK(CommentFocusChange), 0);
  ...
  if (gQueryParameters.find("URL") != gQueryParameters.end()) {
    ...
    g_signal_connect(
        gIncludeURLCheck, "clicked", G_CALLBACK(IncludeURLClicked), 0);
    // on by default
  ...
  gtk_box_pack_start(
      GTK_BOX(emailIndentBox), gtk_label_new(""), FALSE, FALSE, 9);
  ...
  g_signal_connect(
      gEmailEntry, "focus-in-event", G_CALLBACK(EmailFocusChange), 0);
  g_signal_connect(
      gEmailEntry, "focus-out-event", G_CALLBACK(EmailFocusChange), 0);
  ...
  UpdateHintText(gCommentText,
                 FALSE,
                 &gCommentFieldHint,
                 gStrings[ST_COMMENTGRAYTEXT].c_str());
  UpdateHintText(
      gEmailEntry, FALSE, &gEmailFieldHint, gStrings[ST_EMAILGRAYTEXT].c_str());
