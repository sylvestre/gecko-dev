...
void
UIError(const string& message)
{
  string errorMessage;
  if (!gStrings[ST_CRASHREPORTERERROR].empty()) {
    char buf[2048];
    UI_SNPRINTF(
        buf, 2048, gStrings[ST_CRASHREPORTERERROR].c_str(), message.c_str());
    errorMessage = buf;
  } else {
...
bool
ReadStrings(istream& in, StringTable& strings, bool unescape)
{
...
bool
ReadStringsFromFile(const string& path, StringTable& strings, bool unescape)
{
...
bool
WriteStringsToFile(const string& path,
                   const string& header,
                   StringTable& strings,
                   bool escape)
{
...
static void
AppendToEventFile(const string& aKey, const string& aValue)
{
...
static void
WriteSubmissionEvent(SubmissionResult result, const string& remoteId)
{
  if (gEventsPath.empty()) {
...
static bool
MoveCrashData(const string& toDir,
              string& dumpfile,
              string& extrafile,
              string& memoryfile)
{
...
static bool
AddSubmittedReport(const string& serverResponse)
{
  ...
  char buf[1024];
  UI_SNPRINTF(
      buf, 1024, gStrings["CrashID"].c_str(), responseItems["CrashID"].c_str());
  *file << buf << "\n";
...
void
SendCompleted(bool success, const string& serverResponse)
{
  if (success) {
...
ComputeDumpHash()
{
  ...
  SECStatus (*NSS_Initialize)(
      const char*, const char*, const char*, const char*, PRUint32);
...
void
RewriteStrings(StringTable& queryParameters)
{
  ...
  UI_SNPRINTF(
      buf, sizeof(buf), gStrings[ST_CHECKSUBMIT].c_str(), vendor.c_str());
  gStrings[ST_CHECKSUBMIT] = buf;

  UI_SNPRINTF(
      buf, sizeof(buf), gStrings[ST_CHECKEMAIL].c_str(), vendor.c_str());
  ...
  UI_SNPRINTF(
      buf, sizeof(buf), gStrings[ST_ERROR_ENDOFLIFE].c_str(), product.c_str());
  gStrings[ST_ERROR_ENDOFLIFE] = buf;
}

bool
CheckEndOfLifed(string version)
{
...
main(int argc, char** argv)
{
  ...
  if (gReporterDumpFile.empty()) {
    ...
    UIRunProgram(
        GetProgramPath(UI_MINIDUMP_ANALYZER_FILENAME), args, /* wait */ true);
    ...
    if (!MoveCrashData(
            pendingDir, gReporterDumpFile, gExtraFile, gMemoryFile)) {
      return 0;
    }
...
int WINAPI
wWinMain(HINSTANCE, HINSTANCE, LPWSTR args, int)
{
