...
static void
CopySections(const unsigned char* data,
...
static bool
CopyRegion(HANDLE hRemoteProcess,
           void* remoteAddress,
           void* localAddress,
           DWORD size,
           DWORD protect)
{
  if (size > 0) {
    ...
    if (!VirtualAllocEx(
            hRemoteProcess, remoteAddress, size, MEM_COMMIT, PAGE_READWRITE))
      return false;

    if (!WriteProcessMemory(
            hRemoteProcess, remoteAddress, localAddress, size, nullptr)) {
#ifdef DEBUG_OUTPUT
    ...
    DWORD oldProtect;
    if (VirtualProtectEx(
            hRemoteProcess, remoteAddress, size, protect, &oldProtect) == 0) {
#ifdef DEBUG_OUTPUT
...
FinalizeSections(PMEMORYMODULE module, HANDLE hRemoteProcess)
{
  ...
  fprintf(stderr,
          ...
          module->localCodeBase,
          module->remoteCodeBase);
#endif
...
static void*
MemoryGetProcAddress(PMEMORYMODULE module, const char* name);
                               ...
                               const char* symbol)
{
  ...
  nsAutoHandle hLibrary(CreateFile(library,
                                   GENERIC_READ,
                                   FILE_SHARE_READ,
                                   nullptr,
                                   OPEN_EXISTING,
