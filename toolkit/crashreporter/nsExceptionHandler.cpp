...
bool
get_rust_panic_reason(char** reason, size_t* length);
}
...
template<int N>
static void
WriteAnnotation(PlatformWriter& pw, const char (&name)[N], const char* value)
{
...
LaunchProgram(const XP_CHAR* aProgramPath, const XP_CHAR* aMinidumpPath)
{
  ...
  if (pid == -1) {
    ...
    Unused << execl(aProgramPath, aProgramPath, aMinidumpPath, (char*)0);
    _exit(1);
  }
#endif  // XP_MACOSX
...
static bool
LaunchCrashReporterActivity(XP_CHAR* aProgramPath,
    ...
    bool succeeded)
{
...

  {
    ...
    if (timeSinceLastCrash != 0) {
      WriteAnnotation(
          apiData, "SecondsSinceLastCrash", timeSinceLastCrashString);
      WriteAnnotation(
    ...
    if (numOfPendingIPCBuffer[0]) {
      ...
      if (topPendingIPCCountBuffer[0]) {
        WriteAnnotation(
            apiData, "TopPendingIPCCount", topPendingIPCCountBuffer);
        WriteAnnotation(
            eventFile, "TopPendingIPCCount", topPendingIPCCountBuffer);
      }
      ...
      if (topPendingIPCTypeBuffer[2]) {
        ...
        WriteAnnotation(
            eventFile, "TopPendingIPCType", topPendingIPCTypeBuffer);
      }
    }
...
static nsresult
LocateExecutable(nsIFile* aXREDirectory,
...
SetExceptionHandler(nsIFile* aXREDirectory, bool force /*=false*/)
{
  ...
  if (!headlessClient) {
    ...
    if (androidPackageName != nullptr) {
      ...
      nsCString package(ANDROID_PACKAGE_NAME
                        "/org.mozilla.gecko.CrashReporter");
      crashReporterPath = ToNewCString(package);
    }
#endif  // !defined(MOZ_WIDGET_ANDROID)
  }
...
SetupExtraData(nsIFile* aAppDataDirectory, const nsACString& aBuildID)
{
  ...
  rv = SetupCrashReporterDirectory(aAppDataDirectory,
                                   "Pending Pings",
  ...
  if (NS_SUCCEEDED(GetOrInit(
          dataDirectory, NS_LITERAL_CSTRING("LastCrash"), data, nullptr))) {
  ...
  if (headlessClient) {
    ...
    if (markerFilename.Length() < XP_PATH_MAX)
      wcsncpy(
          crashMarkerFilename, markerFilename.get(), markerFilename.Length());
#else
...
static void
ReplaceChar(nsCString& str,
            const nsACString& character,
...
#endif
static nsresult
EscapeAnnotation(const nsACString& key,
                 ...
                 nsCString& escapedData)
{
  ...
  // escape backslashes
  ReplaceChar(
      escapedData, NS_LITERAL_CSTRING("\\"), NS_LITERAL_CSTRING("\\\\"));
  // escape newlines
...
class DelayedNote
{
 public:
  ...
  explicit DelayedNote(const nsACString& aData) : mData(aData), mType(AppNote)
  {
  }
 ...
 */
static nsresult
PrefSubmitReports(bool* aSubmitReports, bool writePref)
{
  nsresult rv;
#if defined(XP_WIN32)
  /*
  ...
  *aSubmitReports = !!value;
  ...
  if (writePref) {
    ...
    ::CFPreferencesSetAppValue(
        CFSTR("submitReport"), cfValue, reporterClientAppID);
...
SetCrashEventsDir(nsIFile* aDir)
{
  ...
  if (env && *env) {
    NS_NewNativeLocalFile(
        nsDependentCString(env), false, getter_AddRefs(eventsDir));
  ...
  nsAutoString eventsDirEnv(
      NS_LITERAL_STRING("MOZ_CRASHREPORTER_EVENTS_DIRECTORY="));
               ...
               bool truncate = false)
{
  ...
  if (writeCrashTime) {
    ...
    WriteAnnotation(
        fd, nsDependentCString("UptimeTS"), nsDependentCString(uptimeTSString));
  }
...
#elif defined(XP_UNIX)
  rv = NS_NewLocalFile(
      NS_LITERAL_STRING("/tmp"), false, getter_AddRefs(extraFile));
  if (NS_FAILED(rv)) {
    return false;
  }
#else
#error "Implement this for your platform"
  ...
  nsAutoString leafName;
#if defined(XP_WIN)
  leafName.AppendPrintf(
      "%S%u%S", childCrashAnnotationBaseName, aPid, extraFileExtension);
#else
  leafName.AppendPrintf(
      "%s%u%s", childCrashAnnotationBaseName, aPid, extraFileExtension);
#endif
...
OOPInit()
{
  ...
  crashServer = new CrashGenerationServer(
      ...
      nullptr,
      nullptr,  // we don't care about process connect here
      OnChildProcessDumpRequested,
      nullptr,
      nullptr,
      nullptr,  // we don't care about process exit here
      nullptr,
      nullptr,  // we don't care about upload request here
  ...
  crashServer = new CrashGenerationServer(childCrashNotifyPipe,
                                          nullptr,
                                          nullptr,
                                          OnChildProcessDumpRequested,
                                          nullptr,
...
{
  if (lastRunCrashID) return true;

  // The exception handler callback leaves the filename of the
  // last minidump in a known file.
  nsCOMPtr<nsIFile> lastCrashFile;
  CreateFileFromPath(crashMarkerFilename, getter_AddRefs(lastCrashFile));
  ...
  CreateFileFromPath(lastMinidump.get(), getter_AddRefs(lastMinidumpFile));

  if (!lastMinidumpFile || NS_FAILED(lastMinidumpFile->Exists(&exists)) ||
      !exists) {
    return false;
  }
  ...
  if (NS_FAILED(rv) || NS_FAILED(memoryReportFile->Exists(&exists)) ||
      !exists) {
...
void
RenameAdditionalHangMinidump(nsIFile* minidump,
                               ...
                               bool aAsync)
{
  ...
  RenameAdditionalHangMinidump(
      aIncomingDumpToPair, targetMinidump, aIncomingPairName);
                  ...
                  size_t file_offset)
{
  if (!gExceptionHandler) {
    ...
    gExceptionHandler->AddMappingInfo(
        library_name, guid.data(), start_address, mapping_length, file_offset);
  }
}
#endif
