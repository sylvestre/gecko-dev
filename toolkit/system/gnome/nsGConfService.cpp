...
#define GCONF_FUNCTIONS                                                        \
  ...
  FUNC(                                                                        \
      gconf_client_get_bool, gboolean, (GConfClient*, const gchar*, GError**)) \
  FUNC(                                                                        \
      gconf_client_get_string, gchar*, (GConfClient*, const gchar*, GError**)) \
  FUNC(gconf_client_get_int, gint, (GConfClient*, const gchar*, GError**))     \
  FUNC(                                                                        \
      gconf_client_get_float, gdouble, (GConfClient*, const gchar*, GError**)) \
  FUNC(gconf_client_get_list,                                                  \
       GSList*,                                                                \
       (GConfClient*, const gchar*, GConfValueType, GError**))                 \
  FUNC(gconf_client_set_bool,                                                  \
       gboolean,                                                               \
       (GConfClient*, const gchar*, gboolean, GError**))                       \
  FUNC(gconf_client_set_string,                                                \
       gboolean,                                                               \
       (GConfClient*, const gchar*, const gchar*, GError**))                   \
  FUNC(gconf_client_set_int,                                                   \
       gboolean,                                                               \
       (GConfClient*, const gchar*, gint, GError**))                           \
  FUNC(gconf_client_set_float,                                                 \
       gboolean,                                                               \
...
nsGConfService::GetStringList(const nsACString& aKey, nsIArray** aResult)
{
  ...
  GSList* list = gconf_client_get_list(
      mClient, PromiseFlatCString(aKey).get(), GCONF_VALUE_STRING, &error);
  if (error) {
    g_error_free(error);
    return NS_ERROR_FAILURE;
  }
...
{
  bool res = gconf_client_set_bool(
      mClient, PromiseFlatCString(aKey).get(), aValue, nullptr);
...
{
  bool res = gconf_client_set_string(mClient,
...
{
  bool res = gconf_client_set_float(
      mClient, PromiseFlatCString(aKey).get(), aValue, nullptr);
...
NS_IMETHODIMP
nsGConfService::GetAppForProtocol(const nsACString& aScheme,
                                  bool* aEnabled,
                                  nsACString& aHandler)
{
