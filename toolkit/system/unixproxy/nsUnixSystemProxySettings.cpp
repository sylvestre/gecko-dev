...
class nsUnixSystemProxySettings final : public nsISystemProxySettings
{
 ...
 private:
  ...
  nsresult SetProxyResultFromGConf(const char* aKeyBase,
                                   const char* aType,
                                   nsACString& aResult);
  nsresult GetProxyFromGConf(const nsACString& aScheme,
  ...
  nsresult GetProxyFromGSettings(const nsACString& aScheme,
                                 ...
                                 int32_t aPort,
                                 nsACString& aResult);
  nsresult SetProxyResultFromGSettings(const char* aKeyBase,
                                       const char* aType,
                                       nsACString& aResult);
};
...
static void
SetProxyResult(const char* aType,
               const nsACString& aHost,
               int32_t aPort,
               nsACString& aResult)
{
...
nsresult
nsUnixSystemProxySettings::SetProxyResultFromGConf(const char* aKeyBase,
                                                   const char* aType,
                                                   nsACString& aResult)
{
...
nsresult
nsUnixSystemProxySettings::SetProxyResultFromGSettings(const char* aKeyBase,
                                                       const char* aType,
                                                       nsACString& aResult)
{
...
static bool
ConvertToIPV6Addr(const nsACString& aName, PRIPv6Addr* aAddr, int32_t* aMask)
{
  PRNetAddr addr;
...
static bool
HostIgnoredByProxy(const nsACString& aIgnore, const nsACString& aHost)
{
  if (aIgnore.Equals(aHost, nsCaseInsensitiveCStringComparator())) return true;

  if (aIgnore.First() == '*' &&
      StringEndsWith(aHost,
                     nsDependentCSubstring(aIgnore, 1),
                                                 ...
                                                 nsACString& aResult)
{
  ...
  if (aScheme.LowerCaseEqualsLiteral("http")) {
    rv = SetProxyResultFromGSettings(
        "org.gnome.system.proxy.http", "PROXY", aResult);
  } else if (aScheme.LowerCaseEqualsLiteral("https")) {
    rv = SetProxyResultFromGSettings(
        "org.gnome.system.proxy.https", "PROXY", aResult);
    ...
    rv = SetProxyResultFromGSettings(
        "org.gnome.system.proxy.ftp", "PROXY", aResult);
  } else {
    rv = NS_ERROR_FAILURE;
  }
  if (rv != NS_OK) {
    ...
    rv = SetProxyResultFromGSettings(
        "org.gnome.system.proxy.socks", "SOCKS", aResult);
  }
