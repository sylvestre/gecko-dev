                       ...
                       public nsSupportsWeakReference
{
 protected:
  ...
  friend nsresult NS_NewLocalStore(nsISupports* aOuter,
                                   REFNSIID aIID,
                                   void** aResult);
 ...
 public:
                          ...
                          bool* hasAssertion) override
  {
    return mInner->HasAssertion(
        aSource, aProperty, aTarget, aTruthValue, hasAssertion);
  }
  ...
  NS_IMETHOD HasArcIn(nsIRDFNode* aNode,
                      nsIRDFResource* aArc,
  ...
  NS_IMETHOD HasArcOut(nsIRDFResource* aSource,
                       nsIRDFResource* aArc,
                       bool* _retval) override
  {
...
LocalStoreImpl::~LocalStoreImpl(void)
{
  if (mRDFService) mRDFService->UnregisterDataSource(this);
}
...
NS_NewLocalStore(nsISupports* aOuter, REFNSIID aIID, void** aResult)
{
  ...
  if (!impl) return NS_ERROR_OUT_OF_MEMORY;
...
NS_IMETHODIMP
LocalStoreImpl::Init(const char* uri) { return (NS_OK); }
...
LocalStoreImpl::CreateLocalStore(nsIFile* aFile)
{
  ...
  if (count != sizeof(defaultRDF) - 1) return NS_ERROR_UNEXPECTED;
  ...
  if (!fileExistsFlag) return NS_ERROR_UNEXPECTED;
...
LocalStoreImpl::LoadData()
{
  ...
  mInner = do_CreateInstance(
      NS_RDF_DATASOURCE_CONTRACTID_PREFIX "xml-datasource", &rv);
  if (NS_FAILED(rv)) return rv;
...
NS_IMETHODIMP
LocalStoreImpl::Observe(nsISupports* aSubject,
                        const char* aTopic,
                        const char16_t* someData)
{
  ...
  if (!nsCRT::strcmp(aTopic, "profile-before-change")) {
    ...
    // profile-less.
    mInner = do_CreateInstance(NS_RDF_DATASOURCE_CONTRACTID_PREFIX
