...
FileSystemDataSource::Init()
{
  ...
  nsresult tmp = mRDFService->GetResource(
      NS_LITERAL_CSTRING(NC_NAMESPACE_URI "child"), getter_AddRefs(mNC_Child));
  if (NS_FAILED(tmp)) {
    rv = tmp;
  }
  ...
  tmp = mRDFService->GetResource(
      NS_LITERAL_CSTRING(NC_NAMESPACE_URI "Content-Length"),
      getter_AddRefs(mNC_Length));
  if (NS_FAILED(tmp)) {
    rv = tmp;
  }
  tmp = mRDFService->GetResource(
      NS_LITERAL_CSTRING(NC_NAMESPACE_URI "IsDirectory"),
      getter_AddRefs(mNC_IsDirectory));
  if (NS_FAILED(tmp)) {
    rv = tmp;
  }
  tmp = mRDFService->GetResource(
      NS_LITERAL_CSTRING(WEB_NAMESPACE_URI "LastModifiedDate"),
      getter_AddRefs(mWEB_LastMod));
  if (NS_FAILED(tmp)) {
    rv = tmp;
  }
  tmp = mRDFService->GetResource(
      NS_LITERAL_CSTRING(NC_NAMESPACE_URI "FileSystemObject"),
  ...
  tmp = mRDFService->GetResource(
      NS_LITERAL_CSTRING(RDF_NAMESPACE_URI "instanceOf"),
  ...
  rv =
      mRDFService->GetResource(NS_LITERAL_CSTRING(NC_NAMESPACE_URI "extension"),
                               getter_AddRefs(mNC_extension));
  NS_ENSURE_SUCCESS(rv, rv);
#endif

#ifdef XP_WIN
  rv = mRDFService->GetResource(
      NS_LITERAL_CSTRING(NC_NAMESPACE_URI "IEFavorite"),
      getter_AddRefs(mNC_IEFavoriteObject));
  tmp = mRDFService->GetResource(
      NS_LITERAL_CSTRING(NC_NAMESPACE_URI "IEFavoriteFolder"),
...
//static
nsresult
FileSystemDataSource::Create(nsISupports* aOuter,
                             const nsIID& aIID,
                             void** aResult)
{
    ...
    if (property == mNC_Name) {
      ...
      *target = (isDirURI(source)) ? mLiteralTrue : mLiteralFalse;
      ...
      if (hasMore) {
        ...
        return isupports->QueryInterface(NS_GET_IID(nsIRDFNode),
                                         (void**)target);
      }
    }
#ifdef USE_NC_EXTENSION
                                 ...
                                 nsISimpleEnumerator** targets /* out */)
{
  ...
  if (!targets) return NS_ERROR_NULL_POINTER;
  ...
  *targets = nullptr;
  ...
  if (source == mNC_FileSystemRoot) {
    ...
    if (property == mNC_Child) {
      ...
      nsCOMPtr<nsIRDFLiteral> pulseLiteral;
      rv = mRDFService->GetLiteral(u"12", getter_AddRefs(pulseLiteral));
      if (NS_FAILED(rv)) return rv;
                                   ...
                                   bool* hasAssertion /* out */)
{
  ...
  if (!hasAssertion) return NS_ERROR_NULL_POINTER;
...
NS_IMETHODIMP
FileSystemDataSource::HasArcIn(nsIRDFNode* aNode,
                               nsIRDFResource* aArc,
                               bool* result)
{
  return NS_ERROR_NOT_IMPLEMENTED;
...
NS_IMETHODIMP
FileSystemDataSource::HasArcOut(nsIRDFResource* aSource,
                                nsIRDFResource* aArc,
                                bool* result)
{
  *result = false;
                                   ...
                                   nsISimpleEnumerator** labels /* out */)
{
  ...
  if (!labels) return NS_ERROR_NULL_POINTER;
...
NS_IMETHODIMP
FileSystemDataSource::GetAllCmds(
    nsIRDFResource* source, nsISimpleEnumerator /*<nsIRDFResource>*/** commands)
{
  return (NS_NewEmptyEnumerator(commands));
}

NS_IMETHODIMP
FileSystemDataSource::IsCommandEnabled(
    nsISupports /*<nsIRDFResource>*/* aSources,
...
NS_IMETHODIMP
FileSystemDataSource::BeginUpdateBatch() { return NS_OK; }

NS_IMETHODIMP
FileSystemDataSource::EndUpdateBatch() { return NS_OK; }
...
FileSystemDataSource::isValidFolder(nsIRDFResource* source)
{
  ...
  if (theURI.Find(ieFavoritesDir) == 0) {
    ...
    if (NS_SUCCEEDED(rv = GetFolderList(
                         source, true, false, getter_AddRefs(folderEnum)))) {
      bool hasAny = false, hasMore;
      while (NS_SUCCEEDED(folderEnum->HasMoreElements(&hasMore)) && hasMore) {
        hasAny = true;
        ...
        if (NS_FAILED(rv = GetName(res, getter_AddRefs(nameLiteral)))) break;

        const char16_t* uniName;
        if (NS_FAILED(rv = nameLiteral->GetValueConst(&uniName))) break;
...
nsresult
FileSystemDataSource::GetFolderList(nsIRDFResource* source,
                                    bool allowHidden,
                                    bool onlyFirst,
                                    nsISimpleEnumerator** aResult)
{
  if (!isDirURI(source)) return (NS_RDF_NO_VALUE);
  ...
  if (NS_FAILED(
          rv = NS_NewURI(getter_AddRefs(aIURI), nsDependentCString(parentURI))))
    return (rv);
  ...
  nsCOMPtr<nsIFile> aDir;
  if (NS_FAILED(rv = fileURL->GetFile(getter_AddRefs(aDir)))) return (rv);
  ...
  nsCOMArray<nsIRDFResource> resources;
  bool hasMore;
  while (NS_SUCCEEDED(rv = dirContents->HasMoreElements(&hasMore)) && hasMore) {
    nsCOMPtr<nsISupports> isupports;
    if (NS_FAILED(rv = dirContents->GetNext(getter_AddRefs(isupports)))) break;
    ...
    if (!allowHidden) {
      ...
      if (NS_FAILED(rv = aFile->IsHidden(&hiddenFlag))) break;
      if (hiddenFlag) continue;
    }

    nsAutoString leafStr;
    if (NS_FAILED(rv = aFile->GetLeafName(leafStr))) break;
    if (leafStr.IsEmpty()) continue;
...
FileSystemDataSource::GetLastMod(nsIRDFResource* source, nsIRDFDate** aResult)
{
  *aResult = nullptr;
  ...
  if (NS_FAILED(rv = fileURL->GetFile(getter_AddRefs(aFile)))) return (rv);
  ...
  PRTime lastModDate;
  if (NS_FAILED(rv = aFile->GetLastModifiedTime(&lastModDate))) return (rv);
...
FileSystemDataSource::GetFileSize(nsIRDFResource* source, nsIRDFInt** aResult)
{
  *aResult = nullptr;
  ...
  bool isDir = false;
  if (NS_FAILED(rv = aFile->IsDirectory(&isDir))) return (rv);
  if (isDir) return (NS_RDF_NO_VALUE);

  int64_t aFileSize64;
  if (NS_FAILED(rv = aFile->GetFileSize(&aFileSize64))) return (rv);
...
FileSystemDataSource::GetName(nsIRDFResource* source, nsIRDFLiteral** aResult)
{
  ...
  nsAutoString name;
  if (NS_FAILED(rv = aFile->GetLeafName(name))) return (rv);
  if (name.IsEmpty()) return (NS_ERROR_UNEXPECTED);
  ...
  if ((strncmp(uri, ieFavoritesDir.get(), ieFavoritesDir.Length()) == 0) &&
      (nameLen > 4)) {
...
#ifdef USE_NC_EXTENSION
nsresult
FileSystemDataSource::GetExtension(nsIRDFResource* source,
...
#ifdef XP_WIN
nsresult
FileSystemDataSource::getIEFavoriteURL(nsIRDFResource* source,
                                       nsString aFileURL,
                                       nsIRDFLiteral** urlLiteral)
{
  ...
  *urlLiteral = nullptr;
  ...
  if (NS_SUCCEEDED(f->IsDirectory(&value)) && value) {
    if (isValidFolder(source)) return (NS_RDF_NO_VALUE);
...
nsresult
FileSystemDataSource::GetURL(nsIRDFResource* source,
                             bool* isFavorite,
                             nsIRDFLiteral** aResult)
{
