                                            ...
                                            nsIRDFNode* aValue)
{
  ...
  if ((resource = do_QueryInterface(aValue)) != nullptr) {
    ...
    rv = rdf_BlockingWrite(
        aStream, kRDFParseTypeInteger, sizeof(kRDFParseTypeInteger) - 1);
    if (NS_FAILED(rv)) return rv;
    ...
    rv = rdf_BlockingWrite(
        aStream, kRDFParseTypeDate, sizeof(kRDFParseTypeDate) - 1);
    if (NS_FAILED(rv)) return rv;
    rv = rdf_BlockingWrite(aStream, s);
    if (NS_FAILED(rv)) return rv;
  } else {
                                      ...
                                      int32_t* aSkipped)
{
  ...
  mDataSource->GetTargets(
      aResource, aProperty, true, getter_AddRefs(assertions));
                                         ...
                                         nsIRDFResource* aResource)
{
  ...
  if (isTypedNode) {
    ...
    rv = rdf_BlockingWrite(
        aStream, kRDFDescriptionOpen, sizeof(kRDFDescriptionOpen) - 1);
    if (NS_FAILED(rv)) return rv;
  }
  if (uri[0] == char16_t('#')) {
    uri.Cut(0, 1);
  ...
  if (skipped) {
    ...
    if (isTypedNode) {
      ...
      rv = rdf_BlockingWrite(
          aStream, kRDFDescriptionClose, sizeof(kRDFDescriptionClose) - 1);
      if (NS_FAILED(rv)) return rv;
    }
  } else {
                                       ...
                                       nsIRDFResource* aContainer)
{
  ...
  rv = NS_NewContainerEnumerator(
      mDataSource, aContainer, getter_AddRefs(elements));
...
nsresult
QNameCollector::Visit(nsIRDFNode* aSubject,
...
bool
nsRDFXMLSerializer::IsA(nsIRDFDataSource* aDataSource,
                        nsIRDFResource* aResource,
                        nsIRDFResource* aType)
{
  ...
  bool result;
  rv = aDataSource->HasAssertion(
      aResource, kRDF_instanceOf, aType, true, &result);
