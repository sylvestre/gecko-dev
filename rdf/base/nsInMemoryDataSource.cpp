                           ...
                           public rdfIDataSource
{
 protected:
  ...
  friend class
      InMemoryResourceEnumeratorImpl;  // b/c it needs to enumerate mForwardArcs

  // Thread-safe writer implementation methods.
  nsresult LockedAssert(nsIRDFResource* source,
                        nsIRDFResource* property,
                        nsIRDFNode* target,
                        bool tv);

  nsresult LockedUnassert(nsIRDFResource* source,
                          nsIRDFResource* property,
                          nsIRDFNode* target);
 ...
 public:
  ...
  void LogOperation(const char* aOperation,
...
InMemoryAssertionEnumeratorImpl::~InMemoryAssertionEnumeratorImpl()
{
  ...
  --gInstanceCount;
  fprintf(
      stdout, "%d - RDF: InMemoryAssertionEnumeratorImpl\n", gInstanceCount);
#endif
                                 ...
                                 bool aTruthValue)
{
  ...
  MOZ_LOG(
      gLog, LogLevel::Debug, ("InMemoryDataSource(%p): %s", this, aOperation));
  ...
  char tv = (aTruthValue ? '-' : '!');
  MOZ_LOG(
      gLog, LogLevel::Debug, ("  --%c[(%p)%s]--", tv, aProperty, uri.get()));
...
NS_IMETHODIMP
InMemoryDataSource::HasArcIn(nsIRDFNode* aNode,
                             nsIRDFResource* aArc,
                             bool* result)
{
...
NS_IMETHODIMP
InMemoryDataSource::HasArcOut(nsIRDFResource* aSource,
                              nsIRDFResource* aArc,
                              bool* result)
{
  ...
  while (as) {
    ...
    if (!(as->mHashEntry)) {
      for (int32_t i = int32_t(mNumObservers) - 1; mPropagateChanges && i >= 0;
        ...
        obs->OnUnassert(
            this, as->mSource, as->u.as.mProperty, as->u.as.mTarget);
        // XXX ignore return value?
      }
    }
