...
#undef RDF_ATOM
 ...
 protected:
  ...
  nsresult GetResourceAttribute(const char16_t** aAttributes,
                                nsIRDFResource** aResource);
  nsresult AddProperties(const char16_t** aAttributes,
  ...
  int32_t PushContext(nsIRDFResource* aContext,
                      RDFContentSinkState aState,
                      RDFContentSinkParseMode aParseMode);

  nsresult PopContext(nsIRDFResource*& aContext,
                      RDFContentSinkState& aState,
                      RDFContentSinkParseMode& aParseMode);
...
RDFContentSinkImpl::HandleEndElement(const char16_t* aName)
{
  ...
  switch (mState) {
    ...
    case eRDFContentSinkState_InPropertyElement: {
      mDataSource->Assert(
          GetContextElement(1), GetContextElement(0), resource, true);
    } break;
    default:
      break;
  }
...
NS_IMETHODIMP
RDFContentSinkImpl::HandleCDataSection(const char16_t* aData, uint32_t aLength)
{
...
NS_IMETHODIMP
RDFContentSinkImpl::HandleCharacterData(const char16_t* aData, uint32_t aLength)
{
  return aData ? AddText(aData, aLength) : NS_OK;
}
    ...
    if (rdf_IsDataInBuffer(mText, mTextLength)) {
      ...
      switch (mState) {
        ...
        case eRDFContentSinkState_InPropertyElement: {
          ...
          mDataSource->Assert(
              GetContextElement(1), GetContextElement(0), node, true);
        } break;
...
nsresult
RDFContentSinkImpl::OpenProperty(const char16_t* aName,
                                 const char16_t** aAttributes)
{
  nsresult rv;
...
nsresult
RDFContentSinkImpl::OpenValue(const char16_t* aName,
                              const char16_t** aAttributes)
{
...
nsresult
RDFContentSinkImpl::InitContainer(nsIRDFResource* aContainerType,
                                  nsIRDFResource* aContainer)
{
  ...
  for (const ContainerInfo* info = gContainerInfo; info->mType != 0; ++info) {
    ...
    bool isContainer;
    rv = (gRDFContainerUtils->*(info->mTestFn))(
        mDataSource, aContainer, &isContainer);
    if (isContainer) {
      rv = ReinitContainer(aContainerType, aContainer);
    } else {
      rv = (gRDFContainerUtils->*(info->mMakeFn))(
          mDataSource, aContainer, nullptr);
    }
    return rv;
  }
...
nsresult
RDFContentSinkImpl::ReinitContainer(nsIRDFResource* aContainerType,
                                    nsIRDFResource* aContainer)
{
  ...
  rv = mDataSource->GetTarget(
      aContainer, kRDF_nextVal, true, getter_AddRefs(nextval));
  if (NS_FAILED(rv)) return rv;
