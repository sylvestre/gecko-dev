...
RDFContainerImpl::GetCount(int32_t* aCount)
{
  ...
  rv = mDataSource->GetTarget(
      mContainer, kRDF_nextVal, true, getter_AddRefs(nextValNode));
  if (NS_FAILED(rv)) return rv;
...
NS_IMETHODIMP
RDFContainerImpl::InsertElementAt(nsIRDFNode* aElement,
                                  int32_t aIndex,
                                  bool aRenumber)
{
  if (!mDataSource || !mContainer) return NS_ERROR_NOT_INITIALIZED;
...
NS_IMETHODIMP
RDFContainerImpl::RemoveElementAt(int32_t aIndex,
                                  bool aRenumber,
                                  nsIRDFNode** _retval)
{
  if (!mDataSource || !mContainer) return NS_ERROR_NOT_INITIALIZED;
...
RDFContainerImpl::RDFContainerImpl() : mDataSource(nullptr), mContainer(nullptr)
{
}
...
RDFContainerImpl::Renumber(int32_t aStartIndex, int32_t aIncrement)
{
  ...
  while (!err && ((aIncrement < 0) ? (i <= count) : (i >= aStartIndex))) {
    ...
    rv = mDataSource->GetTargets(
        mContainer, oldOrdinal, true, getter_AddRefs(targets));
    if (NS_FAILED(rv)) {
      err = true;
      continue;
    }
...
RDFContainerImpl::SetNextValue(int32_t aIndex)
{
  ...
  if (NS_SUCCEEDED(
          rv = mDataSource->GetTarget(
              mContainer, kRDF_nextVal, true, getter_AddRefs(nextValNode)))) {
    if (NS_FAILED(rv = mDataSource->Unassert(
                      mContainer, kRDF_nextVal, nextValNode))) {
