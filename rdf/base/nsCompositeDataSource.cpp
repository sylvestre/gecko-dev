...
class CompositeEnumeratorImpl : public nsISimpleEnumerator
{
  ...
  virtual nsresult HasNegation(nsIRDFDataSource* aDataSource,
                               nsIRDFNode* aNode,
                               bool* aResult) = 0;
...
nsresult
CompositeArcsInOutEnumeratorImpl::GetEnumerator(nsIRDFDataSource* aDataSource,
                                                nsISimpleEnumerator** aResult)
{
  if (mType == eArcsIn) {
...
nsresult
CompositeArcsInOutEnumeratorImpl::HasNegation(nsIRDFDataSource* aDataSource,
                                              nsIRDFNode* aNode,
                                              bool* aResult)
{
  *aResult = false;
...
nsresult
CompositeAssertionEnumeratorImpl::GetEnumerator(nsIRDFDataSource* aDataSource,
                                                nsISimpleEnumerator** aResult)
{
  if (mSource) {
...
nsresult
CompositeAssertionEnumeratorImpl::HasNegation(nsIRDFDataSource* aDataSource,
                                              nsIRDFNode* aNode,
                                              bool* aResult)
{
  if (mSource) {
    return aDataSource->HasAssertion(
        mSource, mProperty, aNode, !mTruthValue, aResult);
  } else {
    nsCOMPtr<nsIRDFResource> source(do_QueryInterface(aNode));
    return aDataSource->HasAssertion(
        source, mProperty, mTarget, !mTruthValue, aResult);
  }
}
                                    ...
                                    nsISimpleEnumerator** aResult)
{
  ...
  *aResult = new CompositeAssertionEnumeratorImpl(this,
                                                  nullptr,
                                                  aProperty,
                                                  aTarget,
                                                  aTruthValue,
                                   ...
                                   nsIRDFNode** aResult)
{
  ...
  for (int32_t i = 0; i < count; ++i) {
    ...
    if (rv == NS_OK) {
      ...
      if (mAllowNegativeAssertions) {
        if (HasAssertionN(
                count - 1, aSource, aProperty, *aResult, !aTruthValue)) {
                                       ...
                                       bool aTruthValue)
{
  ...
  for (int32_t m = 0; m < n; ++m) {
    bool result;
    rv = mDataSources[m]->HasAssertion(
        aSource, aProperty, aTarget, aTruthValue, &result);
                                      ...
                                      bool* aResult)
{
  ...
  for (int32_t i = 0; i < count; ++i) {
    ...
    rv = datasource->HasAssertion(
        aSource, aProperty, aTarget, aTruthValue, aResult);
    if (NS_FAILED(rv)) return rv;
    ...
    if (mAllowNegativeAssertions) {
      bool hasNegation;
      rv = datasource->HasAssertion(
          aSource, aProperty, aTarget, !aTruthValue, &hasNegation);
      if (NS_FAILED(rv)) return rv;
...
NS_IMETHODIMP
CompositeDataSourceImpl::HasArcIn(nsIRDFNode* aNode,
                                  nsIRDFResource* aArc,
                                  bool* result)
{
  nsresult rv;
    ...
    bool* aResult)
{
  ...
  for (int32_t i = mDataSources.Count() - 1; i >= 0; --i) {
    bool enabled = true;
    rv = mDataSources[i]->IsCommandEnabled(
        aSources, aCommand, aArguments, &enabled);
