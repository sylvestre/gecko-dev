...
static PLHashAllocOps dataSourceHashAllocOps = {DataSourceAllocTable,
                                                DataSourceFreeTable,
...
struct ResourceHashEntry : public PLDHashEntryHdr
{
  ...
  static bool MatchEntry(const PLDHashEntryHdr* hdr, const void* key)
  {
    const ResourceHashEntry* entry = static_cast<const ResourceHashEntry*>(hdr);

    return 0 == nsCRT::strcmp(static_cast<const char*>(key), entry->mKey);
  }
};

static const PLDHashTableOps gResourceTableOps = {ResourceHashEntry::HashKey,
...
struct LiteralHashEntry : public PLDHashEntryHdr
{
  ...
  static bool MatchEntry(const PLDHashEntryHdr* hdr, const void* key)
  {
    const LiteralHashEntry* entry = static_cast<const LiteralHashEntry*>(hdr);

    return 0 == nsCRT::strcmp(static_cast<const char16_t*>(key), entry->mKey);
  }
};

static const PLDHashTableOps gLiteralTableOps = {LiteralHashEntry::HashKey,
...
struct IntHashEntry : public PLDHashEntryHdr
{
  ...
  static bool MatchEntry(const PLDHashEntryHdr* hdr, const void* key)
  {
    const IntHashEntry* entry = static_cast<const IntHashEntry*>(hdr);

    return *static_cast<const int32_t*>(key) == entry->mKey;
  }
};

static const PLDHashTableOps gIntTableOps = {IntHashEntry::HashKey,
...
struct DateHashEntry : public PLDHashEntryHdr
{
  ...
  static bool MatchEntry(const PLDHashEntryHdr* hdr, const void* key)
  {
    const DateHashEntry* entry = static_cast<const DateHashEntry*>(hdr);

    return *static_cast<const PRTime*>(key) == entry->mKey;
  }
};

static const PLDHashTableOps gDateTableOps = {DateHashEntry::HashKey,
...
struct BlobHashEntry : public PLDHashEntryHdr
{
  ...
  static PLDHashNumber HashKey(const void* key)
  {
    const BlobImpl::Data* data = static_cast<const BlobImpl::Data*>(key);
    return HashBytes(data->mBytes, data->mLength);
  }

  static bool MatchEntry(const PLDHashEntryHdr* hdr, const void* key)
  {
    const BlobHashEntry* entry = static_cast<const BlobHashEntry*>(hdr);

    const BlobImpl::Data* left = &entry->mBlob->mData;

    const BlobImpl::Data* right = static_cast<const BlobImpl::Data*>(key);
...
static const PLDHashTableOps gBlobTableOps = {BlobHashEntry::HashKey,
...
{
 public:
  static nsresult Create(const char16_t* aValue, nsIRDFLiteral** aResult);
 ...
 protected:
  ...
  {
    size_t objectSize =
        ((sizeof(LiteralImpl) + sizeof(char16_t) - 1) / sizeof(char16_t)) *
        sizeof(char16_t);
...
LiteralImpl::Create(const char16_t* aValue, nsIRDFLiteral** aResult)
{
  ...
  if (!objectPtr) return NS_ERROR_NULL_POINTER;

  char16_t* buf = reinterpret_cast<char16_t*>(
      static_cast<unsigned char*>(objectPtr) + objectSize);
...
LiteralImpl::QueryInterface(REFNSIID iid, void** result)
{
  ...
  *result = nullptr;
  if (iid.Equals(kIRDFLiteralIID) || iid.Equals(kIRDFNodeIID) ||
...
LiteralImpl::GetValue(char16_t** value)
{
  ...
  if (!value) return NS_ERROR_NULL_POINTER;
...
DateImpl::DateImpl(const PRTime s) : mValue(s)
{
...
DateImpl::QueryInterface(REFNSIID iid, void** result)
{
  ...
  *result = nullptr;
  if (iid.Equals(kIRDFDateIID) || iid.Equals(kIRDFNodeIID) ||
      iid.Equals(kISupportsIID)) {
...
DateImpl::EqualsDate(nsIRDFDate* date, bool* result)
{
  ...
  if (!date || !result) return NS_ERROR_NULL_POINTER;

  nsresult rv;
  PRTime p;
  if (NS_FAILED(rv = date->GetValue(&p))) return rv;
...
IntImpl::IntImpl(int32_t s) : mValue(s)
{
...
IntImpl::QueryInterface(REFNSIID iid, void** result)
{
  ...
  *result = nullptr;
  if (iid.Equals(kIRDFIntIID) || iid.Equals(kIRDFNodeIID) ||
      iid.Equals(kISupportsIID)) {
...
IntImpl::EqualsInt(nsIRDFInt* intValue, bool* result)
{
  ...
  if (!intValue || !result) return NS_ERROR_NULL_POINTER;

  nsresult rv;
  int32_t p;
  if (NS_FAILED(rv = intValue->GetValue(&p))) return rv;
...
RDFServiceImpl* RDFServiceImpl::gRDFService;
...
// Per RFC2396.
static const uint8_t kLegalSchemeChars[] = {
...
RDFServiceImpl::GetResource(const nsACString& aURI, nsIRDFResource** aResource)
{
  ...
  if (aURI.IsEmpty()) return NS_ERROR_INVALID_ARG;
  ...
  aURI.BeginReading(p);
  aURI.EndReading(end);
  while (p != end && IsLegalSchemeCharacter(*p)) ++p;
  ...
  rv = factory->CreateInstance(
      nullptr, NS_GET_IID(nsIRDFResource), (void**)&result);
  if (NS_FAILED(rv)) return rv;
...
NS_IMETHODIMP
RDFServiceImpl::GetUnicodeResource(const nsAString& aURI,
...
RDFServiceImpl::GetLiteral(const char16_t* aValue, nsIRDFLiteral** aLiteral)
{
  ...
  if (!aLiteral) return NS_ERROR_NULL_POINTER;
...
NS_IMETHODIMP
RDFServiceImpl::GetBlobLiteral(const uint8_t* aBytes,
                               int32_t aLength,
                               nsIRDFBlob** aResult)
{
...
RDFServiceImpl::RegisterResource(nsIRDFResource* aResource, bool aReplace)
{
  ...
  if (hdr) {
    ...
    MOZ_LOG(
        gLog,
        LogLevel::Debug,
        ("rdfserv   register-resource [%p] %s", aResource, (const char*)uri));
  }
...
RDFServiceImpl::UnregisterResource(nsIRDFResource* aResource)
{
  ...
  MOZ_LOG(gLog,
          LogLevel::Debug,
          ("rdfserv unregister-resource [%p] %s", aResource, (const char*)uri));

#ifdef DEBUG
  if (!mResources.Search(uri)) NS_WARNING("resource was never registered");
#endif
...
RDFServiceImpl::RegisterDataSource(nsIRDFDataSource* aDataSource, bool aReplace)
{
  ...
  PLHashEntry** hep = PL_HashTableRawLookup(
      mNamedDataSources, (*mNamedDataSources->keyHash)(uri.get()), uri.get());

  if (*hep) {
    if (!aReplace) return NS_ERROR_FAILURE;  // already registered
    ...
    MOZ_LOG(gLog,
            LogLevel::Debug,
            ("rdfserv   register-datasource [%p] %s", aDataSource, uri.get()));
...
RDFServiceImpl::UnregisterDataSource(nsIRDFDataSource* aDataSource)
{
  ...
  if (uri.IsVoid()) return NS_ERROR_UNEXPECTED;
  ...
  if (!*hep || ((*hep)->value != aDataSource)) return NS_OK;
  ...
  MOZ_LOG(gLog,
          LogLevel::Debug,
          ("rdfserv unregister-datasource [%p] %s", aDataSource, uri.get()));
...
NS_IMETHODIMP
RDFServiceImpl::GetDataSourceBlocking(const char* aURI,
...
nsresult
RDFServiceImpl::GetDataSource(const char* aURI,
                              bool aBlock,
                              nsIRDFDataSource** aDataSource)
{
  ...
  {
    nsIRDFDataSource* cached = static_cast<nsIRDFDataSource*>(
        PL_HashTableLookup(mNamedDataSources, spec.get()));
...
RDFServiceImpl::RegisterInt(nsIRDFInt* aInt)
{
  ...
  MOZ_LOG(
      gLog, LogLevel::Debug, ("rdfserv   register-int [%p] %d", aInt, value));
...
RDFServiceImpl::UnregisterInt(nsIRDFInt* aInt)
{
  ...
  // reference to it in the hashtable.
  MOZ_LOG(
      gLog, LogLevel::Debug, ("rdfserv unregister-int [%p] %d", aInt, value));
...
RDFServiceImpl::RegisterDate(nsIRDFDate* aDate)
{
  ...
  MOZ_LOG(gLog,
          LogLevel::Debug,
          ("rdfserv   register-date [%p] %" PRId64, aDate, value));
...
RDFServiceImpl::UnregisterDate(nsIRDFDate* aDate)
{
  ...
  MOZ_LOG(gLog,
          LogLevel::Debug,
          ("rdfserv unregister-date [%p] %" PRId64, aDate, value));
...
RDFServiceImpl::RegisterBlob(BlobImpl* aBlob)
{
  ...
  MOZ_LOG(gLog,
          LogLevel::Debug,
          ("rdfserv   register-blob [%p] %s", aBlob, aBlob->mData.mBytes));
...
RDFServiceImpl::UnregisterBlob(BlobImpl* aBlob)
{
  ...
  MOZ_LOG(gLog,
          LogLevel::Debug,
          ("rdfserv unregister-blob [%p] %s", aBlob, aBlob->mData.mBytes));
