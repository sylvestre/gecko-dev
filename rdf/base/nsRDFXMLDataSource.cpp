                             ...
                             public nsIChannelEventSink
{
 ...
 public:
  ...
  bool MakeQName(nsIRDFResource* aResource,
                 nsString& property,
  ...
  nsresult SerializeMember(nsIOutputStream* aStream,
  ...
  bool IsA(nsIRDFDataSource* aDataSource,
           nsIRDFResource* aResource,
...
RDFXMLDataSourceImpl::BlockingParse(nsIURI* aURL, nsIStreamListener* aConsumer)
{
  ...
  rv = NS_NewBufferedInputStream(
      getter_AddRefs(bufStream), in.forget(), 4096 /* buffer size */);
  if (NS_FAILED(rv)) return rv;
  ...
  while (NS_SUCCEEDED(rv)) {
    ...
    rv = aConsumer->OnDataAvailable(
        channel, nullptr, bufStream, offset, (uint32_t)avail);
