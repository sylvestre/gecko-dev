...
static bool
TOKEN_STRING(const Token& t, const char* s)
{
  return !t.value().compare(string(s));
}

static bool
isObject(const Token& t, int children)
{
  return t.isObject() && t.children() == children;
}

static bool
isArray(const Token& t, int children)
{
  return t.isArray() && t.children() == children;
}

static bool
isKey(const Token& t, const char* v)
{
  return t.isString() && t.children() == 1 && t.value() == v;
}

static bool
isPrimitive(const Token& t, const char* v)
{
