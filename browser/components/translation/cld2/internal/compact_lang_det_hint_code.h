...
inline int
GetCLDPriorWeight(OneCLDLangPrior olp)
{
  return olp >> 10;
}
inline Language
GetCLDPriorLang(OneCLDLangPrior olp)
{
  return static_cast<Language>(olp & 0x3ff);
}

inline int32
GetCLDLangPriorCount(CLDLangPriors* lps)
{
  return lps->n;
}

inline void
InitCLDLangPriors(CLDLangPriors* lps)
{
  lps->n = 0;
}
...
void
TrimCLDLangPriors(int max_entries, CLDLangPriors* lps);

// Trim language tag string to canonical form for each language
// Input is from GetLangTagsFromHtml(), already lowercased
std::string
TrimCLDLangTagsHint(const std::string& langtags);

// Add hints to vector of langpriors
// Input is from GetLangTagsFromHtml(), already lowercased
void
SetCLDLangTagsHint(const std::string& langtags, CLDLangPriors* langpriors);

// Add hints to vector of langpriors
// Input is from HTTP content-language
void
SetCLDContentLangHint(const char* contentlang, CLDLangPriors* langpriors);

// Add hints to vector of langpriors
// Input is from GetTLD(), already lowercased
void
SetCLDTLDHint(const char* tld, CLDLangPriors* langpriors);

// Add hints to vector of langpriors
// Input is from DetectEncoding()
void
SetCLDEncodingHint(Encoding enc, CLDLangPriors* langpriors);

// Add hints to vector of langpriors
// Input is from random source
void
SetCLDLanguageHint(Language lang, CLDLangPriors* langpriors);

// Make printable string of priors
std::string
DumpCLDLangPriors(const CLDLangPriors* langpriors);

// Get language tag hints from HTML body
// Normalize: remove spaces and make lowercase comma list
std::string
GetLangTagsFromHtml(const char* utf8_body,
                    int32 utf8_body_len,
