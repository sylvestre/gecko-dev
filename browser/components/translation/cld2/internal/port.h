...
#if defined(ARCH_PIII) || defined(ARCH_ATHLON) || defined(ARCH_K8) || \
    defined(_ARCH_PPC)
...
inline void
UNALIGNED_STORE64(void* p, uint64 v)
{
  memcpy(p, &v, sizeof v);
}
...
inline void
UNALIGNED_STORE16(void* p, uint16 v)
{
  memcpy(p, &v, sizeof v);
}

inline void
UNALIGNED_STORE32(void* p, uint32 v)
{
  memcpy(p, &v, sizeof v);
}
