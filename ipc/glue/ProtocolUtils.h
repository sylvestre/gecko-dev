...
class IPCResult
{
 public:
  ...
  static IPCResult Fail(NotNull<IProtocol*> aActor,
                        const char* aWhere,
                        const char* aWhy = "");
                ...
                Endpoint<PFooChild>* aChildEndpoint)
{
  ...
  nsresult rv;
  if (NS_FAILED(rv = CreateTransport(
                    aParentDestPid, &parentTransport, &childTransport))) {
    AnnotateCrashReportWithErrno("IpcCreateEndpointsNsresult", int(rv));
    return rv;
  }

  *aParentEndpoint = Endpoint<PFooParent>(aPrivate,
                                          mozilla::ipc::Transport::MODE_SERVER,
                                          parentTransport,
                                          aParentDestPid,
                                          aChildDestPid);

  *aChildEndpoint = Endpoint<PFooChild>(aPrivate,
