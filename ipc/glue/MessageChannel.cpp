 ...
 public:
                 ...
                 bool aAnonymize) override
  {
    MOZ_COLLECT_REPORT(
        ...
        KIND_OTHER,
        UNITS_COUNT,
        MessageChannel::gUnresolvedPromises,
...
// before mListener.  But just to be safe, mListener is a weak pointer.
  ...
  for (auto& pair : mPendingPromises) {
    pair.second.mRejectFunction(
        pair.second.mPromise, PromiseRejectReason::ChannelClosed, __func__);
  }
...
bool
MessageChannel::Open(MessageChannel* aTargetChan,
...
MessageChannel::RejectPendingPromisesForActor(ActorIdType aActorId)
{
  ...
  while (itr != mPendingPromises.end()) {
    ...
    itr->second.mRejectFunction(
        promise, PromiseRejectReason::ActorDestroyed, __func__);
...
MessageChannel::OnMessageReceivedFromLink(Message&& aMsg)
{
  ...
  if (aMsg.is_sync() && aMsg.is_reply()) {
    IPC_LOG(
        "Received reply seqno=%d xid=%d", aMsg.seqno(), aMsg.transaction_id());
...
MessageChannel::ProcessPendingRequests(AutoEnterTransaction& aTransaction)
{
  ...
  IPC_LOG("ProcessPendingRequests for seqno=%d, xid=%d",
          aTransaction.SequenceNumber(),
          aTransaction.TransactionID());
  ...
  *aReply = Move(*reply);
  if (aReply->size() >= kMinTelemetryMessageSize) {
    Telemetry::Accumulate(
        Telemetry::IPC_REPLY_SIZE, nsDependentCString(msgName), aReply->size());
  }
...
MessageChannel::ProcessPendingRequest(Message&& aUrgent)
{
  ...
  IPC_LOG("Process pending: seqno=%d, xid=%d",
          aUrgent.seqno(),
...
NS_IMPL_ISUPPORTS_INHERITED(MessageChannel::MessageTask,
                            CancelableRunnable,
                            nsIRunnablePriority)
...
bool
MessageChannel::MessageTask::GetAffectedSchedulerGroups(
...
MessageChannel::DispatchMessage(Message&& aMsg)
{
  ...
  IPC_LOG(
      "DispatchMessage: seqno=%d, xid=%d", aMsg.seqno(), aMsg.transaction_id());
  ...
  if (reply && ChannelConnected == mChannelState) {
    IPC_LOG(
        "Sending reply seqno=%d, xid=%d", aMsg.seqno(), aMsg.transaction_id());
...
bool
MessageChannel::ShouldDeferInterruptMessage(const Message& aMsg,
                                            size_t aStackDepth)
{
  AssertWorkerThread();
  ...
  IPC_LOG(
      "race in %s: %s won", (mSide == ChildSide) ? "child" : "parent", winner);
...
void
MessageChannel::ReportConnectionError(const char* aChannelName,
                                      Message* aMsg) const
{
  AssertWorkerThread();
  mMonitor->AssertCurrentThreadOwns();
  ...
  if (aMsg) {
    char reason[512];
    SprintfLiteral(reason,
                   "(msgtype=0x%X,name=%s) %s",
                   aMsg->type(),
                   aMsg->name(),
                   errorMsg);
...
bool
MessageChannel::MaybeHandleError(Result code,
                                 const Message& aMsg,
...
void
MessageChannel::DebugAbort(
    const char* file, int line, const char* cond, const char* why, bool reply)
{
  printf_stderr(
...
MessageChannel::CancelTransaction(int transaction)
{
  ...
  for (MessageTask* p = mPending.getFirst(); p;) {
    ...
    if (msg.is_sync() && msg.nested_level() != IPC::Message::NOT_NESTED) {
      ...
      IPC_LOG("Removing msg from queue seqno=%d xid=%d",
              msg.seqno(),
              msg.transaction_id());
      foundSync = true;
      if (!IsAlwaysDeferred(msg)) {
