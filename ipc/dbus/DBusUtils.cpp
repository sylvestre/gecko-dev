...
DBusMessageRefPtr::DBusMessageRefPtr(DBusMessage* aMsg) : mMsg(aMsg)
{
  if (mMsg) {
...
void
DBusReplyHandler::Callback(DBusMessage* aReply, void* aData)
{
  MOZ_ASSERT(aData);
...
{
  if (msg) {
    CHROMIUM_LOG("%s: D-Bus error in %s: %s (%s)",
                 function,
    ...
    CHROMIUM_LOG(
        "%s: D-Bus error: %s (%s)", __FUNCTION__, (err)->name, (err)->message);
  }
  dbus_error_free((err));
}
...
dbus_returns_int32(DBusMessage* reply)
{
  ...
  if (!dbus_message_get_args(
          reply, &err, DBUS_TYPE_INT32, &ret, DBUS_TYPE_INVALID)) {
