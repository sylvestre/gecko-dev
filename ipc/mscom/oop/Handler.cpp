...
HRESULT
Handler::GetUnmarshalClass(REFIID riid,
                           void* pv,
                           DWORD dwDestContext,
                           void* pvDestContext,
                           DWORD mshlflags,
                           CLSID* pCid)
{
  return mUnmarshal->GetUnmarshalClass(
      MarshalAs(riid), pv, dwDestContext, pvDestContext, mshlflags, pCid);
}

HRESULT
Handler::GetMarshalSizeMax(REFIID riid,
                           void* pv,
                           DWORD dwDestContext,
                           void* pvDestContext,
                           DWORD mshlflags,
                           DWORD* pSize)
{
  ...
  *pSize = 0;
  ...
  // wrapping...
  hr = mUnmarshal->GetMarshalSizeMax(marshalAs,
                                     unkToMarshal.get(),
                                     dwDestContext,
                                     pvDestContext,
                                     mshlflags,
...
HRESULT
Handler::MarshalInterface(IStream* pStm,
                          REFIID riid,
                          void* pv,
                          DWORD dwDestContext,
                          void* pvDestContext,
                          DWORD mshlflags)
{
  ...
  hr = GetMarshalInterface(
      marshalAs,
      WrapNotNull<IUnknown*>(mInnerUnk),
  ...
  hr = mUnmarshal->MarshalInterface(pStm,
                                    marshalAs,
                                    unkToMarshal.get(),
                                    dwDestContext,
                                    pvDestContext,
...
Handler::Register(REFCLSID aClsid)
{
  ...
  DWORD size = GetModuleFileName(
      thisModule, absLibPath, mozilla::ArrayLength(absLibPath));
  ...
  result = RegSetValueEx(inprocHandlerKey,
                         L"",
                         0,
                         REG_EXPAND_SZ,
