...
static bool
GetLine(char* bufp, FILE* file, const char* prompt)
{
  char line[256];
  fputs(prompt, stdout);
  fflush(stdout);
...
static bool
Load(JSContext* cx, unsigned argc, JS::Value* vp)
{
  JS::CallArgs args = JS::CallArgsFromVp(argc, vp);
  ...
  for (unsigned i = 0; i < args.length(); i++) {
    ...
    if (!file) {
      ...
      JS_ReportErrorUTF8(
          cx, "cannot open file '%s' for reading", filename.ptr());
      return false;
    }
    JS::CompileOptions options(cx);
    options.setUTF8(true).setFileAndLine(filename.ptr(), 1);
...
static bool
Version(JSContext* cx, unsigned argc, JS::Value* vp)
{
  JS::CallArgs args = JS::CallArgsFromVp(argc, vp);
...
static bool
Quit(JSContext* cx, unsigned argc, JS::Value* vp)
{
...
static bool
DumpXPC(JSContext* cx, unsigned argc, JS::Value* vp)
{
...
static bool
GC(JSContext* cx, unsigned argc, JS::Value* vp)
{
  JS::CallArgs args = JS::CallArgsFromVp(argc, vp);
...
bool
XPCShellEnvironment::EvaluateString(const nsString& aString, nsString* aResult)
{
  ...
  JS::Rooted<JSScript*> script(cx);
  if (!JS_CompileUCScript(
          cx, aString.get(), aString.Length(), options, &script)) {
    return false;
  }
