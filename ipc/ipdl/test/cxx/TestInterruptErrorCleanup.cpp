...
void
DeleteSubprocess(Mutex* mutex, CondVar* cvar)
{
...
TestInterruptErrorCleanupParent::Main()
{
  ...
  MessageLoop::current()->PostTask(NewRunnableFunction(DeleteTheWorld));
...
void
TestInterruptErrorCleanupParent::ProcessingError(Result aCode,
                                                 const char* aReason)
{
