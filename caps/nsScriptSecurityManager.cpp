...
static nsresult
GetPrincipalDomainOrigin(nsIPrincipal* aPrincipal, nsACString& aOrigin)
{
  nsCOMPtr<nsIURI> uri;
...
inline void
SetPendingExceptionASCII(JSContext* cx, const char* aMsg)
{
  JS_ReportErrorASCII(cx, "%s", aMsg);
}

inline void
SetPendingException(JSContext* cx, const char16_t* aMsg)
{
...
{
  return GetChannelResultPrincipal(aChannel,
                                   aPrincipal,
...
static void
InheritAndSetCSPOnPrincipalIfNeeded(nsIChannel* aChannel,
                                    nsIPrincipal* aPrincipal)
{
                                           ...
                                           uint32_t aFlags)
{
  ...
  bool hasFlags = false;
  rv = NS_URIChainHasFlags(
      aTargetBaseURI, nsIProtocolHandler::URI_IS_UI_RESOURCE, &hasFlags);
  NS_ENSURE_SUCCESS(rv, rv);
  if (hasFlags) {
  ...
  rv = NS_URIChainHasFlags(
      aTargetURI, nsIProtocolHandler::URI_IS_LOCAL_FILE, &hasFlags);
  NS_ENSURE_SUCCESS(rv, rv);
  if (hasFlags) {
  ...
  if (!hasFlags && !hasSubsumersFlag) {
    ...
    rv = sStrBundle->FormatStringFromName("ProtocolFlagError",
                                          formatStrings,
...
nsresult
nsScriptSecurityManager::ReportError(JSContext* cx,
                                     ...
                                     nsIURI* aTarget)
{
  ...
  rv = sStrBundle->FormatStringFromName(
      aMessageTag, formatStrings, ArrayLength(formatStrings), message);
  NS_ENSURE_SUCCESS(rv, rv);
    ...
    nsIPrincipal* aPrincipal, const nsACString& aTargetURIStr, uint32_t aFlags)
{
  ...
  rv = NS_NewURI(
      getter_AddRefs(target), aTargetURIStr, nullptr, nullptr, sIOService);
  NS_ENSURE_SUCCESS(rv, rv);
  ...
  for (uint32_t i = 0; i < ArrayLength(flags); ++i) {
    rv = fixup->CreateFixupURI(
        aTargetURIStr, flags[i], nullptr, getter_AddRefs(target));
    NS_ENSURE_SUCCESS(rv, rv);
                                          ...
                                          nsIClassInfo* aClassInfo)
{
  ...
  if (originUTF16.IsEmpty()) {
    ...
    rv = sStrBundle->FormatStringFromName(
        "CreateWrapperDenied", formatStrings, 1, errorMsg);
  } else {
    const char16_t* formatStrings[] = {classInfoUTF16.get(), originUTF16.get()};
    rv = sStrBundle->FormatStringFromName(
        "CreateWrapperDeniedForOrigin", formatStrings, 2, errorMsg);
  }
  NS_ENSURE_SUCCESS(rv, rv);
...
NS_IMETHODIMP
nsScriptSecurityManager::Observe(nsISupports* aObject,
                                 const char* aTopic,
...
nsScriptSecurityManager::SystemPrincipalSingletonConstructor()
{
  ...
  if (gScriptSecMan) NS_ADDREF(sysprin = gScriptSecMan->mSystemPrincipal);
...
uint32_t
SkipPast(const nsCString& str, uint32_t base)
{
...
template<typename Predicate>
uint32_t
SkipUntil(const nsCString& str, uint32_t base)
{
  while (base < str.Length() && !Predicate::Test(str[base])) {
    ++base;
  }
