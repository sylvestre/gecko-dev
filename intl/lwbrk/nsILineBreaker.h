...
class nsILineBreaker : public nsISupports
{
 public:
  ...
  virtual int32_t Next(const char16_t* aText, uint32_t aLen, uint32_t aPos) = 0;

  virtual int32_t Prev(const char16_t* aText, uint32_t aLen, uint32_t aPos) = 0;
  ...
  // output array.
  virtual void GetJISx4051Breaks(const char16_t* aText,
                                 uint32_t aLength,
                                 uint8_t aWordBreak,
                                 uint8_t* aBreakBefore) = 0;
  virtual void GetJISx4051Breaks(const uint8_t* aText,
                                 uint32_t aLength,
                                 uint8_t aWordBreak,
                                 uint8_t* aBreakBefore) = 0;
};
