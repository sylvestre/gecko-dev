...
HTMLTableHeaderCellAccessible::NativeRole()
{
  ...
  static nsIContent::AttrValuesArray scopeValues[] = {&nsGkAtoms::col,
                                                      &nsGkAtoms::colgroup,
                                                      &nsGkAtoms::row,
                                                      &nsGkAtoms::rowgroup,
                                                      nullptr};
  int32_t valueIdx = mContent->FindAttrValueIn(
...
HTMLTableRowAccessible::GroupPosition()
{
  ...
  if (table &&
      nsCoreUtils::GetUIntAttr(
          table->GetContent(), nsGkAtoms::aria_rowcount, &count) &&
...
HTMLTableAccessible::NativeAttributes()
{
  ...
  if (IsProbablyLayoutTable()) {
    ...
    attributes->SetStringProperty(
        NS_LITERAL_CSTRING("layout-guess"), NS_LITERAL_STRING("true"), unused);
  }
...
void
HTMLTableAccessible::RowAndColIndicesAt(uint32_t aCellIdx,
                                        int32_t* aRowIdx,
                                        int32_t* aColIdx)
{
  nsTableWrapperFrame* tableFrame = do_QueryFrame(mContent->GetPrimaryFrame());
...
HTMLTableAccessible::Description(nsString& aDescription)
{
  ...
  if (caption) {
    ...
    if (captionContent) {
      ...
      nsTextEquivUtils::AppendTextEquivFromContent(
          this, captionContent, &captionText);

      if (!captionText.IsEmpty()) {  // summary isn't used as a name.
        mContent->GetAttr(kNameSpaceID_None, nsGkAtoms::summary, aDescription);
      }
    }
  }
...
#define RETURN_LAYOUT_ANSWER(isLayout, heuristic) \
  ...
  if (docAccessible) {
    ...
    if (docState &
        states::
            EDITABLE) {  // Need to see all elements while document is being edited
