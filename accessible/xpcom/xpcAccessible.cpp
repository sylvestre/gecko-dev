...
xpcAccessible::GetState(uint32_t* aState, uint32_t* aExtraState)
{
  ...
  if (IntlGeneric().IsNull())
    ...
    nsAccUtils::To32States(
        IntlGeneric().AsProxy()->State(), aState, aExtraState);
...
NS_IMETHODIMP
xpcAccessible::GetBounds(int32_t* aX,
                         int32_t* aY,
                         int32_t* aWidth,
                         int32_t* aHeight)
{
  ...
  *aRelations = nullptr;
  ...
  for (uint32_t idx = 0; idx < ArrayLength(relationTypes); idx++) {
    ...
    if (NS_SUCCEEDED(rv) && relation) {
      ...
      if (targets) relations->AppendElement(relation, false);
    }
  }
...
NS_IMETHODIMP
xpcAccessible::GetChildAtPoint(int32_t aX,
                               int32_t aY,
...
NS_IMETHODIMP
xpcAccessible::GetDeepestChildAtPoint(int32_t aX,
                                      int32_t aY,
                                      nsIAccessible** aAccessible)
{
  NS_ENSURE_ARG_POINTER(aAccessible);
