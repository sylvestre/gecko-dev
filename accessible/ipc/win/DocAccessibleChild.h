...
class DocAccessibleChild : public DocAccessibleChildBase
{
 public:
  ...
  bool SendStateChangeEvent(const uint64_t& aID,
  ...
  bool SendFocusEvent(const uint64_t& aID,
                      const LayoutDeviceIntRect& aCaretRect);
  bool SendTextChangeEvent(const uint64_t& aID,
                           const nsString& aStr,
  ...
  bool SendSelectionEvent(const uint64_t& aID,
 ...
 private:
  ...
  struct SerializedShow final : public DeferredEvent
  {
    SerializedShow(DocAccessibleChild* aTarget,
                   ShowEventData& aEventData,
                   bool aFromUser)
        : DeferredEvent(aTarget),
          mEventData(aEventData.ID(),
                     aEventData.Idx(),
  ...
  {
    SerializedHide(DocAccessibleChild* aTarget,
                   uint64_t aRootID,
                   bool aFromUser)
  ...
  {
    SerializedStateChange(DocAccessibleChild* aTarget,
                          uint64_t aID,
                          uint64_t aState,
                          bool aEnabled)
        : DeferredEvent(aTarget), mID(aID), mState(aState), mEnabled(aEnabled)
    {
    }
  ...
  {
    SerializedCaretMove(DocAccessibleChild* aTarget,
                        uint64_t aID,
                        const LayoutDeviceIntRect& aCaretRect,
                        int32_t aOffset)
        : DeferredEvent(aTarget),
  ...
  {
    SerializedFocus(DocAccessibleChild* aTarget,
                    uint64_t aID,
                    const LayoutDeviceIntRect& aCaretRect)
  ...
  {
    SerializedTextChange(DocAccessibleChild* aTarget,
                         uint64_t aID,
                         const nsString& aStr,
                         int32_t aStart,
                         uint32_t aLen,
    ...
    void Dispatch(DocAccessibleChild* aIPCDoc) override
    {
      Unused << aIPCDoc->SendTextChangeEvent(
          mID, mStr, mStart, mLen, mIsInsert, mFromUser, false);
    }
  ...
  {
    SerializedSelection(DocAccessibleChild* aTarget,
                        uint64_t aID,
                        uint64_t aWidgetID,
                        uint32_t aType)
        : DeferredEvent(aTarget), mID(aID), mWidgetID(aWidgetID), mType(aType)
    {
    }
