...
TextRange::Crop(Accessible* aContainer)
{
  ...
  if (boundaryPos == 0) {
    if (containerPos != 0) {
      aContainer->ToTextPoint(
          mEndContainer.StartAssignment(), &mEndOffset, false);
...
void
TextRange::FindText(const nsAString& aText,
...
void
TextRange::FindAttr(EAttr aAttr,
                    nsIVariant* aValue,
                    EDirection aDirection,
                    TextRange* aFoundRange) const
{
...
bool
TextRange::TextInternal(nsAString& aText,
                        Accessible* aCurrent,
                        uint32_t aStartIntlOffset) const
{
  ...
  if (aCurrent->IsTextLeaf()) {
    aCurrent->AppendTextTo(
        aText, aStartIntlOffset, endIntlOffset - aStartIntlOffset);
...
void
TextRange::MoveInternal(ETextUnit aUnit,
                        int32_t aCount,
...
Accessible*
TextRange::CommonParent(Accessible* aAcc1,
                        Accessible* aAcc2,
