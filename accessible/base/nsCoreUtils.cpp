                                ...
                                const nsAString& aPseudoElt)
{
  ...
  DispatchMouseEvent(
      eMouseDown, cnvdX, cnvdY, tcContent, tcFrame, presShell, rootWidget);

  DispatchMouseEvent(
      eMouseUp, cnvdX, cnvdY, tcContent, tcFrame, presShell, rootWidget);
}

void
nsCoreUtils::DispatchMouseEvent(EventMessage aMessage,
                                int32_t aX,
                                int32_t aY,
                                nsIContent* aContent,
                                nsIFrame* aFrame,
...
void
nsCoreUtils::DispatchTouchEvent(EventMessage aMessage,
                                int32_t aX,
                                int32_t aY,
                                nsIContent* aContent,
                                nsIFrame* aFrame,
                                ...
                                nsIWidget* aRootWidget)
{
  ...
  RefPtr<dom::Touch> t = new dom::Touch(
      -1, LayoutDeviceIntPoint(aX, aY), LayoutDeviceIntPoint(1, 1), 0.0f, 1.0f);
...
nsCoreUtils::GetAccessKeyFor(nsIContent* aContent)
{
  ...
  if (!aContent->HasAttr(kNameSpaceID_None, nsGkAtoms::accesskey)) return 0;

  nsIPresShell* presShell = aContent->OwnerDoc()->GetShell();
  if (!presShell) return 0;
...
nsCoreUtils::GetDOMNodeFromDOMPoint(nsINode* aNode, uint32_t aOffset)
{
  if (aNode && aNode->IsElement()) {
    ...
    if (aOffset != childCount) return aNode->GetChildAt(aOffset);
  }
...
{
  if (!aFrame) return NS_ERROR_FAILURE;
...
void
nsCoreUtils::GetLanguageFor(nsIContent* aContent,
                      ...
                      uint32_t aScrollType)
{
  ...
  aPresShell->ScrollContentIntoView(
      aContent, vertical, horizontal, nsIPresShell::SCROLL_OVERFLOW_HIDDEN);
}
