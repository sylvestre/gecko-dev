                       ...
                       const nsAString& aAttrValue)
{
  ...
  aAttributes->SetStringProperty(
      nsAtomCString(aAttrName), aAttrValue, oldValue);
}
                       ...
                       nsAtom* aAttrValue)
{
  nsAutoString oldValue;
  aAttributes->SetStringProperty(
      nsAtomCString(aAttrName), nsAtomString(aAttrValue), oldValue);
}
...
nsAccUtils::GetARIAOrDefaultLevel(Accessible* aAccessible)
{
  ...
  nsCoreUtils::GetUIntAttr(
      aAccessible->GetContent(), nsGkAtoms::aria_level, &level);
                                       ...
                                       dom::Element* aTopEl)
{
  ...
  while (ancestor) {
    ...
    if (relevant.IsEmpty() &&
        ...
        ancestor->GetAttr(
            kNameSpaceID_None, nsGkAtoms::aria_relevant, relevant))
    ...
    if (ancestor->AttrValueIs(kNameSpaceID_None,
                              nsGkAtoms::aria_atomic,
                              nsGkAtoms::_true,
                              eCaseMatters)) {
      SetAccAttr(
          aAttributes, nsGkAtoms::containerAtomic, NS_LITERAL_STRING("true"));
    }
...
nsAccUtils::HasDefinedARIAToken(nsIContent* aContent, nsAtom* aAtom)
{
  ...
  if (!aContent->HasAttr(kNameSpaceID_None, aAtom) ||
      aContent->AttrValueIs(
          kNameSpaceID_None, aAtom, nsGkAtoms::_empty, eCaseMatters) ||
      aContent->AttrValueIs(
          kNameSpaceID_None, aAtom, nsGkAtoms::_undefined, eCaseMatters)) {
    return false;
  }
  return true;
}
...
{
  return aAccessible->GetContent()->AttrValueIs(kNameSpaceID_None,
...
nsIntPoint
nsAccUtils::ConvertToScreenCoords(int32_t aX,
                                  int32_t aY,
...
void
nsAccUtils::ConvertScreenCoordsTo(int32_t* aX,
                                  int32_t* aY,
                                  uint32_t aCoordinateType,
                                  Accessible* aAccessible)
{
