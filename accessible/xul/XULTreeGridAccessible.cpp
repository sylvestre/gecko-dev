...
XULTreeGridAccessible::CreateTreeItemAccessible(int32_t aRow) const
{
  RefPtr<Accessible> accessible =
      new XULTreeGridRowAccessible(mContent,
                                   mDoc,
...
XULTreeGridRowAccessible::XULTreeGridRowAccessible(nsIContent* aContent,
                                                   DocAccessible* aDoc,
                                                   Accessible* aTreeAcc,
    ...
    : XULTreeItemAccessibleBase(
          aContent, aDoc, aTreeAcc, aTree, aTreeView, aRow),
...
Accessible*
XULTreeGridRowAccessible::ChildAtPoint(int32_t aX,
                                       int32_t aY,
...
NS_IMPL_CYCLE_COLLECTION_INHERITED(XULTreeGridCellAccessible,
                                   LeafAccessible,
                                   mTree,
                                   mColumn)
...
XULTreeGridCellAccessible::NativeAttributes()
{
  ...
  if (NS_SUCCEEDED(rv) && isCycler)
    nsAccUtils::SetAccAttr(
        attributes, nsGkAtoms::cycles, NS_LITERAL_STRING("true"));
