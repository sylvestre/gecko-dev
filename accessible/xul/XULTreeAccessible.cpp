...
XULTreeAccessible::XULTreeAccessible(nsIContent* aContent,
                                     DocAccessible* aDoc,
                                     nsTreeBodyFrame* aTreeFrame)
    : AccessibleWrap(aContent, aDoc), mAccessibleCache(kDefaultTreeCacheLength)
{
  mType = eXULTreeType;
...
Accessible*
XULTreeAccessible::ChildAtPoint(int32_t aX,
                                int32_t aY,
                                EWhichChildAtPoint aWhichChild)
{
  ...
  mTree->GetCellAt(
      clientX, clientY, &row, getter_AddRefs(column), childEltUnused);
...
void
XULTreeAccessible::TreeViewInvalidated(int32_t aStartRow,
                                       int32_t aEndRow,
                                       int32_t aStartCol,
                                       int32_t aEndCol)
{
...
XULTreeAccessible::CreateTreeItemAccessible(int32_t aRow) const
{
  RefPtr<Accessible> accessible =
      new XULTreeItemAccessible(mContent,
                                mDoc,
                                const_cast<XULTreeAccessible*>(this),
                                mTree,
                                mTreeView,
                                aRow);
...
XULTreeItemAccessibleBase::XULTreeItemAccessibleBase(nsIContent* aContent,
                                                     DocAccessible* aDoc,
                                                     Accessible* aParent,
...
XULTreeItemAccessibleBase::Bounds() const
{
  ...
  nsresult rv = mTree->GetCoordsForCellItem(
      mRow, column, EmptyString(), &x, &y, &width, &height);
...
XULTreeItemAccessible::XULTreeItemAccessible(nsIContent* aContent,
                                             DocAccessible* aDoc,
                                             Accessible* aParent,
                                             nsITreeBoxObject* aTree,
                                             nsITreeView* aTreeView,
