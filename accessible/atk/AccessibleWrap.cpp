...
mai_atk_object_get_type(void)
{
  ...
  if (!type) {
    ...
    type = g_type_register_static(
        ATK_TYPE_OBJECT, "MaiAtkObject", &tinfo, GTypeFlags(0));
...
GetMaiAtkType(uint16_t interfacesBits)
{
  ...
  type = g_type_register_static(
      MAI_TYPE_ATK_OBJECT, atkTypeName, &tinfo, GTypeFlags(0));
...
classInitCB(AtkObjectClass* aClass)
{
  ...
  mai_atk_object_signals[ACTIVATE] =
      g_signal_new("activate",
                   ...
                   G_TYPE_NONE,
                   0);
  mai_atk_object_signals[CREATE] = g_signal_new("create",
                                                MAI_TYPE_ATK_OBJECT,
                                                G_SIGNAL_RUN_LAST,
                                                0, /* default signal handler */
                                                nullptr,
  ...
  mai_atk_object_signals[DEACTIVATE] =
      g_signal_new("deactivate",
                   ...
                   G_TYPE_NONE,
                   0);
  mai_atk_object_signals[DESTROY] = g_signal_new("destroy",
                                                 MAI_TYPE_ATK_OBJECT,
                                                 G_SIGNAL_RUN_LAST,
                                                 0, /* default signal handler */
                                                 nullptr,
  ...
  mai_atk_object_signals[MINIMIZE] =
      g_signal_new("minimize",
                   ...
                   G_TYPE_NONE,
                   0);
  mai_atk_object_signals[RESIZE] = g_signal_new("resize",
                                                MAI_TYPE_ATK_OBJECT,
                                                G_SIGNAL_RUN_LAST,
                                                0, /* default signal handler */
                                                nullptr,
                                                ...
                                                G_TYPE_NONE,
                                                0);
  mai_atk_object_signals[RESTORE] = g_signal_new("restore",
                                                 MAI_TYPE_ATK_OBJECT,
                                                 G_SIGNAL_RUN_LAST,
                                                 0, /* default signal handler */
                                                 nullptr,
...
static void
UpdateAtkRelation(RelationType aType,
                  Accessible* aAcc,
  ...
  if (stateIndex >= 0) {
    ...
    if (gAtkStateMap[stateIndex].atkState != kNone) {
      ...
      atk_object_notify_state_change(
          &parent, gAtkStateMap[stateIndex].atkState, aEnabled);
    }
  }
}
...
void
MaiAtkObject::FireTextChangeEvent(const nsString& aStr,
                                  int32_t aStart,
                                  uint32_t aLen,
                                  bool aIsInsert,
                                  bool aFromUser)
{
  ...
  if (gAvailableAtkSignals == eNoNewAtkSignals) {
    ...
    g_signal_emit_by_name(
        this, signal_name, aStart, aLen, NS_ConvertUTF16toUTF8(aStr).get());
  }
}
...
void
MaiAtkObject::FireAtkShowHideEvent(AtkObject* aParent,
                                   bool aIsAdded,
                                   bool aFromUser)
{
