...
{
  CellISizeInfo(nscoord aMinCoord,
                nscoord aPrefCoord,
                float aPrefPercent,
             ...
             bool aIsCell)
{
  ...
  if (iSize.ConvertsToLength()) {
    ...
    if (aIsCell && c > minCoord && isQuirks &&
        aFrame->GetContent()->HasAttr(kNameSpaceID_None, nsGkAtoms::nowrap)) {
      minCoord = c;
    }
 ...
 */
void
BasicTableLayoutStrategy::ComputeColumnIntrinsicISizes(
    gfxContext* aRenderingContext)
{
  ...
  for (col = 0, col_end = cellMap->GetColCount(); col < col_end; ++col) {
    ...
    while ((cellFrame = columnIter.GetNextFrame(&row, &colSpan))) {
      ...
      colFrame->AddCoords(
          info.minCoord, info.prefCoord, info.hasSpecifiedISize);
  ...
  while ((item = spanningCells.GetNext(&colSpan))) {
    ...
    do {
      ...
      DistributeISizeToColumns(
          info.minCoord, col, colSpan, BTLS_MIN_ISIZE, info.hasSpecifiedISize);
      DistributeISizeToColumns(info.prefCoord,
                               col,
                               colSpan,
                               BTLS_PREF_ISIZE,
