...
bool
nsTableFrame::PageBreakAfter(nsIFrame* aSourceFrame, nsIFrame* aNextFrame)
{
...
void
nsTableFrame::SetInitialChildList(ChildListID aListID, nsFrameList& aChildList)
{
...
void
nsTableFrame::AdjustRowIndices(int32_t aRowIndex, int32_t aAdjustment)
{
                              ...
                              const nsFrameList::Slice& aColGroups)
{
  ...
  for (; !colGroups.AtEnd(); colGroups.Next()) {
    ...
    cgFrame->AddColsToTable(
        colIndex, false, colGroups.get()->PrincipalChildList());
...
void
nsTableFrame::InsertCol(nsTableColFrame& aColFrame, int32_t aColIndex)
{
...
nsTableFrame::AppendAnonymousColFrames(int32_t aNumColsToAdd)
{
  ...
  AppendAnonymousColFrames(
      colGroupFrame, aNumColsToAdd, eColAnonymousCell, true);
}
...
void
nsTableFrame::AppendCell(nsTableCellFrame& aCellFrame, int32_t aRowIndex)
{
  nsTableCellMap* cellMap = GetCellMap();
  if (cellMap) {
...
void
nsTableFrame::RemoveCell(nsTableCellFrame* aCellFrame, int32_t aRowIndex)
{
  nsTableCellMap* cellMap = GetCellMap();
  if (cellMap) {
    TableArea damageArea(0, 0, 0, 0);
                         ...
                         bool aConsiderSpans)
{
  ...
  if (cellMap) {
    ...
    cellMap->InsertRows(
        aRowGroupFrame, aRowFrames, aRowIndex, aConsiderSpans, damageArea);
    MatchCellMapToColCache(cellMap);
                         ...
                         bool aConsiderSpans)
{
  ...
  for (nsIFrame* kidFrame = aFirstFrame.FirstChild(); (kidFrame && !stopAsking);
    ...
    if (cellFrame) {
      stopTelling =
          tableFrame->CellChangedWidth(*cellFrame,
                                       cellFrame->GetPass1MaxElementWidth(),
                                       cellFrame->GetMaximumWidth(),
                                       true);
    }
  }
  ...
  if (cellMap) {
    ...
    cellMap->RemoveRows(
        firstRowIndex, aNumRowsToRemove, aConsiderSpans, damageArea);
    MatchCellMapToColCache(cellMap);
    if (IsBorderCollapse()) {
      AddBCDamageArea(damageArea);
    }
...
nsRect
nsDisplayTableItem::GetBounds(nsDisplayListBuilder* aBuilder, bool* aSnap) const
{
  *aSnap = false;
...
void
nsDisplayTableItem::ComputeInvalidationRegion(
...
nsDisplayTableBorderCollapse::BuildLayer(
    nsDisplayListBuilder* aBuilder,
    LayerManager* aManager,
...
bool
nsDisplayTableBorderCollapse::CreateWebRenderCommands(
    mozilla::wr::DisplayListBuilder& aBuilder,
    wr::IpcResourceUpdateQueue& aResources,
    const StackingContextHelper& aSc,
    mozilla::layers::WebRenderLayerManager* aManager,
...
LayerState
nsDisplayTableBorderCollapse::GetLayerState(
    nsDisplayListBuilder* aBuilder,
    LayerManager* aManager,
...
/* static */ void
nsTableFrame::GenericTraversal(nsDisplayListBuilder* aBuilder,
                               nsFrame* aFrame,
...
/* static */ void
nsTableFrame::DisplayGenericTablePart(
    nsDisplayListBuilder* aBuilder,
    nsFrame* aFrame,
    const nsDisplayListSet& aLists,
    ...
    if (!colIdx.IsEmpty()) {
      ...
      for (nsTableRowGroupFrame* rowGroup : rowGroups) {
        ...
        PaintRowGroupBackgroundByColIdx(
            rowGroup, aFrame, aBuilder, aLists, colIdx, offset);
      }
    }
...
void
nsTableFrame::SetColumnDimensions(nscoord aBSize,
                                  WritingMode aWM,
                                  ...
                                  const nsSize& aContainerSize)
{
  ...
  for (nsIFrame* colGroupFrame : mColGroups) {
    ...
    LogicalRect colGroupRect(aWM,
                             colGroupOrigin.I(aWM),
                             colGroupOrigin.B(aWM),
                             colGroupISize,
                             colBSize);
    ...
    for (nsIFrame* colFrame : columnList) {
      if (mozilla::StyleDisplay::TableColumn ==
        ...
        LogicalRect colRect(
            aWM, colOrigin.I(aWM), colOrigin.B(aWM), colISize, colBSize);
...
{
  LogicalSize result = nsContainerFrame::ComputeSize(aRenderingContext,
                                                     aWM,
                                                     aCBSize,
                                                     aAvailableISize,
                                                     aMargin,
                              ...
                              ComputeSizeFlags aFlags)
{
  ...
  return LogicalSize(aWM,
                     TableShrinkISizeToFit(aRenderingContext, cbBased),
                     ...
                     nsReflowStatus& aStatus)
{
  ...
  if (NS_SUBTREE_DIRTY(this) || aReflowInput.ShouldReflowAllKids() ||
    ...
    ReflowTable(
        aDesiredSize, aReflowInput, availBSize, lastChildReflowed, aStatus);
  ...
  LogicalMargin borderPadding = GetChildAreaOffset(wm, &aReflowInput);
  SetColumnDimensions(
      aDesiredSize.BSize(wm), wm, borderPadding, aDesiredSize.PhysicalSize());
  if (NeedToCollapse() &&
                          ...
                          nsReflowStatus& aStatus)
{
  ...
  ReflowChildren(
      reflowInput, aStatus, aLastChildReflowed, aDesiredSize.mOverflowAreas);
...
void
nsTableFrame::PushChildren(const RowGroupArray& aRowGroups, int32_t aPushFrom)
{
...
void
nsTableFrame::AppendFrames(ChildListID aListID, nsFrameList& aFrameList)
{
  ...
  for (uint32_t i = 0; i < ArrayLength(insertions); ++i) {
    ...
    if (!insertions[i].mList.IsEmpty()) {
      HomogenousInsertFrames(
          insertions[i].mID, aPrevFrame, insertions[i].mList);
    }
  }
}
...
void
nsTableFrame::DoRemoveFrame(ChildListID aListID, nsIFrame* aOldFrame)
{
  if (aListID == kColGroupList) {
    ...
    if (cellMap) {
      ...
      cellMap->RebuildConsideringCells(
          nullptr, nullptr, 0, 0, false, damageArea);
...
void
nsTableFrame::RemoveFrame(ChildListID aListID, nsIFrame* aOldFrame)
{
  NS_ASSERTION(
  ...
  if (!GetPrevInFlow() && !HaveReflowedColGroups()) {
    ...
    for (nsIFrame* kidFrame : mColGroups) {
      if (NS_SUBTREE_DIRTY(kidFrame)) {
        ...
        ReflowChild(
            kidFrame, presContext, kidMet, kidReflowInput, 0, 0, 0, cgStatus);
...
static void
ResizeCells(nsTableFrame& aTableFrame)
{
  nsTableFrame::RowGroupArray rowGroups;
  ...
  for (rgIdx = 0; rgIdx < rowGroups.Length(); rgIdx++) {
    ...
    if (!rgFrame->HasStyleBSize()) {
      ...
      while (rowFrame) {
        ...
        LogicalRect rowNormalRect(
            wm, rowFrame->GetNormalRect(), dummyContainerSize);
      ...
      if (amountUsed > 0) {
        ...
        rgFrame->SetSize(wm,
                         LogicalSize(wm,
                                     rgNormalRect.ISize(wm),
                                     rgNormalRect.BSize(wm) + amountUsedByRG));

        nsTableFrame::InvalidateTableFrame(
            rgFrame, origRgNormalRect, origRgVisualOverflow, false);
      }
    ...
    if (!firstUnStyledRG || !rgFrame->HasStyleBSize() || !eligibleRows) {
      for (nsTableRowFrame* rowFrame = rgFrame->GetFirstRow(); rowFrame;
        ...
        if (!firstUnStyledRow || !rowFrame->HasStyleBSize() || !eligibleRows) {
          ...
          nsTableFrame::InvalidateTableFrame(
              rowFrame, origRowRect, rowVisualOverflow, false);
        } else {
          if (amountUsed > 0 && bOriginRow != rowNormalRect.BStart(wm)) {
            rowFrame->InvalidateFrameSubtree();
            rowFrame->MovePositionBy(
      ...
      if (amountUsed > 0) {
        ...
        nsTableFrame::InvalidateTableFrame(
            rgFrame, origRgNormalRect, rgVisualOverflow, false);
      }
...
nscoord
nsTableFrame::GetColSpacing(int32_t aStartColIndex, int32_t aEndColIndex)
{
...
nscoord
nsTableFrame::GetRowSpacing(int32_t aStartRowIndex, int32_t aEndRowIndex)
{
...
nsTableFrame::GetLogicalBaseline(WritingMode aWM) const
{
  ...
  if (!GetNaturalBaselineBOffset(
          aWM, BaselineSharingGroup::eFirst, &baseline)) {
    baseline = BSize(aWM);
  }
  return baseline;
}
...
nsTableFrame::DumpRowGroup(nsIFrame* aKidFrame)
{
  ...
  for (nsIFrame* cFrame : aKidFrame->PrincipalChildList()) {
    ...
    if (rowFrame) {
      printf(
          "row(%d)=%p ", rowFrame->GetRowIndex(), static_cast<void*>(rowFrame));
...
void
nsTableFrame::Dump(bool aDumpRows, bool aDumpCols, bool aDumpCellMap)
{
...
void
BCCellBorder::Reset(uint32_t aRowIndex, uint32_t aRowSpan)
{
               ...
               bool aAja)
{
  ...
  if (aColGroupFrame) {
    GetColorAndStyle(aColGroupFrame,
                     aTableWM,
                     aSide,
                     &tempBorder.style,
                     &tempBorder.color,
  ...
  if (aRowGroupFrame) {
    GetColorAndStyle(aRowGroupFrame,
                     aTableWM,
                     aSide,
                     &tempBorder.style,
                     &tempBorder.color,
...
static bool
Perpendicular(mozilla::LogicalSide aSide1, mozilla::LogicalSide aSide2)
{
...
struct BCCornerInfo
{
  ...
  uint16_t
      subWidth;  // pixel width of the largest border intersecting the border perpendicular
                 // to ownerSide
  uint32_t
  ...
  uint32_t
      hasDashDot : 1;  // does a dashed, dotted segment enter the corner, they cannot be beveled
  uint32_t numSegs : 3;  // number of segments entering corner
  uint32_t
      bevel : 1;  // is the corner beveled (uses the above two fields together with subWidth)
  uint32_t unused : 1;
};

void
BCCornerInfo::Set(mozilla::LogicalSide aSide, BCCellBorder aBorder)
{
...
void
BCCornerInfo::Update(mozilla::LogicalSide aSide, BCCellBorder aBorder)
{
  ...
  if (0xFF ==
    ...
    tempBorder = CompareBorders(
        CELL_CORNER, oldBorder, aBorder, isInline, &existingWins);
    ...
    if (existingWins) {  // existing corner is dominant
      if (::Perpendicular(LogicalSide(ownerSide), aSide)) {
        ...
        tempBorder = CompareBorders(
            CELL_CORNER, subBorder, aBorder, isInline, &firstWins);
...
// segment.
static bool
SetBorder(const BCCellBorder& aNewBorder, BCCellBorder& aBorder)
{
...
BCMapCellInfo::SetColumnBStartIEndContBCBorder()
{
  ...
  if (mNumTableCols == GetCellEndColIndex() + 1) {
    ...
    currentBorder = CompareBorders(nullptr,
                                   mCurrentColGroupFrame,
                                   mCurrentColFrame,
                                   nullptr,
                                   nullptr,
                                                ...
                                                nsTableRowFrame* aNextRow)
{
  ...
  adjacentBorder = CompareBorders(nullptr,
                                  nullptr,
                                  nullptr,
                                  aNextRowGroup,
                                  aNextRow,
  ...
  for (iter.First(info); !iter.mAtEnd; iter.Next(info)) {
    ...
    if (0 == info.mRowIndex) {
      ...
      for (int32_t colIdx = info.mColIndex; colIdx <= info.GetCellEndColIndex();
        ...
        tableCellMap->SetBCBorderEdge(eLogicalSideBStart,
                                      *iter.mCellMap,
                                      0,
                                      0,
                                      colIdx,
    ...
    if (0 == info.mColIndex) {
      ...
      for (int32_t rowB = info.mRowIndex; rowB <= info.GetCellEndRowIndex();
        ...
        // store the border segment in the cell map
        tableCellMap->SetBCBorderEdge(eLogicalSideIStart,
                                      *iter.mCellMap,
                                      iter.mRowGroupStart,
                                      rowB,
                                      info.mColIndex,
    ...
    if (info.mNumTableCols == info.GetCellEndColIndex() + 1) {
      ...
      for (int32_t rowB = info.mRowIndex; rowB <= info.GetCellEndRowIndex();
        ...
        tableCellMap->SetBCBorderCorner(eLogicalCornerBStartIEnd,
                                        *iter.mCellMap,
                                        iter.mRowGroupStart,
                                        rowB,
        ...
        tableCellMap->SetBCBorderCorner(eLogicalCornerBEndIEnd,
                                        *iter.mCellMap,
                                        iter.mRowGroupStart,
                                        rowB,
                                        info.GetCellEndColIndex(),
        ...
        tableCellMap->SetBCBorderEdge(eLogicalSideIEnd,
                                      *iter.mCellMap,
                                      iter.mRowGroupStart,
                                      rowB,
                                      info.GetCellEndColIndex(),
      ...
      for (int32_t rowB = info.mRowIndex; rowB <= info.GetCellEndRowIndex();
        ...
        currentBorder = CompareBorders(
            !CELL_CORNER, currentBorder, adjacentBorder, !INLINE_DIR);
    ...
    for (int32_t colIdx = info.mColIndex + 1;
         colIdx <= info.GetCellEndColIndex();
         colIdx++) {
    ...
    if (info.mNumTableRows == info.GetCellEndRowIndex() + 1) {
      ...
      for (int32_t colIdx = info.mColIndex; colIdx <= info.GetCellEndColIndex();
        ...
        // store the border segment in the cell map and update cellBorders
        tableCellMap->SetBCBorderEdge(eLogicalSideBEnd,
                                      *iter.mCellMap,
                                      iter.mRowGroupStart,
                                      info.GetCellEndRowIndex(),
                                      colIdx,
    ...
    if ((info.mNumTableCols != info.GetCellEndColIndex() + 1) &&
      ...
      if (!IsBlock(LogicalSide(corner.ownerSide))) {
        ...
        if ((thisBorder.color == nextBorder.color) &&
          ...
          // new segment
          if (iter.mCellMap) {
            tableCellMap->ResetBStartStart(eLogicalSideBEnd,
                                           *iter.mCellMap,
                                           info.GetCellEndRowIndex(),
 ...
 */
void
BCPaintBorderIterator::SetNewData(int32_t aY, int32_t aX)
{
                     ...
                     BCPixelSize aInlineSegBSize)
{
  ...
  nsCSSRendering::DrawTableBorderSegment(aDrawTarget,
                                         param->mBorderStyle,
                                         param->mBorderColor,
                                         param->mBGColor,
                                         param->mBorderRect,
                                       ...
                                       const nsPoint& aOffset)
{
  ...
  wr::BorderWidths borderWidths =
      wr::ToBorderWidths(transformedRect.size.width,
                              ...
                              BCPixelSize aIStartSegISize)
{
  ...
  nsPresContext* presContext = aIter.mTable->PresContext();
  mEndOffset = CalcHorCornerOffset(
      presContext, ownerSide, cornerSubWidth, verWidth, false, mIsIEndBevel);
  mLength += mEndOffset;
  mIEndBevelOffset =
...
void
BCInlineDirSeg::CreateWebRenderCommands(
    ...
    const nsPoint& aPt)
{
  ...
  wr::BorderWidths borderWidths =
      wr::ToBorderWidths(transformedRect.size.height,
 ...
 */
void
BCPaintBorderIterator::AccumulateOrDoActionInlineDirSegment(
 ...
 */
void
BCPaintBorderIterator::AccumulateOrDoActionBlockDirSegment(
    BCPaintBorderAction& aAction)
{
  BCBorderOwner borderOwner = eCellOwner;
  BCBorderOwner ignoreBorderOwner;
  ...
  if (!IsDamageAreaBStartMost() &&
    ...
    if (blockDirSeg.mLength > 0) {
      ...
      if (blockDirSeg.mWidth > 0) {
        if (aAction.mMode == BCPaintBorderAction::Mode::PAINT) {
          blockDirSeg.Paint(
              *this, aAction.mPaintData.mDrawTarget, inlineSegBSize);
        } else {
          MOZ_ASSERT(aAction.mMode ==
                     BCPaintBorderAction::Mode::CREATE_WEBRENDER_COMMANDS);
...
void
nsTableFrame::IterateBCBorders(BCPaintBorderAction& aAction,
...
void
nsTableFrame::CreateWebRenderCommandsForBCBorders(
    wr::DisplayListBuilder& aBuilder,
