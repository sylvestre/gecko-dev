...
void
nsDisplayRangeFocusRing::Paint(nsDisplayListBuilder* aBuilder, gfxContext* aCtx)
{
  bool unused;
  nsStyleContext* styleContext =
      static_cast<nsRangeFrame*>(mFrame)->mOuterFocusStyle;
  ...
  DrawResult result = nsCSSRendering::PaintBorder(mFrame->PresContext(),
                                                  *aCtx,
                                                  mFrame,
                                                  mVisibleRect,
                                                  GetBounds(aBuilder, &unused),
                                                  styleContext,
                                                  flags);
                               ...
                               const nsDisplayListSet& aLists)
{
  ...
  aLists.Content()->AppendNewToTop(new (aBuilder)
                                       nsDisplayRangeFocusRing(aBuilder, this));
}
                                     ...
                                     const ReflowInput& aReflowInput)
{
  ...
  if (trackFrame) {  // display:none?
    ...
    ReflowInput trackReflowInput(
        aPresContext, aReflowInput, trackFrame, availSize);
  ...
  if (thumbFrame) {  // display:none?
    ...
    ReflowInput thumbReflowInput(
        aPresContext, aReflowInput, thumbFrame, availSize);
    ...
    MOZ_ASSERT(
        ...
        "We gave our child unconstrained height, so it should be complete");
    FinishReflowChild(
        thumbFrame, aPresContext, thumbDesiredSize, &thumbReflowInput, 0, 0, 0);
  ...
  if (rangeProgressFrame) {  // display:none?
    ...
    LogicalSize availSize = aReflowInput.ComputedSize(wm);
    availSize.BSize(wm) = NS_UNCONSTRAINEDSIZE;
    ReflowInput progressReflowInput(
        aPresContext, aReflowInput, rangeProgressFrame, availSize);
    ...
    ReflowChild(rangeProgressFrame,
                aPresContext,
                progressDesiredSize,
                progressReflowInput,
                0,
    ...
    MOZ_ASSERT(
        frameStatus.IsFullyComplete(),
        "We gave our child unconstrained height, so it should be complete");
    FinishReflowChild(rangeProgressFrame,
                      aPresContext,
                      progressDesiredSize,
                      &progressReflowInput,
                      0,
                                         ...
                                         const nsSize& aRangeSize)
{
  ...
  nsRect progRect(borderAndPadding.left,
