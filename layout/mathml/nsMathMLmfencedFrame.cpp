...
nsMathMLmfencedFrame::~nsMathMLmfencedFrame() { RemoveFencesAndSeparators(); }
    ...
    if (0 < sepCount) {
      ...
      for (int32_t i = 0; i < sepCount; i++) {
        ...
        ResolveMathMLCharStyle(
            aPresContext, mContent, mStyleContext, &mSeparatorsChar[i]);
      }
                             ...
                             nsReflowStatus& aStatus)
{
  ...
  GetAxisHeight(
      aReflowInput.mRenderingContext->GetDrawTarget(), fm, axisHeight);
  GetEmHeight(fm, em);
  ...
  GetPreferredStretchSize(
      drawTarget, STRETCH_CONSIDER_EMBELLISHMENTS, stretchDir, containerSize);
  ...
  ApplyUnstretchedMetrics(
      this, drawTarget, fontSizeInflation, mOpenChar, containerSize, isRTL);
  for (i = 0; i < mSeparatorsCount; i++) {
    ApplyUnstretchedMetrics(this,
                            drawTarget,
                            fontSizeInflation,
                            &mSeparatorsChar[i],
                            containerSize,
  ...
  ApplyUnstretchedMetrics(
      this, drawTarget, fontSizeInflation, mCloseChar, containerSize, isRTL);
                                 ...
                                 bool aRTL)
{
  if (aMathMLChar && 0 < aMathMLChar->Length()) {
    ...
    nsBoundingMetrics charSize;
    nsresult res = aMathMLChar->Stretch(this,
                                        aDrawTarget,
                                        aFontSizeInflation,
...
/* virtual */ void
nsMathMLmfencedFrame::GetIntrinsicISizeMetrics(gfxContext* aRenderingContext,
                                               ReflowOutput& aDesiredSize)
{
  nscoord width = 0;
