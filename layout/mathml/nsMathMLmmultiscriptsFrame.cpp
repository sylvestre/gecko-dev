...
nsMathMLmmultiscriptsFrame::TransmitAutomaticData()
{
  ...
  for (int32_t i = subScriptFrames.Length() - 1; i >= 0; i--) {
    ...
    PropagatePresentationDataFor(
        childFrame, NS_MATHML_COMPRESSED, NS_MATHML_COMPRESSED);
  }
                                  ...
                                  ReflowOutput& aDesiredSize)
{
  ...
  return PlaceMultiScript(PresContext(),
                          aDrawTarget,
                          aPlaceOrigin,
                          aDesiredSize,
                          this,
    ...
    if (childFrame->GetContent()->IsMathMLElement(nsGkAtoms::mprescripts_)) {
      ...
      if (isSubScript) {
        ...
        GetReflowAndBoundingMetricsFor(
            subScriptFrame, subScriptSize, bmSubScript);
        if (!mathFont) {
        ...
        GetReflowAndBoundingMetricsFor(
            supScriptFrame, supScriptSize, bmSupScript);
        if (!mathFont) {
    ...
    do {
      if (!childFrame) {  // end of prescripts,
        ...
        FinishReflowChild(
            ...
            baseSize,
            nullptr,
            aFrame->MirrorIfRTL(aDesiredSize.Width(), baseSize.Width(), dx),
            dy,
            0);
        dx += bmBase.width;
      } else if (prescriptsFrame == childFrame) {
        // Clear reflow flags of prescripts frame.
        prescriptsFrame->DidReflow(
            aPresContext, nullptr, nsDidReflowStatus::FINISHED);
      } else {
        ...
        if (0 == count) {
          ...
          if (subScriptFrame) {
            ...
            FinishReflowChild(
                ...
                nullptr,
                aFrame->MirrorIfRTL(
                    aDesiredSize.Width(), subScriptSize.Width(), x),
                dy,
                0);
          }
          ...
          if (supScriptFrame) {
            ...
            FinishReflowChild(
                ...
                nullptr,
                aFrame->MirrorIfRTL(
                    aDesiredSize.Width(), supScriptSize.Width(), x),
                dy,
                0);
          }
