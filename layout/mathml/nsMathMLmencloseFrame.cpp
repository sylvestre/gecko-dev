 ...
 */
nsresult
nsMathMLmencloseFrame::AddNotation(const nsAString& aNotation)
{
  nsresult rv;
 ...
 */
void
nsMathMLmencloseFrame::InitNotations()
{
...
nsMathMLmencloseFrame::TransmitAutomaticData()
{
  if (IsToDraw(NOTATION_RADICAL)) {
    ...
    UpdatePresentationDataFromChildAt(
        0, -1, NS_MATHML_COMPRESSED, NS_MATHML_COMPRESSED);
  }
                                        ...
                                        const nsDisplayListSet& aLists)
{
  ...
  if (IsToDraw(NOTATION_BOTTOM)) {
    nsRect rect(0,
                mencloseRect.height - mRuleThickness,
                mencloseRect.width,
                mRuleThickness);
  ...
  if (IsToDraw(NOTATION_RIGHT)) {
    nsRect rect(mencloseRect.width - mRuleThickness,
                0,
                mRuleThickness,
                mencloseRect.height);
  ...
  if (IsToDraw(NOTATION_CIRCLE)) {
    DisplayNotation(
        aBuilder, this, mencloseRect, aLists, mRuleThickness, NOTATION_CIRCLE);
  }
  ...
  if (IsToDraw(NOTATION_HORIZONTALSTRIKE)) {
    nsRect rect(0,
                mencloseRect.height / 2 - mRuleThickness / 2,
                mencloseRect.width,
                mRuleThickness);
  ...
  if (IsToDraw(NOTATION_VERTICALSTRIKE)) {
    nsRect rect(mencloseRect.width / 2 - mRuleThickness / 2,
                0,
                mRuleThickness,
                mencloseRect.height);
                                     ...
                                     bool aWidthOnly)
{
  ...
  if (IsToDraw(NOTATION_LONGDIV)) {
    if (aWidthOnly) {
      ...
      mMathMLChar[mLongDivCharIndex].Stretch(this,
                                             aDrawTarget,
                                             fontSizeInflation,
  ...
  if (IsToDraw(NOTATION_RADICAL)) {
    ...
    if (aWidthOnly) {
      ...
      *dx_leading = std::max(*dx_leading, radical_width);
      ...
      mMathMLChar[mRadicalCharIndex].Stretch(this,
                                             aDrawTarget,
                                             fontSizeInflation,
                                             NS_STRETCH_DIRECTION_VERTICAL,
                                             contSize,
  ...
  if (aPlaceOrigin) {
    ...
    if (IsToDraw(NOTATION_LONGDIV))
      mMathMLChar[mLongDivCharIndex].SetRect(
          nsRect(dx_left - bmLongdivChar.width,
                 aDesiredSize.BlockStartAscent() - longdivAscent,
                 bmLongdivChar.width,
    ...
    if (IsToDraw(NOTATION_RADICAL)) {
      ...
      mMathMLChar[mRadicalCharIndex].SetRect(
          nsRect(dx,
                 aDesiredSize.BlockStartAscent() - radicalAscent,
                 bmRadicalChar.width,
...
void
nsDisplayNotation::Paint(nsDisplayListBuilder* aBuilder, gfxContext* aCtx)
{
  ...
  switch (mType) {
    ...
    case NOTATION_UPDIAGONALSTRIKE: {
      aDrawTarget.StrokeLine(
          rect.BottomLeft(), rect.TopRight(), color, strokeOptions);
      return;
    }
    case NOTATION_DOWNDIAGONALSTRIKE: {
      aDrawTarget.StrokeLine(
          rect.TopLeft(), rect.BottomRight(), color, strokeOptions);
      return;
    }
                                       ...
                                       nsMencloseNotation aType)
{
  ...
  aLists.Content()->AppendNewToTop(new (aBuilder) nsDisplayNotation(
