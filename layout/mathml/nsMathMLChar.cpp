...
class nsPropertiesTable final : public nsGlyphTable
{
 public:
                          ...
                          bool aVertical) override
  {
    return (
        ElementAt(
            aDrawTarget, aAppUnitsPerDevPixel, aFontGroup, aChar, aVertical, 0)
            .Exists() ||
        ElementAt(
            aDrawTarget, aAppUnitsPerDevPixel, aFontGroup, aChar, aVertical, 1)
            .Exists() ||
        ElementAt(
            aDrawTarget, aAppUnitsPerDevPixel, aFontGroup, aChar, aVertical, 2)
            .Exists() ||
        ElementAt(
            aDrawTarget, aAppUnitsPerDevPixel, aFontGroup, aChar, aVertical, 3)
            .Exists());
  }
                               ...
                               const nsGlyphCode& aGlyph)
{
  NS_ASSERTION(!aGlyph.IsGlyphID(),
               "nsPropertiesTable can only access glyphs by code point");
  return aFontGroup->MakeTextRun(aGlyph.code,
                                 aGlyph.Length(),
                                 aDrawTarget,
                                 aAppUnitsPerDevPixel,
                                 gfx::ShapedTextFlags(),
                             ...
                             const nsGlyphCode& aGlyph)
{
  ...
  RefPtr<gfxTextRun> textRun = gfxTextRun::Create(&params,
                                                  1,
                                                  aFontGroup,
                                                  gfx::ShapedTextFlags(),
                                                  nsTextFrameUtils::Flags());
...
class nsMathMLChar::StretchEnumContext
{
 public:
  ...
  static bool EnumCallback(const FontFamilyName& aFamily,
                           bool aGeneric,
                           void* aData);
                                              ...
                                              const FontFamilyList& aFamilyList)
{
  ...
  if (largeopOnly) {
    ...
    ch = aGlyphTable->BigOf(
        mDrawTarget, oneDevPixel, *aFontGroup, uchar, isVertical, 0);
    if (ch.IsGlyphID()) {
...
#endif
  while ((ch = aGlyphTable->BigOf(
              mDrawTarget, oneDevPixel, *aFontGroup, uchar, isVertical, size))
             .Exists()) {
    if (!mChar->SetFontFamily(
            mPresContext, aGlyphTable, ch, aFamilyList, font, aFontGroup)) {
                                           ...
                                           const FontFamilyList& aFamilyList)
{
  ...
  char16_t uchar = mChar->mData[0];
  bool maxWidth = (NS_STRETCH_MAXWIDTH & mStretchHint) != 0;
  if (!aGlyphTable->HasPartsOf(
          mDrawTarget, oneDevPixel, *aFontGroup, uchar, isVertical))
    return false;  // to next table

  for (int32_t i = 0; i < 4; i++) {
    nsGlyphCode ch = aGlyphTable->ElementAt(
        mDrawTarget, oneDevPixel, *aFontGroup, uchar, isVertical, i);
    chdata[i] = ch;
    if (ch.Exists()) {
      if (!mChar->SetFontFamily(
              mPresContext, aGlyphTable, ch, aFamilyList, font, aFontGroup))
                                               ...
                                               void* aData)
{
  ...
  if (!aGeneric &&
      !context->mChar->SetFontFamily(
          context->mPresContext, nullptr, kNullGlyph, family, font, &fontGroup))
...
static void
InsertMathFallbacks(FontFamilyList& aFamilyList, nsTArray<nsString>& aFallbacks)
{
                              ...
                              bool aMaxSizeIsAbsolute)
{
  ...
  if (!done) {  // normal case
    ...
    printf(
        ...
        NS_ConvertUTF16toUTF8(fontlistStr).get(),
        mData[0],
        mData[0] & 0x00FF);
#endif
    StretchEnumContext enumData(this,
                                presContext,
                                aDrawTarget,
                                aFontSizeInflation,
                                aStretchDirection,
                      ...
                      bool aRTL)
{
  ...
  mScaleY = mScaleX = 1.0;
  mDirection = aStretchDirection;
  nsresult rv = StretchInternal(aForFrame,
                                aDrawTarget,
                                aFontSizeInflation,
                                mDirection,
                                aContainerSize,
...
void
nsDisplayMathMLSelectionRect::Paint(nsDisplayListBuilder* aBuilder,
                                    gfxContext* aCtx)
{
 ...
 public:
                           ...
                           bool* aSnap) const override
  {
    *aSnap = false;
    ...
    nsRect temp(offset.x + bm.leftBearing,
                offset.y,
                bm.rightBearing - bm.leftBearing,
...
void
nsDisplayMathMLCharDebug::Paint(nsDisplayListBuilder* aBuilder,
                                gfxContext* aCtx)
{
  // for visual debug
  Sides skipSides;
  ...
  Unused << nsCSSRendering::PaintBorder(presContext,
                                        *aCtx,
                                        mFrame,
                                        mVisibleRect,
                                        rect,
  ...
  nsCSSRendering::PaintOutline(
      presContext, *aCtx, mFrame, mVisibleRect, rect, styleContext);
}
#endif
  ...
  if (aSelectedRect && !aSelectedRect->IsEmpty()) {
    ...
    if (styleContext != parentContext &&
      ...
      nsDisplayBackgroundImage::AppendBackgroundItemsToTop(
          ...
          /* aAllowWillPaintBorderOptimization */ true,
          styleContext);
    }
      //else
    ...
    aLists.BorderBackground()->AppendToTop(
        new (aBuilder) nsDisplayMathMLCharDebug(aBuilder, aForFrame, mRect));
#endif
  }
  aLists.Content()->AppendNewToTop(new (aBuilder) nsDisplayMathMLCharForeground(
      aBuilder,
      aForFrame,
      this,
      aIndex,
                              ...
                              nsRect& r)
{
  ...
  if (mMirrored) {
    ...
    aThebesContext->SetMatrix(aThebesContext->CurrentMatrix()
                              ...
                              bool aIsSelected)
{
  ...
  nsRect r = mRect + aPt;
  ApplyTransforms(
      &aRenderingContext, aForFrame->PresContext()->AppUnitsPerDevPixel(), r);
...
class AutoPushClipRect
{
 ...
 public:
  AutoPushClipRect(gfxContext* aThebesContext,
                   int32_t aAppUnitsPerGfxUnit,
                   const nsRect& aRect)
...
static nsPoint
SnapToDevPixels(const gfxContext* aThebesContext,
                int32_t aAppUnitsPerGfxUnit,
                const nsPoint& aPt)
{
                                ...
                                nscolor aColor)
{
  ...
  if (!mGlyphs[3]) {  // null glue : draw a rule
    ...
    while (last <= 2) {
      ...
      // paint the rule between the parts
      nsRect rule(
          end[first], dy - ascent, start[last] - end[first], ascent + descent);
      PaintRule(aDrawTarget, oneDevPixel, rule, aColor);
      first = last;
      last++;
    }
