...
static const MathVarMapping gArabicTailedMapTable[] = {{0x62C, 0x1EE42},
                                                       {0x62D, 0x1EE47},
                                                       {0x62E, 0x1EE57},
                                                       {0x633, 0x1EE4E},
                                                       {0x634, 0x1EE54},
...
static uint32_t
MathvarMappingSearch(uint32_t aKey,
                     const MathVarMapping* aTable,
                     uint32_t aNumElements)
{
  size_t index;
  if (BinarySearch(
          MathVarMappingWrapper(aTable), 0, aNumElements, aKey, &index)) {
...
MathVariant(uint32_t aCh, uint8_t aMathVar)
{
  ...
  if (varType == kIsArabic) {
    ...
    newChar = MathvarMappingSearch(tempChar,
                                     ...
                                     gfxMissingFontRecorder* aMFR)
{
  ...
  if (mInnerTransformingTextRunFactory) {
    ...
    cachedChild = newFontGroup->MakeTextRun(convertedString.BeginReading(),
                                            convertedString.Length(),
                                            &innerParams,
                                            flags,
                                            nsTextFrameUtils::Flags(),
