...
void
nsInlineFrame::InvalidateFrameWithRect(const nsRect& aRect,
...
nsInlineFrame::PeekOffsetCharacter(bool aForward,
                                   int32_t* aOffset,
                                ...
                                int32_t aModType)
{
  ...
  if (nsSVGUtils::IsInSVGTextSubtree(this)) {
    ...
    f->HandleAttributeChangeInDescendant(
        mContent->AsElement(), aNameSpaceID, aAttribute);
  }
                            ...
                            nsReflowStatus& aStatus)
{
  ...
  lineLayout->BeginSpan(
      this, &aReflowInput, startEdge, startEdge + availableISize, &mBaseline);
  ...
  nsLayoutUtils::SetBSizeFromFontMetrics(
      this, aMetrics, framePadding, lineWM, frameWM);
