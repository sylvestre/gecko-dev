...
{
  MOZ_LOG(
      sPluginFrameLog, LogLevel::Debug, ("nsPluginFrame %p deleted\n", this));
}
...
#ifdef XP_WIN
NS_IMETHODIMP
nsPluginFrame::GetPluginPort(HWND* aPort)
{
...
void
nsPluginFrame::GetWidgetConfiguration(
                              ...
                              ReflowOutput& aMetrics)
{
  ...
  if (mContent->IsHTMLElement(nsGkAtoms::embed)) {
    if (aMetrics.Width() == NS_UNCONSTRAINEDSIZE) {
      aMetrics.Width() =
          clamped(nsPresContext::CSSPixelsToAppUnits(EMBED_DEF_WIDTH),
                  aReflowInput.ComputedMinWidth(),
                  aReflowInput.ComputedMaxWidth());
    }
    if (aMetrics.Height() == NS_UNCONSTRAINEDSIZE) {
      aMetrics.Height() =
          clamped(nsPresContext::CSSPixelsToAppUnits(EMBED_DEF_HEIGHT),
                         ...
                         nsDidReflowStatus aStatus)
{
  ...
  if (aStatus != nsDidReflowStatus::FINISHED) return;
...
/* static */ void
nsPluginFrame::PaintPrintPlugin(nsIFrame* aFrame,
                                gfxContext* aCtx,
...
void
nsDisplayPlugin::Paint(nsDisplayListBuilder* aBuilder, gfxContext* aCtx)
{
...
bool
nsDisplayPlugin::CreateWebRenderCommands(
                                ...
                                const nsDisplayListSet& aLists)
{
  ...
  if (type == nsPresContext::eContext_Print) {
    ...
    if (aBuilder->IsPaintingToWindow() && state == LAYER_ACTIVE &&
        IsTransparentMode()) {
      aLists.Content()->AppendNewToTop(
          new (aBuilder) nsDisplayPluginReadback(aBuilder, this));
    }
...
// platform specific printing code
  ...
  nsDidReflowStatus status =
      nsDidReflowStatus::FINISHED;  // should we use a special status?
  frame->DidReflow(
      presContext, nullptr, status);  // DidReflow will take care of it
}
...
nsPluginFrame::GetPaintedRect(const nsDisplayPlugin* aItem) const
{
  ...
  r.IntersectRect(r,
                  nsRect(0,
                         0,
                         pc->DevPixelsToAppUnits(size.width),
...
bool
nsPluginFrame::CreateWebRenderCommands(
    nsDisplayItem* aItem,
    mozilla::wr::DisplayListBuilder& aBuilder,
    mozilla::wr::IpcResourceUpdateQueue& aResources,
    const StackingContextHelper& aSc,
    ...
    nsDisplayListBuilder* aDisplayListBuilder)
{
  ...
  return aManager->CommandBuilder().PushImage(
      aItem, container, aBuilder, aResources, aSc, dest);
}
