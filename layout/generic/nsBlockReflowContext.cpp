...
static nsIFrame*
DescendIntoBlockLevelFrame(nsIFrame* aFrame)
{
      ...
      for (; anyLines && line != line_end; ++line) {
        ...
        if (line->IsInline()) {
          ...
          }
          {
            ...
            ReflowInput innerReflowInput(
                prescontext, *outerReflowInput, kid, availSpace);
            ...
            if (ComputeCollapsedBStartMargin(innerReflowInput,
                                             aMargin,
                                             aClearanceFrame,
                                             aMayNeedRetry,
                                             &isEmpty)) {
                                 ...
                                 const nsReflowStatus& aReflowStatus)
{
  ...
  if (!empty && !aForceFit &&
    ...
    if (bEnd > mSpace.BEnd(mWritingMode)) {
      ...
      mFrame->DidReflow(
          mPresContext, &aReflowInput, nsDidReflowStatus::FINISHED);
      return false;
    }
  }
  ...
  LogicalPoint logPos =
      LogicalPoint(mWritingMode, mICoord, mBCoord)
          .ConvertTo(
              frameWM, mWritingMode, mContainerSize - mMetrics.PhysicalSize());
  ...
  nsContainerFrame::FinishReflowChild(mFrame,
                                      mPresContext,
                                      mMetrics,
                                      &aReflowInput,
                                      frameWM,
