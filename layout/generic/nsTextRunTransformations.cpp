                             ...
                             bool aOwnsFactory)
{
  ...
  RefPtr<nsTransformedTextRun> result =
      new (storage) nsTransformedTextRun(aParams,
                                         aFactory,
                                         aFontGroup,
                                         aString,
                                         aLength,
...
void
nsTransformedTextRun::SetCapitalization(uint32_t aStart,
                                        uint32_t aLength,
                                        bool* aCapitalization)
{
  ...
  memcpy(
      mCapitalize.Elements() + aStart, aCapitalization, aLength * sizeof(bool));
  mNeedsRebuild = true;
}
...
nsTransformingTextRunFactory::MakeTextRun(
    ...
    bool aOwnsFactory)
{
  return nsTransformedTextRun::Create(aParams,
                                      this,
                                      aFontGroup,
                                      aString,
                                      aLength,
    ...
    bool aOwnsFactory)
{
  ...
  NS_ConvertASCIItoUTF16 unicodeString(reinterpret_cast<const char*>(aString),
                                       aLength);
  return MakeTextRun(unicodeString.get(),
                     aLength,
                     aParams,
                     aFontGroup,
...
void
MergeCharactersInTextRun(gfxTextRun* aDest,
                         gfxTextRun* aSrc,
                         ...
                         const bool* aDeletedChars)
{
  ...
  while (iter.NextRun()) {
    ...
    nsresult rv = aDest->AddGlyphRun(
        run->mFont, run->mMatchType, offset, false, run->mOrientation);
...
{
  gfxTextRunFactory::Parameters params = {aRefDrawTarget,
                                          nullptr,
                                          nullptr,
                                          nullptr,
                                          0,
    ...
    switch (style) {
      ...
      case NS_STYLE_TEXT_TRANSFORM_UPPERCASE:
        ...
        if (languageSpecificCasing == eLSCB_Greek) {
          bool markEta;
          bool updateEta;
          ch = mozilla::GreekCasing::UpperCase(
              ch, greekState, markEta, updateEta);
          if (markEta) {
                                              ...
                                              gfxMissingFontRecorder* aMFR)
{
  ...
  if (mInnerTransformingTextRunFactory) {
    ...
    cachedChild = fontGroup->MakeTextRun(convertedString.BeginReading(),
                                         convertedString.Length(),
                                         &innerParams,
                                         flags,
                                         nsTextFrameUtils::Flags(),
  ...
  if (mergeNeeded) {
    ...
    MergeCharactersInTextRun(aTextRun,
                             child,
