...
static uint32_t
GetOverflowChange(const nsRect& aCurScrolledRect,
...
void
nsHTMLScrollFrame::AppendFrames(ChildListID aListID, nsFrameList& aFrameList)
{
...
void
nsHTMLScrollFrame::RemoveFrame(ChildListID aListID, nsIFrame* aOldFrame)
{
  NS_ASSERTION(aListID == kPrincipalList, "Only main list supported");
...
static nsSize
ComputeInsideBorderSize(ScrollReflowInput* aState,
...
static void
GetScrollbarMetrics(nsBoxLayoutState& aState,
                    nsIFrame* aBox,
                    nsSize* aMin,
                    nsSize* aPref,
                    bool aVertical)
                             ...
                             bool aForce)
{
  ...
  if (aAssumeVScroll != aState->mReflowedContentsWithVScrollbar ||
    ...
    ReflowScrolledFrame(
        aState, aAssumeHScroll, aAssumeVScroll, aKidMetrics, false);
  }

  nsSize vScrollbarMinSize(0, 0);
  nsSize vScrollbarPrefSize(0, 0);
  if (mHelper.mVScrollbarBox) {
    GetScrollbarMetrics(aState->mBoxState,
                        mHelper.mVScrollbarBox,
                        &vScrollbarMinSize,
                                       ...
                                       bool aFirstPass)
{
  ...
  ReflowChild(mHelper.mScrolledFrame,
              presContext,
              *aMetrics,
              kidReflowInput,
              wm,
  ...
  FinishReflowChild(mHelper.mScrolledFrame,
                    presContext,
                    *aMetrics,
                    &kidReflowInput,
                    wm,
...
nsHTMLScrollFrame::GetIntrinsicVScrollbarWidth(gfxContext* aRenderingContext)
{
  ...
  nsSize vScrollbarPrefSize(0, 0);
  GetScrollbarMetrics(
      bls, mHelper.mVScrollbarBox, nullptr, &vScrollbarPrefSize, true);
  ...
  for (nsIFrame::ChildListIterator childLists(aCurrentFrame);
    ...
    for (nsIFrame* child : childLists.CurrentList()) {
      ...
      if (child->Extend3DContext()) {
        ...
        GetScrollableOverflowForPerspective(aScrolledFrame,
                                            child,
                                            aScrollPort,
                                            offset,
...
#undef NEEDS_REFLOW
  ...
  if (mHelper.mSkippedScrollbarLayout || reflowHScrollbar || reflowVScrollbar ||
    ...
    if (!mHelper.mSuppressScrollbarUpdate) {
      ...
      mHelper.LayoutScrollbars(
          state.mBoxState, insideBorderArea, oldScrollAreaBounds);
    } else {
...
void
ScrollFrameHelper::ScrollByPage(nsScrollbarFrame* aScrollbar,
                                int32_t aDirection,
...
void
ScrollFrameHelper::ScrollByWhole(nsScrollbarFrame* aScrollbar,
                                 int32_t aDirection,
                                 nsIScrollbarMediator::ScrollSnapMode aSnap)
{
  ScrollByUnit(aScrollbar,
...
void
ScrollFrameHelper::ScrollByLine(nsScrollbarFrame* aScrollbar,
                                int32_t aDirection,
                                nsIScrollbarMediator::ScrollSnapMode aSnap)
{
...
void
nsXULScrollFrame::AppendFrames(ChildListID aListID, nsFrameList& aFrameList)
{
...
void
nsXULScrollFrame::RemoveFrame(ChildListID aListID, nsIFrame* aOldFrame)
{
 ...
 public:
  ...
  bool SetRefreshObserver(ScrollFrameHelper* aCallee)
  {
    ...
    APZCCallbackHelper::SuppressDisplayport(
        true, mCallee->mOuter->PresContext()->PresShell());
    return true;
  }
 ...
 private:
  ...
  void RemoveObserver()
  {
    if (mCallee) {
      RefreshDriver(mCallee)->RemoveRefreshObserver(this, FlushType::Style);
      APZCCallbackHelper::SuppressDisplayport(
          false, mCallee->mOuter->PresContext()->PresShell());
    }
  }
};
 ...
 */
void
ScrollFrameHelper::AsyncSmoothMSDScrollCallback(
                                     ...
                                     nsIScrollableFrame::ScrollMode aMode)
{
  ...
  nscoord halfPixel = nsPresContext::CSSPixelsToAppUnits(0.5f);
  nsRect range(
      pt.x - halfPixel, pt.y - halfPixel, 2 * halfPixel - 1, 2 * halfPixel - 1);
                                                ...
                                                nsAtom* aOrigin)
{
  ...
  nsRect range(
      pt.x - halfRange, pt.y - halfRange, 2 * halfRange - 1, 2 * halfRange - 1);
 ...
 */
void
ScrollFrameHelper::ScrollToWithOrigin(
    ...
    nsIScrollbarMediator::ScrollSnapMode aSnap)
{
  if (aSnap == nsIScrollableFrame::ENABLE_SNAP) {
    GetSnapPointForDestination(
        nsIScrollableFrame::DEVICE_PIXELS, mDestination, aScrollPosition);
  }
    ...
    if (aMode == nsIScrollableFrame::SMOOTH_MSD) {
      ...
      if (!mAsyncSmoothMSDScroll) {
        ...
        mAsyncSmoothMSDScroll =
            new AsyncSmoothMSDScroll(GetScrollPosition(),
                                     mDestination,
                                     currentVelocity,
  ...
  if (isSmoothScroll) {
    mAsyncScroll->InitSmoothScroll(now,
                                   GetScrollPosition(),
                                   mDestination,
                                   aOrigin,
                                   range,
...
bool
ScrollFrameHelper::IsIgnoringViewportClipping() const
{
  if (!mIsRoot) return false;
  nsSubDocumentFrame* subdocFrame =
      static_cast<nsSubDocumentFrame*>(nsLayoutUtils::GetCrossDocParentFrame(
          mOuter->PresContext()->PresShell()->GetRootFrame()));
  return subdocFrame && !subdocFrame->ShouldClipSubdocument();
}

void
ScrollFrameHelper::MarkScrollbarsDirtyForReflow() const
{
  nsIPresShell* presShell = mOuter->PresContext()->PresShell();
  if (mVScrollbarBox) {
    presShell->FrameNeedsReflow(
        mVScrollbarBox, nsIPresShell::eResize, NS_FRAME_IS_DIRTY);
  }
  if (mHScrollbarBox) {
    presShell->FrameNeedsReflow(
        mHScrollbarBox, nsIPresShell::eResize, NS_FRAME_IS_DIRTY);
  }
}
...
bool
ScrollFrameHelper::IsAlwaysActive() const
{
...
void
ScrollFrameHelper::MarkNotRecentlyScrolled()
{
...
void
ScrollFrameHelper::MarkRecentlyScrolled()
{
...
bool
ScrollFrameHelper::AllowDisplayPortExpiration()
{
  if (IsAlwaysActive()) {
    return false;
  }
...
void
ScrollFrameHelper::TriggerDisplayPortExpiration()
{
...
void
ScrollFrameHelper::ScrollVisual()
{
...
{
  return nsPoint(ClampAndAlignWithPixels(aPt.x,
                                         aBounds.x,
                                         aBounds.XMost(),
                                         aRange.x,
                                         aRange.XMost(),
...
bool
ScrollFrameHelper::GetDisplayPortAtLastApproximateFrameVisibilityUpdate(
    nsRect* aDisplayPort)
{
...
void
ScrollFrameHelper::ScrollToImpl(nsPoint aPt,
                                const nsRect& aRange,
                                nsAtom* aOrigin)
{
  if (aOrigin == nullptr) {
  ...
  nsPoint pt = ClampAndAlignWithLayerPixels(aPt,
                                            GetScrollRangeForClamping(),
                                            aRange,
                                            alignWithPos,
                                            appUnitsPerDevPixel,
  ...
  if (nsLayoutUtils::AsyncPanZoomEnabled(mOuter) &&
    ...
    PAINT_SKIP_LOG(
        "New scrollpos %s usingDP %d dpEqual %d scrollableByApz %d plugins %d "
        "perspective %d bglocal %d\n",
        Stringify(CSSPoint::FromAppUnits(GetScrollPosition())).c_str(),
        usingDisplayPort,
        displayPort.IsEqualEdges(oldDisplayPort),
        mScrollableByAPZ,
        HasPluginFrames(),
        HasPerspective(),
...
static void
AppendToTop(nsDisplayListBuilder* aBuilder,
            ...
            uint32_t aFlags)
{
  ...
  if (aFlags & APPEND_OWN_LAYER) {
    uint32_t flags = (aFlags & APPEND_SCROLLBAR_CONTAINER)
                         ...
                         : 0;
    FrameMetrics::ViewID scrollTarget =
        (aFlags & APPEND_SCROLLBAR_CONTAINER)
            ? aBuilder->GetCurrentScrollbarTarget()
            : FrameMetrics::NULL_SCROLL_ID;
    newItem = new (aBuilder) nsDisplayOwnLayer(
        aBuilder, aSourceFrame, aSource, asr, flags, scrollTarget);
  } else {
    newItem =
        new (aBuilder) nsDisplayWrapList(aBuilder, aSourceFrame, aSource, asr);
  }
...
struct HoveredStateComparator
{
  ...
  {
    bool aHovered =
        A->GetContent()->HasAttr(kNameSpaceID_None, nsGkAtoms::hover);
    bool bHovered =
        B->GetContent()->HasAttr(kNameSpaceID_None, nsGkAtoms::hover);
                     ...
                     const DisplayItemClipChain* aExtraClip)
{
  ...
  ClipItemsExceptCaret(
      aLists->BorderBackground(), aBuilder, aClipFrame, aExtraClip, cache);
  ClipItemsExceptCaret(aLists->BlockBorderBackgrounds(),
                       aBuilder,
                       aClipFrame,
                       aExtraClip,
                       cache);
  ClipItemsExceptCaret(
      aLists->Floats(), aBuilder, aClipFrame, aExtraClip, cache);
  ClipItemsExceptCaret(
      aLists->PositionedDescendants(), aBuilder, aClipFrame, aExtraClip, cache);
  ClipItemsExceptCaret(
      aLists->Outlines(), aBuilder, aClipFrame, aExtraClip, cache);
  ClipItemsExceptCaret(
      aLists->Content(), aBuilder, aClipFrame, aExtraClip, cache);
}
                                    ...
                                    const nsDisplayListSet& aLists)
{
  ...
  Unused << DecideScrollableLayer(aBuilder,
                                  &visibleRect,
                                  &dirtyRect,
                                  /* aSetBase = */ !mIsRoot,
  ...
  nsDisplayListCollection scrolledContent(aBuilder);
  {
...

    {
      ...
      mOuter->BuildDisplayListForChild(
          aBuilder, mScrolledFrame, scrolledContent);
  ...
  if (couldBuildLayer) {
    ...
    if (aBuilder->IsPaintingToWindow() && !mWillBuildScrollableLayer &&
        aBuilder->IsBuildingLayerEventRegions()) {
      inactiveRegionItem = new (aBuilder)
          nsDisplayLayerEventRegions(aBuilder, mScrolledFrame, 1);
    ...
    if (aBuilder->ShouldBuildScrollInfoItemsForHoisting()) {
      aBuilder->AppendNewScrollInfoItemForHoisting(new (
          aBuilder) nsDisplayScrollInfoLayer(aBuilder, mScrolledFrame, mOuter));
    }
  }
  // Now display overlay scrollbars and the resizer, if we have one.
  AppendScrollPartsTo(
      aBuilder, scrolledContent, createLayersForScrollbars, true);
                                         ...
                                         bool* aDirtyRectHasBeenOverriden)
{
  ...
  if (aBuilder->IsPaintingToWindow()) {
    ...
    usingDisplayPort = nsLayoutUtils::GetDisplayPort(
        content, &displayPort, RelativeTo::ScrollFrame);
...
ScrollFrameHelper::ComputeScrollMetadata(
    Layer* aLayer,
    ...
    const DisplayItemClip* aClip) const
{
  ...
  return Some(nsLayoutUtils::ComputeScrollMetadata(mScrolledFrame,
                                                   mOuter,
                                                   mOuter->GetContent(),
...
static void
HandleScrollPref(nsIScrollable* aScrollable,
...
ScrollFrameHelper::GetScrollbarStylesFromFrame() const
{
  ...
  if (scrollable) {
    HandleScrollPref(
        scrollable, nsIScrollable::ScrollOrientation_X, result.mHorizontal);
    HandleScrollPref(
        scrollable, nsIScrollable::ScrollOrientation_Y, result.mVertical);
  }
  return result;
}
...
ScrollFrameHelper::GetScrollRangeForClamping() const
{
  if (!ShouldClampScrollPosition()) {
    return nsRect(nscoord_MIN / 2,
                  nscoord_MIN / 2,
                  nscoord_MAX - nscoord_MIN / 2,
                  nscoord_MAX - nscoord_MIN / 2);
  }
  nsSize scrollPortSize = GetScrollPositionClampingScrollPortSize();
 ...
 */
static void
CalcRangeForScrollBy(int32_t aDelta,
                     nscoord aPos,
                     float aNegTolerance,
                     float aPosTolerance,
                     nscoord aMultiplier,
                            ...
                            nsIScrollbarMediator::ScrollSnapMode aSnap)
{
  ...
  CalcRangeForScrollBy(aDelta.x,
                       newPos.x,
                       negativeTolerance,
                       positiveTolerance,
                       deltaMultiplier.width,
                       &rangeLowerX,
                       &rangeUpperX);
  CalcRangeForScrollBy(aDelta.y,
                       newPos.y,
                       negativeTolerance,
                       positiveTolerance,
                       deltaMultiplier.height,
                       &rangeLowerY,
                       &rangeUpperY);
                              ...
                              nsIScrollableFrame::ScrollMode aMode)
{
  ...
  if (GetSnapPointForDestination(
          nsIScrollableFrame::DEVICE_PIXELS, pos, snapDestination)) {
...
ScrollFrameHelper::GetLineScrollAmount() const
{
  ...
  if (sMinLineScrollAmountInPixels < 0) {
    Preferences::AddIntVarCache(
        &sMinLineScrollAmountInPixels, "mousewheel.min_line_scroll_amount", 1);
  }
...
ScrollFrameHelper::ScrollToRestoredPosition()
{
  ...
  if (GetLogicalScrollPosition() == mLastPos) {
    ...
    if (mRestorePos != mLastPos /* GetLogicalScrollPosition() */) {
      ...
      AutoWeakFrame weakFrame(mOuter);
      ScrollToWithOrigin(scrollToPos,
                         nsIScrollableFrame::INSTANT,
                         nsGkAtoms::restore,
                         nullptr);
      if (!weakFrame.IsAlive()) {
...
ScrollFrameHelper::FireScrollPortEvent()
{
  ...
  return EventDispatcher::Dispatch(
      mOuter->GetContent(), mOuter->PresContext(), &event);
}
    ...
    nsTArray<nsIAnonymousContentCreator::ContentInfo>& aElements)
{
  ...
  nodeInfo = nodeInfoManager->GetNodeInfo(nsGkAtoms::scrollbar,
                                          nullptr,
                                          kNameSpaceID_XUL,
  ...
  if (canHaveHorizontal) {
    ...
    mHScrollbarContent->SetAttr(kNameSpaceID_None,
                                nsGkAtoms::orient,
                                NS_LITERAL_STRING("horizontal"),
                                false);
    mHScrollbarContent->SetAttr(kNameSpaceID_None,
                                nsGkAtoms::clickthrough,
                                NS_LITERAL_STRING("always"),
                                false);
    if (mIsRoot) {
      mHScrollbarContent->SetAttr(kNameSpaceID_None,
                                  nsGkAtoms::root_,
  ...
  if (canHaveVertical) {
    ...
    mVScrollbarContent->SetAttr(kNameSpaceID_None,
                                nsGkAtoms::orient,
  ...
  if (isResizable) {
    RefPtr<NodeInfo> nodeInfo;
    nodeInfo = nodeInfoManager->GetNodeInfo(nsGkAtoms::resizer,
                                            nullptr,
                                            kNameSpaceID_XUL,
                                            nsIDOMNode::ELEMENT_NODE);
    NS_ENSURE_TRUE(nodeInfo, NS_ERROR_OUT_OF_MEMORY);
    ...
    if (mIsRoot) {
      ...
      mResizerContent->SetAttr(kNameSpaceID_None,
                               nsGkAtoms::element,
                               NS_LITERAL_STRING("_parent"),
                               false);
    }
  ...
  if (canHaveHorizontal && canHaveVertical) {
    nodeInfo = nodeInfoManager->GetNodeInfo(nsGkAtoms::scrollcorner,
                                            nullptr,
                                            kNameSpaceID_XUL,
                                            nsIDOMNode::ELEMENT_NODE);
...
ScrollFrameHelper::UpdateScrollbarPosition()
{
  ...
  if (mVScrollbarBox) {
    SetCoordAttribute(mVScrollbarBox->GetContent(),
                      nsGkAtoms::curpos,
  ...
  if (mHScrollbarBox) {
    SetCoordAttribute(mHScrollbarBox->GetContent(),
                      nsGkAtoms::curpos,
...
void
ScrollFrameHelper::CurPosAttributeChanged(nsIContent* aContent, bool aDoScroll)
{
  ...
  nsPoint dest;
  nsRect allowedRange;
  dest.x = GetCoordAttribute(mHScrollbarBox,
                             nsGkAtoms::curpos,
                             current.x,
                             &allowedRange.x,
                             &allowedRange.width);
  dest.y = GetCoordAttribute(mVScrollbarBox,
                             nsGkAtoms::curpos,
                             current.y,
                             &allowedRange.y,
                             &allowedRange.height);
...
{
  if (mHelper) {
    mHelper->mOuter->PresContext()->GetPresShell()->FlushPendingNotifications(
...
bool
nsXULScrollFrame::AddRemoveScrollbar(bool& aHasScrollbar,
                                     nscoord& aXY,
                                     nscoord& aSize,
                                     nscoord aSbSize,
                                     bool aOnRightOrBottom,
...
void
ScrollFrameHelper::PostOverflowEvent()
{
...
nsXULScrollFrame::XULLayout(nsBoxLayoutState& aState)
{
  ...
  if (mHelper.mHScrollbarBox && mHelper.mHasHorizontalScrollbar) {
    GetScrollbarMetrics(
        aState, mHelper.mHScrollbarBox, &hMinSize, nullptr, false);
  }
  nsSize vMinSize(0, 0);
  if (mHelper.mVScrollbarBox && mHelper.mHasVerticalScrollbar) {
    GetScrollbarMetrics(
        aState, mHelper.mVScrollbarBox, &vMinSize, nullptr, true);
  }
...
ScrollFrameHelper::ReflowFinished()
{
  ...
  if (vScroll || hScroll) {
    ...
    if (vScroll) {
      ...
      FinishReflowForScrollbar(vScroll,
                               minY,
                               maxY,
                               scrollPos.y,
...
void
ScrollFrameHelper::AdjustScrollbarRectForResizer(nsIFrame* aFrame,
                                                 nsPresContext* aPresContext,
                                                 nsRect& aRect,
                                                 bool aHasResizer,
                                                 bool aVertical)
                                    ...
                                    const nsRect& aOldScrollArea)
{
  ...
  if (mHScrollbarBox) {
    ...
    AdjustScrollbarRectForResizer(
        mOuter, presContext, hRect, hasResizer, false);
  }
...
#if DEBUG
static bool
ShellIsAlive(nsWeakPtr& aWeakPtr)
{
...
ScrollFrameHelper::SetScrollbarEnabled(nsIContent* aContent, nscoord aMaxPos)
{
  ...
  if (aMaxPos) {
    ...
    aContent->SetAttr(kNameSpaceID_None,
                      nsGkAtoms::disabled,
                      NS_LITERAL_STRING("true"),
                      true);
  }
...
void
ScrollFrameHelper::SetCoordAttribute(nsIContent* aContent,
                                     nsAtom* aAtom,
                                     nscoord aSize)
{
  DebugOnly<nsWeakPtr> weakShell(
...
{
  if (!mOuter->nsContainerFrame::GetBorderRadii(
          aFrameSize, aBorderArea, aSkipSides, aRadii)) {
    return false;
  }
...
nsRect
ScrollFrameHelper::GetUnsnappedScrolledRectInternal(
...
nscoord
ScrollFrameHelper::GetCoordAttribute(nsIFrame* aBox,
                                     nsAtom* aAtom,
 ...
 */
void
CollectScrollSnapCoordinates(nsIFrame* aFrame,
...
ScrollFrameHelper::GetSnapPointForDestination(
    ...
    nsPoint& aDestination)
{
  Maybe<nsPoint> snapPoint =
      ScrollSnapUtils::GetSnapPointForDestination(GetScrollSnapInfo(),
                                                  aUnit,
                                                  mScrollPort.Size(),
                                                  GetScrollRangeForClamping(),
                                                  aStartPos,
