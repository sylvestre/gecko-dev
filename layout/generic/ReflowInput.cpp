...
static bool
CheckNextInFlowParenthood(nsIFrame* aFrame, nsIFrame* aParent)
{
      ...
      mWritingMode(aFrame->GetWritingMode())
{
  ...
  LogicalSize cbSize(aContainingBlockWritingMode,
                                        ...
                                        const nsStyleCoord& aCoord) const
{
  ...
  return nsLayoutUtils::ComputeBSizeValue(
      aContainingBlockBSize, inside, aCoord);
}
 ...
 */
void
ReflowInput::CalculateBorderPaddingMargin(LogicalAxis aAxis,
    ...
    LayoutFrameType aFrameType) const
{
  ...
  if (blockFrame) {
    ...
    LogicalPoint blockOffset(
        wm, blockFrame->GetOffsetTo(containingBlock), nullContainerSize);
    bool isValid;
  ...
  if (cbwm.GetBlockDir() != wm.GetBlockDir()) {
    ...
    if (isAutoBSize) {
      ...
      boxBSize = nsLayoutUtils::ComputeBSizeValue(
                     blockContentSize.BSize(wm), insideBoxSizing, styleBSize) +
                 insideBoxSizing + outsideBoxSizing;
    }
                                     ...
                                     LayoutFrameType aFrameType)
{
  ...
  if ((iStartIsAuto && iEndIsAuto) || (bStartIsAuto && bEndIsAuto)) {
    ...
    if (mFlags.mStaticPosIsCBOrigin) {
      ...
      CalculateHypotheticalPosition(aPresContext,
                                    placeholderFrame,
                                    aReflowInput,
                                    hypotheticalPos,
                                    aFrameType);
...
static eNormalLineHeightControl
...
{
  WritingMode wm = GetWritingMode();
  DISPLAY_INIT_CONSTRAINTS(mFrame,
                           this,
  ...
  if (nullptr == mParentReflowInput || mFlags.mDummyParentReflowInput) {
    ...
    InitOffsets(wm,
                OffsetPercentBasis(mFrame, wm, aContainingBlockSize),
                aFrameType,
                mFlags,
                aBorder,
                aPadding,
    ...
    InitOffsets(cbwm,
                OffsetPercentBasis(mFrame, cbwm, cbSize.ConvertTo(cbwm, wm)),
                aFrameType,
                mFlags,
                aBorder,
                aPadding,
    ...
    if (mStyleDisplay->IsRelativelyPositioned(mFrame) &&
      ...
      ComputeRelativeOffsets(
          cbwm, mFrame, cbSize.ConvertTo(cbwm, wm), ComputedPhysicalOffsets());
    } else {
    ...
    if (NS_CSS_FRAME_TYPE_INTERNAL_TABLE == mFrameType) {
      ...
      InitAbsoluteConstraints(aPresContext,
                              cbri,
      ...
      LogicalSize size =
          mFrame->ComputeSize(mRenderingContext,
                              wm,
                              cbSize,
                              AvailableISize(),
                                    ...
                                    const LogicalSize& aPercentBasis)
{
  ...
  if (isCBDependent) {
    ...
    m.IStart(aWM) = nsLayoutUtils::ComputeCBDependentValue(
        aPercentBasis.ISize(aWM), styleMargin->mMargin.GetIStart(aWM));
    m.IEnd(aWM) = nsLayoutUtils::ComputeCBDependentValue(
        aPercentBasis.ISize(aWM), styleMargin->mMargin.GetIEnd(aWM));

    m.BStart(aWM) = nsLayoutUtils::ComputeCBDependentValue(
        aPercentBasis.BSize(aWM), styleMargin->mMargin.GetBStart(aWM));
    m.BEnd(aWM) = nsLayoutUtils::ComputeCBDependentValue(
        aPercentBasis.BSize(aWM), styleMargin->mMargin.GetBEnd(aWM));
                                     ...
                                     LayoutFrameType aFrameType)
{
  ...
  if (LayoutFrameType::TableRowGroup == aFrameType ||
    ...
    LogicalMargin p(aWM);
    p.IStart(aWM) = std::max(
        0,
        nsLayoutUtils::ComputeCBDependentValue(
            aPercentBasis.ISize(aWM), stylePadding->mPadding.GetIStart(aWM)));
    p.IEnd(aWM) = std::max(
        0,
        nsLayoutUtils::ComputeCBDependentValue(
            aPercentBasis.ISize(aWM), stylePadding->mPadding.GetIEnd(aWM)));

    p.BStart(aWM) = std::max(
        0,
        nsLayoutUtils::ComputeCBDependentValue(
            aPercentBasis.BSize(aWM), stylePadding->mPadding.GetBStart(aWM)));
    p.BEnd(aWM) = std::max(
        0,
        nsLayoutUtils::ComputeCBDependentValue(
            aPercentBasis.BSize(aWM), stylePadding->mPadding.GetBEnd(aWM)));
