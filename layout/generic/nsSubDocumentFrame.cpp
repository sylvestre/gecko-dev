...
nsSubDocumentFrame::ShowViewer()
{
  ...
  if (!PresContext()->IsDynamic()) {
    ...
    if (frameloader) {
      ...
      bool didCreateDoc = frameloader->Show(
          margin.width, margin.height, scrolling, scrolling, this);
      if (!weakThis.IsAlive()) {
        return;
      }
      mCallingShow = false;
  ...
  if (GetStateBits() & NS_FRAME_FIRST_REFLOW) {
    ...
    if (subDocRoot) {
      nsRect destRect = nsLayoutUtils::ComputeObjectDestRect(
          nsRect(nsPoint(), docSizeAppUnits),
                              ...
                              nsDisplayList* aList)
{
  ...
  while ((item = aList->RemoveBottom()) != nullptr) {
    if (item->GetType() == DisplayItemType::TYPE_BACKGROUND_COLOR) {
      ...
      item = new (aBuilder) nsDisplayOwnLayer(
                                     ...
                                     const nsDisplayListSet& aLists)
{
  ...
  if (!aBuilder->IsForEventDelivery() || !pointerEventsNone) {
    ...
    if (rfp) {
      ...
      WrapBackgroundColorInOwnLayer(
          aBuilder, this, decorations.BorderBackground());
    }
  ...
  if (subdocRootFrame) {
    ...
    if (nsIFrame* rootScrollFrame = presShell->GetRootScrollFrame()) {
      ...
      haveDisplayPort =
          rootScrollableFrame->DecideScrollableLayer(aBuilder,
                                                     &copyOfVisible,
                                                     &copyOfDirty,
                                                     /* aSetBase = */ true);
  ...
  if (constructZoomItem) {
    ...
    nsDisplayZoom* zoomItem = new (aBuilder) nsDisplayZoom(aBuilder,
                                                           subdocRootFrame,
                                                           &childItems,
                                                           subdocAPD,
                                                           parentAPD,
  ...
  if (constructResolutionItem) {
    nsDisplayResolution* resolutionItem = new (aBuilder)
        nsDisplayResolution(aBuilder, subdocRootFrame, &childItems, flags);
    childItems.AppendToTop(resolutionItem);
    needsOwnLayer = false;
  }
  if (needsOwnLayer) {
    // We always want top level content documents to be in their own layer.
    nsDisplaySubDocument* layerItem = new (aBuilder)
        nsDisplaySubDocument(aBuilder,
                             subdocRootFrame ? subdocRootFrame : this,
                             this,
                             &childItems,
                             flags);
...
nsresult
nsSubDocumentFrame::GetFrameName(nsAString& aResult) const
{
...
{
  if (!IsInline()) {
    return nsFrame::ComputeAutoSize(aRenderingContext,
                                    aWM,
                                    aCBSize,
                                    aAvailableISize,
                                    aMargin,
                                ...
                                ComputeSizeFlags aFlags)
{
  ...
  return nsAtomicContainerFrame::ComputeSize(aRenderingContext,
                                             aWM,
                                             aCBSize,
                                             aAvailableISize,
                                             aMargin,
                                     ...
                                     int32_t aModType)
{
  ...
  if (aAttribute == nsGkAtoms::noresize) {
    ...
    if (rootFrame) {
      rootFrame->PresContext()->PresShell()->FrameNeedsReflow(
          rootFrame, nsIPresShell::eResize, NS_FRAME_IS_DIRTY);
    }
...
nsSubDocumentFrame::EndSwapDocShells(nsIFrame* aOther)
{
  ...
  if (weakOther.IsAlive()) {
    other->PresContext()->PresShell()->FrameNeedsReflow(
        other, nsIPresShell::eTreeChange, NS_FRAME_IS_DIRTY);
