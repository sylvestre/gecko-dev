...
nsContainerFrame*
NS_NewRubyBaseContainerFrame(nsIPresShell* aPresShell, nsStyleContext* aContext)
{
...
static void
GetIsLineBreakAllowed(nsIFrame* aFrame,
...
//       See bug 1134945.
/* virtual */ void
nsRubyBaseContainerFrame::AddInlineMinISize(gfxContext* aRenderingContext,
                                            nsIFrame::InlineMinISizeData* aData)
{
  ...
  GetIsLineBreakAllowed(
      this, !aData->mAtStartOfLine, &allowInitialLineBreak, &allowLineBreak);
                                 ...
                                 nsReflowStatus& aStatus)
{
  ...
  LogicalSize availSize(
      lineWM, aReflowInput.AvailableISize(), aReflowInput.AvailableBSize());
  ...
  bool allowInitialLineBreak, allowLineBreak;
  GetIsLineBreakAllowed(this,
                        aReflowInput.mLineLayout->LineIsBreakable(),
                        &allowInitialLineBreak,
                        &allowLineBreak);
  ...
  nsLayoutUtils::SetBSizeFromFontMetrics(
      this, aDesiredSize, borderPadding, lineWM, frameWM);
}
                                          ...
                                          nsReflowStatus& aStatus)
{
  ...
  if (aColumn.mBaseFrame) {
    ...
    lineLayout->ReflowFrame(
        aColumn.mBaseFrame, reflowStatus, nullptr, pushedFrame);
    if (MOZ_UNLIKELY(reflowStatus.IsInlineBreak() || pushedFrame)) {
      MOZ_ASSERT_UNREACHABLE(
          "Any line break inside ruby box should have been suppressed");
      // For safety, always drain the overflow list, so that
