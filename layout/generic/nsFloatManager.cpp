...
// static
void*
nsFloatManager::operator new(size_t aSize) CPP_THROW_NEW
{
...
nsFlowAreaRect
nsFloatManager::GetFlowArea(WritingMode aWM,
                            nscoord aBCoord,
                            nscoord aBSize,
                            BandInfoType aBandInfoType,
                            ShapeType aShapeType,
                            LogicalRect aContentArea,
                            SavedState* aState,
                            const nsSize& aContainerSize) const
{
  ...
  if (floatCount == 0 || (mFloats[floatCount - 1].mLeftBEnd <= blockStart &&
    ...
    return nsFlowAreaRect(aWM,
                          aContentArea.IStart(aWM),
                          aBCoord,
                          aContentArea.ISize(aWM),
                          aBSize,
                          false);
  }
...
void
nsFloatManager::AddFloat(nsIFrame* aFloatFrame,
                         ...
                         const nsSize& aContainerSize)
{
  ...
  FloatInfo info(
      aFloatFrame, mLineLeft, mBlockStart, aMarginRect, aWM, aContainerSize);
...
LogicalRect
nsFloatManager::GetRegionFor(WritingMode aWM,
                             nsIFrame* aFloat,
                             const nsSize& aContainerSize)
{
...
void
nsFloatManager::StoreRegionFor(WritingMode aWM,
                               nsIFrame* aFloat,
...
nscoord
nsFloatManager::ClearFloats(nscoord aBCoord,
                                              ...
                                              const nscoord aBEnd) const
{
  ...
  nscoord lineLeftDiff =
      ComputeEllipseLineInterceptDiff(mRect.y,
                                      mRect.YMost(),
                                      mRadii[eCornerTopLeftX],
                                      mRadii[eCornerTopLeftY],
                                      mRadii[eCornerBottomLeftX],
                                      mRadii[eCornerBottomLeftY],
                                      aBStart,
                                      aBEnd);
                                               ...
                                               const nscoord aBEnd) const
{
  ...
  nscoord lineRightDiff =
      ComputeEllipseLineInterceptDiff(mRect.y,
                                      mRect.YMost(),
                                      mRadii[eCornerTopRightX],
                                      mRadii[eCornerTopRightY],
                                      mRadii[eCornerBottomRightX],
                                      mRadii[eCornerBottomRightY],
                                      aBStart,
                                      aBEnd);
...
nsFloatManager::EllipseShapeInfo::LineLeft(const nscoord aBStart,
                                           const nscoord aBEnd) const
{
  nscoord lineLeftDiff = ComputeEllipseLineInterceptDiff(BStart(),
                                                         BEnd(),
                                                         mRadii.width,
                                                         mRadii.height,
                                                         mRadii.width,
...
nsFloatManager::EllipseShapeInfo::LineRight(const nscoord aBStart,
                                            const nscoord aBEnd) const
{
  nscoord lineRightDiff = ComputeEllipseLineInterceptDiff(BStart(),
                                                          BEnd(),
                                                          mRadii.width,
                                                          mRadii.height,
                                                          mRadii.width,
  ...
  if (shapeOutside.GetType() == StyleShapeSourceType::Box) {
    ...
    switch (basicShape->GetShapeType()) {
      case StyleBasicShapeType::Polygon:
        mShapeInfo = ShapeInfo::CreatePolygon(
            basicShape, shapeBoxRect, aWM, aContainerSize);
        break;
      case StyleBasicShapeType::Circle:
      case StyleBasicShapeType::Ellipse:
        mShapeInfo = ShapeInfo::CreateCircleOrEllipse(
            basicShape, shapeBoxRect, aWM, aContainerSize);
        break;
      case StyleBasicShapeType::Inset:
        mShapeInfo = ShapeInfo::CreateInset(
            basicShape, shapeBoxRect, aWM, aContainerSize);
        break;
    }
  } else {
...
nsFloatManager::ShapeInfo::ComputeEllipseLineInterceptDiff(
    ...
    const nscoord aShapeBoxBEnd,
    const nscoord aBStartCornerRadiusL,
    const nscoord aBStartCornerRadiusB,
    const nscoord aBEndCornerRadiusL,
    const nscoord aBEndCornerRadiusB,
    const nscoord aBandBStart,
    const nscoord aBandBEnd)
{
...
/* static */ nsPoint
nsFloatManager::ShapeInfo::ConvertToFloatLogical(const nsPoint& aPoint,
                                                 WritingMode aWM,
                                                 const nsSize& aContainerSize)
{
