...
/** If a child is added or removed or changes size
  */
void
nsGridRowLeafLayout::ChildAddedOrRemoved(nsIFrame* aBox,
...
void
nsGridRowLeafLayout::PopulateBoxSizes(nsIFrame* aBox,
                                      nsBoxLayoutState& aState,
                                      nsBoxSize*& aBoxSizes,
                                      nscoord& aMinSize,
                                      nscoord& aMaxSize,
                                      int32_t& aFlexes)
{
  ...
  if (grid) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      grid->GetFirstAndLastRow(
          firstIndex, lastIndex, firstRow, lastRow, !isHorizontal);
  ...
  nsSprocketLayout::PopulateBoxSizes(
      aBox, aState, aBoxSizes, aMinSize, aMaxSize, aFlexes);
}
  ...
  if (aBox) {
    ...
    if (diff > 0) {
      ...
      nsSprocketLayout::ComputeChildSizes(
          aBox, aState, aGivenSize, aBoxSizes, aComputedBoxSizes);
...
nsGridRowLeafLayout::DirtyRows(nsIFrame* aBox, nsBoxLayoutState& aState)
{
  if (aBox) {
    ...
    aState.PresShell()->FrameNeedsReflow(
        aBox, nsIPresShell::eTreeChange, NS_FRAME_IS_DIRTY);
  }
}

void
nsGridRowLeafLayout::CountRowsColumns(nsIFrame* aBox,
                                      int32_t& aRowCount,
