...
void
nsGridRowGroupLayout::ChildAddedOrRemoved(nsIFrame* aBox,
...
void
nsGridRowGroupLayout::AddWidth(nsSize& aSize,
                               nscoord aSize2,
                               bool aIsHorizontal)
{
...
nsGridRowGroupLayout::GetXULMaxSize(nsIFrame* aBox, nsBoxLayoutState& aState)
{
  ...
  if (grid) {
    ...
    int32_t start = grid->GetColumnCount(isHorizontal) -
                    grid->GetExtraColumnCount(isHorizontal);
    for (int32_t i = 0; i < extraColumns; i++) {
      nscoord max = grid->GetMaxRowHeight(
          aState, i + start, !isHorizontal);  // GetMaxColumnWidth
...
nsGridRowGroupLayout::GetXULMinSize(nsIFrame* aBox, nsBoxLayoutState& aState)
{
  ...
  if (grid) {
    ...
    int32_t start = grid->GetColumnCount(isHorizontal) -
                    grid->GetExtraColumnCount(isHorizontal);
    for (int32_t i = 0; i < extraColumns; i++) {
      nscoord min = grid->GetMinRowHeight(
          aState, i + start, !isHorizontal);  // GetMinColumnWidth
...
void
nsGridRowGroupLayout::CountRowsColumns(nsIFrame* aBox,
                                       int32_t& aRowCount,
