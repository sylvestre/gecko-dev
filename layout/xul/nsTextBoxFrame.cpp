                                 ...
                                 bool& aRedraw)
{
  ...
  if (aAttribute == nullptr || aAttribute == nsGkAtoms::crop) {
    static nsIContent::AttrValuesArray strings[] = {&nsGkAtoms::left,
                                                    &nsGkAtoms::start,
                                                    &nsGkAtoms::center,
                                                    &nsGkAtoms::right,
                                                    &nsGkAtoms::end,
...
void
nsDisplayXULTextBox::Paint(nsDisplayListBuilder* aBuilder, gfxContext* aCtx)
{
                                        ...
                                        const nscolor* aColor)
{
  static_cast<nsTextBoxFrame*>(mFrame)->PaintTitle(
      *aCtx, mVisibleRect, ToReferenceFrame() + aOffset, aColor);
}
                         ...
                         const nscolor* aOverrideColor)
{
  ...
  do {  // find decoration colors
    ...
    if (decorMask &
      ...
      if (aOverrideColor) {
        ...
        color = context->StyleColor()->CalcComplexColor(
            styleText->mTextDecorationColor);
      }
  ...
  if (mState & NS_FRAME_IS_BIDI) {
    ...
    if (mAccessKeyInfo && mAccessKeyInfo->mAccesskeyIndex != kNotFound) {
      ...
      rv = nsBidiPresUtils::RenderText(mCroppedTitle.get(),
  ...
  if (NS_FAILED(rv)) {
    ...
    if (mAccessKeyInfo && mAccessKeyInfo->mAccesskeyIndex != kNotFound) {
      ...
      if (mAccessKeyInfo->mAccesskeyIndex > 0)
        mAccessKeyInfo->mBeforeWidth =
            nsLayoutUtils::AppUnitWidthOfString(mCroppedTitle.get(),
    ...
    fontMet->DrawString(mCroppedTitle.get(),
                        mCroppedTitle.Length(),
                        baselinePt.x,
                        baselinePt.y,
                        &aRenderingContext,
                                   ...
                                   nsFontMetrics& aFontMetrics)
{
  if (mAccessKeyInfo && mAccessKeyInfo->mAccesskeyIndex != kNotFound) {
    ...
    mAccessKeyInfo->mAccessWidth = nsLayoutUtils::AppUnitWidthOfString(
        titleString[mAccessKeyInfo->mAccesskeyIndex],
        aFontMetrics,
        aDrawTarget);
      ...
      if (!AlwaysAppendAccessKey()) {
        ...
        if (!found) {
          ...
          found = FindInReadable(
              mAccessKey, start, end, nsCaseInsensitiveStringComparator());
        }
      } else {
                            ...
                            nscoord& aAscent)
{
  ...
  aSize.width = nsLayoutUtils::AppUnitWidthOfStringBidi(
      aString, this, *fontMet, aRenderingContext);
...
nsTextBoxFrame::CalcDrawRect(gfxContext& aRenderingContext)
{
  ...
  if (accService) {
    accService->UpdateLabelValue(
        PresContext()->PresShell(), mContent, mCroppedTitle);
  }
#endif
