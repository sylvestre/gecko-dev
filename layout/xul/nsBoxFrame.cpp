               ...
               nsBoxLayout* aLayoutManager)
{
  return new (aPresShell)
      nsBoxFrame(aContext, nsBoxFrame::kClassID, aIsRoot, aLayoutManager);
}
...
nsBoxFrame::nsBoxFrame(nsStyleContext* aContext,
                       ...
                       bool aIsRoot,
                       nsBoxLayout* aLayoutManager)
    : nsContainerFrame(aContext, aID), mFlex(0), mAscent(0)
{
...
void
nsBoxFrame::SetInitialChildList(ChildListID aListID, nsFrameList& aChildList)
{
...
void
nsBoxFrame::UpdateMouseThrough()
{
  if (mContent) {
    static nsIContent::AttrValuesArray strings[] = {
        &nsGkAtoms::never, &nsGkAtoms::always, nullptr};
    switch (mContent->FindAttrValueIn(
...
nsBoxFrame::GetInitialDebug(bool& aDebug)
{
  ...
  int32_t index = GetContent()->FindAttrValueIn(
      kNameSpaceID_None, nsGkAtoms::debug, strings, eCaseMatters);
  if (index >= 0) {
    aDebug = index == 1;
    return true;
  }
...
nsBoxFrame::GetInitialHAlignment(nsBoxFrame::Halignment& aHalign)
{
  ...
  int32_t index = GetContent()->FindAttrValueIn(
      kNameSpaceID_None, nsGkAtoms::align, alignStrings, eCaseMatters);
  if (index >= 0) {
  ...
  static nsIContent::AttrValuesArray strings[] = {&nsGkAtoms::_empty,
                                                  &nsGkAtoms::start,
                                                  &nsGkAtoms::center,
                                                  &nsGkAtoms::end,
                                                  nullptr};
  ...
  index = GetContent()->FindAttrValueIn(
      kNameSpaceID_None, attrName, strings, eCaseMatters);
...
nsBoxFrame::GetInitialVAlignment(nsBoxFrame::Valignment& aValign)
{
  ...
  static nsIContent::AttrValuesArray valignStrings[] = {&nsGkAtoms::top,
                                                        &nsGkAtoms::baseline,
                                                        &nsGkAtoms::middle,
                                                        &nsGkAtoms::bottom,
                                                        nullptr};
  ...
  int32_t index = GetContent()->FindAttrValueIn(
      kNameSpaceID_None, nsGkAtoms::valign, valignStrings, eCaseMatters);
  if (index >= 0) {
  ...
  static nsIContent::AttrValuesArray strings[] = {&nsGkAtoms::_empty,
                                                  ...
                                                  &nsGkAtoms::end,
                                                  nullptr};
  static const Valignment values[] = {vAlign_Top /*not used*/,
                                      vAlign_Top,
                                      vAlign_Middle,
                                      vAlign_BaseLine,
                                      vAlign_Bottom};
...
static void
printSize(char* aDesc, nscoord aSize)
{
  printf(" %s: ", aDesc);
                   ...
                   nsReflowStatus& aStatus)
{
  ...
  nsBoxLayoutState state(aPresContext,
                         ...
                         &aReflowInput,
                         aReflowInput.mReflowDepth);

  WritingMode wm = aReflowInput.GetWritingMode();
  LogicalSize computedSize(
      wm, aReflowInput.ComputedISize(), aReflowInput.ComputedBSize());
...
nsBoxFrame::GetXULMinSize(nsBoxLayoutState& aBoxLayoutState)
{
  ...
  // if the size was not completely redefined in CSS then ask our children
  bool widthSet, heightSet;
  if (!nsIFrame::AddXULMinSize(
          aBoxLayoutState, this, size, widthSet, heightSet)) {
    if (mLayoutManager) {
...
void
nsBoxFrame::RemoveFrame(ChildListID aListID, nsIFrame* aOldFrame)
{
  NS_PRECONDITION(aListID == kPrincipalList,
...
void
nsBoxFrame::AppendFrames(ChildListID aListID, nsFrameList& aFrameList)
{
  NS_PRECONDITION(aListID == kPrincipalList,
                  "We don't support out-of-flow kids");
  nsBoxLayoutState state(PresContext());
...
class nsDisplayXULDebug : public nsDisplayItem
{
 public:
  ...
  virtual ~nsDisplayXULDebug() { MOZ_COUNT_DTOR(nsDisplayXULDebug); }
#endif
                       ...
                       nsTArray<nsIFrame*>* aOutFrames)
  {
    ...
    static_cast<nsBoxFrame*>(mFrame)->DisplayDebugInfoFor(
        this, rectCenter - ToReferenceFrame());
...
void
nsDisplayXULDebug::Paint(nsDisplayListBuilder* aBuilder, gfxContext* aCtx)
{
  static_cast<nsBoxFrame*>(mFrame)->PaintXULDebugOverlay(*aCtx->GetDrawTarget(),
                                                         ToReferenceFrame());
}

static void
PaintXULDebugBackground(nsIFrame* aFrame,
                             ...
                             const nsDisplayListSet& aLists)
{
...
#ifdef DEBUG_LAYOUT
  if (mState & NS_STATE_CURRENTLY_IN_DEBUG) {
    destination.BorderBackground()->AppendNewToTop(
        new (aBuilder) nsDisplayGeneric(
            aBuilder, this, PaintXULDebugBackground, "XULDebugBackground"));
    destination.Outlines()->AppendNewToTop(
        new (aBuilder) nsDisplayXULDebug(aBuilder, this));
  }
#endif
  ...
  if (forceLayer) {
    ...
    aLists.Content()->AppendNewToTop(new (aBuilder) nsDisplayOwnLayer(
...
#ifdef DEBUG_LAYOUT
void
nsBoxFrame::DrawLine(DrawTarget& aDrawTarget,
                     bool aHorizontal,
                     nscoord x1,
                     nscoord y1,
                     nscoord x2,
                     nscoord y2)
{
  ...
  StrokeLineWithSnapping(
      p1, p2, PresContext()->AppUnitsPerDevPixel(), aDrawTarget, color);
}

void
nsBoxFrame::FillRect(DrawTarget& aDrawTarget,
                     bool aHorizontal,
                     nscoord x,
                     nscoord y,
                     nscoord width,
                     nscoord height)
{
  Rect rect = NSRectToSnappedRect(
      aHorizontal ? nsRect(x, y, width, height) : nsRect(y, x, height, width),
...
void
nsBoxFrame::DrawSpacer(nsPresContext* aPresContext,
                       DrawTarget& aDrawTarget,
                       bool aHorizontal,
                       int32_t flex,
                       nscoord x,
...
void
nsBoxFrame::GetValue(nsPresContext* aPresContext,
                     const nsSize& a,
                     const nsSize& b,
                     char* ch)
{
  ...
  sprintf(ch,
          "(%s%s, %s%s)",
          width,
          (b.width != NS_INTRINSICSIZE ? "[SET]" : ""),
          height,
          (b.height != NS_INTRINSICSIZE ? "[SET]" : ""));
}
...
nsresult
nsBoxFrame::DisplayDebugInfoFor(nsIFrame* aBox, nsPoint& aPoint)
{
  ...
  insideBorder.MoveTo(0, 0) : nsMargin border(0, 0, 0, 0);
  aBox->GetXULBorderAndPadding(border);
  insideBorder.Deflate(border);

  bool isHorizontal = IsXULHorizontal();

  if (!insideBorder.Contains(nsPoint(x, y))) return NS_ERROR_FAILURE;
  ...
  if ((isHorizontal && y < insideBorder.y + m.top) ||
    ...
    while (child) {
      ...
      if ((isHorizontal && x >= r.x && x < r.x + r.width) ||
        ...
        if (mDebugChild == child) return NS_OK;
...
void
nsBoxFrame::SetDebugOnChildList(nsBoxLayoutState& aState,
                                nsIFrame* aChild,
                                bool aDebug)
{
...
bool
IsBoxOrdinalLEQ(nsIFrame* aFrame1, nsIFrame* aFrame2)
{
...
nsresult
nsBoxFrame::LayoutChildAt(nsBoxLayoutState& aState,
                          nsIFrame* aBox,
                          const nsRect& aRect)
{
...
{
  if (!SupportsOrdinalsInChildren()) return NS_OK;
...
void
nsDisplayXULEventRedirector::HitTest(nsDisplayListBuilder* aBuilder,
                                     const nsRect& aRect,
                                     HitTestState* aState,
                                     nsTArray<nsIFrame*>* aOutFrames)
{
...
class nsXULEventRedirectorWrapper final : public nsDisplayWrapper
{
 public:
  ...
  {
    return new (aBuilder) nsDisplayXULEventRedirector(
