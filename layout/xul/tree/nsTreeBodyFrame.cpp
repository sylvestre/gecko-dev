...
nsTreeBodyFrame::CancelImageRequests()
{
  for (auto iter = mImageCache.Iter(); !iter.Done(); iter.Next()) {
    ...
    nsLayoutUtils::DeregisterImageRequest(
        PresContext(), entry.request, nullptr);
...
void
nsTreeBodyFrame::SetXULBounds(nsBoxLayoutState& aBoxLayoutState,
                              const nsRect& aRect,
...
nsTreeBodyFrame::ReflowFinished()
{
  ...
  if (mView) {
    ...
    if (treeContent && treeContent->AttrValueIs(kNameSpaceID_None,
...
nsTreeBodyFrame::SetView(nsITreeView* aView)
{
  ...
  if (treeContent) {
    ...
    if (accService)
      accService->TreeViewChanged(
          PresContext()->GetPresShell(), treeContent, mView);
#endif
...
nsTreeBodyFrame::InvalidateCell(int32_t aIndex, nsITreeColumn* aCol)
{
  ...
  nsRect cellRect;
  nsresult rv = col->GetRect(
      this, mInnerBox.y + mRowHeight * aIndex, mRowHeight, &cellRect);
  NS_ENSURE_SUCCESS(rv, rv);
...
nsresult
nsTreeBodyFrame::InvalidateColumnRange(int32_t aStart,
                                       int32_t aEnd,
                                       nsITreeColumn* aCol)
{
  if (mUpdateBatchNest) return NS_OK;
...
nsTreeBodyFrame::GetScrollParts()
{
...
nsTreeBodyFrame::UpdateScrollbars(const ScrollParts& aParts)
{
  ...
  if (aParts.mVScrollbar) {
    ...
    aParts.mVScrollbarContent->SetAttr(
        kNameSpaceID_None, nsGkAtoms::curpos, curPos, true);
  ...
  if (weakFrame.IsAlive() && aParts.mHScrollbar) {
    ...
    aParts.mHScrollbarContent->SetAttr(
        kNameSpaceID_None, nsGkAtoms::curpos, curPos, true);
    // 'this' might be deleted here
  }
                                      ...
                                      AutoWeakFrame& aWeakColumnsFrame)
{
  ...
  if (aParts.mVScrollbar) {
    ...
    aParts.mVScrollbarContent->SetAttr(
        kNameSpaceID_None, nsGkAtoms::maxpos, maxposStr, true);
    ENSURE_TRUE(weakFrame.IsAlive());
    ...
    aParts.mVScrollbarContent->SetAttr(
        kNameSpaceID_None, nsGkAtoms::pageincrement, pageStr, true);
    ENSURE_TRUE(weakFrame.IsAlive());
  }
  ...
  if (aParts.mHScrollbar && aParts.mColumnsFrame &&
    ...
    aParts.mHScrollbarContent->SetAttr(
        kNameSpaceID_None, nsGkAtoms::increment, pageStr, true);
  }
...
nsresult
nsTreeBodyFrame::GetCellAt(int32_t aX,
                           int32_t aY,
                           int32_t* aRow,
                           nsITreeColumn** aCol,
...
//
nsresult
nsTreeBodyFrame::GetCoordsForCellItem(int32_t aRow,
                                      nsITreeColumn* aCol,
                                      ...
                                      int32_t* aHeight)
{
  ...
  *aHeight = 0;
  ...
  for (nsTreeColumn* currCol = mColumns->GetFirstColumn(); currCol;
    ...
    nsRect cellRect(currX,
                    mInnerBox.y + mRowHeight * (aRow - mTopRowIndex),
                    colWidth,
                    mRowHeight);
    ...
    if (currCol->IsPrimary()) {
      ...
      GetTwistyRect(
          aRow, currCol, imageRect, twistyRect, presContext, twistyContext);
                                   ...
                                   nsRect& aTextRect)
{
      ...
      switch (aColumn->GetCropStyle()) {
        ...
        case 0: {
          ...
          for (i = 0; i < length; ++i) {
            ...
            cwidth = nsLayoutUtils::AppUnitWidthOfString(
                ch, aFontMetrics, drawTarget);
  ...
  width = nsLayoutUtils::AppUnitWidthOfStringBidi(
...
nsICSSAnonBoxPseudo*
nsTreeBodyFrame::GetItemWithinCellAt(nscoord aX,
                                     ...
                                     nsTreeColumn* aColumn)
{
  ...
  if (aColumn->IsPrimary()) {
    ...
    nsRect imageSize;
    GetTwistyRect(
        aRowIndex, aColumn, imageSize, twistyRect, presContext, twistyContext);
...
void
nsTreeBodyFrame::GetCellAt(nscoord aX,
                           nscoord aY,
                           int32_t* aRow,
                           nsTreeColumn** aCol,
...
nsresult
nsTreeBodyFrame::GetCellWidth(int32_t aRow,
                              nsTreeColumn* aCol,
                              gfxContext* aRenderingContext,
                              ...
                              nscoord& aCurrentSize)
{
  ...
  if (aCol->IsPrimary()) {
    ...
    nsRect twistyRect(cellRect);
    GetTwistyRect(
        aRow, aCol, imageSize, twistyRect, PresContext(), twistyContext);
  ...
  nscoord width = nsLayoutUtils::AppUnitWidthOfStringBidi(
      cellText, this, *fm, *aRenderingContext);
                               ...
                               nsStyleContext* aTwistyContext)
{
  ...
  if (twistyDisplayData->mAppearance) {
    ...
    if (theme && theme->ThemeSupportsWidget(
                     aPresContext, nullptr, twistyDisplayData->mAppearance))
      useTheme = true;
  }
  ...
  if (useTheme) {
    ...
    theme->GetMinimumWidgetSize(aPresContext,
                                this,
...
nsresult
nsTreeBodyFrame::GetImage(int32_t aRowIndex,
                          nsTreeColumn* aCol,
                          bool aUseContext,
                          ...
                          imgIContainer** aResult)
{
  *aResult = nullptr;
  ...
  if (mImageCache.Get(imageSrc, &entry)) {
    ...
    imgReq->GetImage(
        aResult);  // We hand back the image here.  The GetImage call addrefs *aResult.
  ...
  if (!*aResult) {
    ...
    if (styleRequest) {
      styleRequest->SyncClone(
          imgNotificationObserver, doc, getter_AddRefs(imageRequest));
    } else {
      nsCOMPtr<nsIURI> baseURI = mContent->GetBaseURI();

      nsCOMPtr<nsIURI> srcURI;
      nsContentUtils::NewURIWithDocumentCharset(
          getter_AddRefs(srcURI), imageSrc, doc, baseURI);
...
nsRect
nsTreeBodyFrame::GetImageSize(int32_t aRowIndex,
                              nsTreeColumn* aCol,
                              bool aUseContext,
                              nsStyleContext* aStyleContext)
{
...
int32_t
nsTreeBodyFrame::GetIndentation()
{
...
nsresult
nsTreeBodyFrame::GetCursor(const nsPoint& aPoint, nsIFrame::Cursor& aCursor)
{
...
static uint32_t
GetDropEffect(WidgetGUIEvent* aEvent)
{
                             ...
                             nsEventStatus* aEventStatus)
{
  if (aEvent->mMessage == eMouseOver || aEvent->mMessage == eMouseMove) {
    ...
    ComputeDropPosition(
        aEvent, &mSlots->mDropRow, &mSlots->mDropOrient, &mSlots->mScrollLines);
    ...
    mView->Drop(
        mSlots->mDropRow, mSlots->mDropOrient, dragEvent->mDataTransfer);
    mSlots->mDropRow = -1;
    mSlots->mDropOrient = -1;
...
class nsDisplayTreeBody final : public nsDisplayItem
{
 public:
                                 ...
                                 nsRegion* aInvalidRegion) const override
  {
    ...
    nsDisplayItem::ComputeInvalidationRegion(
  ...
  virtual void Paint(nsDisplayListBuilder* aBuilder, gfxContext* aCtx) override
  {
    ...
    DrawResult result = static_cast<nsTreeBodyFrame*>(mFrame)->PaintTreeBody(
                               ...
                               nsDisplayListBuilder* aBuilder)
{
  ...
  if (!mHasFixedRowCount) mPageLength = mInnerBox.height / mRowHeight;
  ...
  for (nsTreeColumn* currCol = mColumns->GetFirstColumn(); currCol;
    ...
    if (OffsetForHorzScroll(colRect, false)) {
      ...
      if (dirtyRect.IntersectRect(aDirtyRect, colRect)) {
        result &= PaintColumn(
            currCol, colRect, PresContext(), aRenderingContext, aDirtyRect);
      }
    }
  }
  // Loop through our on-screen rows.
  for (int32_t i = mTopRowIndex;
       i < mRowCount && i <= mTopRowIndex + mPageLength;
       i++) {
    nsRect rowRect(mInnerBox.x,
                   mInnerBox.y + mRowHeight * (i - mTopRowIndex),
                   mInnerBox.width,
                   mRowHeight);
    nsRect dirtyRect;
  ...
  if (mSlots && mSlots->mDropAllowed &&
    ...
    if (dirtyRect.IntersectRect(aDirtyRect, feedbackRect)) {
      result &= PaintDropFeedback(
          feedbackRect, PresContext(), aRenderingContext, aDirtyRect, aPt);
    }
  }
                             ...
                             const nsRect& aDirtyRect)
{
  ...
  return PaintBackgroundLayer(
      colContext, aPresContext, aRenderingContext, colRect, aDirtyRect);
}
                          ...
                          nsDisplayListBuilder* aBuilder)
{
  ...
  aRenderingContext.SetFontSmoothingBackgroundColor(ToDeviceColor(
      rowContext->StyleUserInterface()->mFontSmoothingBackgroundColor));
  if (theme && theme->ThemeSupportsWidget(aPresContext, nullptr, appearance)) {
    ...
    theme->DrawWidgetBackground(
        &aRenderingContext, this, appearance, rowRect, dirty);
  } else {
    result &= PaintBackgroundLayer(
        rowContext, aPresContext, aRenderingContext, rowRect, aDirtyRect);
  }
  ...
  if (isSeparator) {
    ...
    if (primaryCol) {
      ...
      if (currX > rowRect.x) {
        ...
        result &= PaintSeparator(aRowIndex,
                                 separatorRect,
                                 aPresContext,
                                 aRenderingContext,
                                 aDirtyRect);
    ...
    result &= PaintSeparator(
        aRowIndex, separatorRect, aPresContext, aRenderingContext, aDirtyRect);
  } else {
                                ...
                                const nsRect& aDirtyRect)
{
  ...
  if (displayData->mAppearance) {
    theme = aPresContext->GetTheme();
    if (theme && theme->ThemeSupportsWidget(
                     aPresContext, nullptr, displayData->mAppearance))
      useTheme = true;
  }
  ...
  if (useTheme) {
    ...
    nsRect separatorRect(
        aSeparatorRect.x, aSeparatorRect.y, aSeparatorRect.width, height);
    ...
    result &= PaintBackgroundLayer(separatorContext,
                                   aPresContext,
                                   aRenderingContext,
                                   separatorRect,
                                   aDirtyRect);
                           ...
                           nsDisplayListBuilder* aBuilder)
{
  ...
  // Paint our borders and background for our row rect.
  DrawResult result = PaintBackgroundLayer(
    ...
    if (mIndentation && level &&
      ...
      if (srcX <= cellRect.x + cellRect.width) {
        ...
        SnapLineToDevicePixelsForStroking(
            p1, p2, *drawTarget, strokeOptions.mLineWidth);
    ...
    if (dirtyRect.IntersectRect(aDirtyRect, elementRect)) {
      switch (aColumn->GetType()) {
        ...
        case nsITreeColumn::TYPE_CHECKBOX:
          result &= PaintCheckbox(aRowIndex,
                                  aColumn,
                                  elementRect,
                                  aPresContext,
                                  aRenderingContext,
        ...
        case nsITreeColumn::TYPE_PROGRESSMETER:
          ...
          switch (state) {
            ...
            case nsITreeView::PROGRESS_UNDETERMINED:
              result &= PaintProgressMeter(aRowIndex,
                                           aColumn,
                                           elementRect,
                                           aPresContext,
                                           aRenderingContext,
                             ...
                             nscoord& aCurrX)
{
  ...
  nsRect imageSize;
  nsITheme* theme = GetTwistyRect(
      aRowIndex, aColumn, imageSize, twistyRect, aPresContext, twistyContext);
  ...
  if (shouldPaint) {
    ...
    result &= PaintBackgroundLayer(
        twistyContext, aPresContext, aRenderingContext, twistyRect, aDirtyRect);
                            ...
                            nsDisplayListBuilder* aBuilder)
{
  ...
  if (image) {
    ...
    // Paint our borders and background for our image rect
    result &= PaintBackgroundLayer(
        imageContext, aPresContext, aRenderingContext, imageRect, aDirtyRect);
                           ...
                           nscoord& aCurrX)
{
  ...
  // Set our font.
  AdjustForCellText(
      text, aRowIndex, aColumn, aRenderingContext, *fontMet, textRect);
  textRect.Inflate(bp);
  ...
  result &= PaintBackgroundLayer(
      textContext, aPresContext, aRenderingContext, textRect, aDirtyRect);
  ...
  if (decorations & (NS_STYLE_TEXT_DECORATION_LINE_OVERLINE |
    ...
    if (decorations & NS_STYLE_TEXT_DECORATION_LINE_UNDERLINE) {
      nsRect r(
          textRect.x, textRect.y + baseline - offset, textRect.width, size);
      Rect devPxRect = NSRectToSnappedRect(r, appUnitsPerDevPixel, *drawTarget);
      drawTarget->FillRect(devPxRect, color);
    }
  }
                               ...
                               const nsRect& aDirtyRect)
{
  ...
  // Paint our borders and background for our image rect.
  DrawResult result = PaintBackgroundLayer(checkboxContext,
                                           aPresContext,
                                           aRenderingContext,
                                           checkboxRect,
                                           aDirtyRect);
  ...
  if (image) {
    ...
    // Paint the image.
    result &= nsLayoutUtils::DrawSingleUnscaledImage(aRenderingContext,
                                                     aPresContext,
                                                     image,
                                                     SamplingFilter::POINT,
                                                     pt,
                                    ...
                                    nsDisplayListBuilder* aBuilder)
{
  ...
  nsStyleContext* meterContext =
      GetPseudoStyleContext(nsCSSAnonBoxes::mozTreeProgressmeter);
  ...
  DrawResult result = PaintBackgroundLayer(
      meterContext, aPresContext, aRenderingContext, meterRect, aDirtyRect);
  ...
  if (state == nsITreeView::PROGRESS_NORMAL) {
    ...
    nscoord meterWidth =
        NSToCoordRound((float)intValue / 100 * meterRect.width);
    if (StyleVisibility()->mDirection == NS_STYLE_DIRECTION_RTL)
                                   ...
                                   nsPoint aPt)
{
  ...
  if (feedbackContext->StyleVisibility()->IsVisibleOrCollapsed()) {
    ...
    result &= PaintBackgroundLayer(feedbackContext,
                                   aPresContext,
                                   aRenderingContext,
                                   feedbackRect,
                                   aDirtyRect);
                                      ...
                                      const nsRect& aDirtyRect)
{
  ...
  DrawResult result = nsCSSRendering::PaintStyleImageLayerWithSC(
  ...
  nsCSSRendering::PaintOutline(
      aPresContext, aRenderingContext, this, aDirtyRect, aRect, aStyleContext);
...
nsresult
nsTreeBodyFrame::EnsureRowIsVisibleInternal(const ScrollParts& aParts,
                                            int32_t aRow)
{
...
nsresult
nsTreeBodyFrame::ScrollToColumnInternal(const ScrollParts& aParts,
                                        nsITreeColumn* aCol)
{
  RefPtr<nsTreeColumn> col = GetColumnImpl(aCol);
  if (!col) return NS_ERROR_INVALID_ARG;
...
nsresult
nsTreeBodyFrame::ScrollHorzInternal(const ScrollParts& aParts,
                                    int32_t aPosition)
{
...
void
nsTreeBodyFrame::ScrollByPage(nsScrollbarFrame* aScrollbar,
                              int32_t aDirection,
...
void
nsTreeBodyFrame::ScrollByWhole(nsScrollbarFrame* aScrollbar,
                               int32_t aDirection,
                               nsIScrollbarMediator::ScrollSnapMode aSnap)
{
  // CSS Scroll Snapping is not enabled for XUL, aSnap is ignored
...
void
nsTreeBodyFrame::ScrollByLine(nsScrollbarFrame* aScrollbar,
                              int32_t aDirection,
                              nsIScrollbarMediator::ScrollSnapMode aSnap)
{
  // CSS Scroll Snapping is not enabled for XUL, aSnap is ignored
...
{
  return mStyleCache.GetStyleContext(
      PresContext(), mContent, mStyleContext, aPseudoElement, mScratchArray);
}
...
nsTreeBodyFrame::FireRowCountChangedEvent(int32_t aIndex, int32_t aCount)
{
  ...
  domDoc->CreateEvent(NS_LITERAL_STRING("customevent"), getter_AddRefs(event));
  ...
  treeEvent->InitCustomEvent(
      NS_LITERAL_STRING("TreeRowCountChanged"), true, false, detailVariant);
...
void
nsTreeBodyFrame::FireInvalidateEvent(int32_t aStartRowIdx,
                                     int32_t aEndRowIdx,
                                     ...
                                     nsITreeColumn* aEndCol)
{
  ...
  detailVariant->SetAsISupports(propBag);
  treeEvent->InitCustomEvent(
      NS_LITERAL_STRING("TreeInvalidated"), true, false, detailVariant);
