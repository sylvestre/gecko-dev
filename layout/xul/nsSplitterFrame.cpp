...
class nsSplitterFrameInner final : public nsIDOMEventListener
{
 ...
 public:
  ...
  void EnsureOrient();
  void SetPreferredSize(nsBoxLayoutState& aState,
                        nsIFrame* aChildBox,
                        nscoord aOnePixel,
                        bool aIsHorizontal,
                        nscoord* aSize);
...
nsSplitterFrameInner::GetResizeBefore()
{
  ...
  switch (mOuter->GetContent()->FindAttrValueIn(
...
nsresult
nsSplitterFrame::GetCursor(const nsPoint& aPoint, nsIFrame::Cursor& aCursor)
{
  ...
  if (aParent && aParent->IsXULBoxFrame()) {
    if (!aParent->IsXULHorizontal()) {
      if (!nsContentUtils::HasNonEmptyAttr(
              aContent, kNameSpaceID_None, nsGkAtoms::orient)) {
                                ...
                                WidgetGUIEvent* aEvent)
{
  if (mDragging && mOuter) {
    ...
    ResizeChildTo(pos,
                  mChildInfosBefore.get(),
                  mChildInfosAfter.get(),
    ...
    if (isCollapsedBefore || isCollapsedAfter) {
      ...
      if (currentState == Dragging) {
        if (pastEnd) {
          ...
          if (supportsAfter) {
            ...
            outer->SetAttr(kNameSpaceID_None,
                           nsGkAtoms::substate,
      ...
      // we are dragging.
      if (currentState != Dragging)
        mOuter->mContent->SetAttr(kNameSpaceID_None,
                                  nsGkAtoms::state,
...
{
  mOuter->GetContent()->AddEventListener(
      NS_LITERAL_STRING("mouseup"), this, false, false);
  mOuter->GetContent()->AddEventListener(
      NS_LITERAL_STRING("mousedown"), this, false, false);
  mOuter->GetContent()->AddEventListener(
      NS_LITERAL_STRING("mousemove"), this, false, false);
  mOuter->GetContent()->AddEventListener(
      NS_LITERAL_STRING("mouseout"), this, false, false);
}
...
{
  ENSURE_TRUE(mOuter);
  mOuter->GetContent()->RemoveEventListener(
      NS_LITERAL_STRING("mouseup"), this, false);
  mOuter->GetContent()->RemoveEventListener(
      NS_LITERAL_STRING("mousedown"), this, false);
  mOuter->GetContent()->RemoveEventListener(
      NS_LITERAL_STRING("mousemove"), this, false);
  mOuter->GetContent()->RemoveEventListener(
      NS_LITERAL_STRING("mouseout"), this, false);
}
...
nsSplitterFrameInner::MouseDown(nsIDOMEvent* aMouseEvent)
{
  ...
  if (mOuter->GetContent()->AttrValueIs(kNameSpaceID_None,
                                        nsGkAtoms::disabled,
                                        nsGkAtoms::_true,
                                        eCaseMatters))
    return NS_OK;
...
nsSplitterFrameInner::MouseMove(nsIDOMEvent* aMouseEvent)
{
  ...
  mOuter->mContent->SetAttr(
      kNameSpaceID_None, nsGkAtoms::state, NS_LITERAL_STRING("dragging"), true);
...
void
nsSplitterFrameInner::Reverse(UniquePtr<nsSplitterInfo[]>& aChildInfos,
                              int32_t aCount)
{
    ...
    nsSplitterFrameInner::CollapseDirection aDirection)
{
  ...
  switch (mOuter->mContent->FindAttrValueIn(
      ...
      if (sibling) {
        if (mState == CollapsedBefore || mState == CollapsedAfter) {
          ...
          nsContentUtils::AddScriptRunner(new nsSetAttrRunnable(
              sibling, nsGkAtoms::collapsed, NS_LITERAL_STRING("true")));
        }
      }
    }
  }
...
static nsIFrame*
GetChildBoxForContent(nsIFrame* aParentBox, nsIContent* aContent)
{
...
void
nsSplitterFrameInner::SetPreferredSize(nsBoxLayoutState& aState,
                                       nsIFrame* aChildBox,
                                       nscoord aOnePixel,
                                       bool aIsHorizontal,
                                       nscoord* aSize)
{
  ...
  if (content->AttrValueIs(
          kNameSpaceID_None, attribute, prefValue, eCaseMatters))
    return;
                                    ...
                                    bool aBounded)
{
  ...
  if (spaceLeft != 0) {
    if (aBounded) {
      aDiff += spaceLeft;
      AddRemoveSpace(
          spaceLeft, aChildrenBeforeInfos, aChildrenBeforeCount, spaceLeft);
    }
  }
}
