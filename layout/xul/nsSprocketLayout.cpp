...
static void
HandleBoxPack(nsIFrame* aBox,
              const nsFrameState& aFrameState,
              nscoord& aX,
              nscoord& aY,
              ...
              const nsRect& aClientRect)
{
  ...
  if (aFrameState & NS_STATE_IS_HORIZONTAL) {
    ...
    switch (valign) {
      ...
      case nsBoxFrame::
          vAlign_BaseLine:  // This value is technically impossible to specify for pack.
...
void
nsSprocketLayout::PopulateBoxSizes(nsIFrame* aBox,
...
void
nsSprocketLayout::AlignChildren(nsIFrame* aBox, nsBoxLayoutState& aState)
{
  nsFrameState frameState = nsFrameState(0);
  GetFrameState(aBox, frameState);
...

{
  ...
  if (recompute)
    ComputeChildSizes(
        aBox, aState, containingWidth, aBoxSizes, aComputedBoxSizes);
...
void
nsSprocketLayout::SetLargestSize(nsSize& aSize1,
                                 const nsSize& aSize2,
                                 bool aIsHorizontal)
{
  if (aIsHorizontal) {
...
void
nsSprocketLayout::SetSmallestSize(nsSize& aSize1,
                                  const nsSize& aSize2,
                                  bool aIsHorizontal)
{
  if (aIsHorizontal) {
...
void
nsSprocketLayout::AddLargestSize(nsSize& aSize,
...
void
nsSprocketLayout::AddSmallestSize(nsSize& aSize,
                                  const nsSize& aSizeToAdd,
                                  bool aIsHorizontal)
{
  if (aIsHorizontal)
...
void*
nsComputedBoxSize::operator new(size_t sz,
