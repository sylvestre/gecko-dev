...
class nsBoxFrame : public nsContainerFrame
{
...
#ifdef DEBUG_LAYOUT
  virtual void SetDebugOnChildList(nsBoxLayoutState& aState,
                                   nsIFrame* aChild,
                                   bool aDebug);
  nsresult DisplayDebugInfoFor(nsIFrame* aBox, nsPoint& aPoint);
#endif

  static nsresult LayoutChildAt(nsBoxLayoutState& aState,
                                nsIFrame* aBox,
                                const nsRect& aRect);
 ...
 private:
  ...
  void GetValue(nsPresContext* aPresContext,
                const nsSize& a,
                const nsSize& b,
                char* value);
  void GetValue(nsPresContext* aPresContext, int32_t a, int32_t b, char* value);
  void DrawSpacer(nsPresContext* aPresContext,
                  ...
                  nscoord size,
                  nscoord spacerSize);
  void DrawLine(DrawTarget& aDrawTarget,
                bool aHorizontal,
                nscoord x1,
                nscoord y1,
                nscoord x2,
                nscoord y2);
  void FillRect(DrawTarget& aDrawTarget,
                bool aHorizontal,
                nscoord x,
                nscoord y,
                nscoord width,
