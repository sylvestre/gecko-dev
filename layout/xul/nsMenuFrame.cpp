 ...
 public:
  ...
  NS_IMETHOD Run() override
  {
    ...
    EventDispatcher::DispatchDOMEvent(
        mMenu, nullptr, event, mPresContext, nullptr);
  ...
  for (nsFrameList::Enumerator e(aFrameList); !e.AtEnd(); e.Next()) {
    ...
    if (popupFrame) {
      ...
      nsFrameList* popupList =
          new (PresContext()->PresShell()) nsFrameList(popupFrame, popupFrame);
...
void
nsMenuFrame::SetInitialChildList(ChildListID aListID, nsFrameList& aChildList)
{
    ...
    if (aDeselectMenu) {
      ...
      if (current) {
        ...
        nsCOMPtr<nsIRunnable> event =
            new nsMenuActivateEvent(current->GetContent(), PresContext(), true);
...
nsMenuFrame::DoXULLayout(nsBoxLayoutState& aState)
{
  ...
  if (popupFrame) {
    ...
    popupFrame->LayoutPopup(
        aState, this, GetAnchor()->GetPrimaryFrame(), sizeToPopup);
  }
  ...
  switch (mContent->FindAttrValueIn(
    ...
    default:
      if (mType != eMenuType_Normal) {
        AutoWeakFrame weakFrame(this);
        mContent->UnsetAttr(kNameSpaceID_None, nsGkAtoms::checked, true);
...
{
  bool newChecked = mContent->AttrValueIs(
...
nsMenuFrame::BuildAcceleratorText(bool aNotify)
{
    ...
    if (!accelString.IsEmpty()) {
      ...
      if (bundleService) {
        ...
        rv = bundleService->CreateBundle(
            "chrome://global/locale/keys.properties", getter_AddRefs(bundle));
  ...
  mContent->SetAttr(
      kNameSpaceID_None, nsGkAtoms::acceltext, accelText, aNotify);
  ENSURE_TRUE(weakFrame.IsAlive());
...
nsMenuFrame::Execute(WidgetGUIEvent* aEvent)
{
  ...
  if (mType == eMenuType_Checkbox || (mType == eMenuType_Radio && !mChecked)) {
    needToFlipChecked = !mContent->AttrValueIs(kNameSpaceID_None,
                                               nsGkAtoms::autocheck,
                                               nsGkAtoms::_false,
                                               eCaseMatters);
  }
...
nsMenuFrame::CreateMenuCommandEvent(WidgetGUIEvent* aEvent, bool aFlipChecked)
{
  ...
  mDelayedMenuCommandEvent = new nsXULMenuCommandEvent(
      mContent, isTrusted, shift, control, alt, meta, userinput, aFlipChecked);
}
...
void
nsMenuFrame::RemoveFrame(ChildListID aListID, nsIFrame* aOldFrame)
{
  nsFrameList* popupList = GetPopupList();
...
void
nsMenuFrame::AppendFrames(ChildListID aListID, nsFrameList& aFrameList)
{
  if (!HasPopup() && (aListID == kPrincipalList || aListID == kPopupList)) {
    SetPopupFrame(aFrameList);
    if (HasPopup()) {
 ...
 */
NS_IMETHODIMP
nsMenuTimerMediator::Notify(nsITimer* aTimer)
{
 ...
 */
void
nsMenuTimerMediator::ClearFrame()
{
  mFrame = nullptr;
}
