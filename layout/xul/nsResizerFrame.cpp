  ...
  switch (aEvent->mMessage) {
    ...
    case eMouseDown: {
      if (aEvent->mClass == eTouchEventClass ||
        ...
        if (contentToResize) {
          ...
          // resizing ourselves
          window->GetPositionAndSize(&mMouseDownRect.x,
                                     &mMouseDownRect.y,
    ...
    case eMouseMove: {
      if (mTrackingMouseMove) {
        ...
        AdjustDimensions(&rect.x,
                         &rect.width,
                         sizeConstraints.mMinSize.width,
                         sizeConstraints.mMaxSize.width,
                         mouseMove.x,
                         direction.mHorizontal);
        AdjustDimensions(&rect.y,
                         &rect.height,
                         sizeConstraints.mMinSize.height,
                         sizeConstraints.mMaxSize.height,
                         mouseMove.y,
                         direction.mVertical);
        ...
        if (window) {
          ...
          if (sm) {
            ...
            sm->ScreenForRect(NSToIntRound(frameRect.x / scale),
                              NSToIntRound(frameRect.y / scale),
                              1,
                              1,
                              getter_AddRefs(screen));
            if (screen) {
              LayoutDeviceIntRect screenRect;
              screen->GetRect(&screenRect.x,
                              &screenRect.y,
                              &screenRect.width,
                              &screenRect.height);
        ...
        if (contentToResize) {
          ...
          ResizeContent(
              contentToResize, direction, sizeInfo, &originalSizeInfo);
...
void
nsResizerFrame::AdjustDimensions(int32_t* aPos,
                                 int32_t* aSize,
                                 int32_t aMinSize,
                                 int32_t aMaxSize,
                                 int32_t aMovement,
...
/* static */ void
nsResizerFrame::ResizeContent(nsIContent* aContent,
                              ...
                              SizeInfo* aOriginalSizeInfo)
{
  ...
  if (aContent->IsXULElement()) {
    if (aOriginalSizeInfo) {
      aContent->GetAttr(
          kNameSpaceID_None, nsGkAtoms::width, aOriginalSizeInfo->width);
      aContent->GetAttr(
          kNameSpaceID_None, nsGkAtoms::height, aOriginalSizeInfo->height);
    }
    // only set the property if the element could have changed in that direction
    if (aDirection.mHorizontal) {
      aContent->SetAttr(
          kNameSpaceID_None, nsGkAtoms::width, aSizeInfo.width, true);
    }
    if (aDirection.mVertical) {
      aContent->SetAttr(
          kNameSpaceID_None, nsGkAtoms::height, aSizeInfo.height, true);
    }
  } else {
    ...
    if (inlineStyleContent) {
      ...
      if (aDirection.mVertical) {
        ...
        decl->SetProperty(
            NS_LITERAL_STRING("height"), heightstr, EmptyString());
      }
    }
  }
}
                                         ...
                                         const SizeInfo& aSizeInfo)
{
  ...
  rv = aContent->SetProperty(nsGkAtoms::_moz_original_size,
                             sizeInfo.get(),
...
{
  static const nsIContent::AttrValuesArray strings[] = {&nsGkAtoms::topleft,
                                                        &nsGkAtoms::top,
                                                        &nsGkAtoms::topright,
                                                        &nsGkAtoms::left,
                                                        &nsGkAtoms::right,
                                                        &nsGkAtoms::bottomleft,
                                                        &nsGkAtoms::bottom,
                                                        &nsGkAtoms::bottomright,
                                                        &nsGkAtoms::bottomstart,
                                                        &nsGkAtoms::bottomend,
                                                        nullptr};

  static const Direction directions[] = {{-1, -1},
                                         {0, -1},
                                         {1, -1},
                                         {-1, 0},
                                         {1, 0},
