...
void
nsDisplaySVGGeometry::HitTest(nsDisplayListBuilder* aBuilder,
                              const nsRect& aRect,
                              HitTestState* aState,
...
void
nsDisplaySVGGeometry::Paint(nsDisplayListBuilder* aBuilder, gfxContext* aCtx)
{
                                   ...
                                   const nsDisplayListSet& aLists)
{
  ...
  aLists.Content()->AppendNewToTop(new (aBuilder)
                                       nsDisplaySVGGeometry(aBuilder, this));
}
...
SVGGeometryFrame::GetFrameForPoint(const gfxPoint& aPoint)
{
  ...
  if (!isHit && (hitTestFlags & SVG_HIT_TEST_STROKE)) {
    ...
    SVGContentUtils::GetStrokeOptions(
        &stroke, content, StyleContext(), nullptr);
                                      ...
                                      uint32_t aFlags)
{
  ...
  if (getStroke &&
    ...
    gotSimpleBounds = element->GetGeometryBounds(
        &simpleBounds, strokeOptions, aToBBoxUserspace);
  }
                         ...
                         imgDrawingParams& aImgParams)
{
  ...
  if (GetStateBits() & NS_STATE_SVG_CLIPPATH_CHILD) {
    ...
    if (path) {
      ...
      drawTarget->Fill(
          path, white, DrawOptions(1.0f, CompositionOp::OP_OVER, aaMode));
    }
    return;
  }
  ...
  if (aRenderComponents & eRenderFill) {
    ...
    nsSVGUtils::MakeFillPatternFor(
        this, aContext, &fillPattern, aImgParams, contextPaint);
  ...
  if ((aRenderComponents & eRenderStroke) &&
    ...
    nsSVGUtils::MakeStrokePatternFor(
        this, aContext, &strokePattern, aImgParams, contextPaint);
    ...
    if (strokePattern.GetPattern()) {
      ...
      DrawOptions drawOptions(1.0f, CompositionOp::OP_OVER, aaMode);
      if (simplePath.IsRect()) {
        drawTarget->StrokeRect(
            simplePath.AsRect(), strokePattern, strokeOptions, drawOptions);
      } else if (simplePath.IsLine()) {
        drawTarget->StrokeLine(simplePath.Point1(),
                               simplePath.Point2(),
                               strokePattern,
                               strokeOptions,
                               drawOptions);
    ...
    if (properties.MarkersExist()) {
      ...
      if (num) {
        ...
        for (uint32_t i = 0; i < num; i++) {
          ...
          if (frame) {
            frame->PaintMark(
                aContext, aTransform, this, mark, strokeWidth, aImgParams);
          }
        }
      }
    }
