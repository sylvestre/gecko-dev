...
SVGAutoRenderState::~SVGAutoRenderState()
{
  ...
  if (mOriginalRenderState) {
    mDrawTarget->AddUserData(
        &sSVGAutoRenderStateKey, mOriginalRenderState, nullptr);
  }
}
                                            ...
                                            const nsRect& aPreFilterRect)
{
  ...
  return nsFilterInstance::GetPostFilterBounds(
      aFrame, nullptr, &aPreFilterRect);
}
...
float
nsSVGUtils::UserSpace(const UserSpaceMetrics& aMetrics,
...
void
nsSVGUtils::DetermineMaskUsage(nsIFrame* aFrame,
                               bool aHandleOpacity,
                               MaskUsage& aUsage)
{
 ...
 private:
  ...
  IntRect ComputeClipExtsInDeviceSpace(const gfxMatrix& aTransform)
  {
    ...
    if (!(mFrame->GetStateBits() & NS_FRAME_IS_NONDISPLAY)) {
      ...
      mSourceCtx->Clip(
          NSRectToSnappedRect(overflowRect,
                                  ...
                                  const nsIntRect* aDirtyRect)
{
  ...
  if (shouldGenerateMask) {
    ...
    if (maskUsage.shouldGenerateMaskLayer && maskFrame) {
      ...
      nsSVGMaskFrame::MaskParams params(&aContext,
                                        aFrame,
                                        aTransform,
                                        maskUsage.opacity,
                                        &maskTransform,
    ...
    if (maskUsage.shouldGenerateClipMaskLayer) {
      ...
      RefPtr<SourceSurface> clipMaskSurface =
          clipPathFrame->GetClipMask(aContext,
                                     aFrame,
                                     aTransform,
  ...
  if (effectProperties.HasValidFilter()) {
    ...
    nsFilterInstance::PaintFilteredFrame(
        aFrame, target, &paintCallback, dirtyRegion, aImgParams);
  } else {
...
IntSize
nsSVGUtils::ConvertToSurfaceSize(const gfxSize& aSize, bool* aResultOverflows)
{
...
gfxRect
nsSVGUtils::GetClipRectForFrame(
    nsIFrame* aFrame, float aX, float aY, float aWidth, float aHeight)
{
  ...
  if (disp->mOverflowX == NS_STYLE_OVERFLOW_HIDDEN ||
    ...
    gfxRect clipRect = gfxRect(
        clipPxRect.x, clipPxRect.y, clipPxRect.width, clipPxRect.height);
...
static gfxRect
GetBoundingBoxRelativeRect(const nsSVGLength2* aXYWH, const gfxRect& aBBox)
{
                            ...
                            nsIFrame* aFrame)
{
  ...
  return GetRelativeRect(
      aUnits, aXYWH, aBBox, NonSVGFrameUserSpaceMetrics(aFrame));
}
                                          ...
                                          const gfxMatrix& aMatrix)
{
  ...
  return ::PathExtentsToMaxStrokeExtents(
      aPathExtents, aFrame, styleExpansionFactor, aMatrix);
}
                               ...
                               SVGContextPaint* aContextPaint)
{
  ...
  float fillOpacity = GetOpacity(
      style->FillOpacitySource(), style->mFillOpacity, aContextPaint);
  ...
  if (ps) {
    RefPtr<gfxPattern> pattern =
        ps->GetPaintServerPattern(aFrame,
                                  dt,
    ...
    switch (style->mFill.Type()) {
      case eStyleSVGPaintType_ContextFill:
        pattern = aContextPaint->GetFillPattern(
            dt, fillOpacity, aContext->CurrentMatrix(), aImgParams);
        break;
      case eStyleSVGPaintType_ContextStroke:
                                 ...
                                 SVGContextPaint* aContextPaint)
{
  ...
  float strokeOpacity = GetOpacity(
      style->StrokeOpacitySource(), style->mStrokeOpacity, aContextPaint);
  if (opacity < 1.0f && nsSVGUtils::CanOptimizeOpacity(aFrame)) {
  ...
  if (aContextPaint) {
    ...
    switch (style->mStroke.Type()) {
      case eStyleSVGPaintType_ContextFill:
        pattern = aContextPaint->GetFillPattern(
            dt, strokeOpacity, aContext->CurrentMatrix(), aImgParams);
        break;
      case eStyleSVGPaintType_ContextStroke:
        pattern = aContextPaint->GetStrokePattern(
            dt, strokeOpacity, aContext->CurrentMatrix(), aImgParams);
                  ...
                  SVGContextPaint* aContextPaint)
{
  ...
  nsSVGElement* ctx = static_cast<nsSVGElement*>(
      content->IsNodeOfType(nsINode::eTEXT) ? content->GetParent() : content);
  ...
  if (aContextPaint && style->StrokeDasharrayFromObject()) {
    ...
    if (content->IsSVGElement(nsGkAtoms::path)) {
      pathScale = static_cast<SVGPathElement*>(content)->GetPathLengthScale(
    ...
    for (uint32_t i = 0; i < count; i++) {
      aDashes[i] = SVGContentUtils::CoordToFloat(ctx, dasharray[i]) * pathScale;
      if (aDashes[i] < 0.0) {
        return false;
      }
      totalLength += aDashes[i];
  ...
  if (aContextPaint && style->StrokeDashoffsetFromObject()) {
    ...
    *aDashOffset = SVGContentUtils::CoordToFloat(ctx, style->mStrokeDashoffset);
  }
