...
struct MOZ_STACK_CLASS BidiParagraphData
{
  ...
  nsresult SetPara()
  {
    return mPresContext->GetBidiEngine().SetPara(
        mBuffer.get(), BufferLength(), mParaLevel);
  }
  ...
  void GetLogicalRun(int32_t aLogicalStart,
                     ...
                     nsBidiLevel* aLevel)
  {
    mPresContext->GetBidiEngine().GetLogicalRun(
        aLogicalStart, aLogicalLimit, aLevel);
    if (mIsVisual) {
      *aLevel = GetParaLevel();
    }
  }
...
struct MOZ_STACK_CLASS BidiLineData
{
  ...
  BidiLineData(nsIFrame* aFirstFrameOnLine, int32_t aNumFramesOnLine)
  {
    ...
    // Reorder the line
    nsBidi::ReorderVisual(
        mLevels.Elements(), FrameCount(), mIndexMap.Elements());
 ...
 */
static nsresult
SplitInlineAncestors(nsContainerFrame* aParent, nsIFrame* aFrame)
{
...
static nsresult
CreateContinuation(nsIFrame* aFrame, nsIFrame** aNewFrame, bool aIsFluid)
{
  ...
  *aNewFrame = nullptr;
  ...
  if (parent->IsLetterFrame() && parent->IsFloating()) {
    ...
    rv = letterFrame->CreateContinuationForFloatingParent(
        presContext, aFrame, aNewFrame, aIsFluid);
    return rv;
  }
...
nsBidiPresUtils::ResolveParagraph(BidiParagraphData* aBpd)
{
  ...
  printf(
      ...
      "runCount=%d\n",
      (void*)aBpd->mCurrentBlock,
      NS_ConvertUTF16toUTF8(aBpd->mBuffer).get(),
      frameCount,
      runCount);
...
/* static */ void
nsBidiPresUtils::RepositionRubyContentFrame(nsIFrame* aFrame,
  ...
  if (frameType == LayoutFrameType::Ruby) {
    ...
    for (RubySegmentEnumerator e(static_cast<nsRubyFrame*>(aFrame)); !e.AtEnd();
      ...
      for (nsRubyTextContainerFrame* rtc : textContainers) {
        nscoord isize = RepositionFrame(
            rtc, isLTR, icoord, aContinuationStates, frameWM, false, frameSize);
    ...
    for (RubyColumnEnumerator e(rbc, textContainers); !e.AtEnd(); e.Next()) {
      ...
      nscoord columnISize = RepositionFrame(column.mBaseFrame,
                                            isLTR,
                                            icoord,
                                            aContinuationStates,
                                            frameWM,
      ...
      for (nsRubyTextFrame* rt : column.mTextFrames) {
        nscoord isize = RepositionFrame(
            rt, isLTR, icoord, aContinuationStates, frameWM, false, frameSize);
    ...
    const nsSize& aContainerSize)
{
  ...
  if (!IsBidiLeaf(aFrame)) {
    ...
    icoord += RepositionRubyFrame(
        aFrame, aContinuationStates, aContainerWM, borderPadding);
  } else {
    icoord +=
...
void
nsBidiPresUtils::InitContinuationStates(
...
nsresult
nsBidiPresUtils::ProcessText(const char16_t* aText,
                             int32_t aLength,
                             ...
                             nsBidi* aBidiEngine)
{
  ...
  for (i = 0; i < runCount; i++) {
    ...
    while (subRunCount > 0) {
      ...
      FormatUnicodeText(aPresContext,
                        runVisualText.BeginWriting(),
                        subRunLength,
                        (nsCharType)charType);
 ...
 public:
  ...
  virtual nscoord GetWidth() override
  {
    return nsLayoutUtils::AppUnitWidthOfString(
        mText, mLength, *mFontMetrics, mTextRunConstructionDrawTarget);
  }
  ...
  virtual void DrawText(nscoord aIOffset, nscoord) override
  {
    ...
    mFontMetrics->DrawString(
        mText, mLength, pt.x, pt.y, mCtx, mTextRunConstructionDrawTarget);
  }
