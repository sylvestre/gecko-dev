...
class nsLayoutUtils
{
 ...
 public:
   ...
   */
  static bool GetDisplayPort(nsIContent* aContent,
                             nsRect* aResult,
  ...
  static bool FrameHasDisplayPort(nsIFrame* aFrame,
                                  nsIFrame* aScrolledFrame = nullptr);
   ...
   */
  static bool IsGeneratedContentFor(nsIContent* aContent,
                                    nsIFrame* aFrame,
                                     ...
                                     nsIFrame* aCommonAncestor = nullptr)
  {
    return DoCompareTreePosition(
        aFrame1, aFrame2, aFrame2Ancestors, -1, 1, aCommonAncestor);
  }
   ...
   * the function
   */
  static bool IsProperAncestorFrame(nsIFrame* aAncestorFrame,
                                    nsIFrame* aFrame,
                                    nsIFrame* aCommonAncestor = nullptr);
   ...
   */
  static FrameMetrics::ViewID ScrollIdForRootScrollFrame(
      nsPresContext* aPresContext);
   ...
   */
  static nsIFrame* GetFrameForPoint(nsIFrame* aFrame,
                                    nsPoint aPt,
                                    uint32_t aFlags = 0);
   ...
   * @param aFlags some combination of FrameForPointFlags
   */
  static nsresult GetFramesForArea(nsIFrame* aFrame,
                                   const nsRect& aRect,
   ...
   */
  static TransformResult TransformRect(nsIFrame* aFromFrame,
                                       nsIFrame* aToFrame,
                                       nsRect& aRect);
   ...
   */
  static void PostTranslate(Matrix4x4& aTransform,
   ...
   * aPoint.
   */
  static bool ContainsPoint(const nsRect& aRect,
   ...
   */
  static nsresult PaintFrame(gfxContext* aRenderingContext,
                             nsIFrame* aFrame,
   ...
   */
  static void GetAllInFlowRects(nsIFrame* aFrame,
   ...
   * aFrame?
   */
  static bool IsFirstContinuationOrIBSplitSibling(nsIFrame* aFrame);
  ...
  static nscoord IntrinsicForAxis(mozilla::PhysicalAxis aAxis,
                                  ...
                                  const mozilla::Maybe<mozilla::LogicalSize>&
                                      aPercentageBasis = mozilla::Nothing(),
                                  uint32_t aFlags = 0,
  ...
  static nscoord AddPercents(nscoord aCurrent, float aPercent)
  {
    if (aPercent > 0.0f && aCurrent > 0) {
      return MOZ_UNLIKELY(aPercent >= 1.0f)
                 ? nscoord_MAX
   ...
   */
  static nsSize ComputeAutoSizeWithIntrinsicDimensions(nscoord minWidth,
                                                       nscoord minHeight,
                                                       nscoord maxWidth,
                                                       nscoord maxHeight,
                                                       nscoord tentWidth,
  ...
  static gfxFloat GetSnappedBaselineY(nsIFrame* aFrame,
                                      gfxContext* aContext,
                                      nscoord aY,
                                      nscoord aAscent);
  ...
  static gfxFloat GetSnappedBaselineX(nsIFrame* aFrame,
                                      gfxContext* aContext,
                                      nscoord aX,
                                      nscoord aAscent);
                                      ...
                                      DrawTarget* aDrawTarget)
  {
    return nsLayoutUtils::AppUnitWidthOfString(
        aString.get(), aString.Length(), aFontMetrics, aDrawTarget);
  }
   ...
   */
  static DrawResult DrawSingleUnscaledImage(
      gfxContext& aContext,
      nsPresContext* aPresContext,
      imgIContainer* aImage,
      const SamplingFilter aSamplingFilter,
   ...
   *                            be aligned with image pixels.
   */
  static DrawResult DrawSingleImage(
      gfxContext& aContext,
      nsPresContext* aPresContext,
      imgIContainer* aImage,
      const SamplingFilter aSamplingFilter,
  ...
  static SurfaceFromElementResult SurfaceFromElement(
      mozilla::dom::HTMLImageElement* aElement,
      uint32_t aSurfaceFlags,
      RefPtr<DrawTarget>& aTarget);
  static SurfaceFromElementResult SurfaceFromElement(
      mozilla::dom::HTMLCanvasElement* aElement,
      uint32_t aSurfaceFlags,
      RefPtr<DrawTarget>& aTarget);
  static SurfaceFromElementResult SurfaceFromElement(
   ...
   */
  static nsIContent* GetEditableRootContentByContentEditable(
  ...
  static bool PointIsCloserToRect(PointType aPoint,
...
template<typename PointType, typename RectType, typename CoordType>
/* static */ bool
nsLayoutUtils::PointIsCloserToRect(PointType aPoint,
                                   const RectType& aRect,
                                   CoordType& aClosestXDistance,
                                   CoordType& aClosestYDistance)
{
 ...
 */
inline gfx::Point
NSPointToPoint(const nsPoint& aPoint, int32_t aAppUnitsPerPixel)
{
...
{
 public:
  nsSetAttrRunnable(nsIContent* aContent,
                    nsAtom* aAttrName,
                    const nsAString& aValue);
  nsSetAttrRunnable(nsIContent* aContent, nsAtom* aAttrName, int32_t aValue);
