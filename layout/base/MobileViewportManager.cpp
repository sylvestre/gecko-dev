...
static const nsLiteralString FULL_ZOOM_CHANGE =
...
NS_IMETHODIMP
MobileViewportManager::Observe(nsISupports* aSubject,
                               const char* aTopic,
...
MobileViewportManager::ScaleResolutionWithDisplayWidth(
    const LayoutDeviceToLayerScale& aRes,
...
CSSToScreenScale
MobileViewportManager::UpdateResolution(
    const nsViewportInfo& aViewportInfo,
    ...
    const Maybe<float>& aDisplayWidthChangeRatio)
{
  ...
  if (mIsFirstPaint) {
    ...
    if (mRestoreResolution) {
      ...
      MVM_LOG(
          "%p: default zoom from viewport is %f\n", this, defaultZoom.scale);
    ...
    CSSToParentLayerScale zoom = ViewTargetAs<ParentLayerPixel>(
        defaultZoom, PixelCastJustification::ScreenIsParentLayerForRoot);

    LayoutDeviceToLayerScale resolution =
        zoom / cssToDev * ParentLayerToLayerScale(1);
  ...
  if (aDisplayWidthChangeRatio) {
    res = ScaleResolutionWithDisplayWidth(
        res, aDisplayWidthChangeRatio.value(), aViewport, mMobileViewportSize);
...
MobileViewportManager::RefreshViewportSize(bool aForceAdjustResolution)
{
  ...
  if (gfxPrefs::APZAllowZooming()) {
    CSSToScreenScale zoom = UpdateResolution(
        viewportInfo, displaySize, viewport, displayWidthChangeRatio);
