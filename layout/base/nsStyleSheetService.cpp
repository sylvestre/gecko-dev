                                     ...
                                     bool* _retval)
{
  ...
  *_retval = (FindSheetByURI(
                  mozilla::StyleBackendType::Gecko, aSheetType, sheetURI) >= 0);
...
NS_IMETHODIMP
nsStyleSheetService::PreloadSheet(nsIURI* aSheetURI,
                                  uint32_t aSheetType,
                                  nsIPreloadedStyleSheet** aSheet)
{
  ...
  rv = PreloadedStyleSheet::Create(
      aSheetURI, parsingMode, getter_AddRefs(sheet));
  NS_ENSURE_SUCCESS(rv, rv);
...
NS_IMETHODIMP
nsStyleSheetService::PreloadSheetAsync(nsIURI* aSheetURI,
                                       uint32_t aSheetType,
                                       JSContext* aCx,
...
nsStyleSheetService::UnregisterSheet(nsIURI* aSheetURI, uint32_t aSheetType)
{
  ...
  int32_t foundIndex =
      FindSheetByURI(StyleBackendType::Gecko, aSheetType, aSheetURI);
...
size_t
nsStyleSheetService::SizeOfIncludingThis(
