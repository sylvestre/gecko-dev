...
static nsRect
BoxDecorationRectForBorder(nsIFrame* aFrame,
                           const nsRect& aBorderArea,
                           Sides aSkipSides,
...
static nsRect
BoxDecorationRectForBackground(nsIFrame* aFrame,
                               const nsRect& aBorderArea,
                               Sides aSkipSides,
                               const nsStyleBorder* aStyleBorder = nullptr)
{
    ...
    nsDisplayListBuilder* aDisplayListBuilder)
{
  ...
  bir->CreateWebRenderCommands(aItem,
                               aForFrame,
                               aBuilder,
                               aResources,
                               aSc,
                        ...
                        bool* aNeedsClip)
{
  ...
  nsRect joinedBorderArea = ::BoxDecorationRectForBorder(
                                               ...
                                               nsStyleContext* aStyleContext)
{
  ...
  if (outlineStyle == NS_STYLE_BORDER_STYLE_AUTO) {
    if (nsLayoutUtils::IsOutlineStyleAutoEnabled()) {
      nsITheme* theme = aPresContext->GetTheme();
      if (theme && theme->ThemeSupportsWidget(
                       aPresContext, aForFrame, NS_THEME_FOCUS_OUTLINE)) {
...
{
  nsIFrame* rootElementFrame = aForFrame->PresContext()
                                   ->PresShell()
                                   ->FrameConstructor()
...
{
  if (aBorder.mBorderColors) return false;
  NS_FOR_CSS_SIDES(i)
  {
                                           ...
                                           const nsStyleBorder& aBorder)
{
  ...
  if (!drawBackgroundImage && !drawBackgroundColor) return DrawResult::SUCCESS;
    ...
    case NS_STYLE_BORDER_STYLE_RIDGE:
      if ((horizontal && (oneDevPixel >= aBorder.height)) ||
        ...
        nscoord endBevel =
            (aEndBevelOffset > 0)
                ? RoundFloatToPixel(
                      0.5f * (float)aEndBevelOffset, aAppUnitsPerDevPixel, true)
                : 0;
        mozilla::Side ridgeGrooveSide = (horizontal) ? eSideTop : eSideLeft;
        // FIXME: In theory, this should use the visited-dependent
        // background color, but I don't care.
        nscolor bevelColor = MakeBevelColor(
            ridgeGrooveSide, ridgeGroove, aBGColor, aBorderColor);
        nsRect rect(aBorder);
        nscoord half;
  ...
  switch (aParams.style) {
    ...
    case NS_STYLE_TEXT_DECORATION_STYLE_WAVY: {
      ...
      if (textDrawer) {
        ...
        textDrawer->AppendDecoration(p1,
                                     p2,
                                     adv,
                                     aParams.vertical,
                                     color,
