    ...
    int aSides, const nsTArray<nscolor>& aCompositeColors)
{
  ...
  for (uint32_t i = 0; i < uint32_t(maxBorderWidth); i++) {
    ...
    fakeBorderSizes[eSideBottom] =
        soRect.BottomRight().y - siRect.BottomRight().y;
...
nsCSSBorderRenderer::DrawDottedSideSlow(mozilla::Side aSide)
{
  ...
  if (borderLength < dotWidth) {
    ...
    //           |  | |  |
    //           |  | |  |
    if (!mOuterRect.Contains(Rect(
            start.x - radius, start.y - radius, borderWidth, borderWidth))) {
      return;
    }
...
nsCSSBorderRenderer::DrawRectangularCompositeColors()
{
  ...
  NS_FOR_CSS_SIDES(side) { currentColors[side] = mBorderColors[side]; }
  Rect rect = mOuterRect;
  rect.Deflate(0.5);
  ...
  for (int i = 0; i < mBorderWidths[0]; i++) {
    ...
    NS_FOR_CSS_SIDES(side)
    {
      ...
      mDrawTarget->StrokeLine(
          firstCorner, secondCorner, ColorPattern(ToDeviceColor(currentColor)));
...
nsCSSBorderRenderer::DrawBorders()
{
  ...
  if (mAllBordersSameStyle && mCompositeColors[0] == nullptr &&
      mBorderStyles[0] == NS_STYLE_BORDER_STYLE_SOLID && !mAvoidStroke &&
      !mNoBorderRadius) {
    ...
    AppendRoundedRectToPath(
        builder, ToRect(borderInnerRect.rect), borderInnerRect.corners, false);
    RefPtr<Path> path = builder->Finish();
  ...
  if (allBordersSolid && mAllBordersSameWidth &&
      mCompositeColors[0] == nullptr && mBorderWidths[0] == 1 &&
      mNoBorderRadius && !mAvoidStroke) {
    ...
    const layers::StackingContextHelper& aSc)
{
  ...
  aBuilder.PushBorder(transformedRect,
                      transformedRect,
                      mBackfaceIsVisible,
                      wr::ToBorderWidths(mBorderWidths[0],
                                         mBorderWidths[1],
                                         mBorderWidths[2],
                                         mBorderWidths[3]),
                      wrsides,
    ...
    : mImageRenderer(aImageRenderer)
{
  ...
  if (::IsBoxDecorationSlice(aStyleBorder) && !aSkipSides.IsEmpty()) {
    mArea = ::BoxDecorationRectForBorder(
