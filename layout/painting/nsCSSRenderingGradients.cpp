                              ...
                              const nsSize& aIntrinsicSize)
{
  ...
  for (uint32_t i = 0; i < aGradient->mStops.Length(); ++i) {
      ...
      case eStyleUnit_None:
        if (i == 0) {
          ...
          stops.AppendElement(ColorStop(
              0, stop.mIsInterpolationHint, Color::FromABGR(stop.mColor)));
          continue;
        }
        break;
      case eStyleUnit_Percent:
    ...
    stops.AppendElement(ColorStop(
        position, stop.mIsInterpolationHint, Color::FromABGR(stop.mColor)));
    if (firstUnsetPosition > 0) {
                             ...
                             float aOpacity)
{
  ...
  Telemetry::AutoTimer<Telemetry::GRADIENT_DURATION, Telemetry::Microsecond>
      gradientTimer;
        ...
        if (pos < 0.0) {
          ...
          if (i < mStops.Length() - 1) {
            ...
            if (nextPos >= 0.0 && nextPos - pos >= 1e-6) {
              ...
              mStops[i].mColor = InterpolateColor(
