...
void
nsROCSSPrimitiveValue::SetFloatValue(uint16_t aType,
                                     float aVal,
                                     ErrorResult& aRv)
{
...
nsROCSSPrimitiveValue::GetFloatValue(uint16_t aUnitType, ErrorResult& aRv)
{
  switch (aUnitType) {
    ...
    case CSS_PC:
      if (mType == CSS_PX) {
        return mValue.mAppUnits * 6.0f / nsPresContext::AppUnitsPerCSSInch();
      }
...
void
nsROCSSPrimitiveValue::SetStringValue(uint16_t aType,
...
{
  NS_PRECONDITION(aKeyword != eCSSKeyword_UNKNOWN && 0 <= aKeyword &&
