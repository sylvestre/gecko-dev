...
NS_INTERFACE_MAP_BEGIN(ServoStyleRuleDeclaration)
  ...
  nsINode* ServoStyleRuleDeclaration::GetParentObject()
  {
    return Rule()->GetDocument();
  }

  DeclarationBlock* ServoStyleRuleDeclaration::GetCSSDeclaration(
      Operation aOperation)
  {
    return mDecls;
  }

  nsresult ServoStyleRuleDeclaration::SetCSSDeclaration(DeclarationBlock *
                                                        aDecl)
  {
  ...
  nsIDocument* ServoStyleRuleDeclaration::DocToUpdate() { return nullptr; }

  void ServoStyleRuleDeclaration::GetCSSParsingEnvironment(
  ...
  bool ServoStyleRule::IsCCLeaf() const
  {
    if (!Rule::IsCCLeaf()) {
      return false;
    }
  ...
  already_AddRefed<css::Rule> ServoStyleRule::Clone() const
  {
  ...
  size_t ServoStyleRule::SizeOfIncludingThis(MallocSizeOf aMallocSizeOf) const
  {
...
#ifdef DEBUG
  void ServoStyleRule::List(FILE * out, int32_t aIndent) const
  {
    nsAutoCString str;
  ...
  uint16_t ServoStyleRule::Type() const { return nsIDOMCSSRule::STYLE_RULE; }

  void ServoStyleRule::GetCssTextImpl(nsAString & aCssText) const
  {
    Servo_StyleRule_GetCssText(mRawRule, &aCssText);
  }

  nsICSSDeclaration* ServoStyleRule::Style() { return &mDecls; }
  ...
  uint32_t ServoStyleRule::GetSelectorCount()
  {
    uint32_t aCount;
  ...
  nsresult ServoStyleRule::GetSelectorText(uint32_t aSelectorIndex,
                                           nsAString & aText)
  {
  ...
  nsresult ServoStyleRule::GetSpecificity(uint32_t aSelectorIndex,
                                          uint64_t * aSpecificity)
  {
    Servo_StyleRule_GetSpecificityAtIndex(
        mRawRule, aSelectorIndex, aSpecificity);
    return NS_OK;
  }

  nsresult ServoStyleRule::SelectorMatchesElement(Element * aElement,
  ...
  NotNull<DeclarationBlock*> ServoStyleRule::GetDeclarationBlock() const
  {
