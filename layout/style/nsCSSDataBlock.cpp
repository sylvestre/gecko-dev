 ...
 */
static void
TryToStartImageLoadOnValue(const nsCSSValue& aValue,
                           ...
                           bool aForTokenStream)
{
  ...
  if (aValue.GetUnit() == eCSSUnit_URL) {
    ...
    if (aForTokenStream && aContext) {
      CSSVariableImageTable::Add(
          aContext, aProperty, aValue.GetImageStructValue());
    }
    ...
    TryToStartImageLoadOnValue(
        image, aDocument, aContext, aProperty, aForTokenStream);
  }
}

static void
TryToStartImageLoad(const nsCSSValue& aValue,
                    nsIDocument* aDocument,
                    nsStyleContext* aContext,
                    nsCSSPropertyID aProperty,
                    bool aForTokenStream)
{
  if (aValue.GetUnit() == eCSSUnit_List) {
    for (const nsCSSValueList* l = aValue.GetListValue(); l; l = l->mNext) {
      TryToStartImageLoad(
          l->mValue, aDocument, aContext, aProperty, aForTokenStream);
    }
  } else if (nsCSSProps::PropHasFlags(aProperty,
                                      CSS_PROPERTY_IMAGE_IS_IN_ARRAY_0)) {
    if (aValue.GetUnit() == eCSSUnit_Array) {
      TryToStartImageLoadOnValue(aValue.GetArrayValue()->Item(0),
                                 aDocument,
                                 aContext,
                                 aProperty,
                                 aForTokenStream);
    ...
    TryToStartImageLoadOnValue(
        aValue, aDocument, aContext, aProperty, aForTokenStream);
  }
}
                      ...
                      nsRuleData* aRuleData)
{
  ...
  if (ShouldStartImageLoads(aRuleData, aTargetProp)) {
    ...
    TryToStartImageLoad(*aSrcValue,
                        doc,
                        aRuleData->mStyleContext,
                        aTargetProp,
...
size_t
nsCSSCompressedDataBlock::SizeOfIncludingThis(
...
{
  if (!nsCSSProps::IsShorthand(aPropID)) {
    return DoTransferFromBlock(aFromBlock,
                               aPropID,
                               aIsImportant,
                               aOverrideImportant,
  ...
  {
    changed |= DoTransferFromBlock(aFromBlock,
                                   *p,
                                   aIsImportant,
                                   aOverrideImportant,
                                   aMustCallValueAppended,
