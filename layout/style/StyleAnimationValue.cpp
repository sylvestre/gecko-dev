 ...
 */
static StyleAnimationValue::Unit
...
double
CalcPositionSquareDistance(const nsCSSValue& aPos1, const nsCSSValue& aPos2)
{
  NS_ASSERTION(
...
static void
AddCSSValueAngle(double aCoeff1,
                 const nsCSSValue& aValue1,
                 double aCoeff2,
...
static inline void
AddCSSValuePercent(double aCoeff1,
                   const nsCSSValue& aValue1,
                   double aCoeff2,
                   const nsCSSValue& aValue2,
                   nsCSSValue& aResult,
                   uint32_t aValueRestrictions = 0)
{
  ...
  aResult.SetPercentValue(
      RestrictValue(aValueRestrictions,
...
static void
AddCSSValueCanonicalCalc(double aCoeff1,
                         const nsCSSValue& aValue1,
                         double aCoeff2,
                         const nsCSSValue& aValue2,
                         nsCSSValue& aResult)
...
static inline void
AddCSSValuePixel(double aCoeff1,
                 const nsCSSValue& aValue1,
                 double aCoeff2,
                 const nsCSSValue& aValue2,
                 nsCSSValue& aResult,
                            ...
                            nsCSSValue& aResult)
{
  switch (aCommonUnit) {
    case eCSSUnit_Pixel:
      AddCSSValuePixel(
          aCoeff1, aValue1, aCoeff2, aValue2, aResult, aValueRestrictions);
      break;
...
static void
AddTransformTranslate(double aCoeff1,
                      const nsCSSValue& aValue1,
                      double aCoeff2,
                      const nsCSSValue& aValue2,
                      nsCSSValue& aResult)
...
AddWeightedColors(double aCoeff1,
...
AddWeightedFilterFunction(double aCoeff1,
                          ...
                          double& aDistance)
{
  ...
  while (aList1 || aList2) {
    ...
    if (!aList1) {
      ...
      if (!ComputeFilterSquareDistance(
              none.get(), aList2, currentSquareDistance)) {
        return false;
      }
      aList2 = aList2->mNext;
    } else if (!aList2) {
      ...
      if (!ComputeFilterSquareDistance(
              aList1, none.get(), currentSquareDistance)) {
        return false;
      }
      aList1 = aList1->mNext;
    } else {
...
AddTransformLists(double aCoeff1,
                  const nsCSSValueList* aList1,
                  double aCoeff2,
                  const nsCSSValueList* aList2,
...
static double
ComputeTransform2DMatrixDistance(const Matrix& aMatrix1, const Matrix& aMatrix2)
{
                                 ...
                                 const Matrix4x4& aMatrix2)
{
  ...
  nsStyleTransformMatrix::ShearArray shear1{0.0f, 0.0f, 0.0f};
  Decompose3DMatrix(
      aMatrix1, scale1, shear1, rotate1, translate1, perspective1);
  ...
  nsStyleTransformMatrix::ShearArray shear2{0.0f, 0.0f, 0.0f};
  Decompose3DMatrix(
      aMatrix2, scale2, shear2, rotate2, translate2, perspective2);
...
static double
ComputeTransformDistance(nsCSSValue::Array* aArray1, nsCSSValue::Array* aArray2)
{
                                     ...
                                     double& aDistance)
{
  ...
  switch (commonUnit) {
    ...
    case eUnit_CSSValuePair: {
      ...
      nsCSSUnit unit[2];
      unit[0] = GetCommonUnit(
          aProperty, pair1->mXValue.GetUnit(), pair2->mXValue.GetUnit());
      unit[1] = GetCommonUnit(
          aProperty, pair1->mYValue.GetUnit(), pair2->mYValue.GetUnit());
    ...
    case eUnit_CSSValueTriplet: {
      ...
      nsCSSUnit unit[3];
      unit[0] = GetCommonUnit(
          aProperty, triplet1->mXValue.GetUnit(), triplet2->mXValue.GetUnit());
      unit[1] = GetCommonUnit(
          aProperty, triplet1->mYValue.GetUnit(), triplet2->mYValue.GetUnit());
      unit[2] = GetCommonUnit(
          aProperty, triplet1->mZValue.GetUnit(), triplet2->mZValue.GetUnit());
      if (unit[0] == eCSSUnit_Null || unit[1] == eCSSUnit_Null ||
      ...
      static nsCSSValue nsCSSValueTriplet::*const pairValues[3] = {
          &nsCSSValueTriplet::mXValue,
          &nsCSSValueTriplet::mYValue,
    ...
    case eUnit_Dasharray: {
      ...
      if (!AddWeighted(
              aProperty, 1.0, aStartValue, 0.0, aEndValue, normValue1) ||
          !AddWeighted(
              aProperty, 0.0, aStartValue, 1.0, aEndValue, normValue2)) {
        return false;
      }
    ...
    case eUnit_Shadow: {
      ...
      while (shadow1) {
        ...
        if (!ComputeSingleShadowSquareDistance(
                shadow1, shadow2, squareDistance, aProperty)) {
          NS_ERROR(
...
static inline void
AddCSSValueNumber(double aCoeff1,
                  const nsCSSValue& aValue1,
                  double aCoeff2,
                  const nsCSSValue& aValue2,
                  nsCSSValue& aResult,
...
static nscolor
AddWeightedColorsAndClamp(double aCoeff1,
                          const RGBAColorData& aValue1,
                          double aCoeff2,
                          const RGBAColorData& aValue2)
{
...
static UniquePtr<nsCSSValueList>
AddWeightedShadowItems(double aCoeff1,
                       const nsCSSValue& aValue1,
                       double aCoeff2,
                       const nsCSSValue& aValue2,
                       ColorAdditionType aColorAdditionType,
...
static void
AddTransformScale(double aCoeff1,
                  const nsCSSValue& aValue1,
                  double aCoeff2,
                  const nsCSSValue& aValue2,
                  nsCSSValue& aResult)
...
static nsCSSValueList*
AddDifferentTransformLists(double aCoeff1,
                           const nsCSSValueList* aList1,
                           double aCoeff2,
                           const nsCSSValueList* aList2,
                           nsCSSKeyword aOperatorType)
{
  ...
  arr = StyleAnimationValue::AppendTransformFunction(aOperatorType, resultTail);
...
static UniquePtr<nsCSSValueList>
AddWeightedFilterFunctionImpl(double aCoeff1,
                              const nsCSSValueList* aList1,
                              double aCoeff2,
                              const nsCSSValueList* aList2,
                              ColorAdditionType aColorAdditionType)
                          ...
                          ColorAdditionType aColorAdditionType)
{
  ...
  if (!aList1) {
    return AddWeightedFilterFunctionImpl(
        aCoeff2, aList2, 0, aList2, aColorAdditionType);
  }
  if (!aList2) {
    return AddWeightedFilterFunctionImpl(
        aCoeff1, aList1, 0, aList1, aColorAdditionType);
  }
...
static void
AddPositions(double aCoeff1,
             const nsCSSValue& aPos1,
             double aCoeff2,
             const nsCSSValue& aPos2,
...
AddCSSValuePair(nsCSSPropertyID aProperty,
                ...
                const nsCSSValuePair* aPair2)
{
  ...
  nsCSSUnit unit[2];
  unit[0] = GetCommonUnit(
      aProperty, aPair1->mXValue.GetUnit(), aPair2->mXValue.GetUnit());
  unit[1] = GetCommonUnit(
      aProperty, aPair1->mYValue.GetUnit(), aPair2->mYValue.GetUnit());
  if (unit[0] == eCSSUnit_Null || unit[1] == eCSSUnit_Null ||
      unit[0] == eCSSUnit_URL || unit[0] == eCSSUnit_Enumerated) {
  ...
  for (uint32_t i = 0; i < 2; ++i) {
    nsCSSValue nsCSSValuePair::*member = pairValues[i];
    if (!AddCSSValuePixelPercentCalc(aRestrictions,
                                     unit[i],
                                     aCoeff1,
                                     aPair1->*member,
                                     aCoeff2,
...
AddCSSValuePairList(nsCSSPropertyID aProperty,
                    double aCoeff1,
                    const nsCSSValuePairList* aList1,
                    double aCoeff2,
                    const nsCSSValuePairList* aList2)
{
  ...
  do {
    ...
    for (uint32_t i = 0; i < ArrayLength(pairListValues); ++i) {
      ...
      } else if (!AddCSSValuePixelPercentCalc(
                     restrictions, unit, aCoeff1, v1, aCoeff2, v2, vr)) {
        if (v1 != v2) {
          return nullptr;
        }
        vr = v1;
    ...
    switch (tfunc) {
      case eCSSKeyword_translate3d: {
        ...
        AddTransformTranslate(
            aCoeff1, a1->Item(1), aCoeff2, a2->Item(1), arr->Item(1));
        AddTransformTranslate(
            aCoeff1, a1->Item(2), aCoeff2, a2->Item(2), arr->Item(2));
        AddTransformTranslate(
            aCoeff1, a1->Item(3), aCoeff2, a2->Item(3), arr->Item(3));
        break;
      }
      case eCSSKeyword_scale3d: {
        MOZ_ASSERT(a1->Count() == 4, "unexpected count");
      ...
      case eCSSKeyword_rotate3d: {
        ...
        if (vector1 == vector2) {
          ...
          AddCSSValueAngle(
              aCoeff1, a1->Item(4), aCoeff2, a2->Item(4), arr->Item(4));
          break;
        }
        MOZ_FALLTHROUGH;
      }
      ...
      case eCSSKeyword_interpolatematrix: {
        ...
        if (aList1 == aList2) {
          *resultTail = AddDifferentTransformLists(
              aCoeff1, &tempList1, aCoeff2, &tempList1, aOperatorType);
        } else {
          *resultTail = AddDifferentTransformLists(
              aCoeff1, &tempList1, aCoeff2, &tempList2, aOperatorType);
        }
...
static void
AddPositionCoords(double aCoeff1,
                  const nsCSSValue& aPos1,
                  double aCoeff2,
                  const nsCSSValue& aPos2,
                  nsCSSValue& aResultPos)
                      ...
                      nsCSSPropertyID aProperty)
{
  ...
  if (aShadow1 || aShadow2) {
    ...
    while (longShadow) {
      ...
      // the current shadow.
      UniquePtr<nsCSSValueList> shadowValue =
          AddWeightedShadowItems(longCoeff,
                                 longShadow->mValue,
                                 0.0,
                                 longShadow->mValue,
                                 aColorAdditionType,
...
static UniquePtr<nsCSSValueList>
AddWeightedFilterList(double aCoeff1,
                      const nsCSSValueList* aList1,
                      double aCoeff2,
                      const nsCSSValueList* aList2,
                      ColorAdditionType aColorAdditionType)
                                 ...
                                 StyleAnimationValue& aResultValue)
{
  ...
  switch (commonUnit) {
    ...
    case eUnit_Coord: {
      aResultValue.SetCoordValue(
          RestrictValue(aProperty,
                        NSToCoordRound(aCoeff1 * aValue1.GetCoordValue() +
    ...
    case eUnit_ComplexColor: {
      ...
      if (color1.IsNumericColor() && color2.IsCurrentColor()) {
        ...
        RGBAColorData resultColor =
            AddWeightedColors(aCoeff1,
                              color1.mColor.WithAlpha(alpha1),
                              aCoeff2,
    ...
    case eUnit_CSSValuePair: {
      uint32_t restrictions = nsCSSProps::ValueRestrictions(aProperty);
      Maybe<nsCSSValuePair> result =
          AddCSSValuePair(aProperty,
                          restrictions,
                          aCoeff1,
                          aValue1.GetCSSValuePairValue(),
                          aCoeff2,
                          aValue2.GetCSSValuePairValue());
      if (!result) {
        return false;
      }
    ...
    case eUnit_Shape: {
      RefPtr<nsCSSValue::Array> result =
          AddShapeFunction(aProperty,
                           aCoeff1,
                           aValue1.GetCSSValueArrayValue(),
                           aCoeff2,
                           aValue2.GetCSSValueArrayValue());
      if (!result) {
        return false;
      }
    ...
    case eUnit_Transform: {
      ...
      if (list1->mValue.GetUnit() == eCSSUnit_None) {
        if (list2->mValue.GetUnit() == eCSSUnit_None) {
          ...
          result = AddDifferentTransformLists(
              0, list2, aCoeff2, list2, eCSSKeyword_interpolatematrix);
        } else {
        ...
        if (list2->mValue.GetUnit() == eCSSUnit_None) {
          if (HasAccumulateMatrix(list1)) {
            result = AddDifferentTransformLists(
                0, list1, aCoeff1, list1, eCSSKeyword_interpolatematrix);
          } else {
            result = AddTransformLists(0, list1, aCoeff1, list1);
          }
        } else if (TransformFunctionListsMatch(list1, list2)) {
          result = AddTransformLists(
              aCoeff1, list1, aCoeff2, list2, eCSSKeyword_interpolatematrix);
        } else {
          result = AddDifferentTransformLists(
              aCoeff1, list1, aCoeff2, list2, eCSSKeyword_interpolatematrix);
        }
                                ...
                                uint64_t aCount)
{
  ...
  switch (commonUnit) {
    case eUnit_Filter: {
      UniquePtr<nsCSSValueList> resultList =
          AddWeightedFilterList(1.0,
                                result.GetCSSValueListValue(),
                                aCount,
                                aA.GetCSSValueListValue(),
    ...
    case eUnit_Transform: {
      ...
      if (listA->mValue.GetUnit() == eCSSUnit_None) {
        ...
        resultList = AddTransformLists(
            0.0, listA, aCount, listA, eCSSKeyword_accumulatematrix);
      } else if (TransformFunctionListsMatch(listA, listB)) {
        resultList = AddTransformLists(
            1.0, listB, aCount, listA, eCSSKeyword_accumulatematrix);
      } else {
        resultList = AddDifferentTransformLists(
            1.0, listB, aCount, listA, eCSSKeyword_accumulatematrix);
      }
               ...
               bool aUseSVGMode)
{
  ...
  parser.ParseProperty(aProperty,
                       aSpecifiedValue,
                       doc->GetDocumentURI(),
                       baseURI,
    ...
    nsTArray<PropertyStyleAnimationValuePair>& aValues)
{
  ...
  return StyleAnimationValue::ExtractComputedValue(
      aProperty, aStyleContext, pair->mValue.mGecko);
}
                           ...
                           bool* aIsContextSensitive)
{
  ...
  return ComputeValuesFromStyleContext(
      aProperty, aEnabledState, tmpStyleContext, aValues);
}
    ...
    nsTArray<PropertyStyleAnimationValuePair>& aResult)
{
  ...
  aResult.Clear();
  return ComputeValuesFromStyleRule(aProperty,
                                    aEnabledState,
                                    aStyleContext,
                                    styleRule,
                                    aResult,
...
StyleAnimationValue::ComputeValues(
    ...
    nsTArray<PropertyStyleAnimationValuePair>& aResult)
{
  return ComputeValuesFromSpecifiedValue(aProperty,
                                         aEnabledState,
                                         aTargetElement,
                                         aStyleContext,
                                         aSpecifiedValue,
...
static void
SetPositionCoordValue(const Position::Coord& aPosCoord, nsCSSValue& aCSSValue)
{
  ...
  if (aInput.IsCalcUnit()) {
    ...
    for (size_t i = 0, i_end = inputArray->Count(); i < i_end; ++i) {
      SubstitutePixelValues(
          aStyleContext, inputArray->Item(i), outputArray->Item(i));
    }
            ...
            if (type == NS_STYLE_FILTER_URL) {
              ...
              if (type >= NS_STYLE_FILTER_BLUR &&
                ...
                AppendCSSShadowValue(
                    shadowArray->ShadowAt(0), tmpShadowResultTail, aProperty);
...
StyleAnimationValue::StyleAnimationValue(int32_t aInt,
                                         Unit aUnit,
...
StyleAnimationValue::StyleAnimationValue(float aPercent, PercentConstructorType)
{
  mUnit = eUnit_Percent;
...
void
StyleAnimationValue::SetAndAdoptCSSValueValue(nsCSSValue* aValue, Unit aUnit)
{
  FreeValue();
...
AnimationValue::operator==(const AnimationValue& aOther) const
{
  ...
  return !mServo && !aOther.mServo && mGecko == aOther.mGecko;
}
                           ...
                           Element* aElement)
{
  ...
  if (auto servoContext = styleContext->GetAsServo()) {
    ...
    result.mServo = presContext->StyleSet()->AsServo()->ComputeAnimationValue(
