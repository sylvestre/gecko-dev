...
struct MOZ_NEEDS_MEMMOVABLE_MEMBERS nsStyleFont
{
  ...
  uint8_t
      mFontSizeKeyword;  // NS_STYLE_FONT_SIZE_*, is NS_STYLE_FONT_SIZE_NO_KEYWORD
...
struct MOZ_NEEDS_MEMMOVABLE_MEMBERS nsStyleColor
{
  ...
  {
    return mozilla::LinearBlendColors(
        aColor.mColor, mColor, aColor.mForegroundRatio);
  }
...
#define NS_FOR_VISIBLE_IMAGE_LAYERS_BACK_TO_FRONT_WITH_RANGE(               \
    var_, layers_, start_, count_)                                          \
  NS_ASSERTION(                                                             \
...
static bool
IsVisibleBorderStyle(uint8_t aStyle)
{
...
struct nsBorderColors
{
  ...
  {
    NS_FOR_CSS_SIDES(side) { mColors[side] = aOther.mColors[side]; }
  }
...
struct nsStyleGridLine
{
  ...
  nsStyleGridLine() : mHasSpan(false), mInteger(0)
...
class BindingHolder
{
 public:
  ...
  mozilla::css::URLValue* Get() const
  {
    return (mPtr && mPtr->GetURI()) ? mPtr.get() : nullptr;
  }
...
struct MOZ_NEEDS_MEMMOVABLE_MEMBERS nsStyleDisplay
{
  ...
  uint8_t
      mScrollBehavior;  // [reset] see nsStyleConsts.h NS_STYLE_SCROLL_BEHAVIOR_*
  uint8_t
      mScrollSnapTypeX;  // [reset] see nsStyleConsts.h NS_STYLE_SCROLL_SNAP_TYPE_*
  uint8_t
      mScrollSnapTypeY;  // [reset] see nsStyleConsts.h NS_STYLE_SCROLL_SNAP_TYPE_*
  ...
  nsStyleCoord mTransformOrigin
      [3];  // [reset] percent, coord, calc, 3rd param is coord, calc only
  nsStyleCoord mChildPerspective;      // [reset] none, coord
  nsStyleCoord mPerspectiveOrigin[2];  // [reset] percent, coord, calc

  nsStyleCoord
      mVerticalAlign;  // [reset] coord, percent, calc, enum (see nsStyleConsts.h)
...
struct MOZ_NEEDS_MEMMOVABLE_MEMBERS nsStyleContent
{
  ...
  void SetCounterIncrementAt(uint32_t aIndex,
  ...
  void SetCounterResetAt(uint32_t aIndex,
                         const nsString& aCounter,
                         int32_t aValue)
  {
