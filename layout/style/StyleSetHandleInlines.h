...
{
  FORWARD_WITH_PARENT(
      ResolveStyleFor, aParentContext, (aElement, parent, aMayCompute));
}
                                     ...
                                     TreeMatchContext* aTreeMatchContext)
{
  if (IsGecko()) {
    ...
    return AsGecko()->ResolveStyleFor(
        aElement, parent, aMayCompute, *aTreeMatchContext);
  } else {
...
{
  FORWARD_WITH_PARENT(
      ResolveStyleForFirstLetterContinuation, aParentContext, (parent));
}
...
{
  FORWARD_WITH_PARENT(ResolvePseudoElementStyle,
                      aParentContext,
...
{
  FORWARD_WITH_PARENT(
      ResolveInheritingAnonymousBoxStyle, aParentContext, (aPseudoTag, parent));
}
...
{
  FORWARD_WITH_PARENT(ResolveXULTreePseudoStyle,
                      aParentContext,
...
{
  FORWARD_CONCRETE(
      AppendStyleSheet, (aType, aSheet->AsGecko()), (aType, aSheet->AsServo()));
}

nsresult
StyleSetHandle::Ptr::PrependStyleSheet(SheetType aType, StyleSheet* aSheet)
{
  FORWARD_CONCRETE(PrependStyleSheet,
                   (aType, aSheet->AsGecko()),
...
{
  FORWARD_CONCRETE(
      RemoveStyleSheet, (aType, aSheet->AsGecko()), (aType, aSheet->AsServo()));
}

nsresult
StyleSetHandle::Ptr::ReplaceSheets(
...
void
StyleSetHandle::Ptr::AppendAllXBLStyleSheets(
...
{
  FORWARD_CONCRETE(
      RemoveDocStyleSheet, (aSheet->AsGecko()), (aSheet->AsServo()));
}
...
{
  FORWARD_CONCRETE(AddDocStyleSheet,
                   (aSheet->AsGecko(), aDocument),
...
{
  FORWARD_CONCRETE(RecordStyleSheetChange,
                   (aSheet->AsGecko(), aChangeType),
                   (aSheet->AsServo(), aChangeType));
}

void
StyleSetHandle::Ptr::RecordShadowStyleChange(
...
{
  FORWARD_WITH_PARENT(
      ProbePseudoElementStyle, aParentContext, (aParentElement, aType, parent));
}

already_AddRefed<nsStyleContext>
StyleSetHandle::Ptr::ProbePseudoElementStyle(
    dom::Element* aParentElement,
    CSSPseudoElementType aType,
    nsStyleContext* aParentContext,
    TreeMatchContext* aTreeMatchContext)
{
  if (IsGecko()) {
    ...
    return AsGecko()->ProbePseudoElementStyle(
        aParentElement, aType, parent, *aTreeMatchContext);
  }
  auto* parent = aParentContext ? aParentContext->AsServo() : nullptr;
...
bool
StyleSetHandle::Ptr::AppendFontFaceRules(
