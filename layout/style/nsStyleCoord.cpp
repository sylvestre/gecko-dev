...
void
nsStyleCoord::SetCoordValue(nscoord aValue)
{
  Reset();
...
void
nsStyleCoord::SetIntValue(int32_t aValue, nsStyleUnit aUnit)
{
  NS_ASSERTION(
      (aUnit == eStyleUnit_Enumerated) || (aUnit == eStyleUnit_Integer),
      "not an int value");
...
void
nsStyleCoord::SetPercentValue(float aValue)
{
  Reset();
...
void
nsStyleCoord::SetFactorValue(float aValue)
{
  Reset();
...
void
nsStyleCoord::SetAngleValue(float aValue, nsStyleUnit aUnit)
{
  Reset();
...
void
nsStyleCoord::SetFlexFractionValue(float aValue)
{
  Reset();
...
void
nsStyleCoord::SetCalcValue(Calc* aValue)
{
  Reset();
...
nsStyleSides::operator=(const nsStyleSides& aCopy)
{
  if (this != &aCopy) {
    NS_FOR_CSS_SIDES(i)
    {
      nsStyleCoord::SetValue(
          mUnits[i], mValues[i], aCopy.mUnits[i], aCopy.mValues[i]);
    }
  }
  return *this;
}
...
#define CASE(corner, result) \
  static_assert(HalfToFullCorner(corner) == result, "HalfToFullCorner is wrong")
