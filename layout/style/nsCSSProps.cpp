...
SortPropertyAndCount(const void* s1, const void* s2, void* closure)
{
  ...
  if (pc1->count != pc2->count) return pc1->count - pc2->count;
...
nsCSSProps::BuildShorthandsContainingTable()
{
  ...
  NS_QuickSort(&subpropCounts,
               ArrayLength(subpropCounts),
               sizeof(subpropCounts[0]),
...
nsCSSPropertyID
nsCSSProps::LookupProperty(const nsACString& aProperty, EnabledState aEnabled)
{
...
bool
nsCSSProps::FindKeyword(nsCSSKeyword aKeyword,
                        const KTableEntry aTable[],
                        int32_t& aResult)
{
...
bool
nsCSSProps::GetColorName(int32_t aPropValue, nsCString& aStr)
{
  bool rv = false;
...
static const nsCSSPropertyID gFlexSubpropTable[] = {eCSSProperty_flex_grow,
...
static const nsCSSPropertyID gMarkerSubpropTable[] = {eCSSProperty_marker_start,
...
static const nsCSSPropertyID gMozTransformSubpropTable[] = {
    eCSSProperty_transform, eCSSProperty_UNKNOWN};
...
#define ENUM_DATA_FOR_PROPERTY(name_,              \
                               id_,                \
                               method_,            \
                               flags_,             \
                               pref_,              \
