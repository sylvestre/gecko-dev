...
void
GeckoStyleContext::DoClearCachedInheritedStyleDataOnDescendants(
    uint32_t aStructs)
{
...
#define GET_UNIQUE_STYLE_DATA(name_) \
  ...
  if (mPseudoTag == nsCSSPseudoElements::firstLetter) {
    ...
    if (textReset->mInitialLetterSize != 0.0f) {
      ...
      nscoord containerLH = ReflowInput::CalcLineHeight(
          nullptr, containerSC, NS_AUTOHEIGHT, 1.0f);
...
void
GeckoStyleContext::SwapStyleData(GeckoStyleContext* aNewContext,
                                 uint32_t aStructs)
{
...
void
GeckoStyleContext::SetStyleIfVisited(
