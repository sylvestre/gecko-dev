...
#define MOZ_DEFINE_STYLO_METHODS(type_, geckotype_, servotype_)               \
  ...
  geckotype_* type_::GetAsGecko() { return IsGecko() ? AsGecko() : nullptr; } \
  servotype_* type_::GetAsServo() { return IsServo() ? AsServo() : nullptr; } \
...
#define MOZ_STYLO_FORWARD_CONCRETE(method_, geckoargs_, servoargs_)      \
  static_assert(                                                         \
      !mozilla::IsSame<decltype(&MOZ_STYLO_THIS_TYPE::method_),          \
                       decltype(&MOZ_STYLO_GECKO_TYPE::method_)>::value, \
      "Gecko subclass should define its own " #method_);                 \
  static_assert(                                                         \
      !mozilla::IsSame<decltype(&MOZ_STYLO_THIS_TYPE::method_),          \
                       decltype(&MOZ_STYLO_SERVO_TYPE::method_)>::value, \
      "Servo subclass should define its own " #method_);                 \
  if (IsServo()) {                                                       \
