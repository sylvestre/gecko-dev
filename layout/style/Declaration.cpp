                                ...
                                const nsCSSPropertyID aTable[]) const
{
  ...
  for (;;) {
    ...
    AppendSingleImageLayerPositionValue(
        positionX->mValue, positionY->mValue, aTable, aValue);
...
void
Declaration::GetImageLayerPositionValue(nsCSSCompressedDataBlock* data,
                                        nsAString& aValue,
                                        const nsCSSPropertyID aTable[]) const
{
...
void
Declaration::GetPropertyValueInternal(nsCSSPropertyID aProperty,
                                      nsAString& aValue,
                                      bool* aIsTokenStream) const
{
  aValue.Truncate(0);
  ...
  switch (aProperty) {
    ...
    case eCSSProperty_border_width: {
      ...
      const nsCSSValue* vals[4] = {data->ValueFor(subprops[0]),
    ...
    case eCSSProperty_background: {
      GetImageLayerValue(
          data, aValue, nsStyleImageLayers::kBackgroundLayerTable);
      break;
    }
    ...
    case eCSSProperty_mask_position: {
      GetImageLayerPositionValue(
          data, aValue, nsStyleImageLayers::kMaskLayerTable);
      break;
    }
    ...
    for (const nsCSSPropertyID* shorthands =
      ...
      if (!value.IsEmpty()) {
        AppendPropertyAndValueToString(
            shorthand, aString, value, isTokenStream);
      ...
      if (shorthand == eCSSProperty_font) {
        if (haveSystemFont && !didSystemFont) {
          ...
          AppendPropertyAndValueToString(
              eCSSProperty_font, aString, value, isTokenStream);
          value.Truncate();
          didSystemFont = true;
        }
