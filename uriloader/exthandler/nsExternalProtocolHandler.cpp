...
NS_IMETHODIMP
nsExtProtocolChannel::GetLoadGroup(nsILoadGroup** aLoadGroup)
{
...
NS_IMETHODIMP
nsExtProtocolChannel::SetLoadGroup(nsILoadGroup* aLoadGroup)
{
...
NS_IMETHODIMP
nsExtProtocolChannel::SetOriginalURI(nsIURI* aURI)
{
...
NS_IMETHODIMP
nsExtProtocolChannel::GetURI(nsIURI** aURI)
{
  *aURI = mUrl;
  NS_IF_ADDREF(*aURI);
  return NS_OK;
...
nsExtProtocolChannel::OpenURL()
{
  ...
  if (extProtService) {
    ...
    rv = NS_NewNotificationCallbacksAggregation(
        mCallbacks, mLoadGroup, getter_AddRefs(aggCallbacks));
    if (NS_FAILED(rv)) {
      goto finish;
    }
...
NS_IMETHODIMP
nsExtProtocolChannel::Open(nsIInputStream** _retval)
{
  return OpenURL();
}

NS_IMETHODIMP
nsExtProtocolChannel::Open2(nsIInputStream** aStream)
{
...
NS_IMETHODIMP
nsExtProtocolChannel::AsyncOpen(nsIStreamListener* listener, nsISupports* ctxt)
{
  if (mConnectedParent) {
    return NS_OK;
  }
...
NS_IMETHODIMP
nsExtProtocolChannel::AsyncOpen2(nsIStreamListener* aListener)
{
...
NS_IMETHODIMP
nsExtProtocolChannel::GetLoadFlags(nsLoadFlags* aLoadFlags)
{
...
NS_IMETHODIMP
nsExtProtocolChannel::GetContentType(nsACString& aContentType)
{
  return NS_ERROR_NOT_IMPLEMENTED;
}

NS_IMETHODIMP
nsExtProtocolChannel::SetContentType(const nsACString& aContentType)
{
  return NS_ERROR_FAILURE;
}

NS_IMETHODIMP
nsExtProtocolChannel::GetContentCharset(nsACString& aContentCharset)
{
  return NS_ERROR_NOT_IMPLEMENTED;
}

NS_IMETHODIMP
nsExtProtocolChannel::SetContentCharset(const nsACString& aContentCharset)
{
  return NS_ERROR_NOT_IMPLEMENTED;
}
...
NS_IMETHODIMP
nsExtProtocolChannel::GetLoadInfo(nsILoadInfo** aLoadInfo)
{
...
NS_IMETHODIMP
nsExtProtocolChannel::SetLoadInfo(nsILoadInfo* aLoadInfo)
{
  mLoadInfo = aLoadInfo;
  return NS_OK;
}
...
NS_IMETHODIMP
nsExtProtocolChannel::GetName(nsACString& result)
{
...
NS_IMETHODIMP
nsExtProtocolChannel::GetStatus(nsresult* status)
{
  *status = mStatus;
  return NS_OK;
}
...
NS_IMETHODIMP
nsExtProtocolChannel::CompleteRedirectSetup(nsIStreamListener* listener,
                                            nsISupports* context)
{
...
NS_IMETHODIMP
nsExtProtocolChannel::SetParentListener(HttpChannelParentListener* aListener)
{
...
NS_IMETHODIMP
nsExtProtocolChannel::NotifyTrackingProtectionDisabled()
{
  // nothing to do
  return NS_OK;
}
...
NS_IMETHODIMP
nsExtProtocolChannel::SetClassifierMatchedInfo(const nsACString& aList,
...
NS_IMETHODIMP
nsExtProtocolChannel::NotifyTrackingResource()
{
  // nothing to do
  return NS_OK;
}
...
NS_IMETHODIMP
nsExtProtocolChannel::OnStartRequest(nsIRequest* aRequest,
...
NS_IMETHODIMP
nsExtProtocolChannel::OnStopRequest(nsIRequest* aRequest,
                                    nsISupports* aContext,
...
NS_IMETHODIMP
nsExtProtocolChannel::OnDataAvailable(nsIRequest* aRequest,
                                      nsISupports* aContext,
...
NS_IMETHODIMP
nsExternalProtocolHandler::GetScheme(nsACString& aScheme)
{
...
NS_IMETHODIMP
nsExternalProtocolHandler::AllowPort(int32_t port,
                                     const char* scheme,
                                     bool* _retval)
{
...
bool
nsExternalProtocolHandler::HaveExternalProtocolHandler(nsIURI* aURI)
{
  MOZ_ASSERT(aURI);
...
NS_IMETHODIMP
nsExternalProtocolHandler::NewURI(const nsACString& aSpec,
...
NS_IMETHODIMP
nsExternalProtocolHandler::NewChannel(nsIURI* aURI, nsIChannel** _retval)
{
...
//////////////////////////////////////////////////////////////////////
NS_IMETHODIMP
nsExternalProtocolHandler::ExternalAppExistsForScheme(const nsACString& aScheme,
                                                      bool* _retval)
{
  nsCOMPtr<nsIExternalProtocolService> extProtSvc(
      do_GetService(NS_EXTERNALPROTOCOLSERVICE_CONTRACTID));
