...
NS_IMETHODIMP
nsDecodeAppleFile::Initialize(nsIOutputStream* output, nsIFile* file)
{
  m_output = output;
...
nsDecodeAppleFile::Close(void)
{
  ...
  if (m_headerOk) {
    bool dataOk =
        true; /* It's ok if the file doesn't have a datafork, therefore set it to true by default. */
...
NS_IMETHODIMP
nsDecodeAppleFile::Flush(void)
{
  return m_output->Flush();
}

NS_IMETHODIMP
nsDecodeAppleFile::WriteFrom(nsIInputStream* inStr,
                             uint32_t count,
                             uint32_t* _retval)
{
    ...
    switch (m_state) {
      ...
      case parseEntries:
        ...
        if (m_dataBufferLength == entriesSize) {
          for (i = 0; i < m_headers.entriesCount; i++) {
            memcpy(&m_entries[i],
                   &m_dataBuffer[i * sizeof(ap_entry)],
                   sizeof(ap_entry));
            if (m_headers.magic == APPLEDOUBLE_MAGIC) {
          ...
          switch (m_currentPartID) {
            ...
            case ENT_FINFO:
              if (m_currentPartLength ==
                ...
                memcpy(&m_finderExtraInfo,
                       buffPtr + sizeof(m_finderInfo),
