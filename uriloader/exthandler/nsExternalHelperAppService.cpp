 ...
 */
static nsresult
UnescapeFragment(const nsACString& aFragment, nsIURI* aURI, nsAString& aResult)
{
  ...
  return textToSubURI->UnEscapeURIForUI(
      NS_LITERAL_CSTRING("UTF-8"), aFragment, aResult);
}
 ...
 */
static nsresult
UnescapeFragment(const nsACString& aFragment, nsIURI* aURI, nsACString& aResult)
{
  nsAutoString result;
 ...
 */
static bool
GetFilenameAndExtensionFromChannel(nsIChannel* aChannel,
                                   nsString& aFileName,
 ...
 */
static nsresult
GetDownloadDirectory(nsIFile** _directory, bool aSkipChecks = false)
{
  ...
  switch (Preferences::GetInt(NS_PREF_DOWNLOAD_FOLDERLIST, -1)) {
    ...
    case NS_FOLDER_VALUE_CUSTOM: {
      Preferences::GetComplex(
          NS_PREF_DOWNLOAD_DIR, NS_GET_IID(nsIFile), getter_AddRefs(dir));
      if (!dir) break;
  ...
  if (downloadDir) {
    ...
    rv = NS_NewNativeLocalFile(
        downloadDir->ToCString(), true, getter_AddRefs(ldir));
...
#if defined( \
    XP_MACOSX)  // don't define .bin on the mac...use internet config to look that up...
...
NS_IMPL_ISUPPORTS(nsExternalHelperAppService,
    ...
    nsIStreamListener** aStreamListener)
{
  ...
  RefPtr<nsExternalAppHandler> handler =
      new nsExternalAppHandler(nullptr,
                               EmptyCString(),
                               aContentContext,
                               aWindowContext,
                               this,
...
NS_IMETHODIMP
nsExternalHelperAppService::DoContent(const nsACString& aMimeContentType,
                                      nsIRequest* aRequest,
                                      nsIInterfaceRequestor* aContentContext,
                                      bool aForceSave,
                                      nsIInterfaceRequestor* aWindowContext,
                                      nsIStreamListener** aStreamListener)
{
  if (XRE_IsContentProcess()) {
    return DoContentContentProcessHelper(aMimeContentType,
                                         aRequest,
                                         aContentContext,
                                         aForceSave,
                                         aWindowContext,
  ...
  LOG(("HelperAppService::DoContent: mime '%s', extension '%s'\n",
       PromiseFlatCString(aMimeContentType).get(),
       fileExtension.get()));
  ...
  if (aMimeContentType.Equals(APPLICATION_GUESS_FROM_EXT,
    ...
    if (!fileExtension.IsEmpty()) {
      mimeSvc->GetFromTypeAndExtension(
          EmptyCString(), fileExtension, getter_AddRefs(mimeInfo));
      if (mimeInfo) {
    ...
    if (fileExtension.IsEmpty() || mimeType.IsEmpty()) {
      ...
      mimeSvc->GetFromTypeAndExtension(
          NS_LITERAL_CSTRING(APPLICATION_OCTET_STREAM),
          fileExtension,
    ...
    mimeSvc->GetFromTypeAndExtension(
        aMimeContentType, fileExtension, getter_AddRefs(mimeInfo));
  }
...
NS_IMETHODIMP
nsExternalHelperAppService::GetApplicationDescription(const nsACString& aScheme,
                                                      nsAString& _retval)
{
...
void
nsExternalHelperAppService::ExpungeTemporaryFilesHelper(
...
NS_IMETHODIMP
nsExternalAppHandler::GetTargetFile(nsIFile** aTarget)
{
...
NS_IMETHODIMP
nsExternalAppHandler::GetTimeDownloadStarted(PRTime* aTime)
{
...
void
nsExternalAppHandler::RetargetLoadNotifications(nsIRequest* request)
{
 ...
 */
void
nsExternalAppHandler::EnsureSuggestedFileName()
{
...
nsresult
nsExternalAppHandler::SetUpTempFile(nsIChannel* aChannel)
{
      ...
      if (encEnum) {
        ...
        if (NS_SUCCEEDED(rv) && hasMore) {
          ...
          if (NS_SUCCEEDED(rv) && !encType.IsEmpty()) {
            ...
            mExtProtSvc->ApplyDecodingForExtension(
                extension, encType, &applyConversion);
          }
        }
      }
    }
...
NS_IMETHODIMP
nsExternalAppHandler::OnStartRequest(nsIRequest* request, nsISupports* aCtxt)
{
...
void
nsExternalAppHandler::SendStatusChange(ErrorType type,
                                       nsresult rv,
                                       nsIRequest* aRequest,
                                       const nsString& path)
{
  ...
  MOZ_LOG(
      ...
      ("Error: %s, type=%i, listener=0x%p, transfer=0x%p, rv=0x%08" PRIX32 "\n",
       msgId,
       type,
       mDialogProgressListener.get(),
       mTransfer.get(),
       static_cast<uint32_t>(rv)));

  MOZ_LOG(nsExternalHelperAppService::mLog,
          LogLevel::Error,
                                      ...
                                      uint32_t count)
{
  ...
  if (count > 0) {
    ...
    if (NS_SUCCEEDED(rv)) {
      ...
      if (mTransfer) {
        mTransfer->OnProgressChange64(nullptr,
                                      request,
                                      mProgress,
                                      mContentLength,
                                      mProgress,
...
NS_IMETHODIMP
nsExternalAppHandler::OnStopRequest(nsIRequest* request,
                                    nsISupports* aCtxt,
                                    nsresult aStatus)
{
  LOG(
...
void
nsExternalAppHandler::NotifyTransfer(nsresult aStatus)
{
  ...
  (void)mTransfer->OnStateChange(nullptr,
                                 nullptr,
...
NS_IMETHODIMP
nsExternalAppHandler::GetSource(nsIURI** aSourceURI)
{
...
NS_IMETHODIMP
nsExternalAppHandler::GetSuggestedFileName(nsAString& aSuggestedFileName)
{
...
nsExternalAppHandler::CreateTransfer()
{
  ...
  rv = transfer->OnStateChange(nullptr,
                               mRequest,
...
nsresult
nsExternalAppHandler::SaveDestinationAvailable(nsIFile* aFile)
{
  if (aFile)
    ContinueSave(aFile);
  else
...
void
nsExternalAppHandler::RequestSaveDestination(const nsString& aDefaultFile,
                                             const nsString& aFileExtension)
{
  // Display the dialog
...
NS_IMETHODIMP
nsExternalAppHandler::SaveToDisk(nsIFile* aNewFileLocation,
...
nsresult
nsExternalAppHandler::ContinueSave(nsIFile* aNewFileLocation)
{
  if (mCanceled) return NS_OK;
...
NS_IMETHODIMP
nsExternalAppHandler::LaunchWithApplication(nsIFile* aApplication,
                                            bool aRememberThisPreference)
{
  if (mCanceled) return NS_OK;
...
void
nsExternalAppHandler::ProcessAnyRefreshTags()
{
                                              ...
                                              const char* aContentType)
{
  ...
  return !CaseInsensitiveFindInReadable(
      nsDependentCString(aContentType), start, end);
}
...
nsExternalAppHandler::MaybeCloseWindow()
{
  ...
  if (mShouldCloseWindow) {
    ...
    if (opener && !opener->Closed()) {
      ...
      MOZ_TRY_VAR(mTimer,
                  NS_NewTimerWithCallback(this, 0, nsITimer::TYPE_ONE_SHOT));
...
NS_IMETHODIMP
nsExternalHelperAppService::GetFromTypeAndExtension(const nsACString& aMIMEType,
...
NS_IMETHODIMP
nsExternalHelperAppService::GetPrimaryExtension(const nsACString& aMIMEType,
                                                const nsACString& aFileExt,
                                                nsACString& _retval)
{
...
NS_IMETHODIMP
nsExternalHelperAppService::GetTypeFromURI(nsIURI* aURI,
                                           nsACString& aContentType)
{
  NS_ENSURE_ARG_POINTER(aURI);
...
NS_IMETHODIMP
nsExternalHelperAppService::GetTypeFromFile(nsIFile* aFile,
                                            nsACString& aContentType)
{
...
bool
nsExternalHelperAppService::GetMIMETypeFromOSForExtension(
