...
static nsresult
GetIconURLVariant(nsIFile* aApplication, nsIVariant** _retval)
{
...
nsMIMEInfoWin::LoadUriInternal(nsIURI* aURL)
{
  ...
  if (aURL) {
    ...
    if (NS_FAILED(textToSubURI->UnEscapeNonAsciiURI(
            NS_LITERAL_CSTRING("UTF-8"), urlSpec, utf16Spec))) {
...
bool
nsMIMEInfoWin::GetLocalHandlerApp(const nsAString& aCommandHandler,
...
bool
nsMIMEInfoWin::GetAppsVerbCommandHandler(const nsAString& appExeName,
...
bool
nsMIMEInfoWin::GetDllLaunchInfo(nsIFile* aDll,
                                nsIFile* aFile,
                                nsAString& args,
                                bool edit)
{
  ...
  if (NS_SUCCEEDED(
    ...
    if (!::ExpandEnvironmentStringsW(
            appFilesystemCommand.get(), destination.get(), bufLength))
      return false;
...
bool
nsMIMEInfoWin::GetProgIDVerbCommandHandler(const nsAString& appProgIDName,
                                           nsAString& applicationPath,
                                           bool edit)
{
  nsCOMPtr<nsIWindowsRegKey> appKey =
...
void
nsMIMEInfoWin::ProcessPath(nsCOMPtr<nsIMutableArray>& appList,
...
static bool
IsPathInList(nsAString& appPath, nsTArray<nsString>& trackList)
{
    ...
    if (NS_SUCCEEDED(rv)) {
      ...
      if (NS_SUCCEEDED(regKey->ReadStringValue(EmptyString(), appProgId))) {
        ...
        if (appProgId != NS_LITERAL_STRING("XPSViewer.Document")) {
          nsAutoString appFilesystemCommand;
          if (GetProgIDVerbCommandHandler(
                  appProgId, appFilesystemCommand, false) &&
    ...
    if (NS_SUCCEEDED(rv)) {
      ...
      if (NS_SUCCEEDED(regKey->GetValueCount(&count)) && count > 0) {
        for (uint32_t index = 0; index < count; index++) {
          ...
          nsAutoString appFilesystemCommand;
          if (!GetAppsVerbCommandHandler(
                  appName, appFilesystemCommand, false) ||
              IsPathInList(appFilesystemCommand, trackList))
            continue;
          ProcessPath(appList, trackList, appFilesystemCommand);
        }
    ...
    if (NS_SUCCEEDED(rv)) {
      ...
      if (NS_SUCCEEDED(regKey->GetValueCount(&count)) && count > 0) {
        for (uint32_t index = 0; index < count; index++) {
          ...
          nsAutoString appFilesystemCommand;
          if (!GetAppsVerbCommandHandler(
                  appValue, appFilesystemCommand, false) ||
              IsPathInList(appFilesystemCommand, trackList))
            continue;
          ProcessPath(appList, trackList, appFilesystemCommand);
        }
