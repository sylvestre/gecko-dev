...
nsDocumentOpenInfo::nsDocumentOpenInfo(nsIInterfaceRequestor* aWindowContext,
                                       uint32_t aFlags,
                                       nsURILoader* aURILoader)
    : m_originalContext(aWindowContext), mFlags(aFlags), mURILoader(aURILoader)
{
}
...
NS_IMETHODIMP
nsDocumentOpenInfo::OnStartRequest(nsIRequest* request, nsISupports* aCtxt)
{
...
nsDocumentOpenInfo::CheckListenerChain()
{
  ...
  LOG(
      ("[0x%p] nsDocumentOpenInfo::CheckListenerChain %s listener %p rv "
       ...
       (nsIStreamListener*)m_targetStreamListener,
       static_cast<uint32_t>(rv)));
  return rv;
}
                                    ...
                                    uint32_t count)
{
  ...
  if (m_targetStreamListener)
    rv = m_targetStreamListener->OnDataAvailable(
        request, aCtxt, inStr, sourceOffset, count);
  return rv;
}

NS_IMETHODIMP
nsDocumentOpenInfo::OnStopRequest(nsIRequest* request,
                                  nsISupports* aCtxt,
                                  nsresult aStatus)
{
...
nsresult
nsDocumentOpenInfo::DispatchContent(nsIRequest* request, nsISupports* aCtxt)
{
  LOG(("[0x%p] nsDocumentOpenInfo::DispatchContent for type '%s'",
       this,
       mContentType.get()));
  ...
  if (!forceExternalHandling) {
    ...
    if (!(mFlags & nsIURILoader::DONT_RETARGET)) {
      ...
      if (contentHandler) {
        ...
        rv = contentHandler->HandleContent(
            mContentType.get(), m_originalContext, request);
...
nsDocumentOpenInfo::ConvertData(nsIRequest* request,
                                ...
                                const nsACString& aOutContentType)
{
  LOG(("[0x%p] nsDocumentOpenInfo::ConvertData from '%s' to '%s'",
       this,
                                       ...
                                       nsIChannel* aChannel)
{
  ...
  if (mFlags & nsIURILoader::IS_CONTENT_PREFERRED) {
    aListener->IsPreferred(
        mContentType.get(), getter_Copies(typeToUse), &listenerWantsContent);
  } else {
    aListener->CanHandleContent(mContentType.get(),
                                false,
...
NS_IMETHODIMP
nsURILoader::RegisterContentListener(nsIURIContentListener* aContentListener)
{
  nsresult rv = NS_OK;
...
NS_IMETHODIMP
nsURILoader::UnRegisterContentListener(nsIURIContentListener* aContentListener)
{
  nsWeakPtr weakListener = do_GetWeakReference(aContentListener);
                     ...
                     nsIInterfaceRequestor* aWindowContext)
{
  ...
  nsresult rv = OpenChannel(
      channel, aFlags, aWindowContext, false, getter_AddRefs(loader));
...
NS_IMETHODIMP
nsURILoader::Stop(nsISupports* aLoadCookie)
{
  nsresult rv;
