...
OfflineCacheUpdateGlue::EnsureUpdate()
{
  if (!mUpdate) {
    ...
    LOG(("OfflineCacheUpdateGlue [%p] is using update [%p]",
         this,
         mUpdate.get()));
  }
                             ...
                             nsIFile* aCustomProfileDir)
{
  ...
  if (service) {
    service->FindUpdate(
        aManifestURI, originSuffix, aCustomProfileDir, getter_AddRefs(mUpdate));
    mCoalesced = !!mUpdate;
  }
  ...
  if (mCoalesced) {  // already initialized
    LOG(("OfflineCacheUpdateGlue %p coalesced with update %p",
         this,
         mUpdate.get()));
    return NS_OK;
  }

  return mUpdate->Init(aManifestURI,
                       aDocumentURI,
                       aLoadingPrincipal,
                       nullptr,
                       aCustomProfileDir);
