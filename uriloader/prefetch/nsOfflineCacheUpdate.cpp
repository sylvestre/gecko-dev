...
nsresult
nsOfflineManifestItem::HandleManifestLine(
    ...
    const nsCString::const_iterator& aEnd)
{
  ...
  switch (mParserState) {
    ...
    case PARSE_FALLBACK_ENTRIES: {
      ...
      if (!NS_SecurityCompareURIs(
              namespaceURI, fallbackURI, mStrictFileOriginPolicy))
        break;
    ...
    case PARSE_BYPASS_ENTRIES: {
      ...
      AddNamespace(
          nsIApplicationCacheNamespace::NAMESPACE_BYPASS, spec, EmptyCString());
      break;
    }
  }
                           ...
                           nsIFile* aCustomProfileDir)
{
  ...
  if (aCustomProfileDir) {
    rv = cacheService->BuildGroupIDForSuffix(
        aManifestURI, originSuffix, mGroupID);
    NS_ENSURE_SUCCESS(rv, rv);
  ...
  rv = nsOfflineCacheUpdateService::OfflineAppPinnedForURI(
      aDocumentURI, nullptr, &mPinned);
  NS_ENSURE_SUCCESS(rv, rv);
...
void
nsOfflineCacheUpdate::GatherObservers(
...
nsOfflineCacheUpdate::ScheduleImplicit()
{
  ...
  rv = update->InitPartial(
      mManifestURI, clientID, mDocumentURI, mLoadingPrincipal);
  NS_ENSURE_SUCCESS(rv, rv);
...
nsOfflineCacheUpdate::AddObserver(nsIOfflineCacheUpdateObserver* aObserver,
                                  bool aHoldWeak)
{
  LOG(("nsOfflineCacheUpdate::AddObserver [%p] to update [%p]",
       aObserver,
       this));
...
{
  LOG(("nsOfflineCacheUpdate::RemoveObserver [%p] from update [%p]",
       aObserver,
       this));
