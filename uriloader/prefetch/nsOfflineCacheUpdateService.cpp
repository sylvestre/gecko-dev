...
nsOfflineCacheUpdateService::AllowedDomains()
{
                                           ...
                                           nsresult aStatus)
{
  ...
  if (NS_SUCCEEDED(aStatus)) {
    ...
    mService->Schedule(mManifestURI,
                       mDocumentURI,
                       mLoadingPrincipal,
                       updateDoc,
                       innerWindow,
    ...
    : mDisabled(false), mUpdateRunning(false), mLowFreeSpace(false)
{
  ...
  Preferences::AddBoolVarCache(
      &sAllowOfflineCache, "browser.cache.offline.enable", true);
}
...
nsOfflineCacheUpdateService::GetInstance()
{
  if (!gOfflineCacheUpdateService) {
    ...
    if (!gOfflineCacheUpdateService) return nullptr;
...
NS_IMETHODIMP
nsOfflineCacheUpdateService::CheckForUpdate(nsIURI* aManifestURI,
                                            nsIPrincipal* aLoadingPrincipal,
...
nsOfflineCacheUpdateService::AllowOfflineApp(nsIPrincipal* aPrincipal)
{
  ...
  if (GeckoProcessType_Default != XRE_GetProcessType()) {
    ...
    rv = permissionManager->AddFromPrincipal(aPrincipal,
                                             "offline-app",
