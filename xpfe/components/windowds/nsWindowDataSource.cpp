...
nsWindowDataSource::Init()
{
  ...
  if (gRefCnt++ == 0) {
    ...
    gRDFService->GetResource(NS_LITERAL_CSTRING(URINC_WINDOWROOT),
                             &kNC_WindowRoot);
...
NS_IMETHODIMP
nsWindowDataSource::Observe(nsISupports* aSubject,
                            const char* aTopic,
                                        ...
                                        const nsAString& newTitle)
{
  ...
  rv = GetTarget(windowResource, kNC_Name, true, getter_AddRefs(oldTitleNode));
...
nsWindowDataSource::OnOpenWindow(nsIXULWindow* window)
{
  ...
  if (mContainer) mContainer->AppendElement(windowResource);
...
nsWindowDataSource::OnCloseWindow(nsIXULWindow* window)
{
  ...
  rv = GetTarget(resource, kNC_KeyIndex, true, getter_AddRefs(oldKeyNode));
  ...
  while (NS_SUCCEEDED(rv = children->HasMoreElements(&more)) && more) {
    ...
    if (currentIndex < winIndex) continue;

    nsCOMPtr<nsIRDFNode> newKeyNode;
    nsCOMPtr<nsIRDFInt> newKeyInt;

    rv = GetTarget(
        windowResource, kNC_KeyIndex, true, getter_AddRefs(newKeyNode));
    if (NS_SUCCEEDED(rv) && (rv != NS_RDF_NO_VALUE))
...
NS_IMETHODIMP
nsWindowDataSource::GetURI(nsACString& aURI)
{
...
NS_IMETHODIMP
nsWindowDataSource::GetTarget(nsIRDFResource* aSource,
...
NS_IMETHODIMP
nsWindowDataSource::GetSource(nsIRDFResource* aProperty,
                              nsIRDFNode* aTarget,
                              bool aTruthValue,
...
NS_IMETHODIMP
nsWindowDataSource::GetSources(nsIRDFResource* aProperty,
                               nsIRDFNode* aTarget,
                               bool aTruthValue,
...
NS_IMETHODIMP
nsWindowDataSource::GetTargets(nsIRDFResource* aSource,
                               nsIRDFResource* aProperty,
                               bool aTruthValue,
                               nsISimpleEnumerator** _retval)
{
...
NS_IMETHODIMP
nsWindowDataSource::Assert(nsIRDFResource* aSource,
                           nsIRDFResource* aProperty,
                           nsIRDFNode* aTarget,
                           bool aTruthValue)
{
  if (mInner) return mInner->Assert(aSource, aProperty, aTarget, aTruthValue);
  return NS_OK;
}

NS_IMETHODIMP
nsWindowDataSource::Unassert(nsIRDFResource* aSource,
                             nsIRDFResource* aProperty,
                             nsIRDFNode* aTarget)
{
  if (mInner) return mInner->Unassert(aSource, aProperty, aTarget);
  return NS_OK;
}

NS_IMETHODIMP
nsWindowDataSource::Change(nsIRDFResource* aSource,
                           nsIRDFResource* aProperty,
                           nsIRDFNode* aOldTarget,
                           nsIRDFNode* aNewTarget)
{
  if (mInner) return mInner->Change(aSource, aProperty, aOldTarget, aNewTarget);
  return NS_OK;
}

NS_IMETHODIMP
nsWindowDataSource::Move(nsIRDFResource* aOldSource,
                         nsIRDFResource* aNewSource,
                         nsIRDFResource* aProperty,
                         nsIRDFNode* aTarget)
{
  if (mInner) return mInner->Move(aOldSource, aNewSource, aProperty, aTarget);
  return NS_OK;
}

NS_IMETHODIMP
nsWindowDataSource::HasAssertion(nsIRDFResource* aSource,
                                 nsIRDFResource* aProperty,
                                 nsIRDFNode* aTarget,
                                 bool aTruthValue,
                                 bool* _retval)
{
  if (mInner)
    return mInner->HasAssertion(
        aSource, aProperty, aTarget, aTruthValue, _retval);
  return NS_OK;
}

NS_IMETHODIMP
nsWindowDataSource::AddObserver(nsIRDFObserver* aObserver)
{
  if (mInner) return mInner->AddObserver(aObserver);
  return NS_OK;
}

NS_IMETHODIMP
nsWindowDataSource::RemoveObserver(nsIRDFObserver* aObserver)
{
  if (mInner) return mInner->RemoveObserver(aObserver);
  return NS_OK;
}

NS_IMETHODIMP
nsWindowDataSource::ArcLabelsIn(nsIRDFNode* aNode,
                                nsISimpleEnumerator** _retval)
{
  if (mInner) return mInner->ArcLabelsIn(aNode, _retval);
  return NS_OK;
}

NS_IMETHODIMP
nsWindowDataSource::ArcLabelsOut(nsIRDFResource* aSource,
                                 nsISimpleEnumerator** _retval)
{
  if (mInner) return mInner->ArcLabelsOut(aSource, _retval);
  return NS_OK;
}

NS_IMETHODIMP
nsWindowDataSource::GetAllResources(nsISimpleEnumerator** _retval)
{
  if (mInner) return mInner->GetAllResources(_retval);
  return NS_OK;
}

NS_IMETHODIMP
nsWindowDataSource::IsCommandEnabled(nsISupports* aSources,
...
NS_IMETHODIMP
nsWindowDataSource::DoCommand(nsISupports* aSources,
                              nsIRDFResource* aCommand,
                              nsISupports* aArguments)
{
  return NS_ERROR_NOT_IMPLEMENTED;
...
NS_IMETHODIMP
nsWindowDataSource::GetAllCmds(nsIRDFResource* aSource,
                               nsISimpleEnumerator** _retval)
{
  if (mInner) return mInner->GetAllCmds(aSource, _retval);
  return NS_OK;
}

NS_IMETHODIMP
nsWindowDataSource::HasArcIn(nsIRDFNode* aNode,
                             nsIRDFResource* aArc,
                             bool* _retval)
{
  if (mInner) return mInner->HasArcIn(aNode, aArc, _retval);
  return NS_OK;
}

NS_IMETHODIMP
nsWindowDataSource::HasArcOut(nsIRDFResource* aSource,
                              nsIRDFResource* aArc,
                              bool* _retval)
{
  if (mInner) return mInner->HasArcOut(aSource, aArc, _retval);
  return NS_OK;
}
...
{
  if (mInner) return mInner->BeginUpdateBatch();
  return NS_OK;
}

NS_IMETHODIMP
nsWindowDataSource::EndUpdateBatch()
{
  if (mInner) return mInner->EndUpdateBatch();
  return NS_OK;
}
...
static const mozilla::Module kWindowDSModule = {mozilla::Module::kVersion,
                                                kWindowDSCIDs,
                                                kWindowDSContracts,
                                                kWindowDSCategories};
