                             ...
                             uint32_t aCount)
{
  ...
  return mParser->OnDataAvailable(
      request, mDirectory, aStream, aSourceOffset, aCount);
}

nsresult
nsHTTPIndex::OnIndexAvailable(nsIRequest* aRequest,
                              nsISupports* aContext,
                              nsIDirIndex* aIndex)
{
...
nsresult
nsHTTPIndex::Create(nsIURI* aBaseURL,
...
void
nsHTTPIndex::GetDestination(nsIRDFResource* r, nsACString& dest)
{
...
nsresult
nsHTTPIndex::AddElement(nsIRDFResource* parent,
                        nsIRDFResource* prop,
                        nsIRDFNode* child)
{
  if (!mNodeList) {
...
NS_IMETHODIMP
nsHTTPIndex::IsCommandEnabled(nsISupports* aSources,
...
NS_IMETHODIMP
nsHTTPIndex::DoCommand(nsISupports* aSources,
                       nsIRDFResource* aCommand,
                       nsISupports* aArguments)
{
  return NS_ERROR_NOT_IMPLEMENTED;
                                         ...
                                         nsIContentViewer** aDocViewerResult)
{
  ...
  rv = catMan->GetCategoryEntry(
      "Gecko-Content-Viewers", "text/html", getter_Copies(contractID));
  if (NS_FAILED(rv)) return rv;
  ...
  if (viewSource) {
    ...
    rv = factory->CreateInstance("view",
                                 aChannel,
                                 aLoadGroup,
