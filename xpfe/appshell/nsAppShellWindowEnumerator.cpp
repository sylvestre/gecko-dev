...
GetAttribute(nsIXULWindow* inWindow,
             ...
             nsAString& outValue);
static void
GetWindowType(nsIXULWindow* inWindow, nsString& outType);
...
void
GetWindowType(nsIXULWindow* aWindow, nsString& outType)
{
...
bool
nsWindowInfo::TypeEquals(const nsAString& aType)
{
...
void
nsWindowInfo::InsertAfter(nsWindowInfo* inOlder, nsWindowInfo* inHigher)
{
  if (inOlder) {
    mOlder = inOlder;
...
void
nsWindowInfo::Unlink(bool inAge, bool inZ)
{
  if (inAge) {
...
void
nsWindowInfo::ReferenceSelf(bool inAge, bool inZ)
{
  if (inAge) {
    mYounger = this;
    mOlder = this;
...
void
nsAppShellWindowEnumerator::AdjustInitialPosition()
{
...
NS_IMETHODIMP
nsAppShellWindowEnumerator::HasMoreElements(bool* retval)
{
...
void
nsAppShellWindowEnumerator::WindowRemoved(nsWindowInfo* inInfo)
{
...
NS_IMETHODIMP
nsASDOMWindowEnumerator::GetNext(nsISupports** retval)
{
  if (!retval) return NS_ERROR_INVALID_ARG;
...
NS_IMETHODIMP
nsASXULWindowEnumerator::GetNext(nsISupports** retval)
{
  if (!retval) return NS_ERROR_INVALID_ARG;
...
nsWindowInfo*
nsASDOMWindowEarlyToLateEnumerator::FindNext()
{
...
nsWindowInfo*
nsASXULWindowEarlyToLateEnumerator::FindNext()
{
  nsWindowInfo *info, *listEnd;
  bool allWindows = mType.IsEmpty();
...
nsWindowInfo*
nsASDOMWindowFrontToBackEnumerator::FindNext()
{
  nsWindowInfo *info, *listEnd;
  bool allWindows = mType.IsEmpty();
...
nsWindowInfo*
nsASXULWindowFrontToBackEnumerator::FindNext()
{
  nsWindowInfo *info, *listEnd;
  bool allWindows = mType.IsEmpty();
...
nsWindowInfo*
nsASDOMWindowBackToFrontEnumerator::FindNext()
{
  nsWindowInfo *info, *listEnd;
  bool allWindows = mType.IsEmpty();
...
nsWindowInfo*
nsASXULWindowBackToFrontEnumerator::FindNext()
{
  nsWindowInfo *info, *listEnd;
  bool allWindows = mType.IsEmpty();
