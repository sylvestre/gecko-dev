...
NS_IMETHODIMP
nsContentTreeOwner::GetInterface(const nsIID& aIID, void** aSink)
{
...
NS_IMETHODIMP
nsContentTreeOwner::SizeShellTo(nsIDocShellTreeItem* aShellItem,
                                int32_t aCX,
                                int32_t aCY)
{
  NS_ENSURE_STATE(mXULWindow);
                                   ...
                                   bool aPersistSizeMode)
{
  ...
  ErrorResult rv;
  if (saveString) {
    docShellElement->SetAttribute(
        NS_LITERAL_STRING("persist"), persistString, rv);
  }
...
NS_IMETHODIMP
nsContentTreeOwner::OnBeforeLinkTraversal(const nsAString& originalTarget,
                                          nsIURI* linkURI,
                                          nsIDOMNode* linkNode,
                                          bool isAppTab,
                                          nsAString& _retval)
{
  ...
  if (xulBrowserWindow)
    return xulBrowserWindow->OnBeforeLinkTraversal(
        originalTarget, linkURI, linkNode, isAppTab, _retval);
...
NS_IMETHODIMP
nsContentTreeOwner::ShouldLoadURI(nsIDocShell* aDocShell,
                                  nsIURI* aURI,
                                  nsIURI* aReferrer,
                                  bool aHasPostData,
                                  ...
                                  bool* _retval)
{
  ...
  if (xulBrowserWindow)
    return xulBrowserWindow->ShouldLoadURI(aDocShell,
                                           aURI,
                                           aReferrer,
                                           aHasPostData,
...
NS_IMETHODIMP
nsContentTreeOwner::ShouldLoadURIInThisProcess(nsIURI* aURI, bool* aRetVal)
{
...
NS_IMETHODIMP
nsContentTreeOwner::ReloadInFreshProcess(nsIDocShell* aDocShell,
                                         nsIURI* aURI,
                                         nsIURI* aReferrer,
                                         nsIPrincipal* aTriggeringPrincipal,
                                         uint32_t aLoadFlags,
...
NS_IMETHODIMP
nsContentTreeOwner::StartPrerenderingDocument(
    nsIURI* aHref, nsIURI* aReferrer, nsIPrincipal* aTriggeringPrincipal)
{
...
NS_IMETHODIMP
nsContentTreeOwner::ShouldSwitchToPrerenderedDocument(nsIURI* aHref,
                                                      nsIURI* aReferrer,
...
NS_IMETHODIMP
nsContentTreeOwner::SetStatusWithContext(uint32_t aStatusType,
...
NS_IMETHODIMP
nsContentTreeOwner::SetStatus(uint32_t aStatusType, const char16_t* aStatus)
{
...
NS_IMETHODIMP
nsContentTreeOwner::SetWebBrowser(nsIWebBrowser* aWebBrowser)
{
...
NS_IMETHODIMP
nsContentTreeOwner::GetWebBrowser(nsIWebBrowser** aWebBrowser)
{
...
NS_IMETHODIMP
nsContentTreeOwner::DestroyBrowserWindow()
{
  NS_ERROR("Haven't Implemented this yet");
  return NS_ERROR_FAILURE;
}
...
NS_IMETHODIMP
nsContentTreeOwner::SizeBrowserTo(int32_t aCX, int32_t aCY)
{
  NS_ERROR("Haven't Implemented this yet");
  return NS_ERROR_FAILURE;
}
...
NS_IMETHODIMP
nsContentTreeOwner::ExitModalEventLoop(nsresult aStatus)
{
  NS_ENSURE_STATE(mXULWindow);
...
NS_IMETHODIMP
nsContentTreeOwner::GetUnscaledDevicePixelsPerCSSPixel(double* aScale)
{
  NS_ENSURE_STATE(mXULWindow);
...
NS_IMETHODIMP
nsContentTreeOwner::GetDevicePixelsPerDesktopPixel(double* aScale)
{
  NS_ENSURE_STATE(mXULWindow);
...
NS_IMETHODIMP
nsContentTreeOwner::SetPositionDesktopPix(int32_t aX, int32_t aY)
{
  NS_ENSURE_STATE(mXULWindow);
...
NS_IMETHODIMP
nsContentTreeOwner::SetPosition(int32_t aX, int32_t aY)
{
  NS_ENSURE_STATE(mXULWindow);
...
NS_IMETHODIMP
nsContentTreeOwner::GetPosition(int32_t* aX, int32_t* aY)
{
  NS_ENSURE_STATE(mXULWindow);
...
NS_IMETHODIMP
nsContentTreeOwner::SetSize(int32_t aCX, int32_t aCY, bool aRepaint)
{
  NS_ENSURE_STATE(mXULWindow);
...
NS_IMETHODIMP
nsContentTreeOwner::GetSize(int32_t* aCX, int32_t* aCY)
{
  NS_ENSURE_STATE(mXULWindow);
...
NS_IMETHODIMP
nsContentTreeOwner::SetPositionAndSize(
...
NS_IMETHODIMP
nsContentTreeOwner::GetParentWidget(nsIWidget** aParentWidget)
{
  NS_ENSURE_STATE(mXULWindow);
...
NS_IMETHODIMP
nsContentTreeOwner::SetParentWidget(nsIWidget* aParentWidget)
{
  NS_ASSERTION(false, "You can't call this");
...
NS_IMETHODIMP
nsContentTreeOwner::GetParentNativeWindow(nativeWindow* aParentNativeWindow)
{
  NS_ENSURE_STATE(mXULWindow);
...
NS_IMETHODIMP
nsContentTreeOwner::SetParentNativeWindow(nativeWindow aParentNativeWindow)
{
  NS_ASSERTION(false, "You can't call this");
  return NS_ERROR_NOT_IMPLEMENTED;
}
...
NS_IMETHODIMP
nsContentTreeOwner::GetNativeHandle(nsAString& aNativeHandle)
{
  NS_ENSURE_STATE(mXULWindow);
...
NS_IMETHODIMP
nsContentTreeOwner::GetMainWidget(nsIWidget** aMainWidget)
{
...
NS_IMETHODIMP
nsContentTreeOwner::GetTitle(nsAString& aTitle)
{
  NS_ENSURE_STATE(mXULWindow);
...
NS_IMETHODIMP
nsContentTreeOwner::SetTitle(const nsAString& aTitle)
{
  ...
  *aWindowIsNew = (openLocation != nsIBrowserDOMWindow::OPEN_CURRENTWINDOW);

  {
    ...
    return browserDOMWin->CreateContentWindow(
        aURI, aParent, openLocation, flags, nullPrincipal, aReturn);
  }
}
 ...
 public:
  ...
  NS_IMETHOD Run() override
  {
    ...
    mElement->SetAttribute(
        NS_LITERAL_STRING("titledefault"), mTitleDefault, rv);
...
void
nsContentTreeOwner::XULWindow(nsXULWindow* aXULWindow)
{
...
NS_IMETHODIMP
nsSiteWindow::SetDimensions(
    uint32_t aFlags, int32_t aX, int32_t aY, int32_t aCX, int32_t aCY)
{
  // XXX we're ignoring aFlags
  return mAggregator->SetPositionAndSize(
      aX, aY, aCX, aCY, nsIBaseWindow::eRepaint);
}

NS_IMETHODIMP
nsSiteWindow::GetDimensions(
    uint32_t aFlags, int32_t* aX, int32_t* aY, int32_t* aCX, int32_t* aCY)
{
  // XXX we're ignoring aFlags
