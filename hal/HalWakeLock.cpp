...
NS_IMETHODIMP
ClearHashtableOnShutdown::Observe(nsISupports* aSubject,
                                  const char* aTopic,
                                  const char16_t* data)
{
...
NS_IMETHODIMP
CleanupOnContentShutdown::Observe(nsISupports* aSubject,
                                  const char* aTopic,
                                  const char16_t* data)
{
...
Init()
{
  ...
  if (obs) {
    ...
    obs->AddObserver(
        new CleanupOnContentShutdown(), "ipc:content-shutdown", false);
  }
}
