...
template<>
inline void
RefCounted<LibHandle, AtomicRefCount>::Release() const;
...
class SystemElf : public LibHandle
{
 ...
 private:
   ...
   * Private constructor
   */
  SystemElf(const char* path, void* handle) : LibHandle(path), dlhandle(handle)
  {
  }
...
class SEGVHandler
{
 public:
  ...
  static int __wrap_sigaction(int signum,
...
class ElfLoader : public SEGVHandler
{
 public:
   ...
   */
  already_AddRefed<LibHandle> Load(const char* path,
                                   int flags,
 ...
 protected:
   ...
   */
#ifdef __ARM_EABI__
  static int __wrap_aeabi_atexit(void* that,
                                 Destructor destructor,
                                 void* dso_handle);
#else
  static int __wrap_cxa_atexit(Destructor destructor,
                               void* that,
                               void* dso_handle);
#endif
