...
void
debug_phdr(const char* type, const Phdr* phdr)
{
...
// Skip on Android until bug 706116 is fixed
   ...
   * MAP_SHARED. */
  elf->base.Assign(MemoryRange::mmap(
      nullptr, max_vaddr, PROT_NONE, MAP_SHARED | MAP_ANONYMOUS, -1, 0));
  ...
  report_mapping(const_cast<char*>(elf->GetName()),
                 elf->base,
                 (max_vaddr + PAGE_SIZE - 1) & PAGE_MASK,
                 0);
  ...
  DEBUG_LOG("CustomElf::Load(\"%s\", 0x%x) = %p",
            path,
            flags,
...
CustomElf::LoadSegment(const Phdr* pt_load) const
{
  ...
  do {
    ...
    DEBUG_LOG("%s: Loading segment @%p %c%c%c",
              GetPath(),
              where,
              prot & PROT_READ ? 'r' : '-',
              prot & PROT_WRITE ? 'w' : '-',
              prot & PROT_EXEC ? 'x' : '-');
    mapped = mappable->mmap(where,
                            pt_load->p_filesz + align_offset,
                            prot,
...
void
debug_dyn(const char* type, const Dyn* dyn)
{
