...
AlignedPtr(uintptr_t ptr, size_t alignment)
{
...
template<typename T>
static inline T*
AlignedPtr(T* ptr, size_t alignment)
{
...
static inline uintptr_t
AlignedEndPtr(uintptr_t ptr, size_t alignment)
{
...
template<typename T>
static inline T*
AlignedEndPtr(T* ptr, size_t alignment)
{
  return reinterpret_cast<T*>(
...
static inline size_t
AlignedSize(size_t size, size_t alignment)
{
  return (size + alignment - 1) & ~(alignment - 1);
}

static inline size_t
PageAlignedSize(size_t size)
{
  return AlignedSize(size, PageSize());
}

static inline bool
IsAlignedPtr(uintptr_t ptr, size_t alignment)
{
...
template<typename T>
static inline bool
IsAlignedPtr(T* ptr, size_t alignment)
{
...
template<typename T>
static inline bool
IsPageAlignedPtr(T ptr)
{
  return IsAlignedPtr(ptr, PageSize());
}

static inline bool
IsAlignedSize(size_t size, size_t alignment)
{
  return size % alignment == 0;
}

static inline bool
IsPageAlignedSize(size_t size)
{
  return IsAlignedSize(size, PageSize());
}

static inline size_t
PageNumber(size_t size)
{
...
{
  MappedPtr(void* buf, size_t length) : GenericMappedPtr<MappedPtr>(buf, length)
  {
  }
