...
void
setup_sqlite_functions(void* sqlite_handle)
{
#define GETFUNC(name) \
...
static void
throwSqliteException(JNIEnv* jenv, const char* aFormat, ...)
{
  va_list ap;
  va_start(ap, aFormat);
  char* msg = nullptr;
...
Java_org_mozilla_gecko_sqlite_SQLiteBridge_sqliteCall(JNIEnv* jenv,
                                                      jclass,
                                                      jstring jDb,
                                                      jstring jQuery,
                                                      jobjectArray jParams,
...
Java_org_mozilla_gecko_sqlite_SQLiteBridge_openDatabase(JNIEnv* jenv,
                                                        jclass,
                                                        jstring jDb)
{
  JNI_Setup(jenv);
...
Java_org_mozilla_gecko_sqlite_SQLiteBridge_closeDatabase(JNIEnv* jenv,
                                                         jclass,
                                                         jlong jDb)
{
  JNI_Setup(jenv);
                   ...
                   jlongArray jQueryRes)
{
  ...
  if (rc != SQLITE_OK || ppStmt == nullptr) {
    throwSqliteException(
        jenv, "Can't prepare statement: %s", f_sqlite3_errmsg(db));
    return nullptr;
  }
    ...
    if (numPars > 0) {
      for (int i = 0; i < numPars; i++) {
        ...
        if (jObjectParam == nullptr) {
          ...
          rc = f_sqlite3_bind_text(
              ppStmt, i + 1, paramStr, -1, SQLITE_TRANSIENT);
  ...
  if (rc != SQLITE_ROW && rc != SQLITE_DONE) {
    throwSqliteException(
        jenv, "Can't step statement: (%d) %s", rc, f_sqlite3_errmsg(db));
    return nullptr;
  }
  ...
  while (rc != SQLITE_DONE) {
    ...
    if (rc != SQLITE_ROW && rc != SQLITE_DONE) {
      throwSqliteException(
          jenv, "Can't re-step statement:(%d) %s", rc, f_sqlite3_errmsg(db));
      return nullptr;
    }
  }
  ...
  if (rc != SQLITE_OK) {
    throwSqliteException(
        jenv, "Can't finalize statement: %s", f_sqlite3_errmsg(db));
    return nullptr;
  }
