...
void
TransportLayerNSPRAdapter::PacketReceived(const void* data, int32_t len)
{
  if (enabled_) {
...
int32_t
TransportLayerNSPRAdapter::Recv(void* buf, int32_t buflen)
{
  if (input_.empty()) {
...
int32_t
TransportLayerNSPRAdapter::Write(const void* buf, int32_t length)
{
  if (!enabled_) {
...
static PRStatus
TransportLayerClose(PRFileDesc* f)
{
  f->dtor(f);
  return PR_SUCCESS;
}
...
static int32_t
TransportLayerRead(PRFileDesc* f, void* buf, int32_t length)
{
  UNIMPLEMENTED;
  return -1;
}
...
static int32_t
TransportLayerWrite(PRFileDesc* f, const void* buf, int32_t length)
{
...
static int32_t
TransportLayerAvailable(PRFileDesc* f)
{
  UNIMPLEMENTED;
  return -1;
}
...
int64_t
TransportLayerAvailable64(PRFileDesc* f)
{
  UNIMPLEMENTED;
  return -1;
}
...
static PRStatus
TransportLayerSync(PRFileDesc* f)
{
  UNIMPLEMENTED;
  return PR_FAILURE;
}
...
static int32_t
TransportLayerSeek(PRFileDesc* f, int32_t offset, PRSeekWhence how)
{
  UNIMPLEMENTED;
  return -1;
}
...
static int64_t
TransportLayerSeek64(PRFileDesc* f, int64_t offset, PRSeekWhence how)
{
  UNIMPLEMENTED;
  return -1;
}
...
static PRStatus
TransportLayerFileInfo(PRFileDesc* f, PRFileInfo* info)
{
  UNIMPLEMENTED;
  return PR_FAILURE;
}
...
static PRStatus
TransportLayerFileInfo64(PRFileDesc* f, PRFileInfo64* info)
{
  UNIMPLEMENTED;
  return PR_FAILURE;
}
...
static int32_t
TransportLayerWritev(PRFileDesc* f,
                     const PRIOVec* iov,
                     int32_t iov_size,
                     PRIntervalTime to)
{
...
static PRStatus
TransportLayerConnect(PRFileDesc* f, const PRNetAddr* addr, PRIntervalTime to)
{
  UNIMPLEMENTED;
  return PR_FAILURE;
}
...
static PRFileDesc*
TransportLayerAccept(PRFileDesc* sd, PRNetAddr* addr, PRIntervalTime to)
{
  UNIMPLEMENTED;
  return nullptr;
}
...
static PRStatus
TransportLayerBind(PRFileDesc* f, const PRNetAddr* addr)
{
  UNIMPLEMENTED;
  return PR_FAILURE;
}
...
static PRStatus
TransportLayerListen(PRFileDesc* f, int32_t depth)
{
  UNIMPLEMENTED;
  return PR_FAILURE;
}
...
static PRStatus
TransportLayerShutdown(PRFileDesc* f, int32_t how)
{
...
static int32_t
TransportLayerRecv(
    PRFileDesc* f, void* buf, int32_t buflen, int32_t flags, PRIntervalTime to)
{
  MOZ_ASSERT(flags == 0);
  if (flags != 0) {
    PR_SetError(PR_NOT_IMPLEMENTED_ERROR, 0);
...
static int32_t
TransportLayerSend(PRFileDesc* f,
                   const void* buf,
                   int32_t amount,
                   int32_t flags,
                   PRIntervalTime to)
...
static int32_t
TransportLayerRecvfrom(PRFileDesc* f,
                       void* buf,
                       int32_t amount,
                       int32_t flags,
                       PRNetAddr* addr,
...
static int32_t
TransportLayerSendto(PRFileDesc* f,
                     const void* buf,
                     int32_t amount,
                     int32_t flags,
...
static int16_t
TransportLayerPoll(PRFileDesc* f, int16_t in_flags, int16_t* out_flags)
{
  UNIMPLEMENTED;
  return -1;
}
...
static int32_t
TransportLayerAcceptRead(PRFileDesc* sd,
                         PRFileDesc** nd,
                         PRNetAddr** raddr,
                         void* buf,
                         int32_t amount,
...
static int32_t
TransportLayerTransmitFile(PRFileDesc* sd,
                           PRFileDesc* f,
                           const void* headers,
                           int32_t hlen,
...
static PRStatus
TransportLayerGetpeername(PRFileDesc* f, PRNetAddr* addr)
{
...
static PRStatus
TransportLayerGetsockname(PRFileDesc* f, PRNetAddr* addr)
{
  UNIMPLEMENTED;
  return PR_FAILURE;
}
...
static PRStatus
TransportLayerGetsockoption(PRFileDesc* f, PRSocketOptionData* opt)
{
  switch (opt->option) {
...
static PRStatus
TransportLayerSetsockoption(PRFileDesc* f, const PRSocketOptionData* opt)
{
  switch (opt->option) {
    case PR_SockOpt_Nonblocking:
      return PR_SUCCESS;
...
static int32_t
TransportLayerSendfile(PRFileDesc* out,
                       PRSendFileData* in,
                       PRTransmitFileFlags flags,
                       PRIntervalTime to)
{
...
static PRStatus
TransportLayerConnectContinue(PRFileDesc* f, int16_t flags)
{
  UNIMPLEMENTED;
  return PR_FAILURE;
}
...
static int32_t
TransportLayerReserved(PRFileDesc* f)
{
  UNIMPLEMENTED;
  return -1;
}
...
void
TransportLayerDtls::WasInserted()
{
...
nsresult
TransportLayerDtls::SetAlpn(const std::set<std::string>& alpn_allowed,
...
static const SSLNamedGroup NamedGroupPreferences[] = {ssl_grp_ec_curve25519,
...
TransportLayerDtls::Setup()
{
  ...
  rv = SSL_OptionSet(
      ssl_fd.get(), SSL_ENABLE_RENEGOTIATION, SSL_RENEGOTIATE_NEVER);
  if (rv != SECSuccess) {
  ...
  rv = SSL_NamedGroupConfig(ssl_fd.get(),
  ...
  rv = SSL_AuthCertificateHook(
      ssl_fd.get(), AuthCertificateHook, reinterpret_cast<void*>(this));
  if (rv != SECSuccess) {
...
bool
TransportLayerDtls::SetupAlpn(UniquePRFileDesc& ssl_fd) const
{
...
bool
TransportLayerDtls::SetupCipherSuites(UniquePRFileDesc& ssl_fd) const
{
  SECStatus rv;
  ...
  if (!srtp_ciphers_.empty()) {
    ...
    rv = SSL_SetSRTPCiphers(
        ssl_fd.get(), &srtp_ciphers_[0], srtp_ciphers_.size());
    if (rv != SECSuccess) {
  ...
  for (const auto& cipher : DisabledCiphers) {
    ...
    if (rv != SECSuccess) {
      MOZ_MTLOG(
          ML_NOTICE,
          LAYER_INFO << "Unable to check if suite is enabled: " << cipher);
      return false;
    }
    if (enabled) {
...
nsresult
TransportLayerDtls::GetCipherSuite(uint16_t* cipherSuite) const
{
  CheckThread();
  if (!cipherSuite) {
...
void
TransportLayerDtls::StateChange(TransportLayer* layer, State state)
{
  if (state <= state_) {
  ...
  if (rv == SECSuccess) {
    ...
    switch (err) {
      ...
      default:
        ...
        MOZ_MTLOG(ML_ERROR,
                  LAYER_INFO << "DTLS handshake error " << err << " ("
                             << err_msg << ")");
        TL_SET_STATE(TS_ERROR);
        break;
    }
...
TransportLayerDtls::CheckAlpn()
{
  ...
  SECStatus rv = SSL_GetNextProto(ssl_fd_.get(),
                                  &alpnState,
  ...
  if (alpn_allowed_.find(chosen) == alpn_allowed_.end()) {
    ...
    MOZ_MTLOG(ML_ERROR,
              LAYER_INFO << "Bad ALPN string: '" << chosen
                         << "'; permitted:" << ss.str());
    return false;
  }
  alpn_ = chosen;
...
void
TransportLayerDtls::PacketReceived(TransportLayer* layer,
...
void
TransportLayerDtls::SetState(State state, const char* file, unsigned line)
{
  if (state > state_) {
    switch (state) {
      case TS_NONE:
...
TransportResult
TransportLayerDtls::SendPacket(const unsigned char* data, size_t len)
{
  CheckThread();
  if (state_ != TS_OPEN) {
    MOZ_MTLOG(ML_ERROR,
...
nsresult
TransportLayerDtls::SetSrtpCiphers(std::vector<uint16_t> ciphers)
{
...
nsresult
TransportLayerDtls::GetSrtpCipher(uint16_t* cipher) const
{
  CheckThread();
  if (state_ != TS_OPEN) {
    return NS_ERROR_NOT_AVAILABLE;
...
nsresult
TransportLayerDtls::ExportKeyingMaterial(const std::string& label,
                                         bool use_context,
                                ...
                                UniqueCERTCertificate& peer_cert) const
{
  ...
  nsresult res = DtlsIdentity::ComputeFingerprint(peer_cert,
                                                  digest->algorithm_,
                                                  computed_digest,
  ...
  if (NS_FAILED(res)) {
    MOZ_MTLOG(
        ML_ERROR,
        "Could not compute peer fingerprint for digest " << digest->algorithm_);
    // Go to end
  ...
  if (computed_digest_len != digest->len_) {
    MOZ_MTLOG(ML_ERROR,
              "Digest is wrong length "
                  << digest->len_ << " should be " << computed_digest_len
                  << " for algorithm " << digest->algorithm_);
    PR_SetError(SSL_ERROR_BAD_CERTIFICATE, 0);
    return SECFailure;
  }
...
SECStatus
TransportLayerDtls::AuthCertificateHook(PRFileDesc* fd,
                                        PRBool checksig,
                                        PRBool isServer)
{
  CheckThread();
...
void
TransportLayerDtls::TimerCallback(nsITimer* timer, void* arg)
{
