...
class MultiTcpSocketTest : public MtransportTest
{
 public:
  ...
  void Create_s(nr_socket_tcp_type tcp_type,
                ...
                nr_socket** sock)
  {
    ...
    for (int tries = 10; tries && r; --tries) {
      ...
      r = nr_socket_multi_tcp_create(
          ice_ctx_->ctx()->ctx(), &local, tcp_type, 1, 2048, sock);
    }
  ...
  void SendDataToAddress_s(nr_socket* from,
                ...
                size_t len)
  {
    test_utils_->sts_target()->Dispatch(
        WrapRunnable(this,
                     &MultiTcpSocketTest::SendDataToAddress_s,
                     from,
                     to,
                     data,
                     len),
  ...
  void SendDataToSocket_s(nr_socket* from,
                          nr_socket* to,
                          const char* data,
                          size_t len)
  {
                             ...
                             size_t expected_len)
  {
    ...
    printf("Receiving %lu bytes %s <- %s\n",
           ...
           addr_to.as_string,
           expected_from->as_string);
    r = nr_socket_recvfrom(
        sent_to, received_data, buflen, &retlen, 0, &retaddr);
    ASSERT_EQ(0, r);
    r = nr_transport_addr_cmp(
        &retaddr, expected_from, NR_TRANSPORT_ADDR_CMP_MODE_ALL);
    ASSERT_EQ(0, r);
  ...
  void RecvData(nr_transport_addr* expected_from,
                nr_socket* sent_to,
                const char* expected_data = nullptr,
                size_t expected_len = 0)
  {
    ASSERT_TRUE_WAIT(IsReadable(), 1000);
    test_utils_->sts_target()->Dispatch(
        WrapRunnable(this,
                     &MultiTcpSocketTest::RecvDataFromAddress_s,
                     expected_from,
                     sent_to,
                     expected_data,
                     expected_len),
  ...
  void RecvDataFromSocket_s(nr_socket* expected_from,
                            nr_socket* sent_to,
                            const char* expected_data,
                            size_t expected_len)
  {
  ...
  void RecvData(nr_socket* expected_from,
                nr_socket* sent_to,
                const char* expected_data,
                size_t expected_len)
  {
    ...
    test_utils_->sts_target()->Dispatch(
        WrapRunnable(this,
                     &MultiTcpSocketTest::RecvDataFromSocket_s,
                     expected_from,
                     sent_to,
                     expected_data,
                     expected_len),
  ...
  void RecvDataFailed_s(nr_socket* sent_to,
                        size_t expected_len,
                        int expected_err)
  {
    SetReadable(false);
    ...
    ASSERT_EQ(0, r);
    r = nr_socket_recvfrom(
        sent_to, received_data, expected_len + 1, &retlen, 0, &retaddr);
    ASSERT_EQ(expected_err, r) << "Expecting receive failure " << expected_err
                               << " on " << addr_to.as_string;
  }

  void RecvDataFailed(nr_socket* sent_to, size_t expected_len, int expected_err)
  {
    ASSERT_TRUE_WAIT(IsReadable(), 1000);
    test_utils_->sts_target()->Dispatch(
        WrapRunnable(this,
                     &MultiTcpSocketTest::RecvDataFailed_s,
                     sent_to,
                     expected_len,
                     expected_err),
        NS_DISPATCH_SYNC);
  ...
  void TransferData(nr_socket* from,
                    nr_socket* to,
                    const char* data,
                    size_t len)
  {
...
TEST_F(MultiTcpSocketTest, TestActivePassiveWithStunServerMockup)
{
  ...
  int r = nr_str_port_to_transport_addr(
      stun_addr.c_str(), stun_port, IPPROTO_TCP, &stun_srv_addr);
  ASSERT_EQ(0, r);
