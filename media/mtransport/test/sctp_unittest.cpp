...
class TransportTestPeer : public sigslot::has_slots<>
{
 public:
        ...
        test_utils_(utils)
  {
    ...
    l.l_onoff = 1;
    l.l_linger = 0;
    r = usrsctp_setsockopt(
        sctp_, SOL_SOCKET, SO_LINGER, &l, (socklen_t)sizeof(l));
    EXPECT_GE(r, 0);
    ...
    r = usrsctp_setsockopt(
        sctp_, IPPROTO_SCTP, SCTP_EVENT, &subscription, sizeof(subscription));
    EXPECT_GE(r, 0);
  ...
  static TransportResult SendPacket_s(const unsigned char* data,
                                      size_t len,
  ...
  TransportResult SendPacket(const unsigned char* data, size_t len)
  {
    ...
    RUN_ON_THREAD(
        ...
        WrapRunnableNM(&TransportTestPeer::SendPacket_s, buffer, len, flow_),
        NS_DISPATCH_NORMAL);
...
NS_IMETHODIMP
SendPeriodic::Notify(nsITimer* timer)
{
  peer_->SendOne();
  --to_send_;
  if (!to_send_) {
