...
void
TransportLayerLoopback::Connect(TransportLayerLoopback* peer)
{
  peer_ = peer;
...
nsresult
TransportLayerLoopback::QueuePacket(const unsigned char* data, size_t len)
{
  MOZ_ASSERT(packets_lock_);
  ...
  if (combinePackets_ && !packets_.empty()) {
    ...
    MOZ_MTLOG(ML_DEBUG,
              LAYER_INFO << " Enqueuing combined packets of length "
                         << packet->len() << " and " << len);
...
void
TransportLayerLoopback::DeliverPackets()
{
...
NS_IMETHODIMP
TransportLayerLoopback::Deliverer::Notify(nsITimer* timer)
{
...
NS_IMETHODIMP
TransportLayerLoopback::Deliverer::GetName(nsACString& aName)
{
