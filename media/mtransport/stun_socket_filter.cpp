...
bool
STUNUDPSocketFilter::filter_incoming_packet(
    const mozilla::net::NetAddr* remote_addr, const uint8_t* data, uint32_t len)
{
  // Check white list
  ...
  if (it != pending_requests_.end()) {
    if (nr_is_stun_message(reinterpret_cast<UCHAR*>(const_cast<uint8_t*>(data)),
                           len)) {
...
bool
STUNTCPSocketFilter::filter_incoming_packet(const uint8_t* data, uint32_t len)
{
...
bool
STUNTCPSocketFilter::filter_outgoing_packet(const uint8_t* data, uint32_t len)
{
  // check if white listed already
  if (white_listed_) {
    return true;
...
NS_IMETHODIMP
nsStunUDPSocketFilterHandler::NewFilter(nsISocketFilter** result)
{
...
NS_IMETHODIMP
nsStunTCPSocketFilterHandler::NewFilter(nsISocketFilter** result)
{
