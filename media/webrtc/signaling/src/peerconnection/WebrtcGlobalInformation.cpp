...
template<class Request, typename Callback, typename Result, typename QueryParam>
...
class StatsRequest : public RequestManager<StatsRequest,
                                           StatsRequestCallback,
                                           ...
                                           nsAString>
{
 public:
  ...
  explicit StatsRequest(int aId,
                        StatsRequestCallback& aCallback,
                        nsAString& aFilter)
...
class LogRequest : public RequestManager<LogRequest,
                                         LogRequestCallback,
                                         Sequence<nsString>,
                                         const nsACString>
{
 public:
  ...
  explicit LogRequest(int aId,
                      LogRequestCallback& aCallback,
...
void
WebrtcContentParents::Dealloc(WebrtcGlobalParent* aParent)
{
  if (aParent) {
...
static void
OnGetLogging_m(WebrtcGlobalChild* aThisChild,
               const int aRequestId,
...
static void
GetLogging_s(WebrtcGlobalChild* aThisChild,
             const int aRequestId,
...
void
WebrtcGlobalInformation::ClearAllStats(const GlobalObject& aGlobal)
{
  if (!NS_IsMainThread()) {
    return;
  }
    ...
    ErrorResult& aRv)
{
  ...
  if (ctx) {
    rv = RunStatsQuery(
        ctx->mGetPeerConnections(), filter, nullptr, request->mRequestId);
...
RunLogClear()
{
  ...
  return RUN_ON_THREAD(
      stsThread, WrapRunnableNM(&ClearLogs_s), NS_DISPATCH_NORMAL);
}

void
WebrtcGlobalInformation::ClearLogging(const GlobalObject& aGlobal)
{
  if (!NS_IsMainThread()) {
    return;
  }
...
void
WebrtcGlobalInformation::SetDebugLevel(const GlobalObject& aGlobal,
                                       int32_t aLevel)
{
  if (aLevel) {
...
void
WebrtcGlobalInformation::GetAecDebugLogDir(const GlobalObject& aGlobal,
                                           nsAString& aDir)
{
                                       ...
                                       nsTArray<RTCStatsReportInternal>&& Stats)
{
  ...
  if (ctx) {
    rv = RunStatsQuery(
        ctx->mGetPeerConnections(), request->mPcIdFilter, nullptr, aRequestId);
  } else {
...
MOZ_IMPLICIT
WebrtcGlobalParent::WebrtcGlobalParent()
    : mShutdown(false)
{
                                       ...
                                       const nsString& aPcIdFilter)
{
  ...
  if (ctx) {
    nsresult rv = RunStatsQuery(
        ctx->mGetPeerConnections(), aPcIdFilter, this, aRequestId);
    if (NS_FAILED(rv)) {
      return IPC_FAIL_NO_REASON(this);
    }
    return IPC_OK();
...
MOZ_IMPLICIT
WebrtcGlobalChild::WebrtcGlobalChild()
    : mShutdown(false)
{
...
static uint32_t
GetCandidateIpAndTransportMask(const RTCIceCandidateStats* cand)
{
  enum
  {
...
static void
StoreLongTermICEStatisticsImpl_m(nsresult result,
...
static void
GetStatsForLongTermStorage_s(nsAutoPtr<RTCStatsQuery> query)
{
  MOZ_ASSERT(query);
...
void
WebrtcGlobalInformation::StoreLongTermICEStatistics(PeerConnectionImpl& aPc)
{
