...
void
VideoCodecStatistics::Register(bool encoder)
{
...
void
VideoCodecStatistics::OutgoingRate(const int video_channel,
...
void
VideoCodecStatistics::IncomingCodecChanged(const int video_channel,
...
void
VideoCodecStatistics::IncomingRate(const int video_channel,
...
void
VideoCodecStatistics::ReceiveStateChange(const int aChannel,
                                         VideoReceiveState aState)
{
  CSFLogDebug(
      logTag, "New state for %d: %d (was %d)", aChannel, aState, mReceiveState);
...
void
VideoCodecStatistics::EndOfCallStats()
{
  if (!mFirstDecodeTime.IsNull()) {
    ...
    if (callDelta.ToSeconds() != 0) {
      ...
      CSFLogError(
          logTag, "Video recovery before error per min %u", recovered_per_min);
      Telemetry::Accumulate(
...
void
VideoCodecStatistics::SentFrame()
{
  mSentRawFrames++;
}

void
VideoCodecStatistics::Dump()
{
...
void
VideoCodecStatistics::Dump(RunningStat& s, const char* name)
{
  CSFLogDebug(logTag,
