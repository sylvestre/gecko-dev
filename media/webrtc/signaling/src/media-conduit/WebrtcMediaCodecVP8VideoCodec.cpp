...
CreateDecoder(const char* aMimeType)
{
  if (!aMimeType) {
    return nullptr;
  }
...
static MediaCodec::LocalRef
CreateEncoder(const char* aMimeType)
{
  if (!aMimeType) {
    return nullptr;
  }
 ...
 public:
                     ...
                     bool encoder)
  {
    ...
    if (!mCoder) {
      ...
      res = MediaFormat::CreateVideoFormat(
          nsCString(mime), mWidth, mHeight, &format);
      ...
      if (NS_FAILED(res)) {
        CSFLogDebug(LOGTAG,
                    "WebrtcAndroidMediaCodec::%s, err = %d",
                    __FUNCTION__,
                    (int)res);
      }
    }
  ...
  void GenerateVideoFrame(size_t width,
                          size_t height,
                          uint32_t timeStamp,
                          void* decoded,
                          int color_format)
  {
    ...
    libyuv::NV12ToI420(src_nv12,
                       width,
                       src_nv12 + src_nv12_y_size,
                       (width + 1) & ~1,
                       dstY,
                       width,
                       dstU,
                              ...
                              int64_t renderTimeMs)
  {
...
#ifdef WEBRTC_MEDIACODEC_DEBUG
    CSFLogDebug(LOGTAG,
                "%s dequeue input buffer took %u ms",
                __FUNCTION__,
    ...
    if (inputIndex >= 0) {
      CSFLogError(LOGTAG,
                  "%s queue input buffer inputIndex = %d",
                  __FUNCTION__,
                  inputIndex);
                       ...
                       Monitor& aMonitor)
  {
    ...
    if (outputIndex == MediaCodec::INFO_TRY_AGAIN_LATER) {
      ...
      CSFLogDebug(LOGTAG,
                  "%s dequeue output buffer try again:%d",
                  __FUNCTION__,
                  outputIndex);
    } else if (outputIndex == MediaCodec::INFO_OUTPUT_FORMAT_CHANGED) {
      // handle format change
      CSFLogDebug(LOGTAG,
                  "%s dequeue output buffer format changed:%d",
                  __FUNCTION__,
                  outputIndex);
    } else if (outputIndex == MediaCodec::INFO_OUTPUT_BUFFERS_CHANGED) {
      CSFLogDebug(LOGTAG,
                  "%s dequeue output buffer changed:%d",
                  __FUNCTION__,
                  outputIndex);
      GetOutputBuffers();
    } else if (outputIndex < 0) {
      CSFLogDebug(LOGTAG,
                  "%s dequeue output buffer unknow error:%d",
                  __FUNCTION__,
                  outputIndex);
      ...
      if (buffer) {
        ...
        CSFLogDebug(
            ...
            frame.height_,
            frame.timeStamp_);
        GenerateVideoFrame(frame.width_,
                           frame.height_,
                           frame.timeStamp_,
                           directBuffer,
                           color_format);
  ...
  void QueueInputBuffer(int32_t inputIndex,
                        int32_t offset,
                        size_t size,
                        int64_t renderTimes,
                        int32_t flags)
...
static bool
I420toNV12(uint8_t* dstY, uint16_t* dstUV, const webrtc::VideoFrame& inputImage)
{
...
bool
WebrtcMediaCodecVP8VideoEncoder::ResetInputBuffers()
{
...
bool
WebrtcMediaCodecVP8VideoEncoder::ResetOutputBuffers()
{
...
int32_t
WebrtcMediaCodecVP8VideoEncoder::InitEncode(
...
int32_t
WebrtcMediaCodecVP8VideoEncoder::Encode(
    ...
    const std::vector<webrtc::FrameType>* frame_types)
{
  ...
  if (!mMediaCodecEncoder->isStarted) {
    ...
    nsresult res =
        mMediaCodecEncoder->Configure(mFrameWidth,
                                      mFrameHeight,
                                      nullptr,
    ...
    if (res != NS_OK) {
      CSFLogDebug(LOGTAG,
                  "%s, encoder configure return err = %d",
                  __FUNCTION__,
                  (int)res);
      return WEBRTC_VIDEO_CODEC_ERROR;
    }
  ...
  if (inputIndex >= 0) {
    ...
    mMediaCodecEncoder->QueueInputBuffer(
        ...
        0,
        size,
        inputImage.render_time_ms() * PR_USEC_PER_MSEC /* ms to us */,
        0);
#ifdef WEBRTC_MEDIACODEC_DEBUG
    CSFLogDebug(LOGTAG,
    ...
    if (outputIndex == MediaCodec::INFO_TRY_AGAIN_LATER) {
      ...
      if (buffer) {
        ...
        if (VerifyAndAllocate(size) == -1) {
          CSFLogDebug(
              LOGTAG, "%s VerifyAndAllocate buffers failed", __FUNCTION__);
          return WEBRTC_VIDEO_CODEC_ERROR;
        }
...
int32_t
WebrtcMediaCodecVP8VideoEncoder::RegisterEncodeCompleteCallback(
...
int32_t
WebrtcMediaCodecVP8VideoEncoder::Release()
{
  CSFLogDebug(LOGTAG, "%s ", __FUNCTION__);
...
int32_t
WebrtcMediaCodecVP8VideoEncoder::SetChannelParameters(uint32_t packetLoss,
                                                      int64_t rtt)
{
  CSFLogDebug(LOGTAG, "%s ", __FUNCTION__);
  return WEBRTC_VIDEO_CODEC_OK;
...
int32_t
WebrtcMediaCodecVP8VideoEncoder::SetRates(uint32_t newBitRate,
                                          uint32_t frameRate)
{
  CSFLogDebug(LOGTAG, "%s ", __FUNCTION__);
  if (!mMediaCodecEncoder) {
...
int32_t
WebrtcMediaCodecVP8VideoRemoteEncoder::InitEncode(
    const webrtc::VideoCodec* codecSettings,
    int32_t numberOfCores,
    size_t maxPayloadSize)
{
...
int32_t
WebrtcMediaCodecVP8VideoRemoteEncoder::SetRates(uint32_t newBitRate,
                                                uint32_t frameRate)
{
  CSFLogDebug(LOGTAG,
              "%s, newBitRate: %d, frameRate: %d",
              __FUNCTION__,
              newBitRate,
              frameRate);
  if (!mJavaEncoder) {
...
int32_t
WebrtcMediaCodecVP8VideoRemoteEncoder::Encode(
    const webrtc::VideoFrame& inputImage,
    const webrtc::CodecSpecificInfo* codecSpecificInfo,
    const std::vector<webrtc::FrameType>* frame_types)
{
  ...
  if (!mJavaEncoder) {
    ...
    nsresult res =
        MediaFormat::CreateVideoFormat(nsCString(MEDIACODEC_VIDEO_MIME_VP8),
                                       inputImage.width(),
                                       inputImage.height(),
                                       &format);
    ...
    mJavaEncoder = CodecProxy::Create(
        true, format, nullptr, mJavaCallbacks, EmptyString());
...
int32_t
WebrtcMediaCodecVP8VideoRemoteEncoder::RegisterEncodeCompleteCallback(
    webrtc::EncodedImageCallback* callback)
{
  mCallback = callback;
  return WEBRTC_VIDEO_CODEC_OK;
...
int32_t
WebrtcMediaCodecVP8VideoRemoteEncoder::Release()
{
  CSFLogDebug(LOGTAG, "%s %p", __FUNCTION__, this);
...
bool
WebrtcMediaCodecVP8VideoDecoder::ResetInputBuffers()
{
...
bool
WebrtcMediaCodecVP8VideoDecoder::ResetOutputBuffers()
{
...
int32_t
WebrtcMediaCodecVP8VideoDecoder::InitDecode(
...
int32_t
WebrtcMediaCodecVP8VideoDecoder::Decode(
...
{
  CSFLogDebug(
      LOGTAG, "%s, renderTimeMs = %" PRId64, __FUNCTION__, renderTimeMs);

  if (inputImage._length == 0 || !inputImage._buffer) {
    CSFLogDebug(LOGTAG,
                "%s, input Image invalid. length = %" PRIdPTR,
                __FUNCTION__,
                inputImage._length);
    return WEBRTC_VIDEO_CODEC_ERROR;
  }
  ...
  if (!mMediaCodecDecoder->isStarted) {
    ...
    nsresult res = mMediaCodecDecoder->Configure(mFrameWidth,
                                                 mFrameHeight,
                                                 nullptr,
                                                 0,
                                                 MEDIACODEC_VIDEO_MIME_VP8,
...
void
WebrtcMediaCodecVP8VideoDecoder::DecodeFrame(EncodedFrame* frame)
{
  CSFLogDebug(LOGTAG, "%s ", __FUNCTION__);
}

int32_t
WebrtcMediaCodecVP8VideoDecoder::RegisterDecodeCompleteCallback(
...
int32_t
WebrtcMediaCodecVP8VideoDecoder::Release()
{
  CSFLogDebug(LOGTAG, "%s ", __FUNCTION__);
