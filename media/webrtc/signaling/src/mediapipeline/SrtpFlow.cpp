...
RefPtr<SrtpFlow>
SrtpFlow::Create(int cipher_suite,
                 bool inbound,
                 const void* key,
                 size_t key_len)
{
...
nsresult
SrtpFlow::CheckInputs(
    bool protect, void* in, int in_len, int max_len, int* out_len)
{
  MOZ_ASSERT(in);
  if (!in) {
...
nsresult
SrtpFlow::ProtectRtp(void* in, int in_len, int max_len, int* out_len)
{
  ...
  *out_len = len;
  ...
  CSFLogDebug(
      LOGTAG, "Successfully protected an SRTP packet of len %d", *out_len);

  return NS_OK;
}

nsresult
SrtpFlow::UnprotectRtp(void* in, int in_len, int max_len, int* out_len)
{
  ...
  *out_len = len;
  ...
  CSFLogDebug(
      LOGTAG, "Successfully unprotected an SRTP packet of len %d", *out_len);

  return NS_OK;
}

nsresult
SrtpFlow::ProtectRtcp(void* in, int in_len, int max_len, int* out_len)
{
  nsresult res = CheckInputs(true, in, in_len, max_len, out_len);
  if (NS_FAILED(res)) return res;
  ...
  *out_len = len;
  ...
  CSFLogDebug(
      LOGTAG, "Successfully protected an SRTCP packet of len %d", *out_len);

  return NS_OK;
}

nsresult
SrtpFlow::UnprotectRtcp(void* in, int in_len, int max_len, int* out_len)
{
  nsresult res = CheckInputs(false, in, in_len, max_len, out_len);
  if (NS_FAILED(res)) return res;
  ...
  *out_len = len;
  ...
  CSFLogDebug(
      LOGTAG, "Successfully unprotected an SRTCP packet of len %d", *out_len);
...
// Statics
void
SrtpFlow::srtp_event_handler(srtp_event_data_t* data)
{
