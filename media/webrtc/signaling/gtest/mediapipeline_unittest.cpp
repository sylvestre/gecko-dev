...
class FakeSourceMediaStream : public mozilla::SourceMediaStream
{
 public:
  ...
  virtual bool AppendToTrack(TrackID aID,
...
class FakeAudioStreamTrack : public mozilla::dom::AudioStreamTrack
{
 public:
  FakeAudioStreamTrack()
      : AudioStreamTrack(new DOMMediaStream(nullptr, nullptr),
                         0,
                         1,
...
class TestAgent
{
 public:
  ...
  void Stop()
  {
    ...
    if (audio_pipeline_) audio_pipeline_->ShutdownMedia_m();

    mozilla::SyncRunnable::DispatchToThread(
        test_utils->sts_target(), WrapRunnable(this, &TestAgent::StopInt));
  }
  ...
  void Shutdown_s()
  {
    ...
    if (audio_pipeline_) audio_pipeline_->DetachTransport_s();
  }
  ...
  int GetAudioRtpCountSent() { return audio_pipeline_->rtp_packets_sent(); }
  ...
  int GetAudioRtcpCountSent() { return audio_pipeline_->rtcp_packets_sent(); }
...
class TestAgentSend : public TestAgent
{
 public:
  ...
  virtual void CreatePipelines_s(bool aIsRtcpMux)
  {
    ...
    audio_pipeline_ =
        new mozilla::MediaPipelineTransmit(test_pc,
                                           nullptr,
                                           test_utils->sts_target(),
...
class TestAgentReceive : public TestAgent
{
 public:
  ...
  {
    audio_pipeline_->UpdateTransport_s(
        1, audio_rtp_transport_.flow_, audio_rtcp_transport_.flow_, filter);
  }
