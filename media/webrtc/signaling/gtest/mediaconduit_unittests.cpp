...
int
AudioSendAndReceive::WriteWaveHeader(int rate, int channels, FILE* outFile)
{
...
int
AudioSendAndReceive::FinishWaveHeader(FILE* outFile)
{
...
void
AudioSendAndReceive::GenerateMusic(short* buf, int len)
{
...
void
AudioSendAndReceive::GenerateAndReadSamples()
{
  ...
  mozilla::Unused << fwrite(
      inbuf, 1, SAMPLES * sizeof(inbuf[0]) * CHANNELS, inFile);
  ...
  do {
    ...
    mOtherSession->GetAudioFrame(
        audioOutput.get(), PLAYOUT_SAMPLE_FREQUENCY, 10, sampleLengthDecoded);
...
class TransportConduitTest : public ::testing::Test
{
 public:
  ...
  void TestDummyAudioAndTransport()
  {
    ...
    audioTester.Init(
        mAudioSession, mAudioSession2, iAudiofilename, oAudiofilename);
    cerr << "   ******************************************************** "
         << endl;
