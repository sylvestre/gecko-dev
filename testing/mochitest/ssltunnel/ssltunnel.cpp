...
// Copied from nsCRT
char*
strtok2(char* string, const char* delims, char** newStr)
{
  PR_ASSERT(string);
...
int
ClientAuthValueComparator(const void* v1, const void* v2)
{
...
static int
match_hostname(PLHashEntry* he, int index, void* arg)
{
...
bool
ReadConnectRequest(server_info_t* server_info,
                   relayBuffer& buffer,
                   int32_t* result,
                   string& certificate,
 ...
 */
bool
AdjustWebSocketLocation(relayBuffer& buffer, connection_info_t* ci)
{
 ...
 */
bool
AdjustWebSocketHost(relayBuffer& buffer, connection_info_t* ci)
{
 ...
 */
bool
AdjustRequestURI(relayBuffer& buffer, string* host)
{
  assert(buffer.margin());
...
bool
ConnectSocket(UniquePRFileDesc& fd,
...
HandleConnection(void* data)
{
  ...
  if (other_sock) {
    ...
    if (!do_http_proxy) {
      if (!ConfigureSSLServerSocket(ci->client_sock,
                                    ci->server_info,
                                    certificateToUse,
                                    caNone,
                                    flags))
    ...
    while (!((client_error || client_done) && buffers[0].empty() &&
      ...
      for (int32_t s = 0; s < numberOfSockets; ++s) {
        ...
        if (out_flags & PR_POLL_READ && buffers[s].areafree()) {
          LOG_DEBUG((" :reading"));
          int32_t bytesRead = PR_Recv(sockets[s].fd,
                                      buffers[s].buffertail,
                                      buffers[s].areafree(),
                                      0,
          ...
          if (bytesRead == 0) {
            ...
            int32_t response;
            if (!connect_accepted && ReadConnectRequest(ci->server_info,
                                                        buffers[s],
                                                        &response,
                                                        certificateToUse,
                                                        &clientAuth,
              ...
              if (fullHost.find(":443") == string::npos) {
                ...
                PL_HashTableEnumerateEntries(
                    ci->server_info->host_cert_table, match_hostname, &match);
                PL_HashTableEnumerateEntries(
                    ci->server_info->host_clientauth_table,
                    match_hostname,
                    &match);
                PL_HashTableEnumerateEntries(
                    ci->server_info->host_ssl3_table, match_hostname, &match);
                PL_HashTableEnumerateEntries(
                    ci->server_info->host_tls1_table, match_hostname, &match);
                PL_HashTableEnumerateEntries(
                    ci->server_info->host_rc4_table, match_hostname, &match);
                PL_HashTableEnumerateEntries(
                    ci->server_info->host_failhandshake_table,
                    match_hostname,
                    &match);
        ...
        if (out_flags & PR_POLL_WRITE) {
          LOG_DEBUG((" :writing"));
          int32_t bytesWrite = PR_Send(sockets[s].fd,
                ...
                if (ci->http_proxy_only) {
                  LOG_DEBUG(
                      (" not updating to SSL based on http_proxy_only for this "
                       "socket"));
                } else if (!ConfigureSSLServerSocket(ci->client_sock,
                                                     ci->server_info,
                                                     certificateToUse,
                                                     clientAuth,
                                                     flags)) {
...
StartServer(void* data)
{
  ...
  while (!shutdown_server) {
    ...
    ci->client_sock = PR_Accept(
        listen_socket.get(), &ci->client_addr, PR_INTERVAL_NO_TIMEOUT);
...
PLHashTable*
get_ssl3_table(server_info_t* server)
{
  return server->host_ssl3_table;
}

PLHashTable*
get_tls1_table(server_info_t* server)
{
  return server->host_tls1_table;
}

PLHashTable*
get_rc4_table(server_info_t* server)
{
  return server->host_rc4_table;
}

PLHashTable*
get_failhandshake_table(server_info_t* server)
{
  return server->host_failhandshake_table;
}

int
parseWeakCryptoConfig(char* const& keyword,
                      char*& _caret,
...
int
processConfigLine(char* configLine)
{
  ...
  if (!strcmp(keyword, "listen")) {
    ...
    if (server_info_t* existingServer = findServerInfo(port)) {
      ...
      PLHashEntry* entry = PL_HashTableAdd(
          existingServer->host_cert_table, hostname_copy, certnick_copy);
      if (!entry) {
        LOG_ERROR(("Out of memory"));
        return 1;
      }
      ...
      server.host_cert_table = PL_NewHashTable(0,
                                               PL_HashString,
                                               PL_CompareStrings,
                                               PL_CompareStrings,
                                               nullptr,
      ...
      server.host_clientauth_table = PL_NewHashTable(0,
                                                     PL_HashString,
                                                     PL_CompareStrings,
      ...
      server.host_redir_table = PL_NewHashTable(0,
                                                PL_HashString,
                                                PL_CompareStrings,
                                                PL_CompareStrings,
                                                nullptr,
      ...
      server.host_ssl3_table = PL_NewHashTable(0,
                                               PL_HashString,
                                               PL_CompareStrings,
                                               PL_CompareStrings,
                                               nullptr,
      ...
      server.host_tls1_table = PL_NewHashTable(0,
                                               PL_HashString,
                                               PL_CompareStrings,
                                               PL_CompareStrings,
                                               nullptr,
      ...
      server.host_rc4_table = PL_NewHashTable(0,
                                              PL_HashString,
                                              PL_CompareStrings,
                                              PL_CompareStrings,
                                              nullptr,
      ...
      server.host_failhandshake_table = PL_NewHashTable(0,
                                                        PL_HashString,
                                                        PL_CompareStrings,
                                                        PL_CompareStrings,
                                                        nullptr,
  ...
  if (!strcmp(keyword, "redirhost")) {
    ...
    if (server_info_t* existingServer = findServerInfo(port)) {
      ...
      PLHashEntry* entry = PL_HashTableAdd(
          existingServer->host_redir_table, hostname_copy, redir_copy);
      if (!entry) {
        LOG_ERROR(("Out of memory"));
...
int
parseConfigFile(const char* filePath)
{
...
int
freeHostCertHashItems(PLHashEntry* he, int i, void* arg)
{
  delete[](char*) he->key;
...
int
freeHostRedirHashItems(PLHashEntry* he, int i, void* arg)
{
  delete[](char*) he->key;
  delete[](char*) he->value;
  return HT_ENUMERATE_REMOVE;
...
int
freeClientAuthHashItems(PLHashEntry* he, int i, void* arg)
{
  delete[](char*) he->key;
...
int
freeSSL3HashItems(PLHashEntry* he, int i, void* arg)
{
  delete[](char*) he->key;
  return HT_ENUMERATE_REMOVE;
}
...
int
freeTLS1HashItems(PLHashEntry* he, int i, void* arg)
{
  delete[](char*) he->key;
  return HT_ENUMERATE_REMOVE;
}
...
int
freeRC4HashItems(PLHashEntry* he, int i, void* arg)
{
  delete[](char*) he->key;
  return HT_ENUMERATE_REMOVE;
}
...
main(int argc, char** argv)
{
  ...
  for (auto& server : servers) {
    PL_HashTableEnumerateEntries(
        server.host_cert_table, freeHostCertHashItems, nullptr);
    PL_HashTableEnumerateEntries(
        server.host_clientauth_table, freeClientAuthHashItems, nullptr);
    PL_HashTableEnumerateEntries(
        server.host_redir_table, freeHostRedirHashItems, nullptr);
    PL_HashTableEnumerateEntries(
        server.host_ssl3_table, freeSSL3HashItems, nullptr);
    PL_HashTableEnumerateEntries(
        server.host_tls1_table, freeTLS1HashItems, nullptr);
    PL_HashTableEnumerateEntries(
        server.host_rc4_table, freeRC4HashItems, nullptr);
    PL_HashTableEnumerateEntries(
        server.host_failhandshake_table, freeRC4HashItems, nullptr);
