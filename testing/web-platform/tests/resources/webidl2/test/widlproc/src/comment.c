 ...
 */
static struct cnode*
endspecificcnode(struct cnode* cnode,
 ...
 */
static struct cnode*
starthtmlcnode(struct cnode* cnode,
               const struct htmleldesc* htmleldesc,
               const char* attrs,
               unsigned int attrslen,
               const char* filename,
               unsigned int linenum)
{
    ...
    if (cnode->funcs != &html_funcs) {
      ...
      if (!(htmleldesc->flags & HTMLEL_INLINE)) {
        if (!(htmleldesc->flags & HTMLEL_BLOCK))
          locerrorexit(
              filename, linenum, "<%s> not valid here", htmleldesc->name);
 ...
 */
static struct cnode*
startparamcnode(struct cnode* cnode,
                const char* word,
...
def_api_feature_output(struct cnode* cnode, unsigned int indent)
{
  struct paramcnode* paramcnode = (void*)cnode;
  printf(
      "%*s<def-api-feature identifier=\"%s\">\n", indent, "", paramcnode->name);
 ...
 */
static const char*
parsehtmltag(const char* start,
             struct cnode** pcnode,
             const char* filename,
             unsigned int* plinenum)
{
  struct cnode* cnode = *pcnode;
  ...
  for (;;) {
    if (!htmleldesc->namelen) {
      locerrorexit(filename,
                   *plinenum,
                   "unrecognized HTML tag %.*s",
                   end - start,
                   start);
    }
  ...
  if (close == 1) {
    ...
    for (;;) {
      ...
      if (cnode->funcs != &text_funcs) {
        if (cnode->funcs != &html_funcs) {
          locerrorexit(
              filename, *plinenum, "mismatched %.*s", end - start, start);
        }
        htmlcnode = (struct htmlcnode*)cnode;
        if (htmlcnode->desc == htmleldesc) break;
        if (!(htmlcnode->desc->flags & HTMLEL_AUTOCLOSE)) {
          locerrorexit(filename,
                       htmlcnode->cn.linenum,
                       "mismatched <%.*s>",
                       htmlcnode->desc->namelen,
                       htmlcnode->desc->name);
        }
      }
      cnode = endcnode(cnode);
    ...
    /* Opening tag. */
    if (close != 2)
      cnode = starthtmlcnode(
          cnode, htmleldesc, endname, end - 1 - endname, filename, *plinenum);
    else  // don't include the closing "/" in the attributes list
      cnode = starthtmlcnode(
          cnode, htmleldesc, endname, end - 2 - endname, filename, *plinenum);
  ...
  for (;;) {
    ...
    while (ch && ch != '\n') {
      ...
      }
      {
        ...
        if (!cmdlen)
          locerrorexit(
              comment->filename, linenum, "\\ or @ without Doxygen command");
        ...
        for (;;) {
          if (!command->namelen) {
            locerrorexit(comment->filename,
                         linenum,
                         "unrecognized Doxygen command '%.*s'",
                         cmdlen + 1,
                         start - 1);
          }
    ...
    for (;;) {
      ...
      if (cnode->funcs == &param_funcs || cnode->funcs == &return_funcs ||
        ...
        if (cnode->funcs == &param_funcs) {
          ...
          if (!node)
            locerrorexit(
                comment->filename, cnode->linenum, "no return type found");
        } else {
