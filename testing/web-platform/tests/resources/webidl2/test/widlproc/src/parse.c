...
tokerrorexit(struct tok* tok, const char* format, ...)
{
  ...
  if (tok->type == TOK_EOF)
    ...
    locerrorexit(
        tok->filename, tok->linenum, "at '%.*s': %s", tok->len, tok->start, m);
  va_end(ap);
}
...
static struct node*
parsedefaultvalue(struct tok* tok, struct node* parent);
static struct node*
 ...
 */
static struct node*
parsescopednamelist(struct tok* tok,
                    const char* name,
                    const char* name2,
                    int comment)
{
 ...
 */
static struct node*
parsereturntypeandoperationrest(struct tok* tok,
                                struct node* eal,
                                struct node* attrs)
{
 ...
 */
static struct node*
parseoperationoriteratorrest(struct tok* tok,
                             struct node* eal,
                             struct node* attrs)
{
  struct node* node;
...
parseattributeoroperationoriterator(struct tok* tok, struct node* eal)
{
  ...
  for (;;) {
    static const int t[] = {
        TOK_getter, TOK_setter, TOK_creator, TOK_deleter, TOK_legacycaller, 0};
    const int* tt = t;
    char* s;
