...
extern int
iniparser_getint(dictionary* d, char* key, int notfound);
extern char*
iniparser_getstring(dictionary* d, char* key, char* def);

// This is the windows launcher function
int
launchWindows(int children, int maxtime)
{
  _TCHAR cmdline[50];
  STARTUPINFO startup;
  ...
  rv = CreateProcess(
      NULL, cmdline, NULL, NULL, FALSE, 0, NULL, NULL, &startup, &procinfo);
...
main(int argc, char** argv)
{
  ...
  if (argc == 1 || (0 == strcmp(argv[1], "-h")) ||
    ...
    printf(
        "ProcLauncher takes an ini file.  Specify the ini file as the only\n");
    printf("parameter of the command line:\n");
    printf("proclauncher my.ini\n\n");
    printf("The ini file has the form:\n");
    printf("[main]\n");
    printf(
        "children=child1,child2  ; These comma separated values are "
        "sections\n");
    printf("maxtime=60              ; Max time this process lives\n");
    printf("[child1]                ; Here is a child section\n");
    printf(
        "children=3              ; You can have grandchildren: this spawns 3 "
        "of them for child1\n");
    printf(
        "maxtime=30              ; Max time, note it's in seconds. If this "
        "time\n");
    printf(
        "                        ; is > main:maxtime then the child process "
        "will be\n");
    printf(
        "                        ; killed when the parent exits. Also, "
        "grandchildren\n");
    printf(
        "[child2]                ; inherit this maxtime and can't change "
        "it.\n");
    printf(
        "maxtime=25              ; You can call these sections whatever you "
        "want\n");
    printf(
        "children=0              ; as long as you reference them in a children "
        "attribute\n");
    printf("....\n");
    return 0;
  } else if (argc == 2) {
