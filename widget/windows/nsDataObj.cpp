...
nsresult
nsDataObj::CStream::Init(nsIURI* pSourceURI,
...
STDMETHODIMP
nsDataObj::CStream::QueryInterface(REFIID refiid, void** ppvResult)
{
  *ppvResult = nullptr;
    ...
    uint32_t aCount)   // bytes available on this call
{
  ...
  do {
    ...
    rv = aInputStream->Read(
        (char*)(buffer + odaBytesReadTotal), aCount, &bytesReadByCall);
...
NS_IMETHODIMP
nsDataObj::CStream::OnStartRequest(nsIRequest* aRequest, nsISupports* aContext)
{
...
NS_IMETHODIMP
nsDataObj::CStream::OnStopRequest(nsIRequest* aRequest,
                                  nsISupports* aContext,
...
// IStream
STDMETHODIMP
nsDataObj::CStream::Clone(IStream** ppStream)
{
  return E_NOTIMPL;
}
...
//-----------------------------------------------------------------------------
STDMETHODIMP
nsDataObj::CStream::CopyTo(IStream* pDestStream,
...
//-----------------------------------------------------------------------------
STDMETHODIMP
nsDataObj::CStream::Read(void* pvBuffer, ULONG nBytesToRead, ULONG* nBytesRead)
{
...
//-----------------------------------------------------------------------------
STDMETHODIMP
nsDataObj::CStream::Stat(STATSTG* statstg, DWORD dwFlags)
{
...
//-----------------------------------------------------------------------------
HRESULT
nsDataObj::CreateStream(IStream** outStream)
{
...
//-----------------------------------------------------
STDMETHODIMP
nsDataObj::QueryInterface(REFIID riid, void** ppv)
{
  *ppv = nullptr;
...
class RemoveTempFileHelper final : public nsIObserver
{
 public:
  ...
  void Attach()
  {
    ...
    nsresult rv;
    rv = NS_NewTimerWithObserver(
        getter_AddRefs(mTimer), this, 500, nsITimer::TYPE_ONE_SHOT);
...
NS_IMETHODIMP
RemoveTempFileHelper::Observe(nsISupports* aSubject,
                              const char* aTopic,
...
//-----------------------------------------------------
BOOL
nsDataObj::FormatsMatch(const FORMATETC& source, const FORMATETC& target) const
{
...
//-----------------------------------------------------
STDMETHODIMP
nsDataObj::GetData(LPFORMATETC aFormat, LPSTGMEDIUM pSTM)
{
...
//-----------------------------------------------------
STDMETHODIMP
nsDataObj::GetDataHere(LPFORMATETC pFE, LPSTGMEDIUM pSTM)
{
  return E_FAIL;
}
...
//-----------------------------------------------------
STDMETHODIMP
nsDataObj::GetCanonicalFormatEtc(LPFORMATETC pFEIn, LPFORMATETC pFEOut)
{
  return E_NOTIMPL;
}
...
//-----------------------------------------------------
STDMETHODIMP
nsDataObj::SetData(LPFORMATETC aFormat, LPSTGMEDIUM aMedium, BOOL shouldRel)
{
  // Arbitrary system formats are used for image feedback during drag
  // and drop. We are responsible for storing these internally during
  // drag operations.
...
bool
nsDataObj::LookupArbitraryFormat(FORMATETC* aFormat,
...
bool
nsDataObj::CopyMediumData(STGMEDIUM* aMediumDst,
                          STGMEDIUM* aMediumSrc,
                          LPFORMATETC aFormat,
                          BOOL aSetData)
{
...
//-----------------------------------------------------
STDMETHODIMP
nsDataObj::EnumFormatEtc(DWORD dwDir, LPENUMFORMATETC* ppEnum)
{
  switch (dwDir) {
    case DATADIR_GET:
...
//-----------------------------------------------------
STDMETHODIMP
nsDataObj::EnumDAdvise(LPENUMSTATDATA* ppEnum)
{
  return OLE_E_ADVISENOTSUPPORTED;
}
...
STDMETHODIMP
nsDataObj::EndOperation(HRESULT hResult, IBindCtx* pbcReserved, DWORD dwEffects)
{
  mIsInOperation = FALSE;
  return S_OK;
}
...
STDMETHODIMP
nsDataObj::GetAsyncMode(BOOL* pfIsOpAsync)
{
...
STDMETHODIMP
nsDataObj::InOperation(BOOL* pfInAsyncOp)
{
...
STDMETHODIMP
nsDataObj::StartOperation(IBindCtx* pbcReserved)
{
  mIsInOperation = TRUE;
  return S_OK;
}
...
//-----------------------------------------------------
HRESULT
nsDataObj::AddSetFormat(FORMATETC& aFE)
{
  return S_OK;
}
...
//-----------------------------------------------------
HRESULT
nsDataObj::AddGetFormat(FORMATETC& aFE)
{
  return S_OK;
}
...
//
static bool
CreateFilenameFromTextA(nsString& aText,
...
static bool
CreateFilenameFromTextW(nsString& aText,
...
nsDataObj ::GetFileDescriptorInternetShortcutW(FORMATETC& aFE, STGMEDIUM& aSTG)
{
  ...
  // get a valid filename in the following order: 1) from the page title,
  // 2) localized string for an untitled page, 3) just use "Untitled.URL"
  if (!CreateFilenameFromTextW(title,
                               L".URL",
                               fileGroupDescW->fgd[0].cFileName,
                               NS_MAX_FILEDESCRIPTOR)) {
    nsAutoString untitled;
    if (!GetLocalizedString("noPageTitle", untitled) ||
        !CreateFilenameFromTextW(untitled,
                                 L".URL",
                                 fileGroupDescW->fgd[0].cFileName,
                                 NS_MAX_FILEDESCRIPTOR)) {
...
nsDataObj ::GetFileContentsInternetShortcut(FORMATETC& aFE, STGMEDIUM& aSTG)
{
  ...
  if (!Preferences::GetBool(kShellIconPref, true)) {
    ...
    _snprintf(
        contents, totalLen, shortcutFormatStr, asciiUrl.get(), path.get());
  }
...
HRESULT
nsDataObj::GetPreferredDropEffect(FORMATETC& aFE, STGMEDIUM& aSTG)
{
  HRESULT res = S_OK;
  aSTG.tymed = TYMED_HGLOBAL;
...
//-----------------------------------------------------
HRESULT
nsDataObj::GetText(const nsACString& aDataFlavor,
                   FORMATETC& aFE,
                   STGMEDIUM& aSTG)
{
  void* data = nullptr;
  ...
  nsCOMPtr<nsISupports> genericDataWrapper;
  mTransferable->GetTransferData(
      flavorStr, getter_AddRefs(genericDataWrapper), &len);
  ...
  if (aFE.cfFormat == CF_TEXT) {
    ...
    int32_t plainTextLen = WideCharToMultiByte(CP_ACP,
                                               0,
                                               (LPCWSTR)castedUnicode,
                                               len / 2 + 1,
                                               plainTextData,
                                               bufferSize,
                                               NULL,
...
//-----------------------------------------------------
HRESULT
nsDataObj::GetFile(FORMATETC& aFE, STGMEDIUM& aSTG)
{
  uint32_t dfInx = 0;
  ULONG count;
  FORMATETC fe;
...
HRESULT
nsDataObj::DropFile(FORMATETC& aFE, STGMEDIUM& aSTG)
{
  nsresult rv;
  uint32_t len = 0;
  nsCOMPtr<nsISupports> genericDataWrapper;
  ...
  mTransferable->GetTransferData(
      kFileMime, getter_AddRefs(genericDataWrapper), &len);
...
HRESULT
nsDataObj::DropImage(FORMATETC& aFE, STGMEDIUM& aSTG)
{
  nsresult rv;
  if (!mCachedTempFile) {
    uint32_t len = 0;
    ...
    mTransferable->GetTransferData(
        kNativeImageMime, getter_AddRefs(genericDataWrapper), &len);
    nsCOMPtr<imgIContainer> image(do_QueryInterface(genericDataWrapper));
    ...
    if (NS_FAILED(outStream->Write(
            (const char*)&fileHdr, sizeof(fileHdr), &writeCount)) ||
...
HRESULT
nsDataObj::DropTempFile(FORMATETC& aFE, STGMEDIUM& aSTG)
{
  nsresult rv;
  if (!mCachedTempFile) {
...
//-----------------------------------------------------
HRESULT
nsDataObj::GetMetafilePict(FORMATETC&, STGMEDIUM&)
{
  return E_NOTIMPL;
}
...
//-----------------------------------------------------
HRESULT
nsDataObj::SetBitmap(FORMATETC&, STGMEDIUM&)
{
  return E_NOTIMPL;
}
...
//-----------------------------------------------------
HRESULT
nsDataObj::SetDib(FORMATETC&, STGMEDIUM&)
{
  return E_FAIL;
}
...
//-----------------------------------------------------
HRESULT
nsDataObj::SetText(FORMATETC& aFE, STGMEDIUM& aSTG)
{
  return E_FAIL;
}
...
//-----------------------------------------------------
HRESULT
nsDataObj::SetMetafilePict(FORMATETC&, STGMEDIUM&)
{
  return E_FAIL;
}
...
//-----------------------------------------------------
void
nsDataObj::AddDataFlavor(const char* aDataFlavor, LPFORMATETC aFE)
{
...
//-----------------------------------------------------
void
nsDataObj::SetTransferable(nsITransferable* aTransferable)
{
  NS_IF_RELEASE(mTransferable);
...
HRESULT
nsDataObj ::GetUniformResourceLocator(FORMATETC& aFE,
                                      STGMEDIUM& aSTG,
                                      bool aIsUnicode)
{
  HRESULT res = S_OK;
...
HRESULT
nsDataObj::GetDownloadDetails(nsIURI** aSourceURI, nsAString& aFilename)
{
  *aSourceURI = nullptr;
  ...
  uint32_t dataSize = 0;
  mTransferable->GetTransferData(
      kFilePromiseURLMime, getter_AddRefs(urlPrimitive), &dataSize);
  ...
  mTransferable->GetTransferData(
      kFilePromiseDestFilename, getter_AddRefs(fileNamePrimitive), &dataSize);
...
HRESULT
nsDataObj::GetFileDescriptor_IStreamA(FORMATETC& aFE, STGMEDIUM& aSTG)
{
  HGLOBAL fileGroupDescHandle =
      ::GlobalAlloc(GMEM_ZEROINIT | GMEM_SHARE, sizeof(FILEGROUPDESCRIPTORW));
  ...
  strncpy(fileGroupDescA->fgd[0].cFileName,
          nativeFileName.get(),
...
HRESULT
nsDataObj::GetFileDescriptor_IStreamW(FORMATETC& aFE, STGMEDIUM& aSTG)
{
  HGLOBAL fileGroupDescHandle =
      ::GlobalAlloc(GMEM_ZEROINIT | GMEM_SHARE, sizeof(FILEGROUPDESCRIPTORW));
  NS_ENSURE_TRUE(fileGroupDescHandle, E_OUTOFMEMORY);
...
HRESULT
nsDataObj::GetFileContents_IStream(FORMATETC& aFE, STGMEDIUM& aSTG)
{
  IStream* pStream = nullptr;
