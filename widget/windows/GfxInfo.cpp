...
GfxInfo::GfxInfo() : mWindowsVersion(0), mActiveGPUIndex(0), mHasDualGPU(false)
{
}
  ...
  switch (type) {
    case REG_DWORD: {
      ...
      result = RegQueryValueExW(
          key, keyName, nullptr, &resultType, (LPBYTE)&dValue, &dwcbData);
    ...
    case REG_QWORD: {
      ...
      result = RegQueryValueExW(
          key, keyName, nullptr, &resultType, (LPBYTE)&qValue, &dwcbData);
    ...
    case REG_MULTI_SZ: {
      ...
      result = RegQueryValueExW(
          key, keyName, nullptr, &resultType, (LPBYTE)wCharValue, &dwcbData);
...
static nsresult
GetKeyValues(const WCHAR* keyLocation,
             const WCHAR* keyName,
             nsTArray<nsString>& destStrings)
{
  // First ask for the size of the value
  DWORD size;
  LONG rv = RegGetValueW(HKEY_LOCAL_MACHINE,
                         keyLocation,
                         keyName,
                         RRF_RT_REG_MULTI_SZ,
                         nullptr,
  ...
  rv = RegGetValueW(HKEY_LOCAL_MACHINE,
                    keyLocation,
                    keyName,
                    RRF_RT_REG_MULTI_SZ,
                    nullptr,
...
GfxInfo::Init()
{
  ...
  if (_wcsnicmp(displayDevice.DeviceKey,
                DEVICE_KEY_PREFIX,
    ...
    while (SetupDiEnumDeviceInfo(devinfo, memberIndex++, &devinfoData)) {
      ...
      if (SetupDiGetDeviceRegistryPropertyW(devinfo,
        ...
        result = RegOpenKeyExW(
            HKEY_LOCAL_MACHINE, driverKey.get(), 0, KEY_QUERY_VALUE, &key);
        if (result == ERROR_SUCCESS) {
          ...
          dwcbData = sizeof(value);
          result = RegQueryValueExW(
              key, L"DriverDate", nullptr, nullptr, (LPBYTE)value, &dwcbData);
          if (result == ERROR_SUCCESS) {
            mDriverDate[0] = value;
          } else {
    ...
    if (devinfo != INVALID_HANDLE_VALUE) {
      ...
      while (SetupDiEnumDeviceInfo(devinfo, memberIndex++, &devinfoData)) {
        ...
        if (SetupDiGetDeviceRegistryPropertyW(devinfo,
          ...
          driverKey2 += value;
          result = RegOpenKeyExW(
              HKEY_LOCAL_MACHINE, driverKey2.get(), 0, KEY_QUERY_VALUE, &key);
          if (result == ERROR_SUCCESS) {
            dwcbData = sizeof(value);
            result = RegQueryValueExW(key,
                                      L"MatchingDeviceId",
...
CheckForCiscoVPN()
{
  ...
  result = RegOpenKeyExW(HKEY_LOCAL_MACHINE,
                         L"Software\\Cisco Systems\\VPN Client",
                         0,
                         KEY_QUERY_VALUE,
                         &key);
  if (result == ERROR_SUCCESS) {
...
#define IMPLEMENT_INTEL_DRIVER_BLOCKLIST_D2D(                      \
    winVer, devFamily, driverVer, ruleId)                          \
  APPEND_TO_DRIVER_BLOCKLIST2(                                     \
      winVer,                                                      \
      (nsAString&)GfxDriverInfo::GetDeviceVendor(VendorIntel),     \
      (GfxDeviceFamily*)GfxDriverInfo::GetDeviceFamily(devFamily), \
    ...
    IMPLEMENT_INTEL_DRIVER_BLOCKLIST(OperatingSystem::Windows7,
                                     IntelGMA500,
                                     V(5, 0, 0, 2026),
...
GfxInfo::FindMonitors(JSContext* aCx, JS::HandleObject aOutArray)
{
  ...
  for (int deviceIndex = 0;; deviceIndex++) {
    ...
    if (!::EnumDisplaySettingsA(
            device.DeviceName, ENUM_CURRENT_SETTINGS, &mode)) {
      continue;
    }
...
GfxInfo::DescribeFeatures(JSContext* aCx, JS::Handle<JSObject*> aObj)
{
  ...
  if (!InitFeatureObject(
          aCx, aObj, "d3d11", FEATURE_DIRECT3D_11_ANGLE, Some(d3d11), &obj)) {
    return;
  }
  ...
  if (!InitFeatureObject(
          aCx, aObj, "d2d", nsIGfxInfo::FEATURE_DIRECT2D, Some(d2d), &obj)) {
    return;
  }
  {
...
NS_IMETHODIMP
GfxInfo::SpoofVendorID(const nsAString& aVendorID)
{
...
NS_IMETHODIMP
GfxInfo::SpoofDeviceID(const nsAString& aDeviceID)
{
...
NS_IMETHODIMP
GfxInfo::SpoofDriverVersion(const nsAString& aDriverVersion)
{
