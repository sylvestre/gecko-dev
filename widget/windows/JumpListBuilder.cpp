...
NS_IMETHODIMP
JumpListBuilder::InitListBuild(nsIMutableArray* removedItems, bool* _retval)
{
...
nsresult
JumpListBuilder::RemoveIconCacheForItems(nsIMutableArray* items)
{
  ...
  *_retval = false;
  ...
  switch (aCatType) {
    ...
    case nsIJumpListBuilder::JUMPLIST_CATEGORY_CUSTOMLIST: {
      ...
      // Add the tasks
      hr = mJumpListMgr->AppendCategory(
...
NS_IMETHODIMP
JumpListBuilder::CommitListBuild(nsIJumpListCommittedCallback* aCallback)
{
  ReentrantMonitorAutoEnter lock(mMonitor);
  if (!mJumpListMgr) return NS_ERROR_NOT_AVAILABLE;
  ...
  RefPtr<nsIRunnable> event =
      NewNonOwningRunnableMethod<RefPtr<detail::DoneCommitListBuildCallback>>(
          "JumpListBuilder::DoCommitListBuild",
          this,
          &JumpListBuilder::DoCommitListBuild,
          Move(callback));
...
bool
JumpListBuilder::IsSeparator(nsCOMPtr<nsIJumpListItem>& item)
{
  int16_t type;
  item->GetType(&type);
...
// to our objects.
nsresult
JumpListBuilder::TransferIObjectArrayToIMutableArray(
  ...
  for (uint32_t idx = 0; idx < count; idx++) {
    ...
    if (SUCCEEDED(objArray->GetAt(idx, IID_IShellLinkW, (LPVOID*)&pLink))) {
      ...
      item = do_QueryInterface(shortcut);
                   ...
                   objArray->GetAt(idx, IID_IShellItem, (LPVOID*)&pItem))) {
      nsCOMPtr<nsIJumpListLink> link = do_CreateInstance(kJumpListLinkCID, &rv);
      if (NS_FAILED(rv)) return NS_ERROR_UNEXPECTED;
...
NS_IMETHODIMP
JumpListBuilder::Observe(nsISupports* aSubject,
                         const char* aTopic,
