...
GetGutterSize(HANDLE theme, HDC hdc)
{
  SIZE gutterSize;
  GetThemePartSize(
      theme, hdc, MENU_POPUPGUTTER, 0, nullptr, TS_TRUE, &gutterSize);

  SIZE checkboxBGSize(GetCheckboxBGBounds(theme, hdc));

  SIZE itemSize;
  GetThemePartSize(
      theme, hdc, MENU_POPUPITEM, MPI_NORMAL, nullptr, TS_TRUE, &itemSize);
                    ...
                    bool aIsRtl)
{
  ...
  if (!aIsRtl) {
    return DrawThemeBackground(
        aTheme, aHdc, aPart, aState, aWidgetRect, aClipRect);
  }
  ...
  if (hObj && GetObject(hObj, sizeof(bitmap), &bitmap) &&
    ...
    HRESULT hr = DrawThemeBackground(
        aTheme, aHdc, aPart, aState, &newWRect, newCRectPtr);
    SetLayout(aHdc, 0);
    if (SUCCEEDED(hr)) {
      return hr;
    }
                                          ...
                                          RECT* aClipRect)
{
  ...
  DrawThemeBackground(
      aTheme, aHdc, aPart, aState, &adjWidgetRect, &adjClipRect);
  ...
  if (animate) {
    ...
    DrawThemeBackground(
        aTheme, aHdc, overlayPart, aState, &overlayRect, &adjClipRect);
...
nsresult
nsNativeThemeWin::GetCachedWidgetBorder(nsIFrame* aFrame,
                                        HTHEME aTheme,
                                        ...
                                        nsIntMargin* aResult)
{
  ...
  HRESULT res = GetThemeBackgroundContentRect(
      aTheme, nullptr, aPart, aState, &outerRect, &contentRect);
...
mozilla::Maybe<nsUXThemeClass>
nsNativeThemeWin::GetThemeClass(uint8_t aWidgetType)
{
  switch (aWidgetType) {
                                       ...
                                       const nsRect& aDirtyRect)
{
  ...
  if (!theme)
    return ClassicDrawWidgetBackground(
        aContext, aFrame, aWidgetType, aRect, aDirtyRect);
...
RENDER_AGAIN:
  ...
  if (aWidgetType == NS_THEME_RANGE ||
    ...
    GetThemeBackgroundContentRect(
        theme, hdc, part, state, &widgetRect, &contentRect);
    ...
    if (isChecked) {
      ...
      DrawThemeBackground(
          theme, hdc, MENU_POPUPCHECK, state, &checkRect, &clipRect);
    }
  } else if (aWidgetType == NS_THEME_MENUPOPUP) {
    DrawThemeBackground(
        theme, hdc, MENU_POPUPBORDERS, /* state */ 0, &widgetRect, &clipRect);
    SIZE borderSize;
    GetThemePartSize(
        theme, hdc, MENU_POPUPBORDERS, 0, nullptr, TS_TRUE, &borderSize);
    ...
    DrawThemeBackground(
        theme, hdc, MENU_POPUPBACKGROUND, /* state */ 0, &bgRect, &clipRect);
    ...
    DrawThemeBackground(
        theme, hdc, MENU_POPUPSEPARATOR, /* state */ 0, &sepRect, &clipRect);
    ...
    DrawThemeBGRTLAware(
        theme, hdc, part, state, &renderRect, &clipRect, IsFrameRTL(aFrame));
  }
  // The following widgets need to be RTL-aware
  else if (aWidgetType == NS_THEME_RESIZER ||
           aWidgetType == NS_THEME_MENULIST_BUTTON) {
    DrawThemeBGRTLAware(
        theme, hdc, part, state, &widgetRect, &clipRect, IsFrameRTL(aFrame));
    ...
    DrawThemedProgressMeter(
        aFrame, aWidgetType, theme, hdc, part, state, &widgetRect, &clipRect);
    ...
    DTBGOPTS opts = {
        sizeof(DTBGOPTS), DTBG_OMITCONTENT | DTBG_CLIPRECT, clipRect};
  ...
  if (aWidgetType == NS_THEME_RANGE ||
    ...
    if (contentState.HasState(NS_EVENT_STATE_FOCUS)) {
      ...
      ::SetBrushOrgEx(
          hdc, vpOrg.x + widgetRect.left, vpOrg.y + widgetRect.top, nullptr);
      ::SetTextColor(hdc, 0);
    ...
    DrawThemeEdge(
        theme, hdc, RP_BAND, 0, &widgetRect, EDGE_ETCHED, BF_TOP, nullptr);
    ...
    if (GetThemePartSize(
            theme, hdc, gripPart, state, nullptr, TS_TRUE, &gripSize) == S_OK &&
        GetThemeMargins(
            theme, hdc, part, state, TMT_CONTENTMARGINS, nullptr, &thumbMgns) ==
            S_OK &&
                                  ...
                                  nsIntMargin* aResult)
{
  ...
  rv = GetCachedWidgetBorder(
      aFrame, theme, themeClass.value(), aWidgetType, part, state, aResult);
                                       ...
                                       bool* aIsOverridable)
{
  ...
  *aIsOverridable = true;
  ...
  if (!theme) {
    rv = ClassicGetMinimumWidgetSize(
        aFrame, aWidgetType, aResult, aIsOverridable);
    ScaleForFrameDPI(aResult, aFrame);
    return rv;
  }
  ...
  rv = GetCachedMinimumWidgetSize(aFrame,
                                  theme,
                                  themeClass.value(),
                                  aWidgetType,
                                  part,
                                          ...
                                          nsIntMargin* aResult)
{
  switch (aWidgetType) {
    ...
    case NS_THEME_RADIOMENUITEM: {
      ...
      if (NS_FAILED(ClassicGetThemePartAndState(
              aFrame, aWidgetType, part, state, focused)))
        return false;
...
nsresult
nsNativeThemeWin::ClassicGetThemePartAndState(nsIFrame* aFrame,
                                              uint8_t aWidgetType,
                                              int32_t& aPart,
                                              int32_t& aState,
                                              bool& aFocused)
        ...
        bool aDrawRight)
{
  ...
  switch (aPosition) {
    case BF_LEFT:
      ...
      ::SetRect(
          &sideRect, R.left + 2, R.top, R.right - 2 + selectedOffset, R.bottom);
    ...
    case BF_TOP:
      ...
      ::SetRect(
          &sideRect, R.left, R.top + 2, R.right, R.bottom - 1 + selectedOffset);
    ...
    case BF_RIGHT:
      ...
      ::SetRect(&topRect, R.left, R.top + lOffset, R.right, R.bottom - rOffset);
      ::SetRect(
          &sideRect, R.left + 2 - selectedOffset, R.top, R.right - 2, R.bottom);
    ...
    case BF_BOTTOM:
      ...
      ::SetRect(&topRect, R.left + lOffset, R.top, R.right - rOffset, R.bottom);
      ::SetRect(
          &sideRect, R.left, R.top + 2 - selectedOffset, R.right, R.bottom - 2);
...
static void
DrawMenuImage(HDC hdc, const RECT& rc, int32_t aComponent, uint32_t aColor)
{
  ...
  if (hMemoryDC) {
    ...
    if (hMonoBitmap) {
      ...
      if (hPrevBitmap) {
        ...
        ::BitBlt(
            hdc, imgPos.x, imgPos.y, checkW, checkH, hMemoryDC, 0, 0, SRCAND);
        ::SetTextColor(hdc, ::GetSysColor(aColor));
        ::SetBkColor(hdc, 0x00000000);
        ::BitBlt(
            hdc, imgPos.x, imgPos.y, checkW, checkH, hMemoryDC, 0, 0, SRCPAINT);
...
void
nsNativeThemeWin::DrawCheckedRect(
    HDC hdc, const RECT& rc, int32_t fore, int32_t back, HBRUSH defaultBack)
{
...
nsresult
nsNativeThemeWin::ClassicDrawWidgetBackground(gfxContext* aContext,
                                              nsIFrame* aFrame,
                                              uint8_t aWidgetType,
                                              const nsRect& aRect,
                                              const nsRect& aDirtyRect)
...
nsresult
NS_NewNativeTheme(nsISupports* aOuter, REFNSIID aIID, void** aResult)
{
