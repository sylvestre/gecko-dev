...
//
int32_t
nsImageToClipboard ::CalcSize(int32_t aHeight,
                              int32_t aColors,
                              WORD aBitsPerPixel,
                              int32_t aSpanBytes)
{
...
nsImageToClipboard::CreateFromImage(imgIContainer* inImage, HANDLE* outBitmap)
{
  ...
  *outBitmap = nullptr;
  ...
  rv = encoder->InitFromData(map.mData,
                             0,
...
//
nsresult
nsImageFromClipboard ::GetEncodedImageStream(unsigned char* aClipboardData,
  ...
  *aInputStream = nullptr;
  ...
  if (rgbData) {
    ...
    if (NS_SUCCEEDED(rv)) {
      ...
      if (NS_SUCCEEDED(rv)) {
        rv = encoder->InitFromData(rgbData,
                                   0,
                                   width,
                                   height,
                                   3 * width /* RGB * # pixels in a row */,
...
//
void
nsImageFromClipboard::InvertRows(unsigned char* aInitialBuffer,
                                 ...
                                 uint32_t aNumBytesPerRow)
{
  ...
  while (currentRow < lastRow) {
    ...
    memcpy(
        &aInitialBuffer[currentRow], &aInitialBuffer[lastRow], aNumBytesPerRow);
...
//
nsresult
nsImageFromClipboard::ConvertColorBitMap(unsigned char* aInputBuffer,
                                         ...
                                         unsigned char* aOutBuffer)
{
  ...
  uint32_t imageSize =
      pBitMapInfo->bmiHeader
          .biSizeImage;  // may be zero for BI_RGB bitmaps which means we need to calculate by hand
  ...
  if (pBitMapInfo->bmiHeader.biCompression == BI_BITFIELDS) {
    ...
    aInputBuffer += 3 * sizeof(DWORD);
      ...
      !imageSize)  // BI_RGB can have a size of zero which means we figure it out
  {
  ...
  if (!pBitMapInfo->bmiHeader.biCompression ||
    ...
    uint32_t numPixelsLeftInRow =
        pBitMapInfo->bmiHeader
            .biWidth;  // how many more pixels do we still need to read for the current row
    uint32_t pos = 0;
      ...
      switch (bitCount) {
        ...
        case 24:
          if (pBitMapInfo->bmiHeader.biCompression == BI_BITFIELDS) {
            ...
            numPixelsLeftInRow--;
            pos +=
                bytesPerPixel;  // 3 bytes for 24 bit data, 4 bytes for 32 bit data (we skip over the 4th byte)...
          }
          break;
        default:
...
void
nsImageFromClipboard::CalcBitmask(uint32_t aMask,
                                  uint8_t& aBegin,
                                  uint8_t& aLength)
{
...
void
nsImageFromClipboard::CalcBitShift(bitFields* aColorMask)
{
