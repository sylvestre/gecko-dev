...
//----------------------------------------------------------------------------------
NS_IMETHODIMP
nsDeviceContextSpecWin::Init(nsIWidget* aWidget,
...
nsDeviceContextSpecWin::FinishPrintViaPDF()
{
  ...
  if (::StartPage(mDC) > 0) {
    isPrinted = mPDFPrintHelper->DrawPage(mDC,
                                          mPDFCurrentPageNum++,
  ...
  if (mPDFCurrentPageNum < mPDFPageCount && isPrinted && endPageSuccess) {
    nsresult rv = NS_DispatchToCurrentThread(
        NewRunnableMethod("nsDeviceContextSpecWin::PrintPDFOnThread",
                          this,
                          &nsDeviceContextSpecWin::FinishPrintViaPDF));
    if (NS_SUCCEEDED(rv)) {
      return;
                                      ...
                                      int32_t aEndPage)
{
  ...
  if (mPrintViaSkPDF && (mOutputFormat != nsIPrintSettings::kOutputFormatPDF)) {
    ...
    di.lpszOutput =
        printToFileName.Length() > 0 ? printToFileName.get() : nullptr;
...
//----------------------------------------------------------------------------------
void
nsDeviceContextSpecWin::SetDeviceName(const nsAString& aDeviceName)
{
...
//----------------------------------------------------------------------------------
void
nsDeviceContextSpecWin::SetDriverName(const nsAString& aDriverName)
{
...
//----------------------------------------------------------------------------------
void
nsDeviceContextSpecWin::SetDevMode(LPDEVMODEW aDevMode)
{
  if (mDevMode) {
                                           ...
                                           nsIPrintSettings* aPS)
{
  ...
  if (status) {
    ...
    LONG ret = ::DocumentPropertiesW(
        nullptr, hPrinter, name, pDevMode, nullptr, DM_OUT_BUFFER);
    ...
    if (ret == IDOK && aPS) {
      ...
      ret = ::DocumentPropertiesW(nullptr,
                                  hPrinter,
                                  name,
                                  pDevMode,
                                  pDevMode,
...
GlobalPrinters::EnumerateNativePrinters()
{
  ...
  DWORD status = GetProfileStringW(L"devices",
                                   0,
                                   L",",
...
GlobalPrinters::GetDefaultPrinterName(nsAString& aDefaultPrinterName)
{
  ...
  WCHAR szDefaultPrinterName[1024];
  DWORD status = GetProfileStringW(L"windows",
                                   L"device",
                                   0,
                                   szDefaultPrinterName,
                                   ArrayLength(szDefaultPrinterName));
