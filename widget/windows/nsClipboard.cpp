...
nsClipboard::nsClipboard() : nsBaseClipboard()
{
  ...
  if (observerService) {
    observerService->AddObserver(
        this, NS_XPCOM_WILL_SHUTDOWN_OBSERVER_ID, PR_FALSE);
  }
}
...
NS_IMETHODIMP
nsClipboard::Observe(nsISupports* aSubject,
                     const char* aTopic,
...
//-------------------------------------------------------------------------
UINT
nsClipboard::GetFormat(const char* aMimeStr, bool aMapHTMLMime)
{
  UINT format;
...
//-------------------------------------------------------------------------
nsresult
nsClipboard::CreateNativeDataObject(nsITransferable* aTransferable,
...
//-------------------------------------------------------------------------
nsresult
nsClipboard::SetupNativeDataObject(nsITransferable* aTransferable,
                                   IDataObject* aDataObj)
{
...
nsClipboard::SetNativeClipboardData(int32_t aWhichClipboard)
{
  ...
  IDataObject* dataObj;
  if (NS_SUCCEEDED(CreateNativeDataObject(
          mTransferable, &dataObj, nullptr))) {  // this add refs dataObj
...
//-------------------------------------------------------------------------
nsresult
nsClipboard::GetGlobalData(HGLOBAL aHGBL, void** aData, uint32_t* aLen)
{
...
//-------------------------------------------------------------------------
nsresult
nsClipboard::GetNativeDataOffClipboard(nsIWidget* aWidget,
                                       UINT /*aIndex*/,
                                       UINT aFormat,
                                       void** aData,
                                       uint32_t* aLen)
...
//-------------------------------------------------------------------------
static HRESULT
FillSTGMedium(IDataObject* aDataObject,
              UINT aFormat,
              LPFORMATETC pFE,
              LPSTGMEDIUM pSTM,
              DWORD aTymed)
...
nsresult
nsClipboard::GetNativeDataOffClipboard(IDataObject* aDataObject,
                                       UINT aIndex,
                                       UINT aFormat,
...
//-------------------------------------------------------------------------
nsresult
nsClipboard::GetDataFromDataObject(IDataObject* aDataObject,
                                   UINT anIndex,
                                   nsIWidget* aWindow,
  ...
  for (i = 0; i < cnt; i++) {
    ...
    if (currentFlavor) {
      ...
      if (nullptr != aDataObject) {
        if (NS_SUCCEEDED(GetNativeDataOffClipboard(aDataObject,
                                                   anIndex,
                                                   format,
                                                   flavorStr.get(),
                                                   &data,
        ...
        if (NS_SUCCEEDED(GetNativeDataOffClipboard(
                aWindow, anIndex, format, &data, &dataLen))) {
          dataFound = true;
        }
      }
      ...
      if (dataFound) {
        ...
        if (flavorStr.EqualsLiteral(kFileMime)) {
          ...
          if (FindPlatformHTML(
                  aDataObject, anIndex, &data, &startOfData, &dataLen)) {
            dataLen -= startOfData;
            nsPrimitiveHelpers::CreatePrimitiveForCFHTML(
                static_cast<char*>(data) + startOfData,
                &dataLen,
        ...
        aTransferable->SetTransferData(
            flavorStr.get(), genericDataWrapper, dataLen);
        res = NS_OK;
...
//
bool
nsClipboard ::FindPlatformHTML(IDataObject* inDataObject,
                               UINT inIndex,
                               void** outData,
                               ...
                               uint32_t* outDataLen)
{
  ...
  int32_t endOfData = 0;
  int numFound = sscanf((char*)*outData,
                        "Version:%7s\nStartHTML:%d\nEndHTML:%d",
                        version,
                        &startOfData,
                        &endOfData);
...
//
bool
nsClipboard ::FindUnicodeFromPlainText(IDataObject* inDataObject,
                                       UINT inIndex,
                                       void** outData,
                                       uint32_t* outDataLen)
{
  // we are looking for text/unicode and we failed to find it on the clipboard first,
  // try again with text/plain. If that is present, convert it to unicode.
  nsresult rv = GetNativeDataOffClipboard(inDataObject,
                                          inIndex,
                                          GetFormat(kTextMime),
                                          nullptr,
                                          outData,
...
//
bool
nsClipboard ::FindURLFromLocalFile(IDataObject* inDataObject,
                                   UINT inIndex,
                                   void** outData,
                                   uint32_t* outDataLen)
{
  ...
  nsresult loadResult = GetNativeDataOffClipboard(inDataObject,
                                                  inIndex,
                                                  GetFormat(kFileMime),
                                                  nullptr,
                                                  outData,
...
//
bool
nsClipboard ::FindURLFromNativeURL(IDataObject* inDataObject,
                                   UINT inIndex,
                                   void** outData,
                                   uint32_t* outDataLen)
{
  ...
  if (NS_SUCCEEDED(loadResult) && tempOutData) {
    ...
    *outDataLen =
    ...
    if (NS_SUCCEEDED(loadResult) && tempOutData) {
      ...
      bool unescaped = NS_UnescapeURL(static_cast<char*>(tempOutData),
                                      tempDataLen,
...
{
  return StringEndsWith(inFileName,
