...
WinUtils::LogW(const wchar_t* fmt, ...)
{
  ...
  if (len) {
    ...
    if (WideCharToMultiByte(
            CP_ACP, 0, buffer, -1, utf8, len, nullptr, nullptr) > 0) {
      // desktop console
      printf("%s\n", utf8);
...
typedef HRESULT(WINAPI* GETDPIFORMONITORPROC)(HMONITOR,
                                              MONITOR_DPI_TYPE,
                                              UINT*,
                                              UINT*);
...
WinUtils::PeekMessage(
    LPMSG aMsg, HWND aWnd, UINT aFirstMessage, UINT aLastMessage, UINT aOption)
{
#ifdef NS_ENABLE_TSF
  RefPtr<ITfMessagePump> msgPump = TSFTextStore::GetMessagePump();
  if (msgPump) {
    BOOL ret = FALSE;
    HRESULT hr = msgPump->PeekMessageW(
        aMsg, aWnd, aFirstMessage, aLastMessage, aOption, &ret);
...
/* static */
bool
WinUtils::GetMessage(LPMSG aMsg,
                     HWND aWnd,
                     UINT aFirstMessage,
                     UINT aLastMessage)
{
...
WinUtils::WaitForMessage(DWORD aTimeoutMs)
{
  ...
  while (true) {
    ...
    DWORD result = ::MsgWaitForMultipleObjectsEx(
        0, NULL, aTimeoutMs - elapsed, MOZ_QS_ALLEVENT, waitFlags);
                         ...
                         DWORD aBufferLength)
{
  ...
  result = ::RegQueryValueExW(
      key, aValueName, nullptr, &type, (BYTE*)aBuffer, &aBufferLength);
  ::RegCloseKey(key);
...
/* static */
HWND
WinUtils::GetTopLevelHWND(HWND aWnd, bool aStopIfNotChild, bool aStopIfNotPopup)
{
  HWND curWnd = aWnd;
  HWND topWnd = nullptr;
...
GetNSWindowPropName()
{
  ...
  if (!*sPropName) {
    _snwprintf(
        sPropName, 39, L"MozillansIWidgetPtr%u", ::GetCurrentProcessId());
    sPropName[39] = '\0';
  }
  return sPropName;
}
...
static BOOL WINAPI
EnumFirstChild(HWND hwnd, LPARAM lParam)
{
  *((HWND*)lParam) = hwnd;
  return FALSE;
}
...
AsyncEncodeAndWriteIcon::Run()
{
  ...
  RefPtr<DataSourceSurface> surface =
      Factory::CreateWrappingDataSourceSurface(mBuffer.get(),
                                               mStride,
  ...
  if (mURLShortcut) {
    SendNotifyMessage(
        HWND_BROADCAST, WM_SETTINGCHANGE, SPI_SETNONCLIENTMETRICS, 0);
  }
  return rv;
}
 ...
 */
nsresult
FaviconHelper::ObtainCachedIconFile(nsCOMPtr<nsIURI> aFaviconPageURI,
  ...
  if (exists) {
    ...
    if (NS_FAILED(rv) || (nowTime - fileModTime) > icoReCacheSecondsTimeout) {
      CacheIconFileFromFaviconURIAsync(
          aFaviconPageURI, icoFile, aIOThread, aURLShortcut);
...
nsresult
FaviconHelper::HashURI(nsCOMPtr<nsICryptoHash>& aCryptoHash,
                       nsIURI* aUri,
                       nsACString& aUriHash)
{
...
nsresult
FaviconHelper::GetOutputIconPath(nsCOMPtr<nsIURI> aFaviconPageURI,
...
FaviconHelper::CacheIconFileFromFaviconURIAsync(
    nsCOMPtr<nsIURI> aFaviconPageURI,
    nsCOMPtr<nsIFile> aICOFile,
    nsCOMPtr<nsIThread>& aIOThread,
    bool aURLShortcut)
{
  ...
  nsCOMPtr<nsIFaviconDataCallback> callback =
      new mozilla::widget::AsyncFaviconDataReady(
          aFaviconPageURI, aIOThread, aURLShortcut);
...
int32_t
FaviconHelper::GetICOCacheSecondsTimeout()
{
...
/* static */
bool
WinUtils::GetShellItemPath(IShellItem* aItem, nsString& aResultString)
{
...
WinUtils::GetAppInitDLLs(nsAString& aOutput)
{
  ...
  status = RegQueryValueExW(
      hkey, kAppInitDLLs, nullptr, nullptr, nullptr, &numBytes);
  if (status != ERROR_SUCCESS) {
    return false;
  }
  ...
  status = RegQueryValueExW(
      hkey, kAppInitDLLs, nullptr, nullptr, (LPBYTE)data.get(), &numBytes);
  if (status != ERROR_SUCCESS) {
    return false;
  }
  ...
  while (token) {
    ...
    if (SanitizePath(token, cleanPath)) {
      ...
      for (uint32_t i = 0; i < whitelistedPaths.Length(); ++i) {
        ...
        if (StringBeginsWith(
                cleanPath, testPath, nsCaseInsensitiveStringComparator())) {
