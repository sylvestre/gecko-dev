...
nsresult
SetWindowAppUserModelProp(mozIDOMWindow* aParent, const nsString& aIdentifier)
{
  ...
  if (FAILED(SHGetPropertyStoreForWindow(
          toplevelHWND, IID_IPropertyStore, getter_AddRefs(pPropStore)))) {
  ...
  if (GetModuleFileNameW(nullptr, path, MAX_PATH)) {
    ...
    *slash = '\0';  // no trailing slash
    ...
    if (WinUtils::GetRegistryKey(
            HKEY_LOCAL_MACHINE, regKey.get(), path, buf, sizeof buf)) {
      aDefaultGroupId.Assign(buf);
    } else if (WinUtils::GetRegistryKey(
                   HKEY_CURRENT_USER, regKey.get(), path, buf, sizeof buf)) {
      aDefaultGroupId.Assign(buf);
    }
  }
