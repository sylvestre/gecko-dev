...
nsresult
GetAudioSessionData(nsID& aID, nsString& aSessionName, nsString& aIconPath)
{
  return AudioSession::GetSingleton()->GetSessionData(
      aID, aSessionName, aIconPath);
}
...
RecvAudioSessionData(const nsID& aID,
                     ...
                     const nsString& aIconPath)
{
  return AudioSession::GetSingleton()->SetSessionData(
      aID, aSessionName, aIconPath);
}

AudioSession* AudioSession::sService = nullptr;

AudioSession::AudioSession() { mState = UNINITIALIZED; }
...
AudioSession::Start()
{
  ...
  hr = device->Activate(
      IID_IAudioSessionManager, CLSCTX_ALL, nullptr, getter_AddRefs(manager));
  if (FAILED(hr)) {
    return NS_ERROR_FAILURE;
  }

  hr = manager->GetAudioSessionControl(
      &GUID_NULL, 0, getter_AddRefs(mAudioSessionControl));
...
void
CopynsID(nsID& lhs, const nsID& rhs)
{
  lhs.m0 = rhs.m0;
  lhs.m1 = rhs.m1;
  lhs.m2 = rhs.m2;
...
STDMETHODIMP
AudioSession::OnSimpleVolumeChanged(float aVolume, BOOL aMute, LPCGUID aContext)
{
  return S_OK;  // NOOP
}
