...
#ifdef DEBUG_rods_X
static void
WriteDebugStr(const char* aArg1, const char* aArg2, const char16_t* aStr)
{
  nsString str(aStr);
  char16_t s = '&';
  char16_t r = '_';
 ...
 */
nsresult
nsPrintOptions::ReadPrefs(nsIPrintSettings* aPS,
                          const nsAString& aPrinterName,
                          uint32_t aFlags)
{
  ...
  if (aFlags & nsIPrintSettings::kInitSaveMargins) {
    ...
    ReadInchesToTwipsPref(
        GetPrefName(kMarginTop, aPrinterName), margin.top, kMarginTop);
    DUMP_INT(kReadStr, kMarginTop, margin.top);
    ReadInchesToTwipsPref(
        GetPrefName(kMarginLeft, aPrinterName), margin.left, kMarginLeft);
    DUMP_INT(kReadStr, kMarginLeft, margin.left);
    ReadInchesToTwipsPref(
        GetPrefName(kMarginBottom, aPrinterName), margin.bottom, kMarginBottom);
    DUMP_INT(kReadStr, kMarginBottom, margin.bottom);
    ReadInchesToTwipsPref(
        GetPrefName(kMarginRight, aPrinterName), margin.right, kMarginRight);
  ...
  if (aFlags & nsIPrintSettings::kInitSaveEdges) {
    ...
    ReadInchesIntToTwipsPref(
        GetPrefName(kEdgeTop, aPrinterName), margin.top, kEdgeTop);
    DUMP_INT(kReadStr, kEdgeTop, margin.top);
    ReadInchesIntToTwipsPref(
        GetPrefName(kEdgeLeft, aPrinterName), margin.left, kEdgeLeft);
    DUMP_INT(kReadStr, kEdgeLeft, margin.left);
    ReadInchesIntToTwipsPref(
        GetPrefName(kEdgeBottom, aPrinterName), margin.bottom, kEdgeBottom);
    DUMP_INT(kReadStr, kEdgeBottom, margin.bottom);
    ReadInchesIntToTwipsPref(
        GetPrefName(kEdgeRight, aPrinterName), margin.right, kEdgeRight);
    ...
    if (success) {
      ...
      if (saveSanitizedSizePrefs) {
        SavePrintSettingsToPrefs(
            aPS, !aPrinterName.IsEmpty(), nsIPrintSettings::kInitSavePaperSize);
      }
#endif
    }
  }
 ...
 */
nsresult
nsPrintOptions::WritePrefs(nsIPrintSettings* aPS,
                           const nsAString& aPrinterName,
                           uint32_t aFlags)
{
  NS_ENSURE_ARG_POINTER(aPS);
...
nsresult
nsPrintOptions::_CreatePrintSettings(nsIPrintSettings** _retval)
{
 ...
 */
static nsresult
GetAdjustedPrinterName(nsIPrintSettings* aPS,
                       bool aUsePNP,
...
void
nsPrintOptions::ReadInchesToTwipsPref(const char* aPrefId,
                                      int32_t& aTwips,
...
void
nsPrintOptions::ReadInchesIntToTwipsPref(const char* aPrefId,
                                         int32_t& aTwips,
                                         const char* aMarginPref)
{
  int32_t value;
...
void
nsPrintOptions::ReadJustification(const char* aPrefId,
                                  int16_t& aJust,
                                  int16_t aInitValue)
{
  aJust = aInitValue;
