...
nsDeviceContextSpecGTK::MakePrintTarget()
{
  ...
  rv = NS_NewNativeLocalFile(
      nsDependentCString(buf), false, getter_AddRefs(mSpoolFile));
  if (NS_FAILED(rv)) {
    unlink(buf);
    return nullptr;
  }
 ...
 */
NS_IMETHODIMP
nsDeviceContextSpecGTK::Init(nsIWidget* aWidget,
...
/* static */
gboolean
nsDeviceContextSpecGTK::PrinterEnumerator(GtkPrinter* aPrinter, gpointer aData)
{
...
nsDeviceContextSpecGTK::EnumeratePrinters()
{
  gtk_enumerate_printers(
      &nsDeviceContextSpecGTK::PrinterEnumerator, this, nullptr, TRUE);
}
...
NS_IMETHODIMP
nsPrinterEnumeratorGTK::GetPrinterNameList(
...
NS_IMETHODIMP
nsPrinterEnumeratorGTK::GetDefaultPrinterName(nsAString& aDefaultPrinterName)
{
