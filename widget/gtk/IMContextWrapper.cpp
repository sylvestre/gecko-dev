...
IMContextWrapper::Init()
{
  ...
  if (sUseSimpleContext) {
    ...
    g_signal_connect(mSimpleContext,
                     "preedit_changed",
                     G_CALLBACK(&IMContextWrapper::OnChangeCompositionCallback),
                     this);
    g_signal_connect(
                             ...
                             bool aKeyDownEventWasSent /* = false */)
{
  ...
  MOZ_LOG(gGtkIMLog,
          ...
          ("0x%p   OnKeyEvent(), succeeded, filterThisEvent=%s "
           ...
           this,
           ToChar(filterThisEvent),
           ToChar(isFiltered),
           ToChar(mFilterKeyEvent),
           GetCompositionStateName()));
                                  ...
                                  const InputContextAction* aAction)
{
  ...
  MOZ_LOG(gGtkIMLog,
          ...
          ("0x%p SetInputContext(aCaller=0x%p, aContext={ mIMEState={ "
           ...
           this,
           aCaller,
           GetEnabledStateName(aContext->mIMEState.mEnabled),
...
IMContextWrapper::OnRetrieveSurroundingNative(GtkIMContext* aContext)
{
  ...
  gtk_im_context_set_surrounding(
      aContext, utf8Str.get(), utf8Str.Length(), cursorPosInUTF8);
                                       ...
                                       nsAString& aCompositionString)
{
  ...
  gtk_im_context_get_preedit_string(
      aContext, &preedit_string, &feedback_list, &cursor_pos);
...
bool
IMContextWrapper::DispatchCompositionCommitEvent(GtkIMContext* aContext,
                                       ...
                                       const nsAString& aCompositionString)
{
  ...
  PangoAttrList* feedback_list;
  gtk_im_context_get_preedit_string(
      aContext, &preedit_string, &feedback_list, &cursor_pos_in_chars);
  if (!preedit_string || !*preedit_string) {
                               ...
                               TextRange& aTextRange) const
{
  ...
  if (!utf8ClauseStart) {
    ...
    gunichar2* utf16PreviousClausesString =
        g_utf8_to_utf16(aUTF8CompositionString,
                        utf8ClauseStart,
                        nullptr,
...
nsresult
IMContextWrapper::GetCurrentParagraph(nsAString& aText, uint32_t& aCursorPos)
{
  MOZ_LOG(gGtkIMLog,
          LogLevel::Info,
  ...
  WidgetQueryContentEvent queryTextContentEvent(
      true, eQueryTextContent, mLastFocusedWindow);
                             ...
                             uint32_t aNChars)
{
  ...
  nsDependentCSubstring utf8StrBeforeOffset(
      utf8Str, 0, charAtOffset - utf8Str.get());
  selectionEvent.mOffset = NS_ConvertUTF8toUTF16(utf8StrBeforeOffset).Length();

  nsDependentCSubstring utf8DeletingStr(
      utf8Str, utf8StrBeforeOffset.Length(), charAtEnd - charAtOffset);
  ...
  WidgetContentCommandEvent contentCommandEvent(
      true, eContentCommandDelete, mLastFocusedWindow);
...
IMContextWrapper::EnsureToCacheSelection(nsAString* aSelectedString)
{
  ...
  nsEventStatus status;
  WidgetQueryContentEvent selection(
      true, eQuerySelectedText, mLastFocusedWindow);
  InitEvent(selection);
