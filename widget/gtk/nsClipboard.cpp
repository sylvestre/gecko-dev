...
void
clipboard_clear_cb(GtkClipboard* aGtkClipboard, gpointer user_data);
                     ...
                     int32_t aWhichClipboard)
{
  ...
  for (uint32_t i = 0; i < count; i++) {
    ...
    if (flavor) {
      ...
      if (flavorStr.EqualsLiteral(kUnicodeMime)) {
        ...
        gtk_target_list_add(
            list, gdk_atom_intern("COMPOUND_TEXT", FALSE), 0, 0);
  ...
  if (gtkTargets && gtk_clipboard_set_with_data(gtkClipboard,
                                                gtkTargets,
                                                numTargets,
                                                clipboard_get_cb,
                                                clipboard_clear_cb,
...
nsClipboard::GetData(nsITransferable* aTransferable, int32_t aWhichClipboard)
{
    ...
    if (currentFlavor) {
      ...
      if (flavorStr.EqualsLiteral(kJPEGImageMime) ||
        ...
        aTransferable->SetTransferData(
            flavorStr.get(), byteStream, sizeof(nsIInputStream*));
      ...
      if (selectionData) {
        ...
        if (flavorStr.EqualsLiteral(kHTMLMime)) {
          ...
          ConvertHTMLtoUCS2(const_cast<guchar*>(clipboardData),
                            length,
                            &htmlBody,
                            htmlBodyLen);
          // Try next data format?
          if (!htmlBodyLen) continue;
  ...
  if (foundData) {
    ...
    nsPrimitiveHelpers::CreatePrimitiveForData(
        foundFlavor, data, length, getter_AddRefs(wrapper));
    aTransferable->SetTransferData(foundFlavor.get(), wrapper, length);
  }
                                    ...
                                    bool* _retval)
{
  ...
  *_retval = false;
  ...
  if (!selection_data) return NS_OK;
...
{
  if (aWhichClipboard == kGlobalClipboard) return GDK_SELECTION_CLIPBOARD;
                               ...
                               GtkSelectionData* aSelectionData)
{
  ...
  if (gtk_targets_include_image(&selectionTarget, 1, TRUE)) {
    ...
    for (uint32_t i = 0; !ptrPrimitive && i < ArrayLength(imageMimeTypes);
         i++) {
      rv = trans->GetTransferData(
          imageMimeTypes[i], getter_AddRefs(imageItem), &len);
  ...
  nsPrimitiveHelpers::CreateDataFromPrimitive(
      nsDependentCString(target_name), item, &primitive_data, len);
  ...
  if (primitive_data) {
    ...
    if (selectionTarget == gdk_atom_intern(kHTMLMime, FALSE)) {
      /*
             ...
             */
      guchar* buffer =
          (guchar*)moz_xmalloc((len * sizeof(guchar)) + sizeof(char16_t));
      if (!buffer) return;
...
GetHTMLCharset(guchar* data, int32_t dataLength, nsCString& str)
{
  ...
  if (CaseInsensitiveFindInReadable(
    ...
    if (CaseInsensitiveFindInReadable(
            NS_LITERAL_CSTRING("charset="), start, end)) {
      valueStart = end;
      start = end;
      htmlStr.EndReading(end);
...
static gchar*
CopyRetrievedData(const gchar* aData)
{
  return g_strdup(aData);
}

static GtkSelectionData*
CopyRetrievedData(GtkSelectionData* aData)
{
  // A negative length indicates that retrieving the data failed.
  return gtk_selection_data_get_length(aData) >= 0
             ? gtk_selection_data_copy(aData)
             : nullptr;
}

class RetrievalContext
{
  ~RetrievalContext() { MOZ_ASSERT(!mData, "Wait() wasn't called"); }
...
RetrievalContext::Wait()
{
  ...
  if (GDK_IS_X11_DISPLAY(gdkDisplay)) {
    ...
    do {
      ...
      while (XCheckIfEvent(
          xDisplay, &xevent, checkEventProc, (XPointer)&context)) {
...
wait_for_contents(GtkClipboard* clipboard, GdkAtom target)
{
  ...
  gtk_clipboard_request_contents(
      clipboard, target, clipboard_contents_received, context.get());
