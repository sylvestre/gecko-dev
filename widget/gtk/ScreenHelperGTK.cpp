...
root_window_event_filter(GdkXEvent* aGdkXEvent,
                         GdkEvent* aGdkEvent,
                         gpointer aClosure)
{
#ifdef MOZ_X11
...
#endif
{
  ...
  g_signal_connect(
      defaultScreen, "monitors-changed", G_CALLBACK(monitors_changed), this);
#ifdef MOZ_X11
...
MakeScreen(GdkScreen* aScreen, gint aMonitorNum)
{
  ...
  MOZ_LOG(sScreenLog,
          ...
          ("New screen [%d %d %d %d (%d %d %d %d) %d %f %f %f]",
           ...
           pixelDepth,
           contentsScale.scale,
           defaultCssScale.scale,
           dpi));
  RefPtr<Screen> screen = new Screen(rect,
                                     availRect,
                                     pixelDepth,
                                     pixelDepth,
                                     contentsScale,
