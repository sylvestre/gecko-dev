                  ...
                  GdkRGBA* aDarkColor)
{
  ...
  *aLightColor = *aDarkColor;
  ...
  GdkRGBA stop;
  for (int index = 1;
       CAIRO_STATUS_SUCCESS == cairo_pattern_get_color_stop_rgba(pattern,
                                                                 index,
                                                                 nullptr,
                                                                 &stop.red,
                                                                 &stop.green,
                ...
                GdkRGBA* aDarkColor)
{
  ...
  gtk_style_context_get(
      aContext, state, GTK_STYLE_PROPERTY_BORDER_STYLE, &borderStyle, nullptr);
...
static int32_t
CheckWidgetStyle(GtkWidget* aWidget, const char* aStyle, int32_t aResult)
{
...
nsLookAndFeel::GetIntImpl(IntID aID, int32_t& aResult)
{
  ...
  switch (aID) {
    ...
    case eIntID_SelectTextfieldsOnKeyFocus: {
      ...
      g_object_get(
          settings, "gtk-entry-select-on-focus", &select_on_focus, nullptr);
                  ...
                  gfxFontStyle* aFontStyle)
{
  ...
  gtk_style_context_get(
      aStyle, gtk_style_context_get_state(aStyle), "font", &desc, nullptr);
...
bool
nsLookAndFeel::GetFontImpl(FontID aID,
                           nsString& aFontName,
                           gfxFontStyle& aFontStyle,
...
nsLookAndFeel::EnsureInit()
{
  ...
  gtk_style_context_get_background_color(
      style, GTK_STATE_FLAG_INSENSITIVE, &color);
  ...
  style = GetStyleContext(MOZ_GTK_MENUITEM);
  gtk_style_context_get_background_color(
      style, GTK_STATE_FLAG_PRELIGHT, &color);
  ...
  gtk_style_context_get_background_color(
      style, GTK_STATE_FLAG_NORMAL, &bgColor);
  ...
  if (param_spec) {
    if (g_type_is_a(G_PARAM_SPEC_VALUE_TYPE(param_spec), G_TYPE_BOOLEAN)) {
      gtk_widget_style_get(
          menuBar, "window-dragging", &supports_menubar_drag, nullptr);
    }
  }
  ...
  if (gtk_check_version(3, 12, 0) == nullptr) {
    ...
    if (colorValuePtr) {
      colorValue =
          *colorValuePtr;  // we can't pass deref pointers to GDK_COLOR_TO_NS_RGB
  ...
  // caret styles
  gtk_widget_style_get(entry, "cursor-aspect-ratio", &sCaretRatio, nullptr);

  GetSystemFontInfo(
      gtk_widget_get_style_context(entry), &mFieldFontName, &mFieldFontStyle);
