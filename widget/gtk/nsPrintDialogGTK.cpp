...
ShowCustomDialog(GtkComboBox* changed_box, gpointer user_data)
{
  if (gtk_combo_box_get_active(changed_box) != CUSTOM_VALUE_INDEX) {
    g_object_set_data(G_OBJECT(changed_box),
                      "previous-active",
  ...
  GtkWidget* prompt_dialog = gtk_dialog_new_with_buttons(
      NS_ConvertUTF16toUTF8(intlString).get(),
      printDialog,
#if (MOZ_WIDGET_GTK == 2)
  ...
  gtk_box_pack_start(GTK_BOX(custom_vbox),
                     custom_entry,
                     FALSE,
                     FALSE,
  ...
  gtk_box_pack_start(GTK_BOX(custom_hbox),
                     custom_vbox,
                     FALSE,
                     FALSE,
  ...
  if (diag_response == GTK_RESPONSE_ACCEPT) {
    ...
    g_object_set_data_full(G_OBJECT(changed_box),
                           "custom-text",
                           strdup(response_text),
                           (GDestroyNotify)free);
    g_object_set_data(G_OBJECT(changed_box),
                      "previous-active",
                                               ...
                                               nsIPrintSettings* aSettings)
{
  ...
  gtk_box_pack_start(
      GTK_BOX(frames_radio_list), radio_as_laid_out, FALSE, FALSE, 0);
  gtk_box_pack_start(
      GTK_BOX(frames_radio_list), radio_selected_frame, FALSE, FALSE, 0);
  gtk_box_pack_start(
      GTK_BOX(frames_radio_list), radio_separate_frames, FALSE, FALSE, 0);
  ...
  gtk_box_pack_start(
      GTK_BOX(check_buttons_container), shrink_to_fit_toggle, FALSE, FALSE, 0);
  ...
  if (gtk_major_version > 2 ||
    ...
    gtk_box_pack_start(GTK_BOX(check_buttons_container),
  ...
  gtk_box_pack_start(
      GTK_BOX(appearance_vertical_squasher), appearance_label, FALSE, FALSE, 0);
  ...
  gtk_alignment_set_padding(
      GTK_ALIGNMENT(header_footer_container), 8, 0, 12, 0);
  ...
  gtk_box_pack_start(
      GTK_BOX(custom_options_tab), print_frames_label, FALSE, FALSE, 0);
  gtk_box_pack_start(
      GTK_BOX(custom_options_tab), frames_radio_container, FALSE, FALSE, 0);
  ...
  gtk_print_unix_dialog_add_custom_tab(
      GTK_PRINT_UNIX_DIALOG(dialog), custom_options_tab, tab_label);
...
void
nsPrintDialogWidgetGTK::ExportFramePrinting(nsIPrintSettings* aNS,
    ...
    const char16_t* currentString)
{
...
#endif
  const char hf_options[][22] = {"headerFooterBlank",
                                 "headerFooterTitle",
                                 "headerFooterURL",
                                 "headerFooterDate",
                                 "headerFooterPage",
...
#else
    gtk_combo_box_text_append(GTK_COMBO_BOX_TEXT(dropdown),
                              nullptr,
                              GetUTF8FromBundle(hf_options[i]).get());
#endif
  }
  ...
  for (unsigned int i = 0; i < ArrayLength(header_footer_tags); i++) {
    if (!strcmp(currentStringUTF8.get(), header_footer_tags[i])) {
      ...
      g_object_set_data(
          G_OBJECT(dropdown), "previous-active", GINT_TO_POINTER(i));
  ...
  if (shouldBeCustom) {
    ...
    g_object_set_data(G_OBJECT(dropdown),
                      "previous-active",
                      GINT_TO_POINTER(CUSTOM_VALUE_INDEX));
    char* custom_string = strdup(currentStringUTF8.get());
    g_object_set_data_full(
        G_OBJECT(dropdown), "custom-text", custom_string, (GDestroyNotify)free);
  }
                                       ...
                                       nsIPrintSettings* aNSSettings)
{
  ...
  if (psService) {
    ...
    psService->InitPrintSettingsFromPrefs(
        aNSSettings, true, nsIPrintSettings::kInitSaveAll);
  }
