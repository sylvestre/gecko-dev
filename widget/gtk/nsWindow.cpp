...
get_inner_gdk_window(
    GdkWindow* aWindow, gint x, gint y, gint* retx, gint* rety);
...
static void
GetBrandName(nsAString& brandName);
...
#else
static gboolean
expose_event_cb(GtkWidget* widget, cairo_t* rect);
#endif
static gboolean
...
static gboolean
enter_notify_event_cb(GtkWidget* widget, GdkEventCrossing* event);
static gboolean
leave_notify_event_cb(GtkWidget* widget, GdkEventCrossing* event);
static gboolean
...
static void
hierarchy_changed_cb(GtkWidget* widget, GtkWidget* previous_toplevel);
static gboolean
...
popup_take_focus_filter(GdkXEvent* gdk_xevent, GdkEvent* event, gpointer data);
#endif /* MOZ_X11 */
#ifdef __cplusplus
}
#endif /* __cplusplus */
...
static inline bool
TimestampIsNewerThan(guint32 a, guint32 b)
{
...
nsWindow::Destroy()
{
  ...
  g_signal_handlers_disconnect_by_func(
      gtk_settings_get_default(), FuncToGpointer(theme_changed_cb), this);
...
nsWindow::SetParent(nsIWidget* aNewParent)
{
  ...
  if (aNewParent) {
    ...
    ReparentNativeWidgetInternal(
        aNewParent, newContainer, newParentWindow, oldContainer);
  }
}
                                       ...
                                       GtkWidget* aOldContainer)
{
  if (!aNewContainer) {
    ...
    if (!mIsTopLevel) {
      gdk_window_reparent(mGdkWindow,
                          aNewParentWindow,
...
nsWindow::SetSizeConstraints(const SizeConstraints& aConstraints)
{
  ...
  if (mShell) {
    ...
    gtk_window_set_geometry_hints(
        GTK_WINDOW(mShell), nullptr, &geometry, GdkWindowHints(hints));
  }
}
...
nsWindow::GetScreenBounds()
{
  ...
  if (mIsTopLevel && mContainer) {
    ...
    gdk_window_get_root_origin(
        gtk_widget_get_window(GTK_WIDGET(mContainer)), &x, &y);
                    ...
                    uint32_t aHotspotY)
{
  ...
  GdkCursor* cursor = gdk_cursor_new_from_pixbuf(
      gdk_display_get_default(), pixbuf, aHotspotX, aHotspotY);
  g_object_unref(pixbuf);
...
nsWindow::SetIcon(const nsAString& aIconSpec)
{
  ...
  if (!foundIcon) {
    ...
    const char extensions[6][7] = {
        ".png", "16.png", "32.png", "48.png", ".xpm", "16.xpm"};
      ...
      if (iconFile) {
        ...
        if (icon) {
          gtk_icon_theme_add_builtin_icon(
              iconName.get(), gdk_pixbuf_get_height(icon), icon);
          g_object_unref(icon);
          foundIcon = true;
        }
      }
...
nsWindow::OnEnterNotifyEvent(GdkEventCrossing* aEvent)
{
  ...
  WidgetMouseEvent event(
      true, eMouseEnterIntoWidget, this, WidgetMouseEvent::eReal);
...
nsWindow::OnLeaveNotifyEvent(GdkEventCrossing* aEvent)
{
  ...
  WidgetMouseEvent event(
      true, eMouseExitFromWidget, this, WidgetMouseEvent::eReal);
...
nsWindow::DispatchMissedButtonReleases(GdkEventCrossing* aGdkEvent)
{
  ...
  for (guint buttonMask = GDK_BUTTON1_MASK; buttonMask <= GDK_BUTTON3_MASK;
    ...
    if (released & buttonMask) {
      ...
      WidgetMouseEvent synthEvent(
          true, eMouseUp, this, WidgetMouseEvent::eSynthesized);
...
void
nsWindow::InitButtonEvent(WidgetMouseEvent& aEvent, GdkEventButton* aGdkEvent)
{
...
nsWindow::OnButtonPressEvent(GdkEventButton* aEvent)
{
  ...
  if (domButton == WidgetMouseEvent::eRightButton &&
    ...
    WidgetMouseEvent contextMenuEvent(
        true, eContextMenu, this, WidgetMouseEvent::eReal);
...
nsWindow::MaybeDispatchContextMenuEvent(const GdkEventKey* aEvent)
{
  ...
  WidgetMouseEvent contextMenuEvent(true,
                                    eContextMenu,
                                    this,
...
void
nsWindow::DispatchDragEvent(EventMessage aMsg,
                                  ...
                                  gpointer aData)
{
  ...
  RefPtr<nsDragService> dragService = nsDragService::GetInstance();
  dragService->TargetDataReceived(
      aWidget, aDragContext, aX, aY, aSelectionData, aInfo, aTime);
}
...
GetBrandName(nsAString& aBrandName)
{
  ...
  if (bundle) bundle->GetStringFromName("brandShortName", aBrandName);

  if (aBrandName.IsEmpty()) aBrandName.AssignLiteral(u"Mozilla");
}
                 ...
                 nsWidgetInitData* aInitData)
{
    ...
    case eWindowType_invisible: {
      ...
      if (mWindowType == eWindowType_dialog) {
        ...
        gtk_window_set_wmclass(
            GTK_WINDOW(mShell), "Dialog", gdk_get_program_class());
        ...
        gtk_window_set_wmclass(
            GTK_WINDOW(mShell), "Popup", gdk_get_program_class());
        ...
        if (aInitData->mNoAutoHide) {
          ...
          gdk_window_add_filter(
              gtk_widget_get_window(mShell), popup_take_focus_filter, nullptr);
#endif
        }
        ...
        SetDefaultIcon();
        gtk_window_set_wmclass(
            GTK_WINDOW(mShell), "Toplevel", gdk_get_program_class());
  ...
  if (mShell) {
    g_signal_connect(
        mShell, "configure_event", G_CALLBACK(configure_event_cb), nullptr);
    g_signal_connect(
        mShell, "delete_event", G_CALLBACK(delete_event_cb), nullptr);
    g_signal_connect(mShell,
                     "window_state_event",
                     G_CALLBACK(window_state_event_cb),
                     nullptr);
    g_signal_connect(
        mShell, "check-resize", G_CALLBACK(check_resize_cb), nullptr);

    GdkScreen* screen = gtk_widget_get_screen(mShell);

    g_signal_connect(mShell,
                     "composited-changed",
                     G_CALLBACK(widget_composited_changed_cb),
                     nullptr);

    if (!g_signal_handler_find(screen,
                               G_SIGNAL_MATCH_FUNC,
                               0,
                               0,
                               nullptr,
      ...
      g_signal_connect(screen,
                       "composited-changed",
                       G_CALLBACK(screen_composited_changed_cb),
                       nullptr);
    }
  ...
  if (mContainer) {
    // Widget signals
    g_signal_connect(
        mContainer, "unrealize", G_CALLBACK(container_unrealize_cb), nullptr);
    g_signal_connect_after(
        mContainer, "size_allocate", G_CALLBACK(size_allocate_cb), nullptr);
    g_signal_connect(mContainer,
                     "hierarchy-changed",
                     G_CALLBACK(hierarchy_changed_cb),
                     nullptr);
#if (MOZ_WIDGET_GTK == 3)
    g_signal_connect(mContainer,
                     "notify::scale-factor",
...
#if (MOZ_WIDGET_GTK == 2)
    g_signal_connect(
        mContainer, "expose_event", G_CALLBACK(expose_event_cb), nullptr);
#else
    g_signal_connect(
        G_OBJECT(mContainer), "draw", G_CALLBACK(expose_event_cb), nullptr);
#endif
    g_signal_connect(
        mContainer, "focus_in_event", G_CALLBACK(focus_in_event_cb), nullptr);
    g_signal_connect(
        mContainer, "focus_out_event", G_CALLBACK(focus_out_event_cb), nullptr);
    g_signal_connect(
        mContainer, "key_press_event", G_CALLBACK(key_press_event_cb), nullptr);
    g_signal_connect(mContainer,
                     "key_release_event",
    ...
    g_signal_connect(
        mContainer, "drag_motion", G_CALLBACK(drag_motion_event_cb), nullptr);
    g_signal_connect(
        mContainer, "drag_leave", G_CALLBACK(drag_leave_event_cb), nullptr);
    g_signal_connect(
        mContainer, "drag_drop", G_CALLBACK(drag_drop_event_cb), nullptr);
    g_signal_connect(mContainer,
                     "drag_data_received",
                     G_CALLBACK(drag_data_received_event_cb),
                     nullptr);
    ...
    for (size_t i = 0; i < ArrayLength(widgets) && widgets[i]; ++i) {
      ...
      g_signal_connect(widgets[i],
                       ...
                       G_CALLBACK(visibility_notify_event_cb),
                       nullptr);
  ...
  if (eventWidget) {
    ...
    g_signal_connect(eventWidget,
                     "enter-notify-event",
                     G_CALLBACK(enter_notify_event_cb),
                     nullptr);
    g_signal_connect(eventWidget,
                     "leave-notify-event",
    ...
    g_signal_connect(eventWidget,
                     ...
                     nullptr);
    g_signal_connect(
        eventWidget, "scroll-event", G_CALLBACK(scroll_event_cb), nullptr);
#if GTK_CHECK_VERSION(3, 4, 0)
    g_signal_connect(
        eventWidget, "touch-event", G_CALLBACK(touch_event_cb), nullptr);
#endif
  }
...
nsWindow::NativeMoveResize()
{
  ...
  if (mIsTopLevel) {
    ...
    gdk_window_move_resize(
        mGdkWindow, topLeft.x, topLeft.y, size.width, size.height);
  }
...
static bool
ChangedMaskBits(gchar* aMaskBits,
                int32_t aMaskWidth,
                int32_t aMaskHeight,
...
static void
UpdateMaskBits(gchar* aMaskBits,
               int32_t aMaskWidth,
               int32_t aMaskHeight,
               const nsIntRect& aRect,
               uint8_t* aAlphas,
...
nsWindow::ApplyTransparencyBitmap()
{
  ...
  XShapeCombineMask(
      xDisplay, xDrawable, ShapeBounding, 0, 0, maskPixmap, ShapeSet);
  XFreePixmap(xDisplay, maskPixmap);
#else
#if (MOZ_WIDGET_GTK == 2)
  gtk_widget_reset_shapes(mShell);
  GdkBitmap* maskBitmap =
      gdk_bitmap_create_from_data(mGdkWindow,
                                  mTransparencyBitmap,
                                  mTransparencyBitmapWidth,
                                  mTransparencyBitmapHeight);
  ...
  maskBitmap = cairo_image_surface_create_for_data(
      (unsigned char*)mTransparencyBitmap,
      CAIRO_FORMAT_A1,
      mTransparencyBitmapWidth,
      mTransparencyBitmapHeight,
                                               ...
                                               int32_t aStride)
{
  if (!mShell) {
    ...
    return topWindow->UpdateTranslucentWindowAlphaInternal(
  ...
  if (!ChangedMaskBits(mTransparencyBitmap,
                       mBounds.width,
                       mBounds.height,
                       rect,
                       aAlphas,
...
nsWindow::GetWidgetScreen()
{
  ...
  nsCOMPtr<nsIScreen> screen;
  screenManager->ScreenForRect(deskBounds.x,
                               deskBounds.y,
                               deskBounds.width,
                               deskBounds.height,
...
nsWindow::HideWindowChrome(bool aShouldHide)
{
  ...
  if (wmd != -1) gdk_window_set_decorations(mGdkWindow, (GdkWMDecoration)wmd);

  if (wasVisible) gdk_window_show(mGdkWindow);
...
bool
nsWindow::CheckForRollup(gdouble aMouseX,
                         gdouble aMouseY,
                         bool aIsWheel,
                         bool aAlwaysRollup)
{
  ...
  if (aAlwaysRollup || !is_mouse_in_window(currentPopup, aMouseX, aMouseY)) {
    ...
    if (rollup &&
        rollupListener->Rollup(
            popupsToRollup, true, usePoint ? &point : nullptr, nullptr)) {
      retVal = true;
    }
  }
  return retVal;
...
get_gtk_cursor(nsCursor aCursor)
{
  ...
  if (newType != 0xff && !gdkcursor) {
    ...
    gdkcursor = gdk_cursor_new_from_pixbuf(gdk_display_get_default(),
                                           cursor_pixbuf,
                     ...
                     gpointer aData)
{
  ...
  GdkWindow* innerWindow = get_inner_gdk_window(
      gtk_widget_get_window(aWidget), aX, aY, &retx, &rety);
  ...
  RefPtr<nsDragService> dragService = nsDragService::GetInstance();
  return dragService->ScheduleMotionEvent(
      innerMostWindow, aDragContext, point, aTime);
}
                            ...
                            gpointer aData)
{
  ...
  window->OnDragDataReceivedEvent(
      aWidget, aDragContext, aX, aY, aSelectionData, aInfo, aTime, aData);
}
...
static GdkWindow*
get_inner_gdk_window(GdkWindow* aWindow, gint x, gint y, gint* retx, gint* rety)
{
  gint cx, cy, cw, ch;
...
nsWindow::BeginMoveDrag(WidgetMouseEvent* aEvent)
{
  ...
  gdk_window_begin_move_drag(
      gdk_window, button, screenX, screenY, aEvent->mTime);
                          ...
                          int32_t aVertical)
{
  ...
  gint button, screenX, screenY;
  if (!GetDragInfo(
          aEvent->AsMouseEvent(), &gdk_window, &button, &screenX, &screenY)) {
    return NS_ERROR_FAILURE;
  }
  ...
  gdk_window_begin_resize_drag(
      gdk_window, window_edge, button, screenX, screenY, aEvent->mTime);
    ...
    mozilla::widget::CompositorWidgetInitData* aInitData)
{
  ...
  *aInitData = mozilla::widget::X11CompositorWidgetInitData(
      mXWindow, nsCString(XDisplayString(mXDisplay)), GetClientSize());
#endif
}
