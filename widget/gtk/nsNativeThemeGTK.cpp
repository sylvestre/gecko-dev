...
NS_IMPL_ISUPPORTS_INHERITED(nsNativeThemeGTK,
                            nsNativeTheme,
                            nsITheme,
                            nsIObserver)
...
NS_IMETHODIMP
nsNativeThemeGTK::Observe(nsISupports* aSubject,
                          const char* aTopic,
...
static bool
IsFrameContentNodeInNamespace(nsIFrame* aFrame, uint32_t aNamespace)
{
...
static bool
IsWidgetTypeDisabled(uint8_t* aDisabledVector, uint8_t aWidgetType)
{
...
static void
SetWidgetTypeDisabled(uint8_t* aDisabledVector, uint8_t aWidgetType)
{
  MOZ_ASSERT(aWidgetType < ThemeWidgetType_COUNT);
...
static bool
IsWidgetStateSafe(uint8_t* aSafeVector,
                  uint8_t aWidgetType,
...
static void
SetWidgetStateSafe(uint8_t* aSafeVector,
                   uint8_t aWidgetType,
                   GtkWidgetState* aWidgetState)
{
  MOZ_ASSERT(aWidgetType < ThemeWidgetType_COUNT);
...
static bool
ShouldScrollbarButtonBeDisabled(int32_t aCurpos,
                                int32_t aMaxpos,
                                uint8_t aWidgetType)
{
  return (
...
bool
nsNativeThemeGTK::GetGtkWidgetAndState(uint8_t aWidgetType,
                                       nsIFrame* aFrame,
  ...
  if (aState) {
    ...
    if (aFrame && ((aWidgetFlags && (aWidgetType == NS_THEME_CHECKBOX ||
      ...
      if (IsFrameContentNodeInNamespace(aFrame, kNameSpaceID_XUL)) {
        ...
        if (aWidgetFlags) {
          nsCOMPtr<nsIDOMHTMLInputElement> inputElt(
              do_QueryInterface(aFrame->GetContent()));
          *aWidgetFlags = 0;
          if (inputElt) {
            bool isHTMLChecked;
            inputElt->GetChecked(&isHTMLChecked);
            if (isHTMLChecked) *aWidgetFlags |= MOZ_GTK_WIDGET_CHECKED;
          }
...
{
 public:
  ThemeRenderer(GtkWidgetState aState,
                WidgetNodeType aGTKWidgetType,
                gint aFlags,
                GtkTextDirection aDirection,
                const GdkRectangle& aGDKRect,
  ...
  nsresult DrawWithGDK(GdkDrawable* drawable,
                       gint offsetX,
                       gint offsetY,
...
nsresult
ThemeRenderer::DrawWithGDK(GdkDrawable* drawable,
                           gint offsetX,
                           gint offsetY,
                           GdkRectangle* clipRects,
                           uint32_t numClipRects)
{
  ...
  moz_gtk_widget_paint(mGTKWidgetType,
                       drawable,
                       &gdk_rect,
                       &gdk_clip,
                       &mState,
...
class SystemCairoClipper : public ClipExporter
{
 public:
  ...
  void BezierTo(const Point& aCP1,
                const Point& aCP2,
...
static void
DrawThemeWithCairo(gfxContext* aContext,
                   ...
                   nsITheme::Transparency aTransparency)
{
  ...
  if (borrowCairo.mCairo) {
    ...
    moz_gtk_widget_paint(aGTKWidgetType,
                         borrowCairo.mCairo,
                         &aGDKRect,
                         &aState,
                         aFlags,
    ...
    if (!NS_WARN_IF(!surf)) {
      ...
      if (!NS_WARN_IF(!cr)) {
        ...
        moz_gtk_widget_paint(
            aGTKWidgetType, cr, &aGDKRect, &aState, aFlags, aDirection);
                                       ...
                                       const nsRect& aDirtyRect)
{
  ...
  ThemeRenderer renderer(
      state, gtkWidgetType, flags, direction, gdk_rect, gdk_clip);
...
nsNativeThemeGTK::NativeThemeToGtkTheme(uint8_t aWidgetType, nsIFrame* aFrame)
{
  ...
  if (!GetGtkWidgetAndState(
          aWidgetType, aFrame, gtkWidgetType, nullptr, &unusedFlags)) {
  ...
  if (GetGtkWidgetAndState(
    ...
    if (mBorderCacheValid[cacheIndex] & cacheBit) {
      *aResult = mBorderCache[gtkWidgetType];
      ...
      moz_gtk_get_widget_border(gtkWidgetType,
                                &aResult->left,
                                &aResult->top,
                                &aResult->right,
                                &aResult->bottom,
                                  ...
                                  nsIntMargin* aResult)
{
  ...
  switch (aWidgetType) {
    ...
    case NS_THEME_TAB: {
      ...
      if (!GetGtkWidgetAndState(
              aWidgetType, aFrame, gtkWidgetType, nullptr, &flags))
        return NS_OK;

      moz_gtk_get_tab_border(&aResult->left,
                             &aResult->top,
                             &aResult->right,
                             &aResult->bottom,
                             direction,
  ...
  switch (aWidgetType) {
    ...
    case NS_THEME_RADIOMENUITEM: {
      ...
      GetCachedWidgetBorder(
          aFrame, aWidgetType, GetTextDirection(aFrame), aResult);
  ...
  switch (aWidgetType) {
    ...
    case NS_THEME_RANGE_THUMB: {
      ...
      if (IsRangeHorizontal(aFrame)) {
        moz_gtk_get_scalethumb_metrics(
            GTK_ORIENTATION_HORIZONTAL, &thumb_length, &thumb_height);
      } else {
        moz_gtk_get_scalethumb_metrics(
            GTK_ORIENTATION_VERTICAL, &thumb_height, &thumb_length);
      }
    ...
    case NS_THEME_SCALETHUMB_VERTICAL: {
      ...
      if (aWidgetType == NS_THEME_SCALETHUMB_VERTICAL) {
        moz_gtk_get_scalethumb_metrics(
            GTK_ORIENTATION_VERTICAL, &thumb_length, &thumb_height);
    ...
    case NS_THEME_BUTTON_ARROW_PREVIOUS: {
      moz_gtk_get_arrow_size(
          MOZ_GTK_TOOLBARBUTTON_ARROW, &aResult->width, &aResult->height);
      *aIsOverridable = false;
    } break;
    case NS_THEME_CHECKBOX_CONTAINER:
    case NS_THEME_RADIO_CONTAINER:
    ...
    case NS_THEME_WINDOW_BUTTON_RESTORE: {
      if (aWidgetType == NS_THEME_MENULIST) {
        ...
        moz_gtk_get_arrow_size(
            MOZ_GTK_DROPDOWN, &aResult->width, &aResult->height);
      }
      ...
      nsNativeThemeGTK::GetWidgetBorder(
          aFrame->PresContext()->DeviceContext(), aFrame, aWidgetType, &border);
