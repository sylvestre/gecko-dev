...
ca_context_get_default()
{
  ...
  g_static_private_set(
      &ctx_static_private, ctx, (GDestroyNotify)ca_context_destroy);
  ...
  if (g_object_class_find_property(G_OBJECT_GET_CLASS(settings),
    ...
    if (sound_theme_name) {
      ca_context_change_props(
          ctx, "canberra.xdg-theme.name", sound_theme_name, nullptr);
  ...
  if (bundleService) {
    ...
    if (brandingBundle) {
      ...
      ca_context_change_props(ctx, "application.name", brand.get(), nullptr);
    }
  }
...
static void
ca_finish_cb(ca_context* c, uint32_t id, int error_code, void* userdata)
{
                          ...
                          const uint8_t* data)
{
  ...
  nsDirectoryService::gService->Get(
      NS_OS_TEMP_DIR, NS_GET_IID(nsIFile), getter_AddRefs(tmpFile));
  ...
  rv = canberraFile->OpenNSPRFileDesc(
      PR_WRONLY, PR_IRUSR | PR_IWUSR, &fd.rwget());
  if (NS_FAILED(rv)) {
    return rv;
  }
