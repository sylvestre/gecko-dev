...
nsDragService::nsDragService() : mScheduledTask(eDragTaskNone), mTaskSource(0)
{
  ...
  g_signal_connect(
      mHiddenWidget, "drag_begin", G_CALLBACK(invisibleSourceDragBegin), this);
  g_signal_connect(mHiddenWidget,
                   "drag_data_get",
                   G_CALLBACK(invisibleSourceDragDataGet),
                   this);
  g_signal_connect(
      mHiddenWidget, "drag_end", G_CALLBACK(invisibleSourceDragEnd), this);
...
NS_IMETHODIMP
nsDragService::Observe(nsISupports* aSubject,
                       const char* aTopic,
    ...
    nsContentPolicyType aContentPolicyType = nsIContentPolicy::TYPE_OTHER)
{
  ...
  return nsBaseDragService::InvokeDragSession(
      aDOMNode, aArrayTransferables, aRegion, aActionType, aContentPolicyType);
}
                                     ...
                                     uint32_t aActionType)
{
  ...
  if (context) {
    ...
    if (sGrabWidget) {
      ...
      g_signal_connect(
          sGrabWidget, "event-after", G_CALLBACK(OnSourceGrabEventAfter), this);
    }
                              ...
                              const LayoutDeviceIntRect& dragRect)
{
  ...
  if (!alphaColormap) return false;

  GdkPixmap* pixmap =
      gdk_pixmap_new(nullptr,
                     dragRect.width,
                     dragRect.height,
  ...
  RefPtr<DrawTarget> dt = nsWindow::GetDrawTargetForGdkDrawable(
      GDK_DRAWABLE(pixmap), IntSize(dragRect.width, dragRect.height));
  if (!dt) return false;
  ...
  gtk_drag_set_icon_pixmap(
      aContext, alphaColormap, pixmap, nullptr, aXOffset, aYOffset);
...
static uint32_t
CountTextUriListItems(const char* data, uint32_t datalen)
{
  const char* p = data;
...
nsDragService::GetData(nsITransferable* aTransferable, uint32_t aItemIndex)
{
  ...
  if (isList) {
    ...
    for (i = 0; i < cnt; ++i) {
      ...
      rv = item->GetTransferData(
          flavorStr.get(), getter_AddRefs(data), &tmpDataLen);
      if (NS_FAILED(rv)) {
  ...
  for (i = 0; i < cnt; ++i) {
    ...
    if (currentFlavor) {
                ...
                if (NS_SUCCEEDED(rv)) {
                  ...
                  if (NS_SUCCEEDED(rv)) {
                    ...
                    aTransferable->SetTransferData(
                        flavorStr.get(), file, convertedTextLen);
      ...
      if (dataFound) {
        ...
        aTransferable->SetTransferData(
            flavorStr.get(), genericDataWrapper, mTargetDragDataLen);
...
void
nsDragService::SourceEndDragSession(GdkDragContext* aContext, gint aResult)
{
  ...
  for (i = 0; i < count; i++) {
    ...
    if (item) {
      ...
      if (NS_SUCCEEDED(rv)) {
        ...
        UTF16ToNewUTF8(
            castedUnicode, tmpDataLen / 2, &plainTextData, &plainTextLen);
        if (plainTextData) {
          uint32_t j;
    ...
    if (NS_SUCCEEDED(rv)) {
      ...
      if (tmpData) {
        ...
        gtk_selection_data_set(
            aSelectionData, target, 8, (guchar*)tmpData, tmpDataLen);
        // this wasn't allocated with glib
        free(tmpData);
      }
    } else {
      if (mimeFlavor.EqualsLiteral(gTextUriListType)) {
        ...
        gtk_selection_data_set(
            aSelectionData, target, 8, (guchar*)uriList, length);
        g_free(uriList);
        return;
      }
    }
...
void
nsDragService::SetDragIcon(GdkDragContext* aContext)
{
  ...
  DrawDrag(
      mSourceNode, mSourceRegion, mScreenPosition, &dragRect, &surface, &pc);
  if (!pc) return;
...
nsDragService::RunScheduledTask()
{
  ...
  if (task == eDragTaskDrop) {
    ...
    // transferred.
    gtk_drag_finish(mTargetDragContext,
                    success,
                    /* del = */ FALSE,
                    mTargetTime);
