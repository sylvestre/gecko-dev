...
static void
gtk_xtbin_unrealize(GtkWidget* widget);
static void
...
static gboolean
xt_event_prepare(GSource* source_data, gint* timeout)
{
  int mask;
...
static GSourceFuncs xt_event_funcs = {xt_event_prepare,
                                      xt_event_check,
                                      xt_event_dispatch,
                                      NULL,
                                      (GSourceFunc)NULL,
...
gtk_xtbin_realize(GtkWidget* widget)
{
  ...
  xt_client_create(
      &(xtbin->xtclient), gtk_socket_get_id(GTK_SOCKET(xtbin)), h, w);
               ...
               int xtdepth)
{
  ...
  if (!xt_is_initialized) {
    ...
    xtdisplay = XtOpenDisplay(app_context,
                              gdk_get_display(),
                              NULL,
                              "Wrapper",
                              NULL,
...
xt_client_xloop_create(void)
{
  ...
  if (0 == num_widgets) {
    ...
    g_main_context_add_poll(
        (GMainContext*)NULL, &xt_event_poll_fd, G_PRIORITY_LOW);
...
*  */
void
xt_client_create(XtClient* xtclient, Window embedderid, int height, int width)
{
  int n;
  Arg args[6];
  Widget child_widget;
...
#endif
  top_widget = XtAppCreateShell("drawingArea",
                                "Wrapper",
  ...
  XtAddEventHandler(child_widget,
                    ...
                    TRUE,
                    (XtEventHandler)xt_client_event_handler,
                    xtclient);
  XtAddEventHandler(child_widget,
...
xt_client_set_info(Widget xtplug, unsigned long flags)
{
  ...
  XChangeProperty(XtDisplay(xtplug),
                  XtWindow(xtplug),
                  infoAtom,
                  infoAtom,
                  32,
