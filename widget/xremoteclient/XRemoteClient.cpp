...
static const char* XAtomNames[] = {MOZILLA_VERSION_PROP,
                                   MOZILLA_LOCK_PROP,
...
XRemoteClient::Init()
{
  ...
  // get our atoms
  XInternAtoms(mDisplay,
               const_cast<char**>(XAtomNames),
               MOZ_ARRAY_LENGTH(XAtomNames),
               False,
               XAtoms);
...
nsresult
XRemoteClient::SendCommandLine(const char* aProgram,
  ...
  *aWindowFound = false;
  ...
  if (w) {
    ...
    *aWindowFound = true;
    ...
    if (NS_SUCCEEDED(rv)) {
      // send our command
      rv = DoSendCommandLine(
          w, argc, argv, aDesktopStartupID, aResponse, &destroyed);
    ...
    if (!locked && !NS_FAILED(rv)) {
	 ...
	 to come in, and try again. */
      ...
      while (true) {
        ...
        if (select_retval == 0) {
          MOZ_LOG(
              sRemoteLm, LogLevel::Debug, ("timed out waiting for window\n"));
          rv = NS_ERROR_FAILURE;
          break;
        }
...
Window
XRemoteClient::FindBestWindow(const char* aProgram,
                              const char* aUsername,
                              const char* aProfile)
{
  ...
  for (unsigned int i = 0; i < nkids; i++) {
    ...
    int status = XGetWindowProperty(mDisplay,
                                    w,
                                    mMozVersionAtom,
                                    0,
                                    (65536 / sizeof(long)),
    ...
    if (aProgram && strcmp(aProgram, "any")) {
      status = XGetWindowProperty(mDisplay,
                                  w,
                                  mMozProgramAtom,
                                  0,
                                  (65536 / sizeof(long)),
...
nsresult
XRemoteClient::DoSendCommandLine(Window aWindow,
                                 int32_t argc,
                                 char** argv,
                                 const char* aDesktopStartupID,
                                 char** aResponse,
                                 bool* aDestroyed)
{
  *aDestroyed = false;
  ...
  if (!getcwd(cwdbuf, MAX_PATH)) return NS_ERROR_UNEXPECTED;
...
bool
XRemoteClient::WaitForResponse(Window aWindow,
                               char** aResponse,
                               bool* aDestroyed,
                               Atom aCommandAtom)
{
