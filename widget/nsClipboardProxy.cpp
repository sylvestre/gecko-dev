                          ...
                          int32_t aWhichClipboard)
{
  ...
  ContentChild::GetSingleton()->SendGetClipboard(
      types, aWhichClipboard, &dataTransfer);
  ...
  for (uint32_t j = 0; j < items.Length(); ++j) {
    ...
    if (item.data().type() == IPCDataTransferData::TnsString) {
      ...
      rv = aTransferable->SetTransferData(
          item.flavor().get(), dataWrapper, data.Length() * sizeof(char16_t));
      NS_ENSURE_SUCCESS(rv, rv);
      ...
      if (flavor.EqualsLiteral(kJPEGImageMime) ||
        ...
        rv = aTransferable->SetTransferData(
            flavor.get(), stream, sizeof(nsISupports*));
        NS_ENSURE_SUCCESS(rv, rv);
        ...
        rv = aTransferable->SetTransferData(
            item.flavor().get(), dataWrapper, data.Size<char>());
        NS_ENSURE_SUCCESS(rv, rv);
      }
                                         ...
                                         bool* aHasType)
{
  *aHasType = false;
  ...
  ContentChild::GetSingleton()->SendClipboardHasType(
      types, aWhichClipboard, aHasType);
