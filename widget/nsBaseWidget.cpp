...
void
nsBaseWidget::ReleaseContentController()
{
...
void
nsBaseWidget::DestroyLayerManager()
{
  if (mLayerManager) {
...
//-------------------------------------------------------------------------
void
nsBaseWidget::BaseCreate(nsIWidget* aParent, nsWidgetInitData* aInitData)
{
...
void
nsBaseWidget::SetWidgetListener(nsIWidgetListener* aWidgetListener)
{
...
void
nsBaseWidget::SetPreviouslyAttachedWidgetListener(nsIWidgetListener* aListener)
{
  mPreviouslyAttachedWidgetListener = aListener;
}

void
nsBaseWidget::SetAttachedWidgetListener(nsIWidgetListener* aListener)
{
...
nsIWidget::DefaultScaleOverride()
{
  ...
  if (!valueCached) {
    Preferences::AddFloatVarCache(
        &devPixelsPerCSSPixel, "layout.css.devPixelsPerPx", -1.0f);
    valueCached = true;
  }
...
//-------------------------------------------------------------------------
void
nsBaseWidget::AddChild(nsIWidget* aChild)
{
...
//-------------------------------------------------------------------------
void
nsBaseWidget::RemoveChild(nsIWidget* aChild)
{
#ifdef DEBUG
#ifdef XP_MACOSX
...
//-------------------------------------------------------------------------
void
nsBaseWidget::SetZIndex(int32_t aZIndex)
{
...
void
nsBaseWidget::SetTransparencyMode(nsTransparencyMode aMode)
{
}
...
bool
nsBaseWidget::IsWindowClipRegionEqual(
    const nsTArray<LayoutDeviceIntRect>& aRects)
{
  return mClipRects && mClipRectCount == aRects.Length() &&
         memcmp(mClipRects.get(),
                aRects.Elements(),
...
bool
nsBaseWidget::IsSmallPopup() const
{
...
void
nsBaseWidget::SetConfirmedTargetAPZC(
    uint64_t aInputBlockId, const nsTArray<ScrollableLayerGuid>& aTargets) const
{
  APZThreadUtils::RunOnControllerThread(
      NewRunnableMethod<uint64_t,
  ...
  if (mAPZC && !context.WasRoutedToChildProcess() && aInputBlockId) {
    ...
    if (WidgetTouchEvent* touchEvent = aEvent->AsTouchEvent()) {
      ...
      mAPZEventState->ProcessTouchEvent(
          *touchEvent, aGuid, aInputBlockId, aApzResponse, status);
    } else if (WidgetWheelEvent* wheelEvent = aEvent->AsWheelEvent()) {
      MOZ_ASSERT(wheelEvent->mFlags.mHandledByAPZ);
      APZCCallbackHelper::SendSetTargetAPZCNotification(
          this,
          GetDocument(),
          *(original->AsWheelEvent()),
          aGuid,
          aInputBlockId);
...
class DispatchWheelEventOnMainThread : public Runnable
{
 public:
  ...
  NS_IMETHOD Run() override
  {
    ...
    mWidget->ProcessUntransformedAPZEvent(
        &wheelEvent, mGuid, mInputBlockId, mAPZResult);
    return NS_OK;
  }
                                      ...
                                      CompositorOptions* aOptionsOut)
{
  ...
  do {
    ...
    mCompositorSession =
        gpu->CreateTopLevelCompositor(this,
                                      lm,
                                      GetDefaultScale(),
                                      options,
...
void
nsBaseWidget::CreateCompositor(int aWidth, int aHeight)
{
...
void
nsBaseWidget::NotifyCompositorSessionLost(CompositorSession* aSession)
{
...
LayerManager*
nsBaseWidget::GetLayerManager(PLayerTransactionChild* aShadowManager,
...
nsBaseWidget::ResizeClient(double aWidth, double aHeight, bool aRepaint)
{
  ...
  if (BoundsUseDesktopPixels()) {
    ...
    Resize(
        aWidth + desktopDelta.width, aHeight + desktopDelta.height, aRepaint);
  } else {
    Resize(mBounds.width + (aWidth - clientBounds.width),
           mBounds.height + (aHeight - clientBounds.height),
           aRepaint);
  }
}
  ...
  if (BoundsUseDesktopPixels()) {
    ...
    DesktopSize desktopDelta =
        (LayoutDeviceIntSize(mBounds.width, mBounds.height) -
         clientBounds.Size()) /
        scale;
    Resize(aX - desktopOffset.x,
           aY - desktopOffset.y,
           aWidth + desktopDelta.width,
           aHeight + desktopDelta.height,
           aRepaint);
  } else {
    Resize(aX - clientOffset.x,
           aY - clientOffset.y,
...
uint32_t
nsBaseWidget::GetMaxTouchPoints() const
{
  return 0;
}
                              ...
                              nsIFile** aResult)
{
  *aResult = nullptr;
  ...
  dirSvc->Get(NS_APP_CHROME_DIR_LIST,
...
void
nsBaseWidget::SetSizeConstraints(const SizeConstraints& aConstraints)
{
...
nsBaseWidget::StartAsyncScrollbarDrag(const AsyncDragMetrics& aDragMetrics)
{
  ...
  uint64_t layersId = mCompositorSession->RootLayerTreeId();
  ScrollableLayerGuid guid(
      layersId, aDragMetrics.mPresShellId, aDragMetrics.mViewId);
                                    ...
                                    nsIObserver* aObserver)
{
  ...
  sPointerIdCounter++;
  nsresult rv = SynthesizeNativeTouchPoint(
      pointerId, TOUCH_CONTACT, aPoint, 1.0, 90, nullptr);
  if (NS_FAILED(rv)) {
    return rv;
  }

  if (!aLongTap) {
    return SynthesizeNativeTouchPoint(
        pointerId, TOUCH_REMOVE, aPoint, 0, 0, nullptr);
  }
  ...
  if (!mLongTapTimer) {
    ...
    if (!mLongTapTimer) {
      SynthesizeNativeTouchPoint(
          pointerId, TOUCH_CANCEL, aPoint, 0, 0, nullptr);
  ...
  if (mLongTapTouchPoint) {
    SynthesizeNativeTouchPoint(mLongTapTouchPoint->mPointerId,
                               TOUCH_CANCEL,
                               mLongTapTouchPoint->mPosition,
                               0,
                               0,
                               nullptr);
  }
                                          ...
                                          uint32_t aPointerOrientation)
{
  ...
  if (aPointerState == TOUCH_CONTACT) {
    if (index >= 0) {
      ...
      aState->mTouches.AppendElement(SingleTouchData((int32_t)aPointerId,
                                                     pointerScreenPoint,
                                                     ScreenSize(0, 0),
...
// static
void
nsIWidget::UpdateRegisteredPluginWindowVisibility(
...
//////////////////////////////////////////////////////////////
static void
debug_SetCachedBoolPref(const char* aPrefName, bool aValue)
{
  NS_ASSERTION(nullptr != aPrefName, "cmon, pref name is null.");
...
NS_IMETHODIMP
Debug_PrefObserver::Observe(nsISupports* subject,
                            const char* topic,
                            const char16_t* data)
{
