...
void
nsWindow::NativePtr<Impl>::Attach(Instance aInstance,
                                  nsWindow* aWindow,
                                  Args&&... aArgs)
{
    ...
    : public NativePanZoomController::Natives<NPZCSupport>
{
 ...
 public:
  ...
  NPZCSupport(NativePtr<NPZCSupport>* aPtr,
              nsWindow* aWindow,
  ...
  void OnDetach()
  {
    ...
    auto callDestroy = [](const NPZCRef& npzc) { npzc->Destroy(); };
    ...
    uiThread->Dispatch(
        NewRunnableFunction(static_cast<void (*)(const NPZCRef&)>(callDestroy),
                            mozilla::Move(npzc)),
 ...
 public:
  ...
  bool HandleScrollEvent(int64_t aTime,
                         int32_t aMetaState,
                         float aX,
                         float aY,
                         float aHScroll,
                         float aVScroll)
  {
    ...
    ScrollWheelInput input(aTime,
                           GetEventTimeStamp(aTime),
                           GetModifiers(aMetaState),
 ...
 public:
  bool HandleMouseEvent(int32_t aAction,
                        int64_t aTime,
                        int32_t aMetaState,
                        float aX,
                        float aY,
                         ...
                         jni::FloatArray::Param aToolMinor)
  {
    ...
    for (size_t i = startIndex; i < endIndex; i++) {
      ...
      input.mTouches.AppendElement(
          SingleTouchData(pointerId[i],
                          ScreenIntPoint::FromUnknownPoint(point),
                          ScreenSize::FromUnknownSize(radius),
                          orien,
                          pressure[i]));
    }
    ...
    : public LayerView::Compositor::Natives<LayerViewSupport>
{
 ...
 public:
  ...
  static LayerViewSupport* FromNative(
      const LayerView::Compositor::LocalRef& instance)
  {
    return GetNative(instance);
  }

  LayerViewSupport(NativePtr<LayerViewSupport>* aPtr,
                   nsWindow* aWindow,
                   const LayerView::Compositor::LocalRef& aInstance)
      : mWindow(aPtr, aWindow), mCompositor(aInstance), mCompositorPaused(true)
  {
    MOZ_ASSERT(mWindow);
  }
  ...
  void OnDetach() { mCompositor->Destroy(); }

  const GeckoLayerClient::Ref& GetLayerClient() const { return mLayerClient; }
 ...
 public:
  void AttachToJava(jni::Object::Param aClient, jni::Object::Param aNPZC)
  {
    ...
    if (RefPtr<CompositorBridgeChild> bridge =
            mWindow->GetCompositorBridgeChild()) {
      bridge->SendForceIsFirstPaint();
    }
  }

  void OnSizeChanged(int32_t aWindowWidth,
  ...
  void CreateCompositor(int32_t aWidth,
                        int32_t aHeight,
     ...
     public:
      ...
      void Run() override
      {
        ...
        LayerViewSupport* const lvs =
            GetNative(LayerView::Compositor::LocalRef(env, mCompositor));
  ...
  void SyncInvalidateAndScheduleComposite()
  {
    ...
    if (!AndroidBridge::IsJavaUiThread()) {
      ...
      if (uiThread) {
        uiThread->Dispatch(
            NewRunnableMethod(
                "layers::UiCompositorControllerChild::InvalidateAndRender",
                child,
      ...
      if (uiThread) {
        uiThread->Dispatch(
            NewRunnableMethod<int32_t>(
                "layers::UiCompositorControllerChild::"
  ...
  void SendToolbarPixelsToCompositor(int32_t aWidth,
                                     int32_t aHeight,
 ...
 public:
                                     ...
                                     jni::Object::Param aSurface)
  {
    ...
    ANativeWindow* const window =
        ANativeWindow_fromSurface(aCls.Env(), aSurface.Get());
    if (!window) {
      return;
    }
...
nsWindow::GeckoViewSupport::LoadUri(jni::String::Param aUri, int32_t aFlags)
{
  ...
  if (NS_FAILED(browserWin->OpenURI(uri,
                                    nullptr,
                                    flags,
...
{
  ALOG("nsWindow[%p]::Create %p [%d %d %d %d]",
       (void*)this,
       (void*)aParent,
       aRect.x,
       aRect.y,
...
{
  ALOG("nsWindow[%p]::ConstrainPosition %d [%d %d]",
       (void*)this,
       aAllowSlop,
       *aX,
       *aY);
...
{
  ALOG("nsWindow[%p]::Resize [%f %f %f %f] (repaint %d)",
       (void*)this,
       aX,
       aY,
       aWidth,
...
nsWindow::GetLayerManager(PLayerTransactionChild*,
                          LayersBackend,
...
{
  ALOG("nsWindow: %p OnSizeChanged [%d %d]",
       (void*)this,
       aSize.width,
       aSize.height);
...
nsWindow::DispatchHitTest(const WidgetTouchEvent& aEvent)
{
  if (aEvent.mMessage == eTouchStart && aEvent.mTouches.Length() == 1) {
    ...
    WidgetMouseEvent hittest(
        true, eMouseHitTest, this, WidgetMouseEvent::eReal);
                                     ...
                                     nsIObserver* aObserver)
{
  ...
  client->SynthesizeNativeTouchPoint(aPointerId,
                                     eventType,
                                     aPoint.x,
                                     aPoint.y,
                                     aPointerPressure,
                                    ...
                                    nsIObserver* aObserver)
{
  ...
  MOZ_ASSERT(mLayerViewSupport);
  GeckoLayerClient::LocalRef client = mLayerViewSupport->GetLayerClient();
  client->SynthesizeNativeMouseEvent(
      sdk::MotionEvent::ACTION_HOVER_MOVE, aPoint.x, aPoint.y);
...
void
nsWindow::UpdateZoomConstraints(
...
void
nsWindow::UpdateRootFrameMetrics(const ScreenPoint& aScrollOffset,
                                 const CSSToScreenScale& aZoom)
{
  MOZ_ASSERT(AndroidBridge::IsJavaUiThread());
  if (NativePtr<LayerViewSupport>::Locked lvs{mLayerViewSupport}) {
    GeckoLayerClient::LocalRef client = lvs->GetLayerClient();
    client->UpdateRootFrameMetrics(
