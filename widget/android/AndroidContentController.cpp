...
void
AndroidContentController::UpdateOverscrollVelocity(const float aX,
                                                   const float aY,
...
void
AndroidContentController::UpdateOverscrollOffset(const float aX,
                                                 const float aY,
                                                 const bool aIsRootContent)
{
  if (aIsRootContent && mAndroidWindow) {
  ...
  if (NS_IsMainThread()) {
    ...
    if (aChange ==
      ...
      observerService->NotifyObservers(
          nullptr, "PanZoom:StateChange", u"NOTHING");
    } else if (aChange == layers::GeckoContentController::APZStateChange::
                              eTransformBegin) {
      observerService->NotifyObservers(
          nullptr, "PanZoom:StateChange", u"PANNING");
    }
  }
}
