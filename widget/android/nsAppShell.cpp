    ...
    : public java::GeckoAppShell::Natives<GeckoAppShellSupport>
{
 public:
                              ...
                              jni::String::Param aData)
  {
    ...
    obsServ->NotifyObservers(nullptr,
                             aTopic->ToCString().get(),
                             aData ? aData->ToString().get() : nullptr);
  }

  static void OnSensorChanged(int32_t aType,
                              float aX,
                              float aY,
                              float aZ,
                              float aW,
                              ...
                              int64_t aTime)
  {
    ...
    switch (aType) {
      ...
      default:
        __android_log_print(
            ANDROID_LOG_ERROR, "Gecko", "Unknown sensor type %d", aType);
    }

    hal::SensorData sdata(hal::SensorType(aType),
                          aTime,
                          values,
  ...
  static void OnLocationChanged(double aLatitude,
                                double aLongitude,
                                double aAltitude,
                                float aAccuracy,
                                float aBearing,
                                ...
                                int64_t aTime)
  {
    ...
    RefPtr<nsIDOMGeoPosition> geoPosition(new nsGeoPosition(aLatitude,
                                                            aLongitude,
                                                            aAltitude,
                                                            aAccuracy,
                                                            aAccuracy,
                                  ...
                                  jni::String::Param aCookie)
  {
    ...
    AndroidAlerts::NotifyListener(aName->ToString(),
                                  aTopic->ToCString().get(),
                    ...
                    const char16_t* aData)
{
  ...
  if (!strcmp(aTopic, "xpcom-shutdown")) {
    ...
    NS_CreateServicesFromCategory("browser-delayed-startup-finished",
                                  nullptr,
...
nsAppShell::ResolveURI(const nsCString& aUriStr)
{
  ...
  if (fixup && NS_SUCCEEDED(fixup->CreateFixupURI(
                   aUriStr, 0, nullptr, getter_AddRefs(uri)))) {
    return uri.forget();
  }
  return nullptr;
}
