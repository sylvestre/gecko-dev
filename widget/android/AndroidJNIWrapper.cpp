...
jsjni_GetGlobalClassRef(const char* className)
{
  ...
  jclass foundClass;
  nsCOMPtr<nsIRunnable> runnable_ref(
      new GetGlobalClassRefRunnable(className, &foundClass));
  RefPtr<mozilla::SyncRunnable> sr = new mozilla::SyncRunnable(runnable_ref);
  sr->DispatchToThread(mainThread);
  if (!foundClass) return nullptr;

  return foundClass;
}

__attribute__((visibility("default"))) jmethodID
...
jsjni_CallStaticVoidMethodA(jclass cls, jmethodID method, jvalue* values)
{
  JNIEnv* env = mozilla::jni::GetGeckoThreadEnv();
...
jsjni_CallStaticIntMethodA(jclass cls, jmethodID method, jvalue* values)
{
  JNIEnv* env = mozilla::jni::GetGeckoThreadEnv();
...
__attribute__((visibility("default"))) JavaVM*
jsjni_GetVM()
{
  JavaVM* jvm;
...
__attribute__((visibility("default"))) JNIEnv*
