...
class Telemetry final : public java::Telemetry::Natives<Telemetry>
{
  Telemetry() = delete;
 ...
 public:
  ...
  static void AddKeyedHistogram(jni::String::Param aName,
                                jni::String::Param aKey,
                                int32_t aValue)
  {
    MOZ_ASSERT(aName && aKey);
    mozilla::Telemetry::Accumulate(
        aName->ToCString().get(), aKey->ToCString(), aValue);
  }
  ...
  static void AddUIEvent(jni::String::Param aAction,
                         jni::String::Param aMethod,
                         int64_t aTimestamp,
                         jni::String::Param aExtras)
  {
    MOZ_ASSERT(aAction);
    nsCOMPtr<nsIUITelemetryObserver> obs = GetObserver();
    if (obs) {
