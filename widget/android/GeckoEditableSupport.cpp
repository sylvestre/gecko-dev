...
static KeyNameIndex
ConvertAndroidKeyCodeToKeyNameIndex(int32_t keyCode,
                                    int32_t action,
...
static void
InitKeyEvent(WidgetKeyboardEvent& aEvent,
             int32_t aAction,
             int32_t aKeyCode,
             int32_t aScanCode,
             int32_t aMetaState,
                                 ...
                                 const CSSToLayoutDeviceScale aScale)
{
  ...
  for (size_t i = 0; i < length; i++) {
    ...
    auto rect = sdk::RectF::New(tmp.x / aScale.scale,
                                tmp.y / aScale.scale,
...
void
GeckoEditableSupport::OnKeyEvent(int32_t aAction,
                                 int32_t aKeyCode,
                                 int32_t aScanCode,
                                 int32_t aMetaState,
...
GeckoEditableSupport::FlushIMEChanges(FlushChangesFlag aFlags)
{
  ...
  for (const TextRecord& record : textTransaction) {
    mEditable->OnTextChange(
        record.text, record.start, record.oldEnd, record.newEnd);
    if (flushOnException()) {
      return;
    }
  }
...
void
GeckoEditableSupport::OnImeReplaceText(int32_t aStart,
                                       int32_t aEnd,
...
void
GeckoEditableSupport::OnImeAddCompositionRange(int32_t aStart,
                                               int32_t aEnd,
                                               int32_t aRangeType,
                                               int32_t aRangeStyle,
                                               int32_t aRangeLineStyle,
                                               bool aRangeBoldLine,
...
void
GeckoEditableSupport::OnImeUpdateComposition(int32_t aStart,
                                             int32_t aEnd,
                                             int32_t aFlags)
{
  if (mIMEMaskEventsCount > 0) {
                                ...
                                const IMENotification& aNotification)
{
  ...
  switch (aNotification.mMessage) {
    case REQUEST_TO_COMMIT_COMPOSITION: {
      ...
      AsyncNotifyIME(GeckoEditableListener::NOTIFY_IME_TO_COMMIT_COMPOSITION);
      break;
    }
    ...
    case REQUEST_TO_CANCEL_COMPOSITION: {
      ...
      AsyncNotifyIME(GeckoEditableListener::NOTIFY_IME_TO_CANCEL_COMPOSITION);
      break;
    }
    ...
    case NOTIFY_IME_OF_FOCUS: {
      ...
      nsAppShell::PostEvent([this, self, dispatcher] {
        ...
        mEditable->NotifyIME(GeckoEditableListener::NOTIFY_IME_OF_TOKEN);
                                      ...
                                      const InputContextAction& aAction)
{
  ...
  if (mInputContext.mIMEState.mEnabled == IMEState::ENABLED && isUserAction &&
