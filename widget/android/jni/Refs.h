...
class Ref
{
  ...
  {
    CopyableCtx(JNIEnv* env, Type instance) : Context<Cls, Type>(env, instance)
    {
    }
 ...
 public:
  ...
  class AutoLock
  {
   ...
   public:
    AutoLock(AutoLock&& aOther) : mEnv(aOther.mEnv), mInstance(aOther.mInstance)
    {
...
class LocalRef : public Cls::Context
{
 ...
 public:
  ...
  LocalRef(JNIEnv* env, const Ref& ref) : Ctx(env, NewLocalRef(env, ref.Get()))
  {
  }
  ...
  template<class C>
  MOZ_IMPLICIT LocalRef(GenericLocalRef<C>&& ref) : Ctx(ref.mEnv, ref.mInstance)
  {
    ref.mInstance = nullptr;
  }
...
{
 public:
  explicit ByteBuffer(const Context& ctx) : ObjectBase<ByteBuffer, jobject>(ctx)
  {
  }
    ...
    : public ObjectBase<TypedObject<jobjectArray>, jobjectArray>
{
 ...
 public:
                            ...
                            typename Cls::Param initialElement = nullptr)
  {
    JNIEnv* const env = GetEnvForThread();
    jobjectArray array =
        env->NewObjectArray(jsize(length),
