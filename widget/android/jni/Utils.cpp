...
bool
EnsureJNIObject(JNIEnv* env, jobject instance)
{
  if (!sJNIObjectClass) {
    sJNIObjectClass =
...
SetNativeHandle(JNIEnv* env, jobject instance, uintptr_t handle)
{
  ...
  env->SetLongField(
      instance, sJNIObjectHandleField, static_cast<jlong>(handle));
}
...
GetClassRef(JNIEnv* aEnv, const char* aClassName)
{
  ...
  if ((!classRef || aEnv->ExceptionCheck()) && sClassLoader) {
    ...
    classRef = Class::LocalRef::Adopt(
        aEnv,
        jclass(aEnv->CallObjectMethod(sClassLoader,
