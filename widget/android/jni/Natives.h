...
template<class Impl>
inline uintptr_t
CheckNativeHandle(JNIEnv* env, uintptr_t handle)
{
  if (!handle) {
    if (!env->ExceptionCheck()) {
      ThrowException(
          env, "java/lang/NullPointerException", NullHandle<Impl>().str);
    }
    return 0;
  }
  return handle;
...
struct NativePtr<Impl, /* Type = */ NativePtrType::WEAK>
{
  static Impl* Get(JNIEnv* env, jobject instance)
  {
    ...
    if (!impl) {
      ThrowException(
          env, "java/lang/NullPointerException", NullWeakPtr<Impl>().str);
    }
    return impl;
  }
...
class ProxyNativeCall
{
  ...
  typename mozilla::EnableIf<Static && ThisArg, void>::Type Call(
      const Class::LocalRef& cls, mozilla::IndexSequence<Indices...>) const
  {
 ...
 public:
  ...
  void operator()()
  {
    ...
    Call<IsStatic, HasThisArg>(thisArg,
                               typename IndexSequenceFor<Args...>::Type());
...
struct Dispatcher
{
  template<class Traits,
           bool IsStatic = Traits::isStatic,
           typename... ProxyArgs>
  static typename EnableIf<Traits::dispatchTarget == DispatchTarget::PROXY,
                           void>::Type
  Run(ProxyArgs&&... args)
  {
    Impl::OnNativeCall(ProxyNativeCall<Impl,
                                       typename Traits::Owner,
                                       IsStatic,
                                       HasThisArg,
                                       Args...>(Forward<ProxyArgs>(args)...));
  }
  ...
  template<class Traits,
           ...
           typename ThisArg,
           typename... ProxyArgs>
  static typename EnableIf<Traits::dispatchTarget ==
                               DispatchTarget::GECKO_PRIORITY,
                           void>::Type
...
class NativeStub<Traits, Impl, jni::Args<Args...>>
{
  ...
  using ReturnJNIType =
      typename Conditional<isVoid, VoidType, TypeAdapter<ReturnType>>::Type::
          JNIType;
...
MakeNativeMethod(MOZ_JNICALL Ret (*stub)(JNIEnv*, Args...))
{
