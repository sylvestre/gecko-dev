...
//
void
nsPrimitiveHelpers ::CreatePrimitiveForData(const nsACString& aFlavor,
...
//
void
nsPrimitiveHelpers ::CreateDataFromPrimitive(const nsACString& aFlavor,
                                             nsISupports* aPrimitive,
                                             void** aDataBuff,
                                             uint32_t aDataLen)
{
...
//
nsresult
nsLinebreakHelpers ::ConvertPlatformToDOMLinebreaks(const nsACString& inFlavor,
                                                    void** ioData,
                                                    int32_t* ioLengthInBytes)
{
  ...
  if (inFlavor.EqualsLiteral(kTextMime) || inFlavor.EqualsLiteral(kRTFMime)) {
    ...
    retVal = nsLinebreakConverter::ConvertUnicharLineBreaksInSitu(
        ...
        nsLinebreakConverter::eLinebreakAny,
        nsLinebreakConverter::eLinebreakContent,
        *ioLengthInBytes / sizeof(char16_t),
        &newLengthInChars);
    if (NS_SUCCEEDED(retVal)) {
