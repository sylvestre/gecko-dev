...
static void
cmyk_convert_rgb(JSAMPROW row, JDIMENSION width);
...
nsJPEGDecoder::nsJPEGDecoder(RasterImage* aImage,
    ...
    : Decoder(aImage),
      mLexer(Transition::ToUnbuffered(
                 State::FINISHED_JPEG_DATA, State::JPEG_DATA, SIZE_MAX),
      ...
      mDecodeStyle(aDecodeStyle)
{
  ...
  MOZ_LOG(sJPEGDecoderAccountingLog,
          LogLevel::Debug,
  ...
  switch (mState) {
    case JPEG_HEADER: {
      ...
      if (jpeg_read_header(&mInfo, TRUE) == JPEG_SUSPENDED) {
        MOZ_LOG(
            sJPEGDecoderAccountingLog, LogLevel::Debug, ("} (JPEG_SUSPENDED)"));
      ...
      PostSize(
          mInfo.image_width, mInfo.image_height, ReadOrientationFromEXIF());
      if (HasError()) {
      ...
      nsresult rv = AllocateFrame(/* aFrameNum = */ 0,
                                  OutputSize(),
                                  FullOutputFrame(),
      ...
      if (mDownscaler) {
        nsresult rv = mDownscaler->BeginFrame(Size(),
                                              Nothing(),
                                              mImageData,
                                              /* aHasAlpha = */ false);
        if (NS_FAILED(rv)) {
  ...
  *suspend = false;
  ...
  while ((mInfo.output_scanline < mInfo.output_height)) {
    ...
    if (mTransform) {
      ...
      if (mInfo.out_color_space == JCS_CMYK) {
        memmove(
            sampleRow + mInfo.output_width, sampleRow, 3 * mInfo.output_width);
        sampleRow += mInfo.output_width;
      }
    } else {
      if (mInfo.out_color_space == JCS_CMYK) {
      ...
      if (mCMSMode == eCMSMode_All) {
        ...
        if (transform) {
          qcms_transform_data(
              transform, sampleRow, sampleRow, mInfo.output_width);
        }
      }
    }
