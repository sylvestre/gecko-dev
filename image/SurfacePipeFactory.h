...
class SurfacePipeFactory
{
 public:
   ...
   *         initialized.
   */
  static Maybe<SurfacePipe> CreateSurfacePipe(Decoder* aDecoder,
                                              uint32_t aFrameNum,
                                              ...
                                              SurfacePipeFlags aFlags)
  {
    ...
    SurfaceConfig surfaceConfig{
        aDecoder, aFrameNum, aOutputSize, aFormat, flipVertically};
    ...
    if (downscale) {
      if (removeFrameRect) {
        if (deinterlace) {
          pipe = MakePipe(deinterlacingConfig,
                          removeFrameRectConfig,
                          downscalingConfig,
                          surfaceConfig);
        } else if (adam7Interpolate) {
          pipe = MakePipe(interpolatingConfig,
                          removeFrameRectConfig,
                          downscalingConfig,
                          surfaceConfig);
      ...
      if (removeFrameRect) {
        if (deinterlace) {
          pipe = MakePipe(
              deinterlacingConfig, removeFrameRectConfig, surfaceConfig);
        } else if (adam7Interpolate) {
          pipe = MakePipe(
              interpolatingConfig, removeFrameRectConfig, surfaceConfig);
        } else {  // (deinterlace and adam7Interpolate are false)
      ...
      SurfacePipeFlags aFlags)
  {
    ...
    PalettedSurfaceConfig palettedSurfaceConfig{aDecoder,
                                                aFrameNum,
                                                aInputSize,
                                                aFrameRect,
                                                aFormat,
