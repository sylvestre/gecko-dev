...
WithSurfaceSink(Func aFunc)
{
  ...
  WithFilterPipeline(decoder,
                     Forward<Func>(aFunc),
                     SurfaceConfig{decoder,
                                   0,
                                   IntSize(100, 100),
                      ...
                      CheckFunc aCheckFunc)
{
  ...
  AssertCorrectPipelineFinalState(
      aSink, IntRect(0, 0, 100, 100), IntRect(0, 0, 100, 100));
                    ...
                    WriteFunc aWriteFunc)
{
  ...
  DoCheckIterativeWrite(
      aSink, writeFunc, [&] { CheckGeneratedImage(aDecoder, aOutputRect); });
}
...
{
  auto checkEarlyExit = [](Decoder* aDecoder,
                           SurfaceSink* aSink,
                           WriteState aState) {
...
TEST(ImageSurfaceSink, SurfaceSinkWritePixelsToRow)
{
  WithSurfaceSink<Orient::NORMAL>([](Decoder* aDecoder, SurfaceSink* aSink) {
    ...
    AssertCorrectPipelineFinalState(
        aSink, IntRect(0, 99, 100, 1), IntRect(0, 99, 100, 1));
...
TEST(ImageSurfaceSink, SurfaceSinkFlipVertically)
{
  WithSurfaceSink<Orient::FLIP_VERTICALLY>([](Decoder* aDecoder,
...

    {
      ...
      AssertCorrectPipelineFinalState(
          aSink, IntRect(0, 0, 100, 75), IntRect(0, 0, 100, 75));
...
TEST(ImageSurfaceSink, PalettedSurfaceSinkWritePixelsEarlyExit)
{
  ...
  WithPalettedSurfaceSink(IntRect(0, 0, 100, 100),
                          [&](Decoder* aDecoder, PalettedSurfaceSink* aSink) {
                            checkEarlyExit(
                                aDecoder, aSink, WriteState::NEED_MORE_DATA);
                          });

  WithPalettedSurfaceSink(IntRect(0, 0, 100, 100),
                          [&](Decoder* aDecoder, PalettedSurfaceSink* aSink) {
                            checkEarlyExit(
                                aDecoder, aSink, WriteState::FAILURE);
                          });

  WithPalettedSurfaceSink(IntRect(0, 0, 100, 100),
                          [&](Decoder* aDecoder, PalettedSurfaceSink* aSink) {
                            checkEarlyExit(
                                aDecoder, aSink, WriteState::FINISHED);
                          });
}
