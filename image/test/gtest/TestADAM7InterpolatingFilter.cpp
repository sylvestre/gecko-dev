...
InterpolationWeights(int32_t aStride)
{
  ...
  static vector<float> stride8Weights = {1.0f,
                                         7 / 8.0f,
                                         6 / 8.0f,
                                         5 / 8.0f,
                                         4 / 8.0f,
                                 ...
                                 const vector<BGRAColor>& aColors)
{
  ...
  if (IsImportantRow(aRow, aPass)) {
    ...
    generate(result.begin(), result.end(), [&] {
      return HorizontallyInterpolatedPixel(
          col++, aWidth, weights, aShouldInterpolate, aColors);
    });
  } else {
    // This is an unimportant row; just make the entire thing transparent.
    generate(
        result.begin(), result.end(), [] { return BGRAColor::Transparent(); });
  }
                          ...
                          const vector<BGRAColor>& aColors)
{
  ...
  for (int32_t row = 0; row < aSize.height; ++row) {
    ...
    vector<BGRAColor> pixels = Move(ADAM7HorizontallyInterpolatedRow(
                                   ...
                                   const vector<BGRAColor>& aColors)
{
  ...
  for (int32_t row = 0; row < aSize.height; ++row) {
    ...
    vector<BGRAColor> expectedPixels = Move(ADAM7HorizontallyInterpolatedRow(
  ...
  WithADAM7InterpolatingFilter(
      aSize, [&](Decoder* aDecoder, SurfaceFilter* aFilter) {
        ...
        for (uint8_t pass = 1; pass <= 8; ++pass) {
          ...
          EXPECT_TRUE(CheckHorizontallyInterpolatedImage(
              aDecoder, aSize, pass, aColors));
...
BGRAColor
ADAM7RowColor(int32_t aRow, uint8_t aPass, const vector<BGRAColor>& aColors)
{
  EXPECT_LT(0, aPass);
  EXPECT_GE(8, aPass);
  ...
  WithADAM7InterpolatingFilter(
      aSize, [&](Decoder* aDecoder, SurfaceFilter* aFilter) {
        for (uint8_t pass = 1; pass <= 8; ++pass) {
          ...
          WriteState result =
              WriteRowColorPixels(aFilter, aSize, pass, aColors);
