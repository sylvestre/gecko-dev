...
static void
CheckProgressConsistency(Progress aOldProgress,
...
ProgressTracker::Notify(IProgressObserver* aObserver)
{
  ...
  if (MOZ_LOG_TEST(gImgLog, LogLevel::Debug)) {
    ...
    if (image && image->GetURI()) {
      ...
      uri->GetSpec(spec);
      LOG_FUNC_WITH_PARAM(
          gImgLog, "ProgressTracker::Notify async", "uri", spec.get());
    } else {
      LOG_FUNC_WITH_PARAM(
          gImgLog, "ProgressTracker::Notify async", "uri", "<unknown>");
    }
  }
...
ProgressTracker::NotifyCurrentState(IProgressObserver* aObserver)
{
  ...
  if (MOZ_LOG_TEST(gImgLog, LogLevel::Debug)) {
    ...
    LOG_FUNC_WITH_PARAM(
        gImgLog, "ProgressTracker::NotifyCurrentState", "uri", spec.get());
  }
                                    ...
                                    /* = nsIntRect() */)
{
  ...
  if ((aProgress & FLAG_DECODE_COMPLETE) && (mProgress & FLAG_ONLOAD_BLOCKED) &&
...
ProgressTracker::SyncNotify(IProgressObserver* aObserver)
{
  ...
  LOG_SCOPE_WITH_PARAM(
      gImgLog, "ProgressTracker::SyncNotify", "uri", spec.get());
