...
NS_IMPL_ISUPPORTS(nsPNGEncoder,
                  imgIEncoder,
                  nsIInputStream,
                  nsIAsyncInputStream)
                               ...
                               const nsAString& aOutputOptions)
{
  ...
  nsresult rv = ParseOptions(aOutputOptions,
                             &useTransparency,
                             &skipFirstFrame,
                             &numFrames,
                             &numPlays,
  ...
  mPNG = png_create_write_struct(
      PNG_LIBPNG_VER_STRING, nullptr, ErrorCallback, WarningCallback);
  if (!mPNG) {
                            ...
                            const nsAString& aFrameOptions)
{
  ...
  // parse and check any provided output options
  nsresult rv = ParseOptions(aFrameOptions,
                             &useTransparency,
                             nullptr,
                             nullptr,
                             nullptr,
  ...
  if (aInputFormat == INPUT_FORMAT_HOSTARGB) {
    ...
    for (uint32_t y = 0; y < aHeight; y++) {
      ConvertHostARGBRow(
          &aData[y * aStride], row.get(), aWidth, useTransparency);
...
void
nsPNGEncoder::ConvertHostARGBRow(const uint8_t* aSrc,
                                 uint8_t* aDest,
...
void
nsPNGEncoder::StripAlpha(const uint8_t* aSrc,
                         uint8_t* aDest,
                         uint32_t aPixelWidth)
{
  for (uint32_t x = 0; x < aPixelWidth; x++) {
...
void
nsPNGEncoder::WarningCallback(png_structp png_ptr, png_const_charp warning_msg)
{
  MOZ_LOG(
      sPNGEncoderLog, LogLevel::Warning, ("libpng warning: %s\n", warning_msg));
}

// nsPNGEncoder::ErrorCallback

void
nsPNGEncoder::ErrorCallback(png_structp png_ptr, png_const_charp error_msg)
{
  ...
  if (mCallback &&
    ...
    if (mCallbackTarget) {
      callback = NS_NewInputStreamReadyEvent(
          "nsPNGEncoder::NotifyListener", mCallback, mCallbackTarget);
    } else {
      callback = mCallback;
    }
