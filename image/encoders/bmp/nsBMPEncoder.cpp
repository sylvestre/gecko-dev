...
NS_IMPL_ISUPPORTS(nsBMPEncoder,
                  imgIEncoder,
                  nsIInputStream,
                  nsIAsyncInputStream)
...
nsresult
nsBMPEncoder::ParseOptions(const nsAString& aOptions,
                           Version& aVersionOut,
                           uint16_t& aBppOut)
{
...
NS_IMETHODIMP
nsBMPEncoder::ReadSegments(nsWriteSegmentFun aWriter,
                           void* aClosure,
                           uint32_t aCount,
                           uint32_t* _retval)
{
  ...
  if (mCallback && (GetCurrentImageBufferOffset() - mImageBufferReadPoint >=
    ...
    if (mCallbackTarget) {
      callback = NS_NewInputStreamReadyEvent(
          "nsBMPEncoder::NotifyListener", mCallback, mCallbackTarget);
    } else {
      callback = mCallback;
    }
...
nsresult
nsBMPEncoder::InitFileHeader(Version aVersion,
                             uint16_t aBPP,
                             uint32_t aWidth,
                             uint32_t aHeight)
{
...
nsresult
nsBMPEncoder::InitInfoHeader(Version aVersion,
                             uint16_t aBPP,
                             uint32_t aWidth,
                             uint32_t aHeight)
{
...
static inline void
SetPixel24(uint8_t*& imageBufferCurr,
           uint8_t aRed,
           uint8_t aGreen,
           uint8_t aBlue)
{
...
static inline void
SetPixel32(uint8_t*& imageBufferCurr,
           uint8_t aRed,
           uint8_t aGreen,
           uint8_t aBlue,
           uint8_t aAlpha = 0xFF)
...
nsBMPEncoder::EncodeImageDataRow24(const uint8_t* aData)
{
  ...
  for (uint32_t x = 0;
       x < PaddingBytes(mBMPInfoHeader.bpp, mBMPInfoHeader.width);
       x++) {
    *mImageBufferCurr++ = 0;
  }
}
