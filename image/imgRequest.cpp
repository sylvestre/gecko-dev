...
imgRequest::~imgRequest()
{
  ...
  if (mURI) {
    ...
    mURI->GetSpec(spec);
    LOG_FUNC_WITH_PARAM(
        gImgLog, "imgRequest::~imgRequest()", "keyuri", spec.get());
  } else
    LOG_FUNC(gImgLog, "imgRequest::~imgRequest()");
}
...
imgRequest::RemoveProxy(imgRequestProxy* proxy, nsresult aStatus)
{
  ...
  if (progressTracker->ObserverCount() == 0) {
    ...
    if (mCacheEntry) {
      ...
      mURI->GetSpec(spec);
      LOG_MSG_WITH_PARAM(
          gImgLog, "imgRequest::RemoveProxy no cache entry", "uri", spec.get());
    }
    ...
    if (!(progressTracker->GetProgress() & FLAG_LAST_PART_COMPLETE) &&
        NS_FAILED(aStatus)) {
      LOG_MSG(
          gImgLog, "imgRequest::RemoveProxy", "load in progress.  canceling");
...
nsresult
imgRequest::GetURI(ImageURL** aURI)
{
  MOZ_ASSERT(aURI);
...
{
  LOG_FUNC_WITH_PARAM(
      gImgLog, "imgRequest::SetIsCacheable", "aInCache", aInCache);
  MutexAutoLock lock(mMutex);
  mIsInCache = aInCache;
}
...
PrepareForNewPart(nsIRequest* aRequest,
                  ...
                  uint32_t aInnerWindowId)
{
  ...
  if (aIsMultipart) {
    ...
    RefPtr<image::Image> partImage =
        image::ImageFactory::CreateImage(aRequest,
                                         progressTracker,
                                         result.mContentType,
                                         aURI,
                                         /* aIsMultipart = */ true,
    ...
    result.mImage = image::ImageFactory::CreateImage(aRequest,
                                                     aProgressTracker,
                                                     result.mContentType,
                                                     aURI,
                                                     /* aIsMultipart = */ false,
                            ...
                            uint32_t aCount)
{
  ...
  // appropriate image.
  if (newPartPending) {
    NewPartResult result = PrepareForNewPart(aRequest,
                                             aInStr,
                                             aCount,
                                             mURI,
                                             isMultipart,
...
imgRequest::OnRedirectVerifyCallback(nsresult result)
{
  ...
  if (NS_FAILED(mCurrentURI->SchemeIs("https", &isHttps)) ||
    ...
    bool upgradeInsecureRequests =
        loadInfo ? loadInfo->GetUpgradeInsecureRequests() : false;
