...
NS_IMPL_ISUPPORTS(nsEditingSession,
                  nsIEditingSession,
...
nsEditingSession::SetupEditorOnWindow(mozIDOMWindowProxy* aWindow)
{
  ...
  rv = htmlEditor->Init(
      domDoc, nullptr /* root content */, nullptr, mEditorFlags, EmptyString());
  NS_ENSURE_SUCCESS(rv, rv);
                                         ...
                                         nsIEditor* aEditor)
{
  ...
  if (mBaseCommandControllerId) {
    rv = SetContextOnControllerById(
        controllers, editorAsISupports, mBaseCommandControllerId);
    NS_ENSURE_SUCCESS(rv, rv);
  }

  if (mDocStateControllerId) {
    rv = SetContextOnControllerById(
        controllers, editorAsISupports, mDocStateControllerId);
    NS_ENSURE_SUCCESS(rv, rv);
  }

  if (mHTMLCommandControllerId) {
    rv = SetContextOnControllerById(
        controllers, editorAsISupports, mHTMLCommandControllerId);
  }
...
nsEditingSession::ReattachToWindow(mozIDOMWindowProxy* aWindow)
{
  ...
  // The third controller takes an nsIEditor as the context
  rv =
      SetupEditorCommandController("@mozilla.org/editor/htmleditorcontroller;1",
                                   aWindow,
