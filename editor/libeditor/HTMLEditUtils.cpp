...
HTMLEditUtils::IsNodeThatCanOutdent(nsIDOMNode* aNode)
{
  ...
  return (nodeAtom == nsGkAtoms::ul) || (nodeAtom == nsGkAtoms::ol) ||
         (nodeAtom == nsGkAtoms::dl) || (nodeAtom == nsGkAtoms::li) ||
         (nodeAtom == nsGkAtoms::dd) || (nodeAtom == nsGkAtoms::dt) ||
...
{
  return aNode.IsAnyOfHTMLElements(
      nsGkAtoms::td, nsGkAtoms::th, nsGkAtoms::caption);
}
...
HTMLEditUtils::IsNamedAnchor(nsINode* aNode)
{
  ...
  nsAutoString text;
  return aNode->AsElement()->GetAttr(
             kNameSpaceID_None, nsGkAtoms::name, text) &&
         !text.IsEmpty();
}
...
static const ElementInfo kElements[eHTMLTag_userdefined] = {
    ...
    ELEM(
        figure, true, true, GROUP_BLOCK, GROUP_FLOW_ELEMENT | GROUP_FIGCAPTION),
...
HTMLEditUtils::CanContain(int32_t aParent, int32_t aChild)
{
  ...
  if (aParent == eHTMLTag_button) {
    static const nsHTMLTag kButtonExcludeKids[] = {eHTMLTag_a,
                                                   eHTMLTag_fieldset,
                                                   eHTMLTag_form,
                                                   eHTMLTag_iframe,
                                                   eHTMLTag_input,
