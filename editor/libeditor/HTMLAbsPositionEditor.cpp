...
HTMLEditor::GetAbsolutelyPositionedSelectionContainer(nsINode** aContainer)
{
  ...
  while (!resultNode && node && !node->IsHTMLElement(nsGkAtoms::html)) {
    nsresult rv = mCSSEditUtils->GetComputedProperty(
        *node, *nsGkAtoms::position, positionStr);
    if (NS_WARN_IF(NS_FAILED(rv))) {
      *aContainer = nullptr;
      return rv;
    }
...
NS_IMETHODIMP
HTMLEditor::SetElementZIndex(nsIDOMElement* aElement, int32_t aZindex)
{
...
NS_IMETHODIMP
HTMLEditor::GetElementZIndex(nsIDOMElement* aElement, int32_t* aZindex)
{
  nsCOMPtr<Element> element = do_QueryInterface(aElement);
...
nsresult
HTMLEditor::GetElementZIndex(Element* aElement, int32_t* aZindex)
{
  ...
  *aZindex = 0;
  ...
  nsresult rv = mCSSEditUtils->GetSpecifiedProperty(
  ...
  if (zIndexStr.EqualsLiteral("auto")) {
    ...
    while (node && zIndexStr.EqualsLiteral("auto") &&
      ...
      rv = mCSSEditUtils->GetComputedProperty(
          *node, *nsGkAtoms::position, positionStr);
      NS_ENSURE_SUCCESS(rv, rv);
      if (positionStr.EqualsLiteral("absolute")) {
        ...
        rv = mCSSEditUtils->GetComputedProperty(
            *node, *nsGkAtoms::z_index, zIndexStr);
        NS_ENSURE_SUCCESS(rv, rv);
      }
      node = node->GetParentNode();
    }
...
HTMLEditor::CreateGrabber(nsIContent& aParentContent)
{
  ...
  evtTarget->AddEventListener(
      NS_LITERAL_STRING("mousedown"), mEventListener, false);
...
HTMLEditor::RefreshGrabber()
{
  ...
  SetAnonymousElementPosition(
      mPositionedObjectX + 12, mPositionedObjectY - 14, mGrabber);
  return NS_OK;
}
...
HTMLEditor::ShowGrabberOnElement(nsIDOMElement* aElement)
{
  ...
  rv = element->SetAttr(
      kNameSpaceID_None, nsGkAtoms::_moz_abspos, classValue, true);
  NS_ENSURE_SUCCESS(rv, rv);
...
HTMLEditor::StartMoving(nsIDOMElement* aHandle)
{
  ...
  mCSSEditUtils->SetCSSPropertyPixels(
      *mPositioningShadow, *nsGkAtoms::width, mPositionedObjectWidth);
  mCSSEditUtils->SetCSSPropertyPixels(
      *mPositioningShadow, *nsGkAtoms::height, mPositionedObjectHeight);
...
HTMLEditor::GrabberClicked()
{
  ...
  if (!mMouseMotionListenerP) {
    ...
    rv = piTarget->AddEventListener(
        NS_LITERAL_STRING("mousemove"), mMouseMotionListenerP, false, false);
...
nsresult
HTMLEditor::SetFinalPosition(int32_t aX, int32_t aY)
{
  ...
  mCSSEditUtils->SetCSSPropertyPixels(
      *absolutelyPositionedObject, *nsGkAtoms::top, newY);
  mCSSEditUtils->SetCSSPropertyPixels(
      *absolutelyPositionedObject, *nsGkAtoms::left, newX);
...
void
HTMLEditor::AddPositioningOffset(int32_t& aX, int32_t& aY)
{
...
NS_IMETHODIMP
HTMLEditor::AbsolutelyPositionElement(nsIDOMElement* aElement, bool aEnabled)
{
  nsCOMPtr<Element> element = do_QueryInterface(aElement);
  NS_ENSURE_ARG_POINTER(element);
  ...
  if (isPositioned == aEnabled) return NS_OK;
  ...
  if (aEnabled) {
    ...
    mCSSEditUtils->SetCSSProperty(
        *element, *nsGkAtoms::position, NS_LITERAL_STRING("absolute"));
    ...
    mCSSEditUtils->RemoveCSSProperty(
        *element, *nsGkAtoms::position, EmptyString());
    mCSSEditUtils->RemoveCSSProperty(*element, *nsGkAtoms::top, EmptyString());
    mCSSEditUtils->RemoveCSSProperty(*element, *nsGkAtoms::left, EmptyString());
    mCSSEditUtils->RemoveCSSProperty(
        *element, *nsGkAtoms::z_index, EmptyString());

    if (!HTMLEditUtils::IsImage(aElement)) {
      mCSSEditUtils->RemoveCSSProperty(
          *element, *nsGkAtoms::width, EmptyString());
      mCSSEditUtils->RemoveCSSProperty(
          *element, *nsGkAtoms::height, EmptyString());
    }
...
// self-explanatory
NS_IMETHODIMP
HTMLEditor::SetElementPosition(nsIDOMElement* aElement, int32_t aX, int32_t aY)
{
  nsCOMPtr<Element> element = do_QueryInterface(aElement);
  NS_ENSURE_STATE(element);
...
void
HTMLEditor::SetElementPosition(Element& aElement, int32_t aX, int32_t aY)
{
  AutoPlaceholderBatch batchIt(this);
                                          ...
                                          nsAString& aReturn)
{
  ...
  nsresult rv = mCSSEditUtils->GetComputedProperty(
      *element, *nsGkAtoms::background_image, bgImageStr);
  NS_ENSURE_SUCCESS(rv, rv);
  if (bgImageStr.EqualsLiteral("none")) {
    nsAutoString bgColorStr;
    rv = mCSSEditUtils->GetComputedProperty(
        *element, *nsGkAtoms::backgroundColor, bgColorStr);
    NS_ENSURE_SUCCESS(rv, rv);
    if (bgColorStr.EqualsLiteral("transparent")) {
      ...
      if (nsIDOMCSSPrimitiveValue::CSS_RGBCOLOR == val->PrimitiveType()) {
        ...
        NS_ENSURE_TRUE(!error.Failed(), error.StealNSResult());
        float r = rgbVal->Red()->GetFloatValue(
            nsIDOMCSSPrimitiveValue::CSS_NUMBER, error);
        NS_ENSURE_TRUE(!error.Failed(), error.StealNSResult());
        float g = rgbVal->Green()->GetFloatValue(
            nsIDOMCSSPrimitiveValue::CSS_NUMBER, error);
        NS_ENSURE_TRUE(!error.Failed(), error.StealNSResult());
        float b = rgbVal->Blue()->GetFloatValue(
            nsIDOMCSSPrimitiveValue::CSS_NUMBER, error);
        NS_ENSURE_TRUE(!error.Failed(), error.StealNSResult());
