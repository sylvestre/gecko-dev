...
class MOZ_STACK_CLASS AutoSelectionSetterAfterTableEdit final
{
 ...
 public:
  ...
  {
    if (mHTMLEditor) {
      mHTMLEditor->SetSelectionAfterTableEdit(
          mTable, mRow, mCol, mDirection, mSelected);
    }
  }
                       ...
                       nsIDOMElement** aNewCell)
{
  ...
  rv = CreateElementWithDefaults(
      aIsHeader ? NS_LITERAL_STRING("th") : NS_LITERAL_STRING("tb"),
...
NS_IMETHODIMP
HTMLEditor::SetColSpan(nsIDOMElement* aCell, int32_t aColSpan)
{
  NS_ENSURE_TRUE(aCell, NS_ERROR_NULL_POINTER);
...
NS_IMETHODIMP
HTMLEditor::SetRowSpan(nsIDOMElement* aCell, int32_t aRowSpan)
{
  NS_ENSURE_TRUE(aCell, NS_ERROR_NULL_POINTER);
  nsAutoString newSpan;
...
NS_IMETHODIMP
HTMLEditor::InsertTableCell(int32_t aNumber, bool aAfter)
{
  ...
  int32_t curStartRowIndex, curStartColIndex, rowSpan, colSpan, actualRowSpan,
      actualColSpan;
  bool isSelected;
  rv = GetCellDataAt(table,
                     startRowIndex,
                     startColIndex,
                     getter_AddRefs(curCell),
                     &curStartRowIndex,
  ...
  AutoSelectionSetterAfterTableEdit setCaret(
      *this, table, startRowIndex, newCellIndex, ePreviousColumn, false);
...
NS_IMETHODIMP
HTMLEditor::GetFirstRow(nsIDOMElement* aTableElement, nsIDOMNode** aRowNode)
{
  ...
  *aRowNode = nullptr;
  ...
  nsresult rv = GetElementOrParentByTagName(
      NS_LITERAL_STRING("table"), aTableElement, getter_AddRefs(tableElement));
  NS_ENSURE_SUCCESS(rv, rv);
...
NS_IMETHODIMP
HTMLEditor::GetNextRow(nsIDOMNode* aCurrentRowNode, nsIDOMNode** aRowNode)
{
  NS_ENSURE_TRUE(aRowNode, NS_ERROR_NULL_POINTER);
...
nsresult
HTMLEditor::GetLastCellInRow(nsIDOMNode* aRowNode, nsIDOMNode** aCellNode)
{
...
NS_IMETHODIMP
HTMLEditor::InsertTableColumn(int32_t aNumber, bool aAfter)
{
...
NS_IMETHODIMP
HTMLEditor::InsertTableRow(int32_t aNumber, bool aAfter)
{
  nsCOMPtr<nsIDOMElement> table;
  nsCOMPtr<nsIDOMElement> curCell;
  ...
  if (startRowIndex < rowCount) {
    ...
    int32_t colIndex = 0;
    while (NS_SUCCEEDED(GetCellDataAt(table,
                                      startRowIndex,
                                      colIndex,
                                      getter_AddRefs(curCell),
                                      &curStartRowIndex,
...
nsresult
HTMLEditor::DeleteTable2(nsIDOMElement* aTable, Selection* aSelection)
{
  ...
  if (firstCell && selection->RangeCount() > 1) {
    ...
    for (int32_t i = 0; i < aNumber; i++) {
      ...
      if (GetNumberOfCellsInRow(table, startRowIndex) == 1) {
        ...
        rv = GetElementOrParentByTagName(
            NS_LITERAL_STRING("tr"), cell, getter_AddRefs(parentRow));
        NS_ENSURE_SUCCESS(rv, rv);
        NS_ENSURE_TRUE(parentRow, NS_ERROR_NULL_POINTER);
...
NS_IMETHODIMP
HTMLEditor::DeleteColumn(nsIDOMElement* aTable, int32_t aColIndex)
{
  NS_ENSURE_TRUE(aTable, NS_ERROR_NULL_POINTER);
  ...
  do {
    nsresult rv = GetCellDataAt(aTable,
                                rowIndex,
                                aColIndex,
                                getter_AddRefs(cell),
                                &startRowIndex,
...
NS_IMETHODIMP
HTMLEditor::DeleteRow(nsIDOMElement* aTable, int32_t aRowIndex)
{
  NS_ENSURE_TRUE(aTable, NS_ERROR_NULL_POINTER);
  ...
  nsCOMPtr<nsIDOMElement> parentRow;
  rv = GetElementOrParentByTagName(
      NS_LITERAL_STRING("tr"), cellInDeleteRow, getter_AddRefs(parentRow));
  NS_ENSURE_SUCCESS(rv, rv);
...
HTMLEditor::SelectTable()
{
  nsCOMPtr<nsIDOMElement> table;
  nsresult rv = GetElementOrParentByTagName(
      NS_LITERAL_STRING("table"), nullptr, getter_AddRefs(table));
  NS_ENSURE_SUCCESS(rv, rv);
...
HTMLEditor::SelectTableCell()
{
  nsCOMPtr<nsIDOMElement> cell;
  nsresult rv = GetElementOrParentByTagName(
      NS_LITERAL_STRING("td"), nullptr, getter_AddRefs(cell));
  NS_ENSURE_SUCCESS(rv, rv);
  NS_ENSURE_TRUE(cell, NS_SUCCESS_EDITOR_ELEMENT_NOT_FOUND);
                               ...
                               nsIDOMElement* aEndCell)
{
  ...
  nsCOMPtr<nsIDOMElement> table;
  nsresult rv =
      GetElementOrParentByTagName(tableStr, aStartCell, getter_AddRefs(table));
  NS_ENSURE_SUCCESS(rv, rv);
  NS_ENSURE_TRUE(table, NS_ERROR_FAILURE);

  nsCOMPtr<nsIDOMElement> endTable;
  rv =
      GetElementOrParentByTagName(tableStr, aEndCell, getter_AddRefs(endTable));
  NS_ENSURE_SUCCESS(rv, rv);
  ...
  for (int32_t row = minRow; row <= maxRow; row++) {
    for (int32_t col = minColumn; col <= maxColumn;
      ...
      if (!isSelected && cell && row == currentRowIndex &&
          col == currentColIndex) {
...
HTMLEditor::SelectAllTableCells()
{
  ...
  nsCOMPtr<nsIDOMElement> table;
  rv = GetElementOrParentByTagName(
      NS_LITERAL_STRING("table"), cell, getter_AddRefs(table));
  NS_ENSURE_SUCCESS(rv, rv);
  if (!table) {
    return NS_ERROR_NULL_POINTER;
  }
  ...
  int32_t rowSpan, colSpan, actualRowSpan, actualColSpan, currentRowIndex,
      currentColIndex;
  bool isSelected;
...
HTMLEditor::SplitTableCell()
{
  ...
  rv = GetCellSpansAt(
      table, startRowIndex, startColIndex, actualRowSpan, actualColSpan);
  NS_ENSURE_SUCCESS(rv, rv);
  ...
  for (rowSpanBelow = actualRowSpan - 1; rowSpanBelow >= 0; rowSpanBelow--) {
    ...
    for (colSpanAfter = actualColSpan - 1; colSpanAfter > 0; colSpanAfter--) {
      rv = SplitCellIntoColumns(
          table, rowIndex, colIndex, 1, colSpanAfter, getter_AddRefs(newCell));
      NS_ENSURE_SUCCESS(rv, rv);
      CopyCellBackgroundColor(newCell, cell);
      colIndex++;
    }
                                 ...
                                 nsIDOMElement** aNewCell)
{
  ...
  nsCOMPtr<nsIDOMElement> newCell;
  rv = InsertCell(
      cell, actualRowSpan, aColSpanRight, true, false, getter_AddRefs(newCell));
  NS_ENSURE_SUCCESS(rv, rv);
  if (!newCell) {
    return NS_OK;
  }
                              ...
                              nsIDOMElement** aNewCell)
{
  ...
  int32_t startRowIndex2, startColIndex2, rowSpan2, colSpan2, actualRowSpan2,
      actualColSpan2;
  bool isSelected2;
  int32_t colIndex = 0;
                                      ...
                                      nsIDOMElement** aNewCell)
{
  ...
  nsCOMPtr<Element> newNode = ReplaceContainer(
      sourceCell, newCellType, nullptr, nullptr, EditorBase::eCloneAttributes);
...
HTMLEditor::JoinTableCells(bool aMergeNonContiguousContents)
{
  ...
  rv = GetFirstSelectedCellInTable(
      &firstRowIndex, &firstColIndex, getter_AddRefs(firstCell));
  NS_ENSURE_SUCCESS(rv, rv);
  ...
  if (joinSelectedCells) {
    ...
    rv = GetCellSpansAt(
        table, firstRowIndex, firstColIndex, firstRowSpan, firstColSpan);
    NS_ENSURE_SUCCESS(rv, rv);
      ...
      for (colIndex = 0; colIndex < colCount;
        ...
        if (isSelected2 && cell2 != firstCell) {
          if (rowIndex >= firstRowIndex && rowIndex <= lastRowIndex &&
            ...
            NS_ASSERTION(startRowIndex2 == rowIndex,
            ...
            if (actualColSpan2 > 1) {
              ...
              int32_t extraColSpan =
                  (startColIndex2 + actualColSpan2) - (lastColIndex + 1);
              if (extraColSpan > 0) {
    ...
    AutoRules beginRulesSniffing(
        this, EditAction::deleteNode, nsIEditor::eNext);
    ...
    // sanity check
    NS_ASSERTION((startRowIndex >= startRowIndex2),
                          ...
                          int32_t& aNewRowCount)
{
  ...
  for (colIndex = 0; colIndex < colCount;
    ...
    if (rowSpan > 0 && startRowIndex == aRowIndex &&
  ...
  if (minRowSpan > 1) {
    ...
    for (colIndex = 0; colIndex < colCount;
      ...
      if (cell && rowSpan > 0 && startRowIndex == aRowIndex &&
                          ...
                          int32_t& aNewColCount)
{
  ...
  for (rowIndex = 0; rowIndex < rowCount;
    ...
    if (colSpan > 0 && startColIndex == aColIndex &&
  ...
  if (minColSpan > 1) {
    ...
    for (rowIndex = 0; rowIndex < rowCount;
      ...
      if (cell && colSpan > 0 && startColIndex == aColIndex &&
...
HTMLEditor::NormalizeTable(nsIDOMElement* aTable)
{
  ...
  nsCOMPtr<nsIDOMElement> table;
  nsresult rv = GetElementOrParentByTagName(
      NS_LITERAL_STRING("table"), aTable, getter_AddRefs(table));
  NS_ENSURE_SUCCESS(rv, rv);
  // Don't fail if we didn't find a table
  NS_ENSURE_TRUE(table, NS_OK);
  ...
  for (rowIndex = 0; rowIndex < rowCount; rowIndex++) {
    ...
    for (colIndex = 0; colIndex < colCount; colIndex++) {
      ...
      //We are missing a cell at a cellmap location
        ...
        rv = InsertCell(
            previousCellInRow, 1, 1, true, false, getter_AddRefs(cell));
        NS_ENSURE_SUCCESS(rv, rv);
...
int32_t
HTMLEditor::GetNumberOfCellsInRow(nsIDOMElement* aTable, int32_t rowIndex)
{
  int32_t cellCount = 0;
  nsCOMPtr<nsIDOMElement> cell;
  int32_t colIndex = 0;
                           ...
                           int32_t* aColIndex)
{
  ...
  if (!cell) {
    ...
    nsresult rv = GetSelectedOrParentTableElement(
        tagName, &selectedCount, getter_AddRefs(cellOrTableElement));
    NS_ENSURE_SUCCESS(rv, rv);
...
nsresult
HTMLEditor::GetCellFromRange(nsRange* aRange, nsIDOMElement** aCell)
{
...
NS_IMETHODIMP
HTMLEditor::GetFirstSelectedCell(nsIDOMRange** aRange, nsIDOMElement** aCell)
{
  NS_ENSURE_TRUE(aCell, NS_ERROR_NULL_POINTER);
  *aCell = nullptr;
  if (aRange) {
...
NS_IMETHODIMP
HTMLEditor::GetNextSelectedCell(nsIDOMRange** aRange, nsIDOMElement** aCell)
{
  NS_ENSURE_TRUE(aCell, NS_ERROR_NULL_POINTER);
  *aCell = nullptr;
  if (aRange) {
                                            ...
                                            nsIDOMElement** aTableElement)
{
  ...
  *aSelectedCount = 0;
  ...
  nsresult rv =
      GetFirstSelectedCell(nullptr, getter_AddRefs(tableOrCellElement));
  NS_ENSURE_SUCCESS(rv, rv);
  ...
  if (tableOrCellElement) {
    ...
    *aSelectedCount = selection->RangeCount();
    ...
    if (!tableOrCellElement) {
      ...
      rv = GetElementOrParentByTagName(
          tdName, GetAsDOMNode(anchorNode), getter_AddRefs(tableOrCellElement));
      if (NS_FAILED(rv)) {
        return rv;
      }
      if (tableOrCellElement) {
                                 ...
                                 uint32_t* aSelectionType)
{
  ...
  *aSelectionType = 0;
  ...
  nsresult rv = GetElementOrParentByTagName(
      NS_LITERAL_STRING("table"), aElement, getter_AddRefs(table));
  NS_ENSURE_SUCCESS(rv, rv);
  ...
  while (NS_SUCCEEDED(rv) && selectedCell) {
    ...
    if (!indexArray.Contains(startColIndex)) {
      ...
      allCellsInRowAreSelected =
          AllCellsInRowSelected(table, startRowIndex, colCount);
  ...
  while (NS_SUCCEEDED(rv) && selectedCell) {
    ...
    if (!indexArray.Contains(startRowIndex)) {
      ...
      allCellsInColAreSelected =
          AllCellsInColumnSelected(table, startColIndex, rowCount);
                                  ...
                                  int32_t aNumberOfColumns)
{
  ...
  for (int32_t col = 0; col < aNumberOfColumns;
    ...
    NS_ASSERTION((actualColSpan > 0),
                 "ActualColSpan = 0 in AllCellsInRowSelected");
  }
  return true;
}
