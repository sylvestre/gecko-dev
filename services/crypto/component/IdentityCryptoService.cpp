...
class KeyPair : public nsIIdentityKeyPair, public nsNSSShutDownObject
{
 public:
  ...
  KeyPair(SECKEYPrivateKey* aPrivateKey,
...
class KeyGenRunnable : public Runnable, public nsNSSShutDownObject
{
 public:
  ...
  KeyGenRunnable(KeyType keyType,
                 nsIIdentityKeyGenCallback* aCallback,
                 nsIEventTarget* aOperationThread);
...
class SignRunnable : public Runnable, public nsNSSShutDownObject
{
 public:
  ...
  SignRunnable(const nsACString& textToSign,
...
KeyPair::KeyPair(SECKEYPrivateKey* privateKey,
                   ...
                   SECKEYPublicKey** publicKey)
{
  ...
  return GenerateKeyPair(
      slot, privateKey, publicKey, CKM_RSA_PKCS_KEY_PAIR_GEN, &rsaParams);
}
                   ...
                   SECKEYPublicKey** publicKey)
{
  ...
  return GenerateKeyPair(
      slot, privateKey, publicKey, CKM_DSA_KEY_PAIR_GEN, &pqgParams);
}
  ...
  if (!NS_IsMainThread()) {
    ...
    if (isAlreadyShutDown()) {
      ...
      if (sigLength <= 0) {
        ...
        SECItem hashItem = {
            siBuffer, hash, hashAlg == SEC_OID_SHA1 ? 20u : 32u};

        mRv = MapSECStatus(PK11_HashBuf(
            hashAlg,
            hash,
            const_cast<uint8_t*>(
...
const mozilla::Module::CIDEntry kCIDs[] = {{&kNS_IDENTITYCRYPTOSERVICE_CID,
                                            false,
                                            nullptr,
