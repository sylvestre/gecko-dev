...
class nsHostRecord : public PRCList, public nsHostKey
{
 ...
 public:
  ...
  void SetExpiration(const mozilla::TimeStamp& now,
                     unsigned int valid,
                     unsigned int grace);
 ...
 private:
  ...
  bool
      onQueue; /* true if pending and on the queue (not yet given to getaddrinfo())*/
  bool
      usingAnyThread; /* true if off queue and contributing to mActiveAnyThreadCount */
...
class nsHostResolver
{
 ...
 private:
  ...
  LookupStatus OnLookupComplete(nsHostRecord*,
                                nsresult,
