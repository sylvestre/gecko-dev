      ...
      mResolveAgain(false)
{
  ...
  memcpy((char*)netInterface, key->netInterface, strlen(key->netInterface) + 1);
  originSuffix = netInterface + strlen(key->netInterface) + 1;
  memcpy((char*)originSuffix, key->originSuffix, strlen(key->originSuffix) + 1);
  PR_INIT_CLIST(this);
...
nsHostRecord::Create(const nsHostKey* key, nsHostRecord** result)
{
  ...
  size_t size =
      hostLen + netInterfaceLen + originSuffixLen + sizeof(nsHostRecord);
...
void
nsHostRecord::SetExpiration(const mozilla::TimeStamp& now,
                            unsigned int valid,
                            unsigned int grace)
{
  mValidStart = now;
...
void
nsHostRecord::CopyExpirationTimesAndFlagsFrom(
...
bool
nsHostRecord::HasUsableResult(const mozilla::TimeStamp& now,
                                           ...
                                           MallocSizeOf mallocSizeOf)
{
  ...
  while (curr != head) {
    nsResolveHostCallback* callback = static_cast<nsResolveHostCallback*>(curr);
...
HostDB_HashKey(const void* key)
{
  ...
  return AddToHash(HashString(hk->host),
                   RES_KEY_FLAGS(hk->flags),
                   hk->af,
                   HashString(hk->netInterface),
                   HashString(hk->originSuffix));
}

static bool
HostDB_MatchEntry(const PLDHashEntryHdr* entry, const void* key)
{
...
static void
HostDB_InitEntry(PLDHashEntryHdr* entry, const void* key)
{
  nsHostDBEnt* he = static_cast<nsHostDBEnt*>(entry);
...
static void
DnsPrefChanged(const char* aPref, void* aClosure)
{
    ...
    if (mShutdown)
      ...
      // platforms like Win9x.  see bug 219376 for more details.)
        ...
        result = he->rec;
                 ...
                 !IsHighPriority(flags) && !he->rec->resolving) {
        LOG(
      ...
      // an AF_UNSPEC entry we can use. Otherwise, hit the resolver...
        ...
        if (!(flags & RES_BYPASS_CACHE) &&
          ...
          const nsHostKey unspecKey = {
              host, flags, PR_AF_UNSPEC, netInterface, originSuffix.get()};
          auto unspecHe = static_cast<nsHostDBEnt*>(mDB.Search(&unspecKey));
          NS_ASSERTION(!unspecHe || (unspecHe && unspecHe->rec),
                       "Valid host entries should contain a record");
          TimeStamp now = TimeStamp::NowLoRes();
          if (unspecHe && unspecHe->rec->HasUsableResult(now, flags)) {
            ...
            if (unspecHe->rec->negative) {
              ...
              while (addrIter) {
                if ((af == addrIter->mAddress.inet.family) &&
                  ...
                  if (!he->rec->addr_info) {
                    he->rec->addr_info =
                        new AddrInfo(unspecHe->rec->addr_info->mHostName,
                                     unspecHe->rec->addr_info->mCanonicalName);
                    he->rec->CopyExpirationTimesAndFlagsFrom(unspecHe->rec);
                  }
                  he->rec->addr_info->AddAddress(new NetAddrElement(*addrIter));
                }
                               ...
                               nsresult status)
{
  ...
  if (rec) callback->OnLookupComplete(this, rec, status);
}
...
nsHostResolver::GetHostToLookup(nsHostRecord** result)
{
  ...
  timeout = (mNumIdleThreads >= HighThreadThreshold) ? mShortIdleTimeout
                                                     : mLongIdleTimeout;
...
nsHostResolver::OnLookupComplete(nsHostRecord* rec,
                                 nsresult status,
                                 AddrInfo* newRRSet)
{
...
nsHostResolver::ThreadFunc(void* arg)
{
  ...
  while (rec || resolver->GetHostToLookup(&rec)) {
    ...
    nsresult status = GetAddrInfo(
        rec->host, rec->af, rec->flags, rec->netInterface, &ai, getTtl);
#if defined(RES_RETRY_ON_FAILURE)
