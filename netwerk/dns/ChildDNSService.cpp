                              ...
                              nsICancelable** result)
{
  ...
  return AsyncResolveExtendedNative(
      hostname, flags, EmptyCString(), listener, target_, attrs, result);
}
...
ChildDNSService::AsyncResolveNative(const nsACString& hostname,
                                    ...
                                    nsICancelable** result)
{
  return AsyncResolveExtendedNative(hostname,
                                    flags,
                                    EmptyCString(),
                                    listener,
                                    target_,
    ...
    const OriginAttributes& aOriginAttributes)
{
  ...
  nsCString key;
  GetDNSRecordHashKey(
      aHostname, aOriginAttributes, aFlags, aNetworkInterface, aListener, key);
  if (mPendingRequests.Get(key, &hashEntry)) {
    // We cancel just one.
...
ChildDNSService::NotifyRequestDone(DNSRequestChild* aDnsRequest)
{
  ...
  nsCString key;
  GetDNSRecordHashKey(aDnsRequest->mHost,
                      aDnsRequest->mOriginAttributes,
                      originalFlags,
                      aDnsRequest->mNetworkInterface,
                      originalListener,
                      key);
...
NS_IMETHODIMP
ChildDNSService::Observe(nsISupports* subject,
                         const char* topic,
                         const char16_t* data)
{
