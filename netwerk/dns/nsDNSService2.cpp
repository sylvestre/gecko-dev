...
class nsDNSRecord : public nsIDNSRecord
{
 public:
  ...
  explicit nsDNSRecord(nsHostRecord* hostRecord)
      : mHostRecord(hostRecord), mIter(nullptr), mIterGenCnt(-1), mDone(false)
  {
  }
...
nsDNSAsyncRequest::Cancel(nsresult reason)
{
  ...
  mResolver->DetachCallback(mHost.get(),
                            mOriginAttributes,
                            mFlags,
                            mAF,
                            mNetworkInterface.get(),
                            this,
                            reason);
  return NS_OK;
}
...
NS_IMPL_ISUPPORTS(
    nsDNSService, nsIDNSService, nsPIDNSService, nsIObserver, nsIMemoryReporter)
                           ...
                           nsICancelable** result)
{
  ...
  return AsyncResolveExtendedNative(
      aHostname, flags, EmptyCString(), listener, target_, attrs, result);
}
...
nsDNSService::AsyncResolveNative(const nsACString& aHostname,
                                 ...
                                 nsICancelable** result)
{
  return AsyncResolveExtendedNative(aHostname,
                                    flags,
                                    EmptyCString(),
                                    listener,
                                    target_,
                                   ...
                                   nsICancelable** result)
{
  ...
  return AsyncResolveExtendedNative(
      aHostname, flags, aNetworkInterface, listener, target_, attrs, result);
}
    ...
    nsICancelable** result)
{
  ...
  auto* req = new nsDNSAsyncRequest(
  ...
  NS_ADDREF(req);
  rv = res->ResolveHost(req->mHost.get(),
                        req->mOriginAttributes,
                        flags,
                        af,
                                 ...
                                 uint8_t aArgc)
{
  ...
  return CancelAsyncResolveExtendedNative(
      aHostname, aFlags, EmptyCString(), aListener, aReason, attrs);
}
                                         ...
                                         uint8_t aArgc)
{
  ...
  return CancelAsyncResolveExtendedNative(
      aHostname, aFlags, aNetworkInterface, aListener, aReason, attrs);
}
    ...
    const OriginAttributes& aOriginAttributes)
{
  ...
  res->CancelAsyncRequest(hostname.get(),
                          aOriginAttributes,
                          aFlags,
                          af,
                              ...
                              nsIDNSRecord** result)
{
  ...
  rv = res->ResolveHost(
      hostname.get(), aOriginAttributes, flags, af, "", &syncReq);
  if (NS_SUCCEEDED(rv)) {
    // wait for result
    while (!syncReq.mDone) PR_Wait(mon, PR_INTERVAL_NO_TIMEOUT);
...
nsDNSService::CollectReports(nsIHandleReportCallback* aHandleReport,
                             ...
                             bool aAnonymize)
{
  MOZ_COLLECT_REPORT("explicit/network/dns-service",
                     KIND_HEAP,
                     UNITS_BYTES,
