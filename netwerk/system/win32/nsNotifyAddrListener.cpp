...
static bool
macAddr(BYTE addr[], DWORD len, char* buf, size_t buflen)
{
  buf[0] = '\0';
...
bool
nsNotifyAddrListener::findMac(char* gateway)
{
...
static bool
defaultgw(char* aGateway, size_t aGatewayLen)
{
...
//
void
nsNotifyAddrListener::calculateNetworkId(void)
{
  bool found = FALSE;
  char gateway[128];
...
static void WINAPI
OnInterfaceChange(PVOID callerContext,
...
nsNotifyAddrListener::Init(void)
{
  ...
  Preferences::AddBoolVarCache(
      &mAllowChangedEvent, NETWORK_NOTIFY_CHANGED_PREF, true);
...
nsNotifyAddrListener::ChangeEvent::Run()
{
  nsCOMPtr<nsIObserverService> observerService =
      mozilla::services::GetObserverService();
  if (observerService)
    observerService->NotifyObservers(mService,
