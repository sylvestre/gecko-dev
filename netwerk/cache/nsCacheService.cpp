...
static const char* observerList[] = {"profile-before-change",
                                     "profile-do-change",
...
static const char* prefList[] = {DISK_CACHE_ENABLE_PREF,
...
{
 public:
  nsBlockOnCacheThreadEvent() : mozilla::Runnable("nsBlockOnCacheThreadEvent")
  {
  }
  NS_IMETHOD Run() override
  {
 ...
 */
bool
nsCacheProfilePrefObserver::PermittedToSmartSize(nsIPrefBranch* branch,
                                                 bool firstRun)
{
  nsresult rv;
  if (firstRun) {
  ...
  if (!mOfflineCacheParentDirectory) {
    ...
    if (NS_FAILED(rv)) {
      ...
      if (!directory)
        ...
        nsCacheService::MoveOrRemoveDiskCache(
            profDir, directory, "OfflineCache");
      }
    }
#if DEBUG
    if (!directory) {
...
NS_IMPL_ISUPPORTS(nsCacheService,
                  nsICacheService,
...
EvictionNotifierRunnable::Run()
{
  ...
  if (obsSvc) {
    obsSvc->NotifyObservers(
        mSubject, NS_CACHESERVICE_EMPTYCACHE_TOPIC_ID, nullptr);
  }
  return NS_OK;
}
...
bool
nsCacheService::IsStorageEnabledForPolicy_Locked(
    nsCacheStoragePolicy storagePolicy)
{
...
NS_IMETHODIMP
nsCacheService::VisitEntries(nsICacheVisitor* visitor)
{
  return NS_ERROR_NOT_IMPLEMENTED;
}

nsresult
nsCacheService::VisitEntriesInternal(nsICacheVisitor* visitor)
{
...
nsCacheService::FireClearNetworkCacheStoredAnywhereNotification()
{
  ...
  if (obsvc) {
    obsvc->NotifyObservers(
        nullptr, "network-clear-cache-stored-anywhere", nullptr);
  }
}
...
NS_IMETHODIMP
nsCacheService::GetCacheIOTarget(nsIEventTarget** aCacheIOTarget)
{
...
nsCacheService::CreateOfflineDevice()
{
  ...
  nsresult rv =
      CreateCustomOfflineDevice(mObserver->OfflineCacheParentDirectory(),
                               ...
                               nsICacheEntryDescriptor** result)
{
  ...
  while (1) {  // Activate entry loop
    rv = ActivateEntry(
        request, &entry, &doomedEntry);  // get the entry for this request
  ...
  if (NS_SUCCEEDED(rv) && request->mProfileDir) {
    ...
    if (entry->StoragePolicy() != nsICache::STORE_OFFLINE) {
      ...
      rv = GetCustomOfflineDevice(
          request->mProfileDir, -1, getter_AddRefs(customCacheDevice));
                               ...
                               nsICacheEntryDescriptor** result)
{
  ...
  nsresult rv = gService->CreateRequest(
      session, key, accessRequested, blockingMode, listener, &request);
  if (NS_FAILED(rv)) return rv;
                              ...
                              nsCacheEntry** doomedEntry)
{
  ...
  if (!entry) {
    ...
    bool collision = false;
    entry = SearchCacheDevices(
        &(request->mKey), request->StoragePolicy(), &collision);
    CACHE_LOG_DEBUG(
  ...
  if (!entry) {
    ...
    entry = new nsCacheEntry(
        request->mKey, request->IsStreamBased(), request->StoragePolicy());
...
nsCacheEntry*
nsCacheService::SearchCacheDevices(nsCString* key,
...
nsCacheService::EnsureEntryHasDevice(nsCacheEntry* entry)
{
  ...
  if (entry->IsStreamData() && entry->IsAllowedOnDisk() && mEnableDiskDevice) {
    ...
    if (!mDiskDevice) {
      (void)
          CreateDiskDevice();  // ignore the error (check for mDiskDevice instead)
    }
  ...
  if (!device && mEnableMemoryDevice && entry->IsAllowedInMemory()) {
    if (!mMemoryDevice) {
      (void)
          CreateMemoryDevice();  // ignore the error (check for mMemoryDevice instead)
    }
    if (mMemoryDevice) {
  ...
  if (!device && entry->IsStreamData() && entry->IsAllowedOffline() &&
    ...
    if (!mOfflineDevice) {
      (void)
          CreateOfflineDevice();  // ignore the error (check for mOfflineDevice instead)
    }
...
nsresult
nsCacheService::GetFileForEntry(nsCacheEntry* entry, nsIFile** result)
{
...
void
nsCacheService::ReleaseObject_Locked(nsISupports* obj, nsIEventTarget* target)
{
  gService->mLock.AssertCurrentThreadOwns();
...
nsCacheService::ProcessPendingRequests(nsCacheEntry* entry)
{
  ...
  if (!entry->IsDoomed() && entry->IsInvalid()) {
    ...
    if (request ==
        &entry
             ->mRequestQ)  // no requests asked for ACCESS_READ_WRITE, back to top
      request = (nsCacheRequest*)PR_LIST_HEAD(&entry->mRequestQ);
...
// static
void
nsCacheService::GetCacheBaseDirectoty(nsIFile** result)
{
  *result = nullptr;
...
// static
void
nsCacheService::GetDiskCacheDirectory(nsIFile** result)
{
  nsCOMPtr<nsIFile> directory;
...
// static
void
nsCacheService::GetAppCacheDirectory(nsIFile** result)
{
  nsCOMPtr<nsIFile> directory;
  GetCacheBaseDirectoty(getter_AddRefs(directory));
  if (!directory) return;
...
{
  uint32_t hitPercentage = (uint32_t)(
      (((double)mCacheHits) / ((double)(mCacheHits + mCacheMisses))) * 100);
                                      ...
                                      const char* aCacheSubdir)
{
  ...
  if (NS_SUCCEEDED(aNewCacheSubdir->Exists(&exists)) && !exists) {
    ...
    if (NS_SUCCEEDED(rv) || NS_ERROR_FILE_ALREADY_EXISTS == rv) {
      ...
      if (NS_FAILED(rv)) return;
      if (rename(oldPath.get(), newPath.get()) == 0) return;
    }
  }
                               ...
                               bool aAnonymize)
{
  ...
  MOZ_COLLECT_REPORT("explicit/network/disk-cache",
                     KIND_HEAP,
                     UNITS_BYTES,
                     disk,
                     "Memory used by the network disk cache.");

  MOZ_COLLECT_REPORT("explicit/network/memory-cache",
                     KIND_HEAP,
                     UNITS_BYTES,
                     memory,
