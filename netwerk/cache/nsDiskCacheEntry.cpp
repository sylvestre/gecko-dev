...
nsDiskCacheEntry::CreateCacheEntry(nsCacheDevice* device)
{
  ...
  nsresult rv = nsCacheEntry::Create(
      Key(), nsICache::STREAM_BASED, nsICache::STORE_ON_DISK, device, &entry);
...
NS_IMETHODIMP
nsDiskCacheEntryInfo::GetFetchCount(int32_t* aFetchCount)
{
...
NS_IMETHODIMP
nsDiskCacheEntryInfo::GetLastFetched(uint32_t* aLastFetched)
{
...
NS_IMETHODIMP
nsDiskCacheEntryInfo::GetExpirationTime(uint32_t* aExpirationTime)
{
...
NS_IMETHODIMP
nsDiskCacheEntryInfo::IsStreamBased(bool* aStreamBased)
{
...
NS_IMETHODIMP
nsDiskCacheEntryInfo::GetDataSize(uint32_t* aDataSize)
{
