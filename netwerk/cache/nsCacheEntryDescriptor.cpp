      ...
      mClosingDescriptor(false)
{
  ...
  NS_ADDREF(
      nsCacheService::
          GlobalInstance());  // ensure it lives for the lifetime of the descriptor
}
...
NS_IMETHODIMP
nsCacheEntryDescriptor::GetPredictedDataSize(int64_t* result)
{
  NS_ENSURE_ARG_POINTER(result);
  nsCacheServiceAutoLock lock(
...
NS_IMETHODIMP
nsCacheEntryDescriptor::SetPredictedDataSize(int64_t predictedSize)
{
  nsCacheServiceAutoLock lock(
...
NS_IMETHODIMP
nsCacheEntryDescriptor::GetDataSize(uint32_t* result)
{
  NS_ENSURE_ARG_POINTER(result);
...
NS_IMETHODIMP
nsCacheEntryDescriptor::GetStorageDataSize(uint32_t* result)
{
  NS_ENSURE_ARG_POINTER(result);
  nsCacheServiceAutoLock lock(
...
nsresult
nsCacheEntryDescriptor::nsInputStreamWrapper::LazyInit()
{
  ...
  rv = nsCacheService::OpenInputStreamForEntry(
      cacheEntry, mode, mStartOffset, getter_AddRefs(mInput));
...
nsresult
nsCacheEntryDescriptor::nsInputStreamWrapper::EnsureInit()
{
  if (mInitialized) {
...
void
nsCacheEntryDescriptor::nsInputStreamWrapper::CloseInternal()
{
...
nsresult
nsCacheEntryDescriptor::nsInputStreamWrapper::Close()
{
  mozilla::MutexAutoLock lock(mLock);
...
nsresult
nsCacheEntryDescriptor::nsInputStreamWrapper::Close_Locked()
{
...
nsresult
nsCacheEntryDescriptor::nsInputStreamWrapper::Available(uint64_t* avail)
{
  mozilla::MutexAutoLock lock(mLock);
...
nsresult
nsCacheEntryDescriptor::nsInputStreamWrapper::Read(char* buf,
                                                   uint32_t count,
                                                   uint32_t* countRead)
{
  mozilla::MutexAutoLock lock(mLock);
...
nsresult
nsCacheEntryDescriptor::nsInputStreamWrapper::Read_Locked(char* buf,
                                                          uint32_t count,
                                                          uint32_t* countRead)
{
  nsresult rv = EnsureInit();
...
nsresult
nsCacheEntryDescriptor::nsInputStreamWrapper::ReadSegments(
    nsWriteSegmentFun writer,
    void* closure,
    uint32_t count,
    uint32_t* countRead)
{
...
nsresult
nsCacheEntryDescriptor::nsInputStreamWrapper::IsNonBlocking(bool* result)
{
...
nsCacheEntryDescriptor::nsDecompressInputStreamWrapper::Release()
{
  ...
  count = --mRefCnt;
  NS_LOG_RELEASE(
      this, count, "nsCacheEntryDescriptor::nsDecompressInputStreamWrapper");
...
NS_IMETHODIMP
nsCacheEntryDescriptor::nsDecompressInputStreamWrapper::Read(
...
nsresult
nsCacheEntryDescriptor::nsDecompressInputStreamWrapper::Close()
{
  mozilla::MutexAutoLock lock(mLock);
...
nsresult
nsCacheEntryDescriptor::nsDecompressInputStreamWrapper::InitZstream()
{
  if (!mDescriptor) return NS_ERROR_NOT_AVAILABLE;
...
nsresult
nsCacheEntryDescriptor::nsDecompressInputStreamWrapper::EndZstream()
{
...
nsresult
nsCacheEntryDescriptor::nsOutputStreamWrapper::LazyInit()
{
  // Check if we have the descriptor. If not we can't even grab the cache
  // lock since it is not ensured that the cache service still exists.
  if (!mDescriptor) return NS_ERROR_NOT_AVAILABLE;
  ...
  rv = nsCacheService::OpenOutputStreamForEntry(
      cacheEntry, mode, mStartOffset, getter_AddRefs(stream));
  if (NS_FAILED(rv)) return rv;
...
nsresult
nsCacheEntryDescriptor::nsOutputStreamWrapper::EnsureInit()
{
  if (mInitialized) {
    NS_ASSERTION(mDescriptor, "Bad state");
    return NS_OK;
...
nsresult
nsCacheEntryDescriptor::nsOutputStreamWrapper::OnWrite(uint32_t count)
{
...
void
nsCacheEntryDescriptor::nsOutputStreamWrapper::CloseInternal()
{
  mLock.AssertCurrentThreadOwns();
  if (!mDescriptor) {
    NS_ASSERTION(!mInitialized, "Bad state");
...
nsresult
nsCacheEntryDescriptor::nsOutputStreamWrapper::Close_Locked()
{
  nsresult rv = EnsureInit();
  if (NS_SUCCEEDED(rv)) {
    rv = mOutput->Close();
...
NS_IMETHODIMP
nsCacheEntryDescriptor::nsOutputStreamWrapper::Flush()
{
  mozilla::MutexAutoLock lock(mLock);
...
NS_IMETHODIMP
nsCacheEntryDescriptor::nsOutputStreamWrapper::Write(const char* buf,
                                                     uint32_t count,
                                                     uint32_t* result)
{
  mozilla::MutexAutoLock lock(mLock);
...
nsresult
nsCacheEntryDescriptor::nsOutputStreamWrapper::Write_Locked(const char* buf,
                                                            uint32_t count,
                                                            uint32_t* result)
{
  nsresult rv = EnsureInit();
...
NS_IMETHODIMP
nsCacheEntryDescriptor::nsOutputStreamWrapper::WriteFrom(nsIInputStream* inStr,
                                                         uint32_t count,
                                                         uint32_t* result)
{
  return NS_ERROR_NOT_IMPLEMENTED;
...
NS_IMETHODIMP
nsCacheEntryDescriptor::nsOutputStreamWrapper::WriteSegments(
...
NS_IMETHODIMP
nsCacheEntryDescriptor::nsOutputStreamWrapper::IsNonBlocking(bool* result)
{
  // cache streams will never return NS_BASE_STREAM_WOULD_BLOCK
  *result = false;
  return NS_OK;
...
nsCacheEntryDescriptor::nsCompressOutputStreamWrapper::Release()
{
  ...
  count = --mRefCnt;
  NS_LOG_RELEASE(
      this, count, "nsCacheEntryDescriptor::nsCompressOutputStreamWrapper");
...
NS_IMETHODIMP
nsCacheEntryDescriptor::nsCompressOutputStreamWrapper::Write(const char* buf,
                                                             uint32_t count,
                                                             uint32_t* result)
{
  mozilla::MutexAutoLock lock(mLock);
...
NS_IMETHODIMP
nsCacheEntryDescriptor::nsCompressOutputStreamWrapper::Close()
{
  mozilla::MutexAutoLock lock(mLock);
...
nsresult
nsCacheEntryDescriptor::nsCompressOutputStreamWrapper::InitZstream()
{
  if (!mDescriptor) return NS_ERROR_NOT_AVAILABLE;
...
nsresult
nsCacheEntryDescriptor::nsCompressOutputStreamWrapper::WriteBuffer()
{
