...
nsDiskCacheMap::ReadDiskCacheEntry(nsDiskCacheRecord* record)
{
  ...
  if (metaFile == 0) {  // entry/metadata stored in separate file
    ...
    nsCOMPtr<nsIFile> file;
    rv = GetLocalFileForDiskCacheRecord(
        record, nsDiskCache::kMetaData, false, getter_AddRefs(file));
    ...
    rv = mBlockFile[metaFile - 1].ReadBlocks(
...
nsDiskCacheMap::WriteDiskCacheEntry(nsDiskCacheBinding* binding)
{
  ...
  if (fileIndex != 0) {
    while (1) {
      ...
      int32_t startBlock;
      rv = mBlockFile[fileIndex - 1].WriteBlocks(
          diskEntry, size, blocks, &startBlock);
      if (NS_SUCCEEDED(rv)) {
  ...
  if (fileIndex == 0) {
    ...
    PRFileDesc* fd;
    // open the file - restricted to user, the data could be confidential
    rv = localFile->OpenNSPRFileDesc(
        PR_RDWR | PR_TRUNCATE | PR_CREATE_FILE, 00600, &fd);
    NS_ENSURE_SUCCESS(rv, rv);
...
nsresult
nsDiskCacheMap::ReadDataCacheBlocks(nsDiskCacheBinding* binding,
                                    char* buffer,
                                    uint32_t size)
{
  ...
  nsresult rv =
      mBlockFile[fileIndex - 1].ReadBlocks(buffer,
...
nsresult
nsDiskCacheMap::WriteDataCacheBlocks(nsDiskCacheBinding* binding,
                                     char* buffer,
                                     uint32_t size)
{
  ...
  if (size > 0) {
    ...
    while (fileIndex) {
      ...
      rv = mBlockFile[fileIndex - 1].WriteBlocks(
          buffer, size, blockCount, &startBlock);
      if (NS_SUCCEEDED(rv)) {
...
nsDiskCacheMap::DeleteStorage(nsDiskCacheRecord* record, bool metaData)
{
  ...
  if (fileIndex == 0) {
    ...
    rv = GetFileForDiskCacheRecord(
        record, metaData, false, getter_AddRefs(file));
    if (NS_SUCCEEDED(rv)) {
                                          ...
                                          nsIFile** result)
{
  ...
  ::SprintfLiteral(
      name, "%05X%c%02X", hash & 0xFFFFF, (meta ? 'm' : 'd'), generation);
                               ...
                               nsDiskCache::CorruptCacheInfo* corruptInfo)
{
  ...
  rv = cacheCleanFile->OpenNSPRFileDesc(
      PR_RDWR | PR_CREATE_FILE, 00600, &mCleanFD);
  if (NS_FAILED(rv)) {
