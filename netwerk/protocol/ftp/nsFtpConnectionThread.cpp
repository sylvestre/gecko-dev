...
nsFtpState::S_user()
{
  ...
  if (mAnonymous) {
    ...
    if (mUsername.IsEmpty()) {
      ...
      bool retval;
      rv = prompter->PromptAuth(
          mChannel, nsIAuthPrompt2::LEVEL_NONE, info, &retval);
  ...
  if (mResponseCode / 100 == 2) {
    if ((mResponseMsg.Find("L8") > -1) || (mResponseMsg.Find("UNIX") > -1) ||
      ...
      rv = bundle->FormatStringFromName(
          "UnsupportedFTPServer", formatStrings, 1, formattedString);
      free(ucs2Response);
      if (NS_FAILED(rv)) return FTP_ERROR;
...
nsFtpState::R_pasv()
{
  ...
  if (mServerIsIPv6) {
    ...
    if (*ptr) {
      ++ptr;
      fields = PR_sscanf(
          ptr, "%ld,%ld,%ld,%ld,%ld,%ld", &h0, &h1, &h2, &h3, &p0, &p1);
    }
    if (!*ptr || fields < 6) {
  ...
  if (newDataConn) {
    ...
    rv = sts->CreateTransport(nullptr,
                              0,
                              host,
                              port,
                              mChannel->ProxyInfo(),
    ...
    if (mAction == PUT) {
      ...
      rv = mDataTransport->OpenOutputStream(
          nsITransport::OPEN_UNBUFFERED, 0, 0, getter_AddRefs(output));
      if (NS_FAILED(rv)) return FTP_ERROR;
...
nsFtpState::Init(nsFtpChannel* channel)
{
  ...
  if (pps && !mChannel->ProxyInfo()) {
    pps->AsyncResolve(static_cast<nsIChannel*>(mChannel),
                      0,
                      this,
                      nullptr,
...
NS_IMETHODIMP
nsFtpState::OnTransportStatus(nsITransport* transport,
                              nsresult status,
                              int64_t progress,
                              int64_t progressMax)
{
  ...
  mChannel->OnTransportStatus(
      nullptr, status, progress, mFileSize - mChannel->StartPos());
  return NS_OK;
}
...
NS_IMETHODIMP
nsFtpState::ReadSegments(nsWriteSegmentFun writer,
                         void* closure,
                         uint32_t count,
                         uint32_t* result)
{
...
static nsresult
CreateHTTPProxiedChannel(nsIChannel* channel,
                         nsIProxyInfo* pi,
...
NS_IMETHODIMP
nsFtpState::OnProxyAvailable(nsICancelable* request,
                             nsIChannel* channel,
                             nsIProxyInfo* pi,
                             nsresult status)
{
  ...
  if (NS_SUCCEEDED(status)) {
    ...
    if (pi && NS_SUCCEEDED(pi->GetType(type)) && type.EqualsLiteral("http")) {
      ...
      if (NS_SUCCEEDED(CreateHTTPProxiedChannel(
              channel, pi, getter_AddRefs(newChannel))) &&
