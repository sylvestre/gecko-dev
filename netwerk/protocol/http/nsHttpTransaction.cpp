...
void
nsHttpTransaction::ResumeReading()
{
...
bool
nsHttpTransaction::EligibleForThrottling() const
{
...
void
nsHttpTransaction::SetClassOfService(uint32_t cos)
{
                        ...
                        nsIAsyncInputStream** responseBody)
{
  ...
  rv = net_NewTransportEventSinkProxy(
      getter_AddRefs(mTransportSink), eventsink, target);
  ...
  rv = NS_NewByteInputStream(
      getter_AddRefs(headers), mReqHeaderBuf.get(), mReqHeaderBuf.Length());
  if (NS_FAILED(rv)) return rv;
                                    ...
                                    uint32_t* countWritten)
{
  ...
  LOG(("nsHttpTransaction::WritePipeSegment %p written=%u",
       trans,
       *countWritten));
...
bool
nsHttpTransaction::ShouldStopReading()
{
  if (mActivatedAsH2) {
  ...
  if (mContentRead < 16000) {
    ...
    LOG(("nsHttpTransaction::ShouldStopReading too few content (%" PRIi64
         ") this=%p",
         mContentRead,
         this));
    return false;
...
char*
nsHttpTransaction::LocateHttpStart(char* buf,
                                   uint32_t len,
                                   bool aAllowPartialMatch)
{
  ...
  while (len > 0) {
    if (PL_strncasecmp(
            buf, HTTPHeader, std::min<uint32_t>(len, HTTPHeaderLen)) == 0) {
...
nsresult
nsHttpTransaction::ParseHead(char* buf, uint32_t count, uint32_t* countRead)
{
  nsresult rv;
  uint32_t len;
  char* eol;
...
nsHttpTransaction::HandleContentStart()
{
  ...
  if (mResponseHead) {
    ...
    bool reset = false;
    nsresult rv = mConnection->OnHeadersAvailable(
        this, mRequestHead, mResponseHead, &reset);
                                 ...
                                 uint32_t* contentRemaining)
{
  ...
  *contentRemaining = 0;
  ...
  if (mChunkedDecoder) {
    ...
    rv = mChunkedDecoder->HandleChunkedContent(
        buf, count, contentRead, contentRemaining);
    if (NS_FAILED(rv)) return rv;
...
void
nsHttpTransaction::SetDomainLookupStart(mozilla::TimeStamp timeStamp,
                                        bool onlyIfNull)
{
  mozilla::MutexAutoLock lock(mLock);
...
void
nsHttpTransaction::SetDomainLookupEnd(mozilla::TimeStamp timeStamp,
                                      bool onlyIfNull)
{
  mozilla::MutexAutoLock lock(mLock);
...
void
nsHttpTransaction::SetConnectStart(mozilla::TimeStamp timeStamp,
                                   bool onlyIfNull)
{
  mozilla::MutexAutoLock lock(mLock);
...
void
nsHttpTransaction::SetRequestStart(mozilla::TimeStamp timeStamp,
                                   bool onlyIfNull)
{
  mozilla::MutexAutoLock lock(mLock);
...
void
nsHttpTransaction::SetResponseStart(mozilla::TimeStamp timeStamp,
                                    bool onlyIfNull)
{
  mozilla::MutexAutoLock lock(mLock);
