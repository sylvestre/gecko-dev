...
nsHttpConnection::~nsHttpConnection()
{
  ...
  if ((mFastOpenStatus != TFO_FAILED) && (mFastOpenStatus != TFO_HTTP) &&
      ((mFastOpenStatus != TFO_NOT_TRIED) ||
       ...
       (gHttpHandler->UseFastOpen() && gSocketTransportService &&
...
nsresult
nsHttpConnection::TryTakeSubTransactions(
...
nsHttpConnection::StartSpdy(uint8_t spdyVersion)
{
  ...
  if (spdyProxy) {
    ...
    gHttpHandler->ConnMgr()->MoveToWildCardConnEntry(
        mConnInfo, wildCardProxyCi, this);
                                    ...
                                    uint32_t& aOut0RTTBytesWritten)
{
  ...
  if (!m0RTTChecked) {
    ...
    mTransaction->OnTransportStatus(
        mSocketTransport, NS_NET_STATUS_TLS_HANDSHAKE_STARTING, 0);
  }
...
npnComplete:
  ...
  mTransaction->OnTransportStatus(
      mSocketTransport, NS_NET_STATUS_TLS_HANDSHAKE_ENDED, 0);
                                 ...
                                 int32_t priority)
{
  ...
  if (!mSpdySession->AddStream(
          httpTransaction, priority, needTunnel, mCallbacks)) {
...
{
  if (IdleTime() >= mIdleTimeout) return 0;
                                     ...
                                     bool* reset)
{
  ...
  if (responseStatus == 101) {
    ...
    if (!hasUpgradeReq || !hasUpgradeResp ||
        !nsHttp::FindToken(
            upgradeResp.get(), upgradeReq.get(), HTTP_HEADER_VALUE_SEPS)) {
                                ...
                                nsIAsyncOutputStream** aOutputStream)
{
  ...
  if (mTLSFilter) {
    ...
    mTLSFilter->newIODriver(
        ref1, ref2, getter_AddRefs(mSocketIn), getter_AddRefs(mSocketOut));
    mTLSFilter = nullptr;
  }
...
void
nsHttpConnection::CloseTransaction(nsAHttpTransaction* trans,
                                   nsresult reason,
                                   bool aIsShutdown)
{
...
nsHttpConnection::OnSocketWritable()
{
  ...
  do {
    ...
    if (mConnInfo->UsingHttpsProxy() &&
      ...
      rv = mProxyConnectStream->ReadSegments(ReadFromStream,
                                             this,
    ...
    if (NS_FAILED(rv)) {
      ...
      if (mWaitingFor0RTTResponse || mFastOpen) {
        ...
        rv = mSocketOut->AsyncWait(this, 0, 0, nullptr);
          ...
          mTransaction) {  // in case the ReadSegments stack called CloseTransaction()
        //
        ...
        //
        mTransaction->OnTransportStatus(
            mSocketTransport, NS_NET_STATUS_WAITING_FOR, 0);
...
nsHttpConnection::SetupSecondaryTLS()
{
  ...
  mTLSFilter = new TLSFilterTransaction(
      mTransaction, ci->Origin(), ci->OriginPort(), this, this);
                                    ...
                                    nsACString& result)
{
  ...
  rv = nsHttpHandler::GenerateHostPort(
      ...
      trans->ConnectionInfo()->OriginPort(),
      result);
  MOZ_ASSERT(NS_SUCCEEDED(rv));
...
nsHttpConnection::SetFastOpenStatus(uint8_t tfoStatus)
{
  ...
  if ((mFastOpenStatus >= TFO_FAILED_CONNECTION_REFUSED) && mSocketTransport) {
