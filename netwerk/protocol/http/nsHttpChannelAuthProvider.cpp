...
nsHttpChannelAuthProvider::AddAuthorizationHeaders(bool aDontUseCachedWWWCreds)
{
  ...
  if (proxyHost && UsingHttpProxy()) {
    SetAuthorizationHeader(authCache,
...
static void
ParseUserDomain(char16_t* buf, const char16_t** user, const char16_t** domain)
{
  char16_t* p = buf;
  while (*p && *p != '\\') ++p;
  if (!*p) return;
...
static void
GetAuthPrompt(nsIInterfaceRequestor* ifreq,
              bool proxyAuth,
                                       ...
                                       nsISupports* sessionState)
{
  ...
  rv = authCache->SetAuthEntry(scheme,
                               host,
                               port,
                               directory,
                               realm,
                                          ...
                                          nsCString& creds)
{
  ...
  for (const char* eol = challenges - 1; eol;) {
    ...
    if (NS_SUCCEEDED(rv)) {
      ...
      //
      rv = GetCredentialsForChallenge(
          challenge.get(), authType.get(), proxyAuth, auth, creds);
      if (NS_SUCCEEDED(rv)) {
        gotCreds = true;
    ...
    nsCString& creds)
{
  ...
  rv = GetAuthorizationMembers(
      proxyAuth, scheme, host, port, path, ident, continuationState);
  if (NS_FAILED(rv)) return rv;
  ...
  Unused << authCache->GetAuthEntryForDomain(
      scheme.get(), host, port, realm.get(), suffix, &entry);
  ...
  if (identityInvalid) {
    if (entry) {
      if (ident->Equals(entry->Identity())) {
        if (!identFromURI) {
          ...
          authCache->ClearAuthEntry(
              scheme.get(), host, port, realm.get(), suffix);
          entry = nullptr;
          ident->Clear();
        }
    ...
    if (!entry && ident->IsEmpty()) {
      ...
      rv = PromptForIdentity(
          level, proxyAuth, realm.get(), authType, authFlags, *ident);
      if (NS_FAILED(rv)) return rv;
      identFromURI = false;
    }
  }
...
nsHttpChannelAuthProvider::BlockPrompt()
{
  ...
  switch (sAuthAllowPref) {
    ...
    case SUBRESOURCE_AUTH_DIALOG_ALLOW_ALL:
      ...
      // blocked.
      if (!sImgCrossOriginAuthAllowPref && loadInfo &&
          ((loadInfo->GetExternalContentPolicyType() ==
                                              ...
                                              nsHttpAuthIdentity& ident)
{
  ...
  if (!userBuf.IsEmpty()) {
    SetIdent(
        ident, authFlags, (char16_t*)userBuf.get(), (char16_t*)passBuf.get());
  }
}

void
nsHttpChannelAuthProvider::ParseRealm(const char* challenge, nsACString& realm)
{
  //
...
{
 public:
  nsHTTPAuthInformation(uint32_t aFlags,
                                             ...
                                             nsHttpAuthIdentity& ident)
{
  ...
  rv = authPrompt->AsyncPromptAuth(channel,
                                   this,
                                   nullptr,
                                   level,
                                   holder,
...
NS_IMETHODIMP
nsHttpChannelAuthProvider::OnAuthAvailable(nsISupports* aContext,
                                           nsIAuthInformation* aAuthInfo)
{
  ...
  nsISupports** continuationState;
  rv = GetAuthorizationMembers(
      mProxyAuth, scheme, host, port, path, ident, continuationState);
  ...
  rv = ident->Set(
      holder->Domain().get(), holder->User().get(), holder->Password().get());
  MOZ_ASSERT(NS_SUCCEEDED(rv));
...
NS_IMETHODIMP
nsHttpChannelAuthProvider::OnAuthCancelled(nsISupports* aContext,
                                           bool userCancel)
{
                                            ...
                                            nsISupports* aContinuationState)
{
  ...
  rv = GetAuthorizationMembers(mProxyAuth,
                               scheme,
                               host,
                               port,
                               directory,
                                       ...
                                       bool doYesNoPrompt)
{
  ...
  if (userLength > MAX_DISPLAYED_USER_LENGTH) {
    ...
    ucsUser.Replace(desiredLength,
                    userLength - desiredLength,
  ...
  NS_QueryNotificationCallbacks(
      callbacks, loadGroup, NS_GET_IID(nsIPrompt), getter_AddRefs(prompt));
...
NS_IMPL_ISUPPORTS(nsHttpChannelAuthProvider,
                  nsICancelable,
