...
bool
IsInSubpathOfAppCacheManifest(nsIApplicationCache* cache,
...
nsresult
StoreAuthorizationMetaData(nsICacheEntry* entry,
...
nsHttpChannel::Init(nsIURI* uri,
                    ...
                    uint64_t channelId)
{
  nsresult rv = HttpBaseChannel::Init(
...
nsHttpChannel::TryHSTSPriming()
{
  ...
  if ((isHttpScheme || isHttpsScheme) && mLoadInfo) {
    ...
    bool requireHSTSPriming = mLoadInfo->GetForceHSTSPriming();

    if (requireHSTSPriming && nsMixedContentBlocker::sSendHSTSPriming) {
      if (!isHttpsScheme) {
      ...
      if (!mLoadInfo->GetIsHSTSPrimingUpgrade()) {
        ...
        Telemetry::Accumulate(
            Telemetry::MIXED_CONTENT_HSTS_PRIMING_RESULT,
...
static bool
isFileStream(nsIInputStream* stream)
{
  if (!stream) {
    return false;
  }
...
nsHttpChannel::SpeculativeConnect()
{
  ...
  NS_NewNotificationCallbacksAggregation(
      mCallbacks, mLoadGroup, getter_AddRefs(callbacks));
  if (!callbacks) return;
...
nsHttpChannel::SetupTransaction()
{
  ...
  if (mUpgradeProtocolCallback) {
    ...
    MOZ_ASSERT(NS_SUCCEEDED(rv));
    rv = mRequestHead.SetHeaderOnce(
        nsHttp::Connection, nsHttp::Upgrade.get(), true);
    MOZ_ASSERT(NS_SUCCEEDED(rv));
...
nsresult
EnsureMIMEOfScript(nsIURI* aURI,
                   nsHttpResponseHead* aResponseHead,
                   nsILoadInfo* aLoadInfo)
{
  if (!aURI || !aResponseHead || !aLoadInfo) {
...
nsHttpChannel::ProcessSecurityHeaders()
{
  ...
  rv = ProcessSingleSecurityHeader(
      nsISiteSecurityService::HEADER_HSTS, sslStatus, flags);
  NS_ENSURE_SUCCESS(rv, rv);

  rv = ProcessSingleSecurityHeader(
      nsISiteSecurityService::HEADER_HPKP, sslStatus, flags);
  NS_ENSURE_SUCCESS(rv, rv);
...
nsHttpChannel::ProcessContentSignatureHeader(nsHttpResponseHead* aResponseHead)
{
  ...
  rv = contentVerifyingMediator->Init(
      contentSignatureHeader, this, mListenerContext);
...
nsHttpChannel::ProcessAltService()
{
  ...
  AltSvcMapping::ProcessHeader(altSvc,
                               scheme,
                               originHost,
                               originPort,
                               mUsername,
...
nsHttpChannel::ProcessResponse()
{
  ...
  if (referrer) {
    ...
    mozilla::net::Predictor::UpdateCacheability(referrer,
                                                mURI,
                                                httpStatus,
                                                mRequestHead,
                                                mResponseHead,
    ...
    case 407:
      if (MOZ_UNLIKELY(mCustomAuthHeader) && httpStatus == 401) {
        ...
        rv = mAuthProvider->ProcessAuthentication(
            httpStatus,
            mConnectionInfo->EndToEndSSL() && mTransaction &&
...
nsHttpChannel::ProxyFailover()
{
  ...
  rv = pps->GetFailoverForProxy(
      mConnectionInfo->ProxyInfo(), mURI, mStatus, getter_AddRefs(pi));
  if (NS_FAILED(rv)) return rv;
...
nsHttpChannel::ResolveProxy()
{
  ...
  if (pps2) {
    rv = pps2->AsyncResolve2(
        this, mProxyResolveFlags, this, nullptr, getter_AddRefs(mProxyRequest));
  } else {
    rv = pps->AsyncResolve(static_cast<nsIChannel*>(this),
                           mProxyResolveFlags,
                           this,
                           nullptr,
...
bool
nsHttpChannel::IsResumable(int64_t partialLen,
...
nsresult
nsHttpChannel::MaybeSetupByteRangeRequest(int64_t partialLen,
                                          int64_t contentLength,
...
nsHttpChannel::OpenCacheEntry(bool isHttps)
{
  ...
  // byte range requests.
  if (IsSubRangeRequest(mRequestHead)) return NS_OK;
  ...
  if (!mPostID && mApplicationCache) {
    rv = cacheStorageService->AppCacheStorage(
        info, mApplicationCache, getter_AddRefs(cacheStorage));
  ...
  if (sRCWNEnabled && maybeRCWN && !mApplicationCacheForWrite) {
    ...
    rv = cacheStorage->GetCacheIndexEntryAttrs(
        openURI, extension, &hasAltData, &sizeInKb);
  ...
  if (!mCacheOpenDelay) {
    ...
    rv = cacheStorage->AsyncOpenURI(
        openURI, extension, cacheEntryOpenFlags, this);
  } else {
    // We pass `this` explicitly as a parameter due to the raw pointer
    // to refcounted object in lambda analysis.
    mCacheOpenFunc = [openURI, extension, cacheEntryOpenFlags, cacheStorage](
                         nsHttpChannel* self) -> void {
      MOZ_ASSERT(NS_IsMainThread(), "Should be called on the main thread");
...
bypassCacheEntryOpen:
  if (!mApplicationCacheForWrite) return NS_OK;
  ...
  rv = cacheStorageService->AppCacheStorage(
      info, mApplicationCacheForWrite, getter_AddRefs(cacheStorage));
  NS_ENSURE_SUCCESS(rv, rv);
...
nsresult
nsHttpChannel::CheckPartial(nsICacheEntry* aEntry,
                            int64_t* aSize,
                                 ...
                                 uint32_t* aResult)
{
  ...
  *aResult = ENTRY_WANTED;
  ...
  nsHttp::DetermineFramingAndImmutability(
      entry, mCachedResponseHead, isHttps, &weaklyFramed, &isImmutable);
  ...
  if (ResponseWouldVary(entry)) {
    ...
    doValidation = nsHttp::ValidationRequired(isForcedValid,
                                              mCachedResponseHead,
                                              mLoadFlags,
  ...
  if (!doValidation && isCachedRedirect) {
    ...
    LOG(("Redirection-chain %s key %s\n",
         doValidation ? "contains" : "does not contain",
         cacheKey.get()));
...
{
  AssembleCacheKey(
      mFallbackChannel ? mFallbackKey.get() : mSpec.get(), postID, cacheKey);
  return NS_OK;
}

// Assembles a cache-key from the given pieces of information and |mLoadFlags|
void
nsHttpChannel::AssembleCacheKey(const char* spec,
                                uint32_t postID,
                                nsACString& cacheKey)
{
  cacheKey.Truncate();
                       ...
                       uint32_t& aExpirationTime)
{
  ...
  if (!aResponseHead->MustValidate()) {
    ...
    if (freshnessLifetime > 0) {
      ...
      rv = aResponseHead->ComputeCurrentAge(
          now, aSelf->GetRequestTime(), &currentAge);
      if (NS_FAILED(rv)) return rv;
...
/*static*/ inline bool
nsHttpChannel::HasQueryString(nsHttpRequestHead::ParsedMethodType method,
                              nsIURI* uri)
{
...
nsresult
nsHttpChannel::OpenCacheInputStream(nsICacheEntry* cacheEntry,
                                    bool startBuffering,
...
nsHttpChannel::ReadFromCache(bool alreadyMarkedValid)
{
  ...
  rv = nsInputStreamPump::Create(
      getter_AddRefs(mCachePump), inputStream, 0, 0, true);
  if (NS_FAILED(rv)) {
    inputStream->Close();
    return rv;
  }
...
{
  return DoAddCacheEntryHeaders(
      this, entry, &mRequestHead, mResponseHead, mSecurityInfo);
}
...
nsHttpChannel::AsyncProcessRedirection(uint32_t redirectType)
{
  ...
  if (mRedirectCount >= mRedirectionLimit ||
      mInternalRedirectCount >= mRedirectionLimit) {
...
NS_IMETHODIMP
nsHttpChannel::OnAuthCancelled(bool userCancel)
{
...
NS_IMETHODIMP
nsHttpChannel::CloseStickyConnection()
{
...
NS_IMETHODIMP
nsHttpChannel::ConnectionRestartable(bool aRestartable)
{
...
nsHttpChannel::BeginConnect()
{
  ...
  if (!mConnectionInfo && mAllowAltSvc &&  // per channel
    ...
    mapping->GetConnectionInfo(
        getter_AddRefs(mConnectionInfo), proxyInfo, originAttributes);
    ...
    mConnectionInfo = new nsHttpConnectionInfo(host,
                                               port,
                                               EmptyCString(),
                                               mUsername,
                                               proxyInfo,
...
bool
nsHttpChannel::AttachStreamFilter(
...
NS_IMETHODIMP
nsHttpChannel::OnProxyAvailable(nsICancelable* request,
                                nsIChannel* channel,
                                nsIProxyInfo* pi,
                                nsresult status)
{
...
nsHttpChannel::SetWWWCredentials(const nsACString& value)
{
  ...
  return mRequestHead.SetHeader(nsHttp::Authorization,
                                value,
                                false,
                             ...
                             nsresult status)
{
  ...
  if (mTransaction) {
    ...
    if ((mAuthRetryPending || NS_FAILED(status)) &&
      ...
      LOG(("  transaction %p provides connection %p",
           mTransaction.get(),
           conn.get()));
...
class OnTransportStatusAsyncEvent : public Runnable
{
 public:
  ...
  NS_IMETHOD Run() override
  {
    ...
    if (mEventSink) {
      mEventSink->OnTransportStatus(
          nullptr, mTransportStatus, mProgress, mProgressMax);
    }
    return NS_OK;
  }
                               ...
                               uint32_t count)
{
  ...
  if (mListener) {
    ...
    nsresult rv = mListener->OnDataAvailable(
        this, mListenerContext, input, mLogicalOffset, count);
    if (NS_SUCCEEDED(rv)) {
...
NS_IMETHODIMP
nsHttpChannel::OnTransportStatus(nsITransport* trans,
                                 nsresult status,
                                 int64_t progress,
                                 int64_t progressMax)
{
  ...
  if (mProgressSink && NS_SUCCEEDED(mStatus) && mIsPending) {
    ...
    if (!(mLoadFlags & LOAD_BACKGROUND)) {
      nsAutoCString host;
      mURI->GetHost(host);
      mProgressSink->OnStatus(
          this, nullptr, status, NS_ConvertUTF8toUTF16(host).get());
    }
...
nsHttpChannel::ResumeAt(uint64_t aStartPos, const nsACString& aEntityID)
{
  LOG(("nsHttpChannel::ResumeAt [this=%p startPos=%" PRIu64 " id='%s']\n",
       this,
       aStartPos,
       PromiseFlatCString(aEntityID).get()));
  mEntityID = aEntityID;
  mStartPos = aStartPos;
  mResuming = true;
  return NS_OK;
...
nsHttpChannel::DoInvalidateCacheEntry(nsIURI* aURI)
{
  ...
  if (NS_SUCCEEDED(rv)) {
    RefPtr<LoadContextInfo> info = GetLoadContextInfo(this);
    rv = cacheStorageService->DiskCacheStorage(
        info, false, getter_AddRefs(cacheStorage));
  }
  ...
  LOG(("DoInvalidateCacheEntry [channel=%p key=%s rv=%d]",
       this,
       key.get(),
       int(rv)));
}
...
// static
bool
nsHttpChannel::IsRedirectStatus(uint32_t status)
{
...
nsHttpChannel::OnHSTSPrimingSucceeded(bool aCached)
{
  ...
  if (wouldBlock) {
    LOG(("HSTS Priming succeeded, blocking for mixed-content [this=%p]", this));
...
nsHttpChannel::OnHSTSPrimingFailed(nsresult aError, bool aCached)
{
  ...
  LOG(("HSTS Priming Failed [this=%p], %s the load",
       this,
  ...
  if (aError == NS_ERROR_HSTS_PRIMING_TIMEOUT) {
    ...
    Telemetry::Accumulate(Telemetry::MIXED_CONTENT_HSTS_PRIMING_RESULT,
                          (wouldBlock)
                              ? HSTSPrimingResult::eHSTS_PRIMING_FAILED_BLOCK
  ...
  OriginAttributes originAttributes;
  NS_GetOriginAttributes(this, originAttributes);
  nsresult rv = sss->CacheNegativeHSTSResult(
      mURI, nsMixedContentBlocker::sHSTSPrimingCacheTimeout, originAttributes);
  if (NS_FAILED(rv)) {
...
{
  LOG(("nsHttpChannel [this=%p] GetWarningReporter [%p]",
       this,
...
nsHttpChannel::RedirectToInterceptedChannel()
{
  ...
  nsresult rv = intercepted->Init(mURI,
                                  mCaps,
  ...
  rv = SetupReplacementChannel(
      mURI, intercepted, true, nsIChannelEventSink::REDIRECT_INTERNAL);
  NS_ENSURE_SUCCESS(rv, rv);
