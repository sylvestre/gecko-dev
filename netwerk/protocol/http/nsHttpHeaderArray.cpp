                                    ...
                                    bool response)
{
  ...
  if (!entry) {
    ...
    if (response) {
      rv = SetHeader_internal(
          header, headerNameOriginal, value, eVarietyResponseNetOriginal);
    }
    return rv;
  } else if (!IsIgnoreMultipleHeader(header)) {
...
nsresult
nsHttpHeaderArray::SetResponseHeaderFromCache(
    nsHttpAtom header,
    const nsACString& headerNameOriginal,
    const nsACString& value,
    nsHttpHeaderArray::HeaderVariety variety)
...
const char*
nsHttpHeaderArray::PeekHeaderAt(uint32_t index,
                                nsHttpAtom& header,
