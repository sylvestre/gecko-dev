                        ...
                        public nsIClassifiedChannel
{
 ...
 public:
  ...
  virtual MOZ_MUST_USE nsresult Init(nsIURI* aURI,
                                     uint32_t aCaps,
  ...
  NS_IMETHOD SetNetworkInterfaceId(
      const nsACString& aNetworkInterfaceId) override;
 ...
 protected:
  ...
  virtual MOZ_MUST_USE nsresult SetupReplacementChannel(nsIURI*,
                                                        nsIChannel*,
                                                        bool preserveMethod,
...
HttpAsyncAborter<T>::AsyncAbort(nsresult status)
{
  MOZ_LOG(gHttpLog,
          LogLevel::Debug,
...
template<class T>
inline void
HttpAsyncAborter<T>::HandleAsyncAbort()
{
...
template<class T>
nsresult
HttpAsyncAborter<T>::AsyncCall(void (T::*funcPtr)(),
...
class ProxyReleaseRunnable final : public mozilla::Runnable
{
 public:
  explicit ProxyReleaseRunnable(nsTArray<nsCOMPtr<nsISupports>>&& aDoomed)
      : Runnable("ProxyReleaseRunnable"), mDoomed(Move(aDoomed))
  {
  }
