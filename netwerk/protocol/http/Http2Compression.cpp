...
class HpackStaticTableReporter final : public nsIMemoryReporter
{
 public:
  ...
  {
    MOZ_COLLECT_REPORT("explicit/network/hpack/static-table",
                       KIND_HEAP,
                       UNITS_BYTES,
...
class HpackDynamicTableReporter final : public nsIMemoryReporter
{
 public:
  ...
  {
    if (mCompressor) {
      MOZ_COLLECT_REPORT("explicit/network/hpack/dynamic-tables",
                         KIND_HEAP,
                         UNITS_BYTES,
...
size_t
nvPair::SizeOfExcludingThis(MallocSizeOf aMallocSizeOf) const
{
...
size_t
nvPair::SizeOfIncludingThis(MallocSizeOf aMallocSizeOf) const
{
...
size_t
Http2BaseCompressor::SizeOfExcludingThis(
...
nsresult
Http2Decompressor::DecodeHeaderBlock(const uint8_t* data,
                                     uint32_t datalen,
                                     nsACString& output,
                                     bool isPush)
{
...
Http2Decompressor::OutputHeader(const nsACString& name, const nsACString& value)
{
  ...
  LOG(("Http2Decompressor::OutputHeader %s %s",
       name.BeginReading(),
       value.BeginReading()));
  ...
  if (name.EqualsLiteral("www-authenticate") ||
    ...
    if (HasConnectionBasedAuth(value)) {
      LOG3(("Http2Decompressor %p connection-based auth found in %s",
            this,
            name.BeginReading()));
...
nsresult
Http2Decompressor::DecodeFinalHuffmanCharacter(
                                          ...
                                          uint8_t& bitsLeft)
{
  ...
  if (table->IndexHasANextTable(idx)) {
    ...
    return DecodeHuffmanCharacter(
        table->NextTable(idx), c, bytesConsumed, bitsLeft);
  }
...
Http2Decompressor::CopyHuffmanStringFromInput(uint32_t bytes, nsACString& val)
{
  ...
  while (bytesRead < bytes) {
    ...
    rv = DecodeHuffmanCharacter(
        &HuffmanIncomingRoot, c, bytesConsumed, bitsLeft);
    if (NS_FAILED(rv)) {
...
nsresult
Http2Decompressor::DoLiteralInternal(nsACString& name,
                                     nsACString& value,
                               ...
                               bool neverIndex)
{
  ...
  LOG(("Http2Compressor::ProcessHeader %s %s",
       inputPair.mName.get(),
