...
nsresult
nsHttpResponseHead::SetHeader(nsHttpAtom hdr, const nsACString& val, bool merge)
{
  RecursiveMutexAutoLock monitor(mRecursiveMutex);
...
nsHttpResponseHead::SetHeader_locked(nsHttpAtom atom,
                                     ...
                                     bool merge)
{
  nsresult rv = mHeaders.SetHeader(
      atom, hdr, val, merge, nsHttpHeaderArray::eVarietyResponse);
...
{
  RecursiveMutexAutoLock monitor(mRecursiveMutex);
  if (mVersion == NS_HTTP_VERSION_0_9) return;
  ...
  do {
    ...
    *p = 0;
    if (NS_FAILED(nsHttpHeaderArray::ParseHeaderLine(
            nsDependentCString(block, p - block),
            &hdr,
            &headerNameOriginal,
            &val))) {
      return NS_OK;
...
nsresult
nsHttpResponseHead::ParseHeaderLine_locked(const nsACString& line,
...
nsHttpResponseHead::IsResumable()
{
  ...
  // non-2xx responses.
  return mStatus == 200 && mVersion >= NS_HTTP_VERSION_1_1 &&
