...
class nsHttpHeaderArray
{
 public:
  ...
  SetResponseHeaderFromCache(nsHttpAtom header,
                             const nsACString& headerNameOriginal,
                             const nsACString& value,
                             HeaderVariety variety);
  ...
  ParseHeaderLine(const nsACString& line,
  ...
  const char* PeekHeaderAt(uint32_t i,
                           nsHttpAtom& header,
 ...
 private:
  ...
  MOZ_MUST_USE nsresult MergeHeader(nsHttpAtom header,
                                    nsEntry* entry,
                                    const nsACString& value,
                                    HeaderVariety variety);
...
// consider the first.
inline bool
nsHttpHeaderArray::IsIgnoreMultipleHeader(nsHttpAtom header)
{
                               ...
                               nsHttpHeaderArray::HeaderVariety variety)
{
  ...
  if (entry->variety == eVarietyResponseNetOriginalAndResponse) {
    ...
    nsresult rv = SetHeader_internal(
        header, headerNameOriginal, newValue, eVarietyResponse);
    if (NS_FAILED(rv)) {
      return rv;
    }
  } else {
