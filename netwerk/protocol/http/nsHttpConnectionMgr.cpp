...
void
nsHttpConnectionMgr::InsertTransactionSorted(
...
nsresult
nsHttpConnectionMgr::RescheduleTransaction(nsHttpTransaction* trans,
                                           int32_t priority)
{
  LOG(("nsHttpConnectionMgr::RescheduleTransaction [trans=%p %d]\n",
       ...
       priority));
  return PostEvent(
      &nsHttpConnectionMgr::OnMsgReschedTransaction, priority, trans);
}

void
nsHttpConnectionMgr::UpdateClassOfServiceOnTransaction(nsHttpTransaction* trans,
                                                       uint32_t classOfService)
{
  ...
  Unused << PostEvent(
      ...
      static_cast<int32_t>(classOfService),
      trans);
}

nsresult
nsHttpConnectionMgr::CancelTransaction(nsHttpTransaction* trans,
                                       nsresult reason)
{
...
{
  return PostEvent(
      &nsHttpConnectionMgr::OnMsgDoShiftReloadConnectionCleanup, 0, aCI);
}
                                        ...
                                        NullHttpTransaction* nullTransaction)
{
  ...
  NS_NewInterfaceRequestorAggregation(
      callbacks, nullptr, getter_AddRefs(wrappedCallbacks));
...
nsHttpConnectionMgr::UpdateRequestTokenBucket(EventTokenBucket* aBucket)
{
  ...
  return PostEvent(
      &nsHttpConnectionMgr::OnMsgUpdateRequestTokenBucket, 0, aBucket);
}
...
static void
BuildOriginFrameHashKey(nsACString& newKey,
                                               ...
                                               bool justKidding)
{
  ...
  for (uint32_t i = 0; i < keyLen; ++i) {
    conn = FindCoalescableConnectionByHashKey(
        ent, ent->mCoalescingKeys[i], justKidding);
    if (conn) {
                                                ...
                                                nsConnectionEntry* ent)
{
  ...
  for (uint32_t i = 0; i < keyLen; ++i) {
    LOG((
        ...
        newConn,
        newConn->ConnectionInfo()->HashKey().get(),
        ent->mCoalescingKeys[i].get()));
  ...
  for (int32_t index = ent->mHalfOpenFastOpenBackups.Length() - 1; index >= 0;
    ...
    LOG(
        ("UpdateCoalescingForNewConn() shutting down connection in fast "
         ...
         "precedence\n",
         ent->mHalfOpenFastOpenBackups[index].get(),
         newConn));
...
bool
nsHttpConnectionMgr::AtActiveConnectionLimit(nsConnectionEntry* ent,
                                             uint32_t caps)
{
  nsHttpConnectionInfo* ci = ent->mConnInfo;
  uint32_t totalCount = TotalActiveConnections(ent);
                                       ...
                                       PendingTransactionInfo* pendingTransInfo)
{
  ...
  nsresult rv = CreateTransport(
      ent, trans, trans->Caps(), false, false, true, pendingTransInfo);
  if (NS_FAILED(rv)) {
    /* hard failure */
    LOG(
...
nsHttpConnectionMgr::ProcessNewTransaction(nsHttpTransaction* trans)
{
  ...
  if (pushedStream) {
    ...
    return pushedStream->Session()->AddStream(
               trans, trans->Priority(), false, nullptr)
               ? NS_OK
               : NS_ERROR_UNEXPECTED;
  }
  ...
  if (conn) {
    ...
    rv = TryDispatchTransaction(
        ent, !!trans->TunnelProvider(), pendingTransInfo);
  }
                                     ...
                                     PendingTransactionInfo* pendingTransInfo)
{
  ...
  RefPtr<nsHalfOpenSocket> sock =
      new nsHalfOpenSocket(ent, trans, caps, speculative, isFromPredictor);
...
void
nsHttpConnectionMgr::DispatchSpdyPendingQ(
    nsTArray<RefPtr<PendingTransactionInfo>>& pendingQ,
    nsConnectionEntry* ent,
    nsHttpConnection* conn)
{
...
static uint64_t
TabIdForQueuing(nsAHttpTransaction* transaction)
{
...
void
nsHttpConnectionMgr::OnMsgUpdateClassOfServiceOnTransaction(int32_t arg,
                                                            ARefBase* param)
{
  MOZ_ASSERT(OnSocketThread(), "not on socket thread");
  LOG(
...
nsHttpConnectionMgr::CancelTransactions(nsHttpConnectionInfo* ci, nsresult code)
{
  ...
  return PostEvent(
      &nsHttpConnectionMgr::OnMsgCancelTransactions, intReason, ci);
}
...
nsHttpConnectionMgr::OnMsgVerifyTraffic(int32_t, ARefBase*)
{
  ...
  if (mTrafficTimer) {
    ...
    mTrafficTimer->Init(
        this, gHttpHandler->NetworkChangedTimeout(), nsITimer::TYPE_ONE_SHOT);
  } else {
...
void
nsHttpConnectionMgr::OnMsgDoShiftReloadConnectionCleanup(int32_t,
                                                         ARefBase* param)
{
  ...
  if (!ent) {
    ...
    LOG(
        ("nsHttpConnectionMgr::OnMsgReclaimConnection conn %p "
         ...
         conn,
         conn->ConnectionInfo()->HashKey().get()));
  }
...
nsHttpConnectionMgr::OnMsgCompleteUpgrade(int32_t, ARefBase* param)
{
  ...
  if (NS_SUCCEEDED(rv)) {
    rv = data->mUpgradeListener->OnTransportAvailable(
        socketTransport, socketIn, socketOut);
    if (NS_FAILED(rv)) {
      LOG(
          ("nsHttpConnectionMgr::OnMsgCompleteUpgrade "
           "this=%p conn=%p listener=%p\n",
...
void
nsHttpConnectionMgr::SetThrottlingEnabled(bool aEnable)
{
...
bool
nsHttpConnectionMgr::InThrottlingTimeWindow()
{
  MOZ_ASSERT(OnSocketThread(), "not on socket thread");
...
void
nsHttpConnectionMgr::TouchThrottlingTimeWindow(bool aEnsureTicker)
{
...
void
nsHttpConnectionMgr::LogActiveTransactions(char operation)
{
  if (!LOG_ENABLED()) {
    return;
  }
...
nsHttpConnectionMgr::AddActiveTransaction(nsHttpTransaction* aTrans)
{
  ...
  LOG(("nsHttpConnectionMgr::AddActiveTransaction    t=%p tabid=%" PRIx64
       ...
       aTrans,
       tabId,
       tabId == mCurrentTopLevelOuterContentWindowId,
       throttled));
...
bool
nsHttpConnectionMgr::IsConnEntryUnderPressure(nsHttpConnectionInfo* connInfo)
{
...
bool
nsHttpConnectionMgr::IsThrottleTickerNeeded()
{
...
nsHttpConnectionMgr::OnMsgSpeculativeConnect(int32_t, ARefBase* param)
{
  ...
  if (mNumHalfOpenConns < parallelSpeculativeConnectLimit &&
    ...
    DebugOnly<nsresult> rv = CreateTransport(ent,
                                             args->mTrans,
                                             args->mTrans->Caps(),
                                             true,
                                             isFromPredictor,
    ...
    bool isBackup)
{
  ...
  if (routedSTS) {
    rv = routedSTS->CreateRoutedTransport(socketTypes,
                                          typeCount,
                                          ci->GetOrigin(),
                                          ci->OriginPort(),
                                          ci->GetRoutedHost(),
    ...
    rv = sts->CreateTransport(socketTypes,
                              typeCount,
                              ci->GetOrigin(),
                              ci->OriginPort(),
                              ci->ProxyInfo(),
  ...
  rv = socketTransport->OpenOutputStream(
      nsITransport::OPEN_UNBUFFERED, 0, 0, getter_AddRefs(sout));
  NS_ENSURE_SUCCESS(rv, rv);

  nsCOMPtr<nsIInputStream> sin;
  rv = socketTransport->OpenInputStream(
      nsITransport::OPEN_UNBUFFERED, 0, 0, getter_AddRefs(sin));
  NS_ENSURE_SUCCESS(rv, rv);
...
nsHttpConnectionMgr::nsHalfOpenSocket::SetupBackupTimer()
{
  ...
  if (mFastOpenInProgress || (timeout && !mSpeculative)) {
    ...
    NS_NewTimerWithCallback(
        getter_AddRefs(mSynTimer), this, timeout, nsITimer::TYPE_ONE_SHOT);
...
nsHttpConnectionMgr::nsHalfOpenSocket::FastOpenEnabled()
{
  ...
  if ((!info) &&
    ...
    LOG(
        ("nsHalfOpenSocket::FastOpenEnabled - It is a connection without "
  ...
  if ((info) && !mEnt->mConnInfo->FirstHopSSL()) {
    ...
    if (!info->mTransaction->CanDo0RTT()) {
      LOG(
          ("nsHalfOpenSocket::FastOpenEnabled - it is not safe to restart "
           "transaction.\n"));
      return false;
    }
  }
...
nsresult
nsHttpConnectionMgr::nsHalfOpenSocket::StartFastOpen()
{
  MOZ_ASSERT(mStreamOut);
  MOZ_ASSERT(!mBackupTransport);
  MOZ_ASSERT(mEnt);
...
void
nsHttpConnectionMgr::nsHalfOpenSocket::SetFastOpenConnected(nsresult aError,
                                                            bool aWillRetry)
{
...
void
nsHttpConnectionMgr::nsHalfOpenSocket::SetFastOpenStatus(uint8_t tfoStatus)
{
  MOZ_ASSERT(mFastOpenInProgress);
...
void
nsHttpConnectionMgr::nsHalfOpenSocket::CancelFastOpenConnection()
{
  MOZ_ASSERT(mFastOpenInProgress);
...
void
nsHttpConnectionMgr::nsHalfOpenSocket::FastOpenNotSupported()
{
  MOZ_ASSERT(mFastOpenInProgress);
...
nsresult
nsHttpConnectionMgr::nsHalfOpenSocket::SetupConn(nsIAsyncOutputStream* out,
                                                 bool aFastOpen)
{
  ...
  if (status == NS_NET_STATUS_CONNECTING_TO && gHttpHandler->IsSpdyEnabled() &&
    ...
    if (NS_SUCCEEDED(rv) && !addressSet.IsEmpty()) {
      for (uint32_t i = 0; i < addressSet.Length(); ++i) {
        ...
        NetAddrToString(
            &addressSet[i], newKey->BeginWriting(), kIPv6CStrBufSize);
...
void
nsHttpConnectionMgr::nsConnectionEntry::RecordIPFamilyPreference(
    uint16_t family)
{
...
void
nsHttpConnectionMgr::nsConnectionEntry::ResetIPFamilyPreference()
{
  mPreferIPv4 = false;
  mPreferIPv6 = false;
}
...
void
nsHttpConnectionMgr::nsConnectionEntry::AppendPendingQForFocusedWindow(
    uint64_t windowId,
    ...
    uint32_t maxCount)
{
  ...
  result.InsertElementsAt(
      result.Length(), infoArray->Elements(), countToAppend);
...
void
nsHttpConnectionMgr::nsConnectionEntry::AppendPendingQForNonFocusedWindows(
    uint64_t windowId,
    nsTArray<RefPtr<PendingTransactionInfo>>& result,
    uint32_t maxCount)
{
                                             ...
                                             nsHttpConnection* proxyConn)
{
  ...
  LOG(
      ("nsHttpConnectionMgr::MakeConnEntryWildCard wc-ent %p "
       "idle=%zu active=%zu half=%zu pending=%zu\n",
       wcEnt,
       wcEnt->mIdleConns.Length(),
       wcEnt->mActiveConns.Length(),
       wcEnt->mHalfOpens.Length(),
       wcEnt->PendingQLength()));
