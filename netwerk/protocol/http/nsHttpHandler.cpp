...
nsHttpHandler::Init()
{
  ...
  if (prefBranch) {
    ...
    prefBranch->AddObserver(
        HTTP_PREF("tcp_keepalive.short_lived_connections"), this, true);
    prefBranch->AddObserver(
        HTTP_PREF("tcp_keepalive.long_lived_connections"), this, true);
                                         ...
                                         bool isSecure)
{
  ...
  if (!mAcceptLanguages.IsEmpty()) {
    rv = request->SetHeader(nsHttp::Accept_Language,
                            mAcceptLanguages,
                            false,
                            nsHttpHeaderArray::eVarietyRequestOverride);
    if (NS_FAILED(rv)) return rv;
  ...
  if (isSecure) {
    rv = request->SetHeader(nsHttp::Accept_Encoding,
...
nsresult
nsHttpHandler::AddConnectionHeader(nsHttpRequestHead* request, uint32_t caps)
{
...
/* static */ nsresult
nsHttpHandler::GenerateHostPort(const nsCString& host,
                                int32_t port,
                                nsACString& hostLine)
{
...
#pragma warning(pop)
  ...
  mOscpu += nsPrintfCString(
      " %d.%d", static_cast<int>(majorVersion), static_cast<int>(minorVersion));
#elif defined(XP_UNIX)
  struct utsname name;
...
#define MULTI_PREF_CHANGED(p) \
  ...
  if (PREF_CHANGED(HTTP_PREF("accept.default"))) {
    nsCString accept;
    rv = prefs->GetCharPref(HTTP_PREF("accept.default"), getter_Copies(accept));
    if (NS_SUCCEEDED(rv)) {
  ...
  if (PREF_CHANGED(HTTP_PREF("throttle.time-window"))) {
    ...
    if (NS_SUCCEEDED(rv) && mConnMgr) {
      Unused << mConnMgr->UpdateParam(
          nsHttpConnectionMgr::THROTTLING_TIME_WINDOW, mThrottleTimeWindow);
    }
  }
                       ...
                       nsACString& o_AcceptLanguages)
{
  ...
  if (!o_Accept) return NS_ERROR_OUT_OF_MEMORY;
  ...
  *q_Accept = '\0';
  ...
  count_n = 0;
  p2 = q_Accept;
  for (token = nsCRT::strtok(o_Accept, ",", &p); token != nullptr;
...
nsHttpHandler::NewURI(const nsACString& aSpec,
                      ...
                      nsIURI** aURI)
{
  return mozilla::net::NewURI(
      aSpec, aCharset, aBaseURI, NS_HTTP_DEFAULT_PORT, aURI);
}
...
nsHttpHandler::NewProxiedChannel(nsIURI* uri,
                                 ...
                                 nsIChannel** result)
{
  return NewProxiedChannel2(
      uri, givenProxyInfo, proxyResolveFlags, proxyURI, nullptr, result);
}
...
static bool
CanEnableSpeculativeConnect();  // forward declaration
                                          ...
                                          bool anonymous)
{
  if (IsNeckoChild()) {
    ...
    gNeckoChild->SendSpeculativeConnect(
        params, IPC::Principal(aPrincipal), anonymous);
    return NS_OK;
  }
  ...
  if (mDebugObservations && obsService) {
    ...
    obsService->NotifyObservers(
        nullptr, "speculative-connect-request", nullptr);
    for (auto* cp :
  ...
  auto* ci = new nsHttpConnectionInfo(host,
                                      port,
                                      EmptyCString(),
                                      username,
                                      nullptr,
...
{
  return mozilla::net::NewURI(
      aSpec, aOriginCharset, aBaseURI, NS_HTTPS_DEFAULT_PORT, _retval);
}
