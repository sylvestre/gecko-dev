...
void
Http2Session::LogIO(Http2Session* self,
                    Http2Stream* stream,
                    const char* label,
                    const char* data,
                    uint32_t datalen)
                        ...
                        nsIInterfaceRequestor* aCallbacks)
{
  ...
  if (!mFirstHttpTransaction && !mTlsHandshakeFinished) {
    ...
    LOG3(("Http2Session::AddStream first session=%p trans=%p ",
          this,
...
nsresult
Http2Session::NetworkRead(nsAHttpSegmentWriter* writer,
                          char* buf,
                          uint32_t count,
                          uint32_t* countWritten)
{
  ...
  if (NS_SUCCEEDED(rv) && *countWritten > 0) mLastReadEpoch = PR_IntervalNow();
  return rv;
}
...
Http2Session::DontReuse()
{
  ...
  mShouldGoAway = true;
  if (!mStreamTransactionHash.Count()) Close(NS_OK);
}
...
void
Http2Session::CreateFrameHeader(charType dest,
...
Http2Session::EnsureOutputBuffer(uint32_t spaceNeeded)
{
  ...
  EnsureBuffer(mOutputQueueBuffer,
               mOutputQueueUsed + spaceNeeded,
               mOutputQueueUsed,
               mOutputQueueSize);
...
template void
Http2Session::CreateFrameHeader(char* dest,
                                uint16_t frameLength,
                                uint8_t frameType,
                                uint8_t frameFlags,
                                uint32_t streamID);
...
template void
Http2Session::CreateFrameHeader(uint8_t* dest,
                                uint16_t frameLength,
                                uint8_t frameType,
                                uint8_t frameFlags,
                                uint32_t streamID);
...
Http2Session::GeneratePing(bool isAck)
{
  ...
  if (isAck) {
    ...
    memcpy(packet + kFrameHeaderBytes,
           mInputFrameBuffer.get() + kFrameHeaderBytes,
           8);
  } else {
...
Http2Session::GeneratePriority(uint32_t aID, uint8_t aPriorityWeight)
{
  MOZ_ASSERT(OnSocketThread(), "not on socket thread");
  LOG3((
      "Http2Session::GeneratePriority %p %X %X\n", this, aID, aPriorityWeight));
...
Http2Session::SendHello()
{
  ...
  LogIO(
      this, nullptr, "Generate Settings", packet, kFrameHeaderBytes + dataLen);
  ...
  if (gHttpHandler->UseH2Deps() &&
    ...
    CreatePriorityNode(
        kSpeculativeGroupID, kBackgroundGroupID, 0, "speculative");
    mNextStreamID += 2;
...
void
Http2Session::CreatePriorityNode(uint32_t streamID,
                                 uint32_t dependsOn,
                                 uint8_t weight,
                                 const char* label)
{
...
void
Http2Session::CleanupStream(Http2Stream* aStream,
                            nsresult aResult,
...
void
Http2Session::CleanupStream(uint32_t aID,
                            nsresult aResult,
                            errorType aResetCode)
{
  MOZ_ASSERT(OnSocketThread(), "not on socket thread");
...
static void
RemoveStreamFromQueue(Http2Stream* aStream, nsDeque& queue)
{
...
nsresult
Http2Session::ParsePadding(uint8_t& paddingControlBytes,
...
Http2Session::RecvPushPromise(Http2Session* self)
{
  ...
  LOG3(("Http2Session::RecvPushPromise %p origin check %s",
        self,
    ...
    nsICacheEntry* entry, nsIApplicationCache* appCache, uint32_t* result)
{
  ...
  *result = nsICacheEntryOpenCallback::ENTRY_NOT_WANTED;
  ...
  nsHttp::DetermineFramingAndImmutability(
      entry, &cachedResponseHead, true, &weaklyFramed, &isImmutable);
  ...
  bool validationRequired =
      nsHttp::ValidationRequired(isForcedValid,
                                 &cachedResponseHead,
                                 0 /*NWGH: ??? - loadFlags*/,
                                 false,
                                 isImmutable,
  ...
  mSession->CleanupStream(
      mPromisedID, NS_ERROR_FAILURE, Http2Session::REFUSED_STREAM_ERROR);
...
Http2Session::RecvPing(Http2Session* self)
{
  ...
  LOG3(("Http2Session::RecvPing %p PING Flags 0x%X.",
        self,
  ...
  if (self->mInputFrameID) {  // stream window
    ...
    if (delta == 0) {
      LOG3(("Http2Session::RecvWindowUpdate %p received 0 stream window update",
            self));
      self->CleanupStream(
          self->mInputFrameDataStream, NS_ERROR_ILLEGAL_VALUE, PROTOCOL_ERROR);
      self->ResetDownstreamState();
      return NS_OK;
    }
    ...
    if (self->mInputFrameDataStream->ServerReceiveWindow() >= 0x80000000) {
      ...
      LOG3(
          ("Http2Session::RecvWindowUpdate %p stream window "
           "exceeds 2^31 - 1\n",
           self));
      self->CleanupStream(self->mInputFrameDataStream,
...
Http2Session::RecvContinuation(Http2Session* self)
{
  ...
  LOG3(
      ("Http2Session::RecvContinuation %p Flags 0x%X id 0x%X "
       ...
       self->mInputFrameFlags,
       self->mInputFrameID,
       self->mExpectedPushPromiseID,
       self->mExpectedHeaderID));
...
class UpdateAltSvcEvent : public Runnable
{
 public:
  ...
  NS_IMETHOD Run() override
  {
    ...
    AltSvcMapping::ProcessHeader(mHeader,
                                 originScheme,
                                 originHost,
                                 originPort,
                                 mCI->GetUsername(),
...
Http2Session::RecvAltSvc(Http2Session* self)
{
  ...
  if (!self->mConnection || !ci) {
    LOG3(("Http2Session::RecvAltSvc %p no connection or conninfo for %d",
          self,
          self->mInputFrameID));
    self->ResetDownstreamState();
    return NS_OK;
  ...
  switch (aStatus) {
    ...
    case NS_NET_STATUS_TLS_HANDSHAKE_ENDED: {
      if (!mFirstHttpTransaction) {
        ...
        mFirstHttpTransaction->OnTransportStatus(
            aTransport, aStatus, aProgress);
      }
    ...
    if (mGoAwayReason == INADEQUATE_SECURITY) {
      LOG3(
          ("Http2Session::ReadSegments %p returning INADEQUATE_SECURITY "
           "%" PRIx32,
           this,
           static_cast<uint32_t>(NS_ERROR_NET_INADEQUATE_SECURITY)));
                                 ...
                                 bool* again)
{
  ...
  *countWritten = 0;
  ...
  if (pushConnectedStream) {
    return ProcessConnectedPush(
        pushConnectedStream, writer, count, countWritten);
  }
  ...
  if (mDownstreamState == BUFFERING_OPENING_SETTINGS ||
    ...
    if (totallyWastedByte || (mInputFrameDataSize > kMaxFrameData)) {
      LOG3(("Got frame too large 0x%02X%04X",
            totallyWastedByte,
            mInputFrameDataSize));
      RETURN_SESSION_ERROR(this, PROTOCOL_ERROR);
    }
    ...
    if (mInputFrameType != FRAME_TYPE_DATA) {  // control frame
      EnsureBuffer(mInputFrameBuffer,
                   mInputFrameDataSize + kFrameHeaderBytes,
                   kFrameHeaderBytes,
  ...
  if (mDownstreamState == PROCESSING_DATA_FRAME_PADDING_CONTROL) {
    ...
    LogIO(this,
          nullptr,
          "Reading Data Frame Padding Control",
          &mInputFrameBuffer[mInputFrameBufferUsed],
          *countWritten);
    ...
    LOG3(("Http2Session::WriteSegments %p stream 0x%X mPaddingLength=%d",
          this,
          mInputFrameID,
          mPaddingLength));
...
Http2Session::Finish0RTT(bool aRestart, bool aAlpnChanged)
{
  ...
  LOG3(("Http2Session::Finish0RTT %p aRestart=%d aAlpnChanged=%d",
        this,
        aRestart,
        aAlpnChanged));
...
Http2Session::Close(nsresult aReason)
{
  ...
  LOG3((
      "Http2Session::Close %p %" PRIX32, this, static_cast<uint32_t>(aReason)));
...
{
  MOZ_ASSERT(OnSocketThread(), "not on socket thread");
  LOG3(("Http2Session::CloseTransaction %p %p %" PRIx32,
        this,
        aTransaction,
        static_cast<uint32_t>(aResult)));
...
nsresult
Http2Session::OnWriteSegment(char* buf, uint32_t count, uint32_t* countWritten)
{
  MOZ_ASSERT(OnSocketThread(), "not on socket thread");
  nsresult rv;
  ...
  if (mDownstreamState == PROCESSING_DATA_FRAME) {
    ...
    LogIO(
        this, mInputFrameDataStream, "Reading Data Frame", buf, *countWritten);
...
{
  LOG3((
      "Http2Session::ConnectSlowConsumer %p 0x%X\n", this, stream->StreamID()));
...
Http2Session::MaybeReTunnel(nsAHttpTransaction* aHttpTransaction)
{
  ...
  LOG(("Http2Session:MaybeReTunnel %p %p count=%d limit %d\n",
       ...
       trans,
       FindTunnelCount(ci),
       gHttpHandler->MaxConnectionsPerOrigin()));
  if (FindTunnelCount(ci) >= gHttpHandler->MaxConnectionsPerOrigin()) {
...
nsresult
Http2Session::BufferOutput(const char* buf, uint32_t count, uint32_t* countRead)
{
...
Http2Session::OnHeadersAvailable(nsAHttpTransaction* transaction,
                                 ...
                                 bool* reset)
{
  return mConnection->OnHeadersAvailable(
      transaction, requestHead, responseHead, reset);
}
...
bool
Http2Session::RealJoinConnection(const nsACString& hostname,
                                 int32_t port,
                                 bool justKidding)
{
  ...
  if (info->ProtocolEnabled(0)) {
    if (justKidding) {
      rv = sslSocketControl->TestJoinConnection(
          info->VersionString[0], hostname, port, &isJoined);
    } else {
      rv = sslSocketControl->JoinConnection(
          info->VersionString[0], hostname, port, &isJoined);
    }
