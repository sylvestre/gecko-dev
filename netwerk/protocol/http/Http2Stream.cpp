...
nsresult
Http2Stream::MakeOriginURL(const nsACString& scheme,
...
Http2Stream::GenerateOpen()
{
  ...
  messageSize +=
      (numFrames - 1) *
      Http2Session::
          kFrameHeaderBytes;  // frame header overhead in CONTINUATION frames

  EnsureBuffer(
      mTxInlineFrame, messageSize, mTxInlineFrameUsed, mTxInlineFrameSize);
  ...
  for (uint32_t idx = 0; idx < numFrames; ++idx) {
    ...
    memcpy(mTxInlineFrame.get() + outputOffset,
           compressedData.BeginReading() + compressedDataOffset,
           frameLen);
...
Http2Stream::AdjustPushedPriority()
{
  ...
  mSession->CreateFrameHeader(
      packet, 5, Http2Session::FRAME_TYPE_PRIORITY, 0, mPushSource->mStreamID);
...
Http2Stream::UpdateTransportSendEvents(uint32_t count)
{
  ...
  if (mUpstreamState != SENDING_FIN_STREAM)
    mTransaction->OnTransportStatus(
        mSocketTransport, NS_NET_STATUS_SENDING_TO, mTotalSent);
                           ...
                           bool forceCommitment)
{
  ...
  Http2Session::LogIO(mSession,
                      this,
                      "Writing from Inline Buffer",
  ...
  if (mTxStreamFrameSize) {
    ...
    Http2Session::LogIO(mSession,
                        this,
                        "Writing from Transaction Buffer",
                        buf,
                        transmittedCount);
...
void
Http2Stream::SetPriorityDependency(uint32_t newDependency,
                                   uint8_t newWeight,
                                   bool exclusive)
{
...
nsresult
Http2Stream::OnReadSegment(const char* buf, uint32_t count, uint32_t* countRead)
{
...
nsresult
Http2Stream::OnWriteSegment(char* buf, uint32_t count, uint32_t* countWritten)
{
