...
nsHttpAuthCache::nsHttpAuthCache()
    : mDB(nullptr), mObserver(new OriginClearObserver(this))
{
...
nsHttpAuthCache::Init()
{
  ...
  mDB = PL_NewHashTable(128,
                        (PLHashFunction)PL_HashString,
                        (PLHashComparator)PL_CompareStrings,
                        (PLHashComparator)0,
                        &gHashAllocOps,
                        this);
  if (!mDB) return NS_ERROR_OUT_OF_MEMORY;
                              ...
                              nsISupports* metadata)
{
  ...
  if (!node) {
    ...
    if (!node) return NS_ERROR_OUT_OF_MEMORY;
...
PLHashAllocOps nsHttpAuthCache::gHashAllocOps = {nsHttpAuthCache::AllocTable,
...
RemoveEntriesForPattern(PLHashEntry* entry, int32_t number, void* arg)
{
  ...
  OriginAttributesPattern const* pattern =
      static_cast<OriginAttributesPattern const*>(arg);
