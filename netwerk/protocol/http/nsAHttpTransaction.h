...
#define NS_DECL_NSAHTTPTRANSACTION                                          \
  ...
  void OnTransportStatus(                                                   \
      nsITransport* transport, nsresult status, int64_t progress) override; \
  bool IsDone() override;                                                   \
  nsresult Status() override;                                               \
  uint32_t Caps() override;                                                 \
  void SetDNSWasRefreshed() override;                                       \
  virtual MOZ_MUST_USE nsresult ReadSegments(                               \
      nsAHttpSegmentReader*, uint32_t, uint32_t*) override;                 \
  virtual MOZ_MUST_USE nsresult WriteSegments(                              \
      nsAHttpSegmentWriter*, uint32_t, uint32_t*) override;                 \
