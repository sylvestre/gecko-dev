...
inline nsHttpAtom
ResolveAtom(const nsACString& s)
{
...
inline bool
IsValidToken(const nsACString& s)
{
...
const char*
FindToken(const char* input, const char* token, const char* separators);
...
//
MOZ_MUST_USE bool
ParseInt64(const char* input, const char** next, int64_t* result);
...
{
 public:
  ParsedHeaderPair(const char* name,
                   int32_t nameLen,
                   const char* val,
                   int32_t valLen)
  {
