...
nsresult
nsHttpRequestHead::SetHeader(const nsACString& h,
                             const nsACString& v,
                             bool m /*= false*/)
{
  RecursiveMutexAutoLock mon(mRecursiveMutex);
  ...
  return mHeaders.SetHeader(
      h, v, m, nsHttpHeaderArray::eVarietyRequestOverride);
}

nsresult
nsHttpRequestHead::SetHeader(nsHttpAtom h,
                             const nsACString& v,
                             bool m /*= false*/)
{
  RecursiveMutexAutoLock mon(mRecursiveMutex);
...
nsresult
nsHttpRequestHead::SetHeaderOnce(nsHttpAtom h,
                                 const char* v,
                                 bool merge /*= false */)
{
  RecursiveMutexAutoLock mon(mRecursiveMutex);
  ...
  if (!merge || !mHeaders.HasHeaderValue(h, v)) {
    return mHeaders.SetHeader(h,
                              nsDependentCString(v),
                              merge,
...
void
nsHttpRequestHead::SetOrigin(const nsACString& scheme,
