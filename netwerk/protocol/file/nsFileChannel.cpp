...
class nsFileUploadContentStream : public nsBaseContentStream
{
 public:
  ...
  NS_IMETHOD ReadSegments(nsWriteSegmentFun fun,
                          void* closure,
                          uint32_t count,
                          uint32_t* result) override;
  NS_IMETHOD AsyncWait(nsIInputStreamCallback* callback,
                       uint32_t flags,
                       uint32_t count,
                       nsIEventTarget* target) override;
...
NS_IMPL_ISUPPORTS_INHERITED0(nsFileUploadContentStream, nsBaseContentStream)

NS_IMETHODIMP
nsFileUploadContentStream::ReadSegments(nsWriteSegmentFun fun,
                                        void* closure,
                                        uint32_t count,
                                        uint32_t* result)
{
...
nsFileChannel::Init()
{
  ...
  if (fileURL && NS_SUCCEEDED(fileURL->GetFile(getter_AddRefs(file))) &&
      ...
      NS_SUCCEEDED(NS_NewNativeLocalFile(
          fileTarget, true, getter_AddRefs(resolvedFile))) &&
#endif
      NS_SUCCEEDED(
                                 ...
                                 nsIChannel** channel)
{
  ...
  if (mUploadStream) {
    ...
    rv = NS_NewLocalFileOutputStream(getter_AddRefs(fileStream),
                                     file,
    ...
    RefPtr<nsFileUploadContentStream> uploadStream =
        new nsFileUploadContentStream(
            async, fileStream, mUploadStream, mUploadLength, this);
