...
class FailDelayManager
{
 public:
  ...
  FailDelay* Lookup(nsCString& address,
                    int32_t port,
...
class CallOnStop final : public nsIRunnable
{
 public:
  ...
  CallOnStop(WebSocketChannel* aChannel, nsresult aReason)
      : mChannel(aChannel), mListenerMT(mChannel->mListenerMT), mReason(aReason)
  {
  }
...
class CallOnServerClose final : public nsIRunnable
{
 public:
  ...
  NS_IMETHOD Run() override
  {
    ...
    if (mListenerMT) {
      nsresult rv = mListenerMT->mListener->OnServerClose(
          mListenerMT->mContext, mCode, mReason);
      if (NS_FAILED(rv)) {
        LOG(
...
static const char* msgNames[] = {
    "text", "binaryString", "binaryStream", "ping", "pong", "close"};
...
WebSocketChannel::BeginOpenInternal()
{
  ...
  rv = NS_NewTimerWithCallback(
      getter_AddRefs(mOpenTimer), this, mOpenTimeout, nsITimer::TYPE_ONE_SHOT);
  if (NS_FAILED(rv)) {
    LOG(
...
bool
WebSocketChannel::UpdateReadBuffer(uint8_t* buffer,
                                   uint32_t count,
    ...
    if (mServerClosed) {
      ...
      if (mListenerMT) {
        ...
        if (isDeflated) {
          ...
          if (!utf8Data.Assign(
                  (const char*)payload, payloadLength, mozilla::fallible)) {
            return NS_ERROR_OUT_OF_MEMORY;
          }
        }
      ...
      RefPtr<WebSocketFrame> frame =
          mService->CreateFrameIfNeeded(finBit,
                                        rsvBit1,
                                        rsvBit2,
                                        rsvBit3,
                                        opcode,
...
void
WebSocketChannel::EnqueueOutgoingMessage(nsDeque& aQueue, OutboundMessage* aMsg)
{
  MOZ_ASSERT(OnSocketThread(), "not on socket thread");
...
WebSocketChannel::PrimeNewOutgoingMessage()
{
  ...
  RefPtr<WebSocketFrame> frame = mService->CreateFrameIfNeeded(
      ...
      mask,
      payload,
      mHdrOutToSend - (payload - mOutHeader),
...
WebSocketChannel::SetupRequest()
{
  ...
  rv = mHttpChannel->SetRequestHeader(
      ...
      NS_LITERAL_CSTRING(SEC_WEBSOCKET_VERSION),
      false);
  MOZ_ASSERT(NS_SUCCEEDED(rv));

  if (!mOrigin.IsEmpty()) {
    rv = mHttpChannel->SetRequestHeader(
        NS_LITERAL_CSTRING("Origin"), mOrigin, false);
    MOZ_ASSERT(NS_SUCCEEDED(rv));
  }
  ...
  rv = mHttpChannel->SetRequestHeader(
      NS_LITERAL_CSTRING("Sec-WebSocket-Key"), secKeyString, false);
  MOZ_ASSERT(NS_SUCCEEDED(rv));
...
WebSocketChannel::DoAdmissionDNS()
{
  ...
  return dns->AsyncResolveNative(hostName,
                                 0,
                                 this,
                                 main,
...
WebSocketChannel::StartPinging()
{
  ...
  nsresult rv;
  rv = NS_NewTimerWithCallback(
      getter_AddRefs(mPingTimer), this, mPingInterval, nsITimer::TYPE_ONE_SHOT);
  if (NS_SUCCEEDED(rv)) {
...
WebSocketChannel::Notify(nsITimer* timer)
{
  ...
  if (timer == mCloseTimer) {
    ...
    if (!mPingOutstanding) {
      ...
      mPingOutstanding = 1;
      mPingForced = 0;
      mPingTimer->InitWithCallback(
          this, mPingResponseTimeout, nsITimer::TYPE_ONE_SHOT);
      GeneratePing();
    } else {
                            ...
                            nsISupports* aContext)
{
  ...
  if (mConnectionLogService && !mPrivateBrowsing) {
    mConnectionLogService->AddHost(
        mHost, mSerial, BaseWebSocketChannel::mEncrypted);
  }
...
nsresult
WebSocketChannel::SendMsgCommon(const nsACString* aMsg,
                                bool aIsBinary,
                                uint32_t aLength,
