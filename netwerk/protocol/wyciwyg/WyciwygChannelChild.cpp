 ...
 public:
  ...
  void Run()
  {
    mChild->OnStartRequest(
        mStatusCode, mContentLength, mSource, mCharset, mSecurityInfo);
  }
                                     ...
                                     const uint64_t& offset)
{
  ...
  rv = mListener->OnDataAvailable(
      this, mListenerContext, stringStream, offset, data.Length());
  if (NS_FAILED(rv)) Cancel(rv);

  if (mProgressSink && NS_SUCCEEDED(rv)) {
    mProgressSink->OnProgress(
        this, nullptr, offset + data.Length(), mContentLength);
  }
}
...
void
WyciwygChannelChild::CancelEarly(const nsresult& statusCode)
{
                               ...
                               nsISupports* aContext)
{
  ...
  SendAsyncOpen(
      originalURI, mLoadFlags, IPC::SerializedLoadContext(this), browser);
