...
TEST(TestStandardURL, Simple)
{
  ...
  ASSERT_EQ(url->SetPathQueryRef(NS_LITERAL_CSTRING(
  ...
  ASSERT_EQ(url->SetQuery(NS_LITERAL_CSTRING(
...
TEST(TestStandardURL, From_test_standardurldotjs)
{
  ...
  const char* nonIPv4s[] = {"0xfffffffff",
                            "0x100000000",
                            "4294967296",
                            "1.2.0x10000",
                            "1.0x1000000",
...
MOZ_GTEST_BENCH(TestStandardURL, Perf, [] {
  ...
  for (int i = COUNT; i; --i) {
    ...
    url->GetHost(out);
    url->SetPathQueryRef(NS_LITERAL_CSTRING(
        "/some-path/one-the-net/about.html?with-a-query#for-you"));
