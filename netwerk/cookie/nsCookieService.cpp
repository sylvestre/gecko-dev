           ...
           const char* aReason)
{
  ...
  if (aSetCookie)
    MOZ_LOG(
        gCookieLog, LogLevel::Warning, ("cookie string: %s\n", aCookieString));
...
LogCookie(nsCookie* aCookie)
{
  ...
  if (aCookie) {
    ...
    MOZ_LOG(
        gCookieLog, LogLevel::Debug, ("value: %s\n", aCookie->Value().get()));
    MOZ_LOG(gCookieLog,
            LogLevel::Debug,
            ("%s: %s\n",
...
#define NS_ASSERT_SUCCESS(res)                                     \
  ...
  if (NS_FAILED(__rv)) {                                           \
    SmprintfPointer msg = mozilla::Smprintf(                       \
        "NS_ASSERT_SUCCESS(%s) failed with result 0x%" PRIX32,     \
        #res,                                                      \
...
class ReadCookieDBListener final : public DBListenerErrorHandler
{
 ...
 public:
  ...
  explicit ReadCookieDBListener(DBState* dbState)
      : DBListenerErrorHandler(dbState), mCanceled(false)
  {
  }
...
nsCookieService::AppClearDataObserverInit()
{
  ...
  observerService->AddObserver(obs,
...
nsCookieService::TryInitDB(bool aRecreateDB)
{
  ...
  if (!tableExists) {
    ...
    switch (dbSchemaVersion) {
      ...
      case 2: {
        ...
        rv = mDefaultDBState->dbConn->CreateStatement(
      ...
      case 6: {
        ...
        rv = mDefaultDBState->dbConn->CreateFunction(
            setInBrowserName, 1, setInBrowser);
        NS_ENSURE_SUCCESS(rv, RESULT_RETRY);
  ...
  if (aRecreateDB) return RESULT_OK;
...
nsCookieService::CreateTable()
{
  ...
  nsresult rv =
      mDefaultDBState->dbConn->SetSchemaVersion(COOKIES_SCHEMA_VERSION);
                                 ...
                                 nsIChannel* aChannel)
{
  ...
  return SetCookieStringCommon(
      aHostURI, aCookieHeader, nullptr, aChannel, false);
}
                                         ...
                                         nsIChannel* aChannel)
{
  ...
  return SetCookieStringCommon(
      aHostURI, aCookieHeader, aServerTime, aChannel, true);
}
                                       ...
                                       bool aFromHttp)
{
  ...
  SetCookieStringInternal(aHostURI,
                          isForeign,
                          cookieString,
                          serverTime,
                          aFromHttp,
                                         ...
                                         nsIChannel* aChannel)
{
  ...
  CookieStatus cookieStatus = CheckPrefs(mPermissionService,
                                         mCookieBehavior,
                                         mThirdPartySession,
...
void
nsCookieService::NotifyThirdParty(nsIURI* aHostURI,
                                  bool aIsAccepted,
                                  nsIChannel* aChannel)
{
  nsCOMPtr<nsIObserverService> os = mozilla::services::GetObserverService();
                     ...
                     uint8_t aArgc)
{
  ...
  nsresult rv = InitializeOriginAttributes(
      &attrs, aOriginAttributes, aCx, aArgc, u"nsICookieManager2.add()", u"2");
  NS_ENSURE_SUCCESS(rv, rv);

  return AddNative(aHost,
                   aPath,
                   aName,
                   aValue,
                   aIsSecure,
...
nsCookieService::AsyncReadComplete()
{
  ...
  for (uint32_t i = 0; i < mDefaultDBState->hostArray.Length(); ++i) {
    ...
    if (mDefaultDBState->readSet.GetEntry(tuple.key)) continue;
  ...
  COOKIE_LOGSTRING(
      LogLevel::Debug,
      ("Read(): %" PRIu32 " cookies read", mDefaultDBState->cookieCount));
...
nsCookieService::CancelAsyncRead(bool aPurgeReadSet)
{
  ...
  if (aPurgeReadSet) mDefaultDBState->readSet.Clear();
}
...
nsCookieService::EnsureReadDomain(const nsCookieKey& aKey)
{
  ...
  if (MOZ_LIKELY(!mDBState->dbConn || !mDefaultDBState->pendingRead)) return;

  // Fast path 2: already read in this particular domain.
  if (MOZ_LIKELY(mDefaultDBState->readSet.GetEntry(aKey))) return;
...
nsCookieService::EnsureReadComplete()
{
  ...
  while (true) {
    ...
    if (mDefaultDBState->readSet.GetEntry(key)) continue;
...
bool
nsCookieService::DomainMatches(nsCookie* aCookie, const nsACString& aHost)
{
...
bool
nsCookieService::PathMatches(nsCookie* aCookie, const nsACString& aPath)
{
                                  ...
                                  nsTArray<nsCookie*>& aCookieList)
{
  ...
  if (NS_FAILED(rv)) {
    COOKIE_LOGFAILURE(
        GET_COOKIE, aHostURI, nullptr, "invalid host/path from URI");
    return;
  }
                                         ...
                                         nsCString& aCookieString)
{
  ...
  GetCookiesForURI(
      aHostURI, aIsForeign, aHttpBound, aOriginAttrs, foundCookieList);
                              ...
                              mozIThirdPartyUtil* aThirdPartyUtil)
{
  ...
  aCookieAttributes.isSession = GetExpiry(
      aCookieAttributes, aServerTime, currentTimeInUsec / PR_USEC_PER_SEC);
  ...
  if ((aCookieAttributes.name.Length() + aCookieAttributes.value.Length()) >
      kMaxBytesPerCookie) {
    COOKIE_LOGFAILURE(
        SET_COOKIE, aHostURI, savedCookieHeader, "cookie too big (> 4kb)");
    return newCookie;
  }
  ...
  if (aCookieAttributes.name.FindCharInSet(illegalNameCharacters, 0) != -1) {
    COOKIE_LOGFAILURE(
        SET_COOKIE, aHostURI, savedCookieHeader, "invalid name character");
    return newCookie;
  }

  // domain & path checks
  if (!CheckDomain(
          aCookieAttributes, aHostURI, aKey.mBaseDomain, aRequireHostMatch)) {
    COOKIE_LOGFAILURE(
        SET_COOKIE, aHostURI, savedCookieHeader, "failed the domain tests");
    return newCookie;
  }
  if (!CheckPath(aCookieAttributes, aHostURI)) {
    COOKIE_LOGFAILURE(
        SET_COOKIE, aHostURI, savedCookieHeader, "failed the path tests");
    return newCookie;
  }
  // magic prefix checks. MUST be run after CheckDomain() and CheckPath()
  if (!CheckPrefixes(aCookieAttributes, isHTTPS)) {
    COOKIE_LOGFAILURE(
        SET_COOKIE, aHostURI, savedCookieHeader, "failed the prefix tests");
    return newCookie;
  }
  ...
  if (aFromHttp &&
    ...
    COOKIE_LOGFAILURE(
        SET_COOKIE, aHostURI, savedCookieHeader, "invalid value character");
    return newCookie;
  }
                                   ...
                                   nsIChannel* aChannel)
{
  ...
  bool newCookie = CanSetCookie(aHostURI,
                                aKey,
                                cookieAttributes,
                                aRequireHostMatch,
                                aStatus,
  ...
  AddInternal(
      aKey, cookie, PR_Now(), aHostURI, savedCookieHeader.get(), aFromHttp);
  return newCookie;
}
                             ...
                             bool aFromHttp)
{
  ...
  foundCookie = FindCookie(
      aKey, aCookie->Host(), aCookie->Name(), aCookie->Path(), exactIter);
  ...
  if (foundCookie) {
    ...
    if (oldCookie->Expiry() <= currentTime) {
      if (aCookie->Expiry() <= currentTime) {
        ...
        COOKIE_LOGFAILURE(
            SET_COOKIE, aHostURI, aCookieHeader, "cookie has already expired");
        return;
      }
      ...
      COOKIE_LOGFAILURE(
          SET_COOKIE, aHostURI, aCookieHeader, "stale cookie was purged");
  ...
  NotifyChanged(
      aCookie, foundCookie ? u"changed" : u"added", oldCookieIsSession);
}
                                 ...
                                 nsCookieAttributes& aCookieAttributes)
{
  ...
  newCookie = GetTokenValue(
      cookieStart, cookieEnd, tokenString, tokenValue, equalsFound);
  if (equalsFound) {
                                       ...
                                       nsCString& aBaseDomain)
{
  ...
  // get the base domain. this will fail if the host contains a leading dot,
  // more than one trailing dot, or is otherwise malformed.
  nsresult rv = aTLDService->GetBaseDomainFromHost(
      Substring(aHost, domain), 0, aBaseDomain);
                            ...
                            const OriginAttributes& aOriginAttrs)
{
  ...
  if (NS_SUCCEEDED(aHostURI->SchemeIs("ftp", &ftp)) && ftp) {
    COOKIE_LOGFAILURE(aCookieHeader ? SET_COOKIE : GET_COOKIE,
                      aHostURI,
                      aCookieHeader,
                                    ...
                                    bool* aFoundCookie)
{
  ...
  *aFoundCookie = FindCookie(
      nsCookieKey(baseDomain, *aOriginAttributes), host, name, path, iter);
  return NS_OK;
}
...
bool
nsCookieService::FindSecureCookie(const nsCookieKey& aKey, nsCookie* aCookie)
{
...
void
nsCookieService::RemoveCookieFromList(
...
void
nsCookieService::UpdateCookieOldestTime(DBState* aDBState, nsCookie* aCookie)
{
...
nsCookieService::CollectReports(nsIHandleReportCallback* aHandleReport,
                                ...
                                bool aAnonymize)
{
  MOZ_COLLECT_REPORT("explicit/cookie-service",
                     KIND_HEAP,
                     UNITS_BYTES,
