...
void
nsPartChannel::InitializeByteRange(int64_t aStart, int64_t aEnd)
{
...
nsresult
nsPartChannel::SendOnStartRequest(nsISupports* aContext)
{
  return mListener->OnStartRequest(this, aContext);
}

nsresult
nsPartChannel::SendOnDataAvailable(nsISupports* aContext,
...
nsresult
nsPartChannel::SendOnStopRequest(nsISupports* aContext, nsresult aStatus)
{
  // Drop the listener
...
void
nsPartChannel::SetContentDisposition(
    const nsACString& aContentDispositionHeader)
{
  ...
  NS_GetFilenameFromDisposition(
      mContentDispositionFilename, mContentDispositionHeader, uri);
  mContentDisposition =
...
NS_IMETHODIMP
nsMultiMixedConv::AsyncConvertData(const char* aFromType,
                                   const char* aToType,
                                   nsIStreamListener* aListener,
                                   nsISupports* aCtxt)
{
...
nsMultiMixedConv::nsMultiMixedConv()
    : mCurrentPartID(0),
      ...
      mTokenizer(std::bind(
          &nsMultiMixedConv::ConsumeToken, this, std::placeholders::_1))
{
  mContentLength = UINT64_MAX;
  mByteRangeStart = 0;
  mByteRangeEnd = 0;
...
nsMultiMixedConv::SendData()
{
  ...
  return mPartChannel->SendOnDataAvailable(
      mContext, inStream, offset, mRawDataLength);
}
