...
nsresult
nsUnknownDecoder::ConvertedStreamListener::AppendDataToString(
                                  ...
                                  uint32_t aCount)
{
  ...
  if (aCount && NS_SUCCEEDED(rv)) {
    ...
    rv = listener->OnDataAvailable(
        request, aCtxt, aStream, aSourceOffset, aCount);
  }
...
bool
nsUnknownDecoder::AllowSniffing(nsIRequest* aRequest)
{
...
void
nsUnknownDecoder::DetermineContentType(nsIRequest* aRequest)
{
  {
    MutexAutoLock lock(mMutex);
  ...
  NS_SniffContent(NS_DATA_SNIFFER_CATEGORY,
                  aRequest,
...
bool
nsUnknownDecoder::SniffForHTML(nsIRequest* aRequest)
{
  /*
...
bool
nsUnknownDecoder::SniffForXML(nsIRequest* aRequest)
{
...
bool
nsUnknownDecoder::SniffURI(nsIRequest* aRequest)
{
...
bool
nsUnknownDecoder::LastDitchSniff(nsIRequest* aRequest)
{
...
nsresult
nsUnknownDecoder::FireListenerNotifications(nsIRequest* request,
                                            nsISupports* aCtxt)
{
  nsresult rv = NS_OK;
