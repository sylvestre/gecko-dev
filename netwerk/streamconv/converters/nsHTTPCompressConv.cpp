...
{
  if (!PL_strncasecmp(
          aFromType, HTTP_COMPRESS_TYPE, sizeof(HTTP_COMPRESS_TYPE) - 1) ||
      !PL_strncasecmp(
          aFromType, HTTP_X_COMPRESS_TYPE, sizeof(HTTP_X_COMPRESS_TYPE) - 1)) {
    mMode = HTTP_COMPRESS_COMPRESS;
  } else if (!PL_strncasecmp(
                 aFromType, HTTP_GZIP_TYPE, sizeof(HTTP_GZIP_TYPE) - 1) ||
             !PL_strncasecmp(
                 aFromType, HTTP_X_GZIP_TYPE, sizeof(HTTP_X_GZIP_TYPE) - 1)) {
    mMode = HTTP_COMPRESS_GZIP;
  } else if (!PL_strncasecmp(
                 aFromType, HTTP_DEFLATE_TYPE, sizeof(HTTP_DEFLATE_TYPE) - 1)) {
    mMode = HTTP_COMPRESS_DEFLATE;
  } else if (!PL_strncasecmp(
                 aFromType, HTTP_BROTLI_TYPE, sizeof(HTTP_BROTLI_TYPE) - 1)) {
...
/* static */ nsresult
nsHTTPCompressConv::BrotliHandler(nsIInputStream* stream,
                                  void* closure,
                                  const char* dataIn,
                                  uint32_t,
                                  uint32_t aAvail,
  ...
  do {
    ...
    if (outSize > 0) {
      nsresult rv = self->do_OnDataAvailable(
          ...
          outSize);
      LOG(("nsHttpCompressConv %p BrotliHandler ODA rv=%" PRIx32,
           self,
  ...
  switch (mMode) {
    ...
    default:
      ...
      rv = listener->OnDataAvailable(
          request, aContext, iStr, aSourceOffset, aCount);
      if (NS_FAILED(rv)) {
        return rv;
      }
  } /* switch */
...
uint32_t
nsHTTPCompressConv::check_header(nsIInputStream* iStr,
                                 uint32_t streamLen,
                                 nsresult* rs)
{
  enum
