...
static void
AppendNonAsciiToNCR(const nsAString& in, nsCString& out)
{
...
nsresult
nsIndexedToHTML::DoOnStartRequest(nsIRequest* request,
                                  ...
                                  nsCString& aBuffer)
{
  ...
  rv = mTextToSubURI->UnEscapeAndConvert(
      NS_LITERAL_CSTRING("UTF-8"), titleUri, unEscapeSpec);
...
nsresult
nsIndexedToHTML::SendToListener(nsIRequest* aRequest,
                                nsISupports* aContext,
                                const nsACString& aBuffer)
{
  ...
  return mListener->OnDataAvailable(
      aRequest, aContext, inputData, 0, aBuffer.Length());
}
                                  ...
                                  nsIDirIndex* aIndex)
{
  ...
  if (t == -1LL) {
    ...
    mozilla::DateTimeFormat::FormatPRTime(
        kDateFormatShort, kTimeFormatNone, t, formatted);
    AppendNonAsciiToNCR(formatted, pushBuffer);
    pushBuffer.AppendLiteral("</td>\n <td>");
    mozilla::DateTimeFormat::FormatPRTime(
        kDateFormatNone, kTimeFormatSeconds, t, formatted);
...
void
nsIndexedToHTML::FormatSizeString(int64_t inSize, nsCString& outSizeString)
{
