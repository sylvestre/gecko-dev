...
static inline int
ParsingFailed(struct list_state* state)
{
...
int
ParseFTPList(const char* line,
             struct list_state* state,
             struct list_result* result,
             PRTimeParamFn timeParam,
             NowTimeFn nowTimeFn)
{
          ...
          if (*p == '/')
            ...
            if (isdigit(line[pos])) {
              ...
              if (pos < linelen && line[pos] == ',' &&
                ...
                memcpy(
                    result->fe_size, p + 1, (unsigned)(&line[pos] - (p + 1)));
            ...
            if (result->fe_time.tm_mday) {
              ...
              if (pos > 24)
                ...
                if (!state->now_time) {
                  state->now_time = nowTimeFn();
                  PR_ExplodeTime(
                      (state->now_time), timeParam, &(state->now_tm));
                }
                result->fe_time.tm_year = state->now_tm.tm_year;
