...
// aStringToAppendTo
void
mozTXTToHTMLConv::EscapeChar(const char16_t ch,
...
void
mozTXTToHTMLConv::UnescapeStr(const char16_t* aInString,
                              int32_t aStartPos,
                              int32_t aLength,
                              nsString& aOutString)
{
  ...
  for (uint32_t i = aStartPos; int32_t(i) - aStartPos < aLength;) {
    ...
    if (aInString[i] == '&') {
      ...
      if (!nsCRT::strncmp(
              subString, u"&lt;", std::min(4, aLength - remainingChars))) {
        aOutString.Append(char16_t('<'));
        i += 4;
      } else if (!nsCRT::strncmp(subString,
                                 u"&gt;",
...
void
mozTXTToHTMLConv::CompleteAbbreviatedURL(const char16_t* aInString,
                                         int32_t aInLength,
                                         const uint32_t pos,
                                         nsString& aOutString)
{
  ...
  if (aInString[pos] == '@') {
    ...
    if (ItMatchesDelimited(
            aInString, aInLength, u"www.", 4, LT_IGNORE, LT_IGNORE)) {
      aOutString.AssignLiteral("http://");
      aOutString += aInString;
    } else if (ItMatchesDelimited(
                   aInString, aInLength, u"ftp.", 4, LT_IGNORE, LT_IGNORE)) {
...
bool
mozTXTToHTMLConv::FindURLStart(const char16_t* aInString,
                               int32_t aInLength,
                               const uint32_t pos,
                               ...
                               uint32_t& start)
{
  switch (check) {  // no breaks, because end of blocks is never reached
    case RFC1738: {
      if (!nsCRT::strncmp(
              &aInString[std::max(int32_t(pos - 4), 0)], u"<URL:", 5)) {
        start = pos + 1;
        return true;
      } else
        return false;
...
bool
mozTXTToHTMLConv::FindURLEnd(const char16_t* aInString,
...
void
mozTXTToHTMLConv::CalculateURLBoundaries(const char16_t* aInString,
                                         int32_t aInStringLength,
                                         const uint32_t pos,
                                         ...
                                         int32_t& replaceAfter)
{
  ...
  // FIX ME
  nsAutoString temp2;
  ScanTXT(&aInString[descstart],
          pos - descstart,
          ~kURLs /*prevents loop*/ & whathasbeendone,
          temp2);
  replaceBefore = temp2.Length();
}

bool
mozTXTToHTMLConv::ShouldLinkify(const nsCString& aURL)
{
...
bool
mozTXTToHTMLConv::CheckURLAndCreateHTML(const nsString& txtURL,
                                        const nsString& desc,
                                        const modetype mode,
                                        nsString& outputHTML)
{
...
NS_IMETHODIMP
mozTXTToHTMLConv::FindURLInPlaintext(const char16_t* aInString,
                                     int32_t aInLength,
                                     int32_t aPos,
                                     int32_t* aStartPos,
                                     int32_t* aEndPos)
...
bool
mozTXTToHTMLConv::FindURL(const char16_t* aInString,
                          int32_t aInLength,
                          const uint32_t pos,
                          const uint32_t whathasbeendone,
                          nsString& outputHTML,
                          ...
                          int32_t& replaceAfter)
{
       ...
       If this is the first, check this one, which isn't = success yet */
  {
    ...
    if (state[check] == endok) {
      ...
      CalculateURLBoundaries(aInString,
                             aInLength,
                             pos,
                             whathasbeendone,
                             check,
                                     ...
                                     LIMTYPE after)
{
  ...
  if ((before == LT_ALPHA && !nsCRT::IsAsciiAlpha(text0)) ||
      (before == LT_DIGIT && !nsCRT::IsAsciiDigit(text0)) ||
      (before == LT_DELIMITER &&
       (nsCRT::IsAsciiAlpha(text0) || nsCRT::IsAsciiDigit(text0) ||
        text0 == *rep)) ||
      (after == LT_ALPHA && !nsCRT::IsAsciiAlpha(textAfterPos)) ||
      (after == LT_DIGIT && !nsCRT::IsAsciiDigit(textAfterPos)) ||
      (after == LT_DELIMITER &&
       (nsCRT::IsAsciiAlpha(textAfterPos) ||
        nsCRT::IsAsciiDigit(textAfterPos) || textAfterPos == *rep)) ||
...
uint32_t
mozTXTToHTMLConv::NumberOfMatches(const char16_t* aInString,
                                  int32_t aInStringLength,
                                  const char16_t* rep,
                                  int32_t aRepLen,
                                  LIMTYPE before,
                                  LIMTYPE after)
{
  ...
  for (int32_t i = 0; i < aInStringLength; i++) {
    ...
    if (ItMatchesDelimited(
            indexIntoString, aInStringLength - i, rep, aRepLen, before, after))
      result++;
  }
  return result;
}
...
bool
mozTXTToHTMLConv::StructPhraseHit(const char16_t* aInString,
                                  int32_t aInStringLength,
                                  bool col0,
                                  ...
                                  uint32_t& openTags)
{
  ...
  // opening tag
  if (ItMatchesDelimited(aInString,
                         aInStringLength,
                         tagTXT,
                         aTagTXTLen,
                         (col0 ? LT_IGNORE : LT_DELIMITER),
                         LT_ALPHA)  // is opening tag
      && NumberOfMatches(newOffset,
                         newLength,
                         tagTXT,
                         aTagTXTLen,
                         LT_ALPHA,
  ...
  // closing tag
  else if (openTags > 0 && ItMatchesDelimited(aInString,
                                              aInStringLength,
                                              tagTXT,
                                              aTagTXTLen,
                                              LT_ALPHA,
...
bool
mozTXTToHTMLConv::SmilyHit(const char16_t* aInString,
                           int32_t aLength,
                           bool col0,
                           const char* tagTXT,
                           ...
                           int32_t& glyphTextLen)
{
  ...
  if ((col0 || IsSpace(aInString[0])) &&
      ...
      ItMatchesDelimited(aInString,
                         aLength,
                         NS_ConvertASCIItoUTF16(tagTXT).get(),
                         tagLen,
                         col0 ? LT_IGNORE : LT_DELIMITER,
                         LT_IGNORE)
...
bool
mozTXTToHTMLConv::GlyphHit(const char16_t* aInString,
                           int32_t aInLength,
                           bool col0,
                           ...
                           int32_t& glyphTextLen)
{
  ...
  while (i < 2) {
    ...
    if (bTestSmilie && (SmilyHit(aInString,
                                 aInLength,
                                 bArg,
                                 ":-)",
                                 "moz-smiley-s1",  // smile
  ...
  if (text0 == '+' || text1 == '+') {
    if (ItMatchesDelimited(
            aInString, aInLength, u" +/-", 4, LT_IGNORE, LT_IGNORE)) {
      aOutputString.AppendLiteral(" &plusmn;");
      glyphTextLen = 4;
      return true;
    }
    if (col0 && ItMatchesDelimited(
                    aInString, aInLength, u"+/-", 3, LT_IGNORE, LT_IGNORE)) {
      aOutputString.AppendLiteral("&plusmn;");
      glyphTextLen = 3;
      return true;
    }
...
void
mozTXTToHTMLConv::ScanTXT(const char16_t* aInString,
                          int32_t aInStringLength,
                          uint32_t whattodo,
                          nsString& aOutString)
{
...
void
mozTXTToHTMLConv::ScanHTML(nsString& aInString,
                           uint32_t whattodo,
                           nsString& aOutString)
{
...
NS_IMETHODIMP
mozTXTToHTMLConv::ScanTXT(const char16_t* text,
                          uint32_t whattodo,
                          char16_t** _retval)
{
  NS_ENSURE_ARG(text);
...
NS_IMETHODIMP
mozTXTToHTMLConv::ScanHTML(const char16_t* text,
                           uint32_t whattodo,
                           char16_t** _retval)
{
  NS_ENSURE_ARG(text);
