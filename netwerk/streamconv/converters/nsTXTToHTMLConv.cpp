...
nsTXTToHTMLConv::OnStartRequest(nsIRequest* request, nsISupports* aContext)
{
  ...
  rv = mListener->OnDataAvailable(
      request, aContext, inputData, 0, mBuffer.Length());
  if (NS_FAILED(rv)) return rv;
  mBuffer.Truncate();
  return rv;
}
...
NS_IMETHODIMP
nsTXTToHTMLConv::OnStopRequest(nsIRequest* request,
...
NS_IMETHODIMP
nsTXTToHTMLConv::OnDataAvailable(nsIRequest* request,
                                 nsISupports* aContext,
                                 ...
                                 uint32_t aCount)
{
  ...
  do {
    ...
    if (!pushBuffer.IsEmpty()) {
      ...
      rv = mListener->OnDataAvailable(
          request, aContext, inputData, 0, pushBuffer.Length());
      if (NS_FAILED(rv)) return rv;
    }
...
nsTXTToHTMLConv::CatHTML(int32_t front, int32_t back)
{
  ...
  if (!mToken->prepend) {
    ...
    nsCString escaped;
    if (NS_EscapeURL(
            linkTextUTF8.Data(), linkTextUTF8.Length(), esc_Minimal, escaped)) {
