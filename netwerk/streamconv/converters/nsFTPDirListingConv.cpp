                                     ...
                                     uint32_t count)
{
  ...
  rv = mFinalListener->OnDataAvailable(
      request, ctxt, inputData, 0, indexFormat.Length());
...
nsresult
nsFTPDirListingConv::GetHeaders(nsACString& headers, nsIURI* uri)
{
  nsresult rv = NS_OK;
  // build up 300 line
...
nsFTPDirListingConv::DigestBufferLines(char* aBuffer, nsCString& aString)
{
  ...
  while (line && (eol = PL_strchr(line, nsCRT::LF))) {
    ...
    aString.Append(NS_EscapeURL(
        ...
        esc_Minimal | esc_OnlyASCII | esc_Forced,
        buf));
    ...
    PR_FormatTimeUSEnglish(
        buffer, sizeof(buffer), "%a, %d %b %Y %H:%M:%S", &result.fe_time);
