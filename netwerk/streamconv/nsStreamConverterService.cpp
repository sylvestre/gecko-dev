...
nsresult
nsStreamConverterService::FindConverter(const char* aContractID,
                                  ...
                                  nsIInputStream** _retval)
{
  ...
  if (NS_FAILED(rv)) {
    ...
    for (int32_t i = edgeCount - 1; i >= 0; i--) {
      ...
      rv = converter->Convert(dataToConvert,
                              fromStr.get(),
                              toStr.get(),
                              aContext,
  ...
  if (NS_FAILED(rv)) {
    ...
    for (int i = 0; i < edgeCount; i++) {
      ...
      rv = converter->AsyncConvertData(
          fromStr.get(), toStr.get(), finalListener, aContext);
      if (NS_FAILED(rv)) {
        delete converterChain;
        return rv;
      }

      nsCOMPtr<nsIStreamListener> chainListener(
          do_QueryInterface(converter, &rv));
      if (NS_FAILED(rv)) {
        delete converterChain;
        return rv;
      }
