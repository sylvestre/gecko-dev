...
OutgoingMsg::OutgoingMsg(struct sctp_sendv_spa& info,
                         const uint8_t* data,
                         size_t length)
...
void
OutgoingMsg::Advance(size_t offset)
{
  mPos += offset;
  if (mPos > mLength) {
    mPos = mLength;
...
DataChannelConnection::DataChannelConnection(DataConnectionListener* listener,
    ...
    : NeckoTargetHolder(aTarget), mLock("netwerk::sctp::DataChannelConnection")
{
  mCurrentStream = 0;
  mState = CLOSED;
  mSocket = nullptr;
  ...
  LOG(("Constructor DataChannelConnection=%p, listener=%p",
       this,
       mListener.get()));
...
DataChannelConnection::~DataChannelConnection()
{
  ...
  if (!IsSTSThread()) {
    ...
    if (mTransportFlow) {
      ASSERT_WEBRTC(mSTS);
      NS_ProxyRelease("DataChannelConnection::mTransportFlow",
                      mSTS,
                      mTransportFlow.forget());
    }
...
void
DataChannelConnection::DestroyOnSTS(struct socket* aMasterSocket,
...
bool
DataChannelConnection::Init(unsigned short aPort,
                            ...
                            uint64_t aMaxMessageSize)
{
  ...
  if ((mMasterSocket = usrsctp_socket(AF_CONN,
                                      SOCK_STREAM,
                                      IPPROTO_SCTP,
                                      receive_cb,
                                      threshold_event,
  ...
  l.l_onoff = 1;
  l.l_linger = 0;
  if (usrsctp_setsockopt(mMasterSocket,
                         SOL_SOCKET,
                         SO_LINGER,
                         (const void*)&l,
  ...
  for (unsigned short event_type : event_types) {
    ...
    if (usrsctp_setsockopt(
            mMasterSocket, IPPROTO_SCTP, SCTP_EVENT, &event, sizeof(event)) <
        0) {
  ...
  if (usrsctp_getsockopt(
          mMasterSocket, IPPROTO_SCTP, SCTP_INITMSG, &initmsg, &len) < 0) {
    LOG(("*** failed getsockopt SCTP_INITMSG"));
    goto error_cleanup;
  }
  LOG(("Setting number of SCTP streams to %u, was %u/%u",
       aNumStreams,
...
void
DataChannelConnection::SetMaxMessageSize(bool aMaxMessageSizeSet,
                                         uint64_t aMaxMessageSize)
{
...
bool
DataChannelConnection::ConnectViaTransportFlow(TransportFlow* aFlow,
                                               uint16_t localport,
                                               uint16_t remoteport)
{
...
void
DataChannelConnection::CompleteConnect(TransportFlow* flow,
                                       TransportLayer::State state)
{
  ...
  if (r < 0) {
    ...
    if (r >= 0 || errno == EINPROGRESS) {
      ...
      r = usrsctp_getsockopt(mMasterSocket,
                             IPPROTO_SCTP,
      ...
      if (r < 0) {
        ...
        opt_len = (socklen_t)sizeof(struct sctp_paddrparams);
        r = usrsctp_setsockopt(mMasterSocket,
                               IPPROTO_SCTP,
                               SCTP_PEER_ADDR_PARAMS,
                               &paddrparams,
                               opt_len);
...
DataChannelConnection::ProcessQueuedOpens()
{
  ...
  while (nullptr !=
    ...
    if (channel->mFlags & DATA_CHANNEL_FLAGS_FINISH_OPEN) {
      LOG(("Processing queued open for %p (%u)",
           channel.get(),
           channel->mStream));
...
int
DataChannelConnection::SendPacket(unsigned char data[],
                                  size_t len,
                                  bool release)
{
...
DataChannelConnection::Connect(const char* addr, unsigned short port)
{
...
#else
  {
    ...
    if (!WSAStringToAddressA(const_cast<char*>(addr),
                             AF_INET6,
                             nullptr,
                             (struct sockaddr*)&ss,
                             &sslen)) {
    ...
    } else if (!WSAStringToAddressA(const_cast<char*>(addr),
                                    AF_INET,
                                    nullptr,
                                    (struct sockaddr*)&ss,
                                    &sslen)) {
...
DataChannelConnection::RequestMoreStreams(int32_t aNeeded)
{
  ...
  if (usrsctp_getsockopt(
          mMasterSocket, IPPROTO_SCTP, SCTP_STATUS, &status, &len) < 0) {
...
int
DataChannelConnection::SendControlMessage(const uint8_t* data,
                                          uint32_t len,
                                          uint16_t stream)
{
...
DataChannelConnection::SendDeferredMessages()
{
  ...
  do {
    ...
    if (wasOverThreshold && bufferedAmount < threshold) {
      LOG(("%s: sending BUFFER_LOW_THRESHOLD for %s/%s: %u",
           __FUNCTION__,
           channel->mLabel.get(),
           channel->mProtocol.get(),
           channel->mStream));
      Dispatch(do_AddRef(new DataChannelOnMessageAvailable(
    ...
    if (bufferedAmount == 0) {
      ...
      // available for GC
      LOG(("%s: sending NO_LONGER_BUFFERED for %s/%s: %u",
           __FUNCTION__,
           channel->mLabel.get(),
           channel->mProtocol.get(),
           channel->mStream));
...
// returns if we're still blocked (true)
bool
DataChannelConnection::SendBufferedMessages(
    ...
    uint16_t stream)
{
  ...
  LOG((
      "%s: length %u, sizeof(*req) = %zu", __FUNCTION__, length, sizeof(*req)));
  ...
  if (stream >= mStreams.Length()) {
    LOG(("%s: stream %u out of bounds (%zu)",
         __FUNCTION__,
         stream,
         mStreams.Length()));
    return;
  ...
  LOG(("%s: sending ON_CHANNEL_CREATED for %s/%s: %u (state %u)",
       __FUNCTION__,
       channel->mLabel.get(),
       channel->mProtocol.get(),
       stream,
  ...
  LOG(("%s: deferring sending ON_CHANNEL_OPEN for %p",
       __FUNCTION__,
       channel.get()));
...
DataChannelConnection::DeliverQueuedData(uint16_t stream)
{
  ...
  while (i < mQueuedData.Length()) {
    ...
    if (mQueuedData[i]->mStream == stream) {
      ...
      HandleDataMessage(mQueuedData[i]->mData,
                        mQueuedData[i]->mLength,
                        mQueuedData[i]->mPpid,
...
// Caller must ensure that length <= SIZE_MAX
void
DataChannelConnection::HandleOpenAckMessage(
    const struct rtcweb_datachannel_ack* ack, uint32_t length, uint16_t stream)
{
  DataChannel* channel;
  ...
  LOG(("OpenAck received for stream %u, waiting=%d",
       stream,
...
// Caller must ensure that length <= SIZE_MAX
void
DataChannelConnection::HandleUnknownMessage(uint32_t ppid,
                                            uint32_t length,
                                            uint16_t stream)
{
  /* XXX: Send an error message? */
  LOG(("unknown DataChannel message received: %u, len %u on stream %d",
       ppid,
       length,
       stream));
...
uint8_t
DataChannelConnection::BufferMessage(nsACString& recvBuffer,
                                     const void* data,
                                     uint32_t length,
                                     uint32_t ppid,
                                     int flags)
...
void
DataChannelConnection::HandleDataMessage(
    const void* data, size_t length, uint32_t ppid, uint16_t stream, int flags)
{
  DataChannel* channel;
  const char* buffer = (const char*)data;
  ...
  // Notify onmessage
  LOG(("%s: sending ON_DATA_%s%s for %p",
       __FUNCTION__,
...
void
DataChannelConnection::HandleDCEPMessage(const void* buffer,
                                         size_t length,
                                         uint32_t ppid,
                                         uint16_t stream,
                                         int flags)
...
#if (SIZE_MAX > UINT32_MAX)
  if (length > UINT32_MAX) {
    LOG(("DataChannel: Cannot handle message of size %zu (max=%u)",
         length,
         UINT32_MAX));
    Stop();
    return;
...
// Called with mLock locked!
void
DataChannelConnection::HandleMessage(const void* buffer,
                                     size_t length,
                                     uint32_t ppid,
                                     uint16_t stream,
                                     int flags)
  ...
  switch (ppid) {
    ...
    default:
      LOG(("Message of length %zu PPID %u on stream %u received (%s).",
           ...
           ppid,
           stream,
           (flags & MSG_EOR) ? "complete" : "partial"));
      break;
  }
}

void
DataChannelConnection::HandleAssociationChangeEvent(
...
void
DataChannelConnection::HandlePeerAddressChangeEvent(
...
void
DataChannelConnection::HandleRemoteErrorEvent(
...
void
DataChannelConnection::HandleShutdownEvent(
...
void
DataChannelConnection::HandleAdaptationIndication(
...
void
DataChannelConnection::HandlePartialDeliveryEvent(
    const struct sctp_pdapi_event* spde)
{
  ...
  LOG(("(flags = %x), stream = %" PRIu32 ", sn = %" PRIu32,
       spde->pdapi_flags,
       spde->pdapi_stream,
       spde->pdapi_seq));
...
void
DataChannelConnection::HandleSendFailedEvent(
    const struct sctp_send_failed_event* ssfe)
{
  ...
  LOG(("message with PPID = %u, SID = %d, flags: 0x%04x due to error = 0x%08x",
       ...
       ssfe->ssfe_info.snd_sid,
       ssfe->ssfe_info.snd_flags,
       ssfe->ssfe_error));
...
DataChannelConnection::ClearResets()
{
  ...
  for (uint32_t i = 0; i < mStreamsResetting.Length(); ++i) {
    ...
    if (channel) {
      LOG(("Forgetting channel %u (%p) with pending reset",
           channel->mStream,
           channel.get()));
...
void
DataChannelConnection::HandleStreamResetEvent(
...
void
DataChannelConnection::HandleStreamChangeEvent(
    const struct sctp_stream_change_event* strchg)
{
  ...
  if (strchg->strchange_outstrms > mStreams.Length() ||
    ...
    LOG((
        "%zu of %d new streams already needed", num_needed, new_len - old_len));
      ...
      if ((strchg->strchange_flags & SCTP_STREAM_CHANGE_DENIED) ||
        ...
        if (stream != INVALID_STREAM) {
          ...
          if (error) {
            ...
            LOG(("%s: sending ON_CHANNEL_OPEN for %p",
                 __FUNCTION__,
                 channel.get()));
            Dispatch(do_AddRef(new DataChannelOnMessageAvailable(
...
// Called with mLock locked!
void
DataChannelConnection::HandleNotification(const union sctp_notification* notif,
                                          size_t n)
{
  mLock.AssertCurrentThreadOwns();
...
int
DataChannelConnection::ReceiveCallback(struct socket* sock,
                                       void* data,
                                       size_t datalen,
                                       struct sctp_rcvinfo rcv,
                                       int flags)
...
DataChannelConnection::Open(const nsACString& label,
                            const nsACString& protocol,
                            Type type,
                            bool inOrder,
                            uint32_t prValue,
                            ...
                            uint16_t aStream)
{
  ...
  LOG(
      ("DC Open: label %s/%s, type %u, inorder %d, prValue %u, listener %p, "
       ...
       PromiseFlatCString(label).get(),
       PromiseFlatCString(protocol).get(),
       type,
       inOrder,
       prValue,
       aListener,
       aContext,
       aExternalNegotiated ? "true" : "false",
       aStream));
  switch (type) {
    case DATA_CHANNEL_RELIABLE:
      prPolicy = SCTP_PR_SCTP_NONE;
...
DataChannelConnection::OpenFinish(already_AddRefed<DataChannel>&& aChannel)
{
  ...
  SendDataMsgInternalOrBuffer(
      channel, "Help me!", 8, DATA_CHANNEL_PPID_DOMSTRING);
#endif
...
DataChannelConnection::SendMsgInternal(OutgoingMsg& msg)
{
  ...
  do {
    ...
    ssize_t written = usrsctp_sendv(mSocket,
                                    msg.GetData(),
                                    length,
                                    nullptr,
                                    0,
...
int
DataChannelConnection::SendMsgInternalOrBuffer(
    nsTArray<nsAutoPtr<BufferedOutgoingMsg>>& buffer,
    OutgoingMsg& msg,
    bool& buffered)
{
...
// Returns a POSIX error code.
int
DataChannelConnection::SendDataMsgInternalOrBuffer(DataChannel& channel,
                                                   const uint8_t* data,
                                                   size_t len,
                                                   uint32_t ppid)
{
...
// Returns a POSIX error code.
int
DataChannelConnection::SendDataMsg(DataChannel& channel,
                                   const uint8_t* data,
                                   size_t len,
                                   ...
                                   uint32_t ppidFinal)
{
  ...
  if (mPpidFragmentation) {
    ...
    if (mMaxMessageSize != 0 && len > mMaxMessageSize) {
      LOG(("Message rejected, too large (%zu > %" PRIu64 ")",
           len,
           mMaxMessageSize));
      return EMSGSIZE;
    }
...
// Returns a POSIX error code.
int
DataChannelConnection::SendDataMsgCommon(uint16_t stream,
                                         ...
                                         bool isBinary)
{
  ...
  LOG(("Sending %sto stream %u: %u bytes",
       isBinary ? "binary " : "",
       stream,
       len));
...
DataChannelConnection::CloseAll()
{
  ...
  while (nullptr != (channel = dont_AddRef(
    ...
    LOG(("closing pending channel %p, stream %u",
         channel.get(),
         channel->mStream));
