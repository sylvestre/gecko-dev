...
{
 public:
  QueuedDataMessage(uint16_t stream,
                    uint32_t ppid,
                    int flags,
                    const void* data,
                    uint32_t length)
...
#endif
{
 ...
 public:
  ...
  void SetEvenOdd();
  bool ConnectViaTransportFlow(TransportFlow* aFlow,
                               uint16_t localport,
                               uint16_t remoteport);
  ...
  int ReceiveCallback(struct socket* sock,
                      void* data,
                      size_t datalen,
  ...
  void ReadBlob(already_AddRefed<DataChannelConnection> aThis,
                uint16_t aStream,
 ...
 private:
  ...
  void SctpDtlsInput(TransportFlow* flow,
                     const unsigned char* data,
                     size_t len);
  static int SctpDtlsOutput(
  ...
  uint8_t BufferMessage(nsACString& recvBuffer,
                        ...
                        uint32_t ppid,
                        int flags);
  void HandleDataMessage(const void* buffer,
                         size_t length,
                         uint32_t ppid,
                         uint16_t stream,
                         int flags);
  void HandleDCEPMessage(const void* buffer,
                         size_t length,
                         uint32_t ppid,
                         uint16_t stream,
                         int flags);
  void HandleMessage(const void* buffer,
                     size_t length,
                     uint32_t ppid,
                     uint16_t stream,
                     int flags);
