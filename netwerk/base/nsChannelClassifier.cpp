...
CachedPrefs::Init()
{
  ...
  Preferences::RegisterCallbackAndCall(
      CachedPrefs::OnPrefsChange, URLCLASSIFIER_SKIP_HOSTNAMES, this);
  Preferences::RegisterCallbackAndCall(
      CachedPrefs::OnPrefsChange, URLCLASSIFIER_TRACKING_WHITELIST, this);
  Preferences::RegisterCallbackAndCall(
      CachedPrefs::OnPrefsChange, URLCLASSIFIER_TRACKING_TABLE, this);
}
    ...
    nsIChannel* aChannel, bool aAnnotationsOnly, bool* result)
{
  ...
  *result = false;
  ...
  if (permissions == nsIPermissionManager::ALLOW_ACTION) {
    LOG(("nsChannelClassifier[%p]: Allowlisting channel[%p] for %s",
         this,
         aChannel,
         escaped.get()));
    mIsAllowListed = true;
...
nsChannelClassifier::StartInternal()
{
  ...
  rv = NS_URIChainHasFlags(
      uri, nsIProtocolHandler::URI_DANGEROUS_TO_LOAD, &hasFlags);
  NS_ENSURE_SUCCESS(rv, rv);
  if (hasFlags) return NS_ERROR_UNEXPECTED;

  rv = NS_URIChainHasFlags(
      uri, nsIProtocolHandler::URI_IS_LOCAL_FILE, &hasFlags);
  NS_ENSURE_SUCCESS(rv, rv);
  if (hasFlags) return NS_ERROR_UNEXPECTED;

  rv = NS_URIChainHasFlags(
      uri, nsIProtocolHandler::URI_IS_UI_RESOURCE, &hasFlags);
  NS_ENSURE_SUCCESS(rv, rv);
  if (hasFlags) return NS_ERROR_UNEXPECTED;
...
TrackingURICallback::OnTrackerFound(nsresult aErrorCode)
{
  ...
  if (mChannelClassifier->ShouldEnableTrackingProtection()) {
    mChannelClassifier->SetBlockedContent(
        channel, aErrorCode, mList, mProvider, mFullHash);
...
nsresult
nsChannelClassifier::CheckIsTrackerWithLocalTable(
    std::function<void()>&& aCallback)
{
  nsresult rv;
  ...
  LOG(("nsChannelClassifier[%p]:CheckIsTrackerWithLocalTable for uri=%s\n",
       ...
       uri->GetSpecOrDefault().get()));
  return uriClassifier->AsyncClassifyLocalWithTables(
      uri, trackingBlacklist, callback);
}
...
NS_IMETHODIMP
nsChannelClassifier::Observe(nsISupports* aSubject,
                             const char* aTopic,
