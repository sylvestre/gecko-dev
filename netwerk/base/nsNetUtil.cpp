                      ...
                      nsIIOService* aIoService /* = nullptr */)
{
  ...
  rv = aIoService->NewChannelFromURIWithLoadInfo(
      aUri, aLoadInfo, getter_AddRefs(channel));
  NS_ENSURE_SUCCESS(rv, rv);
                      ...
                      nsIIOService* aIoService /* = nullptr */)
{
  ...
  rv = aIoService->NewChannelFromURI2(
      aUri,
      aLoadingNode ? aLoadingNode->AsDOMNode() : nullptr,
      aLoadingPrincipal,
...
nsresult
NS_MakeAbsoluteURI(nsACString& result, const nsACString& spec, nsIURI* baseURI)
{
  nsresult rv;
  if (!baseURI) {
...
nsresult
NS_MakeAbsoluteURI(char** result, const char* spec, nsIURI* baseURI)
{
  nsresult rv;
...
nsresult
NS_MakeAbsoluteURI(nsAString& result, const nsAString& spec, nsIURI* baseURI)
{
  nsresult rv;
  if (!baseURI) {
    NS_WARNING("It doesn't make sense to not supply a base URI");
...
int32_t
NS_GetDefaultPort(const char* scheme, nsIIOService* ioService /* = nullptr */)
{
  nsresult rv;
                                 ...
                                 nsContentPolicyType aContentPolicyType)
{
  ...
  return NS_NewInputStreamChannelInternal(
      outChannel, aUri, aStream, aContentType, aContentCharset, loadInfo);
}
                                 ...
                                 bool aIsSrcdocChannel /* = false */)
{
  nsCOMPtr<nsILoadInfo> loadInfo = new mozilla::LoadInfo(aLoadingPrincipal,
                                                         ...
                                                         aContentPolicyType);
  return NS_NewInputStreamChannelInternal(
      outChannel, aUri, aData, aContentType, loadInfo, aIsSrcdocChannel);
}
...
bool
NS_IsReasonableHTTPHeaderValue(const nsACString& aValue)
{
  return mozilla::net::nsHttp::IsReasonableHeaderValue(aValue);
}

bool
NS_IsValidHTTPToken(const nsACString& aToken)
{
...
bool
NS_LoadGroupMatchesPrincipal(nsILoadGroup* aLoadGroup, nsIPrincipal* aPrincipal)
{
  if (!aPrincipal) {
    return false;
  }
  ...
  NS_QueryNotificationCallbacks(nullptr,
                                aLoadGroup,
...
nsresult
NS_NewSyncStreamListener(nsIStreamListener** result, nsIInputStream** stream)
{
  nsresult rv;
...
nsresult
NS_ImplementChannelOpen(nsIChannel* channel, nsIInputStream** result)
{
...
nsresult
NS_GetReferrerFromChannel(nsIChannel* channel, nsIURI** referrer)
{
  ...
  *referrer = nullptr;
  ...
  if (props) {
    ...
    rv = props->GetPropertyAsInterface(
        NS_LITERAL_STRING("docshell.internalReferrer"),
        NS_GET_IID(nsIURI),
                           ...
                           nsCString& contentCharset)
{
  ...
  nsCString charset;
  bool hadCharset;
  rv = util->ParseRequestContentType(
      rawContentType, charset, &hadCharset, contentType);
                            ...
                            nsCString& contentCharset)
{
  ...
  nsCString charset;
  bool hadCharset;
  rv = util->ParseResponseContentType(
      rawContentType, charset, &hadCharset, contentType);
  if (NS_SUCCEEDED(rv) && hadCharset) contentCharset = charset;
  return rv;
}
...
NS_BufferOutputStream(nsIOutputStream* aOutputStream, uint32_t aBufferSize)
{
  NS_ASSERTION(aOutputStream, "No output stream given!");

  nsCOMPtr<nsIOutputStream> bos;
  nsresult rv = NS_NewBufferedOutputStream(
      getter_AddRefs(bos), aOutputStream, aBufferSize);
  if (NS_SUCCEEDED(rv)) return bos.forget();
  ...
  if (isFile) {
    ...
    if (NS_SUCCEEDED(rv)) {
      ...
      if (NS_SUCCEEDED(rv)) {
        ...
        rv = NS_NewBufferedInputStream(result, fileStream.forget(), 8192);
      }
    }
    return rv;
  }
                           ...
                           uint32_t aCount)
{
  ...
  if (!*aDest) {
    *aDest = malloc(aCount);
    if (!*aDest) return NS_ERROR_OUT_OF_MEMORY;
  }
  ...
  while (1) {
    ...
    totalRead += bytesRead;
    if (totalRead == aCount) break;
    // if Read reads 0 bytes, we've hit EOF
    if (bytesRead == 0) return NS_ERROR_UNEXPECTED;
  }
  return rv;
}
...
{
  return NS_NewURI(
      result, NS_ConvertUTF16toUTF8(spec), charset, baseURI, ioService);
}
...
{
  return NS_NewURI(
      result, NS_ConvertUTF16toUTF8(spec), encoding, baseURI, ioService);
}
...
{
  return NS_NewURI(
      result, nsDependentCString(spec), nullptr, baseURI, ioService);
}
...
void
NS_QueryAuthPrompt2(nsIChannel* aChannel, nsIAuthPrompt2** aAuthPrompt)
{
  *aAuthPrompt = nullptr;
...
NS_NewNotificationCallbacksAggregation(nsIInterfaceRequestor* callbacks,
                                       ...
                                       nsIInterfaceRequestor** result)
{
  return NS_NewNotificationCallbacksAggregation(
      callbacks, loadGroup, nullptr, result);
}
...
NS_TryToMakeImmutable(nsIURI* uri, nsresult* outRv /* = nullptr */)
{
  nsresult rv;
  nsCOMPtr<nsINetUtil> util = do_GetNetUtil(&rv);
...
nsresult
NS_URIChainHasFlags(nsIURI* uri, uint32_t flags, bool* result)
{
  nsresult rv;
  nsCOMPtr<nsINetUtil> util = do_GetNetUtil(&rv);
  NS_ENSURE_SUCCESS(rv, rv);
...
nsresult
NS_MaybeOpenChannelUsingOpen2(nsIChannel* aChannel, nsIInputStream** aStream)
{
  nsCOMPtr<nsILoadInfo> loadInfo = aChannel->GetLoadInfo();
...
nsresult
NS_MaybeOpenChannelUsingAsyncOpen2(nsIChannel* aChannel,
...
NS_CheckIsJavaCompatibleURLString(nsCString& urlString, bool* result)
{
  *result = false;  // Default to "no"
  ...
  int32_t schemeLen = 0;
  urlParser->ParseURL(urlString.get(),
                      -1,
                      &schemePos,
                      &schemeLen,
                      nullptr,
                                   ...
                                   nsIChannel* aChan /* = nullptr */)
{
  ...
  rv = mimehdrpar->GetParameterHTTP(
      aHeader, "", EmptyCString(), true, nullptr, dispToken);
                              ...
                              nsIURI* aURI /* = nullptr */)
{
  ...
  rv = mimehdrpar->GetParameterHTTP(
      aDisposition, "filename", EmptyCString(), true, nullptr, aFilename);
...
nsresult
NS_GenerateHostPort(const nsCString& host, int32_t port, nsACString& hostLine)
{
  if (strchr(host.get(), ':')) {
...
void
NS_SniffContent(const char* aSnifferType,
                nsIRequest* aRequest,
                const uint8_t* aData,
                uint32_t aLength,
                nsACString& aSniffedType)
{
  ...
  for (int32_t i = 0; i < sniffers.Count(); ++i) {
    nsresult rv = sniffers[i]->GetMIMETypeFromContent(
        aRequest, aData, aLength, aSniffedType);
