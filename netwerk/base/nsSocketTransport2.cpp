...
NS_IMETHODIMP
nsSocketInputStream::ReadSegments(nsWriteSegmentFun writer,
                                  void* closure,
                                  uint32_t count,
                                  uint32_t* countRead)
{
  ...
  bool hasError = false;
  {
    ...
    if (callback && target) {
      ...
      mCallback = NS_NewInputStreamReadyEvent(
          "nsSocketInputStream::AsyncWait", callback, target);
    } else
      mCallback = callback;
...
NS_IMETHODIMP
nsSocketOutputStream::Write(const char* buf,
                            uint32_t count,
...
NS_IMETHODIMP
nsSocketOutputStream::WriteSegments(nsReadSegmentFun reader,
                                    void* closure,
                                    uint32_t count,
                                    uint32_t* countRead)
{
...
NS_IMETHODIMP
nsSocketOutputStream::WriteFrom(nsIInputStream* stream,
                                uint32_t count,
                                uint32_t* countRead)
{
...
nsresult
nsSocketTransport::InitPreResolved(const char** socketTypes,
                                   uint32_t typeCount,
                                   const nsACString& host,
                                   uint16_t port,
                                   const nsACString& hostRoute,
...
{
  nsresult rv =
      Init(socketTypes, typeCount, host, port, hostRoute, portRoute, proxyInfo);
...
nsSocketTransport::ResolveHost()
{
  ...
  rv = dns->AsyncResolveExtendedNative(SocketHost(),
                                       dnsFlags,
                                       mNetworkInterfaceId,
                                       this,
                                       nullptr,
...
nsresult
nsSocketTransport::BuildSocket(PRFileDesc*& fd,
...
nsSocketTransport::InitiateSocket()
{
  ...
  if (mConnectionFlags & nsISocketTransport::DISABLE_RFC1918 &&
    ...
    if (SOCKET_LOG_ENABLED()) {
      ...
      if (!NetAddrToString(
              &mNetAddr, netAddrCString.BeginWriting(), kIPv6CStrBufSize))
...
{
 public:
  explicit ThunkPRClose(PRFileDesc* fd) : Runnable("net::ThunkPRClose"), mFD(fd)
  {
  }
...
void
nsSocketTransport::OnSocketEvent(uint32_t type,
                                 nsresult status,
                                 nsISupports* param)
{
  SOCKET_LOG(
...
#define TCP_FASTOPEN 15
    ...
    if (mFDFastOpenInProgress && mFastOpenCallback &&
      ...
      PRInt32 rv = getsockopt(
          (SOCKET)osfd, IPPROTO_TCP, TCP_FASTOPEN, (char*)&option, &len);
      if ((rv != 0) && !option) {
...
{
  SOCKET_LOG((
      "nsSocketTransport::OpenOutputStream [this=%p flags=%x]\n", this, flags));
...
nsSocketTransport::SetSecurityCallbacks(nsIInterfaceRequestor* callbacks)
{
  ...
  NS_NewNotificationCallbacksAggregation(callbacks,
                                         nullptr,
...
nsSocketTransport::SetKeepaliveVals(int32_t aIdleTime, int32_t aRetryInterval)
{
  ...
  SOCKET_LOG(
      ("nsSocketTransport::SetKeepaliveVals [%p] "
       ...
       "packet count[%d]",
       this,
       mKeepaliveEnabled ? "enabled" : "disabled",
       mKeepaliveIdleTimeS,
       mKeepaliveRetryIntervalS,
       mKeepaliveProbeCount));
...
static void
DumpBytesToFile(const char* path,
                const char* header,
                const char* buf,
                int32_t n)
{
...
static void
LogNSPRError(const char* aPrefix, const void* aObjPtr)
{
#if defined(DEBUG)
  ...
  NS_WARNING(
      nsPrintfCString("%s [%p] NSPR error[0x%x] %s.",
                      aPrefix ? aPrefix : "nsSocketTransport",
                      aObjPtr,
                      errCode,
...
static void
LogOSError(const char* aPrefix, const void* aObjPtr)
{
#if defined(DEBUG)
  MOZ_ASSERT(OnSocketThread(), "not on socket thread");
                                                        ...
                                                        int aProbeCount)
{
  ...
  int err = setsockopt(
      sock, IPPROTO_TCP, TCP_KEEPALIVE, &aIdleTime, sizeof(aIdleTime));
  if (NS_WARN_IF(err)) {
...
// build errors will tell us if they are not.
  ...
  int err = setsockopt(
      sock, IPPROTO_TCP, TCP_KEEPIDLE, &aIdleTime, sizeof(aIdleTime));
  if (NS_WARN_IF(err)) {
  ...
  err = setsockopt(
      sock, IPPROTO_TCP, TCP_KEEPCNT, &aProbeCount, sizeof(aProbeCount));
  if (NS_WARN_IF(err)) {
...
nsSocketTransport::CloseSocket(PRFileDesc* aFd, nsSocketTransportService* aSTS)
{
  ...
  if (aSTS->HasFileDesc2PlatformOverlappedIOHandleFunc()) {
    ...
    if (aSTS->CallFileDesc2PlatformOverlappedIOHandleFunc(aFd, (void**)&ol) ==
        PR_SUCCESS) {
      SOCKET_LOG(
          ("nsSocketTransport::CloseSocket - we have an overlapped "
           "structure=%p aFd=%p\n",
           ol,
           aFd));
    ...
    SOCKET_LOG(
        ("nsSocketTransport::CloseSocket - there is no "
  ...
  if (canClose) {
    ...
    if (telemetryEnabled) {
      Telemetry::ScalarAdd(
          Telemetry::ScalarID::NETWORK_TCP_OVERLAPPED_RESULT_DELAYED, 1);
    }
...
void
nsSocketTransport::SendPRBlockingTelemetry(
