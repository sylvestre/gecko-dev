...
NS_IMPL_CLASSINFO(nsBufferedInputStream,
                  nullptr,
...
nsresult
nsBufferedInputStream::Create(nsISupports* aOuter,
                              REFNSIID aIID,
                              void** aResult)
{
...
NS_IMETHODIMP
nsBufferedInputStream::ReadSegments(nsWriteSegmentFun writer,
                                    void* closure,
                                    uint32_t count,
                                    uint32_t* result)
{
  *result = 0;
  ...
  while (count > 0) {
    ...
    if (amt > 0) {
      uint32_t read = 0;
      rv = writer(static_cast<nsIBufferedInputStream*>(this),
                  closure,
                  mBuffer + mCursor,
                  *result,
                  amt,
                                 ...
                                 FileDescriptorArray& aFileDescriptors)
{
  ...
  if (mStream) {
    ...
    InputStreamHelper::SerializeInputStream(
        stream, wrappedParams, aFileDescriptors);
...
nsresult
nsBufferedOutputStream::Create(nsISupports* aOuter,
                               REFNSIID aIID,
                               void** aResult)
{
  NS_ENSURE_NO_AGGREGATION(aOuter);
...
NS_IMETHODIMP
nsBufferedOutputStream::WriteFrom(nsIInputStream* inStr,
                                  uint32_t count,
                                  uint32_t* _retval)
{
...
NS_IMETHODIMP
nsBufferedOutputStream::WriteSegments(nsReadSegmentFun reader,
                                      void* closure,
                                      uint32_t count,
                                      uint32_t* _retval)
{
