...
nsFileStreamBase::~nsFileStreamBase() { Close(); }
...
nsresult
nsFileStreamBase::WriteFrom(nsIInputStream* inStr,
                            uint32_t count,
                            uint32_t* _retval)
{
...
nsresult
nsFileStreamBase::MaybeOpen(nsIFile* aFile,
                            int32_t aIoFlags,
                            int32_t aPerm,
                            bool aDeferred)
{
...
nsFileStreamBase::DoOpen()
{
  ...
  if (mBehaviorFlags & nsIFileInputStream::SHARE_DELETE) {
    ...
    rv = file->OpenNSPRFileDescShareDelete(
        mOpenParams.ioFlags, mOpenParams.perm, &fd);
  } else
#endif  // XP_WIN
  {
    rv = mOpenParams.localFile->OpenNSPRFileDesc(
        mOpenParams.ioFlags, mOpenParams.perm, &fd);
  }
...
NS_IMPL_CLASSINFO(nsFileInputStream,
                  nullptr,
...
nsFileInputStream::Open(nsIFile* aFile, int32_t aIOFlags, int32_t aPerm)
{
  ...
  return MaybeOpen(
      aFile, aIOFlags, aPerm, mBehaviorFlags & nsIFileInputStream::DEFER_OPEN);
}

NS_IMETHODIMP
nsFileInputStream::Init(nsIFile* aFile,
                        int32_t aIOFlags,
                        int32_t aPerm,
...
nsFileInputStream::Clone(nsIInputStream** aResult)
{
  ...
  return NS_NewLocalFileInputStream(
      aResult, mFile, mIOFlags, mPerm, mBehaviorFlags);
}
...
NS_IMETHODIMP
nsFileOutputStream::Init(nsIFile* file,
                         int32_t ioFlags,
                         int32_t perm,
                         int32_t behaviorFlags)
{
  ...
  return MaybeOpen(
      file, ioFlags, perm, mBehaviorFlags & nsIFileOutputStream::DEFER_OPEN);
}
...
NS_IMETHODIMP
nsAtomicFileOutputStream::Init(nsIFile* file,
                               int32_t ioFlags,
                               int32_t perm,
                               int32_t behaviorFlags)
{
...
NS_IMETHODIMP
nsAtomicFileOutputStream::Write(const char* buf,
                                uint32_t count,
                                uint32_t* result)
{
...
NS_IMETHODIMP
nsFileStream::Init(nsIFile* file,
                   int32_t ioFlags,
                   int32_t perm,
                   int32_t behaviorFlags)
{
  ...
  return MaybeOpen(
      file, ioFlags, perm, mBehaviorFlags & nsIFileStream::DEFER_OPEN);
}
