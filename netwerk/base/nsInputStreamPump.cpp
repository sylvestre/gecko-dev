...
static nsresult
CallPeekFunc(nsIInputStream* aInStream,
             void* aClosure,
             ...
             uint32_t* aWriteCount)
{
  ...
  data->mFunc(
      data->mClosure, reinterpret_cast<const uint8_t*>(aFromSegment), aCount);
...
nsInputStreamPump::PeekStream(PeekSegmentFun callback, void* closure)
{
  ...
  return mBufferedStream->ReadSegments(
      CallPeekFunc, &data, nsIOService::gDefaultSegmentSize, &dummy);
}
...
nsInputStreamPump::AsyncRead(nsIStreamListener* listener, nsISupports* ctxt)
{
  ...
  if (!mAsyncStream) {
    ...
    rv = sts->CreateInputTransport(
        mStream, mCloseWhenDone, getter_AddRefs(transport));
    if (NS_FAILED(rv)) return rv;

    nsCOMPtr<nsIInputStream> wrapper;
    rv = transport->OpenInputStream(
        0, mSegSize, mSegCount, getter_AddRefs(wrapper));
    if (NS_FAILED(rv)) return rv;
  ...
  if (rv == NS_BASE_STREAM_CLOSED) {
...

    {
      ...
      rv = mListener->OnDataAvailable(
          this, mListenerContext, mBufferedStream, mStreamOffset, odaAvail);
    }
...
nsInputStreamPump::CreateBufferedStreamIfNeeded()
{
  ...
  nsresult rv = NS_NewBufferedInputStream(
      getter_AddRefs(mBufferedStream), stream.forget(), 4096);
  NS_ENSURE_SUCCESS(rv, rv);
...
nsInputStreamPump::RetargetDeliveryTo(nsIEventTarget* aNewTarget)
{
  ...
  LOG(
      ("nsInputStreamPump::RetargetDeliveryTo [this=%p aNewTarget=%p] "
       ...
       this,
       aNewTarget,
       (mTargetThread == aNewTarget ? "success" : "failure"),
