...
InitGlobals()
{
  ...
  Preferences::AddIntVarCache(
      &gMaxLength, "network.standard-url.max-length", 1048576);
}
                 ...
                 nsACString& outFileExtension)
{
  ...
  rv = parser->ParseURL(url,
                        flatURL.Length(),
                        nullptr,
                        nullptr,  // don't care about scheme
                        nullptr,
                        nullptr,  // don't care about authority
                        &pathPos,
                        &pathLen);
  if (NS_FAILED(rv)) return rv;

  // invoke the parser to extract filepath from the path
  rv = parser->ParsePath(url + pathPos,
                         pathLen,
                         &filepathPos,
                         &filepathLen,
                         nullptr,
                       ...
                       nsACString& aResult)
{
  ...
  const ASCIIMaskArray& mask = ASCIIMask::MaskCRLFTab();
  return NS_EscapeAndFilterURL(
      Substring(newStart, newEnd), aFlags, &mask, aResult, fallible);
}
                     ...
                     bool* aHadCharset)
{
  ...
  net_ParseContentType(
      aHeaderStr, aContentType, aContentCharset, aHadCharset, &dummy1, &dummy2);
}
...
net_IsValidHostName(const nsACString& host)
{
  ...
  if (net_FindCharNotInSet(host.BeginReading(),
                           end,
