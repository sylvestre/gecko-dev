                                        ...
                                        nsIInputStream** result)
{
  ...
  rv = NS_AsyncCopy(
      this, pipeOut, target, NS_ASYNCCOPY_VIA_WRITESEGMENTS, segsize);
                                     ...
                                     nsIEventTarget* target)
{
  ...
  if (target)
    return net_NewTransportEventSinkProxy(
        getter_AddRefs(mEventSink), sink, target);
...
NS_IMETHODIMP
nsInputStreamTransport::ReadSegments(nsWriteSegmentFun writer,
                                     void* closure,
                                     uint32_t count,
                                     uint32_t* result)
{
...
NS_IMETHODIMP
nsStreamTransportService::Observe(nsISupports* subject,
                                  const char* topic,
                                  const char16_t* data)
{
