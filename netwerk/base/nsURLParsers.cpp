...
NS_IMETHODIMP
nsBaseURLParser::ParseURL(const char* spec,
                          int32_t specLen,
                          uint32_t* schemePos,
                          int32_t* schemeLen,
                          ...
                          int32_t* pathLen)
{
  ...
  if (colon && (colon < slash || !slash)) {
    ...
    if (authorityLen || pathLen) {
      ParseAfterScheme(
          spec, specLen, authorityPos, authorityLen, pathPos, pathLen);
      OFFSET_RESULT(authority, offset);
      OFFSET_RESULT(path, offset);
    }
  }
...
NS_IMETHODIMP
nsBaseURLParser::ParseAuthority(const char* auth,
                                int32_t authLen,
...
NS_IMETHODIMP
nsBaseURLParser::ParseUserInfo(const char* userinfo,
                               int32_t userinfoLen,
                               uint32_t* usernamePos,
                               int32_t* usernameLen,
                               uint32_t* passwordPos,
...
NS_IMETHODIMP
nsBaseURLParser::ParseServerInfo(const char* serverinfo,
...
NS_IMETHODIMP
nsBaseURLParser::ParsePath(const char* path,
                           int32_t pathLen,
                           uint32_t* filepathPos,
                           int32_t* filepathLen,
                           uint32_t* queryPos,
                           int32_t* queryLen,
                           uint32_t* refPos,
...
NS_IMETHODIMP
nsBaseURLParser::ParseFilePath(const char* filepath,
                               int32_t filepathLen,
                               uint32_t* directoryPos,
...
nsresult
nsBaseURLParser::ParseFileName(const char* filename,
                               int32_t filenameLen,
                               uint32_t* basenamePos,
                               int32_t* basenameLen,
                               uint32_t* extensionPos,
  ...
  if (filename[filenameLen - 1] != '.') {
    ...
    for (const char* p = filename + filenameLen - 1; p > filename; --p) {
      if (*p == '.') {
        ...
        SET_RESULT(
            extension, p + 1 - filename, filenameLen - (p - filename + 1));
        return NS_OK;
      }
    }
  }
...
NS_IMETHODIMP
nsNoAuthURLParser::ParseAuthority(const char* auth,
                                  int32_t authLen,
                                  uint32_t* usernamePos,
                                  int32_t* usernameLen,
                                  uint32_t* passwordPos,
...
void
nsNoAuthURLParser::ParseAfterScheme(const char* spec,
                                    int32_t specLen,
                                    uint32_t* authPos,
                                    int32_t* authLen,
                                    uint32_t* pathPos,
...
#if defined(XP_WIN)
NS_IMETHODIMP
nsNoAuthURLParser::ParseFilePath(const char* filepath,
                                 int32_t filepathLen,
                                 uint32_t* directoryPos,
                                 int32_t* directoryLen,
                                 uint32_t* basenamePos,
                                 ...
                                 int32_t* extensionLen)
{
  ...
  return nsBaseURLParser::ParseFilePath(filepath,
                                        filepathLen,
                                        directoryPos,
                                        directoryLen,
                                        basenamePos,
...
NS_IMETHODIMP
nsAuthURLParser::ParseAuthority(const char* auth,
                                int32_t authLen,
                                uint32_t* usernamePos,
                                int32_t* usernameLen,
                                uint32_t* passwordPos,
                                ...
                                int32_t* port)
{
  ...
  if (*p == '@') {
    ...
    rv = ParseUserInfo(
        auth, p - auth, usernamePos, usernameLen, passwordPos, passwordLen);
    if (NS_FAILED(rv)) return rv;
    rv = ParseServerInfo(
        p + 1, authLen - (p - auth + 1), hostnamePos, hostnameLen, port);
    if (NS_FAILED(rv)) return rv;
...
NS_IMETHODIMP
nsAuthURLParser::ParseUserInfo(const char* userinfo,
                               int32_t userinfoLen,
                               uint32_t* usernamePos,
                               int32_t* usernameLen,
                               uint32_t* passwordPos,
...
NS_IMETHODIMP
nsAuthURLParser::ParseServerInfo(const char* serverinfo,
                                 int32_t serverinfoLen,
                                 uint32_t* hostnamePos,
                                 int32_t* hostnameLen,
                                 int32_t* port)
...
void
nsAuthURLParser::ParseAfterScheme(const char* spec,
                                  int32_t specLen,
                                  uint32_t* authPos,
                                  int32_t* authLen,
                                  uint32_t* pathPos,
...
void
nsStdURLParser::ParseAfterScheme(const char* spec,
                                 int32_t specLen,
                                 uint32_t* authPos,
                                 int32_t* authLen,
                                 uint32_t* pathPos,
