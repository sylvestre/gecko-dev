...
class nsFileStreamBase : public nsISeekableStream, public nsIFileMetadata
{
 ...
 protected:
  ...
  nsresult WriteFrom(nsIInputStream* aFromStream,
                     uint32_t aCount,
                     uint32_t* _retval);
  nsresult WriteSegments(nsReadSegmentFun aReader,
                         void* aClosure,
                         uint32_t aCount,
                         uint32_t* _retval);
  ...
  /**
     ...
     * DoPendingOpen().
     */
  nsresult MaybeOpen(nsIFile* aFile,
                     int32_t aIoFlags,
                     int32_t aPerm,
                     bool aDeferred);
                          ...
                          public nsICloneableInputStream
{
 ...
 protected:
  virtual ~nsFileInputStream() { Close(); }

  nsresult SeekInternal(int32_t aWhence,
                        int64_t aOffset,
                        bool aClearBuf = true);
...
class nsFileOutputStream : public nsFileStreamBase, public nsIFileOutputStream
{
 ...
 protected:
  virtual ~nsFileOutputStream() { Close(); }
};
                                 ...
                                 public nsISafeOutputStream
{
 ...
 protected:
  virtual ~nsAtomicFileOutputStream() { Close(); }
