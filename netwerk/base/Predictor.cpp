...
Predictor::Action::Action(bool fullUri,
                          bool predict,
...
NS_IMETHODIMP
Predictor::Action::OnCacheEntryAvailable(nsICacheEntry* entry,
                                         bool isNew,
                                         nsIApplicationCache* appCache,
                                         nsresult result)
{
  ...
  if (mPredict) {
    bool predicted = mPredictor->PredictInternal(mPredictReason,
                                                 entry,
                                                 isNew,
                                                 mFullUri,
                                                 mTargetURI,
    ...
    mPredictor->LearnInternal(
        mLearnReason, entry, isNew, mFullUri, mTargetURI, mSourceURI);
...
Predictor::InstallObserver()
{
  ...
  Preferences::AddBoolVarCache(
      &mEnableHoverOnSSL, PREDICTOR_SSL_HOVER_PREF, false);
  ...
  Preferences::AddUintVarCache(&mMaxURILength,
...
NS_IMETHODIMP
Predictor::Observe(nsISupports* subject,
                   const char* topic,
...
Predictor::Init()
{
  ...
  rv = NS_NewURI(
      getter_AddRefs(mStartupURI), "predictor://startup", nullptr, mIOService);
  NS_ENSURE_SUCCESS(rv, rv);
...
{
 public:
  PredictorLearnRunnable(nsIURI* targetURI,
                         nsIURI* sourceURI,
...
nsresult
Predictor::Create(nsISupports* aOuter, const nsIID& aIID, void** aResult)
{
  MOZ_ASSERT(NS_IsMainThread());
...
NS_IMETHODIMP
Predictor::Predict(nsIURI* targetURI,
                   nsIURI* sourceURI,
...
NS_IMETHODIMP
Predictor::PredictNative(nsIURI* targetURI,
                         nsIURI* sourceURI,
                         PredictorPredictReason reason,
                         ...
                         nsINetworkPredictorVerifier* verifier)
{
  ...
  if (IsNeckoChild()) {
    ...
    gNeckoChild->SendPredPredict(
        serTargetURI, serSourceURI, reason, originAttributes, verifier);
    return NS_OK;
  }
  ...
  PREDICTOR_LOG(("    Predict uri=%s reason=%d action=%p",
                 uriKeyStr.get(),
                 reason,
                 uriAction.get()));
...
bool
Predictor::PredictInternal(PredictorPredictReason reason,
                           nsICacheEntry* entry,
                           bool isNew,
                           bool fullUri,
                           nsIURI* targetURI,
...
void
Predictor::PredictForLink(nsIURI* targetURI,
                          nsIURI* sourceURI,
                          const OriginAttributes& originAttributes,
                          nsINetworkPredictorVerifier* verifier)
{
...
bool
Predictor::PredictForPageload(nsICacheEntry* entry,
                              nsIURI* targetURI,
                              uint8_t stackCount,
                              bool fullUri,
                              nsINetworkPredictorVerifier* verifier)
{
  ...
  if (WouldRedirect(entry,
    ...
    PREDICTOR_LOG(("    Predict redirect uri=%s action=%p",
                   redirectUriString.get(),
                   redirectAction.get()));
    uint32_t openFlags =
        nsICacheStorage::OPEN_READONLY | nsICacheStorage::OPEN_SECRETLY |
        nsICacheStorage::OPEN_PRIORITY | nsICacheStorage::CHECK_MULTITHREADED;
    cacheDiskStorage->AsyncOpenURI(
        redirectURI, EmptyCString(), openFlags, redirectAction);
    return RunPredictions(nullptr, *lci->OriginAttributesPtr(), verifier);
  }

  CalculatePredictions(
      entry, targetURI, lastLoad, loadCount, globalDegradation, fullUri);
...
int32_t
Predictor::CalculateConfidence(uint32_t hitCount,
...
static void
MakeMetadataEntry(const uint32_t hitCount,
...
void
Predictor::UpdateRollingLoadCount(nsICacheEntry* entry,
                                  const uint32_t flags,
                                  const char* key,
...
void
Predictor::CalculatePredictions(nsICacheEntry* entry,
                                nsIURI* referrer,
                                uint32_t lastLoad,
                                uint32_t loadCount,
                                int32_t globalDegradation,
...
void
Predictor::SetupPrediction(int32_t confidence,
                           uint32_t flags,
                           const nsCString& uri)
{
  MOZ_ASSERT(NS_IsMainThread());
  ...
  if (mEnablePrefetch && (flags & FLAG_PREFETCHABLE) &&
    ...
    rv = NS_NewURI(
        getter_AddRefs(prefetchURI), uri, nullptr, nullptr, mIOService);
    if (NS_SUCCEEDED(rv)) {
    ...
    rv = NS_NewURI(
        getter_AddRefs(preconnectURI), uri, nullptr, nullptr, mIOService);
    if (NS_SUCCEEDED(rv)) {
    ...
    rv = NS_NewURI(
        getter_AddRefs(preresolveURI), uri, nullptr, nullptr, mIOService);
    if (NS_SUCCEEDED(rv)) {
...
bool
Predictor::WouldRedirect(nsICacheEntry* entry,
                         uint32_t loadCount,
                         uint32_t lastLoad,
                         int32_t globalDegradation,
                         nsIURI** redirectURI)
...
NS_IMETHODIMP
Predictor::Learn(nsIURI* targetURI,
                 nsIURI* sourceURI,
                 PredictorLearnReason reason,
                 JS::HandleValue originAttributes,
                 JSContext* aCx)
...
NS_IMETHODIMP
Predictor::LearnNative(nsIURI* targetURI,
                       nsIURI* sourceURI,
                       PredictorLearnReason reason,
                       const OriginAttributes& originAttributes)
{
  ...
  cacheDiskStorage->AsyncOpenURI(
      uriKey, EmptyCString(), uriOpenFlags, uriAction);
...
void
Predictor::LearnInternal(PredictorLearnReason reason,
                         nsICacheEntry* entry,
                         bool isNew,
                         bool fullUri,
                         nsIURI* targetURI,
...
Predictor::LearnForSubresource(nsICacheEntry* entry, nsIURI* targetURI)
{
  ...
  uint32_t hitCount, lastHit, flags;
  bool isNewResource =
      (NS_FAILED(rv) || !ParseMetaDataEntry(key.BeginReading(),
                                            value.BeginReading(),
                                            uri,
                                            hitCount,
                                            lastHit,
...
void
Predictor::MaybeLearnForStartup(nsIURI* uri,
                                bool fullUri,
                                const OriginAttributes& originAttributes)
{
  MOZ_ASSERT(NS_IsMainThread());
...
bool
Predictor::ParseMetaDataEntry(const char* key,
                              const char* value,
                              nsCString& uri,
                              uint32_t& hitCount,
                              uint32_t& lastHit,
...
NS_IMETHODIMP
Predictor::Resetter::OnCacheEntryAvailable(nsICacheEntry* entry,
                                           bool isNew,
                                           nsIApplicationCache* appCache,
                                           nsresult result)
{
...
NS_IMETHODIMP
Predictor::Resetter::OnCacheEntryInfo(nsIURI* uri,
                                      const nsACString& idEnhance,
                                      int64_t dataSize,
                                      int32_t fetchCount,
...
Predictor::Resetter::OnCacheEntryVisitCompleted()
{
  ...
  for (uint32_t i = 0; i < entriesToVisit; ++i) {
    ...
    cacheDiskStorage->AsyncOpenURI(urisToVisit[i],
                                   EmptyCString(),
...
nsresult
PredictorPredict(nsIURI* targetURI,
                 nsIURI* sourceURI,
                 PredictorPredictReason reason,
                 const OriginAttributes& originAttributes,
                 nsINetworkPredictorVerifier* verifier)
{
  ...
  return predictor->PredictNative(
      targetURI, sourceURI, reason, originAttributes, verifier);
}

nsresult
PredictorLearn(nsIURI* targetURI,
               nsIURI* sourceURI,
               PredictorLearnReason reason,
               const OriginAttributes& originAttributes)
{
...
nsresult
PredictorLearnRedirect(nsIURI* targetURI,
                       nsIChannel* channel,
                       const OriginAttributes& originAttributes)
{
  MOZ_ASSERT(NS_IsMainThread());
  ...
  return predictor->LearnNative(targetURI,
                                sourceURI,
                                             ...
                                             const uint32_t aCount)
{
  ...
  return aInputStream->ReadSegments(
      NS_DiscardSegment, nullptr, aCount, &result);
}

// Miscellaneous Predictor

void
Predictor::UpdateCacheability(nsIURI* sourceURI,
                              nsIURI* targetURI,
                              uint32_t httpStatus,
                              ...
                              bool isTracking)
{
  ...
  if (self) {
    ...
    self->UpdateCacheabilityInternal(sourceURI,
                                     targetURI,
                                     httpStatus,
                                     method,
...
void
Predictor::UpdateCacheabilityInternal(nsIURI* sourceURI,
                                      nsIURI* targetURI,
                                      uint32_t httpStatus,
                                      ...
                                      bool couldVary)
{
  ...
  RefPtr<Predictor::CacheabilityAction> action =
      new Predictor::CacheabilityAction(
          targetURI, httpStatus, method, isTracking, couldVary, this);
  nsAutoCString uri;
  targetURI->GetAsciiSpec(uri);
    ...
    nsresult result)
{
  ...
  for (size_t i = 0; i < keysToCheck.Length(); ++i) {
    ...
    if (!mPredictor->ParseMetaDataEntry(
            key, value, uri, hitCount, lastHit, flags)) {
    ...
    if (strTargetURI.Equals(uri)) {
      if (mHttpStatus == 200 && mMethod.EqualsLiteral("GET") &&
          !hasQueryString && !mIsTracking && !mCouldVary) {
