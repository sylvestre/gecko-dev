...
static nsresult
ResolveHost(const nsACString& host,
            ...
            nsIDNSListener* listener)
{
  ...
  return dns->AsyncResolveNative(host,
                                 0,
                                 listener,
                                 nullptr,
                                 aOriginAttributes,
...
class SetSocketOptionRunnable : public Runnable
{
 public:
  SetSocketOptionRunnable(nsUDPSocket* aSocket, const PRSocketOptionData& aOpt)
      : Runnable("net::SetSocketOptionRunnable"), mSocket(aSocket), mOpt(aOpt)
  {
  }
...
nsUDPOutputStream::nsUDPOutputStream(nsUDPSocket* aSocket,
    ...
    : mSocket(aSocket), mFD(aFD), mPrClientAddr(aPrClientAddr), mIsClosed(false)
{
}
...
NS_IMETHODIMP
nsUDPOutputStream::Flush()
{
  return NS_OK;
}

NS_IMETHODIMP
nsUDPOutputStream::Write(const char* aBuf, uint32_t aCount, uint32_t* _retval)
{
  if (mIsClosed) return NS_BASE_STREAM_CLOSED;
...
NS_IMETHODIMP
nsUDPOutputStream::WriteFrom(nsIInputStream* aFromStream,
                             uint32_t aCount,
                             uint32_t* _retval)
{
...
nsUDPSocket::OnSocketReady(PRFileDesc* fd, int16_t outFlags)
{
  ...
  rv = NS_AsyncCopy(
      pipeIn, os, mSts, NS_ASYNCCOPY_VIA_READSEGMENTS, UDP_PACKET_CHUNK_SIZE);
...
NS_IMETHODIMP
nsUDPSocket::Init(int32_t aPort,
                  bool aLoopbackOnly,
...
NS_IMETHODIMP
nsUDPSocket::Init2(const nsACString& aAddr,
                   int32_t aPort,
                   nsIPrincipal* aPrincipal,
                   bool aAddressReuse,
                   uint8_t aOptionalArgc)
...
class PendingSend : public nsIDNSListener
{
 public:
  ...
  PendingSend(nsUDPSocket* aSocket,
              uint16_t aPort,
              FallibleTArray<uint8_t>& aData)
                              ...
                              nsresult status)
{
  ...
  if (NS_SUCCEEDED(rec->GetNextAddr(mPort, &addr))) {
    uint32_t count;
    nsresult rv = mSocket->SendWithAddress(
        &addr, mData.Elements(), mData.Length(), &count);
...
class PendingSendStream : public nsIDNSListener
{
 public:
  ...
  PendingSendStream(nsUDPSocket* aSocket,
                    uint16_t aPort,
...
NS_IMETHODIMP
nsUDPSocket::Send(const nsACString& aHost,
                  uint16_t aPort,
                  const uint8_t* aData,
...
NS_IMETHODIMP
nsUDPSocket::SendWithAddr(nsINetAddr* aAddr,
                          const uint8_t* aData,
                          uint32_t aDataLength,
                          uint32_t* _retval)
{
                             ...
                             uint32_t* _retval)
{
  ...
  *_retval = 0;
  ...
  if (onSTSThread) {
    ...
    int32_t count = PR_SendTo(mFD,
                              aData,
                              sizeof(uint8_t) * aDataLength,
                              0,
                              &prAddr,
                              PR_INTERVAL_NO_WAIT);
    if (count < 0) {
...
NS_IMETHODIMP
nsUDPSocket::SendBinaryStream(const nsACString& aHost,
                              uint16_t aPort,
                              nsIInputStream* aStream)
{
                                         ...
                                         nsIInputStream* aStream)
{
  ...
  return NS_AsyncCopy(
      aStream, os, mSts, NS_ASYNCCOPY_VIA_READSEGMENTS, UDP_PACKET_CHUNK_SIZE);
}
