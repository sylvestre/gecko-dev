...
nsIOService::Init()
{
  ...
  Preferences::AddBoolVarCache(
      ...
      false);
  Preferences::AddBoolVarCache(
      &mOfflineMirrorsConnectivity, OFFLINE_MIRRORS_CONNECTIVITY, true);
...
nsIOService::~nsIOService() { gIOService = nullptr; }
...
nsresult
nsIOService::AsyncOnChannelRedirect(nsIChannel* oldChan,
                                    nsIChannel* newChan,
                                    uint32_t flags,
...
nsresult
nsIOService::CacheProtocolHandler(const char* scheme,
...
nsresult
nsIOService::GetCachedProtocolHandler(const char* scheme,
...
nsresult
nsIOService::NewURI(const nsACString& aSpec,
                    const char* aCharset,
                    nsIURI* aBaseURI,
                    nsIURI** result)
{
                                                     ...
                                                     nsIChannel** result)
{
  ...
  if (pph) {
    ...
    if (NS_FAILED(rv)) {
      rv = pph->NewProxiedChannel(
          aURI, nullptr, aProxyFlags, aProxyURI, getter_AddRefs(channel));
    ...
    nsIChannel** result)
{
  ...
  return NewChannelFromURIWithProxyFlagsInternal(
      aURI, aProxyURI, aProxyFlags, loadInfo, result);
}
...
{
  NS_ASSERTION(false,
               "Deprecated, use NewChannelFromURIWithProxyFlags2 providing "
               "loadInfo arguments!");
  ...
  return NewChannelFromURIWithProxyFlags2(
      aURI,
      aProxyURI,
      aProxyFlags,
      nullptr,  // aLoadingNode
 ...
 */
NS_IMETHODIMP
nsIOService::NewChannel(const nsACString& aSpec,
                        const char* aCharset,
                        nsIURI* aBaseURI,
                        nsIChannel** result)
{
...
void
nsIOService::ParsePortList(nsIPrefBranch* prefBranch,
                           const char* pref,
                           bool remove)
{
  nsCString portList;
  ...
  if (!portList.IsVoid()) {
    ...
    for (index = 0; index < portListArray.Length(); index++) {
      ...
      if (PR_sscanf(
              portListArray[index].get(), "%d-%d", &portBegin, &portEnd) == 2) {
...
nsIOService::NotifyWakeup()
{
  ...
  if (observerService && mNetworkNotifyChanged) {
    (void)observerService->NotifyObservers(
        nullptr, NS_NETWORK_LINK_TOPIC, (u"" NS_NETWORK_LINK_DATA_CHANGED));
  }
                       ...
                       nsACString& aResult)
{
  aResult.Truncate();
  NS_EscapeURL(
      aStr.BeginReading(), aStr.Length(), aFlags | esc_AlwaysCopy, aResult);
  return NS_OK;
}
                                           ...
                                           bool* aHadCharset)
{
  ...
  net_ParseContentType(
      aTypeHeader, ignored, aCharset, aHadCharset, aCharsetStart, aCharsetEnd);
                                         ...
                                         nsresult status)
{
  ...
  if (loadFlags & nsIRequest::LOAD_ANONYMOUS) {
    speculativeHandler->SpeculativeAnonymousConnect2(
        uri, principal, mCallbacks);
  } else {
                                        ...
                                        bool aAnonymous)
{
  ...
  if (IsNeckoChild()) {
    ...
    gNeckoChild->SendSpeculativeConnect(
        params, IPC::Principal(aPrincipal), aAnonymous);
    return NS_OK;
  }
  ...
  if (pps2) {
    return pps2->AsyncResolve2(
        channel, 0, callback, nullptr, getter_AddRefs(cancelable));
  }
  return pps->AsyncResolve(
      channel, 0, callback, nullptr, getter_AddRefs(cancelable));
}

NS_IMETHODIMP
nsIOService::SpeculativeConnect(nsIURI* aURI, nsIInterfaceRequestor* aCallbacks)
{
