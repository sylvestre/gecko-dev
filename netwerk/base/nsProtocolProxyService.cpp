                                    ...
                                    public nsICancelable
{
 public:
  ...
  nsAsyncResolveRequest(nsProtocolProxyService* pps,
                        nsIChannel* channel,
 ...
 private:
  ...
  void DoCallback()
  {
    ...
    if (NS_SUCCEEDED(mStatus) && !mProxyInfo && !mPACString.IsEmpty()) {
      mPPS->ProcessPACString(
          mPACString, mResolveFlags, getter_AddRefs(mProxyInfo));
      nsCOMPtr<nsIURI> proxyURI;
...
class AsyncGetPACURIRequest final : public nsIRunnable
{
 public:
  ...
  using CallbackFunc = nsresult (nsProtocolProxyService::*)(bool,
                                                            bool,
                                                            nsresult,
                                                            const nsACString&);
...
static void
proxy_GetBoolPref(nsIPrefBranch* aPrefBranch, const char* aPref, bool& aResult)
{
  bool temp;
...
NS_IMPL_CLASSINFO(nsProtocolProxyService,
                  nullptr,
                                     ...
                                     const char* pref)
{
  ...
  if (!pref || !strcmp(pref, PROXY_PREF("socks_remote_dns")))
    proxy_GetBoolPref(
        prefBranch, PROXY_PREF("socks_remote_dns"), mSOCKSProxyRemoteDNS);
  ...
  if (!pref || !strcmp(pref, PROXY_PREF("failover_timeout")))
    proxy_GetIntPref(
        prefBranch, PROXY_PREF("failover_timeout"), mFailedProxyTimeout);
...
{
  if (mHostFiltersArray.Length() == 0) return true;
...
nsProtocolProxyService::SetupPACThread(nsIEventTarget* mainThreadEventTarget)
{
  ...
  if (NS_FAILED(rv)) mPACMan = nullptr;
  return rv;
}
...
// nsIProtocolProxyService
NS_IMETHODIMP
nsProtocolProxyService::AsyncResolve2(nsIChannel* channel,
                                      uint32_t flags,
                                      nsIProtocolProxyCallback* callback,
                                      nsIEventTarget* mainThreadEventTarget,
                                      nsICancelable** result)
{
  return AsyncResolveInternal(
      channel, flags, callback, result, true, mainThreadEventTarget);
}

NS_IMETHODIMP
nsProtocolProxyService::AsyncResolve(nsISupports* channelOrURI,
                                     uint32_t flags,
                                     nsIProtocolProxyCallback* callback,
                                     nsIEventTarget* mainThreadEventTarget,
                                     nsICancelable** result)
{
  ...
  return AsyncResolveInternal(
      channel, flags, callback, result, false, mainThreadEventTarget);
}
...
{
  return NewProxyInfoWithAuth(aType,
                              aHost,
                              aPort,
                              EmptyCString(),
                              EmptyCString(),
...
nsProtocolProxyService::NewProxyInfoWithAuth(const nsACString& aType,
                                             ...
                                             nsIProxyInfo** aResult)
{
  static const char* types[] = {kProxyType_HTTP,
                                kProxyType_HTTPS,
                                kProxyType_SOCKS,
                                kProxyType_SOCKS4,
                                kProxyType_DIRECT};
  ...
  return NewProxyInfo_Internal(type,
                               aHost,
                               aPort,
                               aUsername,
                               aPassword,
...
nsProtocolProxyService::ApplyFilters(nsIChannel* channel,
                                     ...
                                     nsIProxyInfo** list)
{
  if (!(info.flags & nsIProtocolHandler::ALLOWS_PROXY)) return;
  ...
  for (FilterLink* iter = mFilters; iter; iter = iter->next) {
    ...
    if (iter->filter) {
      ...
      if (uri) {
        rv =
            iter->filter->ApplyFilter(this, uri, *list, getter_AddRefs(result));
      }
    } else if (iter->channelFilter) {
      rv = iter->channelFilter->ApplyFilter(
          this, channel, *list, getter_AddRefs(result));
    }
