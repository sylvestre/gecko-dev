...
#define CALL_RUST_SETTER(func, ...)                                            \
  do {                                                                         \
    ...
    if (mSpec != rustSpec) {                                                   \
      LOG((                                                                    \
          "Spec diff detected after setter (%s): rust: %s standard-url: %s\n", \
          #func,                                                               \
          rustSpec.get(),                                                      \
          mSpec.get()));                                                       \
    }                                                                          \
...
//
constexpr bool
TestForInvalidHostCharacters(char c)
{
  // Testing for these:
  // CONTROL_CHARACTERS " #/:?@[\\]*<>|\"";
...
NS_IMETHODIMP
nsStandardURL::nsPrefObserver::Observe(nsISupports* subject,
                                       const char* topic,
                                       const char16_t* data)
{
...
int32_t
nsStandardURL::nsSegmentEncoder::EncodeSegmentCount(const char* str,
...
const nsACString&
nsStandardURL::nsSegmentEncoder::EncodeSegment(const nsACString& str,
                                               int16_t mask,
                                               nsCString& result)
{
  const char* text;
  bool encoded;
  EncodeSegmentCount(str.BeginReading(text),
                     URLSegment(0, str.Length()),
                     mask,
                     result,
                     encoded);
...
nsStandardURL::InitGlobalObjects()
{
  ...
  Preferences::AddBoolVarCache(
      &gPunycodeHost, "network.standard-url.punycode-host", true);
...
inline nsresult
ParseIPv4Number(const nsACString& input,
                int32_t base,
                uint32_t& number,
                uint32_t maxNumber)
{
...
nsStandardURL::NormalizeIPv4(const nsACString& host, nsCString& result)
{
  ...
  result = nsPrintfCString("%d.%d.%d.%d",
                           ipSegments[0],
                           ipSegments[1],
                           ipSegments[2],
                           ipSegments[3]);
...
uint32_t
nsStandardURL::AppendSegmentToBuf(char* buf,
                                  uint32_t i,
                                  const char* str,
...
nsresult
nsStandardURL::BuildNormalizedSpec(const char* spec, const Encoding* encoding)
{
  ...
  // appropriate encoding.
  {
    ...
    // Username@
    approxLen += encoder.EncodeSegmentCount(
        spec, mUsername, esc_Username, encUsername, useEncUsername, 1);
    // :password - we insert the ':' even if there's no actual password if "user:@" was in the spec
    if (mPassword.mLen >= 0)
      approxLen +=
          1 + encoder.EncodeSegmentCount(
                  spec, mPassword, esc_Password, encPassword, useEncPassword);
    ...
    approxLen += encoder.EncodeSegmentCount(
        spec, mDirectory, esc_Directory, encDirectory, useEncDirectory, 1);
    approxLen += encoder.EncodeSegmentCount(
        spec, mBasename, esc_FileBaseName, encBasename, useEncBasename);
    approxLen += encoder.EncodeSegmentCount(
        spec, mExtension, esc_FileExtension, encExtension, useEncExtension, 1);

    // These next ones *always* add their leading character even if length is 0
    // Handles items like "http://#"
    // ?query
    if (mQuery.mLen >= 0)
      approxLen += 1 + queryEncoder.EncodeSegmentCount(
                           spec, mQuery, esc_Query, encQuery, useEncQuery);
    // #ref

    if (mRef.mLen >= 0) {
      approxLen += 1 + encoder.EncodeSegmentCount(
                           spec, mRef, esc_Ref, encRef, useEncRef);
    }
  }
  ...
  if (mHost.mLen > 0) {
    ...
    NS_UnescapeURL(
        spec + mHost.mPos, mHost.mLen, esc_AlwaysCopy | esc_Host, tempHost);
  ...
  // append authority
  if (mUsername.mLen > 0) {
    i = AppendSegmentToBuf(
        buf, i, spec, username, mUsername, &encUsername, useEncUsername, &diff);
    ShiftFromPassword(diff);
    if (password.mLen > 0) {
      buf[i++] = ':';
      i = AppendSegmentToBuf(buf,
                             i,
                             spec,
                             password,
                             mPassword,
  ...
  if (host.mLen > 0) {
    i = AppendSegmentToBuf(
        buf, i, spec, host, mHost, &encHost, useEncHost, &diff);
    ShiftFromPath(diff);
  ...
  if (mPath.mLen <= 0) {
    ...
    i = AppendSegmentToBuf(
        buf, i, spec, basename, mBasename, &encBasename, useEncBasename, &diff);
    ...
    if (mQuery.mLen >= 0) {
      buf[i++] = '?';
      i = AppendSegmentToBuf(
          buf, i, spec, query, mQuery, &encQuery, useEncQuery, &diff);
      ShiftFromRef(diff);
    }
    if (mRef.mLen >= 0) {
      buf[i++] = '#';
      i = AppendSegmentToBuf(
          buf, i, spec, ref, mRef, &encRef, useEncRef, &diff);
    }
...
bool
nsStandardURL::SegmentIs(const URLSegment& seg,
                         const char* val,
                         bool ignoreCase)
{
...
bool
nsStandardURL::SegmentIs(const char* spec,
                         const URLSegment& seg,
                         const char* val,
                         bool ignoreCase)
{
...
bool
nsStandardURL::SegmentIs(const URLSegment& seg1,
                         const char* val,
                         const URLSegment& seg2,
                         bool ignoreCase)
{
...
int32_t
nsStandardURL::ReplaceSegment(uint32_t pos,
                              uint32_t len,
                              const char* val,
                              uint32_t valLen)
{
...
nsStandardURL::ParseURL(const char* spec, int32_t specLen)
{
  ...
  if (mAuthority.mLen > 0) {
    rv = mParser->ParseAuthority(spec + mAuthority.mPos,
                                 mAuthority.mLen,
                                 &mUsername.mPos,
                                 &mUsername.mLen,
                                 &mPassword.mPos,
...
nsStandardURL::ParsePath(const char* spec, uint32_t pathPos, int32_t pathLen)
{
  ...
  nsresult rv = mParser->ParsePath(spec + pathPos,
                                   pathLen,
                                   &mFilepath.mPos,
                                   &mFilepath.mLen,
                                   &mQuery.mPos,
  ...
  if (mFilepath.mLen > 0) {
    rv = mParser->ParseFilePath(spec + mFilepath.mPos,
                                mFilepath.mLen,
                                &mDirectory.mPos,
                                &mDirectory.mLen,
                                &mBasename.mPos,
...
char*
nsStandardURL::AppendToSubstring(uint32_t pos, int32_t len, const char* tail)
{
...
nsresult
nsStandardURL::WriteSegment(nsIBinaryOutputStream* stream,
                            const URLSegment& seg)
{
  nsresult rv;
...
nsStandardURL::SetUserPass(const nsACString& input)
{
  ...
  rv = mParser->ParseUserInfo(userpass.get(),
                              userpass.Length(),
                              &usernamePos,
                              &usernameLen,
                              &passwordPos,
  ...
  if (usernameLen > 0) {
    ...
    bool ignoredOut;
    usernameLen =
        encoder.EncodeSegmentCount(userpass.get(),
...
nsStandardURL::SetHost(const nsACString& input)
{
  ...
  nsAutoCString flat;
  NS_UnescapeURL(unescapedHost.BeginReading(),
                 unescapedHost.Length(),
                 esc_AlwaysCopy | esc_Host,
                 flat);
...
nsresult
nsStandardURL::CopyMembers(nsStandardURL* source,
                           nsStandardURL::RefHandlingEnum refHandlingMode,
                           const nsACString& newRef,
                           bool copyCached)
{
...
nsStandardURL::Resolve(const nsACString& in, nsACString& out)
{
  ...
  if (scheme.mLen >= 0) {
    ...
    if (SegmentIs(mScheme, relpath, scheme, true)) {
      ...
      if (nsCRT::strncmp(relpath + scheme.mPos + scheme.mLen, "://", 3) == 0) {
...
nsStandardURL::SetFilePath(const nsACString& input)
{
  ...
  if (filepath && *filepath) {
    ...
    rv = mParser->ParseFilePath(filepath,
                                flat.Length(),
                                &dirPos,
                                &dirLen,
                                &basePos,
                                    ...
                                    const Encoding* encoding)
{
  ...
  encoder.EncodeSegmentCount(
      query, URLSegment(0, queryLen), esc_Query, buf, encoded);
  if (encoded) {
    query = buf.get();
    queryLen = buf.Length();
  }
...
nsStandardURL::SetFileName(const nsACString& input)
{
  ...
  if (!(filename && *filename)) {
    ...
    rv = mParser->ParseFileName(filename,
                                flat.Length(),
                                &basename.mPos,
                                &basename.mLen,
                                &extension.mPos,
    ...
    if (basename.mLen < 0) {
      ...
      nsSegmentEncoder encoder;
      basename.mLen =
          encoder.EncodeSegmentCount(filename,
                                     basename,
                                     esc_FileBaseName | esc_AlwaysCopy,
                                     newFilename,
                                     ignoredOut);
