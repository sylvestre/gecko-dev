...
static void
SetRunning(ProxyAutoConfig* arg)
{
  MOZ_ASSERT(sRunningIndex != 0xdeadbeef);
                          ...
                          public nsINamed
{
 public:
  ...
  {
    if (mRequest) mRequest->Cancel(NS_ERROR_NET_TIMEOUT);
    mTimer = nullptr;
    return NS_OK;
  }
...
static void
PACLogToConsole(nsString& aMessage)
{
...
static bool
PACResolve(const nsCString& aHostName, NetAddr* aNetAddr, unsigned int aTimeout)
{
  if (!GetRunning()) {
...
static bool
PACResolveToString(const nsCString& aHostName,
...
static bool
PACDnsResolve(JSContext* cx, unsigned int argc, JS::Value* vp)
{
...
static bool
PACMyIpAddress(JSContext* cx, unsigned int argc, JS::Value* vp)
{
...
static bool
PACProxyAlert(JSContext* cx, unsigned int argc, JS::Value* vp)
{
  JS::CallArgs args = CallArgsFromVp(argc, vp);
 ...
 private:
  ...
  nsresult Init()
  {
    ...
    mGlobal = JS_NewGlobalObject(
        mContext, &sGlobalClass, nullptr, JS::DontFireOnNewGlobalHook, options);
    if (!mGlobal) {
...
ProxyAutoConfig::SetupJS()
{
  ...
  if (!JS_CompileScript(
          cx, mPACScript.get(), mPACScript.Length(), options, &script) ||
                                ...
                                nsACString& result)
{
  ...
  if (!mIncludePath) {
    ...
    if (urlParser) {
      ...
      uint32_t pathPos;
      rv = urlParser->ParseURL(aTestURI.get(),
                               aTestURI.Length(),
                               &schemePos,
                               &schemeLen,
                               &authorityPos,
...
bool
ProxyAutoConfig::SrcAddress(const NetAddr* remoteAddress,
