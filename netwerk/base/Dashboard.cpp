...
NS_IMPL_ISUPPORTS(ConnectionData,
...
NS_IMETHODIMP
ConnectionData::OnTransportStatus(nsITransport* aTransport,
                                  nsresult aStatus,
                                  int64_t aProgress,
...
Dashboard::GetSocketsDispatch(SocketData* aSocketData)
{
  ...
  socketData->mEventTarget->Dispatch(
      NewRunnableMethod<RefPtr<SocketData>>("net::Dashboard::GetSockets",
                                            this,
...
NS_IMETHODIMP
Dashboard::NewMsgSent(const nsACString& aHost,
                      uint32_t aSerial,
                      uint32_t aLength)
{
  if (mEnableLogging) {
...
NS_IMETHODIMP
Dashboard::NewMsgReceived(const nsACString& aHost,
                          uint32_t aSerial,
                          uint32_t aLength)
{
  if (mEnableLogging) {
                            ...
                            NetDashboardCallback* aCallback)
{
  ...
  rv = mDnsService->AsyncResolveNative(aHost,
                                       0,
                                       helper.get(),
                                       NS_GetCurrentThread(),
                                       attrs,
...
NS_IMETHODIMP
Dashboard::RequestConnection(const nsACString& aHost,
                             uint32_t aPort,
...
Dashboard::TestNewConnection(ConnectionData* aConnectionData)
{
  ...
  if (connectionData->mProtocol &&
    ...
    rv = gSocketTransportService->CreateTransport(
        &connectionData->mProtocol,
        1,
        connectionData->mHost,
        connectionData->mPort,
        nullptr,
