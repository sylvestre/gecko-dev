...
NS_IMPL_ISUPPORTS(ArrayBufferInputStream,
...
NS_IMETHODIMP
ArrayBufferInputStream::ReadSegments(nsWriteSegmentFun writer,
                                     void* closure,
                                     uint32_t aCount,
                                     uint32_t* result)
{
  ...
  *result = 0;
  while (mPos < mBufferLength) {
    ...
    uint32_t written;
    nsresult rv = writer(
        this, closure, &mArrayBuffer[0] + mPos, *result, count, &written);
    if (NS_FAILED(rv)) {
