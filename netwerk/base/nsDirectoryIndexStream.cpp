...
static int
compare(nsIFile* aElement1, nsIFile* aElement2, void* aData)
{
  if (!NS_IsNativeUTF8()) {
...
nsDirectoryIndexStream::Init(nsIFile* aDir)
{
  ...
  if (MOZ_LOG_TEST(gLog, LogLevel::Debug)) {
    ...
    MOZ_LOG(
        gLog,
        LogLevel::Debug,
        ("nsDirectoryIndexStream[%p]: initialized on %s", this, path.get()));
  }
...
{
  MOZ_LOG(
      gLog, LogLevel::Debug, ("nsDirectoryIndexStream[%p]: destroyed", this));
}
  ...
  if (aCount > 0) {
    ...
    while (uint32_t(mBuf.Length()) < aCount) {
      ...
      if (MOZ_LOG_TEST(gLog, LogLevel::Debug)) {
        ...
        MOZ_LOG(gLog,
                LogLevel::Debug,
                ("nsDirectoryIndexStream[%p]: iterated %s", this, path.get()));
      }
...
NS_IMETHODIMP
nsDirectoryIndexStream::ReadSegments(nsWriteSegmentFun writer,
                                     void* closure,
                                     uint32_t count,
                                     uint32_t* _retval)
{
