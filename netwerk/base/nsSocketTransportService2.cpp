...
nsresult
nsSocketTransportService::DetachSocket(SocketContext* listHead,
                                       SocketContext* sock)
{
...
nsSocketTransportService::AddToPollList(SocketContext* sock)
{
  ...
  if (ChaosMode::isActive(ChaosFeature::NetworkScheduling)) {
    ...
    PodMove(mActiveList + newSocketIndex + 1,
            mActiveList + newSocketIndex,
            mActiveCount - newSocketIndex);
    PodMove(mPollList + newSocketIndex + 2,
            mPollList + newSocketIndex + 1,
            mActiveCount - newSocketIndex);
  }
...
int32_t
nsSocketTransportService::Poll(uint32_t* interval, TimeDuration* pollDuration)
{
  PRPollDesc* pollList;
  uint32_t pollCount;
...
nsSocketTransportService::Init()
{
  ...
  if (tmpPrefService) {
    ...
    tmpPrefService->AddObserver(
        MAX_TIME_FOR_PR_CLOSE_DURING_SHUTDOWN, this, false);
  }
  UpdatePrefs();
...
{
  return CreateRoutedTransport(types,
                               typeCount,
                               host,
                               port,
                               NS_LITERAL_CSTRING(""),
                                                ...
                                                nsISocketTransport** result)
{
  ...
  if (fws) {
    nsresult rv = fws->CreateTransportForHost(
        types, typeCount, host, port, hostRoute, portRoute, proxyInfo, result);
  ...
  nsresult rv = trans->Init(
      types, typeCount, host, port, hostRoute, portRoute, proxyInfo);
  if (NS_FAILED(rv)) {
    return rv;
  }
...
nsSocketTransportService::CheckOverlappedPendingSocketsAreDone()
{
  ...
  SOCKET_LOG(
      ("STS CheckOverlappedPendingSocketsAreDone: "
       "pending sockets = %d\n",
  ...
  for (int i = mOverlappedPendingSockets.Length() - 1; i >= 0; i--) {
    ...
    if (mFileDesc2PlatformOverlappedIOHandleFunc(fd, (void**)&ol) ==
        PR_SUCCESS) {
  ...
  SOCKET_LOG(
      ("STS CheckOverlappedPendingSocketsAreDone: end a check, "
       "pending sockets = %d\n",
       mOverlappedPendingSockets.Length()));
}
  ...
  if (library) {
    mFileDesc2PlatformOverlappedIOHandleFunc =
        reinterpret_cast<FileDesc2PlatformOverlappedIOHandleFunc>(
            GetProcAddress(
                library, "PR_EXPERIMENTAL_ONLY_IN_4_17_GetOverlappedIOHandle"));
    if (mFileDesc2PlatformOverlappedIOHandleFunc) {
      ...
      SOCKET_LOG(
          ("No PR_EXPERIMENTAL_ONLY_IN_4_17_GetOverlappedIOHandle function: "
           "%d\n",
           rv));
    }
  }
...
PRStatus
nsSocketTransportService::CallFileDesc2PlatformOverlappedIOHandleFunc(
    PRFileDesc* fd, void** ol)
{
  CheckFileDesc2PlatformOverlappedIOHandleFunc();
  if (mFileDesc2PlatformOverlappedIOHandleFunc) {
