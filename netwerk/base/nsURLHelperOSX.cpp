...
static bool
pathBeginsWithVolName(const nsACString& path, nsACString& firstPathComponent)
{
...
static nsresult
convertHFSPathtoPOSIX(const nsACString& hfsPath, nsACString& posixPath)
{
  ...
  CFURLRef urlRef = CFURLCreateWithFileSystemPath(
      nullptr, pathStrRef, kCFURLHFSPathStyle, true);
  if (urlRef) {
    UInt8 pathBuf[PATH_MAX];
    if (CFURLGetFileSystemRepresentation(
            urlRef, true, pathBuf, sizeof(pathBuf))) {
...
net_GetURLSpecFromActualFile(nsIFile* aFile, nsACString& result)
{
  ...
  if (NS_EscapeURL(
          ePath.get(), ePath.Length(), esc_Directory + esc_Forced, escPath))
