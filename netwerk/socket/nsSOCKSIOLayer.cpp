...
void
nsSOCKSSocketInfo::Init(int32_t version,
                        int32_t family,
                        nsIProxyInfo* proxy,
                        const char* host,
                        uint32_t flags,
...
nsSOCKSSocketInfo::StartDNS(PRFileDesc* fd)
{
  ...
  mFD = fd;
  nsresult rv = dns->AsyncResolveNative(proxyHost,
                                        0,
                                        this,
...
static int32_t
nsSOCKSIOLayerAcceptRead(PRFileDesc* sd,
                         PRFileDesc** nd,
                         PRNetAddr** raddr,
                         void* buf,
                         int32_t amount,
                         PRIntervalTime timeout)
{
  ...
  return sd->lower->methods->acceptread(
      sd->lower, nd, raddr, buf, amount, timeout);
}
