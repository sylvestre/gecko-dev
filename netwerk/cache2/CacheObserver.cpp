...
CacheObserver::AttachToPreferences()
{
  ...
  if (sHalfLifeExperiment == 0) {
    ...
    // browser sessions.
    mozilla::Preferences::SetInt("browser.cache.frecency_experiment",
                                 sHalfLifeExperiment);
  }
...
// static
void
CacheObserver::ParentDirOverride(nsIFile** aDir)
{
...
nsresult
ClearStorage(bool const aPrivate, bool const aAnonymous, OriginAttributes& aOa)
{
  nsresult rv;
...
// static
bool
CacheObserver::EntryIsTooBig(int64_t aSize, bool aUsingDisk)
{
