...
class CacheFileHandle : public nsISupports
{
 public:
  ...
  CacheFileHandle(const nsACString& aKey,
                  bool aPriority,
                  PinningStatus aPinning);
  void Log();
...
class CacheFileHandles
{
 public:
  ...
  nsresult NewHandle(const SHA1Sum::Hash* aHash,
                     bool aPriority,
...
class CacheFileIOManager : public nsITimerCallback, public nsINamed
{
 public:
  ...
  static nsresult Read(CacheFileHandle* aHandle,
                       int64_t aOffset,
                       char* aBuf,
                       int32_t aCount,
                       CacheFileIOListener* aCallback);
  static nsresult Write(CacheFileHandle* aHandle,
                        int64_t aOffset,
                        const char* aBuf,
                        int32_t aCount,
                        bool aValidate,
 ...
 private:
  ...
  nsresult ReadInternal(CacheFileHandle* aHandle,
                        int64_t aOffset,
                        char* aBuf,
                        int32_t aCount);
  nsresult WriteInternal(CacheFileHandle* aHandle,
                         int64_t aOffset,
                         const char* aBuf,
                         int32_t aCount,
                         bool aValidate,
  ...
  nsresult CheckAndCreateDir(nsIFile* aFile,
                             const char* aDir,
