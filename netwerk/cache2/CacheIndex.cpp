...
class FileOpenHelper : public CacheFileIOListener
{
 public:
  ...
  explicit FileOpenHelper(CacheIndex* aIndex) : mIndex(aIndex), mCanceled(false)
  {
  }
...
NS_IMETHODIMP
FileOpenHelper::OnFileOpened(CacheFileHandle* aHandle, nsresult aResult)
{
...
// static
nsresult
CacheIndex::HasEntry(const nsACString& aKey,
                     EntryStatus* _retval,
...
// static
nsresult
CacheIndex::HasEntry(const SHA1Sum::Hash& hash,
                     EntryStatus* _retval,
                     const std::function<void(const CacheIndexEntry*)>& aCB)
{
  StaticMutexAutoLock lock(sLock);
...
// static
bool
CacheIndex::IsForcedValidEntry(const SHA1Sum::Hash* aHash)
{
...
// static
nsresult
CacheIndex::GetCacheStats(nsILoadContextInfo* aInfo,
                          uint32_t* aSize,
                          uint32_t* aCount)
{
...
// static
nsresult
CacheIndex::GetIterator(nsILoadContextInfo* aInfo,
                        bool aAddNew,
...
CacheIndex::WriteRecords()
{
  ...
  rv = CacheFileIOManager::Write(mIndexHandle,
                                 fileOffset,
                                 mRWBuf,
                                 mRWBufPos,
...
CacheIndex::WriteLogToDisk()
{
  ...
  rv = logFile->OpenNSPRFileDesc(
      PR_RDWR | PR_CREATE_FILE | PR_TRUNCATE, 0600, &fd);
  NS_ENSURE_SUCCESS(rv, rv);
...
CacheIndex::ParseRecords()
{
    ...
    if (tmpEntry.IsDirty() || !tmpEntry.IsInitialized() ||
      ...
      LOG(
          ("CacheIndex::ParseRecords() - Invalid entry found in index, removing"
           ...
           "removed=%d]",
           tmpEntry.IsDirty(),
           tmpEntry.IsInitialized(),
           tmpEntry.IsFileEmpty(),
           tmpEntry.IsFresh(),
           tmpEntry.IsRemoved()));
      FinishRead(false);
      return;
  ...
  rv = CacheFileIOManager::Read(
      mIndexHandle, fileOffset, mRWBuf + pos, toRead, this);
  if (NS_FAILED(rv)) {
    LOG(
...
CacheIndex::ParseJournal()
{
  ...
  rv = CacheFileIOManager::Read(
      mJournalHandle, fileOffset, mRWBuf + pos, toRead, this);
  if (NS_FAILED(rv)) {
    LOG(
...
{
  LOG(("CacheIndex::ChangeState() changing state %s -> %s",
       StateString(mState),
...
nsresult
CacheIndex::OnDataWritten(CacheFileHandle* aHandle,
                          const char* aBuf,
                          nsresult aResult)
{
...
{
  LOG(("CacheIndex::OnDataRead() [handle=%p, result=0x%08" PRIx32 "]",
       aHandle,
       static_cast<uint32_t>(aResult)));
...
size_t
CacheIndex::SizeOfExcludingThisInternal(
