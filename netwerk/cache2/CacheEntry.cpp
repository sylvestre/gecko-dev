...
void
CacheEntry::Callback::ExchangeEntry(CacheEntry* aEntry)
{
...
bool
CacheEntry::Callback::DeferDoom(bool* aDoom) const
{
...
nsresult
CacheEntry::Callback::OnCheckThread(bool* aOnCheckThread) const
{
...
nsresult
CacheEntry::Callback::OnAvailThread(bool* aOnAvailThread) const
{
...
NS_IMPL_ISUPPORTS(CacheEntry, nsICacheEntry, nsIRunnable, CacheFileListener)

/* static */
uint64_t
CacheEntry::GetNextId()
{
...
char const*
CacheEntry::StateString(uint32_t aState)
{
  switch (aState) {
    case NOTLOADED:
      return "NOTLOADED";
...
nsresult
CacheEntry::HashingKeyWithStorage(nsACString& aResult) const
{
  return HashingKey(mStorageID, mEnhanceID, mURI, aResult);
}

nsresult
CacheEntry::HashingKey(nsACString& aResult) const
{
...
// static
nsresult
CacheEntry::HashingKey(const nsACString& aStorageID,
                       const nsACString& aEnhanceID,
                       nsIURI* aURI,
                       nsACString& aResult)
{
...
void
CacheEntry::AsyncOpen(nsICacheEntryOpenCallback* aCallback, uint32_t aFlags)
{
...
bool
CacheEntry::Open(Callback& aCallback,
                 bool aTruncate,
                 bool aPriority,
                 bool aBypassIfBusy)
{
...
bool
CacheEntry::Load(bool aTruncate, bool aPriority)
{
...
NS_IMETHODIMP
CacheEntry::OnFileReady(nsresult aResult, bool aIsNew)
{
...
void
CacheEntry::TransferCallbacks(CacheEntry& aFromEntry)
{
  mozilla::MutexAutoLock lock(mLock);
...
void
CacheEntry::RememberCallback(Callback& aCallback)
{
  mLock.AssertCurrentThreadOwns();

  LOG(("CacheEntry::RememberCallback [this=%p, cb=%p, state=%s]",
       this,
       aCallback.mCallback.get(),
       StateString(mState)));
  ...
  while (i < mCallbacks.Length()) {
    ...
    if (NS_SUCCEEDED(rv) && !onCheckThread) {
      ...
      rv = mCallbacks[i].mTarget->Dispatch(
          NewRunnableMethod("net::CacheEntry::InvokeCallbacksLock",
                            this,
                            &CacheEntry::InvokeCallbacksLock),
...
bool
CacheEntry::InvokeCallback(Callback& aCallback)
{
...
void
CacheEntry::InvokeAvailableCallback(Callback const& aCallback)
{
  LOG(
...
void
CacheEntry::OnFetched(Callback const& aCallback)
{
...
void
CacheEntry::OnHandleClosed(CacheEntryHandle const* aHandle)
{
  LOG(("CacheEntry::OnHandleClosed [this=%p, state=%s, handle=%p]",
       this,
       StateString(mState),
       aHandle));
...
uint32_t
CacheEntry::GetMetadataMemoryConsumption()
{
...
NS_IMETHODIMP
CacheEntry::GetKey(nsACString& aKey)
{
  aKey.Assign(mURI);
  return NS_OK;
}
...
nsresult
CacheEntry::SetNetworkTimes(uint64_t aOnStartTime, uint64_t aOnStopTime)
{
  if (NS_SUCCEEDED(mFileStatus)) {
...
CacheEntry::GetIsForcedValid(bool* aIsForcedValid)
{
  ...
  *aIsForcedValid =
  ...
  LOG(("CacheEntry::GetIsForcedValid [this=%p, IsForcedValid=%d]",
       this,
       *aIsForcedValid));
...
{
  LOG(("CacheEntry::ForceValidFor [this=%p, aSecondsToTheFuture=%d]",
       this,
       aSecondsToTheFuture));
  ...
  CacheStorageService::Self()->ForceEntryValidFor(
      mStorageID, key, aSecondsToTheFuture);
...
NS_IMETHODIMP
CacheEntry::OpenInputStream(int64_t offset, nsIInputStream** _retval)
{
...
NS_IMETHODIMP
CacheEntry::OpenAlternativeInputStream(const nsACString& type,
                                       nsIInputStream** _retval)
{
  LOG(("CacheEntry::OpenAlternativeInputStream [this=%p, type=%s]",
       this,
                                    ...
                                    nsIInputStream** _retval)
{
  ...
  if (aAltDataType) {
    rv = mFile->OpenAlternativeInputStream(
        selfHandle, aAltDataType, getter_AddRefs(stream));
    if (NS_FAILED(rv)) {
...
NS_IMETHODIMP
CacheEntry::OpenOutputStream(int64_t offset, nsIOutputStream** _retval)
{
...
NS_IMETHODIMP
CacheEntry::OpenAlternativeOutputStream(const nsACString& type,
                                        nsIOutputStream** _retval)
{
  LOG(("CacheEntry::OpenAlternativeOutputStream [this=%p, type=%s]",
       this,
       PromiseFlatCString(type).get()));
...
nsresult
CacheEntry::OpenOutputStreamInternal(int64_t offset, nsIOutputStream** _retval)
{
...
NS_IMETHODIMP
CacheEntry::GetSecurityInfo(nsISupports** aSecurityInfo)
{
  {
    mozilla::MutexAutoLock lock(mLock);
...
NS_IMETHODIMP
CacheEntry::SetSecurityInfo(nsISupports* aSecurityInfo)
{
  nsresult rv;
...
NS_IMETHODIMP
CacheEntry::AsyncDoom(nsICacheEntryDoomCallback* aCallback)
{
...
NS_IMETHODIMP
CacheEntry::GetMetaDataElement(const char* aKey, char** aRetval)
{
  NS_ENSURE_SUCCESS(mFileStatus, NS_ERROR_NOT_AVAILABLE);
...
NS_IMETHODIMP
CacheEntry::SetMetaDataElement(const char* aKey, const char* aValue)
{
  NS_ENSURE_SUCCESS(mFileStatus, NS_ERROR_NOT_AVAILABLE);
...
NS_IMETHODIMP
CacheEntry::VisitMetaData(nsICacheEntryMetaDataVisitor* aVisitor)
{
  NS_ENSURE_SUCCESS(mFileStatus, NS_ERROR_NOT_AVAILABLE);
...
CacheEntry::MetaDataReady()
{
  ...
  LOG(("CacheEntry::MetaDataReady [this=%p, state=%s]",
       this,
       StateString(mState)));
...
NS_IMETHODIMP
CacheEntry::Recreate(bool aMemoryOnly, nsICacheEntry** _retval)
{
...
NS_IMETHODIMP
CacheEntry::HasWriteAccess(bool aWriteAllowed, bool* aWriteAccess)
{
...
NS_IMETHODIMP
CacheEntry::GetLoadContextInfo(nsILoadContextInfo** aInfo)
{
...
uint32_t
CacheEntry::GetExpirationTime() const
{
  MOZ_ASSERT(CacheStorageService::IsOnManagementThread());
...
void
CacheEntry::SetRegistered(bool aRegistered)
{
  MOZ_ASSERT(CacheStorageService::IsOnManagementThread());
...
bool
CacheEntry::DeferOrBypassRemovalOnPinStatus(bool aPinned)
{
...
bool
CacheEntry::Purge(uint32_t aWhat)
{
...
void
CacheEntry::RemoveForcedValidity()
{
  mLock.AssertCurrentThreadOwns();
...
void
CacheEntry::BackgroundOp(uint32_t aOperations, bool aForceAsync)
{
  mLock.AssertCurrentThreadOwns();
...
#define M_LN2 0.69314718055994530942
      ...
      LOG(("CacheEntry FRECENCYUPDATE [this=%p, frecency=%1.10f]",
           this,
           mFrecency));
...
void
CacheEntry::StoreFrecency(double aFrecency)
{
...
void
CacheOutputCloseListener::OnOutputClosed()
{
...
size_t
CacheEntry::SizeOfExcludingThis(mozilla::MallocSizeOf mallocSizeOf) const
{
  size_t n = 0;
...
size_t
CacheEntry::SizeOfIncludingThis(mozilla::MallocSizeOf mallocSizeOf) const
{
