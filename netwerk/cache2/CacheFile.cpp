...
class DoomFileHelper : public CacheFileIOListener
{
 public:
  ...
  explicit DoomFileHelper(CacheFileListener* aListener) : mListener(aListener)
  {
  }
...
nsresult
CacheFile::OnChunkAvailable(nsresult aResult,
                            uint32_t aChunkIdx,
                            CacheFileChunk* aChunk)
{
...
nsresult
CacheFile::OnDataWritten(CacheFileHandle* aHandle,
                         const char* aBuf,
                         nsresult aResult)
{
...
nsresult
CacheFile::OpenInputStream(nsICacheEntry* aEntryHandle,
                                      ...
                                      nsIInputStream** _retval)
{
  ...
  rv = CacheFileUtils::ParseAlternativeDataInfo(
      altData, &offset, &availableAltData);
...
nsresult
CacheFile::OpenOutputStream(CacheOutputCloseListener* aCloseListener,
                                       ...
                                       nsIOutputStream** _retval)
{
  ...
  CacheFileUtils::BuildAlternativeDataInfo(
      aAltDataType, mAltDataOffset, altMetadata);
...
CacheFile::SetExpirationTime(uint32_t aExpirationTime)
{
  ...
  if (mHandle && !mHandle->IsDoomed())
    CacheFileIOManager::UpdateIndexEntry(
        mHandle, nullptr, &aExpirationTime, nullptr, nullptr, nullptr);
...
CacheFile::SetFrecency(uint32_t aFrecency)
{
  ...
  if (mHandle && !mHandle->IsDoomed())
    CacheFileIOManager::UpdateIndexEntry(
        mHandle, &aFrecency, nullptr, nullptr, nullptr, nullptr);
...
nsresult
CacheFile::SetNetworkTimes(uint64_t aOnStartTime, uint64_t aOnStopTime)
{
  CacheFileAutoLock lock(this);
  ...
  if (mHandle && !mHandle->IsDoomed()) {
    CacheFileIOManager::UpdateIndexEntry(
        mHandle, nullptr, nullptr, nullptr, &onStartTime16, &onStopTime16);
  }
  return NS_OK;
}

nsresult
CacheFile::GetOnStartTime(uint64_t* _retval)
{
  CacheFileAutoLock lock(this);
...
nsresult
CacheFile::GetOnStopTime(uint64_t* _retval)
{
  CacheFileAutoLock lock(this);
...
CacheFile::SetAltMetadata(const char* aAltMetadata)
{
  ...
  if (mHandle && !mHandle->IsDoomed()) {
    CacheFileIOManager::UpdateIndexEntry(
        mHandle, nullptr, nullptr, &hasAltData, nullptr, nullptr);
  }
  return rv;
}
...
nsresult
CacheFile::GetChunkLocked(uint32_t aIndex,
                          ECallerType aCaller,
...
CacheFile::Truncate(int64_t aOffset)
{
  ...
  if (bytesInNewLastChunk == kChunkSize) {
    ...
    if (mChunks.Get(newLastChunk, getter_AddRefs(chunk))) {
      ...
      rv = GetChunkLocked(
          newLastChunk, PRELOADER, nullptr, getter_AddRefs(chunk));
      if (NS_FAILED(rv)) {
        return rv;
      }
      // We've checked that we don't have this chunk, so no chunk must be
  ...
  if (mHandle) {
    rv = CacheFileIOManager::TruncateSeekSetEOF(
        mHandle, aOffset, aOffset, nullptr);
    if (NS_FAILED(rv)) {
      return rv;
    }
  }
...
nsresult
CacheFile::NotifyChunkListeners(uint32_t aIndex,
                                nsresult aResult,
                                CacheFileChunk* aChunk)
{
  ...
  for (uint32_t i = 0; i < listeners->mItems.Length(); i++) {
    ...
    rv2 = NotifyChunkListener(
        item->mCallback, item->mTarget, aResult, aIndex, aChunk);
...
CacheFile::NotifyListenersAboutOutputRemoval()
{
  ...
  for (auto iter = mChunkListeners.Iter(); !iter.Done(); iter.Next()) {
    ...
    for (uint32_t i = 0; i < listeners->mItems.Length(); i++) {
      ChunkListenerItem* item = listeners->mItems[i];
      NotifyChunkListener(
          item->mCallback, item->mTarget, NS_ERROR_NOT_AVAILABLE, idx, nullptr);
      delete item;
    }
...
CacheFile::CleanUpCachedChunks()
{
  for (auto iter = mCachedChunks.Iter(); !iter.Done(); iter.Next()) {
    ...
    LOG(("CacheFile::CleanUpCachedChunks() [this=%p, idx=%u, chunk=%p]",
         this,
         idx,
         chunk.get()));
...
CacheFile::InitIndexEntry()
{
  ...
  rv = CacheFileIOManager::InitIndexEntry(
      mHandle,
      GetOriginAttrsHash(mMetadata->OriginAttributes()),
