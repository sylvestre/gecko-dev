...
AppCacheStorage::AppCacheStorage(nsILoadContextInfo* aInfo,
    ...
    : CacheStorage(aInfo,
                   true /* disk */,
                   false /* lookup app cache */,
                   false /* skip size check */,
                   false /* pin */),
      mAppCache(aAppCache)
{
}
...
NS_IMETHODIMP
AppCacheStorage::AsyncOpenURI(nsIURI* aURI,
...
NS_IMETHODIMP
AppCacheStorage::OpenTruncate(nsIURI* aURI,
                              const nsACString& aIdExtension,
...
NS_IMETHODIMP
AppCacheStorage::Exists(nsIURI* aURI,
                        const nsACString& aIdExtension,
                        bool* aResult)
{
  *aResult = false;
...
NS_IMETHODIMP
AppCacheStorage::AsyncDoomURI(nsIURI* aURI,
                              const nsACString& aIdExtension,
...
NS_IMETHODIMP
AppCacheStorage::AsyncEvictStorage(nsICacheEntryDoomCallback* aCallback)
{
  if (!CacheStorageService::Self()) return NS_ERROR_NOT_INITIALIZED;
...
NS_IMETHODIMP
AppCacheStorage::AsyncVisitStorage(nsICacheStorageVisitor* aVisitor,
                                   bool aVisitEntries)
{
  if (!CacheStorageService::Self()) return NS_ERROR_NOT_INITIALIZED;
...
NS_IMETHODIMP
AppCacheStorage::GetCacheIndexEntryAttrs(nsIURI* aURI,
                                         const nsACString& aIdExtension,
                                         bool* aHasAltData,
                                         uint32_t* aSizeInKB)
{
