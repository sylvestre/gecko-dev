...
CacheFileMetadata::CacheFileMetadata(bool aMemoryOnly,
                                     bool aPinned,
                                     const nsACString& aKey)
...
{
  LOG(("CacheFileMetadata::ReadMetadata() [this=%p, listener=%p]",
       this,
       aListener));
                                 ...
                                 CacheFileMetadataListener* aListener)
{
  ...
  *p = 0;
  ...
  rv = CacheFileIOManager::Write(mHandle,
                                 aOffset,
                                 writeBuffer,
                                 p - writeBuffer,
                                 true,
                                 true,
                                 aListener ? this : nullptr);
  if (NS_FAILED(rv)) {
    LOG(
...
nsresult
CacheFileMetadata::OnDataWritten(CacheFileHandle* aHandle,
                                 const char* aBuf,
                                 nsresult aResult)
{
  LOG(
...
nsresult
CacheFileMetadata::OnDataRead(CacheFileHandle* aHandle,
                              char* aBuf,
                              nsresult aResult)
{
  LOG((
    ...
    if (mMetaHdr.mKeySize != mKey.Length()) {
      LOG(
          ("CacheFileMetadata::ParseMetadata() - Key collision (1), key=%s "
           "[this=%p]",
           nsCString(mBuf + keyOffset, mMetaHdr.mKeySize).get(),
           this));
      return NS_ERROR_FILE_CORRUPTED;
    }
