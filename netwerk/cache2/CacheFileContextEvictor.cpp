                                    ...
                                    bool aPinned)
{
  ...
  if (mIndexIsUpToDate) {
    ...
    rv = CacheIndex::GetIterator(
        aLoadContextInfo, false, getter_AddRefs(entry->mIterator));
    if (NS_FAILED(rv)) {
...
nsresult
CacheFileContextEvictor::WasEvicted(const nsACString& aKey,
                                    nsIFile* aFile,
...
CacheFileContextEvictor::CreateIterators()
{
  ...
  for (uint32_t i = 0; i < mEntries.Length();) {
    rv = CacheIndex::GetIterator(
        mEntries[i]->mInfo, false, getter_AddRefs(mEntries[i]->mIterator));
    if (NS_FAILED(rv)) {
      LOG(
