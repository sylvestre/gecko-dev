...
#define MOZ_STATIC_ASSERT(cond, reason) \
  typedef int MOZ_STATIC_ASSERT_GLUE(   \
      moz_static_assert, __COUNTER__)[(cond) ? 1 : -1] MOZ_UNUSED_ATTRIBUTE
#else
#define MOZ_STATIC_ASSERT(cond, reason)                            \
  extern void MOZ_STATIC_ASSERT_GLUE(moz_static_assert, __LINE__)( \
      int arg[(cond) ? 1 : -1]) MOZ_UNUSED_ATTRIBUTE
#endif
...
MOZ_ReportAssertionFailure(const char* aStr,
...
MOZ_MAYBE_UNUSED static MOZ_COLD MOZ_NEVER_INLINE void
MOZ_ReportCrash(const char* aStr,
                const char* aFilename,
                int aLine) MOZ_PRETEND_NORETURN_FOR_STATIC_ANALYSIS
{
#ifdef ANDROID
...
static MOZ_COLD MOZ_NORETURN MOZ_NEVER_INLINE void
...
MFBT_API MOZ_COLD MOZ_NORETURN MOZ_NEVER_INLINE
    MOZ_FORMAT_PRINTF(3, 4) void MOZ_CrashPrintf(const char* aFilename,
                                                 int aLine,
                                                 const char* aFormat,
                                                 ...);
#define MOZ_CALL_CRASH_PRINTF(format, ...) \
  ...
  do {                                                                   \
    ...
    if (MOZ_UNLIKELY(!MOZ_CHECK_ASSERT_ASSIGNMENT(expr))) {              \
      MOZ_REPORT_ASSERTION_FAILURE(                                      \
          #expr " (" explain ")", __FILE__, __LINE__);                   \
