...
class MOZ_STACK_CLASS DragDataProducer
{
 ...
 private:
  ...
  static void CreateLinkText(const nsAString& inURL,
                                   ...
                                   nsIContent** aDragNode)
{
  ...
  *aCanDrag = true;
  ...
  DragDataProducer provider(
      aWindow, aTarget, aSelectionTargetNode, aIsAltKeyPressed);
                                                 ...
                                                 bool isPrivate)
{
  ...
  return persist->SavePrivacyAwareURI(sourceURI,
                                      nullptr,
                                      nullptr,
  ...
  *aDataLen = 0;
  ...
  if (strcmp(aFlavor, kFilePromiseMime) == 0) {
    ...
    aTransferable->GetTransferData(
        kFilePromiseURLMime, getter_AddRefs(tmp), &dataSize);
    ...
    aTransferable->GetTransferData(
        kFilePromiseDestFilename, getter_AddRefs(tmp), &dataSize);
    ...
    dataSize = 0;
    aTransferable->GetTransferData(
        kFilePromiseDirectoryMime, getter_AddRefs(dirPrimitive), &dataSize);
...
//
void
DragDataProducer::GetNodeString(nsIContent* inNode, nsAString& outNodeString)
{
  ...
  *aSelection = nullptr;
          ...
          if (imgUrl) {
            ...
            if (mimeInfo) {
              ...
              if (extension.IsEmpty() ||
                  NS_FAILED(
                      mimeInfo->ExtensionExists(extension, &validExtension)) ||
                  !validExtension) {
  ...
  if (nodeToSerialize || *aSelection) {
    ...
    if (*aSelection) {
      ...
      rv = nsCopySupport::GetTransferableForNode(
          nodeToSerialize, doc, getter_AddRefs(transferable));
    }
    NS_ENSURE_SUCCESS(rv, rv);
    ...
    uint32_t dataSize;
    rv = transferable->GetTransferData(
        kHTMLMime, getter_AddRefs(supports), &dataSize);
    data = do_QueryInterface(supports);
    if (NS_SUCCEEDED(rv)) {
      data->GetData(mHtmlString);
    }
    rv = transferable->GetTransferData(
        kHTMLContext, getter_AddRefs(supports), &dataSize);
    data = do_QueryInterface(supports);
    if (NS_SUCCEEDED(rv)) {
      data->GetData(mContextString);
    }
    rv = transferable->GetTransferData(
        kHTMLInfo, getter_AddRefs(supports), &dataSize);
    data = do_QueryInterface(supports);
    if (NS_SUCCEEDED(rv)) {
      data->GetData(mInfoString);
    }
    rv = transferable->GetTransferData(
        kUnicodeMime, getter_AddRefs(supports), &dataSize);
    data = do_QueryInterface(supports);
                                           ...
                                           DataTransfer* aDataTransfer)
{
  ...
  if (!mUrlString.IsEmpty() && mIsAnchor) {
    ...
    AddString(
        aDataTransfer, NS_LITERAL_STRING(kURLDataMime), mUrlString, principal);
    AddString(aDataTransfer,
              NS_LITERAL_STRING(kURLDescriptionMime),
              mTitleString,
              principal);
    AddString(aDataTransfer,
              NS_LITERAL_STRING("text/uri-list"),
              mUrlString,
              principal);
  }
  ...
  if (!mInfoString.IsEmpty())
    AddString(
        aDataTransfer, NS_LITERAL_STRING(kHTMLInfo), mInfoString, principal);

  // add the full html
  if (!mHtmlString.IsEmpty())
    AddString(
        aDataTransfer, NS_LITERAL_STRING(kHTMLMime), mHtmlString, principal);
  ...
  AddString(aDataTransfer,
            ...
            mIsAnchor ? mUrlString : mTitleString,
            principal);
  ...
  if (mImage) {
    ...
    aDataTransfer->SetDataWithPrincipal(
        NS_LITERAL_STRING(kNativeImageMime), variant, 0, principal);
    ...
    if (dataProvider) {
      ...
      aDataTransfer->SetDataWithPrincipal(
          NS_LITERAL_STRING(kFilePromiseMime), variant, 0, principal);
    }
