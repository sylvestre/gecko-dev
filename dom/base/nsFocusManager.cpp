...
inline void
ImplCycleCollectionUnlink(nsDelayedBlurOrFocusEvent& aField)
{
                            ...
                            uint32_t aFlags = 0)
{
  ...
  CycleCollectionNoteChild(
      aCallback, aField.mRelatedTarget.get(), aName, aFlags);
}
...
NS_IMETHODIMP
nsFocusManager::SetFocusedWindow(mozIDOMWindowProxy* aWindowToFocus)
{
...
NS_IMETHODIMP
nsFocusManager::ElementIsFocusable(nsIDOMElement* aElement,
                                   uint32_t aFlags,
                                   bool* aIsFocusable)
{
...
nsFocusManager::WindowRaised(mozIDOMWindowProxy* aWindow)
{
  ...
  if (MOZ_LOG_TEST(gFocusLog, LogLevel::Debug)) {
    LOGFOCUS(("Window %p Raised [Currently: %p %p]",
              aWindow,
              mActiveWindow.get(),
              mFocusedWindow.get()));
    nsIDocument* doc = window->GetExtantDoc();
    if (doc && doc->GetDocumentURI()) {
      LOGFOCUS(("  Raised Window: %p %s",
                aWindow,
                doc->GetDocumentURI()->GetSpecOrDefault().get()));
    }
    if (mActiveWindow) {
...
nsFocusManager::WindowLowered(mozIDOMWindowProxy* aWindow)
{
  ...
  if (MOZ_LOG_TEST(gFocusLog, LogLevel::Debug)) {
    LOGFOCUS(("Window %p Lowered [Currently: %p %p]",
              aWindow,
              mActiveWindow.get(),
              mFocusedWindow.get()));
    nsIDocument* doc = window->GetExtantDoc();
...
nsFocusManager::WindowShown(mozIDOMWindowProxy* aWindow, bool aNeedsFocus)
{
  ...
  if (MOZ_LOG_TEST(gFocusLog, LogLevel::Debug)) {
    LOGFOCUS(("Window %p Shown [Currently: %p %p]",
              window.get(),
              mActiveWindow.get(),
              mFocusedWindow.get()));
    nsIDocument* doc = window->GetExtantDoc();
...
nsFocusManager::WindowHidden(mozIDOMWindowProxy* aWindow)
{
  ...
  if (MOZ_LOG_TEST(gFocusLog, LogLevel::Debug)) {
    LOGFOCUS(("Window %p Hidden [Currently: %p %p]",
              window.get(),
              mActiveWindow.get(),
              mFocusedWindow.get()));
    nsAutoCString spec;
  ...
  IMEStateManager::OnChangeFocus(
      focusedPresContext, nullptr, GetFocusMoveActionCause(0));
  if (presShell) {
    ...
    if (mDelayedBlurFocusEvents[i].mDocument == aDocument) {
      if (!aDocument->GetInnerWindow() ||
        ...
        FireFocusOrBlurEvent(
            message, presShell, target, false, false, relatedTarget);
        --i;
      }
    }
  }
...
nsFocusManager::ActivateOrDeactivate(nsPIDOMWindowOuter* aWindow, bool aActive)
{
  ...
  nsContentUtils::CallOnAllRemoteChildren(
      aWindow, ActivateOrDeactivateChild, (void*)aActive);
}

void
nsFocusManager::SetFocusInner(nsIContent* aNewContent,
                              int32_t aFlags,
                              bool aFocusChanged,
                              bool aAdjustWidget)
{
  ...
  if (subWindow) {
    contentToFocus = GetFocusedDescendant(
        subWindow, eIncludeAllDescendants, getter_AddRefs(newWindow));
  ...
  if (content) {
    if (sendBlurEvent) {
      NotifyFocusStateChange(
          content, aContentToFocus, shouldShowFocusRing, false);
    }
  ...
  if (mFocusedWindow != window ||
    ...
    if (mFocusedWindow == nullptr)
      SendFocusOrBlurEvent(
          eBlur, presShell, doc, window->GetCurrentInnerWindow(), 1, false);
                      ...
                      nsIContent* aContentLostFocus)
{
  ...
  if (aIsNewDocument) {
    ...
    if (doc && doc->HasFlag(NODE_IS_EDITABLE)) {
      IMEStateManager::OnChangeFocus(presShell->GetPresContext(),
                                     nullptr,
                                     GetFocusMoveActionCause(aFlags));
    }
    if (doc)
      SendFocusOrBlurEvent(eFocus,
                           presShell,
                           doc,
                           doc,
                           aFlags & FOCUSMETHOD_MASK,
                           aWindowRaised);
  ...
  if (CheckIfFocusable(aContent, aFlags) && mFocusedWindow == aWindow &&
    ...
    if (sendFocusEvent) {
      NotifyFocusStateChange(
          aContent, nullptr, aWindow->ShouldShowFocusRing(), true);
      ...
      IMEStateManager::OnChangeFocus(
          presContext, aContent, GetFocusMoveActionCause(aFlags));
      ...
      IMEStateManager::OnChangeFocus(
          presContext, nullptr, GetFocusMoveActionCause(aFlags));
      if (!aWindowRaised) {
        aWindow->UpdateCommands(NS_LITERAL_STRING("focus"), nullptr, 0);
      }
    }
  ...
  if (mFocusedContent == aContent)
    UpdateCaret(aFocusChanged && !(aFlags & FLAG_BYMOUSE),
                aIsNewDocument,
                mFocusedContent);
...
void
nsFocusManager::FireFocusInOrOutEvent(EventMessage aEventMessage,
                                      nsIPresShell* aPresShell,
                                      nsISupports* aTarget,
                                     ...
                                     EventTarget* aRelatedTarget)
{
  ...
  FireFocusOrBlurEvent(aEventMessage,
                       aPresShell,
                       aTarget,
                       aWindowRaised,
                       aIsRefocus,
                                     ...
                                     EventTarget* aRelatedTarget)
{
  ...
  if (!dontDispatchEvent) {
    ...
    if (!targetWindow && !targetDocument) {
      ...
      FireFocusInOrOutEvent(focusInOrOutMessage,
                            aPresShell,
                            aTarget,
                            currentWindow,
...
nsFocusManager::RaiseWindow(nsPIDOMWindowOuter* aWindow)
{
  ...
  GetFocusedDescendant(
      aWindow, eIncludeAllDescendants, getter_AddRefs(childWindow));
...
nsFocusManager::MoveCaretToFocus(nsIPresShell* aPresShell, nsIContent* aContent)
{
  ...
  nsCOMPtr<nsIDOMDocument> domDoc =
      do_QueryInterface(aPresShell->GetDocument());
  if (domDoc) {
            ...
            if (frame) {
              ...
              if (widget == newCaretWidget && caretRect.y == newCaretOffset.y &&
                ...
                if (endOfSelectionInStartNode) {
                  endContent =
                      newCaretContent;  // Ensure end of selection is not before start
                }
              }
            }
          }
  ...
  *aNextContent = nullptr;
  ...
  if (!startContent && aType != MOVEFOCUS_CARET) {
    if (aType == MOVEFOCUS_FORWARDDOC || aType == MOVEFOCUS_BACKWARDDOC) {
      ...
      nsCOMPtr<nsPIDOMWindowOuter> focusedWindow;
      startContent = GetFocusedDescendant(
          aWindow, eIncludeAllDescendants, getter_AddRefs(focusedWindow));
  ...
  if (aType == MOVEFOCUS_FIRST) {
    ...
    return GetNextTabbableContent(presShell,
                                  startContent,
                                  nullptr,
                                  startContent,
                                  true,
    ...
    if (popupFrame) {
      ...
      if (docShell && docShell->ItemType() != nsIDocShellTreeItem::typeChrome) {
        ...
        if (aType == MOVEFOCUS_CARET) {
          ...
          if (startContent) {
            GetFocusInSelection(
                aWindow, startContent, endSelectionContent, aNextContent);
          }
          return NS_OK;
        }
  ...
  if (forDocumentNavigation && doc->IsXULDocument()) {
    ...
    if (rootContent->GetAttr(
            kNameSpaceID_None, nsGkAtoms::retargetdocumentfocus, retarget)) {
  ...
  while (1) {
      ...
      if (aForDocumentNavigation && currentContent && (aCurrentTabIndex == 0) &&
        ...
        if (popupFrame && popupFrame->IsOpen()) {
          ...
          if (validPopup) {
            ...
            rv = GetNextTabbableContent(aPresShell,
                                        currentContent,
                                        nullptr,
                                        currentContent,
                                        true,
      ...
      if (tabIndex >= 0) {
        ...
        if (!aForDocumentNavigation &&
          ...
          nsIContent* areaContent = GetNextTabbableMapArea(
              aForward, aCurrentTabIndex, currentContent, iterStartContent);
          if (areaContent) {
          ...
          if (checkSubDocument) {
            ...
            if (subdoc && !subdoc->EventHandlingSuppressed()) {
              ...
              if (rootElement && subShell) {
                rv = GetNextTabbableContent(subShell,
                                            rootElement,
    ...
    if (aCurrentTabIndex == (aForward ? 0 : 1)) {
      ...
      if (!aForward) {
        ...
        nsCOMPtr<nsIContent> docRoot = GetRootForFocus(
                                       ...
                                       nsIContent* aStartContent)
{
  ...
  if (doc) {
    ...
    while ((areaContent = mapContent->GetChildAt(
