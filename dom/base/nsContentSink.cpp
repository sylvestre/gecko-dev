...
nsContentSink::InitializeStatics()
{
  ...
  Preferences::AddIntVarCache(
      &sBackoffCount, "content.notify.backoffcount", -1);
  ...
  Preferences::AddIntVarCache(
      &sInteractiveDeflectCount, "content.sink.interactive_deflect_count", 0);
  Preferences::AddIntVarCache(
      &sPerfDeflectCount, "content.sink.perf_deflect_count", 200);
  Preferences::AddIntVarCache(
      &sPendingEventMode, "content.sink.pending_event_mode", 1);
  Preferences::AddIntVarCache(
      &sEventProbeRate, "content.sink.event_probe_rate", 1);
  Preferences::AddIntVarCache(
      &sInteractiveParseTime, "content.sink.interactive_parse_time", 3000);
  Preferences::AddIntVarCache(
      &sPerfParseTime, "content.sink.perf_parse_time", 360000);
  Preferences::AddIntVarCache(
      &sInteractiveTime, "content.sink.interactive_time", 750000);
  Preferences::AddIntVarCache(
      &sInitialPerfTime, "content.sink.initial_perf_time", 2000000);
  Preferences::AddIntVarCache(
      &sEnablePerfMode, "content.sink.enable_perf_mode", 0);
}
...
nsresult
nsContentSink::ProcessHeaderData(nsAtom* aHeader,
...
nsresult
nsContentSink::ProcessLink(const nsAString& aAnchor,
                           ...
                           const nsAString& aAs)
{
  ...
  return ProcessStyleLink(
      nullptr, aHref, isAlternate, aTitle, aType, aMedia, aReferrerPolicy);
}
                                ...
                                const nsAString& aReferrerPolicy)
{
  ...
  nsresult rv = NS_NewURI(
      getter_AddRefs(url), aHref, nullptr, mDocument->GetDocBaseURI());
  ...
  NS_ASSERTION(!aElement || aElement->NodeType() ==
  ...
  if (!integrity.IsEmpty()) {
    MOZ_LOG(dom::SRILogHelper::GetSriLog(),
  ...
  bool isAlternate;
  rv = mCSSLoader->LoadStyleLink(aElement,
                                 url,
                                 nullptr,
                                 aTitle,
                                 aMedia,
    ...
    if (uri) {
      ...
      if (aLinkTypes & nsStyleLinkElement::ePRELOAD) {
        ...
        if (!nsStyleLinkElement::CheckPreloadAttrs(
                asAttr, mimeType, aMedia, mDocument)) {
...
nsContentSink::SelectDocAppCacheNoManifest(
    nsIApplicationCache* aLoadApplicationCache,
    nsIURI** aManifestURI,
    CacheSelectionAction* aAction)
{
...
nsContentSink::ProcessOfflineManifest(const nsAString& aManifestSpec)
{
  ...
  if (aManifestSpec.IsEmpty()) {
    ...
    if (NS_FAILED(rv)) {
      ...
      rv = SelectDocAppCache(
          applicationCache, manifestURI, fetchedWithHTTPGetOrEquiv, &action);
      if (NS_FAILED(rv)) {
        return;
      }
    }
  ...
  if (action == CACHE_SELECTION_RESELECT_WITHOUT_MANIFEST) {
    rv = SelectDocAppCacheNoManifest(
        applicationCache, getter_AddRefs(manifestURI), &action);
    if (NS_FAILED(rv)) {
      return;
    }
  }
...
void
nsContentSink::FavorPerformanceHint(bool perfOverStarvation,
...
nsContentSink::NotifyDocElementCreated(nsIDocument* aDoc)
{
  ...
  if (observerService) {
    ...
    observerService->NotifyObservers(
        domDoc, "document-element-inserted", EmptyString().get());
  }
