...
static nsresult
AppendString(nsITransferable* aTransferable,
             const nsAString& aString,
             const char* aFlavor);

// copy HTML node data
static nsresult
AppendDOMNode(nsITransferable* aTransferable, nsINode* aDOMNode);

#ifdef XP_WIN
// copy image as file promise onto the transferable
static nsresult
AppendImagePromise(nsITransferable* aTransferable,
...
// share common code.
static nsresult
SelectionCopyHelper(nsISelection* aSel,
                    nsIDocument* aDoc,
                    bool doPutOnClipboard,
                    ...
                    nsITransferable** aTransferable)
{
  ...
  if (encodedTextHTML) {
    ...
    rv = docEncoder->EncodeToStringWithContext(
        htmlParentsBuf, htmlInfoBuf, textHTMLBuf);
    NS_ENSURE_SUCCESS(rv, rv);
  }
...
nsresult
nsCopySupport::HTMLCopy(nsISelection* aSel,
                        nsIDocument* aDoc,
                        int16_t aClipboardID,
...
nsCopySupport::GetTransferableForSelection(nsISelection* aSel,
                                           ...
                                           nsITransferable** aTransferable)
{
  return SelectionCopyHelper(aSel,
                             aDoc,
                             false,
                             0,
                             nsIDocumentEncoder::SkipInvisibleContent,
...
nsresult
nsCopySupport::DoHooks(nsIDocument* aDoc,
...
nsresult
nsCopySupport::GetContents(const nsACString& aMimeType,
                           uint32_t aFlags,
                           nsISelection* aSel,
                           nsIDocument* aDoc,
                           nsAString& outdata)
                         ...
                         int32_t aCopyFlags)
{
  ...
  if (aCopyFlags & nsIContentViewerEdit::COPY_IMAGE_DATA) {
    ...
    rv = trans->SetTransferData(kNativeImageMime, imgPtr, sizeof(nsISupports*));
    NS_ENSURE_SUCCESS(rv, rv);
  }
             ...
             const char* aFlavor)
{
  ...
  return aTransferable->SetTransferData(
      aFlavor, data, aString.Length() * sizeof(char16_t));
}
...
AppendDOMNode(nsITransferable* aTransferable, nsINode* aDOMNode)
{
  ...
  // selializer
  nsCOMPtr<nsIDocumentEncoder> docEncoder(
      do_CreateInstance(NS_HTMLCOPY_ENCODER_CONTRACTID, &rv));
  NS_ENSURE_SUCCESS(rv, rv);
  ...
  rv =
      docEncoder->NativeInit(document,
...
nsIContent*
nsCopySupport::GetSelectionForCopy(nsIDocument* aDocument,
                                  ...
                                  bool* aActionTaken)
{
  ...
  if (!sel) content = GetSelectionForCopy(doc, getter_AddRefs(sel));
  ...
  if (sel) {
    ...
    if (NS_SUCCEEDED(rv) && range) {
      ...
      if (startContainer) content = do_QueryInterface(startContainer);
    }
  }
  ...
  if (chromeShell ||
    ...
    clipboardData = new DataTransfer(doc->GetScopeObject(),
                                     aEventMessage,
                                     originalEventMessage == ePaste,
                                     aClipboardType);
    ...
    EventDispatcher::Dispatch(
        content, presShell->GetPresContext(), &evt, nullptr, &status);
