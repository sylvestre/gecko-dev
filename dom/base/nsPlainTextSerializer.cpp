...
static int32_t
HeaderLevel(nsAtom* aTag);
static int32_t
GetUnicharWidth(char16_t ucs);
static int32_t
GetUnicharStringWidth(const char16_t* pwcs, int32_t n);
    ...
    : kSpace(NS_LITERAL_STRING(" "))  // Init of "constant"
{
  ...
  if (!gPreferenceInitialized) {
    Preferences::AddBoolVarCache(
        &gAlwaysIncludeRuby, PREF_ALWAYS_INCLUDE_RUBY, true);
                                 ...
                                 int32_t aLineFragmentLength)
{
  ...
  if (MayWrap()) {
    ...
    while (mCurrentLineWidth + prefixwidth > mWrapColumn + bonuswidth) {
      ...
      if (mLineBreaker) {
        goodSpace = mLineBreaker->Prev(
            mCurrentLine.get(), mCurrentLine.Length(), goodSpace);
 ...
 */
nsresult
nsPlainTextSerializer::GetAttributeValue(nsAtom* aName, nsString& aValueRet)
{
  if (mElement) {
...
GetUnicharWidth(char16_t ucs)
{
  ...
  size_t idx;
  if (BinarySearchIf(combining,
                     0,
                     ArrayLength(combining),
                     CombiningComparator(ucs),
                     &idx)) {
    return 0;
  }
