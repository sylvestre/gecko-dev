...
nsIContent::GetFlattenedTreeParentNodeInternal(FlattenedParentType aType) const
{
  ...
  if (aType == eForStyle && IsRootOfNativeAnonymousSubtree() &&
...
#ifdef DEBUG
    {
      ...
      nsContentUtils::AppendDocumentLevelNativeAnonymousContentTo(OwnerDoc(),
                                                                  docLevelNAC);
                                       ...
                                       nsAString& aNamespaceURI) const
{
  ...
  do {
    if (content->GetAttr(kNameSpaceID_XMLNS, name, aNamespaceURI)) return NS_OK;
...
nsIContent::GetBaseURI(bool aTryUseXHRDocBaseURI) const
{
  ...
  if (!baseAttrs.IsEmpty()) {
    ...
    for (uint32_t i = baseAttrs.Length() - 1; i != uint32_t(-1); --i) {
      ...
      nsresult rv = NS_NewURI(getter_AddRefs(newBase),
                              baseAttrs[i],
...
FragmentOrElement::Children()
{
  ...
  if (!slots->mChildrenList) {
    slots->mChildrenList = new nsContentList(this,
...
FragmentOrElement::nsDOMSlots::nsDOMSlots()
    : nsINode::nsSlots(), mDataset(nullptr)
{
}
...
FragmentOrElement::nsDOMSlots::Traverse(nsCycleCollectionTraversalCallback& cb)
{
  ...
  cb.NoteXPCOMChild(
      NS_ISUPPORTS_CAST(nsIDOMNodeList*, mExtendedSlots->mLabelsList));

  NS_CYCLE_COLLECTION_NOTE_EDGE_NAME(cb, "mExtendedSlots->mShadowRoot");
  cb.NoteXPCOMChild(
      NS_ISUPPORTS_CAST(nsIContent*, mExtendedSlots->mShadowRoot));

  NS_CYCLE_COLLECTION_NOTE_EDGE_NAME(cb, "mExtendedSlots->mContainingShadow");
  cb.NoteXPCOMChild(
      NS_ISUPPORTS_CAST(nsIContent*, mExtendedSlots->mContainingShadow));
  ...
  if (mExtendedSlots->mCustomElementData) {
    for (uint32_t i = 0;
         i < mExtendedSlots->mCustomElementData->mReactionQueue.Length();
         i++) {
    ...
    if (mExtendedSlots->mCustomElementData->mCustomElementDefinition) {
      NS_CYCLE_COLLECTION_NOTE_EDGE_NAME(
          cb, "mExtendedSlots->mCustomElementData->mCustomElementDefinition");
      cb.NoteNativeChild(
...
size_t
FragmentOrElement::nsDOMSlots::SizeOfIncludingThis(
...
bool
nsIContent::GetAttr(int32_t aNameSpaceID,
                    nsAtom* aName,
...
FragmentOrElement::SetXBLInsertionParent(nsIContent* aContent)
{
  ...
  if (oldInsertionParent != aContent && IsStyledByServo() && IsElement() &&
...
FragmentOrElement::RemoveChildAt(uint32_t aIndex, bool aNotify)
{
  ...
  NS_ASSERTION(oldKid == GetChildAt(aIndex),
...
FragmentOrElement::DestroyContent()
{
  ...
  document->BindingManager()->RemovedFromDocument(
      this, document, nsBindingManager::eRunDtor);
...
NS_IMPL_CYCLE_COLLECTION_UNLINK_BEGIN(FragmentOrElement)
  ...
  doc->BindingManager()->RemovedFromDocument(
      tmp, doc, nsBindingManager::eDoNotRunDtor);
...
void
FragmentOrElement::MarkUserData(void* aObject,
                                nsAtom* aKey,
                                void* aChild,
                                void* aData)
{
...
nsresult
FragmentOrElement::SetText(const char16_t* aBuffer,
                           uint32_t aLength,
                           bool aNotify)
{
...
nsresult
FragmentOrElement::AppendText(const char16_t* aBuffer,
                              uint32_t aLength,
                              bool aNotify)
{
...
{
  static const nsAtom* voidElements[] = {nsGkAtoms::area,
                                         nsGkAtoms::base,
                                         nsGkAtoms::basefont,
                                         nsGkAtoms::bgsound,
                                         nsGkAtoms::br,
...
void
FragmentOrElement::SetInnerHTMLInternal(const nsAString& aInnerHTML,
                                        ErrorResult& aError)
{
