...
NS_IMETHODIMP
nsJSEnvironmentObserver::Observe(nsISupports* aSubject,
                                 const char* aTopic,
  ...
  if (docShell) {
    ...
    if (errorDepth < 2) {
      ...
      EventDispatcher::DispatchDOMEvent(
          win, nullptr, event, presContext, aStatus);
      called = true;
    }
    --errorDepth;
  }
  ...
  NS_IMETHOD Run() override
  {
    ...
    if (win->IsCurrentInnerWindow() && win->GetDocShell() &&
      ...
      RefPtr<ErrorEvent> event = ErrorEvent::Constructor(
          nsGlobalWindow::Cast(win), NS_LITERAL_STRING("error"), init);
      event->SetTrusted(true);

      EventDispatcher::DispatchDOMEvent(
          win, nullptr, event, presContext, &status);
    }
...
nsresult
nsJSContext::SetProperty(JS::Handle<JSObject*> aTarget,
                                     ...
                                     JS::AutoValueVector& aArgsOut)
{
  ...
  if (argsArray) {
    for (uint32_t argCtr = 0; argCtr < argCount && NS_SUCCEEDED(rv); argCtr++) {
      ...
      argsArray->QueryElementAt(
          argCtr, NS_GET_IID(nsISupports), getter_AddRefs(arg));
      if (!arg) {
        thisVal.setNull();
        continue;
      }
...
{
  AUTO_PROFILER_LABEL_DYNAMIC("nsJSContext::GarbageCollectNow",
                              GC,
...
static void
FireForgetSkippable(uint32_t aSuspected,
                    ...
                    TimeStamp aDeadline)
{
  ...
  nsCycleCollector_forgetSkippable(
      budget, aRemoveChildless, earlyForgetSkippable);
...
nsJSContext::EndCycleCollectionCallback(CycleCollectorResults& aResults)
{
  ...
  if (sPostGCEventsToConsole || gCCStats.mFile) {
    ...
    nsTextFormatter::ssprintf(
        ...
        aResults.mNumSlices,
        gCCStats.mSuspected,
        aResults.mVisitedRefCounted,
        aResults.mVisitedGCed,
        mergeMsg.get(),
        aResults.mFreedRefCounted,
        aResults.mFreedGCed,
  ...
  if (sPostGCEventsToObserver) {
    ...
    if (observerService) {
      observerService->NotifyObservers(
          nullptr, "cycle-collection-statistics", json.get());
    }
  }
...
nsJSContext::MaybePokeCC()
{
  ...
  if (ShouldTriggerCC(nsCycleCollector_suspectedCount())) {
    ...
    sCCRunner = IdleTaskRunner::Create(CCRunnerFired,
...
NotifyGCEndRunnable::Run()
{
  ...
  observerService->NotifyObservers(
      nullptr, "garbage-collection-statistics", toSend);

  return NS_OK;
}

static void
DOMGCSliceCallback(JSContext* aCx,
  ...
  switch (aProgress) {
    ...
    case JS::GC_CYCLE_END: {
      ...
      if (sPostGCEventsToConsole) {
        ...
        nsAutoString prefix;
        nsTextFormatter::ssprintf(prefix,
                                  u"GC(T+%.1f)[%s-%i] ",
                                  double(delta) / PR_USEC_PER_SEC,
                                  ProcessNameForCollectorLog(),
                                  getpid());
    ...
    case JS::GC_SLICE_END:
      ...
      if (sPostGCEventsToConsole) {
        ...
        nsAutoString prefix;
        nsTextFormatter::ssprintf(
            prefix, u"[%s-%i] ", ProcessNameForCollectorLog(), getpid());
        nsString msg = prefix + gcstats;
        nsCOMPtr<nsIConsoleService> cs =
            do_GetService(NS_CONSOLESERVICE_CONTRACTID);
        if (cs) {
...
{
  return FetchUtil::StreamResponseToJS(
      aCx, aObj, aMimeType, aConsumer, nullptr);
}
...
{
 public:
  nsJSArgArray(JSContext* aContext,
               uint32_t argc,
               const JS::Value* argv,
               nsresult* prv);
...
nsJSArgArray::nsJSArgArray(JSContext* aContext,
                           uint32_t argc,
                           const JS::Value* argv,
                           nsresult* prv)
...
NS_IMETHODIMP
nsJSArgArray::QueryElementAt(uint32_t index, const nsIID& uuid, void** result)
{
  *result = nullptr;
  ...
  if (uuid.Equals(NS_GET_IID(nsIVariant)) ||
    ...
    return nsContentUtils::XPConnect()->JSToVariant(
        mContext, val, (nsIVariant**)result);
  }
...
NS_IMETHODIMP
nsJSArgArray::Enumerate(nsISimpleEnumerator** _retval)
{
  return NS_ERROR_NOT_IMPLEMENTED;
}
...
nsresult
NS_CreateJSArgv(JSContext* aContext,
                uint32_t argc,
                const JS::Value* argv,
