...
static void
SetupCapitalization(const char16_t* aWord,
                    uint32_t aLength,
...
nsLineBreaker::FlushCurrentWord()
{
  ...
  for (i = 0; i < mTextItems.Length(); ++i) {
    ...
    if (ti->mSink) {
      ti->mSink->SetBreaks(ti->mSinkOffset + skipSet,
                           ti->mLength - skipSet,
      ...
      if (ti->mFlags & BREAK_NEED_CAPITALIZATION) {
        if (capitalizationState.Length() == 0) {
          ...
          SetupCapitalization(
              mCurrentWord.Elements(), length, capitalizationState.Elements());
        }
        ti->mSink->SetCapitalization(ti->mSinkOffset,
                                     ti->mLength,
...
nsresult
nsLineBreaker::AppendText(nsAtom* aHyphenationLanguage,
                          ...
                          nsILineBreakSink* aSink)
{
    ...
    if (isSpace || ch == '\n') {
      if (offset > wordStart && aSink) {
        ...
        if (!noCapitalizationNeeded) {
          SetupCapitalization(aText + wordStart,
                              offset - wordStart,
  ...
  if (aSink) {
    ...
    if (!noCapitalizationNeeded) {
      aSink->SetCapitalization(
          start, offset - start, capitalizationState.Elements() + start);
    }
  }
  return NS_OK;
}
