...
static void
InvalidateAllFrames(nsINode* aNode)
{
...
// static
nsresult
nsRange::CompareNodeToRange(nsINode* aNode,
                            nsRange* aRange,
                            bool* outNodeBefore,
                            bool* outNodeAfter)
{
...
/* static */ bool
nsRange::IsNodeSelected(nsINode* aNode,
  ...
  if (!ancestorSelectionRanges.IsEmpty()) {
    ...
    for (auto iter = ancestorSelections.ConstIter(); !iter.Done();
      ...
      if (mozilla::BinarySearchIf(
              sortedRanges, 0, sortedRanges.Length(), comparator, &unused)) {
        return true;
      }
...
/* static */
nsresult
nsRange::CreateRange(nsINode* aStartContainer,
                     uint32_t aStartOffset,
                     nsINode* aEndParent,
                     uint32_t aEndOffset,
                     nsRange** aRange)
{
  ...
  *aRange = nullptr;
  ...
  nsresult rv = range->SetStartAndEnd(
      aStartContainer, aStartOffset, aEndParent, aEndOffset);
...
/* static */
nsresult
nsRange::CreateRange(nsIDOMNode* aStartContainer,
                     uint32_t aStartOffset,
                     ...
                     nsRange** aRange)
{
  ...
  return CreateRange(
      startContainer, aStartOffset, endContainer, aEndOffset, aRange);
}
...
static void
UnmarkDescendants(nsINode* aNode)
{
                              ...
                              CharacterDataChangeInfo* aInfo)
{
  ...
  if (aContent == mEnd.Container() && aInfo->mChangeStart < mEnd.Offset()) {
    if (aInfo->mDetails &&
        (aContent->GetParentNode() || newStart.Container())) {
      // splitText(), aInfo->mDetails->mNextSibling is the new text node
      NS_ASSERTION(
          aInfo->mDetails->mType == CharacterDataChangeInfo::Details::eSplit,
          "only a split can start before the end");
                        ...
                        nsIContent* aPreviousSibling)
{
  ...
  if (newStart.IsSet() || newEnd.IsSet()) {
    DoSetRange(newStart.IsSet() ? newStart : mStart.AsRaw(),
               newEnd.IsSet() ? newEnd : mEnd.AsRaw(),
               mRoot);
  }
...
NS_IMETHODIMP
nsRange::ComparePoint(nsIDOMNode* aContainer,
                      uint32_t aOffset,
                      int16_t* aResult)
{
  nsCOMPtr<nsINode> container = do_QueryInterface(aContainer);
...
nsRange::ComparePoint(nsINode& aContainer, uint32_t aOffset, ErrorResult& aRv)
{
  ...
  int32_t cmp = nsContentUtils::ComparePoints(
      &aContainer, aOffset, mStart.Container(), mStart.Offset());
  if (cmp <= 0) {
    return cmp;
  }
  if (nsContentUtils::ComparePoints(
          mEnd.Container(), mEnd.Offset(), &aContainer, aOffset) == -1) {
    return 1;
  }
...
nsRange::IntersectsNode(nsINode& aNode, ErrorResult& aRv)
{
  ...
  bool result =
      ...
      nsContentUtils::ComparePoints(
          parent, nodeIndex, mEnd.Container(), mEnd.Offset(), &disconnected) <
          0;
                    ...
                    bool aNotInsertedYet)
{
  ...
  if (mSelection) {
    nsContentUtils::AddScriptRunner(
        NewRunnableMethod("NotifySelectionListenersAfterRangeSet",
                          this,
                          &nsRange::NotifySelectionListenersAfterRangeSet));
  }
}
...
/* static */
bool
nsRange::IsValidPoints(nsINode* aStartContainer,
                       uint32_t aStartOffset,
...
nsresult
nsRange::SetStartAndEnd(nsINode* aStartContainer,
                        uint32_t aStartOffset,
                        nsINode* aEndContainer,
                        uint32_t aEndOffset)
{
...
RangeSubtreeIterator::Init(nsRange* aRange)
{
  ...
  if (startData || (node->IsElement() && node->AsElement()->GetChildCount() ==
                                             aRange->GetStartOffset(rv))) {
    mStart = node;
  }
...
CollapseRangeAfterDelete(nsRange* aRange)
{
  ...
  if (startContainer == commonAncestor) return aRange->Collapse(true);
  if (endContainer == commonAncestor) return aRange->Collapse(false);
...
nsRange::CutContents(DocumentFragment** aFragment)
{
  ...
  while (!iter.IsDone()) {
        ...
        if (node == endContainer) {
          ...
          if (endOffset > startOffset) {
            if (retval) {
              ...
              rv = charData->SubstringData(
                  startOffset, endOffset - startOffset, cutValue);
    ...
    if (retval) {
      ...
      rv = CloneParentsBetween(oldCommonAncestor,
                               node,
...
NS_IMETHODIMP
nsRange::DeleteContents() { return CutContents(nullptr); }
...
NS_IMETHODIMP
nsRange::CompareBoundaryPoints(uint16_t aHow,
                               nsIDOMRange* aOtherRange,
                               int16_t* aCmpRet)
{
...
nsRange::CloneContents(ErrorResult& aRv)
{
  ...
  while (!iter.IsDone()) {
    ...
    aRv = CloneParentsBetween(commonAncestor,
                              node,
                              getter_AddRefs(closestAncestor),
                              getter_AddRefs(farthestAncestor));
...
nsRange::InsertNode(nsINode& aNode, ErrorResult& aRv)
{
  ...
  if (startTextNode) {
    ...
    referenceParentNode->EnsurePreInsertionValidity(
        aNode, tStartContainer, aRv);
    if (aRv.Failed()) {
      return;
    }
    ...
    aRv =
        tStartContainer->AsDOMNode()->GetChildNodes(getter_AddRefs(tChildList));
    if (aRv.Failed()) {
      return;
    }
...
nsRange::ToString(nsAString& aReturn)
{
  ...
  if (mStart.Container() == mEnd.Container()) {
    ...
    if (textNode) {
      ...
      // grab the text
      if (NS_FAILED(textNode->SubstringData(
              mStart.Offset(), mEnd.Offset() - mStart.Offset(), aReturn)))
    ...
    if (textNode)  // if it's a text node, get the text
    {
      if (n == mStart.Container()) {  // only include text past start offset
        ...
        textNode->SubstringData(
            mStart.Offset(), strLength - mStart.Offset(), tempString);
        aReturn += tempString;
      } else if (n ==
                                  ...
                                  nsIDOMDocumentFragment** aReturn)
{
  if (mIsPositioned) {
    return nsContentUtils::CreateContextualFragment(
        mStart.Container(), aFragment, false, aReturn);
  }
  return NS_ERROR_FAILURE;
}
...
nsRange::CreateContextualFragment(const nsAString& aFragment, ErrorResult& aRv)
{
  ...
  return nsContentUtils::CreateContextualFragment(
      mStart.Container(), aFragment, false, aRv);
}

static void
ExtractRectFromOffset(nsIFrame* aFrame,
...
static nsresult
GetPartialTextRect(nsLayoutUtils::RectCallback* aCallback,
    ...
    if (content && content->IsNodeOfType(nsINode::eTEXT)) {
      ...
      if (textFrame) {
        ...
        if (outFrame) {
          ...
          ExtractRectFromOffset(outFrame,
                                static_cast<int32_t>(aStartOffset),
                                &r,
                                false,
                                aClampToEdge);
          r.SetWidth(0);
          r = nsLayoutUtils::TransformFrameRectToAncestor(
              outFrame, r, relativeTo);
...
nsRange::GetBoundingClientRect(bool aClampToEdge, bool aFlushLayout)
{
  ...
  CollectClientRectsAndText(&accumulator,
                            nullptr,
                            this,
                            mStart.Container(),
                            mStart.Offset(),
...
nsRange::GetClientRects(bool aClampToEdge, bool aFlushLayout)
{
  ...
  CollectClientRectsAndText(&builder,
                            nullptr,
                            this,
                            mStart.Container(),
                            mStart.Offset(),
...
void
nsRange::GetClientRectsAndTexts(mozilla::dom::ClientRectsAndTexts& aResult,
                                ErrorResult& aErr)
{
  if (!mStart.Container()) {
    return;
    ...
    if (content->IsNodeOfType(nsINode::eTEXT)) {
      if (node == startContainer) {
        ...
        nsLayoutUtils::GetFontFacesForText(
            frame, mStart.Offset(), offset, true, fontFaceList);
        continue;
      }
      if (node == endContainer) {
        nsLayoutUtils::GetFontFacesForText(
            frame, 0, mEnd.Offset(), true, fontFaceList);
        continue;
      }
    }
...
static bool
ExcludeIfNextToNonSelectable(nsIContent* aContent)
{
...
void
nsRange::GetInnerTextNoFlush(DOMString& aValue,
                             ErrorResult& aError,
