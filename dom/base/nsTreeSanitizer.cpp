...
//
nsAtom** const kElementsHTML[] = {&nsGkAtoms::a,
                                  &nsGkAtoms::abbr,
                                  &nsGkAtoms::acronym,
                                  &nsGkAtoms::address,
                                  &nsGkAtoms::area,
...
nsAtom** const kAttributesHTML[] = {&nsGkAtoms::abbr,
                                    &nsGkAtoms::accept,
...
nsAtom** const kPresAttributesHTML[] = {&nsGkAtoms::align,
...
nsAtom** const kURLAttributesHTML[] = {&nsGkAtoms::action,
                                       &nsGkAtoms::href,
                                       &nsGkAtoms::src,
                                       &nsGkAtoms::longdesc,
                                       &nsGkAtoms::cite,
...
nsAtom** const kURLAttributesSVG[] = {&nsGkAtoms::href, nullptr};
...
nsAtom** const kURLAttributesMathML[] = {&nsGkAtoms::href,
                                         &nsGkAtoms::src,
                                         &nsGkAtoms::cdgroup_,
                                         &nsGkAtoms::altimg_,
                                    ...
                                    nsIURI* aBaseURI)
{
  ...
  if (aDocument->IsStyledByServo()) {
    rv = sheet->AsServo()->ParseSheet(aDocument->CSSLoader(),
    ...
    rv = parser.ParseSheet(aOriginal,
                           aDocument->GetDocumentURI(),
                           aBaseURI,
                           aDocument->NodePrincipal(),
                           0);
    ...
    bool aAllowDangerousSrc)
{
    ...
    if (kNameSpaceID_None == attrNs) {
      if (aAllowStyle && nsGkAtoms::style == attrLocal) {
        ...
        if (document->IsStyledByServo()) {
          ...
          decl = parser.ParseStyleAttribute(value,
        ...
        if (decl) {
          if (SanitizeStyleDeclaration(decl)) {
            ...
            aElement->SetAttr(
                kNameSpaceID_None, nsGkAtoms::style, cleanValue, false);
          }
        }
        continue;
      }
  ...
  if (aElement->IsAnyOfHTMLElements(nsGkAtoms::video, nsGkAtoms::audio)) {
    aElement->SetAttr(
        kNameSpaceID_None, nsGkAtoms::controls, EmptyString(), false);
  }
}
  ...
  while (node) {
    if (node->IsElement()) {
      ...
      if (nsGkAtoms::style == localName) {
        ...
        if (SanitizeStyleSheet(
                styleText, sanitizedStyle, aRoot->OwnerDoc(), baseURI)) {
