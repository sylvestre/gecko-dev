...
#define FOR_EACH_SIZE(macro) \
  macro(Other, mLineBoxes) macro(Style, mRuleNodes) macro(Style, mStyleContexts)
...
{
#define FOR_EACH_SIZE(macro)                                                       \
  macro(DOM, mDOMElementNodesSize) macro(DOM, mDOMTextNodesSize) macro(            \
      DOM, mDOMCDATANodesSize) macro(DOM, mDOMCommentNodesSize)                    \
      macro(DOM, mDOMEventTargetsSize) macro(                                      \
          DOM,                                                                     \
          mDOMPerformanceUserEntries) macro(DOM,                                   \
                                            mDOMPerformanceResourceEntries)        \
          macro(DOM, mDOMOtherSize) macro(Style, mLayoutStyleSheetsSize) macro(    \
              Other, mLayoutPresShellSize) macro(Style, mLayoutGeckoStyleSets)     \
              macro(Style, mLayoutServoStyleSetsStylistRuleTree) macro(            \
                  Style, mLayoutServoStyleSetsStylistElementAndPseudosMaps)        \
                  macro(Style, mLayoutServoStyleSetsStylistInvalidationMap) macro( \
                      Style,                                                       \
                      mLayoutServoStyleSetsStylistRevalidationSelectors)           \
                      macro(Style, mLayoutServoStyleSetsStylistOther) macro(       \
                          Style, mLayoutServoStyleSetsOther)                       \
                          macro(Style, mLayoutServoElementDataObjects) macro(      \
                              Other, mLayoutTextRunsSize)                          \
                              macro(Other, mLayoutPresContextSize) macro(          \
                                  Other, mLayoutFramePropertiesSize)               \
