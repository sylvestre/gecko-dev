...
PendingIPCBlobParent::Create(PBackgroundParent* aManager, BlobImpl* aBlobImpl)
{
  MOZ_ASSERT(aManager);
...
PendingIPCBlobParent::Recv__delete__(const PendingIPCBlobData& aData)
{
  if (aData.file().type() == PendingIPCFileUnion::Tvoid_t) {
    ...
    mBlobImpl->SetLazyData(
        fileData.name(), aData.type(), aData.size(), fileData.lastModified());
  }
