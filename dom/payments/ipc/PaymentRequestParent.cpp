...
PaymentRequestParent::RecvRequestPayment(
    const IPCPaymentActionRequest& aRequest)
{
  ...
  switch (aRequest.type()) {
    ...
    case IPCPaymentActionRequest::TIPCPaymentCompleteActionRequest: {
      ...
      rv = completeAction->InitRequest(
          request.requestId(), callback, request.completeStatus());
      if (NS_WARN_IF(NS_FAILED(rv))) {
        return IPC_FAIL_NO_REASON(this);
      }
                                            ...
                                            nsIPaymentAddress* aAddress)
{
  ...
  IPCPaymentAddress ipcAddress(country,
                               addressLine,
                               region,
                               city,
                               dependentLocality,
