 ...
 */
nsresult
GetPathToSpecialDir(const char* aKey, nsString& aOutPath)
{
...
DelayedPathSetter::Observe(nsISupports*, const char* aTopic, const char16_t*)
{
  ...
  nsresult rv =
      GetPathToSpecialDir(NS_APP_USER_PROFILE_50_DIR, gPaths->profileDir);
  if (NS_FAILED(rv)) {
    return rv;
  }
  rv = GetPathToSpecialDir(NS_APP_USER_PROFILE_LOCAL_50_DIR,
                           gPaths->localProfileDir);
  if (NS_FAILED(rv)) {
    return rv;
  }
...
InitOSFileConstants()
{
  ...
  GetPathToSpecialDir(NS_OSX_LOCAL_APPLICATIONS_DIR,
                      paths->macLocalApplicationsDir);
 ...
 */
JSObject*
GetOrCreateObjectProperty(JSContext* cx,
 ...
 */
bool
SetStringProperty(JSContext* cx,
                  JS::Handle<JSObject*> aObject,
                  const char* aProperty,
...
DefineOSFileConstants(JSContext* cx, JS::Handle<JSObject*> global)
{
  ...
  if (!gPaths->localProfileDir.IsVoid() &&
      !SetStringProperty(
          cx, objPath, "localProfileDir", gPaths->localProfileDir)) {
    return false;
  }
  ...
  if (!SetStringProperty(cx,
                         objPath,
                         "userApplicationDataDir",
                         gPaths->userApplicationDataDir)) {
    return false;
  }
  ...
  if (!SetStringProperty(
          cx, objPath, "winStartMenuProgsDir", gPaths->winStartMenuProgsDir)) {
    return false;
  }
#endif  // defined(XP_WIN)
  ...
  if (!SetStringProperty(cx,
                         ...
                         gPaths->macLocalApplicationsDir)) {
    return false;
  }
