...
static nsAtom** const unitMap[] = {nullptr, /* SVG_LENGTHTYPE_UNKNOWN */
...
{
  if (unitStr.IsEmpty()) return nsIDOMSVGLength::SVG_LENGTHTYPE_NUMBER;
...
static bool
GetValueFromString(const nsAString& aString, float& aValue, uint16_t* aUnitType)
{
...
float
nsSVGLength2::GetUnitScaleFactor(SVGViewportElement* aCtx,
                                 uint8_t aUnitType) const
{
...
float
nsSVGLength2::GetUnitScaleFactor(const UserSpaceMetrics& aMetrics,
                                 uint8_t aUnitType) const
{
  switch (aUnitType) {
...
void
nsSVGLength2::SetBaseValue(float aValue,
                           nsSVGElement* aSVGElement,
                           bool aDoSetAttr)
{
...
nsresult
nsSVGLength2::SMILLength::ValueFromString(
