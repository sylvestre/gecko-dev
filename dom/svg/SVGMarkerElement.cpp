...
nsresult
nsSVGOrientType::SetBaseValue(uint16_t aValue, nsSVGElement* aSVGElement)
{
  ...
  if (aValue == SVG_MARKER_ORIENT_AUTO || aValue == SVG_MARKER_ORIENT_ANGLE ||
    ...
    SetBaseValue(aValue);
    aSVGElement->SetAttr(kNameSpaceID_None,
                         nsGkAtoms::orient,
                         nullptr,
...
bool
SVGMarkerElement::ParseAttribute(int32_t aNameSpaceID,
                                 nsAtom* aName,
                                 ...
                                 nsAttrValue& aResult)
{
  ...
  return SVGMarkerElementBase::ParseAttribute(
      aNameSpaceID, aName, aValue, aResult);
}

nsresult
SVGMarkerElement::UnsetAttr(int32_t aNamespaceID, nsAtom* aName, bool aNotify)
{
...
{
  return AngleAttributesInfo(
      mAngleAttributes, sAngleInfo, ArrayLength(sAngleInfo));
}
...
gfx::Matrix
SVGMarkerElement::GetMarkerTransform(float aStrokeWidth, const nsSVGMark& aMark)
{
  float scale =
  ...
  return gfx::Matrix(cos(angle) * scale,
                     sin(angle) * scale,
                     -sin(angle) * scale,
                     cos(angle) * scale,
                     aMark.x,
...
SVGMarkerElement::GetViewBoxTransform()
{
  if (!mViewBoxToViewportTransform) {
    ...
    gfx::Matrix viewBoxTM =
        SVGContentUtils::GetViewBoxTransform(viewportWidth,
                                             viewportHeight,
                                             viewbox.x,
                                             viewbox.y,
                                             viewbox.width,
