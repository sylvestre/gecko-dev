...
static void
ThrowExceptionValueIfSafe(JSContext* aCx,
...
Throw(JSContext* aCx, nsresult aRv, const nsACString& aMessage)
{
  ...
  if (aMessage.IsEmpty() && existingException) {
    nsresult nr;
    if (NS_SUCCEEDED(existingException->GetResult(&nr)) && aRv == nr) {
...
NS_IMETHODIMP
JSStackFrame::GetLineNumber(JSContext* aCx, int32_t* aLineNumber)
{
...
NS_IMETHODIMP
JSStackFrame::GetColumnNumber(JSContext* aCx, int32_t* aColumnNumber)
{
...
NS_IMETHODIMP
JSStackFrame::GetSourceLine(nsACString& aSourceLine)
{
...
NS_IMETHODIMP
JSStackFrame::GetAsyncCause(JSContext* aCx, nsAString& aAsyncCause)
{
  if (!mStack) {
...
NS_IMETHODIMP
JSStackFrame::GetAsyncCaller(JSContext* aCx, nsIStackFrame** aAsyncCaller)
{
  if (!mStack) {
...
NS_IMETHODIMP
JSStackFrame::GetCaller(JSContext* aCx, nsIStackFrame** aCaller)
{
  if (!mStack) {
    *aCaller = nullptr;
    return NS_OK;
...
NS_IMETHODIMP
JSStackFrame::GetNativeSavedFrame(JS::MutableHandle<JS::Value> aSavedFrame)
{
