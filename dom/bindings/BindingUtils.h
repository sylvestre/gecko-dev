...
{
  return UnwrapArgImpl(
      cx, src, NS_GET_TEMPLATE_IID(Interface), reinterpret_cast<void**>(ppArg));
}

template<>
inline nsresult
UnwrapArg<nsPIDOMWindowOuter>(JSContext* cx,
                              JS::Handle<JSObject*> src,
                              nsPIDOMWindowOuter** ppArg)
{
...
template<class Interface>
inline nsresult
UnwrapXPConnect(JSContext* cx,
                JS::MutableHandle<JS::Value> val,
                Interface** ppThis)
{
  return UnwrapXPConnectImpl(cx,
                             val,
                     ...
                     uint32_t protoDepth)
{
  ...
  T* tempValue = nullptr;
  nsresult rv = UnwrapObjectInternal<T, false>(
      unwrappedObj, tempValue, protoID, protoDepth);
  if (NS_SUCCEEDED(rv)) {
...
class ProtoAndIfaceCache
{
  ...
  {
   public:
    PageTableCache() { memset(&mPages, 0, sizeof(mPages)); }
...
AllocateProtoAndIfaceCache(JSObject* obj, ProtoAndIfaceCache::Kind aKind)
{
  ...
  js::SetReservedSlot(
      obj, DOM_PROTOTYPE_SLOT, JS::PrivateValue(protoAndIfaceCache));
}
...
MOZ_ALWAYS_INLINE
bool
MaybeWrapNonDOMObjectOrNullValue(JSContext* cx,
...
MOZ_ALWAYS_INLINE bool
DoGetOrCreateDOMReflector(JSContext* cx,
                          T* value,
...
template<class T>
MOZ_ALWAYS_INLINE bool
GetOrCreateDOMReflector(JSContext* cx,
                        T* value,
                        JS::MutableHandle<JS::Value> rval,
...
template<class T>
MOZ_ALWAYS_INLINE bool
GetOrCreateDOMReflectorNoWrap(JSContext* cx,
                              T* value,
                              JS::MutableHandle<JS::Value> rval)
{
  using namespace binding_detail;
...
WrapNewBindingNonWrapperCachedObject(JSContext* cx,
                                     ...
                                     JS::Handle<JSObject*> givenProto = nullptr)
{
  return WrapNewBindingNonWrapperCachedObject(
      cx, scope, value.get(), rval, givenProto);
}
...
{
  return WrapNewBindingNonWrapperCachedObject(
      cx, scope, &value, rval, givenProto);
}
...
template<class T>
MOZ_ALWAYS_INLINE bool
HandleNewBindingWrappingFailure(JSContext* cx,
                                JS::Handle<JSObject*> scope,
                                T* value,
                                JS::MutableHandle<JS::Value> rval)
{
  ...
  return NativeInterface2JSObjectAndThrowIfFailed(
      cx, scope, rval, helper, nullptr, true);
}
...
{
  static inline bool Wrap(JSContext* cx,
                          JS::Handle<JSObject*> scope,
                          const T& value,
                          JS::MutableHandle<JS::Value> rval)
  {
...
template<bool Fatal>
inline bool
EnumValueNotFound(JSContext* cx,
                  JS::HandleString str,
                  const char* type,
...
template<>
inline bool
EnumValueNotFound<false>(JSContext* cx,
                         JS::HandleString str,
                         const char* type,
                         const char* sourceDescription)
{
...
template<>
inline bool
EnumValueNotFound<true>(JSContext* cx,
                        JS::HandleString str,
                        const char* type,
                        const char* sourceDescription)
{
  ...
  return ThrowErrorMessage(
      cx, MSG_INVALID_ENUM_VALUE, sourceDescription, deflated.ptr(), type);
}

template<typename CharT>
inline int
FindEnumStringIndexImpl(const CharT* chars,
                        size_t length,
...
inline bool
FindEnumStringIndex(JSContext* cx,
...
template<>
inline bool
WrapObject<nsIVariant>(JSContext* cx,
                       nsIVariant* p,
                       nsWrapperCache* cache,
                       const nsIID* iid,
                       JS::MutableHandle<JS::Value> rval)
...
template<class T>
inline bool
WrapObject(JSContext* cx, const RefPtr<T>& p, JS::MutableHandle<JS::Value> rval)
{
  return WrapObject(cx, p, nullptr, rval);
}
...
template<>
inline bool
WrapObject<JSObject>(JSContext* cx,
                     JSObject* p,
                     JS::MutableHandle<JS::Value> rval)
{
...
template<typename T>
static inline JSObject*
FindAssociatedGlobal(JSContext* cx,
                     T* p,
                     nsWrapperCache* cache,
...
{
  return FindAssociatedGlobal(
      cx, GetParentPointer(p), GetWrapperCache(p), GetUseXBLScope(p));
}
...
{
  static inline bool GetOrCreate(JSContext* cx,
                                 const T& value,
                                 JS::Handle<JSObject*> givenProto,
                                 JS::MutableHandle<JS::Value> rval)
  {
...
GetOrCreateDOMReflector(JSContext* cx,
                        ...
                        JS::Handle<JSObject*> givenProto = nullptr)
{
  return GetOrCreateDOMReflectorHelper<T>::GetOrCreate(
      cx, value, givenProto, rval);
}
...
template<typename T>
static inline bool
ConvertJSValueToString(JSContext* cx,
                       JS::Handle<JS::Value> v,
...
template<typename T>
void
DoTraceSequence(JSTracer* trc, FallibleTArray<T>& seq);
template<typename T>
void
DoTraceSequence(JSTracer* trc, InfallibleTArray<T>& seq);
...
class SequenceTracer<Nullable<T>, false, false, false>
{
 ...
 public:
  static void TraceSequence(JSTracer* trc, Nullable<T>* seqp, Nullable<T>* end)
  {
    for (; seqp != end; ++seqp) {
      if (!seqp->IsNull()) {
...
void
TraceRecord(JSTracer* trc, Record<K, V>& record)
{
...
class SequenceTracer<Record<K, V>, false, false, false>
{
 ...
 public:
  static void TraceSequence(JSTracer* trc,
                            Record<K, V>* seqp,
                            Record<K, V>* end)
  {
    for (; seqp != end; ++seqp) {
...
DoTraceSequence(JSTracer* trc, FallibleTArray<T>& seq)
{
  SequenceTracer<T>::TraceSequence(
      trc, seq.Elements(), seq.Elements() + seq.Length());
}
...
class MOZ_RAII RootedUnion : public T, private JS::CustomAutoRooter
{
 public:
  explicit RootedUnion(JSContext* cx MOZ_GUARD_OBJECT_NOTIFIER_PARAM)
      : T(), JS::CustomAutoRooter(cx MOZ_GUARD_OBJECT_NOTIFIER_PARAM_TO_PARENT)
  {
  }
...
AddStringToIDVector(JSContext* cx, JS::AutoIdVector& vector, const char* name)
{
  return vector.growBy(1) &&
         AtomizeAndPinJSString(
             cx, *(vector[vector.length() - 1]).address(), name);
}
 ...
 */
inline bool
XrayGetNativeProto(JSContext* cx,
                   JS::Handle<JSObject*> obj,
...
inline JSObject*
GetCachedSlotStorageObject(JSContext* cx,
                           JS::Handle<JSObject*> obj,
                           bool* isXray)
{
  if (IsDOMObject(obj)) {
...
template<typename T>
const T&
NonNullHelper(const NonNull<T>& aArg)
{
  return aArg;
}
...
template<typename T>
T&
NonNullHelper(OwningNonNull<T>& aArg)
{
  return aArg;
}
...
template<typename T>
const T&
NonNullHelper(const OwningNonNull<T>& aArg)
{
  return aArg;
}

inline void
NonNullHelper(NonNull<binding_detail::FakeString>& aArg)
{
...
inline void
NonNullHelper(const NonNull<binding_detail::FakeString>& aArg)
{
  // This overload is here to make sure that we never end up applying
  // NonNullHelper to a NonNull<binding_detail::FakeString>. If we
  // try to, it should fail to compile, since presumably the caller will try to
...
inline void
NonNullHelper(binding_detail::FakeString& aArg)
{
  // This overload is here to make sure that we never end up applying
...
MOZ_ALWAYS_INLINE
const nsAString&
NonNullHelper(const binding_detail::FakeString& aArg)
{
  return aArg;
}
...
NonVoidByteStringToJsval(JSContext* cx,
                         ...
                         JS::MutableHandle<JS::Value> rval);
inline bool
ByteStringToJsval(JSContext* cx,
                  const nsACString& str,
                  JS::MutableHandle<JS::Value> rval)
{
  if (str.IsVoid()) {
...
class MOZ_STACK_CLASS BindingJSObjectCreator
{
 public:
  ...
  void CreateProxyObject(JSContext* aCx,
                         const js::Class* aClass,
                         ...
                         JS::MutableHandle<JSObject*> aReflector)
  {
    ...
    if (aReflector) {
      js::SetProxyReservedSlot(
          aReflector, DOM_OBJECT_SLOT, JS::PrivateValue(aNative));
      mNative = aNative;
  ...
  void CreateObject(JSContext* aCx,
...
struct DeferredFinalizerImpl
{
  typedef typename Conditional<
      IsSame<T, nsISupports>::value,
      nsCOMPtr<T>,
      typename Conditional<IsRefcounted<T>::value, RefPtr<T>, nsAutoPtr<T>>::
          Type>::Type SmartPtr;
             ...
             JS::MutableHandle<JSObject*> aGlobal)
{
  ...
  aGlobal.set(JS_NewGlobalObject(
      aCx, aClass, aPrincipal, JS::DontFireOnNewGlobalHook, aOptions));
  if (!aGlobal) {
...
template<
    class T,
    class ReturnType =
        typename Conditional<IsRefcounted<T>::value, T*, nsAutoPtr<T>>::Type>
inline ReturnType
