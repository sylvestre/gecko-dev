...
struct DisallowedConversion
{
 ...
 private:
  static inline bool converter(JSContext* cx,
...
template<typename T>
inline bool
PrimitiveConversionTraits_EnforceRange(JSContext* cx,
                                       const double& d,
                                       T* retval)
{
  ...
  if (!mozilla::IsFinite(d)) {
    return ThrowErrorMessage(
        cx, MSG_ENFORCE_RANGE_NON_FINITE, TypeName<T>::value());
  }
  ...
  if (rounded < PrimitiveConversionTraits_Limits<T>::min() ||
    ...
    return ThrowErrorMessage(
        cx, MSG_ENFORCE_RANGE_OUT_OF_RANGE, TypeName<T>::value());
  }
...
struct PrimitiveConversionTraits<T, eEnforceRange>
    : public PrimitiveConversionTraits_ToCheckedIntHelper<
          T,
          PrimitiveConversionTraits_EnforceRange<T> >
{
};
...
struct PrimitiveConversionTraits<T, eClamp>
    : public PrimitiveConversionTraits_ToCheckedIntHelper<
          T,
          PrimitiveConversionTraits_Clamp<T> >
{
};
...
struct PrimitiveConversionTraits<bool, eDefault>
{
  ...
  static inline bool converter(JSContext* /* unused */,
                               JS::Handle<JS::Value> v,
                               jstype* retval)
  {
...
template<typename T, ConversionBehavior B>
bool
ValueToPrimitive(JSContext* cx, JS::Handle<JS::Value> v, T* retval)
{
