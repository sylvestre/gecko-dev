...
static void
AssignSourceNameHelper(nsString& aSourceNameDest,
               ...
               nsACString& /*UTF8*/ aResult)
{
  ...
  if (nullptr != tempSourceName && nullptr != tempSourceLine) {
    ...
    temp = JS_smprintf(
        format1, aSeverity, tempMessage, tempSourceName, aLineNumber);
  } else {
...
nsScriptErrorBase::ToString(nsACString& /*UTF8*/ aResult)
{
  ...
  return ToStringHelper(severity,
                        mMessage,
                        mSourceName,
                        &mSourceLine,
                        mLineNumber,
...
nsScriptErrorBase::GetNotes(nsIArray** aNotes)
{
  ...
  for (uint32_t i = 0; i < len; i++) array->AppendElement(mNotes[i], false);
  array.forget(aNotes);
...
{
  return ToStringHelper("JavaScript Note",
                        mMessage,
                        mSourceName,
                        nullptr,
                        mLineNumber,
