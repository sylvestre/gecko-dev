...
inline nsresult
GetKeyLengthForAlgorithm(JSContext* aCx,
...
class AesTask : public ReturnArrayBufferViewTask, public DeferredData
{
 ...
 private:
  ...
  virtual nsresult DoCrypto() override
  {
    ...
    if (mEncrypt) {
      rv = MapSECStatus(PK11_Encrypt(symKey.get(),
                                     mMechanism,
                                     &param,
                                     mResult.Elements(),
                                     &outLen,
      ...
      rv = MapSECStatus(PK11_Decrypt(symKey.get(),
                                     mMechanism,
                                     &param,
                                     mResult.Elements(),
                                     &outLen,
 ...
 private:
  ...
  virtual nsresult DoCrypto() override
  {
    ...
    if (mEncrypt) {
      ...
      UniquePK11SymKey keyToWrap(PK11_ImportSymKey(slot.get(),
                                                   fakeMechanism,
                                                   PK11_OriginUnwrap,
                                                   fakeOperation,
                                                   &dataItem,
      ...
      SECItem resultItem = {
          siBuffer, mResult.Elements(), (unsigned int)mResult.Length()};
      rv = MapSECStatus(PK11_WrapSymKey(
          mMechanism, nullptr, symKey.get(), keyToWrap.get(), &resultItem));
      NS_ENSURE_SUCCESS(rv, NS_ERROR_DOM_OPERATION_ERR);
    } else {
      // Decrypt the ciphertext into a temporary PK11SymKey
      // Unwrapped key should be 64 bits shorter
      int keySize = mData.Length() - 8;
      UniquePK11SymKey unwrappedKey(PK11_UnwrapSymKey(symKey.get(),
                                                      mMechanism,
                                                      nullptr,
                                                      &dataItem,
                                                      fakeMechanism,
 ...
 private:
  ...
  virtual nsresult DoCrypto() override
  {
    ...
    if (mEncrypt) {
      ...
      rv = MapSECStatus(PK11_PubEncrypt(mPubKey.get(),
                                        CKM_RSA_PKCS_OAEP,
                                        &param,
                                        mResult.Elements(),
                                        &outLen,
      ...
      rv = MapSECStatus(PK11_PrivDecrypt(mPrivKey.get(),
                                         CKM_RSA_PKCS_OAEP,
                                         &param,
                                         mResult.Elements(),
                                         &outLen,
...
class HmacTask : public WebCryptoTask
{
 ...
 private:
  ...
  virtual nsresult DoCrypto() override
  {
    ...
    NS_ENSURE_SUCCESS(rv, NS_ERROR_DOM_OPERATION_ERR);
    rv = MapSECStatus(PK11_DigestFinal(
        ctx.get(), mResult.Elements(), &outLen, mResult.Length()));
    NS_ENSURE_SUCCESS(rv, NS_ERROR_DOM_OPERATION_ERR);
  ...
  virtual void Resolve() override
  {
    if (mSign) {
      ...
      if (equal) {
        int cmp = NSS_SecureMemcmp(
            mSignature.Elements(), mResult.Elements(), mSignature.Length());
        equal = (cmp == 0);
      }
  ...
  virtual nsresult DoCrypto() override
  {
    ...
    if (mAlgorithm == Algorithm::RSA_PKCS1) {
      ...
      if (!SEC_ASN1EncodeItem(
              nullptr, hash.get(), di.get(), SGN_DigestInfoTemplate)) {
        return NS_ERROR_DOM_OPERATION_ERR;
      }
    }
    ...
    if (mSign) {
      ...
      rv = PK11_SignWithMechanism(
          mPrivKey.get(), mech, params, sig.get(), hash.get());
      NS_ENSURE_SUCCESS(MapSECStatus(rv), NS_ERROR_DOM_OPERATION_ERR);
      ...
      rv = PK11_VerifyWithMechanism(
          mPubKey.get(), mech, params, sig.get(), hash.get(), nullptr);
...
class DigestTask : public ReturnArrayBufferViewTask
{
 ...
 private:
  ...
  virtual nsresult DoCrypto() override
  {
    ...
    // Compute the hash
    nsresult rv = MapSECStatus(PK11_HashBuf(
        mOidTag, mResult.Elements(), mData.Elements(), mData.Length()));
    if (NS_FAILED(rv)) {
      return NS_ERROR_DOM_UNKNOWN_ERR;
    }
...
{
 public:
  ImportSymmetricKeyTask(nsIGlobalObject* aGlobal,
                         JSContext* aCx,
                         const nsAString& aFormat,
                         const ObjectOrString& aAlgorithm,
                         bool aExtractable,
  ...
  {
    ImportKeyTask::Init(
        aGlobal, aCx, aFormat, aAlgorithm, aExtractable, aKeyUsages);
    if (NS_FAILED(mEarlyRv)) {
      return;
    }
...
class ImportRsaKeyTask : public ImportKeyTask
{
 ...
 private:
  ...
  virtual nsresult AfterCrypto() override
  {
    ...
    if (!mKey->Algorithm().MakeRsa(
            mAlgName, mModulusLength, mPublicExponent, mHashName)) {
      return NS_ERROR_DOM_OPERATION_ERR;
    }
...
{
 public:
  ImportEcKeyTask(nsIGlobalObject* aGlobal,
                  JSContext* aCx,
                  const nsAString& aFormat,
                  const ObjectOrString& aAlgorithm,
                  bool aExtractable,
...
{
 public:
  ImportDhKeyTask(nsIGlobalObject* aGlobal,
                  JSContext* aCx,
                  const nsAString& aFormat,
                  const ObjectOrString& aAlgorithm,
                  bool aExtractable,
  ...
  virtual nsresult DoCrypto() override
  {
    ...
    if (mFormat.EqualsLiteral(WEBCRYPTO_KEY_FORMAT_RAW)) {
      if (mPublicKey && mPublicKey->keyType == dhKey) {
        nsresult rv =
            CryptoKey::PublicDhKeyToRaw(mPublicKey.get(), mResult, locker);
        if (NS_FAILED(rv)) {
          return NS_ERROR_DOM_OPERATION_ERR;
        }
        return NS_OK;
      ...
      if (mPublicKey && mPublicKey->keyType == ecKey) {
        nsresult rv =
            CryptoKey::PublicECKeyToRaw(mPublicKey.get(), mResult, locker);
        if (NS_FAILED(rv)) {
          return NS_ERROR_DOM_OPERATION_ERR;
        }
        return NS_OK;
      ...
      if (mKeyType == CryptoKey::SECRET) {
        ...
        nsresult rv =
            CryptoKey::PrivateKeyToJwk(mPrivateKey.get(), mJwk, locker);
        if (NS_FAILED(rv)) {
          return NS_ERROR_DOM_OPERATION_ERR;
        }
      }
...
{
 public:
  GenerateSymmetricKeyTask(nsIGlobalObject* aGlobal,
                           JSContext* aCx,
                           const ObjectOrString& aAlgorithm,
                           bool aExtractable,
                           const Sequence<nsString>& aKeyUsages)
    ...
    : mKeyPair(new CryptoKeyPair())
{
  ...
  if (mAlgName.EqualsLiteral(WEBCRYPTO_ALG_RSASSA_PKCS1) ||
    ...
    // Create algorithm.
    if (!mKeyPair->mPublicKey.get()->Algorithm().MakeDh(
            mAlgName, prime, generator)) {
      mEarlyRv = NS_ERROR_DOM_OPERATION_ERR;
      return;
    }
    if (!mKeyPair->mPrivateKey.get()->Algorithm().MakeDh(
            mAlgName, prime, generator)) {
      mEarlyRv = NS_ERROR_DOM_OPERATION_ERR;
      return;
    }
...
class DeriveHkdfBitsTask : public ReturnArrayBufferViewTask
{
 ...
 private:
  ...
  virtual nsresult DoCrypto() override
  {
    ...
    UniquePK11SymKey baseKey(PK11_ImportSymKey(slot.get(),
                                               mMechanism,
                                               PK11_OriginUnwrap,
                                               CKA_WRAP,
                                               &keyItem,
    ...
    CK_NSS_HKDFParams hkdfParams = {
        true, salt.data, salt.len, true, info.data, info.len};
    SECItem params = {
        siBuffer, (unsigned char*)&hkdfParams, sizeof(hkdfParams)};

    // CKM_SHA512_HMAC and CKA_SIGN are key type and usage attributes of the
    // derived symmetric key and don't matter because we ignore them anyway.
    UniquePK11SymKey symKey(PK11_Derive(baseKey.get(),
                                        mMechanism,
                                        &params,
                                        CKM_SHA512_HMAC,
                                        CKA_SIGN,
...
class DerivePbkdfBitsTask : public ReturnArrayBufferViewTask
{
 ...
 private:
  ...
  virtual nsresult DoCrypto() override
  {
    ...
    UniqueSECAlgorithmID algID(PK11_CreatePBEV2AlgorithmID(SEC_OID_PKCS5_PBKDF2,
                                                           SEC_OID_HMAC_SHA1,
                                                           mHashOidTag,
                                                           mLength,
                                                           mIterations,
...
{
 public:
  DeriveKeyTask(nsIGlobalObject* aGlobal,
                JSContext* aCx,
                const ObjectOrString& aAlgorithm,
                CryptoKey& aBaseKey,
        ...
        mResolved(false)
  {
    ...
    mTask = new ImportSymmetricKeyTask(
...
class DeriveEcdhBitsTask : public ReturnArrayBufferViewTask
{
 ...
 private:
  ...
  virtual nsresult DoCrypto() override
  {
    // CKM_SHA512_HMAC and CKA_SIGN are key type and usage attributes of the
    // derived symmetric key and don't matter because we ignore them anyway.
    UniquePK11SymKey symKey(PK11_PubDeriveWithKDF(mPrivKey.get(),
                                                  mPubKey.get(),
                                                  PR_FALSE,
                                                  nullptr,
                                                  nullptr,
...
class WrapKeyTask : public ExportKeyTask
{
 public:
  WrapKeyTask(JSContext* aCx,
      ...
      : ExportKeyTask(aFormat, aKey), mResolved(false)
  {
    if (NS_FAILED(mEarlyRv)) {
      return;
    }
...
class UnwrapKeyTask : public KeyEncryptTask
{
 public:
  UnwrapKeyTask(JSContext* aCx,
                ...
                ImportKeyTask* aTask)
      : KeyEncryptTask(
            aCx, aUnwrapAlgorithm, aUnwrappingKey, aWrappedKey, false),
        mTask(aTask),
        mResolved(false)
  {
  }
                                    ...
                                    bool aSign)
{
  ...
  if (algName.EqualsLiteral(WEBCRYPTO_ALG_HMAC)) {
    ...
    return new AsymmetricSignVerifyTask(
        aCx, aAlgorithm, aKey, aSignature, aData, aSign);
  }
...
WebCryptoTask*
WebCryptoTask::CreateExportKeyTask(const nsAString& aFormat, CryptoKey& aKey)
{
                                     ...
                                     const Sequence<nsString>& aKeyUsages)
{
  ...
  if (algName.EqualsASCII(WEBCRYPTO_ALG_AES_CBC) ||
    ...
    return new GenerateSymmetricKeyTask(
        aGlobal, aCx, aAlgorithm, aExtractable, aKeyUsages);
    ...
    return new GenerateAsymmetricKeyTask(
        aGlobal, aCx, aAlgorithm, aExtractable, aKeyUsages);
  } else {
    return new FailureTask(NS_ERROR_DOM_NOT_SUPPORTED_ERR);
  }
                                   ...
                                   const Sequence<nsString>& aKeyUsages)
{
  ...
  if (algName.EqualsASCII(WEBCRYPTO_ALG_HKDF)) {
    return new DeriveKeyTask<DeriveHkdfBitsTask>(aGlobal,
                                                 aCx,
                                                 aAlgorithm,
                                                 aBaseKey,
                                                 aDerivedKeyType,
  ...
  if (algName.EqualsASCII(WEBCRYPTO_ALG_PBKDF2)) {
    return new DeriveKeyTask<DerivePbkdfBitsTask>(aGlobal,
                                                  aCx,
                                                  aAlgorithm,
                                                  aBaseKey,
                                                  aDerivedKeyType,
  ...
  if (algName.EqualsASCII(WEBCRYPTO_ALG_ECDH)) {
    return new DeriveKeyTask<DeriveEcdhBitsTask>(aGlobal,
                                                 aCx,
                                                 aAlgorithm,
                                                 aBaseKey,
                                                 aDerivedKeyType,
    ...
    const Sequence<nsString>& aKeyUsages)
{
  ...
  if (keyAlgName.EqualsASCII(WEBCRYPTO_ALG_AES_CBC) ||
    ...
    importTask = new ImportSymmetricKeyTask(aGlobal,
                                            aCx,
                                            aFormat,
    ...
    importTask = new ImportRsaKeyTask(aGlobal,
                                      aCx,
                                      aFormat,
                                      aUnwrappedKeyAlgorithm,
                                      aExtractable,
...
WebCryptoTask::~WebCryptoTask()
{
  ...
  if (mWorkerHolder) {
    NS_ProxyRelease("WebCryptoTask::mWorkerHolder",
