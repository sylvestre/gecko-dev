...
IMEStateManager::Init()
{
  ...
  Preferences::AddBoolVarCache(
      &sInputModeSupported, "dom.forms.inputmode", false);
...
IMEStateManager::Shutdown()
{
  MOZ_LOG(
      ...
      ("Shutdown(), sTextCompositions=0x%p, sTextCompositions->Length()=%zu",
       sTextCompositions,
       sTextCompositions ? sTextCompositions->Length() : 0));
                                       ...
                                       InputContextAction aAction)
{
  ...
  MOZ_LOG(sISMLog,
          ...
          ("OnChangeFocusInternal(aPresContext=0x%p (available: %s), "
           ...
           sPresContext.get(),
           GetBoolName(CanHandleWith(sPresContext)),
           sContent.get(),
           sWidget,
  ...
  if (setIMEState) {
    if (!focusActuallyChanging) {
      ...
      if (sPresContext && oldWidget && !focusActuallyChanging) {
        NotifyIME(
            REQUEST_TO_COMMIT_COMPOSITION, oldWidget, sFocusedIMETabParent);
      }
                                ...
                                EditorBase* aEditorBase)
{
  MOZ_LOG(sISMLog,
          ...
          ("UpdateIMEState(aNewIMEState={ mEnabled=%s, "
           ...
           GetIMEStateSetOpenName(aNewIMEState.mOpen),
           aContent,
           aEditorBase,
           sPresContext.get(),
           sContent.get(),
  ...
  if (sActiveIMEContentObserver && IsIMEObserverNeeded(aNewIMEState)) {
    ...
    if (!contentObserver->MaybeReinitialize(
            widget, sPresContext, aContent, aEditorBase)) {
      MOZ_LOG(sISMLog,
              LogLevel::Error,
                             ...
                             InputContext::Origin aOrigin)
{
  MOZ_LOG(sISMLog,
          ...
          ("SetIMEState(aState={ mEnabled=%s, mOpen=%s }, "
           ...
           GetIMEStateSetOpenName(aState.mOpen),
           aContent,
  ...
  if (aContent &&
    ...
    if (!aContent->IsHTMLElement(nsGkAtoms::textarea)) {
      ...
      content->GetAttr(
          kNameSpaceID_None, nsGkAtoms::type, context.mHTMLInputType);
    } else {
    ...
    if (sInputModeSupported ||
      ...
      aContent->GetAttr(
          kNameSpaceID_None, nsGkAtoms::inputmode, context.mHTMLInputInputmode);
    ...
    bool aIsSynthesized)
{
  ...
  if (!composition) {
    ...
    composition = new TextComposition(
        aPresContext, aEventTargetNode, tabParent, aCompositionEvent);
  ...
  composition->DispatchCompositionEvent(
      aCompositionEvent, aStatus, aCallBack, aIsSynthesized);
                                      ...
                                      WidgetSelectionEvent* aSelectionEvent)
{
  ...
  if (composition) {
    ...
    TextComposition::HandleSelectionEvent(
        aPresContext, tabParent, aSelectionEvent);
  }
}
...
IMEStateManager::NotifyIME(IMEMessage aMessage,
                           ...
                           TabParent* aTabParent)
{
  return IMEStateManager::NotifyIME(
      IMENotification(aMessage), aWidget, aTabParent);
}
                           ...
                           TabParent* aTabParent)
{
  ...
  if (NS_WARN_IF(!aWidget)) {
    MOZ_LOG(
        sISMLog, LogLevel::Error, ("  NotifyIME(), FAILED due to no widget"));
