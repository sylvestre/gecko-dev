                                           ...
                                           int16_t aReason)
{
  ...
  if (count > 0 && mWidget) {
    ...
    MaybeNotifyIMEOfSelectionChange(
        causedByComposition, causedBySelectionEvent, duringComposition);
  }
  return NS_OK;
}
...
NS_IMETHODIMP
IMEContentObserver::Reflow(DOMHighResTimeStamp aStart, DOMHighResTimeStamp aEnd)
{
  if (!NeedsPositionChangeNotification()) {
    return NS_OK;
  }
                                       ...
                                       WidgetMouseEvent* aMouseEvent)
{
  ...
  WidgetQueryContentEvent charAtPt(
      true, eQueryCharacterAtPoint, aMouseEvent->mWidget);
...
{
  MOZ_ASSERT(aChild);
  NotifyContentAdded(NODE_FROM(aContainer, aDocument), aChild, aChild);
}
                                   ...
                                   nsIContent* aPreviousSibling)
{
  ...
  if (!mStartOfRemovingTextRangeCache.Match(containerNode, aPreviousSibling)) {
    ...
    mStartOfRemovingTextRangeCache.Cache(
        containerNode, aPreviousSibling, offset);
  } else {
                                     ...
                                     const nsAttrValue* aOldValue)
{
  ...
  TextChangeData data(start,
                      start + mPreAttrChangeLength,
...
IMEContentObserver::BeginDocumentUpdate()
{
  MOZ_LOG(sIMECOLog,
          ...
          ("0x%p IMEContentObserver::BeginDocumentUpdate(), "
           ...
           this,
           ToChar(HasAddedNodesDuringDocumentChange())));
...
IMEContentObserver::PostTextChangeNotification()
{
  MOZ_LOG(sIMECOLog,
          ...
          ("0x%p IMEContentObserver::PostTextChangeNotification("
           "mTextChangeData=%s)",
           this,
           TextChangeDataToString(mTextChangeData).get()));
...
IMEContentObserver::PostSelectionChangeNotification()
{
  MOZ_LOG(
      ...
      ("0x%p IMEContentObserver::PostSelectionChangeNotification(), "
       ...
       this,
       ToChar(mSelectionData.mCausedByComposition),
    ...
    const TextChangeDataBase& aTextChangeData)
{
  MOZ_LOG(sIMECOLog,
          ...
          ("0x%p IMEContentObserver::MaybeNotifyIMEOfTextChange("
           "aTextChangeData=%s)",
           this,
           TextChangeDataToString(aTextChangeData).get()));
    ...
    bool aOccurredDuringComposition)
{
  MOZ_LOG(sIMECOLog,
          ...
          ("0x%p IMEContentObserver::MaybeNotifyIMEOfSelectionChange("
           ...
           this,
           ToChar(aCausedByComposition),
           ToChar(aCausedBySelectionEvent)));
  ...
  *mSelectionData.mString = selection.mReply.mString;
  ...
  MOZ_LOG(sIMECOLog,
          ...
          ("0x%p IMEContentObserver::UpdateSelectionCache(), "
           "mSelectionData=%s",
           this,
           SelectionChangeDataToString(mSelectionData).get()));
  ...
  if (observer->mSendingNotification != NOTIFY_IME_OF_NOTHING) {
    MOZ_LOG(
        ...
        ("0x%p   IMEContentObserver::AChangeEvent::IsSafeToNotifyIME(), "
         ...
         this,
         ToChar(observer->mSendingNotification)));
    return false;
  }
  State state = observer->GetState();
  if (aChangeEventType == eChangeEventType_Focus) {
...
IMEContentObserver::IMENotificationSender::SendSelectionChange()
{
  ...
  MOZ_LOG(sIMECOLog,
          ...
          ("0x%p IMEContentObserver::IMENotificationSender::"
           ...
           "newSelChangeData=%s",
           this,
           SelectionChangeDataToString(newSelChangeData).get()));
...
IMEContentObserver::IMENotificationSender::SendTextChange()
{
  ...
  MOZ_LOG(sIMECOLog,
          ...
          ("0x%p IMEContentObserver::IMENotificationSender::"
           ...
           this,
           TextChangeDataToString(observer->mTextChangeData).get()));
