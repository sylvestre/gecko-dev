...
class AudioBuffer final : public nsWrapperCache
{
 public:
  ...
  {
    return Create(
        aWindow, aNumberOfChannels, aLength, aSampleRate, nullptr, aRv);
  }
   ...
   */
  void GetChannelData(JSContext* aJSContext,
                      uint32_t aChannel,
  ...
  void CopyFromChannel(const Float32Array& aDestination,
                       ...
                       ErrorResult& aRv);
  void CopyToChannel(JSContext* aJSContext,
 ...
 protected:
  AudioBuffer(nsPIDOMWindowInner* aWindow,
              uint32_t aNumberOfChannels,
              uint32_t aLength,
              float aSampleRate,
              ErrorResult& aRv);
