...
NS_IMPL_CYCLE_COLLECTION_UNLINK_BEGIN_INHERITED(PannerNode, AudioNode)
  ...
  NS_IMPL_CYCLE_COLLECTION_UNLINK(mPositionX,
                                  mPositionY,
                                  mPositionZ,
                                  mOrientationX,
                                  mOrientationY,
...
NS_IMPL_CYCLE_COLLECTION_TRAVERSE_BEGIN_INHERITED(PannerNode, AudioNode)
  NS_IMPL_CYCLE_COLLECTION_TRAVERSE(mPositionX,
                                    mPositionY,
                                    mPositionZ,
                                    mOrientationX,
                                    mOrientationY,
...
class PannerNodeEngine final : public AudioNodeEngine
{
 public:
  ...
  void ComputeAzimuthAndElevation(const ThreeDPoint& position,
                                  float& aAzimuth,
                                  float& aElevation);
  ...
  void EqualPowerPanningFunction(const AudioBlock& aInput,
                                 AudioBlock* aOutput,
                                 StreamTime tick);
  void HRTFPanningFunction(const AudioBlock& aInput,
                           AudioBlock* aOutput,
                           StreamTime tick);
...
PannerNode::PannerNode(AudioContext* aContext)
    : AudioNode(aContext,
      ...
      mOrientationX(new AudioParam(
          this, PannerNode::ORIENTATIONX, this->NodeType(), 1.0f)),
      mOrientationY(new AudioParam(
          this, PannerNode::ORIENTATIONY, this->NodeType(), 0.f)),
      mOrientationZ(new AudioParam(
          this, PannerNode::ORIENTATIONZ, this->NodeType(), 0.f)),
      mVelocity(),
      mRefDistance(1.),
      mMaxDistance(10000.),
      mRolloffFactor(1.),
                   ...
                   ErrorResult& aRv)
{
  ...
  audioNode->SetPosition(
      aOptions.mPositionX, aOptions.mPositionY, aOptions.mPositionZ);
  audioNode->SetOrientation(
      aOptions.mOrientationX, aOptions.mOrientationY, aOptions.mOrientationZ);
...
void
PannerNode::SetPanningModel(PanningModelType aPanningModel)
{
...
ThreeDPoint
PannerNodeEngine::ConvertAudioParamTimelineTo3DP(AudioParamTimeline& aX,
                                                 AudioParamTimeline& aY,
                                                 AudioParamTimeline& aZ,
                                                 StreamTime& tick)
{
                                            ...
                                            StreamTime tick)
{
  ...
  if (mPositionX.HasSimpleValue() && mPositionY.HasSimpleValue() &&
    ...
    // this node is noop.
    if (mListenerPosition == position && mConeInnerAngle == 360 &&
        mConeOuterAngle == 360) {
      *aOutput = aInput;
      return;
    }
...
void
PannerNodeEngine::ComputeAzimuthAndElevation(const ThreeDPoint& position,
                                             float& aAzimuth,
                                             float& aElevation)
{
