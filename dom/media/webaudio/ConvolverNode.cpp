...
class ConvolverNodeEngine final : public AudioNodeEngine
{
 ...
 public:
  ...
  void SetBuffer(AudioChunk&& aBuffer) override
  {
    ...
    mReverb = new WebCore::Reverb(
        aBuffer, MaxFFTSize, mUseBackgroundThreads, mNormalize, mSampleRate);
  }
...
ConvolverNode::Create(JSContext* aCx,
  ...
  if (aBuffer) {
    ...
    if (data.mBufferFormat == AUDIO_FORMAT_S16) {
      ...
      for (size_t i = 0; i < data.ChannelCount(); ++i) {
        ConvertAudioSamples(
            data.ChannelData<int16_t>()[i], floatData, data.mDuration);
