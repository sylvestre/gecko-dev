...
IIRFilter::IIRFilter(const AudioDoubleArray* feedforwardCoef,
    ...
    : m_bufferIndex(0), m_feedback(feedbackCoef), m_feedforward(feedforwardCoef)
{
...
evaluatePolynomial(const double* coef, std::complex<double> z, int order)
{
...
void
IIRFilter::process(const float* sourceP, float* destP, size_t framesToProcess)
{
  // Compute
  //
...
void
IIRFilter::getFrequencyResponse(int nFrequencies,
