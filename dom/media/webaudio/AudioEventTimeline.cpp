...
static float
LinearInterpolate(double t0, float v0, double t1, float v1, double t)
{
  return v0 + (v1 - v0) * ((t - t0) / (t1 - t0));
}

static float
ExponentialInterpolate(double t0, float v0, double t1, float v1, double t)
{
...
static float
ExponentialApproach(
    double t0, double v0, float v1, double timeConstant, double t)
{
...
static float
ExtractValueFromCurve(double startTime,
                      float* aCurve,
                      ...
                      double t)
{
  ...
  if (next < aCurveLength) {
    ...
    return LinearInterpolate(
        t0, aCurve[current], t1, aCurve[next], t - startTime);
  } else {
...
void
AudioEventTimeline::GetValuesAtTimeHelper(TimeType aTime,
                                          float* aBuffer,
                                          const size_t aSize)
{
  MOZ_ASSERT(aBuffer);
...
template void
AudioEventTimeline::GetValuesAtTimeHelper(double aTime,
                                          float* aBuffer,
                                          const size_t aSize);
template void
AudioEventTimeline::GetValuesAtTimeHelper(int64_t aTime,
                                          float* aBuffer,
                                          const size_t aSize);
  ...
  switch (aNext->mType) {
    case AudioTimelineEvent::SetTarget:
      ...
      return ExponentialApproach(
          time, mLastComputedValue, aNext->mValue, aNext->mTimeConstant, time);
      break;
    case AudioTimelineEvent::SetValueCurve:
      // SetValueCurve events can be handled no matter what their event
      // node is (if they have one)
      return ExtractValueFromCurve(
          time, aNext->mCurve, aNext->mCurveLength, aNext->mDuration, time);
      break;
    default:
...
template<class TimeType>
float
AudioEventTimeline::GetValuesAtTimeHelperInternal(
    TimeType aTime,
