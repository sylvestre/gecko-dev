 ...
 * cancel each other. */
  void MixerCallback(AudioDataValue* aData,
    ...
    for (uint32_t i = 0; i < aChannels * aFrames; i++) {
      if (aData[i] != 0.0) {
        if (aFormat == mozilla::AUDIO_FORMAT_S16) {
          fprintf(
              stderr, "Sample at %d is not silent: %d\n", i, (short)aData[i]);
        } else {
          fprintf(
              stderr, "Sample at %d is not silent: %f\n", i, (float)aData[i]);
        }
        silent = false;
      }
    }
...
void
FillBuffer(AudioDataValue* aBuffer, uint32_t aLength, AudioDataValue aValue)
{
...
TEST(AudioMixer, Test)
{
  ...
  FillBuffer(
      a + CHANNEL_LENGTH, CHANNEL_LENGTH, GetHighValue<AudioDataValue>());
...

  {
    ...
    FillBuffer(b, CHANNEL_LENGTH, GetHighValue<AudioDataValue>());
    FillBuffer(
        b + CHANNEL_LENGTH, CHANNEL_LENGTH, GetLowValue<AudioDataValue>());
    mixer.Mix(a, 2, CHANNEL_LENGTH, AUDIO_RATE);
    mixer.Mix(b, 2, CHANNEL_LENGTH, AUDIO_RATE);
    mixer.FinishMixing();
    FillBuffer(a, CHANNEL_LENGTH / 2, GetLowValue<AudioDataValue>());
