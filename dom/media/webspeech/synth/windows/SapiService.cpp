...
{
 public:
  SapiCallback(nsISpeechTask* aTask,
               ISpVoice* aSapiClient,
               uint32_t aTextOffset,
               uint32_t aSpeakTextLen)
      : mTask(aTask),
  ...
  switch (speechEvent.eEventId) {
    ...
    case SPEI_WORD_BOUNDARY:
      ...
      mTask->DispatchBoundary(NS_LITERAL_STRING("word"),
                              GetTickCount() - mStartingTime,
                              mCurrentIndex,
                              static_cast<ULONG>(speechEvent.wParam),
                              1);
      break;
...
// static
void __stdcall SapiService::SpeechEventCallback(WPARAM aWParam, LPARAM aLParam)
{
...
SapiService::RegisterVoices()
{
  ...
  if (FAILED(category->EnumTokens(
          nullptr, nullptr, getter_AddRefs(voiceTokens)))) {
    return false;
  }
  ...
  while (true) {
    ...
    rv = registry->AddVoice(this,
                            uri,
                            nsDependentString(description),
...
NS_IMETHODIMP
SapiService::Speak(const nsAString& aText,
...
NS_IMETHODIMP
SapiService::Observe(nsISupports* aSubject,
                     const char* aTopic,
