...
DecoderDoctorDocumentWatcher::RetrieveOrCreate(nsIDocument* aDocument)
{
  ...
  if (!watcher) {
    ...
    if (NS_WARN_IF(NS_FAILED(aDocument->SetProperty(nsGkAtoms::decoderDoctor,
                                                    watcher.get(),
                                                    DestroyPropertyCallback,
                                                    /*transfer*/ false)))) {
      DD_WARN(
...
MediaResultDescription(const MediaResult& aResult, bool aIsError)
{
  ...
  return NS_ConvertUTF8toUTF16(
      nsPrintfCString("%s Code: %s (0x%08" PRIx32 ")%s%s",
                      aIsError ? "Error" : "Warning",
                      name.get(),
                ...
                nsTArray<const char16_t*>& aParams)
{
  ...
  nsContentUtils::ReportToConsole(
      ...
      aConsoleStringId,
      aParams.IsEmpty() ? nullptr : aParams.Elements(),
      aParams.Length());
}
               ...
               const nsAString& aResourceURL = NS_LITERAL_STRING(""))
{
  ...
  if (AllowNotification(aNotification) &&
    ...
    DispatchNotification(aDocument->GetInnerWindow(),
                         aNotification,
                         aIsSolved,
                         aFormats,
...
DecoderDoctorDocumentWatcher::SynthesizeAnalysis()
{
  ...
  if (!unsupportedKeySystems.IsEmpty() && supportedKeySystems.IsEmpty()) {
    ...
    switch (lastKeySystemIssue) {
      case DecoderDoctorDiagnostics::eWidevineWithNoWMF:
        ...
        ReportAnalysis(
            mDocument, sMediaWidevineNoWMF, false, unsupportedKeySystems);
        return;
      default:
        break;
    }
  ...
  if (!unplayableFormats.IsEmpty()) {
    ...
    // going through expected decoders from most to least desirable.
      ...
      ReportAnalysis(
          mDocument, sMediaCannotPlayNoDecoders, false, unplayableFormats);
      return;
    }
...
void
DecoderDoctorDocumentWatcher::AddDiagnostics(
    DecoderDoctorDiagnostics&& aDiagnostics, const char* aCallSite)
{
  ...
  DD_DEBUG(
      ...
      this,
      mDocument,
      aDiagnostics.GetDescription().Data(),
      aCallSite);
                                                 ...
                                                 const char* aCallSite)
{
  ...
  if (NS_WARN_IF(!aDocument)) {
    DD_WARN(
        ...
        this,
        NS_ConvertUTF16toUTF8(aFormat).get(),
        aCanPlay,
        aCallSite);
    return;
  }
