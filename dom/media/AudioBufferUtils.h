...
FramesToSamples(uint32_t aChannels, uint32_t aFrames)
{
  return aFrames * aChannels;
}

static inline uint32_t
SamplesToFrames(uint32_t aChannels, uint32_t aSamples)
{
...
class AudioCallbackBufferWrapper
{
 public:
  ...
  void WriteFrames(T* aBuffer, uint32_t aFrames)
  {
    ...
    PodCopy(mBuffer + mSampleWriteOffset,
            aBuffer,
...
class SpillBuffer
{
 public:
  ...
  explicit SpillBuffer(uint32_t aChannels) : mPosition(0), mChannels(aChannels)
  {
    MOZ_ASSERT(aChannels);
