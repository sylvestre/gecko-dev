...
static const char*
EnumToASCII(const EnumValuesStrings& aStrings, Enum aValue)
{
...
template<class EnumValuesStrings, class Enum>
static Enum
StringToEnum(const EnumValuesStrings& aStrings,
 ...
 public:
  NormalizedConstraintSet(const dom::MediaTrackConstraintSet& aOther,
      ...
      : mWidth(&T::mWidth, "width", aOther.mWidth, advanced, aList),
        ...
        mFrameRate(
            &T::mFrameRate, "frameRate", aOther.mFrameRate, advanced, aList),
        mFacingMode(
            &T::mFacingMode, "facingMode", aOther.mFacingMode, advanced, aList),
        mMediaSource(
            &T::mMediaSource, "mediaSource", aOther.mMediaSource, aList),
        mBrowserWindow(&T::mBrowserWindow,
                       ...
                       aOther.mBrowserWindow.WasPassed()
                           ...
                           : 0,
                       aList),
        mScrollWithPage(&T::mScrollWithPage,
                        "scrollWithPage",
        ...
        mViewportOffsetX(&T::mViewportOffsetX,
                         "viewportOffsetX",
                         aOther.mViewportOffsetX,
                         advanced,
                         aList),
        mViewportOffsetY(&T::mViewportOffsetY,
                         "viewportOffsetY",
                         aOther.mViewportOffsetY,
                         advanced,
                         aList),
        ...
        mAutoGainControl(&T::mAutoGainControl,
                         "autoGainControl",
                         aOther.mAutoGainControl,
                         advanced,
                         aList),
        mChannelCount(&T::mChannelCount,
                      "channelCount",
                      aOther.mChannelCount,
                      advanced,
                      aList)
  {
  }
                                    ...
                                    bool aIsChrome)
  {
    ...
    for (uint32_t i = 0; i < aDevices.Length();) {
      ...
      if (distance == UINT32_MAX) {
        ...
        ordered.insert(
            std::pair<uint32_t, RefPtr<DeviceType>>(distance, aDevices[i]));
        ++i;
      }
    }
    if (!aDevices.Length()) {
