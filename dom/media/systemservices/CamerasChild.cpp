...
class FakeOnDeviceChangeEventRunnable : public Runnable
{
 public:
  explicit FakeOnDeviceChangeEventRunnable(uint8_t counter)
      : Runnable("camera::FakeOnDeviceChangeEventRunnable"), mCounter(counter)
  {
  }
...
int
CamerasChild::AddDeviceChangeCallback(DeviceChangeCallback* aCallback)
{
                                   ...
                                   webrtc::VideoCaptureCapability& capability)
{
  ...
  nsCOMPtr<nsIRunnable> runnable = mozilla::
      NewNonOwningRunnableMethod<CaptureEngine, nsCString, unsigned int>(
    ...
    const VideoCaptureCapability& ipcCapability)
{
  ...
  mReplyCapability.rawType =
      static_cast<webrtc::RawVideoType>(ipcCapability.rawType());
  mReplyCapability.codecType =
      static_cast<webrtc::VideoCodecType>(ipcCapability.codecType());
                               ...
                               bool* scary)
{
  ...
  if (dispatcher.Success()) {
    base::strlcpy(
        device_nameUTF8, mReplyDeviceName.get(), device_nameUTF8Length);
...
int
CamerasChild::AllocateCaptureDevice(
...
void
CamerasChild::AddCallback(const CaptureEngine aCapEngine,
                          const int capture_id,
                          FrameRelay* render)
{
...
void
CamerasChild::RemoveCallback(const CaptureEngine aCapEngine,
                             const int capture_id)
{
  MutexAutoLock lock(mCallbackMutex);
...
CamerasChild::ShutdownChild()
{
  ...
  if (CamerasSingleton::Thread()) {
    ...
    RefPtr<ShutdownRunnable> runnable =
        new ShutdownRunnable(NewRunnableMethod("nsIThread::Shutdown",
...
FrameRelay*
CamerasChild::Callback(CaptureEngine aCapEngine, int capture_id)
{
  for (unsigned int i = 0; i < mCallbacks.Length(); i++) {
    CapturerElement ce = mCallbacks[i];
    if (ce.engine == aCapEngine && ce.id == capture_id) {
