...
TEST(ExtractVPXCodecDetails, TestDataLength)
{
  ...
  for (const auto& data : tests) {
    ...
    uint8_t level = 0;
    uint8_t bitDepth = 0;
    bool result = ExtractVPXCodecDetails(
        nsString(data.mCodecParameterString), profile, level, bitDepth);
...
TEST(ExtractVPXCodecDetails, TestParsingOutput)
{
  ...
  bool result = ExtractVPXCodecDetails(
      nsString(data), profile, level, bitDepth, chromaSubsampling, colorSpace);
