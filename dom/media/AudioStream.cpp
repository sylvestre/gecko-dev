...
#define LOGE(x, ...)                                                 \
  NS_DebugBreak(NS_DEBUG_WARNING,                                    \
                nsPrintfCString("%p " x, this, ##__VA_ARGS__).get(), \
                nullptr,                                             \
                __FILE__,                                            \
                __LINE__)
...
nsresult
AudioStream::EnsureTimeStretcherInitializedUnlocked()
{
...
static void
SetUint16LE(uint8_t* aDest, uint16_t aValue)
{
  aDest[0] = aValue & 0xFF;
  aDest[1] = aValue >> 8;
}
...
static void
SetUint32LE(uint8_t* aDest, uint32_t aValue)
{
...
int
AudioStream::InvokeCubeb(Function aFunction, Args&&... aArgs)
{
...
nsresult
AudioStream::OpenCubeb(cubeb* aContext,
...
AudioStream::IsValidAudioFormat(Chunk* aChunk)
{
  if (aChunk->Rate() != mAudioClock.GetInputRate()) {
    LOGW("mismatched sample %u, mInRate=%u",
         aChunk->Rate(),
  ...
  while (mTimeStretcher->numSamples() < aWriter.Available()) {
    ...
    if (IsValidAudioFormat(c.get())) {
      ...
      if (!size.isValid()) {
        ...
        LOGW("Invalid member data: %d channels, %d frames",
             mOutChannels,
             c->Frames());
        return;
      }
...
void
AudioClock::Init(uint32_t aRate)
{
  mOutRate = aRate;
  mInRate = aRate;
}
...
void
AudioClock::UpdateFrameHistory(uint32_t aServiced, uint32_t aUnderrun)
{
  mFrameHistory->Append(aServiced, aUnderrun, mOutRate);
}

int64_t
AudioClock::GetPositionInFrames(int64_t aFrames) const
{
...
int64_t
AudioClock::GetPosition(int64_t frames) const
{
  return mFrameHistory->GetPosition(frames);
}

void
AudioClock::SetPlaybackRate(double aPlaybackRate)
{
  mOutRate = static_cast<uint32_t>(mInRate / aPlaybackRate);
}

double
AudioClock::GetPlaybackRate() const
{
  return static_cast<double>(mInRate) / mOutRate;
}

void
AudioClock::SetPreservesPitch(bool aPreservesPitch)
{
  mPreservesPitch = aPreservesPitch;
}

bool
AudioClock::GetPreservesPitch() const
{
