...
void
_ReOrderInterleavedChannels(AudioDataType* aOut,
                            const AudioDataType* aIn,
                            uint32_t aFrames,
                            uint32_t aChannels,
...
void
AudioConverter::ReOrderInterleavedChannels(void* aOut,
                                           const void* aIn,
                                           size_t aFrames) const
{
  ...
  switch (bits) {
    case 8:
      _ReOrderInterleavedChannels((uint8_t*)aOut,
                                  (const uint8_t*)aIn,
                                  aFrames,
                                  mIn.Channels(),
                                  mChannelOrderMap);
      break;
    case 16:
      _ReOrderInterleavedChannels((int16_t*)aOut,
                                  (const int16_t*)aIn,
                                  aFrames,
                                  mIn.Channels(),
                                  mChannelOrderMap);
    ...
    default:
      ...
      _ReOrderInterleavedChannels((int32_t*)aOut,
                                  (const int32_t*)aIn,
                                  aFrames,
                                  mIn.Channels(),
                                  mChannelOrderMap);
...
AudioConverter::ResampleAudio(void* aOut, const void* aIn, size_t aFrames)
{
  ...
  if (mOut.Format() == AudioConfig::FORMAT_FLT) {
    ...
    error = speex_resampler_process_interleaved_int(
        mResampler, in, &inframes, out, &outframes);
  } else {
    MOZ_DIAGNOSTIC_ASSERT(false, "Unsupported data type");
