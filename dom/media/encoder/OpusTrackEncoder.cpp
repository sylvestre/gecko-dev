...
static void
SerializeOpusIdHeader(uint8_t aChannelCount,
                      uint16_t aPreskip,
...
OpusTrackEncoder::Init(int aChannels, int aSamplingRate)
{
  ...
  int error = 0;
  mEncoder = opus_encoder_create(
      GetOutputSampleRate(), mChannels, OPUS_APPLICATION_AUDIO, &error);
...
OpusTrackEncoder::GetMetadata()
{
  ...
  SerializeOpusIdHeader(
      mChannels,
      mLookahead * (kOpusSamplingRate / GetOutputSampleRate()),
      mSamplingRate,
      &meta->mIdHeader);
  ...
  while (result >= 0 && !mEncodingComplete) {
    ...
    for (AudioSegment::ChunkIterator iter(mSourceSegment);
      ...
      if (!chunk.IsNull()) {
        ...
        memset(
            pcm.Elements() + frameCopied * mChannels, 0, memsetLength.value());
      }
    ...
    if (mResampler) {
      ...
      speex_resampler_process_interleaved_int(
          mResampler, in, &inframes, out, &outframes);
#else
    ...
    result = opus_encode_float(mEncoder,
                               pcmBuf,
                               GetPacketDuration(),
                               frameData.Elements(),
                               MAX_DATA_BYTES);
