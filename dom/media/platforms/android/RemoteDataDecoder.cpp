 ...
 public:
  ...
  class InputInfo
  {
   public:
    ...
    InputInfo(const int64_t aDurationUs,
              const gfx::IntSize& aImageSize,
    ...
    void HandleOutput(Sample::Param aSample) override
    {
      ...
      if (ok && (size > 0 || presentationTimeUs >= 0)) {
        RefPtr<layers::Image> img =
            new SurfaceTextureImage(mDecoder->mSurfaceHandle,
                                    inputInfo.mImageSize,
                                    false /* NOT continuous */,
                                    gl::OriginPos::BottomLeft);

        RefPtr<VideoData> v = VideoData::CreateFromImage(
            inputInfo.mDisplaySize,
            offset,
  ...
  RemoteVideoDecoder(const VideoInfo& aConfig,
      ...
      : RemoteDataDecoder(MediaData::Type::VIDEO_DATA,
                          aConfig.mMimeType,
                          aFormat,
                          aDrmStubId,
                          aTaskQueue),
...
class RemoteAudioDecoder : public RemoteDataDecoder
{
 public:
  RemoteAudioDecoder(const AudioInfo& aConfig,
                     ...
                     const nsString& aDrmStubId,
                     TaskQueue* aTaskQueue)
      : RemoteDataDecoder(MediaData::Type::AUDIO_DATA,
                          aConfig.mMimeType,
                          aFormat,
                          aDrmStubId,
                          aTaskQueue),
...
RemoteDataDecoder::Shutdown()
{
  ...
  RefPtr<RemoteDataDecoder> self = this;
  return InvokeAsync(
      mTaskQueue, this, __func__, &RemoteDataDecoder::ProcessShutdown);
}
...
RemoteDataDecoder::Decode(MediaRawData* aSample)
{
  ...
  return InvokeAsync(mTaskQueue, __func__, [self, sample, this]() {
    ...
    return mJavaDecoder->Input(
               bytes, bufferInfo, GetCryptoInfoFromSample(sample))
...
{
  if (!mTaskQueue->IsCurrentThreadIn()) {
    mTaskQueue->Dispatch(NewRunnableMethod("RemoteDataDecoder::DrainComplete",
                                           this,
                                           &RemoteDataDecoder::DrainComplete));
    return;
  }
