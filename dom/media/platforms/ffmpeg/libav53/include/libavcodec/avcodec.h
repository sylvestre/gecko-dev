...
#define CODEC_FLAG_4MV \
  0x0004  ///< 4 MV per MB allowed / advanced prediction for H.263.
...
#define CODEC_FLAG_PASS1 \
  0x0200  ///< Use internal 2pass ratecontrol in first pass mode.
#define CODEC_FLAG_PASS2 \
  0x0400  ///< Use internal 2pass ratecontrol in second pass mode.
#define CODEC_FLAG_GRAY 0x2000      ///< Only decode/encode grayscale.
#define CODEC_FLAG_EMU_EDGE 0x4000  ///< Don't draw edges.
#define CODEC_FLAG_PSNR \
  0x8000  ///< error[?] variables will be set during encoding.
...
#define CODEC_FLAG2_SKIP_RD \
  0x00004000  ///< RD optimal MB level residual skipping
...
#define CODEC_FLAG_SVCD_SCAN_OFFSET \
  0x40000000  ///< Will reserve space for SVCD scan offset user data.
#define CODEC_FLAG2_INTRA_VLC 0x00000800  ///< Use MPEG-2 intra VLC table.
#define CODEC_FLAG2_DROP_FRAME_TIMECODE \
  0x00002000  ///< timecode is in drop frame format.
#define CODEC_FLAG2_NON_LINEAR_QUANT \
  0x00010000  ///< Use MPEG-2 nonlinear quantizer.
#endif
#if FF_API_MJPEG_GLOBAL_OPTS
#define CODEC_FLAG_EXTERN_HUFF \
  0x1000  ///< Use external Huffman table (for MJPEG).
#endif
#if FF_API_X264_GLOBAL_OPTS
#define CODEC_FLAG2_BPYRAMID \
  0x00000010  ///< H.264 allow B-frames to be used as references.
#define CODEC_FLAG2_WPRED \
  0x00000020  ///< H.264 weighted biprediction for B-frames
#define CODEC_FLAG2_MIXED_REFS \
  0x00000040  ///< H.264 one reference per partition, as opposed to one reference per macroblock
#define CODEC_FLAG2_8X8DCT 0x00000080     ///< H.264 high profile 8x8 transform
#define CODEC_FLAG2_FASTPSKIP 0x00000100  ///< H.264 fast pskip
#define CODEC_FLAG2_AUD 0x00000200        ///< H.264 access unit delimiters
#define CODEC_FLAG2_BRDO 0x00000400  ///< B-frame rate-distortion optimization
#define CODEC_FLAG2_MBTREE \
  0x00040000  ///< Use macroblock tree ratecontrol (x264 only)
#define CODEC_FLAG2_PSY 0x00080000  ///< Use psycho visual optimizations.
#define CODEC_FLAG2_SSIM \
  0x00100000  ///< Compute SSIM during encoding, error[] values are undefined.
#define CODEC_FLAG2_INTRA_REFRESH \
  0x00200000  ///< Use periodic insertion of intra blocks instead of keyframes.
#endif
#if FF_API_SNOW_GLOBAL_OPTS
#define CODEC_FLAG2_MEMC_ONLY \
  0x00001000  ///< Only do ME/MC (I frames -> ref, P frame -> ME+MC).
#endif
#if FF_API_LAME_GLOBAL_OPTS
#define CODEC_FLAG2_BIT_RESERVOIR \
...
#define CODEC_CAP_DRAW_HORIZ_BAND \
  0x0001  ///< Decoder can use draw_horiz_band callback.
/**
...
#define FF_BUFFER_TYPE_USER \
  2  ///< direct rendering buffers (image is (de)allocated by user)
#define FF_BUFFER_TYPE_SHARED \
  4  ///< Buffer from somewhere else; don't deallocate image (data/base), all other tables are not shared.
#define FF_BUFFER_TYPE_COPY \
  8  ///< Just a (modified) copy of some other buffer, don't deallocate anything.
...
#define FF_CMP_CHROMA 256
  ...
  /**
     ...
     */
  enum PixelFormat (*get_format)(struct AVCodecContext* s,
...
typedef struct AVCodec
{
  ...
  /**
     ...
     */
  int (*encode2)(AVCodecContext* avctx,
                 AVPacket* avpkt,
                 const AVFrame* frame,
                 int* got_packet_ptr);
} AVCodec;
...
av_destruct_packet_nofree(AVPacket* pkt);
 ...
 */
void
av_shrink_packet(AVPacket* pkt, int size);
 ...
 */
void
audio_resample_close(ReSampleContext* s);
 ...
 */
int
av_resample(struct AVResampleContext* c,
            short* dst,
            short* src,
            int* consumed,
            int src_size,
 ...
 */
void
av_resample_compensate(struct AVResampleContext* c,
                       int sample_delta,
                       int compensation_distance);
void
av_resample_close(struct AVResampleContext* c);
 ...
 */
int
avpicture_layout(const AVPicture* src,
                 enum PixelFormat pix_fmt,
                 int width,
                 int height,
                 unsigned char* dest,
 ...
 */
int
avpicture_get_size(enum PixelFormat pix_fmt, int width, int height);
void
avcodec_get_chroma_sub_sample(enum PixelFormat pix_fmt,
                              int* h_shift,
                              int* v_shift);
...
avcodec_get_pix_fmt_name(enum PixelFormat pix_fmt);
#endif
 ...
 */
unsigned int
avcodec_pix_fmt_to_codec_tag(enum PixelFormat pix_fmt);
 ...
 */
int
avcodec_get_pix_fmt_loss(enum PixelFormat dst_pix_fmt,
...
img_get_alpha_info(const AVPicture* src,
                   enum PixelFormat pix_fmt,
                   int width,
                   int height);
#endif
...
int
avpicture_deinterlace(AVPicture* dst,
                      const AVPicture* src,
                      enum PixelFormat pix_fmt,
                      int width,
                      int height);
...
attribute_deprecated void
avcodec_get_context_defaults2(AVCodecContext* s, enum AVMediaType);
#endif
 ...
 */
attribute_deprecated AVCodecContext*
 ...
 */
attribute_deprecated int
av_get_bits_per_sample_format(enum AVSampleFormat sample_fmt);
#endif
 ...
 */
void
av_log_ask_for_sample(void* avc, const char* msg, ...) av_printf_format(2, 3);
