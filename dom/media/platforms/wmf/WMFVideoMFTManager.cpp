...
template<class T>
void
DeleteOnMainThread(nsAutoPtr<T>& aObject)
{
...
WMFVideoMFTManager::Output(int64_t aStreamOffset, RefPtr<MediaData>& aOutData)
{
  ...
  while (true) {
    ...
    if (hr == MF_E_TRANSFORM_STREAM_CHANGE) {
      ...
      hr = mDecoder->SetDecoderOutputType(
          false /* check all attribute */, nullptr, nullptr);
      NS_ENSURE_TRUE(SUCCEEDED(hr), hr);
...
WMFVideoMFTManager::GetDescriptionName() const
{
  ...
  return nsPrintfCString(
      "wmf %s video decoder",
      IsHardwareAccelerated(failureReason) ? "hardware" : "software");
}
