...
#define LOG(arg, ...)               \
  MOZ_LOG(gMediaDecoderLog,         \
          ...
          ("TheoraDecoder(%p)::%s: " arg, this, __func__, ##__VA_ARGS__))
...
ogg_packet
InitTheoraPacket(const unsigned char* aData,
                 size_t aLength,
                 bool aBOS,
                 bool aEOS,
                 int64_t aGranulepos,
...
TheoraDecoder::Init()
{
  ...
  if (!XiphExtradataToHeaders(headers,
                              headerLens,
...
TheoraDecoder::DoDecodeHeader(const unsigned char* aData, size_t aLength)
{
  ...
  int r = th_decode_headerin(
      &mTheoraInfo, &mTheoraComment, &mTheoraSetupInfo, &pkt);
...
TheoraDecoder::ProcessDecode(MediaRawData* aSample)
{
  ...
  bool bos = mPacketCount == 0;
  ogg_packet pkt = InitTheoraPacket(aData,
                                    aLength,
                                    bos,
                                    false,
  ...
  if (ret == 0 || ret == TH_DUPFRAME) {
    ...
    IntRect pictureArea(mTheoraInfo.pic_x,
                        mTheoraInfo.pic_y,
