    ...
    RequestType aType)
{
  ...
  if (!MediaPrefs::EMEEnabled() && !IsClearkeyKeySystem(aKeySystem)) {
    ...
    MediaKeySystemAccess::NotifyObservers(
        mWindow, aKeySystem, MediaKeySystemStatus::Api_disabled);
    aPromise->MaybeReject(NS_ERROR_DOM_NOT_SUPPORTED_ERR,
  ...
  if (MediaKeySystemAccess::GetSupportedConfig(aKeySystem,
                                               aConfigs,
                                               config,
                                               &diagnostics,
                                               isPrivateBrowsing,
    ...
    diagnostics.StoreMediaKeySystemAccess(
        mWindow->GetExtantDoc(), aKeySystem, true, __func__);
    ...
    const Sequence<MediaKeySystemConfiguration>& aConfigs)
{
  ...
  NS_NewTimerWithObserver(
      getter_AddRefs(timer), this, 60 * 1000, nsITimer::TYPE_ONE_SHOT);
  if (!timer) {
