...
void
GraphDriver::SetGraphTime(GraphDriver* aPreviousDriver,
...
void
GraphDriver::SwitchAtNextIteration(GraphDriver* aNextDriver)
{
  GraphImpl()->GetMonitor().AssertCurrentThreadOwns();
  LOG(LogLevel::Debug,
...
void
GraphDriver::EnsureNextIteration()
{
  mGraphImpl->EnsureNextIteration();
}

bool
GraphDriver::Switching()
{
  GraphImpl()->GetMonitor().AssertCurrentThreadOwns();
...
void
GraphDriver::SetNextDriver(GraphDriver* aNextDriver)
{
  GraphImpl()->GetMonitor().AssertCurrentThreadOwns();
...
void
GraphDriver::SetPreviousDriver(GraphDriver* aPreviousDriver)
{
  GraphImpl()->GetMonitor().AssertCurrentThreadOwns();
...
ThreadedDriver::RunThread()
{
  ...
  while (stillProcessing) {
    ...
    GraphTime nextStateComputedTime = mGraphImpl->RoundUpToNextAudioBlock(
...
AudioCallbackDriver::Init()
{
  ...
  if (latencyPref) {
    ...
    if (cubeb_get_min_latency(cubebContext, &output, &latency_frames) !=
        CUBEB_OK) {
  ...
  // freed whenever enumerate is called.
  {
    ...
    if ((!mGraphImpl->mInputWanted
        ...
        cubeb_stream_init(cubebContext,
                          &stream,
                          "AudioCallbackDriver",
                          input_id,
                          mGraphImpl->mInputWanted ? &input : nullptr,
                          output_id,
                          mGraphImpl->mOutputWanted ? &output : nullptr,
                          latency_frames,
                          DataCallback_s,
                          StateCallback_s,
                          this) == CUBEB_OK) {
...
/* static */ void
AudioCallbackDriver::StateCallback_s(cubeb_stream* aStream,
                                     void* aUser,
                                     cubeb_state aState)
{
  AudioCallbackDriver* driver = reinterpret_cast<AudioCallbackDriver*>(aUser);
...
bool
AudioCallbackDriver::InCallback()
{
  return mInCallback;
}
                                  ...
                                  long aFrames)
{
  ...
  if (aInputBuffer) {
    if (mAudioInput) {  // for this specific input-only or full-duplex stream
      mAudioInput->NotifyInputData(mGraphImpl,
                                   aInputBuffer,
  ...
  mGraphImpl->NotifyOutputData(aOutputBuffer,
                               static_cast<size_t>(aFrames),
                               mSampleRate,
                               mOutputChannels);
...
void
AudioCallbackDriver::PanOutputIfNeeded(bool aMicrophoneActive)
{
#ifdef XP_MACOSX
  cubeb_device* out;
  int rv;
...
void
AudioCallbackDriver::EnqueueStreamAndPromiseForOperation(
    MediaStream* aStream, void* aPromise, dom::AudioContextOperation aOperation)
{
  MonitorAutoLock mon(mGraphImpl->GetMonitor());
...
void
AudioCallbackDriver::CompleteAudioContextOperations(
    AsyncCubebOperation aOperation)
{
  ...
  for (uint32_t i = 0; i < array.Length(); i++) {
    ...
    if ((aOperation == AsyncCubebOperation::INIT &&
        ...
        (aOperation == AsyncCubebOperation::SHUTDOWN &&
         s.mOperation != dom::AudioContextOperation::Resume)) {
      GraphImpl()->AudioContextOperationCompleted(
          s.mStream, s.mPromise, s.mOperation);
