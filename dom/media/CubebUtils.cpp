...
extern void
audioipc_server_stop(void*);
// These functions are provided by audioipc-client crate
extern int
audioipc_client_init(cubeb**, const char*);
}
...
const char* AUDIOSTREAM_BACKEND_ID_STR[] = {"jack",
                                            "pulse",
                                            "alsa",
                                            "audiounit",
                                            "audioqueue",
...
uint16_t
ConvertCubebType(cubeb_device_type aType)
{
  uint16_t map[] = {
...
uint16_t
ConvertCubebState(cubeb_device_state aState)
{
  uint16_t map[] = {
...
uint16_t
ConvertCubebPreferred(cubeb_device_pref aPreferred)
{
...
uint16_t
ConvertCubebFormat(cubeb_device_fmt aFormat)
{
  uint16_t format = 0;
...
void
GetDeviceCollection(nsTArray<RefPtr<AudioDeviceInfo>>& aDeviceInfos, Side aSide)
{
  cubeb* context = GetCubebContext();
  if (context) {
