...
NS_IMETHODIMP
nsPluginInstanceOwner::GetWindow(NPWindow*& aWindow)
{
...
NS_IMETHODIMP
nsPluginInstanceOwner::GetMode(int32_t* aMode)
{
  nsCOMPtr<nsIDocument> doc;
...
void
nsPluginInstanceOwner::GetAttributes(nsTArray<MozPluginParameter>& attributes)
{
...
NS_IMETHODIMP
nsPluginInstanceOwner::GetDOMElement(nsIDOMElement** result)
{
  return CallQueryReferent(mContent.get(), result);
}

nsresult
nsPluginInstanceOwner::GetInstance(nsNPAPIPluginInstance** aInstance)
{
...
NS_IMETHODIMP
nsPluginInstanceOwner::GetDocument(nsIDocument** aDocument)
{
  nsCOMPtr<nsIContent> content = do_QueryReferent(mContent);
...
nsPluginInstanceOwner::InvalidateRect(NPRect* invalidRect)
{
  ...
  if (mWidget) {
    mWidget->Invalidate(
        LayoutDeviceIntRect(invalidRect->left,
                            invalidRect->top,
...
NS_IMETHODIMP
nsPluginInstanceOwner::SetEventModel(int32_t eventModel)
{
#ifdef XP_MACOSX
...
#ifdef XP_MACOSX
NPBool
nsPluginInstanceOwner::ConvertPointPuppet(PuppetWidget* widget,
                                          ...
                                          NPCoordinateSpace destSpace)
{
  ...
  CSSIntPoint chromeSize =
      CSSIntPoint::Truncate(LayoutDeviceIntPoint::FromUnknownPoint(
                                rootWidget->GetChromeDimensions()) /
                            scaleFactor);
...
NPBool
nsPluginInstanceOwner::ConvertPointNoPuppet(nsIWidget* widget,
                                            nsPluginFrame* pluginFrame,
                                            double sourceX,
                                            double sourceY,
                                            NPCoordinateSpace sourceSpace,
...
NPError
nsPluginInstanceOwner::InitAsyncSurface(NPSize* size,
...
NPError
nsPluginInstanceOwner::FinalizeAsyncSurface(NPAsyncSurface*)
{
  return NPERR_INCOMPATIBLE_VERSION_ERROR;
}

void
nsPluginInstanceOwner::SetCurrentAsyncSurface(NPAsyncSurface*, NPRect*)
{
}

NS_IMETHODIMP
nsPluginInstanceOwner::GetTagType(nsPluginTagType* result)
{
...
void
nsPluginInstanceOwner::GetParameters(nsTArray<MozPluginParameter>& parameters)
{
  nsCOMPtr<nsIObjectLoadingContent> content = do_QueryReferent(mContent);
  nsObjectLoadingContent* loadingContent =
      static_cast<nsObjectLoadingContent*>(content.get());
...
static void
InitializeNPCocoaEvent(NPCocoaEvent* event)
{
...
void
nsPluginInstanceOwner::CARefresh(nsITimer* aTimer, void* aClosure)
{
...
void
nsPluginInstanceOwner::AddToCARefreshTimer()
{
  if (!mInstance) {
    return;
  }
...
void
nsPluginInstanceOwner::RemoveFromCARefreshTimer()
{
...
void
nsPluginInstanceOwner::SetPluginPort()
{
...
nsresult
nsPluginInstanceOwner::DispatchFocusToPlugin(nsIDOMEvent* aFocusEvent)
{
#ifndef XP_MACOSX
  ...
  if (theEvent) {
    WidgetGUIEvent focusEvent(
        theEvent->IsTrusted(), theEvent->mMessage, nullptr);
...
nsresult
nsPluginInstanceOwner::ProcessKeyPress(nsIDOMEvent* aKeyEvent)
{
#ifdef XP_MACOSX
...
nsresult
nsPluginInstanceOwner::DispatchKeyToPlugin(nsIDOMEvent* aKeyEvent)
{
#if !defined(XP_MACOSX)
  if (!mPluginWindow || (mPluginWindow->type == NPWindowTypeWindow))
...
nsresult
nsPluginInstanceOwner::DispatchMouseToPlugin(nsIDOMEvent* aMouseEvent,
...
#ifdef MOZ_X11
static unsigned int
XInputEventState(const WidgetInputEvent& anEvent)
{
...
void
nsPluginInstanceOwner::PerformDelayedBlurs()
{
  nsCOMPtr<nsIContent> content = do_QueryReferent(mContent);
...
nsEventStatus
nsPluginInstanceOwner::ProcessEvent(const WidgetGUIEvent& anEvent)
{
  ...
  mInstance->HandleEvent(
      &cocoaEvent, &response, NS_PLUGIN_CALL_SAFE_TO_REENTER_GECKO);
  if ((response == kNPEventStartIME) &&
    ...
    if (widget) {
      ...
      ConvertPoint(0.0,
                   mPluginFrame->GetScreenRect().height,
            ...
            switch (wheelEvent->mDeltaMode) {
              ...
              case nsIDOMWheelEvent::DOM_DELTA_LINE: {
                ...
                if (::SystemParametersInfo(
                        SPI_GETWHEELSCROLLCHARS, 0, &charsPerWheelDelta, 0)) {
                  // Use system default scroll amount, 3, when
  ...
  int16_t response = kNPEventNotHandled;
  mInstance->HandleEvent(
      &pluginEvent, &response, NS_PLUGIN_CALL_SAFE_TO_REENTER_GECKO);
  if (response == kNPEventHandled) rv = nsEventStatus_eConsumeNoDefault;
#endif
...
nsPluginInstanceOwner::Destroy()
{
  ...
  content->RemoveSystemEventListener(
      NS_LITERAL_STRING("compositionstart"), this, true);
  content->RemoveSystemEventListener(
      NS_LITERAL_STRING("compositionend"), this, true);
...
#ifdef XP_MACOSX
void
nsPluginInstanceOwner::Paint(const gfxRect& aDirtyRect, CGContextRef cgContext)
{
...
void
nsPluginInstanceOwner::DoCocoaEventDrawRect(const gfxRect& aDrawRect,
...
#ifdef XP_WIN
void
nsPluginInstanceOwner::Paint(const RECT& aDirty, HDC aDC)
{
  if (!mInstance || !mPluginFrame) return;
...
#if defined(MOZ_X11)
void
nsPluginInstanceOwner::Paint(gfxContext* aContext,
                             ...
                             const gfxRect& aDirtyRect)
{
  ...
  renderer.Draw(aContext,
                nsIntSize(window->width, window->height),
                rendererFlags,
                screen,
                visual);
}
                                              ...
                                              uint32_t numClipRects)
{
  ...
  if (mInstanceOwner->mFlash10Quirks) {
    ...
    dirtyRect.SetRect(
        offset.x, offset.y, mDirtyRect.XMost(), mDirtyRect.YMost());
  }
...
nsresult
nsPluginInstanceOwner::Init(nsIContent* aContent)
{
  ...
  aContent->AddEventListener(
      NS_LITERAL_STRING("mousedown"), this, false, false);
  aContent->AddEventListener(
      NS_LITERAL_STRING("mousemove"), this, false, false);
  ...
  aContent->AddEventListener(
      NS_LITERAL_STRING("mouseover"), this, false, false);
  ...
  aContent->AddSystemEventListener(
      NS_LITERAL_STRING("compositionstart"), this, true);
  aContent->AddSystemEventListener(
      NS_LITERAL_STRING("compositionend"), this, true);
...
void
nsPluginInstanceOwner::ReleasePluginPort(void* pluginPort)
{
}
...
nsPluginInstanceOwner::CreateWidget(void)
{
  ...
  if (!windowless) {
    ...
    if (!mWidget) {
      ...
      rv = mWidget->Create(parentWidget.get(),
                           nullptr,
...
void
nsPluginInstanceOwner::FixUpPluginWindow(int32_t inPaintState)
{
...
nsPluginInstanceOwner::SendWindowFocusChanged(bool aIsActive)
{
  ...
  mInstance->HandleEvent(
      &cocoaEvent, nullptr, NS_PLUGIN_CALL_SAFE_TO_REENTER_GECKO);
}
...
void
nsPluginInstanceOwner::UpdateWindowPositionAndClipRect(bool aSetWindow)
{
...
void
nsPluginInstanceOwner::SetFrame(nsPluginFrame* aFrame)
{
    ...
    if (content && content->OwnerDoc() && content->OwnerDoc()->GetWindow()) {
      ...
      if (windowRoot) {
        windowRoot->RemoveEventListener(
            NS_LITERAL_STRING("activate"), this, false);
        windowRoot->RemoveEventListener(
            NS_LITERAL_STRING("deactivate"), this, false);
        windowRoot->RemoveEventListener(
    ...
    if (content && content->OwnerDoc() && content->OwnerDoc()->GetWindow()) {
      ...
      if (windowRoot) {
        windowRoot->AddEventListener(
            NS_LITERAL_STRING("activate"), this, false, false);
        windowRoot->AddEventListener(
            NS_LITERAL_STRING("deactivate"), this, false, false);
        windowRoot->AddEventListener(
            NS_LITERAL_STRING("MozPerformDelayedBlur"), this, false, false);
      }
    }
  }
}
...
nsPluginFrame*
nsPluginInstanceOwner::GetFrame()
{
  return mPluginFrame;
}

NS_IMETHODIMP
nsPluginInstanceOwner::PrivateModeChanged(bool aEnabled)
{
  return mInstance ? mInstance->PrivateModeStateChanged(aEnabled) : NS_OK;
}

already_AddRefed<nsIURI>
nsPluginInstanceOwner::GetBaseURI() const
{
  nsCOMPtr<nsIContent> content = do_QueryReferent(mContent);
  if (!content) {
    return nullptr;
...
void
nsPluginDOMContextMenuListener::Destroy(nsIContent* aContent)
{
  // Unregister context menu listener
