...
void
NS_NotifyBeginPluginCall(NSPluginCallReentry aReentryState)
{
  nsNPAPIPluginInstance::BeginPluginCall(aReentryState);
}

void
NS_NotifyPluginCall(NSPluginCallReentry aReentryState)
{
...
GetNewPluginLibrary(nsPluginTag* aPluginTag)
{
  ...
  if (nsNPAPIPlugin::RunPluginOOP(aPluginTag)) {
    return PluginModuleChromeParent::LoadModule(
        aPluginTag->mFullPath.get(), aPluginTag->mId, aPluginTag);
  }
...
#else
  NPError pluginCallError;
  nsresult rv = pluginLib->NP_Initialize(
      &sBrowserFuncs, &plugin->mPluginFuncs, &pluginCallError);
  if (rv != NS_OK || pluginCallError != NPERR_NO_ERROR) {
    return NS_ERROR_FAILURE;
  }
#endif
...
NPError
MakeNewNPAPIStreamInternal(NPP npp,
                           ...
                           const char* buf = nullptr)
{
  ...
  switch (type) {
    case eNPPStreamTypeInternal_Get: {
      if (NS_FAILED(pluginHost->GetURL(
              inst, relativeURL, target, listener, nullptr, nullptr, false)))
        return NPERR_GENERIC_ERROR;
      break;
    }
    case eNPPStreamTypeInternal_Post: {
      if (NS_FAILED(pluginHost->PostURL(inst,
                                        relativeURL,
                                        len,
                                        buf,
                                        target,
...
NPPExceptionAutoHolder::NPPExceptionAutoHolder() : mOldException(gNPPException)
{
...
_geturl(NPP npp, const char* relativeURL, const char* target)
{
  ...
  return MakeNewNPAPIStreamInternal(
      npp, relativeURL, target, eNPPStreamTypeInternal_Get);
}
               ...
               void* notifyData)
{
  ...
  return MakeNewNPAPIStreamInternal(npp,
                                    relativeURL,
                                    target,
                                    eNPPStreamTypeInternal_Post,
                                    true,
...
_getwindowobject(NPP npp)
{
  ...
  JS::Rooted<JSObject*> global(
      dom::RootingCx(), nsGlobalWindow::Cast(outer)->GetGlobalJSObject());
...
void
_getstringidentifiers(const NPUTF8** names,
                      int32_t nameCount,
...
_evaluate(NPP npp, NPObject* npobj, NPString* script, NPVariant* result)
{
  ...
  options.setFileAndLine(spec, 0).setVersion(JSVERSION_DEFAULT);
...
bool
_enumerate(NPP npp, NPObject* npobj, NPIdentifier** identifier, uint32_t* count)
{
  if (!NS_IsMainThread()) {
    NPN_PLUGIN_LOG(PLUGIN_LOG_ALWAYS,
