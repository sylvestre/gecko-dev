...
static void
DisplayPR_LoadLibraryErrorMessage(const char* libName)
{
...
static void
SearchForSoname(const char* name, char** soname)
{
  if (!(name && soname)) return;
  ...
  while ((dirEntry = PR_ReadDir(fdDir, PR_SKIP_BOTH))) {
    if (!PL_strncmp(dirEntry->name, name, n)) {
      if (dirEntry->name[n] == '.' && dirEntry->name[n + 1] &&
          !dirEntry->name[n + 2]) {
...
static bool
LoadExtraSharedLib(const char* name, char** soname, bool tryToGetSoname)
{
  bool ret = true;
  PRLibSpec tempSpec;
  PRLibrary* handle;
...
#define DEFAULT_EXTRA_LIBS_LIST                                      \
  "libXt" LOCAL_PLUGIN_DLL_SUFFIX ":libXext" LOCAL_PLUGIN_DLL_SUFFIX \
  ":libXm" LOCAL_PLUGIN_DLL_SUFFIX
#else
#define DEFAULT_EXTRA_LIBS_LIST \
  "libXt" LOCAL_PLUGIN_DLL_SUFFIX ":libXext" LOCAL_PLUGIN_DLL_SUFFIX
#endif
  ...
  if (NS_SUCCEEDED(res) && (prefs != nullptr)) {
    ...
    if (sonameList) {
      ...
      if (sonameListToSave[0])
        for (p = &sonameListToSave[strlen(sonameListToSave) - 1]; *p == ':';
             p--)
