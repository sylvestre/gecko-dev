...
StreamNotifyParent::RecvRedirectNotifyResponse(const bool& allow)
{
  ...
  instance->mNPNIface->urlredirectresponse(
      instance->mNPP, this, static_cast<NPBool>(allow));
  return IPC_OK();
}
...
PluginInstanceParent::AnswerNPN_GetValue_NPNVPluginElementNPObject(
    PPluginScriptableObjectParent** aValue, NPError* aResult)
{
  if (!InternalGetValueForNPObject(
          NPNVPluginElementNPObject, aValue, aResult)) {
    return IPC_FAIL_NO_REASON(this);
  }
  return IPC_OK();
}
                                                          ...
                                                          NPError* result)
{
  ...
  *result = mNPNIface->setvalue(
      mNPP, NPPVpluginWindowBool, (void*)(intptr_t)windowed);
  return IPC_OK();
}
...
{
  *result = mNPNIface->setvalue(
      mNPP, NPPVpluginTransparentBool, (void*)(intptr_t)transparent);
  return IPC_OK();
}
...
{
  *result = mNPNIface->setvalue(
      mNPP, NPPVpluginUsesDOMForCursorBool, (void*)(intptr_t)useDOMForCursor);
  return IPC_OK();
}
    ...
    const int& drawingModel, NPError* result)
{
  ...
  *result = mNPNIface->setvalue(
      mNPP, NPPVpluginDrawingModel, (void*)(intptr_t)requestModel);
...
#ifdef XP_MACOSX
  *result = mNPNIface->setvalue(
      mNPP, NPPVpluginEventModel, (void*)(intptr_t)eventModel);
  return IPC_OK();
#else
  *result = NPERR_GENERIC_ERROR;
  return IPC_OK();
...
{
  *result = mNPNIface->setvalue(
      mNPP, NPPVpluginIsPlayingAudio, (void*)(intptr_t)isAudioPlaying);
  return IPC_OK();
}
                                                     ...
                                                     NPError* result)
{
  ...
  if (!post) {
    *result = mNPNIface->geturlnotify(
        mNPP, NullableStringGet(url), NullableStringGet(target), actor);
  } else {
...
bool
PluginInstanceParent::DeallocPStreamNotifyParent(
    PStreamNotifyParent* notifyData)
{
  delete notifyData;
  return true;
  ...
  *outError = NPERR_GENERIC_ERROR;
  ...
  RefPtr<ID3D11Texture2D> back;
  {
    CD3D11_TEXTURE2D_DESC desc(
        DXGI_FORMAT_B8G8R8A8_UNORM, size.width, size.height, 1, 1);
                                           ...
                                           const IntRect& dirty)
{
  ...
  RefPtr<gfx::DataSourceSurface> source =
      gfx::Factory::CreateWrappingDataSourceSurface(
          buffer.get<uint8_t>(), stride, size, format);
  if (!source) {
    return IPC_FAIL_NO_REASON(this);
  }
  ...
  PLUGIN_LOG_DEBUG(
      ("   (RecvShowDirectBitmap received shmem=%p stride=%d size=%s dirty=%s)",
       buffer.get<unsigned char>(),
       stride,
       Stringify(size).c_str(),
                                                ...
                                                const gfx::IntRect& dirty)
{
  ...
  RefPtr<TextureClient> texture =
      allocator->CreateOrRecycle(surface->GetFormat(),
                                 surface->GetSize(),
                                 BackendSelector::Content,
                                 TextureFlags::NO_FLAGS,
...
PluginInstanceParent::NPP_SetWindow(const NPWindow* aWindow)
{
  ...
  if (mShWidth != window.width * scaleFactor ||
    ...
    if (mDrawingModel == NPDrawingModelCoreAnimation ||
      ...
      mIOSurface = MacIOSurface::CreateIOSurface(
          window.width, window.height, floatScaleFactor);
...
NPError
PluginInstanceParent::NPP_GetValue(NPPVariable aVariable, void* _retval)
{
  switch (aVariable) {
...
void
PluginInstanceParent::NPP_URLRedirectNotify(const char* url,
                                            int32_t status,
                                            void* notifyData)
{
...
PluginInstanceParent::NPP_HandleEvent(void* event)
{
  ...
  if (npevent->type == NPCocoaEventDrawRect) {
    if (mDrawingModel == NPDrawingModelCoreAnimation ||
      ...
      if (cgContext) {
        nsCARenderer::DrawSurfaceToCGContext(cgContext,
                                             mIOSurface,
                                             mShColorSpace,
                                             npevent->data.draw.x,
                                             npevent->data.draw.y,
      ...
      if (!CallNPP_HandleEvent_Shmem(
              npremoteevent, mShSurface, &handled, &mShSurface))
        return false;  // no good way to handle errors here...
      ...
      if (shImage) {
        ...
        ::CGContextDrawImage(
            cgContext, CGRectMake(0, 0, mShWidth, mShHeight), shImage);
...
NPError
PluginInstanceParent::NPP_NewStream(NPMIMEType type,
                                    NPStream* stream,
                                    NPBool seekable,
                                    uint16_t* stype)
{
...
void
PluginInstanceParent::NPP_URLNotify(const char* url,
                                    NPReason reason,
                                    void* notifyData)
{
  PLUGIN_LOG_DEBUG(
...
PluginInstanceParent::AllocPPluginSurfaceParent(
                                               ...
                                               NPError* result)
{
  ...
  *result = mNPNIface->getvalueforurl(
      mNPP, (NPNURLVariable)variable, url.get(), &v, &len);
...
PluginInstanceParent::AnswerNPN_SetValueForURL(const NPNURLVariable& variable,
                                               ...
                                               NPError* result)
{
  *result = mNPNIface->setvalueforurl(
      mNPP, (NPNURLVariable)variable, url.get(), value.get(), value.Length());
  return IPC_OK();
}
...
PluginInstanceParent::AnswerNPN_ConvertPoint(
    ...
    bool* result)
{
  *result = mNPNIface->convertpoint(mNPP,
                                    sourceX,
                                    sourceY,
                                    sourceSpace,
...
mozilla::ipc::IPCResult
PluginInstanceParent::RecvSetNetscapeWindowAsParent(
...
PluginInstanceParent::UnsubclassPluginWindow()
{
  ...
  if (mPluginHWND && mPluginWndProc) {
    ::SetWindowLongPtrA(
        mPluginHWND, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(mPluginWndProc));
