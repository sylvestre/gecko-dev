...
typedef HANDLE(WINAPI* CreateFileWPtr)(LPCWSTR fname,
                                       DWORD access,
                                       DWORD share,
...
typedef HANDLE(WINAPI* CreateFileAPtr)(LPCSTR fname,
                                       DWORD access,
                                       DWORD share,
                                       LPSECURITY_ATTRIBUTES security,
                                       DWORD creation,
...
/* static */
bool
PluginModuleChild::CreateForContentProcess(
...
PluginModuleChild::RecvInitProfiler(
                                 ...
                                 IPC::Channel* aChannel)
{
  ...
  NS_NewLocalFile(
      NS_ConvertUTF8toUTF16(mPluginFilename), true, getter_AddRefs(localFile));
...
mozilla::ipc::IPCResult
PluginModuleChild::RecvInitPluginModuleChild(
    Endpoint<PPluginModuleChild>&& aEndpoint)
{
...
mozilla::ipc::IPCResult
PluginModuleChild::AnswerInitCrashReporter(Shmem&& aShmem,
...
static NPError
_initasyncsurface(NPP instance,
                  NPSize* size,
...
NPError
_requestread(NPStream* aStream, NPByteRange* aRangeList)
{
...
NPError
_getvalue(NPP aNPP, NPNVariable aVariable, void* aValue)
{
  PLUGIN_LOG_DEBUG_FUNCTION;
  ENSURE_PLUGIN_THREAD(NPERR_INVALID_PARAM);
...
NPError
_setvalue(NPP aNPP, NPPVariable aVariable, void* aValue)
{
  PLUGIN_LOG_DEBUG_FUNCTION;
  ENSURE_PLUGIN_THREAD(NPERR_INVALID_PARAM);
...
NPError
_geturl(NPP aNPP, const char* aRelativeURL, const char* aTarget)
{
  PLUGIN_LOG_DEBUG_FUNCTION;
  ENSURE_PLUGIN_THREAD(NPERR_INVALID_PARAM);

  NPError err;
  InstCast(aNPP)->CallNPN_GetURL(
      NullableString(aRelativeURL), NullableString(aTarget), &err);
  return err;
}
               ...
               void* aNotifyData)
{
  ...
  NPError err;
  InstCast(aNPP)->CallPStreamNotifyConstructor(sn,
                                               url,
                                               NullableString(aTarget),
                                               true,
                                               nsCString(aBuffer, aLength),
                                               aIsFile,
                                               &err);
...
void
_status(NPP aNPP, const char* aMessage)
{
...
void
_invalidaterect(NPP aNPP, NPRect* aInvalidRect)
{
  PLUGIN_LOG_DEBUG_FUNCTION;
  ENSURE_PLUGIN_THREAD_VOID();
...
void
_invalidateregion(NPP aNPP, NPRegion aInvalidRegion)
{
  PLUGIN_LOG_DEBUG_FUNCTION;
  ENSURE_PLUGIN_THREAD_VOID();
...
bool
_evaluate(NPP aNPP, NPObject* aObject, NPString* aScript, NPVariant* aResult)
{
  PLUGIN_LOG_DEBUG_FUNCTION;
  ENSURE_PLUGIN_THREAD(false);
...
bool
_removeproperty(NPP aNPP, NPObject* aNPObj, NPIdentifier aPropertyName)
{
  PLUGIN_LOG_DEBUG_FUNCTION;
  ENSURE_PLUGIN_THREAD(false);
...
bool
_hasproperty(NPP aNPP, NPObject* aNPObj, NPIdentifier aPropertyName)
{
  PLUGIN_LOG_DEBUG_FUNCTION;
  ENSURE_PLUGIN_THREAD(false);
...
bool
_hasmethod(NPP aNPP, NPObject* aNPObj, NPIdentifier aMethodName)
{
  PLUGIN_LOG_DEBUG_FUNCTION;
  ENSURE_PLUGIN_THREAD(false);
...
void
_setexception(NPObject* aNPObj, const NPUTF8* aMessage)
{
  PLUGIN_LOG_DEBUG_FUNCTION;
  ENSURE_PLUGIN_THREAD_VOID();
...
void
_pushpopupsenabledstate(NPP aNPP, NPBool aEnabled)
{
  PLUGIN_LOG_DEBUG_FUNCTION;
  ENSURE_PLUGIN_THREAD_VOID();
                ...
                uint32_t* len)
{
  ...
  if (variable == NPNURLVProxy) {
    nsCString v;
    NPError result;
    InstCast(npp)->CallNPN_GetValueForURL(
        variable, nsCString(url), &v, &result);
...
#ifdef OS_MACOSX
static void
ProcessBrowserEvents(void* pluginModule)
{
              ...
              NPCoordinateSpace destSpace)
{
  ...
  bool result = false;
  InstCast(instance)->CallNPN_ConvertPoint(sourceX,
                                           ignoreDestX,
                                           sourceY,
                                           ignoreDestY,
                                           sourceSpace,
...
_initasyncsurface(NPP instance,
                  ...
                  NPAsyncSurface* surface)
{
  return InstCast(instance)->NPN_InitAsyncSurface(
      size, format, initData, surface);
}
...
mozilla::ipc::IPCResult
PluginModuleChild::AnswerNP_Initialize(const PluginSettings& aSettings,
                                       NPError* rv)
{
...
CreateFileAHookFn(LPCSTR fname,
                  DWORD access,
                  DWORD share,
                  LPSECURITY_ATTRIBUTES security,
                  DWORD creation,
                  ...
                  HANDLE ftemplate)
{
  while (true) {  // goto out
    ...
    int newLen = MultiByteToWideChar(
        CP_ACP, MB_ERR_INVALID_CHARS, fname, len, unicodeName, MAX_PATH);
    ...
    return CreateFileW(
        unicodeName, access, share, security, creation, flags, ftemplate);
  }

  return sCreateFileAStub(
      fname, access, share, security, creation, flags, ftemplate);
}
...
HANDLE WINAPI
CreateFileWHookFn(LPCWSTR fname,
                  DWORD access,
                  DWORD share,
                  LPSECURITY_ATTRIBUTES security,
                  DWORD creation,
                  ...
                  HANDLE ftemplate)
{
  ...
  while (true) {  // goto out, in sheep's clothing
    ...
    HANDLE original = sCreateFileWStub(
        fname, access, share, security, creation, flags, ftemplate);
    ...
    WriteFile(replacement,
              static_cast<const void*>(kSettingString),
              sizeof(kSettingString) - 1,
              &wbytes,
              NULL);
    SetFilePointer(replacement, 0, NULL, FILE_BEGIN);
    return replacement;
  }
  return sCreateFileWStub(
      fname, access, share, security, creation, flags, ftemplate);
}
...
PMCGetWindowInfoHook(HWND hWnd, PWINDOWINFO pwi)
{
  ...
  if (sBrowserHwnd && sBrowserHwnd == hWnd) pwi->rcWindow = pwi->rcClient;
  return result;
}
...
PMCGetKeyState(int aVirtKey)
{
  if (!IsPluginThread()) {
    ...
    SHORT keyState;
    RefPtr<GetKeyStateTask> task =
        new GetKeyStateTask(aVirtKey, semaphore, &keyState);
    ...
    PLUGIN_LOG_DEBUG(
        ("Error while waiting for GetKeyState semaphore: %d", GetLastError()));
...
class PluginThreadTask : public Runnable
{
 ...
 public:
  explicit PluginThreadTask(PluginThreadTaskData* aTaskData, HANDLE aSemaphore)
...
PostToPluginThread(PluginThreadTaskData* aTaskData)
{
  ...
  PLUGIN_LOG_DEBUG(("Error while waiting for semaphore: %d", GetLastError()));
  MOZ_ASSERT(err != WAIT_FAILED);
  return FALSE;
}

BOOL WINAPI
PMCGetSaveFileNameW(LPOPENFILENAMEW lpofn);
BOOL WINAPI
PMCGetOpenFileNameW(LPOPENFILENAMEW lpofn);
...
class GetFileNameTaskData : public PluginThreadTaskData
{
 public:
  ...
  bool RunTask()
  {
    switch (mFunc) {
      case OPEN_FUNC:
        return PMCGetOpenFileNameW(
            static_cast<LPOPENFILENAMEW>(mLpOpenFileName));
      case SAVE_FUNC:
        return PMCGetSaveFileNameW(
            static_cast<LPOPENFILENAMEW>(mLpOpenFileName));
    }
    return false;
  }
...
PMCGetFileNameW(GetFileNameFunc aFunc, LPOPENFILENAMEW aLpofn)
{
  ...
  if (chromeInstance) {
    ...
    if (chromeInstance->CallGetFileName(aFunc, inputOfn, &outputOfn, &ret)) {
      if (ret) {
...
PluginModuleChild::AllocPPluginInstanceChild(
    ...
    const InfallibleTArray<nsCString>& aValues)
{
  ...
  if ((mQuirks & QUIRK_FLASH_HOOK_GETWINDOWINFO) && !sGetWindowInfoPtrStub) {
    sUser32Intercept.AddHook("GetWindowInfo",
                             reinterpret_cast<intptr_t>(PMCGetWindowInfoHook),
                             (void**)&sGetWindowInfoPtrStub);
  }

  if ((mQuirks & QUIRK_FLASH_HOOK_GETKEYSTATE) && !sGetKeyStatePtrStub) {
    sUser32Intercept.AddHook("GetKeyState",
                             reinterpret_cast<intptr_t>(PMCGetKeyState),
                             (void**)&sGetKeyStatePtrStub);
  }

  if (!sSetCursorPosPtrStub) {
    sUser32Intercept.AddHook("SetCursorPos",
                             reinterpret_cast<intptr_t>(PMCSetCursorPos),
                             (void**)&sSetCursorPosPtrStub);
  }

  sComDlg32Intercept.Init("comdlg32.dll");
  if (!sGetSaveFileNameWPtrStub) {
    sComDlg32Intercept.AddHook("GetSaveFileNameW",
                               reinterpret_cast<intptr_t>(PMCGetSaveFileNameW),
                               (void**)&sGetSaveFileNameWPtrStub);
  }

  if (!sGetOpenFileNameWPtrStub) {
    sComDlg32Intercept.AddHook("GetOpenFileNameW",
                               reinterpret_cast<intptr_t>(PMCGetOpenFileNameW),
                               (void**)&sGetOpenFileNameWPtrStub);
  }

  if ((mQuirks & QUIRK_FLASH_HOOK_PRINTDLGW) && !sPrintDlgWPtrStub) {
    sComDlg32Intercept.AddHook("PrintDlgW",
                               reinterpret_cast<intptr_t>(PMCPrintDlgW),
...
mozilla::ipc::IPCResult
PluginModuleChild::RecvPPluginInstanceConstructor(
...
PluginModuleChild::CallWindowProcHook(int nCode, WPARAM wParam, LPARAM lParam)
{
  ...
  if (nCode >= 0 &&
    ...
    if (pCwp->message == WM_KILLFOCUS) {
      ...
      wchar_t szClass[26];
      if (GetClassNameW(
              pCwp->hwnd, szClass, sizeof(szClass) / sizeof(char16_t)) &&
...
PluginModuleChild::SetEventHooks()
{
  ...
  mNestedEventHook = SetWindowsHookEx(
      WH_MSGFILTER, NestedInputEventHook, nullptr, GetCurrentThreadId());

  // WH_CALLWNDPROC event hook for trapping sync messages sent from
  // parent that can cause deadlocks.
  mGlobalCallWndProcHook = SetWindowsHookEx(
      WH_CALLWNDPROC, CallWindowProcHook, nullptr, GetCurrentThreadId());
}
