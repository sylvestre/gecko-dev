...
class PluginInstanceChild : public PPluginInstanceChild
{
 ...
 protected:
  ...
  virtual PPluginSurfaceChild* AllocPPluginSurfaceChild(
      const WindowsSharedMemoryHandle&,
      const gfx::IntSize&,
      const bool&) override
  {
  ...
  virtual mozilla::ipc::IPCResult RecvPBrowserStreamConstructor(
      ...
      const uint32_t& aLastmodified,
      PStreamNotifyChild* aNotifyData,
      const nsCString& aHeaders) override;
  ...
  virtual PStreamNotifyChild* AllocPStreamNotifyChild(const nsCString& url,
 ...
 public:
  ...
  NPError NPN_NewStream(NPMIMEType aMIMEType,
                        const char* aWindow,
                        NPStream** aStream);
  ...
  NPError NPN_InitAsyncSurface(NPSize* size,
 ...
 private:
  ...
  static LONG WINAPI ImmGetCompositionStringProc(HIMC aIMC,
                                                 DWORD aIndex,
                                                 LPVOID aBuf,
                                                 DWORD aLen);
  static BOOL WINAPI ImmSetCandidateWindowProc(HIMC hIMC,
                                               LPCANDIDATEFORM plCandidate);
  static BOOL WINAPI ImmNotifyIME(HIMC aIMC,
                                  DWORD aAction,
                                  DWORD aIndex,
                                  DWORD aValue);
  ...
  {
   public:
    FlashThrottleMsg(PluginInstanceChild* aInstance,
                     HWND aWnd,
                     UINT aMsg,
                     WPARAM aWParam,
                     LPARAM aLParam,
  ...
  struct DirectBitmap
  {
    DirectBitmap(PluginInstanceChild* aOwner,
                 const Shmem& shmem,
