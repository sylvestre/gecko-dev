...
typedef BOOL(WINAPI* Imm32ImmNotifyIME)(HIMC hIMC,
                                        DWORD dwAction,
                                        DWORD dwIndex,
                                        DWORD dwValue);
...
PluginInstanceChild::DoNPP_New()
{
  ...
  NPError rv = mPluginIface->newp((char*)NullableStringGet(mMimeType),
                                  npp,
                                  NP_EMBED,
                                  argc,
                                  argn.get(),
...
NPError
PluginInstanceChild::NPN_GetValue(NPNVariable aVar, void* aValue)
{
    ...
    bool* wantsAllStreams, NPError* rv)
{
  ...
  if (!mPluginIface->getvalue) {
    ...
    *rv = mPluginIface->getvalue(
        GetNPP(), NPPVpluginWantsAllNetworkStreams, &value);
  }
...
#if defined(XP_WIN)
NPError
PluginInstanceChild::DefaultAudioDeviceChanged(
    NPAudioDeviceChangeDetails& details)
{
  if (!mPluginIface->setvalue) {
    return NPERR_GENERIC_ERROR;
  }
  return mPluginIface->setvalue(
      GetNPP(), NPNVaudioDeviceChangeDetails, (void*)&details);
}
#endif
  ...
  if (evcopy.type == NPCocoaEventDrawRect) {
    ...
    if (!mShContext) {
      ...
      mShContext = ::CGBitmapContextCreate(
          ...
          mWindow.height * scaleFactor,
          8,
          mWindow.width * 4 * scaleFactor,
          mShColorSpace,
...
void
CallCGDraw(CGContextRef ref, void* aPluginInstance, nsIntRect aUpdateRect)
{
                                                     ...
                                                     int16_t* handled)
{
  ...
  if (evcopy.type == NPCocoaEventDrawRect) {
    ...
    if (!mCARenderer->isInit()) {
      ...
      mCARenderer->SetupRenderer(caLayer,
                                 mWindow.width,
                                 mWindow.height,
  ...
  mCARenderer->Render(
      mWindow.width, mWindow.height, mContentsScaleFactor, nullptr);
...
mozilla::ipc::IPCResult
PluginInstanceChild::RecvContentsScaleFactorChanged(
...
#else
  NS_RUNTIMEABORT(
      "ContentsScaleFactorChanged is an Windows or OSX only message");
  return IPC_FAIL_NO_REASON(this);
#endif
}
...
mozilla::ipc::IPCResult
PluginInstanceChild::RecvCreateChildPopupSurrogate(
...
PluginInstanceChild::AnswerNPP_SetWindow(const NPRemoteWindow& aWindow)
{
  ...
  int depth;
  FindVisualAndDepth(
      mWsInfo.display, aWindow.visualID, &mWsInfo.visual, &depth);
  mWsInfo.depth = depth;
  ...
  PLUGIN_LOG_DEBUG(
      ("[InstanceChild][%p] Answer_SetWindow w=<x=%d,y=%d, w=%d,h=%d>, "
       ...
       mWindow.clipRect.top,
       mWindow.clipRect.right,
      ...
      if (mPluginIface->setwindow) {
        ...
        if (wndProc != PluginWindowProc) {
          mPluginWndProc = reinterpret_cast<WNDPROC>(
              SetWindowLongPtr(mPluginWindowHWND,
                               GWLP_WNDPROC,
    ...
    const NativeEventData& aKeyEventData, const bool& aIsConsumed)
{
  ...
  PluginWindowProcInternal(
      mPluginWindowHWND, message, eventData->mWParam, eventData->mLParam);
#endif
  return IPC_OK();
}
...
PluginInstanceChild::CreatePluginWindow()
{
  ...
  SetWindowLongPtrA(mPluginWindowHWND,
                    GWLP_WNDPROC,
...
void
PluginInstanceChild::SizePluginWindow(int width, int height)
{
  if (mPluginWindowHWND) {
    mPluginSize.x = width;
    mPluginSize.y = height;
    SetWindowPos(mPluginWindowHWND,
                 nullptr,
                 0,
                 0,
                 width,
                                      ...
                                      LPARAM lParam)
{
  return mozilla::CallWindowProcCrashProtected(
      PluginWindowProcInternal, hWnd, message, wParam, lParam);
}
                                         ...
                                         LPARAM lParam)
{
  ...
  ModifierKeyState modifierState;
  WinNativeKeyEventData winNativeKeyData(
      message, wParam, lParam, modifierState);
...
NeuteredWindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam);
...
#else
LONG WINAPI
PluginInstanceChild::SetWindowLongAHook(HWND hWnd, int nIndex, LONG newLong)
#endif
{
...
#else
LONG WINAPI
PluginInstanceChild::SetWindowLongWHook(HWND hWnd, int nIndex, LONG newLong)
#endif
{
  if (SetWindowLongHookCheck(hWnd, nIndex, newLong))
                                        ...
                                        CONST RECT* prcRect)
{
  ...
  if (!haveClass || (wcscmp(szClass, L"MozillaWindowClass") &&
    ...
    // Unrecognized parent
    return sUser32TrackPopupMenuStub(
  ...
  DWORD res = sUser32TrackPopupMenuStub(
      hMenu, uFlags | TPM_RETURNCMD, x, y, nReserved, surrogateHwnd, prcRect);
...
PluginInstanceChild::CreateWinlessPopupSurrogate()
{
  ...
  mWinlessPopupSurrogateHWND = CreateWindowEx(WS_EX_NOPARENTNOTIFY,
                                              L"Static",
                                              nullptr,
                                              WS_POPUP,
                                              0,
...
// static
LONG
PluginInstanceChild::ImmGetCompositionStringProc(HIMC aIMC,
                                                 DWORD aIndex,
                                                 LPVOID aBuf,
                                                 DWORD aLen)
{
...
// static
BOOL
PluginInstanceChild::ImmNotifyIME(HIMC aIMC,
                                  DWORD aAction,
                                  DWORD aIndex,
                                  DWORD aValue)
{
...
PluginInstanceChild::InitImm32Hook()
{
  ...
  sImm32Intercept.AddHook("ImmReleaseContext",
...
PluginInstanceChild::WinlessHandleEvent(NPEvent& event)
{
  ...
  if (event.event == WM_IME_STARTCOMPOSITION ||
      event.event == WM_IME_COMPOSITION || event.event == WM_KILLFOCUS) {
...
PluginInstanceChild::UnhookWinlessFlashThrottle()
{
  ...
  // reset the subclass
  SetWindowLongPtr(
      mWinlessHiddenMsgHWND, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(tmpProc));
                                               ...
                                               LPARAM lParam)
{
  ...
  return CallWindowProc(
      self->mWinlessThrottleOldWndProc, hWnd, message, wParam, lParam);
}
...
// static
BOOL CALLBACK
PluginInstanceChild::EnumThreadWindowsCallback(HWND hWnd, LPARAM aParam)
{
...
void
PluginInstanceChild::FlashThrottleMessage(
    HWND aWnd, UINT aMsg, WPARAM aWParam, LPARAM aLParam, bool isWindowed)
{
...
{
  *rv = DoNPP_NewStream(
      static_cast<BrowserStreamChild*>(actor), mimeType, seekable, stype);
  return IPC_OK();
}
...
{
  AssertPluginThread();
  return new BrowserStreamChild(this,
                                url,
                                length,
                                lastmodified,
...
mozilla::ipc::IPCResult
StreamNotifyChild::RecvRedirectNotify(const nsCString& url,
                                      const int32_t& status)
{
  ...
  if (instance->mPluginIface->urlredirectnotify)
    instance->mPluginIface->urlredirectnotify(
        instance->GetNPP(), url.get(), status, mClosure);
...
StreamNotifyChild::NPP_URLNotify(NPReason reason)
{
  ...
  if (mClosure)
    instance->mPluginIface->urlnotify(
        instance->GetNPP(), mURL.get(), reason, mClosure);
}
...
NPError
PluginInstanceChild::NPN_InitAsyncSurface(NPSize* size,
                                          ...
                                          NPAsyncSurface* surface)
{
  ...
  switch (mDrawingModel) {
    ...
    case NPDrawingModelAsyncWindowsDXGISurface: {
      ...
      SurfaceFormat mozformat = NPImageFormatToSurfaceFormat(format);
      if (!SendInitDXGISurface(
              mozformat, IntSize(size->width, size->height), &handle, &error)) {
        return NPERR_GENERIC_ERROR;
      }
...
void
PluginInstanceChild::NPN_SetCurrentAsyncSurface(NPAsyncSurface* surface,
                                                NPRect* changed)
{
  AssertPluginThread();
  ...
  switch (mDrawingModel) {
    case NPDrawingModelAsyncBitmapSurface: {
      ...
      SendShowDirectBitmap(
          shmemHolder, bitmap->mFormat, bitmap->mStride, bitmap->mSize, dirty);
      break;
    }
#if defined(XP_WIN)
    case NPDrawingModelAsyncWindowsDXGISurface: {
                                                ...
                                                gfxASurface* aSurface)
{
  ...
  ::IntersectClipRect(
      (HDC)mWindow.window, rect.left, rect.top, rect.right, rect.bottom);
                                        ...
                                        const Color& aColor)
{
  ...
  if (mIsTransparent && !CanPaintOnBackground()) {
    ...
    gfx::Rect rect(
        plPaintRect.x, plPaintRect.y, plPaintRect.width, plPaintRect.height);
                                                  ...
                                                  gfxASurface* aSurface)
{
  ...
  if (!useSurfaceSubimageForBlack) {
    ...
    dt->CopySurface(
        surface, IntRect(0, 0, rect.width, rect.height), rect.TopLeft());
  }
}
...
PluginInstanceChild::ShowPluginFrame()
{
  ...
  PLUGIN_LOG_DEBUG(
      ("[InstanceChild][%p] Painting%s <x=%d,y=%d, w=%d,h=%d> on surface "
       "<w=%d,h=%d>",
       this,
       haveTransparentPixels ? " with alpha" : "",
       rect.x,
       rect.y,
       rect.width,
       rect.height,
       mCurrentSurface->GetSize().width,
...
PluginInstanceChild::ReadbackDifferenceRect(const nsIntRect& rect)
{
  ...
  PLUGIN_LOG_DEBUG(
      ("[InstanceChild][%p] Reading back part of <x=%d,y=%d, w=%d,h=%d>",
       this,
       mSurfaceDifferenceRect.x,
       mSurfaceDifferenceRect.y,
       mSurfaceDifferenceRect.width,
       mSurfaceDifferenceRect.height));
...
uint32_t
PluginInstanceChild::ScheduleTimer(uint32_t interval,
                                   bool repeat,
                                   TimerFunc func)
{
