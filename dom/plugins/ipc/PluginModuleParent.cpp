...
class PluginModuleMapping : public PRCList
{
 public:
  ...
  static PluginModuleMapping* AssociateWithProcessId(uint32_t aPluginId,
                                   ...
                                   bool aAsync)
{
  ...
  if (chromeParent) {
    chromeParent->TakeFullMinidump(
        aContentProcessId, aBrowserDumpId, Move(aCallback), aAsync);
  } else {
...
PluginModuleChromeParent::InitCrashReporter()
{
...

  {
    ...
    mCrashReporter = MakeUnique<ipc::CrashReporterHost>(
        GeckoProcessType_Plugin, shmem, threadId);
  }
#endif
...
PluginModuleChromeParent::WriteExtraDataForMinidump()
{
  ...
  mCrashReporter->AddNote(NS_LITERAL_CSTRING("PluginFilename"),
                          CS(pluginFile.substr(filePos).c_str()));
...
bool
GetProcessCpuUsage(const InfallibleTArray<base::ProcessHandle>& processHandles,
                   InfallibleTArray<float>& cpuUsage)
{
  ...
  for (uint32_t i = 0; i < processHandles.Length(); ++i) {
    ...
    res = ::GetProcessTimes(
        processHandles[i], &creationTime, &exitTime, &kernelTime, &userTime);
    if (!res) {
...
PluginModuleChromeParent::GetManagingInstance(
 ...
 */
void
PluginModuleChromeParent::AnnotateHang(
...
#ifdef MOZ_CRASHREPORTER
static bool
CreatePluginMinidump(base::ProcessId processId,
...
void
PluginModuleChromeParent::TakeFullMinidump(
  ...
  if (CrashReporter::GetMinidumpForID(aBrowserDumpId,
    ...
    std::function<void(bool)> callback =
        [this, aContentPid, browserDumpId, aAsync](bool aResult) {
          ...
          this->TakeBrowserAndPluginMinidumps(
              aResult, aContentPid, browserDumpId, aAsync);
          if (aAsync) {
...
void
PluginModuleChromeParent::TakeBrowserAndPluginMinidumps(
    bool aReportsReady,
    base::ProcessId aContentPid,
    const nsAString& aBrowserDumpId,
    bool aAsync)
...
void
PluginModuleChromeParent::OnTakeFullMinidumpComplete(
    bool aReportsReady,
    base::ProcessId aContentPid,
    const nsAString& aBrowserDumpId)
{
  ...
  if (aReportsReady) {
    ...
    if (GetMinidumpForID(dumpId, getter_AddRefs(pluginDumpFile))) {
      ...
      if (CreatePluginMinidump(mFlashProcess1,
                               0,
                               pluginDumpFile,
                               NS_LITERAL_CSTRING("flash1"))) {
        additionalDumps.AppendLiteral(",flash1");
      }
      if (CreatePluginMinidump(mFlashProcess2,
                               0,
                               pluginDumpFile,
      ...
      if (aContentPid != mozilla::ipc::kInvalidProcessId) {
        ...
        if (CreatePluginMinidump(aContentPid,
                                 0,
                                 pluginDumpFile,
...
void
PluginModuleChromeParent::TerminateChildProcess(
    MessageLoop* aMsgLoop,
    base::ProcessId aContentPid,
    const nsCString& aMonitorDescription,
    const nsAString& aDumpId,
    std::function<void(bool)>&& aCallback,
...
void
PluginModuleChromeParent::TerminateChildProcessOnDumpComplete(
...
PluginModuleChromeParent::ProcessFirstMinidump()
{
  ...
  if (mFlashProcess1 &&
      TakeMinidumpForChild(
          mFlashProcess1, getter_AddRefs(childDumpFile), &childSequence)) {
  ...
  if (mFlashProcess2 &&
      TakeMinidumpForChild(
          mFlashProcess2, getter_AddRefs(childDumpFile), &childSequence)) {
    if (childSequence < sequence &&
        mCrashReporter->AdoptMinidump(childDumpFile)) {
      RemoveMinidump(dumpFile);
      dumpFile = childDumpFile;
...
PluginModuleChromeParent::ActorDestroy(ActorDestroyReason why)
{
  if (why == AbnormalShutdown) {
    ...
    Telemetry::Accumulate(
        Telemetry::SUBPROCESS_ABNORMAL_ABORT, NS_LITERAL_CSTRING("plugin"), 1);
  }
...
PluginModuleParent::AllocPPluginInstanceParent(
...
NPError
PluginModuleParent::NPP_Destroy(NPP instance, NPSavedData** saved)
{
  // FIXME/cjones:
...
NPError
PluginModuleParent::NPP_NewStream(NPP instance,
                                  NPMIMEType type,
                                  NPStream* stream,
                                  NPBool seekable,
                                  uint16_t* stype)
...
int32_t
PluginModuleParent::NPP_WriteReady(NPP instance, NPStream* stream)
{
...
void
PluginModuleParent::NPP_URLNotify(NPP instance,
                                  const char* url,
                                  NPReason reason,
                                  void* notifyData)
{
...
NPError
PluginModuleParent::NPP_SetValue(NPP instance,
...
void
PluginModuleParent::NPP_URLRedirectNotify(NPP instance,
                                          const char* url,
                                          int32_t status,
                                          void* notifyData)
{
...
nsresult
PluginModuleParent::GetImageContainer(
...
nsresult
PluginModuleParent::GetImageSize(NPP instance, nsIntSize* aSize)
{
  PluginInstanceParent* pip = PluginInstanceParent::Cast(instance);
...
#if defined(XP_WIN)
nsresult
PluginModuleParent::GetScrollCaptureContainer(
...
PluginModuleChromeParent::RegisterSettingsCallbacks()
{
  for (size_t i = 0; i < ArrayLength(kSettingsPrefs); i++) {
    Preferences::RegisterCallback(
        CachedSettingChanged, kSettingsPrefs[i], this);
  }
  ...
  if (observerService) {
    ...
    observerService->AddObserver(
        mPluginOfflineObserver, "ipc:network:set-offline", false);
  }
}
...
nsresult
PluginModuleParent::NP_Initialize(NPNetscapeFuncs* bFuncs,
...
nsresult
PluginModuleChromeParent::NP_Initialize(NPNetscapeFuncs* bFuncs,
                                        NPPluginFuncs* pFuncs,
                                        NPError* error)
{
  PLUGIN_LOG_DEBUG_METHOD;
...
nsresult
PluginModuleContentParent::NP_Initialize(NPNetscapeFuncs* bFuncs,
                                         NPError* error)
{
  PLUGIN_LOG_DEBUG_METHOD;
...
nsresult
PluginModuleParent::NP_GetValue(void* future,
                                ...
                                NPError* error)
{
  MOZ_LOG(GetPluginLog(),
          LogLevel::Warning,
          ("%s Not implemented, requested variable %i",
           __FUNCTION__,
           (int)aVariable));
...
nsresult
PluginModuleChromeParent::NP_GetEntryPoints(NPPluginFuncs* pFuncs,
                                            NPError* error)
{
#if !defined(XP_MACOSX)
...
nsresult
PluginModuleParent::NPP_NewInternal(NPMIMEType pluginType,
                                    NPP instance,
                                    InfallibleTArray<nsCString>& names,
                                    InfallibleTArray<nsCString>& values,
                                    NPSavedData* saved,
...
nsresult
PluginModuleParent::NPP_ClearSiteData(
    const char* site,
    uint64_t flags,
    uint64_t maxAge,
...
nsresult
PluginModuleParent::NPP_GetSitesWithData(
...
nsresult
PluginModuleParent::ContentsScaleFactorChanged(NPP instance,
...
mozilla::ipc::IPCResult
PluginModuleParent::RecvPluginShowWindow(const uint32_t& aWindowId,
                                         const bool& aModal,
                                         const int32_t& aX,
                                         const int32_t& aY,
                                         const size_t& aWidth,
                                         const size_t& aHeight)
{
  ...
  mac_plugin_interposing::parent::OnPluginShowWindow(
      aWindowId, windowBound, aModal);
  return IPC_OK();
#else
...
PluginModuleChromeParent::OnCrash(DWORD processID)
{
  if (!mShutdown) {
    ...
    if (base::OpenProcessHandle(OtherPid(), &geckoPluginChild.rwget())) {
      if (!base::KillProcess(
              geckoPluginChild, base::PROCESS_END_KILLED_BY_USER, false)) {
  ...
  if (*aResult) {
    if (ofn.Flags & OFN_ALLOWMULTISELECT) {
      ...
      if (ofn.lpstrFile[ofn.nFileOffset - 1] != L'\0') {
        mSandboxPermissions.GrantFileAccess(
            OtherPid(), path.c_str(), aFunc == SAVE_FUNC);
      } else {
        // This is condition #2
        ...
        while (*nextFile != L'\0') {
          ...
          mSandboxPermissions.GrantFileAccess(
              OtherPid(), fullPath.c_str(), aFunc == SAVE_FUNC);
          nextFile += nextFileStr.size() + 1;
        }
      }
    } else {
      mSandboxPermissions.GrantFileAccess(
          OtherPid(), ofn.lpstrFile, aFunc == SAVE_FUNC);
    }
...
mozilla::ipc::IPCResult
PluginModuleChromeParent::AnswerSetCursorPos(const int& x,
                                             const int& y,
                                             bool* aResult)
{
#if defined(XP_WIN)
