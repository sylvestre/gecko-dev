...
class AudioDeviceChangedRunnable : public Runnable
{
 public:
  explicit AudioDeviceChangedRunnable(
      const PluginModuleSet* aAudioNotificationSet,
  ...
  NS_IMETHOD Run() override
  {
    ...
    for (auto iter = mAudioNotificationSet->ConstIter(); !iter.Done();
      ...
      if (!pluginModule->SendNPP_SetValue_NPNVaudioDeviceChangeDetails(
              mChangeDetails)) {
...
class AudioNotification final : public IMMNotificationClient
{
 ...
 private:
  bool
      mIsRegistered;  // only used to make sure that Unregister is called before destroying a Valid instance.
  LONG mRefCt;
