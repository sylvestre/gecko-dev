...
#endif
{
 ...
 protected:
  ...
  virtual mozilla::ipc::IPCResult AnswerGetFileName(
      const GetFileNameFunc& aFunc,
  ...
  virtual mozilla::ipc::IPCResult AnswerSetCursorPos(const int& x,
                                                     const int& y,
                                                     bool* aResult) override
  {
    return IPC_FAIL_NO_REASON(this);
 ...
 protected:
  ...
  virtual mozilla::ipc::IPCResult AnswerGetKeyState(const int32_t& aVirtKey,
                                                    int16_t* aRet) override;
  ...
  nsresult NPP_NewInternal(NPMIMEType pluginType,
                           NPP instance,
  ...
  static int32_t NPP_Write(NPP instance,
                           NPStream* stream,
                           int32_t offset,
                           int32_t len,
                           void* buffer);
  ...
  static void NPP_URLRedirectNotify(NPP instance,
                                    const char* url,
                                    int32_t status,
                                    void* notifyData);
  ...
  virtual nsresult NP_GetValue(void* future,
  ...
  virtual nsresult NP_GetEntryPoints(NPPluginFuncs* pFuncs,
                                     NPError* error) override;
#endif
  virtual nsresult NPP_New(NPMIMEType pluginType,
                           NPP instance,
                           int16_t argc,
                           char* argn[],
                           char* argv[],
                                 ...
                                 public mozilla::HangMonitor::Annotator
{
 ...
 private:
  ...
  explicit PluginModuleChromeParent(const char* aFilePath,
                                    uint32_t aPluginId,
  ...
  {
   public:
    explicit LaunchedTask(PluginModuleChromeParent* aModule) : mModule(aModule)
    {
      MOZ_ASSERT(aModule);
    }
