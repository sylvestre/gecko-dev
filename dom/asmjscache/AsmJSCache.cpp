...
class AutoDecreaseUsageForOrigin
{
 ...
 public:
  ...
  ~AutoDecreaseUsageForOrigin()
  {
    ...
    qm->DecreaseUsageForOrigin(
        quota::PERSISTENCE_TYPE_TEMPORARY, mGroup, mOrigin, mFreed);
  }
};

static void
EvictEntries(nsIFile* aDirectory,
...
ParentRunnable::InitOnMainThread()
{
  ...
  rv = QuotaManager::GetInfoFromPrincipal(
      principal, &mSuffix, &mGroup, &mOrigin);
  NS_ENSURE_SUCCESS(rv, rv);
...
ParentRunnable::OpenCacheFileForWrite()
{
  ...
  mQuotaObject = qm->GetQuotaObject(
      quota::PERSISTENCE_TYPE_TEMPORARY, mGroup, mOrigin, file);
...
bool
FindHashMatch(const Metadata& aMetadata,
              const ReadParams& aReadParams,
...
ChildRunnable::ActorCreated(PBackgroundChild* aActor)
{
  ...
  if (!aActor->SendPAsmJSCacheEntryConstructor(
          this, mOpenMode, mWriteParams, *mPrincipalInfo)) {
...
class Client : public quota::Client
{
 ...
 public:
  ...
  Type GetType() override { return ASMJS; }
                      ...
                      UsageInfo* aUsageInfo) override
  {
    ...
    return GetUsageForOrigin(
        aPersistenceType, aGroup, aOrigin, aCanceled, aUsageInfo);
  }
                             ...
                             UsageInfo* aUsageInfo) override
  {
    ...
    nsresult rv = qm->GetDirectoryForOrigin(
...
bool
ParamTraits<Metadata>::Read(const Message* aMsg,
...
bool
ParamTraits<WriteParams>::Read(const Message* aMsg,
                               PickleIterator* aIter,
                               paramType* aResult)
{
