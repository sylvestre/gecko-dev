...
bool
ParseOffsetValue(const nsAString& aSpec, nsSMILTimeValue* aResult)
{
...
ParseAccessKey(const nsAString& aSpec, nsSMILTimeValueSpecParams& aResult)
{
  ...
  if (NS_IS_HIGH_SURROGATE(c)) {
    if (end - iter < 2)  // Expecting at least low-surrogate + ')'
      return false;
    uint32_t lo = *iter++;
    if (!NS_IS_LOW_SURROGATE(lo)) return false;
...
already_AddRefed<nsAtom>
ConvertTokenToAtom(const nsAString& aToken, bool aUnescapeToken)
{
...
bool
nsSMILParserUtils::ParseSemicolonDelimitedProgressList(
    const nsAString& aSpec, bool aNonDecreasing, FallibleTArray<double>& aArray)
{
    ...
    : public nsSMILParserUtils::GenericValueParser
{
 public:
  ...
  virtual bool Parse(const nsAString& aValueStr) override
  {
    ...
    if (NS_FAILED(mSMILAttr->ValueFromString(
            aValueStr, mSrcElement, newValue, tmpPreventCachingOfSandwich)))
      return false;
                               ...
                               bool& aPreventCachingOfSandwich)
{
  ...
  SMILValueParser valueParser(
      aSrcElement, &aAttribute, &aValuesArray, &aPreventCachingOfSandwich);
