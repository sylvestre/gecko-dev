...
GetGlobalObject(nsIChannel* aChannel)
{
...
nsresult
nsJSThunk::EvaluateScript(nsIChannel* aChannel,
                          PopupControlState aPopupState,
                          uint32_t aExecutionPolicy,
                          nsPIDOMWindowInner* aOriginalInnerWindow)
{
  ...
  options.setFileAndLine(mURL.get(), 1).setVersion(JSVERSION_DEFAULT);
  {
  ...
  if (NS_FAILED(rv) || !(v.isString() || v.isUndefined())) {
    ...
    if (bytes)
      rv = NS_NewByteInputStream(
          getter_AddRefs(mInnerStream), bytes, bytesLen, NS_ASSIGNMENT_ADOPT);
    else
...
nsresult
nsJSChannel::Init(nsIURI* aURI, nsILoadInfo* aLoadInfo)
{
      ...
      if (aLoadGroup != curLoadGroup) {
        ...
        if (curLoadGroup) {
          curLoadGroup->RemoveRequest(
              mStreamChannel, nullptr, NS_BINDING_RETARGETED);
        }
      }
    }
  }
                             ...
                             uint32_t aCount)
{
  ...
  return mListener->OnDataAvailable(
      this, aContext, aInputStream, aOffset, aCount);
}
...
nsresult
nsJSProtocolHandler::EnsureUTF8Spec(const nsCString& aSpec,
                                    const char* aCharset,
