...
PersistNodeFixup::FixupURI(nsAString& aURI)
{
  ...
  nsresult rv = NS_NewURI(
      getter_AddRefs(uri), aURI, mParent->GetCharacterSet(), mCurrentBaseURI);
  NS_ENSURE_SUCCESS(rv, rv);
  nsAutoCString spec;
  rv = uri->GetSpec(spec);
  NS_ENSURE_SUCCESS(rv, rv);
                                         ...
                                         const nsAString& aHref)
{
  ...
  if (!aHref.IsEmpty() && !href.IsEmpty()) {
    ...
    nsContentUtils::GetPseudoAttributeValue(
        data, nsGkAtoms::alternate, alternate);
                            ...
                            nsIDOMNode** aNodeOut)
{
  ...
  *aSerializeCloneKids = false;
  ...
  nsCOMPtr<nsIDOMHTMLTextAreaElement> nodeAsTextArea =
      do_QueryInterface(aNodeIn);
  if (nodeAsTextArea) {
    rv = GetNodeToFixup(aNodeIn, aNodeOut);
    if (NS_SUCCEEDED(rv) && *aNodeOut) {
    ...
    nsIWebBrowserPersistResourceVisitor* aVisitor)
{
  ...
  rv = oldStyleDoc->CreateTreeWalker(
      docAsNode,
    ...
    nsIWebBrowserPersistWriteCompletion* aCompletion)
{
  ...
  if (aMap) {
    ...
    if (NS_SUCCEEDED(rv) && !targetURISpec.IsEmpty()) {
      rv = NS_NewURI(getter_AddRefs(targetURI),
                     targetURISpec,
                     /* charset: */ nullptr,
                     /* base: */ nullptr);
