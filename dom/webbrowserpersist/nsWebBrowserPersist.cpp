...
class nsWebBrowserPersist::FlatURIMap final : public nsIWebBrowserPersistURIMap
{
 public:
  explicit FlatURIMap(const nsACString& aTargetBase) : mTargetBase(aTargetBase)
  {
  }
...
NS_IMETHODIMP
nsWebBrowserPersist::GetInterface(const nsIID& aIID, void** aIFace)
{
...
NS_IMETHODIMP
nsWebBrowserPersist::GetResult(nsresult* aResult)
{
...
NS_IMETHODIMP
nsWebBrowserPersist::SaveURI(nsIURI* aURI,
...
NS_IMETHODIMP
nsWebBrowserPersist::SavePrivacyAwareURI(nsIURI* aURI,
                                         nsISupports* aCacheKey,
                                         nsIURI* aReferrer,
                                         uint32_t aReferrerPolicy,
                                         nsIInputStream* aPostData,
...
NS_IMETHODIMP
nsWebBrowserPersist::SaveChannel(nsIChannel* aChannel, nsISupports* aFile)
{
  NS_ENSURE_TRUE(mFirstAndOnlyUse, NS_ERROR_FAILURE);
  mFirstAndOnlyUse = false;  // Stop people from reusing this object!
...
NS_IMETHODIMP
nsWebBrowserPersist::SaveDocument(nsISupports* aDocument,
                                  nsISupports* aFile,
...
NS_IMETHODIMP
nsWebBrowserPersist::OnStartRequest(nsIRequest* request, nsISupports* ctxt)
{
  if (mProgressListener) {
...
NS_IMETHODIMP
nsWebBrowserPersist::OnStopRequest(nsIRequest* request,
                                   nsISupports* ctxt,
                                   nsresult status)
{
  nsCOMPtr<nsISupports> keyPtr = do_QueryInterface(request);
  ...
  if (!cancel) {
    ...
    if (cancel) {
      SendErrorStatusChange(
          readError, rv, readError ? request : nullptr, data->mFile);
    }
  }
...
NS_IMETHODIMP
nsWebBrowserPersist::OnProgress(nsIRequest* request,
                                nsISupports* ctxt,
                                int64_t aProgress,
                                int64_t aProgressMax)
{
  ...
  if (mProgressListener2) {
    mProgressListener2->OnProgressChange64(nullptr,
                                           request,
                                           aProgress,
                                           aProgressMax,
    ...
    mProgressListener->OnProgressChange(nullptr,
                                        request,
                                        uint64_t(aProgress),
...
NS_IMETHODIMP
nsWebBrowserPersist::OnStatus(nsIRequest* request,
                              nsISupports* ctxt,
                              nsresult status,
...
nsresult
nsWebBrowserPersist::GetValidURIFromObject(nsISupports* aObject,
                                           nsIURI** aURI) const
{
...
nsresult
nsWebBrowserPersist::SaveURIInternal(nsIURI* aURI,
                                     nsISupports* aCacheKey,
                                     nsIURI* aReferrer,
                                     uint32_t aReferrerPolicy,
                                     nsIInputStream* aPostData,
...
nsresult
nsWebBrowserPersist::SaveChannelInternal(nsIChannel* aChannel,
                                         nsIURI* aFile,
                                         bool aCalcFileExt)
{
...
nsresult
nsWebBrowserPersist::GetExtensionForContentType(const char16_t* aContentType,
                                                char16_t** aExt)
{
    ...
    nsIWebBrowserPersistDocument* aDocument, nsIURI* aFile, nsIURI* aDataPath)
{
  ...
  NS_ENSURE_SUCCESS(rv, rv);
  rv = NS_NewURI(
      getter_AddRefs(mCurrentBaseURI), uriSpec, mCurrentCharset.get());
  NS_ENSURE_SUCCESS(rv, rv);
...
void
nsWebBrowserPersist::CleanupLocalFiles()
{
...
nsresult
nsWebBrowserPersist::MakeOutputStream(nsIURI* aURI,
...
nsresult
nsWebBrowserPersist::MakeOutputStreamFromFile(nsIFile* aFile,
                                              nsIOutputStream** aOutputStream)
{
  nsresult rv = NS_OK;
  ...
  *aOutputStream =
      NS_BufferOutputStream(fileOutputStream, BUFFERED_OUTPUT_SIZE).take();
...
nsresult
nsWebBrowserPersist::MakeOutputStreamFromURI(nsIURI* aURI,
                                             nsIOutputStream** aOutputStream)
{
...
nsresult
nsWebBrowserPersist::StoreURI(const char* aURI,
...
nsresult
nsWebBrowserPersist::StoreURI(nsIURI* aURI,
                              bool aNeedsPersisting,
                              URIData** aData)
{
  NS_ENSURE_ARG_POINTER(aURI);
...
nsresult
nsWebBrowserPersist::URIData::GetLocalURI(nsIURI* targetBaseURI,
                                          nsCString& aSpecOut)
{
...
nsresult
nsWebBrowserPersist::MakeAndStoreLocalFilenameInURIMap(nsIURI* aURI,
                                                       bool aNeedsPersisting,
                                                       URIData** aData)
{
  NS_ENSURE_ARG_POINTER(aURI);
...
void
nsWebBrowserPersist::SetApplyConversionIfNeeded(nsIChannel* aChannel)
{
  nsresult rv = NS_OK;
  ...
  if (NS_SUCCEEDED(rv) && hasMore) {
    ...
    if (NS_SUCCEEDED(rv)) {
      ...
      rv = helperAppService->ApplyDecodingForExtension(
