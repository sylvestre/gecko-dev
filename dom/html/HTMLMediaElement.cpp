...
class HTMLMediaElement::ChannelLoader final
{
 public:
  ...
  void LoadInternal(HTMLMediaElement* aElement)
  {
    ...
    bool setAttrs = nsContentUtils::GetLoadingPrincipalForXULNode(
        aElement,
...
NS_IMETHODIMP_(bool)
HTMLMediaElement::IsVideo() { return false; }
                                              ...
                                              public nsSupportsWeakReference
{
 ...
 protected:
  virtual ~ForceReloadListener() { MOZ_DIAGNOSTIC_ASSERT(!mWeak); }
...
class HTMLMediaElement::StreamListener : public MediaStreamListener
{
 public:
                            ...
                            GraphTime aCurrentTime) override
  {
    ...
    aGraph->DispatchToMainThreadAfterStreamStateUpdate(NewRunnableMethod(
        "dom::HTMLMediaElement::StreamListener::DoNotifyOutput",
        this,
...
HTMLMediaElement::PrincipalHandleChangedForVideoFrameContainer(
    VideoFrameContainer* aContainer, const PrincipalHandle& aNewPrincipalHandle)
{
  MOZ_ASSERT(NS_IsMainThread());
                               ...
                               ErrorResult& aRv)
{
  ...
  if (mMediaKeys) {
    ...
    if (mDecoder) {
      ...
      promise->MaybeReject(
          ...
          NS_LITERAL_CSTRING("Can't change MediaKeys on HTMLMediaElement after "
                             "load has started"));
      return promise.forget();
    }
  ...
  if (aMediaKeys) {
    if (!aMediaKeys->GetCDMProxy()) {
      promise->MaybeReject(
          NS_ERROR_DOM_INVALID_STATE_ERR,
          NS_LITERAL_CSTRING("CDM crashed before binding MediaKeys object to "
                             "HTMLMediaElement"));
      return promise.forget();
    }
