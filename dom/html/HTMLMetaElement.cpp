...
{
  if (!aDocument || !AttrValueIs(kNameSpaceID_None,
                                 nsGkAtoms::name,
                                 nsGkAtoms::referrer,
                                 eIgnoreCase)) {
    return NS_OK;
...
nsresult
HTMLMetaElement::AfterSetAttr(int32_t aNameSpaceID,
                              nsAtom* aName,
                              ...
                              bool aNotify)
{
  if (aNameSpaceID == kNameSpaceID_None) {
    ...
    if (aName == nsGkAtoms::content) {
      if (document && AttrValueIs(kNameSpaceID_None,
                                  nsGkAtoms::name,
                                  nsGkAtoms::viewport,
                                  eIgnoreCase)) {
        nsAutoString content;
...
nsresult
HTMLMetaElement::BindToTree(nsIDocument* aDocument,
                            nsIContent* aParent,
