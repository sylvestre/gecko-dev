...
static nsresult
QueryAll(mozIStorageConnection* aConn,
         CacheId aCacheId,
         nsTArray<EntryId>& aEntryIdListOut);
static nsresult
QueryCache(mozIStorageConnection* aConn,
           CacheId aCacheId,
...
DeleteEntries(mozIStorageConnection* aConn,
              ...
              int32_t aLen = -1);
static nsresult
InsertSecurityInfo(mozIStorageConnection* aConn,
...
static nsresult
InsertEntry(mozIStorageConnection* aConn,
            CacheId aCacheId,
            const CacheRequest& aRequest,
...
static nsresult
ReadResponse(mozIStorageConnection* aConn,
             EntryId aEntryId,
             SavedResponse* aSavedResponseOut);
static nsresult
ReadRequest(mozIStorageConnection* aConn,
            EntryId aEntryId,
...
nsresult
GetEffectiveSchemaVersion(mozIStorageConnection* aConn, int32_t& schemaVersion);
nsresult
         ...
         int64_t* aDeletedPaddingSizeOut)
{
  ...
  rv = InsertEntry(
      aConn, aCacheId, aRequest, aRequestBodyId, aResponse, aResponseBodyId);
  if (NS_WARN_IF(NS_FAILED(rv))) {
    return rv;
  }
             ...
             SavedResponse* aSavedResponseOut)
{
  ...
  *aFoundResponseOut = false;
  ...
  if (!aParams.cacheName().EqualsLiteral("")) {
    ...
    CacheId cacheId = INVALID_CACHE_ID;
    rv = StorageGetCacheId(
        aConn, aNamespace, aParams.cacheName(), &foundCache, &cacheId);
    if (NS_WARN_IF(NS_FAILED(rv))) {
      return rv;
    }
    if (!foundCache) {
              ...
              int32_t aLen)
{
  ...
  if (aLen > kMaxEntriesPerStatement) {
    ...
    while (remaining > 0) {
      ...
      nsresult rv = DeleteEntries(aConn,
                                  aEntryIdList,
                                  aDeletedBodyIdListOut,
                       ...
                       const nsTArray<IdCount>& aDeletedStorageIdList)
{
  for (uint32_t i = 0; i < aDeletedStorageIdList.Length(); ++i) {
    nsresult rv = DeleteSecurityInfo(
        aConn, aDeletedStorageIdList[i].mId, aDeletedStorageIdList[i].mCount);
    if (NS_WARN_IF(NS_FAILED(rv))) {
      return rv;
    }
  }
...
nsresult
MigrateFrom15To16(mozIStorageConnection* aConn, bool& aRewriteSchema);
nsresult
...
nsresult
MigrateFrom19To20(mozIStorageConnection* aConn, bool& aRewriteSchema);
nsresult
MigrateFrom20To21(mozIStorageConnection* aConn, bool& aRewriteSchema);
nsresult
MigrateFrom21To22(mozIStorageConnection* aConn, bool& aRewriteSchema);
nsresult
MigrateFrom22To23(mozIStorageConnection* aConn, bool& aRewriteSchema);
nsresult
MigrateFrom23To24(mozIStorageConnection* aConn, bool& aRewriteSchema);
nsresult
MigrateFrom24To25(mozIStorageConnection* aConn, bool& aRewriteSchema);
nsresult
MigrateFrom25To26(mozIStorageConnection* aConn, bool& aRewriteSchema);
nsresult
MigrateFrom26To27(mozIStorageConnection* aConn, bool& aRewriteSchema);
