...
static nsresult
LockedGetPaddingSizeFromDB(nsIFile* aDir,
                           ...
                           int64_t* aPaddingSizeOut)
{
  ...
  *aPaddingSizeOut = 0;
  ...
  nsresult rv = mozilla::dom::cache::OpenDBConnection(
      quotaInfo, aDir, getter_AddRefs(conn));
 ...
 public:
                                     ...
                                     UsageInfo* aUsageInfo) override
  {
    ...
    nsresult rv = qm->GetDirectoryForOrigin(
        aPersistenceType, aOrigin, getter_AddRefs(dir));
    if (NS_WARN_IF(NS_FAILED(rv))) {
      return rv;
    }
