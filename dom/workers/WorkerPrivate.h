...
class SharedMutex
{
 ...
 public:
  explicit SharedMutex(const char* aName) : mMutex(new RefCountedMutex(aName))
  {
  }
...
class WorkerPrivateParent : public DOMEventTargetHelper
{
 ...
 private:
  Derived* ParentAsWorkerPrivate() const
  {
    return static_cast<Derived*>(const_cast<WorkerPrivateParent*>(this));
  }

  bool NotifyPrivate(Status aStatus);
 ...
 public:
  ...
  bool Notify(Status aStatus) { return NotifyPrivate(aStatus); }
  ...
  void UpdatePreference(WorkerPreference aPref, bool aValue);
  ...
  uint64_t ServiceWorkerID() const { return mLoadInfo.mServiceWorkerID; }
  ...
  void CopyJSSettings(JSSettings& aSettings)
  {
...
class WorkerDebugger : public nsIWorkerDebugger
{
 ...
 public:
  ...
  void ReportErrorToDebugger(const nsAString& aFilename,
                             uint32_t aLineno,
...
class WorkerPrivate : public WorkerPrivateParent<WorkerPrivate>
{
 ...
 public:
  ...
  static bool WorkerAvailable(JSContext* /* unused */, JSObject* /* unused */);
  ...
  static nsresult GetLoadInfo(JSContext* aCx,
  ...
  bool AddChildWorker(ParentType* aChildWorker);

  void RemoveChildWorker(ParentType* aChildWorker);

  void PostMessageToParent(JSContext* aCx,
                           JS::Handle<JS::Value> aMessage,
                           const Sequence<JSObject*>& aTransferable,
                           ErrorResult& aRv)
  {
  ...
  bool NotifyInternal(JSContext* aCx, Status aStatus);

  void ReportError(JSContext* aCx,
  ...
  void UpdatePreferenceInternal(WorkerPreference aPref, bool aValue);

  void UpdateJSWorkerMemoryParameterInternal(JSContext* aCx,
                                             JSGCParamKey key,
                                             uint32_t aValue);
...
#ifdef JS_GC_ZEAL
  void UpdateGCZealInternal(JSContext* aCx,
                            uint8_t aGCZeal,
                            uint32_t aFrequency);
#endif

  void GarbageCollectInternal(JSContext* aCx,
                              bool aShrinking,
  ...
  WorkerCrossThreadDispatcher* GetCrossThreadDispatcher();
...
#undef WORKER_PREF
 ...
 private:
  ...
  void GetAllPreferences(bool aPreferences[WORKERPREF_COUNT]) const
  {
    AssertIsOnWorkerThread();
  ...
  already_AddRefed<nsIEventTarget> CreateNewSyncLoop(Status aFailStatus);
  ...
  bool AddHolder(WorkerHolder* aHolder, Status aFailStatus);

  void RemoveHolder(WorkerHolder* aHolder);

  void NotifyHolders(JSContext* aCx, Status aStatus);
...
{
 public:
  static already_AddRefed<ChromeWorkerPrivate> Constructor(
      const GlobalObject& aGlobal,
      const nsAString& aScriptURL,
      ErrorResult& rv);
