...
inline const char*
CSP_CSPDirectiveToString(CSPDirective aDir)
{
  return CSPStrDirectives[static_cast<uint32_t>(aDir)];
}

inline CSPDirective
CSP_StringToCSPDirective(const nsAString& aDir)
{
...
#define FOR_EACH_CSP_KEYWORD(macro)                                     \
  macro(CSP_SELF, "'self'") macro(CSP_UNSAFE_INLINE, "'unsafe-inline'") \
      macro(CSP_UNSAFE_EVAL, "'unsafe-eval'") macro(CSP_NONE, "'none'") \
...
inline const char*
CSP_EnumToUTF8Keyword(enum CSPKeyword aKey)
{
...
inline CSPKeyword
CSP_UTF16KeywordToEnum(const nsAString& aKey)
{
...
class nsCSPBaseSrc
{
 public:
  ...
  virtual bool permits(nsIURI* aUri,
