...
{
 public:
  explicit nsRefMapEntry(const nsAString& aKey) : nsStringHashKey(&aKey) {}
  explicit nsRefMapEntry(const nsAString* aKey) : nsStringHashKey(aKey) {}
  nsRefMapEntry(const nsRefMapEntry& aOther) : nsStringHashKey(&aOther.GetKey())
  {
                          ...
                          public nsIOffThreadScriptReceiver
{
 public:
  ...
  virtual void ResetDocumentLWTheme() override
  {
  ...
  already_AddRefed<nsINodeList> GetElementsByAttributeNS(
      ...
      ErrorResult& aRv);
  void AddBroadcastListenerFor(Element& aBroadcaster,
                               Element& aListener,
                               const nsAString& aAttr,
                               ErrorResult& aRv);
  void RemoveBroadcastListenerFor(Element& aBroadcaster,
                                  Element& aListener,
                                  const nsAString& aAttr);
  ...
  void LoadOverlay(const nsAString& aURL,
 ...
 protected:
  ...
  friend nsresult(::NS_NewXULDocument(nsIXULDocument** aResult));

  nsresult Init(void) override;
  nsresult StartLayout(void);

  nsresult AddElementToRefMap(Element* aElement);
  void RemoveElementFromRefMap(Element* aElement);
  ...
  nsresult LoadOverlayInternal(nsIURI* aURI,
                               bool aIsDynamic,
                               bool* aShouldReturn,
  ...
  nsresult ApplyPersistentAttributesToElements(
  ...
  nsresult Persist(nsIContent* aElement,
                   int32_t aNameSpaceID,
                   nsAtom* aAttribute);
  ...
  class ContextStack
  {
   ...
   public:
    ...
    nsresult Pop();
    nsresult Peek(nsXULPrototypeElement** aPrototype,
                  nsIContent** aElement,
                  int32_t* aIndex);
  ...
  /**
     ...
     */
  nsresult AddAttributes(nsXULPrototypeElement* aPrototype,
                         nsIContent* aElement);
  ...
  /**
     ...
     */
  static nsresult CheckTemplateBuilderHookup(nsIContent* aElement,
                                             bool* aNeedsHookup);

  /**
     * Create a XUL template builder on the specified node.
     */
  static nsresult CreateTemplateBuilder(Element* aElement);
 ...
 protected:
  ...
  class OverlayForwardReference : public nsForwardReference
  {
   protected:
    ...
    nsresult Merge(nsIContent* aTargetNode,
  ...
  class TemplateBuilderHookup : public nsForwardReference
  {
   ...
   public:
    explicit TemplateBuilderHookup(Element* aElement) : mElement(aElement) {}
  ...
  nsresult FindBroadcaster(Element* aElement,
                           Element** aListener,
                           nsString& aBroadcasterID,
                           nsString& aAttribute,
                           Element** aBroadcaster);

  nsresult CheckBroadcasterHookup(Element* aElement,
                                  bool* aNeedsHookup,
                                  bool* aDidResolve);

  void SynchronizeBroadcastListener(Element* aBroadcaster,
                                    Element* aListener,
                                    const nsAString& aAttr);

  static nsresult InsertElement(nsINode* aParent,
                                nsIContent* aChild,
                                bool aNotify);

  static nsresult RemoveElement(nsINode* aParent, nsINode* aChild);
