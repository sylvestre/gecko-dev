...
nsRDFConMemberTestNode::nsRDFConMemberTestNode(
    TestNode* aParent,
      ...
      mMemberVariable(aMemberVariable)
{
  if (MOZ_LOG_TEST(gXULTemplateLog, LogLevel::Debug)) {
    ...
    if (mContainerVariable) mContainerVariable->ToString(cvar);

    nsAutoString mvar(NS_LITERAL_STRING("(none)"));
    if (mMemberVariable) mMemberVariable->ToString(mvar);

    MOZ_LOG(gXULTemplateLog,
            LogLevel::Debug,
            ("nsRDFConMemberTestNode[%p]: parent=%p member-props=(%s) "
             "container-var=%s member-var=%s",
             this,
             aParent,
             props.get(),
                                             ...
                                             bool* aCantHandleYet) const
{
  ...
  if (aCantHandleYet) *aCantHandleYet = false;

  nsCOMPtr<nsIRDFContainerUtils> rdfc =
      do_GetService("@mozilla.org/rdf/container-utils;1");

  if (!rdfc) return NS_ERROR_FAILURE;

  nsIRDFDataSource* ds = mProcessor->GetDataSource();

  InstantiationSet::Iterator last = aInstantiations.Last();
  for (InstantiationSet::Iterator inst = aInstantiations.First(); inst != last;
       ++inst) {
    bool hasContainerBinding;
    nsCOMPtr<nsIRDFNode> containerValue;
    hasContainerBinding = inst->mAssignments.GetAssignmentFor(
        mContainerVariable, getter_AddRefs(containerValue));
    ...
    hasMemberBinding = inst->mAssignments.GetAssignmentFor(
        mMemberVariable, getter_AddRefs(memberValue));

    if (MOZ_LOG_TEST(gXULTemplateLog, LogLevel::Debug)) {
      const char* container = "(unbound)";
      if (hasContainerBinding) containerRes->GetValueConst(&container);
      ...
      MOZ_LOG(gXULTemplateLog,
              LogLevel::Debug,
              ("nsRDFConMemberTestNode[%p]: FilterInstantiations() "
               "container=[%s] member=[%s]",
               this,
               container,
               NS_ConvertUTF16toUTF8(member).get()));
    }
    ...
    if (hasContainerBinding && hasMemberBinding) {
      ...
      if (!isconsistent) {
        ...
        for (nsResourceSet::ConstIterator property = containmentProps.First();
          ...
          bool hasAssertion;
          rv = ds->HasAssertion(
              containerRes, *property, memberValue, true, &hasAssertion);
          if (NS_FAILED(rv)) return rv;
      ...
      if (isconsistent) {
        ...
        Element* element =
            new nsRDFConMemberTestNode::Element(containerRes, memberValue);
    ...
    if (hasMemberBinding) {
      ...
      while (1) {
...

        {
          ...
          if (!property) return NS_ERROR_UNEXPECTED;
        }
        ...
        if (isordinal) {
          ...
          rv = ds->GetSources(
              property, memberValue, true, getter_AddRefs(sources));
          if (NS_FAILED(rv)) return rv;
          ...
          while (1) {
            ...
            if (!source) return NS_ERROR_UNEXPECTED;
            ...
            Element* element =
                new nsRDFConMemberTestNode::Element(source, memberValue);
            newinst.AddSupportingElement(element);
    ...
    if ((hasContainerBinding && !hasMemberBinding) ||
      ...
      for (nsResourceSet::ConstIterator property = containmentProps.First();
        ...
        if (hasContainerBinding) {
          rv = ds->GetTargets(
              containerRes, *property, true, getter_AddRefs(results));
        } else {
          rv = ds->GetSources(
              *property, memberValue, true, getter_AddRefs(results));
        }
        if (NS_FAILED(rv)) return rv;
        ...
        while (1) {
          ...
          if (hasContainerBinding) {
            ...
            if (MOZ_LOG_TEST(gXULTemplateLog, LogLevel::Debug)) {
              ...
              MOZ_LOG(
                  gXULTemplateLog, LogLevel::Debug, ("    container => %s", s));
            }
          }
          ...
          Element* element;
          if (hasContainerBinding) {
            element = new nsRDFConMemberTestNode::Element(containerRes, value);
          } else {
            element =
