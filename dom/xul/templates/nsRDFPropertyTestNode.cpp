      ...
      mTarget(nullptr)
{
  if (MOZ_LOG_TEST(gXULTemplateLog, LogLevel::Debug)) {
    ...
    if (aProperty) aProperty->GetValueConst(&prop);

    nsAutoString svar(NS_LITERAL_STRING("(none)"));
    if (mSourceVariable) mSourceVariable->ToString(svar);

    nsAutoString tvar(NS_LITERAL_STRING("(none)"));
    if (mTargetVariable) mTargetVariable->ToString(tvar);
    ...
    MOZ_LOG(
        ...
        ("nsRDFPropertyTestNode[%p]: parent=%p source=%s property=%s target=%s",
         this,
         aParent,
         NS_ConvertUTF16toUTF8(svar).get(),
         prop,
         NS_ConvertUTF16toUTF8(tvar).get()));
  }
}
      ...
      mTarget(nullptr)
{
  if (MOZ_LOG_TEST(gXULTemplateLog, LogLevel::Debug)) {
    ...
    if (aSource) aSource->GetValueConst(&source);
                                            ...
                                            bool* aCantHandleYet) const
{
  ...
  for (InstantiationSet::Iterator inst = aInstantiations.First(); inst != last;
    ...
    if (mSource) {
      ...
      hasSourceBinding = inst->mAssignments.GetAssignmentFor(
          mSourceVariable, getter_AddRefs(sourceValue));
    ...
    if (mTarget) {
      ...
      hasTargetBinding = inst->mAssignments.GetAssignmentFor(
          mTargetVariable, getter_AddRefs(targetValue));
    }

    if (MOZ_LOG_TEST(gXULTemplateLog, LogLevel::Debug)) {
      const char* source = "(unbound)";
      if (hasSourceBinding) sourceRes->GetValueConst(&source);
      ...
      MOZ_LOG(gXULTemplateLog,
              LogLevel::Debug,
              ("nsRDFPropertyTestNode[%p]: FilterInstantiations() source=[%s] "
               "target=[%s]",
               this,
               source,
               NS_ConvertUTF16toUTF8(target).get()));
    ...
    if (hasSourceBinding && hasTargetBinding) {
      ...
      bool hasAssertion;
      rv = ds->HasAssertion(
          sourceRes, mProperty, targetValue, true, &hasAssertion);
      ...
      if (hasAssertion) {
        // it's consistent.
        Element* element = new nsRDFPropertyTestNode::Element(
            sourceRes, mProperty, targetValue);
      ...
      if (hasSourceBinding) {
        rv =
            ds->GetTargets(sourceRes, mProperty, true, getter_AddRefs(results));
      } else {
        rv = ds->GetSources(
            mProperty, targetValue, true, getter_AddRefs(results));
        if (NS_FAILED(rv)) return rv;
      }
      ...
      while (1) {
        ...
        if (hasSourceBinding) {
          ...
          if (MOZ_LOG_TEST(gXULTemplateLog, LogLevel::Debug)) {
            ...
            if (value) nsXULContentUtils::GetTextForNode(value, s);
          ...
          if (MOZ_LOG_TEST(gXULTemplateLog, LogLevel::Debug)) {
            ...
            if (source) source->GetValueConst(&s);

            MOZ_LOG(gXULTemplateLog, LogLevel::Debug, ("    source => %s", s));
          }
                                    ...
                                    Instantiation& aInitialBindings) const
{
  ...
  if ((mProperty.get() != aProperty) || (mSource && mSource.get() != aSource) ||
