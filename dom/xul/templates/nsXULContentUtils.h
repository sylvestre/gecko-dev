...
#define ERROR_TEMPLATE_INVALID_QUERYSET "unexpected <queryset> element"
#define ERROR_TEMPLATE_NO_MEMBERVAR                                        \
  "no member variable found. Action body should have an element with uri " \
  "attribute"
...
#define ERROR_TEMPLATE_CONTENT_NOT_FIRST "expected <content> to be first"
...
#define ERROR_TEMPLATE_BAD_XPATH "XPath expression in query could not be parsed"
...
#define ERROR_TEMPLATE_STORAGE_BAD_URI "only profile: or file URI are allowed"
#define ERROR_TEMPLATE_STORAGE_CANNOT_OPEN_DATABASE "cannot open given database"
#define ERROR_TEMPLATE_STORAGE_BAD_QUERY "syntax error in the SQL query"
...
class nsXULContentUtils
{
 ...
 public:
  ...
  static nsresult FindChildByTag(nsIContent* aElement,
                                 int32_t aNameSpaceID,
                                 nsAtom* aTag,
                                 nsIContent** aResult);

  static nsresult FindChildByResource(nsIContent* aElement,
                                      nsIRDFResource* aResource,
                                      nsIContent** aResult);

  static nsresult GetTextForNode(nsIRDFNode* aNode, nsAString& aResult);

  static nsresult GetResource(int32_t aNameSpaceID,
                              nsAtom* aAttribute,
  ...
  /**
     ...
     */
  static void LogTemplateError(const char* aMsg);
