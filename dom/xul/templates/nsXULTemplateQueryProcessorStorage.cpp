...
{
  nsXULTemplateResultStorage* result = new nsXULTemplateResultStorage(this);
  *aResult = result;
  NS_ADDREF(result);
  return NS_OK;
}
...
nsXULTemplateResultSetStorage::GetColumnIndex(nsAtom* aColumnName)
{
  ...
  for (int32_t c = 0; c < count; c++) {
    if (mColumnNames[c] == aColumnName) return c;
  }
...
NS_IMETHODIMP
nsXULTemplateQueryProcessorStorage::InitializeForBuilding(
    ...
    nsIDOMNode* aRootNode)
{
  ...
  if (!mStorageConnection) return NS_ERROR_INVALID_ARG;
...
NS_IMETHODIMP
nsXULTemplateQueryProcessorStorage::CompileQuery(
    nsIXULTemplateBuilder* aBuilder,
    ...
    nsISupports** aReturn)
{
  ...
  if (!nsContentUtils::GetNodeTextContent(
          queryContent, false, sqlQuery, fallible)) {
    return NS_ERROR_OUT_OF_MEMORY;
  }

  nsresult rv = mStorageConnection->CreateStatement(
      NS_ConvertUTF16toUTF8(sqlQuery), getter_AddRefs(statement));
  if (NS_FAILED(rv)) {
  ...
  for (nsIContent* child = queryContent->GetFirstChild(); child;
    ...
    if (child->NodeInfo()->Equals(nsGkAtoms::param, kNameSpaceID_XUL)) {
      ...
      if (child->GetAttr(kNameSpaceID_None, nsGkAtoms::name, name)) {
        rv = statement->GetParameterIndex(NS_ConvertUTF16toUTF8(name), &index);
        if (NS_FAILED(rv)) {
          nsXULContentUtils::LogTemplateError(
        ...
        parameterCount++;
      } else if (child->GetAttr(
                     kNameSpaceID_None, nsGkAtoms::index, indexValue)) {
      ...
      static nsIContent::AttrValuesArray sTypeValues[] = {&nsGkAtoms::int32,
                                                          &nsGkAtoms::integer,
                                                          &nsGkAtoms::int64,
                                                          &nsGkAtoms::null,
                                                          &nsGkAtoms::double_,
      ...
      int32_t typeError = 1;
      int32_t typeValue = child->FindAttrValueIn(
          kNameSpaceID_None, nsGkAtoms::type, sTypeValues, eCaseMatters);
      ...
      switch (typeValue) {
        ...
        case 1:
          typeError =
              PR_sscanf(NS_ConvertUTF16toUTF8(value).get(), "%d", &valInt32);
          if (typeError > 0) rv = statement->BindInt32ByIndex(index, valInt32);
          break;
        case 2:
          typeError =
              PR_sscanf(NS_ConvertUTF16toUTF8(value).get(), "%lld", &valInt64);
          if (typeError > 0) rv = statement->BindInt64ByIndex(index, valInt64);
          break;
        case 3:
        ...
        case 4:
          typeError =
              PR_sscanf(NS_ConvertUTF16toUTF8(value).get(), "%lf", &valFloat);
          if (typeError > 0) rv = statement->BindDoubleByIndex(index, valFloat);
          break;
        case 5:
...
NS_IMETHODIMP
nsXULTemplateQueryProcessorStorage::GenerateResults(
    nsISupports* aDatasource,
    ...
    nsISimpleEnumerator** aResults)
{
  ...
  if (!statement) return NS_ERROR_FAILURE;
...
nsXULTemplateQueryProcessorStorage::TranslateRef(nsISupports* aDatasource,
                                                 ...
                                                 nsIXULTemplateResult** aRef)
{
  nsXULTemplateResultStorage* result = new nsXULTemplateResultStorage(nullptr);
  *aRef = result;
  NS_ADDREF(*aRef);
  return NS_OK;
}
                                                   ...
                                                   int32_t* aResult)
{
  *aResult = 0;
  ...
  if (aLeft) aLeft->GetBindingObjectFor(aVar, getter_AddRefs(leftValue));
  if (aRight) aRight->GetBindingObjectFor(aVar, getter_AddRefs(rightValue));
  ...
  if (aLeft) aLeft->GetBindingFor(aVar, leftVal);

  nsAutoString rightVal;
  if (aRight) aRight->GetBindingFor(aVar, rightVal);
