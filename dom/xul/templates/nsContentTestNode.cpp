      ...
      mTag(nullptr)
{
  if (MOZ_LOG_TEST(gXULTemplateLog, LogLevel::Debug)) {
    ...
    if (mTag) mTag->ToString(tag);

    nsAutoString refvar(NS_LITERAL_STRING("(none)"));
    if (aRefVariable) aRefVariable->ToString(refvar);

    MOZ_LOG(gXULTemplateLog,
            LogLevel::Debug,
            ("nsContentTestNode[%p]: ref-var=%s tag=%s",
             this,
             NS_ConvertUTF16toUTF8(refvar).get(),
...
nsContentTestNode::Constrain(InstantiationSet& aInstantiations)
{
  ...
  for (InstantiationSet::Iterator inst = aInstantiations.First(); inst != last;
    ...
    bool hasRefBinding = inst->mAssignments.GetAssignmentFor(
        mRefVariable, getter_AddRefs(refValue));
    if (hasRefBinding) {
