...
nsTreeRows::Subtree*
nsTreeRows::EnsureSubtreeFor(Subtree* aParent, int32_t aChildIndex)
{
                          ...
                          int32_t* aSubtreeSize)
{
  ...
  if (aSubtreeSize) *aSubtreeSize = result ? result->mSubtreeSize : 0;
...
{
  NS_PRECONDITION(aParent, "no parent");
  NS_PRECONDITION(aChildIndex >= 0 && aChildIndex < aParent->mCount,
                  "bad child index");
  ...
  if (row.mSubtree) {
    ...
    for (Subtree* subtree = aParent; subtree != nullptr;
         subtree = subtree->mParent)
...
nsTreeRows::iterator nsTreeRows::operator[](int32_t aRow)
{
...
nsTreeRows::FindByResource(nsIRDFResource* aResource)
{
  ...
  for (iter = First(); iter != last; ++iter) {
    ...
    if (stringmode) {
      ...
      if (resourceid.Equals(findid)) break;
    }
  }
...
nsTreeRows::Find(nsIXULTemplateResult* aResult)
{
  ...
  for (iter = First(); iter != last; ++iter) {
    if (aResult == iter->mMatch->mResult) break;
  }
...
nsTreeRows::Subtree::~Subtree() { Clear(); }

void
nsTreeRows::Subtree::Clear()
{
  for (int32_t i = mCount - 1; i >= 0; --i) delete mRows[i].mSubtree;
...
nsTreeRows::Subtree::InsertRowAt(nsTemplateMatch* aMatch, int32_t aIndex)
{
  if (mCount >= mCapacity || aIndex >= mCapacity) {
    ...
    if (!newRows) return iterator();

    for (int32_t i = mCount - 1; i >= 0; --i) newRows[i] = mRows[i];
  ...
  for (int32_t i = mCount - 1; i >= aIndex; --i) mRows[i + 1] = mRows[i];
  ...
  for (; --aIndex >= 0; ++rowIndex) {
    ...
    if (subtree) rowIndex += subtree->mSubtreeSize;
  }
  ...
  do {
    ...
    for (aIndex = 0; aIndex < count; ++aIndex, ++rowIndex) {
      ...
      if (child) rowIndex += child->mSubtreeSize;
    }
...
nsTreeRows::Subtree::RemoveRowAt(int32_t aIndex)
{
  ...
  int32_t subtreeSize =
      mRows[aIndex].mSubtree ? mRows[aIndex].mSubtree->GetSubtreeSize() : 0;

  ++subtreeSize;

  delete mRows[aIndex].mSubtree;

  for (int32_t i = aIndex + 1; i < mCount; ++i) mRows[i - 1] = mRows[i];
...
nsTreeRows::iterator::iterator(const iterator& aIterator)
    : mRowIndex(aIterator.mRowIndex), mLink(aIterator.mLink)
{
}
...
{
  if (GetDepth() != aIterator.GetDepth()) return false;

  if (GetDepth() == 0) return true;
...
nsTreeRows::iterator::Next()
{
  ...
  if (top.mChildIndex >= top.mParent->Count() - 1) {
    ...
    for (unfinished = GetDepth() - 2; unfinished >= 0; --unfinished) {
      ...
      if (link.mChildIndex < link.mParent->Count() - 1) break;
    }
