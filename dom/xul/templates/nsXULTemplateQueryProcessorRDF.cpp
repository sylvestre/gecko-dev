...
NS_IMPL_CYCLE_COLLECTION_TRAVERSE_BEGIN(nsXULTemplateQueryProcessorRDF)
  ...
  for (auto it = tmp->mMemoryElementToResultMap.Iter(); !it.Done(); it.Next()) {
                                              ...
                                              nsISupports** aResult)
{
  ...
  *aShouldDelayBuilding = false;
  ...
  if (!infer.IsEmpty()) {
    ...
    nsCOMPtr<nsIRDFInferDataSource> inferDB = do_CreateInstance(inferCID.get());
...
NS_IMETHODIMP
nsXULTemplateQueryProcessorRDF::InitializeForBuilding(
    nsISupports* aDatasource,
    nsIXULTemplateBuilder* aBuilder,
    nsIDOMNode* aRootNode)
{
  ...
  if (mGenerationStarted) return NS_ERROR_UNEXPECTED;
...
{
  if (!mQueryProcessorRDFInited) return NS_OK;
  ...
  for (ReteNodeSet::Iterator node = mAllTests.First(); node != mAllTests.Last();
       ++node)
    delete *node;
                                             ...
                                             nsISupports** _retval)
{
  ...
  if (!mRefVariable) mRefVariable = aRefVariable;
                                                ...
                                                nsISimpleEnumerator** aResults)
{
  ...
  if (!rdfquery) return NS_ERROR_INVALID_ARG;
  ...
  *aResults = nullptr;
  ...
  if (aRef) {
    ...
    if (!results) {
      if (!query->mRefVariable) query->mRefVariable = NS_Atomize("?uri");
      ...
      if (MOZ_LOG_TEST(gXULTemplateLog, LogLevel::Debug)) {
        ...
        MOZ_LOG(gXULTemplateLog,
                LogLevel::Debug,
                ("QueryProcessor::GenerateResults using ref %s and vars [ ref: "
                 "%s  member: %s]",
      ...
      if (root) {
        ...
        if (!owned) delete instantiations;
        if (NS_FAILED(rv)) return rv;
                                           ...
                                           const nsAString& aExpr)
{
  ...
  nsresult rv =
      gRDFService->GetUnicodeResource(aExpr, getter_AddRefs(property));
  if (NS_FAILED(rv)) return rv;
                                             ...
                                             nsIXULTemplateResult** aRef)
{
  ...
  if (!refresult) return NS_ERROR_OUT_OF_MEMORY;
                                               ...
                                               int32_t* aResult)
{
  ...
  *aResult = 0;
  ...
  if (!sortkey.IsEmpty() && sortkey[0] != '?' &&
      !StringBeginsWith(sortkey, NS_LITERAL_STRING("rdf:")) && mDB) {
    ...
    nsresult rv =
        gRDFService->GetUnicodeResource(sortkey, getter_AddRefs(predicate));
    NS_ENSURE_SUCCESS(rv, rv);
    ...
    rv =
        gRDFService->GetUnicodeResource(sortkey, getter_AddRefs(sortPredicate));
    NS_ENSURE_SUCCESS(rv, rv);

    rv =
        GetSortValue(aLeft, predicate, sortPredicate, getter_AddRefs(leftNode));
    NS_ENSURE_SUCCESS(rv, rv);

    rv = GetSortValue(
        aRight, predicate, sortPredicate, getter_AddRefs(rightNode));
    NS_ENSURE_SUCCESS(rv, rv);
  } else {
    ...
    if (l) {
      ...
      if (r) {
        ...
        *aResult = XULSortServiceImpl::CompareValues(
            nsDependentString(lstr), nsDependentString(rstr), aSortHints);
      }
...
NS_IMETHODIMP
nsXULTemplateQueryProcessorRDF::OnBeginUpdateBatch(
  ...
  if (MOZ_LOG_TEST(gXULTemplateLog, LogLevel::Debug)) {
    ...
    MOZ_LOG(
        ...
        ("nsXULTemplateQueryProcessorRDF::Propagate: [%s] -> [%s] -> [%s]\n",
         sourceStr,
         propertyStr,
         NS_ConvertUTF16toUTF8(targetStr).get()));
  }
                                        ...
                                        nsIRDFNode* aTarget)
{
  ...
  for (ReteNodeSet::ConstIterator node = mRDFTests.First(); node != lastnode;
       ++node) {
                                               ...
                                               nsIRDFNode* aNewTarget)
{
  ...
  ResultArray* results;
  if (!mBindingDependencies.Get(aSource, &results) || !mBuilder) return NS_OK;
                                           ...
                                           bool* aIsEmpty)
{
  ...
  *aIsEmpty = true;
  ...
  if (*aIsEmpty) {
    return nsXULTemplateQueryProcessorRDF::gRDFContainerUtils->IsEmpty(
        mDB, aResource, aIsEmpty);
  }
...
{
  NS_ENSURE_STATE(mDB);
  return mDB->HasAssertion(
      aResource, kRDF_type, kNC_BookmarkSeparator, true, aIsSeparator);
}
    ...
    nsIDOMNode* aRootNode)
{
  ...
  uint32_t offset = 0;
  while (offset < len) {
    while (offset < len && nsCRT::IsAsciiSpace(containment[offset])) ++offset;

    if (offset >= len) break;

    uint32_t end = offset;
    while (end < len && !nsCRT::IsAsciiSpace(containment[end])) ++end;
                                                     ...
                                                     TestNode** aLastNode)
{
  ...
  nsContentTestNode* idnode = new nsContentTestNode(this, aQuery->mRefVariable);
  ...
  for (nsIContent* condition = aConditions->GetFirstChild(); condition;
    ...
    if (condition->IsXULElement(nsGkAtoms::content)) {
      ...
      nsCOMPtr<nsIDOMDocument> doc =
          do_QueryInterface(condition->GetComposedDoc());
                                             ...
                                             nsIRDFNode** aResult)
{
  ...
  *aResult = nullptr;
  ...
  if (aParseType.EqualsLiteral(PARSE_TYPE_INTEGER)) {
    ...
    if (NS_FAILED(errorCode)) return NS_ERROR_FAILURE;
                                                       ...
                                                       TestNode** aResult)
{
  ...
  TestNode* testnode =
      new nsRDFConMemberTestNode(aParentNode, this, containervar, childvar);
                                                      ...
                                                      TestNode** aChildNode)
{
  ...
  nsRDFConMemberTestNode* membernode = new nsRDFConMemberTestNode(
      idnode, this, aQuery->mRefVariable, aQuery->mMemberVariable);
                                                   ...
                                                   TestNode** aLastNode)
{
  ...
  for (uint32_t i = 0; (name = aQueryElement->GetAttrNameAt(i)); ++i) {
    ...
    if (attrNameSpaceID == kNameSpaceID_XMLNS) continue;
    ...
    if (name->Equals(nsGkAtoms::iscontainer, kNameSpaceID_None) ||
      ...
      switch (aQueryElement->FindAttrValueIn(
          kNameSpaceID_None, nsGkAtoms::iscontainer, strings, eCaseMatters)) {
        case 0:
      ...
      switch (aQueryElement->FindAttrValueIn(
          kNameSpaceID_None, nsGkAtoms::isempty, strings, eCaseMatters)) {
        case 0:
      ...
      testnode = new nsRDFConInstanceTestNode(
          prevnode, this, aQuery->mMemberVariable, iscontainer, isempty);
      ...
      if (NS_FAILED(rv)) return rv;
               ...
               attr != nsGkAtoms::parent) {
      // It's a simple RDF test
      nsCOMPtr<nsIRDFResource> property;
      rv = nsXULContentUtils::GetResource(
          attrNameSpaceID, attr, getter_AddRefs(property));
      if (NS_FAILED(rv)) return rv;
      ...
      if (value.FindChar(':') != -1) {  // XXXwaterson WRONG WRONG WRONG!
        ...
        nsAutoString parseType;
        aQueryElement->GetAttr(
            kNameSpaceID_None, nsGkAtoms::parsetype, parseType);
        rv = ParseLiteral(parseType, value, getter_AddRefs(target));
        if (NS_FAILED(rv)) return rv;
      }

      testnode = new nsRDFPropertyTestNode(
          prevnode, this, aQuery->mMemberVariable, property, target);
      rv = mAllTests.Add(testnode);
      if (NS_FAILED(rv)) {
        delete testnode;
        return rv;
...
void
nsXULTemplateQueryProcessorRDF::AddBindingDependency(
    nsXULTemplateResultRDF* aResult, nsIRDFResource* aResource)
{
  ResultArray* arr;
  ...
  if (index == -1) arr->AppendElement(aResult);
}

void
nsXULTemplateQueryProcessorRDF::RemoveBindingDependency(
    nsXULTemplateResultRDF* aResult, nsIRDFResource* aResource)
{
  ResultArray* arr;
  if (mBindingDependencies.Get(aResource, &arr)) {
    int32_t index = arr->IndexOf(aResult);
    if (index >= 0) arr->RemoveElementAt(index);
  }
}

nsresult
nsXULTemplateQueryProcessorRDF::AddMemoryElements(
    const Instantiation& aInst, nsXULTemplateResultRDF* aResult)
{
...
nsresult
nsXULTemplateQueryProcessorRDF::RemoveMemoryElements(
    const Instantiation& aInst, nsXULTemplateResultRDF* aResult)
{
  ...
  for (MemoryElementSet::ConstIterator element = aInst.mSupport.First();
    ...
    if (mMemoryElementToResultMap.Get(hash, &arr)) {
      int32_t index = arr->IndexOf(aResult);
      if (index >= 0) arr->RemoveObjectAt(index);

      uint32_t length = arr->Count();
      if (!length) mMemoryElementToResultMap.Remove(hash);
    }
  }
...
void
nsXULTemplateQueryProcessorRDF::RetractElement(
    const MemoryElement& aMemoryElement)
{
  if (!mBuilder) return;
    ...
    for (int32_t r = length - 1; r >= 0; r--) {
      ...
      if (result) {
        ...
        if (result->HasMemoryElement(aMemoryElement)) {
          ...
          if (!mMemoryElementToResultMap.Get(hash, nullptr)) return;

          // the array should have been reduced by one, but check
          // just to make sure
          uint32_t newlength = arr->Count();
          if (r > (int32_t)newlength) r = newlength;
        }
      }
    }

    // if there are no items left, remove the memory element from the hashtable
    if (!arr->Count()) mMemoryElementToResultMap.Remove(hash);
  }
}

int32_t
nsXULTemplateQueryProcessorRDF::GetContainerIndexOf(
                                             ...
                                             nsISupports** aResultNode)
{
  ...
  if (source && mDB) {
    ...
    rv = mDB->GetTarget(source, aSortPredicate, true, getter_AddRefs(value));
    if (NS_FAILED(rv)) return rv;

    if (!value) {
      rv = mDB->GetTarget(source, aPredicate, true, getter_AddRefs(value));
      if (NS_FAILED(rv)) return rv;
    }
  }
