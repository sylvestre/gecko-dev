...
NS_IMPL_CYCLE_COLLECTION_INHERITED(nsXULTreeBuilder,
...
NS_IMETHODIMP
nsXULTreeBuilder::GetResourceAtIndex(int32_t aRowIndex,
...
{
  if (aElement.AttrValueIs(kNameSpaceID_None,
  ...
  if (mBoxObject) mBoxObject->Invalidate();
...
void
nsXULTreeBuilder::GetRowProperties(int32_t aRow,
...
void
nsXULTreeBuilder::GetCellProperties(int32_t aRow,
...
nsXULTreeBuilder::IsContainer(int32_t aRow, ErrorResult& aError)
{
  ...
  iter->mContainerType = isContainer ? nsTreeRows::eContainerType_Container
...
nsXULTreeBuilder::IsContainerOpen(int32_t aRow, ErrorResult& aError)
{
  ...
  if (iter->mContainerState == nsTreeRows::eContainerState_Unknown) {
    ...
    iter->mContainerState = isOpen ? nsTreeRows::eContainerState_Open
...
nsXULTreeBuilder::IsContainerEmpty(int32_t aRow, ErrorResult& aError)
{
  ...
  if (iter->mContainerFill == nsTreeRows::eContainerFill_Unknown) {
    ...
    iter->mContainerFill = isEmpty ? nsTreeRows::eContainerFill_Empty
...
nsXULTreeBuilder::GetParentIndex(int32_t aRow, ErrorResult& aError)
{
  ...
  while (--index >= 0) aRow -= mRows.GetSubtreeSizeFor(parent, index) + 1;
...
bool
nsXULTreeBuilder::HasNextSibling(int32_t aRow,
                                 int32_t aAfterIndex,
                                 ErrorResult& aError)
{
  if (!IsValidRowIndex(aRow)) {
...
NS_IMETHODIMP
nsXULTreeBuilder::HasNextSibling(int32_t aRowIndex,
                                 int32_t aAfterIndex,
                                 bool* aResult)
{
  ErrorResult rv;
...
void
nsXULTreeBuilder::GetImageSrc(int32_t aRow,
                              nsTreeColumn& aColumn,
                              nsAString& aSrc,
                              ErrorResult& aError)
{
...
int32_t
nsXULTreeBuilder::GetProgressMode(int32_t aRow,
                                  nsTreeColumn& aColumn,
                                  ErrorResult& aError)
{
  if (!IsValidRowIndex(aRow)) {
...
void
nsXULTreeBuilder::GetCellValue(int32_t aRow,
                               nsTreeColumn& aColumn,
                               nsAString& aValue,
                               ErrorResult& aError)
{
...
void
nsXULTreeBuilder::GetCellText(int32_t aRow,
                              nsTreeColumn& aColumn,
                              nsAString& aText,
                              ErrorResult& aError)
{
...
nsXULTreeBuilder::SetTree(nsITreeBoxObject* aTree)
{
  ...
  EnsureSortVariables();
  if (mSortVariable) SortSubtree(mRows.GetRoot());
...
nsXULTreeBuilder::ToggleOpenState(int32_t aRow, ErrorResult& aError)
{
  ...
  for (uint32_t i = 0; i < count; ++i) {
    ...
    if (observer) observer->OnToggleOpenState(aRow);
  }
  ...
  if (mLocalStore && mRoot) {
    ...
    if (isOpen) {
      ...
      mLocalStore->SetValue(
          uri, nodeid, NS_LITERAL_STRING("open"), NS_LITERAL_STRING("true"));
...
nsXULTreeBuilder::CycleHeader(nsTreeColumn& aColumn, ErrorResult& aError)
{
  ...
  for (uint32_t i = 0; i < count; ++i) {
    nsCOMPtr<nsIXULTreeBuilderObserver> observer = mObservers.SafeElementAt(i);
    if (observer) observer->OnCycleHeader(id.get(), element);
  }
...
nsXULTreeBuilder::SelectionChanged()
{
  ...
  for (uint32_t i = 0; i < count; ++i) {
    nsCOMPtr<nsIXULTreeBuilderObserver> observer = mObservers.SafeElementAt(i);
    if (observer) observer->OnSelectionChanged();
  }
...
nsXULTreeBuilder::CycleCell(int32_t aRow, nsTreeColumn& aColumn)
{
  ...
  for (uint32_t i = 0; i < count; ++i) {
    nsCOMPtr<nsIXULTreeBuilderObserver> observer = mObservers.SafeElementAt(i);
    if (observer) observer->OnCycleCell(aRow, id.get());
  }
}
...
bool
nsXULTreeBuilder::IsEditable(int32_t aRow,
                             nsTreeColumn& aColumn,
                             ErrorResult& aError)
{
  if (!IsValidRowIndex(aRow)) {
...
bool
nsXULTreeBuilder::IsSelectable(int32_t aRow,
                               nsTreeColumn& aColumn,
                               ErrorResult& aError)
{
  if (!IsValidRowIndex(aRow)) {
...
NS_IMETHODIMP
nsXULTreeBuilder::SetCellValue(int32_t aRow,
                               nsITreeColumn* aCol,
...
NS_IMETHODIMP
nsXULTreeBuilder::SetCellText(int32_t aRow,
                              nsITreeColumn* aCol,
                              const nsAString& aValue)
{
  NS_ENSURE_ARG_POINTER(aCol);
...
{
  PerformActionOnRow(aAction ? nsDependentString(aAction) : EmptyString(),
                     aRow);
...
void
nsXULTreeBuilder::PerformActionOnCell(const nsAString& aAction,
                                      int32_t aRow,
                                      nsTreeColumn& aColumn)
{
  nsAutoString id;
  ...
  for (uint32_t i = 0; i < count; ++i) {
    nsCOMPtr<nsIXULTreeBuilderObserver> observer = mObservers.SafeElementAt(i);
    if (observer) {
      observer->OnPerformActionOnCell(
          PromiseFlatString(aAction).get(), aRow, id.get());
    }
  }
}

NS_IMETHODIMP
nsXULTreeBuilder::PerformActionOnCell(const char16_t* aAction,
                                      int32_t aRow,
                                      nsITreeColumn* aCol)
{
  RefPtr<nsTreeColumn> col = nsTreeColumn::From(aCol);
  ...
  PerformActionOnCell(
      aAction ? nsDependentString(aAction) : EmptyString(), aRow, *col);
                                        ...
                                        nsCOMArray<nsIContent>** aLocations)
{
  *aLocations = nullptr;
  ...
  if (NS_FAILED(rv) || ref.IsEmpty()) return false;
  ...
  if (container == mRows.GetRootResource()) return true;

  nsTreeRows::iterator iter = mRows.FindByResource(container);
  if (iter == mRows.Last()) return false;
...
struct ResultComparator
{
  ...
  ResultComparator(nsXULTreeBuilder* aTreebuilder,
                               ...
                               void* aLocation)
{
  ...
  if (aOldResult) {
    ...
    if (iter == mRows.Last()) return NS_ERROR_FAILURE;
    ...
    if (delta) RemoveMatchesFor(*(iter->mSubtree));
  ...
  if (aNewMatch && aNewMatch->mResult) {
    ...
    if (container != mRows.GetRootResource()) {
      ...
      if (open) parent = mRows.EnsureSubtreeFor(iter);
    ...
    if (parent) {
      ...
      if (mSortVariable) {
        ...
        // binary search.
        mozilla::BinarySearchIf(*parent,
                                0,
                                parent->Count(),
                                ResultComparator(this, result),
                                &index);
      }

      nsTreeRows::iterator iter = mRows.InsertRowAt(aNewMatch, parent, index);
...
nsXULTreeBuilder::SynchronizeResult(nsIXULTemplateResult* aResult)
{
  if (mBoxObject) {
    ...
    int32_t row = iter.GetRowIndex();
    if (row >= 0) mBoxObject->InvalidateRow(row);
...
nsXULTreeBuilder::EnsureSortVariables()
{
  ...
  nsXULContentUtils::FindChildByTag(
      mRoot, kNameSpaceID_XUL, nsGkAtoms::treecols, getter_AddRefs(treecols));

  if (!treecols) return NS_OK;

  for (nsIContent* child = treecols->GetFirstChild(); child;
       child = child->GetNextSibling()) {
    if (child->NodeInfo()->Equals(nsGkAtoms::treecol, kNameSpaceID_XUL)) {
      if (child->AttrValueIs(kNameSpaceID_None,
...
nsXULTreeBuilder::RebuildAll()
{
  ...
  if (NS_SUCCEEDED(rv) && mQuerySets.Length() > 0) {
    ...
    if (!ref.IsEmpty()) {
      rv = mQueryProcessor->TranslateRef(
...
nsXULTreeBuilder::GetTemplateActionRowFor(int32_t aRow, nsIContent** aResult)
{
  ...
  if (ruleindex >= 0) {
    ...
    if (rule) {
      ...
      nsXULContentUtils::FindChildByTag(rule->GetAction(),
                                        kNameSpaceID_XUL,
                                        nsGkAtoms::treechildren,
                                        getter_AddRefs(children));
      if (children) {
        nsCOMPtr<nsIContent> item;
        nsXULContentUtils::FindChildByTag(children,
                                          kNameSpaceID_XUL,
                                          nsGkAtoms::treeitem,
                                          getter_AddRefs(item));
        if (item)
          return nsXULContentUtils::FindChildByTag(
              item, kNameSpaceID_XUL, nsGkAtoms::treerow, aResult);
      }
    }
  }
...
nsXULTreeBuilder::GetTemplateActionCellFor(int32_t aRow, nsTreeColumn& aCol)
{
  ...
  uint32_t j = 0;
  for (nsIContent* child = row->GetFirstChild(); child;
       child = child->GetNextSibling()) {
    if (child->NodeInfo()->Equals(nsGkAtoms::treecell, kNameSpaceID_XUL)) {
      if (colAtom &&
          child->AttrValueIs(
              kNameSpaceID_None, nsGkAtoms::ref, colAtom, eCaseMatters)) {
        return child;
      }
...
nsXULTreeBuilder::OpenContainer(int32_t aIndex, nsIXULTemplateResult* aResult)
{
  ...
  if (aIndex < -1 || aIndex >= mRows.Count()) return NS_ERROR_INVALID_ARG;

  nsTreeRows::Subtree* container;

  if (aIndex >= 0) {
    nsTreeRows::iterator iter = mRows[aIndex];
    container = mRows.EnsureSubtreeFor(iter.GetParent(), iter.GetChildIndex());
  ...
  // Notify the box object
  if (mBoxObject) {
    if (aIndex >= 0) mBoxObject->InvalidateRow(aIndex);

    if (count) mBoxObject->RowCountChanged(aIndex + 1, count);
  }
                                ...
                                int32_t* aDelta)
{
  ...
  for (int32_t i = open.Length() - 1; i >= 0; --i) {
    ...
    nsTreeRows::Subtree* child = mRows.EnsureSubtreeFor(aSubtree, index);
                                         ...
                                         nsTArray<int32_t>& open)
{
  ...
  for (; NS_SUCCEEDED(rv) && hasMoreResults;
    ...
    if (generateContent) {
      ...
      if (aIndex >= 0) {
        for (nsTreeRows::iterator iter = mRows[aIndex]; iter.GetDepth() > 0;
             iter.Pop()) {
          nsCOMPtr<nsIRDFResource> parentid;
          rv = GetResultResource(iter->mMatch->mResult,
      ...
      rv = DetermineMatchedRule(
          nullptr, nextresult, aQuerySet, &matchedrule, &ruleindex);
      if (NS_FAILED(rv)) {
        nsTemplateMatch::Destroy(newmatch, false);
        return rv;
      }
...
nsXULTreeBuilder::CloseContainer(int32_t aIndex)
{
  ...
  if (iter->mSubtree) RemoveMatchesFor(*iter->mSubtree);
...
nsXULTreeBuilder::Compare(const void* aLeft, const void* aRight, void* aClosure)
{
  ...
  nsTreeRows::Row* left =
      static_cast<nsTreeRows::Row*>(const_cast<void*>(aLeft));

  nsTreeRows::Row* right =
      static_cast<nsTreeRows::Row*>(const_cast<void*>(aRight));

  return self->CompareResults(left->mMatch->mResult, right->mMatch->mResult);
}

int32_t
nsXULTreeBuilder::CompareResults(nsIXULTemplateResult* aLeft,
                                 nsIXULTemplateResult* aRight)
{
  ...
  int32_t sortorder;
  if (!mQueryProcessor) return 0;

  mQueryProcessor->CompareResults(
      aLeft, aRight, mSortVariable, mSortHints, &sortorder);

  if (sortorder) sortorder = sortorder * mSortDirection;
  return sortorder;
}
...
bool
nsXULTreeBuilder::CanDrop(int32_t aRow,
...
NS_IMETHODIMP
nsXULTreeBuilder::CanDrop(int32_t index,
                          int32_t orientation,
                          nsIDOMDataTransfer* dataTransfer,
                          bool* _retval)
{
  ErrorResult rv;
  *_retval = CanDrop(index, orientation, DataTransfer::Cast(dataTransfer), rv);
  return rv.StealNSResult();
}

void
nsXULTreeBuilder::Drop(int32_t aRow,
                       int32_t aOrientation,
                       DataTransfer* aDataTransfer,
                       ErrorResult& aError)
{
  ...
  for (uint32_t i = 0; i < count; ++i) {
    ...
    if (observer) {
      bool canDrop = false;
      observer->CanDrop(aRow, aOrientation, aDataTransfer, &canDrop);
      if (canDrop) observer->OnDrop(aRow, aOrientation, aDataTransfer);
    }
  }
}

NS_IMETHODIMP
nsXULTreeBuilder::Drop(int32_t row,
                       int32_t orient,
                       nsIDOMDataTransfer* dataTransfer)
{
  ErrorResult rv;
