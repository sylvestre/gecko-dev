                                       ...
                                       const nsAString& aRightString)
{
  ...
  if (aRightString.IsEmpty()) {
    if ((mRelation == eEquals) && aLeftString.IsEmpty()) match = true;
  } else {
    switch (mRelation) {
      case eEquals:
        if (mIgnoreCase)
      ...
      case eBefore: {
        ...
        if (collation) {
          ...
          collation->CompareString(
              (mIgnoreCase ? static_cast<int32_t>(
      ...
      case eStartswith:
        if (mIgnoreCase)
          match = (StringBeginsWith(
              aLeftString, aRightString, nsCaseInsensitiveStringComparator()));
        else
...
void
nsTemplateRule::SetCondition(nsTemplateCondition* aCondition)
{
  while (mConditions) {
    nsTemplateCondition* cdel = mConditions;
    mConditions = mConditions->GetNext();
...
nsTemplateRule::CheckMatch(nsIXULTemplateResult* aResult) const
{
  ...
  while (condition) {
    if (!condition->CheckMatch(aResult)) return false;
...
{
  for (Binding* binding = mBindings; binding != nullptr;
                           ...
                           nsAtom* aTargetVariable)
{
  ...
  if (!aSourceVariable) return NS_ERROR_INVALID_ARG;

  NS_PRECONDITION(aTargetVariable != 0, "no target variable!");
  if (!aTargetVariable) return NS_ERROR_INVALID_ARG;

  NS_ASSERTION(!HasBinding(aSourceVariable, aExpr, aTargetVariable),
               "binding added twice");

  Binding* newbinding = new Binding;
  if (!newbinding) return NS_ERROR_OUT_OF_MEMORY;
...
nsresult
nsTemplateRule::AddBindingsToQueryProcessor(
    nsIXULTemplateQueryProcessor* aProcessor)
{
  Binding* binding = mBindings;

  while (binding) {
    nsresult rv = aProcessor->AddBinding(mRuleNode,
                                         binding->mTargetVariable,
                                         binding->mSourceVariable,
                                         binding->mExpr);
