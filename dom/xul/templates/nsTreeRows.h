...
class nsTreeRows
{
 public:
  ...
  enum Direction
  {
    eDirection_Forwards = +1,
    eDirection_Backwards = -1
  };
 ...
 protected:
  ...
  struct Link
  {
    ...
    {
      return (mParent == aLink.mParent) && (mChildIndex == aLink.mChildIndex);
    }
 ...
 public:
  ...
  /**
     ...
     */
  Subtree* EnsureSubtreeFor(Subtree* aParent, int32_t aChildIndex);

  /**
     * Ensure that a child subtree exists at the iterator's position.
     */
  Subtree* EnsureSubtreeFor(iterator& aIterator)
  {
    return EnsureSubtreeFor(aIterator.GetParent(), aIterator.GetChildIndex());
  }
  ...
  /**
     ...
     */
  int32_t GetSubtreeSizeFor(const Subtree* aParent, int32_t aChildIndex)
  {
    int32_t size;
  ...
  /**
     * Retrieve the size of the subtree within the specified parent.
     */
  int32_t GetSubtreeSizeFor(const iterator& aIterator)
  {
    int32_t size;
  ...
  /**
     ...
     * intact.
     */
  void RemoveSubtreeFor(Subtree* aParent, int32_t aChildIndex);
  ...
  /**
     ...
     */
  int32_t RemoveRowAt(iterator& aIterator)
  {
  ...
  /**
     ...
     */
  iterator InsertRowAt(nsTemplateMatch* aMatch,
                       Subtree* aSubtree,
                       int32_t aChildIndex)
  {
    InvalidateCachedRow();
  ...
  /**
     ...
     */
  Row* GetRowsFor(Subtree* aSubtree) { return aSubtree->mRows; }
  ...
  /**
     ...
     */
  void SetRootResource(nsIRDFResource* aResource) { mRootResource = aResource; }

  /**
     * Retrieve the root resource for the view
     */
  nsIRDFResource* GetRootResource() { return mRootResource.get(); }

  /**
     * Invalidate the cached row; e.g., because the view has changed
     * in a way that would corrupt the iterator.
     */
  void InvalidateCachedRow() { mLastRow = iterator(); }
