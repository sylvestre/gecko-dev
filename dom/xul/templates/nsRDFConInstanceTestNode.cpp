...
{
  switch (aTest) {
    case nsRDFConInstanceTestNode::eFalse:
      return "false";
    case nsRDFConInstanceTestNode::eTrue:
      return "true";
    case nsRDFConInstanceTestNode::eDontCare:
      return "dontcare";
  }
  return "?";
}

nsRDFConInstanceTestNode::nsRDFConInstanceTestNode(
    TestNode* aParent,
      ...
      mEmpty(aEmpty)
{
  if (MOZ_LOG_TEST(gXULTemplateLog, LogLevel::Debug)) {
    ...
    MOZ_LOG(gXULTemplateLog,
            LogLevel::Debug,
            ("nsRDFConInstanceTestNode[%p]: parent=%p member-props=(%s) "
             "container-var=%s container=%s empty=%s",
             this,
             aParent,
             props.get(),
...
nsRDFConInstanceTestNode::FilterInstantiations(
    InstantiationSet& aInstantiations, bool* aCantHandleYet) const
{
  nsresult rv;
  ...
  for (InstantiationSet::Iterator inst = aInstantiations.First(); inst != last;
    ...
    if (!inst->mAssignments.GetAssignmentFor(mContainerVariable,
    ...
    if (MOZ_LOG_TEST(gXULTemplateLog, LogLevel::Debug)) {
      ...
      MOZ_LOG(gXULTemplateLog,
              LogLevel::Debug,
              ("nsRDFConInstanceTestNode[%p]::FilterInstantiations() "
               "container=[%s]",
               this,
               container));
    }
