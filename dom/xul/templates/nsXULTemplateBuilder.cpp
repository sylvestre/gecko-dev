...
#define NS_QUERY_PROCESSOR_CONTRACTID_PREFIX \
  "@mozilla.org/xul/xul-query-processor;1?name="
...
nsXULTemplateBuilder::CleanUp(bool aIsFinal)
{
  ...
  if (aIsFinal) mQueryProcessor = nullptr;
}
...
nsXULTemplateBuilder::Uninit(bool aIsFinal)
{
  ...
  if (mQueryProcessor) mQueryProcessor->Done();
...
NS_IMETHODIMP
nsXULTemplateBuilder::AddRuleFilter(nsIDOMNode* aRule,
                                    nsIXULTemplateRuleFilter* aFilter)
{
                                   ...
                                   nsINode* aQueryNode)
{
  ...
  if (!mRoot || !mQueriesCompiled) return NS_OK;
  ...
  if (aOldResult) {
    ...
    // activation stack.
    if (IsActivated(oldId)) return NS_OK;
  }
  ...
  if (aNewResult) {
    ...
    if (!newId) return NS_OK;

    // Ignore re-entrant builds for content that is currently in our
    // activation stack.
    if (IsActivated(newId)) return NS_OK;
  ...
  if (insertionPoints) {
    ...
    for (uint32_t t = 0; t < count; t++) {
      ...
      if (insertionPoint) {
        rv = UpdateResultInContainer(
            aOldResult, aNewResult, queryset, oldId, newId, insertionPoint);
        if (NS_FAILED(rv)) return rv;
      }
    }
  } else {
    ...
    rv = UpdateResultInContainer(
        aOldResult, aNewResult, queryset, oldId, newId, nullptr);
  }
                                              ...
                                              nsIContent* aInsertionPoint)
{
  ...
  if (aOldResult) {
    ...
    if (mMatchMap.Get(aOldId, &firstmatch)) {
      ...
      if (oldmatch) {
        ...
        if (oldmatch->IsActive()) {
          ...
          while (findmatch) {
            ...
            if (findmatch->GetContainer() == aInsertionPoint) {
              ...
              if (matchedrule) {
                rv = findmatch->RuleMatched(
                    qs, matchedrule, ruleindex, findmatch->mResult);
                if (NS_FAILED(rv)) return rv;
        ...
        if (prevmatch) prevmatch->mNext = nextmatch;
  ...
  if (aNewResult) {
    ...
    newmatch =
        nsTemplateMatch::Create(findpriority, aNewResult, aInsertionPoint);
    ...
    if (mMatchMap.Get(aNewId, &firstmatch)) {
      ...
      while (oldmatch) {
        ...
        if (oldmatch->GetContainer() == aInsertionPoint) {
          if (priority == findpriority) break;

          // If a match with a lower priority is active, the new
          // match can't replace it.
          if (oldmatch->IsActive()) hasEarlierActiveMatch = true;
        }
      ...
      if (!hasEarlierActiveMatch) {
        ...
        if (oldmatch) {
          if (oldmatch->IsActive()) replacedmatch = oldmatch;
          replacedmatchtodelete = oldmatch;
        }

        // check if the new result matches the rules
        rv = DetermineMatchedRule(aInsertionPoint,
                                  newmatch->mResult,
                                  aQuerySet,
                                  &matchedrule,
                                  &ruleindex);
        ...
        if (matchedrule) {
          rv = newmatch->RuleMatched(
              aQuerySet, matchedrule, ruleindex, newmatch->mResult);
          if (NS_FAILED(rv)) {
            nsTemplateMatch::Destroy(newmatch, false);
            return rv;
          }
      ...
      if (matchedrule) {
        rv = newmatch->RuleMatched(
            aQuerySet, matchedrule, ruleindex, aNewResult);
        if (NS_FAILED(rv)) {
          nsTemplateMatch::Destroy(newmatch, false);
          return rv;
        }
  ...
  if (replacedmatch) {
    rv =
        ReplaceMatch(replacedmatch->mResult, nullptr, nullptr, aInsertionPoint);
  ...
  if (removedmatch) nsTemplateMatch::Destroy(removedmatch, true);
...
{
  CallbackObjectHolder<XULBuilderListener, nsIXULBuilderListener> holder(
      &aListener);
...
nsXULTemplateBuilder::AddListener(nsIXULBuilderListener* aListener)
{
  ...
  if (!mListeners.AppendElement(aListener)) return NS_ERROR_OUT_OF_MEMORY;
                              ...
                              const char16_t* aData)
{
  ...
  if (!strcmp(aTopic, DOM_WINDOW_DESTROYED_TOPIC)) {
    if (nsCOMPtr<mozIDOMWindow> window = do_QueryInterface(aSubject)) {
      ...
      if (doc && doc == mObservedDocument) NodeWillBeDestroyed(doc);
    }
  }
  return NS_OK;
}
                                     ...
                                     nsIContent* aPreviousSibling)
{
  if (mRoot && nsContentUtils::ContentIsDescendantOf(mRoot, aChild)) {
    ...
    if (xuldoc) xuldoc->SetTemplateBuilderFor(mRoot, nullptr);
                                      ...
                                      bool* aShouldDelayBuilding)
{
  ...
  *aShouldDelayBuilding = false;
  ...
  if (querytype.IsEmpty()) querytype.AssignLiteral("rdf");
  ...
  rv = LoadDataSourceUrls(
      aDocument, datasources, isRDFQuery, aShouldDelayBuilding);
  NS_ENSURE_SUCCESS(rv, rv);

  // Now set the database on the element, so that script writers can
  // access it.
  nsCOMPtr<nsIXULDocument> xuldoc = do_QueryInterface(aDocument);
  if (xuldoc) xuldoc->SetTemplateBuilderFor(mRoot, this);
                                         ...
                                         bool* aShouldDelayBuilding)
{
  ...
  if (!uriList) return NS_ERROR_FAILURE;

  nsresult rv;
  nsAutoString datasources(aDataSources);
  uint32_t first = 0;
  while (1) {
    while (first < datasources.Length() &&
           nsCRT::IsAsciiSpace(datasources.CharAt(first)))
      ++first;

    if (first >= datasources.Length()) break;

    uint32_t last = first;
    while (last < datasources.Length() &&
           !nsCRT::IsAsciiSpace(datasources.CharAt(last)))
      ++last;
    ...
    if (uriStr.EqualsLiteral("rdf:null")) continue;
    ...
    if (uriStr.CharAt(0) == '#') {
      ...
      domdoc->GetElementById(Substring(uriStr, 1), getter_AddRefs(dsnode));

      if (dsnode) uriList->AppendElement(dsnode, false);
      continue;
    }
    ...
    if (NS_FAILED(rv) || !uri) continue;  // Necko will barf if our URI is weird
  ...
  if (aIsRDFQuery && mDataSource) {
    ...
    if (inferDB) {
      ...
      if (ds) mCompDB = do_QueryInterface(ds);
    }

    if (!mCompDB) mCompDB = do_QueryInterface(mDataSource);
...
nsXULTemplateBuilder::InitHTMLTemplateRoot()
{
  ...
  if (!global) return NS_ERROR_UNEXPECTED;

  nsCOMPtr<nsIGlobalObject> innerWin = do_QueryInterface(doc->GetInnerWindow());
  ...
  if (mDB) {
    ...
    rv = nsContentUtils::WrapNative(
        jscontext, mDB, &NS_GET_IID(nsIRDFCompositeDataSource), &jsdatabase);
    NS_ENSURE_SUCCESS(rv, rv);
                                           ...
                                           int16_t* aRuleIndex)
{
  ...
  for (int16_t r = 0; r < count; r++) {
    ...
    if ((!aContainer || !tag || tag == aContainer->NodeInfo()->NameAtom()) &&
    ...
    void* aClosure)
{
  ...
  for (; iter != done_parsing; backup = ++iter) {
    ...
    while (iter != done_parsing) {
      c = *iter;
      if ((c == char16_t(' ')) || (c == char16_t('^'))) break;
...
{
  NS_PRECONDITION(mRoot != nullptr, "not initialized");
  if (!mRoot) return NS_ERROR_NOT_INITIALIZED;
  ...
  if (!templateID.IsEmpty()) {
    nsCOMPtr<nsIDOMDocument> domDoc =
        do_QueryInterface(mRoot->GetComposedDoc());
    ...
    if (domElement) {
      ...
      NS_ENSURE_STATE(content &&
                      !nsContentUtils::ContentIsDescendantOf(mRoot, content));
  ...
  for (nsIContent* child = mRoot->GetFirstChild(); child;
...
nsXULTemplateBuilder::CompileQueries()
{
  ...
  if (MOZ_LOG_TEST(gXULTemplateLog, LogLevel::Debug)) mFlags |= eLoggingEnabled;
                                      ...
                                      bool* aCanUseTemplate)
{
  ...
  for (nsIContent* rulenode = aTemplate->GetFirstChild(); rulenode;
    ...
    if (!aIsQuerySet && ni->Equals(nsGkAtoms::queryset, kNameSpaceID_XUL)) {
      ...
      rv = CompileTemplate(
          rulenode, aQuerySet, true, aPriority, aCanUseTemplate);
      if (NS_FAILED(rv)) return rv;
    }

    // once a queryset is used, everything must be a queryset
    if (isQuerySetMode) continue;
    ...
    if (ni->Equals(nsGkAtoms::rule, kNameSpaceID_XUL)) {
      ...
      if (action) {
        ...
        if (hasQuery) {
          ...
          if (tag) aQuerySet->SetTag(tag);

          if (!aQuerySet->mCompiledQuery) {
            nsCOMPtr<nsIDOMNode> query(
                do_QueryInterface(aQuerySet->mQueryNode));

            rv = mQueryProcessor->CompileQuery(
                this,
                query,
                mRefVariable,
                memberVariable,
          ...
          if (aQuerySet->mCompiledQuery) {
            rv = CompileExtendedQuery(
                rulenode, action, memberVariable, aQuerySet);
            if (NS_FAILED(rv)) return rv;
      ...
      if (aQuerySet->mCompiledQuery) {
        nsTemplateRule* rule =
            aQuerySet->NewRule(aTemplate, rulenode, aQuerySet);
        if (!rule) return NS_ERROR_OUT_OF_MEMORY;
                                           ...
                                           nsTemplateQuerySet* aQuerySet)
{
  ...
  nsTemplateRule* rule =
      aQuerySet->NewRule(aRuleElement, aActionElement, aQuerySet);
  if (!rule) return NS_ERROR_OUT_OF_MEMORY;
  ...
  if (!conditions) conditions = aRuleElement;
  ...
  if (!bindings) bindings = aRuleElement;
...
void
nsXULTemplateBuilder::DetermineRDFQueryRef(nsIContent* aQueryElement,
                                           nsAtom** aTag)
{
  // check for a tag
  ...
  if (content) {
    ...
    if (!uri.IsEmpty()) mRefVariable = NS_Atomize(uri);

    nsAutoString tag;
    content->GetAttr(kNameSpaceID_None, nsGkAtoms::tag, tag);

    if (!tag.IsEmpty()) *aTag = NS_Atomize(tag).take();
  }
}
                                         ...
                                         bool* aCanUseTemplate)
{
  ...
  nsresult rv =
      mQueryProcessor->CompileQuery(this,
                                    query,
                                    mRefVariable,
                                    memberVariable,
                                    getter_AddRefs(aQuerySet->mCompiledQuery));
  ...
  nsTemplateRule* rule =
      aQuerySet->NewRule(aRuleElement, aRuleElement, aQuerySet);
  if (!rule) return NS_ERROR_OUT_OF_MEMORY;
                                        ...
                                        nsIContent* aCondition)
{
  ...
  for (nsIContent* node = aCondition->GetFirstChild(); node;
...
nsresult
nsXULTemplateBuilder::CompileWhereCondition(
    ...
    nsTemplateCondition** aCurrentCondition)
{
  ...
  RefPtr<nsAtom> svar;
  if (subject[0] == char16_t('?')) svar = NS_Atomize(subject);
  ...
  // multiple
  bool shouldMultiple = aCondition->AttrValueIs(
      kNameSpaceID_None, nsGkAtoms::multiple, nsGkAtoms::_true, eCaseMatters);
  ...
  // ignorecase
  bool shouldIgnoreCase = aCondition->AttrValueIs(
      kNameSpaceID_None, nsGkAtoms::ignorecase, nsGkAtoms::_true, eCaseMatters);

  // negate
  bool shouldNegate = aCondition->AttrValueIs(
      kNameSpaceID_None, nsGkAtoms::negate, nsGkAtoms::_true, eCaseMatters);

  nsTemplateCondition* condition;

  if (svar && vvar) {
    condition = new nsTemplateCondition(
        svar, relstring, vvar, shouldIgnoreCase, shouldNegate);
  } else if (svar && !value.IsEmpty()) {
    condition = new nsTemplateCondition(
        svar, relstring, value, shouldIgnoreCase, shouldNegate, shouldMultiple);
  } else if (vvar) {
    condition = new nsTemplateCondition(
        subject, relstring, vvar, shouldIgnoreCase, shouldNegate);
  } else {
...
nsresult
nsXULTemplateBuilder::CompileBindings(nsTemplateRule* aRule,
                                      nsIContent* aBindings)
{
  // Add an extended rule's bindings.
  nsresult rv;

  for (nsIContent* binding = aBindings->GetFirstChild(); binding;
       binding = binding->GetNextSibling()) {
    if (binding->NodeInfo()->Equals(nsGkAtoms::binding, kNameSpaceID_XUL)) {
                                            ...
                                            nsIContent* aElement)
{
  ...
  while (elements.Length()) {
    ...
    for (nsIContent* child = element->GetLastChild(); child;
         child = child->GetPreviousSibling()) {
      if (!elements.AppendElement(child)) return NS_ERROR_OUT_OF_MEMORY;
    }
  }
                                     ...
                                     void* aClosure)
{
  ...
  // sure...
  if (!StringBeginsWith(aVariable, NS_LITERAL_STRING("rdf:"))) return;
...
{
  for (ActivationEntry* entry = mTop; entry != nullptr;
       entry = entry->mPrevious) {
    if (entry->mResource == aResource) return true;
  }
  return false;
}
