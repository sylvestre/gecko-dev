...
nsXULTemplateResultRDF::nsXULTemplateResultRDF(nsRDFQuery* aQuery,
    ...
    : mQuery(aQuery), mNode(aNode), mInst(aInst)
{
}
...
nsXULTemplateResultRDF::GetIsContainer(bool* aIsContainer)
{
  *aIsContainer = false;
  ...
  if (mNode) {
    ...
    if (processor) return processor->CheckContainer(mNode, aIsContainer);
  }
...
nsXULTemplateResultRDF::GetIsEmpty(bool* aIsEmpty)
{
  *aIsEmpty = true;
  ...
  if (mNode) {
    nsXULTemplateQueryProcessorRDF* processor = GetProcessor();
    if (processor) return processor->CheckEmpty(mNode, aIsEmpty);
  }
...
nsXULTemplateResultRDF::GetType(nsAString& aType)
{
  ...
  if (processor) {
    ...
    if (NS_SUCCEEDED(rv) && found) aType.AssignLiteral("separator");
  }
...
nsXULTemplateResultRDF::HasBeenRemoved()
{
  ...
  nsXULTemplateQueryProcessorRDF* processor = GetProcessor();
  if (processor) processor->RemoveMemoryElements(mInst, this);
...
nsXULTemplateResultRDF::GetAssignment(nsAtom* aVar, nsIRDFNode** aValue)
{
  ...
  *aValue = nullptr;
  ...
  if (!*aValue) mBindingValues.GetAssignmentFor(this, aVar, aValue);
}
                                        ...
                                        nsIRDFNode* aTarget)
{
  ...
  if (bindingset) {
    return bindingset->SyncAssignments(
        aSubject,
        aPredicate,
        aTarget,
...
nsXULTemplateResultRDF::HasMemoryElement(const MemoryElement& aMemoryElement)
{
  ...
  for (MemoryElementSet::ConstIterator element = mInst.mSupport.First();
       element != last;
       ++element) {
    if ((*element).Equals(aMemoryElement)) return true;
  }
