...
nsAssignmentSet::Add(const nsAssignment& aAssignment)
{
  ...
  if (HasAssignmentFor(aAssignment.mVariable)) return NS_ERROR_UNEXPECTED;
...
{
  for (ConstIterator assignment = First(); assignment != Last(); ++assignment) {
    if (assignment->mVariable == aVariable) return true;
  }
...
{
  if (aSet.mAssignments == mAssignments) return true;

  // If they have a different number of assignments, then they're different.
  if (Count() != aSet.Count()) return false;
  ...
  for (ConstIterator assignment = First(); assignment != Last(); ++assignment) {
    ...
    if (assignment->mValue != value) return false;
  }
...
{
  Iterator inst = First();
  while (inst != Last()) Erase(inst++);
}

InstantiationSet::Iterator
InstantiationSet::Insert(Iterator aIterator,
...
TestNode::TestNode(TestNode* aParent) : mParent(aParent) {}

nsresult
TestNode::Propagate(InstantiationSet& aInstantiations,
                    bool aIsUpdate,
                    bool& aTakenInstantiations)
{
...
ReteNodeSet::ReteNodeSet() : mNodes(nullptr), mCount(0), mCapacity(0) {}

ReteNodeSet::~ReteNodeSet() { Clear(); }
...
ReteNodeSet::Add(ReteNode* aNode)
{
  ...
  if (mCount >= mCapacity) {
    ...
    for (int32_t i = mCount - 1; i >= 0; --i) nodes[i] = mNodes[i];
