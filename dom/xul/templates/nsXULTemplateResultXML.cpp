    ...
    : mQuery(aQuery), mNode(aNode)
{
  ...
  if (aBindings) mRequiredValues.SetBindingSet(aBindings);
}
...
nsXULTemplateResultXML::GetBindingFor(nsAtom* aVar, nsAString& aValue)
{
  ...
  if (attr.Length() > 1) {
    ...
    if (element) return element->GetAttribute(Substring(attr, 1), aValue);
  }
                                    ...
                                    nsIDOMNode* aRuleNode)
{
  ...
  nsXULTemplateQueryProcessorXML* processor =
      mQuery ? mQuery->Processor() : nullptr;
  if (processor) {
    nsXMLBindingSet* bindings =
        processor->GetOptionalBindingsForRule(aRuleNode);
    if (bindings) mOptionalValues.SetBindingSet(bindings);
  }
...
NS_IMETHODIMP
nsXULTemplateResultXML::HasBeenRemoved() { return NS_OK; }
