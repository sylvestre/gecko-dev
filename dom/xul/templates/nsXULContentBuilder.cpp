...
class nsXULContentBuilder : public nsXULTemplateBuilder
{
 ...
 protected:
  friend nsresult NS_NewXULContentBuilder(Element* aElement,
  ...
  /**
     ...
     */
  nsresult BuildContentFromTemplate(nsIContent* aTemplateNode,
  ...
  /**
     ...
     */
  nsresult CopyAttributesToElement(nsIContent* aTemplateNode,
                                   nsIContent* aRealNode,
  ...
  /**
     ...
     * @param aResult result to look up variable->value bindings in
     */
  nsresult AddPersistentAttributes(Element* aTemplateNode,
                                   nsIXULTemplateResult* aResult,
                                   nsIContent* aRealNode);
  ...
  /**
     ...
     * @param aResult result to look up variable->value bindings in
     */
  nsresult SynchronizeUsingTemplate(nsIContent* aTemplateNode,
                                    nsIContent* aRealNode,
                                    nsIXULTemplateResult* aResult);
  ...
  /**
     ...
     */
  nsresult CreateTemplateAndContainerContents(nsIContent* aElement,
                                              bool aForceCreation);
  ...
  /**
     ...
     */
  nsresult CreateContainerContents(nsIContent* aElement,
                                   nsIXULTemplateResult* aResult,
                                   bool aForceCreation,
                                   bool aNotify,
                                   bool aNotifyAtEnd);
  ...
  /**
     ...
     */
  nsresult CreateContainerContentsForQuerySet(nsIContent* aElement,
                                              nsIXULTemplateResult* aResult,
                                              bool aNotify,
  ...
  /**
     ...
     */
  nsresult EnsureElementHasGenericChild(nsIContent* aParent,
                                        int32_t aNameSpaceID,
                                        nsAtom* aTag,
                                        bool aNotify,
                                        nsIContent** aResult);

  bool IsOpen(nsIContent* aElement);

  nsresult RemoveGeneratedContent(nsIContent* aElement);

  nsresult GetElementsForResult(nsIXULTemplateResult* aResult,
                                nsCOMArray<nsIContent>& aElements);

  nsresult CreateElement(int32_t aNameSpaceID, nsAtom* aTag, Element** aResult);
  ...
  /**
     ...
     * @param aNotify true to notify of DOM changes
     */
  nsresult SetContainerAttrs(nsIContent* aElement,
                             nsIXULTemplateResult* aResult,
                             bool aIgnoreNonContainers,
                             bool aNotify);

  virtual nsresult RebuildAll() override;
  ...
  virtual bool GetInsertionLocations(
      nsIXULTemplateResult* aOldResult,
  ...
  /**
     ...
     */
  virtual nsresult ReplaceMatch(nsIXULTemplateResult* aOldResult,
  ...
  /**
     ...
     */
  virtual nsresult SynchronizeResult(nsIXULTemplateResult* aResult) override;
  ...
  /**
     ...
     */
  nsresult CompareResultToNode(nsIXULTemplateResult* aResult,
                               nsIContent* aContent,
                               int32_t* aSortOrder);
  ...
  /**
     ...
     */
  nsresult InsertSortedNode(nsIContent* aContainer,
                            nsIContent* aNode,
                            nsIXULTemplateResult* aResult,
                            bool aNotify);
                                              ...
                                              int32_t* aNewIndexInContainer)
{
  ...
  for (nsIContent* tmplKid = aTemplateNode->GetFirstChild(); tmplKid;
    ...
    // "uri='...'" syntax.)
    if (tmplKid->HasAttr(kNameSpaceID_None, nsGkAtoms::uri) &&
        aMatch->IsActive()) {
    ...
    if (isUnique) {
      ...
      // already.
      rv = EnsureElementHasGenericChild(
          aRealNode, nameSpaceID, tag, aNotify, getter_AddRefs(realKid));
      ...
      // encounter it.
      rv = BuildContentFromTemplate(tmplKid,
                                    aResourceNode,
                                    realKid,
                                    true,
                                    aIsSelfReference,
    ...
    if (realKid && !realKidAlreadyExisted) {
      ...
      if (isGenerationElement) {
        rv = AddPersistentAttributes(tmplKid->AsElement(), aChild, realKid);
        if (NS_FAILED(rv)) return rv;
      }
      ...
      if (!aIsSelfReference && !isUnique) {
        ...
        if (isGenerationElement) {
          ...
          rv = CreateContainerContents(realKid, aChild, false, false, false);
          if (NS_FAILED(rv)) return rv;
        }
      }
  ...
  for (uint32_t attr = 0; attr < numAttribs; attr++) {
    ...
    if (attribName != nsGkAtoms::id && attribName != nsGkAtoms::uri) {
      ...
      if (!attribValue.IsEmpty()) {
        ...
        // attribute
        if (!value.IsEmpty()) {
          rv = aRealNode->SetAttr(
              attribNameSpaceID, attribName, name->GetPrefix(), value, aNotify);
        } else {
          rv = aRealNode->UnsetAttr(attribNameSpaceID, attribName, aNotify);
        }
                                             ...
                                             nsIContent* aRealNode)
{
  ...
  while (!persist.IsEmpty()) {
    ...
    rv = nsXULContentUtils::GetResource(
        nameSpaceID, tag, getter_AddRefs(property));
    NS_ENSURE_SUCCESS(rv, rv);
    ...
    rv = aRealNode->SetAttr(
        nameSpaceID, tag, nsDependentString(valueStr), false);
    NS_ENSURE_SUCCESS(rv, rv);
  }
                                              ...
                                              nsIXULTemplateResult* aResult)
{
  ...
  for (uint32_t loop = 0; loop < count; ++loop) {
    ...
    if (tmplKid->NodeInfo()->Equals(nsGkAtoms::textnode, kNameSpaceID_XUL)) {
      nsAutoString attrValue;
      tmplKid->GetAttr(kNameSpaceID_None, nsGkAtoms::value, attrValue);
      if (!attrValue.IsEmpty()) {
        nsAutoString value;
                                                        ...
                                                        bool aForceCreation)
{
  ...
  MOZ_LOG(gXULTemplateLog,
          LogLevel::Info,
          ("nsXULContentBuilder::CreateTemplateAndContainerContents start - "
           "flags: %d",
           mFlags));

  if (!mQueryProcessor) return NS_OK;
  ...
  if (aElement == mRoot) {
    if (!mRootResult) {
      ...
      if (!ref.IsEmpty()) {
        nsresult rv = mQueryProcessor->TranslateRef(
            mDataSource, ref, getter_AddRefs(mRootResult));
        if (NS_FAILED(rv)) return rv;
      }
    }
    ...
    if (mRootResult) {
      CreateContainerContents(
          aElement, mRootResult, aForceCreation, false, true);
    }
    ...
    if (mContentSupportMap.Get(aElement, &match))
      CreateContainerContents(
          aElement, match->mResult, aForceCreation, false, true);
  }
                                             ...
                                             bool aNotifyAtEnd)
{
  ...
  if (!refResource) return NS_ERROR_FAILURE;

  // Avoid re-entrant builds for the same resource.
  if (IsActivated(refResource)) return NS_OK;
  ...
  if (mQuerySets.Length() == 0) return NS_OK;
  ...
  if (xulcontent) {
    if (xulcontent->GetTemplateGenerated()) return NS_OK;
  ...
  for (int32_t r = 0; r < querySetCount; r++) {
    ...
    if (tag && tag != aElement->NodeInfo()->NameAtom()) continue;

    CreateContainerContentsForQuerySet(
        aElement, aResult, aNotify, queryset, &container, &newIndexInContainer);
  }

  if (aNotifyAtEnd && container) {
    MOZ_AUTO_DOC_UPDATE(
        container->GetUncomposedDoc(), UPDATE_CONTENT_MODEL, true);
...
nsresult
nsXULContentBuilder::CreateContainerContentsForQuerySet(
    nsIContent* aElement,
    nsIXULTemplateResult* aResult,
    bool aNotify,
    nsTemplateQuerySet* aQuerySet,
    ...
    int32_t* aNewIndexInContainer)
{
  if (MOZ_LOG_TEST(gXULTemplateLog, LogLevel::Debug)) {
    ...
    MOZ_LOG(gXULTemplateLog,
            LogLevel::Debug,
            ("nsXULContentBuilder::CreateContainerContentsForQuerySet start "
             "for ref %s\n",
             NS_ConvertUTF16toUTF8(id).get()));
  }
  ...
  nsresult rv = mQueryProcessor->GenerateResults(
      mDataSource, aResult, aQuerySet->mCompiledQuery, getter_AddRefs(results));
  ...
  for (; NS_SUCCEEDED(rv) && hasMoreResults;
    ...
    if (!nextresult) return NS_ERROR_UNEXPECTED;
    ...
    nsTemplateMatch* newmatch =
        nsTemplateMatch::Create(aQuerySet->Priority(), nextresult, aElement);
    if (!newmatch) return NS_ERROR_OUT_OF_MEMORY;
    ...
    if (mMatchMap.Get(resultid, &existingmatch)) {
      ...
      while (existingmatch) {
        ...
        if (priority > aQuerySet->Priority()) break;
        ...
        if (existingmatch->GetContainer() == aElement) {
          ...
          if (existingmatch->IsActive()) generateContent = false;
        }
    ...
    if (generateContent) {
      ...
      rv = DetermineMatchedRule(
          aElement, nextresult, aQuerySet, &matchedrule, &ruleindex);
      if (NS_FAILED(rv)) {
      ...
      if (matchedrule) {
        rv = newmatch->RuleMatched(
            aQuerySet, matchedrule, ruleindex, nextresult);
        if (NS_FAILED(rv)) {
          nsTemplateMatch::Destroy(newmatch, false);
          return rv;
        }
    ...
    if (mFlags & eLoggingEnabled) OutputMatchToLog(resultid, newmatch, true);
...
nsXULContentBuilder::RemoveGeneratedContent(nsIContent* aElement)
{
  ...
  while (0 != (count = ungenerated.Length())) {
    ...
    while (i-- > 0) {
      ...
      if (element->NodeInfo()->Equals(nsGkAtoms::_template, kNameSpaceID_XUL) ||
          !element->IsElement())
        continue;
                                   ...
                                   Element** aResult)
{
  ...
  RefPtr<mozilla::dom::NodeInfo> nodeInfo = doc->NodeInfoManager()->GetNodeInfo(
      aTag, nullptr, aNameSpaceID, nsIDOMNode::ELEMENT_NODE);
                                       ...
                                       bool aNotify)
{
  ...
  if (aIgnoreNonContainers && !iscontainer) return NS_OK;

  NS_NAMED_LITERAL_STRING(true_, "true");
  NS_NAMED_LITERAL_STRING(false_, "false");

  const nsAString& newcontainer = iscontainer ? true_ : false_;

  aElement->SetAttr(
      kNameSpaceID_None, nsGkAtoms::container, newcontainer, aNotify);

  if (iscontainer && !(mFlags & eDontTestEmpty)) {
    bool isempty;
    aResult->GetIsEmpty(&isempty);

    const nsAString& newempty = (iscontainer && isempty) ? true_ : false_;

    aElement->SetAttr(kNameSpaceID_None, nsGkAtoms::empty, newempty, aNotify);
  }
  ...
  for (int32_t i = int32_t(cnt) - 1; i >= 0; --i) {
    ...
    do {
      ...
      if (content == mRoot || mContentSupportMap.Get(content, &match)) {
        ...
        if (DOMStringIsNull(aTag) ||
            content->NodeInfo()->LocalName().Equals(aTag)) {
          return true;
        }
      }
...
nsXULContentBuilder::GetResultForContent(Element& aElement)
{
  ...
  nsTemplateMatch* match;
  return mContentSupportMap.Get(&aElement, &match) ? match->mResult.get()
                                                   : nullptr;
}
                                      ...
                                      const nsAttrValue* aOldValue)
{
  ...
  nsXULTemplateBuilder::AttributeChanged(
      aDocument, aElement, aNameSpaceID, aAttribute, aModType, aOldValue);
}
...

{
  ...
  if (content) {
    ...
    if (!ref.IsEmpty()) {
      ...
      if (refResult) SetContainerAttrs(content, refResult, false, true);
    }
  }
  ...
  if (aOldResult) {
    ...
    for (int32_t e = int32_t(count) - 1; e >= 0; --e) {
      ...
      if (mContentSupportMap.Get(child, &match)) {
        if (content == match->GetContainer()) RemoveMember(child);
      }
    }
  }
...
nsXULContentBuilder::SynchronizeResult(nsIXULTemplateResult* aResult)
{
  ...
  for (int32_t i = int32_t(cnt) - 1; i >= 0; --i) {
    ...
    nsTemplateMatch* match;
    if (!mContentSupportMap.Get(element, &match)) continue;
...
nsXULContentBuilder::OpenContainer(nsIContent* aElement)
{
  if (aElement != mRoot) {
    ...
    nsCOMPtr<nsIXULDocument> xuldoc =
        do_QueryInterface(aElement->GetComposedDoc());
    if (!xuldoc) return NS_OK;
    ...
    do {
      ...
      if (builder) {
        if (builder == this) rightBuilder = true;
        break;
      }
...
nsXULContentBuilder::RebuildAll()
{
  ...
  if (mQueriesCompiled) Uninit(false);
  ...
  if (xulcontent) xulcontent->ClearTemplateGenerated();
                                         ...
                                         int32_t* aSortOrder)
{
  ...
  *aSortOrder = 0;
  ...
  if (mSortState.direction == nsSortState_natural) {
    ...
    nsresult rv = mQueryProcessor->CompareResults(
        aResult, match->mResult, nullptr, mSortState.sortHints, aSortOrder);
    NS_ENSURE_SUCCESS(rv, rv);
  } else {
    ...
    for (int32_t t = 0; t < length; t++) {
      nsresult rv = mQueryProcessor->CompareResults(aResult,
                                                    match->mResult,
                                                    mSortState.sortKeys[t],
                                                    mSortState.sortHints,
                                                    aSortOrder);
      NS_ENSURE_SUCCESS(rv, rv);
                                      ...
                                      bool aNotify)
{
  ...
  if (!mSortState.initialized) {
    ...
    rv = XULSortServiceImpl::InitializeSortState(
        mRoot, aContainer, sort, sortDirection, &mSortState);
    NS_ENSURE_SUCCESS(rv, rv);
  }
  ...
  if (mSortState.direction == nsSortState_natural) {
    ...
    nsresult rv =
        aResult->GetBindingObjectFor(mRefVariable, getter_AddRefs(ref));
    NS_ENSURE_SUCCESS(rv, rv);

    nsCOMPtr<nsIRDFResource> container = do_QueryInterface(ref);

    if (container) {
      rv = gRDFContainerUtils->IsSeq(
          mDB, container, &mSortState.isContainerRDFSeq);
      NS_ENSURE_SUCCESS(rv, rv);
    }
  }
  ...
  if (mSortState.direction != nsSortState_natural ||
      (mSortState.direction == nsSortState_natural &&
       mSortState.isContainerRDFSeq)) {
    ...
    if (!staticValue.IsEmpty()) {
      ...
      if (NS_FAILED(strErr)) staticCount = 0;
    } else {
      ...
      for (nsIContent* child = aContainer->GetFirstChild(); child;
        ...
        if (nsContentUtils::HasNonEmptyAttr(
                child, kNameSpaceID_None, nsGkAtoms::_template))
          break;
        else
          ++staticCount;
      }
      ...
      aContainer->SetAttr(
          kNameSpaceID_None, nsGkAtoms::staticHint, valueStr, false);
    }
  ...
  if (!childAdded) aContainer->InsertChildAt(aNode, numChildren, aNotify);
