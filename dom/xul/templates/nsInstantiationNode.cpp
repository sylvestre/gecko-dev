...
nsInstantiationNode::nsInstantiationNode(
    nsXULTemplateQueryProcessorRDF* aProcessor, nsRDFQuery* aQuery)
    : mProcessor(aProcessor), mQuery(aQuery)
{
                               ...
                               bool& aTakenInstantiations)
{
  ...
  if (aIsUpdate) {
    ...
    for (InstantiationSet::ConstIterator inst = aInstantiations.First();
         inst != last;
         ++inst) {
      ...
      if (node) {
        ...
        if (resource) {
          ...
          if (!nextresult) return NS_ERROR_OUT_OF_MEMORY;
    ...
    if (NS_SUCCEEDED(rv)) aTakenInstantiations = true;
  }
