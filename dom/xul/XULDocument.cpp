...
XULDocument::~XULDocument()
{
  ...
  Preferences::UnregisterCallback(
      XULDocument::DirectionChanged, "intl.uidirection", this);
...
NS_IMPL_CYCLE_COLLECTION_TRAVERSE_BEGIN_INHERITED(XULDocument, XMLDocument)
  ...
  if (tmp->mTemplateBuilderTable) {
    for (auto iter = tmp->mTemplateBuilderTable->Iter(); !iter.Done();
         iter.Next()) {
...
void
XULDocument::ResetToURI(nsIURI* aURI,
...
// being cached or not.
nsresult
XULDocument::StartDocumentLoad(const char* aCommand,
                               nsIChannel* aChannel,
                               nsILoadGroup* aLoadGroup,
                               ...
                               nsIContentSink* aSink)
{
  ...
  if (proto) {
    ...
    *aDocListener = new CachedChromeStreamListener(this, loaded);
    ...
    rv = PrepareToLoadPrototype(
        mDocumentURI, aCommand, principal, getter_AddRefs(parser));
    if (NS_FAILED(rv)) return rv;
...
XULDocument::OnDocumentParserError()
{
  ...
  if (mCurrentPrototype && mMasterPrototype != mCurrentPrototype) {
    ...
    if (IsChromeURI(uri)) {
      ...
      if (os)
        os->NotifyObservers(
            uri, "xul-overlay-parsererror", EmptyString().get());
    }
  ...
  if (aAttr.EqualsLiteral("*")) {
    ...
    while (count-- > 0) {
      ...
      if (aBroadcaster->GetAttr(nameSpaceID, name, value)) {
        aListener->SetAttr(
            nameSpaceID, name, attributes[count].mPrefix, value, notify);
      }
...
void
XULDocument::AddBroadcastListenerFor(Element& aBroadcaster,
                                     Element& aListener,
                                     const nsAString& aAttr,
                                     ErrorResult& aRv)
{
  ...
  static const PLDHashTableOps gOps = {PLDHashTable::HashVoidPtrKeyStub,
                                          ...
                                          nsAtom* aAttr)
{
  ...
  for (nsIContent* child = aListener->GetFirstChild(); child;
    ...
    child->GetAttr(
        kNameSpaceID_None, nsGkAtoms::attribute, listeningToAttribute);
    ...
    if (shell) {
      ...
      EventDispatcher::Dispatch(child, aPresContext, &event, nullptr, &status);
    }
  }
      ...
      for (size_t i = entry->mListeners.Length() - 1; i != (size_t)-1; --i) {
        ...
        if ((bl->mAttribute == aAttribute) ||
          ...
          if (listenerEl) {
            ...
            bool hasAttr = listenerEl->GetAttr(
                kNameSpaceID_None, aAttribute, currentValue);
...
nsresult
XULDocument::GetViewportSize(int32_t* aWidth, int32_t* aHeight)
{
  *aWidth = *aHeight = 0;
...
XULDocument::GetPopupNode(nsIDOMNode** aNode)
{
  *aNode = nullptr;
  ...
  if (rootWin) node = rootWin->GetPopupNode();  // addref happens here
...
XULDocument::SetPopupNode(nsIDOMNode* aNode)
{
  ...
  nsCOMPtr<nsPIWindowRoot> rootWin = GetWindowRoot();
  if (rootWin) rootWin->SetPopupNode(aNode);  // addref happens here
...
XULDocument::GetTooltipNode(nsIDOMNode** aNode)
{
  *aNode = nullptr;
  ...
  if (pm) {
    ...
    if (node && nsContentUtils::CanCallerAccess(node)) node.forget(aNode);
  }
...
{
  if (!CheckGetElementByIdArg(aId)) return nullptr;
...
XULDocument::AddElementToRefMap(Element* aElement)
{
  ...
  if (!value.IsEmpty()) {
    ...
    if (!entry->AddElement(aElement)) return NS_ERROR_OUT_OF_MEMORY;
  }
...
nsresult
XULDocument::Clone(mozilla::dom::NodeInfo* aNodeInfo,
                   nsINode** aResult,
                            ...
                            void* aData)
{
  ...
  if (aNamespaceID != kNameSpaceID_Unknown &&
    ...
    return attrValue->EqualsLiteral("*")
               ...
               : aElement->AttrValueIs(
                     aNamespaceID, aAttrName, *attrValue, eCaseMatters);
  }
  ...
  for (uint32_t i = 0; i < count; ++i) {
    ...
    if (nameMatch) {
      return attrValue->EqualsLiteral("*") ||
             aElement->AttrValueIs(name->NamespaceID(),
                                   name->LocalName(),
                                   *attrValue,
                                   eCaseMatters);
    }
...
nsresult
XULDocument::PrepareToLoadPrototype(nsIURI* aURI,
                                    const char* aCommand,
...
nsresult
XULDocument::ApplyPersistentAttributesToElements(
    const nsAString& aID, nsCOMArray<nsIContent>& aElements)
{
  nsAutoCString utf8uri;
  nsresult rv = mDocumentURI->GetSpec(utf8uri);
  if (NS_WARN_IF(NS_FAILED(rv))) {
...
XULDocument::PrepareToWalk()
{
  ...
  if (!proto) {
    if (MOZ_LOG_TEST(gXULLog, LogLevel::Error)) {
      ...
      MOZ_LOG(
          gXULLog, LogLevel::Error, ("xul: error parsing '%s'", urlspec.get()));
    }
  ...
  for (uint32_t i = 0; i < total; ++i) {
    rv = CreateAndInsertPI(
        processingInstructions[i], this, piInsertionPoint + i);
    if (NS_FAILED(rv)) return rv;
  }
                                ...
                                nsIContent* aPINode)
{
  ...
  nsAutoString href;
  nsContentUtils::GetPseudoAttributeValue(
      aProtoPI->mData, nsGkAtoms::href, href);
  ...
  rv = NS_NewURI(
      getter_AddRefs(uri), href, nullptr, mCurrentPrototype->GetURI());
  if (NS_SUCCEEDED(rv)) {
...
XULDocument::LoadOverlay(const nsAString& aURL, nsIObserver* aObserver)
{
  ...
  if (NS_FAILED(rv) && mOverlayLoadObservers)
    mOverlayLoadObservers->Remove(
        uri);  // remove the observer if LoadOverlayInternal generated an error
  return rv;
}

nsresult
XULDocument::LoadOverlayInternal(nsIURI* aURI,
                                 bool aIsDynamic,
                                 bool* aShouldReturn,
                                 bool* aFailureFromContent)
{
  ...
  *aFailureFromContent = false;
  ...
  if (useXULCache && mCurrentPrototype) {
    ...
    if (mIsGoingAway) {
      MOZ_LOG(
          gXULLog, LogLevel::Debug, ("xul: ...and document already destroyed"));
      return NS_ERROR_NOT_AVAILABLE;
    }
      ...
      switch (childproto->mType) {
        case nsXULPrototypeNode::eType_Element: {
          ...
          if (!processingOverlayHookupNodes) {
            rv = CreateElementFromPrototype(
                protoele, getter_AddRefs(child), false);
            if (NS_FAILED(rv)) return rv;
        ...
        case nsXULPrototypeNode::eType_PI: {
          ...
          if (parent) {
            ...
            rv = CreateAndInsertPI(piProto, parent, parent->GetChildCount());
            NS_ENSURE_SUCCESS(rv, rv);
          }
        } break;
                              ...
                              const uint8_t* string)
{
  ...
  if (NS_SUCCEEDED(aStatus)) {
    ...
    rv = ScriptLoader::ConvertToUTF16(channel,
                                      string,
                                      stringLen,
                                      EmptyString(),
                                      this,
                                        ...
                                        bool aIsRoot)
{
  ...
  *aResult = nullptr;
  ...
  if (aPrototype->mNodeInfo->NamespaceEquals(kNameSpaceID_XUL)) {
    ...
    // monkeys with it.
    rv = nsXULElement::Create(
        aPrototype, this, true, aIsRoot, getter_AddRefs(result));
    if (NS_FAILED(rv)) return rv;
  } else {
    ...
    newNodeInfo =
        mNodeInfoManager->GetNodeInfo(aPrototype->mNodeInfo->NameAtom(),
    ...
    rv = NS_NewElement(
        getter_AddRefs(result), newNodeInfo.forget(), NOT_FROM_PARSER);
    if (NS_FAILED(rv)) return rv;
                                        ...
                                        bool* aNeedsHookup)
{
  ...
  *aNeedsHookup = aElement->HasAttr(kNameSpaceID_None, nsGkAtoms::datasources);
  return NS_OK;
}
...
XULDocument::CreateTemplateBuilder(Element* aElement)
{
  ...
  int32_t nameSpaceID;
  nsAtom* baseTag =
      document->BindingManager()->ResolveTag(aElement, &nameSpaceID);
  ...
  if (isTreeBuilder) {
    ...
    nsXULContentUtils::FindChildByTag(aElement,
                                      kNameSpaceID_XUL,
...
XULDocument::OverlayForwardReference::Resolve()
{
  ...
  if (MOZ_LOG_TEST(gXULLog, LogLevel::Debug)) {
    ...
    MOZ_LOG(
        gXULLog, LogLevel::Debug, ("xul: overlay resolved '%s'", idC.get()));
  }
                                            ...
                                            bool aNotify)
{
  ...
  for (i = 0; (name = aOverlayNode->GetAttrNameAt(i)); ++i) {
    ...
    if (!aNotify) {
      ...
      if (name->Equals(nsGkAtoms::command) &&
          ...
          !aTargetNode->NodeInfo()->Equals(nsGkAtoms::key, kNameSpaceID_XUL) &&
    ...
    if (!NS_FAILED(rv) && !aNotify)
      rv = mDocument->BroadcastAttributeChangeFromOverlay(
          aTargetNode, nameSpaceID, attr, prefix, value);
    if (NS_FAILED(rv)) return rv;
  }
  ...
  for (i = 0; i < childCount; ++i) {
    ...
    if (elementInDocument) {
      ...
      if (parentID && aTargetNode->AttrValueIs(kNameSpaceID_None,
                                               nsGkAtoms::id,
...
XULDocument::BroadcasterHookup::Resolve()
{
  ...
  bool listener;
  rv = mDocument->CheckBroadcasterHookup(
      mObservesElement, &listener, &mResolved);
  if (NS_FAILED(rv)) return eResolve_Error;
...
XULDocument::BroadcasterHookup::~BroadcasterHookup()
{
  if (MOZ_LOG_TEST(gXULLog, LogLevel::Warning) && !mResolved) {
    ...
    if (mObservesElement->IsXULElement(nsGkAtoms::observes)) {
      mObservesElement->GetAttr(
          kNameSpaceID_None, nsGkAtoms::element, broadcasterID);
      mObservesElement->GetAttr(
          kNameSpaceID_None, nsGkAtoms::attribute, attribute);
    } else {
      mObservesElement->GetAttr(
          kNameSpaceID_None, nsGkAtoms::observes, broadcasterID);
      attribute.Assign('*');
    }
...
XULDocument::IsDocumentRightToLeft()
{
  ...
  if (element) {
    ...
    switch (element->FindAttrValueIn(
        kNameSpaceID_None, nsGkAtoms::localedir, strings, eCaseMatters)) {
      case 0:
        return false;
      case 1:
        return true;
