...
void
XULContentSinkImpl::ContextStack::Traverse(
                                             ...
                                             nsAttrName& aName)
{
  ...
  nsContentUtils::SplitExpatName(aExpatName,
  ...
  switch (node->mType) {
    ...
    case nsXULPrototypeNode::eType_Script: {
      ...
      if (!script->mSrcURI && !script->HasScriptObject()) {
        ...
        if (doc)
          script->Compile(
              mText, mTextLength, mDocumentURL, script->mLineNo, doc);
      }
...
NS_IMETHODIMP
XULContentSinkImpl::HandleCharacterData(const char16_t* aData, uint32_t aLength)
{
                                  ...
                                  nsXULPrototypeElement* aElement)
{
  ...
  for (i = 0; i < aAttrLen; ++i) {
    ...
    rv = aElement->SetAttrAt(
        i, nsDependentString(aAttributes[i * 2 + 1]), mDocumentURL);
    NS_ENSURE_SUCCESS(rv, rv);
...
nsresult
XULContentSinkImpl::AddText(const char16_t* aText, int32_t aLength)
{
