...
{
  if (AttrValueIs(kNameSpaceID_None,
                  nsGkAtoms::windowtype,
                     ...
                     Element** aResult)
{
  ...
  if (aDocument) {
    ...
    nodeInfo =
        aDocument->NodeInfoManager()->GetNodeInfo(ni->NameAtom(),
                                                  ni->GetPrefixAtom(),
                                                  ni->NamespaceID(),
...
nsresult
NS_NewXULElement(Element** aResult,
...
NS_INTERFACE_TABLE_HEAD_CYCLE_COLLECTION_INHERITED(nsXULElement)
  NS_INTERFACE_TABLE_INHERITED(
      nsXULElement, nsIDOMNode, nsIDOMElement, nsIDOMXULElement)
...
nsresult
nsXULElement::Clone(mozilla::dom::NodeInfo* aNodeInfo,
                    nsINode** aResult,
  ...
  *aResult = nullptr;
  ...
  for (uint32_t i = 0; i < count; ++i) {
    ...
    if (originalName->IsAtom()) {
      rv = element->mAttrsAndChildren.SetAndSwapAttr(
          originalName->Atom(), attrValue, &oldValueSet);
    } else {
      rv = element->mAttrsAndChildren.SetAndSwapAttr(
          originalName->NodeInfo(), attrValue, &oldValueSet);
    }
    NS_ENSURE_SUCCESS(rv, rv);
...
static bool
IsNonList(mozilla::dom::NodeInfo* aNodeInfo)
{
...
bool
nsXULElement::PerformAccesskey(bool aKeyCausesActivation, bool aIsTrustedEvent)
{
 ...
 */
static inline bool
XULElementsRulesInMinimalXULSheet(nsAtom* aTag)
{
                         ...
                         bool aCompileEventHandlers)
{
  ...
  nsresult rv = nsStyledElement::BindToTree(
                           ...
                           bool aNotify)
{
  ...
  return nsStyledElement::AfterSetAttr(
                             ...
                             nsAttrValue& aResult)
{
  ...
  if (!nsStyledElement::ParseAttribute(
...
nsXULElement::RemoveBroadcaster(const nsAString& broadcasterId)
{
  ...
  if (xuldoc) {
    ...
    if (broadcaster) {
      xuldoc->RemoveBroadcastListenerFor(
                                 ...
                                 nsAutoString& aCommand)
{
  ...
  if (commandContent) {
    ...
    while (domEvent) {
      ...
      NS_ENSURE_STATE(
          !SameCOMIdentity(event->GetOriginalTarget(), commandContent));
    ...
    nsContentUtils::DispatchXULCommand(commandContent,
                                       orig->IsTrusted(),
                                       aVisitor.mDOMEvent,
                                       nullptr,
                                       orig->IsControl(),
...
nsXULElement::GetResource(ErrorResult& rv)
{
  ...
  rv = nsXULContentUtils::RDFService()->GetUnicodeResource(
      id, getter_AddRefs(resource));
...
nsXULElement::GetControllers(ErrorResult& rv)
{
  if (!Controllers()) {
    ...
    rv = NS_NewXULControllers(nullptr,
...
nsXULElement::LoadSrc()
{
  ...
  // can be obtained.
  if (!IsAnyOfXULElements(
          nsGkAtoms::browser, nsGkAtoms::editor, nsGkAtoms::iframe)) {
    return NS_OK;
  }
...
void
nsXULElement::SwapFrameLoaders(nsXULElement& aOtherLoaderOwner, ErrorResult& rv)
{
...
nsXULElement::ClickWithInputSource(uint16_t aInputSource, bool aIsTrustedEvent)
{
  ...
  if (doc) {
    ...
    if (shell) {
      ...
      WidgetMouseEvent eventDown(
          aIsTrustedEvent, eMouseDown, nullptr, WidgetMouseEvent::eReal);
      WidgetMouseEvent eventUp(
          aIsTrustedEvent, eMouseUp, nullptr, WidgetMouseEvent::eReal);
      WidgetMouseEvent eventClick(
      ...
      EventDispatcher::Dispatch(
          static_cast<nsIContent*>(this), context, &eventUp, nullptr, &status);
...
nsXULElement::MakeHeavyweight(nsXULPrototypeElement* aPrototype)
{
  ...
  for (i = 0; i < aPrototype->mNumAttributes; ++i) {
    ...
    if (protoattr->mName.IsAtom()) {
      rv = mAttrsAndChildren.SetAndSwapAttr(
          protoattr->mName.Atom(), attrValue, &oldValueSet);
    } else {
      rv = mAttrsAndChildren.SetAndSwapAttr(
          protoattr->mName.NodeInfo(), attrValue, &oldValueSet);
    }
    NS_ENSURE_SUCCESS(rv, rv);
  }
  return NS_OK;
    ...
    const nsTArray<RefPtr<mozilla::dom::NodeInfo>>* aNodeInfos)
{
  ...
  for (i = 0; i < mNumAttributes; ++i) {
    ...
    if (mAttributes[i].mName.IsAtom()) {
      ni =
          mNodeInfo->NodeInfoManager()->GetNodeInfo(mAttributes[i].mName.Atom(),
                                                    nullptr,
                                                    kNameSpaceID_None,
    ...
    switch (child->mType) {
      ...
      case eType_Script:
        ...
        if (!script->mOutOfLine) {
          ...
          tmp = aStream->WriteCompoundObject(
              script->mSrcURI, NS_GET_IID(nsIURI), true);
          if (NS_FAILED(tmp)) {
            rv = tmp;
          }
    ...
    for (uint32_t i = 0; i < numChildren; i++) {
      ...
      switch (childType) {
        ...
        case eType_Script: {
          ...
          if (NS_WARN_IF(NS_FAILED(rv))) return rv;
          if (!script->mOutOfLine) {
            rv = script->Deserialize(
                aStream, aProtoDoc, aDocumentURI, aNodeInfos);
            if (NS_WARN_IF(NS_FAILED(rv))) return rv;
          } else {
                                 ...
                                 nsIURI* aDocumentURI)
{
  ...
  if (mAttributes[aPos].mName.Equals(nsGkAtoms::id) && !aValue.IsEmpty()) {
    ...
    if (nsLayoutUtils::StyloEnabled() &&
      ...
      nsCSSParser parser;
      declaration = parser.ParseStyleAttribute(
          aValue, aDocumentURI, aDocumentURI, principal);
    }
    if (declaration) {
    ...
    const nsTArray<RefPtr<mozilla::dom::NodeInfo>>* aNodeInfos)
{
  ...
  rv = nsContentUtils::XPConnect()->ReadScript(
      aStream, cx, newScriptObject.address());
  NS_ENSURE_SUCCESS(rv, rv);
  Set(newScriptObject);
  return NS_OK;
}
    ...
    nsIOffThreadScriptReceiver* aOffThreadReceiver /* = nullptr */)
{
  JS::SourceBufferHolder srcBuf(
      aText, aTextLength, JS::SourceBufferHolder::NoOwnership);
