...
struct FullIndexMetadata
{
 ...
 public:
  FullIndexMetadata()
      : mCommonMetadata(
            0, nsString(), KeyPath(0), nsCString(), false, false, false),
        mDeleted(false)
  {
...
struct FullDatabaseMetadata
{
 ...
 public:
  explicit FullDatabaseMetadata(const DatabaseMetadata& aCommonMetadata)
      : mCommonMetadata(aCommonMetadata), mNextObjectStoreId(0), mNextIndexId(0)
  {
...
class MOZ_STACK_CLASS MetadataNameOrIdMatcher final
{
 ...
 public:
  ...
  static MetadataType* Match(const Enumerable& aEnumerable,
                             uint64_t aId,
...
struct IndexDataValue final
{
  ...
  IndexDataValue(int64_t aIndexId,
                 bool aUnique,
                 const Key& aKey,
                 const Key& aSortKey)
...
void
GetDatabaseFilename(const nsAString& aName, nsAutoString& aDatabaseFilename)
{
                                        ...
                                        nsTArray<IndexDataValue>& aIndexValues)
{
  ...
  rv = ReadCompressedIndexDataValuesFromBlob(
      blobData, blobDataLength, aIndexValues);
  if (NS_WARN_IF(NS_FAILED(rv))) {
    return rv;
  }
...
ReadCompressedIndexDataValues(mozIStorageStatement* aStatement,
                              ...
                              nsTArray<IndexDataValue>& aIndexValues)
{
  return ReadCompressedIndexDataValuesFromSource(
      aStatement, aColumnIndex, aIndexValues);
}
...
{
  return ReadCompressedIndexDataValuesFromSource(
      aValues, aColumnIndex, aIndexValues);
}
    ...
    OnFunctionCall(mozIStorageValueArray* aValues, nsIVariant** _retval)
{
  ...
  rv = MakeCompressedIndexDataValues(
      indexValues, indexValuesBlob, &indexValuesBlobLength);
  if (NS_WARN_IF(NS_FAILED(rv))) {
    return rv;
  }
    ...
    : public mozIStorageFunction
{
 ...
 private:
  ...
  static uint32_t AdjustedSize(uint32_t aMaxSize,
    ...
    mozIStorageValueArray* aValues, nsIVariant** _retval)
{
  ...
  rv = CopyAndUpgradeKeyBuffer(
      blobData, blobData + blobDataLength, upgradedBlobData.get());
  if (NS_WARN_IF(NS_FAILED(rv))) {
    return rv;
  }
                   ...
                   nsIFileURL** aResult)
{
  ...
  rv = fileUrl->SetQuery(
      ...
      NS_LITERAL_CSTRING("&group=") + aGroup + NS_LITERAL_CSTRING("&origin=") +
      aOrigin + NS_LITERAL_CSTRING("&cache=private") + telemetryFilenameClause);
  if (NS_WARN_IF(NS_FAILED(rv))) {
    return rv;
  }
                          ...
                          mozIStorageConnection** aConnection)
{
  ...
  if (rv == NS_ERROR_STORAGE_BUSY) {
    ...
    while (true) {
      ...
      rv = StorageOpenTraits<FileOrURLType>::Open(
          aStorageService, aFileOrURL, getter_AddRefs(connection));
      if (rv != NS_ERROR_STORAGE_BUSY ||
                     ...
                     mozIStorageConnection** aConnection)
{
  ...
  return GetStorageConnection(
      dbFile, aPersistenceType, aGroup, aOrigin, aTelemetryId, aConnection);
}
...
class ConnectionPool::CloseConnectionRunnable final : public ConnectionRunnable
{
 public:
...
class DatabaseOperationBase : public Runnable, public mozIStorageProgressHandler
{
...
class Database final : public PBackgroundIDBDatabaseParent
{
 ...
 private:
  ...
  AllocPBackgroundIDBVersionChangeTransactionParent(
      ...
      const int64_t& aNextIndexId) override;
                  ...
                  public PBackgroundIDBFactoryRequestParent
{
 ...
 public:
  ...
  const nsString& DatabaseFilePath() const { return mDatabaseFilePath; }
...
class DeleteDatabaseOp final : public FactoryOp
{
 ...
 public:
  DeleteDatabaseOp(Factory* aFactory,
                   ...
                   const CommonFactoryRequestParams& aParams)
      : FactoryOp(
            aFactory, Move(aContentParent), aParams, /* aDeleting */ true),
        mPreviousVersion(0)
  {
  }
...
class DeleteDatabaseOp::VersionChangeOp final : public DatabaseOperationBase
{
 ...
 private:
  ...
  nsresult DeleteFile(nsIFile* aDirectory,
...
class NormalJSContext
{
 ...
 protected:
  NormalJSContext() : mContext(nullptr), mGlobal(nullptr)
  {
...
class CreateIndexOp::ThreadLocalJSContext final : public NormalJSContext
{
  friend class CreateIndexOp;
  friend class nsAutoPtr<ThreadLocalJSContext>;

 public:
  static ThreadLocalJSContext* GetOrCreate();
    ...
    : public mozIStorageFunction
{
 ...
 public:
  UpdateIndexDataValuesFunction(CreateIndexOp* aOp,
                                DatabaseConnection* aConnection,
                                JSContext* aCx)
      : mOp(aOp), mConnection(aConnection), mCx(aCx)
  {
    MOZ_ASSERT(aOp);
    MOZ_ASSERT(aConnection);
...
class ObjectStoreAddOrPutRequestOp::SCInputStream final : public nsIInputStream
{
                                      ...
                                      public PIndexedDBPermissionRequestParent
{
 ...
 public:
  PermissionRequestHelper(Element* aOwnerElement, nsIPrincipal* aPrincipal)
      : PermissionRequestBase(aOwnerElement, aPrincipal), mActorDestroyed(false)
  {
  }
...
class MOZ_STACK_CLASS FileHelper final
{
 ...
 public:
  ...
  nsresult CreateFileFromStream(nsIFile* aFile,
                                nsIFile* aJournalFile,
                                nsIInputStream* aInputStream,
                                bool aCompress);

  nsresult ReplaceFile(nsIFile* aFile,
                       nsIFile* aNewFile,
 ...
 private:
  ...
  nsresult SyncRead(nsIInputStream* aInputStream,
                    char* aBuffer,
                    uint32_t aBufferSize,
                    uint32_t* aRead);
};
                ...
                StructuredCloneFile* aCompiledFile)
{
  ...
  bool match;
  {
    ...
    rv = compiledFile->OpenNSPRFileDesc(
        PR_RDONLY, 0644, &compiledFileDesc.rwget());
    if (NS_WARN_IF(NS_FAILED(rv))) {
      return rv;
    }
  ...
  rv = bytecodeFile->OpenNSPRFileDesc(
      PR_RDONLY, 0644, &bytecodeFileDesc.rwget());
  if (NS_WARN_IF(NS_FAILED(rv))) {
    return rv;
  }
...
void
DatabaseConnection::CachedStatement::Reset()
{
  AssertIsOnConnectionThread();
...
void
DatabaseConnection::CachedStatement::Assign(
    DatabaseConnection* aConnection,
    already_AddRefed<mozIStorageStatement> aStatement)
{
#ifdef DEBUG
...
nsresult
DatabaseConnection::AutoSavepoint::Start(const TransactionBase* aTransaction)
{
...
DatabaseConnection::UpdateRefcountFunction::UpdateRefcountFunction(
    ...
    : mConnection(aConnection), mFileManager(aFileManager), mInSavepoint(false)
{
  MOZ_ASSERT(aConnection);
  aConnection->AssertIsOnConnectionThread();
  MOZ_ASSERT(aFileManager);
...
void
DatabaseConnection::UpdateRefcountFunction::StartSavepoint()
{
  MOZ_ASSERT(mConnection);
  mConnection->AssertIsOnConnectionThread();
...
void
DatabaseConnection::UpdateRefcountFunction::ReleaseSavepoint()
{
  MOZ_ASSERT(mConnection);
  mConnection->AssertIsOnConnectionThread();
  MOZ_ASSERT(mInSavepoint);
...
void
DatabaseConnection::UpdateRefcountFunction::RollbackSavepoint()
{
  MOZ_ASSERT(mConnection);
  mConnection->AssertIsOnConnectionThread();
  MOZ_ASSERT(!IsOnBackgroundThread());
...
void
DatabaseConnection::UpdateRefcountFunction::Reset()
{
  MOZ_ASSERT(mConnection);
  mConnection->AssertIsOnConnectionThread();
  MOZ_ASSERT(!mSavepointEntriesIndex.Count());
...
bool
DatabaseConnection::UpdateRefcountFunction::DatabaseUpdateFunction::Update(
                                      ...
                                      DatabaseConnection** aConnection)
{
  ...
  if (!connection) {
    ...
    nsresult rv = GetStorageConnection(aDatabase->FilePath(),
                                       aDatabase->Type(),
                                       aDatabase->Group(),
                                       aDatabase->Origin(),
                      ...
                      TransactionDatabaseOperationBase* aTransactionOp)
{
  ...
  auto* transactionInfo = new TransactionInfo(dbInfo,
    ...
    if (mIdleThreads.IsEmpty()) {
      ...
      if (mTotalThreadCount < kMaxConnectionThreadCount) {
        ...
        nsresult rv = NS_NewNamedThread(
            runnable->GetThreadName(), getter_AddRefs(newThread), runnable);
        if (NS_SUCCEEDED(rv)) {
...
void
ConnectionPool::TransactionInfo::AddBlockingTransaction(
...
void
ConnectionPool::TransactionInfo::RemoveBlockingTransactions()
{
  AssertIsOnBackgroundThread();
...
void
ConnectionPool::TransactionInfo::MaybeUnblock(TransactionInfo* aTransactionInfo)
{
  AssertIsOnBackgroundThread();
...
void
Database::StartTransactionOp::RunOnConnectionThread()
{
  MOZ_ASSERT(!IsOnBackgroundThread());
  MOZ_ASSERT(Transaction());
  MOZ_ASSERT(NS_SUCCEEDED(mResultCode));
...
nsresult
Database::StartTransactionOp::DoDatabaseWork(DatabaseConnection* aConnection)
{
  MOZ_ASSERT(aConnection);
  aConnection->AssertIsOnConnectionThread();
...
bool Database::StartTransactionOp::SendFailureResult(nsresult /* aResultCode */)
{
  IDB_REPORT_INTERNAL_ERR();
...
void
Database::StartTransactionOp::Cleanup()
{
#ifdef DEBUG
...
TransactionBase::GetMetadataForIndexId(
    FullObjectStoreMetadata* const aObjectStoreMetadata, int64_t aIndexId) const
{
  AssertIsOnBackgroundThread();
  MOZ_ASSERT(aIndexId);
...
bool
TransactionBase::VerifyRequestParams(
...
nsresult
FileManager::Init(nsIFile* aDirectory, mozIStorageConnection* aConnection)
{
  AssertIsOnIOThread();
                          ...
                          nsIFile** aDirectory)
{
  ...
  nsCOMPtr<nsIFile> directory;
  nsresult rv = quotaManager->GetDirectoryForOrigin(
      aPersistenceType, aOrigin, getter_AddRefs(directory));
  if (NS_WARN_IF(NS_FAILED(rv))) {
    return rv;
  }
    ...
    while (true) {
      ...
      while (true) {
        ...
        if (databasePaths.IsEmpty()) {
          ...
          if (NS_WARN_IF(
                  NS_FAILED(quotaManager->GetDirectoryMetadata2(originDir,
                                                                &dummyTimeStamp,
                                                                &dummyPersisted,
                                                                dummySuffix,
                                                                group,
    ...
    mozIStorageConnection* aConnection)
{
  ...
  nsresult rv = aConnection->SetProgressHandler(
      kProgressGranularity, this, getter_AddRefs(oldHandler));
  if (NS_WARN_IF(NS_FAILED(rv))) {
    return rv;
  }
...
void
DatabaseMaintenance::AutoProgressHandler::Unregister()
{
  MOZ_ASSERT(!NS_IsMainThread());
  MOZ_ASSERT(!IsOnBackgroundThread());
  MOZ_ASSERT(mConnection);
                             ...
                             mozIStorageConnection* aConnection)
{
  ...
  RefPtr<FileManager> fileManager = new FileManager(PERSISTENCE_TYPE_INVALID,
                                                    EmptyCString(),
                                                    EmptyCString(),
                                                    EmptyString(),
                                                    false);
                                       ...
                                       nsIVariant** aResult)
{
  ...
  if (NS_WARN_IF(
          !IDBObjectStore::DeserializeUpgradeValue(cx, cloneInfo, &clone))) {
    return NS_ERROR_DOM_DATA_CLONE_ERR;
  }

  nsAutoString fileIds;

  for (uint32_t count = cloneInfo.mFiles.Length(), index = 0; index < count;
       index++) {
    ...
    StructuredCloneReadInfo* aInfo)
{
  ...
  if (columnType == mozIStorageStatement::VALUE_TYPE_INTEGER) {
    ...
    rv = GetStructuredCloneReadInfoFromExternalBlob(
        intData, aFileManager, fileIds, aInfo);
  } else {
    const uint8_t* blobData;
    uint32_t blobDataLength;
    ...
    rv = GetStructuredCloneReadInfoFromBlob(
        blobData, blobDataLength, aFileManager, fileIds, aInfo);
  }
  if (NS_WARN_IF(NS_FAILED(rv))) {
    return rv;
  }
    ...
    StructuredCloneReadInfo* aInfo)
{
  ...
  if (NS_WARN_IF(!snappy::RawUncompress(
          compressed, compressedLength, uncompressedBuffer))) {
    return NS_ERROR_FILE_CORRUPTED;
  }
    ...
    StructuredCloneReadInfo* aInfo)
{
  ...
  if (!aFileIds.IsVoid()) {
    rv = DeserializeStructuredCloneFiles(
        aFileManager, aFileIds, aInfo->mFiles, &aInfo->mHasPreprocessInfo);
    if (NS_WARN_IF(NS_FAILED(rv))) {
      return rv;
    }
  ...
  if (singleRowOnly) {
    ...
    if (aKeyRange.type() == OptionalKeyRange::TSerializedKeyRange) {
      GetBindingClauseForKeyRange(
          aKeyRange.get_SerializedKeyRange(), keyString, keyRangeClause);
    }
    ...
    const FallibleTArray<IndexDataValue>& aIndexValues)
{
  ...
  nsresult rv = MakeCompressedIndexDataValues(
      aIndexValues, indexDataValues, &indexDataValuesLength);
  if (NS_WARN_IF(NS_FAILED(rv))) {
    return rv;
  }
...
MutableFile::Create(nsIFile* aFile, Database* aDatabase, FileInfo* aFileInfo)
{
  AssertIsOnBackgroundThread();
...
MutableFile::CreateStream(bool aReadOnly)
{
  ...
  if (aReadOnly) {
    RefPtr<FileInputStream> stream =
        FileInputStream::Create(persistenceType,
                                group,
                                origin,
                                mFile,
                                -1,
    ...
    RefPtr<FileStream> stream = FileStream::Create(persistenceType,
                                                   group,
                                                   origin,
                                                   mFile,
                                                   -1,
...
FactoryOp::Open()
{
  ...
  QuotaManager::GetStorageId(
      metadata.persistenceType(), mOrigin, Client::IDB, mDatabaseId);
...
FactoryOp::OpenDirectory()
{
  ...
  nsresult rv = quotaManager->GetDirectoryForOrigin(
      persistenceType, mOrigin, getter_AddRefs(dbFile));
  if (NS_WARN_IF(NS_FAILED(rv))) {
    return rv;
  }
...
OpenDatabaseOp::LoadDatabaseInformation(mozIStorageConnection* aConnection)
{
  ...
  while (NS_SUCCEEDED((rv = stmt->ExecuteStep(&hasResult))) && hasResult) {
    ...
    if (NS_WARN_IF(!objectStoreMetadata->mIndexes.Put(
            indexId, indexMetadata, fallible))) {
      return NS_ERROR_OUT_OF_MEMORY;
    }
...
nsresult
OpenDatabaseOp::VersionChangeOp::DoDatabaseWork(DatabaseConnection* aConnection)
{
  MOZ_ASSERT(aConnection);
  aConnection->AssertIsOnConnectionThread();
  MOZ_ASSERT(mOpenDatabaseOp);
...
bool
OpenDatabaseOp::VersionChangeOp::SendFailureResult(nsresult aResultCode)
{
  AssertIsOnOwningThread();
  MOZ_ASSERT(mOpenDatabaseOp);
  MOZ_ASSERT(mOpenDatabaseOp->mState == State::DatabaseWorkVersionChange);
...
void
OpenDatabaseOp::VersionChangeOp::Cleanup()
{
  AssertIsOnOwningThread();
  MOZ_ASSERT(mOpenDatabaseOp);
  MOZ_ASSERT(mOpenDatabaseOp->mVersionChangeOp == this);
...
DeleteDatabaseOp::DoDatabaseWork()
{
  ...
  nsCOMPtr<nsIFile> directory;
  nsresult rv = quotaManager->GetDirectoryForOrigin(
      persistenceType, mOrigin, getter_AddRefs(directory));
  if (NS_WARN_IF(NS_FAILED(rv))) {
    return rv;
  }
...
nsresult
DeleteDatabaseOp::VersionChangeOp::DeleteFile(nsIFile* aDirectory,
                                              const nsAString& aFilename,
                                              QuotaManager* aQuotaManager)
{
  AssertIsOnIOThread();
...
void
DeleteDatabaseOp::VersionChangeOp::RunOnOwningThread()
{
  AssertIsOnOwningThread();
  MOZ_ASSERT(mDeleteDatabaseOp->mState == State::DatabaseWorkVersionChange);
...
void
TransactionBase::CommitOp::CommitOrRollbackAutoIncrementCounts()
{
  MOZ_ASSERT(mTransaction);
  mTransaction->AssertIsOnConnectionThread();
  MOZ_ASSERT(mTransaction->GetMode() == IDBTransaction::READ_WRITE ||
...
void
TransactionBase::CommitOp::AssertForeignKeyConsistency(
    DatabaseConnection* aConnection)
{
  MOZ_ASSERT(aConnection);
  MOZ_ASSERT(mTransaction);
...
void
TransactionBase::CommitOp::TransactionFinishedBeforeUnblock()
{
  AssertIsOnBackgroundThread();
  MOZ_ASSERT(mTransaction);
...
void
TransactionBase::CommitOp::TransactionFinishedAfterUnblock()
{
  AssertIsOnBackgroundThread();
  MOZ_ASSERT(mTransaction);
...
DeleteObjectStoreOp::DoDatabaseWork(DatabaseConnection* aConnection)
{
  ...
  if (mIsLastObjectStore) {
    ...
    rv = ObjectStoreHasIndexes(
        aConnection, mMetadata->mCommonMetadata.id(), &hasIndexes);
    if (NS_WARN_IF(NS_FAILED(rv))) {
      return rv;
    }
...
CreateIndexOp::Init(TransactionBase* aTransaction)
{
  ...
  {
    static void Destroy(void* aThreadLocal)
    {
  ...
  if (sThreadLocalIndex == kBadThreadLocalIndex) {
    if (NS_WARN_IF(PR_SUCCESS != PR_NewThreadPrivateIndex(&sThreadLocalIndex,
                                                          &Helper::Destroy))) {
      return false;
    }
  }
  ...
  nsresult rv = GetUniqueIndexTableForObjectStore(
      aTransaction, mObjectStoreId, mMaybeUniqueIndexTable);
  if (NS_WARN_IF(NS_FAILED(rv))) {
    return false;
  }
...
NormalJSContext::Init()
{
  ...
  mGlobal = JS_NewGlobalObject(
      mContext, &sGlobalClass, nullptr, JS::FireOnNewGlobalHook, options);
...
// static
auto
CreateIndexOp::ThreadLocalJSContext::GetOrCreate() -> ThreadLocalJSContext*
{
  MOZ_ASSERT(!IsOnBackgroundThread());
    ...
    mozIStorageValueArray* aValues, nsIVariant** _retval)
{
  ...
  StructuredCloneReadInfo cloneInfo;
  nsresult rv = GetStructuredCloneReadInfoFromValueArray(aValues,
                                                         /* aDataIndex */ 3,
                                                         /* aFileIdsIndex */ 2,
                                                         mOp->mFileManager,
                                                         &cloneInfo);
  ...
  if (NS_WARN_IF(
          !IDBObjectStore::DeserializeIndexValue(mCx, cloneInfo, &clone))) {
    return NS_ERROR_DOM_DATA_CLONE_ERR;
  }
                                       ...
                                       nsTArray<IndexDataValue>& aIndexValues)
{
  ...
  nsresult rv = UpdateIndexValues(
      aConnection, mObjectStoreId, aObjectStoreKey, aIndexValues);
  if (NS_WARN_IF(NS_FAILED(rv))) {
    return rv;
  }
...
ObjectStoreAddOrPutRequestOp::DoDatabaseWork(DatabaseConnection* aConnection)
{
  ...
  if (mDataOverThreshold) {
    ...
    snappy::RawCompress(
        uncompressed, uncompressedLength, compressed.get(), &compressedLength);

    uint8_t* dataBuffer = reinterpret_cast<uint8_t*>(compressed.release());
    size_t dataBufferLength = compressedLength;

    rv = stmt->BindAdoptedBlobByName(
        NS_LITERAL_CSTRING("data"), dataBuffer, dataBufferLength);
    if (NS_WARN_IF(NS_FAILED(rv))) {
      return rv;
    }
  }
  ...
  if (!mParams.indexUpdateInfos().IsEmpty()) {
    ...
    AutoTArray<IndexDataValue, 32> indexValues;
    rv = IndexDataValuesFromUpdateInfos(
        mParams.indexUpdateInfos(), mUniqueIndexTable.ref(), indexValues);
    if (NS_WARN_IF(NS_FAILED(rv))) {
      return rv;
    }
...
NS_IMETHODIMP
ObjectStoreAddOrPutRequestOp::SCInputStream::Read(char* aBuf,
                                                  uint32_t aCount,
                                                  uint32_t* _retval)
{
...
template<typename T>
void
MoveData(StructuredCloneReadInfo& aInfo, T& aResult);

template<>
void
MoveData<SerializedStructuredCloneReadInfo>(
    StructuredCloneReadInfo& aInfo, SerializedStructuredCloneReadInfo& aResult)
{
...
bool
Cursor::CursorOpBase::SendFailureResult(nsresult aResultCode)
{
  AssertIsOnOwningThread();
  MOZ_ASSERT(NS_FAILED(aResultCode));
  MOZ_ASSERT(mCursor);
  ...
  switch (mCursor->mType) {
    ...
    case OpenCursorParams::TIndexOpenKeyCursorParams: {
      ...
      mResponse = IndexKeyCursorResponse(
          mCursor->mKey, mCursor->mSortKey, mCursor->mObjectKey);
      break;
    }
...
void
Cursor::OpenOp::GetRangeKeyInfo(bool aLowerBound, Key* aKey, bool* aOpen)
{
  AssertIsOnConnectionThread();
  MOZ_ASSERT(aKey);
  MOZ_ASSERT(aKey->IsUnset());
...
nsresult
Cursor::OpenOp::DoObjectStoreDatabaseWork(DatabaseConnection* aConnection)
{
  MOZ_ASSERT(aConnection);
  aConnection->AssertIsOnConnectionThread();
  MOZ_ASSERT(mCursor);
  ...
  if (usingKeyRange) {
    GetBindingClauseForKeyRange(
        mOptionalKeyRange.get_SerializedKeyRange(), keyString, keyRangeClause);
  }
  ...
  switch (mCursor->mDirection) {
    ...
    case IDBCursor::NEXT_UNIQUE: {
      ...
      AppendConditionClause(
          keyString, currentKey, false, false, keyRangeClause);
      AppendConditionClause(
          keyString, currentKey, false, true, continueToKeyRangeClause);
      if (usingKeyRange && !upper.IsUnset()) {
        AppendConditionClause(keyString, rangeKey, true, !open, keyRangeClause);
        AppendConditionClause(
            keyString, rangeKey, true, !open, continueToKeyRangeClause);
    ...
    case IDBCursor::PREV_UNIQUE: {
      ...
      AppendConditionClause(
          keyString, currentKey, true, true, continueToKeyRangeClause);
      if (usingKeyRange && !lower.IsUnset()) {
        AppendConditionClause(
            keyString, rangeKey, false, !open, keyRangeClause);
        AppendConditionClause(
            keyString, rangeKey, false, !open, continueToKeyRangeClause);
...
nsresult
Cursor::OpenOp::DoObjectStoreKeyDatabaseWork(DatabaseConnection* aConnection)
{
  MOZ_ASSERT(aConnection);
  aConnection->AssertIsOnConnectionThread();
  MOZ_ASSERT(mCursor);
...
nsresult
Cursor::OpenOp::DoIndexDatabaseWork(DatabaseConnection* aConnection)
{
  MOZ_ASSERT(aConnection);
  aConnection->AssertIsOnConnectionThread();
  MOZ_ASSERT(mCursor);
  ...
  if (usingKeyRange) {
    GetBindingClauseForKeyRange(
        mOptionalKeyRange.get_SerializedKeyRange(), sortColumn, keyRangeClause);
  }
  ...
  if (usingKeyRange) {
    if (mCursor->IsLocaleAware()) {
      rv = BindKeyRangeToStatement(
          mOptionalKeyRange.get_SerializedKeyRange(), stmt, mCursor->mLocale);
    } else {
      rv = BindKeyRangeToStatement(mOptionalKeyRange.get_SerializedKeyRange(),
                                   stmt);
    }
...
nsresult
Cursor::OpenOp::DoIndexKeyDatabaseWork(DatabaseConnection* aConnection)
{
  MOZ_ASSERT(aConnection);
  aConnection->AssertIsOnConnectionThread();
  MOZ_ASSERT(mCursor);
...
nsresult
Cursor::OpenOp::DoDatabaseWork(DatabaseConnection* aConnection)
{
  MOZ_ASSERT(aConnection);
  aConnection->AssertIsOnConnectionThread();
  MOZ_ASSERT(mCursor);
...
nsresult
Cursor::OpenOp::SendSuccessResult()
{
  AssertIsOnOwningThread();
  MOZ_ASSERT(mCursor);
  MOZ_ASSERT(mCursor->mCurrentlyRunningOp == this);
...
nsresult
Cursor::ContinueOp::DoDatabaseWork(DatabaseConnection* aConnection)
{
  MOZ_ASSERT(aConnection);
  aConnection->AssertIsOnConnectionThread();
  MOZ_ASSERT(mCursor);
...
nsresult
Cursor::ContinueOp::SendSuccessResult()
{
  AssertIsOnOwningThread();
  MOZ_ASSERT(mCursor);
  MOZ_ASSERT(mCursor->mCurrentlyRunningOp == this);
                             ...
                             bool* aResult)
{
  ...
  nsresult rv = helper->DispatchAndReturnFileReferences(
      aRefCnt, aDBRefCnt, aSliceRefCnt, aResult);
  if (NS_WARN_IF(NS_FAILED(rv))) {
    return IPC_FAIL_NO_REASON(this);
  }
...
nsresult
FileHelper::RemoveFile(nsIFile* aFile, nsIFile* aJournalFile)
{
  nsresult rv;
...
class FileHelper::ReadCallback final : public nsIInputStreamCallback
{
 public:
  ...
  nsresult AsyncWait(nsIAsyncInputStream* aStream,
                     uint32_t aBufferSize,
