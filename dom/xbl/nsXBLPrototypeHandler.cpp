    ...
    KeyboardShortcut* aOut) const
{
  ...
  if (!nsGlobalWindowCommands::FindScrollCommand(commandText.get(), &action)) {
    ...
    *aOut = KeyboardShortcut(
        eventType, keyCode, charCode, modifiers, modifiersMask);
    return true;
  }
  ...
  *aOut = KeyboardShortcut(
      eventType, keyCode, charCode, modifiers, modifiersMask, action);
  return true;
}
...
nsresult
nsXBLPrototypeHandler::ExecuteHandler(EventTarget* aTarget, nsIDOMEvent* aEvent)
{
  ...
  rv = NS_NewJSEventHandler(
      scriptTarget, onEventAtom, typedHandler, getter_AddRefs(jsEventHandler));
  NS_ENSURE_SUCCESS(rv, rv);
...
nsresult
nsXBLPrototypeHandler::EnsureEventHandler(AutoJSAPI& jsapi,
                                          nsAtom* aName,
                                          JS::MutableHandle<JSObject*> aHandler)
{
  ...
  nsContentUtils::GetEventArgNames(
      kNameSpaceID_XBL, aName, false, &argCount, &argNames);
...
nsresult
nsXBLPrototypeHandler::DispatchXBLCommand(EventTarget* aTarget,
                                          nsIDOMEvent* aEvent)
{
  ...
  if (windowRoot) {
    ...
    windowRoot->GetControllerForCommand(
        command.get(), true, getter_AddRefs(controller));
  } else {
    controller =
  ...
  if (mEventName == nsGkAtoms::keypress &&
      mDetail == nsIDOMKeyEvent::DOM_VK_SPACE && mMisc == 1) {
...
nsXBLPrototypeHandler::DispatchXULKeyCommand(nsIDOMEvent* aEvent)
{
  ...
  nsContentUtils::DispatchXULCommand(handlerElement,
                                     true,
                                     nullptr,
                                     nullptr,
                                     isControl,
...
int32_t
nsXBLPrototypeHandler::GetMatchingKeyCode(const nsAString& aKeyName)
{
...
int32_t
nsXBLPrototypeHandler::KeyToMask(int32_t key)
{
  switch (key) {
                                          ...
                                          const char16_t* aAllowUntrusted)
{
  ...
  if (!key.IsEmpty()) {
    ...
    if ((mType & NS_HANDLER_TYPE_XUL) &&
        ...
        (mDetail == 'u' || mDetail == 'U'))
      ReportKeyConflict(
          key.get(), modifiers.get(), aKeyElement, "GTK2Conflict2");
    ...
    if ((mType & NS_HANDLER_TYPE_XUL) &&
        ...
        (('A' <= mDetail && mDetail <= 'Z') ||
         ('a' <= mDetail && mDetail <= 'z')))
      ReportKeyConflict(
          key.get(), modifiers.get(), aKeyElement, "WinConflict2");
  } else {
    key.Assign(aKeyCode);
    if (mType & NS_HANDLER_TYPE_XUL)
...
void
nsXBLPrototypeHandler::ReportKeyConflict(const char16_t* aKey,
                                         const char16_t* aModifiers,
                                         nsIContent* aKeyElement,
                                         const char* aMessageName)
{
  ...
  nsContentUtils::ReportToConsole(nsIScriptError::warningFlag,
                                  NS_LITERAL_CSTRING("XBL Prototype Handler"),
                                  doc,
