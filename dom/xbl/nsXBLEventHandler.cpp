...
nsXBLKeyEventHandler::nsXBLKeyEventHandler(nsAtom* aEventType,
                                           uint8_t aPhase,
                                           uint8_t aType)
    ...
    const IgnoreModifierState& aIgnoreModifierState)
{
  ...
  nsCOMPtr<EventTarget> target =
      aKeyEvent->AsEvent()->InternalDOMEvent()->GetCurrentTarget();
...
nsXBLKeyEventHandler::HandleEvent(nsIDOMEvent* aEvent)
{
  ...
  for (uint32_t i = 0; i < shortcutKeys.Length(); ++i) {
    ...
    if (ExecuteMatchedHandlers(
            key, shortcutKeys[i].mCharCode, ignoreModifierState)) {
      return NS_OK;
    }
  }
  return NS_OK;
