...
nsresult
nsXBLContentSink::Init(nsIDocument* aDoc, nsIURI* aURI, nsISupports* aContainer)
{
  nsresult rv;
                              ...
                              bool* _retval)
{
  ...
  return nsXMLContentSink::ReportError(
      aErrorText, aSourceText, aError, _retval);
}
...
nsXBLContentSink::AddMember(nsXBLProtoImplMember* aMember)
{
  ...
  mImplMember =
      aMember;  // Adjust our pointer to point to the new last member in the chain.
}
...
nsXBLContentSink::AddField(nsXBLProtoImplField* aField)
{
  ...
  mImplField =
      aField;  // Adjust our pointer to point to the new last field in the chain.
}
                                     ...
                                     uint32_t aLineNumber)
{
  nsresult rv = nsXMLContentSink::HandleStartElement(
      aName, aAtts, aAttsCount, aLineNumber);
...
nsXBLContentSink::ConstructHandler(const char16_t** aAtts, uint32_t aLineNumber)
{
  ...
  newHandler = new nsXBLPrototypeHandler(event,
                                         phase,
                                         action,
                                         command,
                                         keycode,
...
void
nsXBLContentSink::ConstructProperty(const char16_t** aAtts,
                                    uint32_t aLineNumber)
{
  const char16_t* name = nullptr;
  const char16_t* readonly = nullptr;
...
nsresult
nsXBLContentSink::CreateElement(const char16_t** aAtts,
                                uint32_t aAttsCount,
...
#endif
    return nsXMLContentSink::CreateElement(aAtts,
                                           aAttsCount,
                                           aNodeInfo,
                                           aLineNumber,
                                           aResult,
...
nsresult
nsXBLContentSink::AddAttributes(const char16_t** aAtts, nsIContent* aContent)
{
                                              ...
                                              nsXULPrototypeElement* aElement)
{
  ...
  for (i = 0; i < aAttsCount; ++i) {
    int32_t nameSpaceID;
    nsContentUtils::SplitExpatName(aAtts[i * 2],
                                   getter_AddRefs(prefix),
                                   getter_AddRefs(localName),
                                   &nameSpaceID);
    ...
    if (nameSpaceID == kNameSpaceID_None) {
      ...
      RefPtr<NodeInfo> ni;
      ni = mNodeInfoManager->GetNodeInfo(
          localName, prefix, nameSpaceID, nsIDOMNode::ATTRIBUTE_NODE);
      attrs[i].mName.SetTo(ni);
    }

    rv = aElement->SetAttrAt(
        i, nsDependentString(aAtts[i * 2 + 1]), mDocumentURI);
    NS_ENSURE_SUCCESS(rv, rv);
  }
