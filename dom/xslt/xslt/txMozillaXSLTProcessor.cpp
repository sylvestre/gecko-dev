  ...
  *aHandler = nullptr;
  switch (aFormat->mMethod) {
    ...
    case eHTMLOutput: {
      ...
      nsresult rv = handler->createResultDocument(
          aName, aNsID, mSourceDocument, mDocumentIsData);
      if (NS_SUCCEEDED(rv)) {
        *aHandler = handler.forget();
      }
...
{
 public:
  txXSLTParamContext(txNamespaceMap* aResolver,
                     const txXPathNode& aContext,
  ...
  nsresult resolveFunctionCall(nsAtom* aName,
                               int32_t aID,
                               FunctionCall** aFunction)
  {
    return NS_ERROR_XPATH_UNKNOWN_FUNCTION;
  }
  bool caseInsensitiveNameTests() { return false; }
  void SetErrorOffset(uint32_t aOffset) {}

  // txIEvalContext
  nsresult getVariable(int32_t aNamespace,
                       nsAtom* aLName,
  ...
  void* getPrivateContext() { return nullptr; }
  txResultRecycler* recycler() { return mRecycler; }
  void receiveError(const nsAString& aMsg, nsresult aRes) {}
  const txXPathNode& getContextNode() { return mContext; }
  uint32_t size() { return 1; }
  uint32_t position() { return 1; }
                                     ...
                                     nsIDOMNode* aContext)
{
  ...
  if (!aSelect.IsVoid()) {
    ...
    txXSLTParamContext paramContext(
        &mParamNamespaceMap, *contextNode, mRecycler);

    // Parse
    nsAutoPtr<Expr> expr;
    rv = txExprParser::createExpr(
        aSelect, &paramContext, getter_Transfers(expr));
    NS_ENSURE_SUCCESS(rv, rv);
...
txMozillaXSLTProcessor::ImportStylesheet(nsIDOMNode* aStyle)
{
  ...
  NS_ENSURE_TRUE(styleNode && (styleNode->IsElement() ||
                               styleNode->IsNodeOfType(nsINode::eDOCUMENT)),
                 NS_ERROR_INVALID_ARG);

  nsresult rv =
      TX_CompileStylesheet(styleNode, this, getter_AddRefs(mStylesheet));
                                       ...
                                       bool aCreateDataDocument)
{
  ...
  txToDocHandlerFactory handlerFactory(
      &es, sourceDOMDocument, mObserver, aCreateDataDocument);
      ...
      if (bundle) {
        ...
        if (mStylesheet) {
          bundle->FormatStringFromName(
              "TransformError", error, 1, errorMessage);
        } else {
                                            ...
                                            mozilla::ErrorResult& aRv)
{
  ...
  aRv =
      TransformToFragment(source.AsDOMNode(), domDoc, getter_AddRefs(fragment));
                                     ...
                                     mozilla::ErrorResult& aRv)
{
  ...
  aRv = nsContentUtils::XPConnect()->JSToVariant(
      aCx, aValue, getter_AddRefs(val));
  if (aRv.Failed()) {
    return;
  }
