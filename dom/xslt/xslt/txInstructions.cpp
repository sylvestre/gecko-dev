...
txApplyTemplates::txApplyTemplates(const txExpandedName& aMode) : mMode(aMode)
{
}
...
txAttribute::txAttribute(nsAutoPtr<Expr>&& aName,
  ...
  switch (txXPathNodeUtils::getNodeType(aNode)) {
    case txXPathNodeType::ATTRIBUTE_NODE: {
      ...
      return aEs.mResultHandler->attribute(
          txXPathNodeUtils::getPrefix(aNode),
          localName,
          nullptr,
      ...
      if (walker.moveToFirstAttribute()) {
        do {
          ...
          rv = aEs.mResultHandler->attribute(
              txXPathNodeUtils::getPrefix(attr),
              localName,
              nullptr,
  ...
  switch (txXPathNodeUtils::getNodeType(node)) {
    ...
    case txXPathNodeType::ELEMENT_NODE: {
      ...
      rv = aEs.mResultHandler->startElement(
          txXPathNodeUtils::getPrefix(node),
          localName,
          nullptr,
...
txLREAttribute::txLREAttribute(int32_t aNamespaceID,
                               nsAtom* aLocalName,
                               nsAtom* aPrefix,
...
txLREAttribute::execute(txExecutionState& aEs)
{
  ...
  return aEs.mResultHandler->attribute(
      mPrefix, mLocalName, mLowercaseLocalName, mNamespaceID, valueStr);
}
...
txNumber::txNumber(txXSLTNumber::LevelType aLevel,
                   nsAutoPtr<txPattern>&& aCount,
                   nsAutoPtr<txPattern>&& aFrom,
                   nsAutoPtr<Expr>&& aValue,
                   nsAutoPtr<Expr>&& aFormat,
...
{
  nsAutoString res;
  nsresult rv = txXSLTNumber::createNumber(mValue,
                                           mCount,
                                           mFrom,
                                           mLevel,
                                           mGroupingSize,
...
txPushNewContext::execute(txExecutionState& aEs)
{
  ...
  for (i = 0; i < count; ++i) {
    ...
    rv = sorter.addSortElement(sort.mSelectExpr,
                               sort.mLangExpr,
                               sort.mDataTypeExpr,
                               sort.mOrderExpr,
                               sort.mCaseOrderExpr,
...
txPushStringHandler::txPushStringHandler(bool aOnlyText) : mOnlyText(aOnlyText)
{
}
...
txRemoveVariable::txRemoveVariable(const txExpandedName& aName) : mName(aName)
{
}
...
txStartElement::execute(txExecutionState& aEs)
{
  ...
  if (nsId != kNameSpaceID_Unknown) {
    rv = aEs.mResultHandler->startElement(
        prefix, Substring(name, lnameStart), nsId);
  } else {
