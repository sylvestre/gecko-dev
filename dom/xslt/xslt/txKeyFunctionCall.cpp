...
txKeyFunctionCall::evaluate(txIEvalContext* aContext, txAExprResult** aResult)
{
  ...
  if (exprResult->getResultType() == txAExprResult::NODESET &&
    ...
    for (i = 0; i < nodeSet->size(); ++i) {
      ...
      rv = es->getKeyNodes(keyName,
                           walker.getCurrentPosition(),
                           val,
                           i == 0,
    ...
    rv = es->getKeyNodes(
        keyName, walker.getCurrentPosition(), val, true, getter_AddRefs(res));
    NS_ENSURE_SUCCESS(rv, rv);
  }
 ...
 */
bool
txXSLKey::addKey(nsAutoPtr<txPattern>&& aMatch, nsAutoPtr<Expr>&& aUse)
{
 ...
 */
nsresult
txXSLKey::indexSubtreeRoot(const txXPathNode& aRoot,
                           txKeyValueHash& aKeyValueHash,
                           txExecutionState& aEs)
{
  txKeyValueHashKey key(
      mName, txXPathNodeUtils::getUniqueIdentifier(aRoot), EmptyString());
 ...
 */
nsresult
txXSLKey::indexTree(const txXPathNode& aNode,
 ...
 */
nsresult
txXSLKey::testNode(const txXPathNode& aNode,
                   txKeyValueHashKey& aKey,
                   txKeyValueHash& aKeyValueHash,
                   txExecutionState& aEs)
{
