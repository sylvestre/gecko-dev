                                  ...
                                  uint32_t aCount)
{
  ...
  return mListener->OnDataAvailable(
      aRequest, mParser, aInputStream, aOffset, aCount);
}
...
nsresult
txCompileObserver::startLoad(nsIURI* aUri,
                             txStylesheetCompiler* aCompiler,
                               ...
                               txStylesheetCompiler* aCompiler)
{
  ...
  if (mProcessor) {
    source = do_QueryInterface(mProcessor->GetSourceContentModel());
  }
...
void
txSyncCompileObserver::onDoneCompiling(txStylesheetCompiler* aCompiler,
                                       nsresult aResult,
                                       const char16_t* aErrorText,
                                       const char16_t* aParam)
{
