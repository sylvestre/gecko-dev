...
nsresult
txMozillaTextOutput::attribute(nsAtom* aPrefix,
                               nsAtom* aLocalName,
                                          ...
                                          bool aLoadedAsData)
{
  ...
  if (!observer) {
    ...
    rv = mTextParent->SetAttr(kNameSpaceID_None,
                              nsGkAtoms::id,
...
nsresult
txMozillaTextOutput::startElement(nsAtom* aPrefix,
                                  nsAtom* aLocalName,
                                  nsAtom* aLowercaseLocalName,
                                  int32_t aNsID)
{
...
void
txMozillaTextOutput::getOutputDocument(nsIDOMDocument** aDocument)
{
  CallQueryInterface(mDocument, aDocument);
}

nsresult
txMozillaTextOutput::createXHTMLElement(nsAtom* aName, nsIContent** aResult)
{
