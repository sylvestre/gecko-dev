...
txResultTreeFragment::txResultTreeFragment(nsAutoPtr<txResultBuffer>&& aBuffer)
    : txAExprResult(nullptr), mBuffer(Move(aBuffer))
{
}

short
txResultTreeFragment::getResultType()
{
...
bool
txResultTreeFragment::booleanValue()
{
  return true;
}

double
txResultTreeFragment::numberValue()
{
  if (!mBuffer) {
    return 0;
  }
...
nsresult
txResultTreeFragment::flushToHandler(txAXMLEventHandler* aHandler)
{
  if (!mBuffer) {
