...
nsresult
txStylesheetCompiler::startElement(int32_t aNamespaceID,
                                   nsAtom* aLocalName,
                                   nsAtom* aPrefix,
                                   ...
                                   int32_t aAttrCount)
{
  ...
  return startElementInternal(
      aNamespaceID, aLocalName, aPrefix, aAttributes, aAttrCount);
}
                                   ...
                                   int32_t aAttrCount)
{
  ...
  return startElementInternal(
      namespaceID, localname, prefix, atts.get(), aAttrCount);
}
                                           ...
                                           int32_t aAttrCount)
{
  ...
  do {
    ...
    rv = (handler->mStartFunction)(
        aNamespaceID, aLocalName, aPrefix, aAttributes, aAttrCount, *this);
                              ...
                              txStylesheetCompiler* aCompiler)
{
  ...
  return mObserver ? mObserver->loadURI(
                         aUri, aReferrerUri, aReferrerPolicy, aCompiler)
                   : NS_ERROR_FAILURE;
}
...
txStylesheetCompilerState::pushPtr(void* aPtr, enumStackType aType)
{
  ...
  MOZ_LOG(
      txLog::xslt, LogLevel::Debug, ("pushPtr: 0x%x type %u\n", aPtr, aType));
#endif
...
nsresult
txStylesheetCompilerState::openInstructionContainer(
...
nsresult
txStylesheetCompilerState::addInstruction(
...
nsresult
txStylesheetCompilerState::loadImportedStylesheet(
...
nsresult
txStylesheetCompilerState::resolveNamespacePrefix(nsAtom* aPrefix, int32_t& aID)
{
...
nsresult
txErrorFunctionCall::evaluate(txIEvalContext* aContext, txAExprResult** aResult)
{
  *aResult = nullptr;
...
static nsresult
TX_ConstructXSLTFunction(nsAtom* aName,
             ...
             FunctionCall** aResult)
{
  ...
  return TX_ResolveFunctionCallXPCOM(
      map->mContractID, aNamespaceID, aName, nullptr, aResult);
}
...
nsresult
txStylesheetCompilerState::resolveFunctionCall(nsAtom* aName,
                                               int32_t aID,
                                               FunctionCall** aFunction)
{
  *aFunction = nullptr;
