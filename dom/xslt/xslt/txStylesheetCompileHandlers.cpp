                 ...
                 txStylesheetCompilerState& aState)
{
  ...
  nsresult rv = getStyleAttr(aAttributes,
                             aAttrCount,
                             aInXSLTNS ? kNameSpaceID_XSLT : kNameSpaceID_None,
                             nsGkAtoms::useAttributeSets,
                             false,
                             &attr);
  if (!attr) {
    return rv;
                           ...
                           int32_t aNamespaceID)
{
  ...
  nsresult rv = getStyleAttr(aAttributes,
                             aAttrCount,
                             aNamespaceID,
                             nsGkAtoms::excludeResultPrefixes,
                             false,
                             &attr);
  if (!attr) {
    return rv;
             ...
             txExpandedName& aExpName)
{
  aExpName.reset();
  txStylesheetAttr* attr = nullptr;
  nsresult rv = getStyleAttr(
      aAttributes, aAttrCount, kNameSpaceID_None, aName, aRequired, &attr);
  if (!attr) {
    return rv;
  }
               ...
               nsAutoPtr<txPattern>& aPattern)
{
  ...
  rv = txPatternParser::createPattern(
      attr->mValue, &aState, getter_Transfers(aPattern));
             ...
             txThreeState& aRes)
{
  aRes = eNotSet;
  RefPtr<nsAtom> atom;
  nsresult rv = getAtomAttr(
      aAttributes, aAttrCount, aName, aRequired, aState, getter_AddRefs(atom));
  if (!atom) {
    return rv;
  }
...
void
clearAttributes(txStylesheetAttr* aAttributes, int32_t aAttrCount)
{
  int32_t i;
  for (i = 0; i < aAttrCount; ++i) {
                    ...
                    txStylesheetCompilerState& aState)
{
  ...
  nsresult rv = getStyleAttr(
      aAttributes, aAttrCount, kNameSpaceID_None, nsGkAtoms::id, false, &attr);
  NS_ENSURE_SUCCESS(rv, rv);

  rv = parseExcludeResultPrefixes(aAttributes, aAttrCount, kNameSpaceID_None);
  NS_ENSURE_SUCCESS(rv, rv);

  rv = getStyleAttr(aAttributes,
                    aAttrCount,
                    kNameSpaceID_None,
                    nsGkAtoms::version,
                    true,
                       ...
                       txStylesheetCompilerState& aState)
{
  ...
  nsAutoPtr<txTemplateItem> templ(
      new txTemplateItem(Move(match), nullExpr, nullExpr, prio));
  ...
  return txFnStartLRE(
      aNamespaceID, aLocalName, aPrefix, aAttributes, aAttrCount, aState);
}
                      ...
                      txStylesheetCompilerState& aState)
{
  ...
  txExpandedName name;
  rv = getQNameAttr(
      aAttributes, aAttrCount, nsGkAtoms::name, true, aState, name);
  NS_ENSURE_SUCCESS(rv, rv);
                       ...
                       txStylesheetCompilerState& aState)
{
  ...
  txExpandedName name;
  rv = getQNameAttr(
      aAttributes, aAttrCount, nsGkAtoms::name, false, aState, name);
  NS_ENSURE_SUCCESS(rv, rv);

  nsAutoPtr<txDecimalFormat> format(new txDecimalFormat);
  rv = getCharAttr(aAttributes,
                   aAttrCount,
  ...
  rv = getStyleAttr(
      aAttributes, aAttrCount, kNameSpaceID_None, nsGkAtoms::NaN, false, &attr);
  NS_ENSURE_SUCCESS(rv, rv);
  ...
  rv = getCharAttr(
      aAttributes, aAttrCount, nsGkAtoms::digit, false, aState, format->mDigit);
  NS_ENSURE_SUCCESS(rv, rv);
                ...
                txStylesheetCompilerState& aState)
{
  ...
  txStylesheetAttr* attr = nullptr;
  rv = getStyleAttr(
      aAttributes, aAttrCount, kNameSpaceID_None, nsGkAtoms::href, true, &attr);
  NS_ENSURE_SUCCESS(rv, rv);
             ...
             txStylesheetCompilerState& aState)
{
  ...
  rv = getPatternAttr(
      aAttributes, aAttrCount, nsGkAtoms::match, true, aState, match);
  NS_ENSURE_SUCCESS(rv, rv);
                ...
                txStylesheetCompilerState& aState)
{
  ...
  rv = getQNameAttr(
      aAttributes, aAttrCount, nsGkAtoms::method, false, aState, methodExpName);
  NS_ENSURE_SUCCESS(rv, rv);
  ...
  getStyleAttr(aAttributes,
               aAttrCount,
               kNameSpaceID_None,
               nsGkAtoms::cdataSectionElements,
               false,
               &attr);
  if (attr) {
    ...
    while (tokens.hasMoreTokens()) {
      ...
      rv = qname->init(
          tokens.nextToken(), aState.mElementContext->mMappings, false);
      NS_ENSURE_SUCCESS(rv, rv);
                    ...
                    txStylesheetCompilerState& aState)
{
  ...
  while (tokenizer.hasMoreTokens()) {
    ...
    rv = XMLUtils::splitQName(
        name, getter_AddRefs(prefix), getter_AddRefs(localName));
    if (NS_FAILED(rv)) {
      // check for "*" or "prefix:*"
                  ...
                  txStylesheetCompilerState& aState)
{
  ...
  txExpandedName mode;
  rv = getQNameAttr(
      aAttributes, aAttrCount, nsGkAtoms::mode, false, aState, mode);
  NS_ENSURE_SUCCESS(rv, rv);

  double prio = UnspecifiedNaN<double>();
  rv = getNumberAttr(
      aAttributes, aAttrCount, nsGkAtoms::priority, false, aState, prio);
  NS_ENSURE_SUCCESS(rv, rv);

  nsAutoPtr<txPattern> match;
  rv = getPatternAttr(
      aAttributes, aAttrCount, nsGkAtoms::match, name.isNull(), aState, match);
  NS_ENSURE_SUCCESS(rv, rv);

  nsAutoPtr<txTemplateItem> templ(
      new txTemplateItem(Move(match), name, mode, prio));
  aState.openInstructionContainer(templ);
  rv = aState.addToplevelItem(templ);
  NS_ENSURE_SUCCESS(rv, rv);
                     ...
                     txStylesheetCompilerState& aState)
{
  ...
  rv = getExprAttr(
      aAttributes, aAttrCount, nsGkAtoms::select, false, aState, select);
  NS_ENSURE_SUCCESS(rv, rv);

  nsAutoPtr<txVariableItem> var(
      new txVariableItem(name, Move(select), aLocalName == nsGkAtoms::param));
             ...
             txStylesheetCompilerState& aState)
{
  ...
  for (i = 0; i < aAttrCount; ++i) {
    ...
    instr = new txLREAttribute(
        attr->mNamespaceID, attr->mLocalName, attr->mPrefix, Move(avt));
    rv = aState.addInstruction(Move(instr));
    NS_ENSURE_SUCCESS(rv, rv);
  }
                   ...
                   txStylesheetCompilerState& aState)
{
  ...
  rv = getAVTAttr(
      aAttributes, aAttrCount, nsGkAtoms::_namespace, false, aState, nspace);
  NS_ENSURE_SUCCESS(rv, rv);

  instr = new txAttribute(
      Move(name), Move(nspace), aState.mElementContext->mMappings);
  rv = aState.pushObject(instr);
  NS_ENSURE_SUCCESS(rv, rv);
                ...
                txStylesheetCompilerState& aState)
{
  ...
  nsAutoPtr<Expr> select;
  rv = getExprAttr(
      aAttributes, aAttrCount, nsGkAtoms::select, true, aState, select);
  NS_ENSURE_SUCCESS(rv, rv);
            ...
            txStylesheetCompilerState& aState)
{
  ...
  nsAutoPtr<txConditionalGoto> condGoto(
      new txConditionalGoto(Move(test), nullptr));
                 ...
                 txStylesheetCompilerState& aState)
{
  ...
  txThreeState term;
  rv = getYesNoAttr(
      aAttributes, aAttrCount, nsGkAtoms::terminate, false, aState, term);
  NS_ENSURE_SUCCESS(rv, rv);
                ...
                txStylesheetCompilerState& aState)
{
  ...
  rv = getPatternAttr(
      aAttributes, aAttrCount, nsGkAtoms::count, false, aState, count);
  NS_ENSURE_SUCCESS(rv, rv);

  nsAutoPtr<txPattern> from;
  rv = getPatternAttr(
      aAttributes, aAttrCount, nsGkAtoms::from, false, aState, from);
  NS_ENSURE_SUCCESS(rv, rv);

  nsAutoPtr<Expr> value;
  rv = getExprAttr(
      aAttributes, aAttrCount, nsGkAtoms::value, false, aState, value);
  NS_ENSURE_SUCCESS(rv, rv);

  nsAutoPtr<Expr> format;
  rv = getAVTAttr(
      aAttributes, aAttrCount, nsGkAtoms::format, false, aState, format);
  NS_ENSURE_SUCCESS(rv, rv);
  ...
  nsAutoPtr<txInstruction> instr(new txNumber(level,
                                              Move(count),
                                              Move(from),
                                              Move(value),
                                              Move(format),
              ...
              txStylesheetCompilerState& aState)
{
  ...
  rv = getAVTAttr(
      aAttributes, aAttrCount, nsGkAtoms::dataType, false, aState, dataType);
  NS_ENSURE_SUCCESS(rv, rv);

  nsAutoPtr<Expr> order;
  rv = getAVTAttr(
      aAttributes, aAttrCount, nsGkAtoms::order, false, aState, order);
  NS_ENSURE_SUCCESS(rv, rv);

  nsAutoPtr<Expr> caseOrder;
  rv = getAVTAttr(
      aAttributes, aAttrCount, nsGkAtoms::caseOrder, false, aState, caseOrder);
  NS_ENSURE_SUCCESS(rv, rv);

  rv = aState.mSorter->addSort(
      Move(select), Move(lang), Move(dataType), Move(order), Move(caseOrder));
  NS_ENSURE_SUCCESS(rv, rv);
