...
void
txMozillaXMLOutput::getOutputDocument(nsIDOMDocument** aDocument)
{
  CallQueryInterface(mDocument, aDocument);
}

nsresult
txMozillaXMLOutput::processingInstruction(const nsString& aTarget,
                                 ...
                                 const int32_t aNsID)
{
  ...
  if (mOutputFormat.mMethod == eHTMLOutput && aNsID == kNameSpaceID_None) {
    ...
    return startElementInternal(
        nullptr, aLowercaseLocalName, kNameSpaceID_XHTML);
  }
                                         ...
                                         int32_t aNsID)
{
  ...
  RefPtr<NodeInfo> ni = mNodeInfoManager->GetNodeInfo(
      aLocalName, aPrefix, aNsID, nsIDOMNode::ELEMENT_NODE);

  NS_NewElement(getter_AddRefs(mOpenedElement),
                ni.forget(),
...
txMozillaXMLOutput::startHTMLElement(nsIContent* aElement, bool aIsHTML)
{
  ...
  if (aElement->IsHTMLElement(nsGkAtoms::table) && aIsHTML) {
    ...
    rv = meta->SetAttr(kNameSpaceID_None,
                       nsGkAtoms::httpEquiv,
    ...
    rv = meta->SetAttr(
        kNameSpaceID_None, nsGkAtoms::content, metacontent, false);
    NS_ENSURE_SUCCESS(rv, rv);
...
void
txMozillaXMLOutput::processHTTPEquiv(nsAtom* aHeader, const nsString& aValue)
{
...
nsresult
txMozillaXMLOutput::createResultDocument(const nsAString& aName,
                                         int32_t aNsID,
                                         ...
                                         bool aLoadedAsData)
{
  ...
  if (mOutputFormat.mMethod == eXMLOutput &&
    ...
    mDocument->SetXMLDeclaration(
        kOneDotZero, mOutputFormat.mEncoding.get(), standalone);
  }
...
nsresult
txMozillaXMLOutput::createHTMLElement(nsAtom* aName, nsIContent** aResult)
{
