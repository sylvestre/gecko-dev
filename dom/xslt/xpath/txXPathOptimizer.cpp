...
class txEarlyEvalContext : public txIEvalContext
{
 public:
  ...
  void* getPrivateContext() { MOZ_CRASH("shouldn't depend on this context"); }
  ...
  uint32_t size() { MOZ_CRASH("shouldn't depend on this context"); }
  uint32_t position() { MOZ_CRASH("shouldn't depend on this context"); }
