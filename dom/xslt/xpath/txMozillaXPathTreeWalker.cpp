...
/* static */
bool
txXPathNodeUtils::getAttr(const txXPathNode& aNode,
                          nsAtom* aLocalName,
                          int32_t aNSID,
                          nsAString& aValue)
{
...
txXPathNodeUtils::appendNodeValue(const txXPathNode& aNode, nsAString& aResult)
{
  if (aNode.isAttribute()) {
    ...
    if (aResult.IsEmpty()) {
      aNode.Content()->GetAttr(name->NamespaceID(), name->LocalName(), aResult);
    } else {
      nsAutoString result;
      aNode.Content()->GetAttr(name->NamespaceID(), name->LocalName(), result);
  ...
  if (aNode.isDocument() || aNode.mNode->IsElement() ||
    ...
    nsContentUtils::AppendNodeTextContent(
        aNode.mNode, true, aResult, mozilla::fallible);
                                  ...
                                  const txXPathNode& aOtherNode)
{
  ...
  while (node && otherNode) {
    ...
    if (parent == otherParent) {
      ...
      return parent->IndexOf(node) < parent->IndexOf(otherNode) ? -1 : 1;
    }
