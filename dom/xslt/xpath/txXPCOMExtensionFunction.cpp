...
txFunctionEvaluationContext::txFunctionEvaluationContext(
    txIEvalContext* aContext, nsISupports* aState)
    : mContext(aContext), mState(aState)
{
}
...
{
 public:
  txXPCOMExtensionFunctionCall(nsISupports* aHelper,
                               const nsIID& aIID,
...
txXPCOMExtensionFunctionCall::txXPCOMExtensionFunctionCall(
    nsISupports* aHelper,
    const nsIID& aIID,
    uint16_t aMethodIndex,
#ifdef TX_TO_STRING
...
{
 public:
  txInterfacesArrayHolder(nsIID** aArray, uint32_t aCount)
      : mArray(aArray), mCount(aCount)
  {
  }
...
static nsresult
LookupFunction(const char* aContractID,
               nsAtom* aName,
               nsIID& aIID,
               uint16_t& aMethodIndex,
               nsISupports** aHelper)
{
  ...
  while ((letter = *name)) {
    if (letter == '-') {
      ...
      MOZ_ASSERT(nsCRT::IsAscii(letter), "invalid static_cast coming up");
      methodName.Append(upperNext ? nsCRT::ToUpper(static_cast<char>(letter))
                                  : letter);
      upperNext = false;
    }
    ++name;
  ...
  for (i = 0; i < iidCount; ++i) {
    ...
    rv =
        info->GetMethodInfoForName(methodName.get(), &methodIndex, &methodInfo);
    if (NS_SUCCEEDED(rv)) {
      ...
      if (methodInfo->IsNotXPCOM() || methodInfo->IsHidden() ||
          paramCount == 0 || !methodInfo->GetParam(paramCount - 1).IsRetval()) {
...
/* static */
nsresult
TX_ResolveFunctionCallXPCOM(const nsCString& aContractID,
                            ...
                            FunctionCall** aFunction)
{
  ...
  nsresult rv = LookupFunction(
      aContractID.get(), aName, iid, methodIndex, getter_AddRefs(helper));
  NS_ENSURE_SUCCESS(rv, rv);
  ...
  *aFunction = new txXPCOMExtensionFunctionCall(helper,
                                                iid,
                                                methodIndex,
#ifdef TX_TO_STRING
                                                aName,
...
{
 public:
  txParamArrayHolder() : mCount(0) {}
  txParamArrayHolder(txParamArrayHolder&& rhs)
      : mArray(mozilla::Move(rhs.mArray)), mCount(rhs.mCount)
  {
    rhs.mCount = 0;
  }
  ~txParamArrayHolder();
  ...
  operator nsXPTCVariant*() const { return mArray.get(); }
                                       ...
                                       txAExprResult** aResult)
{
  ...
  for (i = paramStart; i < inArgs; ++i) {
    ...
    switch (type) {
      ...
      case eOBJECT: {
        ...
        nsCOMPtr<txIXPathObject> adaptor = new txXPathObjectAdaptor(exprRes);
        if (!adaptor) {
          return NS_ERROR_OUT_OF_MEMORY;
        }
  ...
  switch (returnType) {
    ...
    case eNUMBER: {
      return aContext->recycler()->getNumberResult(returnParam.val.d, aResult);
    }
    case eSTRING: {
      nsString* returned = static_cast<nsString*>(returnParam.val.p);
      return aContext->recycler()->getStringResult(*returned, aResult);
    }
    case eOBJECT: {
      txIXPathObject* object = static_cast<txIXPathObject*>(returnParam.val.p);
