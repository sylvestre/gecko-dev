...
const txXPathNode&
txForwardContext::getContextNode()
{
  return mContextNode;
}

uint32_t
txForwardContext::size()
{
  return (uint32_t)mContextSet->size();
}

uint32_t
txForwardContext::position()
{
...
nsresult
txForwardContext::getVariable(int32_t aNamespace,
                              nsAtom* aLName,
...
void*
txForwardContext::getPrivateContext()
{
  NS_ASSERTION(mInner, "mInner is null!!!");
...
void
txForwardContext::receiveError(const nsAString& aMsg, nsresult aRes)
{
  NS_ASSERTION(mInner, "mInner is null!!!");
#ifdef DEBUG
