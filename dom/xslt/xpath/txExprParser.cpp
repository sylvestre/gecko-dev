 ...
 */
nsresult
txExprParser::createBinaryExpr(nsAutoPtr<Expr>& left,
...
nsresult
txExprParser::createExpr(txExprLexer& lexer,
                         txIParseContext* aContext,
                         Expr** aResult)
{
  *aResult = nullptr;
  ...
  while (NS_SUCCEEDED(rv) && !exprs.isEmpty()) {
    ...
    nsAutoPtr<Expr> right(Move(expr));
    rv = createBinaryExpr(
        left, right, static_cast<Token*>(ops.pop()), getter_Transfers(expr));
  }
  // clean up on error
  while (!exprs.isEmpty()) {
...
nsresult
txExprParser::createFilterOrStep(txExprLexer& lexer,
                                 txIParseContext* aContext,
                                 Expr** aResult)
{
  *aResult = nullptr;
...
nsresult
txExprParser::createFunctionCall(txExprLexer& lexer,
                                 txIParseContext* aContext,
                                 Expr** aResult)
{
  *aResult = nullptr;
  ...
  if (!fnCall) {
    rv = aContext->resolveFunctionCall(
        lName, namespaceID, getter_Transfers(fnCall));
...
nsresult
txExprParser::createLocationStep(txExprLexer& lexer,
                                 txIParseContext* aContext,
                                 Expr** aExpr)
{
  *aExpr = nullptr;
 ...
 */
nsresult
txExprParser::createPathExpr(txExprLexer& lexer,
                             txIParseContext* aContext,
                             Expr** aResult)
{
  *aResult = nullptr;
 ...
 */
nsresult
txExprParser::createUnionExpr(txExprLexer& lexer,
                              txIParseContext* aContext,
                              Expr** aResult)
{
  *aResult = nullptr;
 ...
 */
nsresult
txExprParser::parseParameters(FunctionCall* aFnCall,
                              txExprLexer& lexer,
                              txIParseContext* aContext)
{
  if (lexer.peek()->mType == Token::R_PAREN) {
                           ...
                           bool aIsNameTest)
{
  ...
  if (idx > 0) {
    ...
    *aLocalName =
        NS_Atomize(
            Substring(aQName, (uint32_t)idx + 1, aQName.Length() - (idx + 1)))
            .take();
    if (!*aLocalName) {
      NS_RELEASE(*aPrefix);
      return NS_ERROR_OUT_OF_MEMORY;
