                         ...
                         nsISupports** aResult)
{
  ...
  nsAutoPtr<XPathExpression> expression(
      CreateExpression(aExpression, resolver, rv));
  if (rv.Failed()) {
  ...
  RefPtr<XPathResult> result = expression->Evaluate(
      *node, aType, static_cast<XPathResult*>(inResult.get()), rv);
  if (rv.Failed()) {
    return rv.StealNSResult();
  }
                                 ...
                                 ErrorResult& aRv)
{
  ...
  aRv = txExprParser::createExpr(
      PromiseFlatString(aExpression), aContext, getter_Transfers(expression));
  if (aRv.Failed()) {
...
XPathEvaluator::Constructor(const GlobalObject& aGlobal, ErrorResult& rv)
{
...
XPathEvaluator::Evaluate(JSContext* aCx,
...
nsresult
XPathEvaluatorParseContext::resolveNamespacePrefix(nsAtom* aPrefix,
                                                   int32_t& aID)
{
...
bool
XPathEvaluatorParseContext::caseInsensitiveNameTests()
{
