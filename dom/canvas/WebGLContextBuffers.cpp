...
WebGLContext::ValidateIndexedBufferSlot(const char* funcName,
                                        GLenum target,
                                        GLuint index)
{
...
bool
WebGLContext::ValidateIndexedBufferBinding(
    const char* funcName,
    GLenum target,
    GLuint index,
...
WebGLContext::BindBufferBase(GLenum target, GLuint index, WebGLBuffer* buffer)
{
  ...
  if (!ValidateIndexedBufferBinding(
          funcName, target, index, &genericBinding, &indexedBinding)) {
    return;
  }
...
void
WebGLContext::BindBufferRange(GLenum target,
                              GLuint index,
                              WebGLBuffer* buffer,
                              WebGLintptr offset,
                              WebGLsizeiptr size)
{
  ...
  switch (target) {
    case LOCAL_GL_TRANSFORM_FEEDBACK_BUFFER:
      if (offset % 4 != 0 || size % 4 != 0) {
        ErrorInvalidValue(
            "%s: For %s, `offset` and `size` must be multiples of 4.",
            funcName,
  ...
  gl->fBindBufferRange(
      target, index, buffer ? buffer->mGLName : 0, offset, size);
...
void
WebGLContext::BufferDataImpl(GLenum target,
                             size_t dataLen,
                             const uint8_t* data,
                             GLenum usage)
{
                         ...
                         GLuint srcElemCountOverride)
{
  ...
  uint8_t* bytes;
  size_t byteLen;
  if (!ValidateArrayBufferView(funcName,
                               src,
                               srcElemOffset,
...
void
WebGLContext::BufferSubDataImpl(GLenum target,
...
void
WebGLContext::BufferSubData(GLenum target,
                            WebGLsizeiptr dstByteOffset,
                            const dom::ArrayBuffer& src)
{
  ...
  src.ComputeLengthAndData();
  BufferSubDataImpl(
      target, dstByteOffset, src.LengthAllowShared(), src.DataAllowShared());
}
