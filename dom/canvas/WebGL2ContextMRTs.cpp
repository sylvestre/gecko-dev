...
bool
WebGL2Context::ValidateClearBuffer(const char* funcName,
                                   GLenum buffer,
                                   GLint drawBuffer,
                                   ...
                                   GLenum funcType)
{
  ...
  if (drawBuffer < 0 || drawBuffer > maxDrawBuffer) {
    ErrorInvalidValue(
        "%s: Invalid drawbuffer %d. This buffer only supports"
  ...
  if (fb) {
    if (!fb->ValidateAndInitAttachments(funcName)) return false;
    ...
    if (funcType != LOCAL_GL_FLOAT) {
      ErrorInvalidOperation(
          "%s: For default framebuffer, COLOR is always of type"
          " FLOAT.",
          funcName);
      return false;
    }
...
void
WebGL2Context::ClearBufferfv(GLenum buffer,
                             GLint drawBuffer,
                             const Float32Arr& src,
                             GLuint srcElemOffset)
{
  ...
  if (!ValidateClearBuffer(funcName,
                           buffer,
                           drawBuffer,
                           src.elemCount,
                           srcElemOffset,
...
void
WebGL2Context::ClearBufferiv(GLenum buffer,
                             GLint drawBuffer,
                             const Int32Arr& src,
                             GLuint srcElemOffset)
{
...
void
WebGL2Context::ClearBufferuiv(GLenum buffer,
                              GLint drawBuffer,
                              const Uint32Arr& src,
                              GLuint srcElemOffset)
{
...
void
WebGL2Context::ClearBufferfi(GLenum buffer,
                             GLint drawBuffer,
                             GLfloat depth,
                             GLint stencil)
{
  ...
  if (!ValidateClearBuffer(funcName, buffer, drawBuffer, 2, 0, 0)) return;
