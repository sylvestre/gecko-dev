...
class Canvas2dPixelsReporter final : public nsIMemoryReporter
{
 ...
 public:
  ...
  {
    MOZ_COLLECT_REPORT("canvas-2d-pixels",
                       KIND_OTHER,
                       UNITS_BYTES,
...
bool
CanvasRenderingContext2D::PatternIsOpaque(
 ...
 public:
                    ...
                    DrawTarget* aRT)
  {
    ...
    if (state.StyleIsColor(aStyle)) {
      ...
      mPattern.InitSurfacePattern(state.patternStyles[aStyle]->mSurface,
                                  mode,
...
class AdjustedTargetForFilter
{
 public:
        ...
        mCompositionOp(aCompositionOp)
  {
    ...
    FilterSupport::ComputeSourceNeededRegions(aCtx->CurrentState().filter,
                                              mPostFilterBounds,
 ...
 public:
  ...
  ~AdjustedTargetForShadow()
  {
    ...
    mFinalTarget->DrawSurfaceWithShadow(
        ...
        mCtx->CurrentState().shadowOffset,
        mSigma,
        mCompositionOp);
  }
                          ...
                          const gfx::Rect* aBounds = nullptr)
  {
    ...
    if (applyFilter) {
      ...
      mFilterTarget = MakeUnique<AdjustedTargetForFilter>(
          ...
          offsetToFinalDT,
          intBounds,
          gfx::RoundedToInt(boundsAfterFilter),
          op);
...
void
CanvasGradient::AddColorStop(float aOffset,
                             const nsAString& aColorstr,
                             ErrorResult& aRv)
{
  ...
  if (servoStyleSet) {
    ok = ServoCSSParser::ComputeColor(
        servoStyleSet, NS_RGB(0, 0, 0), aColorstr, &color);
  } else {
    nsCSSValue value;
    nsCSSParser parser;
...
class CanvasFilterChainObserver : public nsSVGFilterChainObserver
{
 public:
  CanvasFilterChainObserver(nsTArray<nsStyleFilter>& aFilters,
      ...
      : nsSVGFilterChainObserver(aFilters, aCanvasElement), mContext(aContext)
  {
  }
...
NS_IMPL_CYCLE_COLLECTION_TRAVERSE_BEGIN(CanvasRenderingContext2D)
  ...
  for (uint32_t i = 0; i < tmp->mStyleStack.Length(); i++) {
    ImplCycleCollectionTraverse(
        cb,
        tmp->mStyleStack[i].patternStyles[Style::STROKE],
    ...
    ImplCycleCollectionTraverse(cb,
                                ...
                                "Fill CanvasGradient");
    ImplCycleCollectionTraverse(
        cb, tmp->mStyleStack[i].filterChainObserver, "Filter Chain Observer");
  }
  for (size_t x = 0; x < tmp->mHitRegionsOptions.Length(); x++) {
    RegionInfo& info = tmp->mHitRegionsOptions[x];
    if (info.mElement) {
      ImplCycleCollectionTraverse(
          cb, info.mElement, "Hit region fallback element");
    }
  }
...
JSObject*
CanvasRenderingContext2D::WrapObject(JSContext* aCx,
...
bool
CanvasRenderingContext2D::ParseColor(const nsAString& aString, nscolor* aColor)
{
  ...
  if (value.IsNumericColorUnit()) {
    ...
    *aColor = value.GetColorValue();
    ...
    if (mCanvasElement && mCanvasElement->IsInUncomposedDoc()) {
      ...
      parentContext = nsComputedDOMStyle::GetStyleContext(
...
void
CanvasRenderingContext2D::GetStyleAsUnion(
...
// static
void
CanvasRenderingContext2D::StyleColorToString(const nscolor& aColor,
                                             nsAString& aStr)
{
...
bool
CanvasRenderingContext2D::SwitchRenderingMode(RenderingMode aRenderingMode)
{
  ...
  if (oldBufferProvider && mTarget) {
    CopyBufferProvider(
        *oldBufferProvider, *mTarget, IntRect(0, 0, mWidth, mHeight));
  }
...
void
CanvasRenderingContext2D::AddDemotableContext(
    CanvasRenderingContext2D* aContext)
{
  MOZ_ASSERT(sMaxContextsInitialized);
...
void
CanvasRenderingContext2D::RemoveDemotableContext(
    CanvasRenderingContext2D* aContext)
{
  MOZ_ASSERT(sMaxContextsInitialized);
  if (sMaxContexts <= 0) return;
...
bool
CanvasRenderingContext2D::TrySkiaGLTarget(
    ...
    RefPtr<layers::PersistentBufferProvider>& aOutProvider)
{
  ...
  aOutDT = Factory::CreateDrawTargetSkiaWithGrContext(
      glue->GetGrContext(), size, format);
  if (!aOutDT) {
    gfxCriticalNote
...
bool
CanvasRenderingContext2D::TrySharedTarget(
    RefPtr<gfx::DrawTarget>& aOutDT,
    RefPtr<layers::PersistentBufferProvider>& aOutProvider)
{
  aOutDT = nullptr;
...
CanvasRenderingContext2D::ClearTarget()
{
  ...
  if (mCanvasElement && mCanvasElement->IsInUncomposedDoc()) {
    nsCOMPtr<nsIPresShell> presShell = GetPresShell();
    if (presShell) {
      canvasStyle = nsComputedDOMStyle::GetStyleContext(
          mCanvasElement, nullptr, presShell);
      if (canvasStyle) {
...
nsString
CanvasRenderingContext2D::GetHitRegion(const mozilla::gfx::Point& aPoint)
{
                                         ...
                                         nsIInputStream** aStream)
{
  ...
  return ImageEncoder::GetInputStream(mWidth,
                                      mHeight,
                                      imageBuffer.get(),
                                      format,
                                      encoder,
...
void
CanvasRenderingContext2D::Transform(double aM11,
                                    double aM12,
                                    double aM21,
                                    double aM22,
                                    double aDx,
...
void
CanvasRenderingContext2D::SetTransform(double aM11,
                                       double aM12,
                                       double aM21,
                                       double aM22,
                                       double aDx,
...
static void
MatrixToJSObject(JSContext* aCx,
                 const Matrix& aMatrix,
                 JS::MutableHandle<JSObject*> aResult,
                 ErrorResult& aError)
{
  double elts[6] = {aMatrix._11,
                    aMatrix._12,
                    aMatrix._21,
                    aMatrix._22,
                    aMatrix._31,
...
void
CanvasRenderingContext2D::SetMozCurrentTransform(
    JSContext* aCx,
...
void
CanvasRenderingContext2D::GetMozCurrentTransform(
    JSContext* aCx, JS::MutableHandle<JSObject*> aResult, ErrorResult& aError)
{
  EnsureTarget();

  MatrixToJSObject(
      aCx, mTarget ? mTarget->GetTransform() : Matrix(), aResult, aError);
}
...
void
CanvasRenderingContext2D::GetMozCurrentTransformInverse(
    JSContext* aCx, JS::MutableHandle<JSObject*> aResult, ErrorResult& aError)
{
  EnsureTarget();
...
CanvasRenderingContext2D::CreateLinearGradient(double aX0,
                                               double aY0,
                                               double aX1,
                                               double aY1)
{
...
already_AddRefed<CanvasGradient>
CanvasRenderingContext2D::CreateRadialGradient(double aX0,
                                               double aY0,
                                               double aR0,
                                               double aX1,
                                               double aY1,
                                        ...
                                        ErrorResult& aError)
{
  ...
  if (aSource.IsHTMLCanvasElement()) {
    ...
    if (srcCanvas) {
      ...
      RefPtr<CanvasPattern> pat = new CanvasPattern(this,
                                                    srcSurf,
                                                    repeatMode,
...
CreateDeclaration(nsINode* aNode,
                  const nsCSSPropertyID aProp1,
                  const nsAString& aValue1,
                  bool* aChanged1,
                  const nsCSSPropertyID aProp2,
                          ...
                          ErrorResult& aError)
{
  if (aElement && aElement->IsInUncomposedDoc()) {
    ...
    return already_AddRefed<GeckoStyleContext>(
        result.forget().take()->AsGecko());
  }
...
static bool
PropertyIsInheritOrInitial(Declaration* aDeclaration,
...
static already_AddRefed<GeckoStyleContext>
GetFontStyleContext(Element* aElement,
...
{
  RefPtr<URLExtraData> data = new URLExtraData(aDocument->GetDocBaseURI(),
...
static already_AddRefed<RawServoDeclarationBlock>
CreateFontDeclarationForServo(const nsAString& aFont, nsIDocument* aDocument)
{
  return CreateDeclarationForServo(eCSSProperty_font, aFont, aDocument);
}

static already_AddRefed<ServoStyleContext>
GetFontStyleForServo(Element* aElement,
                     const nsAString& aFont,
                     nsIPresShell* aPresShell,
                     nsAString& aOutUsedFont,
                     ErrorResult& aError)
...
void
CanvasRenderingContext2D::SetFilter(const nsAString& aFilter,
                                    ErrorResult& aError)
{
...
void
CanvasRenderingContext2D::ClearRect(double aX, double aY, double aW, double aH)
{
...
CanvasRenderingContext2D::StrokeRect(double aX, double aY, double aW, double aH)
{
  ...
  if (needBounds) {
    const ContextState& state = CurrentState();
    bounds = gfx::Rect(aX - state.lineWidth / 2.0f,
                       aY - state.lineWidth / 2.0f,
                       aW + state.lineWidth,
                       aH + state.lineWidth);
...
void
CanvasRenderingContext2D::Fill(const CanvasPath& aPath,
...
void
CanvasRenderingContext2D::DrawFocusIfNeeded(mozilla::dom::Element& aElement,
                                            ErrorResult& aRv)
{
  EnsureUserSpacePath();
  if (!mPath) {
...
bool
CanvasRenderingContext2D::DrawCustomFocusRing(mozilla::dom::Element& aElement)
{
  EnsureUserSpacePath();
...
void
CanvasRenderingContext2D::Clip(const CanvasPath& aPath,
                               const CanvasWindingRule& aWinding)
{
  EnsureTarget();
  if (!IsTargetValid()) {
...
void
CanvasRenderingContext2D::ArcTo(double aX1,
                                double aY1,
                                double aX2,
                                double aY2,
                                double aRadius,
...
void
CanvasRenderingContext2D::Arc(double aX,
                              double aY,
                              double aR,
                              double aStartAngle,
                              double aEndAngle,
...
void
CanvasRenderingContext2D::Ellipse(double aX,
                                  double aY,
                                  double aRadiusX,
                                  double aRadiusY,
                                  double aRotation,
...
void
CanvasRenderingContext2D::SetFont(const nsAString& aFont, ErrorResult& aError)
{
                                          ...
                                          ErrorResult& aError)
{
  ...
  if (presShell->StyleSet()->IsServo()) {
    sc = GetFontStyleForServo(
        mCanvasElement, aFont, presShell, usedFont, aError);
  } else {
    sc =
...
void
CanvasRenderingContext2D::FillText(const nsAString& aText,
                                   double aX,
                                   double aY,
                                   const Optional<double>& aMaxWidth,
                                   ErrorResult& aError)
{
  aError = DrawOrMeasureText(
      aText, aX, aY, aMaxWidth, TextDrawOperation::FILL, nullptr);
}

void
CanvasRenderingContext2D::StrokeText(const nsAString& aText,
                                     double aX,
                                     double aY,
                                     const Optional<double>& aMaxWidth,
                                     ErrorResult& aError)
{
  aError = DrawOrMeasureText(
      aText, aX, aY, aMaxWidth, TextDrawOperation::STROKE, nullptr);
}
                                      ...
                                      ErrorResult& aError)
{
  ...
  aError = DrawOrMeasureText(
      aRawText, 0, 0, maxWidth, TextDrawOperation::MEASURE, &width);
  if (aError.Failed()) {
    return nullptr;
  }
...
void
CanvasRenderingContext2D::AddHitRegion(const HitRegionOptions& aOptions,
                                       ErrorResult& aError)
{
...
#ifdef ACCESSIBILITY
    aOptions.mControl->SetProperty(
        nsGkAtoms::hitregion, new bool(true), nsINode::DeleteProperty<bool>);
#endif
  }
  ...
  already_AddRefed<gfxPattern> GetGradientFor(Style aStyle)
  {
    ...
    switch (type) {
      case CanvasGradient::Type::RADIAL: {
        ...
        pattern = new gfxPattern(radial->mCenter1.x,
                                 radial->mCenter1.y,
                                 radial->mRadius1,
                                 radial->mCenter2.x,
                                 radial->mCenter2.y,
      ...
      case CanvasGradient::Type::LINEAR: {
        ...
        pattern = new gfxPattern(
            linear->mBegin.x, linear->mBegin.y, linear->mEnd.x, linear->mEnd.y);
        break;
      }
      default:
  ...
  if (!CurrentState().fontGroup) {
    ...
    if (err.Failed() || !fontUpdated) {
      ...
      CurrentState().fontGroup = gfxPlatform::GetPlatform()->CreateFontGroup(
          FontFamilyList(eFamily_sans_serif),
          &style,
          tp,
          nullptr,
          devToCssSize);
...
void
CanvasRenderingContext2D::GetLineJoin(nsAString& aLinejoinStyle,
                                      ErrorResult& aError)
{
...
bool
CanvasRenderingContext2D::IsPointInPath(JSContext* aCx,
                                        double aX,
                                        double aY,
                                        const CanvasWindingRule& aWinding)
{
...
CanvasRenderingContext2D::IsPointInStroke(JSContext* aCx, double aX, double aY)
{
  ...
  return mPath->StrokeContainsPoint(
      strokeOptions, Point(aX, aY), mTarget->GetTransform());
}

bool
CanvasRenderingContext2D::IsPointInStroke(JSContext* aCx,
                                          const CanvasPath& aPath,
                                          double aX,
                                          double aY)
{
  ...
  return tempPath->StrokeContainsPoint(
      strokeOptions, Point(aX, aY), mTarget->GetTransform());
}
...
ExtractSubrect(SourceSurface* aSurface,
...
static void
ClipImageDimension(double& aSourceCoord,
                   double& aSourceSize,
                   int32_t aImageSize,
                   double& aDestCoord,
                   double& aDestSize)
                                    ...
                                    ErrorResult& aError)
{
  ...
  if (aImage.IsHTMLCanvasElement()) {
    ...
    srcSurf = CanvasImageCache::LookupCanvas(
        element, mCanvasElement, &imgSize, mIsSkiaGL);
  }
  ...
  if (!srcSurf) {
    ...
    if (mCanvasElement) {
      CanvasUtils::DoDrawImageSecurityCheck(
          mCanvasElement, res.mPrincipal, res.mIsWriteOnly, res.mCORSUsed);
    }
...
void
CanvasRenderingContext2D::DrawWindow(nsGlobalWindow& aWindow,
                                     double aX,
                                     double aY,
                                     double aW,
                                     double aH,
                                     ...
                                     ErrorResult& aError)
{
  ...
  if (op == CompositionOp::OP_OVER &&
    ...
    thebes->SetMatrix(gfxMatrix(matrix._11,
                                matrix._12,
                                matrix._21,
                                matrix._22,
                                matrix._31,
...
CanvasRenderingContext2D::GetImageData(JSContext* aCx,
                                       double aSx,
                                       double aSy,
                                       double aSw,
                                       double aSh,
                                            ...
                                            JSObject** aRetval)
{
  ...
  if (mCanvasElement) {
    nsCOMPtr<nsIDocument> ownerDoc = mCanvasElement->OwnerDoc();
    usePlaceholder =
        !ownerDoc || !CanvasUtils::IsImageExtractionAllowed(ownerDoc, aCx);
  }
...
void
CanvasRenderingContext2D::PutImageData(ImageData& aImageData,
                                       double aDx,
                                       double aDy,
                                       ErrorResult& aError)
{
  ...
  aError = PutImageData_explicit(JS::ToInt32(aDx),
                                 JS::ToInt32(aDy),
                                 aImageData.Width(),
                                 aImageData.Height(),
                                 &arr,
...
nsresult
CanvasRenderingContext2D::PutImageData_explicit(int32_t aX,
                                                int32_t aY,
                                                uint32_t aW,
                                                uint32_t aH,
                                                ...
                                                int32_t aDirtyHeight)
{
  ...
  if (lockedBits) {
    ...
    mTarget->CopySurface(
        sourceSurface, dirtyRect - dirtyRect.TopLeft(), dirtyRect.TopLeft());
  }
                                          ...
                                          ErrorResult& aError)
{
  return mozilla::dom::CreateImageData(
      aCx, this, aImagedata.Width(), aImagedata.Height(), aError);
}
...
void
CanvasRenderingContext2D::RemoveDrawObserver()
{
  if (mDrawObserver) {
...
void
CanvasPath::BezierCurveTo(double aCp1x,
                          double aCp1y,
                          double aCp2x,
                          double aCp2y,
                          double aX,
...
void
CanvasPath::AddPath(CanvasPath& aCanvasPath,
...
CanvasPath::GetPath(const CanvasWindingRule& aWinding,
                    const DrawTarget* aTarget) const
{
