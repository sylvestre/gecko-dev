...
static bool
Translate(const nsACString& source,
...
template<size_t N>
static bool
SubstringStartsWith(const std::string& testStr,
                    size_t offset,
                    const char (&refStr)[N])
{
 ...
 */
static bool
TranslateWithoutValidation(const nsACString& sourceNS,
                           bool isWebGL2,
                           nsACString* const out_translationLog,
                           nsACString* const out_translatedSource)
{
...
static void
GetCompilationStatusAndLog(gl::GLContext* gl,
                           GLuint shader,
                           ...
                           nsACString* const out_log)
{
  ...
  if (lenWithNull > 1) {
    ...
    gl->fGetShaderInfoLog(
        shader, lenWithNull, nullptr, out_log->BeginWriting());
  } else {
    out_log->SetLength(0);
  }
...
WebGLShader::CompileShader()
{
  ...
  if (mValidator) {
    success = Translate(
        mCleanSource, mValidator.get(), &mValidationLog, &mTranslatedSource);
  } else {
    success = TranslateWithoutValidation(mCleanSource,
                                         mContext->IsWebGL2(),
                                         &mValidationLog,
                                         &mTranslatedSource);
  }
  ...
  GetCompilationStatusAndLog(
      gl, mGLName, &mCompilationSuccessful, &mCompilationLog);
}
...
void
WebGLShader::BindAttribLocation(GLuint prog,
                                     ...
                                     bool* const out_isArray) const
{
  ...
  if (!mValidator->FindVaryingByMappedName(
          mappedNameStr, &userNameStr, out_isArray))
    return false;
                                     ...
                                     bool* const out_isArray) const
{
  ...
  const std::string mappedNameStr(mappedName.BeginReading(),
                                  mappedName.Length());
  std::string userNameStr;
  if (!mValidator->FindUniformByMappedName(
          mappedNameStr, &userNameStr, out_isArray))
    return false;
...
void
WebGLShader::EnumerateFragOutputs(
...
void
WebGLShader::MapTransformFeedbackVaryings(
