 ...
 */
static bool
ParseName(const nsCString& name,
          nsCString* const out_baseName,
          bool* const out_isArray,
...
static void
AssembleName(const nsCString& baseName,
             bool isArray,
             size_t arrayIndex,
...
QueryProgramInfo(WebGLProgram* prog, gl::GLContext* gl)
{
  ...
  gl->fGetProgramiv(
      prog->mGLName, LOCAL_GL_ACTIVE_ATTRIBUTES, (GLint*)&numActiveAttribs);
  ...
  for (GLuint i = 0; i < numActiveAttribs; i++) {
    ...
    gl->fGetActiveAttrib(prog->mGLName,
                         i,
                         mappedName.Length() + 1,
                         &lengthWithoutNull,
                         &elemCount,
  ...
  gl->fGetProgramiv(
      prog->mGLName, LOCAL_GL_ACTIVE_UNIFORMS, (GLint*)&numActiveUniforms);
  ...
  for (GLuint i = 0; i < numActiveUniforms; i++) {
    ...
    GLint elemCount = 0;  // `size`
    GLenum elemType = 0;  // `type`
    gl->fGetActiveUniform(prog->mGLName,
                          i,
                          mappedName.Length() + 1,
                          &lengthWithoutNull,
                          &elemCount,
    ...
    if (!prog->FindUniformByMappedName(
            baseMappedName, &baseUserName, &isArray)) {
  ...
  if (gl->IsSupported(gl::GLFeature::uniform_buffer_object)) {
    ...
    for (GLuint i = 0; i < numActiveUniformBlocks; i++) {
      ...
      gl->fGetActiveUniformBlockiv(prog->mGLName,
                                   i,
                                   LOCAL_GL_UNIFORM_BLOCK_NAME_LENGTH,
                                   &lengthWithoutNull);
      gl->fGetActiveUniformBlockName(prog->mGLName,
                                     i,
  ...
  if (gl->IsSupported(gl::GLFeature::transform_feedback2)) {
    ...
    for (GLuint i = 0; i < numTransformFeedbackVaryings; i++) {
      ...
      GLsizei elemCount;
      GLenum elemType;
      gl->fGetTransformFeedbackVarying(prog->mGLName,
                                       i,
...
static GLint
GetFragDataByUserName(const WebGLProgram* prog, const nsCString& userName)
{
...
{
  if (!ValidateGLSLVariableName(
          userName_wide, mContext, "getUniformBlockIndex"))
...
void
WebGLProgram::GetActiveUniformBlockName(GLuint uniformBlockIndex,
...
JS::Value
WebGLProgram::GetActiveUniformBlockActiveUniforms(
    JSContext* cx, GLuint uniformBlockIndex, ErrorResult* const out_error) const
{
  ...
  if (uniformBlockIndex >= uniformBlockCount) {
    mContext->ErrorInvalidValue(
        "%s: Index %u invalid.", funcName, uniformBlockIndex);
    return JS::NullValue();
  }

  gl::GLContext* gl = mContext->GL();
  GLint activeUniformCount = 0;
  gl->fGetActiveUniformBlockiv(mGLName,
                               uniformBlockIndex,
                                  ...
                                  GLuint uniformBlockBinding) const
{
  ...
  if (uniformBlockBinding >= indexedBindings.size()) {
    mContext->ErrorInvalidValue(
        "%s: Binding %u invalid.", funcName, uniformBlockBinding);
    return;
  }
...
WebGLProgram::LinkAndUpdate()
{
  ...
  if (logLenWithNull > 1) {
    ...
    gl->fGetProgramInfoLog(
        mGLName, logLenWithNull, nullptr, mLinkLog.BeginWriting());
  } else {
...
bool
WebGLProgram::FindAttribUserNameByMappedName(
...
{
  if (mVertShader->FindVaryingByMappedName(
          mappedName, out_userName, out_isArray))
    return true;
...
WebGLProgram::FindUniformByMappedName(const nsACString& mappedName,
                                      ...
                                      bool* const out_isArray) const
{
  if (mVertShader->FindUniformByMappedName(
          mappedName, out_userName, out_isArray))
    return true;

  if (mFragShader->FindUniformByMappedName(
          mappedName, out_userName, out_isArray))
    return true;
  ...
  switch (bufferMode) {
    ...
    default:
      mContext->ErrorInvalidEnum(
          "%s: Bad `bufferMode`: 0x%04x.", funcName, bufferMode);
      return;
  }
...
void
WebGLProgram::EnumerateFragOutputs(
