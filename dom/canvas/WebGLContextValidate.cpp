...
bool
WebGLContext::ValidateAttribArraySetter(const char* name,
                                        ...
                                        uint32_t arrayLength)
{
  ...
  if (arrayLength < setterElemSize) {
    ErrorInvalidValue(
        "%s: Array must have >= %d elements.", name, setterElemSize);
    return false;
  }
...
bool
WebGLContext::ValidateUniformArraySetter(
...
bool
WebGLContext::ValidateUniformMatrixArraySetter(
    WebGLUniformLocation* loc,
    uint8_t setterCols,
    uint8_t setterRows,
    GLenum setterType,
...
WebGLContext::InitAndValidateGL(FailureReason* const out_failReason)
{
  ...
  AssertUintParamCorrect(
      gl, LOCAL_GL_STENCIL_VALUE_MASK, mStencilValueMaskFront);
  AssertUintParamCorrect(
      gl, LOCAL_GL_STENCIL_BACK_VALUE_MASK, mStencilValueMaskBack);
  AssertUintParamCorrect(
      gl, LOCAL_GL_STENCIL_WRITEMASK, mStencilWriteMaskFront);
  AssertUintParamCorrect(
      gl, LOCAL_GL_STENCIL_BACK_WRITEMASK, mStencilWriteMaskBack);
  ...
  std::fill_n(
      mGenericVertexAttribTypes.get(), mGLMaxVertexAttribs, LOCAL_GL_FLOAT);

  static const float kDefaultGenericVertexAttribData[4] = {0, 0, 0, 1};
  memcpy(mGenericVertexAttrib0Data,
...
bool
WebGLContext::ValidateFramebufferTarget(GLenum target, const char* const info)
{
  bool isValid = true;
  switch (target) {
