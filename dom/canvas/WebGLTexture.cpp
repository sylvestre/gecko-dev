...
void
WebGLTexture::ImageInfo::SetIsDataInitialized(bool isDataInitialized,
                                              WebGLTexture* tex)
{
  MOZ_ASSERT(tex);
...
void
WebGLTexture::SetImageInfo(const char* funcName,
                           ImageInfo* target,
...
void
WebGLTexture::SetImageInfosAtLevel(const char* funcName,
                                   uint32_t level,
                                   const ImageInfo& newInfo)
{
...
bool
WebGLTexture::IsMipmapComplete(const char* funcName,
                               uint32_t texUnit,
...
bool
WebGLTexture::IsComplete(const char* funcName,
                         uint32_t texUnit,
...
bool
WebGLTexture::MaxEffectiveMipmapLevel(uint32_t texUnit,
...
bool
WebGLTexture::GetFakeBlackType(const char* funcName,
                               uint32_t texUnit,
...
{
  static const GLint kNoSwizzle[4] = {
      LOCAL_GL_RED, LOCAL_GL_GREEN, LOCAL_GL_BLUE, LOCAL_GL_ALPHA};
  if (!swizzle) {
    swizzle = kNoSwizzle;
  ...
  gl->fTexParameteri(
      LOCAL_GL_TEXTURE_2D, LOCAL_GL_TEXTURE_SWIZZLE_R, swizzle[0]);
  gl->fTexParameteri(
      LOCAL_GL_TEXTURE_2D, LOCAL_GL_TEXTURE_SWIZZLE_G, swizzle[1]);
  gl->fTexParameteri(
      LOCAL_GL_TEXTURE_2D, LOCAL_GL_TEXTURE_SWIZZLE_B, swizzle[2]);
  gl->fTexParameteri(
      LOCAL_GL_TEXTURE_2D, LOCAL_GL_TEXTURE_SWIZZLE_A, swizzle[3]);
}

bool
WebGLTexture::ResolveForDraw(const char* funcName,
                             uint32_t texUnit,
                             FakeBlackType* const out_fakeBlack)
{
  if (!mIsResolved) {
...
bool
WebGLTexture::EnsureImageDataInitialized(const char* funcName,
...
static void
ZeroANGLEDepthTexture(WebGLContext* webgl,
                      GLuint tex,
                      ...
                      uint32_t height)
{
  ...
  gl->fFramebufferTexture2D(
      LOCAL_GL_FRAMEBUFFER, attachPoint, LOCAL_GL_TEXTURE_2D, tex, 0);
...
static bool
ZeroTextureData(WebGLContext* webgl,
                const char* funcName,
                GLuint tex,
                TexImageTarget target,
                uint32_t level,
                ...
                uint32_t depth)
{
  ...
  if (compression) {
    ...
    const auto error = DoCompressedTexSubImage(gl,
                                               target.get(),
                                               level,
                                               0,
                                               0,
  ...
  const auto error = DoTexSubImage(
      gl, target, level, 0, 0, 0, width, height, depth, packing, zeros.get());
  return !error;
}

bool
WebGLTexture::InitializeImageData(const char* funcName,
                                  TexImageTarget target,
                                  uint32_t level)
{
  auto& imageInfo = ImageInfoAt(target, level);
...
WebGLTexture::ClampLevelBaseAndMax()
{
  ...
  mMaxMipmapLevel = Clamp<uint32_t>(
      mMaxMipmapLevel, mBaseMipmapLevel, mImmutableLevelCount - 1);
}

void
WebGLTexture::PopulateMipChain(const char* funcName,
                               uint32_t firstLevel,
                               uint32_t lastLevel)
{
  ...
  for (uint32_t level = firstLevel + 1; level <= lastLevel; level++) {
    ...
    const ImageInfo cur(baseImageInfo.mFormat,
                        refWidth,
                        refHeight,
                        refDepth,
...
WebGLTexture::BindTexture(TexTarget texTarget)
{
  ...
  if (isFirstBinding) {
    ...
    if (IsCubeMap() && hasWrapR && !mContext->IsWebGL2()) {
      gl->fTexParameteri(
          texTarget.get(), LOCAL_GL_TEXTURE_WRAP_R, LOCAL_GL_CLAMP_TO_EDGE);
    }
  }
...
WebGLTexture::GenerateMipmap(TexTarget texTarget)
{
  ...
  if (gl->WorkAroundDriverBugs()) {
    ...
    gl->fTexParameteri(texTarget.get(),
                       LOCAL_GL_TEXTURE_MIN_FILTER,
                       LOCAL_GL_NEAREST_MIPMAP_NEAREST);
    gl->fGenerateMipmap(texTarget.get());
    gl->fTexParameteri(
        texTarget.get(), LOCAL_GL_TEXTURE_MIN_FILTER, mMinFilter.get());
  } else {
    gl->fGenerateMipmap(texTarget.get());
  }
...
void
WebGLTexture::TexParameter(TexTarget texTarget,
                           GLenum pname,
