...
static bool
IsValidTexTarget(WebGLContext* webgl,
                 uint8_t funcDims,
                 GLenum rawTexTarget,
                 TexTarget* const out)
{
...
static bool
IsValidTexImageTarget(WebGLContext* webgl,
                      uint8_t funcDims,
                  ...
                  WebGLTexture** const out_tex)
{
  ...
  if (!tex) {
    webgl->ErrorInvalidOperation("%s: No texture is bound to this target.",
                                 funcName);
    return false;
  }
                       ...
                       WebGLTexture** const out_tex)
{
  ...
  if (!IsValidTexImageTarget(
          webgl, funcDims, rawTexImageTarget, &texImageTarget)) {
...
WebGLContext::BindTexture(GLenum rawTarget, WebGLTexture* newTex)
{
  ...
  if (newTex && !ValidateObject("bindTexture", *newTex)) return;
...
WebGLContext::GenerateMipmap(GLenum rawTexTarget)
{
  ...
  TexTarget texTarget;
  WebGLTexture* tex;
  if (!ValidateTexTarget(
          this, funcName, funcDims, rawTexTarget, &texTarget, &tex))
    return;
...
{
  if (!ValidateIsObject("isTexture", tex)) return false;

  return tex->IsTexture();
}

void
WebGLContext::TexParameter_base(GLenum rawTexTarget,
                                GLenum pname,
...
void
WebGLContext::CompressedTexImage(const char* funcName,
                                 uint8_t funcDims,
                                 GLenum rawTarget,
                                 GLint level,
                                 ...
                                 const Maybe<GLsizei>& expectedImageSize)
{
  ...
  WebGLTexture* tex;
  if (!ValidateTexImageTarget(
          this, funcName, funcDims, rawTarget, &target, &tex))
    return;

  tex->CompressedTexImage(funcName,
                          target,
                          level,
                          internalFormat,
                          width,
...
void
WebGLContext::CompressedTexSubImage(const char* funcName,
                                    uint8_t funcDims,
                                    GLenum rawTarget,
                                    GLint level,
                                    GLint xOffset,
                                    ...
                                    const Maybe<GLsizei>& expectedImageSize)
{
  ...
  tex->CompressedTexSubImage(funcName,
                             target,
                             level,
                             xOffset,
                             yOffset,
...
void
WebGLContext::CopyTexImage2D(GLenum rawTarget,
                             GLint level,
                             GLenum internalFormat,
                             GLint x,
                             GLint y,
...
void
WebGLContext::CopyTexSubImage(const char* funcName,
                              uint8_t funcDims,
                              GLenum rawTarget,
                              GLint level,
                              GLint xOffset,
                              ...
                              GLsizei height)
{
  ...
  tex->CopyTexSubImage(
      funcName, target, level, xOffset, yOffset, zOffset, x, y, width, height);
}

////

void
WebGLContext::TexImage(const char* funcName,
                       uint8_t funcDims,
                       GLenum rawTarget,
                       GLint level,
                       GLenum internalFormat,
...
void
WebGLContext::TexSubImage(const char* funcName,
                          uint8_t funcDims,
                          GLenum rawTarget,
                          GLint level,
                          GLint xOffset,
