...
static bool
ValidatePIForDOM(WebGLContext* webgl,
                 const char* funcName,
...
static bool
ValidateUnpackPixels(WebGLContext* webgl,
                     const char* funcName,
                     uint32_t fullRows,
                     uint32_t tailPixels,
                    ...
                    webgl::TexUnpackBlob* blob)
{
  ...
  return ValidateUnpackPixels(
      webgl, funcName, fullRows.value(), tailPixels, blob);
}
...
TexUnpackBlob::TexUnpackBlob(const WebGLContext* webgl,
  ...
  *out_begin = srcBegin;
  ...
  if (srcFormat != dstFormat) {
    webgl->GeneratePerfWarning(
        "%s: Conversion requires pixel reformatting. (%u->%u)",
        funcName,
        uint32_t(srcFormat),
        uint32_t(dstFormat));
    ...
    webgl->GeneratePerfWarning(
        "%s: Conversion requires change in stride. (%u->%u)",
        funcName,
        uint32_t(srcStride),
        uint32_t(dstStride));
  } else {
...
TexUnpackBytes::TexUnpackBytes(const WebGLContext* webgl,
                               TexImageTarget target,
                               uint32_t width,
                               uint32_t height,
                               uint32_t depth,
...
bool
TexUnpackBytes::Validate(WebGLContext* webgl,
                         const char* funcName,
                         const webgl::PackingInfo& pi)
{
...
bool
TexUnpackBytes::TexOrSubImage(bool isSubImage,
                              bool needsRespec,
                              const char* funcName,
                              WebGLTexture* tex,
                              TexImageTarget target,
                              ...
                              GLenum* const out_error) const
{
  ...
  if (!useParanoidHandling) {
    ...
    *out_error = DoTexOrSubImage(isSubImage,
                                 gl,
                                 target,
                                 level,
                                 dui,
  ...
  if (!isSubImage) {
    ...
    *out_error = DoTexOrSubImage(false,
                                 gl,
                                 target,
                                 level,
                                 dui,
  ...
  const ScopedLazyBind bindPBO(
      gl, LOCAL_GL_PIXEL_UNPACK_BUFFER, webgl->mBoundPixelUnpackBuffer);
  ...
  if (mDepth > 1) {
    *out_error = DoTexOrSubImage(true,
                                 gl,
                                 target,
                                 level,
                                 dui,
...
TexUnpackImage::TexUnpackImage(const WebGLContext* webgl,
                               TexImageTarget target,
                               uint32_t width,
                               uint32_t height,
                               uint32_t depth,
...
bool
TexUnpackImage::Validate(WebGLContext* webgl,
                         const char* funcName,
                         const webgl::PackingInfo& pi)
{
...
bool
TexUnpackImage::TexOrSubImage(bool isSubImage,
                              bool needsRespec,
                              const char* funcName,
                              WebGLTexture* tex,
                              TexImageTarget target,
                              ...
                              GLenum* const out_error) const
{
  ...
  do {
    ...
    const auto dstOrigin =
        (webgl->mPixelStore_FlipY ? gl::OriginPos::TopLeft
                                  : gl::OriginPos::BottomLeft);
    if (!gl->BlitHelper()->BlitImageToFramebuffer(
            mImage, destSize, dstOrigin)) {
  ...
  const nsPrintfCString perfMsg(
      "%s: Failed to hit GPU-copy fast-path: %s (src type %u)",
      funcName,
      fallbackReason,
  ...
  const TexUnpackSurface surfBlob(
      webgl, target, mWidth, mHeight, mDepth, dataSurf, mSrcAlphaType);

  return surfBlob.TexOrSubImage(isSubImage,
                                needsRespec,
                                funcName,
                                tex,
                                target,
...
static bool
GetFormatForSurf(gfx::SourceSurface* surf,
...
bool
TexUnpackSurface::Validate(WebGLContext* webgl,
                           const char* funcName,
                           const webgl::PackingInfo& pi)
{
  if (!ValidatePIForDOM(webgl, funcName, pi)) return false;
...
bool
TexUnpackSurface::TexOrSubImage(bool isSubImage,
                                bool needsRespec,
                                const char* funcName,
                                WebGLTexture* tex,
                                TexImageTarget target,
