...
bool
GetCanvasContextType(const nsAString& str,
                     dom::CanvasContextType* const out_type);

// Check that the rectangle [x,y,w,h] is a subrectangle of [0,0,realWidth,realHeight]

inline bool
CheckSaneSubrectSize(int32_t x,
                     int32_t y,
                     int32_t w,
                     int32_t h,
                     int32_t realWidth,
...
bool
HasDrawWindowPrivilege(JSContext* aCx, JSObject* aObj);

// Check site-specific permission and display prompt if appropriate.
bool
IsImageExtractionAllowed(nsIDocument* aDocument, JSContext* aCx);
...
inline bool
FloatValidate(double f1, double f2)
{
  VALIDATE(f1);
  VALIDATE(f2);
  return true;
...
inline bool
FloatValidate(double f1, double f2, double f3)
{
  VALIDATE(f1);
  VALIDATE(f2);
  VALIDATE(f3);
...
inline bool
FloatValidate(double f1, double f2, double f3, double f4)
{
  VALIDATE(f1);
  VALIDATE(f2);
  VALIDATE(f3);
...
inline bool
FloatValidate(double f1, double f2, double f3, double f4, double f5)
{
  VALIDATE(f1);
  VALIDATE(f2);
  VALIDATE(f3);
...
inline bool
FloatValidate(double f1, double f2, double f3, double f4, double f5, double f6)
{
  VALIDATE(f1);
  VALIDATE(f2);
  VALIDATE(f3);
...
template<typename T>
nsresult
JSValToDashArray(JSContext* cx,
                 const JS::Value& patternArray,
                 nsTArray<T>& dashes)
{
