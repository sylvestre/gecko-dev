...
class ImageCache final : public nsExpirationTracker<ImageCacheEntryData, 4>
{
 public:
  ...
  virtual void NotifyExpired(ImageCacheEntryData* aObject)
  {
    ...
    mCache.RemoveEntry(ImageCacheKey(
        aObject->mImage, aObject->mCanvas, aObject->mIsAccelerated));
  }
      ...
      mTotal(0)
{
  if (!sPrefsInitialized) {
    ...
    Preferences::AddIntVarCache(
        &sCanvasImageCacheLimit, "canvas.image.cache.limit", 0);
  }
...
SourceSurface*
CanvasImageCache::LookupAllCanvas(Element* aImage, bool aIsAccelerated)
{
  if (!gImageCache) {
    return nullptr;
  }
