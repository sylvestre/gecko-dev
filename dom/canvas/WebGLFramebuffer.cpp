...
void
WebGLFBAttachPoint::SetTexImage(const char* funcName,
                                WebGLTexture* tex,
...
void
WebGLFBAttachPoint::Size(uint32_t* const out_width,
...
bool
WebGLFBAttachPoint::IsComplete(WebGLContext* webgl,
...
JS::Value
WebGLFBAttachPoint::GetParameter(const char* funcName,
                                 WebGLContext* webgl,
                                 JSContext* cx,
                                 GLenum target,
                                 GLenum attachment,
                                 ...
                                 ErrorResult* const out_error) const
{
  ...
  if (!hasAttachment) {
    ...
    if (webgl->IsWebGL2()) {
      webgl->ErrorInvalidOperation(
          "%s: No attachment at %s.", funcName, attachmentName.BeginReading());
    } else {
  ...
  switch (pname) {
    ...
    case LOCAL_GL_FRAMEBUFFER_ATTACHMENT_OBJECT_NAME:
      return (mTexturePtr ? webgl->WebGLObjectAsJSValue(
                                cx, mTexturePtr.get(), *out_error)
...
void
WebGLFramebuffer::DetachRenderbuffer(const char* funcName,
...
bool
WebGLFramebuffer::ValidateClearBufferType(const char* funcName,
                                          GLenum buffer,
                                          uint32_t drawBuffer,
...
bool
WebGLFramebuffer::ValidateForRead(
    const char* funcName,
    const webgl::FormatUsageInfo** const out_format,
    uint32_t* const out_width,
    uint32_t* const out_height)
{
  if (!ValidateAndInitAttachments(funcName)) return false;
...
WebGLFramebuffer::ResolveAttachments() const
{
  ...
  for (uint32_t i = 0; i < mContext->mGLMaxColorAttachments; i++) {
    ...
    gl->fFramebufferRenderbuffer(
        LOCAL_GL_FRAMEBUFFER, attachEnum, LOCAL_GL_RENDERBUFFER, 0);
  }
...
WebGLFramebuffer::ResolveAttachmentData(const char* funcName) const
{
  ...
  for (const auto& attach : tex3DAttachmentsToInit) {
    const auto& tex = attach->Texture();
    if (!tex->InitializeImageData(
            funcName, attach->ImageTarget(), attach->MipLevel())) {
      return false;
    }
  }
...
void
WebGLFramebuffer::DrawBuffers(const char* funcName,
...
void
WebGLFramebuffer::FramebufferRenderbuffer(const char* funcName,
                                          GLenum attachEnum,
                                          GLenum rbtarget,
                                          WebGLRenderbuffer* rb)
{
  ...
  if (!maybeAttach || !maybeAttach.value()) {
    mContext->ErrorInvalidEnum(
        "%s: Bad `attachment`: 0x%x.", funcName, attachEnum);
    return;
  }
...
void
WebGLFramebuffer::FramebufferTexture2D(const char* funcName,
                                       GLenum attachEnum,
...
void
WebGLFramebuffer::FramebufferTextureLayer(const char* funcName,
                                          GLenum attachEnum,
                                          WebGLTexture* tex,
                                          GLint level,
                                          GLint layer)
    ...
    switch (texImageTarget) {
      case LOCAL_GL_TEXTURE_3D:
        if (uint32_t(layer) >= mContext->mGLMax3DTextureSize) {
          mContext->ErrorInvalidValue(
              "%s: `layer` must be < %s.", funcName, "MAX_3D_TEXTURE_SIZE");
          return;
        }
      ...
      case LOCAL_GL_TEXTURE_2D_ARRAY:
        if (uint32_t(layer) >= mContext->mGLMaxArrayTextureLayers) {
          mContext->ErrorInvalidValue("%s: `layer` must be < %s.",
                                      funcName,
        ...
        if (uint32_t(level) > FloorLog2(mContext->mGLMaxTextureSize)) {
          mContext->ErrorInvalidValue(
              "%s: `level` must be <= log2(%s).", funcName, "MAX_TEXTURE_SIZE");
          return;
        }
        break;
...
JS::Value
WebGLFramebuffer::GetAttachmentParameter(const char* funcName,
                                         JSContext* cx,
                                         GLenum target,
                                         GLenum attachEnum,
                                         GLenum pname,
                                         ErrorResult* const out_error)
{
  ...
  return attach->GetParameter(
      funcName, mContext, cx, target, attachEnum, pname, out_error);
}
...
WebGLFramebuffer::BlitFramebuffer(WebGLContext* webgl,
                                  const WebGLFramebuffer* srcFB,
                                  GLint srcX0,
                                  GLint srcY0,
                                  GLint srcX1,
                                  GLint srcY1,
                                  const WebGLFramebuffer* dstFB,
                                  GLint dstX0,
                                  GLint dstY0,
                                  GLint dstX1,
                                  GLint dstY1,
                                  ...
                                  GLenum filter)
{
  ...
  if (srcFB) {
    ...
    GetBackbufferFormats(
        webgl, &srcColorFormat, &srcDepthFormat, &srcStencilFormat);
  }
  ...
  if (dstFB) {
    ...
    GetBackbufferFormats(
        webgl, &dstColorFormat, &dstDepthFormat, &dstStencilFormat);
