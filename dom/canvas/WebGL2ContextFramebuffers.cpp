...
void
WebGL2Context::BlitFramebuffer(GLint srcX0,
                               GLint srcY0,
                               GLint srcX1,
                               GLint srcY1,
                               GLint dstX0,
                               ...
                               GLenum filter)
{
  ...
  if (readFB && !readFB->ValidateAndInitAttachments(
                    "blitFramebuffer's READ_FRAMEBUFFER")) {
    return;
  }

  const auto& drawFB = mBoundDrawFramebuffer;
  if (drawFB && !drawFB->ValidateAndInitAttachments(
                    "blitFramebuffer's DRAW_FRAMEBUFFER")) {
    return;
  }
...
void
WebGL2Context::FramebufferTextureLayer(GLenum target,
                                       GLenum attachment,
...
{
  return WebGLContext::GetFramebufferAttachmentParameter(
      cx, target, attachment, pname, out_error);
}

////

static bool
ValidateBackbufferAttachmentEnum(WebGLContext* webgl,
                                 const char* funcName,
                                 GLenum attachment)
{
  switch (attachment) {
    ...
    default:
      webgl->ErrorInvalidEnum(
          "%s: attachment: invalid enum value 0x%x.", funcName, attachment);
      return false;
  }
}
...
bool
WebGLContext::ValidateInvalidateFramebuffer(
    const char* funcName,
    GLenum target,
                                     ...
                                     ErrorResult& rv)
{
  ...
  if (!ValidateInvalidateFramebuffer(funcName,
                                     target,
                                     attachments,
                                     &rv,
                                     &scopedVector,
    ...
    ErrorResult& rv)
{
  ...
  const bool useFBInvalidation =
      (mAllowFBInvalidation &&
       gl->IsSupported(gl::GLFeature::invalidate_framebuffer));
  if (useFBInvalidation) {
    gl->fInvalidateSubFramebuffer(
        target, glNumAttachments, glAttachments, x, y, width, height);
    return;
  }
