...
static bool
ValidateExtents(WebGLContext* webgl,
                const char* funcName,
                GLsizei width,
                GLsizei height,
                GLsizei depth,
...
static bool
ValidateViewType(WebGLContext* webgl,
                 const char* funcName,
                 GLenum unpackType,
...
static bool
ValidateUnpackInfo(WebGLContext* webgl,
                   const char* funcName,
         ...
         GLuint viewElemLengthOverride)
{
  ...
  if (view) {
    if (!webgl->ValidateArrayBufferView(funcName,
                                        *view,
                                        viewElemOffset,
  ...
  return MakeUnique<webgl::TexUnpackBytes>(
      webgl, target, width, height, depth, isClientData, bytes, availByteCount);
}

static UniquePtr<webgl::TexUnpackBytes>
FromPboOffset(WebGLContext* webgl,
              const char* funcName,
              TexImageTarget target,
              uint32_t width,
              uint32_t height,
              ...
              const Maybe<GLsizei>& expectedImageSize)
{
  ...
  return MakeUnique<webgl::TexUnpackBytes>(
      webgl, target, width, height, depth, isClientData, ptr, availBufferBytes);
}

static UniquePtr<webgl::TexUnpackBlob>
FromImageBitmap(WebGLContext* webgl,
                const char* funcName,
                TexImageTarget target,
                uint32_t width,
                uint32_t height,
...
static UniquePtr<webgl::TexUnpackBlob>
FromImageData(WebGLContext* webgl,
              const char* funcName,
              TexImageTarget target,
              uint32_t width,
              uint32_t height,
              ...
              dom::Uint8ClampedArray* scopedArr)
{
  ...
  const RefPtr<gfx::DataSourceSurface> surf =
      gfx::Factory::CreateWrappingDataSourceSurface(
          wrappableData, stride, size, surfFormat);
  if (!surf) {
  ...
  return MakeUnique<webgl::TexUnpackSurface>(
      webgl, target, width, height, depth, surf, alphaType);
}

UniquePtr<webgl::TexUnpackBlob>
WebGLContext::FromDomElem(const char* funcName,
                          TexImageTarget target,
                          uint32_t width,
                          uint32_t height,
                          uint32_t depth,
                          ...
                          ErrorResult* const out_error)
{
  ...
  if (!layersImage && !dataSurf) {
    const bool isClientData = true;
    return MakeUnique<webgl::TexUnpackBytes>(
        this, target, width, height, depth, isClientData, nullptr, 0);
  }
  ...
  if (layersImage) {
    return MakeUnique<webgl::TexUnpackImage>(
        this, target, width, height, depth, layersImage, sfer.mAlphaType);
  }

  MOZ_ASSERT(dataSurf);
  return MakeUnique<webgl::TexUnpackSurface>(
      this, target, width, height, depth, dataSurf, sfer.mAlphaType);
}

////////////////////////////////////////

UniquePtr<webgl::TexUnpackBlob>
WebGLContext::From(const char* funcName,
                   TexImageTarget target,
                   GLsizei rawWidth,
                   GLsizei rawHeight,
                   GLsizei rawDepth,
                   ...
                   dom::Uint8ClampedArray* const scopedArr)
{
  ...
  if (src.mImageBitmap) {
    return FromImageBitmap(
        this, funcName, target, width, height, depth, *(src.mImageBitmap));
  }

  if (src.mImageData) {
    return FromImageData(this,
                         funcName,
                         target,
                         width,
                         height,
...
static UniquePtr<webgl::TexUnpackBlob>
ValidateTexOrSubImage(WebGLContext* webgl,
                      const char* funcName,
                      TexImageTarget target,
                      GLsizei rawWidth,
                      GLsizei rawHeight,
                      ...
                      dom::Uint8ClampedArray* const scopedArr)
{
  ...
  auto blob = webgl->From(
      funcName, target, rawWidth, rawHeight, rawDepth, border, src, scopedArr);
...
void
WebGLTexture::TexImage(const char* funcName,
                       TexImageTarget target,
                       GLint level,
                       ...
                       const TexImageSource& src)
{
  dom::RootedSpiderMonkeyInterface<dom::Uint8ClampedArray> scopedArr(
      dom::RootingCx());
  const auto blob = ValidateTexOrSubImage(mContext,
                                          funcName,
                                          target,
                                          width,
                                          height,
...
void
WebGLTexture::TexSubImage(const char* funcName,
                          TexImageTarget target,
                          GLint level,
                          GLint xOffset,
                          GLint yOffset,
                          ...
                          const TexImageSource& src)
{
  ...
  TexSubImage(
      funcName, target, level, xOffset, yOffset, zOffset, pi, blob.get());
}
...
static bool
ValidateTexImage(WebGLContext* webgl,
...
// For *TexImage*
bool
WebGLTexture::ValidateTexImageSpecification(
    const char* funcName,
    TexImageTarget target,
    GLint rawLevel,
    uint32_t width,
...
// For *TexSubImage*
bool
WebGLTexture::ValidateTexImageSelection(
    const char* funcName,
    TexImageTarget target,
    GLint level,
    GLint xOffset,
...
static bool
ValidateCompressedTexUnpack(WebGLContext* webgl,
                            const char* funcName,
                            GLsizei width,
                            GLsizei height,
                            GLsizei depth,
             ...
             GLsizei depth)
{
  ...
  switch (target.get()) {
    ...
    case LOCAL_GL_TEXTURE_2D_ARRAY:
      gl->fTexStorage3D(
          target.get(), levels, sizedFormat, width, height, depth);
      break;
...
static inline GLenum
DoCompressedTexImage(gl::GLContext* gl,
                     TexImageTarget target,
                     GLint level,
                     GLenum internalFormat,
                     GLsizei width,
                     ...
                     const void* data)
{
  ...
  if (IsTarget3D(target)) {
    gl->fCompressedTexImage3D(target.get(),
                              level,
                              internalFormat,
                              width,
                              height,
    ...
    MOZ_ASSERT(depth == 1);
    gl->fCompressedTexImage2D(target.get(),
                              level,
                              internalFormat,
                              width,
                              height,
...
GLenum
DoCompressedTexSubImage(gl::GLContext* gl,
                        TexImageTarget target,
                        GLint level,
                        GLint xOffset,
                        GLint yOffset,
                        ...
                        const void* data)
{
  ...
  if (IsTarget3D(target)) {
    gl->fCompressedTexSubImage3D(target.get(),
                                 level,
                                 xOffset,
                                 yOffset,
                                 zOffset,
    ...
    MOZ_ASSERT(zOffset == 0);
    MOZ_ASSERT(depth == 1);
    gl->fCompressedTexSubImage2D(target.get(),
                                 level,
                                 xOffset,
                                 yOffset,
                                 width,
...
static inline GLenum
DoCopyTexSubImage(gl::GLContext* gl,
                  TexImageTarget target,
                  GLint level,
                  GLint xOffset,
                  GLint yOffset,
                  ...
                  GLsizei height)
{
  ...
  if (IsTarget3D(target)) {
    gl->fCopyTexSubImage3D(
        target.get(), level, xOffset, yOffset, zOffset, x, y, width, height);
  } else {
    MOZ_ASSERT(zOffset == 0);
    gl->fCopyTexSubImage2D(
        target.get(), level, xOffset, yOffset, x, y, width, height);
  }
...
static bool
ValidateCompressedTexImageRestrictions(const char* funcName,
                                       WebGLContext* webgl,
                                       TexImageTarget target,
                                       uint32_t level,
                                       const webgl::FormatInfo* format,
                                       ...
                                       uint32_t depth)
{
  ...
  switch (format->compression->family) {
    ...
    case webgl::CompressionFamily::PVRTC:
      if (!IsPowerOfTwo(width) || !IsPowerOfTwo(height)) {
        webgl->ErrorInvalidValue(
            "%s: %s requires power-of-two width and height.",
            funcName,
            format->name);
        return false;
      }
...
static bool
ValidateTargetForFormat(const char* funcName,
                        WebGLContext* webgl,
                        TexImageTarget target,
                        const webgl::FormatInfo* format)
{
  ...
  switch (format->effectiveFormat) {
    ...
    case webgl::EffectiveFormat::COMPRESSED_RGBA_S3TC_DXT5_EXT:
      if (target == LOCAL_GL_TEXTURE_3D) {
        webgl->ErrorInvalidOperation(
            "%s: Format %s cannot be used with TEXTURE_3D.",
            funcName,
            format->name);
        return false;
      }
...
void
WebGLTexture::TexStorage(const char* funcName,
                         TexTarget target,
                         GLsizei levels,
                         GLenum sizedFormat,
                         GLsizei width,
                         ...
                         GLsizei depth)
{
  ...
  if (!ValidateTexImageSpecification(funcName,
                                     testTarget,
                                     testLevel,
                                     width,
                                     height,
  ...
  if (!dstUsage) {
    mContext->ErrorInvalidEnum(
        "%s: Invalid internalformat: 0x%04x", funcName, sizedFormat);
    return;
  }
  ...
  if (dstFormat->compression) {
    if (!ValidateCompressedTexImageRestrictions(funcName,
                                                mContext,
                                                testTarget,
                                                testLevel,
                                                dstFormat,
  ...
  GLenum error = DoTexStorage(
      mContext->gl, target.get(), levels, sizedFormat, width, height, depth);
  ...
  const WebGLTexture::ImageInfo newInfo(
      dstUsage, width, height, depth, isDataInitialized);
                       ...
                       const webgl::TexUnpackBlob* blob)
{
  ...
  if (!fua->IsInternalFormatEnumValid(internalFormat)) {
    mContext->ErrorInvalidValue(
        "%s: Invalid internalformat: 0x%04x", funcName, internalFormat);
    return;
  }
  ...
  const ImageInfo newImageInfo(
      dstUsage, blob->mWidth, blob->mHeight, blob->mDepth, blob->HasData());
  ...
  GLenum glError;
  if (!blob->TexOrSubImage(isSubImage,
                           needsRespec,
                           funcName,
                           this,
                           target,
                          ...
                          const webgl::TexUnpackBlob* blob)
{
  ...
  WebGLTexture::ImageInfo* imageInfo;
  if (!ValidateTexImageSelection(funcName,
                                 target,
                                 level,
                                 xOffset,
                                 yOffset,
  ...
  if (!EnsureImageDataInitializedForUpload(this,
                                           funcName,
                                           target,
                                           level,
                                           xOffset,
...
WebGLContext::FromCompressed(const char* funcName,
                             TexImageTarget target,
                             GLsizei rawWidth,
                             GLsizei rawHeight,
                             GLsizei rawDepth,
...
void
WebGLTexture::CompressedTexImage(const char* funcName,
                                 TexImageTarget target,
                                 GLint level,
                                 GLenum internalFormat,
                                 GLsizei rawWidth,
                                 ...
                                 const Maybe<GLsizei>& expectedImageSize)
{
  const auto blob = mContext->FromCompressed(funcName,
                                             target,
                                             rawWidth,
                                             rawHeight,
                                             rawDepth,
  ...
  if (!ValidateCompressedTexUnpack(mContext,
                                   funcName,
                                   blob->mWidth,
                                   blob->mHeight,
                                   blob->mDepth,
  ...
  mContext->gl->MakeCurrent();
  const ScopedLazyBind bindPBO(mContext->gl,
                               LOCAL_GL_PIXEL_UNPACK_BUFFER,
                               mContext->mBoundPixelUnpackBuffer);

  // Warning: Possibly shared memory.  See bug 1225033.
  GLenum error = DoCompressedTexImage(mContext->gl,
                                      target,
                                      level,
                                      internalFormat,
                                      blob->mWidth,
...
void
WebGLTexture::CompressedTexSubImage(const char* funcName,
                                    TexImageTarget target,
                                    GLint level,
                                    GLint xOffset,
                                    GLint yOffset,
                                    ...
                                    const Maybe<GLsizei>& expectedImageSize)
{
  ...
  switch (format->compression->family) {
    ...
    // Block-aligned:
    case webgl::CompressionFamily::
        ES3:  // Yes, the ES3 formats don't match the ES3
    case webgl::CompressionFamily::S3TC:  // default behavior.
      if (!IsSubImageBlockAligned(dstFormat->compression,
                                  imageInfo,
                                  xOffset,
                                  yOffset,
                                  blob->mWidth,
  ...
  // Warning: Possibly shared memory.  See bug 1225033.
  GLenum error = DoCompressedTexSubImage(mContext->gl,
                                         target,
                                         level,
                                         xOffset,
                                         yOffset,
...
class ScopedCopyTexImageSource
{
 ...
 public:
  ScopedCopyTexImageSource(WebGLContext* webgl,
                           const char* funcName,
                           uint32_t srcWidth,
                           uint32_t srcHeight,
                           const webgl::FormatInfo* srcFormat,
    ...
    : mWebGL(webgl), mRB(0), mFB(0)
{
  ...
  gl::ScopedBindTexture scopedBindTex(
      gl, scopedTex.Texture(), LOCAL_GL_TEXTURE_2D);

  gl->fTexParameteri(
      LOCAL_GL_TEXTURE_2D, LOCAL_GL_TEXTURE_MIN_FILTER, LOCAL_GL_NEAREST);
  gl->fTexParameteri(
      LOCAL_GL_TEXTURE_2D, LOCAL_GL_TEXTURE_MAG_FILTER, LOCAL_GL_NEAREST);
  ...
  gl->fTexParameteri(
      LOCAL_GL_TEXTURE_2D, LOCAL_GL_TEXTURE_SWIZZLE_R, blitSwizzle[0]);
  gl->fTexParameteri(
      LOCAL_GL_TEXTURE_2D, LOCAL_GL_TEXTURE_SWIZZLE_G, blitSwizzle[1]);
  gl->fTexParameteri(
      LOCAL_GL_TEXTURE_2D, LOCAL_GL_TEXTURE_SWIZZLE_B, blitSwizzle[2]);
  gl->fTexParameteri(
      LOCAL_GL_TEXTURE_2D, LOCAL_GL_TEXTURE_SWIZZLE_A, blitSwizzle[3]);

  gl->fCopyTexImage2D(
      LOCAL_GL_TEXTURE_2D, 0, sizedFormat, 0, 0, srcWidth, srcHeight, 0);
  ...
  gl->fRenderbufferStorage(
      LOCAL_GL_RENDERBUFFER, sizedFormat, srcWidth, srcHeight);
  ...
  const gfx::IntSize srcSize(srcWidth, srcHeight);
  {
    ...
    gl->BlitHelper()->DrawBlitTextureToFramebuffer(
        scopedTex.Texture(), srcSize, srcSize);
  }
...
ValidateCopyDestUsage(const char* funcName,
                      WebGLContext* webgl,
                      const webgl::FormatInfo* srcFormat,
                      GLenum internalFormat)
{
  ...
  if (!dstUsage) {
    ...
    if (!GetUnsizedFormatForCopy(internalFormat, &unsizedFormat)) {
      webgl->ErrorInvalidEnum(
          "%s: Unrecongnized internalFormat 0x%04x.", funcName, internalFormat);
      return nullptr;
    }
...
bool
WebGLTexture::ValidateCopyTexImageForFeedback(const char* funcName,
                                              uint32_t level,
                                              GLint layer) const
{
...
static bool
DoCopyTexOrSubImage(WebGLContext* webgl,
                    const char* funcName,
                    bool isSubImage,
                    ...
                    const webgl::FormatUsageInfo* dstUsage)
{
  ...
  if (!Intersect(
          srcTotalWidth, xWithinSrc, dstWidth, &readX, &writeX, &rwWidth) ||
      !Intersect(
          srcTotalHeight, yWithinSrc, dstHeight, &readY, &writeY, &rwHeight)) {
  ...
  do {
    ...
    if (!isSubImage) {
      ...
      error = DoTexImage(
          gl, target, level, idealUnpack, dstWidth, dstHeight, 1, buffer.get());
      if (error) break;
    }
    ...
    ScopedCopyTexImageSource maybeSwizzle(
        webgl, funcName, srcTotalWidth, srcTotalHeight, srcFormat, dstUsage);

    error = DoCopyTexSubImage(gl,
                              target,
                              level,
                              writeX,
                              writeY,
...
void
WebGLTexture::CopyTexImage2D(TexImageTarget target,
                             GLint level,
                             GLenum internalFormat,
                             GLint x,
                             GLint y,
                             ...
                             GLint border)
{
  ...
  WebGLTexture::ImageInfo* imageInfo;
  if (!ValidateTexImageSpecification(
          funcName, target, level, width, height, depth, &imageInfo)) {
    return;
  }
  MOZ_ASSERT(imageInfo);
  ...
  uint32_t srcTotalWidth;
  uint32_t srcTotalHeight;
  if (!mContext->ValidateCurFBForRead(
          funcName, &srcUsage, &srcTotalWidth, &srcTotalHeight)) {
    return;
  }
  ...
  if (!mContext->IsWebGL2() && dstFormat->d) {
    mContext->ErrorInvalidOperation(
        "%s: Function may not be called with format %s.",
        funcName,
        dstFormat->name);
    return;
  }
  ...
  const bool isSubImage = false;
  if (!DoCopyTexOrSubImage(mContext,
                           funcName,
                           isSubImage,
                           this,
                           target,
...
void
WebGLTexture::CopyTexSubImage(const char* funcName,
                              TexImageTarget target,
                              GLint level,
                              GLint xOffset,
                              GLint yOffset,
