...
void
WebGLBuffer::BufferData(GLenum target,
                        size_t size,
                        const void* data,
                        GLenum usage)
{
  ...
  if (sizeChanges) {
    ...
    if (error) {
      ...
      mContext->ErrorOutOfMemory(
          "%s: Error from driver: 0x%04x", funcName, error);
      return;
    }
  } else {
    gl->fBufferData(target, size, uploadData, usage);
  ...
  if (mIndexCache) {
    if (!mIndexRanges.empty()) {
      mContext->GeneratePerfWarning(
          "[%p] Invalidating %u ranges.", this, uint32_t(mIndexRanges.size()));
      mIndexRanges.clear();
    }
  }
}
...
void
WebGLBuffer::BufferSubData(GLenum target,
...
bool
WebGLBuffer::ValidateRange(const char* funcName,
                           size_t byteOffset,
...
WebGLBuffer::InvalidateCacheRange(size_t byteOffset, size_t byteLength) const
{
  ...
  if (!invalids.empty()) {
    mContext->GeneratePerfWarning("[%p] Invalidating %u/%u ranges.",
                                  this,
...
template<typename T>
static size_t
MaxForRange(const void* data,
            size_t first,
            size_t count,
            const uint32_t ignoredVal)
{
...
bool
WebGLBuffer::ValidateIndexedFetch(GLenum type,
                                  uint32_t numFetchable,
                                  size_t first,
                                  size_t count) const
{
  if (!mIndexCache) return true;
  ...
  if (mIndexRanges.size() > kMaxIndexRanges) {
    mContext->GeneratePerfWarning(
        "[%p] Clearing mIndexRanges after exceeding %u.",
        this,
        kMaxIndexRanges);
    mIndexRanges.clear();
  ...
  if (didInsert) {
    ...
    mContext->GeneratePerfWarning("[%p] New range #%u: (0x%04x, %u, %u): %u",
                                  this,
                                  uint32_t(mIndexRanges.size()),
                                  type,
                                  uint32_t(first),
...
WebGLBuffer::ValidateCanBindToTarget(const char* funcName, GLenum target)
{
  ...
  mContext->ErrorInvalidOperation(
      "%s: Buffer already contains %s data.", funcName, dataType);
  return false;
}
