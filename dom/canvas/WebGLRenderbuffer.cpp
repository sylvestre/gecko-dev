...
static GLenum
DoRenderbufferStorageMaybeMultisample(gl::GLContext* gl,
                                      GLsizei samples,
                                      ...
                                      GLsizei height)
{
  ...
  if (samples > 0) {
    gl->fRenderbufferStorageMultisample(
        LOCAL_GL_RENDERBUFFER, samples, internalFormat, width, height);
  } else {
    gl->fRenderbufferStorage(
        LOCAL_GL_RENDERBUFFER, internalFormat, width, height);
  }
                                         ...
                                         uint32_t height)
{
  ...
  if (secondaryFormat) {
    ...
    error = DoRenderbufferStorageMaybeMultisample(
        gl, samples, secondaryFormat, width, height);
    if (error) return error;
...
void
WebGLRenderbuffer::RenderbufferStorage(const char* funcName,
                                       uint32_t samples,
                                       GLenum internalFormat,
                                       uint32_t width,
                                       uint32_t height)
...
void
WebGLRenderbuffer::DoFramebufferRenderbuffer(FBTarget target,
                                             GLenum attachment) const
{
  ...
  if (attachment == LOCAL_GL_DEPTH_STENCIL_ATTACHMENT) {
    ...
    gl->fFramebufferRenderbuffer(target.get(),
  ...
  gl->fFramebufferRenderbuffer(
      target.get(), attachment, LOCAL_GL_RENDERBUFFER, mPrimaryRB);
}
