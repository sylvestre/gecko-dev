...
NS_IMETHODIMP
WebGLContext::SetContextOptions(JSContext* cx,
                                ...
                                ErrorResult& aRvForDictionaryInit)
{
  ...
  if (attributes.mAlpha.WasPassed()) newOpts.alpha = attributes.mAlpha.Value();

  // Don't do antialiasing if we've disabled MSAA.
  if (!gfxPrefs::MSAALevel()) newOpts.antialias = false;
...
static bool
IsFeatureInBlacklist(const nsCOMPtr<nsIGfxInfo>& gfxInfo,
                     int32_t feature,
...
BaseCaps(const WebGLContextOptions& options, WebGLContext* webgl)
{
  ...
  if (!baseCaps.alpha) baseCaps.premultAlpha = true;
  ...
  if (!gfxPrefs::WebGLForceMSAA()) {
    ...
    if (IsFeatureInBlacklist(
            gfxInfo, nsIGfxInfo::FEATURE_WEBGL_MSAA, &blocklistId)) {
      webgl->GenerateWarning(
          "Disallowing antialiased backbuffers due"
          " to blacklisting.");
...
CreateGLWithEGL(const gl::SurfaceCaps& caps,
                ...
                std::vector<WebGLContext::FailureReason>* const out_failReasons)
{
  ...
  nsCString failureId;
  RefPtr<GLContext> gl = gl::GLContextProviderEGL::CreateOffscreen(
      dummySize, caps, flags, &failureId);
  if (gl && gl->IsANGLE()) {
    gl = nullptr;
  }
  ...
  if (!gl) {
    out_failReasons->push_back(WebGLContext::FailureReason(
        failureId, "Error during EGL OpenGL init."));
    return nullptr;
  }
    ...
    std::vector<WebGLContext::FailureReason>* const out_failReasons)
{
  ...
  if (!gl) {
    out_failReasons->push_back(WebGLContext::FailureReason(
        failureId, "Error during ANGLE OpenGL init."));
    return nullptr;
  }
    ...
    std::vector<WebGLContext::FailureReason>* const out_failReasons)
{
  const gfx::IntSize dummySize(16, 16);
  nsCString failureId;
  RefPtr<GLContext> gl = gl::GLContextProvider::CreateOffscreen(
      dummySize, caps, flags, &failureId);
  if (gl && gl->IsANGLE()) {
    gl = nullptr;
  }
  ...
  if (!gl) {
    out_failReasons->push_back(WebGLContext::FailureReason(
        failureId, "Error during native OpenGL init."));
    return nullptr;
  }
...
bool
WebGLContext::CreateAndInitGLWith(
    FnCreateGL_T fnCreateGL,
    ...
    std::vector<FailureReason>* const out_failReasons)
{
  ...
  while (!fallbackCaps.empty()) {
    ...
    if (potentialGL) break;

    fallbackCaps.pop();
  }
  if (!potentialGL) {
    out_failReasons->push_back(FailureReason(
        "FEATURE_FAILURE_WEBGL_EXHAUSTED_CAPS", "Exhausted GL driver caps."));
    return false;
  }
                              ...
                              std::vector<FailureReason>* const out_failReasons)
{
  ...
  if (tryNativeGL) {
    if (useEGL)
      return CreateAndInitGLWith(
          CreateGLWithEGL, baseCaps, flags, out_failReasons);

    if (CreateAndInitGLWith(
            CreateGLWithDefault, baseCaps, flags, out_failReasons))
      return true;
  }
  ...
  if (tryANGLE) {
    ...
    return CreateAndInitGLWith(
        CreateGLWithANGLE, angleCaps, flags, out_failReasons);
  }
...
WebGLContext::SetDimensions(int32_t signedWidth, int32_t signedHeight)
{
  ...
  if (gl) {
    if ((uint32_t)mWidth == width && (uint32_t)mHeight == height) {
      return NS_OK;
    }
  ...
  if (gl->WorkAroundDriverBugs()) {
    if (!mOptions.alpha && gl->Caps().alpha) mNeedsFakeNoAlpha = true;

    if (!mOptions.depth && gl->Caps().depth) mNeedsFakeNoDepth = true;

    if (!mOptions.stencil && gl->Caps().stencil) mNeedsFakeNoStencil = true;
  ...
  // Update mOptions.
  if (!gl->Caps().depth) mOptions.depth = false;

  if (!gl->Caps().stencil) mOptions.stencil = false;
...
{
  if (!mBackbufferNeedsClear) return;
...
WebGLContext::LoseOldestWebGLContextIfLimitExceeded()
{
  ...
  if (contexts.Length() <= kMaxWebGLContextsPerPrincipal) return;
  ...
  for (size_t i = 0; i < contexts.Length(); ++i) {
    ...
    if (contexts[i]->IsContextLost()) continue;
  ...
  if (numContextsThisPrincipal > kMaxWebGLContextsPerPrincipal) {
    GenerateWarning(
        "Exceeded %zu live WebGL contexts for this principal, losing the "
        "least recently used one.",
        kMaxWebGLContextsPerPrincipal);
    MOZ_ASSERT(
        oldestContextThisPrincipal);  // if we reach this point, this can't be null
    const_cast<WebGLContext*>(oldestContextThisPrincipal)->LoseContext();
  } else if (numContexts > kMaxWebGLContexts) {
    GenerateWarning(
        "Exceeded %zu live WebGL contexts, losing the least "
        "recently used one.",
        kMaxWebGLContexts);
...
WebGLContext::GetImageBuffer(int32_t* out_format)
{
  *out_format = 0;
  ...
  return gfxUtils::GetImageBuffer(
      dataSurface, mOptions.premultipliedAlpha, out_format);
}
                             ...
                             nsIInputStream** out_stream)
{
  ...
  RefPtr<DataSourceSurface> dataSurface = snapshot->GetDataSurface();
  return gfxUtils::GetInputStream(dataSurface,
                                  mOptions.premultipliedAlpha,
                                  mimeType,
                                  encoderOptions,
                                  out_stream);
}
...
WebGLContext::ClearScreen()
{
  ...
  if (mOptions.depth) bufferBits |= LOCAL_GL_DEPTH_BUFFER_BIT;
  if (mOptions.stencil) bufferBits |= LOCAL_GL_STENCIL_BUFFER_BIT;
                                                     ...
                                                     bool fakeNoAlpha)
{
  ...
  const bool initializeColorBuffer =
      bool(clearBits & LOCAL_GL_COLOR_BUFFER_BIT);
  const bool initializeDepthBuffer =
      bool(clearBits & LOCAL_GL_DEPTH_BUFFER_BIT);
  const bool initializeStencilBuffer =
      bool(clearBits & LOCAL_GL_STENCIL_BUFFER_BIT);
  ...
  // And reset!
  if (mScissorTestEnabled) gl->fEnable(LOCAL_GL_SCISSOR_TEST);
...
{
  if (NS_FAILED(SetDimensions(mWidth, mHeight))) return false;
...
WebGLContext::GetSurfaceSnapshot(gfxAlphaType* const out_alphaType)
{
  ...
  surf = Factory::CreateDataSourceSurfaceWithStride(
      IntSize(mWidth, mHeight), surfFormat, mWidth * 4);
...
bool
WebGLContext::ValidateCurFBForRead(
    const char* funcName,
    ...
    uint32_t* const out_height)
{
  if (!mBoundReadFramebuffer) {
    ...
    if (readBufferMode == LOCAL_GL_NONE) {
      ErrorInvalidOperation(
          "%s: Can't read from backbuffer when readBuffer mode is"
          " NONE.",
          funcName);
      return false;
    }
  ...
  return mBoundReadFramebuffer->ValidateForRead(
      funcName, out_format, out_width, out_height);
}
...
/*static*/ bool
WebGLContext::ScopedDrawCallWrapper::HasDepthButNoStencil(
...
{
  if (mWebGL->mPixelStore_UnpackAlignment != 4)
    mGL->fPixelStorei(LOCAL_GL_UNPACK_ALIGNMENT, 4);

  if (mWebGL->IsWebGL2()) {
    if (mWebGL->mPixelStore_UnpackRowLength != 0)
      mGL->fPixelStorei(LOCAL_GL_UNPACK_ROW_LENGTH, 0);
    if (mWebGL->mPixelStore_UnpackImageHeight != 0)
      mGL->fPixelStorei(LOCAL_GL_UNPACK_IMAGE_HEIGHT, 0);
    if (mWebGL->mPixelStore_UnpackSkipPixels != 0)
      mGL->fPixelStorei(LOCAL_GL_UNPACK_SKIP_PIXELS, 0);
    if (mWebGL->mPixelStore_UnpackSkipRows != 0)
      mGL->fPixelStorei(LOCAL_GL_UNPACK_SKIP_ROWS, 0);
    if (mWebGL->mPixelStore_UnpackSkipImages != 0)
      mGL->fPixelStorei(LOCAL_GL_UNPACK_SKIP_IMAGES, 0);
...
{
  mGL->fPixelStorei(LOCAL_GL_UNPACK_ALIGNMENT,
                    mWebGL->mPixelStore_UnpackAlignment);

  if (mWebGL->IsWebGL2()) {
    mGL->fPixelStorei(LOCAL_GL_UNPACK_ROW_LENGTH,
                      mWebGL->mPixelStore_UnpackRowLength);
    mGL->fPixelStorei(LOCAL_GL_UNPACK_IMAGE_HEIGHT,
                      mWebGL->mPixelStore_UnpackImageHeight);
    mGL->fPixelStorei(LOCAL_GL_UNPACK_SKIP_PIXELS,
                      mWebGL->mPixelStore_UnpackSkipPixels);
    mGL->fPixelStorei(LOCAL_GL_UNPACK_SKIP_ROWS,
                      mWebGL->mPixelStore_UnpackSkipRows);
    mGL->fPixelStorei(LOCAL_GL_UNPACK_SKIP_IMAGES,
                      mWebGL->mPixelStore_UnpackSkipImages);
...
ScopedLazyBind::ScopedLazyBind(gl::GLContext* gl,
                               GLenum target,
...
bool
Intersect(const int32_t srcSize,
          const int32_t read0,
          const int32_t readSize,
          int32_t* const out_intRead0,
          int32_t* const out_intWrite0,
...
CheckedUint32
WebGLContext::GetUnpackSize(bool isFunc3D,
                            uint32_t width,
                            uint32_t height,
                            uint32_t depth,
                                      ...
                                      size_t* const out_byteLen)
{
  ...
  if (elemCountOverride) {
    if (elemCountOverride > elemCount) {
      ErrorInvalidValue("%s: Invalid sub-length for ArrayBufferView.",
                        funcName);
      return false;
    }
