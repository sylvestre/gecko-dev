...
WebGLContext::ErrorInvalidEnumInfo(const char* info, GLenum enumValue)
{
  ...
  return ErrorInvalidEnum(
      "%s: invalid enum value %s", info, name.BeginReading());
}

void
WebGLContext::ErrorInvalidEnumInfo(const char* info,
                                   const char* funcName,
                                   GLenum enumValue)
{
  nsCString name;
  ...
  ErrorInvalidEnum("%s: %s: Invalid enum: 0x%04x (%s).",
                   funcName,
                   info,
                   enumValue,
                   name.BeginReading());
...
void
WebGLContext::ErrorInvalidEnumArg(const char* funcName,
                                  const char* argName,
                                  GLenum val)
{
  nsCString enumName;
  ...
  ErrorInvalidEnum(
      "%s: Bad `%s`: %s", funcName, argName, enumName.BeginReading());
}
...
void
AssertMaskedUintParamCorrect(gl::GLContext* gl,
                             GLenum pname,
                             GLuint mask,
                             GLuint shadow)
{
...
WebGLContext::AssertCachedBindings()
{
  ...
  // Framebuffers
  if (IsWebGL2()) {
    GLuint bound = mBoundDrawFramebuffer ? mBoundDrawFramebuffer->mGLName : 0;
  ...
  if (GetStencilBits(&stencilBits)) {  // Depends on current draw framebuffer.
    ...
    AssertMaskedUintParamCorrect(
        gl, LOCAL_GL_STENCIL_REF, stencilRefMask, mStencilRefFront);
    AssertMaskedUintParamCorrect(
        gl, LOCAL_GL_STENCIL_BACK_REF, stencilRefMask, mStencilRefBack);
  }
...
WebGLContext::AssertCachedGlobalState()
{
  ...
  AssertMaskedUintParamCorrect(
      gl, LOCAL_GL_STENCIL_CLEAR_VALUE, maxStencilBitsMask, mStencilClearValue);
  ...
  AssertMaskedUintParamCorrect(gl,
  ...
  AssertUintParamCorrect(
      gl, LOCAL_GL_PACK_ALIGNMENT, mPixelStore_PackAlignment);
  AssertUintParamCorrect(
      gl, LOCAL_GL_UNPACK_ALIGNMENT, mPixelStore_UnpackAlignment);

  if (IsWebGL2()) {
    AssertUintParamCorrect(
        gl, LOCAL_GL_UNPACK_IMAGE_HEIGHT, mPixelStore_UnpackImageHeight);
    AssertUintParamCorrect(
        gl, LOCAL_GL_UNPACK_SKIP_IMAGES, mPixelStore_UnpackSkipImages);
    AssertUintParamCorrect(
        gl, LOCAL_GL_UNPACK_ROW_LENGTH, mPixelStore_UnpackRowLength);
    AssertUintParamCorrect(
        gl, LOCAL_GL_UNPACK_SKIP_ROWS, mPixelStore_UnpackSkipRows);
    AssertUintParamCorrect(
        gl, LOCAL_GL_UNPACK_SKIP_PIXELS, mPixelStore_UnpackSkipPixels);
    AssertUintParamCorrect(
        gl, LOCAL_GL_PACK_ROW_LENGTH, mPixelStore_PackRowLength);
    AssertUintParamCorrect(
        gl, LOCAL_GL_PACK_SKIP_ROWS, mPixelStore_PackSkipRows);
    AssertUintParamCorrect(
        gl, LOCAL_GL_PACK_SKIP_PIXELS, mPixelStore_PackSkipPixels);
  }
