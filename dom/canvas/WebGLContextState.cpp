...
WebGLContext::Disable(GLenum cap)
{
  ...
  if (!ValidateCapabilityEnum(cap, "disable")) return;
...
WebGLContext::Enable(GLenum cap)
{
  ...
  if (!ValidateCapabilityEnum(cap, "enable")) return;
...
bool
WebGLContext::GetChannelBits(const char* funcName,
                             GLenum pname,
                             GLint* const out_val)
{
  if (mBoundDrawFramebuffer) {
  ...
  gl->fGetFramebufferAttachmentParameteriv(
      LOCAL_GL_DRAW_FRAMEBUFFER, fbAttachment, fbPName, out_val);
  return true;
}
  ...
  switch (pname) {
    ...
    case LOCAL_GL_STENCIL_BACK_VALUE_MASK:
      return JS::DoubleValue(
          mStencilValueMaskBack);  // pass as FP value to allow large values such as 2^32-1.
    ...
    case LOCAL_GL_COLOR_WRITEMASK: {
      ...
      bool vals[4] = {
          bool(gl_bv[0]), bool(gl_bv[1]), bool(gl_bv[2]), bool(gl_bv[3])};
    ...
    case LOCAL_GL_TEXTURE_BINDING_2D: {
      return WebGLObjectAsJSValue(
          cx, mBound2DTextures[mActiveTexture].get(), rv);
    }
...
void
WebGLContext::GetParameterIndexed(JSContext* cx,
                                  GLenum pname,
                                  GLuint index,
