...
void
nsKeygenThread::SetParams(PK11SlotInfo* a_slot,
...
nsresult
nsKeygenThread::ConsumeResult(PK11SlotInfo** a_used_slot,
...
nsresult
nsKeygenThread::StartKeyGeneration(nsIObserver* aObserver)
{
  if (!NS_IsMainThread()) {
  ...
  threadHandle = PR_CreateThread(PR_USER_THREAD,
...
nsresult
nsKeygenThread::UserCanceled(bool* threadAlreadyClosedDialog)
{
...
nsKeygenThread::Run(void)
{
  ...
  if (canGenerate) {
    privateKey = PK11_GenerateKeyPairWithFlags(
        slot, keyGenMechanism, params, &publicKey, flags, wincx);
