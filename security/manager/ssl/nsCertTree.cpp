...
CompareCacheHashEntry::CompareCacheHashEntry() : key(nullptr)
{
...
static const PLDHashTableOps gMapOps = {PLDHashTable::HashVoidPtrKeyStub,
...
nsCertTree::nsCertTree()
    : mTreeArray(nullptr),
      mCompareCache(
          &gMapOps, sizeof(CompareCacheHashEntryPtr), kInitialCacheLength)
{
...
void
nsCertTree::RemoveCacheEntry(void* key)
{
...
nsCertTree::GetDispInfoAtIndex(int32_t index, int32_t* outAbsoluteCertOffset)
{
...
static void
MatchingCertOverridesCallback(const nsCertOverride& aSettings, void* aUserData)
{
  ...
  nsAutoCString hostPort;
  nsCertOverrideService::GetHostWithPort(
      aSettings.mAsciiHost, aSettings.mPort, hostPort);
  ...
  for (node = CERT_LIST_HEAD(aCertList); !CERT_LIST_END(node, aCertList);
    ...
    if (!wantThisCert) {
      ...
      if (aWantedType == nsIX509Cert::SERVER_CERT &&
        ...
        // If yes, display them.
        addOverrides = true;
      } else if (aWantedType == nsIX509Cert::UNKNOWN_CERT &&
                 thisCertType == nsIX509Cert::UNKNOWN_CERT) {
        // This unknown cert was stored without trust.
                           ...
                           void* aCertCmpFnArg)
{
  ...
  return GetCertsByTypeFromCertList(
      certList.get(), aType, aCertCmpFn, aCertCmpFnArg);
}
...
NS_IMETHODIMP
nsCertTree::GetCellProperties(int32_t row,
                              nsITreeColumn* col,
                              nsAString& aProps)
{
  return NS_OK;
...
NS_IMETHODIMP
nsCertTree::HasNextSibling(int32_t rowIndex, int32_t afterIndex, bool* _retval)
{
  if (!mTreeArray) return NS_ERROR_NOT_INITIALIZED;
...
nsCertTree::GetCellText(int32_t row, nsITreeColumn* col, nsAString& _retval)
{
  ...
  if (NS_LITERAL_STRING("certcol").Equals(colID)) {
    ...
    if (certdi->mTypeOfEntry == nsCertTreeDispInfo::host_port_override) {
      nsAutoCString hostPort;
      nsCertOverrideService::GetHostWithPort(
          certdi->mAsciiHost, certdi->mPort, hostPort);
...
NS_IMETHODIMP
nsCertTree::SetCellValue(int32_t row,
                         nsITreeColumn* col,
...
NS_IMETHODIMP
nsCertTree::SetCellText(int32_t row, nsITreeColumn* col, const nsAString& value)
{
  return NS_OK;
}
...
NS_IMETHODIMP
nsCertTree::PerformActionOnCell(const char16_t* action,
                                int32_t row,
                                nsITreeColumn* col)
{
  return NS_OK;
...
nsCertTree::dumpMap()
{
  for (int i = 0; i < mNumOrgs; i++) {
    ...
    MOZ_LOG(
        gPIPNSSLog, LogLevel::Debug, ("INDEX[%d]", mTreeArray[i].certIndex));
    MOZ_LOG(
        gPIPNSSLog, LogLevel::Debug, ("NCHILD[%d]", mTreeArray[i].numChildren));
  }
...
void
nsCertTree::CmpInitCriterion(nsIX509Cert* cert,
  ...
  switch (crit) {
    ...
    case sort_IssuedDateDescending: {
      ...
      if (NS_SUCCEEDED(rv)) {
        ...
        if (0 != PR_FormatTime(
                     datebuf, sizeof(datebuf), "%Y%m%d%H%M%S", &explodedTime)) {
...
int32_t
nsCertTree::CmpByCrit(nsIX509Cert* a,
...
nsCertTree::CmpUserCert(void* cache, nsIX509Cert* a, nsIX509Cert* b)
{
  // XXX we assume issuer org is always criterion 1
  return CmpBy(
      cache, a, b, sort_IssuerOrg, sort_Token, sort_IssuedDateDescending);
}
