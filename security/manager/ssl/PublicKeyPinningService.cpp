...
GetBase64HashSPKI(const CERTCertificate* cert, nsACString& hashSPKIDigest)
{
  ...
  nsresult rv = digest.DigestBuf(
      SEC_OID_SHA256, cert->derPublicKey.data, cert->derPublicKey.len);
  if (NS_FAILED(rv)) {
    return rv;
  }
 ...
 */
static nsresult
EvalCert(const CERTCertificate* cert,
         ...
         /*out*/ bool& certMatchesPinset)
{
  ...
  if (!fingerprints && !dynamicFingerprints) {
    MOZ_LOG(
        gPublicKeyPinningLog, LogLevel::Debug, ("pkpin: No hashes found\n"));
          ...
          /*out*/ bool& certListIntersectsPinset)
{
  ...
  for (node = CERT_LIST_HEAD(certList); !CERT_LIST_END(node, certList);
    ...
    nsresult rv = EvalCert(currentCert,
                           fingerprints,
                           dynamicFingerprints,
...
nsresult
PublicKeyPinningService::ChainMatchesPinset(
    ...
    /*out*/ const TransportSecurityPreload*& staticFingerprints)
{
  ...
  while (!foundEntry && (evalPart = strchr(evalHost, '.'))) {
    ...
    rv = sssService->GetKeyPinsForHostname(nsDependentCString(evalHost),
                                           time,
                                           originAttributes,
                                           pinArray,
                                           &includeSubdomains,
    ...
    if (BinarySearchIf(kPublicKeyPinningPreloadList,
                       0,
    ...
    /*optional out*/ PinningTelemetryInfo* pinningTelemetryInfo)
{
  ...
  nsresult rv = FindPinningInformation(hostname,
                                       time,
                                       originAttributes,
                                       dynamicFingerprints,
                                       staticFingerprints);
  ...
  if (dynamicFingerprints.Length() > 0) {
    return EvalChain(
        certList, nullptr, &dynamicFingerprints, chainHasValidPins);
  }
  if (staticFingerprints) {
    bool enforceTestModeResult;
    rv = EvalChain(
        certList, staticFingerprints->pinset, nullptr, enforceTestModeResult);
    if (NS_FAILED(rv)) {
      return rv;
    }
    ...
    /*optional out*/ PinningTelemetryInfo* pinningTelemetryInfo)
{
  ...
  return CheckPinsForHostname(certList,
                              canonicalizedHostname.get(),
                              enforceTestMode,
                              time,
                              originAttributes,
                              chainHasValidPins,
                                     ...
                                     /*out*/ bool& hostHasPins)
{
  ...
  nsTArray<nsCString> dynamicFingerprints;
  const TransportSecurityPreload* staticFingerprints = nullptr;
  nsresult rv = FindPinningInformation(canonicalizedHostname.get(),
                                       time,
                                       originAttributes,
                                       dynamicFingerprints,
                                       staticFingerprints);
