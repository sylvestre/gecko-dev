...
nsresult
nsKeygenFormProcessor::Create(nsISupports* aOuter,
                              const nsIID& aIID,
                              void** aResult)
{
...
uint32_t
MapGenMechToAlgoMech(uint32_t mechanism)
{
  uint32_t searchMech;
  ...
  if (!slotList->head->next) {
    ...
    *aSlot = slotList->head->slot;
    ...
    if (!tokenNameList || !*tokenNameList) {
      ...
      rv = dialogs->ChooseToken(m_ctx,
                                (const char16_t**)tokenNameList,
                                numSlots,
                                tokenStr,
                                &canceled);
    }
                                    ...
                                    const nsAString& aKeyParams)
{
  ...
  if (NS_FAILED(rv) || !KeygenRunnable) {
    ...
    privateKey = PK11_GenerateKeyPairWithFlags(
        slot, keyGenMechanism, params, &publicKey, attrFlags, m_ctx);
  } else {
    KeygenRunnable->SetParams(
        slot, attrFlags, nullptr, 0, keyGenMechanism, params, m_ctx);
  ...
  /*
     ...
     */
  srv = DER_Encode(
      arena.get(), &spkiItem, CERTSubjectPublicKeyInfoTemplate, spkInfo);
  if (srv != SECSuccess) {
    goto loser;
  }
  ...
  srv = DER_Encode(
      arena.get(), &pkacItem, CERTPublicKeyAndChallengeTemplate, &pkac);
  if (srv != SECSuccess) {
    goto loser;
  }
                                    ...
                                    nsAString& aValue)
{
  ...
  return GetPublicKey(
      aValue, challengeValue, keyTypeValue, aValue, keyParamsValue);
}
...
{
  return GetPublicKey(
      aOldValue, aChallenge, PromiseFlatString(aKeyType), newValue, aKeyParams);
}
