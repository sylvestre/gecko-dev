...
const OCSPHost sOCSPHosts[] = {
    ...
    {"ocsp-stapling-bad-signature.example.com",
     ORTBadSignature,
     nullptr,
     nullptr},
    ...
    {"keysize-ocsp-delegated.example.com",
     ORTDelegatedIncluded,
     "rsa-1016-keysizeDelegatedSigner",
     nullptr},
    ...
    {"ocsp-stapling-must-staple-ee-with-must-staple-int.example.com",
     ORTGood,
     nullptr,
     "must-staple-ee-with-must-staple-int"},
    {"ocsp-stapling-plain-ee-with-must-staple-int.example.com",
     ORTGood,
     nullptr,
     "must-staple-missing-ee"},
...
int32_t
DoSNISocketConfig(PRFileDesc* aFd,
                  ...
                  void* aArg)
{
  ...
  SECItemArray* response = GetOCSPResponseForType(
      host->mORT, cert, arena, host->mAdditionalCertName, 0);
  if (!response) {
    return SSL_SNI_SEND_ALERT;
  }
