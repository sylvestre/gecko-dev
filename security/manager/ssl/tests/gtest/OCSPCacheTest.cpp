...
static void
PutAndGet(mozilla::psm::OCSPCache& cache,
          const CertID& certID,
          Result result,
          Time time,
...
TEST_F(psm_OCSPCacheTest, TestPutAndGet)
{
  ...
  SCOPED_TRACE("");
  PutAndGet(
      cache, CertID(fakeIssuer1, fakeKey000, fakeSerial001), Success, now);
  Result resultOut;
  Time timeOut(Time::uninitialized);
...
TEST_F(psm_OCSPCacheTest, TestVariousGets)
{
  ...
  for (int i = 0; i < MaxCacheEntries; i++) {
    ...
    PutAndGet(
        cache, CertID(fakeIssuer1, fakeKey000, fakeSerial), Success, timeIn);
  }
...
TEST_F(psm_OCSPCacheTest, TestEverythingIsRevoked)
{
  ...
  for (int i = 0; i < MaxCacheEntries; i++) {
    ...
    ASSERT_EQ(Success, timeIn.AddSeconds(i));
    PutAndGet(cache,
              CertID(fakeIssuer1, fakeKey000, fakeSerial),
  ...
  Result result = cache.Put(
      good, OriginAttributes(), Success, timeInPlus1025Minus50, timeInPlus1025);
