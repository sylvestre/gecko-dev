...
Buffer
ExtractCertSPKI(pkix::Input cert);
Buffer
...
void
ExtractEmbeddedSCTList(pkix::Input cert, Buffer& result);
void
...
void
ExtractSCTListFromOCSPResponse(pkix::Input cert,
...
std::ostream&
operator<<(std::ostream& stream, const ct::Buffer& buf);
