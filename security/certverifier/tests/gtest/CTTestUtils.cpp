...
ExtractEmbeddedSCTList(Input cert, Buffer& result)
{
  ...
  if (scts) {
    ...
    ASSERT_EQ(
        Success,
        ExtractSignedCertificateTimestampListFromExtension(*scts, sctList));
...
{
 public:
  pkix::Result GetCertTrust(EndEntityOrCA,
                            const CertPolicyId&,
                            Input,
                            TrustLevel&) override
  {
  ...
  pkix::Result CheckRevocation(EndEntityOrCA,
                               const CertID&,
                               Time,
                               Duration,
                               const Input*,
  ...
  pkix::Result DigestBuf(Input item,
  ...
  pkix::Result CheckSignatureDigestAlgorithm(DigestAlgorithm,
                                             EndEntityOrCA,
                                             Time) override
  {
    ADD_FAILURE();
                                       ...
                                       Input subjectPublicKeyInfo) override
  {
    return VerifyECDSASignedDigestNSS(
        signedDigest, subjectPublicKeyInfo, nullptr);
  }
  ...
  pkix::Result CheckValidityIsAcceptable(Time,
                                         Time,
                                         EndEntityOrCA,
                               ...
                               Buffer& result)
{
  ...
  pkix::Result rv = VerifyEncodedOCSPResponse(trustDomain,
                                              certID,
                                              time, /*time*/
...
Input
InputForSECItem(const SECItem& item)
{
  Input input;
