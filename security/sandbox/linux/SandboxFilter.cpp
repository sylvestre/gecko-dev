...
class ContentSandboxPolicy : public SandboxPolicyCommon
{
 private:
  ...
  static intptr_t OpenAtTrap(ArgsRef aArgs, void* aux)
  {
    ...
    if (fd != AT_FDCWD && path[0] != '/') {
      SANDBOX_LOG_ERROR(
          "unsupported fd-relative openat(%d, \"%s\", 0%o)", fd, path, flags);
      return BlockedSyscallTrap(aArgs, nullptr);
    }
    return broker->Open(path, flags);
  }
  ...
  static intptr_t AccessAtTrap(ArgsRef aArgs, void* aux)
  {
    ...
    if (fd != AT_FDCWD && path[0] != '/') {
      SANDBOX_LOG_ERROR(
          "unsupported fd-relative faccessat(%d, \"%s\", %d)", fd, path, mode);
      return BlockedSyscallTrap(aArgs, nullptr);
    }
    return broker->Access(path, mode);
  }
 ...
 public:
  explicit ContentSandboxPolicy(SandboxBrokerClient* aBroker,
      ...
      : mBroker(aBroker), mSyscallWhitelist(aSyscallWhitelist)
  {
  }
...
class GMPSandboxPolicy : public SandboxPolicyCommon
{
  static intptr_t OpenTrap(const sandbox::arch_seccomp_data& aArgs, void* aux)
  {
    ...
    if ((flags & O_ACCMODE) != O_RDONLY) {
      SANDBOX_LOG_ERROR(
          "non-read-only open of file %s attempted (flags=0%o)", path, flags);
      return -EROFS;
    }
  ...
  static intptr_t SchedTrap(const sandbox::arch_seccomp_data& aArgs, void* aux)
  {
  ...
  static intptr_t UnameTrap(const sandbox::arch_seccomp_data& aArgs, void* aux)
  {
  ...
  static intptr_t FcntlTrap(const sandbox::arch_seccomp_data& aArgs, void* aux)
  {
