...
class SandboxReportWrapper final : public mozISandboxReport
{
 public:
  ...
  explicit SandboxReportWrapper(const SandboxReport& aReport) : mReport(aReport)
  {
  }
...
NS_IMETHODIMP
SandboxReportWrapper::GetMsecAgo(uint64_t* aMsec)
{
...
NS_IMETHODIMP
SandboxReportWrapper::GetPid(int32_t* aPid)
{
  *aPid = mReport.mPid;
  return NS_OK;
}
...
NS_IMETHODIMP
SandboxReportWrapper::GetTid(int32_t* aTid)
{
  *aTid = mReport.mTid;
  return NS_OK;
}
...
NS_IMETHODIMP
SandboxReportWrapper::GetProcType(nsACString& aProcType)
{
...
NS_IMETHODIMP
SandboxReportWrapper::GetSyscall(uint32_t* aSyscall)
{
...
NS_IMETHODIMP
SandboxReportWrapper::GetNumArgs(uint32_t* aNumArgs)
{
...
NS_IMETHODIMP
SandboxReportWrapper::GetArg(uint32_t aIndex, nsACString& aRetval)
{
...
class SandboxReportArray final : public mozISandboxReportArray
{
 public:
  ...
  explicit SandboxReportArray(SandboxReporter::Snapshot&& aSnap)
      : mOffset(aSnap.mOffset), mArray(Move(aSnap.mReports))
  {
  }
...
NS_IMETHODIMP
SandboxReportArray::GetBegin(uint64_t* aBegin)
{
  *aBegin = mOffset;
  return NS_OK;
}
...
NS_IMETHODIMP
SandboxReportArray::GetEnd(uint64_t* aEnd)
{
...
NS_IMETHODIMP
SandboxReportArray::GetElement(uint64_t aIndex, mozISandboxReport** aRetval)
{
...
NS_IMETHODIMP
SandboxReporterWrapper::Snapshot(mozISandboxReportArray** aRetval)
{
