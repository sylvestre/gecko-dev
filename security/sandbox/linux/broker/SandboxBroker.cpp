...
SandboxBroker::SandboxBroker(UniquePtr<const Policy> aPolicy,
                             int aChildPid,
                             int& aClientFd)
...
SandboxBroker::Create(UniquePtr<const Policy> aPolicy,
                      int aChildPid,
...
void
SandboxBroker::Policy::AddPath(int aPerms,
                               const char* aPath,
                               AddCondition aCond)
{
...
void
SandboxBroker::Policy::AddFilePrefix(int aPerms,
                                     const char* aDir,
                                     const char* aPrefix)
{
...
SandboxBroker::ThreadMain(void)
{
  ...
  while (true) {
    ...
    if (recvd < static_cast<ssize_t>(sizeof(req))) {
      SANDBOX_LOG_ERROR(
          "bad read from pid %d (%d < %d)", mChildPid, recvd, sizeof(req));
      shutdown(mFileDesc, SHUT_RD);
      break;
    }
    if (respfd == -1) {
            ...
            if (respSize >= 0) {
              if (respSize > 0) {
                ...
                if (!orig.Equals(xlat) && xlat[0] == '/') {
                  if (SandboxInfo::Get().Test(SandboxInfo::kVerbose)) {
                    SANDBOX_LOG_ERROR(
                        "Recording mapping %s -> %s", xlat.get(), orig.get());
                  }
...
void
SandboxBroker::AuditPermissive(int aOp,
                               int aFlags,
                               int aPerms,
                               const char* aPath)
{
  ...
  SANDBOX_LOG_ERROR(
      "SandboxBroker: would have denied op=%d rflags=%o perms=%d path=%s for "
      "pid=%d"
