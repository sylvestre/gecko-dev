...
nsScannerBufferList::SplitBuffer(const Position& pos)
{
  ...
  if (new_buffer) {
    nsCharTraits<char16_t>::copy(
        new_buffer->DataStart(), bufferToSplit->DataStart() + splitOffset, len);
...
size_t
nsScannerBufferList::Position::Distance(const Position& aStart,
                                        const Position& aEnd)
{
  size_t result = 0;
...
void
nsScannerString::UngetReadable(const nsAString& aReadable,
...
static inline nsAString::iterator&
copy_multifragment_string(nsScannerIterator& first,
