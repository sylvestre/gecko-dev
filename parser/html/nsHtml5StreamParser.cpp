                                     ...
                                     uint32_t aLength)
{
  ...
  if (NS_IsMainThread()) {
    ...
    rv = aInStream->Read(
        reinterpret_cast<char*>(data.get()), aLength, &totalRead);
    NS_ENSURE_SUCCESS(rv, rv);
    ...
    rv = aInStream->ReadSegments(
        CopySegmentsToParser, this, aLength, &totalRead);
    if (NS_FAILED(rv)) {
...
nsHtml5StreamParser::internalEncodingDeclaration(nsHtml5String aEncoding)
{
  ...
  NS_ASSERTION(IsParserThread(), "Wrong thread!");
  if (mCharsetSource >=
      kCharsetFromMetaTag) {  // this threshold corresponds to "confident" in the HTML5 spec
    return false;
  }
