...
nsHtml5TreeOpExecutor::FlushSpeculativeLoads()
{
  ...
  for (nsHtml5SpeculativeLoad* iter =
           const_cast<nsHtml5SpeculativeLoad*>(start);
       iter < end;
       ++iter) {
    ...
    if (mReadingFromStage) {
      ...
      for (nsHtml5SpeculativeLoad* iter = (nsHtml5SpeculativeLoad*)start;
        ...
        if (MOZ_UNLIKELY(!mParser)) {
          // An extension terminated the parser from a HTTP observer.
          mOpQueue
              .Clear();  // clear in order to be able to assert in destructor
          return;
        }
      }
    } else {
...
nsHtml5TreeOpExecutor::IsScriptEnabled()
{
  ...
  // <noscript> tags!
  if (!mDocument || !mDocShell) return true;
  nsCOMPtr<nsIScriptGlobalObject> globalObject =
      do_QueryInterface(mDocument->GetInnerWindow());
...
nsHtml5TreeOpExecutor::RunScript(nsIContent* aScriptElement)
{
  ...
  NS_ASSERTION(mFlushState == eNotFlushing,
               "Tried to run script when flushing.");
...
{
  NS_PRECONDITION(mFlushState == eNotFlushing,
                  "mOpQueue modified during tree op execution.");
...
{
  auto encoding = mDocument->GetDocumentCharacterSet();
  nsresult rv = NS_NewURI(getter_AddRefs(mViewSourceBaseURI),
                          aURL,
                          encoding,
