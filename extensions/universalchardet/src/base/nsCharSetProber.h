...
typedef enum {
  eDetecting =
      0,  //We are still detecting, no sure answer yet, but caller can ask for confidence.
...
class nsCharSetProber
{
 public:
  ...
  static bool FilterWithoutEnglishLetters(const char* aBuf,
                                          uint32_t aLen,
                                          char** newBuf,
                                          uint32_t& newLen);
  static bool FilterWithEnglishLetters(const char* aBuf,
                                       uint32_t aLen,
                                       char** newBuf,
                                       uint32_t& newLen);
};
