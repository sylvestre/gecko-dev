...
NS_IMETHODIMP
mozHunspell::GetDictionary(char16_t** aDictionary)
{
 ...
 */
NS_IMETHODIMP
mozHunspell::SetDictionary(const char16_t* aDictionary)
{
  NS_ENSURE_ARG_POINTER(aDictionary);
...
NS_IMETHODIMP
mozHunspell::GetLanguage(char16_t** aLanguage)
{
...
NS_IMETHODIMP
mozHunspell::GetProvidesPersonalDictionary(bool* aProvidesPersonalDictionary)
{
...
NS_IMETHODIMP
mozHunspell::GetName(char16_t** aName)
{
  return NS_ERROR_NOT_IMPLEMENTED;
}

NS_IMETHODIMP
mozHunspell::GetCopyright(char16_t** aCopyright)
{
  return NS_ERROR_NOT_IMPLEMENTED;
}

NS_IMETHODIMP
mozHunspell::GetPersonalDictionary(mozIPersonalDictionary** aPersonalDictionary)
{
...
NS_IMETHODIMP
mozHunspell::SetPersonalDictionary(mozIPersonalDictionary* aPersonalDictionary)
{
...
NS_IMETHODIMP
mozHunspell::GetDictionaryList(char16_t*** aDictionaries, uint32_t* aCount)
{
...
mozHunspell::LoadDictionaryList(bool aNotifyChildProcesses)
{
  ...
  if (prefs) {
    ...
    rv = prefs->GetCharPref("spellchecker.dictionary_path",
                            getter_Copies(extDictPath));
    if (NS_SUCCEEDED(rv)) {
...
mozHunspell::CollectReports(nsIHandleReportCallback* aHandleReport,
                            ...
                            bool aAnonymize)
{
  MOZ_COLLECT_REPORT("explicit/spell-check",
                     KIND_HEAP,
                     UNITS_BYTES,
...
NS_IMETHODIMP
mozHunspell::Check(const char16_t* aWord, bool* aResult)
{
...
NS_IMETHODIMP
mozHunspell::Suggest(const char16_t* aWord,
    ...
    if (*aSuggestions) {
      ...
      if (NS_FAILED(rv))
        NS_FREE_XPCOM_ALLOCATED_POINTER_ARRAY(
            index,
            *aSuggestions);  // free the char16_t strings up to the point at which the error occurred
...
NS_IMETHODIMP
mozHunspell::Observe(nsISupports* aSubj,
                     const char* aTopic,
...
NS_IMETHODIMP
mozHunspell::AddDirectory(nsIFile* aDir)
{
...
NS_IMETHODIMP
mozHunspell::RemoveDirectory(nsIFile* aDir)
{
  ...
  if (obs) {
    obs->NotifyObservers(
        nullptr, SPELLCHECK_DICTIONARY_REMOVE_NOTIFICATION, nullptr);
  }
#endif
  return NS_OK;
}
