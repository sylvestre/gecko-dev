...
NS_IMETHODIMP
mozEnglishWordUtils::GetLanguage(char16_t** aLanguage)
{
...
NS_IMETHODIMP
mozEnglishWordUtils::GetRootForm(const char16_t* aWord,
                                 uint32_t type,
                                 char16_t*** words,
                                 uint32_t* count)
{
...
NS_IMETHODIMP
mozEnglishWordUtils::FindNextWord(const char16_t* word,
                                  uint32_t length,
                                  uint32_t offset,
                                  int32_t* begin,
                                  int32_t* end)
  ...
  if (p < endbuf) {
    ...
    if ((*p == ':' || *p == '@' || *p == '.') && p < endbuf - 1) {
      ...
      if (mURLDetector) {
        ...
        mURLDetector->FindURLInPlaintext(
            startWord, endbuf - startWord, p - startWord, &startPos, &endPos);
...
NS_IMETHODIMP
mozEnglishWordUtils::FromRootForm(const char16_t* aWord,
