...
typedef KLStatus (*KLCacheHasValidTickets_type)(
    KLPrincipal, KLKerberosVersion, KLBoolean*, KLPrincipal*, char**);
#endif
...
gssInit()
{
  ...
  if (!libPath.IsEmpty()) {
    ...
    const char* const verLibNames[] = {"libasn1.so",
                                       "libcrypto.so",
                                       "libroken.so",
                                       "libheimbase.so",
                                       "libcom_err.so",
  ...
  if (mech_set) {
    for (i = 0; i < mech_set->count; i++) {
      ...
      if (item->length == gss_spnego_mech_oid_desc.length &&
          !memcmp(item->elements,
                  gss_spnego_mech_oid_desc.elements,
                  item->length)) {
        // ok, we found it
                           ...
                           uint32_t* outTokenLen)
{
  ...
  res_ninit(&_res);
#endif
  major_status = gss_import_name_ptr(
      &minor_status, &input_token, &gss_c_nt_hostbased_service, &server);
  ...
  if (!doingMailTask &&
      (gssNativeImp &&
       (KLCacheHasValidTickets_ptr(
            nullptr, kerberosVersion_V5, &found, nullptr, nullptr) != klNoErr ||
        !found))) {
                     ...
                     uint32_t* outTokenLen)
{
  ...
  major_status = gss_unwrap_ptr(
      &minor_status, mCtx, &input_token, &output_token, nullptr, nullptr);
  if (GSS_ERROR(major_status)) {
