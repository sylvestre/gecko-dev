...
ReadPixelsIntoDataSurface(GLContext* gl, DataSourceSurface* dest)
{
  ...
  int readAlignment =
      GuessAlignment(dest->GetSize().width, destPixelSize, dest->Stride());
  if (!readAlignment) {
    needsTempSurf = true;
  }
  if (needsTempSurf) {
  ...
  MOZ_ASSERT(reinterpret_cast<uintptr_t>(readSurf->GetData()) % readAlignment ==
             0);
...

  {
    ...
    gl->fReadPixels(
        0, 0, width, height, readFormat, readType, readSurf->GetData());
  }
...
YInvertImageSurface(gfx::DataSourceSurface* aSurf)
{
  RefPtr<DataSourceSurface> temp = Factory::CreateDataSourceSurfaceWithStride(
      aSurf->GetSize(), aSurf->GetFormat(), aSurf->Stride());
  if (NS_WARN_IF(!temp)) {
    return nullptr;
  }
                ...
                SurfaceFormat aFormat)
{
  ...
  IntSize size;
  gl->fGetTexLevelParameteriv(
      LOCAL_GL_TEXTURE_2D, 0, LOCAL_GL_TEXTURE_WIDTH, &size.width);
  gl->fGetTexLevelParameteriv(
  ...
  gl->fGetTexImage(LOCAL_GL_TEXTURE_2D,
                   0,
                   LOCAL_GL_RGBA,
                                   ...
                                   bool aYInvert)
{
  ...
  if (!ReadTexImage(
          isurf, aTextureId, aTextureTarget, aSize, aConfig, aYInvert)) {
    return nullptr;
  }
  ...
  do {
    ...
    GLenum rbInternalFormat =
        mGL->IsGLES() ? (mGL->IsExtensionSupported(GLContext::OES_rgb8_rgba8)
                             ...
                             : LOCAL_GL_RGBA4)
                      : LOCAL_GL_RGBA;
    mGL->fRenderbufferStorage(
        LOCAL_GL_RENDERBUFFER, rbInternalFormat, aSize.width, aSize.height);
    ...
    const float vertexArray[4 * 2] = {
        -1.0f, -1.0f, 1.0f, -1.0f, -1.0f, 1.0f, 1.0f, 1.0f};
    ScopedVertexAttribPointer autoAttrib0(
        mGL, 0, 2, LOCAL_GL_FLOAT, LOCAL_GL_FALSE, 0, 0, vertexArray);
    ...
    ScopedVertexAttribPointer autoAttrib1(
        mGL, 1, 2, LOCAL_GL_FLOAT, LOCAL_GL_FALSE, 0, 0, texCoordArray);
