 ...
 public:
  ...
  static already_AddRefed<DXInterop2Device> Open(WGLLibrary* wgl, GLContext* gl)
  {
    ...
    if (gl->WorkAroundDriverBugs() && gl->Vendor() == GLVendor::ATI) {
      ...
      hr = d3d11_1->CreateDeviceContextState(0,
                                             &featureLevel10_0,
                                             1,
                                             D3D11_SDK_VERSION,
    ...
    return MakeAndAddRef<DXInterop2Device>(
        wgl, d3d, interopDevice, gl, d3dContext, contextState);
  }

  DXInterop2Device(WGLLibrary* wgl,
                   ID3D11Device* d3d,
  ...
  HANDLE RegisterObject(void* d3dObject,
                        GLuint name,
                        GLenum type,
                        GLenum access) const
  {
                                   ...
                                   bool hasAlpha)
{
  ...
  GLuint interopFB = 0;
  {
    GLint samples = 0;
    {
      ...
      gl->fGetRenderbufferParameteriv(
  ...
  UniquePtr<ptrT> ret(new ptrT(gl,
                               size,
                               hasAlpha,
                               prodTex,
                               interopFB,
...
SurfaceFactory_D3D11Interop::SurfaceFactory_D3D11Interop(
    ...
    DXInterop2Device* interop)
    : SurfaceFactory(
          SharedSurfaceType::DXGLInterop2, gl, caps, allocator, flags),
      mInterop(interop)
{
}
