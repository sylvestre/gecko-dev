                              ...
                              const layers::TextureFlags& flags)
{
  ...
  if (useGl) {
    ...
    factory = MakeUnique<SurfaceFactory_GLTexture>(
        mGLContext, caps, ipcChannel, mFlags);
...
bool
GLScreenBuffer::CopyTexImage2D(GLenum target,
                               GLint level,
...
bool
GLScreenBuffer::ReadPixels(GLint x,
                           GLint y,
                           GLsizei width,
                           GLsizei height,
                           GLenum format,
...
static void
RenderbufferStorageBySamples(GLContext* aGL,
                             GLsizei aSamples,
                             ...
                             const gfx::IntSize& aSize)
{
  if (aSamples) {
    ...
    aGL->fRenderbufferStorage(
        LOCAL_GL_RENDERBUFFER, aInternalFormat, aSize.width, aSize.height);
  }
}

static GLuint
CreateRenderbuffer(GLContext* aGL,
                   GLenum aFormat,
                   GLsizei aSamples,
                   const gfx::IntSize& aSize)
{
...
static void
CreateRenderbuffersForOffscreen(GLContext* aGL,
                   ...
                   UniquePtr<DrawBuffer>* out_buffer)
{
  ...
  *out_buffer = nullptr;
  ...
  CreateRenderbuffersForOffscreen(
      gl, formats, size, caps.antialias, pColorMSRB, pDepthRB, pStencilRB);
                   ...
                   SharedSurface* surf)
{
  ...
  CreateRenderbuffersForOffscreen(
      gl, formats, surf->mSize, caps.antialias, nullptr, pDepthRB, pStencilRB);
