...
WGLLibrary::CreateDummyWindow(HDC* aWindowDC)
{
  ...
  HWND win = CreateWindowW(L"GLContextWGLClass",
                           L"GLContextWGL",
                           0,
                           0,
                           0,
...
WGLLibrary::EnsureInitialized()
{
  ...
  if (PR_GetEnv("SU_SPIES_DIRECTORY")) {
    libGLFilename =
        std::string(PR_GetEnv("SU_SPIES_DIRECTORY")) + "\\opengl32.dll";
  }
...
#define END_OF_SYMBOLS   \
  ...
  const GLLibraryLoader::SymLoadStruct pbufferSymbols[] = {
      {(PRFuncPtr*)&mSymbols.fCreatePbuffer,
       {"wglCreatePbufferARB", "wglCreatePbufferEXT", nullptr}},
      {(PRFuncPtr*)&mSymbols.fDestroyPbuffer,
       {"wglDestroyPbufferARB", "wglDestroyPbufferEXT", nullptr}},
      {(PRFuncPtr*)&mSymbols.fGetPbufferDC,
       {"wglGetPbufferDCARB", "wglGetPbufferDCEXT", nullptr}},
      {(PRFuncPtr*)&mSymbols.fBindTexImage,
       {"wglBindTexImageARB", "wglBindTexImageEXT", nullptr}},
      {(PRFuncPtr*)&mSymbols.fReleaseTexImage,
       {"wglReleaseTexImageARB", "wglReleaseTexImageEXT", nullptr}},
      END_OF_SYMBOLS};

  const GLLibraryLoader::SymLoadStruct pixFmtSymbols[] = {
      {(PRFuncPtr*)&mSymbols.fChoosePixelFormat,
       {"wglChoosePixelFormatARB", "wglChoosePixelFormatEXT", nullptr}},
      {(PRFuncPtr*)&mSymbols.fGetPixelFormatAttribiv,
  ...
  const GLLibraryLoader::SymLoadStruct extensionsSymbols[] = {
      SYMBOL(GetExtensionsStringARB), END_OF_SYMBOLS};
  ...
  if (GLLibraryLoader::LoadSymbols(
          mOGLLibrary, extensionsSymbols, lookupFunc)) {
    ...
    if (HasExtension(extString, "WGL_ARB_create_context")) {
      if (GLLibraryLoader::LoadSymbols(
              mOGLLibrary, robustnessSymbols, lookupFunc)) {
    ...
    if (hasDXInterop2) {
      if (!GLLibraryLoader::LoadSymbols(
              mOGLLibrary, dxInteropSymbols, lookupFunc)) {
        NS_ERROR(
...
GetMaxSize(HDC hDC, int format, IntSize& size)
{
  ...
  if (!sWGLLib.mSymbols.fGetPixelFormatAttribiv(
          hDC, format, 0, 2, query, result))
    return false;
...
static bool
IsValidSizeForFormat(HDC hDC, int format, const IntSize& requested)
{
  IntSize max;
...
already_AddRefed<GLContext>
CreateForWidget(HWND aHwnd, bool aWebRender, bool aForceAccelerated)
{
...
CreatePBufferOffscreenContext(CreateContextFlags flags, const IntSize& aSize)
{
  ...
  if (!wgl.mSymbols.fChoosePixelFormat(
          windowDC, pfAttribs, nullptr, kMaxFormats, formats, &foundFormats) ||
      foundFormats == 0) {
    return nullptr;
  }
