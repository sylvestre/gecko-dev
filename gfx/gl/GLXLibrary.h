...
#define VOID_WRAP(X) \
  ...
  void fDestroyContext(Display* display, GLXContext context) const
      VOID_WRAP(fDestroyContext(display, context))
          ...
          Bool fMakeCurrent(Display* display,
      ...
      WRAP(fMakeCurrent(display, drawable, context))
          ...
          GLXContext fGetCurrentContext() const WRAP(fGetCurrentContext())
              ...
              GLXFBConfig* fChooseFBConfig(Display* display,
                                           int screen,
                                           const int* attrib_list,
                                           int* nelements) const
      WRAP(fChooseFBConfig(display, screen, attrib_list, nelements))

          GLXFBConfig* fGetFBConfigs(Display* display,
                                     int screen,
                                     int* nelements) const
      WRAP(fGetFBConfigs(display, screen, nelements))

          GLXContext fCreateNewContext(Display* display,
                                       GLXFBConfig config,
                                       int render_type,
      ...
      WRAP(fCreateNewContext(display, config, render_type, share_list, direct))
          ...
          int fGetFBConfigAttrib(Display* display,
                                 GLXFBConfig config,
                                 int attribute,
                                 int* value) const
      ...
      WRAP(fQueryServerString(display, screen, name))
          ...
          GLXPixmap fCreatePixmap(Display* display,
                                  GLXFBConfig config,
                                  Pixmap pixmap,
                                  const int* attrib_list) const
      WRAP(fCreatePixmap(display, config, pixmap, attrib_list))

          GLXPixmap fCreateGLXPixmapWithConfig(Display* display,
                                               GLXFBConfig config,
                                               Pixmap pixmap) const
      ...
      WRAP(fQueryVersion(display, major, minor))
          ...
          void fBindTexImage(Display* display,
                             GLXDrawable drawable,
                             int buffer,
                             const int* attrib_list) const
      VOID_WRAP(fBindTexImageEXT(display, drawable, buffer, attrib_list))

          void fReleaseTexImage(Display* display,
                                GLXDrawable drawable,
                                int buffer) const
      VOID_WRAP(fReleaseTexImageEXT(display, drawable, buffer))
          ...
          void fWaitGL() const VOID_WRAP(fWaitGL())
              ...
              void fWaitX() const VOID_WRAP(fWaitX())
                  ...
                  GLXContext fCreateContextAttribs(Display* display,
                                                   GLXFBConfig config,
                                                   GLXContext share_list,
                                                   Bool direct,
                                                   const int* attrib_list) const
      WRAP(fCreateContextAttribsARB(
          display, config, share_list, direct, attrib_list))

          int fGetVideoSync(unsigned int* count) const
      WRAP(fGetVideoSyncSGI(count))

          int fWaitVideoSync(int divisor,
                             int remainder,
                             unsigned int* count) const
      WRAP(fWaitVideoSyncSGI(divisor, remainder, count))

          void fSwapInterval(Display* dpy,
                             GLXDrawable drawable,
                             int interval) const
...
#undef AFTER_CALL
 ...
 private:
  struct
  {
    ...
    GLXContext(GLAPIENTRY* fCreateNewContext)(
    ...
    GLXPixmap(GLAPIENTRY* fCreatePixmap)(Display*,
                                         GLXFBConfig,
                                         Pixmap,
                                         const int*);
    GLXPixmap(GLAPIENTRY* fCreateGLXPixmapWithConfig)(Display*,
                                                      GLXFBConfig,
                                                      Pixmap);
    ...
    GLXContext(GLAPIENTRY* fCreateContextAttribsARB)(
