...
/*static*/ void
SharedSurface::ProdCopy(SharedSurface* src,
                        SharedSurface* dest,
                        SurfaceFactory* factory)
{
  ...
  if (src->mAttachType == AttachmentType::Screen &&
    ...
    tempSurf = SharedSurface_Basic::Create(
        gl, factory->mFormats, src->mSize, factory->mCaps.alpha);
  ...
  if (src->mAttachType == AttachmentType::Screen) {
    ...
    if (dest->mAttachType == AttachmentType::GLTexture) {
      ...
      gl->BlitHelper()->BlitFramebufferToTexture(
          destTex, src->mSize, dest->mSize, destTarget);
      ...
      gl->BlitHelper()->BlitFramebufferToFramebuffer(
          0, destWrapper.FB(), src->mSize, dest->mSize);
    } else {
  ...
  if (dest->mAttachType == AttachmentType::Screen) {
    ...
    if (src->mAttachType == AttachmentType::GLTexture) {
      ...
      gl->BlitHelper()->BlitTextureToFramebuffer(
          srcTex, src->mSize, dest->mSize, srcTarget);
    } else if (src->mAttachType == AttachmentType::GLRenderbuffer) {
      GLuint srcRB = src->ProdRenderbuffer();
      ScopedFramebufferForRenderbuffer srcWrapper(gl, srcRB);

      gl->BlitHelper()->BlitFramebufferToFramebuffer(
          srcWrapper.FB(), 0, src->mSize, dest->mSize);
    } else {
...
SurfaceFactory::NewTexClient(const gfx::IntSize& size)
{
  ...
  ret = layers::SharedSurfaceTextureClient::Create(
      Move(surf), this, mAllocator, mFlags);
      ...
      mSurfToLock(nullptr)
{
  ...
  if (src->NeedsIndirectReads()) {
...

    {
      ...
      mGL->fCopyTexImage2D(
          LOCAL_GL_TEXTURE_2D, 0, format, 0, 0, width, height, 0);
    }
...
ReadbackSharedSurface(SharedSurface* src, gfx::DrawTarget* dst)
{
  ...
  MOZ_ASSERT_IF(!isDstRGBA,
                dstFormat == gfx::SurfaceFormat::B8G8R8A8 ||
...

  {
    ...
    // ReadPixels from the current FB into lockedBits.
    {
      ...
      gl->raw_fReadPixels(
          0, 0, width, height, readGLFormat, readType, dstBytes);
    }
  }
...
ReadPixel(SharedSurface* src)
{
  ...
  ScopedReadbackFB a(src);
  {
    ...
    gl->raw_fReadPixels(
        0, 0, 1, 1, LOCAL_GL_RGBA, LOCAL_GL_UNSIGNED_BYTE, bytes.get());
