...
bool
SharedSurface_IOSurface::CopyTexImage2D(GLenum target,
                                        GLint level,
                                        ...
                                        GLint border)
{
  ...
  ScopedTexture destTex(mGL);
  {
    ...
    mGL->raw_fCopyTexImage2D(
        LOCAL_GL_TEXTURE_2D, 0, LOCAL_GL_RGBA, x, y, width, height, 0);
  }

  ScopedFramebufferForTexture tmpFB(
      mGL, destTex.Texture(), LOCAL_GL_TEXTURE_2D);
...
bool
SharedSurface_IOSurface::ReadPixels(GLint x,
                                    GLint y,
                                    GLsizei width,
                                    GLsizei height,
                                    GLenum format,
                                    ...
                                    GLvoid* pixels)
{
  ...
  ScopedTexture destTex(mGL);
  {
    ...
    ScopedBindTexture bindTex(mGL, destTex.Texture());
    mGL->raw_fCopyTexImage2D(LOCAL_GL_TEXTURE_2D,
                             0,
...
bool
SharedSurface_IOSurface::ReadbackBySharedHandle(
...
SurfaceFactory_IOSurface::Create(
    GLContext* gl,
