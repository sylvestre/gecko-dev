                     ...
                     const EGLAttrib* const postAttribs)
{
  ...
  if (!egl.IsExtensionSupported(
      ...
      !egl.IsExtensionSupported(
          GLLibraryEGL::ANGLE_stream_producer_d3d_texture_nv12)) {
    return 0;
  }
...
OpenSharedTexture(ID3D11Device* const d3d, const WindowsHandle handle)
{
  ...
  auto hr =
      d3d->OpenSharedResource((HANDLE)handle,
...
class BindAnglePlanes final
{
 ...
 public:
  BindAnglePlanes(const GLBlitHelper* const parent,
  ...
  switch (subdescUnion.type()) {
    ...
    case subdescUnion.TSurfaceDescriptorDXGIYCbCr: {
      ...
      const WindowsHandle handles[3] = {
          subdesc.handleY(), subdesc.handleCb(), subdesc.handleCr()};
      return BlitAngleYCbCr(
          handles, clipRect, ySize, uvSize, colorSpace, destSize, destOrigin);
    }
    default:
                        ...
                        const OriginPos destOrigin) const
{
  ...
  return BlitAngleYCbCr(handles,
                        srcImage->mPictureRect,
                        srcImage->mYSize,
                        srcImage->mCbCrSize,
                        srcImage->mColorSpace,
                        destSize,
                        destOrigin);
}
                             ...
                             OriginPos destOrigin) const
{
  ...
  const EGLAttrib postAttribs0[] = {
      LOCAL_EGL_NATIVE_BUFFER_PLANE_OFFSET_IMG, 0, LOCAL_EGL_NONE};
  const EGLAttrib postAttribs1[] = {
      LOCAL_EGL_NATIVE_BUFFER_PLANE_OFFSET_IMG, 1, LOCAL_EGL_NONE};
