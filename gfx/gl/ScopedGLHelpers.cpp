...
ScopedBindTexture::ScopedBindTexture(GLContext* aGL,
                                     GLuint aNewTex,
                                     GLenum aTarget)
      ...
      mFB(0)
{
  ...
  mGL->fFramebufferTexture2D(
      LOCAL_GL_FRAMEBUFFER, LOCAL_GL_COLOR_ATTACHMENT0, aTarget, aTexture, 0);
...
void
ScopedFramebufferForTexture::UnwrapImpl()
{
  if (!mFB) return;
...
ScopedViewportRect::ScopedViewportRect(
    GLContext* aGL, GLint x, GLint y, GLsizei width, GLsizei height)
...
ScopedVertexAttribPointer::WrapImpl(GLuint index)
{
  ...
  mGL->fGetVertexAttribiv(
      mAttribIndex, LOCAL_GL_VERTEX_ATTRIB_ARRAY_ENABLED, &mAttribEnabled);
  mGL->fGetVertexAttribiv(
      mAttribIndex, LOCAL_GL_VERTEX_ATTRIB_ARRAY_SIZE, &mAttribSize);
  mGL->fGetVertexAttribiv(
      mAttribIndex, LOCAL_GL_VERTEX_ATTRIB_ARRAY_STRIDE, &mAttribStride);
  mGL->fGetVertexAttribiv(
      mAttribIndex, LOCAL_GL_VERTEX_ATTRIB_ARRAY_TYPE, &mAttribType);
...
ScopedVertexAttribPointer::UnwrapImpl()
{
  ...
  mGL->fVertexAttribPointer(mAttribIndex,
                            mAttribSize,
                            mAttribType,
                            mAttribNormalized,
                            mAttribStride,
