...
bool
SharedSurface_GLXDrawable::ReadbackBySharedHandle(
...
SurfaceFactory_GLXDrawable::Create(
    GLContext* prodGL,
    const SurfaceCaps& caps,
...
SurfaceFactory_GLXDrawable::CreateShared(const gfx::IntSize& size)
{
  ...
  return SharedSurface_GLXDrawable::Create(
      mGL, mCaps, size, deallocateClient, mAllocator->IsSameProcess());
}
