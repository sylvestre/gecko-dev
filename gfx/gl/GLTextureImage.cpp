                   ...
                   TextureImage::ImageFormat aImageFormat)
{
  switch (gl->GetContextType()) {
    case GLContextType::EGL:
      return CreateTextureImageEGL(
          gl, aSize, aContentType, aWrapMode, aFlags, aImageFormat);
    default: {
      ...
      if (aSize.width > maxTextureSize || aSize.height > maxTextureSize) {
        ...
        return CreateTiledTextureImage(
            gl, aSize, aContentType, aFlags, aImageFormat);
      } else {
        return CreateBasicTextureImage(
            gl, aSize, aContentType, aWrapMode, aFlags);
      }
    }
  }
}
            ...
            TextureImage::ImageFormat aImageFormat)
{
  switch (gl->GetContextType()) {
    ...
    default:
      return CreateBasicTextureImage(
          gl, aSize, aContentType, LOCAL_GL_CLAMP_TO_EDGE, aFlags);
  }
}
                                   ...
                                   const gfx::IntPoint* aSrcPoint)
{
  nsIntRegion destRegion =
      aDestRegion
          ? *aDestRegion
          : IntRect(
                0, 0, aSurface->GetSize().width, aSurface->GetSize().height);
    ...
    const gfx::IntPoint& aFrom /* = gfx::IntPoint(0, 0) */)
{
  ...
  mTextureFormat = UploadSurfaceToTexture(
      mGLContext, aSurf, region, mTexture, mSize, &uploadSize, needInit, aFrom);
...
bool
TiledTextureImage::DirectUpdate(
    gfx::DataSourceSurface* aSurf,
    const nsIntRegion& aRegion,
    const gfx::IntPoint& aFrom /* = gfx::IntPoint(0, 0) */)
{
...
void
TiledTextureImage::BeginBigImageIteration()
{
  mCurrentImage = 0;
}

bool
TiledTextureImage::NextTile()
{
...
void
TiledTextureImage::SetIterationCallback(BigImageIterationCallback aCallback,
                                        void* aCallbackData)
{
 ...
 */
void
TiledTextureImage::Resize(const gfx::IntSize& aSize)
{
...
uint32_t
TiledTextureImage::GetTileCount()
{
                        ...
                        TextureImage::Flags aFlags)
{
  ...
  aGL->fTexParameteri(
      LOCAL_GL_TEXTURE_2D, LOCAL_GL_TEXTURE_MIN_FILTER, texfilter);
  aGL->fTexParameteri(
      LOCAL_GL_TEXTURE_2D, LOCAL_GL_TEXTURE_MAG_FILTER, texfilter);
