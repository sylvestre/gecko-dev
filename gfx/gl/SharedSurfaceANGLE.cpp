                     ...
                     const gfx::IntSize& size)
{
  ...
  EGLint attribs[] = {
      LOCAL_EGL_WIDTH, width, LOCAL_EGL_HEIGHT, height, LOCAL_EGL_NONE};
    ...
    layers::SurfaceDescriptor* const out_descriptor)
{
  ...
  *out_descriptor = layers::SurfaceDescriptorD3D10(
      (WindowsHandle)mShareHandle, format, mSize);
  return true;
}
      ...
      : mIsLocked(false), mTexture(texture)
  {
    ...
    *succeeded = false;
    ...
    hr = device->CreateTexture2D(
        &mDesc, nullptr, getter_AddRefs(mCopiedTexture));
    ...
    hr = mDeviceContext->Map(
        mCopiedTexture, 0, D3D11_MAP_READ, 0, &mSubresource);
    if (FAILED(hr)) {
      return;
    }
...
bool
SharedSurface_ANGLEShareHandle::ReadbackBySharedHandle(
    gfx::DataSourceSurface* out_surface)
{
  ...
  if (map.GetStride() == srcStride) {
    memcpy(
        map.GetData(), data, out_surface->GetSize().height * map.GetStride());
  } else {
...
SurfaceFactory_ANGLEShareHandle::SurfaceFactory_ANGLEShareHandle(
    ...
    EGLConfig config)
    : SurfaceFactory(
          SharedSurfaceType::EGLSurfaceANGLE, gl, caps, allocator, flags),
      mProdGL(gl),
      mEGL(egl),
      mConfig(config)
{
