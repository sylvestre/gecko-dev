...
static EGLSurface
CreateSurfaceFromNativeWindow(EGLNativeWindowType window,
...
GLContextEGLFactory::Create(EGLNativeWindowType aWindow, bool aWebRender)
{
  MOZ_ASSERT(aWindow);
...
GLContextEGL::GLContextEGL(CreateContextFlags flags,
...
GLContextEGL::ReleaseTexImage()
{
  ...
  EGLBoolean success;
  success = sEGLLibrary.fReleaseTexImage(
      EGL_DISPLAY(), (EGLSurface)mSurface, LOCAL_EGL_BACK_BUFFER);
  if (success == LOCAL_EGL_FALSE) return false;
...
GLContextEGL::MakeCurrentImpl(bool aForce)
{
  ...
  bool needsMakeCurrent =
      (aForce || sEGLLibrary.fGetCurrentContext() != mContext);
  if (needsMakeCurrent) {
    EGLSurface surface =
                              ...
                              nsACString* const out_failureId)
{
  ...
  const auto fnCreate = [&](const std::vector<EGLint>& attribs) {
    ...
    return sEGLLibrary.fCreateContext(
        EGL_DISPLAY(), config, EGL_NO_CONTEXT, terminated_attribs.data());
  };
...
CreateConfig(EGLConfig* aConfig, int32_t depth, bool aEnableDepthBuffer)
{
  ...
  if (!sEGLLibrary.fChooseConfig(
          EGL_DISPLAY(), attribs, configs, ncfg, &ncfg) ||
      ncfg < 1) {
    return false;
  }
  ...
  for (int j = 0; j < ncfg; ++j) {
    ...
    EGLint r, g, b, a;
    if (sEGLLibrary.fGetConfigAttrib(
            EGL_DISPLAY(), config, LOCAL_EGL_RED_SIZE, &r) &&
        sEGLLibrary.fGetConfigAttrib(
            EGL_DISPLAY(), config, LOCAL_EGL_GREEN_SIZE, &g) &&
        sEGLLibrary.fGetConfigAttrib(
            EGL_DISPLAY(), config, LOCAL_EGL_BLUE_SIZE, &b) &&
        sEGLLibrary.fGetConfigAttrib(
            EGL_DISPLAY(), config, LOCAL_EGL_ALPHA_SIZE, &a) &&
      ...
      if (aEnableDepthBuffer) {
        if (!sEGLLibrary.fGetConfigAttrib(
                EGL_DISPLAY(), config, LOCAL_EGL_DEPTH_SIZE, &z) ||
            z != 24) {
          continue;
        }
      }
             ...
             SurfaceCaps* const out_configCaps)
{
  ...
  if (!egl->fChooseConfig(
          egl->Display(), configAttribs, configs, kMaxConfigs, &foundConfigs) ||
      foundConfigs == 0) {
