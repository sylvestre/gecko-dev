                      ...
                      nsACString* const out_failureId)
{
  ...
  gfxUtils::ThreadSafeGetFeatureStatus(
      gfxInfo, nsIGfxInfo::FEATURE_WEBGL_ANGLE, failureId, &angleSupport);
...
bool
GLLibraryEGL::ReadbackEGLImage(EGLImage image,
                               gfx::DataSourceSurface* out_surface)
{
  ...
  mReadbackGL->fTexParameteri(
      target, LOCAL_GL_TEXTURE_WRAP_S, LOCAL_GL_CLAMP_TO_EDGE);
  mReadbackGL->fTexParameteri(
      target, LOCAL_GL_TEXTURE_WRAP_T, LOCAL_GL_CLAMP_TO_EDGE);
  mReadbackGL->fTexParameteri(
...
#define END_OF_SYMBOLS   \
  ...
  if (IsExtensionSupported(ANGLE_stream_producer_d3d_texture_nv12)) {
    ...
    if (!fnLoadSymbols(nvStreamSymbols)) {
      NS_ERROR(
          "EGL supports ANGLE_stream_producer_d3d_texture_nv12 without "
          "exposing its functions!");
...
template<size_t N>
static void
MarkExtensions(const char* rawExtString,
               bool shouldDumpExts,
               const char* extType,
...
GLLibraryEGL::InitDisplayExtensions()
{
  ...
  MarkExtensions(
      rawExtString, shouldDumpExts, "display", &mAvailableExtensions);
}
