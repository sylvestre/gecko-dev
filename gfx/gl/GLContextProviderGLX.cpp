...
GLXLibrary::CreatePixmap(gfxASurface* aSurface)
{
  ...
  for (int i = 0; i < numConfigs; i++) {
    ...
    if (haveNonColorBits) {
      ...
      // matches.
      int size = 0;
      sGLXLibrary.fGetFBConfigAttrib(
          display, cfgs[i], LOCAL_GLX_ALPHA_SIZE, &size);
      if (size != alphaSize) {
        continue;
      }
    }
  ...
  int pixmapAttribs[] = {LOCAL_GLX_TEXTURE_TARGET_EXT,
...
static int
GLXErrorHandler(Display* display, XErrorEvent* ev)
{
  ...
  do {
    ...
    if (glx.HasCreateContextAttribs()) {
      ...
      context = glx.fCreateContextAttribs(
          display, cfg, nullptr, True, attrib_list.Elements());
    } else {
      context = glx.fCreateNewContext(
          display, cfg, LOCAL_GLX_RGBA_TYPE, nullptr, True);
    }

    if (context) {
      glContext = new GLContextGLX(flags,
                                   caps,
                                   isOffscreen,
                                   display,
                                   drawable,
...
already_AddRefed<GLContext>
CreateForWidget(Display* aXDisplay,
                Window aXWindow,
                bool aWebRender,
                bool aForceAccelerated)
{
  ...
  if (!GLContextGLX::FindFBConfigForWindow(
          aXDisplay, xscreen, aXWindow, &cfgs, &config, &visid, aWebRender)) {
    return nullptr;
  }
  ...
  return GLContextGLX::CreateGLContext(flags,
                                       SurfaceCaps::Any(),
                                       false,
                                       aXDisplay,
                                       aXWindow,
             ...
             int* const out_visid)
{
  ...
  for (int i = 0; i < numConfigs; ++i) {
    ...
    int visid;
    if (glx->fGetFBConfigAttrib(
            display, curConfig, LOCAL_GLX_VISUAL_ID, &visid) != Success) {
      continue;
    }
...
bool
GLContextGLX::FindFBConfigForWindow(
    Display* display,
    int screen,
    Window window,
    ScopedXFree<GLXFBConfig>* const out_scopedConfigArr,
    ...
    bool aWebRender)
{
  ...
  const int webrenderAttribs[] = {
      LOCAL_GLX_DEPTH_SIZE, 24, LOCAL_GLX_DOUBLEBUFFER, True, 0};

  if (aWebRender) {
    cfgs = sGLXLibrary.fChooseFBConfig(
        display, screen, webrenderAttribs, &numConfigs);
  } else {
  ...
  if (aWebRender) {
    for (int i = 0; i < numConfigs; i++) {
      int visid = X11None;
      sGLXLibrary.fGetFBConfigAttrib(
          display, cfgs[i], LOCAL_GLX_VISUAL_ID, &visid);
      if (!visid) {
        continue;
      }
...
CreateOffscreenPixmapContext(CreateContextFlags flags,
                             const IntSize& size,
                             const SurfaceCaps& minCaps,
                             nsACString* const out_failureId)
{
  ...
  int visid;
  if (!ChooseConfig(
          glx, display, screen, minCaps, &scopedConfigArr, &config, &visid)) {
  ...
  return GLContextGLX::CreateGLContext(
      flags, minCaps, true, display, pixmap, config, true, surface);
}
                                     ...
                                     nsACString* const out_failureId)
{
  ...
  return CreateOffscreenPixmapContext(
                                      ...
                                      nsACString* const out_failureId)
{
  ...
  RefPtr<GLContext> gl;
  gl = CreateOffscreenPixmapContext(
