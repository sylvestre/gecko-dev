...
DrawBlitProg::Draw(const BaseArgs& args, const YUVArgs* const argsYUV) const
{
  ...
  if (argsYUV) {
    gl->fUniform2f(
        mLoc_uTexSize1, argsYUV->texSize1.width, argsYUV->texSize1.height);
    gl->fUniform2f(
        mLoc_uDivisors, argsYUV->divisors.width, argsYUV->divisors.height);
  ...
  if (mParent.mQuadVAO) {
    ...
    gl->fGetVertexAttribiv(
        0, LOCAL_GL_VERTEX_ATTRIB_ARRAY_ENABLED, &vaa0Enabled);
    gl->fGetVertexAttribiv(0, LOCAL_GL_VERTEX_ATTRIB_ARRAY_SIZE, &vaa0Size);
    gl->fGetVertexAttribiv(
        0, LOCAL_GL_VERTEX_ATTRIB_ARRAY_TYPE, (GLint*)&vaa0Type);
    gl->fGetVertexAttribiv(
        0, LOCAL_GL_VERTEX_ATTRIB_ARRAY_NORMALIZED, &vaa0Normalized);
    gl->fGetVertexAttribiv(
        0, LOCAL_GL_VERTEX_ATTRIB_ARRAY_STRIDE, (GLint*)&vaa0Stride);
    gl->fGetVertexAttribPointerv(
        0, LOCAL_GL_VERTEX_ATTRIB_ARRAY_POINTER, &vaa0Pointer);
  ...
  if (mParent.mQuadVAO) {
    ...
    gl->fVertexAttribPointer(
        0, vaa0Size, vaa0Type, bool(vaa0Normalized), vaa0Stride, vaa0Pointer);
  }
}
      ...
      mYuvUploads_UVSize(0, 0)
{
  ...
  mGL->fShaderSource(
      mDrawBlitProg_VertShader, ArrayLength(parts), parts, nullptr);
...
GLBlitHelper::CreateDrawBlitProg(const DrawBlitProg::Key& key) const
{
  ...
  const char* const parts[] = {mDrawBlitProg_VersionLine.get(),
                               key.fragHeader,
                               kFragHeader_Global,
                               key.fragBody};
                                     ...
                                     OriginPos destOrigin)
{
  switch (srcImage->GetFormat()) {
    case ImageFormat::PLANAR_YCBCR:
      return BlitImage(
          static_cast<PlanarYCbCrImage*>(srcImage), destSize, destOrigin);
    ...
    case ImageFormat::GPU_VIDEO:
      return BlitImage(
          static_cast<layers::GPUVideoImage*>(srcImage), destSize, destOrigin);
    case ImageFormat::D3D11_YCBCR_IMAGE:
      return BlitImage(
          (layers::D3D11YCbCrImage*)srcImage, destSize, destOrigin);
...
bool
GuessDivisors(const gfx::IntSize& ySize,
                        ...
                        const OriginPos destOrigin) const
{
  ...
  const DrawBlitProg::BaseArgs baseArgs = {
      destSize, yFlip, clipRect, texRectNormFactor};
  DrawBlitProg::YUVArgs yuvArgs = {
      texRectNormFactor, {2, 2}, YUVColorSpace::BT601};
  ...
  const char formatStr[] = {
      formatChars[3], formatChars[2], formatChars[1], formatChars[0], 0};
  if (mGL->ShouldSpew()) {
    printf_stderr(
        "iosurf format: %s (0x%08x)\n", formatStr, uint32_t(pixelFormat));
  }

  const char* fragBody;
  GLenum internalFormats[3] = {0, 0, 0};
  GLenum unpackFormats[3] = {0, 0, 0};
  GLenum unpackTypes[3] = {
      LOCAL_GL_UNSIGNED_BYTE, LOCAL_GL_UNSIGNED_BYTE, LOCAL_GL_UNSIGNED_BYTE};
  switch (planes) {
    case 1:
  ...
  for (uint32_t p = 0; p < planes; p++) {
    ...
    auto err = iosurf->CGLTexImageIOSurface2D(cglContext,
                                              texTarget,
                                              internalFormats[p],
                                              width,
                                              height,
                                           ...
                                           const GLenum srcTarget) const
{
  ...
  const DrawBlitProg::BaseArgs baseArgs = {
      destSize, yFlip, clipRect, texSizeDivisor};
  prog->Draw(baseArgs);
}
...
void
GLBlitHelper::BlitFramebufferToFramebuffer(const GLuint srcFB,
                                           const GLuint destFB,
                                           const gfx::IntSize& srcSize,
                                           const gfx::IntSize& destSize) const
{
...
void
GLBlitHelper::BlitTextureToFramebuffer(GLuint srcTex,
                                       const gfx::IntSize& srcSize,
                                       const gfx::IntSize& destSize,
                                       ...
                                       GLenum destTarget) const
{
  ...
  mGL->fCopyTexSubImage2D(
      destTarget, 0, 0, 0, 0, 0, srcSize.width, srcSize.height);
}

void
GLBlitHelper::BlitTextureToTexture(GLuint srcTex,
                                   GLuint destTex,
                                   const gfx::IntSize& srcSize,
                                   const gfx::IntSize& destSize,
                                   GLenum srcTarget,
