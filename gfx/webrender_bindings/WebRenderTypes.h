...
inline uint64_t
AsUint64(const FontInstanceKey& aId)
{
  return (static_cast<uint64_t>(aId.mNamespace.mHandle) << 32) +
         static_cast<uint64_t>(aId.mHandle);
}

inline FontInstanceKey
...
ToImageRendering(gfx::SamplingFilter aFilter)
{
  return aFilter == gfx::SamplingFilter::POINT ? ImageRendering::Pixelated
                                               : ImageRendering::Auto;
}

static inline FontRenderMode
ToFontRenderMode(gfx::AntialiasMode aMode, bool aPermitSubpixelAA = true)
{
  switch (aMode) {
...
static inline MixBlendMode
ToMixBlendMode(gfx::CompositionOp compositionOp)
{
...
static inline wr::ColorF
ToColorF(const gfx::Color& color)
{
  wr::ColorF c;
  c.r = color.r;
  c.g = color.g;
...
static inline wr::LayoutPoint
ToLayoutPoint(const mozilla::LayoutDevicePoint& point)
{
  wr::LayoutPoint p;
  p.x = point.x;
  p.y = point.y;
...
static inline wr::LayoutPoint
ToLayoutPoint(const mozilla::LayoutDeviceIntPoint& point)
{
  return ToLayoutPoint(LayoutDevicePoint(point));
}

static inline wr::LayoutVector2D
ToLayoutVector2D(const mozilla::LayoutDevicePoint& point)
{
  wr::LayoutVector2D p;
  p.x = point.x;
  p.y = point.y;
...
static inline wr::LayoutVector2D
ToLayoutVector2D(const mozilla::LayoutDeviceIntPoint& point)
{
  return ToLayoutVector2D(LayoutDevicePoint(point));
}

static inline wr::LayoutRect
ToLayoutRect(const mozilla::LayoutDeviceRect& rect)
{
  wr::LayoutRect r;
  r.origin.x = rect.x;
  r.origin.y = rect.y;
...
static inline wr::LayoutRect
ToLayoutRect(const gfxRect rect)
{
  wr::LayoutRect r;
  r.origin.x = rect.x;
  r.origin.y = rect.y;
...
static inline wr::LayoutRect
ToLayoutRect(const mozilla::LayoutDeviceIntRect& rect)
{
  return ToLayoutRect(IntRectToRect(rect));
}

static inline wr::LayoutSize
ToLayoutSize(const mozilla::LayoutDeviceSize& size)
{
  wr::LayoutSize ls;
  ls.width = size.width;
...
static inline wr::ComplexClipRegion
ToComplexClipRegion(const RoundedRect& rect)
{
...
static inline wr::LayoutSize
ToLayoutSize(const mozilla::LayoutDeviceIntSize& size)
{
  return ToLayoutSize(LayoutDeviceSize(size));
}

template<class S, class T>
static inline wr::LayoutTransform
ToLayoutTransform(const gfx::Matrix4x4Typed<S, T>& m)
{
...
static inline wr::BorderStyle
ToBorderStyle(const uint8_t& style)
{
  switch (style) {
...
static inline wr::BorderSide
ToBorderSide(const gfx::Color& color, const uint8_t& style)
{
  wr::BorderSide bs;
...
static inline wr::BorderRadius
ToBorderRadius(const mozilla::LayoutDeviceSize& topLeft,
...
static inline wr::BorderWidths
ToBorderWidths(float top, float right, float bottom, float left)
{
  wr::BorderWidths bw;
  bw.top = top;
  bw.right = right;
...
static inline wr::NinePatchDescriptor
ToNinePatchDescriptor(uint32_t width,
                      uint32_t height,
...
static inline wr::SideOffsets2D_u32
ToSideOffsets2D_u32(uint32_t top,
                    uint32_t right,
                    uint32_t bottom,
                    uint32_t left)
{
...
static inline wr::SideOffsets2D_f32
ToSideOffsets2D_f32(float top, float right, float bottom, float left)
{
...
static inline wr::RepeatMode
ToRepeatMode(uint8_t repeatMode)
{
  switch (repeatMode) {
...
template<class S, class T>
static inline wr::WrTransformProperty
ToWrTransformProperty(uint64_t id, const gfx::Matrix4x4Typed<S, T>& transform)
{
...
static inline wr::WrOpacityProperty
ToWrOpacityProperty(uint64_t id, const float opacity)
{
...
inline uint64_t
AsUint64(const ExternalImageId& aId)
{
  return static_cast<uint64_t>(aId.mHandle);
}

static inline ExternalImageId
...
static inline wr::WrExternalImage
RawDataToWrExternalImage(const uint8_t* aBuff, size_t size)
{
...
inline wr::ByteSlice
RangeToByteSlice(mozilla::Range<uint8_t> aRange)
{
  return wr::ByteSlice{aRange.begin().get(), aRange.length()};
}

inline mozilla::Range<const uint8_t>
ByteSliceToRange(wr::ByteSlice aWrSlice)
{
  return mozilla::Range<const uint8_t>(aWrSlice.buffer, aWrSlice.len);
}

inline mozilla::Range<uint8_t>
MutByteSliceToRange(wr::MutByteSlice aWrSlice)
{
...
static inline wr::WrFilterOpType
ToWrFilterOpType(uint32_t type)
{
  switch (type) {
