...
GenerateBuffer(int bytesPerPixel,
               int width,
               int height,
               int stride,
               int xBoundary,
...
static bool
CheckBuffer(
    unsigned char* buffer, int bytesPerPixel, int width, int height, int stride)
{
  int xBoundary = 0;
  int yBoundary = 0;
  for (int y = 0; y < height; y++) {
  ...
  for (int bytesPerId = 0; bytesPerId < 2; bytesPerId++) {
    ...
    for (int testId = 0; testId < NUM_OF_TESTS; testId++) {
      ...
      BufferUnrotate(buffer,
                     width[testId] * bytesPerPixel,
                     height[testId],
                     stride,
                     xBoundary[testId] * bytesPerPixel,
                     yBoundary[testId]);

      EXPECT_TRUE(CheckBuffer(
          buffer, bytesPerPixel, width[testId], height[testId], stride));
      delete[] buffer;
    }
  }
}
...
TEST(Gfx, BufferUnrotateBoth)
{
  ...
  int bytesPerPixelList[2] = {2, 4};
  int width[NUM_OF_TESTS] = {
      100, 100, 99, 99, 100, 100, 99, 99, 100, 100, 99, 99, 100, 100, 99, 99};
  int height[NUM_OF_TESTS] = {
      100, 99, 100, 99, 100, 99, 100, 99, 100, 99, 100, 99, 100, 99, 100, 99};
  int xBoundary[NUM_OF_TESTS] = {
      30, 30, 30, 30, 31, 31, 31, 31, 30, 30, 30, 30, 31, 31, 31, 31};
  int yBoundary[NUM_OF_TESTS] = {
...
TEST(Gfx, BufferUnrotateUneven)
{
  ...
  int bytesPerPixelList[2] = {2, 4};
  int width[NUM_OF_TESTS] = {
      10, 100, 99, 39, 100, 40, 99, 39, 100, 50, 39, 99, 74, 60, 99, 39};
  int height[NUM_OF_TESTS] = {
      100, 39, 10, 99, 10, 99, 40, 99, 73, 39, 100, 39, 67, 99, 84, 99};
  int xBoundary[NUM_OF_TESTS] = {
      0, 0, 30, 30, 99, 31, 0, 31, 30, 30, 30, 30, 31, 31, 31, 38};
  int yBoundary[NUM_OF_TESTS] = {
      30, 30, 0, 30, 0, 30, 0, 30, 31, 31, 31, 31, 31, 31, 31, 98};
