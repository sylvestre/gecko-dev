...
VRDisplayPuppet::VRDisplayPuppet()
    : VRDisplayHost(VRDeviceType::Puppet), mIsPresenting(false)
{
...
VRDisplayPuppet::SetDisplayInfo(const VRDisplayInfo& aDisplayInfo)
{
  ...
  memcpy(&mDisplayInfo.mEyeFOV,
         &aDisplayInfo.mEyeFOV,
         sizeof(mDisplayInfo.mEyeFOV[0]) * VRDisplayInfo::NumEyes);
  memcpy(&mDisplayInfo.mEyeTranslation,
         &aDisplayInfo.mEyeTranslation,
...
void
VRSystemManagerPuppet::HandleAxisMove(uint32_t aControllerIdx,
                                      uint32_t aAxis,
                                      float aValue)
{
  RefPtr<impl::VRControllerPuppet> controller(
...
void
VRSystemManagerPuppet::GetControllers(
...
VRSystemManagerPuppet::ScanForControllers()
{
  ...
  if (newControllerCount != mControllerCount) {
    ...
    for (uint32_t i = 0; i < newControllerCount; ++i) {
      ...
      RefPtr<VRControllerPuppet> puppetController = new VRControllerPuppet(
          hand, mPuppetHMD->GetDisplayInfo().GetDisplayID());
