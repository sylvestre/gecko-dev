...
class AutoTextRun
{
 public:
  ...
  AutoTextRun(nsFontMetrics* aMetrics,
...
nsFontMetrics::nsFontMetrics(const nsFont& aFont,
      ...
      mTextOrientation(0)
{
  ...
  mFontGroup = gfxPlatform::GetPlatform()->CreateFontGroup(aFont.fontlist,
                                                           &style,
                                                           aParams.textPerf,
                                                           aParams.userFontSet,
                                                           devToCssSize);
...
static gfxFloat
ComputeMaxDescent(const gfxFont::Metrics& aMetrics, gfxFontGroup* aFontGroup)
{
...
static gfxFloat
ComputeMaxAscent(const gfxFont::Metrics& aMetrics)
{
...
nscoord
nsFontMetrics::GetWidth(const char16_t* aString,
                        uint32_t aLength,
                        DrawTarget* aDrawTarget)
{
  if (aLength == 0) return 0;
...
void
nsFontMetrics::DrawString(const char* aString,
                          uint32_t aLength,
                          nscoord aX,
                          nscoord aY,
                          gfxContext* aContext)
...
void
nsFontMetrics::DrawString(const char16_t* aString,
                          uint32_t aLength,
                          nscoord aX,
                          nscoord aY,
                          gfxContext* aContext,
...
GetTextBoundingMetrics(nsFontMetrics* aMetrics,
                       const char16_t* aString,
                       uint32_t aLength,
...
nsBoundingMetrics
nsFontMetrics::GetBoundingMetrics(const char16_t* aString,
                                  uint32_t aLength,
                                  DrawTarget* aDrawTarget)
{
  return GetTextBoundingMetrics(this,
                                aString,
                                aLength,
                                aDrawTarget,
...
nsBoundingMetrics
nsFontMetrics::GetInkBoundsForVisualOverflow(const char16_t* aString,
                                             uint32_t aLength,
                                             DrawTarget* aDrawTarget)
{
  return GetTextBoundingMetrics(
      this, aString, aLength, aDrawTarget, gfxFont::LOOSE_INK_EXTENTS);
}
