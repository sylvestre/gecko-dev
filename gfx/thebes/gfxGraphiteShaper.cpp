                             ...
                             gfxShapedText* aShapedText)
{
  ...
  if (!mGrFont) {
    ...
    if (mFont->ProvidesGlyphWidths()) {
      ...
      mGrFont = gr_make_font_with_ops(
          mFont->GetAdjustedSize(), &mCallbackData, &ops, mGrFace);
    } else {
    ...
    if (style->variantCaps != NS_FONT_VARIANT_CAPS_NORMAL) {
      switch (style->variantCaps) {
        ...
        case NS_FONT_VARIANT_CAPS_PETITECAPS:
          ...
          mFont->SupportsVariantCaps(aScript,
                                     style->variantCaps,
  ...
  gr_segment* seg = gr_make_seg(
      mGrFont, mGrFace, 0, grFeatures, gr_utf16, aText, numChars, grBidi);
  ...
  nsresult rv = SetGlyphsFromSegment(
      aShapedText, aOffset, aLength, aText, seg, aRounding);
                                        ...
                                        RoundingFlags aRounding)
{
  ...
  for (uint32_t i = 0; i <= cIndex; ++i) {
    ...
    if (c.nGlyphs == 1 &&
      ...
      g.SetComplex(charGlyphs[offs].IsClusterStart(), true, details.Length());
