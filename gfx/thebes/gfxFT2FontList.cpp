      ...
      : mFace(nullptr), mFontDataBuf(nullptr), mOwnsFace(false)
  {
    ...
    if (aFontEntry->mFilename[0] != '/') {
      ...
      if (mFontDataBuf) {
        ...
        mFace = Factory::NewFTFaceFromData(
            nullptr, mFontDataBuf, bufSize, aFontEntry->mFTFontIndex);
        if (!mFace) {
          NS_WARNING("failed to create freetype face");
        }
      }
    } else {
      mFace = Factory::NewFTFace(
          nullptr, aFontEntry->mFilename.get(), aFontEntry->mFTFontIndex);
      if (!mFace) {
        NS_WARNING("failed to create freetype face");
      }
    }
...
FT2FontEntry::CreateScaledFont(const gfxFontStyle* aStyle)
{
  ...
  scaledFont = cairo_scaled_font_create(
      cairoFace, &sizeMatrix, &identityMatrix, fontOptions);
                              ...
                              uint32_t aLength)
{
  ...
  FT2FontEntry* fe =
      FT2FontEntry::CreateFontEntry(face, nullptr, 0, aFontName, aFontData);
  if (fe) {
    fe->mStyle = aStyle;
    fe->mWeight = aWeight;
                              ...
                              const uint8_t* aFontData)
{
  ...
  if (aFontData) {
    ...
    cairo_font_face_set_user_data(
        fe->mFontFace, &sFTUserFontDataKey, userFontData, FTFontDestroyFunc);
  }
...
FT2FontEntry::CairoFontFace()
{
  if (!mFontFace) {
    ...
    cairo_font_face_set_user_data(
        mFontFace, &sFTUserFontDataKey, userFontData, FTFontDestroyFunc);
  }
  return mFontFace;
}
...
FT2FontEntry::ReadCMAP(FontInfoData* aFontInfoData)
{
  ...
  if (NS_SUCCEEDED(rv)) {
    rv = gfxFontUtils::ReadCMAP(
        buffer.Elements(), buffer.Length(), *charmap, mUVSOffset);
  }
...
class FontNameCache
{
 public:
  ...
  FontNameCache() : mMap(&mOps, sizeof(FNCMapEntry), 0), mWriteNeeded(false)
  {
    ...
    // it.
    mOps = (PLDHashTableOps){StringHash,
                             HashMatchEntry,
                             MoveEntry,
  ...
  void GetInfoForFile(const nsCString& aFileName,
                      nsCString& aFaceList,
                      uint32_t* aTimestamp,
                      uint32_t* aFilesize)
  {
  ...
  void CacheFileInfo(const nsCString& aFileName,
                                              ...
                                              StandardFile aStdFile)
{
  ...
  while (end) {
    ...
    FontListEntry fle(
        familyName, faceName, aFileName, weight, stretch, italic, index);
                                        ...
                                        StandardFile aStdFile)
{
  ...
  if (!cachedFaceList.IsEmpty() && 0 == statRetval && s.st_mtime == timestamp &&
      s.st_size == filesize) {
...
gfxFT2FontList::FindFontsInOmnijar(FontNameCache* aCache)
{
  ...
  uint32_t longSize;
  if (cache &&
      NS_SUCCEEDED(cache->GetBuffer(
          JAR_LAST_MODIFED_TIME, &cachedModifiedTimeBuf, &longSize)) &&
...
void
gfxFT2FontList::AddFaceToList(const nsCString& aEntryName,
                              uint32_t aIndex,
                              StandardFile aStdFile,
                              FT_Face aFace,
                              nsCString& aFaceList)
...
searchDone:
  ...
  FT2FontEntry* fe = FT2FontEntry::CreateFontEntry(fontEntry->mFTFace,
...
void
gfxFT2FontList::GetFontFamilyList(
