...
static void
PrepareFontOptions(FcPattern* aPattern, cairo_font_options_t* aFontOptions)
{
...
static double
ChooseFontSize(gfxFontconfigFontEntry* aEntry, const gfxFontStyle& aStyle)
{
                                           ...
                                           bool aNeedsBold)
{
  ...
  if (!mFontData) {
    if (FcPatternGetString(
            renderPattern, FC_FILE, 0, const_cast<FcChar8**>(&file)) !=
            FcResultMatch ||
  ...
  gfxFont* newFont = new gfxFontconfigFont(unscaledFont,
                                           scaledFont,
                                           renderPattern,
                                           size,
                                           this,
                                      ...
                                      nsTArray<uint8_t>& aBuffer)
{
  ...
  if (!mFTFaceInitialized) {
    mFTFaceInitialized = true;
    FcChar8* filename;
    if (FcPatternGetString(mFontPattern, FC_FILE, 0, &filename) !=
        FcResultMatch) {
      return NS_ERROR_FAILURE;
    }
    int index;
    if (FcPatternGetInteger(mFontPattern, FC_INDEX, 0, &index) !=
        FcResultMatch) {
  ...
  if (FT_Load_Sfnt_Table(mFTFace, aTableTag, 0, aBuffer.Elements(), &length) !=
      0) {
    aBuffer.Clear();
    return NS_ERROR_FAILURE;
  }
...
void
gfxFontconfigFontFamily::FindAllFontsForStyle(
...
gfxFcPlatformFontList::AddFontSetFamilies(FcFontSet* aFontSet, bool aAppFonts)
{
  ...
  for (int f = 0; f < aFontSet->nfont; f++) {
    ...
    if (FcStrCmp(canonical, lastFamilyName) != 0) {
      ...
      FcChar8* otherName;
      int n = (cIndex == 0 ? 1 : 0);
      while (FcPatternGetString(font, FC_FAMILY, n, &otherName) ==
             FcResultMatch) {
                                       ...
                                       uint8_t aStyle)
{
  ...
  return new gfxFontconfigFontEntry(
      aFontName, fontPattern, aWeight, aStretch, aStyle);
}
                                        ...
                                        uint32_t aLength)
{
  ...
  return new gfxFontconfigFontEntry(
      aFontName, aWeight, aStretch, aStyle, aFontData, face);
}
                                          ...
                                          gfxFloat aDevToCssSize)
{
  ...
  FcPatternAddString(
      fontWithSentinel, FC_FAMILY, ToFcChar8Ptr(familyToFind.get()));
  ...
  for (int i = 0;
       FcPatternGetString(fontWithSentinel, FC_FAMILY, i, &substName) ==
       FcResultMatch;
       i++) {
                                             ...
                                             nsAString& aFamilyName)
{
  ...
  for (int i = 0; i < givenFS->nfont; i++) {
    ...
    if (FcPatternGetString(
            givenFS->fonts[i], FC_FAMILY, 0, (FcChar8**)&firstFamily) !=
        FcResultMatch) {
      continue;
    }
                                       ...
                                       nsTArray<gfxFontFamily*>& aFamilyList)
{
  ...
  if (usePrefFontList) {
    return gfxPlatformFontList::AddGenericFonts(
        aGenericType, aLanguage, aFamilyList);
  }
