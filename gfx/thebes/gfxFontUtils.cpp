...
nsresult
gfxFontUtils::ReadCMAPTableFormat12or13(const uint8_t* aBuf,
                                        uint32_t aLength,
                                        gfxSparseBitSet& aCharacterMap)
{
...
uint32_t
gfxFontUtils::FindPreferredSubtable(const uint8_t* aBuf,
                                    uint32_t aBufLength,
                       ...
                       uint32_t& aUVSOffset)
{
  ...
  switch (format) {
    case 4:
      return ReadCMAPTableFormat4(
          aBuf + offset, aBufLength - offset, aCharacterMap);

    case 10:
      return ReadCMAPTableFormat10(
          aBuf + offset, aBufLength - offset, aCharacterMap);

    case 12:
    case 13:
      return ReadCMAPTableFormat12or13(
          aBuf + offset, aBufLength - offset, aCharacterMap);
...
uint16_t
gfxFontUtils::MapUVSToGlyphFormat14(const uint8_t* aBuf,
                                    uint32_t aCh,
                                    uint32_t aVS)
{
  ...
  if (BinarySearch(NonDefUVSTableWrapper(*table),
                   0,
                   table->numUVSMappings,
                   aCh,
                   &index)) {
...
uint32_t
gfxFontUtils::MapCharToGlyph(const uint8_t* aCmapBuf,
                             uint32_t aBufLength,
                             uint32_t aUnicode,
...
void
gfxFontUtils::ParseFontList(const nsAString& aFamilyList,
...
void
gfxFontUtils::AppendPrefsFontList(const char* aPrefName,
                                  nsTArray<nsString>& aFontList)
{
...
void
gfxFontUtils::GetPrefsFontList(const char* aPrefName,
                               nsTArray<nsString>& aFontList)
{
  aFontList.Clear();
...
nsresult
gfxFontUtils::MakeUniqueUserFontName(nsAString& aName)
{
...
gfxFontUtils::FindTableDirEntry(const void* aFontData, uint32_t aTableTag)
{
  ...
  return static_cast<TableDirEntry*>(bsearch(&aTableTag,
                                             dir,
                                             uint16_t(header->numTables),
...
gfxFontUtils::GetTableFromFontData(const void* aFontData, uint32_t aTableTag)
{
  ...
  if (dir) {
    return hb_blob_create(
        reinterpret_cast<const char*>(aFontData) + dir->offset,
        dir->length,
...
nsresult
gfxFontUtils::RenameFont(const nsAString& aName,
                         ...
                         FallibleTArray<uint8_t>* aNewFont)
{
  ...
  memset(
      newFontData + aFontDataLength, 0, paddedFontDataSize - aFontDataLength);
  ...
  mozilla::NativeEndian::copyAndSwapToBigEndian(
      strData, aName.BeginReading(), aName.Length());
                                  ...
                                  nsAString& aFullName)
{
  ...
  hb_blob_t* nameBlob =
      hb_blob_create((const char*)aFontData + dirEntry->offset,
                     len,
                     HB_MEMORY_MODE_READONLY,
                     nullptr,
                     nullptr);
...
nsresult
gfxFontUtils::GetFullNameFromTable(hb_blob_t* aNameTable, nsAString& aFullName)
{
  nsAutoString name;
...
nsresult
gfxFontUtils::ReadCanonicalName(hb_blob_t* aNameTable,
                                uint32_t aNameID,
                                nsString& aName)
{
                                ...
                                nsString& aName)
{
  ...
  rv = ReadNames(
      aNameData, aDataLen, aNameID, CANONICAL_LANG_ID, PLATFORM_ID, names);
  NS_ENSURE_SUCCESS(rv, rv);
  ...
  if (names.Length() == 0) {
    ...
    if (names.Length() == 0) {
      rv = ReadNames(
          aNameData, aDataLen, aNameID, LANG_ALL, PLATFORM_ID_MICROSOFT, names);
      NS_ENSURE_SUCCESS(rv, rv);
    }
  }
#endif
...
const char* gfxFontUtils::gISOFontNameCharsets[] = {
    /* 0 */ "windows-1252", /* US-ASCII */
                            /* 1 */
    nullptr, /* spec says "ISO 10646" but does not specify encoding form! */
  ...
  switch (aPlatform) {
    ...
    case PLATFORM_ID_MAC: {
      ...
      for (uint32_t i = 0; i < 2; ++i) {
        size_t idx;
        if (BinarySearchIf(gMacFontNameCharsets,
                           0,
                           ArrayLength(gMacFontNameCharsets),
                           MacCharsetMappingComparator(searchValue),
                           &idx)) {
...
bool
gfxFontUtils::DecodeFontName(const char* aNameData,
                             int32_t aByteLen,
                             uint32_t aPlatformCode,
                             ...
                             nsAString& aName)
{
  ...
  const char* csName =
      GetCharsetForFontName(aPlatformCode, aScriptCode, aLangCode);
  ...
  if (csName[0] == 0) {
    ...
    CopySwapUTF16(
        aNameData, reinterpret_cast<char*>(aName.BeginWriting()), strLen);
#else
    ...
    if (aPlatformCode == PLATFORM_ID_MAC) {
      ...
      if (str) {
        ...
        CFStringGetCharacters(
            str, CFRangeMake(0, length), (UniChar*)aName.BeginWriting());
        CFRelease(str);
        return true;
      }
    }
                        ...
                        nsTArray<nsString>& aNames)
{
  ...
  for (i = 0; i < nameCount; i++, nameRecord++) {
    ...
    DecodeFontName(aNameData + nameStringsBase + nameoff,
                   namelen,
                   platformID,
