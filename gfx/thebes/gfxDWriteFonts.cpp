...
{
  if ((GetStyle()->style != NS_FONT_STYLE_NORMAL) && aFontEntry->IsUpright() &&
...
gfxDWriteFont::CopyWithAntialiasOption(AntialiasOption anAAOption)
{
  ...
  return MakeUnique<gfxDWriteFont>(
      unscaledFont, entry, &mStyle, mNeedsBold, anAAOption);
}
...
gfxDWriteFont::GetScaledFont(mozilla::gfx::DrawTarget* aTarget)
{
  if (!mAzureScaledFont) {
    ...
    mAzureScaledFont =
        Factory::CreateScaledFontForDWriteFont(mFontFace,
                                               fontStyle,
                                               GetUnscaledFont(),
                                               GetAdjustedSize(),
                                               useEmbeddedBitmap,
