    ...
    nsIHandleReportCallback* aHandleReport, nsISupports* aData, bool aAnonymize)
{
  ...
  MOZ_COLLECT_REPORT("explicit/gfx/font-cache",
                     KIND_HEAP,
                     UNITS_BYTES,
                     sizes.mFontInstances,
                     "Memory used for active font instances.");

  MOZ_COLLECT_REPORT("explicit/gfx/font-shaped-words",
                     KIND_HEAP,
                     UNITS_BYTES,
                     sizes.mShapedWords,
...
{
  Key key(
      aFont->GetFontEntry(), aFont->GetStyle(), aFont->GetUnicodeRangeMap());
                      ...
                      nsTArray<gfxFontFeature>& aFontFeatures)
{
  ...
  for (uint32_t i = 0; i < numAlternates; i++) {
    ...
    bool found = featureLookup->GetFontFeatureValuesFor(
        aFamily, av.alternate, av.value, values);
...
/* static */ void
gfxFontShaper::MergeFontFeatures(const gfxFontStyle* aStyle,
...
void
gfxShapedText::SetGlyphs(uint32_t aIndex,
...
gfxFont::GetRoundOffsetsToPixels(DrawTarget* aDrawTarget)
{
  ...
  if (!scaled_font) {
    result |= RoundingFlags::
        kRoundX;  // default to the same as the fallback path below
    return result;
  }
...
static void
CollectLookupsByFeature(hb_face_t* aFace,
                        hb_tag_t aTableTag,
                        ...
                        hb_set_t* aLookups)
{
  ...
  do {
    ...
    hb_ot_layout_feature_get_lookups(
        aFace, aTableTag, aFeatureIndex, offset, &len, lookups);
    for (i = 0; i < len; i++) {
...
static void
CollectLookupsByLanguage(hb_face_t* aFace,
                         hb_tag_t aTableTag,
                         ...
                         uint32_t aLangIndex)
{
  ...
  do {
    ...
    hb_ot_layout_language_get_feature_indexes(aFace,
                                              aTableTag,
                                              aScriptIndex,
                                              aLangIndex,
                                              offset,
    ...
    for (i = 0; i < len; i++) {
      ...
      hb_tag_t featureTag;
      uint32_t tagLen = 1;
      hb_ot_layout_language_get_feature_tags(aFace,
                                             aTableTag,
                                             aScriptIndex,
                                             aLangIndex,
                                             offset + i,
    ...
    bool& aHasDefaultFeatureWithGlyph)
{
  ...
  // default lang
  CollectLookupsByLanguage(aFace,
                           aTableTag,
                           aDefaultFeatures,
  ...
  while (hb_set_next(defaultFeatureLookups, &index)) {
    hb_ot_layout_lookup_collect_glyphs(
        aFace, aTableTag, index, glyphs, glyphs, glyphs, nullptr);
...
static void
HasLookupRuleWithGlyph(hb_face_t* aFace,
                       hb_tag_t aTableTag,
                       bool& aHasGlyph,
                       hb_tag_t aSpecificFeature,
                       bool& aHasGlyphSpecific,
                       uint16_t aGlyph)
{
  ...
  if (hb_ot_layout_has_substitution(face)) {
    ...
    do {
      ...
      hb_ot_layout_table_get_script_tags(
          face, HB_OT_TAG_GSUB, offset, &len, scriptTags);
      for (uint32_t i = 0; i < len; i++) {
        bool isDefaultFeature = false;
        Script s;
        if (!HasLookupRuleWithGlyphByScript(face,
                                            HB_OT_TAG_GSUB,
                                            scriptTags[i],
                                            offset + i,
                                            spaceGlyph,
                                ...
                                Script aRunScript)
{
  NS_ConvertASCIItoUTF16 unicodeString(reinterpret_cast<const char*>(aString),
                                       aLength);
  return SupportsSubSuperscript(
      aSubSuperscript, unicodeString.get(), aLength, aRunScript);
}
...
class GlyphBufferAzure
{
 ...
 public:
  GlyphBufferAzure(const TextRunDrawParams& aRunParams,
      ...
      : mRunParams(aRunParams), mFontParams(aFontParams), mNumGlyphs(0)
  {
  }
...
#define GLYPH_BUFFER_SIZE (2048 / sizeof(Glyph))
  ...
  Glyph* GlyphBuffer() { return *mGlyphBuffer.addr(); }

  Glyph* AppendGlyph() { return &GlyphBuffer()[mNumGlyphs++]; }
    ...
    if (mRunParams.drawMode & DrawMode::GLYPH_FILL) {
      if (state.pattern || mFontParams.contextPaint) {
        ...
        if (pat) {
          ...
          mRunParams.dt->FillGlyphs(mFontParams.scaledFont,
                                    buf,
                                    *pat,
        ...
        mRunParams.dt->FillGlyphs(
            mFontParams.scaledFont,
            buf,
            SurfacePattern(
                state.sourceSurface, ExtendMode::CLAMP, state.surfTransform),
            mFontParams.drawOptions,
            mFontParams.renderingOptions);
      } else {
        mRunParams.dt->FillGlyphs(mFontParams.scaledFont,
  ...
  {
    mRunParams.dt->StrokeGlyphs(mFontParams.scaledFont,
                                aBuf,
                                aPattern,
                                *mRunParams.strokeOpts,
                                mFontParams.drawOptions,
...
void
gfxFont::DrawOneGlyph(uint32_t aGlyphID,
                      double aAdvance,
                      gfxPoint* aPt,
          ...
          if (glyphData->IsMissing()) {
            ...
            if (aRunParams.drawMode != DrawMode::GLYPH_PATH && advance > 0) {
              ...
              if (aFontParams.passedInvMatrix) {
                ...
                aRunParams.dt->SetTransform(*aFontParams.passedInvMatrix *
                                            oldMat);
              }
            ...
            DrawOneGlyph(
                details->mGlyphID, advance, &glyphXY, buffer, &emittedGlyphs);
          }
...
void
gfxFont::DrawEmphasisMarks(const gfxTextRun* aShapedText,
                           gfxPoint* aPt,
                           uint32_t aOffset,
                           uint32_t aCount,
...
void
gfxFont::Draw(const gfxTextRun* aTextRun,
              uint32_t aStart,
              uint32_t aEnd,
              gfxPoint* aPt,
              const TextRunDrawParams& aRunParams,
              gfx::ShapedTextFlags aOrientation)
{
  ...
  if (mScaledFont && aRunParams.dt->GetBackendType() != BackendType::CAIRO) {
  ...
  if (mStyle.baselineOffset != 0.0) {
    baseline += mStyle.baselineOffset * aTextRun->GetAppUnitsPerDevUnit();
  }

  bool emittedGlyphs =
      DrawGlyphs(aTextRun, aStart, aEnd - aStart, aPt, aRunParams, fontParams);
...
bool
gfxFont::RenderSVGGlyph(gfxContext* aContext,
                        gfxPoint aPoint,
                        uint32_t aGlyphId,
                          ...
                          uint32_t aGlyphId) const
{
  ...
  if (!GetFontEntry()->GetColorLayersInfo(
          aGlyphId, defaultColor, layerGlyphs, layerColors)) {
    return false;
  }
  ...
  for (uint32_t layerIndex = 0; layerIndex < layerGlyphs.Length();
    ...
    aDrawTarget->FillGlyphs(scaledFont,
                            buffer,
                               ...
                               const gfxTextRun* aTextRun)
{
  if (!mFontEntry->mSpaceGlyphIsInvisibleInitialized &&
    ...
    mFontEntry->mSpaceGlyphIsInvisible =
        extents->GetTightGlyphExtentsAppUnits(
                 ...
                 gfx::ShapedTextFlags aOrientation)
{
  ...
  if (aBoundingBoxType == TIGHT_HINTED_OUTLINE_EXTENTS &&
    ...
    if (mNonAAFont) {
      return mNonAAFont->Measure(aTextRun,
                                 aStart,
                                 aEnd,
    ...
    if (glyphData->IsSimpleGlyph()) {
      ...
      if ((aBoundingBoxType != LOOSE_INK_EXTENTS || needsGlyphExtents) &&
        ...
        if (extentsWidth != gfxGlyphExtents::INVALID_WIDTH &&
          ...
          gfxRect glyphRect;
          if (!extents->GetTightGlyphExtentsAppUnits(
                  this, aRefDrawTarget, glyphIndex, &glyphRect)) {
            glyphRect = gfxRect(0,
                                metrics.mBoundingBox.Y(),
                                advance,
  ...
  if (allGlyphsInvisible) {
    ...
    if (aBoundingBoxType == LOOSE_INK_EXTENTS) {
      UnionRange(x, &advanceMin, &advanceMax);
      gfxRect fontBox(advanceMin,
                      -metrics.mAscent,
                      advanceMax - advanceMin,
                      metrics.mAscent + metrics.mDescent);
                       ...
                       gfxTextPerfMetrics* aTextPerf GFX_MAYBE_UNUSED)
{
  ...
  sw = gfxShapedWord::Create(
      aText, aLength, aRunScript, aAppUnitsPerDevUnit, aFlags, aRounding);
  ...
  DebugOnly<bool> ok = ShapeText(
      aDrawTarget, aText, 0, aLength, aRunScript, aVertical, aRounding, sw);
...
gfxFont::CacheHashEntry::KeyEquals(const KeyTypePointer aKey) const
{
  ...
  return (0 == memcmp(sw->TextUnicode(),
                      aKey->mText.mDouble,
                   ...
                   gfxShapedText* aShapedText)
{
  ...
  if (FontCanSupportGraphite() && !aVertical) {
    if (gfxPlatform::GetPlatform()->UseGraphiteShaping()) {
      ...
      ok = mGraphiteShaper->ShapeText(aDrawTarget,
                                      aText,
                                      aOffset,
                                      aLength,
                                      aScript,
  ...
  if (!ok) {
    ...
    ok = mHarfBuzzShaper->ShapeText(aDrawTarget,
                                    aText,
                                    aOffset,
                                    aLength,
                                    aScript,
  ...
  PostShapingFixup(
      aDrawTarget, aText, aOffset, aLength, aVertical, aShapedText);
                          ...
                          gfxShapedText* aShapedText)
{
  if (IsSyntheticBold()) {
    ...
    if (metrics.maxAdvance > metrics.aveCharWidth) {
      ...
      aShapedText->AdjustAdvancesForSyntheticBold(
          synBoldOffset, aOffset, aLength);
    }
  }
}
                                   ...
                                   gfxTextRun* aTextRun)
{
  ...
  for (uint32_t i = 0; i <= aLength && ok; ++i) {
    ...
    if (length > 0) {
      ok = ShapeFragmentWithoutWordCache(aDrawTarget,
                                         aText + fragStart,
                                         aOffset + fragStart,
                                         length,
                                         aScript,
    ...
    if (ch == '\t') {
      ...
      if (GetFontEntry()->IsUserFont() && HasCharacter(ch)) {
        ShapeFragmentWithoutWordCache(aDrawTarget,
                                      aText + i,
                                      aOffset + i,
                                      1,
                                      aScript,
...
inline static bool
IsChar8Bit(char16_t aCh)
{
  return aCh < 0x100;
}

inline static bool
HasSpaces(const uint8_t* aString, uint32_t aLen)
{
  return memchr(aString, 0x20, aLen) != nullptr;
}

inline static bool
HasSpaces(const char16_t* aString, uint32_t aLen)
{
                             ...
                             ShapedTextFlags aOrientation)
{
  ...
  if (SpaceMayParticipateInShaping(aRunScript)) {
    if (aRunLength > wordCacheCharLimit || HasSpaces(aString, aRunLength)) {
      ...
      return ShapeTextWithoutWordCache(aDrawTarget,
                                       aString,
                                       aRunStart,
                                       aRunLength,
                                       aRunScript,
    ...
    if ((i == aLength || runAction != chAction) && runStart < i) {
      ...
      switch (runAction) {
        case kNoChange:
          ...
          aTextRun->AddGlyphRun(
              f, aMatchType, aOffset + runStart, true, aOrientation);
          if (!f->SplitAndInitTextRun(aDrawTarget,
                                      aTextRun,
                                      aText + runStart,
                                      aOffset + runStart,
                                      runLength,
            ...
            if (!f->SplitAndInitTextRun(aDrawTarget,
              ...
              RefPtr<gfxTextRun> mergedRun(
                  gfxTextRun::Create(&params,
                                     ...
                                     gfx::ShapedTextFlags(),
                                     nsTextFrameUtils::Flags()));
              MergeCharactersInTextRun(mergedRun.get(),
                                       tempRun.get(),
                                       charsToMergeArray.Elements(),
                                       deletedCharsArray.Elements());
              gfxTextRun::Range runRange(0, runLength);
              aTextRun->CopyGlyphDataFrom(
                  mergedRun.get(), runRange, aOffset + runStart);
            }
          } else {
            aTextRun->AddGlyphRun(
                f, aMatchType, aOffset + runStart, true, aOrientation);
                              ...
                              bool aSyntheticUpper)
{
  NS_ConvertASCIItoUTF16 unicodeString(reinterpret_cast<const char*>(aText),
                                       aLength);
  return InitFakeSmallCapsRun(aDrawTarget,
                              aTextRun,
...
void
gfxFont::SetupGlyphExtents(DrawTarget* aDrawTarget,
                           uint32_t aGlyphID,
                           bool aNeedTight,
                           gfxGlyphExtents* aExtents)
{
  ...
  if (!aNeedTight && extents.x_bearing >= 0 &&
    ...
    uint32_t appUnitsWidth = uint32_t(
        ceil((extents.x_bearing + extents.width) * appUnitsPerDevUnit));
  ...
  gfxRect bounds(extents.x_bearing * d2a,
                 extents.y_bearing * d2a,
                 extents.width * d2a,
                 extents.height * d2a);
...
void
gfxFont::CalculateDerivedMetrics(Metrics& aMetrics)
{
  aMetrics.maxAscent =
