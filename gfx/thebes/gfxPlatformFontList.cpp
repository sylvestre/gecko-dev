...
void
gfxPlatformFontList::GenerateFontListKey(const nsAString& aKeyName,
                                         nsAString& aResult)
{
  aResult = aKeyName;
  ToLowerCase(aResult);
...
gfxPlatformFontList::SearchFamiliesForFaceName(const nsAString& aFaceName)
{
  ...
  Telemetry::AccumulateTimeDelta(
      Telemetry::FONTLIST_INITFACENAMELISTS, start, end);
  if (LOG_FONTINIT_ENABLED()) {
...
void
gfxPlatformFontList::GetFontFamilyList(
...
gfxFontEntry*
gfxPlatformFontList::SystemFindFontForChar(uint32_t aCh,
                                           uint32_t aNextCh,
                                           Script aRunScript,
...
gfxFontEntry*
gfxPlatformFontList::CommonFontFallback(uint32_t aCh,
                                        uint32_t aNextCh,
                                        Script aRunScript,
                                        ...
                                        gfxFontFamily** aMatchedFamily)
{
  ...
  gfxPlatform::GetPlatform()->GetCommonFallbackFonts(
      aCh, aNextCh, aRunScript, defaultFallbacks);
                                        ...
                                        gfxFontFamily** aMatchedFamily)
{
  ...
  if (!useCmaps) {
    ...
    gfxFontEntry* fe = PlatformGlobalFontFallback(
        aCh, aRunScript, aMatchStyle, aMatchedFamily);
    if (fe) {
      return fe;
    }
  }
...
void
gfxPlatformFontList::AddOtherFamilyName(gfxFontFamily* aFamilyEntry,
...
bool
gfxPlatformFontList::GetStandardFamilyName(const nsAString& aFontName,
...
void
gfxPlatformFontList::GetLangPrefs(eFontPrefLang aPrefLangs[],
                                  uint32_t& aLen,
...
void
gfxPlatformFontList::AppendCJKPrefLangs(eFontPrefLang aPrefLangs[],
                                        uint32_t& aLen,
                                        eFontPrefLang aCharLang,
                                        eFontPrefLang aPageLang)
{
...
void
gfxPlatformFontList::AppendPrefLang(eFontPrefLang aPrefLangs[],
                                    uint32_t& aLen,
...
gfxPlatformFontList::ClearLangGroupPrefFonts()
{
  for (uint32_t i = eFontPrefLang_First;
       i < eFontPrefLang_First + eFontPrefLang_Count;
       i++) {
    auto& prefFontsLangGroup = mLangGroupPrefFonts[i];
    for (uint32_t j = eFamily_generic_first;
         j < eFamily_generic_first + eFamily_generic_count;
         j++) {
...
void
gfxPlatformFontList::InitOtherFamilyNamesInternal(
    bool aDeferOtherFamilyNamesLoading)
{
  ...
  if (aDeferOtherFamilyNamesLoading) {
    ...
    TimeStamp end = TimeStamp::Now();
    Telemetry::AccumulateTimeDelta(
        Telemetry::FONTLIST_INITOTHERFAMILYNAMES, start, end);
