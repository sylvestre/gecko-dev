                                         ...
                                         gfxFloat aOpacity)
{
  ...
  IntRect intRect = IntRect::RoundOut(aSamplingRect.x,
                                      aSamplingRect.y,
                                      aSamplingRect.width,
                                      aSamplingRect.height);
...

{
  ...
  DrawInternal(aContext->GetDrawTarget(),
               aContext->CurrentOp(),
                                 ...
                                 const gfxMatrix& aTransform)
{
  ...
  SurfacePattern pattern(mSourceSurface,
                         aExtendMode,
                         patternTransform,
                         aSamplingFilter,
                         aSamplingRect);
  ...
  if (aOp == CompositionOp::OP_SOURCE && aOpacity == 1.0) {
    ...
    aDrawTarget->FillRect(
        fillRect, pattern, DrawOptions(aOpacity, aOp, aAntialiasMode));
  }
}
...
gfxCallbackDrawable::MakeSurfaceDrawable(gfxContext* aContext,
                                         const SamplingFilter aSamplingFilter)
{
  ...
  Draw(ctx,
       gfxRect(0, 0, mSize.width, mSize.height),
       ExtendMode::CLAMP,
       aSamplingFilter);
                          ...
                          const gfxMatrix& aTransform)
{
  ...
  if (mSurfaceDrawable)
    return mSurfaceDrawable->Draw(aContext,
                                  aFillRect,
                                  aExtendMode,
                                  aSamplingFilter,
                                  aOpacity,
                         ...
                         const gfxMatrix& aTransform)
{
  ...
  if (IsRepeatingExtendMode(aExtendMode)) {
    ...
    return callbackDrawable->Draw(aContext,
                                  aFillRect,
                                  aExtendMode,
                                  aSamplingFilter,
                                  aOpacity,
  ...
  aDrawTarget.FillRect(
      ToRect(aFillRect), *mPattern->GetPattern(&aDrawTarget), drawOptions);
