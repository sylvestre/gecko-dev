  ...
  if (aFontInfoData &&
    ...
    if (NS_SUCCEEDED(rv)) {
      rv = gfxFontUtils::ReadCMAP(
          cmap.Elements(), cmap.Length(), *charmap, mUVSOffset);
    }
  }
...
void
GDIFontEntry::FillLogFont(LOGFONTW* aLogFont, uint16_t aWeight, gfxFloat aSize)
{
...
void
GDIFontEntry::InitLogFont(const nsAString& aName, gfxWindowsFontType aFontType)
{
                              ...
                              bool aFamilyHasItalicFace)
{
  ...
  GDIFontEntry* fe = new GDIFontEntry(aName,
                                      aFontType,
                                      aStyle,
                                      aWeight,
                                      aStretch,
...
GDIFontFamily::FindStyleVariations(FontInfoData* aFontInfoData)
{
  ...
  if (hasItalicFace) {
    for (RefPtr<gfxFontEntry>& fontEntry : mAvailableFonts) {
      static_cast<GDIFontEntry*>(fontEntry.get())->mFamilyHasItalicFace = true;
    }
  }
}
...
gfxGDIFontList::InitFontListForPlatform()
{
  ...
  int result = EnumFontFamiliesExW(
      hdc.GetDC(), &logfont, (FONTENUMPROCW)&EnumFontFamExProc, 0, 0);
...
int CALLBACK
GDIFontInfo::EnumerateFontsForFamily(const ENUMLOGFONTEXW* lpelfe,
                                     const NEWTEXTMETRICEXW* nmetrics,
                                     DWORD fontType,
                                     LPARAM data)
{
  ...
  if (famData->mFontInfo.mLoadCmaps && (feType == GFX_FONT_TYPE_PS_OPENTYPE ||
    ...
    if (cmapSize != GDI_ERROR && cmapSize > 0 &&
      ...
      if (NS_SUCCEEDED(gfxFontUtils::ReadCMAP(
              cmapData.Elements(), cmapSize, *charmap, offset))) {
