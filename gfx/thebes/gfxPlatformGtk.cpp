...
void
gfxPlatformGtk::GetCommonFallbackFonts(uint32_t aCh,
                                       uint32_t aNextCh,
                                       Script aRunScript,
...
nsresult
gfxPlatformGtk::GetStandardFamilyName(const nsAString& aFontName,
...
void
gfxPlatformGtk::FontsPrefsChanged(const char* aPref)
{
...
uint32_t
gfxPlatformGtk::MaxGenericSubstitions()
{
  ...
  if (edidAtom) {
    if (Success == XGetWindowProperty(dpy,
      ...
      qcms_data_create_rgb_with_gamma(
          whitePoint, primaries, gamma, &mem, &size);

#ifdef DEBUG_tor
      if (size > 0) {
        fprintf(
            stderr, "ICM profile read from %s successfully\n", EDID1_ATOM_NAME);
      }
#endif
    }
  }
...
#if (MOZ_WIDGET_GTK == 2)
void
gfxPlatformGtk::SetGdkDrawable(cairo_surface_t* target, GdkDrawable* drawable)
{
  if (cairo_surface_status(target)) return;

  g_object_ref(drawable);

  cairo_surface_set_user_data(
      target, &cairo_gdk_drawable_key, drawable, g_object_unref);
}

GdkDrawable*
gfxPlatformGtk::GetGdkDrawable(cairo_surface_t* target)
{
  if (cairo_surface_status(target)) return nullptr;
...
#ifdef MOZ_X11
  if (cairo_surface_get_type(target) != CAIRO_SURFACE_TYPE_XLIB) return nullptr;

  // try looking it up in gdk's table
  result = (GdkDrawable*)gdk_xid_table_lookup(
      cairo_xlib_surface_get_drawable(target));
  if (result) {
  ...
  class GLXDisplay final : public VsyncSource::Display
  {
   ...
   public:
    ...
    void SetupGLContext()
    {
      ...
      mGLContext =
          gl::GLContextGLX::CreateGLContext(gl::CreateContextFlags::NONE,
                                            gl::SurfaceCaps::Any(),
                                            false,
                                            mXDisplay,
                                            root,
   ...
   private:
    ...
    void RunVsync()
    {
      ...
      for (;;) {
        ...
        int status;
        if ((status = gl::sGLXLibrary.fWaitVideoSync(
                 2, nextSync % 2, &syncCounter)) != 0) {
