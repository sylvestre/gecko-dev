 ...
 */
bool
gfxXlibNativeRenderer::DrawDirect(DrawTarget* aDT,
                                  IntSize size,
                                  uint32_t flags,
                                  Screen* screen,
                                  Visual* visual)
...
bool
gfxXlibNativeRenderer::DrawCairo(
    cairo_t* cr, IntSize size, uint32_t flags, Screen* screen, Visual* visual)
{
...
// both directions).
static bool
FormatConversionIsExact(Screen* screen,
                        Visual* visual,
...
void
gfxXlibNativeRenderer::Draw(gfxContext* ctx,
                            IntSize size,
                            uint32_t flags,
                            Screen* screen,
                            Visual* visual)
{
  ...
  cairo_surface_t* tempXlibSurface =
      CreateTempXlibSurface(cairoTarget,
                            drawTarget,
                            size,
  ...
  if (method != eAlphaExtraction) {
    RefPtr<SourceSurface> sourceSurface =
        Factory::CreateSourceSurfaceForCairoSurface(
            tempXlibSurface, size, moz2DFormat);
    if (sourceSurface) {
