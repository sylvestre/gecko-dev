...
gfxImageSurface::gfxImageSurface()
    : mSize(0, 0), mOwnsData(false), mFormat(SurfaceFormat::UNKNOWN), mStride(0)
{
}
...
void
gfxImageSurface::InitWithData(unsigned char* aData,
                              const IntSize& aSize,
                              long aStride,
                              gfxImageFormat aFormat)
{
...
// Use fallible allocators here
  ...
  return moz_posix_memalign(
             &ptr, 1 << gfxAlphaRecovery::GoodAlignmentLog2(), aSize)
             ? nullptr
             : ptr;
#else
...
gfxImageSurface::gfxImageSurface(const IntSize& size,
...
static void
CopyForStride(unsigned char* aDest,
              unsigned char* aSrc,
              const IntSize& aSize,
              long aDestStride,
              long aSrcStride)
