...
class MOZ_STACK_CLASS gfxOTSContext : public ots::OTSContext
{
 public:
  ...
  virtual void Message(int level,
                       const char* format,
...
gfxUserFontEntry::DoLoadNextSrc(bool aForceAsync)
{
  ...
  while (mSrcIndex < numSrc) {
        ...
        if (NS_SUCCEEDED(rv) && principal != nullptr) {
          ...
          if (NS_SUCCEEDED(rv) && loadDoesntSpin) {
            ...
            if (NS_SUCCEEDED(rv) && LoadPlatformFont(buffer, bufferLength)) {
              ...
              mFontSet->LogMessage(
                  this, "font load failed", nsIScriptError::errorFlag, rv);
            }
            ...
            if (loadOK) {
              ...
              mFontSet->LogMessage(
                  this, "download failed", nsIScriptError::errorFlag, rv);
            }
          }
        } else {
          mFontSet->LogMessage(
              this, "download not allowed", nsIScriptError::errorFlag, rv);
        }
      } else {
    ...
    // FontFace buffer ==> load immediately
      ...
      if (buffer && LoadPlatformFont(buffer, bufferLength)) {
        ...
        mFontSet->LogMessage(
            this, "font load failed", nsIScriptError::errorFlag);
      }
    }
  ...
  if (mUnsupportedFormat) {
    mFontSet->LogMessage(
        this, "no supported format found", nsIScriptError::warningFlag);
  }
...
gfxUserFontEntry::LoadPlatformFont(const uint8_t* aFontData, uint32_t& aLength)
{
  ...
  if (fe) {
    ...
    if (fontType == GFX_USERFONT_WOFF) {
      ...
      CopyWOFFMetadata<WOFF2Header>(
          aFontData, aLength, &metadata, &metaOrigLen);
    ...
    uint8_t aFontDisplay)
{
  ...
  if (family) {
    entry = FindExistingUserFontEntry(family,
                                      aFontFaceSrcList,
                                      aWeight,
                                      aStretch,
                                      aStyle,
  ...
  if (!entry) {
    entry = CreateUserFontEntry(aFontFaceSrcList,
                                aWeight,
                                aStretch,
                                aStyle,
                                aFeatureSettings,
                                       ...
                                       bool aPrivate)
{
  ...
  if (ServoStyleSet::IsInServoTraversal()) {
    ...
    allowed =
        aUserFontEntry->mFontSet->IsFontLoadAllowed(aSrcURI->get(),
                                                    principal,
                                                    /* aViolations */ nullptr);
    MOZ_ASSERT(
    ...
    nsIHandleReportCallback* aHandleReport, nsISupports* aData, bool aAnonymize)
{
  ...
  MOZ_COLLECT_REPORT(
      "explicit/gfx/user-fonts/cache-overhead",
      KIND_HEAP,
      UNITS_BYTES,
