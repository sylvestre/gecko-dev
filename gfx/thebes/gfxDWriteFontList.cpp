                       ...
                       nsAString& aFontName)
{
  ...
  hr = aFont->GetInformationalStrings(
      aWhichName, getter_AddRefs(infostrings), &exists);
  if (FAILED(hr) || !exists) {
    return E_FAIL;
  }
                                   ...
                                   FontInfoData* aFontInfoData)
{
  ...
  if (!mOtherFamilyNamesInitialized || !mFaceNamesInitialized) {
    gfxFontFamily::ReadFaceNames(
        aPlatformFontList, aNeedFullnamePostscriptNames, aFontInfoData);
  }
}
                                  ...
                                  nsTArray<uint8_t>& aBuffer)
{
      ...
      if (font.IsValid()) {
        ...
        if (tableSize != GDI_ERROR) {
          if (aBuffer.SetLength(tableSize, fallible)) {
            ::GetFontData(
                dc.GetDC(), tagBE, 0, aBuffer.Elements(), aBuffer.Length());
            return NS_OK;
          }
  ...
  BOOL exists;
  HRESULT hr = fontFace->TryGetFontTable(
      tagBE, (const void**)&tableData, &len, &tableContext, &exists);
  if (FAILED(hr) || !exists) {
...
gfxDWriteFontEntry::GetFontTable(uint32_t aTag)
{
  ...
  HRESULT hr = mFontFace->TryGetFontTable(
      NativeEndian::swapToBigEndian(aTag), &data, &size, &context, &exists);
  if (SUCCEEDED(hr) && exists) {
    FontTableRec* ftr = new FontTableRec(mFontFace, context);
    return hb_blob_create(static_cast<const char*>(data),
                          size,
...
gfxDWriteFontEntry::ReadCMAP(FontInfoData* aFontInfoData)
{
  ...
  if (LOG_CMAPDATA_ENABLED()) {
    char prefix[256];
    SprintfLiteral(
        prefix, "(cmapdata) name: %.220s", NS_ConvertUTF16toUTF8(mName).get());
                                   ...
                                   DWRITE_FONT_SIMULATIONS aSimulations)
{
  ...
  if ((aSimulations & DWRITE_FONT_SIMULATIONS_BOLD) &&
    ...
    HRESULT hr =
        Factory::GetDWriteFactory()->CreateFontFace(mFontFace->GetType(),
                                                    numberOfFiles,
                                                    files.Elements(),
                                                    mFontFace->GetIndex(),
                                                    aSimulations,
                                    ...
                                    uint32_t aLength)
{
  ...
  HRESULT hr = gfxDWriteFontFileLoader::CreateCustomFontFile(
      newFontData, getter_AddRefs(fontFile), getter_AddRefs(fontFileStream));
...
gfxDWriteFontList::InitFontListForPlatform()
{
  ...
  if (LOG_FONTINIT_ENABLED()) {
    GetTimeFormatA(LOCALE_INVARIANT,
                                      ...
                                      gfxFloat aDevToCssSize)
{
  ...
  return gfxPlatformFontList::FindAndAddFamilies(
      aFamily, aOutput, aFlags, aStyle, aDevToCssSize);
}
...
static HRESULT
GetFamilyName(IDWriteFont* aFont, nsString& aFamilyName)
{
  HRESULT hr;
  RefPtr<IDWriteFontFamily> family;
                                              ...
                                              gfxFontFamily** aMatchedFamily)
{
  ...
  if (!mFallbackFormat) {
    hr = dwFactory->CreateTextFormat(L"Arial",
                                     nullptr,
  ...
  hr = dwFactory->CreateTextLayout(str,
                                   strLen,
                                   mFallbackFormat,
                                   200.0f,
                                   200.0f,
...
DirectWriteFontInfo::LoadFontFamilyData(const nsAString& aFamilyName)
{
  ...
  memcpy(
      famName.Elements(), aFamilyName.BeginReading(), len * sizeof(char16_t));
  famName[len] = 0;
  ...
  for (uint32_t i = 0; i < family->GetFontCount(); i++) {
      ...
      if (!loadFaceNamesUsingDirectWrite) {
        ...
        if (SUCCEEDED(hr)) {
          ...
          hr = dwFontFace->TryGetFontTable(
              kNAME, (const void**)&nameData, &nameSize, &ctx, &exists);

          if (SUCCEEDED(hr) && nameData && nameSize > 0) {
            gfxFontUtils::ReadCanonicalName(nameData,
                                            nameSize,
    ...
    if (mLoadCmaps) {
      ...
      hr = dwFontFace->TryGetFontTable(
          kCMAP, (const void**)&cmapData, &cmapSize, &ctx, &exists);
...
BundledFontFileEnumerator::GetCurrentFontFile(IDWriteFontFile** aFontFile)
{
  ...
  return mFactory->CreateFontFileReference(
      (const WCHAR*)path.get(), nullptr, aFontFile);
}
...
gfxDWriteFontList::CreateBundledFontsCollection(IDWriteFactory* aFactory)
{
  ...
  HRESULT hr = aFactory->CreateCustomFontCollection(
      loader, &key, sizeof(key), getter_AddRefs(collection));
