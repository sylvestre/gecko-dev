...
D3D11Checks::DoesRenderTargetViewNeedRecreating(ID3D11Device* aDevice)
{
  ...
  HRESULT hr = aDevice->CreateTexture2D(
      &offscreenTextureDesc, NULL, getter_AddRefs(offscreenTexture));
  if (FAILED(hr)) {
  ...
  hr = aDevice->CreateRenderTargetView(
      offscreenTexture, &offscreenRTVDesc, getter_AddRefs(offscreenRTView));
  if (FAILED(hr)) {
...
D3D11Checks::DoesDeviceWork()
{
  ...
  if (GetModuleHandleW(L"igd10umd32.dll")) {
    const wchar_t* checkModules[] = {
        L"dlumd32.dll", L"dlumd11.dll", L"dlumd10.dll"};
...
// with E_OUTOFMEMORY.
static bool
DoesTextureSharingWorkInternal(ID3D11Device* device,
                               DXGI_FORMAT format,
                               UINT bindflags)
{
  // CreateTexture2D is known to crash on lower feature levels, see bugs
  ...
  deviceContext->UpdateSubresource(
      texture, 0, nullptr, color, stride, stride * texture_size);
  ...
  if (FAILED(device->OpenSharedResource(shareHandle,
  ...
  if (FAILED(device->CreateShaderResourceView(
          sharedTexture, NULL, getter_AddRefs(sharedView)))) {
...
{
  return DoesTextureSharingWorkInternal(
      device, DXGI_FORMAT_R8_UNORM, D3D11_BIND_SHADER_RESOURCE);
}
