...
gfxGDIFont::Initialize()
{
  ...
  if (mAdjustedSize == 0.0) {
    ...
    if (mStyle.sizeAdjust > 0.0 && mAdjustedSize > 0.0) {
      ...
      FillLogFont(
          logFont, mAdjustedSize, wantFakeItalic && !useCairoFakeItalic);
  ...
  if (mAdjustedSize > 0.0) {
    ...
    if (0 < GetOutlineTextMetrics(dc.GetDC(), sizeof(oMetrics), &oMetrics)) {
      ...
      GLYPHMETRICS gm;
      DWORD len = GetGlyphOutlineW(dc.GetDC(),
                                   char16_t('x'),
                                   GGO_METRICS,
                                   &gm,
                                   0,
    ...
    WORD glyph;
    SIZE size;
    DWORD ret = GetGlyphIndicesW(
        dc.GetDC(), L" ", 1, &glyph, GGI_MARK_NONEXISTING_GLYPHS);
    ...
    ret = GetGlyphIndicesW(
        dc.GetDC(), L"0", 1, &glyph, GGI_MARK_NONEXISTING_GLYPHS);
    if (ret != GDI_ERROR && glyph != 0xFFFF) {
...
void
gfxGDIFont::FillLogFont(LOGFONTW& aLogFont,
                        gfxFloat aSize,
...
gfxGDIFont::GetGlyph(uint32_t aUnicode, uint32_t aVarSelector)
{
  ...
  if (ret != S_OK) {
    ...
    if (ret != S_OK) {
      ...
      // (see bug 1105807).
      ret = GetGlyphIndicesW(
          dc.GetDC(), &ch, 1, &glyph, GGI_MARK_NONEXISTING_GLYPHS);
      if (ret == GDI_ERROR || glyph == 0xFFFF) {
        glyph = 0;
      }
    }
