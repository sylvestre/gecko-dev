                             ...
                             gfxShapedText* aShapedText)
{
  ...
  auto handleFeatureTag =
      [](const uint32_t& aTag, uint32_t& aValue, void* aUserArg) -> void {
  ...
  if (IsBuggyIndicScript(aScript)) {
    ...
    tempCTFont = CreateCTFontWithFeatures(
        ...
        aShapedText->DisableLigatures() ? GetIndicDisableLigaturesDescriptor()
    ...
    tempCTFont = CreateCTFontWithFeatures(
        ::CTFontGetSize(mCTFont),
        addSmallCaps ? GetSmallCapDisableLigDescriptor()
                                    ...
                                    CTRunRef aCTRun)
{
  ...
  if (!glyphToChar) {
    ...
    ::CTRunGetStringIndices(
        aCTRun, ::CFRangeMake(0, 0), glyphToCharArray.get());
    glyphToChar = glyphToCharArray.get();
  }

  double runWidth = ::CTRunGetTypographicBounds(
      aCTRun, ::CFRangeMake(0, 0), nullptr, nullptr, nullptr);
  ...
  while (glyphStart <
    ...
    if (glyphsInClump == 1 &&
      ...
      aShapedText->SetGlyphs(
          aOffset + baseCharIndex, textRunGlyph, detailedGlyphs.Elements());
    ...
    while (++baseCharIndex != endCharIndex && baseCharIndex < wordLength) {
      gfxShapedText::CompressedGlyph& shapedTextGlyph =
          charGlyphs[baseCharIndex];
      NS_ASSERTION(!shapedTextGlyph.IsSimpleGlyph(),
                   "overwriting a simple glyph");
      shapedTextGlyph.SetComplex(
          inOrder && shapedTextGlyph.IsClusterStart(), false, 0);
    }
    ...
    const std::pair<SInt16, SInt16> aFeatures[], size_t aCount)
{
  ...
  for (size_t i = 0; i < aCount; i++) {
    ...
    featureSettings[i] = ::CFDictionaryCreate(kCFAllocatorDefault,
                                              (const void**)keys,
                                              (const void**)values,
                                              ArrayLength(keys),
                                              &kCFTypeDictionaryKeyCallBacks,
