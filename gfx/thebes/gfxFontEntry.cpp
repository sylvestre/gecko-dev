...
bool
gfxFontEntry::TestCharacterMap(uint32_t aCh)
{
  if (!mCharacterMap) {
    ReadCMAP();
...
uint16_t
gfxFontEntry::GetUVSGlyph(uint32_t aCh, uint32_t aVS)
{
  InitializeUVSMap();
...
bool
gfxFontEntry::GetSVGGlyphExtents(DrawTarget* aDrawTarget,
                                 uint32_t aGlyphId,
                                 gfxRect* aResult)
{
  ...
  gfxMatrix svgToAppSpace(fontMatrix.xx,
                          fontMatrix.yx,
                          fontMatrix.xy,
                          fontMatrix.yy,
                          fontMatrix.x0,
...
void
gfxFontEntry::RenderSVGGlyph(gfxContext* aContext,
                             uint32_t aGlyphId,
...
/*static*/ const void*
gfxFontEntry::GrGetTable(const void* aAppFaceHandle,
                         unsigned int aName,
                         size_t* aLen)
{
...
gfxFontEntry::SupportsOpenTypeFeature(Script aScript, uint32_t aFeatureTag)
{
  ...
  if (hb_ot_layout_has_substitution(face)) {
    ...
    hb_tag_t scriptTags[4] = {
        HB_TAG_NONE, HB_TAG_NONE, HB_TAG_NONE, HB_TAG_NONE};
    ...
    *scriptTag = HB_OT_TAG_DEFAULT_SCRIPT;
    ...
    while (*scriptTag != HB_TAG_NONE) {
      ...
      if (hb_ot_layout_table_find_script(
  ...
  if (hb_ot_layout_has_substitution(face)) {
    ...
    *scriptTag = HB_OT_TAG_DEFAULT_SCRIPT;
    ...
    hb_ot_layout_collect_lookups(
        face, kGSUB, scriptTags, nullptr, features, featurelookups);
    hb_codepoint_t index = -1;
    while (hb_set_next(featurelookups, &index)) {
      hb_ot_layout_lookup_collect_glyphs(
          face, kGSUB, index, nullptr, inputGlyphs, nullptr, nullptr);
    }
...
size_t
gfxFontEntry::FontTableHashEntry::SizeOfExcludingThis(
                                ...
                                bool aIgnoreSizeTolerance)
{
  ...
  FindAllFontsForStyle(
      aFontStyle, matched, aNeedsSyntheticBold, aIgnoreSizeTolerance);
  if (!matched.IsEmpty()) {
    return matched[0];
  }
  return nullptr;
...
void
gfxFontFamily::LocalizedName(nsAString& aLocalizedName)
{
...
gfxFontFamily::FindFontForChar(GlobalFontMatch* aMatchData)
{
  ...
  gfxFontEntry* fe = FindFontForStyle(
      aMatchData->mStyle ? *aMatchData->mStyle : gfxFontStyle(),
      needsBold,
      true);
    ...
    bool useFullName)
{
  ...
  ReadOtherFamilyNamesForFace(
      mName, nameData, dataLength, otherFamilyNames, useFullName);
