...
gfxTextRun::AllocateStorageForTextRun(size_t aSize, uint32_t aLength)
{
  ...
  memset(reinterpret_cast<char*>(storage) + aSize,
         0,
                                ...
                                PropertyProvider* aProvider) const
{
  ...
  for (i = aPartRange.start + 1;
       i < GetLength() && !charGlyphs[i].IsLigatureGroupStart();
       ++i) {
  }
  result.mRange.end = i;
...
static void
GetAdjustedSpacing(const gfxTextRun* aTextRun,
    ...
    nsTArray<PropertyProvider::Spacing>* aSpacing) const
{
  ...
  GetAdjustedSpacing(
      this, aSpacingRange, aProvider, aSpacing->Elements() + spacingOffset);
  memset(aSpacing->Elements() + aSpacingRange.end - aRange.start,
         0,
...
void
gfxTextRun::DrawGlyphs(gfxFont* aFont,
                       Range aRange,
                       gfxPoint* aPt,
                       PropertyProvider* aProvider,
                       Range aSpacingRange,
...
void
gfxTextRun::DrawPartialLigature(gfxFont* aFont,
                                Range aRange,
                                gfxPoint* aPt,
                                PropertyProvider* aProvider,
                                TextRunDrawParams& aParams,
                                gfx::ShapedTextFlags aOrientation) const
{
...

  {
    ...
    Rect clipRect = aParams.isVerticalRun
                        ...
                        : Rect(start / mAppUnitsPerDevUnit,
                               clipExtents.Y(),
                               (end - start) / mAppUnitsPerDevUnit,
                               clipExtents.Height());
...
struct MOZ_STACK_CLASS BufferAlphaColor
{
  ...
  void PushSolidColor(const gfxRect& aBounds,
                      const Color& aAlphaColor,
...
gfxTextRun::Draw(Range aRange, gfxPoint aPt, const DrawParams& aParams) const
{
  ...
  if (aParams.drawMode & DrawMode::GLYPH_FILL &&
    ...
    syntheticBoldBuffer.PushSolidColor(
        metrics.mBoundingBox, currentColor, GetAppUnitsPerDevUnit());
  }
  ...
  while (iter.NextRun()) {
    ...
    if (drawPartial) {
      DrawPartialLigature(font,
                          Range(start, ligatureRange.start),
                          &aPt,
                          aParams.provider,
                          params,
                              ...
                              PropertyProvider* aProvider) const
{
  ...
  while (iter.NextRun()) {
    ...
    font->DrawEmphasisMarks(
        this, &aPt, ligatureRange.start, ligatureRange.Length(), params);
...
void
gfxTextRun::AccumulatePartialLigatureMetrics(
    gfxFont* aFont,
    Range aRange,
    gfxFont::BoundingBoxType aBoundingBoxType,
    DrawTarget* aRefDrawTarget,
                        ...
                        PropertyProvider* aProvider) const
{
  ...
  while (iter.NextRun()) {
    ...
    AccumulatePartialLigatureMetrics(font,
                                     Range(start, ligatureRange.start),
                                     aBoundingBoxType,
                                     aRefDrawTarget,
                                     aProvider,
...
void
gfxTextRun::ClassifyAutoHyphenations(uint32_t aStart,
                                     Range aRange,
...
uint32_t
gfxTextRun::BreakAndMeasureText(uint32_t aStart,
                                uint32_t aMaxLength,
                                ...
                                gfxBreakPriority* aBreakPriority)
{
        ...
        if (hyphenBuffer.AppendElements(bufferRange.Length(), fallible)) {
          ...
          if (aProvider->GetHyphensOption() == StyleHyphens::Auto) {
            ...
            ClassifyAutoHyphenations(
                aStart, bufferRange, hyphenBuffer, &wordState);
  ...
  if (aMetrics) {
    ...
    *aMetrics = MeasureText(
        Range(aStart, fitEnd), aBoundingBoxType, aRefDrawTarget, aProvider);
    if (aTrimWhitespace || aWhitespaceCanHang) {
                            ...
                            PropertyProvider::Spacing* aSpacing) const
{
  ...
  if (aProvider && (mFlags & gfx::ShapedTextFlags::TEXT_ENABLE_SPACING)) {
    ...
    if (spacingBuffer.AppendElements(aRange.Length())) {
      GetAdjustedSpacing(
          this, ligatureRange, aProvider, spacingBuffer.Elements());
...
nsresult
gfxTextRun::AddGlyphRun(gfxFont* aFont,
                        uint8_t aMatchType,
...
gfxTextRun::CopyGlyphDataFrom(gfxTextRun* aSource, Range aRange, uint32_t aDest)
{
  ...
  while (iter.NextRun()) {
    ...
    nsresult rv = AddGlyphRun(font,
                              iter.GetGlyphRun()->mMatchType,
                              start - aRange.start + aDest,
                              false,
                              iter.GetGlyphRun()->mOrientation);
...
{
  ResetGlyphRuns();
  memset(reinterpret_cast<char*>(mCharacterGlyphs),
         0,
...
void
gfxTextRun::SetSpaceGlyph(gfxFont* aFont,
                          ...
                          gfx::ShapedTextFlags aOrientation)
{
  ...
  if (sw) {
    AddGlyphRun(
        aFont, gfxTextRange::kFontGroup, aCharIndex, false, aOrientation);
    CopyGlyphDataFrom(sw, aCharIndex);
  }
}

bool
gfxTextRun::SetSpaceGlyphIfSimple(gfxFont* aFont,
                                  uint32_t aCharIndex,
                                  char16_t aSpaceChar,
                                  gfx::ShapedTextFlags aOrientation)
{
...
gfxFontGroup::BuildFontList()
{
  ...
  if (mFamilyList.GetDefaultFontType() != eFamily_none &&
    ...
    pfl->AddGenericFonts(
        mFamilyList.GetDefaultFontType(), mStyle.language, fonts);
    if (mTextPerf) {
      mTextPerf->current.genericLookups++;
    }
  }
...
bool
gfxFontGroup::FamilyFace::EqualsUserFont(
...
gfxFontGroup::GetDefaultFont()
{
  ...
  if (defaultFamily) {
    gfxFontEntry* fe = defaultFamily->FindFontForStyle(mStyle, needsBold);
    if (fe) {
  ...
  if (!mDefaultFont) {
    ...
    for (uint32_t i = 0; i < numFonts; ++i) {
      gfxFontEntry* fe = familyList[i]->FindFontForStyle(mStyle, needsBold);
      if (fe) {
        mDefaultFont = fe->FindOrMakeFont(&mStyle, needsBold);
        if (mDefaultFont) {
          break;
...
{
  gfxFontGroup* fg = new gfxFontGroup(
      mFamilyList, aStyle, mTextPerf, mUserFontSet, mDevToCssSize);
  return fg;
}
                               ...
                               nsTextFrameUtils::Flags aFlags2)
{
  ...
  textRun->AddGlyphRun(
      GetFirstValidFont(), gfxTextRange::kFontGroup, 0, false, orientation);
  return textRun.forget();
}
...
already_AddRefed<gfxTextRun>
gfxFontGroup::MakeTextRun(const uint8_t* aString,
                          uint32_t aLength,
                          const Parameters* aParams,
                          gfx::ShapedTextFlags aFlags,
                          nsTextFrameUtils::Flags aFlags2,
...
already_AddRefed<gfxTextRun>
gfxFontGroup::MakeTextRun(const char16_t* aString,
                          uint32_t aLength,
                          const Parameters* aParams,
                          gfx::ShapedTextFlags aFlags,
                          nsTextFrameUtils::Flags aFlags2,
                          ...
                          gfxMissingFontRecorder* aMFR)
{
  ...
  do {
    ...
    if (sizeof(T) == sizeof(uint8_t) && !transformedString) {
                ...
                ("(%s) fontgroup: [%s] default: %s lang: %s script: %d "
                 ...
                 (mFamilyList.GetDefaultFontType() == eFamily_serif
                      ...
                      : (mFamilyList.GetDefaultFontType() == eFamily_sans_serif
                             ...
                             : "none")),
                 lang.get(),
                 static_cast<int>(Script::LATIN),
                 aLength,
                 uint32_t(mStyle.weight),
                 uint32_t(mStyle.stretch),
      ...
      InitScriptRun(
          aDrawTarget, aTextRun, aString, 0, aLength, Script::LATIN, aMFR);
    } else {
                            ...
                            gfxMissingFontRecorder* aMFR)
{
  ...
  for (uint32_t r = 0; r < numRanges; r++) {
    ...
    if (matchedFont && mStyle.noFallbackVariantFeatures) {
      ...
      aTextRun->AddGlyphRun(matchedFont,
                            range.matchType,
                            aOffset + runStart,
                            (matchedLength > 0),
                            range.orientation);
      ...
      if (mStyle.variantSubSuper != NS_FONT_VARIANT_POSITION_NORMAL &&
          (aTextRun->GetShapingState() ==
           ...
           !matchedFont->SupportsSubSuperscript(
               mStyle.variantSubSuper, aString, aLength, aRunScript))) {
        ...
        aTextRun->AddGlyphRun(subSuperFont,
                              range.matchType,
                              aOffset + runStart,
                              (matchedLength > 0),
                              range.orientation);
        ...
        if (!matchedFont->InitFakeSmallCapsRun(aDrawTarget,
                                               aTextRun,
                                               aString + runStart,
                                               aOffset + runStart,
                                               matchedLength,
    ...
    if (!matchedFont) {
      ...
      aTextRun->SetupClusterBoundaries(
          aOffset + runStart, aString + runStart, matchedLength);
    ...
    LazyReferenceDrawTargetGetter& aRefDrawTargetGetter)
{
  ...
  mCachedEllipsisTextRun = MakeTextRun(ellipsis.get(),
                                       ellipsis.Length(),
                                       &params,
                                       aFlags,
                                       nsTextFrameUtils::Flags(),
...
gfxFont*
gfxFontGroup::FindFallbackFaceForChar(gfxFontFamily* aFamily,
                                      uint32_t aCh,
                                      Script aRunScript)
{
                              ...
                              uint8_t* aMatchType)
{
  ...
  if (aCh == NARROW_NO_BREAK_SPACE) {
    ...
    if (!aPrevCh && aNextCh && aNextCh != NARROW_NO_BREAK_SPACE) {
      gfxFont* nextFont = FindFontForChar(
          aNextCh, 0, 0, aRunScript, aPrevMatchedFont, aMatchType);
  ...
  if (!isJoinControl && !wasJoinCauser && !isVarSelector) {
    ...
    if (firstFont) {
      ...
      if (mFonts[0].CheckForFallbackFaces()) {
        font = FindFallbackFaceForChar(mFonts[0].Family(), aCh, aRunScript);
        ...
        if (!fe->IsUpright() || fe->Weight() != NS_FONT_WEIGHT_NORMAL ||
  ...
  for (uint32_t i = nextIndex; i < fontListLength; i++) {
    ...
    if (ff.CheckForFallbackFaces()) {
      ...
      if (!fe->mIsUserFontContainer && !fe->IsUserFont() &&
          (!fe->IsUpright() || fe->Weight() != NS_FONT_WEIGHT_NORMAL ||
           fe->Stretch() != NS_FONT_STRETCH_NORMAL)) {
        font = FindFallbackFaceForChar(ff.Family(), aCh, aRunScript);
        if (font) {
          *aMatchType = gfxTextRange::kFontGroup;
...
template<typename T>
void
gfxFontGroup::ComputeRanges(nsTArray<gfxTextRange>& aRanges,
                            const T* aString,
                            uint32_t aLength,
                            Script aRunScript,
                            gfx::ShapedTextFlags aOrientation)
...
gfxFont*
gfxFontGroup::WhichSystemFontSupportsChar(uint32_t aCh,
                                          uint32_t aNextCh,
                                          Script aRunScript)
{
  gfxFontEntry* fe =
