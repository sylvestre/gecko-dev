...
// radial
gfxPattern::gfxPattern(gfxFloat cx0,
                       gfxFloat cy0,
                       gfxFloat radius0,
                       gfxFloat cx1,
                       gfxFloat cy1,
    ...
    : mExtend(ExtendMode::CLAMP)
{
  mGfxPattern.InitRadialGradientPattern(
      Point(cx0, cy0), Point(cx1, cy1), radius0, radius1, nullptr);
}
                       ...
                       const Matrix* aOriginalUserToDevice)
{
  ...
  if (!mStops && !mStopsList.IsEmpty()) {
    mStops = aTarget->CreateGradientStops(
        mStopsList.Elements(), mStopsList.Length(), mExtend);
  }
