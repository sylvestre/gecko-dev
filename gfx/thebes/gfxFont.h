...
struct gfxFontStyle
{
  ...
  PLDHashNumber Hash() const
  {
    return ((style + (systemFont << 7) + (weight << 8)) +
            uint32_t(size * 1000) + uint32_t(sizeAdjust * 1000)) ^
...
class gfxFontCacheExpirationTracker
    : public ExpirationTrackerImpl<gfxFont,
                                   3,
...
class gfxFontCache final : private gfxFontCacheExpirationTracker
{
 ...
 protected:
  ...
  struct MOZ_STACK_CLASS Key
  {
    ...
    Key(const gfxFontEntry* aFontEntry,
        const gfxFontStyle* aStyle,
        const gfxCharacterMap* aUnicodeRangeMap)
   ...
   public:
    ...
    static PLDHashNumber HashKey(const KeyTypePointer aKey)
    {
      return mozilla::HashGeneric(
          aKey->mStyle->Hash(), aKey->mFontEntry, aKey->mUnicodeRangeMap);
    }
    enum
    {
      ALLOW_MEMMOVE = true
...
class gfxFontShaper
{
 public:
  ...
  static void MergeFontFeatures(const gfxFontStyle* aStyle,
...
class gfxShapedText
{
 public:
  ...
  gfxShapedText(uint32_t aLength,
   ...
   public:
    ...
    CompressedGlyph& SetSimpleGlyph(uint32_t aAdvanceAppUnits, uint32_t aGlyph)
    {
      ...
      mValue = (mValue & (FLAGS_CAN_BREAK_BEFORE | FLAG_CHAR_IS_SPACE)) |
               FLAG_IS_SIMPLE_GLYPH | (aAdvanceAppUnits << ADVANCE_SHIFT) |
               aGlyph;
      return *this;
    }
    CompressedGlyph& SetComplex(bool aClusterStart,
                                bool aLigatureStart,
                                uint32_t aGlyphCount)
    {
      mValue = (mValue & (FLAGS_CAN_BREAK_BEFORE | FLAG_CHAR_IS_SPACE)) |
               FLAG_NOT_MISSING | CharTypeFlags() |
  ...
  void SetGlyphs(uint32_t aCharIndex,
  ...
  void EnsureComplexGlyph(uint32_t aIndex, CompressedGlyph& aGlyph)
  {
    ...
    if (aGlyph.IsSimpleGlyph()) {
      DetailedGlyph details = {
          aGlyph.GetSimpleGlyph(), (int32_t)aGlyph.GetSimpleAdvance(), 0, 0};
...
class gfxShapedWord final : public gfxShapedText
{
 public:
  ...
  static gfxShapedWord* Create(const uint8_t* aText,
                               uint32_t aLength,
                               Script aRunScript,
                               int32_t aAppUnitsPerDevUnit,
                               mozilla::gfx::ShapedTextFlags aFlags,
                               gfxFontShaper::RoundingFlags aRounding)
  {
    ...
    return new (storage) gfxShapedWord(
        aText, aLength, aRunScript, aAppUnitsPerDevUnit, aFlags, aRounding);
  }

  static gfxShapedWord* Create(const char16_t* aText,
                               uint32_t aLength,
                               Script aRunScript,
                               int32_t aAppUnitsPerDevUnit,
                               mozilla::gfx::ShapedTextFlags aFlags,
 ...
 private:
  ...
  gfxShapedWord(const uint8_t* aText,
                uint32_t aLength,
                Script aRunScript,
                int32_t aAppUnitsPerDevUnit,
                mozilla::gfx::ShapedTextFlags aFlags,
  ...
  gfxShapedWord(const char16_t* aText,
                uint32_t aLength,
                Script aRunScript,
                int32_t aAppUnitsPerDevUnit,
                mozilla::gfx::ShapedTextFlags aFlags,
...
class gfxFont
{
 ...
 public:
  ...
  /**
     ...
     */
  void Draw(const gfxTextRun* aTextRun,
            uint32_t aStart,
            uint32_t aEnd,
            gfxPoint* aPt,
  ...
  void SetupGlyphExtents(DrawTarget* aDrawTarget,
                         uint32_t aGlyphID,
                         bool aNeedTight,
 ...
 protected:
  ...
  struct CacheHashKey
  {
    ...
    CacheHashKey(const uint8_t* aText,
                 uint32_t aLength,
    ...
    CacheHashKey(const char16_t* aText,
                 uint32_t aLength,
                 uint32_t aStringHash,
                 Script aScriptCode,
                 int32_t aAppUnitsPerDevUnit,
