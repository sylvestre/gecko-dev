      ...
      mVariationFont(aFontEntry->HasVariations())
{
  ...
  bool needsOblique = mFontEntry != nullptr && mFontEntry->IsUpright() &&
                      ...
                      gfxShapedText* aShapedText)
{
  ...
  if (macFontEntry->RequiresAATLayout() && !aVertical) {
    ...
    if (mCoreTextShaper->ShapeText(aDrawTarget,
                                   aText,
                                   aOffset,
                                   aLength,
                                   aScript,
  ...
  return gfxFont::ShapeText(aDrawTarget,
                            aText,
                            aOffset,
                            aLength,
                            aScript,
...
{
  gfxFont::RunMetrics metrics = gfxFont::Measure(aTextRun,
                                                 aStart,
                                                 aEnd,
                                                 aBoundingBoxType,
                                                 aRefDrawTarget,
                         ...
                         gfxFloat aConvFactor)
{
  ...
  if (aCmap) {
    glyph = gfxFontUtils::MapCharToGlyph(
        ::CFDataGetBytePtr(aCmap), ::CFDataGetLength(aCmap), aUniChar);
  }
...
gfxMacFont::GetGlyphWidth(DrawTarget& aDrawTarget, uint16_t aGID)
{
  ...
  ::CTFontGetAdvancesForGlyphs(
      mCTFont, kCTFontDefaultOrientation, &aGID, &advance, 1);
...
gfxMacFont::GetScaledFont(DrawTarget* aTarget)
{
  if (!mAzureScaledFont) {
    ...
    mAzureScaledFont = Factory::CreateScaledFontWithCairo(
        nativeFont, GetUnscaledFont(), GetAdjustedSize(), mScaledFont);
  }
...
{
  if (aRunParams) {
    return Factory::CreateCGGlyphRenderingOptions(
        aRunParams->fontSmoothingBGColor);
  }
  return nullptr;
}
