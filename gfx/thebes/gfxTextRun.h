...
class gfxTextRun : public gfxShapedText
{
 ...
 public:
  ...
  void ClassifyAutoHyphenations(uint32_t aStart,
                                Range aRange,
  ...
  /**
     ...
     * spacing is provided.
     */
  uint32_t BreakAndMeasureText(uint32_t aStart,
                               uint32_t aMaxLength,
                               bool aLineBreakBefore,
                               gfxFloat aWidth,
                               PropertyProvider* aProvider,
  ...
  class MOZ_STACK_CLASS GlyphRunIterator
  {
   public:
    GlyphRunIterator(const gfxTextRun* aTextRun, Range aRange)
        : mTextRun(aTextRun), mStartOffset(aRange.start), mEndOffset(aRange.end)
    {
  ...
  /**
     ...
     * TextRun.
     */
  nsresult AddGlyphRun(gfxFont* aFont,
                       uint8_t aMatchType,
                       uint32_t aStartCharIndex,
                       bool aForceNewRun,
  ...
  void SetSpaceGlyph(gfxFont* aFont,
  ...
  bool SetSpaceGlyphIfSimple(gfxFont* aFont,
                             uint32_t aCharIndex,
                             char16_t aSpaceChar,
                             mozilla::gfx::ShapedTextFlags aOrientation);
 ...
 private:
  ...
  gfxFloat ComputePartialLigatureWidth(Range aPartRange,
                                       PropertyProvider* aProvider) const;
  void DrawPartialLigature(gfxFont* aFont,
                           Range aRange,
                           gfxPoint* aPt,
                           PropertyProvider* aProvider,
  ...
  // **** measurement helper ****
  void AccumulateMetricsForRun(gfxFont* aFont,
                               Range aRange,
                               gfxFont::BoundingBoxType aBoundingBoxType,
                               DrawTarget* aRefDrawTarget,
                               PropertyProvider* aProvider,
...
class gfxFontGroup final : public gfxTextRunFactory
{
 public:
  ...
  /**
     ...
     */
  template<typename T>
  already_AddRefed<gfxTextRun> MakeTextRun(const T* aString,
                                           uint32_t aLength,
                                           DrawTarget* aRefDrawTarget,
  ...
  enum
  {
    UNDERLINE_OFFSET_NOT_SET = INT16_MAX
  };
  gfxFloat GetUnderlineOffset();

  gfxFont* FindFontForChar(uint32_t ch,
                           uint32_t prevCh,
                           uint32_t aNextCh,
                           Script aRunScript,
 ...
 protected:
  ...
  gfxFont* WhichSystemFontSupportsChar(uint32_t aCh,
                                       uint32_t aNextCh,
                                       Script aRunScript);
  ...
  class FamilyFace
  {
   public:
    ...
    FamilyFace(gfxFontFamily* aFamily,
  ...
  uint64_t
      mCurrGeneration;  // track the current user font set generation, rebuild font list if needed
  ...
  bool
      mLastPrefFirstFont;  // is this the first font in the list of pref fonts for this lang group?
  ...
  gfxFont* FindFallbackFaceForChar(gfxFontFamily* aFamily,
                                   uint32_t aCh,
                                   Script aRunScript);
