...
class gfxCharacterMap : public gfxSparseBitSet
{
 public:
  ...
  explicit gfxCharacterMap(const gfxSparseBitSet& aOther)
      : gfxSparseBitSet(aOther), mHash(0), mBuildOnTheFly(false), mShared(false)
  {
  }
...
class gfxFontEntry
{
 public:
  ...
  bool GetSVGGlyphExtents(DrawTarget* aDrawTarget,
                          uint32_t aGlyphId,
                          gfxRect* aResult);
  void RenderSVGGlyph(gfxContext* aContext,
                      uint32_t aGlyphId,
 ...
 protected:
  ...
  virtual nsresult CopyFontTable(uint32_t aTableTag, nsTArray<uint8_t>& aBuffer)
  {
