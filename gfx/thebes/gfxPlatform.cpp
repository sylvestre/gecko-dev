...
void
CrashStatsLogForwarder::SetCircularBufferSize(uint32_t aCapacity)
{
...
void
CrashStatsLogForwarder::UpdateCrashReport()
{
...
void
CrashStatsLogForwarder::Log(const std::string& aString)
{
  MutexAutoLock lock(mMutex);
...
#endif
  InitBackendPrefs(
      canvasMask, BackendType::CAIRO, contentMask, BackendType::CAIRO);
...
void
RecordingPrefChanged(const char* aPrefName, void* aClosure)
{
  if (Preferences::GetBool("gfx.2d.recording", false)) {
    ...
    if (NS_SUCCEEDED(rv)) {
      ...
      fileName.AppendPrintf(
          "moz2drec_%i_%i.aer", XRE_GetProcessType(), getpid());
...
#error "No gfxPlatform implementation available"
  ...
  if (obs) {
    ...
    obs->AddObserver(
        gPlatform->mMemoryPressureObserver, "memory-pressure", false);
  }
...
gfxPlatform::IsDXInterop2Blocked()
{
  ...
  if (!NS_SUCCEEDED(gfxInfo->GetFeatureStatus(
          nsIGfxInfo::FEATURE_DX_INTEROP2, blockId, &status))) {
    return true;
  }
...
void
SourceBufferDestroy(void* srcSurfUD)
{
...
void
SourceSurfaceDestroyed(void* aData)
{
...
gfxPlatform::CreateDrawTargetForBackend(BackendType aBackend,
                                        const IntSize& aSize,
...
already_AddRefed<DrawTarget>
gfxPlatform::CreateOffscreenCanvasDrawTarget(const IntSize& aSize,
                                             SurfaceFormat aFormat)
{
...
void
gfxPlatform::InitBackendPrefs(uint32_t aCanvasBitmask,
...
void
gfxPlatform::TransformPixel(const Color& in,
                            Color& out,
                            qcms_transform* transform)
{
  ...
  /* we want the bytes in RGB order */
    ...
    qcms_transform_data(
        transform, (uint8_t*)&packed + 1, (uint8_t*)&packed + 1, 1);
...
gfxPlatform::GetCMSRGBTransform()
{
  if (!gCMSRGBTransform) {
    ...
    gCMSRGBTransform = qcms_transform_create(inProfile,
                                             QCMS_DATA_RGB_8,
                                             outProfile,
                                             QCMS_DATA_RGB_8,
...
gfxPlatform::GetCMSInverseRGBTransform()
{
  if (!gCMSInverseRGBTransform) {
    ...
    gCMSInverseRGBTransform = qcms_transform_create(inProfile,
                                                    QCMS_DATA_RGB_8,
                                                    outProfile,
                                                    QCMS_DATA_RGB_8,
                                                    QCMS_INTENT_PERCEPTUAL);
...
gfxPlatform::GetCMSRGBATransform()
{
  if (!gCMSRGBATransform) {
    ...
    gCMSRGBATransform = qcms_transform_create(inProfile,
                                              QCMS_DATA_RGBA_8,
                                              outProfile,
                                              QCMS_DATA_RGBA_8,
                                              QCMS_INTENT_PERCEPTUAL);
...
void
gfxPlatform::SetupClusterBoundaries(gfxTextRun* aTextRun,
...
void
VideoDecodingFailedChangedCallback(const char* aPref, void*)
{
...
gfxPlatform::InitGPUProcessPrefs()
{
  ...
  if (!BrowserTabsRemoteAutostart()) {
    ...
    gpuProc.SetDefaultFromPref(gfxPrefs::GetGPUProcessEnabledPrefName(),
                               true,
...
gfxPlatform::InitOMTPConfig()
{
  ...
  if (InSafeMode()) {
    ...
    omtp.ForceDisable(FeatureStatus::Blocked,
                      "OMTP does not yet support tiling",
...
void
gfxPlatform::GetCompositorBackends(
...
void
gfxPlatform::ImportContentDeviceData(
...
gfxPlatform::InitOpenGLConfig()
{
  ...
  if (!IsGfxInfoStatusOkay(
          nsIGfxInfo::FEATURE_OPENGL_LAYERS, &message, failureId)) {
    openGLFeature.Disable(FeatureStatus::Blacklisted, message.get(), failureId);
  }
}

bool
gfxPlatform::IsGfxInfoStatusOkay(int32_t aFeature,
