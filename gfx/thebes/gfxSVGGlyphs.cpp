...
gfxSVGGlyphs::FindOrCreateGlyphsDocument(uint32_t aGlyphId)
{
  ...
  IndexEntry* entry = (IndexEntry*)bsearch(&aGlyphId,
                                           mDocIndex->mEntries,
...
gfxSVGGlyphsDocument::SetupPresentation()
{
  ...
  rv = docLoaderFactory->CreateInstanceForDocument(
 ...
 */
void
gfxSVGGlyphs::RenderGlyph(gfxContext* aContext,
                          uint32_t aGlyphId,
...
bool
gfxSVGGlyphs::GetGlyphExtents(uint32_t aGlyphId,
                     ...
                     nsCOMPtr<nsIInputStream>& aResult)
{
  ...
  if (!NS_InputStreamIsBuffered(stream)) {
    rv = NS_NewBufferedInputStream(
        getter_AddRefs(aBufferedStream), stream.forget(), 4096);
    NS_ENSURE_SUCCESS(rv, rv);
...
gfxSVGGlyphsDocument::ParseDocument(const uint8_t* aBuffer, uint32_t aBufLen)
{
  ...
  rv = document->StartDocumentLoad("external-resource",
                                   channel,
                                   nullptr,  // aLoadGroup
                                   nullptr,  // aContainer
  ...
  if (NS_SUCCEEDED(rv) && NS_SUCCEEDED(status)) {
    rv = listener->OnDataAvailable(
        channel, nullptr /* aContext */, stream, 0, aBufLen);
    if (NS_FAILED(rv)) {
      channel->Cancel(rv);
    }
    channel->GetStatus(&status);
...
size_t
gfxSVGGlyphsDocument::SizeOfIncludingThis(
