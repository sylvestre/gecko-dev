...
class GfxD2DVramReporter final : public nsIMemoryReporter
{
 ...
 public:
  ...
  {
    MOZ_COLLECT_REPORT("gfx-d2d-vram-draw-target",
                       KIND_OTHER,
                       UNITS_BYTES,
                       Factory::GetD2DVRAMUsageDrawTarget(),
                       "Video memory used by D2D DrawTargets.");

    MOZ_COLLECT_REPORT("gfx-d2d-vram-source-surface",
                       KIND_OTHER,
                       UNITS_BYTES,
...
class GPUAdapterReporter final : public nsIMemoryReporter
{
 ...
 public:
                 ...
                 bool aAnonymize) override
  {
    ...
    MOZ_COLLECT_REPORT("gpu-committed",
                       KIND_OTHER,
                       UNITS_BYTES,
                       committedBytesUsed,
 ...
 public:
                            ...
                            bool aAnonymize) override
  {
    if (gfxWindowsPlatform::sD3D11SharedTextures > 0) {
      MOZ_COLLECT_REPORT("d3d11-shared-textures",
                         KIND_OTHER,
                         UNITS_BYTES,
    ...
    if (gfxWindowsPlatform::sD3D9SharedTextures > 0) {
      MOZ_COLLECT_REPORT("d3d9-shared-textures",
                         KIND_OTHER,
                         UNITS_BYTES,
...
UpdateANGLEConfig()
{
  if (!gfxConfig::IsEnabled(Feature::D3D11_COMPOSITING)) {
    gfxConfig::Disable(Feature::D3D11_HW_ANGLE,
...
gfxWindowsPlatform::CreatePlatformFontList()
{
  ...
  if (IsNotWin7PreRTM() && DWriteEnabled()) {
    ...
    DisableD2D(FeatureStatus::Failed,
...
void
gfxWindowsPlatform::DisableD2D(FeatureStatus aStatus,
                               const char* aMessage,
...
void
gfxWindowsPlatform::GetCommonFallbackFonts(uint32_t aCh,
                                           uint32_t aNextCh,
                                           Script aRunScript,
...
{
  return new gfxFontGroup(
      aFontFamilyList, aStyle, aTextPerf, aUserFontSet, aDevToCssSize);
}
...
{
  RedrawWindow(
      aWnd, nullptr, nullptr, RDW_INVALIDATE | RDW_INTERNALPAINT | RDW_FRAME);
  return TRUE;
}
...
gfxWindowsPlatform::SchedulePaintIfDeviceReset()
{
  ...
  if (XRE_IsParentProcess()) {
    ...
    ::EnumThreadWindows(
        GetCurrentThreadId(), InvalidateWindowForDeviceReset, 0);
  } else {
...
gfxWindowsPlatform::GetDLLVersion(char16ptr_t aDLLPath, nsAString& aVersion)
{
  ...
  if (!GetFileVersionInfoW(
          aDLLPath, 0, versInfoSize, LPBYTE(versionInfo.Elements()))) {
    return;
  }

  UINT len = 0;
  VS_FIXEDFILEINFO* fileInfo = nullptr;
  if (!VerQueryValue(LPBYTE(versionInfo.Elements()),
                     TEXT("\\"),
                     (LPVOID*)&fileInfo,
                     &len) ||
...
void
gfxWindowsPlatform::GetCleartypeParams(
    nsTArray<ClearTypeParameterInfo>& aParams)
{
  ...
  for (i = 0, rv = ERROR_SUCCESS; rv != ERROR_NO_MORE_ITEMS; i++) {
    ...
    rv = RegEnumKeyExW(
        hKey, i, displayName, &size, nullptr, nullptr, nullptr, nullptr);
    if (rv != ERROR_SUCCESS) {
      continue;
    }
    ...
    swprintf_s(subkeyName,
               ArrayLength(subkeyName),
               L"Software\\Microsoft\\Avalon.Graphics\\%s",
               displayName);

    // subkey for gamma, pixel structure
    subrv = RegOpenKeyExW(
        HKEY_LOCAL_MACHINE, subkeyName, 0, KEY_QUERY_VALUE, &subKey);

    if (subrv == ERROR_SUCCESS) {
      size = sizeof(value);
      subrv = RegQueryValueExW(
          subKey, L"GammaLevel", nullptr, &type, (LPBYTE)&value, &size);
      ...
      size = sizeof(value);
      subrv = RegQueryValueExW(
          subKey, L"PixelStructure", nullptr, &type, (LPBYTE)&value, &size);
      if (subrv == ERROR_SUCCESS && type == REG_DWORD) {
        foundData = true;
    ...
    subrv = RegOpenKeyExW(
        HKEY_CURRENT_USER, subkeyName, 0, KEY_QUERY_VALUE, &subKey);

    if (subrv == ERROR_SUCCESS) {
      size = sizeof(value);
      subrv = RegQueryValueExW(
          subKey, L"ClearTypeLevel", nullptr, &type, (LPBYTE)&value, &size);
      if (subrv == ERROR_SUCCESS && type == REG_DWORD) {
        foundData = true;
      ...
      size = sizeof(value);
      subrv = RegQueryValueExW(subKey,
                               L"EnhancedContrastLevel",
                               nullptr,
                               &type,
                               (LPBYTE)&value,
                               &size);
...
InitializeANGLEConfig()
{
  ...
  if (!gfxPlatform::IsGfxInfoStatusOkay(
          nsIGfxInfo::FEATURE_DIRECT3D_11_ANGLE, &message, failureId)) {
...
gfxWindowsPlatform::InitializeD3D11Config()
{
  ...
  nsCString failureId;
  if (!gfxPlatform::IsGfxInfoStatusOkay(
          nsIGfxInfo::FEATURE_DIRECT3D_11_LAYERS, &message, failureId)) {
...
gfxWindowsPlatform::InitializeAdvancedLayersConfig()
{
  ...
  if (!IsGfxInfoStatusOkay(
          nsIGfxInfo::FEATURE_ADVANCED_LAYERS, &message, failureId)) {
    ...
    al.Disable(FeatureStatus::Broken,
...
gfxWindowsPlatform::InitializeD2DConfig()
{
  ...
  d2d1.SetDefaultFromPref(gfxPrefs::GetDirect2DDisabledPrefName(),
                          false,
  ...
  nsCString failureId;
  if (!gfxPlatform::IsGfxInfoStatusOkay(
          nsIGfxInfo::FEATURE_DIRECT2D, &message, failureId)) {
...
void
gfxWindowsPlatform::GetAcceleratedCompositorBackends(
...
void
gfxWindowsPlatform::ImportGPUDeviceData(
...
void
gfxWindowsPlatform::ImportContentDeviceData(
