...
struct GradientCacheKey : public PLDHashEntryHdr
{
  ...
  GradientCacheKey(const nsTArray<GradientStop>& aStops,
                   ExtendMode aExtend,
...
struct GradientCacheData
{
  ...
  GradientCacheData(const GradientCacheData& aOther)
      : mStops(aOther.mStops), mKey(aOther.mKey)
  {
  }
...
GradientStops*
gfxGradientCache::GetGradientStops(const DrawTarget* aDT,
...
gfxGradientCache::GetOrCreateGradientStops(const DrawTarget* aDT,
                                           nsTArray<GradientStop>& aStops,
                                           ExtendMode aExtend)
{
  if (aDT->IsRecording()) {
    return aDT->CreateGradientStops(
        aStops.Elements(), aStops.Length(), aExtend);
  }
