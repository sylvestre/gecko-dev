...
ScrollingLayersHelper::ScrollingLayersHelper(
    ...
    bool aApzEnabled)
    : mBuilder(&aBuilder), mPushedClipAndScroll(false)
{
  ...
  if (!aApzEnabled) {
    ...
    DefineAndPushChain(aItem->GetClipChain(),
                       aBuilder,
                       aStackingContext,
                       auPerDevPixel,
                       aCache);
  ...
  if (aItem->GetClipChain()) {
    leafmostASR = ActiveScrolledRoot::PickDescendant(
        leafmostASR, aItem->GetClipChain()->mASR);
  }
  DefineAndPushScrollLayers(aItem,
                            leafmostASR,
                            aItem->GetClipChain(),
                            aBuilder,
                            auPerDevPixel,
  ...
  DefineAndPushChain(
      aItem->GetClipChain(), aBuilder, aStackingContext, auPerDevPixel, aCache);
...
void
ScrollingLayersHelper::DefineAndPushScrollLayers(
    nsDisplayItem* aItem,
    ...
    WebRenderCommandBuilder::ClipIdMap& aCache)
{
  ...
  DefineAndPushChain(
      asrClippedBy, aBuilder, aStackingContext, aAppUnitsPerDevPixel, aCache);
...
void
ScrollingLayersHelper::DefineAndPushChain(
    const DisplayItemClipChain* aChain,
    wr::DisplayListBuilder& aBuilder,
    const StackingContextHelper& aStackingContext,
    int32_t aAppUnitsPerDevPixel,
    WebRenderCommandBuilder::ClipIdMap& aCache)
{
  ...
  Maybe<wr::WrClipId> clipId =
      (it != aCache.end() ? Some(it->second) : Nothing());
  ...
  // iterator `it`.
  DefineAndPushChain(aChain->mParent,
                     aBuilder,
                     aStackingContext,
                     aAppUnitsPerDevPixel,
                     aCache);
  ...
  if (!clipId || aBuilder.HasExtraClip()) {
    ...
    aChain->mClip.ToComplexClipRegions(
        aAppUnitsPerDevPixel, aStackingContext, wrRoundedRects);
    clipId = Some(aBuilder.DefineClip(
...
bool
ScrollingLayersHelper::DefineAndPushScrollLayer(
    const FrameMetrics& aMetrics, const StackingContextHelper& aStackingContext)
{
  ...
  LayoutDeviceRect clipBounds = LayoutDeviceRect::FromUnknownRect(
      aMetrics.GetCompositionBounds().ToUnknownRect());
