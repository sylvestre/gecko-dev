...
bool
CrossProcessCompositorBridgeParent::DeallocPLayerTransactionParent(
    ...
    const uint64_t& aLayersId)
{
  ...
  state.mApzcTreeManagerParent =
      new APZCTreeManagerParent(aLayersId, state.mParent->GetAPZCTreeManager());

  return state.mApzcTreeManagerParent;
}
bool
CrossProcessCompositorBridgeParent::DeallocPAPZCTreeManagerParent(
    ...
    wr::IdNamespace* aIdNamespace)
{
  ...
  parent = new WebRenderBridgeParent(this,
                                     aPipelineId,
                                     nullptr,
                                     root->CompositorScheduler(),
                                     Move(api),
                                     Move(holder),
                                     Move(animStorage));
  parent->AddRef();  // IPDL reference
...
bool
CrossProcessCompositorBridgeParent::DeallocPWebRenderBridgeParent(
    ...
    bool aHitTestUpdate)
{
  ...
  state->mParent->NotifyShadowTreeTransaction(id,
                                              aInfo.isFirstPaint(),
                                              aInfo.focusTarget(),
  ...
  aLayerTree->SetPendingTransactionId(
      aInfo.id(), aInfo.transactionStart(), aInfo.fwdTime());
}

void
CrossProcessCompositorBridgeParent::DidComposite(uint64_t aId,
  ...
  if (LayerTransactionParent* layerTree = sIndirectLayerTrees[aId].mLayerTree) {
    ...
    if (transactionId) {
      Unused << SendDidComposite(
          aId, transactionId, aCompositeStart, aCompositeEnd);
    }
...
void
CrossProcessCompositorBridgeParent::ForceComposite(
    LayerTransactionParent* aLayerTree)
{
  uint64_t id = aLayerTree->GetId();
  MOZ_ASSERT(id != 0);
...
void
CrossProcessCompositorBridgeParent::NotifyClearCachedResources(
    LayerTransactionParent* aLayerTree)
{
  uint64_t id = aLayerTree->GetId();
  MOZ_ASSERT(id != 0);
...
void
CrossProcessCompositorBridgeParent::GetAPZTestData(const uint64_t& aLayersId,
...
PTextureParent*
CrossProcessCompositorBridgeParent::AllocPTextureParent(
...
void
CrossProcessCompositorBridgeParent::UpdatePaintTime(
...
void
CrossProcessCompositorBridgeParent::ObserveLayerUpdate(uint64_t aLayersId,
                                                       uint64_t aEpoch,
                                                       bool aActive)
{
  MOZ_ASSERT(aLayersId != 0);
