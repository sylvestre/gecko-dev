...
static void
BindMaskForProgram(ShaderProgramOGL* aProgram,
...
void
CompositorOGL::BindBackdrop(ShaderProgramOGL* aProgram,
                            GLuint aBackdrop,
                            GLenum aTexUnit)
{
  ...
  mGLContext->fTexParameteri(
      LOCAL_GL_TEXTURE_2D, LOCAL_GL_TEXTURE_MIN_FILTER, LOCAL_GL_LINEAR);
  mGLContext->fTexParameteri(
      LOCAL_GL_TEXTURE_2D, LOCAL_GL_TEXTURE_MAG_FILTER, LOCAL_GL_LINEAR);
...
CompositorOGL::Initialize(nsCString* const out_failureReason)
{
  ...
  mGLContext->fBlendFuncSeparate(LOCAL_GL_ONE,
  ...
  if (mGLContext->WorkAroundDriverBugs()) {
    ...
    */
    ...
    for (uint32_t i = 0; i < ArrayLength(textureTargets); i++) {
      ...
      mGLContext->fTexParameteri(
          target, LOCAL_GL_TEXTURE_MIN_FILTER, LOCAL_GL_NEAREST);
      mGLContext->fTexParameteri(
          target, LOCAL_GL_TEXTURE_MAG_FILTER, LOCAL_GL_NEAREST);
...
gfx::Rect
CompositorOGL::GetTextureCoordinates(gfx::Rect textureRect,
    ...
    const IntPoint& aSourcePoint)
{
  ...
  RefPtr<CompositingRenderTargetOGL> surface =
      new CompositingRenderTargetOGL(this, aRect.TopLeft(), tex, fbo);
  surface->Initialize(
      aRect.Size(), sourceRect.Size(), mFBOTextureTarget, INIT_MODE_NONE);
  return surface.forget();
}
...
CompositorOGL::ClearRect(const gfx::Rect& aRect)
{
  ...
  ScopedScissorRect autoScissorRect(
      mGLContext, aRect.x, y, aRect.Width(), aRect.Height());
                          ...
                          IntRect* aRenderBoundsOut)
{
  ...
  if (mUseExternalSurfaceSize) {
    ...
    rect = gfx::IntRect(aRenderBounds.x,
                        aRenderBounds.y,
                        aRenderBounds.Width(),
                        aRenderBounds.Height());
  }
...
GLuint
CompositorOGL::CreateTexture(const IntRect& aRect,
                             bool aCopyFromSource,
                             GLuint aSourceFrameBuffer,
                             IntSize* aAllocSize)
{
  ...
  if (aCopyFromSource) {
    ...
    if (isFormatCompatibleWithRGBA) {
      mGLContext->fCopyTexImage2D(mFBOTextureTarget,
                                  ...
                                  LOCAL_GL_RGBA,
                                  clampedRect.x,
                                  FlipY(clampedRect.y + clampedRect.Height()),
                                  clampedRect.Width(),
                                  clampedRect.Height(),
                                  0);
    } else {
      ...
      mGLContext->fReadPixels(clampedRect.x,
                              clampedRect.y,
                              clampedRect.Width(),
                              clampedRect.Height(),
                              LOCAL_GL_RGBA,
  ...
  mGLContext->fTexParameteri(
      mFBOTextureTarget, LOCAL_GL_TEXTURE_MIN_FILTER, LOCAL_GL_LINEAR);
  mGLContext->fTexParameteri(
      mFBOTextureTarget, LOCAL_GL_TEXTURE_MAG_FILTER, LOCAL_GL_LINEAR);
  mGLContext->fTexParameteri(
      mFBOTextureTarget, LOCAL_GL_TEXTURE_WRAP_S, LOCAL_GL_CLAMP_TO_EDGE);
  mGLContext->fTexParameteri(
      mFBOTextureTarget, LOCAL_GL_TEXTURE_WRAP_T, LOCAL_GL_CLAMP_TO_EDGE);
                            ...
                            const gfx::Rect& aVisibleRect)
{
  ...
  if (BlendOpIsMixBlendMode(blendMode)) {
    ...
    if (gl()->IsExtensionSupported(GLContext::NV_texture_barrier)) {
      ...
      gfx::IntRect rect = ComputeBackdropCopyRect(
          aRect, aClipRect, aTransform, &backdropTransform);
      mixBlendBackdrop =
  ...
  switch (aEffectChain.mPrimaryEffect->mType) {
    case EffectTypes::SOLID_COLOR: {
      ...
      if (maskType != MaskType::MaskNone) {
        BindMaskForProgram(
            program, sourceMask, LOCAL_GL_TEXTURE0, maskQuadTransform);
      }
      if (mixBlendBackdrop) {
    ...
    case EffectTypes::RGB: {
      ...
      if (maskType != MaskType::MaskNone) {
        BindMaskForProgram(
            program, sourceMask, LOCAL_GL_TEXTURE1, maskQuadTransform);
      }
      if (mixBlendBackdrop) {
    ...
    case EffectTypes::YCBCR: {
      ...
      if (maskType != MaskType::MaskNone) {
        BindMaskForProgram(
            program, sourceMask, LOCAL_GL_TEXTURE3, maskQuadTransform);
      }
      if (mixBlendBackdrop) {
    ...
    case EffectTypes::COMPONENT_ALPHA: {
      ...
      // Pass 1.
      gl()->fBlendFuncSeparate(LOCAL_GL_ZERO,
  ...
  if (didSetBlendMode) {
    gl()->fBlendFuncSeparate(LOCAL_GL_ONE,
                             LOCAL_GL_ONE_MINUS_SRC_ALPHA,
                             LOCAL_GL_ONE,
                             LOCAL_GL_ONE_MINUS_SRC_ALPHA);
  }
...
void
CompositorOGL::BindAndDrawGeometry(
                                                  ...
                                                  TextureSource* aTexture)
{
  ...
  Rect layerRects[4];
  Rect textureRects[4];
  size_t rects = DecomposeIntoNoRepeatRects(
      aRect, scaledTexCoordRect, &layerRects, &textureRects);

  BindAndDrawQuads(aProg, rects, layerRects, textureRects);
}

void
CompositorOGL::BindAndDrawGeometryWithTextureRect(
...
void
CompositorOGL::InitializeVAO(const GLuint aAttrib,
                             const GLint aComponents,
                             const GLsizei aStride,
                             const size_t aOffset)
{
  mGLContext->fVertexAttribPointer(aAttrib,
                                   aComponents,
                                   LOCAL_GL_FLOAT,
                                   LOCAL_GL_FALSE,
                                   aStride,
...
void
CompositorOGL::CopyToTarget(DrawTarget* aTarget,
...
PerUnitTexturePoolOGL::GetTexture(GLenum aTarget, GLenum aTextureUnit)
{
  ...
  if (!mTextures[index]) {
    ...
    mGL->fTexParameteri(
        aTarget, LOCAL_GL_TEXTURE_WRAP_S, LOCAL_GL_CLAMP_TO_EDGE);
    mGL->fTexParameteri(
        aTarget, LOCAL_GL_TEXTURE_WRAP_T, LOCAL_GL_CLAMP_TO_EDGE);
  }
