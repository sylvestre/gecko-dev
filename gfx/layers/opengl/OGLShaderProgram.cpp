...
void
ProgramProfileOGL::BuildMixBlender(const ShaderConfigOGL& aConfig,
                                ...
                                const char* aFragmentShaderString)
{
  ...
  for (Pair<nsCString, GLuint>& attribute : mProfile.mAttributes) {
    mGL->fBindAttribLocation(
        result, attribute.second(), attribute.first().get());
  }
  ...
  if (!success
    ...
    nsAutoCString log;
    log.SetCapacity(len);
    mGL->fGetProgramInfoLog(
        result, len, (GLint*)&len, (char*)log.BeginWriting());
    log.SetLength(len);
...
ShaderProgramOGL::SetBlurRadius(float aRX, float aRY)
{
  ...
  SetArrayUniform(KnownUniform::BlurGaussianKernel,
