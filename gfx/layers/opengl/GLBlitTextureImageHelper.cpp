...
GLBlitTextureImageHelper::GLBlitTextureImageHelper(CompositorOGL* aCompositor)
    : mCompositor(aCompositor), mBlitProgram(0), mBlitFramebuffer(0)
...
void
GLBlitTextureImageHelper::BlitTextureImage(TextureImage* aSrc,
                                           ...
                                           const gfx::IntRect& aDstRect)
{
  ...
  do {
    ...
    do {
      ...
      float dx1 = 2.0f * float(srcSubInDstRect.x + srcSubInDstRect.Width()) /
                      float(dstSize.width) -
                  1.0f;
      float dy1 = 2.0f * float(srcSubInDstRect.y + srcSubInDstRect.Height()) /
                      float(dstSize.height) -
                  1.0f;
      ScopedViewportRect autoViewportRect(
          gl, 0, 0, dstSize.width, dstSize.height);
      ...
      ScopedVertexAttribPointer autoAttrib0(gl,
                                            0,
                                            2,
                                            LOCAL_GL_FLOAT,
                                            LOCAL_GL_FALSE,
      ...
      ScopedVertexAttribPointer autoAttrib1(gl,
                                            1,
                                            2,
                                            LOCAL_GL_FLOAT,
                                            LOCAL_GL_FALSE,
...
GLBlitTextureImageHelper::UseBlitProgram()
{
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    if (!success) {
      ...
      gl->fGetShaderInfoLog(
          shaders[i], len, (GLint*)&len, (char*)log.BeginWriting());
      log.SetLength(len);
  ...
  if (!success) {
    ...
    gl->fGetProgramInfoLog(
        mBlitProgram, len, (GLint*)&len, (char*)log.BeginWriting());
    log.SetLength(len);
