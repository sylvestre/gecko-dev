...
template<typename T>
static void
DumpRect(T* aPacketRect, const Rect& aRect)
{
...
static void
DumpFilter(TexturePacket* aTexturePacket, const SamplingFilter aSamplingFilter)
{
    ...
    if (aImage) {
      ...
      if (compresseddata) {
        int ndatasize = LZ4::compress(
            (char*)aImage->GetData(), mDatasize, compresseddata.get());
        if (ndatasize > 0) {
          mDatasize = ndatasize;
...
{
  return std::find(sSentTextureIds.begin(),
                   sSentTextureIds.end(),
                   aTextureId) != sSentTextureIds.end();
}

void
SenderHelper::SendLayer(LayerComposite* aLayer, int aWidth, int aHeight)
{
...
GLuint
SenderHelper::GetTextureID(GLContext* aGLContext, TextureSourceOGL* aSource)
{
                                ...
                                UniquePtr<Packet> aPacket)
{
  ...
  RefPtr<DataSourceSurface> img =
      aGLContext->ReadTexImageHelper()->ReadTexImage(
          0, textureTarget, size, shaderConfig, aFlipY);
  gLayerScopeManager.GetSocketManager()->AppendDebugData(
      new DebugGLTextureData(aGLContext,
                             aLayerRef,
                             textureTarget,
                             texID,
                             img,
                              ...
                              int aHeight)
{
  ...
  switch (primaryEffect->mType) {
    ...
    case EffectTypes::SOLID_COLOR: {
      ...
      SendColor(
          aEffectChain.mLayerRef, solidColorEffect->mColor, aWidth, aHeight);
      break;
    }
...
// ----------------------------------------------
void
LayerScopeWebSocketManager::SocketHandler::OpenStream(
    nsISocketTransport* aTransport)
{
  MOZ_ASSERT(aTransport);

  mTransport = aTransport;
  mTransport->OpenOutputStream(
      nsITransport::OPEN_BLOCKING, 0, 0, getter_AddRefs(mOutputStream));
                                                         ...
                                                         uint32_t aSize)
{
  ...
  rv = mOutputStream->Write(
      reinterpret_cast<char*>(wsHeader), wsHeaderSize, &cnt);
  ...
  while (written < aSize) {
    uint32_t cnt;
    rv = mOutputStream->Write(
        reinterpret_cast<char*>(aPtr) + written, aSize - written, &cnt);
    if (NS_FAILED(rv)) return false;
...
void
LayerScopeWebSocketManager::SocketHandler::ReadInputStreamData(
...
bool
LayerScopeWebSocketManager::SocketHandler::WebSocketHandshake(
    nsTArray<nsCString>& aProtocolString)
{
  nsresult rv;
  ...
  while (written < size) {
    uint32_t cnt;
    rv = mOutputStream->Write(
        const_cast<char*>(response.get()) + written, size - written, &cnt);
    if (NS_FAILED(rv)) return false;
                    ...
                    int aHeight)
{
  ...
  gLayerScopeManager.GetSocketManager()->AppendDebugData(
      new DebugGLDrawData(draws.mOffsetX,
                          draws.mOffsetY,
                          draws.mMVMatrix,
                          draws.mRects,
                          draws.mLayerRects,
...
/*static*/
void
LayerScope::SendLayer(LayerComposite* aLayer, int aWidth, int aHeight)
{
  // Protect this public function
  if (!CheckSendable()) {
    return;
