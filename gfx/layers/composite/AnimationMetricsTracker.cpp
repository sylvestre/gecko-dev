...
void
AnimationMetricsTracker::UpdateAnimationInProgress(
    AnimationProcessTypes aActive,
    uint64_t aLayerArea,
...
void
AnimationMetricsTracker::UpdateApzAnimationInProgress(
...
void
AnimationMetricsTracker::UpdateAnimationThroughput(
    const char* aLabel,
    bool aInProgress,
    ...
    Telemetry::HistogramID aMaxDropsHistogram)
{
  if (aInProgress && !aAnimation.mStart) {
    ...
    AMT_LOG("Type %s ran for %fms (interval: %fms), %u frames (expected: %u)\n",
            aLabel,
            animationLength.ToMilliseconds(),
            vsyncIntervalMs,
            aAnimation.mFrameCount,
            expectedFrameCount);
