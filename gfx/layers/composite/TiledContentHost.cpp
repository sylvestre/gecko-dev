...
void
TiledLayerBufferComposite::SetTextureSourceProvider(
...
TiledContentHost::GenEffect(const gfx::SamplingFilter aSamplingFilter)
{
  ...
  return CreateTexturedEffect(
      tile.mTextureSource, nullptr, aSamplingFilter, true);
}
...
void
TiledContentHost::Detach(Layer* aLayer, AttachFlags aFlags /* = NO_FLAGS */)
{
...
bool
TiledContentHost::UseTiledLayerBuffer(
                                    ...
                                    ISurfaceAllocator* aAllocator)
{
  ...
  TilesPlacement newTiles(aTiles.firstTileX(),
                          aTiles.firstTileY(),
                          aTiles.retainedWidth(),
                          aTiles.retainedHeight());
  ...
  for (size_t i = 0; i < tileDescriptors.Length(); i++) {
    ...
    tile.mTextureHost->DeserializeReadLock(texturedDesc.sharedLock(),
                                           aAllocator);
                            ...
                            const Maybe<gfx::Polygon>& aGeometry)
{
  ...
  RenderLayerBuffer(
      ...
      aEffectChain,
      lowPrecisionOpacityReduction * aOpacity,
      aSamplingFilter,
      aClipRect,
      *renderRegion,
      aTransform,
  ...
  RenderLayerBuffer(mTiledBuffer,
                    aCompositor,
                    nullptr,
                    aEffectChain,
                    aOpacity,
                             ...
                             const Maybe<gfx::Polygon>& aGeometry)
{
  ...
  for (auto iter = aScreenRegion.RectIter(); !iter.Done(); iter.Next()) {
    ...
    Rect textureRect(rect.x - aTextureOffset.x,
                     rect.y - aTextureOffset.y,
                     rect.Width(),
                     rect.Height());
    ...
    aCompositor->DrawGeometry(graphicsRect,
                              aClipRect,
                              aEffectChain,
                              opacity,
                              aTransform,
  ...
  aCompositor->DrawDiagnostics(
      flags, aScreenRegion, aClipRect, aTransform, mFlashCounter);
}
                                    ...
                                    const Maybe<Polygon>& aGeometry)
{
  ...
  aTransform.PreScale(1 / (resolution * layerScale.width),
                      1 / (resolution * layerScale.height),
                      1);
  ...
  if (aBackgroundColor) {
    ...
    for (auto iter = backgroundRegion.RectIter(); !iter.Done(); iter.Next()) {
      const IntRect& rect = iter.Get();
      Rect graphicsRect(rect.x, rect.y, rect.Width(), rect.Height());
      aCompositor->DrawGeometry(
          graphicsRect, aClipRect, effect, 1.0, aTransform, aGeometry);
    }
  }
  ...
  for (size_t i = 0; i < aLayerBuffer.GetTileCount(); ++i) {
    ...
    RenderTile(tile,
               ...
               aClipRect,
               tileDrawRegion,
               tileOffset * resolution,
               aLayerBuffer.GetTileSize(),
               gfx::Rect(visibleRect.x,
                         visibleRect.y,
                         visibleRect.Width(),
                         visibleRect.Height()),
               aGeometry);
  ...
  aCompositor->DrawDiagnostics(
      DiagnosticFlags::CONTENT | componentAlphaDiagnostic,
      rect,
      aClipRect,
      aTransform,
      mFlashCounter);
