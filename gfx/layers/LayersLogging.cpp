...
{
  aStream << pfx;
  aStream << nsPrintfCString("rgba(%d, %d, %d, %f)",
                             uint8_t(c.r * 255.f),
                             uint8_t(c.g * 255.f),
                             uint8_t(c.b * 255.f),
                             c.a)
...
void
AppendToString(std::stringstream& aStream,
               const wr::LayoutRect& r,
               const char* pfx,
               const char* sfx)
{
  aStream << pfx;
  aStream << nsPrintfCString("(x=%f, y=%f, w=%f, h=%f)",
                             r.origin.x,
                             r.origin.y,
                             r.size.width,
                             r.size.height)
...
void
AppendToString(std::stringstream& aStream,
               const wr::StickySideConstraint& s,
               const char* pfx,
               const char* sfx)
{
  aStream << pfx;
  aStream
      << nsPrintfCString("(margin=%f max=%f)", s.margin, s.max_offset).get();
  aStream << sfx;
}
               ...
               const char* sfx)
{
  ...
  if (!e.mHorizontalPanRegion.IsEmpty()) {
    AppendToString(
        aStream, e.mHorizontalPanRegion, " HorizontalPanRegion=", "");
  }
               ...
               const char* sfx)
{
  ...
  if (m.HasMaskLayer()) {
    AppendToString(
        aStream, m.ScrollClip().GetMaskLayerIndex().value(), "] [mask=");
  }
  aStream << "] }" << sfx;
}
...
void
AppendToString(std::stringstream& aStream,
               const ScrollableLayerGuid& s,
               const char* pfx,
               const char* sfx)
{
  aStream << pfx
          << nsPrintfCString("{ l=%" PRIu64 ", p=%u, v=%" PRIu64 " }",
                             s.mLayersId,
                             s.mPresShellId,
                             s.mScrollId)
                 .get()
...
{
  aStream << pfx
          << nsPrintfCString("{ z=%d dt=%d min=%f max=%f }",
                             z.mAllowZoom,
                             z.mAllowDoubleTapZoom,
                             z.mMinZoom.scale,
                             z.mMaxZoom.scale)
                 .get()
          << sfx;
               ...
               const char* sfx)
{
  ...
  if (m.IsIdentity()) {
    ...
    aStream << nsPrintfCString("[ %g %g; %g %g; %g %g; ]",
                               m._11,
                               m._12,
                               m._21,
                               m._22,
