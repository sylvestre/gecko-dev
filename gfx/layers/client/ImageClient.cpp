...
TextureInfo
ImageClientSingle::GetTextureInfo() const
{
                                         ...
                                         KnowsCompositor* aForwarder)
{
  ...
  if (aImage->GetFormat() == ImageFormat::PLANAR_YCBCR) {
    ...
    texture = AndroidSurfaceTextureData::CreateTextureClient(
        typedImage->GetHandle(),
        size,
        typedImage->GetContinuous(),
    ...
    MOZ_ASSERT(surface);
    texture = TextureClient::CreateForDrawing(aForwarder,
                                              surface->GetFormat(),
                                              aImage->GetSize(),
...

    {
      ...
      dt->CopySurface(
          surface, IntRect(IntPoint(), surface->GetSize()), IntPoint());
    }
...
bool
ImageClientBridge::UpdateImage(ImageContainer* aContainer,
                               uint32_t aContentFlags)
{
