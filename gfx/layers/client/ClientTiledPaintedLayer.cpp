...
GetTransformToAncestorsParentLayer(Layer* aStart,
                                   const LayerMetricsWrapper& aAncestor)
{
...
void
ClientTiledPaintedLayer::GetAncestorLayers(
...
ClientTiledPaintedLayer::BeginPaint()
{
  ...
  GetAncestorLayers(
      &scrollAncestor, &displayPortAncestor, &hasTransformAnimation);
  ...
  TILING_LOG(
      ...
      "%d\n",
      this,
      scrollAncestor.GetLayer(),
      displayPortAncestor.GetLayer(),
  ...
  if (!mPaintData.mHasTransformAnimation &&
    ...
    Maybe<LayerRect> criticalDisplayPortTransformed =
        ApplyParentLayerToLayerTransform(
            transformDisplayPortToLayer, criticalDisplayPort, layerBounds);
...
bool
ClientTiledPaintedLayer::IsScrollingOnCompositor(
...
bool
ClientTiledPaintedLayer::RenderHighPrecision(
    ...
    void* aCallbackData)
{
  ...
  if (!aInvalidRegion.IsEmpty() && UseProgressiveDraw() &&
    ...
    bool updatedBuffer =
        mContentClient->GetTiledBuffer()->ProgressiveUpdate(GetValidRegion(),
                                                            aInvalidRegion,
                                                            oldValidRegion,
                                                            drawnRegion,
                                                            &mPaintData,
  ...
  mContentClient->GetTiledBuffer()->PaintThebes(GetValidRegion(),
                                                aInvalidRegion,
                                                aInvalidRegion,
                                                aCallback,
                                                aCallbackData);
...
bool
ClientTiledPaintedLayer::RenderLowPrecision(
    const nsIntRegion& aInvalidRegion,
    const nsIntRegion& aVisibleRegion,
    LayerManager::DrawPaintedLayerCallback aCallback,
    void* aCallbackData)
...
ClientTiledPaintedLayer::RenderLayer()
{
  ...
  bool updatedLowPrecision = RenderLowPrecision(
      lowPrecisionInvalidRegion, neededRegion, callback, data);
...
bool
ClientTiledPaintedLayer::IsOptimizedFor(
...
void
ClientTiledPaintedLayer::PrintInfo(std::stringstream& aStream,
                                   const char* aPrefix)
{
