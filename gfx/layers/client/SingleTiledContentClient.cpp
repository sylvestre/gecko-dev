...
/* static */ bool
SingleTiledContentClient::ClientSupportsLayerSize(const gfx::IntSize& aSize,
...
ClientSingleTiledLayerBuffer::GetTextureClient()
{
  ...
  return mCompositableClient.CreateTextureClientForDrawing(
      gfx::ImageFormatToSurfaceFormat(mFormat),
      mSize,
...
void
ClientSingleTiledLayerBuffer::PaintThebes(
    ...
    bool aIsProgressive)
{
  ...
  RefPtr<TextureClient> backBuffer = mTile.GetBackBuffer(mCompositableClient,
                                                         tileDirtyRegion,
                                                         content,
                                                         mode,
                                                         extraPainted,
  ...
  mTile.mUpdateRect =
      tileDirtyRegion.GetBounds().Union(extraPainted.GetBounds());
    ...
    if (!copyableRegion.IsEmpty()) {
      ...
      if (discardedFrontBufferOnWhite && backBufferOnWhite) {
        ...
        if (frontOnWhiteLock.Succeeded()) {
          for (auto iter = copyableRegion.RectIter(); !iter.Done();
               iter.Next()) {
            const gfx::IntRect rect =
                iter.Get() - discardedValidRegion.GetBounds().TopLeft();
            const gfx::IntPoint dest = iter.Get().TopLeft() - mTilingOrigin;

            discardedFrontBufferOnWhite->CopyToTextureClient(
                backBufferOnWhite, &rect, &dest);
          }
        }
      }
      ...
      TILING_LOG("TILING %p: Region copied from discarded frontbuffer %s\n",
                 &mPaintedLayer,
                 Stringify(copyableRegion).c_str());
