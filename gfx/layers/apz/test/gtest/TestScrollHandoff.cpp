...
class APZScrollHandoffTester : public APZCTreeManagerTester
{
 protected:
  ...
  void CreateScrollHandoffLayerTree1()
  {
    ...
    SetScrollableFrameMetrics(
        layers[1], FrameMetrics::START_SCROLL_ID + 1, CSSRect(0, 0, 100, 100));
  ...
  void CreateScrollHandoffLayerTree2()
  {
    ...
    SetScrollableFrameMetrics(layers[1],
                              FrameMetrics::START_SCROLL_ID + 2,
                              CSSRect(-100, -100, 200, 200));
    SetScrollableFrameMetrics(
        layers[2], FrameMetrics::START_SCROLL_ID + 1, CSSRect(0, 0, 100, 100));
    SetScrollHandoff(layers[1], root);
  ...
  void CreateScrollHandoffLayerTree3()
  {
    ...
    root = CreateLayerTree(
        layerTreeSyntax, layerVisibleRegion, nullptr, lm, layers);
    SetScrollableFrameMetrics(
        layers[0], FrameMetrics::START_SCROLL_ID, CSSRect(0, 0, 100, 100));
    SetScrollableFrameMetrics(
        layers[1], FrameMetrics::START_SCROLL_ID + 1, CSSRect(0, 0, 100, 100));
    SetScrollableFrameMetrics(
        layers[2], FrameMetrics::START_SCROLL_ID + 2, CSSRect(0, 0, 100, 100));
    SetScrollableFrameMetrics(
        layers[3], FrameMetrics::START_SCROLL_ID + 3, CSSRect(0, 50, 100, 100));
    SetScrollableFrameMetrics(
        layers[4], FrameMetrics::START_SCROLL_ID + 4, CSSRect(0, 50, 100, 100));
  ...
  void CreateScrollHandoffLayerTree4()
  {
    ...
    root = CreateLayerTree(
        layerTreeSyntax, layerVisibleRegion, nullptr, lm, layers);
    SetScrollableFrameMetrics(
        root, FrameMetrics::START_SCROLL_ID, CSSRect(0, 0, 200, 100));
    SetScrollableFrameMetrics(
  ...
  void CreateScrollgrabLayerTree(bool makeParentScrollable = true)
  {
    ...
    SetScrollableFrameMetrics(
        root, FrameMetrics::START_SCROLL_ID, CSSRect(0, 0, 100, parentHeight));
    SetScrollableFrameMetrics(
        layers[1], FrameMetrics::START_SCROLL_ID + 1, CSSRect(0, 0, 100, 200));
    SetScrollHandoff(layers[1], root);
    registration =
  ...
  void TestCrossApzcAxisLock()
  {
    ...
    RefPtr<TestAsyncPanZoomController> childApzc = ApzcOf(layers[1]);
    Pan(childApzc,
        ScreenIntPoint(10, 60),
        ScreenIntPoint(15, 90),
...
TEST_F(APZScrollHandoffTester, StuckInOverscroll_Bug1073250)
{
  ...
  MultiTouchInput secondFingerDown(
      MultiTouchInput::MULTITOUCH_START, 0, TimeStamp(), 0);
...
TEST_F(APZScrollHandoffTester, ImmediateHandoffDisallowed_Fling)
{
  ...
  ASSERT_NEAR(
      50, childApzc->GetFrameMetrics().GetScrollOffset().y, COORDINATE_EPSILON);
  EXPECT_EQ(0, parentApzc->GetFrameMetrics().GetScrollOffset().y);
