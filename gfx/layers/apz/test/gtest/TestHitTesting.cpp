 ...
 protected:
  ...
  void CreateHitTesting2LayerTree()
  {
    ...
    SetScrollableFrameMetrics(
        root, FrameMetrics::START_SCROLL_ID, CSSRect(0, 0, 200, 200));
    SetScrollableFrameMetrics(
        layers[1], FrameMetrics::START_SCROLL_ID + 1, CSSRect(0, 0, 80, 80));
    SetScrollableFrameMetrics(
        layers[3], FrameMetrics::START_SCROLL_ID + 2, CSSRect(0, 0, 80, 80));
  }
  ...
  void CreateComplexMultiLayerTree()
  {
    ...
    nsIntRegion layerVisibleRegion[] = {
        ...
        nsIntRegion(IntRect(
            ...
            0,
            100,
            200)),  // container(6) taking up the top half of parent container(5)
        nsIntRegion(
            IntRect(200,
                    0,
                    100,
        ...
        nsIntRegion(IntRect(
            200, 200, 100, 100)),  // thebes(8) in bottom-right (below (6))
        nsIntRegion(IntRect(
            200, 300, 100, 100)),  // thebes(9) in bottom-right (below (8))
    };
    root = CreateLayerTree(
        layerTreeSyntax, layerVisibleRegion, nullptr, lm, layers);
...
TEST_F(APZHitTestingTester, HitTesting3)
{
  ...
  root = CreateLayerTree(
      layerTreeSyntax, layerVisibleRegions, transforms, lm, layers);
  // No actual room to scroll
  SetScrollableFrameMetrics(
      root, FrameMetrics::START_SCROLL_ID, CSSRect(0, 0, 200, 200));
  SetScrollableFrameMetrics(
      layers[1], FrameMetrics::START_SCROLL_ID + 1, CSSRect(0, 0, 50, 50));
...
TEST_F(APZHitTestingTester, TestForceDisableApz)
{
  ...
  // flushed).
  swi = ScrollWheelInput(MillisecondsSinceStartup(mcc->Time()),
                         mcc->Time(),
                         0,
                         ScrollWheelInput::SCROLLMODE_INSTANT,
                         ScrollWheelInput::SCROLLDELTA_PIXEL,
...
TEST_F(APZHitTestingTester, Bug1148350)
{
  ...
  MockFunction<void(std::string checkPointName)> check;
  {
    InSequence s;
    EXPECT_CALL(*mcc,
                HandleTap(TapType::eSingleTap,
                          LayoutDevicePoint(100, 100),
                          0,
                          ApzcOf(layers[1])->GetGuid(),
                          _))
        .Times(1);
...
TEST_F(APZHitTestingTester, HitTestingRespectsScrollClip_Bug1257288)
{
  ...
  SetScrollableFrameMetrics(
      layers[1], FrameMetrics::START_SCROLL_ID, CSSRect(0, 0, 200, 200));
  ...
  ScrollMetadata rootMetadata =
      BuildScrollMetadata(FrameMetrics::START_SCROLL_ID,
                          CSSRect(0, 0, 200, 200),
                          ParentLayerRect(0, 0, 200, 200));
  ScrollMetadata subframeMetadata =
      BuildScrollMetadata(FrameMetrics::START_SCROLL_ID + 1,
                          CSSRect(0, 0, 200, 200),
