 ...
 protected:
                   ...
                   nsTArray<uint32_t>* aAllowedTouchBehaviors = nullptr)
  {
    ...
    if (mGestureBehavior == AsyncPanZoomController::USE_GESTURE_DETECTOR) {
      PinchWithTouchInputAndCheckStatus(apzc,
                                        ScreenIntPoint(250, 300),
                                        1.25,
                                        touchInputId,
                                        aShouldTriggerPinch,
      ...
      PinchWithPinchInputAndCheckStatus(
          apzc, ScreenIntPoint(250, 300), 1.25, aShouldTriggerPinch);
    }
...
TEST_F(APZCPinchTester, Panning_TwoFinger_ZoomDisabled)
{
  ...
  PinchWithPinchInput(
      apzc, ScreenIntPoint(250, 350), ScreenIntPoint(200, 300), 10, &statuses);
...
TEST_F(APZCPinchGestureDetectorTester, Pinch_APZZoom_Disabled)
{
  ...
  EXPECT_CALL(*mcc,
              NotifyPinchGesture(
                  PinchGestureInput::PINCHGESTURE_SCALE, apzc->GetGuid(), _, _))
      .Times(AtLeast(1));
  EXPECT_CALL(*mcc,
...
TEST_F(APZCPinchGestureDetectorTester, Pinch_NoSpan)
{
  ...
  focus.y -=
      35 +
      1;  // this is to get over the PINCH_START_THRESHOLD in GestureEventListener.cpp
