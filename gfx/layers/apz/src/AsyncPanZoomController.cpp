...
{
 public:
  ZoomAnimation(CSSPoint aStartOffset,
                CSSToParentLayerScale2D aStartZoom,
                CSSPoint aEndOffset,
                CSSToParentLayerScale2D aEndZoom)
...
class SmoothScrollAnimation : public AsyncPanZoomAnimation
{
 public:
  SmoothScrollAnimation(AsyncPanZoomController& aApzc,
      ...
      : mApzc(aApzc),
        mXAxisModel(aInitialPosition.x,
                    aDestination.x,
                    aInitialVelocity.x,
                    aSpringConstant,
                    aDampingRatio),
        mYAxisModel(aInitialPosition.y,
                    aDestination.y,
                    aInitialVelocity.y,
                    aSpringConstant,
                    aDampingRatio)
...
bool
AsyncPanZoomController::ArePointerEventsConsumable(TouchBlockState* aBlock,
...
nsEventStatus
AsyncPanZoomController::HandleDragEvent(const MouseInput& aEvent,
                                        ...
                                        CSSCoord aInitialThumbPos)
{
  ...
  if (int snapMultiplier = gfxPrefs::SliderSnapMultiplier()) {
    ...
    ScrollDirection otherDirection =
        GetPerpendicularDirection(aDragMetrics.mDirection);
  ...
  CSSCoord minScrollPosition = GetAxisStart(
      aDragMetrics.mDirection, mFrameMetrics.GetScrollableRect().TopLeft());
  CSSCoord maxScrollPosition =
      GetAxisStart(aDragMetrics.mDirection,
                   mFrameMetrics.GetScrollableRect().BottomRight()) -
      GetAxisLength(aDragMetrics.mDirection,
...
nsEventStatus
AsyncPanZoomController::HandleGestureEvent(const InputData& aEvent)
{
  APZThreadUtils::AssertOnControllerThread();
...
void
AsyncPanZoomController::HandleTouchVelocity(uint32_t aTimesampMs, float aSpeedY)
{
  mY.HandleTouchVelocity(aTimesampMs, aSpeedY);
}

void
AsyncPanZoomController::StartAutoscroll(const ScreenPoint& aPoint)
{
...
nsEventStatus
AsyncPanZoomController::OnTouchStart(const MultiTouchInput& aEvent)
{
...
nsEventStatus
AsyncPanZoomController::OnTouchMove(const MultiTouchInput& aEvent)
{
...
nsEventStatus
AsyncPanZoomController::OnTouchEnd(const MultiTouchInput& aEvent)
{
...
nsEventStatus
AsyncPanZoomController::OnTouchCancel(const MultiTouchInput& aEvent)
{
...
nsEventStatus
AsyncPanZoomController::OnScaleBegin(const PinchGestureInput& aEvent)
{
  ...
  if (!gfxPrefs::APZAllowZooming()) {
    if (RefPtr<GeckoContentController> controller =
      ...
      controller->NotifyPinchGesture(
          aEvent.mType, GetGuid(), 0, aEvent.modifiers);
    }
  }
...
nsEventStatus
AsyncPanZoomController::OnScale(const PinchGestureInput& aEvent)
{
...
nsEventStatus
AsyncPanZoomController::OnScaleEnd(const PinchGestureInput& aEvent)
{
...
AsyncPanZoomController::HandleEndOfPan()
{
  ...
  if (APZCTreeManager* treeManagerLocal = GetApzcTreeManager()) {
    FlingHandoffState handoffState{
        flingVelocity,
...
bool
AsyncPanZoomController::ConvertToGecko(const ScreenIntPoint& aPoint,
...
static void
ReportKeyboardScrollAction(const KeyboardScrollAction& aAction)
{
...
AsyncPanZoomController::OnKeyboard(const KeyboardInput& aEvent)
{
  ...
  if (mState != KEYBOARD_SCROLL) {
    ...
    StartAnimation(new KeyboardScrollAnimation(
        *this, initialPosition, aEvent.mAction.mType));
  }
...
nsEventStatus
AsyncPanZoomController::OnScrollWheel(const ScrollWheelInput& aEvent)
{
  ParentLayerPoint delta = GetScrollWheelDelta(aEvent);
  APZC_LOG(
      "%p got a scroll-wheel with delta %s\n", this, Stringify(delta).c_str());
  ...
  switch (aEvent.mScrollMode) {
    ...
    case ScrollWheelInput::SCROLLMODE_SMOOTH: {
      ...
      animation->UpdateDelta(
          aEvent.mTimeStamp, deltaInAppUnits, nsSize(velocity.x, velocity.y));
      break;
    }
  }
...
nsEventStatus
AsyncPanZoomController::OnPanMayBegin(const PanGestureInput& aEvent)
{
...
nsEventStatus
AsyncPanZoomController::OnPanCancelled(const PanGestureInput& aEvent)
{
...
nsEventStatus
AsyncPanZoomController::OnPanBegin(const PanGestureInput& aEvent)
{
...
nsEventStatus
AsyncPanZoomController::OnPan(const PanGestureInput& aEvent,
                              bool aFingersOnTouchpad)
{
  ...
  mX.UpdateWithTouchAtDevicePoint(
      aEvent.mLocalPanStartPoint.x, logicalPanDisplacement.x, aEvent.mTime);
  mY.UpdateWithTouchAtDevicePoint(
      aEvent.mLocalPanStartPoint.y, logicalPanDisplacement.y, aEvent.mTime);
...
nsEventStatus
AsyncPanZoomController::OnPanEnd(const PanGestureInput& aEvent)
{
...
nsEventStatus
AsyncPanZoomController::OnPanMomentumStart(const PanGestureInput& aEvent)
{
...
nsEventStatus
AsyncPanZoomController::OnPanMomentumEnd(const PanGestureInput& aEvent)
{
...
nsEventStatus
AsyncPanZoomController::OnLongPress(const TapGestureInput& aEvent)
{
  ...
  if (controller) {
    ...
    if (ConvertToGecko(aEvent.mPoint, &geckoScreenPoint)) {
      ...
      controller->HandleTap(TapType::eLongTap,
                            geckoScreenPoint,
                            aEvent.modifiers,
                            GetGuid(),
                            blockId);
...
nsEventStatus
AsyncPanZoomController::OnLongPressUp(const TapGestureInput& aEvent)
{
  APZC_LOG("%p got a long-tap-up in state %d\n", this, mState);
  return GenerateSingleTap(
      TapType::eLongTapUp, aEvent.mPoint, aEvent.modifiers);
}

nsEventStatus
AsyncPanZoomController::GenerateSingleTap(TapType aType,
  ...
  if (controller) {
    ...
    if (ConvertToGecko(aPoint, &geckoScreenPoint)) {
      ...
      RefPtr<Runnable> runnable = NewRunnableMethod<TapType,
                                                    ...
                                                    ScrollableLayerGuid,
                                                    uint64_t>(
          "layers::GeckoContentController::HandleTap",
          controller,
...
void
AsyncPanZoomController::OnTouchEndOrCancel()
{
  if (RefPtr<GeckoContentController> controller = GetGeckoContentController()) {
    MOZ_ASSERT(GetCurrentTouchBlock());
    controller->NotifyAPZStateChange(
        GetGuid(),
        APZStateChange::eEndTouch,
        GetCurrentTouchBlock()->SingleTapOccurred());
  }
}

nsEventStatus
AsyncPanZoomController::OnSingleTapUp(const TapGestureInput& aEvent)
{
  ...
  if (!(mZoomConstraints.mAllowDoubleTapZoom &&
    ...
    return GenerateSingleTap(
        TapType::eSingleTap, aEvent.mPoint, aEvent.modifiers);
  }
  return nsEventStatus_eIgnore;
}

nsEventStatus
AsyncPanZoomController::OnSingleTapConfirmed(const TapGestureInput& aEvent)
{
...
nsEventStatus
AsyncPanZoomController::OnDoubleTap(const TapGestureInput& aEvent)
{
  ...
  if (controller) {
    ...
    if (mZoomConstraints.mAllowDoubleTapZoom &&
      ...
      LayoutDevicePoint geckoScreenPoint;
      if (ConvertToGecko(aEvent.mPoint, &geckoScreenPoint)) {
        controller->HandleTap(TapType::eDoubleTap,
                              geckoScreenPoint,
                              aEvent.modifiers,
                              GetGuid(),
...
nsEventStatus
AsyncPanZoomController::OnSecondTap(const TapGestureInput& aEvent)
{
  APZC_LOG("%p got a second-tap in state %d\n", this, mState);
  return GenerateSingleTap(
      TapType::eSecondTap, aEvent.mPoint, aEvent.modifiers);
}

nsEventStatus
AsyncPanZoomController::OnCancelTap(const TapGestureInput& aEvent)
{
...
AsyncPanZoomController::GetTransformToThis() const
{
  if (APZCTreeManager* treeManagerLocal = GetApzcTreeManager()) {
...
bool
AsyncPanZoomController::Contains(const ScreenIntPoint& aPoint) const
{
...
ScreenCoord
AsyncPanZoomController::PanDistance() const
{
...
void
AsyncPanZoomController::SetVelocityVector(
...
void
AsyncPanZoomController::HandlePanningWithTouchAction(double aAngle)
{
...
void
AsyncPanZoomController::HandlePanning(double aAngle)
{
  RecursiveMutexAutoLock lock(mRecursiveMutex);
...
void
AsyncPanZoomController::HandlePanningUpdate(const ScreenPoint& aPanDistance)
{
...
void
AsyncPanZoomController::UpdateWithTouchAtDevicePoint(
  ...
  if (scrollThisApzc) {
    ...
    bool forceVerticalOverscroll =
        (aOverscrollHandoffState.mScrollSource == ScrollSource::Wheel &&
         !mScrollMetadata.AllowVerticalScrollWithWheel());
    bool yChanged = mY.AdjustDisplacement(displacement.y,
                                          adjustedDisplacement.y,
                                          overscroll.y,
                                          forceVerticalOverscroll);
    bool xChanged = mX.AdjustDisplacement(
        displacement.x, adjustedDisplacement.x, overscroll.x);
...
void
AsyncPanZoomController::OverscrollForPanning(ParentLayerPoint& aOverscroll,
...
void
AsyncPanZoomController::OverscrollBy(ParentLayerPoint& aOverscroll)
{
  ...
  mOverscrollEffect->ConsumeOverscroll(
      aOverscroll, shouldOverscrollX, shouldOverscrollY);
}
...
void
AsyncPanZoomController::AcceptFling(FlingHandoffState& aHandoffState)
{
  RecursiveMutexAutoLock lock(mRecursiveMutex);
...
bool
AsyncPanZoomController::AttemptFling(FlingHandoffState& aHandoffState)
{
    ...
    const RefPtr<const AsyncPanZoomController>& aScrolledApzc)
{
  ...
  if (treeManagerLocal) {
    FlingHandoffState handoffState{
        aVelocity, aOverscrollHandoffChain, true /* handoff */, aScrolledApzc};
    treeManagerLocal->DispatchFling(this, handoffState);
    if (!IsZero(handoffState.mVelocity) && IsPannable() &&
...
void
AsyncPanZoomController::SmoothScrollTo(const CSSPoint& aDestination)
{
  if (mState == SMOOTH_SCROLL && mAnimation) {
    ...
    StartAnimation(
        new SmoothScrollAnimation(*this,
                                  initialPosition,
                                  initialVelocity,
                                  destination,
    ...
    OverscrollHandoffState& aOverscrollHandoffState)
{
  ...
  treeManagerLocal->DispatchScroll(
      this, aStartPoint, aEndPoint, aOverscrollHandoffState);
}

void
AsyncPanZoomController::TrackTouch(const MultiTouchInput& aEvent)
{
...
ParentLayerPoint
AsyncPanZoomController::GetFirstTouchPoint(const MultiTouchInput& aEvent)
{
  return ((SingleTouchData&)aEvent.mTouches[0]).mLocalScreenPoint;
}

void
AsyncPanZoomController::StartAnimation(AsyncPanZoomAnimation* aAnimation)
{
  RecursiveMutexAutoLock lock(mRecursiveMutex);
...
void
AsyncPanZoomController::CancelAnimation(CancelAnimationFlags aFlags)
{
  RecursiveMutexAutoLock lock(mRecursiveMutex);
  APZC_LOG(
      "%p running CancelAnimation(0x%x) in state %d\n", this, aFlags, mState);
...
void
AsyncPanZoomController::ClearOverscroll()
{
  RecursiveMutexAutoLock lock(mRecursiveMutex);
  mX.ClearOverscroll();
  mY.ClearOverscroll();
...
void
AsyncPanZoomController::SetCompositorController(
...
void
AsyncPanZoomController::SetMetricsSharingController(
...
void
AsyncPanZoomController::AdjustScrollForSurfaceShift(const ScreenPoint& aShift)
{
  RecursiveMutexAutoLock lock(mRecursiveMutex);
  CSSPoint adjustment =
...
void
AsyncPanZoomController::ScrollBy(const CSSPoint& aOffset)
{
  mFrameMetrics.ScrollBy(aOffset);
}

void
AsyncPanZoomController::ScrollByAndClamp(const CSSPoint& aOffset)
{
...
void
AsyncPanZoomController::ScaleWithFocus(float aScale, const CSSPoint& aFocus)
{
    ...
    const FrameMetrics& aFrameMetrics, const ParentLayerPoint& aVelocity)
{
  ...
  CSSRect displayPort((compositionSize.width - displayPortSize.width) / 2.0f,
                      ...
                      displayPortSize.width,
                      displayPortSize.height);
...
void
AsyncPanZoomController::ScheduleComposite()
{
...
void
AsyncPanZoomController::ScheduleCompositeAndMaybeRepaint()
{
  ScheduleComposite();
  RequestContentRepaint();
}
...
void
AsyncPanZoomController::FlushRepaintForOverscrollHandoff()
{
  RecursiveMutexAutoLock lock(mRecursiveMutex);
  RequestContentRepaint();
  UpdateSharedCompositorFrameMetrics();
...
void
AsyncPanZoomController::FlushRepaintForNewInputBlock()
{
...
bool
AsyncPanZoomController::SnapBackIfOverscrolled()
{
  RecursiveMutexAutoLock lock(mRecursiveMutex);
...
bool
AsyncPanZoomController::IsFlingingFast() const
{
  RecursiveMutexAutoLock lock(mRecursiveMutex);
  if (mState == FLING &&
...
bool
AsyncPanZoomController::IsPannable() const
{
  RecursiveMutexAutoLock lock(mRecursiveMutex);
...
bool
AsyncPanZoomController::IsScrollInfoLayer() const
{
  RecursiveMutexAutoLock lock(mRecursiveMutex);
...
int32_t
AsyncPanZoomController::GetLastTouchIdentifier() const
{
  RefPtr<GestureEventListener> listener = GetGestureEventListener();
...
void
AsyncPanZoomController::RequestContentRepaint(bool aUserAction)
{
  ...
  {  // scope lock
    ...
    if (mCheckerboardEvent && mCheckerboardEvent->IsRecordingTrace()) {
      ...
      mCheckerboardEvent->UpdateRendertraceProperty(
          ...
          GetDisplayPortRect(aFrameMetrics),
          str);
    }
  }
...
AsyncPanZoomController::UpdateAnimation(
    const TimeStamp& aSampleTime, nsTArray<RefPtr<Runnable>>* aOutDeferredTasks)
{
  APZThreadUtils::AssertOnCompositorThread();
    ...
    AsyncTransformConsumer aMode) const
{
  ...
  return AsyncTransformComponentMatrix().PostTranslate(
      overscrollOffset.x, overscrollOffset.y, 0);
}

bool
AsyncPanZoomController::AdvanceAnimations(const TimeStamp& aSampleTime)
{
  APZThreadUtils::AssertOnCompositorThread();
...
CSSPoint
AsyncPanZoomController::GetCurrentAsyncScrollOffsetInCssPixels(
    AsyncTransformConsumer aMode) const
{
  RecursiveMutexAutoLock lock(mRecursiveMutex);
...
Matrix4x4
AsyncPanZoomController::GetTransformToLastDispatchedPaint() const
{
  RecursiveMutexAutoLock lock(mRecursiveMutex);
...
void
AsyncPanZoomController::UpdateCheckerboardEvent(
...
bool
AsyncPanZoomController::IsCurrentlyCheckerboarding() const
{
  RecursiveMutexAutoLock lock(mRecursiveMutex);
  ...
  APZC_LOG_FM(mFrameMetrics,
              "%p is currently checkerboarding (painted %s visble %s)",
              this,
              Stringify(painted).c_str(),
              Stringify(visible).c_str());
  return true;
}
...
void
AsyncPanZoomController::ZoomToRect(CSSRect aRect, const uint32_t aFlags)
{
  if (!aRect.IsFinite()) {
...

  {
    ...
    StartAnimation(new ZoomAnimation(mFrameMetrics.GetScrollOffset(),
                                     mFrameMetrics.GetZoom(),
...
{
  MultiTouchInput cancel(
      MultiTouchInput::MULTITOUCH_CANCEL, 0, TimeStamp::Now(), 0);
  RefPtr<GestureEventListener> listener = GetGestureEventListener();
  if (listener) {
...
void
AsyncPanZoomController::SetState(PanZoomState aNewState)
{
...
void
AsyncPanZoomController::DispatchStateChangeNotification(PanZoomState aOldState,
...
bool
AsyncPanZoomController::IsTransformingState(PanZoomState aState)
{
  return !(aState == NOTHING || aState == TOUCHING);
}

bool
AsyncPanZoomController::IsInPanningState() const
{
...
void
AsyncPanZoomController::UpdateZoomConstraints(
    const ZoomConstraints& aConstraints)
{
  APZC_LOG("%p updating zoom constraints to %d %d %f %f\n",
           this,
           aConstraints.mAllowZoom,
           aConstraints.mAllowDoubleTapZoom,
           aConstraints.mMinZoom.scale,
           aConstraints.mMaxZoom.scale);
...
void
AsyncPanZoomController::PostDelayedTask(already_AddRefed<Runnable> aTask,
                                        int aDelayMs)
{
  APZThreadUtils::AssertOnControllerThread();
...
bool
AsyncPanZoomController::Matches(const ScrollableLayerGuid& aGuid)
{
  return aGuid == GetGuid();
}

bool
AsyncPanZoomController::HasTreeManager(
...
void
AsyncPanZoomController::GetGuid(ScrollableLayerGuid* aGuidOut) const
{
  if (aGuidOut) {
    *aGuidOut = GetGuid();
  }
...
void
AsyncPanZoomController::UpdateSharedCompositorFrameMetrics()
{
  mRecursiveMutex.AssertCurrentThreadIn();
...
void
AsyncPanZoomController::ShareCompositorFrameMetrics()
{
  APZThreadUtils::AssertOnCompositorThread();
  ...
  if (!mSharedFrameMetricsBuffer && mMetricsSharingController &&
    ...
    if (frame) {
      ...
      if (!mMetricsSharingController->StartSharingMetrics(
              mem, handle, mLayersId, mAPZCId)) {
...
Maybe<CSSPoint>
AsyncPanZoomController::FindSnapPointNear(const CSSPoint& aDestination,
                                          nsIScrollableFrame::ScrollUnit aUnit)
{
  mRecursiveMutex.AssertCurrentThreadIn();
  APZC_LOG(
      "%p scroll snapping near %s\n", this, Stringify(aDestination).c_str());
  CSSRect scrollRange = mFrameMetrics.CalculateScrollRange();
...
void
AsyncPanZoomController::ScrollSnapNear(const CSSPoint& aDestination)
{
...
AsyncPanZoomController::ScrollSnapToDestination()
{
  ...
  if (!flingWillOverscroll) {
    APZC_LOG(
        ...
        velocity.x,
        velocity.y,
        (float)predictedDelta.x,
        (float)predictedDelta.y,
        (float)mFrameMetrics.GetScrollOffset().x,
        (float)mFrameMetrics.GetScrollOffset().y,
        (float)predictedDestination.x,
