 ...
 */
MOZ_ALWAYS_INLINE __m128i
avg_sse2_8x2(__m128i* a, __m128i* b, __m128i* c, __m128i* d)
{
...
#define shuffle_si128(arga, argb, imm) \
  _mm_castps_si128(_mm_shuffle_ps(     \
      _mm_castsi128_ps((arga)), _mm_castsi128_ps((argb)), (imm)));
...
void
ImageHalfScaler::HalfImage2D_SSE2(uint8_t* aSource,
                                  ...
                                  uint32_t aDestStride)
{
  ...
  for (int y = 0; y < aSourceSize.height; y += 2) {
    ...
    for (; x < aSourceSize.width; x += 2) {
      ...
      *unalignedStorage++ = Avg2x2(*(uint32_t*)upperRow,
                                   *((uint32_t*)upperRow + 1),
                                   *(uint32_t*)lowerRow,
                                   *((uint32_t*)lowerRow + 1));
    }
...
void
ImageHalfScaler::HalfImageVertical_SSE2(uint8_t* aSource,
                                        int32_t aSourceStride,
                                        const IntSize& aSourceSize,
                                        uint8_t* aDest,
                                        uint32_t aDestStride)
...
void
ImageHalfScaler::HalfImageHorizontal_SSE2(uint8_t* aSource,
                                          int32_t aSourceStride,
                                          const IntSize& aSourceSize,
                                          uint8_t* aDest,
                                          uint32_t aDestStride)
