...
MOZ_ALWAYS_INLINE
uint16x4_t
Divide(uint32x4_t aValues, uint32x2_t aDivisor)
{
...
MOZ_ALWAYS_INLINE
uint16x4_t
BlurFourPixels(const uint32x4_t& aTopLeft,
...
GenerateIntegralImage_NEON(int32_t aLeftInflation,
                           int32_t aRightInflation,
  ...
  if (aBottomInflation) {
    ...
    for (int y = aSize.height + aTopInflation; y < integralImageSize.height;
      ...
      for (int x = 0; x < integralImageSize.width; x += 4) {
        vst1q_u32(
            intRow + x,
            vaddq_u32(vld1q_u32(intLastRow + x), vld1q_u32(intPrevRow + x)));
      }
    }
  }
}
                           ...
                           size_t aIntegralImageStride)
{
  ...
  GenerateIntegralImage_NEON(leftInflation,
                             aRightLobe,
                             aTopLobe,
                             aBottomLobe,
                             aIntegralImage,
