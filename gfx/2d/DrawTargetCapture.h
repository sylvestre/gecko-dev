...
{
 public:
  DrawTargetCaptureImpl(BackendType aBackend,
                        const IntSize& aSize,
  ...
  bool ContainsOnlyColoredGlyphs(RefPtr<ScaledFont>& aScaledFont,
                                 Color& aColor,
                                 std::vector<Glyph>& aGlyphs) override;
 ...
 private:
  ...
  T* AppendToCommandList()
  {
    ...
    mDrawCommandStorage.resize(mDrawCommandStorage.size() + sizeof(T) +
                               sizeof(uint32_t));
