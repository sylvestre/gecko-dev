...
struct TileInternal : public Tile
{
  ...
  explicit TileInternal(const Tile& aOther) : Tile(aOther), mClippedOut(false)
  {
  }
...
class DrawTargetTiled : public DrawTarget
{
 public:
  ...
  {
    return mTiles[0].mDrawTarget->CreateSourceSurfaceFromData(
        aData, aSize, aStride, aFormat);
  }
  ...
  {
    return mTiles[0].mDrawTarget->CreateGradientStops(
        aStops, aNumStops, aExtendMode);
  }
