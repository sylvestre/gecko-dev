...
TILED_COMMAND(Flush)
TILED_COMMAND4(
    DrawFilter, FilterNode*, const Rect&, const Point&, const DrawOptions&)
TILED_COMMAND1(ClearRect, const Rect&)
TILED_COMMAND4(
    MaskSurface, const Pattern&, SourceSurface*, Point, const DrawOptions&)
TILED_COMMAND5(FillGlyphs,
               ScaledFont*,
               const GlyphBuffer&,
               const Pattern&,
               const DrawOptions&,
                             ...
                             const IntPoint& aDestination)
{
  for (size_t i = 0; i < mTiles.size(); i++) {
    ...
    mTiles[i].mDrawTarget->CopySurface(
        aSurface, aSourceRect, aDestination - tileOrigin);
  }
}
...
void
DrawTargetTiled::DrawSurface(SourceSurface* aSurface,
                             const Rect& aDest,
                             const Rect& aSource,
  ...
  for (size_t i = 0; i < mTiles.size(); i++) {
    if (!mTiles[i].mClippedOut &&
        deviceRect.Intersects(Rect(mTiles[i].mTileOrigin.x,
                                   ...
                                   mTiles[i].mDrawTarget->GetSize().width,
                                   mTiles[i].mDrawTarget->GetSize().height))) {
      mTiles[i].mDrawTarget->DrawSurface(
          aSurface, aDest, aSource, aSurfaceOptions, aDrawOptions);
    }
  }
}

void
DrawTargetTiled::FillRect(const Rect& aRect,
...
void
DrawTargetTiled::Stroke(const Path* aPath,
                        const Pattern& aPattern,
  ...
  for (size_t i = 0; i < mTiles.size(); i++) {
    if (!mTiles[i].mClippedOut &&
        deviceRect.Intersects(Rect(mTiles[i].mTileOrigin.x,
                                   ...
                                   mTiles[i].mDrawTarget->GetSize().width,
                                   mTiles[i].mDrawTarget->GetSize().height))) {
      mTiles[i].mDrawTarget->Stroke(
          aPath, aPattern, aStrokeOptions, aDrawOptions);
    }
  }
}

void
DrawTargetTiled::StrokeRect(const Rect& aRect,
                            const Pattern& aPattern,
                            const StrokeOptions& aStrokeOptions,
                            const DrawOptions& aDrawOptions)
{
  ...
  for (size_t i = 0; i < mTiles.size(); i++) {
    ...
    if (outerRect.Intersects(tileRect) && !innerRect.Contains(tileRect)) {
      mTiles[i].mDrawTarget->StrokeRect(
          aRect, aPattern, aStrokeOptions, aDrawOptions);
    }
  }
}

void
DrawTargetTiled::StrokeLine(const Point& aStart,
                            const Point& aEnd,
                            const Pattern& aPattern,
                            const StrokeOptions& aStrokeOptions,
                            const DrawOptions& aDrawOptions)
  ...
  for (size_t i = 0; i < mTiles.size(); i++) {
    if (!mTiles[i].mClippedOut &&
        deviceRect.Intersects(Rect(mTiles[i].mTileOrigin.x,
                                   ...
                                   mTiles[i].mDrawTarget->GetSize().width,
                                   mTiles[i].mDrawTarget->GetSize().height))) {
      mTiles[i].mDrawTarget->StrokeLine(
...
void
DrawTargetTiled::Fill(const Path* aPath,
                      const Pattern& aPattern,
                      const DrawOptions& aDrawOptions)
{
  Rect deviceRect = aPath->GetBounds(mTransform);
...
void
DrawTargetTiled::PushLayer(bool aOpaque,
                           Float aOpacity,
                           SourceSurface* aMask,
                           ...
                           bool aCopyBackground)
{
  ...
  for (size_t i = 0; i < mTiles.size(); i++) {
    if (!mTiles[i].mClippedOut) {
      ...
      mTiles[i].mDrawTarget->PushLayer(
          aOpaque, aOpacity, aMask, aMaskTransform, bounds, aCopyBackground);
    }
  }
