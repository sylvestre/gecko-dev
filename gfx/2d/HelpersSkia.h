...
SkiaColorTypeToGfxFormat(SkColorType aColorType,
                         SkAlphaType aAlphaType = kPremul_SkAlphaType)
{
  switch (aColorType) {
...
{
  return SkImageInfo::Make(aSize.width,
                           aSize.height,
...
{
  retval.setAll(SkFloatToScalar(mat._11),
                SkFloatToScalar(mat._21),
                SkFloatToScalar(mat._31),
                SkFloatToScalar(mat._12),
                SkFloatToScalar(mat._22),
                SkFloatToScalar(mat._32),
                0,
                0,
                SK_Scalar1);
}
...
{
  aResult.setAll(SkFloatToScalar(aMatrix._11),
                 SkFloatToScalar(aMatrix._21),
                 SkFloatToScalar(aMatrix._41),
                 SkFloatToScalar(aMatrix._12),
                 SkFloatToScalar(aMatrix._22),
                 SkFloatToScalar(aMatrix._42),
                 SkFloatToScalar(aMatrix._14),
                 SkFloatToScalar(aMatrix._24),
...
ColorToSkColor(const Color& color, Float aAlpha)
{
  return SkColorSetARGB(ColorFloatToByte(color.a * aAlpha),
                        ColorFloatToByte(color.r),
...
{
  return SkRect::MakeXYWH(SkFloatToScalar(aRect.x),
                          SkFloatToScalar(aRect.y),
                          SkFloatToScalar(aRect.Width()),
                          SkFloatToScalar(aRect.Height()));
}

static inline SkRect
IntRectToSkRect(const IntRect& aRect)
{
  return SkRect::MakeXYWH(SkIntToScalar(aRect.x),
                          SkIntToScalar(aRect.y),
...
{
  return SkIRect::MakeXYWH(int32_t(aRect.x),
                           int32_t(aRect.y),
                           int32_t(aRect.Width()),
...
{
  return Rect(SkScalarToFloat(aRect.x()),
              SkScalarToFloat(aRect.y()),
              SkScalarToFloat(aRect.width()),
              SkScalarToFloat(aRect.height()));
}
...
static inline FillRule
GetFillRule(SkPath::FillType aFillType)
{
  switch (aFillType) {
 ...
 */
static inline bool
IsBackedByPixels(const SkCanvas* aCanvas)
{
