                                    ...
                                    float aOpacity)
{
  ...
  return MakeAndAddRef<SourceSurfaceD2D1>(
      luminanceOutput, mDC, SurfaceFormat::A8, mSize);
}
                            ...
                            const DrawOptions& aOptions)
{
  ...
  if (aSurfOptions.mSamplingBounds == SamplingBounds::BOUNDED) {
    ...
    samplingBounds = D2D1::RectF(0,
                                 0,
                                 Float(aSurface->GetSize().width),
  ...
  if (SUCCEEDED(hr) && bitmap &&
    ...
    mDC->DrawBitmap(bitmap,
                    D2DRect(aDest),
                    aOptions.mAlpha,
                    D2DFilter(aSurfOptions.mSamplingFilter),
                    D2DRect(aSource));
  } else {
                           ...
                           const DrawOptions& aOptions)
{
  ...
  mDC->DrawImage(
      node->OutputEffect(), D2DPoint(aDestPoint), D2DRect(aSourceRect));
...
DrawTargetD2D1::ClearRect(const Rect& aRect)
{
  ...
  if (mTransform.IsRectilinear() &&
    ...
    mDC->PushAxisAlignedClip(clipRect,
                             isPixelAligned
  ...
  mDC->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::White),
                             getter_AddRefs(brush));
  mDC->PushAxisAlignedClip(D2D1::RectF(addClipRect.x,
                                       addClipRect.y,
                                       addClipRect.XMost(),
                                       addClipRect.YMost()),
                           D2D1_ANTIALIAS_MODE_PER_PRIMITIVE);
                            ...
                            const DrawOptions& aOptions)
{
  ...
  if (!bitmap || FAILED(hr)) {
    ...
    mDC->PushLayer(D2D1::LayerParameters1(D2D1::InfiniteRect(),
                                          nullptr,
                                          D2D1_ANTIALIAS_MODE_PER_PRIMITIVE,
                                          D2D1::IdentityMatrix(),
                                          1.0f,
                            ...
                            const IntPoint& aDestination)
{
  ...
  Rect srcRect(Float(sourceRect.x),
               Float(sourceRect.y),
               Float(aSourceRect.Width()),
               Float(aSourceRect.Height()));

  Rect dstRect(Float(aDestination.x),
               Float(aDestination.y),
               Float(aSourceRect.Width()),
               Float(aSourceRect.Height()));
                           ...
                           const DrawOptions& aOptions)
{
  ...
  mDC->DrawRectangle(
      D2DRect(aRect), brush, aStrokeOptions.mLineWidth, strokeStyle);
                       ...
                       const DrawOptions& aOptions)
{
  ...
  mDC->DrawGeometry(
      d2dPath->mGeometry, brush, aStrokeOptions.mLineWidth, strokeStyle);
                           ...
                           const GlyphRenderingOptions*)
{
  ...
  if (forceClearType) {
    ...
    if (needsRepushedLayers) {
      mDC->GetGlyphRunWorldBounds(
          D2D1::Point2F(), &autoRun, DWRITE_MEASURING_MODE_NATURAL, &rect);
      ...
      if (!mTransform.IsRectilinear()) {
        ...
        mDC->SetTransform(D2D1::IdentityMatrix());
        mDC->GetGlyphRunWorldBounds(D2D1::Point2F(),
                                    &autoRun,
                     ...
                     const DrawOptions& aOptions)
{
  ...
  mDC->PushLayer(D2D1::LayerParameters(D2D1::InfiniteRect(),
                                       nullptr,
                                       D2D1_ANTIALIAS_MODE_PER_PRIMITIVE,
                                       D2D1::IdentityMatrix(),
                                       1.0f,
...
DrawTargetD2D1::PushClipRect(const Rect& aRect)
{
  ...
  if (CurrentLayer().mClipsArePushed) {
    mDC->PushAxisAlignedClip(clip.mBounds,
                             clip.mIsPixelAligned
                                 ? D2D1_ANTIALIAS_MODE_ALIASED
                                 : D2D1_ANTIALIAS_MODE_PER_PRIMITIVE);
  }
}
...
void
DrawTargetD2D1::PushLayer(bool aOpaque,
                          Float aOpacity,
                          SourceSurface* aMask,
                          ...
                          bool aCopyBackground)
{
  ...
  if (aMask) {
    ...
    if (image) {
      ...
      AddRectToSink(
          sink,
          D2D1::RectF(0, 0, aMask->GetSize().width, aMask->GetSize().height));
      sink->Close();
    } else {
                                            ...
                                            SurfaceFormat aFormat) const
{
  ...
  HRESULT hr =
      mDC->CreateBitmap(D2DIntSize(aSize),
                        aData,
                        aStride,
...
DrawTargetD2D1::CreateSimilarDrawTarget(const IntSize& aSize,
                                        SurfaceFormat aFormat) const
{
...
DrawTargetD2D1::CreatePathBuilder(FillRule aFillRule) const
{
  ...
  return MakeAndAddRef<PathBuilderD2D>(
      sink, path, aFillRule, BackendType::DIRECT2D1_1);
}

already_AddRefed<GradientStops>
DrawTargetD2D1::CreateGradientStops(GradientStop* rawStops,
                                    uint32_t aNumStops,
                                    ExtendMode aExtendMode) const
{
  ...
  HRESULT hr =
      mDC->CreateGradientStopCollection(stops,
                                        aNumStops,
                                        D2D1_GAMMA_2_2,
                                        D2DExtend(aExtendMode, Axis::BOTH),
...
void
DrawTargetD2D1::GetGlyphRasterizationMetrics(ScaledFont* aScaledFont,
...
DrawTargetD2D1::Init(const IntSize& aSize, SurfaceFormat aFormat)
{
  ...
  props.colorContext = nullptr;
  props.bitmapOptions = D2D1_BITMAP_OPTIONS_TARGET;
  hr = mDC->CreateBitmap(D2DIntSize(aSize),
                         nullptr,
                         0,
                         props,
...
DrawTargetD2D1::FinalizeDrawing(CompositionOp aOp, const Pattern& aPattern)
{
  ...
  mDC->DrawImage(radialGradientEffect,
                 D2D1_INTERPOLATION_MODE_NEAREST_NEIGHBOR,
                 D2DCompositionMode(aOp));
}
...
bool
DrawTargetD2D1::GetDeviceSpaceClipRect(D2D1_RECT_F& aClipRect,
                                       bool& aIsPixelAligned)
{
  ...
  for (auto iter = CurrentLayer().mPushedClips.begin();
       iter != CurrentLayer().mPushedClips.end();
       iter++) {
    if (iter->mGeometry) {
      return false;
    }
...
DrawTargetD2D1::GetImageForLayerContent(bool aShouldPreserveContent)
{
  ...
  if (!CurrentLayer().mCurrentList) {
    ...
    HRESULT hr =
        mDC->CreateBitmap(D2DIntSize(mSize),
    ...
    if (mComplexBlendsWithListInList >= sComplexBlendsWithListAllowedInList) {
      ...
      mDC->CreateBitmap(mBitmap->GetPixelSize(),
                        nullptr,
                        0,
                        &props,
                        getter_AddRefs(tmpBitmap));
  ...
  for (; iter != CurrentLayer().mPushedClips.end(); iter++) {
    ...
    if (!iter->mGeometry && iter->mIsPixelAligned) {
      mCurrentClipBounds.IntersectRect(
          mCurrentClipBounds,
          IntRect(int32_t(iter->mBounds.left),
                  int32_t(iter->mBounds.top),
    ...
    if (!pathGeom) {
      if (pathRectIsAxisAligned) {
        mCurrentClipBounds.IntersectRect(
            mCurrentClipBounds,
            IntRect(int32_t(pathRect.left),
                    int32_t(pathRect.top),
    ...
    if (iter->mGeometry) {
      ...
      pathGeom->CombineWithGeometry(rectGeom,
                                    D2D1_COMBINE_MODE_INTERSECT,
                                    D2D1::IdentityMatrix(),
                                    currentSink);
    }
...
DrawTargetD2D1::GetInverseClippedGeometry()
{
  ...
  rectGeom->CombineWithGeometry(
      geom, D2D1_COMBINE_MODE_EXCLUDE, D2D1::IdentityMatrix(), sink);
  sink->Close();
...
DrawTargetD2D1::CreateBrushForPattern(const Pattern& aPattern, Float aAlpha)
{
  ...
  if (aPattern.GetType() == PatternType::SURFACE) {
      ...
      if (SUCCEEDED(hr) && bitmap) {
        ...
        mDC->CreateBitmapBrush(
            ...
            D2D1::BitmapBrushProperties(
                xRepeat, yRepeat, D2DFilter(pat->mSamplingFilter)),
    ...
    if (pat->mSamplingRect.IsEmpty()) {
      ...
      samplingBounds = D2D1::RectF(
          0, 0, pat->mSamplingRect.Width(), pat->mSamplingRect.Height());
    }
...
already_AddRefed<ID2D1Image>
DrawTargetD2D1::GetImageForSurface(SourceSurface* aSurface,
                                   Matrix& aSourceTransform,
                                   ...
                                   bool aUserSpace)
{
  ...
  switch (aSurface->GetType()) {
    ...
    default: {
      ...
      return CreatePartialBitmapForSurface(dataSurf,
                                           transform,
                                           mSize,
                                           aExtendMode,
                                           aSourceTransform,
...
DrawTargetD2D1::OptimizeSourceSurface(SourceSurface* aSurface) const
{
  ...
  return MakeAndAddRef<SourceSurfaceD2D1>(
      bitmap.get(), mDC, data->GetFormat(), data->GetSize());
}

void
DrawTargetD2D1::PushD2DLayer(ID2D1DeviceContext* aDC,
                             ...
                             const D2D1_RECT_F& aMaxRect)
{
  ...
  mDC->PushLayer(
      D2D1::LayerParameters1(
          aMaxRect, aGeometry, antialias, aTransform, 1.0, nullptr, options),
      nullptr);
}
