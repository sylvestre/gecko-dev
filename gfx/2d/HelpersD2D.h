...
static inline D2D1_POINT_2F
D2DPoint(const Point& aPoint)
{
  return D2D1::Point2F(aPoint.x, aPoint.y);
}

static inline D2D1_SIZE_U
D2DIntSize(const IntSize& aSize)
{
  return D2D1::SizeU(aSize.width, aSize.height);
}

template<typename T>
static inline D2D1_RECT_F
D2DRect(const T& aRect)
{
  return D2D1::RectF(aRect.x, aRect.y, aRect.XMost(), aRect.YMost());
}

static inline D2D1_EXTEND_MODE
D2DExtend(ExtendMode aExtendMode, Axis aAxis)
{
...
static inline D2D1_BITMAP_INTERPOLATION_MODE
D2DFilter(const SamplingFilter aSamplingFilter)
{
...
static inline D2D1_INTERPOLATION_MODE
D2DInterpolationMode(const SamplingFilter aSamplingFilter)
{
  switch (aSamplingFilter) {
    case SamplingFilter::POINT:
...
static inline D2D1_MATRIX_5X4_F
D2DMatrix5x4(const Matrix5x4& aMatrix)
{
  return D2D1::Matrix5x4F(aMatrix._11,
                          aMatrix._12,
                          aMatrix._13,
                          aMatrix._14,
                          aMatrix._21,
...
static inline D2D1_VECTOR_3F
D2DVector3D(const Point3D& aPoint)
{
  return D2D1::Vector3F(aPoint.x, aPoint.y, aPoint.z);
}

static inline D2D1_ANTIALIAS_MODE
...
static inline D2D1_MATRIX_3X2_F
D2DMatrix(const Matrix& aTransform)
{
  return D2D1::Matrix3x2F(aTransform._11,
                          aTransform._12,
                          aTransform._21,
                          aTransform._22,
                          aTransform._31,
...
D2DColor(const Color& aColor)
{
  return D2D1::ColorF(aColor.r, aColor.g, aColor.b, aColor.a);
}

static inline IntSize
ToIntSize(const D2D1_SIZE_U& aSize)
{
  return IntSize(aSize.width, aSize.height);
}

static inline SurfaceFormat
ToPixelFormat(const D2D1_PIXEL_FORMAT& aFormat)
{
...
static inline Rect
ToRect(const D2D1_RECT_F& aRect)
{
  return Rect(aRect.left,
              aRect.top,
...
ToMatrix(const D2D1_MATRIX_3X2_F& aTransform)
{
  return Matrix(aTransform._11,
                aTransform._12,
                aTransform._21,
                aTransform._22,
                aTransform._31,
...
static inline Point
ToPoint(const D2D1_POINT_2F& aPoint)
{
...
static inline D2D1_PIXEL_FORMAT
...
static inline bool
D2DSupportsCompositeMode(CompositionOp aOp)
{
  switch (aOp) {
...
static inline D2D1_COMPOSITE_MODE
...
static inline bool
D2DSupportsPrimitiveBlendMode(CompositionOp aOp)
{
  switch (aOp) {
    case CompositionOp::OP_OVER:
      //  case CompositionOp::OP_SOURCE:
...
static inline D2D1_PRIMITIVE_BLEND
...
static inline bool
IsPatternSupportedByD2D(const Pattern& aPattern)
{
...
static inline void
DWriteGlyphRunFromGlyphs(const GlyphBuffer& aGlyphs,
...
static inline already_AddRefed<ID2D1Geometry>
GetTransformedGeometry(ID2D1Geometry* aGeometry,
...
IntersectGeometry(ID2D1Geometry* aGeometryA, ID2D1Geometry* aGeometryB)
{
  ...
  aGeometryA->CombineWithGeometry(
      aGeometryB, D2D1_COMBINE_MODE_INTERSECT, nullptr, sink);
  sink->Close();
...
CreateStrokeStyleForOptions(const StrokeOptions& aStrokeOptions)
{
  ...
  if (aStrokeOptions.mDashLength > 0 && aStrokeOptions.mDashPattern) {
    ...
    hr = D2DFactory()->CreateStrokeStyle(
        D2D1::StrokeStyleProperties(capStyle,
                                    capStyle,
                                    capStyle,
                                    joinStyle,
...
CreatePartialBitmapForSurface(DataSourceSurface* aSurface,
                              const Matrix& aDestinationTransform,
                              const IntSize& aDestinationSize,
                              ...
                              const IntRect* aSourceRect = nullptr)
{
  ...
  Rect rect(
      0, 0, Float(aDestinationSize.width), Float(aDestinationSize.height));
  ...
  if (uploadRect.Width() <= aRT->GetMaximumBitmapSize() &&
...

    {
      ...
      aRT->CreateBitmap(
          D2D1::SizeU(newSize.width, newSize.height),
          scaler.GetScaledData(),
          scaler.GetStride(),
          D2D1::BitmapProperties(D2DPixelFormat(aSurface->GetFormat())),
          getter_AddRefs(bitmap));
...
static inline void
AddRectToSink(ID2D1GeometrySink* aSink, const D2D1_RECT_F& aRect)
{
...
class DCCommandSink : public ID2D1CommandSink
{
 public:
  ...
  {
    mCtx->DrawGlyphRun(baselineOrigin,
                       glyphRun,
                       glyphRunDescription,
                       foregroundBrush,
                       measuringMode);
  ...
  STDMETHODIMP DrawLine(D2D1_POINT_2F point0,
  ...
  STDMETHODIMP DrawBitmap(_In_ ID2D1Bitmap* bitmap,
