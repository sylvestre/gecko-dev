...
extern GFX2D_API mozilla::LogModule*
GetGFX2DLog();
#endif
...
inline bool
MOZ2D_error_if_impl(bool aCondition,
                    const char* aExpr,
                    const char* aFile,
                    int32_t aLine)
{
...
#ifdef DEBUG
inline bool
MOZ2D_warn_if_impl(bool aCondition,
                   const char* aExpr,
                   const char* aFile,
                   int32_t aLine)
{
...
class TreeLog
{
 public:
  ...
  TreeLog& operator<<(const T& aObject)
  {
    ...
    if (mStartOfLine) {
      mLog << '[' << mPrefix << "] "
           << std::string(mDepth * INDENT_PER_LEVEL, ' ');
      mStartOfLine = false;
    }
    mLog << aObject;
