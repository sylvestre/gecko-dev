...
template<typename u8x16_t>
inline void
ExtractAlpha_SIMD(const IntSize& size,
                  uint8_t* sourceData,
...
inline void
BlendTwoComponentsOfFourPixels(i16x8_t source,
                               i16x8_t sourceAlpha,
                               i16x8_t dest,
...
inline u8x16_t
ShuffleAndPackComponents(i32x4_t bbbb1234,
                         i32x4_t gggg1234,
                         i32x4_t rrrr1234,
...
ApplyBlending_SIMD(DataSourceSurface* aInput1, DataSourceSurface* aInput2)
{
  ...
  for (int32_t y = 0; y < size.height; y++) {
    for (int32_t x = 0; x < size.width; x += 4) {
      ...
      BlendTwoComponentsOfFourPixels<i16x8_t, i32x4_t, mode>(s_bbbbgggg1234,
                                                             s_aaaaaaaa1234,
                                                             d_bbbbgggg1234,
                                                             d_aaaaaaaa1234,
                                                             blendedB,
                                                             blendedG);
      BlendTwoComponentsOfFourPixels<i16x8_t, i32x4_t, mode>(s_rrrraaaa1234,
                                                             s_aaaaaaaa1234,
                                                             d_rrrraaaa1234,
                                                             d_aaaaaaaa1234,
                                                             blendedR,
...
ApplyBlending_SIMD(DataSourceSurface* aInput1,
...
inline void
ApplyMorphologyHorizontal_SIMD(uint8_t* aSourceData,
                               ...
                               MorphologyOperator aOp)
{
  if (aOp == MORPHOLOGY_OPERATOR_ERODE) {
    ...
    ApplyMorphologyHorizontal_SIMD<MORPHOLOGY_OPERATOR_DILATE,
                                   i16x8_t,
                                   u8x16_t>(
        aSourceData, aSourceStride, aDestData, aDestStride, aDestRect, aRadius);
  }
...
template<MorphologyOperator op, typename i16x8_t, typename u8x16_t>
static void
ApplyMorphologyVertical_SIMD(uint8_t* aSourceData,
                             int32_t aSourceStride,
                             uint8_t* aDestData,
                             int32_t aDestStride,
                             const IntRect& aDestRect,
...
ApplyColorMatrix_SIMD(DataSourceSurface* aInput, const Matrix5x4& aMatrix)
{
  ...
  ptrdiff_t componentOffsets[4] = {B8G8R8A8_COMPONENT_BYTEOFFSET_R,
  ...
  i16x8_t row_bg_v = simd::FromI16<i16x8_t>(rows_bgra[0][0],
                                            rows_bgra[0][1],
                                            rows_bgra[0][2],
                                            rows_bgra[0][3],
                                            rows_bgra[0][4],
  ...
  i16x8_t row_ra_v = simd::FromI16<i16x8_t>(rows_bgra[1][0],
                                            rows_bgra[1][1],
                                            rows_bgra[1][2],
                                            rows_bgra[1][3],
                                            rows_bgra[1][4],
  ...
  for (int32_t y = 0; y < size.height; y++) {
    for (int32_t x = 0; x < size.width; x += 4) {
      ...
      static_assert(factor == 1 << 7,
                    ...
                    "different factor.");
      u8x16_t result_p1234 =
          simd::PackAndSaturate32To8(simd::ShiftRight32<7>(result_p1),
...
CompositeTwoPixels(u16x8_t source,
                   u16x8_t sourceAlpha,
                   u16x8_t dest,
...
template<typename i32x4_t, typename i16x8_t, typename u8x16_t>
static void
ApplyComposition_SIMD(DataSourceSurface* aSource,
...
template<typename u8x16_t>
static void
SeparateColorChannels_SIMD(const IntSize& size,
                           uint8_t* sourceData,
                           int32_t sourceStride,
                           uint8_t* channel0Data,
                           uint8_t* channel1Data,
                           uint8_t* channel2Data,
                           uint8_t* channel3Data,
...
template<typename u8x16_t>
static void
CombineColorChannels_SIMD(const IntSize& size,
...
DoPremultiplicationCalculation_SIMD(const IntSize& aSize,
                                    ...
                                    int32_t aSourceStride)
{
  const u8x16_t alphaMask = simd::From8<u8x16_t>(
...
static already_AddRefed<DataSourceSurface>
RenderTurbulence_SIMD(const IntSize& aSize,
                      const Point& aOffset,
...
ArithmeticCombineTwoPixels(i16x8_t in1,
                           i16x8_t in2,
                           const i16x8_t& k1And4,
                           const i16x8_t& k2And3)
{
...
template<typename i32x4_t, typename i16x8_t, typename u8x16_t>
static already_AddRefed<DataSourceSurface>
ApplyArithmeticCombine_SIMD(DataSourceSurface* aInput1,
                            DataSourceSurface* aInput2,
                            Float aK1,
                            Float aK2,
                            Float aK3,
