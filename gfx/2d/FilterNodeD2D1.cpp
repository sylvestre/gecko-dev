...
GetImageForSourceSurface(DrawTarget* aDT, SourceSurface* aSurface)
{
...
uint32_t
ConvertValue(FilterType aType, uint32_t aAttribute, uint32_t aValue)
{
  switch (aType) {
...
void
ConvertValue(FilterType aType, uint32_t aAttribute, IntSize& aValue)
{
  switch (aType) {
    case FilterType::MORPHOLOGY:
...
bool
GetD2D1PropsForIntSize(FilterType aType,
                       uint32_t aIndex,
                       UINT32* aPropWidth,
                       UINT32* aPropHeight)
{
  ...
  switch (mType) {
    ...
    default:
      mEffect->SetValue(input,
                        D2D1::Vector4F(aValue.r * aValue.a,
                                       aValue.g * aValue.a,
                                       aValue.b * aValue.a,
                                       aValue.a));
  }
...
void
FilterNodeD2D1::SetAttribute(uint32_t aIndex,
                             const Float* aValues,
                             uint32_t aSize)
{
  UINT32 input = GetD2D1PropForAttribute(mType, aIndex);
...
FilterNodeConvolveD2D1::UpdateSourceRect()
{
  mExtendInputEffect->SetValue(EXTENDINPUT_PROP_OUTPUT_RECT,
                               D2D1::Vector4F(Float(mSourceRect.x),
                                              Float(mSourceRect.y),
                                              Float(mSourceRect.XMost()),
                                              Float(mSourceRect.YMost())));
}
