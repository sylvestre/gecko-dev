...
void test_parent() { // expected-note {{parent function declared here}}
  test(); // expected-error {{functions marked as MOZ_CAN_RUN_SCRIPT can only be
          // called from functions also marked as MOZ_CAN_RUN_SCRIPT}}
}
...
struct RefCountedBase {
  ...
  virtual void method_test3() { test(); }
};

void test2_parent() { // expected-note {{parent function declared here}}
  test2(
      new RefCountedBase); // expected-error {{arguments must all be strong refs or parent parameters when calling a function marked as MOZ_CAN_RUN_SCRIPT (including the implicit object argument)}} \
...
MOZ_CAN_RUN_SCRIPT void test2_parent2() {
  test2(new RefCountedBase); // expected-error {{arguments must all be strong
                             // refs or parent parameters when calling a
                             // function marked as MOZ_CAN_RUN_SCRIPT (including
                             // the implicit object argument)}}
}

MOZ_CAN_RUN_SCRIPT void test2_parent3(RefCountedBase *param) { test2(param); }
...
MOZ_CAN_RUN_SCRIPT void test2_parent7() {
  ...
  t->method_test();  // expected-error {{arguments must all be strong refs or
                     // parent parameters when calling a function marked as
                     // MOZ_CAN_RUN_SCRIPT (including the implicit object
                     // argument)}}
  t->method_test2(); // expected-error {{arguments must all be strong refs or
                     // parent parameters when calling a function marked as
                     // MOZ_CAN_RUN_SCRIPT (including the implicit object
                     // argument)}}
}
