...
// Subclasses
struct S_NonMovable : NonMovable {}; // expected-note 51 {{'S_NonMovable' is a
                                     // non-memmove()able type because it
                                     // inherits from a non-memmove()able type
                                     // 'NonMovable'}}
struct S_Movable : Movable {};

// Members
struct W_NonMovable {
  NonMovable m; // expected-note 34 {{'W_NonMovable' is a non-memmove()able type
                // because member 'm' is a non-memmove()able type 'NonMovable'}}
};
struct W_Movable {
  Movable m;
};
...
struct WS_NonMovable {
  S_NonMovable m; // expected-note 34 {{'WS_NonMovable' is a non-memmove()able
                  // type because member 'm' is a non-memmove()able type
                  // 'S_NonMovable'}}
};
struct WS_Movable {
  S_Movable m;
...
struct SW_NonMovable : W_NonMovable {}; // expected-note 17 {{'SW_NonMovable' is
                                        // a non-memmove()able type because it
                                        // inherits from a non-memmove()able
                                        // type 'W_NonMovable'}}
struct SW_Movable : W_Movable {};

struct SWS_NonMovable
    : WS_NonMovable {}; // expected-note 17 {{'SWS_NonMovable' is a
                        // non-memmove()able type because it inherits from a
                        // non-memmove()able type 'WS_NonMovable'}}
struct SWS_Movable : WS_Movable {};

// Basic templated wrapper
template <class T> struct Template_Inline {
  T m; // expected-note-re 56 {{'Template_Inline<{{.*}}>' is a non-memmove()able
       // type because member 'm' is a non-memmove()able type '{{.*}}'}}
};

template <class T> struct Template_Ref { T *m; };

template <class T> struct Template_Unused {};

template <class T> struct MOZ_NON_MEMMOVABLE Template_NonMovable {};

/*
  These tests take the following form:
  DECLARATIONS => Declarations of the templates which are either marked with
  MOZ_NEEDS_MEMMOVABLE_TYPE or which instantiate a MOZ_NEEDS_MEMMOVABLE_TYPE
  through some mechanism. BAD N        => Instantiations of the wrapper template
  with each of the non-memmovable types. The prefix S_ means subclass, W_ means
  wrapped. Each of these rows should produce an error on the NeedyTemplate in
  question, and a note at the instantiation location of that template.
                  Unfortunately, on every case more complicated than bad1, the
  instantiation location is within another template. Thus, the notes are
  expected on the template in question which actually instantiates the
  MOZ_NEEDS_MEMMOVABLE_TYPE template. GOOD N       => Instantiations of the
  wrapper template with each of the memmovable types. This is meant as a sanity
  check to ensure that we don't reject valid instantiations of templates.


  Note 1: Each set uses it's own types to ensure that they don't re-use
  each-other's template specializations. If they did, then some of the error
  messages would not be emitted (as error messages are emitted for template
  specializations, rather than for variable declarations)

  Note 2: Every instance of NeedyTemplate contains a member of type T. This is
  to ensure that T is actually instantiated (if T is a template) by clang. If T
  isn't instantiated, then we can't actually tell if it is NON_MEMMOVABLE. (This
  is OK in practice, as you cannot memmove a type which you don't know the size
  of).

  Note 3: There are a set of tests for specializations of NeedyTemplate at the
  bottom. For each set of tests, these tests contribute two expected errors to
  the templates.
*/
...
template <class T> struct MOZ_NEEDS_MEMMOVABLE_TYPE NeedyTemplate1 {
  T m;
}; // expected-error-re 26 {{Cannot instantiate 'NeedyTemplate1<{{.*}}>' with
   // non-memmovable template argument '{{.*}}'}}

void bad1() {
  NeedyTemplate1<NonMovable> a1;    // expected-note-re {{instantiation of
                                    // 'NeedyTemplate1<{{.*}}>' requested here}}
  NeedyTemplate1<S_NonMovable> a2;  // expected-note-re {{instantiation of
                                    // 'NeedyTemplate1<{{.*}}>' requested here}}
  NeedyTemplate1<W_NonMovable> a3;  // expected-note-re {{instantiation of
                                    // 'NeedyTemplate1<{{.*}}>' requested here}}
  NeedyTemplate1<WS_NonMovable> a4; // expected-note-re {{instantiation of
                                    // 'NeedyTemplate1<{{.*}}>' requested here}}
  NeedyTemplate1<SW_NonMovable> a5; // expected-note-re {{instantiation of
                                    // 'NeedyTemplate1<{{.*}}>' requested here}}
  NeedyTemplate1<SWS_NonMovable> a6; // expected-note-re {{instantiation of
                                     // 'NeedyTemplate1<{{.*}}>' requested
                                     // here}}

  NeedyTemplate1<Template_Inline<NonMovable>> b1;    // expected-note-re
                                                     // {{instantiation of
                                                     // 'NeedyTemplate1<{{.*}}>'
                                                     // requested here}}
  NeedyTemplate1<Template_Inline<S_NonMovable>> b2;  // expected-note-re
                                                     // {{instantiation of
                                                     // 'NeedyTemplate1<{{.*}}>'
                                                     // requested here}}
  NeedyTemplate1<Template_Inline<W_NonMovable>> b3;  // expected-note-re
                                                     // {{instantiation of
                                                     // 'NeedyTemplate1<{{.*}}>'
                                                     // requested here}}
  NeedyTemplate1<Template_Inline<WS_NonMovable>> b4; // expected-note-re
                                                     // {{instantiation of
                                                     // 'NeedyTemplate1<{{.*}}>'
                                                     // requested here}}
  NeedyTemplate1<Template_Inline<SW_NonMovable>> b5; // expected-note-re
                                                     // {{instantiation of
                                                     // 'NeedyTemplate1<{{.*}}>'
                                                     // requested here}}
  NeedyTemplate1<Template_Inline<SWS_NonMovable>>
      b6; // expected-note-re {{instantiation of 'NeedyTemplate1<{{.*}}>'
          // requested here}}

  NeedyTemplate1<Template_NonMovable<NonMovable>>
      c1; // expected-note-re {{instantiation of 'NeedyTemplate1<{{.*}}>'
          // requested here}}
  NeedyTemplate1<Template_NonMovable<S_NonMovable>>
      c2; // expected-note-re {{instantiation of 'NeedyTemplate1<{{.*}}>'
          // requested here}}
  NeedyTemplate1<Template_NonMovable<W_NonMovable>>
      c3; // expected-note-re {{instantiation of 'NeedyTemplate1<{{.*}}>'
          // requested here}}
  NeedyTemplate1<Template_NonMovable<WS_NonMovable>>
      c4; // expected-note-re {{instantiation of 'NeedyTemplate1<{{.*}}>'
          // requested here}}
  NeedyTemplate1<Template_NonMovable<SW_NonMovable>>
      c5; // expected-note-re {{instantiation of 'NeedyTemplate1<{{.*}}>'
          // requested here}}
  NeedyTemplate1<Template_NonMovable<SWS_NonMovable>>
      c6; // expected-note-re {{instantiation of 'NeedyTemplate1<{{.*}}>'
          // requested here}}
  NeedyTemplate1<Template_NonMovable<Movable>> c7;   // expected-note-re
                                                     // {{instantiation of
                                                     // 'NeedyTemplate1<{{.*}}>'
                                                     // requested here}}
  NeedyTemplate1<Template_NonMovable<S_Movable>> c8; // expected-note-re
                                                     // {{instantiation of
                                                     // 'NeedyTemplate1<{{.*}}>'
                                                     // requested here}}
  NeedyTemplate1<Template_NonMovable<W_Movable>> c9; // expected-note-re
                                                     // {{instantiation of
                                                     // 'NeedyTemplate1<{{.*}}>'
                                                     // requested here}}
  NeedyTemplate1<Template_NonMovable<WS_Movable>>
      c10; // expected-note-re {{instantiation of 'NeedyTemplate1<{{.*}}>'
           // requested here}}
  NeedyTemplate1<Template_NonMovable<SW_Movable>>
      c11; // expected-note-re {{instantiation of 'NeedyTemplate1<{{.*}}>'
           // requested here}}
  NeedyTemplate1<Template_NonMovable<SWS_Movable>>
      c12; // expected-note-re {{instantiation of 'NeedyTemplate1<{{.*}}>'
           // requested here}}
}
...
template <class T> struct MOZ_NEEDS_MEMMOVABLE_TYPE NeedyTemplate2 {
  T m;
}; // expected-error-re 26 {{Cannot instantiate 'NeedyTemplate2<{{.*}}>' with
   // non-memmovable template argument '{{.*}}'}}
template <class T>
struct S_NeedyTemplate2
    : NeedyTemplate2<T> {}; // expected-note-re 26 {{instantiation of
                            // 'NeedyTemplate2<{{.*}}>' requested here}}
...
template <class T> struct MOZ_NEEDS_MEMMOVABLE_TYPE NeedyTemplate3 {
  T m;
}; // expected-error-re 26 {{Cannot instantiate 'NeedyTemplate3<{{.*}}>' with
   // non-memmovable template argument '{{.*}}'}}
template <class T> struct W_NeedyTemplate3 {
  NeedyTemplate3<T> m; // expected-note-re 26 {{instantiation of
                       // 'NeedyTemplate3<{{.*}}>' requested here}}
};
void bad3() {
...
template <class T> struct MOZ_NEEDS_MEMMOVABLE_TYPE NeedyTemplate4 {
  T m;
}; // expected-error-re 26 {{Cannot instantiate 'NeedyTemplate4<{{.*}}>' with
   // non-memmovable template argument '{{.*}}'}}
template <class T>
struct S_NeedyTemplate4
    : NeedyTemplate4<T> {}; // expected-note-re 26 {{instantiation of
                            // 'NeedyTemplate4<{{.*}}>' requested here}}
template <class T> struct WS_NeedyTemplate4 { S_NeedyTemplate4<T> m; };
void bad4() {
...
template <class T> struct MOZ_NEEDS_MEMMOVABLE_TYPE NeedyTemplate5 {
  T m;
}; // expected-error-re 26 {{Cannot instantiate 'NeedyTemplate5<{{.*}}>' with
   // non-memmovable template argument '{{.*}}'}}
template <class T> struct W_NeedyTemplate5 {
  NeedyTemplate5<T> m; // expected-note-re 26 {{instantiation of
                       // 'NeedyTemplate5<{{.*}}>' requested here}}
};
template <class T> struct SW_NeedyTemplate5 : W_NeedyTemplate5<T> {};
void bad5() {
...
//
// Note: This has an extra error, because it also includes a test with the
// default template argument.
//

template <class T> struct MOZ_NEEDS_MEMMOVABLE_TYPE NeedyTemplate6 {
  T m;
}; // expected-error-re 27 {{Cannot instantiate 'NeedyTemplate6<{{.*}}>' with
   // non-memmovable template argument '{{.*}}'}}
template <class T> struct W_NeedyTemplate6 {
  NeedyTemplate6<T> m; // expected-note-re 27 {{instantiation of
                       // 'NeedyTemplate6<{{.*}}>' requested here}}
};
template <class T> struct SW_NeedyTemplate6 : W_NeedyTemplate6<T> {};
// We create a different NonMovable type here, as NeedyTemplate6 will already be
// instantiated with NonMovable
struct MOZ_NON_MEMMOVABLE NonMovable2 {};
template <class T = NonMovable2> struct Defaulted_SW_NeedyTemplate6 {
...
template <class T> struct MOZ_NEEDS_MEMMOVABLE_TYPE NeedyTemplate7 {
  T m;
}; // expected-error-re 26 {{Cannot instantiate 'NeedyTemplate7<{{.*}}>' with
   // non-memmovable template argument '{{.*}}'}}
template <class T, class Q = NeedyTemplate7<T>>
struct Defaulted_Templated_NeedyTemplate7 {
  Q m;
}; // expected-note-re 26 {{instantiation of 'NeedyTemplate7<{{.*}}>' requested
   // here}}
void bad7() {
...
//
// 8 - Wrapped MOZ_NEEDS_MEMMOVABLE_TYPE instantiated as default template
// argument
//

template <class T> struct MOZ_NEEDS_MEMMOVABLE_TYPE NeedyTemplate8 {
  T m;
}; // expected-error-re 26 {{Cannot instantiate 'NeedyTemplate8<{{.*}}>' with
   // non-memmovable template argument '{{.*}}'}}
template <class T, class Q = NeedyTemplate8<T>>
struct Defaulted_Templated_NeedyTemplate8 {
  Q m;
}; // expected-note-re 26 {{instantiation of 'NeedyTemplate8<{{.*}}>' requested
   // here}}
template <class T> struct W_Defaulted_Templated_NeedyTemplate8 {
...
/*
  SpecializedNonMovable is a non-movable class which has an explicit
  specialization of NeedyTemplate for it. Instantiations of
  NeedyTemplateN<SpecializedNonMovable> should be legal as the explicit
  specialization isn't annotated with MOZ_NEEDS_MEMMOVABLE_TYPE.

  However, as it is MOZ_NON_MEMMOVABLE, derived classes and members shouldn't be
  able to be used to instantiate NeedyTemplate.
*/

struct MOZ_NON_MEMMOVABLE SpecializedNonMovable {};
struct S_SpecializedNonMovable
    : SpecializedNonMovable {}; // expected-note 8 {{'S_SpecializedNonMovable'
                                // is a non-memmove()able type because it
                                // inherits from a non-memmove()able type
                                // 'SpecializedNonMovable'}}

// Specialize all of the NeedyTemplates with SpecializedNonMovable.
template <> struct NeedyTemplate1<SpecializedNonMovable> {};
template <> struct NeedyTemplate2<SpecializedNonMovable> {};
template <> struct NeedyTemplate3<SpecializedNonMovable> {};
template <> struct NeedyTemplate4<SpecializedNonMovable> {};
template <> struct NeedyTemplate5<SpecializedNonMovable> {};
template <> struct NeedyTemplate6<SpecializedNonMovable> {};
template <> struct NeedyTemplate7<SpecializedNonMovable> {};
template <> struct NeedyTemplate8<SpecializedNonMovable> {};

void specialization() {
  /*
    SpecializedNonMovable has a specialization for every variant of
    NeedyTemplate, so these templates are valid, even though
    SpecializedNonMovable isn't memmovable
  */
  ...
  /*
    These entries contain an element which is SpecializedNonMovable, and are
    non-movable as there is no valid specialization, and their member is
    non-memmovable
  */
  NeedyTemplate1<Template_Inline<SpecializedNonMovable>>
      b1; // expected-note {{instantiation of
          // 'NeedyTemplate1<Template_Inline<SpecializedNonMovable> >' requested
          // here}}
  ...
  W_Defaulted_Templated_NeedyTemplate8<Template_Inline<SpecializedNonMovable>>
      b8;
  ...
  */
  NeedyTemplate1<S_SpecializedNonMovable>
      c1; // expected-note {{instantiation of
          // 'NeedyTemplate1<S_SpecializedNonMovable>' requested here}}
...
class MOZ_NEEDS_MEMMOVABLE_MEMBERS NeedsMemMovableMembers {
  Movable m1;
  NonMovable m2; // expected-error {{class 'NeedsMemMovableMembers' cannot have
                 // non-memmovable member 'm2' of type 'NonMovable'}}
  S_Movable sm1;
  S_NonMovable sm2; // expected-error {{class 'NeedsMemMovableMembers' cannot
                    // have non-memmovable member 'sm2' of type 'S_NonMovable'}}
  W_Movable wm1;
  W_NonMovable wm2; // expected-error {{class 'NeedsMemMovableMembers' cannot
                    // have non-memmovable member 'wm2' of type 'W_NonMovable'}}
  SW_Movable swm1;
  SW_NonMovable swm2; // expected-error {{class 'NeedsMemMovableMembers' cannot
                      // have non-memmovable member 'swm2' of type
                      // 'SW_NonMovable'}}
  WS_Movable wsm1;
  WS_NonMovable wsm2; // expected-error {{class 'NeedsMemMovableMembers' cannot
                      // have non-memmovable member 'wsm2' of type
                      // 'WS_NonMovable'}}
  SWS_Movable swsm1;
  SWS_NonMovable swsm2; // expected-error {{class 'NeedsMemMovableMembers'
                        // cannot have non-memmovable member 'swsm2' of type
                        // 'SWS_NonMovable'}}
};
