...
public:
  void fo() MOZ_REQUIRED_BASE_METHOD { // expected-error
                                       // {{MOZ_REQUIRED_BASE_METHOD can be used
...
public:
  void fo() MOZ_REQUIRED_BASE_METHOD { Base::fo(); }
};

class BaseOperator {
public:
  BaseOperator &operator++()
      MOZ_REQUIRED_BASE_METHOD { // expected-error {{MOZ_REQUIRED_BASE_METHOD
                                 // can be used only on virtual methods}}
    return *this;
  }
};
...
public:
  virtual DerivOperator &operator++() { return *this; }
};

class DerivPrimeOperator : public DerivOperator {
public:
  DerivPrimeOperator &operator++() { return *this; }
};
