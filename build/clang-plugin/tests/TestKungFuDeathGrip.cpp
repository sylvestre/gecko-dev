...
public:
  ...
  void f(nsCOMPtr<Type> ignoredArgument, Type *param) {
    ...
    nsCOMPtr<Type> kfdg_m1(p); // expected-error {{Unused "kungFuDeathGrip"
                               // 'nsCOMPtr<Type>' objects constructed from
                               // members are prohibited}} expected-note
                               // {{Please switch all accesses to this member to
                               // go through 'kfdg_m1', or explicitly pass
                               // 'kfdg_m1' to `mozilla::Unused`}}
    nsCOMPtr<Type> kfdg_m2 =
        p; // expected-error {{Unused "kungFuDeathGrip" 'nsCOMPtr<Type>' objects
           ...
           // explicitly pass 'kfdg_m2' to `mozilla::Unused`}}
    ...
    RefPtr<Type> kfdg_m5(p); // expected-error {{Unused "kungFuDeathGrip"
                             // 'RefPtr<Type>' objects constructed from members
                             // are prohibited}} expected-note {{Please switch
                             // all accesses to this member to go through
                             // 'kfdg_m5', or explicitly pass 'kfdg_m5' to
                             // `mozilla::Unused`}}
    RefPtr<Type> kfdg_m6 =
        p; // expected-error {{Unused "kungFuDeathGrip" 'RefPtr<Type>' objects
           // constructed from members are prohibited}} expected-note {{Please
           // switch all accesses to this member to go through 'kfdg_m6', or
           // explicitly pass 'kfdg_m6' to `mozilla::Unused`}}
...
void f(nsCOMPtr<Type> ignoredArgument, Type *param) {
  ...
  Type t;
  // Type *p = nullptr;
  nsCOMPtr<Type> kfdg_m1(t.p); // expected-error {{Unused "kungFuDeathGrip"
                               // 'nsCOMPtr<Type>' objects constructed from
                               // members are prohibited}} expected-note
                               // {{Please switch all accesses to this member to
                               // go through 'kfdg_m1', or explicitly pass
  ...
  RefPtr<Type> never_referenced2;
  RefPtr<Type> kfdg_m5(t.p); // expected-error {{Unused "kungFuDeathGrip"
                             // 'RefPtr<Type>' objects constructed from members
                             // are prohibited}} expected-note {{Please switch
                             // all accesses to this member to go through
                             // 'kfdg_m5', or explicitly pass 'kfdg_m5' to
