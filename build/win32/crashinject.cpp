...
main(int argc, char** argv)
{
  ...
  if (GetModuleFileNameW(
          nullptr, filename, sizeof(filename) / sizeof(wchar_t)) == 0)
    return 1;
  ...
  void* pLibRemote = VirtualAllocEx(
      targetProc, nullptr, sizeof(filename), MEM_COMMIT, PAGE_READWRITE);
  ...
  if (!WriteProcessMemory(
          targetProc, pLibRemote, (void*)filename, sizeof(filename), nullptr)) {
