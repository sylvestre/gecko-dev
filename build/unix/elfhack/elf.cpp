...
ElfSection*
Elf::getSection(int index)
{
  if ((index < -1) || (index >= ehdr->e_shnum))
    throw std::runtime_error("Section index out of bounds");
  if (index == -1)
    index =
        ehdr->e_shstrndx;  // TODO: should be fixed to use the actual current number
...
ElfSection*
Elf::getSectionAt(unsigned int offset)
{
  for (int i = 1; i < ehdr->e_shnum; i++) {
...
ElfSegment*
Elf::getSegmentByType(unsigned int type, ElfSegment* last)
{
...
void
Elf::removeSegment(ElfSegment* segment)
{
  if (!segment) return;
  std::vector<ElfSegment*>::iterator seg;
...
Elf::write(std::ofstream& file)
{
  ...
  for (ElfSection* section = ehdr; section != nullptr;
    ...
    if (section == phdr_section) {
      ...
      null_section.serialize(
          file, ehdr->e_ident[EI_CLASS], ehdr->e_ident[EI_DATA]);
      for (ElfSection* sec = ehdr; sec != nullptr; sec = sec->getNext()) {
        if (sec->getType() != SHT_NULL)
          sec->getShdr().serialize(
              file, ehdr->e_ident[EI_CLASS], ehdr->e_ident[EI_DATA]);
      }
    } else
...
void
ElfSegment::addSection(ElfSection* section)
{
...
void
ElfSegment::removeSection(ElfSection* section)
{
...
ElfSegment::getFileSize()
{
  ...
  for (i = sections.rbegin();
       (i != sections.rend()) && ((*i)->getType() == SHT_NOBITS);
       ++i)
    ;
...
ElfValue*
ElfDynamic_Section::getValueForType(unsigned int tag)
{
...
ElfSection*
ElfDynamic_Section::getSectionForType(unsigned int tag)
{
...
bool
ElfDynamic_Section::setValueForType(unsigned int tag, ElfValue* val)
{
  unsigned int i;
...
void
ElfDynamic_Section::serialize(std::ofstream& file, char ei_class, char ei_data)
{
  for (unsigned int i = 0; i < shdr.sh_size / shdr.sh_entsize; i++) {
    Elf_Dyn dyn;
