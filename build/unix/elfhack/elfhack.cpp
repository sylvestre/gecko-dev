...
{
 public:
  ElfRelHackCode_Section(Elf_Shdr& s,
                         Elf& e,
 ...
 private:
  ...
  {
   public:
    Elf32_Addr operator()(unsigned int base_addr,
                          Elf32_Off offset,
                          Elf32_Word addend,
                          unsigned int addr)
    {
  ...
  void apply_relocation(ElfSection* the_code,
                        char* base,
                        Elf_Rel* r,
                        unsigned int addr)
  {
...
unsigned int
get_addend(Elf_Rel* rel, Elf* elf)
{
  ElfLocation loc(rel->r_offset, elf);
  Elf_Addr addr(loc.getBuffer(),
...
unsigned int
get_addend(Elf_Rela* rel, Elf* elf)
{
  return rel->r_addend;
}

void
set_relative_reloc(Elf_Rel* rel, Elf* elf, unsigned int value)
{
  ElfLocation loc(rel->r_offset, elf);
  Elf_Addr addr;
  addr.value = value;
...
void
set_relative_reloc(Elf_Rela* rel, Elf* elf, unsigned int value)
{
...
void
maybe_split_segment(Elf* elf, ElfSegment* segment, bool fill)
{
                      ...
                      bool fill)
{
  ...
  Elf32_Shdr relhack32_section = {
      ...
      Elf_RelHack::size(
          elf->getClass())};  // TODO: sh_addralign should be an alignment, not size
    ...
    if (sym) {
      if (sym->defined) {
        ...
        if (ELF32_R_TYPE(i->r_info) == rel_type) {
          Elf_Addr addr(
              loc.getBuffer(), entry_sz, elf->getClass(), elf->getData());
  ...
  if (init_array) {
    ...
    set_relative_reloc(
        &section->rels[init_array_reloc - 1], elf, init->getValue());
        ...
        bool fill = false)
{
  ...
  switch (elf.getMachine()) {
    case EM_386:
      exit = do_relocation_section<Elf_Rel>(
          &elf, R_386_RELATIVE, R_386_32, force, fill);
      break;
    case EM_X86_64:
      exit = do_relocation_section<Elf_Rela>(
          &elf, R_X86_64_RELATIVE, R_X86_64_64, force, fill);
      break;
    case EM_ARM:
      exit = do_relocation_section<Elf_Rel>(
          &elf, R_ARM_RELATIVE, R_ARM_ABS32, force, fill);
      break;
  }
  if (exit == 0) {
...
void
undo_file(const char* name, bool backup = false)
{
  std::ifstream file(name, std::ios::in | std::ios::binary);
  Elf elf(file);
  unsigned int size = elf.getSize();
  ...
  if (data != text->getNext()) {
    fprintf(stderr,
            elfhack_data " section not following " elfhack_text ". Skipping\n");
    return;
  }
