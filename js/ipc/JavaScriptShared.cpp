  ...
  if (!sLoggingInitialized) {
    ...
    if (PR_GetEnv("MOZ_CPOW_LOG")) {
      ...
      Preferences::AddBoolVarCache(
          &sLoggingEnabled, "dom.ipc.cpows.log.enabled", false);
      Preferences::AddBoolVarCache(
          &sStackLoggingEnabled, "dom.ipc.cpows.log.stack", false);
    }
  }
}
...
bool
JavaScriptShared::convertIdToGeckoString(JSContext* cx,
                                         JS::HandleId id,
                                         nsString* to)
{
...
bool
JavaScriptShared::convertGeckoStringToId(JSContext* cx,
                                         const nsString& from,
...
bool
JavaScriptShared::fromVariant(JSContext* cx,
...
bool
JavaScriptShared::fromJSIDVariant(JSContext* cx,
                                  const JSIDVariant& from,
                                  MutableHandleId to)
{
  switch (from.type()) {
...
bool
JavaScriptShared::toSymbolVariant(JSContext* cx,
                                  JS::Symbol* symArg,
...
bool
JavaScriptShared::fromDescriptor(JSContext* cx,
...
bool
UnknownPropertyStub(JSContext* cx,
                    HandleObject obj,
                    HandleId id,
...
bool
UnknownStrictPropertyStub(JSContext* cx,
                          HandleObject obj,
                          HandleId id,
                          HandleValue v,
...
bool
JavaScriptShared::toDescriptor(JSContext* cx,
...
bool
JavaScriptShared::toObjectOrNullVariant(JSContext* cx,
                                        JSObject* obj,
...
JSObject*
JavaScriptShared::fromObjectOrNullVariant(JSContext* cx,
...
bool
JavaScriptShared::Unwrap(JSContext* cx,
                         ...
                         JS::MutableHandleObject objp)
{
  ...
  for (size_t i = 0; i < aCpows.Length(); i++) {
    ...
    if (!JS_DefineUCProperty(
            cx, obj, name.BeginReading(), name.Length(), v, JSPROP_ENUMERATE)) {
      return false;
    }
  }
...
bool
JavaScriptShared::Wrap(JSContext* cx,
                       HandleObject aObj,
