...
JSCompartment::JSCompartment(Zone* zone,
    ...
    MOZ_ASSERT_IF(creationOptions_.mergeable(), creationOptions_.invisibleToDebugger());
}
...
JSCompartment::~JSCompartment() {
    ...
    if (rt->lcovOutput().isEnabled()) rt->lcovOutput().writeLCovResult(lcovOutput);
...
void js::DtoaCache::checkCacheAfterMovingGC() { MOZ_ASSERT(!s || !IsForwarded(s)); }
...
bool JSCompartment::putWrapper(JSContext* cx, const CrossCompartmentKey& wrapped,
...
bool JSCompartment::wrap(JSContext* cx, MutableHandleString strp) {
    ...
    if (!putWrapper(cx, CrossCompartmentKey(key), StringValue(copy))) return false;

    strp.set(copy);
    return true;
}

bool JSCompartment::getNonWrapperObjectForCurrentCompartment(JSContext* cx,
    ...
    if (obj->is<StopIterationObject>()) {
        ...
        if (!GetBuiltinConstructor(cx, JSProto_StopIteration, &stopIteration)) return false;
...
bool JSCompartment::getOrCreateWrapper(JSContext* cx, HandleObject existing,
    ...
    if (!putWrapper(cx, CrossCompartmentKey(key), ObjectValue(*wrapper))) {
        ...
        // reference to it.
        if (wrapper->is<CrossCompartmentWrapperObject>()) NukeCrossCompartmentWrapper(cx, wrapper);
        return false;
    }
...
bool JSCompartment::rewrap(JSContext* cx, MutableHandleObject obj, HandleObject existingArg) {
    MOZ_ASSERT(!cx->runtime()->isAtomsCompartment(this));
    MOZ_ASSERT(cx->compartment() == this);
    MOZ_ASSERT(obj);
...
bool JSCompartment::wrap(JSContext* cx, MutableHandle<PropertyDescriptor> desc) {
...
LexicalEnvironmentObject* JSCompartment::getOrCreateNonSyntacticLexicalEnvironment(
    ...
    if (!lexicalEnv) {
        ...
        if (!nonSyntacticLexicalEnvironments_->add(cx, key, lexicalEnv)) return nullptr;
    }
...
/* static */ HashNumber TemplateRegistryHashPolicy::hash(const Lookup& lookup) {
...
/* static */ bool TemplateRegistryHashPolicy::match(const Key& key, const Lookup& lookup) {
    size_t length = lookup->as<NativeObject>().getDenseInitializedLength();
    if (key->as<NativeObject>().getDenseInitializedLength() != length) return false;
...
bool JSCompartment::getTemplateLiteralObject(JSContext* cx, HandleArrayObject rawStrings,
    ...
    if (TemplateRegistry::AddPtr p = templateLiteralMap_.lookupForAdd(rawStrings)) {
        ...
        if (!DefineDataProperty(cx, templateObj, cx->names().raw, rawValue, 0)) return false;
        if (!FreezeObject(cx, rawStrings)) return false;
        if (!FreezeObject(cx, templateObj)) return false;

        if (!templateLiteralMap_.relookupOrAdd(p, rawStrings, templateObj)) return false;
    }

    return true;
}

JSObject* JSCompartment::getExistingTemplateLiteralObject(ArrayObject* rawStrings) {
...
/* static */ void JSCompartment::traceIncomingCrossCompartmentEdgesForZoneGC(JSTracer* trc) {
    ...
    for (CompartmentsIter c(trc->runtime(), SkipAtoms); !c.done(); c.next()) {
        if (!c->zone()->isCollecting()) c->traceOutgoingCrossCompartmentWrappers(trc);
    }
...
void JSCompartment::traceRoots(JSTracer* trc, js::gc::GCRuntime::TraceOrMarkRuntime traceOrMark) {
    ...
    if (traceOrMark == js::gc::GCRuntime::TraceRuntime) {
        if (watchpointMap) watchpointMap->trace(trc);
    }
    ...
    if (nonSyntacticLexicalEnvironments_) nonSyntacticLexicalEnvironments_->trace(trc);
}

void JSCompartment::finishRoots() {
    if (watchpointMap) watchpointMap->clear();
    ...
    if (nonSyntacticLexicalEnvironments_) nonSyntacticLexicalEnvironments_->clear();
}
...
void JSCompartment::sweepTemplateLiteralMap() { templateLiteralMap_.sweep(); }
...
void JSCompartment::sweepJitCompartment(FreeOp* fop) {
    if (jitCompartment_) jitCompartment_->sweep(fop, this);
}
...
void JSCompartment::sweepDebugEnvironments() {
    ...
    if (debugEnvs) debugEnvs->sweep(rt);
}
 ...
 */
void JSCompartment::sweepCrossCompartmentWrappers() { crossCompartmentWrappers.sweep(); }

void JSCompartment::sweepVarNames() { varNames_.sweep(); }

void JSCompartment::sweepWatchpoints() {
    if (watchpointMap) watchpointMap->sweep();
}

void JSCompartment::sweepMapAndSetObjectsAfterMinorGC() {
    auto fop = runtime_->defaultFreeOp();

    for (auto mapobj : mapsWithNurseryMemory) MapObject::sweepAfterMinorGC(fop, mapobj);
    mapsWithNurseryMemory.clearAndFree();

    for (auto setobj : setsWithNurseryMemory) SetObject::sweepAfterMinorGC(fop, setobj);
...
/* static */ void JSCompartment::fixupCrossCompartmentWrappersAfterMovingGC(JSTracer* trc) {
...
void JSCompartment::setAllocationMetadataBuilder(const js::AllocationMetadataBuilder* builder) {
...
void JSCompartment::setNewObjectMetadata(JSContext* cx, HandleObject obj) {
    ...
    if (JSObject* metadata = allocationMetadataBuilder->build(cx, obj, oomUnsafe)) {
        ...
        if (!objectMetadataTable->add(cx, obj, metadata)) oomUnsafe.crash("setNewObjectMetadata");
    }
}

static bool AddInnerLazyFunctionsFromScript(JSScript* script, AutoObjectVector& lazyFunctions) {
...
static bool AddLazyFunctionsForCompartment(JSContext* cx, AutoObjectVector& lazyFunctions,
                                           AllocKind kind) {
    ...
    for (auto i = cx->zone()->cellIter<JSObject>(kind); !i.done(); i.next()) {
        ...
        if (gc::IsAboutToBeFinalizedUnbarriered(&fun) || fun->compartment() != cx->compartment()) {
            continue;
        }
...
static bool CreateLazyScriptsForCompartment(JSContext* cx) {
    ...
    if (!AddLazyFunctionsForCompartment(cx, lazyFunctions, AllocKind::FUNCTION)) return false;
...
bool JSCompartment::ensureDelazifyScriptsForDebugger(JSContext* cx) {
    ...
    if (needsDelazificationForDebugger() && !CreateLazyScriptsForCompartment(cx)) return false;
...
void JSCompartment::updateDebuggerObservesFlag(unsigned flag) {
    ...
    MOZ_ASSERT(flag == DebuggerObservesAllExecution || flag == DebuggerObservesCoverage ||
               flag == DebuggerObservesAsmJS || flag == DebuggerObservesBinarySource);
...
void JSCompartment::updateDebuggerObservesCoverage() {
    ...
    if (debuggerObservesCoverage()) {
        ...
        for (ActivationIterator iter(cx); !iter.done(); ++iter) {
            if (iter->isInterpreter()) iter->asInterpreter()->enableInterruptsUnconditionally();
        }
        return;
    }
...
bool JSCompartment::collectCoverageForPGO() const { return !JitOptions.disablePgo; }

bool JSCompartment::collectCoverageForDebug() const {
    return debuggerObservesCoverage() || runtimeFromAnyThread()->profilingScripts ||
...
void JSCompartment::clearBreakpointsIn(FreeOp* fop, js::Debugger* dbg, HandleObject handler) {
...
void JSCompartment::addSizeOfIncludingThis(
    mozilla::MallocSizeOf mallocSizeOf, size_t* tiAllocationSiteTables, size_t* tiArrayTypeTables,
    size_t* tiObjectTypeTables, size_t* compartmentObject, size_t* compartmentTables,
    size_t* innerViewsArg, size_t* lazyArrayBuffersArg, size_t* objectMetadataTablesArg,
    size_t* crossCompartmentWrappersArg, size_t* savedStacksSet, size_t* varNamesSet,
    size_t* nonSyntacticLexicalEnvironmentsArg, size_t* templateLiteralMap, size_t* jitCompartment,
    size_t* privateData) {
    *compartmentObject += mallocSizeOf(this);
    objectGroups.addSizeOfExcludingThis(mallocSizeOf, tiAllocationSiteTables, tiArrayTypeTables,
                                        tiObjectTypeTables, compartmentTables);
    ...
    *templateLiteralMap += templateLiteralMap_.sizeOfExcludingThis(mallocSizeOf);
    if (jitCompartment_) *jitCompartment += jitCompartment_->sizeOfIncludingThis(mallocSizeOf);
...
void JSCompartment::reportTelemetry() {
    ...
    for (size_t i = 0; i < size_t(DeprecatedLanguageExtension::Count); i++) {
        if (sawDeprecatedLanguageExtension[i]) runtime_->addTelemetry(id, i);
    }
}

void JSCompartment::addTelemetry(const char* filename, DeprecatedLanguageExtension e) {
    // Only report telemetry for web content and add-ons, not chrome JS.
    if (isSystem_) return;
...
mozilla::HashCodeScrambler JSCompartment::randomHashCodeScrambler() {
    return mozilla::HashCodeScrambler(randomKeyGenerator_.next(), randomKeyGenerator_.next());
}
...
AutoSetNewObjectMetadata::AutoSetNewObjectMetadata(
    ...
    if (cx_) cx_->compartment()->objectMetadataState = NewObjectMetadataState(DelayMetadata());
}
