...
class Thread {
   public:
    ...
    struct Hasher {
        ...
        static bool match(const Id& key, const Lookup& lookup) { return key == lookup; }
    };
    ...
    template <typename O = Options,
              ...
              typename =
                  typename mozilla::EnableIf<mozilla::IsSame<DerefO, Options>::value, void*>::Type>
    explicit Thread(O&& options = Options())
        : idMutex_(mutexid::ThreadId), id_(Id()), options_(mozilla::Forward<O>(options)) {}
    ...
    MOZ_MUST_USE bool init(F&& f, Args&&... args) {
        ...
        auto trampoline =
            js_new<Trampoline>(mozilla::Forward<F>(f), mozilla::Forward<Args>(args)...);
