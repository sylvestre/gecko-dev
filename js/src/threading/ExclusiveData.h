...
class ExclusiveData {
   ...
   public:
    ...
    class MOZ_STACK_CLASS Guard {
       ...
       public:
        explicit Guard(const ExclusiveData& parent) : parent_(&parent) { parent_->acquire(); }
...
class ExclusiveWaitableData : public ExclusiveData<T> {
   ...
   public:
    template <typename U>
    explicit ExclusiveWaitableData(const MutexId& id, U&& u) : Base(id, mozilla::Forward<U>(u)) {}
    ...
    class MOZ_STACK_CLASS Guard : public ExclusiveData<T>::Guard {
       ...
       public:
        ...
        Guard& operator=(Guard&& rhs) { return Base::operator=(mozilla::Move(rhs)); }
