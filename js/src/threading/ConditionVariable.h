...
class ConditionVariable {
   public:
    ...
    CVStatus wait_until(UniqueLock<Mutex>& lock, const mozilla::TimeStamp& abs_time) {
    ...
    // to true.
    template <typename Predicate>
    bool wait_until(UniqueLock<Mutex>& lock, const mozilla::TimeStamp& abs_time, Predicate pred) {
        while (!pred()) {
    ...
    CVStatus wait_for(UniqueLock<Mutex>& lock, const mozilla::TimeDuration& rel_time) {
    ...
    // true.
    template <typename Predicate>
    bool wait_for(UniqueLock<Mutex>& lock, const mozilla::TimeDuration& rel_time, Predicate pred) {
        return wait_until(lock, mozilla::TimeStamp::Now() + rel_time, mozilla::Move(pred));
    }
