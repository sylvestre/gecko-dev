...
JS::CallArgs::requireAtLeast(JSContext* cx, const char* fnname, unsigned required) const {
    if (length() < required) {
        ...
        JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr, JSMSG_MORE_ARGS_NEEDED, fnname,
                                  numArgsStr, required == 2 ? "" : "s");
        return false;
    }
...
JS::ObjectOpResult::reportStrictErrorOrWarning(JSContext* cx, HandleObject obj, HandleId id,
    ...
    if (code_ == JSMSG_OBJECT_NOT_EXTENSIBLE) {
        ...
        return ReportValueErrorFlags(cx, flags, code_, JSDVG_IGNORE_STACK, val, nullptr, nullptr,
                                     nullptr);
    }
    ...
    if (ErrorTakesArguments(code_)) {
        ...
        if (code_ == JSMSG_SET_NON_OBJECT_RECEIVER) {
            ...
            return ReportValueErrorFlags(cx, flags, code_, JSDVG_IGNORE_STACK, val, nullptr,
                                         propName.ptr(), nullptr);
        }
...
JS_PUBLIC_API(bool)
JS::ObjectOpResult::failCantRedefineProp() { return fail(JSMSG_CANT_REDEFINE_PROP); }

JS_PUBLIC_API(bool)
JS::ObjectOpResult::failReadOnly() { return fail(JSMSG_READ_ONLY); }

JS_PUBLIC_API(bool)
JS::ObjectOpResult::failGetterOnly() { return fail(JSMSG_GETTER_ONLY); }

JS_PUBLIC_API(bool)
JS::ObjectOpResult::failCantDelete() { return fail(JSMSG_CANT_DELETE); }

JS_PUBLIC_API(bool)
JS::ObjectOpResult::failCantSetInterposed() { return fail(JSMSG_CANT_SET_INTERPOSED); }
...
JS_PUBLIC_API(bool)
JS::ObjectOpResult::failCantPreventExtensions() { return fail(JSMSG_CANT_PREVENT_EXTENSIONS); }

JS_PUBLIC_API(bool)
JS::ObjectOpResult::failCantSetProto() { return fail(JSMSG_CANT_SET_PROTO); }

JS_PUBLIC_API(bool)
JS::ObjectOpResult::failNoNamedSetter() { return fail(JSMSG_NO_NAMED_SETTER); }

JS_PUBLIC_API(bool)
JS::ObjectOpResult::failNoIndexedSetter() { return fail(JSMSG_NO_INDEXED_SETTER); }

JS_PUBLIC_API(int64_t)
JS_Now() { return PRMJ_Now(); }

JS_PUBLIC_API(Value)
JS_GetNaNValue(JSContext* cx) { return cx->runtime()->NaNValue; }

JS_PUBLIC_API(Value)
JS_GetNegativeInfinityValue(JSContext* cx) { return cx->runtime()->negativeInfinityValue; }

JS_PUBLIC_API(Value)
JS_GetPositiveInfinityValue(JSContext* cx) { return cx->runtime()->positiveInfinityValue; }

JS_PUBLIC_API(Value)
JS_GetEmptyStringValue(JSContext* cx) { return StringValue(cx->runtime()->emptyString); }
...
static void AssertHeapIsIdleOrIterating() { MOZ_ASSERT(!JS::CurrentThreadIsHeapCollecting()); }
...
JS_PUBLIC_API(bool)
JS_DoubleIsInt32(double d, int32_t* ip) { return mozilla::NumberIsInt32(d, ip); }
...
JS_PUBLIC_API(bool)
JS_IsBuiltinEvalFunction(JSFunction* fun) { return IsAnyBuiltinEval(fun); }

JS_PUBLIC_API(bool)
JS_IsBuiltinFunctionConstructor(JSFunction* fun) { return fun->isBuiltinFunctionConstructor(); }

JS_PUBLIC_API(bool)
JS_IsFunctionBound(JSFunction* fun) { return fun->isBoundFunction(); }
...
#ifdef DEBUG
JS_FRIEND_API(bool)
JS::isGCEnabled() { return !TlsContext.get()->suppressGC; }
#else
...
JS_PUBLIC_API(void)
JS_YieldCooperativeContext(JSContext* cx) { YieldCooperativeContext(cx); }

JS_PUBLIC_API(void)
JS_ResumeCooperativeContext(JSContext* cx) { ResumeCooperativeContext(cx); }

JS_PUBLIC_API(void)
JS_DestroyContext(JSContext* cx) { DestroyContext(cx); }
...
JS_PUBLIC_API(void)
JS_SetFutexCanWait(JSContext* cx) { cx->fx.setCanWait(true); }
...
JS_PUBLIC_API(void)
JS::SetSingleThreadedExecutionCallbacks(JSContext* cx, BeginSingleThreadedExecutionCallback begin,
...
JS_GetVersion(JSContext* cx) { return VersionNumber(cx->findVersion()); }

static const struct v2smap {
    JSVersion version;
    const char* string;
} v2smap[] = {
    {JSVERSION_ECMA_3, "ECMAv3"}, {JSVERSION_1_6, "1.6"},
    {JSVERSION_1_7, "1.7"},       {JSVERSION_1_8, "1.8"},
    {JSVERSION_ECMA_5, "ECMAv5"}, {JSVERSION_DEFAULT, js_default_str},
    {JSVERSION_DEFAULT, "1.0"},   {JSVERSION_DEFAULT, "1.1"},
    {JSVERSION_DEFAULT, "1.2"},   {JSVERSION_DEFAULT, "1.3"},
    {JSVERSION_DEFAULT, "1.4"},   {JSVERSION_DEFAULT, "1.5"},
...
JS_VersionToString(JSVersion version) {
    ...
    for (i = 0; v2smap[i].string; i++)
        if (v2smap[i].version == version) return v2smap[i].string;
    return "unknown";
}
...
JS_StringToVersion(const char* string) {
    ...
    for (i = 0; v2smap[i].string; i++)
        if (strcmp(v2smap[i].string, string) == 0) return v2smap[i].version;
    return JSVERSION_UNKNOWN;
}

JS_PUBLIC_API(JS::ContextOptions&)
JS::ContextOptionsRef(JSContext* cx) { return cx->options(); }
...
JS_PUBLIC_API(const char*)
JS_GetImplementationVersion(void) { return "JavaScript-C" MOZILLA_VERSION; }
...
JSAutoCompartment::JSAutoCompartment(JSContext* cx,
                                     JSObject* target MOZ_GUARD_OBJECT_NOTIFIER_PARAM_IN_IMPL)
...
JSAutoCompartment::JSAutoCompartment(JSContext* cx,
                                     JSScript* target MOZ_GUARD_OBJECT_NOTIFIER_PARAM_IN_IMPL)
    : cx_(cx), oldCompartment_(cx->compartment()) {
    AssertHeapIsIdleOrIterating();
    MOZ_GUARD_OBJECT_NOTIFIER_INIT;
    cx_->enterCompartmentOf(target);
...
JSAutoCompartment::~JSAutoCompartment() { cx_->leaveCompartment(oldCompartment_); }

JSAutoNullableCompartment::JSAutoNullableCompartment(
    JSContext* cx, JSObject* targetOrNull MOZ_GUARD_OBJECT_NOTIFIER_PARAM_IN_IMPL)
    : cx_(cx), oldCompartment_(cx->compartment()) {
    AssertHeapIsIdleOrIterating();
    MOZ_GUARD_OBJECT_NOTIFIER_INIT;
    if (targetOrNull)
...
JSAutoNullableCompartment::~JSAutoNullableCompartment() { cx_->leaveCompartment(oldCompartment_); }

JS_PUBLIC_API(void)
JS_SetCompartmentPrivate(JSCompartment* compartment, void* data) { compartment->data = data; }

JS_PUBLIC_API(void*)
JS_GetCompartmentPrivate(JSCompartment* compartment) { return compartment->data; }

JS_PUBLIC_API(void)
JS_MarkCrossZoneId(JSContext* cx, jsid id) { cx->markId(id); }

JS_PUBLIC_API(void)
JS_MarkCrossZoneIdValue(JSContext* cx, const Value& value) { cx->markAtomValue(value); }
...
JS_PUBLIC_API(JSString*)
JS::StringOfAddonId(JSAddonId* id) { return id; }
...
JS_PUBLIC_API(void)
JS_SetZoneUserData(JS::Zone* zone, void* data) { zone->data = data; }

JS_PUBLIC_API(void*)
JS_GetZoneUserData(JS::Zone* zone) { return zone->data; }
...
static void ReleaseAssertObjectHasNoWrappers(JSContext* cx, HandleObject target) {
...
JS_ResolveStandardClass(JSContext* cx, HandleObject obj, HandleId id, bool* resolved) {
    ...
    *resolved = false;
    ...
    if (!stdnm) stdnm = LookupStdName(cx->names(), idAtom, builtin_property_names);

    if (stdnm && GlobalObject::skipDeselectedConstructor(cx, stdnm->key)) stdnm = nullptr;
...
JS_MayResolveStandardClass(const JSAtomState& names, jsid id, JSObject* maybeObj) {
    ...
    return atom == names.undefined || LookupStdName(names, atom, standard_class_names) ||
...
static bool EnumerateUnresolvedStandardClasses(JSContext* cx, Handle<GlobalObject*> global,
...
JS_IdToProtoKey(JSContext* cx, HandleId id) {
    ...
    if (GlobalObject::skipDeselectedConstructor(cx, stdnm->key)) return JSProto_Null;
...
extern JS_PUBLIC_API(bool) JS_IsGlobalObject(JSObject* obj) { return obj->is<GlobalObject>(); }
...
JS_realloc(JSContext* cx, void* p, size_t oldBytes, size_t newBytes) {
    ...
    return static_cast<void*>(
        cx->zone()->pod_realloc<uint8_t>(static_cast<uint8_t*>(p), oldBytes, newBytes));
}

JS_PUBLIC_API(void)
JS_free(JSContext* cx, void* p) { return js_free(p); }

JS_PUBLIC_API(void)
JS_freeop(JSFreeOp* fop, void* p) { return FreeOp::get(fop)->free_(p); }

JS_PUBLIC_API(void)
JS_updateMallocCounter(JSContext* cx, size_t nbytes) { return cx->updateMallocCounter(nbytes); }
...
JS_SetGCParametersBasedOnAvailableMemory(JSContext* cx, uint32_t availMem) {
    static const JSGCConfig minimal[NumGCConfigs] = {
        {JSGC_MAX_MALLOC_BYTES, 6 * 1024 * 1024},   {JSGC_SLICE_TIME_BUDGET, 30},
        {JSGC_HIGH_FREQUENCY_TIME_LIMIT, 1500},     {JSGC_HIGH_FREQUENCY_HIGH_LIMIT, 40},
        {JSGC_HIGH_FREQUENCY_LOW_LIMIT, 0},         {JSGC_HIGH_FREQUENCY_HEAP_GROWTH_MAX, 300},
        {JSGC_HIGH_FREQUENCY_HEAP_GROWTH_MIN, 120}, {JSGC_LOW_FREQUENCY_HEAP_GROWTH, 120},
        {JSGC_HIGH_FREQUENCY_TIME_LIMIT, 1500},     {JSGC_HIGH_FREQUENCY_TIME_LIMIT, 1500},
        {JSGC_HIGH_FREQUENCY_TIME_LIMIT, 1500},     {JSGC_ALLOCATION_THRESHOLD, 1},
...
extern JS_PUBLIC_API(bool) JS_IsExternalString(JSString* str) { return str->isExternal(); }
...
static void SetNativeStackQuotaAndLimit(JSContext* cx, JS::StackKind kind, size_t stackSize) {
...
JS::GetFirstArgumentAsTypeHint(JSContext* cx, CallArgs args, JSType* result) {
    if (!args.get(0).isString()) {
        JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr, JSMSG_NOT_EXPECTED_TYPE,
                                  "Symbol.toPrimitive", "\"string\", \"number\", or \"default\"",
...
JS_PUBLIC_API(JSObject*)
JS_InitClass(JSContext* cx, HandleObject obj, HandleObject parent_proto, const JSClass* clasp,
    ...
    return InitClass(cx, obj, parent_proto, Valueify(clasp), constructor, nargs, ps, fs, static_ps,
                     static_fs);
}
...
JS_GetClass(JSObject* obj) { return obj->getJSClass(); }
...
JS_GetConstructor(JSContext* cx, HandleObject proto) {
    ...
    RootedValue cval(cx);
    if (!GetProperty(cx, proto, proto, cx->names().constructor, &cval)) return nullptr;
...
JS::CompartmentCreationOptions& JS::CompartmentCreationOptions::setSystemZone() {
    zoneSpec_ = JS::SystemZone;
    zonePointer_ = nullptr;
    return *this;
}

JS::CompartmentCreationOptions& JS::CompartmentCreationOptions::setExistingZone(JSObject* obj) {
    zoneSpec_ = JS::ExistingZone;
    zonePointer_ = obj->zone();
    return *this;
}

JS::CompartmentCreationOptions& JS::CompartmentCreationOptions::setNewZoneInNewZoneGroup() {
    zoneSpec_ = JS::NewZoneInNewZoneGroup;
    zonePointer_ = nullptr;
    return *this;
}

JS::CompartmentCreationOptions& JS::CompartmentCreationOptions::setNewZoneInSystemZoneGroup() {
    zoneSpec_ = JS::NewZoneInSystemZoneGroup;
    zonePointer_ = nullptr;
    return *this;
}

JS::CompartmentCreationOptions& JS::CompartmentCreationOptions::setNewZoneInExistingZoneGroup(
    JSObject* obj) {
...
const JS::CompartmentCreationOptions& JS::CompartmentCreationOptionsRef(JSObject* obj) {
    return obj->compartment()->creationOptions();
}

const JS::CompartmentCreationOptions& JS::CompartmentCreationOptionsRef(JSContext* cx) {
...
JS::CompartmentCreationOptions& JS::CompartmentCreationOptions::setSharedMemoryAndAtomicsEnabled(
    bool flag) {
#if defined(ENABLE_SHARED_ARRAY_BUFFER)
...
JS::CompartmentBehaviors& JS::CompartmentBehaviorsRef(JSCompartment* compartment) {
...
JS_GlobalObjectTraceHook(JSTracer* trc, JSObject* global) {
    ...
    if (JSTraceOp trace = global->compartment()->creationOptions().getTrace()) trace(trc, global);
}
...
JS_PUBLIC_API(bool)
JS_IsNative(JSObject* obj) { return obj->isNative(); }
...
static bool DefinePropertyByDescriptor(JSContext* cx, HandleObject obj, HandleId id,
...
static bool DefineAccessorPropertyById(JSContext* cx, HandleObject obj, HandleId id,
    ...
    assertSameCompartment(
        cx, obj, id, (attrs & JSPROP_GETTER) ? JS_FUNC_TO_DATA_PTR(JSObject*, getter) : nullptr,
        (attrs & JSPROP_SETTER) ? JS_FUNC_TO_DATA_PTR(JSObject*, setter) : nullptr);

    return js::DefineAccessorProperty(cx, obj, id, getter, setter, attrs);
}

static bool DefineDataPropertyById(JSContext* cx, HandleObject obj, HandleId id, HandleValue value,
                                   unsigned attrs) {
...
static bool DefineAccessorProperty(JSContext* cx, HandleObject obj, const char* name,
...
static bool DefineDataProperty(JSContext* cx, HandleObject obj, const char* name,
...
static bool DefineUCAccessorProperty(JSContext* cx, HandleObject obj, const char16_t* name,
                                     size_t namelen, Native getter, Native setter,
                                     unsigned attrs) {
...
static bool DefineUCDataProperty(JSContext* cx, HandleObject obj, const char16_t* name,
                                 size_t namelen, HandleValue value, unsigned attrs) {
    JSAtom* atom = AtomizeChars(cx, name, AUTO_NAMELEN(name, namelen));
    if (!atom) return false;
    RootedId id(cx, AtomToId(atom));
    return DefineDataPropertyById(cx, obj, id, value, attrs);
...
static bool DefineAccessorElement(JSContext* cx, HandleObject obj, uint32_t index, unsigned attrs,
...
static bool DefineDataElement(JSContext* cx, HandleObject obj, uint32_t index, HandleValue value,
                              unsigned attrs) {
    assertSameCompartment(cx, obj, value);
    AssertHeapIsIdle();
    CHECK_REQUEST(cx);
...
JS_PUBLIC_API(bool)
JS_HasUCProperty(JSContext* cx, HandleObject obj, const char16_t* name, size_t namelen,
                 bool* foundp) {
    JSAtom* atom = AtomizeChars(cx, name, AUTO_NAMELEN(name, namelen));
    if (!atom) return false;
    RootedId id(cx, AtomToId(atom));
...
static bool SetElement(JSContext* cx, HandleObject obj, uint32_t index, HandleValue v) {
    AssertHeapIsIdle();
    CHECK_REQUEST(cx);
...
JS_PUBLIC_API(bool)
JS::IsCallable(JSObject* obj) { return obj->isCallable(); }

JS_PUBLIC_API(bool)
JS::IsConstructor(JSObject* obj) { return obj->isConstructor(); }

JS_PUBLIC_API(bool)
JS_CallFunctionValue(JSContext* cx, HandleObject obj, HandleValue fval,
                     const HandleValueArray& args, MutableHandleValue rval) {
    MOZ_ASSERT(!cx->runtime()->isAtomsCompartment(cx->compartment()));
    AssertHeapIsIdle();
    CHECK_REQUEST(cx);
...
JS_PUBLIC_API(bool)
JS_CallFunctionName(JSContext* cx, HandleObject obj, const char* name,
                    const HandleValueArray& args, MutableHandleValue rval) {
    MOZ_ASSERT(!cx->runtime()->isAtomsCompartment(cx->compartment()));
    AssertHeapIsIdle();
    CHECK_REQUEST(cx);
...
JS_PUBLIC_API(bool)
JS::Construct(JSContext* cx, HandleValue fval, HandleObject newTarget,
...
static bool DefineSelfHostedProperty(JSContext* cx, HandleObject obj, HandleId id,
...
template <typename T>
static bool DefineConstScalar(JSContext* cx, HandleObject obj, const JSConstScalarSpec<T>* cds) {
    AssertHeapIsIdle();
    CHECK_REQUEST(cx);
...
JS::PropertySpecNameToPermanentId(JSContext* cx, const char* name, jsid* idp) {
    ...
    // marked.
    return PropertySpecNameToId(cx, name, MutableHandleId::fromMarkedLocation(idp), js::PinAtom);
}
...
JS_DefineProperties(JSContext* cx, HandleObject obj, const JSPropertySpec* ps) {
    ...
    for (; ps->name; ps++) {
        ...
        if (ps->isAccessor()) {
            if (ps->isSelfHosted()) {
                if (!DefineSelfHostedProperty(cx, obj, id, ps->accessors.getter.selfHosted.funname,
                                              ps->accessors.setter.selfHosted.funname,
                                              ps->flags)) {
                    return false;
                }
            } else {
                if (!DefineAccessorPropertyById(cx, obj, id, ps->accessors.getter.native,
...
JS_PUBLIC_API(bool)
JS::ObjectToCompletePropertyDescriptor(JSContext* cx, HandleObject obj, HandleValue descObj,
                                       MutableHandle<PropertyDescriptor> desc) {
...
inline bool IsGivenTypeObject(JSContext* cx, JS::HandleObject obj, const ESClass& typeClass,
                              bool* isType) {
    assertSameCompartment(cx, obj);
...
JS_PUBLIC_API(void)
JS_HoldPrincipals(JSPrincipals* principals) { ++principals->refcount; }
...
extern JS_PUBLIC_API(void) JS_InitReadPrincipalsCallback(JSContext* cx, JSReadPrincipalsOp read) {
    MOZ_ASSERT(read);
...
JS_PUBLIC_API(JSFunction*)
JS_NewFunction(JSContext* cx, JSNative native, unsigned nargs, unsigned flags, const char* name) {
    MOZ_ASSERT(!cx->runtime()->isAtomsCompartment(cx->compartment()));
    ...
    return (flags & JSFUN_CONSTRUCTOR) ? NewNativeConstructor(cx, native, nargs, atom)
...
static bool CreateNonSyntacticEnvironmentChain(JSContext* cx, AutoObjectVector& envChain,
                                               MutableHandleObject env, MutableHandleScope scope) {
    RootedObject globalLexical(cx, &cx->global()->lexicalEnvironment());
    if (!js::CreateObjectsForEnvironmentChain(cx, envChain, globalLexical, env)) return false;
...
static bool IsFunctionCloneable(HandleFunction fun) {
    if (!fun->isInterpreted()) return true;
...
static JSObject* CloneFunctionObject(JSContext* cx, HandleObject funobj, HandleObject env,
                                     HandleScope scope) {
    AssertHeapIsIdle();
    CHECK_REQUEST(cx);
...
extern JS_PUBLIC_API(JSObject*) JS::CloneFunctionObject(JSContext* cx, HandleObject funobj,
                                                        AutoObjectVector& envChain) {
    RootedObject env(cx);
    RootedScope scope(cx);
    if (!CreateNonSyntacticEnvironmentChain(cx, envChain, &env, &scope)) return nullptr;
    return CloneFunctionObject(cx, funobj, env, scope);
}

JS_PUBLIC_API(JSObject*)
JS_GetFunctionObject(JSFunction* fun) { return fun; }

JS_PUBLIC_API(JSString*)
JS_GetFunctionId(JSFunction* fun) { return fun->explicitName(); }

JS_PUBLIC_API(JSString*)
JS_GetFunctionDisplayId(JSFunction* fun) { return fun->displayAtom(); }

JS_PUBLIC_API(uint16_t)
JS_GetFunctionArity(JSFunction* fun) { return fun->nargs(); }

JS_PUBLIC_API(bool)
JS_ObjectIsFunction(JSContext* cx, JSObject* obj) { return obj->is<JSFunction>(); }
...
extern JS_PUBLIC_API(bool) JS_IsConstructor(JSFunction* fun) { return fun->isConstructor(); }
...
JS_PUBLIC_API(JSFunction*)
JS_DefineFunction(JSContext* cx, HandleObject obj, const char* name, JSNative call, unsigned nargs,
                  unsigned attrs) {
    MOZ_ASSERT(!cx->runtime()->isAtomsCompartment(cx->compartment()));
    AssertHeapIsIdle();
    CHECK_REQUEST(cx);
...
JS_PUBLIC_API(JSFunction*)
JS_DefineUCFunction(JSContext* cx, HandleObject obj, const char16_t* name, size_t namelen,
...
bool AutoFile::open(JSContext* cx, const char* filename) {
    if (!filename || strcmp(filename, "-") == 0) {
        ...
        if (!fp_) {
             ...
             * platform dependent.
             */
            JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, JSMSG_CANT_OPEN, filename,
...
void JS::TransitiveCompileOptions::copyPODTransitiveOptions(const TransitiveCompileOptions& rhs) {
...
void JS::ReadOnlyCompileOptions::copyPODOptions(const ReadOnlyCompileOptions& rhs) {
...
bool JS::OwningCompileOptions::copy(JSContext* cx, const ReadOnlyCompileOptions& rhs) {
    copyPODOptions(rhs);
...
bool JS::OwningCompileOptions::setFileAndLine(JSContext* cx, const char* f, unsigned l) {
...
bool JS::OwningCompileOptions::setSourceMapURL(JSContext* cx, const char16_t* s) {
...
bool JS::OwningCompileOptions::setIntroducerFilename(JSContext* cx, const char* s) {
    char* copy = nullptr;
    if (s) {
        copy = JS_strdup(cx, s);
        if (!copy) return false;
...
static bool Compile(JSContext* cx, const ReadOnlyCompileOptions& options, ScopeKind scopeKind,
...
static bool Compile(JSContext* cx, const ReadOnlyCompileOptions& optionsArg, ScopeKind scopeKind,
...
bool JS::Compile(JSContext* cx, const ReadOnlyCompileOptions& options, SourceBufferHolder& srcBuf,
                 JS::MutableHandleScript script) {
    return ::Compile(cx, options, ScopeKind::Global, srcBuf, script);
}

bool JS::Compile(JSContext* cx, const ReadOnlyCompileOptions& options, const char* bytes,
                 size_t length, JS::MutableHandleScript script) {
    return ::Compile(cx, options, ScopeKind::Global, bytes, length, script);
}

bool JS::Compile(JSContext* cx, const ReadOnlyCompileOptions& options, const char16_t* chars,
                 size_t length, JS::MutableHandleScript script) {
    return ::Compile(cx, options, ScopeKind::Global, chars, length, script);
}

bool JS::Compile(JSContext* cx, const ReadOnlyCompileOptions& options, FILE* file,
                 JS::MutableHandleScript script) {
    return ::Compile(cx, options, ScopeKind::Global, file, script);
}

bool JS::Compile(JSContext* cx, const ReadOnlyCompileOptions& options, const char* filename,
                 JS::MutableHandleScript script) {
    return ::Compile(cx, options, ScopeKind::Global, filename, script);
}

bool JS::CompileForNonSyntacticScope(JSContext* cx, const ReadOnlyCompileOptions& options,
...
static bool CanDoOffThread(JSContext* cx, const ReadOnlyCompileOptions& options, size_t length,
                           OffThread what) {
...
JS_PUBLIC_API(bool)
JS::CompileOffThread(JSContext* cx, const ReadOnlyCompileOptions& options, const char16_t* chars,
...
JS_PUBLIC_API(bool)
JS::FinishMultiOffThreadScriptsDecoder(JSContext* cx, void* token,
...
JS_GetScriptBaseLineNumber(JSContext* cx, JSScript* script) { return script->lineno(); }
 ...
 */
static bool CompileFunction(JSContext* cx, const ReadOnlyCompileOptions& optionsArg,
                            HandleAtom name, bool isInvalidName, SourceBufferHolder& srcBuf,
                            uint32_t parameterListEnd, HandleObject enclosingEnv,
                            HandleScope enclosingScope, MutableHandleFunction fun) {
    MOZ_ASSERT(!cx->runtime()->isAtomsCompartment(cx->compartment()));
    AssertHeapIsIdle();
    CHECK_REQUEST(cx);
    ...
    fun.set(NewScriptedFunction(
        cx, 0, JSFunction::INTERPRETED_NORMAL, isInvalidName ? nullptr : name,
        /* proto = */ nullptr, gc::AllocKind::FUNCTION, TenuredObject, enclosingEnv));
    if (!fun) return false;
    ...
    if (!frontend::CompileStandaloneFunction(cx, fun, optionsArg, srcBuf, Some(parameterListEnd),
                                             enclosingScope)) {
        return false;
    }
...
static MOZ_MUST_USE bool BuildFunctionString(const char* name, size_t nameLen, unsigned nargs,
...
JS::CompileFunction(JSContext* cx, AutoObjectVector& envChain,
    ...
    RootedObject env(cx);
    RootedScope scope(cx);
    if (!CreateNonSyntacticEnvironmentChain(cx, envChain, &env, &scope)) return false;
...
JS::CompileFunction(JSContext* cx, AutoObjectVector& envChain,
                    ...
                    MutableHandleFunction fun) {
    SourceBufferHolder srcBuf(chars, length, SourceBufferHolder::NoOwnership);
    return CompileFunction(cx, envChain, options, name, nargs, argnames, srcBuf, fun);
}
...
JS::CompileFunction(JSContext* cx, AutoObjectVector& envChain,
    ...
    return CompileFunction(cx, envChain, options, name, nargs, argnames, chars.get(), length, fun);
}
...
MOZ_NEVER_INLINE static bool ExecuteScript(JSContext* cx, HandleObject scope, HandleScript script,
                                           Value* rval) {
    MOZ_ASSERT(!cx->runtime()->isAtomsCompartment(cx->compartment()));
    AssertHeapIsIdle();
    CHECK_REQUEST(cx);
...
static bool ExecuteScript(JSContext* cx, AutoObjectVector& envChain, HandleScript scriptArg,
                          Value* rval) {
    RootedObject env(cx);
    RootedScope dummy(cx);
    if (!CreateNonSyntacticEnvironmentChain(cx, envChain, &env, &dummy)) return false;
...
MOZ_NEVER_INLINE JS_PUBLIC_API(bool) JS_ExecuteScript(JSContext* cx, HandleScript scriptArg) {
    RootedObject globalLexical(cx, &cx->global()->lexicalEnvironment());
...
JS_PUBLIC_API(bool)
JS::CloneAndExecuteScript(JSContext* cx, HandleScript scriptArg, JS::MutableHandleValue rval) {
    CHECK_REQUEST(cx);
    RootedScript script(cx, scriptArg);
    RootedObject globalLexical(cx, &cx->global()->lexicalEnvironment());
...
JS_PUBLIC_API(bool)
JS::CloneAndExecuteScript(JSContext* cx, JS::AutoObjectVector& envChain, HandleScript scriptArg,
...
static bool Evaluate(JSContext* cx, ScopeKind scopeKind, HandleObject env,
                     const ReadOnlyCompileOptions& optionsArg, SourceBufferHolder& srcBuf,
                     MutableHandleValue rval) {
    CompileOptions options(cx, optionsArg);
    MOZ_ASSERT(!cx->runtime()->isAtomsCompartment(cx->compartment()));
    AssertHeapIsIdle();
    ...
    RootedScript script(
        cx, frontend::CompileGlobalScript(cx, cx->tempLifoAlloc(), scopeKind, options, srcBuf));
    if (!script) return false;

    MOZ_ASSERT(script->getVersion() == options.version);

    bool result = Execute(cx, script, *env, options.noScriptRval ? nullptr : rval.address());
...
extern JS_PUBLIC_API(bool) JS::Evaluate(JSContext* cx, const ReadOnlyCompileOptions& options,
                                        const char* bytes, size_t length,
                                        MutableHandleValue rval) {
    char16_t* chars;
    if (options.utf8)
...
static bool Evaluate(JSContext* cx, const ReadOnlyCompileOptions& optionsArg, const char* filename,
                     MutableHandleValue rval) {
    FileContents buffer(cx);
    {
        AutoFile file;
...
JS_PUBLIC_API(bool)
JS::Evaluate(JSContext* cx, const ReadOnlyCompileOptions& optionsArg, SourceBufferHolder& srcBuf,
             MutableHandleValue rval) {
    RootedObject globalLexical(cx, &cx->global()->lexicalEnvironment());
    return ::Evaluate(cx, ScopeKind::Global, globalLexical, optionsArg, srcBuf, rval);
}
...
JS_PUBLIC_API(bool)
JS::Evaluate(JSContext* cx, const ReadOnlyCompileOptions& optionsArg, const char16_t* chars,
             size_t length, MutableHandleValue rval) {
...
JS_PUBLIC_API(bool)
JS::Evaluate(JSContext* cx, const ReadOnlyCompileOptions& optionsArg, const char* filename,
             MutableHandleValue rval) {
...
JS_PUBLIC_API(bool)
JS::CompileModule(JSContext* cx, const ReadOnlyCompileOptions& options, SourceBufferHolder& srcBuf,
...
JS_PUBLIC_API(void)
JS::GetRequestedModuleSourcePos(JSContext* cx, JS::HandleValue value, uint32_t* lineNumber,
...
JS_PUBLIC_API(bool)
JS_CheckForInterrupt(JSContext* cx) { return js::CheckForInterrupt(cx); }
...
JS_PUBLIC_API(void)
JS_ResetInterruptCallback(JSContext* cx, bool enable) { cx->interruptCallbackDisabled = enable; }
...
extern JS_PUBLIC_API(void) JS::SetPromiseRejectionTrackerCallback(
    JSContext* cx, JSPromiseRejectionTrackerCallback callback, void* data /* = nullptr */) {
...
JS_PUBLIC_API(bool)
JS::IsPromiseObject(JS::HandleObject obj) { return obj->is<PromiseObject>(); }
...
JS::GetPromiseState(JS::HandleObject promiseObj_) {
    ...
    if (!promiseObj || !promiseObj->is<PromiseObject>()) return JS::PromiseState::Pending;

    return promiseObj->as<PromiseObject>().state();
}

JS_PUBLIC_API(uint64_t)
JS::GetPromiseID(JS::HandleObject promise) { return promise->as<PromiseObject>().getID(); }
...
static bool ResolveOrRejectPromise(JSContext* cx, JS::HandleObject promiseObj,
                                   JS::HandleValue resultOrReason_, bool reject) {
    AssertHeapIsIdle();
    CHECK_REQUEST(cx);
...
static bool CallOriginalPromiseThenImpl(JSContext* cx, JS::HandleObject promiseObj,
...
JS_PUBLIC_API(bool)
JS::AddPromiseReactions(JSContext* cx, JS::HandleObject promiseObj, JS::HandleObject onResolvedObj,
...
JS_PUBLIC_API(JSObject*)
JS::NewReadableByteStreamObject(JSContext* cx, JS::HandleObject underlyingSource /* = nullptr */,
                                double highWaterMark /* = 1 */,
                                JS::HandleObject proto /* = nullptr */) {
    MOZ_ASSERT(!cx->runtime()->isAtomsCompartment(cx->compartment()));
    AssertHeapIsIdle();
...
JS_PUBLIC_API(bool)
JS::IsReadableStream(const JSObject* obj) { return obj->is<ReadableStream>(); }
...
JS_PUBLIC_API(bool)
JS::IsReadableStreamBYOBReader(const JSObject* obj) { return obj->is<ReadableStreamBYOBReader>(); }
...
JS::ReadableStreamGetMode(const JSObject* stream) { return stream->as<ReadableStream>().mode(); }
...
JS_PUBLIC_API(bool)
JS::ReadableStreamTee(JSContext* cx, HandleObject streamObj, MutableHandleObject branch1Obj,
    ...
    if (!ReadableStream::tee(cx, stream, false, &branch1Stream, &branch2Stream)) return false;
...
JS_PUBLIC_API(void)
JS::InitDispatchToEventLoop(JSContext* cx, JS::DispatchToEventLoopCallback callback,
                            void* closure) {
    cx->runtime()->offThreadPromiseState.ref().init(callback, closure);
}

JS_PUBLIC_API(void)
JS::ShutdownAsyncTasks(JSContext* cx) { cx->runtime()->offThreadPromiseState.ref().shutdown(cx); }
...
JS::AutoSetAsyncStackForNewCalls::~AutoSetAsyncStackForNewCalls() {
    ...
    cx->asyncStackForNewActivations() = oldAsyncStack ? &oldAsyncStack->as<SavedFrame>() : nullptr;
...
JS_PUBLIC_API(JSString*)
JS_AtomizeString(JSContext* cx, const char* s) { return JS_AtomizeStringN(cx, s, strlen(s)); }
...
JS_GetStringLength(JSString* str) { return str->length(); }

JS_PUBLIC_API(bool)
JS_StringIsFlat(JSString* str) { return str->isFlat(); }

JS_PUBLIC_API(bool)
JS_StringHasLatin1Chars(JSString* str) { return str->hasLatin1Chars(); }
...
JS_PUBLIC_API(const char16_t*)
JS_GetTwoByteExternalStringChars(JSString* str) { return str->asExternal().twoByteChars(); }
...
JS_GetFlatStringCharAt(JSFlatString* str, size_t index) { return str->latin1OrTwoByteChar(index); }
...
JS_PUBLIC_API(JSString*)
JS::GetSymbolDescription(HandleSymbol symbol) { return symbol->description(); }

JS_PUBLIC_API(JS::SymbolCode)
JS::GetSymbolCode(Handle<Symbol*> symbol) { return symbol->code(); }
...
JS_PUBLIC_API(bool)
JS_Stringify(JSContext* cx, MutableHandleValue vp, HandleObject replacer, HandleValue space,
    ...
    if (!Stringify(cx, vp, replacer, space, sb, StringifyBehavior::Normal)) return false;
    if (sb.empty() && !sb.append(cx->names().null)) return false;
    return callback(sb.rawTwoByteBegin(), sb.length(), data);
}

JS_PUBLIC_API(bool)
JS::ToJSONMaybeSafely(JSContext* cx, JS::HandleObject input, JSONWriteCallback callback,
                      void* data) {
    AssertHeapIsIdle();
    CHECK_REQUEST(cx);
...
JS_ParseJSON(JSContext* cx, const char16_t* chars, uint32_t len, MutableHandleValue vp) {
    AssertHeapIsIdle();
    CHECK_REQUEST(cx);
    return ParseJSONWithReviver(cx, mozilla::Range<const char16_t>(chars, len), NullHandleValue,
                                vp);
}
...
JS_PUBLIC_API(bool)
JS_ParseJSONWithReviver(JSContext* cx, const char16_t* chars, uint32_t len, HandleValue reviver,
                        MutableHandleValue vp) {
    AssertHeapIsIdle();
    CHECK_REQUEST(cx);
...
JS_PUBLIC_API(bool)
JS_ParseJSONWithReviver(JSContext* cx, HandleString str, HandleValue reviver,
                        MutableHandleValue vp) {
    AssertHeapIsIdle();
    CHECK_REQUEST(cx);
    assertSameCompartment(cx, str);
...
JS_ReportErrorNumberASCIIVA(JSContext* cx, JSErrorCallback errorCallback, void* userRef,
    ...
    AssertHeapIsIdle();
    ReportErrorNumberVA(cx, JSREPORT_ERROR, errorCallback, userRef, errorNumber, ArgumentsAreASCII,
                        ap);
}
...
JS_ReportErrorNumberUTF8VA(JSContext* cx, JSErrorCallback errorCallback, void* userRef,
                           const unsigned errorNumber, va_list ap) {
    AssertHeapIsIdle();
    ReportErrorNumberVA(cx, JSREPORT_ERROR, errorCallback, userRef, errorNumber, ArgumentsAreUTF8,
                        ap);
}
...
JS_ReportErrorNumberUCArray(JSContext* cx, JSErrorCallback errorCallback, void* userRef,
    ...
    AssertHeapIsIdle();
    ReportErrorNumberUCArray(cx, JSREPORT_ERROR, errorCallback, userRef, errorNumber, args);
}
...
JS_ReportErrorFlagsAndNumberASCII(JSContext* cx, unsigned flags, JSErrorCallback errorCallback,
    ...
    va_start(ap, errorNumber);
    ok =
        ReportErrorNumberVA(cx, flags, errorCallback, userRef, errorNumber, ArgumentsAreASCII, ap);
    va_end(ap);
    return ok;
}

JS_PUBLIC_API(bool)
JS_ReportErrorFlagsAndNumberLatin1(JSContext* cx, unsigned flags, JSErrorCallback errorCallback,
                                   void* userRef, const unsigned errorNumber, ...) {
    va_list ap;
    bool ok;
    ...
    AssertHeapIsIdle();
    va_start(ap, errorNumber);
    ok = ReportErrorNumberVA(cx, flags, errorCallback, userRef, errorNumber, ArgumentsAreLatin1,
                             ap);
    va_end(ap);
    return ok;
}
...
JS_ReportErrorFlagsAndNumberUTF8(JSContext* cx, unsigned flags, JSErrorCallback errorCallback,
    ...
    AssertHeapIsIdle();
    va_start(ap, errorNumber);
    ok = ReportErrorNumberVA(cx, flags, errorCallback, userRef, errorNumber, ArgumentsAreUTF8, ap);
    va_end(ap);
    return ok;
}
...
JS_ReportErrorFlagsAndNumberUC(JSContext* cx, unsigned flags, JSErrorCallback errorCallback,
    ...
    AssertHeapIsIdle();
    va_start(ap, errorNumber);
    ok = ReportErrorNumberVA(cx, flags, errorCallback, userRef, errorNumber, ArgumentsAreUnicode,
                             ap);
    va_end(ap);
    return ok;
}
...
JS_PUBLIC_API(void)
JS_ReportOutOfMemory(JSContext* cx) { ReportOutOfMemory(cx); }

JS_PUBLIC_API(void)
JS_ReportAllocationOverflow(JSContext* cx) { ReportAllocationOverflow(cx); }

JS_PUBLIC_API(JS::WarningReporter)
JS::GetWarningReporter(JSContext* cx) { return cx->runtime()->warningReporter; }
...
JS_NewRegExpObject(JSContext* cx, const char* bytes, size_t length, unsigned flags) {
    ...
    RegExpObject* reobj = RegExpObject::create(cx, chars, length, RegExpFlag(flags), nullptr,
                                               nullptr, cx->tempLifoAlloc(), GenericObject);
    return reobj;
}
...
JS_NewUCRegExpObject(JSContext* cx, const char16_t* chars, size_t length, unsigned flags) {
    AssertHeapIsIdle();
    CHECK_REQUEST(cx);
    return RegExpObject::create(cx, chars, length, RegExpFlag(flags), nullptr, nullptr,
...
void JSErrorReport::initBorrowedLinebuf(const char16_t* linebufArg, size_t linebufLengthArg,
...
static UniquePtr<JSErrorNotes::Note> CreateErrorNoteVA(JSContext* cx, const char* filename,
    ...
    if (!ExpandErrorArgumentsVA(cx, errorCallback, userRef, errorNumber, nullptr, argumentsType,
                                note.get(), ap)) {
        return nullptr;
    }
...
bool JSErrorNotes::addNoteASCII(JSContext* cx, const char* filename, unsigned lineno,
...
bool JSErrorNotes::addNoteLatin1(JSContext* cx, const char* filename, unsigned lineno,
                                 unsigned column, JSErrorCallback errorCallback, void* userRef,
                                 const unsigned errorNumber, ...) {
    va_list ap;
    va_start(ap, errorNumber);
...
bool JSErrorNotes::addNoteUTF8(JSContext* cx, const char* filename, unsigned lineno,
                               unsigned column, JSErrorCallback errorCallback, void* userRef,
                               const unsigned errorNumber, ...) {
    va_list ap;
    va_start(ap, errorNumber);
...
JS_PUBLIC_API(size_t)
JSErrorNotes::length() { return notes_.length(); }
...
JSErrorNotes::begin() { return iterator(notes_.begin()); }

JS_PUBLIC_API(JSErrorNotes::iterator)
JSErrorNotes::end() { return iterator(notes_.end()); }
...
extern MOZ_NEVER_INLINE JS_PUBLIC_API(void) JS_AbortIfWrongThread(JSContext* cx) {
...
JS_PUBLIC_API(void)
JS_ScheduleGC(JSContext* cx, uint32_t count) { cx->runtime()->gc.setNextScheduled(count); }
#endif
...
JS_PUBLIC_API(bool)
DescribeScriptedCaller(JSContext* cx, AutoFilename* filename, unsigned* lineno, unsigned* column) {
...
static bool GetScriptedCallerActivationFast(JSContext* cx, Activation** activation) {
    ...
    *activation = activationIter.activation();
    ...
    if (activationIter->isJit()) {
        for (OnlyJSJitFrameIter iter(activationIter); !iter.done(); ++iter) {
            if (iter.frame().isScripted() && !iter.frame().script()->selfHosted()) return true;
        }
...
JS::DecodeInterpretedFunction(JSContext* cx, TranscodeBuffer& buffer,
                              JS::MutableHandleFunction funp, size_t cursorIndex) {
    XDRDecoder decoder(cx, buffer, cursorIndex);
...
JS_PUBLIC_API(void)
JS::SetBuildIdOp(JSContext* cx, JS::BuildIdOp buildIdOp) { cx->runtime()->buildIdOp = buildIdOp; }
...
JS::FirstSubsumedFrame::FirstSubsumedFrame(JSContext* cx, bool ignoreSelfHostedFrames /* = true */)
...
JS_PUBLIC_API(bool)
JS::CopyAsyncStack(JSContext* cx, JS::HandleObject asyncStack, JS::HandleString asyncCause,
                   JS::MutableHandleObject stackp, unsigned maxFrameCount) {
    AssertHeapIsIdle();
    CHECK_REQUEST(cx);
    MOZ_RELEASE_ASSERT(cx->compartment());
    ...
    JSCompartment* compartment = cx->compartment();
    Rooted<SavedFrame*> frame(cx);
    if (!compartment->savedStacks().copyAsyncStack(cx, asyncStack, asyncCause, &frame,
                                                   maxFrameCount))
        return false;
    stackp.set(frame.get());
    return true;
...
JS_PUBLIC_API(Zone*)
JS::GetObjectZone(JSObject* obj) { return obj->zone(); }
...
js::GetStackFormat(JSContext* cx) { return cx->runtime()->stackFormat(); }
