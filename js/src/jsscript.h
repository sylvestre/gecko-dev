...
class YieldAndAwaitOffsetArray {
    friend bool detail::CopyScript(JSContext* cx, HandleScript src, HandleScript dst,
                                   MutableHandle<GCVector<Scope*>> scopes);
...
typedef HashMap<JSScript*, ScriptCounts*, DefaultHasher<JSScript*>, SystemAllocPolicy>
    ScriptCountsMap;
typedef HashMap<JSScript*, const char*, DefaultHasher<JSScript*>, SystemAllocPolicy> ScriptNameMap;
...
typedef HashMap<JSScript*, DebugScript*, DefaultHasher<JSScript*>, SystemAllocPolicy>
    DebugScriptMap;
...
struct ScriptSourceChunk {
    ...
    ScriptSourceChunk() : ss(nullptr), chunk(0) {}
...
class UncompressedSourceCache {
    typedef HashMap<ScriptSourceChunk, UniqueTwoByteChars, ScriptSourceChunkHasher,
                    SystemAllocPolicy>
        Map;
...
class ScriptSource {
   ...
   public:
    ...
    class PinnedChars {
       ...
       public:
        ...
        const char16_t* get() const { return chars_; }
    };
   ...
   private:
    ...
    struct Uncompressed {
        ...
        explicit Uncompressed(SharedImmutableTwoByteString&& str) : string(mozilla::Move(str)) {}
    };
    ...
    struct Compressed {
        ...
        Compressed(SharedImmutableString&& raw, size_t uncompressedLength)
            : raw(mozilla::Move(raw)), uncompressedLength(uncompressedLength) {}
    };
    ...
    const char16_t* chars(JSContext* cx, UncompressedSourceCache::AutoHoldEntry& asp, size_t begin,
                          size_t len);
   ...
   public:
    ...
    size_t length() const {
        struct LengthMatcher {
            size_t match(const Uncompressed& u) { return u.string.length(); }

            size_t match(const Compressed& c) { return c.uncompressedLength; }
    ...
    MOZ_MUST_USE bool appendSubstring(JSContext* cx, js::StringBuffer& buf, size_t start,
                                      size_t stop);
    ...
    MOZ_MUST_USE bool setSource(JSContext* cx, UniqueTwoByteChars&& source, size_t length);
    ...
    // ScriptSource.
    bool xdrEncodeFunction(JSContext* cx, HandleFunction fun, HandleScriptSource sourceObject);
...
class ScriptSourceObject : public NativeObject {
   ...
   public:
    ...
    JSObject* element() const { return getReservedSlot(ELEMENT_SLOT).toObjectOrNull(); }
    ...
    JSScript* introductionScript() const {
        if (getReservedSlot(INTRODUCTION_SCRIPT_SLOT).isUndefined()) return nullptr;
...
static inline unsigned AsyncKindAsBits(FunctionAsyncKind asyncKind) {
    return static_cast<unsigned>(asyncKind);
}

static inline FunctionAsyncKind AsyncKindFromBits(unsigned val) {
 ...
 */
template <XDRMode mode>
bool XDRScript(XDRState<mode>* xdr, HandleScope enclosingScope, HandleScriptSource sourceObject,
               HandleFunction fun, MutableHandleScript scriptp);

template <XDRMode mode>
bool XDRLazyScript(XDRState<mode>* xdr, HandleScope enclosingScope,
 ...
 */
template <XDRMode mode>
bool XDRScriptConst(XDRState<mode>* xdr, MutableHandleValue vp);
...
class SharedScriptData {
   ...
   public:
    static SharedScriptData* new_(JSContext* cx, uint32_t codeLength, uint32_t srcnotesLength,
                                  uint32_t natoms);
...
struct ScriptBytecodeHasher {
    ...
    static bool match(SharedScriptData* entry, const Lookup& lookup) {
        if (entry->dataLength() != lookup.length) return false;
        return mozilla::PodEqual<uint8_t>(entry->data(), lookup.data, lookup.length);
    }
};

typedef HashSet<SharedScriptData*, ScriptBytecodeHasher, SystemAllocPolicy> ScriptDataTable;

extern void SweepScriptData(JSRuntime* rt, AutoLockForExclusiveAccess& lock);

extern void FreeScriptData(JSRuntime* rt, AutoLockForExclusiveAccess& lock);

} /* namespace js */

class JSScript : public js::gc::TenuredCell {
    template <js::XDRMode mode>
    friend bool js::XDRScript(js::XDRState<mode>* xdr, js::HandleScope enclosingScope,
                              js::HandleScriptSource sourceObject, js::HandleFunction fun,
                              js::MutableHandleScript scriptp);

    friend bool js::detail::CopyScript(JSContext* cx, js::HandleScript src, js::HandleScript dst,
   ...
   public:
    ...
    enum ArrayKind { CONSTS, OBJECTS, TRYNOTES, SCOPENOTES, ARRAY_KIND_BITS };
   ...
   public:
    static JSScript* Create(JSContext* cx, const JS::ReadOnlyCompileOptions& options,
    ...
    static bool partiallyInit(JSContext* cx, JS::Handle<JSScript*> script, uint32_t nscopes,
   ...
   public:
    ...
    bool containsPC(const jsbytecode* pc) const { return pc >= code() && pc < codeEnd(); }
    ...
    bool functionHasParameterExprs() const {
        ...
        if (!scope->is<js::FunctionScope>()) return false;
    ...
    bool hasNonSyntacticScope() const { return hasNonSyntacticScope_; }
    ...
    bool isLikelyConstructorWrapper() const { return isLikelyConstructorWrapper_; }
    ...
    bool isDefaultClassConstructor() const { return isDefaultClassConstructor_; }
    ...
    bool argumentsAliasesFormals() const { return argumentsHasVarBinding() && hasMappedArgsObj(); }
    ...
    void setAsyncKind(js::FunctionAsyncKind kind) { isAsync_ = kind == js::AsyncFunction; }
    ...
    void setNeedsHomeObject() { needsHomeObject_ = true; }
    ...
    bool hasMappedArgsObj() const { return hasMappedArgsObj_; }
     ...
     */
    bool argsObjAliasesFormals() const { return needsArgsObj() && hasMappedArgsObj(); }
    ...
    void setDoNotRelazify(bool b) { doNotRelazify_ = b; }

    void setHasInnerFunctions(bool b) { hasInnerFunctions_ = b; }
    ...
    bool canBaselineCompile() const { return baseline != BASELINE_DISABLED_SCRIPT; }
    js::jit::BaselineScript* baselineScript() const {
        MOZ_ASSERT(hasBaselineScript());
        return baseline;
    }
    inline void setBaselineScript(JSRuntime* maybeRuntime,
                                  js::jit::BaselineScript* baselineScript);

    void updateBaselineOrIonRaw(JSRuntime* maybeRuntime);

    static size_t offsetOfBaselineScript() { return offsetof(JSScript, baseline); }
    static size_t offsetOfIonScript() { return offsetof(JSScript, ion); }
    static size_t offsetOfBaselineOrIonRaw() { return offsetof(JSScript, baselineOrIonRaw); }
    uint8_t* baselineOrIonRawPointer() const { return baselineOrIonRaw; }
    ...
    bool isRelazifiable() const {
        return (selfHosted() || lazyScript) && !hasInnerFunctions_ && !types_ &&
               ...
               !isDefaultClassConstructor() && !hasBaselineScript() && !hasAnyIonScript() &&
               !doNotRelazify_;
    }
    ...
    js::ModuleObject* module() const {
        if (bodyScope()->is<js::ModuleScope>()) return bodyScope()->as<js::ModuleScope>().module();
        return nullptr;
    }
   ...
   public:
    ...
    js::VarScope* functionExtraBodyVarScope() const {
        ...
        for (uint32_t i = 0; i < scopes()->length; i++) {
            ...
            if (scope->kind() == js::ScopeKind::FunctionBodyVar) return &scope->as<js::VarScope>();
        }
    ...
    bool needsBodyEnvironment() const {
        for (uint32_t i = 0; i < scopes()->length; i++) {
            js::Scope* scope = getScope(i);
            if (ScopeKindIsInBody(scope->kind()) && scope->hasEnvironment()) return true;
        }
        return false;
    }
   ...
   public:
    ...
    bool hasArray(ArrayKind kind) const { return hasArrayBits & (1 << kind); }
...
#undef OFF
    ...
    js::PropertyName* getName(size_t index) { return getAtom(index)->asPropertyName(); }
    ...
    js::Scope* getScope(jsbytecode* pc) const {
        ...
        // use lookupScope.
        MOZ_ASSERT(containsPC(pc) && containsPC(pc + sizeof(uint32_t)));
        MOZ_ASSERT(js::JOF_OPTYPE(JSOp(*pc)) == JOF_SCOPE, "Did you mean to use lookupScope(pc)?");
        return getScope(GET_UINT32_INDEX(pc));
    }

    JSVersion getVersion() const { return JSVersion(version); }

    inline JSFunction* getFunction(size_t index);
    JSFunction* function() const {
        if (functionNonDelazifying()) return functionNonDelazifying();
        return nullptr;
    }
    ...
    bool isEmpty() const {
        ...
        jsbytecode* pc = code();
        if (noScriptRval() && JSOp(*pc) == JSOP_FALSE) ++pc;
...
class LazyScript : public gc::TenuredCell {
   ...
   private:
    ...
    LazyScript(JSFunction* fun, void* table, uint64_t packedFields, uint32_t begin, uint32_t end,
               uint32_t toStringStart, uint32_t lineno, uint32_t column);

    // Create a LazyScript without initializing the closedOverBindings and the
    // innerFunctions. To be GC-safe, the caller must initialize both vectors
    // with valid atoms and functions.
    static LazyScript* CreateRaw(JSContext* cx, HandleFunction fun, uint64_t packedData,
   ...
   public:
    ...
    static LazyScript* Create(JSContext* cx, HandleFunction fun,
                              ...
                              Handle<GCVector<JSFunction*, 8>> innerFunctions, JSVersion version,
                              uint32_t begin, uint32_t end, uint32_t toStringStart,
                              uint32_t lineno, uint32_t column);
    ...
    static LazyScript* Create(JSContext* cx, HandleFunction fun, HandleScript script,
    ...
    const JSScript* maybeScriptUnbarriered() const { return script_.unbarrieredGet(); }
    ...
    bool isExprBody() const { return p_.isExprBody; }
    void setIsExprBody() { p_.isExprBody = true; }

    bool strict() const { return p_.strict; }
    void setStrict() { p_.strict = true; }

    bool bindingsAccessedDynamically() const { return p_.bindingsAccessedDynamically; }
    void setBindingsAccessedDynamically() { p_.bindingsAccessedDynamically = true; }
    ...
    bool isLikelyConstructorWrapper() const { return p_.isLikelyConstructorWrapper; }
    ...
    bool isDerivedClassConstructor() const { return p_.isDerivedClassConstructor; }
    ...
    size_t sizeOfExcludingThis(mozilla::MallocSizeOf mallocSizeOf) { return mallocSizeOf(table_); }
...
struct ScriptAndCounts {
    ...
    void trace(JSTracer* trc) { TraceRoot(trc, &script, "ScriptAndCounts::script"); }
};
...
extern unsigned PCToLineNumber(JSScript* script, jsbytecode* pc, unsigned* columnp = nullptr);

extern unsigned PCToLineNumber(unsigned startLine, jssrcnote* notes, jsbytecode* code,
                               jsbytecode* pc, unsigned* columnp = nullptr);
...
enum LineOption { CALLED_FROM_JSOP_EVAL, NOT_CALLED_FROM_JSOP_EVAL };

extern void DescribeScriptedCallerForCompilation(JSContext* cx, MutableHandleScript maybeScript,
                                                 const char** file, unsigned* linenop,
                                                 uint32_t* pcOffset, bool* mutedErrors,
                                                 LineOption opt = NOT_CALLED_FROM_JSOP_EVAL);

JSScript* CloneScriptIntoFunction(JSContext* cx, HandleScope enclosingScope, HandleFunction fun,
                                  HandleScript src);

JSScript* CloneGlobalScript(JSContext* cx, ScopeKind scopeKind, HandleScript src);
