...
void PerformanceMonitoring::start() {
    ...
    if (stopwatchStartCallback) stopwatchStartCallback(iteration_, stopwatchStartClosure);
}
...
AutoStopwatch::~AutoStopwatch() {
    ...
    for (auto group = groups_.begin(); group < groups_.end(); group++) releaseGroup(*group);
}
...
void AutoStopwatch::updateTelemetry(const cpuid_t& cpuStart_, const cpuid_t& cpuEnd) {
    JSRuntime* runtime = cx_->runtime();
...
bool AutoStopwatch::addToGroup(JSRuntime* runtime, uint64_t cyclesDelta, uint64_t CPOWTimeDelta,
...
uint64_t AutoStopwatch::getDelta(const uint64_t end, const uint64_t start) const {
...
bool PerformanceGroup::isAcquired(uint64_t it, const AutoStopwatch* owner) const {
...
void PerformanceGroup::setIsUsedInThisIteration(bool value) { isUsedInThisIteration_ = value; }
bool PerformanceGroup::isUsedInThisIteration() const { return isUsedInThisIteration_; }
