...
template <bool Eq>
static MOZ_ALWAYS_INLINE bool LooseEqualityOp(JSContext* cx, InterpreterRegs& regs) {
...
bool js::BoxNonStrictThis(JSContext* cx, HandleValue thisv, MutableHandleValue vp) {
    /*
...
bool js::GetFunctionThis(JSContext* cx, AbstractFramePtr frame, MutableHandleValue res) {
...
void js::GetNonSyntacticGlobalThis(JSContext* cx, HandleObject envChain, MutableHandleValue res) {
...
static inline bool GetPropertyOperation(JSContext* cx, InterpreterFrame* fp, HandleScript script,
                                        jsbytecode* pc, MutableHandleValue lval,
...
static inline bool GetNameOperation(JSContext* cx, InterpreterFrame* fp, jsbytecode* pc,
                                    MutableHandleValue vp) {
    ...
    if (op2 == JSOP_TYPEOF) return GetEnvironmentName<GetNameMode::TypeOf>(cx, envChain, name, vp);
    return GetEnvironmentName<GetNameMode::Normal>(cx, envChain, name, vp);
}

static inline bool GetImportOperation(JSContext* cx, InterpreterFrame* fp, jsbytecode* pc,
                                      MutableHandleValue vp) {
...
static bool SetPropertyOperation(JSContext* cx, JSOp op, HandleValue lval, HandleId id,
                                 HandleValue rval) {
...
JSFunction* js::MakeDefaultConstructor(JSContext* cx, HandleScript script, jsbytecode* pc,
                                       HandleObject proto) {
    JSOp op = JSOp(*pc);
...
static bool MaybeCreateThisForConstructor(JSContext* cx, JSScript* calleeScript,
                                          const CallArgs& args, bool createSingleton) {
...
InterpreterFrame* ExecuteState::pushInterpreterFrame(JSContext* cx) {
    return cx->interpreterStack().pushExecuteFrame(cx, script_, newTargetValue_, envChain_,
                                                   evalInFrame_);
}
...
bool js::RunScript(JSContext* cx, RunState& state) {
    ...
    MOZ_DIAGNOSTIC_ASSERT(cx->compartment()->isSystem() || cx->runtime()->allowContentJS());
 ...
 *       this step already!
 */
bool js::InternalCallOrConstruct(JSContext* cx, const CallArgs& args, MaybeConstruct construct) {
    ...
    if (MOZ_UNLIKELY(!args.callee().is<JSFunction>())) {
        ...
        if (!call) return ReportIsNotFunction(cx, args.calleev(), skipForCallee, construct);
    ...
    if (construct) {
        ...
        if (JSScript* script = cx->currentScript(&pc)) {
            if (ObjectGroup::useSingletonForNewObject(cx, script, pc)) createSingleton = true;
        }
...
bool js::Call(JSContext* cx, HandleValue fval, HandleValue thisv, const AnyInvokeArgs& args,
...
static bool InternalConstruct(JSContext* cx, const AnyConstructArgs& args) {
    ...
    if (callee.is<JSFunction>()) {
        ...
        if (fun->isNative()) return CallJSNativeConstructor(cx, fun->native(), args);
...
static bool StackCheckIsConstructorCalleeNewTarget(JSContext* cx, HandleValue callee,
...
bool js::Construct(JSContext* cx, HandleValue fval, const AnyConstructArgs& args,
                   HandleValue newTarget, MutableHandleObject objp) {
...
bool js::InternalConstructWithProvidedThis(JSContext* cx, HandleValue fval, HandleValue thisv,
...
bool js::CallSetter(JSContext* cx, HandleValue thisv, HandleValue setter, HandleValue v) {
    if (!CheckRecursionLimit(cx)) return false;
...
bool js::ExecuteKernel(JSContext* cx, HandleScript script, JSObject& envChainArg,
                       const Value& newTargetValue, AbstractFramePtr evalInFrame, Value* result) {
    MOZ_ASSERT_IF(script->isGlobalCode(), IsGlobalLexicalEnvironment(&envChainArg) ||
    ...
    MOZ_ASSERT(terminatingEnv->is<GlobalObject>() || script->hasNonSyntacticScope());
#endif
...
bool js::Execute(JSContext* cx, HandleScript script, JSObject& envChainArg, Value* rval) {
    ...
    return ExecuteKernel(cx, script, *envChain, NullValue(), NullFramePtr() /* evalInFrame */,
                         rval);
}
 ...
 */
extern bool js::InstanceOfOperator(JSContext* cx, HandleObject obj, HandleValue v, bool* bp) {
...
static inline bool EqualGivenSameType(JSContext* cx, HandleValue lval, HandleValue rval,
                                      bool* equal) {
    MOZ_ASSERT(SameType(lval, rval));

    if (lval.isString()) return EqualStrings(cx, lval.toString(), rval.toString(), equal);
    if (lval.isDouble()) {
...
static inline bool LooselyEqualBooleanAndOther(JSContext* cx, HandleValue lval, HandleValue rval,
                                               bool* result) {
...
bool js::LooselyEqual(JSContext* cx, HandleValue lval, HandleValue rval, bool* result) {
    // Step 3.
    ...
    if (lval.isNullOrUndefined()) {
        ...
        *result =
            rval.isNullOrUndefined() || (rval.isObject() && EmulatesUndefined(&rval.toObject()));
        return true;
    }
    ...
    // Step 8.
    if (lval.isBoolean()) return LooselyEqualBooleanAndOther(cx, lval, rval, result);

    // Step 9.
    if (rval.isBoolean()) return LooselyEqualBooleanAndOther(cx, rval, lval, result);
...
bool js::StrictlyEqual(JSContext* cx, HandleValue lval, HandleValue rval, bool* equal) {
...
static inline bool IsNaN(const Value& v) { return v.isDouble() && mozilla::IsNaN(v.toDouble()); }
...
JSObject* js::FunWithProtoOperation(JSContext* cx, HandleFunction fun, HandleObject parent,
                                    HandleObject proto) {
 ...
 */
bool js::EnterWithOperation(JSContext* cx, AbstractFramePtr frame, HandleValue val,
...
static void PopEnvironment(JSContext* cx, EnvironmentIter& ei) {
    switch (ei.scope().kind()) {
        ...
        case ScopeKind::StrictEval:
            if (MOZ_UNLIKELY(cx->compartment()->isDebuggee())) DebugEnvironments::onPopVar(cx, ei);
            if (ei.scope().hasEnvironment())
...
static bool ForcedReturn(JSContext* cx, EnvironmentIter& ei, InterpreterRegs& regs,
                         bool frameOk = true) {
...
class InterpreterFrameStackDepthOp {
   ...
   public:
    explicit InterpreterFrameStackDepthOp(const InterpreterRegs& regs) : regs_(regs) {}
...
static void UnwindIteratorsForUncatchableException(JSContext* cx, const InterpreterRegs& regs) {
...
static HandleErrorContinuation ProcessTryNotes(JSContext* cx, EnvironmentIter& ei,
                                               InterpreterRegs& regs) {
...
bool js::HandleClosingGeneratorReturn(JSContext* cx, AbstractFramePtr frame, bool ok) {
    /*
...
static HandleErrorContinuation HandleError(JSContext* cx, InterpreterRegs& regs) {
        ...
        if (!cx->isClosingGenerator()) {
            ...
            switch (status) {
                ...
                case JSTRAP_RETURN:
                    ...
                    if (!ForcedReturn(cx, ei, regs)) return ErrorReturnContinuation;
                    return SuccessfulReturnContinuation;
...
static MOZ_ALWAYS_INLINE bool AddOperation(JSContext* cx, MutableHandleValue lhs,
...
static MOZ_ALWAYS_INLINE bool SubOperation(JSContext* cx, HandleValue lhs, HandleValue rhs,
...
static MOZ_ALWAYS_INLINE bool MulOperation(JSContext* cx, HandleValue lhs, HandleValue rhs,
                                           MutableHandleValue res) {
    double d1, d2;
    if (!ToNumber(cx, lhs, &d1) || !ToNumber(cx, rhs, &d2)) return false;
    res.setNumber(d1 * d2);
...
static MOZ_ALWAYS_INLINE bool DivOperation(JSContext* cx, HandleValue lhs, HandleValue rhs,
                                           MutableHandleValue res) {
    double d1, d2;
    if (!ToNumber(cx, lhs, &d1) || !ToNumber(cx, rhs, &d2)) return false;
...
static MOZ_ALWAYS_INLINE bool ModOperation(JSContext* cx, HandleValue lhs, HandleValue rhs,
                                           MutableHandleValue res) {
    int32_t l, r;
    if (lhs.isInt32() && rhs.isInt32() && (l = lhs.toInt32()) >= 0 && (r = rhs.toInt32()) > 0) {
        int32_t mod = l % r;
        res.setInt32(mod);
        return true;
    }
...
static MOZ_ALWAYS_INLINE bool SetObjectElementOperation(JSContext* cx, HandleObject obj,
    ...
    if (obj->isNative() && JSID_IS_ATOM(id) && !obj->as<NativeObject>().inDictionaryMode() &&
        !obj->as<NativeObject>().hadElementsAccess() &&
        obj->as<NativeObject>().slotSpan() > PropertyTree::MAX_HEIGHT_WITH_ELEMENTS_ACCESS / 3) {
        if (!NativeObject::setHadElementsAccess(cx, obj.as<NativeObject>())) return false;
    }
...
class ReservedRooted : public RootedBase<T, ReservedRooted<T>> {
   ...
   public:
    ReservedRooted(Rooted<T>* root, const T& ptr) : savedRoot(root) { *root = ptr; }
 ...
 */
#if (defined(__GNUC__) || (__IBMC__ >= 700 && defined __IBM_COMPUTED_GOTO) || __SUNPRO_C >= 0x570)
...
#define TRAILING_LABEL(v) \
    ((v) == EnableInterruptsPseudoOpcode ? LABEL(EnableInterruptsPseudoOpcode) : LABEL(default)),
...
#define SANITY_CHECKS()              \
    ...
    INTERPRETER_LOOP() {
        ...
        CASE(JSOP_ENTERWITH) {
            ...
            if (!EnterWithOperation(cx, REGS.fp(), val, scope.as<WithScope>())) goto error;
        }
...
#undef SIGNED_SHIFT_OP
        ...
        CASE(JSOP_NEG) {
            ...
            if (!NegOperation(cx, script, REGS.pc, val, res)) goto error;
        }
        END_CASE(JSOP_NEG)
        ...
        CASE(JSOP_TOID) {
            ...
            MutableHandleValue res = REGS.stackHandleAt(-1);
            if (!ToIdOperation(cx, script, REGS.pc, idval, res)) goto error;
        }
        END_CASE(JSOP_TOID)

        CASE(JSOP_TYPEOFEXPR)
        CASE(JSOP_TYPEOF) { REGS.sp[-1].setString(TypeOfOperation(REGS.sp[-1], cx->runtime())); }
        ...
        CASE(JSOP_CALLPROP) {
            ...
            if (!GetPropertyOperation(cx, REGS.fp(), script, REGS.pc, lval, lval)) goto error;
        ...
        CASE(JSOP_GETPROP_SUPER) {
            ...
            if (!GetProperty(cx, obj, receiver, script->getName(REGS.pc), rref)) goto error;
        ...
        CASE(JSOP_STRICTSETPROP) {
            ...
            ReservedRooted<jsid> id(&rootId0, NameToId(script->getName(REGS.pc)));
            if (!SetPropertyOperation(cx, JSOp(*REGS.pc), lval, id, rval)) goto error;
        ...
        CASE(JSOP_STRICTSETPROP_SUPER) {
            ...
            if (!SetPropertySuper(cx, obj, receiver, name, rval, strict)) goto error;
        ...
        CASE(JSOP_CALLELEM) {
            ...
            bool done = false;
            if (!GetElemOptimizedArguments(cx, REGS.fp(), lval, rval, res, &done)) goto error;

            if (!done) {
                if (!GetElementOperation(cx, JSOp(*REGS.pc), lval, rval, res)) goto error;
            }
        ...
        CASE(JSOP_STRICTSETELEM_SUPER) {
            ...
            if (!SetObjectElement(cx, obj, index, value, receiver, strict)) goto error;
            REGS.sp[-4] = value;
            REGS.sp -= 3;
        }
        ...
        CASE(JSOP_STRICTSPREADEVAL) {
            ...
            bool construct =
                JSOp(*REGS.pc) == JSOP_SPREADNEW || JSOp(*REGS.pc) == JSOP_SPREADSUPERCALL;
            ;
        ...
        CASE(JSOP_FUNCALL) {
...

            {
                ...
                ReservedRooted<JSScript*> funScript(&rootScript0,
        ...
        CASE(JSOP_CALLSITEOBJ) {
            ...
            ReservedRooted<JSObject*> raw(&rootObject1,
                                          script->getObject(GET_UINT32_INDEX(REGS.pc) + 1));
        ...
        CASE(JSOP_GETALIASEDVAR) {
            ...
            ReservedRooted<Value> val(&rootValue0,
        ...
        CASE(JSOP_DEFLET) {
            ...
            if (!DefLexicalOperation(cx, lexicalEnv, varObj, script, REGS.pc)) goto error;
        }
        ...
        CASE(JSOP_DEFFUN) {
            ...
            if (!DefFunOperation(cx, script, REGS.fp()->environmentChain(), fun)) goto error;
            REGS.sp--;
        }
        ...
        CASE(JSOP_LAMBDA) {
            ...
            ReservedRooted<JSFunction*> fun(&rootFunction0,
                                            script->getFunction(GET_UINT32_INDEX(REGS.pc)));
        ...
        CASE(JSOP_NEWARRAY_COPYONWRITE) {
            ReservedRooted<JSObject*> baseobj(
                &rootObject0, ObjectGroup::getOrFixupCopyOnWriteObject(cx, script, REGS.pc));
        ...
        CASE(JSOP_INITHIDDENPROP) {
            ...
            if (!InitPropertyOperation(cx, JSOp(*REGS.pc), obj, id, rval)) goto error;
        ...
        CASE(JSOP_PUSHLEXICALENV) {
            ...
            if (!REGS.fp()->pushLexicalEnvironment(cx, scope.as<LexicalScope>())) goto error;
        }
        ...
        CASE(JSOP_INITIALYIELD) {
            ...
            POP_RETURN_VALUE();
            MOZ_ASSERT(REGS.stackDepth() == 0);
            if (!GeneratorObject::initialSuspend(cx, obj, REGS.fp(), REGS.pc)) goto error;
            goto successful_return_continuation;
        }
        ...
        CASE(JSOP_ARRAYPUSH) {
            ReservedRooted<JSObject*> obj(&rootObject0, &REGS.sp[-1].toObject());
            if (!NewbornArrayPush(cx, obj, REGS.sp[-2])) goto error;
            REGS.sp -= 2;
        }
        ...
        CASE(JSOP_BUILTINPROTO) {
            ...
            if (!GetBuiltinPrototype(cx, key, &builtin)) goto error;
            PUSH_OBJECT(*builtin);
        }
        ...
        CASE(JSOP_INITHOMEOBJECT) {
            ...
            ReservedRooted<JSFunction*> func(&rootFunction0,
                                             &REGS.sp[-1].toObject().as<JSFunction>());
        ...
        CASE(JSOP_CHECKOBJCOERCIBLE) {
            ...
            if (checkVal.isNullOrUndefined() && !ToObjectFromStack(cx, checkVal)) goto error;
        }
...
leave_on_safe_point:
    ...
    if (interpReturnOK) state.setReturnValue(activation.entryFrame()->returnValue());
...
JSObject* js::LambdaArrow(JSContext* cx, HandleFunction fun, HandleObject parent,
                          HandleValue newTargetv) {
    MOZ_ASSERT(fun->isArrow());

    JSFunction* clone = CloneFunctionObjectIfNotSingleton(cx, fun, parent, nullptr, TenuredObject);
    if (!clone) return nullptr;
...
bool js::DefFunOperation(JSContext* cx, HandleScript script, HandleObject envChain,
                         HandleFunction fun) {
    /*
     ...
     */
    unsigned attrs =
        script->isActiveEval() ? JSPROP_ENUMERATE : JSPROP_ENUMERATE | JSPROP_PERMANENT;
...
bool js::DeletePropertyJit(JSContext* cx, HandleValue v, HandlePropertyName name, bool* bp) {
    RootedObject obj(cx, ToObjectFromStack(cx, v));
    if (!obj) return false;
...
template <bool strict>
bool js::DeleteElementJit(JSContext* cx, HandleValue val, HandleValue index, bool* bp) {
...
bool js::SetObjectElement(JSContext* cx, HandleObject obj, HandleValue index, HandleValue value,
                          bool strict) {
    RootedId id(cx);
    if (!ToPropertyKey(cx, index, &id)) return false;
...
bool js::InitElementArray(JSContext* cx, jsbytecode* pc, HandleObject obj, uint32_t index,
                          HandleValue value) {
    return InitArrayElemOperation(cx, pc, obj, index, value);
}

bool js::AddValues(JSContext* cx, MutableHandleValue lhs, MutableHandleValue rhs,
                   MutableHandleValue res) {
    return AddOperation(cx, lhs, rhs, res);
}

bool js::SubValues(JSContext* cx, MutableHandleValue lhs, MutableHandleValue rhs,
                   MutableHandleValue res) {
    return SubOperation(cx, lhs, rhs, res);
}

bool js::MulValues(JSContext* cx, MutableHandleValue lhs, MutableHandleValue rhs,
                   MutableHandleValue res) {
    return MulOperation(cx, lhs, rhs, res);
}

bool js::DivValues(JSContext* cx, MutableHandleValue lhs, MutableHandleValue rhs,
                   MutableHandleValue res) {
    return DivOperation(cx, lhs, rhs, res);
}

bool js::ModValues(JSContext* cx, MutableHandleValue lhs, MutableHandleValue rhs,
                   MutableHandleValue res) {
    return ModOperation(cx, lhs, rhs, res);
}

bool js::UrshValues(JSContext* cx, MutableHandleValue lhs, MutableHandleValue rhs,
                    MutableHandleValue res) {
...
bool js::DeleteNameOperation(JSContext* cx, HandlePropertyName name, HandleObject scopeObj,
                             MutableHandleValue res) {
...
bool js::ImplicitThisOperation(JSContext* cx, HandleObject scopeObj, HandlePropertyName name,
                               MutableHandleValue res) {
    RootedObject obj(cx);
...
bool js::InitGetterSetterOperation(JSContext* cx, jsbytecode* pc, HandleObject obj, HandleId id,
                                   HandleObject val) {
...
bool js::InitGetterSetterOperation(JSContext* cx, jsbytecode* pc, HandleObject obj,
...
bool js::SpreadCallOperation(JSContext* cx, HandleScript script, jsbytecode* pc, HandleValue thisv,
                             HandleValue callee, HandleValue arr, HandleValue newTarget,
                             MutableHandleValue res) {
    ...
    if (length > ARGS_LENGTH_MAX) {
        JS_ReportErrorNumberASCII(
            ...
            constructing ? JSMSG_TOO_MANY_CON_SPREADARGS : JSMSG_TOO_MANY_FUN_SPREADARGS);
        return false;
    }
    ...
    if (constructing) {
        if (!StackCheckIsConstructorCalleeNewTarget(cx, callee, newTarget)) return false;
        ...
        if ((op == JSOP_SPREADEVAL || op == JSOP_STRICTSPREADEVAL) &&
            ...
            MOZ_ASSERT(
                op == JSOP_SPREADCALL || op == JSOP_SPREADEVAL || op == JSOP_STRICTSPREADEVAL,
                "bad spread opcode");
...
JSObject* js::NewObjectOperation(JSContext* cx, HandleScript script, jsbytecode* pc,
    ...
    if (ObjectGroup::useSingletonForAllocationSite(script, pc, JSProto_Object)) {
        ...
        if (group->shouldPreTenure() || group->maybePreliminaryObjects()) newKind = TenuredObject;

        if (group->maybeUnboxedLayout()) return UnboxedPlainObject::create(cx, group, newKind);
    }
...
JSObject* js::NewObjectOperationWithTemplate(JSContext* cx, HandleObject templateObject) {
    ...
    NewObjectKind newKind =
        templateObject->group()->shouldPreTenure() ? TenuredObject : GenericObject;
...
JSObject* js::NewArrayOperation(JSContext* cx, HandleScript script, jsbytecode* pc,
                                uint32_t length, NewObjectKind newKind /* = GenericObject */) {
    MOZ_ASSERT(newKind != SingletonObject);
...
JSObject* js::NewArrayOperationWithTemplate(JSContext* cx, HandleObject templateObject) {
    MOZ_ASSERT(!templateObject->isSingleton());
...
void js::ReportRuntimeLexicalError(JSContext* cx, unsigned errorNumber, HandleId id) {
...
void js::ReportRuntimeLexicalError(JSContext* cx, unsigned errorNumber, HandlePropertyName name) {
    RootedId id(cx, NameToId(name));
    ReportRuntimeLexicalError(cx, errorNumber, id);
}

void js::ReportRuntimeLexicalError(JSContext* cx, unsigned errorNumber, HandleScript script,
                                   jsbytecode* pc) {
    JSOp op = JSOp(*pc);
...
void js::ReportRuntimeRedeclaration(JSContext* cx, HandlePropertyName name,
    ...
    if (AtomToPrintableString(cx, name, &printable)) {
        JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, JSMSG_REDECLARED_VAR, redeclKind,
                                   printable.ptr());
    }
}
...
JSObject* js::HomeObjectSuperBase(JSContext* cx, HandleObject homeObj) {
    ...
    if (!superBase) {
        JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr, JSMSG_CANT_CONVERT_TO, "null",
                                  "object");
        return nullptr;
    }
...
bool js::ThrowInitializedThis(JSContext* cx, AbstractFramePtr frame) {
