...
const Class DebuggerFrame::class_ = {"Frame",
                                     JSCLASS_HAS_PRIVATE |
...
const Class DebuggerArguments::class_ = {"Arguments",
...
const Class DebuggerEnvironment::class_ = {
    "Environment",
    JSCLASS_HAS_PRIVATE | JSCLASS_HAS_RESERVED_SLOTS(DebuggerEnvironment::RESERVED_SLOTS),
    &classOps_};
...
const Class DebuggerObject::class_ = {
    "Object", JSCLASS_HAS_PRIVATE | JSCLASS_HAS_RESERVED_SLOTS(RESERVED_SLOTS), &classOps_};
...
enum { JSSLOT_DEBUGSOURCE_OWNER, JSSLOT_DEBUGSOURCE_TEXT, JSSLOT_DEBUGSOURCE_COUNT };
...
static inline JSScript* GetOrCreateFunctionScript(JSContext* cx, HandleFunction fun) {
    MOZ_ASSERT(fun->isInterpreted());
    if (!EnsureFunctionHasScript(cx, fun)) return nullptr;
    return fun->nonLazyScript();
}

static bool ValueToIdentifier(JSContext* cx, HandleValue v, MutableHandleId id) {
    if (!ValueToId<CanGC>(cx, v, id)) return false;
    if (!JSID_IS_ATOM(id) || !IsIdentifier(JSID_TO_ATOM(id))) {
        RootedValue val(cx, v);
        ReportValueErrorFlags(cx, JSREPORT_ERROR, JSMSG_UNEXPECTED_TYPE, JSDVG_SEARCH_STACK, val,
                              nullptr, "not an identifier", nullptr);
        return false;
    }
    return true;
}
        ...
        if (EnterDebuggeeNoExecute* nx = findInStack(cx)) {
            ...
            if (!warning || !nx->reported_) {
                ...
                return JS_ReportErrorFlagsAndNumberLatin1(cx, flags, GetErrorMessage, nullptr,
                                                          JSMSG_DEBUGGEE_WOULD_RUN, filename,
                                                          linenoStr);
            }
        }
        return true;
...
/* static */ bool Debugger::slowPathCheckNoExecute(JSContext* cx, HandleScript script) {
...
static bool ValueToStableChars(JSContext* cx, const char* fnname, HandleValue value,
                               AutoStableStringChars& stableChars) {
    if (!value.isString()) {
        JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr, JSMSG_NOT_EXPECTED_TYPE, fnname,
                                  "string", InformalValueTypeName(value));
        return false;
    }
...
static bool ParseEvalOptions(JSContext* cx, HandleValue value, EvalOptions& options) {
...
static bool RequireGlobalObject(JSContext* cx, HandleValue dbgobj, HandleObject referent) {
    ...
    if (!obj->is<GlobalObject>()) {
        ...
        if (obj->is<GlobalObject>()) {
            ...
            ReportValueErrorFlags(cx, JSREPORT_ERROR, JSMSG_DEBUG_BAD_REFERENT, JSDVG_SEARCH_STACK,
                                  dbgobj, nullptr, "a global object", nullptr);
        }
        return false;
    }
...
void JSBreakpointSite::recompile(FreeOp* fop) {
    if (script->hasBaselineScript()) script->baselineScript()->toggleDebugTraps(script, pc);
}
...
Debugger::~Debugger() {
    ...
    if (onNewGlobalObjectWatchersLink.mPrev || onNewGlobalObjectWatchersLink.mNext ||
...
bool Debugger::init(JSContext* cx) {
    if (!debuggees.init() || !debuggeeZones.init() || !frames.init() || !scripts.init() ||
        !sources.init() || !objects.init() || !observedGCs.init() || !environments.init() ||
...
/* static */ Debugger* Debugger::fromChildJSObject(JSObject* obj) {
    MOZ_ASSERT(
        obj->getClass() == &DebuggerFrame::class_ || obj->getClass() == &DebuggerScript_class ||
        obj->getClass() == &DebuggerSource_class || obj->getClass() == &DebuggerObject::class_ ||
        obj->getClass() == &DebuggerEnvironment::class_);
    JSObject* dbgobj =
        &obj->as<NativeObject>().getReservedSlot(JSSLOT_DEBUGOBJECT_OWNER).toObject();
...
bool Debugger::getScriptFrameWithIter(JSContext* cx, AbstractFramePtr referent,
    ...
    if (!Debugger::getScriptFrameWithIter(cx, referent, maybeIter, &result)) return false;
...
bool Debugger::getScriptFrameWithIter(JSContext* cx, AbstractFramePtr referent,
    ...
    if (referent.hasScript() && !referent.script()->ensureHasAnalyzedArgsUsage(cx)) return false;
    ...
    if (!p) {
        ...
        RootedDebuggerFrame frame(cx,
                                  DebuggerFrame::create(cx, proto, referent, maybeIter, debugger));
...
bool Debugger::hasAnyLiveHooks(JSRuntime* rt) const {
    ...
    if (getHook(OnDebuggerStatement) || getHook(OnExceptionUnwind) || getHook(OnNewScript) ||
...
/* static */ JSTrapStatus Debugger::slowPathOnEnterFrame(JSContext* cx, AbstractFramePtr frame) {
    RootedValue rval(cx);
    JSTrapStatus status = dispatchHook(
        ...
        [&](Debugger* dbg) -> JSTrapStatus { return dbg->fireEnterFrame(cx, &rval); });
...
static void DebuggerFrame_maybeDecrementFrameScriptStepModeCount(FreeOp* fop,
 ...
 */
/* static */ bool Debugger::slowPathOnLeaveFrame(JSContext* cx, AbstractFramePtr frame,
...
/* static */ JSTrapStatus Debugger::slowPathOnDebuggerStatement(JSContext* cx,
                                                                AbstractFramePtr frame) {
    RootedValue rval(cx);
    JSTrapStatus status = dispatchHook(
        cx, [](Debugger* dbg) -> bool { return dbg->getHook(OnDebuggerStatement); },
        [&](Debugger* dbg) -> JSTrapStatus { return dbg->fireDebuggerStatement(cx, &rval); });
...
/* static */ JSTrapStatus Debugger::slowPathOnExceptionUnwind(JSContext* cx,
                                                              AbstractFramePtr frame) {
    // Invoking more JS on an over-recursed stack or after OOM is only going
    // to result in more of the same error.
    if (cx->isThrowingOverRecursed() || cx->isThrowingOutOfMemory()) return JSTRAP_CONTINUE;
    ...
    JSTrapStatus status = dispatchHook(
        ...
        [&](Debugger* dbg) -> JSTrapStatus { return dbg->fireExceptionUnwind(cx, &rval); });
...
bool Debugger::wrapEnvironment(JSContext* cx, Handle<Env*> env, MutableHandleValue rval) {
    if (!env) {
        rval.setNull();
        return true;
    }
...
static NativeObject* ToNativeDebuggerObject(JSContext* cx, MutableHandleObject obj) {
...
static bool CheckArgCompartment(JSContext* cx, JSObject* obj, HandleValue v,
                                const char* methodname, const char* propname) {
    if (v.isObject()) return CheckArgCompartment(cx, obj, &v.toObject(), methodname, propname);
    return true;
}

bool Debugger::unwrapPropertyDescriptor(JSContext* cx, HandleObject obj,
    ...
    if (desc.hasGetterObject()) {
        ...
        if (get) {
            ...
            if (!CheckArgCompartment(cx, obj, get, "defineProperty", "get")) return false;
        }
    ...
    if (desc.hasSetterObject()) {
        ...
        if (set) {
            ...
            if (!CheckArgCompartment(cx, obj, set, "defineProperty", "set")) return false;
        }
...
JSTrapStatus Debugger::handleUncaughtExceptionHelper(Maybe<AutoCompartment>& ac,
...
JSTrapStatus Debugger::handleUncaughtException(Maybe<AutoCompartment>& ac, MutableHandleValue vp,
                                               const Maybe<HandleValue>& thisVForCheck,
                                               AbstractFramePtr frame) {
...
/* static */ void Debugger::resultToCompletion(JSContext* cx, bool ok, const Value& rv,
...
bool Debugger::newCompletionValue(JSContext* cx, JSTrapStatus status, const Value& value_,
    ...
    if (!obj || !NativeDefineDataProperty(cx, obj, key, value, JSPROP_ENUMERATE)) {
        return false;
    }
...
bool Debugger::receiveCompletionValue(Maybe<AutoCompartment>& ac, bool ok, HandleValue val,
                                      MutableHandleValue vp) {
    JSContext* cx = ac->context();
    ...
    ac.reset();
    return wrapDebuggeeValue(cx, &value) && newCompletionValue(cx, status, value, vp);
}

static bool GetStatusProperty(JSContext* cx, HandleObject obj, HandlePropertyName name,
                              JSTrapStatus status, JSTrapStatus& statusp, MutableHandleValue vp,
                              int* hits) {
    bool found;
...
static bool ParseResumptionValueAsObject(JSContext* cx, HandleValue rv, JSTrapStatus& statusp,
                                         MutableHandleValue vp) {
    int hits = 0;
    if (rv.isObject()) {
...
static bool ParseResumptionValue(JSContext* cx, HandleValue rval, JSTrapStatus& statusp,
                                 MutableHandleValue vp) {
    if (rval.isUndefined()) {
...
static bool CheckResumptionValue(JSContext* cx, AbstractFramePtr frame,
                                 const Maybe<HandleValue>& maybeThisv, JSTrapStatus status,
                                 MutableHandleValue vp) {
...
static bool GetThisValueForCheck(JSContext* cx, AbstractFramePtr frame, jsbytecode* pc,
    ...
    if (frame.debuggerNeedsCheckPrimitiveReturn()) {
        {
            ...
            if (!GetThisValueForDebuggerMaybeOptimizedOut(cx, frame, pc, thisv)) return false;
        }
...
bool Debugger::processResumptionValue(Maybe<AutoCompartment>& ac, AbstractFramePtr frame,
                                      const Maybe<HandleValue>& maybeThisv, HandleValue rval,
                                      JSTrapStatus& statusp, MutableHandleValue vp) {
    JSContext* cx = ac->context();

    if (!ParseResumptionValue(cx, rval, statusp, vp) || !unwrapDebuggeeValue(cx, vp) ||
...
JSTrapStatus Debugger::processParsedHandlerResultHelper(Maybe<AutoCompartment>& ac,
                                                        AbstractFramePtr frame,
    ...
    if (!unwrapDebuggeeValue(cx, vp) || !CheckResumptionValue(cx, frame, maybeThisv, status, vp)) {
...
JSTrapStatus Debugger::processHandlerResult(Maybe<AutoCompartment>& ac, bool success,
...
static bool CallMethodIfPresent(JSContext* cx, HandleObject obj, const char* name, size_t argc,
...
JSTrapStatus Debugger::fireDebuggerStatement(JSContext* cx, MutableHandleValue vp) {
    ...
    if (!getScriptFrame(cx, iter, &scriptFrame)) return reportUncaughtException(ac);
...
JSTrapStatus Debugger::fireExceptionUnwind(JSContext* cx, MutableHandleValue vp) {
...
void Debugger::fireNewScript(JSContext* cx, Handle<DebuggerScriptReferent> scriptReferent) {
...
void Debugger::fireOnGarbageCollectionHook(JSContext* cx,
...
/* static */ JSTrapStatus Debugger::dispatchHook(JSContext* cx, HookIsEnabledFun hookIsEnabled,
    ...
    if (GlobalObject::DebuggerVector* debuggers = global->getDebuggers()) {
        for (auto p = debuggers->begin(); p != debuggers->end(); p++) {
            ...
            if (dbg->enabled && hookIsEnabled(dbg)) {
                if (!triggered.append(ObjectValue(*dbg->toJSObject()))) return JSTRAP_ERROR;
            }
        }
    }
...
void Debugger::slowPathOnNewWasmInstance(JSContext* cx, Handle<WasmInstanceObject*> wasmInstance) {
    JSTrapStatus status = dispatchHook(
        cx,
...
/* static */ JSTrapStatus Debugger::onTrap(JSContext* cx, MutableHandleValue vp) {
    FrameIter iter(cx);
    ...
    for (Breakpoint* bp = site->firstBreakpoint(); bp; bp = bp->nextInSite()) {
        ...
        if (!isJS && &bp->asWasm()->wasmInstance->instance() != iter.wasmInstance()) continue;
    ...
    for (Breakpoint** p = triggered.begin(); p != triggered.end(); p++) {
        ...
        if (hasDebuggee) {
            ...
            JSTrapStatus st =
                dbg->processHandlerResult(ac, ok, rv, iter.abstractFramePtr(), iter.pc(), vp);
            if (st != JSTRAP_CONTINUE) return st;
...
/* static */ JSTrapStatus Debugger::onSingleStep(JSContext* cx, MutableHandleValue vp) {
    FrameIter iter(cx);
...
JSTrapStatus Debugger::fireNewGlobalObject(JSContext* cx, Handle<GlobalObject*> global,
                                           MutableHandleValue vp) {
    ...
    if (!wrapDebuggeeValue(cx, &wrappedGlobal)) return reportUncaughtException(ac);
...
void Debugger::slowPathOnNewGlobalObject(JSContext* cx, Handle<GlobalObject*> global) {
...
/* static */ bool Debugger::slowPathOnLogAllocationSite(JSContext* cx, HandleObject obj,
...
bool Debugger::appendAllocationSite(JSContext* cx, HandleObject obj, HandleSavedFrame frame,
                                    mozilla::TimeStamp when) {
...
JSTrapStatus Debugger::firePromiseHook(JSContext* cx, Hook hook, HandleObject promise,
                                       MutableHandleValue vp) {
...
/* static */ void Debugger::slowPathPromiseHook(JSContext* cx, Hook hook,
    ...
    RootedValue rval(cx);
    JSTrapStatus status =
        dispatchHook(cx, [hook](Debugger* dbg) -> bool { return dbg->getHook(hook); },
                     [&](Debugger* dbg) -> JSTrapStatus {
...
class MOZ_RAII ExecutionObservableCompartments : public Debugger::ExecutionObservableSet {
   ...
   public:
    explicit ExecutionObservableCompartments(JSContext* cx MOZ_GUARD_OBJECT_NOTIFIER_PARAM)
...
class MOZ_RAII ExecutionObservableFrame : public Debugger::ExecutionObservableSet {
   ...
   public:
    explicit ExecutionObservableFrame(AbstractFramePtr frame MOZ_GUARD_OBJECT_NOTIFIER_PARAM)
        : frame_(frame) {
        MOZ_GUARD_OBJECT_NOTIFIER_INIT;
    }
...
class MOZ_RAII ExecutionObservableScript : public Debugger::ExecutionObservableSet {
   ...
   public:
    ExecutionObservableScript(JSContext* cx, JSScript* script MOZ_GUARD_OBJECT_NOTIFIER_PARAM)
        : script_(cx, script) {
        MOZ_GUARD_OBJECT_NOTIFIER_INIT;
    }
...
/* static */ bool Debugger::updateExecutionObservabilityOfFrames(JSContext* cx,
                                                                 const ExecutionObservableSet& obs,
...
static inline void MarkBaselineScriptActiveIfObservable(
    ...
    if (obs.shouldRecompileOrInvalidate(script)) script->baselineScript()->setActive();
}

static bool AppendAndInvalidateScript(JSContext* cx, Zone* zone, JSScript* script,
...
/* static */ void Debugger::forEachDebuggerFrame(AbstractFramePtr frame, FrameFn fn) {
...
/* static */ bool Debugger::getDebuggerFrames(AbstractFramePtr frame,
...
/* static */ bool Debugger::updateExecutionObservability(JSContext* cx,
...
/* static */ bool Debugger::ensureExecutionObservabilityOfScript(JSContext* cx, JSScript* script) {
...
/* static */ bool Debugger::ensureExecutionObservabilityOfOsrFrame(JSContext* cx,
...
/* static */ bool Debugger::ensureExecutionObservabilityOfFrame(JSContext* cx,
                                                                AbstractFramePtr frame) {
...
/* static */ bool Debugger::ensureExecutionObservabilityOfCompartment(JSContext* cx,
                                                                      JSCompartment* comp) {
...
/* static */ bool Debugger::hookObservesAllExecution(Hook which) { return which == OnEnterFrame; }
...
bool Debugger::updateObservesAllExecutionOnDebuggees(JSContext* cx, IsObserving observing) {
    ExecutionObservableCompartments obs(cx);
...
bool Debugger::updateObservesCoverageOnDebuggees(JSContext* cx, IsObserving observing) {
    ExecutionObservableCompartments obs(cx);
    if (!obs.init()) return false;
...
/* static */ bool Debugger::isObservedByDebuggerTrackingAllocations(const GlobalObject& debuggee) {
...
/* static */ bool Debugger::addAllocationsTracking(JSContext* cx, Handle<GlobalObject*> debuggee) {
...
void Debugger::traceForMovingGC(JSTracer* trc) {
    ...
    for (Breakpoint* bp = firstBreakpoint(); bp; bp = bp->nextInDebugger()) {
        switch (bp->site->type()) {
            case BreakpointSite::Type::JS:
                TraceManuallyBarrieredEdge(trc, &bp->site->asJS()->script, "breakpoint script");
                break;
            case BreakpointSite::Type::Wasm:
...
/* static */ void Debugger::detachAllDebuggersFromGlobal(FreeOp* fop, GlobalObject* global) {
    const GlobalObject::DebuggerVector* debuggers = global->getDebuggers();
    MOZ_ASSERT(!debuggers->empty());
    while (!debuggers->empty()) debuggers->back()->removeDebuggeeGlobal(fop, global, nullptr);
}

/* static */ void Debugger::findZoneEdges(Zone* zone, js::gc::ZoneComponentFinder& finder) {
    /*
    ...
    for (ZoneGroupsIter group(zone->runtimeFromActiveCooperatingThread()); !group.done();
        ...
        for (Debugger* dbg : group->debuggerList()) {
            ...
            if (w == zone || !w->isGCMarking()) continue;
...
static Debugger* Debugger_fromThisValue(JSContext* cx, const CallArgs& args, const char* fnname) {
...
/* static */ bool Debugger::getEnabled(JSContext* cx, unsigned argc, Value* vp) {
    THIS_DEBUGGER(cx, argc, vp, "get enabled", args, dbg);
    args.rval().setBoolean(dbg->enabled);
    return true;
}

/* static */ bool Debugger::setEnabled(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool Debugger::getHookImpl(JSContext* cx, CallArgs& args, Debugger& dbg, Hook which) {
    MOZ_ASSERT(which >= 0 && which < HookCount);
    args.rval().set(dbg.object->getReservedSlot(JSSLOT_DEBUG_HOOK_START + which));
    return true;
}

/* static */ bool Debugger::setHookImpl(JSContext* cx, CallArgs& args, Debugger& dbg, Hook which) {
    MOZ_ASSERT(which >= 0 && which < HookCount);
...
/* static */ bool Debugger::getUncaughtExceptionHook(JSContext* cx, unsigned argc, Value* vp) {
    THIS_DEBUGGER(cx, argc, vp, "get uncaughtExceptionHook", args, dbg);
    args.rval().setObjectOrNull(dbg->uncaughtExceptionHook);
    return true;
}

/* static */ bool Debugger::setUncaughtExceptionHook(JSContext* cx, unsigned argc, Value* vp) {
    THIS_DEBUGGER(cx, argc, vp, "set uncaughtExceptionHook", args, dbg);
    if (!args.requireAtLeast(cx, "Debugger.set uncaughtExceptionHook", 1)) return false;
...
/* static */ bool Debugger::getAllowUnobservedAsmJS(JSContext* cx, unsigned argc, Value* vp) {
    THIS_DEBUGGER(cx, argc, vp, "get allowUnobservedAsmJS", args, dbg);
    args.rval().setBoolean(dbg->allowUnobservedAsmJS);
    return true;
}

/* static */ bool Debugger::setAllowUnobservedAsmJS(JSContext* cx, unsigned argc, Value* vp) {
    THIS_DEBUGGER(cx, argc, vp, "set allowUnobservedAsmJS", args, dbg);
    if (!args.requireAtLeast(cx, "Debugger.set allowUnobservedAsmJS", 1)) return false;
...
/* static */ bool Debugger::getAllowWasmBinarySource(JSContext* cx, unsigned argc, Value* vp) {
    THIS_DEBUGGER(cx, argc, vp, "get allowWasmBinarySource", args, dbg);
    args.rval().setBoolean(dbg->allowWasmBinarySource);
    return true;
}

/* static */ bool Debugger::setAllowWasmBinarySource(JSContext* cx, unsigned argc, Value* vp) {
    THIS_DEBUGGER(cx, argc, vp, "set allowWasmBinarySource", args, dbg);
    if (!args.requireAtLeast(cx, "Debugger.set allowWasmBinarySource", 1)) return false;
...
/* static */ bool Debugger::getCollectCoverageInfo(JSContext* cx, unsigned argc, Value* vp) {
    THIS_DEBUGGER(cx, argc, vp, "get collectCoverageInfo", args, dbg);
    args.rval().setBoolean(dbg->collectCoverageInfo);
    return true;
}

/* static */ bool Debugger::setCollectCoverageInfo(JSContext* cx, unsigned argc, Value* vp) {
    THIS_DEBUGGER(cx, argc, vp, "set collectCoverageInfo", args, dbg);
    if (!args.requireAtLeast(cx, "Debugger.set collectCoverageInfo", 1)) return false;
...
GlobalObject* Debugger::unwrapDebuggeeArgument(JSContext* cx, const Value& v) {
    if (!v.isObject()) {
        JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr, JSMSG_UNEXPECTED_TYPE, "argument",
                                  "not a global object");
        return nullptr;
    }
...
/* static */ bool Debugger::addDebuggee(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool Debugger::addAllGlobalsAsDebuggees(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool Debugger::removeDebuggee(JSContext* cx, unsigned argc, Value* vp) {
    ...
    if (dbg->debuggees.has(global)) {
        ...
        if (global->getDebuggers()->empty() && !obs.add(global->compartment())) return false;
...
/* static */ bool Debugger::removeAllDebuggees(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool Debugger::hasDebuggee(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool Debugger::getDebuggees(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool Debugger::getNewestFrame(JSContext* cx, unsigned argc, Value* vp) {
    ...
    for (AllFramesIter i(cx); !i.done(); ++i) {
        if (dbg->observesFrame(i)) {
            ...
            while (!iter.hasUsableAbstractFramePtr() || iter.abstractFramePtr() != frame) ++iter;
...
/* static */ bool Debugger::clearAllBreakpoints(JSContext* cx, unsigned argc, Value* vp) {
    THIS_DEBUGGER(cx, argc, vp, "clearAllBreakpoints", args, dbg);
    for (WeakGlobalObjectSet::Range r = dbg->debuggees.all(); !r.empty(); r.popFront())
        r.front()->compartment()->clearBreakpointsIn(cx->runtime()->defaultFreeOp(), dbg, nullptr);
    return true;
}
...
/* static */ bool Debugger::construct(JSContext* cx, unsigned argc, Value* vp) {
    ...
    RootedNativeObject obj(
        cx, NewNativeObjectWithGivenProto(cx, &Debugger::class_, proto, TenuredObject));
    if (!obj) return false;
    ...
    for (unsigned i = 0; i < args.length(); i++) {
        Rooted<GlobalObject*> debuggee(
            cx, &args[i].toObject().as<ProxyObject>().private_().toObject().global());
...
bool Debugger::addDebuggeeGlobal(JSContext* cx, Handle<GlobalObject*> global) {
    ...
    for (size_t i = 0; i < visited.length(); i++) {
        ...
        if (c->isDebuggee()) {
            ...
            for (auto p = v->begin(); p != v->end(); p++) {
                ...
                if (Find(visited, next) == visited.end() && !visited.append(next)) return false;
            }
        }
    }
    ...
    auto allocationsTrackingGuard = MakeScopeExit([&] {
        if (trackingAllocationSites && enabled) Debugger::removeAllocationsTracking(*global);
    });
...
template <typename T>
static T* findDebuggerInVector(Debugger* dbg, Vector<T, 0, js::SystemAllocPolicy>* vec) {
    T* p;
...
class MOZ_STACK_CLASS Debugger::ScriptQuery {
   public:
    ...
    bool parseQuery(HandleObject query) {
        ...
        if (!GetProperty(cx, query, query, cx->names().global, &global)) return false;
        ...
        if (!GetProperty(cx, query, query, cx->names().source, &debuggerSource)) return false;
        if (!debuggerSource.isUndefined()) {
            ...
            Value owner = debuggerSource.toObject().as<NativeObject>().getReservedSlot(
        ...
        if (!GetProperty(cx, query, query, cx->names().displayURL, &displayURL)) return false;
        ...
        if (!GetProperty(cx, query, query, cx->names().line, &lineProperty)) return false;
        ...
        if (!GetProperty(cx, query, query, innermostName, &innermostProperty)) return false;
    ...
    bool findScripts() {
        ...
        for (JSScript** i = vector.begin(); i != vector.end(); ++i) JS::ExposeScriptToActiveJS(*i);
         ...
         */
        if (innermost) {
            for (CompartmentToScriptMap::Range r = innermostForCompartment.all(); !r.empty();
                 r.popFront()) {
    ...
    Handle<WasmInstanceObjectVector> foundWasmInstances() const { return wasmInstanceVector; }
   ...
   private:
    ...
    typedef HashSet<JSCompartment*, DefaultHasher<JSCompartment*>, ZoneAllocPolicy> CompartmentSet;
    ...
    void consider(JSScript* script) {
        ...
        if (displayURLString) {
            if (!script->scriptSource() || !script->scriptSource()->hasDisplayURL()) return;
...
/* static */ bool Debugger::findScripts(JSContext* cx, unsigned argc, Value* vp) {
...
class MOZ_STACK_CLASS Debugger::ObjectQuery {
   public:
    ...
    ObjectQuery(JSContext* cx, Debugger* dbg) : objects(cx), cx(cx), dbg(dbg), className(cx) {}
    ...
    bool findObjects() {
...

        {
            ...
            return traversal.addStart(JS::ubi::Node(&rootList)) && traversal.traverse();
        }
    }
    ...
    bool operator()(Traversal& traversal, JS::ubi::Node origin, const JS::ubi::Edge& edge,
        ...
        if (!referent.is<JSObject>() || referent.exposeToJS().isUndefined()) return true;
   ...
   private:
    ...
    bool prepareQuery() {
        if (className.isString()) {
            if (!classNameCString.encodeLatin1(cx, className.toString())) return false;
        }
...
/* static */ bool Debugger::findAllGlobals(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool Debugger::makeGlobalObjectReference(JSContext* cx, unsigned argc, Value* vp) {
    THIS_DEBUGGER(cx, argc, vp, "makeGlobalObjectReference", args, dbg);
    if (!args.requireAtLeast(cx, "Debugger.makeGlobalObjectReference", 1)) return false;
...
const JSPropertySpec Debugger::properties[] = {
    ...
    JS_PSGS("onExceptionUnwind", Debugger::getOnExceptionUnwind, Debugger::setOnExceptionUnwind,
            0),
    ...
    JS_PSGS("onNewGlobalObject", Debugger::getOnNewGlobalObject, Debugger::setOnNewGlobalObject,
            0),
...
NativeObject* Debugger::newDebuggerScript(JSContext* cx, Handle<DebuggerScriptReferent> referent) {
    assertSameCompartment(cx, object.get());

    RootedObject proto(cx, &object->getReservedSlot(JSSLOT_DEBUG_SCRIPT_PROTO).toObject());
    MOZ_ASSERT(proto);
    NativeObject* scriptobj =
        NewNativeObjectWithGivenProto(cx, &DebuggerScript_class, proto, TenuredObject);
...
JSObject* Debugger::wrapVariantReferent(JSContext* cx, Map& map, Handle<CrossCompartmentKey> key,
...
JSObject* Debugger::wrapVariantReferent(JSContext* cx, Handle<DebuggerScriptReferent> referent) {
    JSObject* obj;
    if (referent.is<JSScript*>()) {
        ...
        obj = wrapVariantReferent<DebuggerScriptReferent, JSScript*, ScriptWeakMap>(cx, scripts,
                                                                                    key, referent);
    } else {
        ...
        obj =
            wrapVariantReferent<DebuggerScriptReferent, WasmInstanceObject*, WasmInstanceWeakMap>(
...
JSObject* Debugger::wrapWasmScript(JSContext* cx, Handle<WasmInstanceObject*> wasmInstance) {
    Rooted<DebuggerScriptReferent> referent(cx, wasmInstance.get());
    return wrapVariantReferent(cx, referent);
}

static JSObject* DebuggerScript_check(JSContext* cx, const Value& v, const char* fnname) {
...
static JSObject* DebuggerScript_checkThis(JSContext* cx, const CallArgs& args, const char* fnname,
                                          const char* refname) {
    ...
    if (!GetScriptReferent(thisobj).is<ReferentT>()) {
        ReportValueErrorFlags(cx, JSREPORT_ERROR, JSMSG_DEBUG_BAD_REFERENT, JSDVG_SEARCH_STACK,
                              args.thisv(), nullptr, refname, nullptr);
        return nullptr;
    }
...
#define THIS_DEBUGSCRIPT_SCRIPT(cx, argc, vp, fnname, args, obj, script)                        \
    CallArgs args = CallArgsFromVp(argc, vp);                                                   \
    RootedObject obj(cx, DebuggerScript_checkThis<JSScript*>(cx, args, fnname, "a JS script")); \
    if (!obj) return false;                                                                     \
    RootedScript script(cx, GetScriptReferent(obj).as<JSScript*>())

static bool DebuggerScript_getIsGeneratorFunction(JSContext* cx, unsigned argc, Value* vp) {
...
static bool DebuggerScript_getIsAsyncFunction(JSContext* cx, unsigned argc, Value* vp) {
    THIS_DEBUGSCRIPT_SCRIPT(cx, argc, vp, "(get isAsyncFunction)", args, obj, script);
    args.rval().setBoolean(script->isAsync());
    return true;
}

static bool DebuggerScript_getDisplayName(JSContext* cx, unsigned argc, Value* vp) {
...
static bool DebuggerScript_getStartLine(JSContext* cx, unsigned argc, Value* vp) {
...
struct DebuggerScriptGetLineCountMatcher {
    ...
    ReturnType match(Handle<WasmInstanceObject*> wasmInstance) {
        uint32_t result;
        if (!wasmInstance->instance().debug().totalSourceLines(cx_, &result)) return false;
        totalLines = double(result);
        return true;
    }
};

static bool DebuggerScript_getLineCount(JSContext* cx, unsigned argc, Value* vp) {
...
class DebuggerScriptGetSourceMatcher {
   ...
   public:
    DebuggerScriptGetSourceMatcher(JSContext* cx, Debugger* dbg) : cx_(cx), dbg_(dbg) {}

    using ReturnType = JSObject*;

    ReturnType match(HandleScript script) {
        RootedScriptSource source(
            cx_, &UncheckedUnwrap(script->sourceObject())->as<ScriptSourceObject>());
...
static bool DebuggerScript_getSourceStart(JSContext* cx, unsigned argc, Value* vp) {
    THIS_DEBUGSCRIPT_SCRIPT(cx, argc, vp, "(get sourceStart)", args, obj, script);
    args.rval().setNumber(uint32_t(script->sourceStart()));
    return true;
}

static bool DebuggerScript_getSourceLength(JSContext* cx, unsigned argc, Value* vp) {
...
static bool DebuggerScript_getChildScripts(JSContext* cx, unsigned argc, Value* vp) {
...
static bool EnsureScriptOffsetIsValid(JSContext* cx, JSScript* script, size_t offset) {
   ...
   public:
    class Entry {
       public:
        ...
        bool hasNoEdges() const { return lineno_ == SIZE_MAX && column_ != SIZE_MAX; }

        bool hasSingleEdge() const { return lineno_ != SIZE_MAX && column_ != SIZE_MAX; }
    ...
    bool populate(JSContext* cx, JSScript* script) {
        ...
        for (BytecodeRangeWithPosition r(cx, script); !r.empty(); r.popFront()) {
            ...
            if (FlowsIntoNext(prevOp)) addEdge(prevLineno, prevColumn, r.frontOffset());
...
class DebuggerScriptGetOffsetLocationMatcher {
   ...
   public:
    ...
    ReturnType match(HandleScript script) {
        ...
        if (!DefineDataProperty(cx_, result_, cx_->names().columnNumber, value)) return false;
        ...
        if (!DefineDataProperty(cx_, result_, cx_->names().isEntryPoint, value)) return false;
    ...
    ReturnType match(Handle<WasmInstanceObject*> instance) {
        ...
        size_t column;
        bool found;
        if (!instance->instance().debug().getOffsetLocation(cx_, offset_, &found, &lineno,
                                                            &column))
            return false;
...
static bool DebuggerScript_getOffsetLocation(JSContext* cx, unsigned argc, Value* vp) {
    THIS_DEBUGSCRIPT_REFERENT(cx, argc, vp, "getOffsetLocation", args, obj, referent);
    if (!args.requireAtLeast(cx, "Debugger.Script.getOffsetLocation", 1)) return false;
    size_t offset;
...
static bool DebuggerScript_getAllOffsets(JSContext* cx, unsigned argc, Value* vp) {
    ...
    for (BytecodeRangeWithPosition r(cx, script); !r.empty(); r.popFront()) {
        ...
        if (!flowData[offset].hasNoEdges() && flowData[offset].lineno() != lineno) {
            ...
            if (found && !GetProperty(cx, result, result, id, &offsetsv)) return false;
...
class DebuggerScriptGetAllColumnOffsetsMatcher {
    ...
    bool appendColumnOffsetEntry(size_t lineno, size_t column, size_t offset) {
        ...
        value = NumberValue(column);
        if (!DefineDataProperty(cx_, entry, cx_->names().columnNumber, value)) return false;
   ...
   public:
    ...
    ReturnType match(HandleScript script) {
        ...
        for (BytecodeRangeWithPosition r(cx_, script); !r.empty(); r.popFront()) {
            ...
            if (r.frontIsEntryPoint() && !flowData[offset].hasNoEdges() &&
                (flowData[offset].lineno() != lineno || flowData[offset].column() != column)) {
    ...
    ReturnType match(Handle<WasmInstanceObject*> instance) {
        ...
        if (!instance->instance().debug().getAllColumnOffsets(cx_, &offsets)) return false;
...
static bool DebuggerScript_getAllColumnOffsets(JSContext* cx, unsigned argc, Value* vp) {
...
class DebuggerScriptGetLineOffsetsMatcher {
   ...
   public:
    ...
    ReturnType match(Handle<WasmInstanceObject*> instance) {
        ...
        if (!instance->instance().debug().getLineOffsets(cx_, lineno_, &offsets)) return false;

        result_.set(NewDenseEmptyArray(cx_));
        if (!result_) return false;

        for (uint32_t i = 0; i < offsets.length(); i++) {
            if (!NewbornArrayPush(cx_, result_, NumberValue(offsets[i]))) return false;
        }
        return true;
    }
};

static bool DebuggerScript_getLineOffsets(JSContext* cx, unsigned argc, Value* vp) {
    THIS_DEBUGSCRIPT_REFERENT(cx, argc, vp, "getLineOffsets", args, obj, referent);
    if (!args.requireAtLeast(cx, "Debugger.Script.getLineOffsets", 1)) return false;
...
bool Debugger::observesFrame(const FrameIter& iter) const {
    ...
    if (iter.isInterp() && iter.isFunctionFrame()) {
        ...
        if (thisVal.isMagic() && thisVal.whyMagic() == JS_IS_CONSTRUCTING) return false;
    }
    if (iter.isWasm()) {
...
/* static */ bool Debugger::replaceFrameGuts(JSContext* cx, AbstractFramePtr from,
...
/* static */ void Debugger::removeFromFrameMapsAndClearBreakpointsIn(JSContext* cx,
                                                                     AbstractFramePtr frame) {
...
/* static */ bool Debugger::handleBaselineOsr(JSContext* cx, InterpreterFrame* from,
...
/* static */ bool Debugger::handleIonBailout(JSContext* cx, jit::RematerializedFrame* from,
                                             jit::BaselineFrame* to) {
...
/* static */ void Debugger::handleUnrecoverableIonBailoutError(JSContext* cx,
...
/* static */ void Debugger::propagateForcedReturn(JSContext* cx, AbstractFramePtr frame,
                                                  HandleValue rval) {
...
struct DebuggerScriptSetBreakpointMatcher {
   ...
   public:
    explicit DebuggerScriptSetBreakpointMatcher(JSContext* cx, Debugger* dbg, size_t offset,
...
static bool DebuggerScript_setBreakpoint(JSContext* cx, unsigned argc, Value* vp) {
    THIS_DEBUGSCRIPT_REFERENT(cx, argc, vp, "setBreakpoint", args, obj, referent);
    if (!args.requireAtLeast(cx, "Debugger.Script.setBreakpoint", 2)) return false;
    Debugger* dbg = Debugger::fromChildJSObject(obj);
...
static bool DebuggerScript_getBreakpoints(JSContext* cx, unsigned argc, Value* vp) {
...
class DebuggerScriptClearBreakpointMatcher {
   ...
   public:
    explicit DebuggerScriptClearBreakpointMatcher(JSContext* cx, Debugger* dbg, JSObject* handler)
...
static bool DebuggerScript_clearBreakpoint(JSContext* cx, unsigned argc, Value* vp) {
    THIS_DEBUGSCRIPT_REFERENT(cx, argc, vp, "clearBreakpoint", args, obj, referent);
    if (!args.requireAtLeast(cx, "Debugger.Script.clearBreakpoint", 1)) return false;
    Debugger* dbg = Debugger::fromChildJSObject(obj);
...
static bool DebuggerScript_clearAllBreakpoints(JSContext* cx, unsigned argc, Value* vp) {
   ...
   public:
    ...
    ReturnType match(HandleScript script) {
        ...
        if (script->hasTrynotes()) {
            ...
            while (tnBegin != tnEnd) {
                if (tnBegin->start <= offset && offset <= tnBegin->start + tnBegin->length &&
...
static bool DebuggerScript_isInCatchScope(JSContext* cx, unsigned argc, Value* vp) {
    THIS_DEBUGSCRIPT_REFERENT(cx, argc, vp, "isInCatchScope", args, obj, referent);
    if (!args.requireAtLeast(cx, "Debugger.Script.isInCatchScope", 1)) return false;
...
static bool DebuggerScript_getOffsetsCoverage(JSContext* cx, unsigned argc, Value* vp) {
...
NativeObject* Debugger::newDebuggerSource(JSContext* cx, Handle<DebuggerSourceReferent> referent) {
    assertSameCompartment(cx, object.get());

    RootedObject proto(cx, &object->getReservedSlot(JSSLOT_DEBUG_SOURCE_PROTO).toObject());
    MOZ_ASSERT(proto);
    NativeObject* sourceobj =
        NewNativeObjectWithGivenProto(cx, &DebuggerSource_class, proto, TenuredObject);
...
JSObject* Debugger::wrapVariantReferent(JSContext* cx, Handle<DebuggerSourceReferent> referent) {
    JSObject* obj;
    if (referent.is<ScriptSourceObject*>()) {
        ...
        obj =
            wrapVariantReferent<DebuggerSourceReferent, WasmInstanceObject*, WasmInstanceWeakMap>(
...
JSObject* Debugger::wrapWasmSource(JSContext* cx, Handle<WasmInstanceObject*> wasmInstance) {
...
static NativeObject* DebuggerSource_check(JSContext* cx, HandleValue thisv, const char* fnname) {
...
template <typename ReferentT>
static NativeObject* DebuggerSource_checkThis(JSContext* cx, const CallArgs& args,
...
#define THIS_DEBUGSOURCE_SOURCE(cx, argc, vp, fnname, args, obj, sourceObject)               \
    CallArgs args = CallArgsFromVp(argc, vp);                                                \
    RootedNativeObject obj(                                                                  \
        cx, DebuggerSource_checkThis<ScriptSourceObject*>(cx, args, fnname, "a JS source")); \
    if (!obj) return false;                                                                  \
...
class DebuggerSourceGetTextMatcher {
   ...
   public:
    ...
    ReturnType match(HandleScriptSource sourceObject) {
        ...
        if (!ss->hasSourceData() && !JSScript::loadSource(cx_, ss, &hasSourceData)) return nullptr;
...
static bool DebuggerSource_getBinary(JSContext* cx, unsigned argc, Value* vp) {
    ...
    if (!referent.is<WasmInstanceObject*>()) {
        ReportValueErrorFlags(cx, JSREPORT_ERROR, JSMSG_DEBUG_BAD_REFERENT, JSDVG_SEARCH_STACK,
                              args.thisv(), nullptr, "a wasm source", nullptr);
        return false;
    }

    RootedWasmInstanceObject wasmInstance(cx, referent.as<WasmInstanceObject*>());
    if (!wasmInstance->instance().debug().binarySource()) {
        JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr, JSMSG_DEBUG_NO_BINARY_SOURCE);
        return false;
    }
...
static bool DebuggerSource_getDisplayURL(JSContext* cx, unsigned argc, Value* vp) {
    THIS_DEBUGSOURCE_REFERENT(cx, argc, vp, "(get url)", args, obj, referent);
...
struct DebuggerSourceGetElementMatcher {
    using ReturnType = JSObject*;
    ReturnType match(HandleScriptSource sourceObject) { return sourceObject->element(); }
...
static bool DebuggerSource_getElement(JSContext* cx, unsigned argc, Value* vp) {
    ...
    if (JSObject* element = referent.match(matcher)) {
        ...
        if (!Debugger::fromChildJSObject(obj)->wrapDebuggeeValue(cx, args.rval())) return false;
    } else {
        args.rval().setUndefined();
    }
    return true;
...
struct DebuggerSourceGetElementPropertyMatcher {
    ...
    ReturnType match(Handle<WasmInstanceObject*> wasmInstance) { return UndefinedValue(); }
};

static bool DebuggerSource_getElementProperty(JSContext* cx, unsigned argc, Value* vp) {
...
static bool DebuggerSource_getIntroductionScript(JSContext* cx, unsigned argc, Value* vp) {
...
static bool DebuggerSource_getIntroductionOffset(JSContext* cx, unsigned argc, Value* vp) {
...
static bool DebuggerSource_getIntroductionType(JSContext* cx, unsigned argc, Value* vp) {
...
static bool DebuggerSource_setSourceMapURL(JSContext* cx, unsigned argc, Value* vp) {
...
class DebuggerSourceGetSourceMapURLMatcher {
   ...
   public:
    ...
    ReturnType match(Handle<WasmInstanceObject*> wasmInstance) {
        ...
        if (!wasmInstance->instance().debug().getSourceMappingURL(cx_, &str)) return false;
        result_.set(str);
        return true;
    }
};

static bool DebuggerSource_getSourceMapURL(JSContext* cx, unsigned argc, Value* vp) {
...
ScriptedOnStepHandler::ScriptedOnStepHandler(JSObject* object) : object_(object) {
...
bool ScriptedOnStepHandler::onStep(JSContext* cx, HandleDebuggerFrame frame, JSTrapStatus& statusp,
                                   MutableHandleValue vp) {
...
bool ScriptedOnPopHandler::onPop(JSContext* cx, HandleDebuggerFrame frame, JSTrapStatus& statusp,
                                 MutableHandleValue vp) {
...
/* static */ NativeObject* DebuggerFrame::initClass(JSContext* cx, HandleObject dbgCtor,
                                                    HandleObject obj) {
    Handle<GlobalObject*> global = obj.as<GlobalObject>();
    RootedObject objProto(cx, GlobalObject::getOrCreateObjectPrototype(cx, global));

    return InitClass(cx, dbgCtor, objProto, &class_, construct, 0, properties_, methods_, nullptr,
                     nullptr);
}

/* static */ DebuggerFrame* DebuggerFrame::create(JSContext* cx, HandleObject proto,
...
/* static */ bool DebuggerFrame::getCallee(JSContext* cx, HandleDebuggerFrame frame,
                                           MutableHandleDebuggerObject result) {
...
/* static */ bool DebuggerFrame::getIsConstructing(JSContext* cx, HandleDebuggerFrame frame,
                                                   bool& result) {
    MOZ_ASSERT(frame->isLive());
...
static void UpdateFrameIterPc(FrameIter& iter) {
    ...
    if (iter.abstractFramePtr().isRematerializedFrame()) {
        ...
        while (!jitIter.frame().isIonJS() || jitIter.frame().jsFrame() != jsFrame) ++jitIter;
...
/* static */ bool DebuggerFrame::getEnvironment(JSContext* cx, HandleDebuggerFrame frame,
                                                MutableHandleDebuggerEnvironment result) {
    MOZ_ASSERT(frame->isLive());
...
/* static */ bool DebuggerFrame::getIsGenerator(HandleDebuggerFrame frame) {
    ...
    return referent.hasScript() &&
           (referent.script()->isStarGenerator() || referent.script()->isLegacyGenerator());
}

/* static */ bool DebuggerFrame::getOffset(JSContext* cx, HandleDebuggerFrame frame,
                                           size_t& result) {
    MOZ_ASSERT(frame->isLive());
...
/* static */ bool DebuggerFrame::getOlder(JSContext* cx, HandleDebuggerFrame frame,
                                          MutableHandleDebuggerFrame result) {
    MOZ_ASSERT(frame->isLive());
...
/* static */ bool DebuggerFrame::getThis(JSContext* cx, HandleDebuggerFrame frame,
                                         MutableHandleValue result) {
    MOZ_ASSERT(frame->isLive());
...

    {
        ...
        if (!GetThisValueForDebuggerMaybeOptimizedOut(cx, frame, iter.pc(), result)) return false;
    }

    return dbg->wrapDebuggeeValue(cx, result);
}

/* static */ DebuggerFrameType DebuggerFrame::getType(HandleDebuggerFrame frame) {
    AbstractFramePtr referent = DebuggerFrame::getReferent(frame);
 ...
 */
/* static */ bool DebuggerFrame::setOnStepHandler(JSContext* cx, HandleDebuggerFrame frame,
    ...
    if (referent.isWasmDebugFrame()) {
        ...
        if (handler && !prior) {
            ...
            Debugger* dbg = frame->owner();
            if (!dbg->ensureExecutionObservabilityOfScript(cx, referent.script())) return false;
...
/* static */ bool DebuggerFrame::getArguments(JSContext* cx, HandleDebuggerFrame frame,
    ...
    if (!argumentsv.isUndefined()) {
        result.set(argumentsv.isObject() ? &argumentsv.toObject().as<DebuggerArguments>()
                                         : nullptr);
        return true;
    }
 ...
 */
static bool EvaluateInEnv(JSContext* cx, Handle<Env*> env, AbstractFramePtr frame,
    ...
    if (frame) {
        ...
        script = frontend::CompileEvalScript(cx, cx->tempLifoAlloc(), env, scope, options, srcBuf);
        ...
        // the global scope.
        script =
            frontend::CompileGlobalScript(cx, cx->tempLifoAlloc(), scopeKind, options, srcBuf);
    }
...
static bool DebuggerGenericEval(JSContext* cx, const mozilla::Range<const char16_t> chars,
                                HandleObject bindings, const EvalOptions& options,
                                JSTrapStatus& status, MutableHandleValue value, Debugger* dbg,
    ...
    if (bindings) {
        ...
        if (!CreateObjectsForEnvironmentChain(cx, envChain, env, &newEnv)) return false;
...
static bool DebuggerFrame_requireLive(JSContext* cx, HandleDebuggerFrame frame) {
    if (!frame->isLive()) {
...
/* static */ AbstractFramePtr DebuggerFrame::getReferent(HandleDebuggerFrame frame) {
...
/* static */ bool DebuggerFrame::getFrameIter(JSContext* cx, HandleDebuggerFrame frame,
    ...
    if (referent.isScriptFrameIterData()) {
        ...
        while (!iter.hasUsableAbstractFramePtr() || iter.abstractFramePtr() != referent) ++iter;
...
/* static */ bool DebuggerFrame::requireScriptReferent(JSContext* cx, HandleDebuggerFrame frame) {
    AbstractFramePtr referent = DebuggerFrame::getReferent(frame);
    if (!referent.hasScript()) {
        RootedValue frameobj(cx, ObjectValue(*frame));
        ReportValueErrorFlags(cx, JSREPORT_ERROR, JSMSG_DEBUG_BAD_REFERENT, JSDVG_SEARCH_STACK,
                              frameobj, nullptr, "a script frame", nullptr);
        return false;
    }
    return true;
}
...
static void DebuggerFrame_freeScriptFrameIterData(FreeOp* fop, JSObject* obj) {
    ...
    if (frame.isScriptFrameIterData()) fop->delete_((FrameIter::Data*)frame.raw());
...
static void DebuggerFrame_maybeDecrementFrameScriptStepModeCount(FreeOp* fop,
    ...
    if (frameobj->getReservedSlot(JSSLOT_DEBUGFRAME_ONSTEP_HANDLER).isUndefined()) return;
...
static DebuggerFrame* DebuggerFrame_checkThis(JSContext* cx, const CallArgs& args,
     ...
     * frames.
     */
    if (!frame->getPrivate() && frame->getReservedSlot(JSSLOT_DEBUGFRAME_OWNER).isUndefined()) {
        JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr, JSMSG_INCOMPATIBLE_PROTO,
...
#define THIS_FRAME_ITER(cx, argc, vp, fnname, args, thisobj, maybeIter, iter)                 \
    ...
    {                                                                                         \
        ...
        if (f.isScriptFrameIterData()) {                                                      \
            ...
            FrameIter& iter = *maybeIter;                                                     \
            while (!iter.hasUsableAbstractFramePtr() || iter.abstractFramePtr() != f) ++iter; \
            AbstractFramePtr data = iter.copyDataAsAbstractFramePtr();                        \
            if (!data) return false;                                                          \
...
/* static */ bool DebuggerFrame::typeGetter(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerFrame::implementationGetter(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerFrame::environmentGetter(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerFrame::calleeGetter(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerFrame::generatorGetter(JSContext* cx, unsigned argc, Value* vp) {
    THIS_DEBUGGER_FRAME(cx, argc, vp, "get callee", args, frame);
...
/* static */ bool DebuggerFrame::constructingGetter(JSContext* cx, unsigned argc, Value* vp) {
    THIS_DEBUGGER_FRAME(cx, argc, vp, "get callee", args, frame);
...
/* static */ bool DebuggerFrame::thisGetter(JSContext* cx, unsigned argc, Value* vp) {
    THIS_DEBUGGER_FRAME(cx, argc, vp, "get this", args, frame);

    return DebuggerFrame::getThis(cx, frame, args.rval());
}

/* static */ bool DebuggerFrame::olderGetter(JSContext* cx, unsigned argc, Value* vp) {
...
static bool DebuggerArguments_getArg(JSContext* cx, unsigned argc, Value* vp) {
    ...
    if (!Debugger::fromChildJSObject(thisobj)->wrapDebuggeeValue(cx, &arg)) return false;
    args.rval().set(arg);
    return true;
}

/* static */ DebuggerArguments* DebuggerArguments::create(JSContext* cx, HandleObject proto,
                                                          HandleDebuggerFrame frame) {
    AbstractFramePtr referent = DebuggerFrame::getReferent(frame);

    RootedNativeObject obj(cx,
                           NewNativeObjectWithGivenProto(cx, &DebuggerArguments::class_, proto));
    if (!obj) return nullptr;
    ...
    for (unsigned i = 0; i < fargc; i++) {
        ...
        id = INT_TO_JSID(i);
        if (!getobj ||
            !NativeDefineAccessorProperty(cx, obj, id, JS_DATA_TO_FUNC_PTR(GetterOp, getobj.get()),
...
/* static */ bool DebuggerFrame::argumentsGetter(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerFrame::offsetGetter(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerFrame::liveGetter(JSContext* cx, unsigned argc, Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);
...
/* static */ bool DebuggerFrame::onStepGetter(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerFrame::onStepSetter(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerFrame::onPopGetter(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerFrame::onPopSetter(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerFrame::evalMethod(JSContext* cx, unsigned argc, Value* vp) {
    THIS_DEBUGGER_FRAME(cx, argc, vp, "eval", args, frame);
    if (!args.requireAtLeast(cx, "Debugger.Frame.prototype.eval", 1)) return false;
    ...
    JSTrapStatus status;
    RootedValue value(cx);
    if (!DebuggerFrame::eval(cx, frame, chars, nullptr, options, status, &value)) return false;

    return frame->owner()->newCompletionValue(cx, status, value, args.rval());
}

/* static */ bool DebuggerFrame::evalWithBindingsMethod(JSContext* cx, unsigned argc, Value* vp) {
    THIS_DEBUGGER_FRAME(cx, argc, vp, "evalWithBindings", args, frame);
    if (!args.requireAtLeast(cx, "Debugger.Frame.prototype.evalWithBindings", 2)) return false;
    ...
    JSTrapStatus status;
    RootedValue value(cx);
    if (!DebuggerFrame::eval(cx, frame, chars, bindings, options, status, &value)) return false;

    return frame->owner()->newCompletionValue(cx, status, value, args.rval());
}

/* static */ bool DebuggerFrame::construct(JSContext* cx, unsigned argc, Value* vp) {
    JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr, JSMSG_NO_CONSTRUCTOR,
                              "Debugger.Frame");
    return false;
}
...
const JSFunctionSpec DebuggerFrame::methods_[] = {
    ...
    JS_FN("evalWithBindings", DebuggerFrame::evalWithBindingsMethod, 1, 0), JS_FS_END};
...
static DebuggerObject* DebuggerObject_checkThis(JSContext* cx, const CallArgs& args,
                                                const char* fnname) {
    JSObject* thisobj = NonNullObject(cx, args.thisv());
    if (!thisobj) return nullptr;
...
/* static */ bool DebuggerObject::construct(JSContext* cx, unsigned argc, Value* vp) {
    JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr, JSMSG_NO_CONSTRUCTOR,
                              "Debugger.Object");
    return false;
}

/* static */ bool DebuggerObject::callableGetter(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerObject::isBoundFunctionGetter(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerObject::isArrowFunctionGetter(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerObject::isAsyncFunctionGetter(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerObject::isGeneratorFunctionGetter(JSContext* cx, unsigned argc,
                                                            Value* vp) {
...
/* static */ bool DebuggerObject::protoGetter(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerObject::classGetter(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerObject::nameGetter(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerObject::displayNameGetter(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerObject::parameterNamesGetter(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerObject::scriptGetter(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerObject::environmentGetter(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerObject::boundTargetFunctionGetter(JSContext* cx, unsigned argc,
                                                            Value* vp) {
    ...
    RootedDebuggerObject result(cx);
    if (!DebuggerObject::getBoundTargetFunction(cx, object, &result)) return false;

    args.rval().setObject(*result);
    return true;
}

/* static */ bool DebuggerObject::boundThisGetter(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerObject::boundArgumentsGetter(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerObject::globalGetter(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerObject::allocationSiteGetter(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerObject::errorMessageNameGetter(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerObject::errorNotesGetter(JSContext* cx, unsigned argc, Value* vp) {
    THIS_DEBUGOBJECT(cx, argc, vp, "get errorNotes", args, object)

    return DebuggerObject::getErrorNotes(cx, object, args.rval());
}

/* static */ bool DebuggerObject::errorLineNumberGetter(JSContext* cx, unsigned argc, Value* vp) {
    THIS_DEBUGOBJECT(cx, argc, vp, "get errorLineNumber", args, object)

    return DebuggerObject::getErrorLineNumber(cx, object, args.rval());
}

/* static */ bool DebuggerObject::errorColumnNumberGetter(JSContext* cx, unsigned argc,
                                                          Value* vp) {
...
/* static */ bool DebuggerObject::isProxyGetter(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerObject::proxyTargetGetter(JSContext* cx, unsigned argc, Value* vp) {
    ...
    if (!DebuggerObject::getScriptedProxyTarget(cx, object, &result)) return false;

    args.rval().setObjectOrNull(result);
    return true;
}

/* static */ bool DebuggerObject::proxyHandlerGetter(JSContext* cx, unsigned argc, Value* vp) {
    ...
    Rooted<DebuggerObject*> result(cx);
    if (!DebuggerObject::getScriptedProxyHandler(cx, object, &result)) return false;

    args.rval().setObjectOrNull(result);
    return true;
}

/* static */ bool DebuggerObject::isPromiseGetter(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerObject::promiseStateGetter(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerObject::promiseValueGetter(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerObject::promiseReasonGetter(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerObject::promiseLifetimeGetter(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerObject::promiseTimeToResolutionGetter(JSContext* cx, unsigned argc,
                                                                Value* vp) {
...
/* static */ bool DebuggerObject::promiseAllocationSiteGetter(JSContext* cx, unsigned argc,
                                                              Value* vp) {
...
/* static */ bool DebuggerObject::promiseResolutionSiteGetter(JSContext* cx, unsigned argc,
                                                              Value* vp) {
...
/* static */ bool DebuggerObject::promiseIDGetter(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerObject::promiseDependentPromisesGetter(JSContext* cx, unsigned argc,
                                                                 Value* vp) {
    THIS_DEBUGOBJECT_OWNER_PROMISE(cx, argc, vp, "get promiseDependentPromises", args, dbg,
                                   refobj);
...
/* static */ bool DebuggerObject::isExtensibleMethod(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerObject::isSealedMethod(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerObject::isFrozenMethod(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerObject::getOwnPropertyNamesMethod(JSContext* cx, unsigned argc,
                                                            Value* vp) {
...
/* static */ bool DebuggerObject::getOwnPropertySymbolsMethod(JSContext* cx, unsigned argc,
                                                              Value* vp) {
...
/* static */ bool DebuggerObject::getOwnPropertyDescriptorMethod(JSContext* cx, unsigned argc,
                                                                 Value* vp) {
    ...
    if (!DebuggerObject::getOwnPropertyDescriptor(cx, object, id, &desc)) return false;

    return JS::FromPropertyDescriptor(cx, desc, args.rval());
}

/* static */ bool DebuggerObject::preventExtensionsMethod(JSContext* cx, unsigned argc,
                                                          Value* vp) {
...
/* static */ bool DebuggerObject::sealMethod(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerObject::freezeMethod(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerObject::definePropertyMethod(JSContext* cx, unsigned argc, Value* vp) {
    THIS_DEBUGOBJECT(cx, argc, vp, "defineProperty", args, object)
    if (!args.requireAtLeast(cx, "Debugger.Object.defineProperty", 2)) return false;
...
/* static */ bool DebuggerObject::definePropertiesMethod(JSContext* cx, unsigned argc, Value* vp) {
    THIS_DEBUGOBJECT(cx, argc, vp, "defineProperties", args, object);
    if (!args.requireAtLeast(cx, "Debugger.Object.defineProperties", 1)) return false;
 ...
 */
/* static */ bool DebuggerObject::deletePropertyMethod(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerObject::callMethod(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerObject::applyMethod(JSContext* cx, unsigned argc, Value* vp) {
    ...
    if (callArgs.length() >= 2 && !callArgs[1].isNullOrUndefined()) {
        ...
        if (!args.growBy(argc) || !GetElements(cx, argsobj, argc, args.begin())) return false;
    }

    return object->call(cx, object, thisv, args, callArgs.rval());
}

/* static */ bool DebuggerObject::asEnvironmentMethod(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerObject::forceLexicalInitializationByNameMethod(JSContext* cx,
                                                                         unsigned argc,
                                                                         Value* vp) {
    ...
    bool result;
    if (!DebuggerObject::forceLexicalInitializationByName(cx, object, id, result)) return false;

    args.rval().setBoolean(result);
    return true;
}

/* static */ bool DebuggerObject::executeInGlobalMethod(JSContext* cx, unsigned argc, Value* vp) {
    THIS_DEBUGOBJECT(cx, argc, vp, "executeInGlobal", args, object);
    if (!args.requireAtLeast(cx, "Debugger.Object.prototype.executeInGlobal", 1)) return false;
...
/* static */ bool DebuggerObject::executeInGlobalWithBindingsMethod(JSContext* cx, unsigned argc,
                                                                    Value* vp) {
...
/* static */ bool DebuggerObject::makeDebuggeeValueMethod(JSContext* cx, unsigned argc,
                                                          Value* vp) {
    THIS_DEBUGOBJECT(cx, argc, vp, "makeDebuggeeValue", args, object);
    if (!args.requireAtLeast(cx, "Debugger.Object.prototype.makeDebuggeeValue", 1)) return false;

    return DebuggerObject::makeDebuggeeValue(cx, object, args[0], args.rval());
}

/* static */ bool DebuggerObject::unsafeDereferenceMethod(JSContext* cx, unsigned argc,
                                                          Value* vp) {
...
/* static */ bool DebuggerObject::unwrapMethod(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ NativeObject* DebuggerObject::initClass(JSContext* cx, HandleObject obj,
                                                     HandleObject debugCtor) {
    Handle<GlobalObject*> global = obj.as<GlobalObject>();
    RootedObject objProto(cx, GlobalObject::getOrCreateObjectPrototype(cx, global));

    RootedNativeObject objectProto(cx, InitClass(cx, debugCtor, objProto, &class_, construct, 0,
...
/* static */ DebuggerObject* DebuggerObject::create(JSContext* cx, HandleObject proto,
...
bool DebuggerObject::isScriptedProxy() const { return js::IsScriptedProxy(referent()); }
...
/* static */ bool DebuggerObject::getClassName(JSContext* cx, HandleDebuggerObject object,
...
/* static */ bool DebuggerObject::getGlobal(JSContext* cx, HandleDebuggerObject object,
                                            MutableHandleDebuggerObject result) {
    RootedObject referent(cx, object->referent());
...
JS::PromiseState DebuggerObject::promiseState() const { return promise()->state(); }
...
/* static */ bool DebuggerObject::getParameterNames(JSContext* cx, HandleDebuggerObject object,
...
/* static */ bool DebuggerObject::getBoundTargetFunction(JSContext* cx,
...
/* static */ bool DebuggerObject::getBoundThis(JSContext* cx, HandleDebuggerObject object,
                                               MutableHandleValue result) {
    MOZ_ASSERT(object->isBoundFunction());
...
/* static */ bool DebuggerObject::getBoundArguments(JSContext* cx, HandleDebuggerObject object,
...
/* static */ SavedFrame* Debugger::getObjectAllocationSite(JSObject& obj) {
    ...
    return SavedFrame::isSavedFrameAndNotProto(*metadata) ? &metadata->as<SavedFrame>() : nullptr;
}

/* static */ bool DebuggerObject::getAllocationSite(JSContext* cx, HandleDebuggerObject object,
                                                    MutableHandleObject result) {
    RootedObject referent(cx, object->referent());
...
/* static */ bool DebuggerObject::getErrorReport(JSContext* cx, HandleObject maybeError,
...
/* static */ bool DebuggerObject::getErrorMessageName(JSContext* cx, HandleDebuggerObject object,
                                                      MutableHandleString result) {
    RootedObject referent(cx, object->referent());
    JSErrorReport* report;
...
/* static */ bool DebuggerObject::getErrorNotes(JSContext* cx, HandleDebuggerObject object,
                                                MutableHandleValue result) {
    RootedObject referent(cx, object->referent());
    JSErrorReport* report;
    if (!getErrorReport(cx, referent, report)) return false;
...
/* static */ bool DebuggerObject::getErrorLineNumber(JSContext* cx, HandleDebuggerObject object,
                                                     MutableHandleValue result) {
    RootedObject referent(cx, object->referent());
    JSErrorReport* report;
    if (!getErrorReport(cx, referent, report)) return false;
...
/* static */ bool DebuggerObject::getErrorColumnNumber(JSContext* cx, HandleDebuggerObject object,
                                                       MutableHandleValue result) {
    RootedObject referent(cx, object->referent());
    JSErrorReport* report;
    if (!getErrorReport(cx, referent, report)) return false;
...
/* static */ bool DebuggerObject::getPromiseValue(JSContext* cx, HandleDebuggerObject object,
                                                  MutableHandleValue result) {
...
/* static */ bool DebuggerObject::getPromiseReason(JSContext* cx, HandleDebuggerObject object,
                                                   MutableHandleValue result) {
...
/* static */ bool DebuggerObject::isExtensible(JSContext* cx, HandleDebuggerObject object,
                                               bool& result) {
    RootedObject referent(cx, object->referent());
...
/* static */ bool DebuggerObject::isSealed(JSContext* cx, HandleDebuggerObject object,
                                           bool& result) {
    RootedObject referent(cx, object->referent());
...
/* static */ bool DebuggerObject::isFrozen(JSContext* cx, HandleDebuggerObject object,
                                           bool& result) {
    RootedObject referent(cx, object->referent());
...
/* static */ bool DebuggerObject::getPrototypeOf(JSContext* cx, HandleDebuggerObject object,
                                                 MutableHandleDebuggerObject result) {
    RootedObject referent(cx, object->referent());
    Debugger* dbg = object->owner();
...
/* static */ bool DebuggerObject::getOwnPropertyNames(JSContext* cx, HandleDebuggerObject object,
    ...
    AutoIdVector ids(cx);
    {
        ...
        ErrorCopier ec(ac);
        if (!GetPropertyKeys(cx, referent, JSITER_OWNONLY | JSITER_HIDDEN, &ids)) return false;
    }
...
/* static */ bool DebuggerObject::getOwnPropertySymbols(JSContext* cx, HandleDebuggerObject object,
                                                        MutableHandle<IdVector> result) {
    RootedObject referent(cx, object->referent());
...
/* static */ bool DebuggerObject::preventExtensions(JSContext* cx, HandleDebuggerObject object) {
    RootedObject referent(cx, object->referent());
...
/* static */ bool DebuggerObject::seal(JSContext* cx, HandleDebuggerObject object) {
    RootedObject referent(cx, object->referent());
...
/* static */ bool DebuggerObject::freeze(JSContext* cx, HandleDebuggerObject object) {
    RootedObject referent(cx, object->referent());
...
/* static */ bool DebuggerObject::defineProperty(JSContext* cx, HandleDebuggerObject object,
...
/* static */ bool DebuggerObject::defineProperties(JSContext* cx, HandleDebuggerObject object,
                                                   Handle<IdVector> ids,
...
/* static */ bool DebuggerObject::deleteProperty(JSContext* cx, HandleDebuggerObject object,
...
/* static */ bool DebuggerObject::call(JSContext* cx, HandleDebuggerObject object,
...
/* static */ bool DebuggerObject::forceLexicalInitializationByName(JSContext* cx,
                                                                   HandleDebuggerObject object,
...
/* static */ bool DebuggerObject::executeInGlobal(JSContext* cx, HandleDebuggerObject object,
...
/* static */ bool DebuggerObject::makeDebuggeeValue(JSContext* cx, HandleDebuggerObject object,
                                                    HandleValue value_,
                                                    MutableHandleValue result) {
    RootedObject referent(cx, object->referent());
    Debugger* dbg = object->owner();
...
/* static */ bool DebuggerObject::unsafeDereference(JSContext* cx, HandleDebuggerObject object,
                                                    MutableHandleObject result) {
    RootedObject referent(cx, object->referent());
...
/* static */ bool DebuggerObject::unwrap(JSContext* cx, HandleDebuggerObject object,
                                         MutableHandleDebuggerObject result) {
    RootedObject referent(cx, object->referent());
    Debugger* dbg = object->owner();
...
/* static */ bool DebuggerObject::requireGlobal(JSContext* cx, HandleDebuggerObject object) {
    if (!object->isGlobal()) {
        RootedObject referent(cx, object->referent());
...
/* static */ bool DebuggerObject::requirePromise(JSContext* cx, HandleDebuggerObject object) {
    RootedObject referent(cx, object->referent());
...
/* static */ bool DebuggerObject::getScriptedProxyTarget(JSContext* cx,
                                                         HandleDebuggerObject object,
                                                         MutableHandleDebuggerObject result) {
...
/* static */ bool DebuggerObject::getScriptedProxyHandler(JSContext* cx,
                                                          HandleDebuggerObject object,
                                                          MutableHandleDebuggerObject result) {
    MOZ_ASSERT(object->isScriptedProxy());
    RootedObject referent(cx, object->referent());
...
static DebuggerEnvironment* DebuggerEnvironment_checkThis(JSContext* cx, const CallArgs& args,
                                                          const char* fnname,
...
/* static */ bool DebuggerEnvironment::construct(JSContext* cx, unsigned argc, Value* vp) {
    JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr, JSMSG_NO_CONSTRUCTOR,
                              "Debugger.Environment");
    return false;
}
...
static bool IsDeclarative(Env* env) {
    return env->is<DebugEnvironmentProxy>() && env->as<DebugEnvironmentProxy>().isForDeclarative();
}
...
/* static */ bool DebuggerEnvironment::parentGetter(JSContext* cx, unsigned argc, Value* vp) {
    THIS_DEBUGGER_ENVIRONMENT(cx, argc, vp, "get type", args, environment);
...
/* static */ bool DebuggerEnvironment::objectGetter(JSContext* cx, unsigned argc, Value* vp) {
    THIS_DEBUGGER_ENVIRONMENT(cx, argc, vp, "get type", args, environment);
...
/* static */ bool DebuggerEnvironment::calleeGetter(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerEnvironment::inspectableGetter(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerEnvironment::optimizedOutGetter(JSContext* cx, unsigned argc,
                                                          Value* vp) {
...
/* static */ bool DebuggerEnvironment::namesMethod(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerEnvironment::findMethod(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool DebuggerEnvironment::getVariableMethod(JSContext* cx, unsigned argc, Value* vp) {
    THIS_DEBUGGER_ENVIRONMENT(cx, argc, vp, "getVariable", args, environment);
    if (!args.requireAtLeast(cx, "Debugger.Environment.getVariable", 1)) return false;
...
/* static */ bool DebuggerEnvironment::setVariableMethod(JSContext* cx, unsigned argc, Value* vp) {
    THIS_DEBUGGER_ENVIRONMENT(cx, argc, vp, "setVariable", args, environment);
    if (!args.requireAtLeast(cx, "Debugger.Environment.setVariable", 2)) return false;
    ...
    if (!DebuggerEnvironment::setVariable(cx, environment, id, args[1])) return false;
...
const JSFunctionSpec DebuggerEnvironment::methods_[] = {
    ...
    JS_FN("setVariable", DebuggerEnvironment::setVariableMethod, 2, 0), JS_FS_END};

/* static */ NativeObject* DebuggerEnvironment::initClass(JSContext* cx, HandleObject dbgCtor,
                                                          HandleObject obj) {
    Handle<GlobalObject*> global = obj.as<GlobalObject>();
    RootedObject objProto(cx, GlobalObject::getOrCreateObjectPrototype(cx, global));
...
/* static */ DebuggerEnvironment* DebuggerEnvironment::create(JSContext* cx, HandleObject proto,
                                                              HandleObject referent,
                                                              HandleNativeObject debugger) {
    NewObjectKind newKind = IsInsideNursery(referent) ? GenericObject : TenuredObject;
    RootedObject obj(cx,
                     NewObjectWithGivenProto(cx, &DebuggerEnvironment::class_, proto, newKind));
    if (!obj) return nullptr;
...
bool DebuggerEnvironment::getParent(JSContext* cx, MutableHandleDebuggerEnvironment result) const {
...
bool DebuggerEnvironment::getObject(JSContext* cx, MutableHandleDebuggerObject result) const {
...
bool DebuggerEnvironment::getCallee(JSContext* cx, MutableHandleDebuggerObject result) const {
...
/* static */ bool DebuggerEnvironment::getNames(JSContext* cx,
...
/* static */ bool DebuggerEnvironment::find(JSContext* cx, HandleDebuggerEnvironment environment,
...
/* static */ bool DebuggerEnvironment::getVariable(JSContext* cx,
                                                   HandleDebuggerEnvironment environment,
...

    {
        ...
        if (referent->is<DebugEnvironmentProxy>()) {
            ...
            if (!DebugEnvironmentProxy::getMaybeSentinelValue(cx, env, id, result)) return false;
        } else {
    ...
    if (result.isObject()) {
        ...
        if (obj->is<JSFunction>() && IsInternalFunctionObject(obj->as<JSFunction>()))
...
/* static */ bool DebuggerEnvironment::setVariable(JSContext* cx,
                                                   HandleDebuggerEnvironment environment,
...
bool Builder::Object::defineProperty(JSContext* cx, const char* name, JS::HandleValue propval_) {
...
bool Builder::Object::defineProperty(JSContext* cx, const char* name, JS::HandleObject propval_) {
    RootedValue propval(cx, ObjectOrNullValue(propval_));
    return defineProperty(cx, name, propval);
}

bool Builder::Object::defineProperty(JSContext* cx, const char* name, Builder::Object& propval_) {
    AutoCompartment ac(cx, debuggerObject());
...
AutoEntryMonitor::AutoEntryMonitor(JSContext* cx) : cx_(cx), savedMonitor_(cx->entryMonitor) {
...
extern JS_PUBLIC_API(bool) JS_DefineDebuggerObject(JSContext* cx, HandleObject obj) {
    RootedNativeObject objProto(cx), debugCtor(cx), debugProto(cx), frameProto(cx),
        scriptProto(cx), sourceProto(cx), objectProto(cx), envProto(cx), memoryProto(cx);
    ...
    debugProto = InitClass(cx, obj, objProto, &Debugger::class_, Debugger::construct, 1,
    ...
    scriptProto =
        InitClass(cx, debugCtor, objProto, &DebuggerScript_class, DebuggerScript_construct, 0,
                  DebuggerScript_properties, DebuggerScript_methods, nullptr, nullptr);
    if (!scriptProto) return false;

    sourceProto =
        InitClass(cx, debugCtor, sourceProto, &DebuggerSource_class, DebuggerSource_construct, 0,
                  DebuggerSource_properties, DebuggerSource_methods, nullptr, nullptr);
    ...
    memoryProto =
        InitClass(cx, debugCtor, objProto, &DebuggerMemory::class_, DebuggerMemory::construct, 0,
...
#ifdef DEBUG
/* static */ bool Debugger::isDebuggerCrossCompartmentEdge(JSObject* obj, const gc::Cell* target) {
    MOZ_ASSERT(target);
...
static bool DefineStringProperty(JSContext* cx, HandleObject obj, PropertyName* propName,
                                 const char* strVal) {
...
JSObject* GarbageCollectionEvent::toJSObject(JSContext* cx) const {
    ...
    for (auto range = collections.all(); !range.empty(); range.popFront()) {
        ...
        if (!DefineDataElement(cx, slicesArray, idx++, collectionVal)) return nullptr;
    }

    RootedValue slicesValue(cx, ObjectValue(*slicesArray));
    if (!DefineDataProperty(cx, obj, cx->names().collections, slicesValue)) return nullptr;
