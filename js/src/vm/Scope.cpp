...
static Shape* EmptyEnvironmentShape(JSContext* cx, const Class* cls, uint32_t numSlots,
    ...
    return EmptyShape::getInitialShape(cx, cls, TaggedProto(nullptr), numFixed, baseShapeFlags);
}

static Shape* NextEnvironmentShape(JSContext* cx, HandleAtom name, BindingKind bindKind,
                                   uint32_t slot, StackBaseShape& stackBase, HandleShape shape) {
...
static Shape* CreateEnvironmentShape(JSContext* cx, BindingIter& bi, const Class* cls,
...
static UniquePtr<typename ConcreteScope::Data> CopyScopeData(
    JSContext* cx, Handle<typename ConcreteScope::Data*> data) {
    ...
    for (size_t i = 0; i < length; i++) {
        if (JSAtom* name = names[i].name()) cx->markAtom(name);
    }
...
template <typename ConcreteScope>
static bool PrepareScopeData(JSContext* cx, BindingIter& bi,
                             Handle<UniquePtr<typename ConcreteScope::Data>> data,
                             const Class* cls, uint32_t baseShapeFlags,
    ...
    if (bi.nextEnvironmentSlot() == JSSLOT_FREE(cls)) {
        ...
        envShape.set(
            CreateEnvironmentShape(cx, freshBi, cls, bi.nextEnvironmentSlot(), baseShapeFlags));
        if (!envShape) return false;
    }
...
template <typename ConcreteScope>
static UniquePtr<typename ConcreteScope::Data> NewEmptyScopeData(JSContext* cx,
                                                                 uint32_t length = 0) {
    uint8_t* bytes = cx->zone()->pod_calloc<uint8_t>(ConcreteScope::sizeOfData(length));
    if (!bytes) ReportOutOfMemory(cx);
    auto data = reinterpret_cast<typename ConcreteScope::Data*>(bytes);
    if (data) new (data) typename ConcreteScope::Data();
    return UniquePtr<typename ConcreteScope::Data>(data);
}

static bool XDRBindingName(XDRState<XDR_ENCODE>* xdr, BindingName* bindingName) {
    JSContext* cx = xdr->cx();
    ...
    if (!xdr->codeUint8(&u8)) return false;

    if (atom && !XDRAtom(xdr, &atom)) return false;

    return true;
}

static bool XDRBindingName(XDRState<XDR_DECODE>* xdr, BindingName* bindingName) {
    JSContext* cx = xdr->cx();
    ...
    RootedAtom atom(cx);
    if (hasAtom && !XDRAtom(xdr, &atom)) return false;
...
/* static */ bool Scope::XDRSizedBindingNames(XDRState<mode>* xdr, Handle<ConcreteScope*> scope,
    ...
    uint32_t length;
    if (mode == XDR_ENCODE) length = scope->data().length;
...
/* static */ Scope* Scope::create(JSContext* cx, ScopeKind kind, HandleScope enclosing,
                                  HandleShape envShape) {
    Scope* scope = Allocate<Scope>(cx);
    if (scope) new (scope) Scope(kind, enclosing, envShape);
    return scope;
}
...
uint32_t Scope::chainLength() const {
    uint32_t length = 0;
    for (ScopeIter si(const_cast<Scope*>(this)); si; si++) length++;
    return length;
}
...
uint32_t Scope::environmentChainLength() const {
    ...
    for (ScopeIter si(const_cast<Scope*>(this)); si; si++) {
        if (si.hasSyntacticEnvironment()) length++;
    }
    return length;
}
...
/* static */ Scope* Scope::clone(JSContext* cx, HandleScope scope, HandleScope enclosing) {
    RootedShape envShape(cx);
    if (scope->environmentShape()) {
        envShape = scope->maybeCloneEnvironmentShape(cx);
        if (!envShape) return nullptr;
    }
...
size_t Scope::sizeOfExcludingThis(mozilla::MallocSizeOf mallocSizeOf) const {
    if (data_) return mallocSizeOf(reinterpret_cast<void*>(data_));
    return 0;
}
...
void Scope::dump() {
    for (ScopeIter si(this); si; si++) {
        ...
        if (si.scope()->enclosing()) fprintf(stderr, " -> ");
    }
    fprintf(stderr, "\n");
}
...
/* static */ LexicalScope* LexicalScope::create(JSContext* cx, ScopeKind kind, Handle<Data*> data,
...
/* static */ LexicalScope* LexicalScope::createWithData(JSContext* cx, ScopeKind kind,
...
/* static */ Shape* LexicalScope::getEmptyExtensibleEnvironmentShape(JSContext* cx) {
...
/* static */ bool LexicalScope::XDR(XDRState<mode>* xdr, ScopeKind kind, HandleScope enclosing,
                                    MutableHandleScope scope) {
    JSContext* cx = xdr->cx();

    Rooted<Data*> data(cx);
    if (!XDRSizedBindingNames<LexicalScope>(xdr, scope.as<LexicalScope>(), &data)) return false;

    {
        Maybe<Rooted<UniquePtr<Data>>> uniqueData;
        if (mode == XDR_DECODE) uniqueData.emplace(cx, data);
        ...
        if (!xdr->codeUint32(&data->constStart)) return false;
        if (!xdr->codeUint32(&firstFrameSlot)) return false;
        if (!xdr->codeUint32(&nextFrameSlot)) return false;
...
static inline uint32_t FunctionScopeEnvShapeFlags(bool hasParameterExprs) {
    if (hasParameterExprs) return BaseShape::DELEGATE;
...
/* static */ FunctionScope* FunctionScope::create(JSContext* cx, Handle<Data*> dataArg,
...
JSScript* FunctionScope::script() const { return canonicalFunction()->nonLazyScript(); }
...
/* static */ Shape* FunctionScope::getEmptyEnvironmentShape(JSContext* cx,
...
/* static */ FunctionScope* FunctionScope::clone(JSContext* cx, Handle<FunctionScope*> scope,
                                                 HandleFunction fun, HandleScope enclosing) {
...
template <XDRMode mode>
/* static */ bool FunctionScope::XDR(XDRState<mode>* xdr, HandleFunction fun,
                                     HandleScope enclosing, MutableHandleScope scope) {
    JSContext* cx = xdr->cx();
    Rooted<Data*> data(cx);
    if (!XDRSizedBindingNames<FunctionScope>(xdr, scope.as<FunctionScope>(), &data)) return false;

    {
        ...
        if (!xdr->codeUint8(&needsEnvironment)) return false;
        if (!xdr->codeUint8(&hasParameterExprs)) return false;
        if (!xdr->codeUint16(&data->nonPositionalFormalStart)) return false;
        if (!xdr->codeUint16(&data->varStart)) return false;
        if (!xdr->codeUint32(&nextFrameSlot)) return false;
        ...
        if (mode == XDR_DECODE) {
            ...
            scope.set(createWithData(cx, &uniqueData.ref(), hasParameterExprs, needsEnvironment,
                                     fun, enclosing));
            if (!scope) return false;
...
static const uint32_t VarScopeEnvShapeFlags = BaseShape::QUALIFIED_VAROBJ | BaseShape::DELEGATE;

static UniquePtr<VarScope::Data> NewEmptyVarScopeData(JSContext* cx, uint32_t firstFrameSlot) {
    UniquePtr<VarScope::Data> data(NewEmptyScopeData<VarScope>(cx));
    if (data) data->nextFrameSlot = firstFrameSlot;

    return data;
}

/* static */ VarScope* VarScope::create(JSContext* cx, ScopeKind kind, Handle<Data*> dataArg,
...
/* static */ VarScope* VarScope::createWithData(JSContext* cx, ScopeKind kind,
                                                MutableHandle<UniquePtr<Data>> data,
                                                uint32_t firstFrameSlot, bool needsEnvironment,
                                                HandleScope enclosing) {
    MOZ_ASSERT(data);
...
uint32_t VarScope::firstFrameSlot() const {
    if (enclosing()->is<FunctionScope>()) return enclosing()->as<FunctionScope>().nextFrameSlot();
    return 0;
}

template <XDRMode mode>
/* static */ bool VarScope::XDR(XDRState<mode>* xdr, ScopeKind kind, HandleScope enclosing,
                                MutableHandleScope scope) {
    JSContext* cx = xdr->cx();
    Rooted<Data*> data(cx);
    if (!XDRSizedBindingNames<VarScope>(xdr, scope.as<VarScope>(), &data)) return false;
...
/* static */ GlobalScope* GlobalScope::createWithData(JSContext* cx, ScopeKind kind,
                                                      MutableHandle<UniquePtr<Data>> data) {
    MOZ_ASSERT(data);
...
/* static */ GlobalScope* GlobalScope::clone(JSContext* cx, Handle<GlobalScope*> scope,
                                             ScopeKind kind) {
...
template <XDRMode mode>
/* static */ bool GlobalScope::XDR(XDRState<mode>* xdr, ScopeKind kind, MutableHandleScope scope) {
    MOZ_ASSERT((mode == XDR_DECODE) == !scope);

    JSContext* cx = xdr->cx();
    Rooted<Data*> data(cx);
    if (!XDRSizedBindingNames<GlobalScope>(xdr, scope.as<GlobalScope>(), &data)) return false;

    {
        Maybe<Rooted<UniquePtr<Data>>> uniqueData;
        if (mode == XDR_DECODE) uniqueData.emplace(cx, data);

        if (!xdr->codeUint32(&data->varStart)) return false;
        if (!xdr->codeUint32(&data->letStart)) return false;
        if (!xdr->codeUint32(&data->constStart)) return false;
...
/* static */ WithScope* WithScope::create(JSContext* cx, HandleScope enclosing) {
    Scope* scope = Scope::create(cx, ScopeKind::With, enclosing, nullptr);
    return static_cast<WithScope*>(scope);
}

static const uint32_t EvalScopeEnvShapeFlags = BaseShape::QUALIFIED_VAROBJ | BaseShape::DELEGATE;
...
/* static */ EvalScope* EvalScope::create(JSContext* cx, ScopeKind scopeKind,
    ...
    Rooted<UniquePtr<Data>> data(
        cx, dataArg ? CopyScopeData<EvalScope>(cx, dataArg) : NewEmptyScopeData<EvalScope>(cx));
    if (!data) return nullptr;

    return createWithData(cx, scopeKind, &data, enclosing);
}

/* static */ EvalScope* EvalScope::createWithData(JSContext* cx, ScopeKind scopeKind,
                                                  MutableHandle<UniquePtr<Data>> data,
                                                  HandleScope enclosing) {
    MOZ_ASSERT(data);
...
template <XDRMode mode>
/* static */ bool EvalScope::XDR(XDRState<mode>* xdr, ScopeKind kind, HandleScope enclosing,
                                 MutableHandleScope scope) {
    JSContext* cx = xdr->cx();
    Rooted<Data*> data(cx);

    {
        ...
        if (!XDRSizedBindingNames<EvalScope>(xdr, scope.as<EvalScope>(), &data)) return false;

        if (mode == XDR_DECODE) {
            if (!data->length) MOZ_ASSERT(!data->nextFrameSlot);
...
Zone* ModuleScope::Data::zone() const { return module ? module->zone() : nullptr; }

/* static */ ModuleScope* ModuleScope::create(JSContext* cx, Handle<Data*> dataArg,
...
/* static */ ModuleScope* ModuleScope::createWithData(JSContext* cx,
                                                      MutableHandle<UniquePtr<Data>> data,
...
JSScript* ModuleScope::script() const { return module()->script(); }

static const uint32_t WasmInstanceEnvShapeFlags = BaseShape::NOT_EXTENSIBLE | BaseShape::DELEGATE;

template <size_t ArrayLength>
static JSAtom* GenerateWasmName(JSContext* cx, const char (&prefix)[ArrayLength], uint32_t index) {
    StringBuffer sb(cx);
    if (!sb.append(prefix)) return nullptr;
    if (!NumberValueToStringBuffer(cx, Int32Value(index), sb)) return nullptr;

    return sb.finishAtom();
}

/* static */ WasmInstanceScope* WasmInstanceScope::create(JSContext* cx,
...

    {
        ...
        Scope* scope =
            Scope::create(cx, ScopeKind::WasmInstance, enclosingScope, /* envShape = */ nullptr);
        if (!scope) return nullptr;
...
// var declaration.
static const uint32_t WasmFunctionEnvShapeFlags = BaseShape::NOT_EXTENSIBLE | BaseShape::DELEGATE;

/* static */ WasmFunctionScope* WasmFunctionScope::create(JSContext* cx, HandleScope enclosing,
                                                          uint32_t funcIndex) {
...
BindingIter::BindingIter(Scope* scope) {
    switch (scope->kind()) {
        ...
        case ScopeKind::SimpleCatch:
        case ScopeKind::Catch:
            init(scope->as<LexicalScope>().data(), scope->as<LexicalScope>().firstFrameSlot(), 0);
            break;
        case ScopeKind::NamedLambda:
        case ScopeKind::StrictNamedLambda:
        ...
        case ScopeKind::Function: {
            ...
            if (scope->as<FunctionScope>().hasParameterExprs()) flags |= HasFormalParameterExprs;
            init(scope->as<FunctionScope>().data(), flags);
            break;
        }
        case ScopeKind::FunctionBodyVar:
        case ScopeKind::ParameterExpressionVar:
            init(scope->as<VarScope>().data(), scope->as<VarScope>().firstFrameSlot());
            break;
        case ScopeKind::Eval:
        case ScopeKind::StrictEval:
...
void BindingIter::init(LexicalScope::Data& data, uint32_t firstFrameSlot, uint8_t flags) {
    ...
    if (flags & IsNamedLambda) {
        ...
        // don't apply.
        init(0, 0, 0, 0, 0, 0, CanHaveEnvironmentSlots | flags, firstFrameSlot,
             JSSLOT_FREE(&LexicalEnvironmentObject::class_), data.names, data.length);
    } else {
        //            imports - [0, 0)
        ...
        init(0, 0, 0, 0, 0, data.constStart, CanHaveFrameSlots | CanHaveEnvironmentSlots | flags,
             firstFrameSlot, JSSLOT_FREE(&LexicalEnvironmentObject::class_), data.names,
             data.length);
    }
}
...
void BindingIter::init(FunctionScope::Data& data, uint8_t flags) {
    ...
    if (!(flags & HasFormalParameterExprs)) flags |= CanHaveArgumentSlots;
    ...
    init(0, data.nonPositionalFormalStart, data.varStart, data.varStart, data.length, data.length,
         flags, 0, JSSLOT_FREE(&CallObject::class_), data.names, data.length);
}
...
void BindingIter::init(VarScope::Data& data, uint32_t firstFrameSlot) {
    ...
    //               lets - [data.length, data.length)
    //             consts - [data.length, data.length)
    init(0, 0, 0, 0, data.length, data.length, CanHaveFrameSlots | CanHaveEnvironmentSlots,
         firstFrameSlot, JSSLOT_FREE(&VarEnvironmentObject::class_), data.names, data.length);
}
...
void BindingIter::init(GlobalScope::Data& data) {
    ...
    //             consts - [data.constStart, data.length)
    init(0, 0, 0, data.varStart, data.letStart, data.constStart, CannotHaveSlots, UINT32_MAX,
         UINT32_MAX, data.names, data.length);
}
...
void BindingIter::init(EvalScope::Data& data, bool strict) {
    ...
    //               lets - [data.length, data.length)
    //             consts - [data.length, data.length)
    init(0, 0, 0, data.varStart, data.length, data.length, flags, firstFrameSlot,
         firstEnvironmentSlot, data.names, data.length);
}
...
void BindingIter::init(ModuleScope::Data& data) {
    ...
    //               lets - [data.letStart, data.constStart)
    //             consts - [data.constStart, data.length)
    init(data.varStart, data.varStart, data.varStart, data.varStart, data.letStart,
         data.constStart, CanHaveFrameSlots | CanHaveEnvironmentSlots, 0,
         JSSLOT_FREE(&ModuleEnvironmentObject::class_), data.names, data.length);
}
...
void BindingIter::init(WasmInstanceScope::Data& data) {
    ...
    //             consts - [data.length, data.length)
    init(0, 0, 0, 0, data.length, data.length, CanHaveFrameSlots | CanHaveEnvironmentSlots,
         UINT32_MAX, UINT32_MAX, data.names, data.length);
}
...
void js::DumpBindings(JSContext* cx, Scope* scopeArg) {
    ...
    for (Rooted<BindingIter> bi(cx, BindingIter(scope)); bi; bi++) {
        ...
        if (!AtomToPrintableString(cx, bi.name(), &bytes)) return;
...
static JSAtom* GetFrameSlotNameInScope(Scope* scope, uint32_t slot) {
    for (BindingIter bi(scope); bi; bi++) {
        BindingLocation loc = bi.location();
        if (loc.kind() == BindingLocation::Kind::Frame && loc.slot() == slot) return bi.name();
    }
    return nullptr;
}
...
JSAtom* js::FrameSlotName(JSScript* script, jsbytecode* pc) {
    ...
    if (JSAtom* name = GetFrameSlotNameInScope(script->bodyScope(), slot)) return name;
    ...
    for (ScopeIter si(script->innermostScope(pc)); si; si++) {
        if (!si.scope()->is<LexicalScope>()) continue;
        LexicalScope& lexicalScope = si.scope()->as<LexicalScope>();

        // Is the slot within bounds of the current lexical scope?
        if (slot < lexicalScope.firstFrameSlot()) continue;
        if (slot >= lexicalScope.nextFrameSlot()) break;

        // If so, get the name.
        if (JSAtom* name = GetFrameSlotNameInScope(&lexicalScope, slot)) return name;
    }

    MOZ_CRASH("Frame slot not found");
}

JS::ubi::Node::Size JS::ubi::Concrete<Scope>::size(mozilla::MallocSizeOf mallocSizeOf) const {
