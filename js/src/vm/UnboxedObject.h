...
class UnboxedLayout : public mozilla::LinkedListElement<UnboxedLayout> {
   public:
    struct Property {
        ...
        Property() : name(nullptr), offset(UINT32_MAX), type(JSVAL_TYPE_MAGIC) {}
    };
   ...
   public:
    explicit UnboxedLayout(Zone* zone)
        : zone_(zone),
          nativeGroup_(nullptr),
          nativeShape_(nullptr),
          allocationScript_(nullptr),
          allocationPc_(nullptr),
          replacementGroup_(nullptr),
          size_(0),
          newScript_(nullptr),
          traceList_(nullptr),
    ...
    ~UnboxedLayout() {
        if (newScript_) newScript_->clear();
    ...
    const Property* lookup(JSAtom* atom) const {
        for (size_t i = 0; i < properties_.length(); i++) {
            if (properties_[i].name == atom) return &properties_[i];
        }
        return nullptr;
    }

    const Property* lookup(jsid id) const {
        if (JSID_IS_STRING(id)) return lookup(JSID_TO_ATOM(id));
        return nullptr;
    }
...
class UnboxedObject : public JSObject {
   protected:
    static JS::Result<UnboxedObject*, JS::OOM&> createInternal(JSContext* cx,
...
class UnboxedPlainObject : public UnboxedObject {
   ...
   public:
    ...
    static bool obj_getProperty(JSContext* cx, HandleObject obj, HandleValue receiver, HandleId id,
    ...
    UnboxedExpandoObject* maybeExpando() const { return expando_; }

    void initExpando() { expando_ = nullptr; }

    // For use during GC.
    JSObject** addressOfExpando() { return reinterpret_cast<JSObject**>(&expando_); }
    ...
    void fillAfterConvert(JSContext* cx, Handle<GCVector<Value>> values, size_t* valueCursor);

    static void trace(JSTracer* trc, JSObject* object);

    static size_t offsetOfExpando() { return offsetof(UnboxedPlainObject, expando_); }

    static size_t offsetOfData() { return offsetof(UnboxedPlainObject, data_[0]); }
};
...
bool TryConvertToUnboxedLayout(JSContext* cx, AutoEnterAnalysis& enter, Shape* templateShape,
                               ObjectGroup* group, PreliminaryObjectArray* objects);

inline gc::AllocKind UnboxedLayout::getAllocKind() const {
    MOZ_ASSERT(size());
