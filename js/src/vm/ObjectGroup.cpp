...
void ObjectGroup::finalize(FreeOp* fop) {
    if (newScriptDontCheckGeneration()) newScriptDontCheckGeneration()->clear();
...
size_t ObjectGroup::sizeOfExcludingThis(mozilla::MallocSizeOf mallocSizeOf) const {
    size_t n = 0;
...
void ObjectGroup::setAddendum(AddendumKind kind, void* addendum, bool writeBarrier /* = true */) {
...
/* static */ bool ObjectGroup::useSingletonForClone(JSFunction* fun) {
    if (!fun->isInterpreted()) return false;

    if (fun->isArrow()) return false;
    ...
    uint32_t begin, end;
    if (fun->hasScript()) {
        if (!fun->nonLazyScript()->isLikelyConstructorWrapper()) return false;
        begin = fun->nonLazyScript()->sourceStart();
        end = fun->nonLazyScript()->sourceEnd();
    } else {
        if (!fun->lazyScript()->isLikelyConstructorWrapper()) return false;
...
/* static */ bool ObjectGroup::useSingletonForNewObject(JSContext* cx, JSScript* script,
                                                        jsbytecode* pc) {
    /*
    ...
    if (script->isStarGenerator() || script->isLegacyGenerator() || script->isAsync())
        return false;
    if (JSOp(*pc) != JSOP_NEW) return false;
    pc += JSOP_NEW_LENGTH;
    if (JSOp(*pc) == JSOP_SETPROP) {
        if (script->getName(pc) == cx->names().prototype) return true;
    }
    return false;
}

/* static */ bool ObjectGroup::useSingletonForAllocationSite(JSScript* script, jsbytecode* pc,
                                                             JSProtoKey key) {
    ...
    if (script->functionNonDelazifying() && !script->treatAsRunOnce()) return GenericObject;

    if (key != JSProto_Object) return GenericObject;

    // All loops in the script will have a try note indicating their boundary.

    if (!script->hasTrynotes()) return SingletonObject;
    ...
    for (; tn < tnlimit; tn++) {
        ...
        if (offset >= startOffset && offset < endOffset) return GenericObject;
    }
...
bool JSObject::shouldSplicePrototype() {
     ...
     */
    if (staticPrototype() != nullptr) return false;
    return isSingleton();
}

/* static */ bool JSObject::splicePrototype(JSContext* cx, HandleObject obj, const Class* clasp,
...
/* static */ ObjectGroup* JSObject::makeLazyGroup(JSContext* cx, HandleObject obj) {
    ...
    if (obj->isIteratedSingleton()) initialFlags |= OBJECT_FLAG_ITERATED;
    ...
    ObjectGroup* group =
        ObjectGroupCompartment::makeGroup(cx, obj->getClass(), proto, initialFlags);
...
/* static */ bool JSObject::setNewGroupUnknown(JSContext* cx, const js::Class* clasp,
                                               JS::HandleObject obj) {
...
struct ObjectGroupCompartment::NewEntry {
    ...
    NewEntry(ObjectGroup* group, JSObject* associated) : group(group), associated(associated) {}
    ...
    struct Lookup {
        ...
        explicit Lookup(const NewEntry& entry)
            ...
            if (associated && associated->is<JSFunction>()) clasp = nullptr;
        }
    };
    ...
    static inline bool match(const ObjectGroupCompartment::NewEntry& key, const Lookup& lookup) {
        if (lookup.clasp && key.group.unbarrieredGet()->clasp() != lookup.clasp) return false;

        TaggedProto proto = key.group.unbarrieredGet()->proto();
        if (!MovableCellHasher<TaggedProto>::match(proto, lookup.proto)) return false;
...
class ObjectGroupCompartment::NewTable
    : public JS::WeakCache<js::GCHashSet<NewEntry, NewEntry, SystemAllocPolicy>> {
...
MOZ_ALWAYS_INLINE ObjectGroup* ObjectGroupCompartment::DefaultNewGroupCache::lookup(
...
/* static */ ObjectGroup* ObjectGroup::defaultNewGroup(JSContext* cx, const Class* clasp,
    ...
    if (associated && !associated->is<TypeDescr>()) {
        ...
        if (!associated) clasp = &PlainObject::class_;
    }
    ...
    if (proto.isObject() && !proto.toObject()->isDelegate()) {
        RootedObject protoObj(cx, proto.toObject());
        if (!JSObject::setDelegate(cx, protoObj)) return nullptr;
        ...
        if (protoObj->is<PlainObject>() && !protoObj->isSingleton()) {
            if (!JSObject::changeToSingleton(cx, protoObj)) return nullptr;
        }
    }
    ...
    ObjectGroup* group = ObjectGroupCompartment::makeGroup(
        cx, clasp ? clasp : &PlainObject::class_, protoRoot, initialFlags);
    if (!group) return nullptr;
    ...
    if (associated) {
        if (associated->is<JSFunction>()) {
            if (!TypeNewScript::make(cx, group, &associated->as<JSFunction>())) return nullptr;
        } else {
...
/* static */ ObjectGroup* ObjectGroup::lazySingletonGroup(JSContext* cx, const Class* clasp,
                                                          TaggedProto proto) {
    ...
    Rooted<TaggedProto> protoRoot(cx, proto);
    ObjectGroup* group = ObjectGroupCompartment::makeGroup(
        cx, clasp, protoRoot, OBJECT_FLAG_SINGLETON | OBJECT_FLAG_LAZY_SINGLETON);
    if (!group) return nullptr;
...
/* static */ void ObjectGroup::setDefaultNewGroupUnknown(JSContext* cx, const Class* clasp,
                                                         HandleObject obj) {
    ...
    if (table) {
        ...
        if (p) MarkObjectGroupUnknownProperties(cx, p->group);
    }
}

#ifdef DEBUG
/* static */ bool ObjectGroup::hasDefaultNewGroup(JSObject* proto, const Class* clasp,
                                                  ObjectGroup* group) {
...
/* static */ ObjectGroup* ObjectGroup::defaultNewGroup(JSContext* cx, JSProtoKey key) {
    RootedObject proto(cx);
    if (key != JSProto_Null && !GetBuiltinPrototype(cx, key, &proto)) return nullptr;
...
struct ObjectGroupCompartment::ArrayObjectKey : public DefaultHasher<ArrayObjectKey> {
    ...
    bool needsSweep() {
        ...
        if (!type.isUnknown() && type.isGroup()) {
            ...
            if (IsAboutToBeFinalizedUnbarriered(&group)) return true;
            if (group != type.groupNoBarrier()) type = TypeSet::ObjectType(group);
        }
        return false;
    }
};
...
/* static */ ArrayObject* ObjectGroup::newArrayObject(JSContext* cx, const Value* vp,
    ...
    if (arrayKind == NewArrayKind::CopyOnWrite) {
        ...
        if (!obj || !ObjectElements::MakeElementsCopyOnWrite(cx, obj)) return nullptr;
        return obj;
    }
    ...
    if (p) {
        ...
        RootedObject proto(cx);
        if (!GetBuiltinPrototype(cx, JSProto_Array, &proto)) return nullptr;
...
static bool GiveObjectGroup(JSContext* cx, JSObject* source, JSObject* target) {
    ...
    if (!target->is<ArrayObject>() || !source->is<ArrayObject>()) return true;
...
bool js::CombineArrayElementTypes(JSContext* cx, JSObject* newObj, const Value* compare,
                                  size_t ncompare) {
    if (!ncompare || !compare[0].isObject()) return true;

    JSObject* oldObj = &compare[0].toObject();
    if (SameGroup(oldObj, newObj)) return true;

    if (!GiveObjectGroup(cx, newObj, oldObj)) return false;

    if (SameGroup(oldObj, newObj)) return true;

    if (!GiveObjectGroup(cx, oldObj, newObj)) return false;

    if (SameGroup(oldObj, newObj)) {
        for (size_t i = 1; i < ncompare; i++) {
            if (compare[i].isObject() && !SameGroup(&compare[i].toObject(), newObj)) {
                if (!GiveObjectGroup(cx, &compare[i].toObject(), newObj)) return false;
            }
        }
    }
...
bool js::CombinePlainObjectPropertyTypes(JSContext* cx, JSObject* newObj, const Value* compare,
                                         size_t ncompare) {
    if (!ncompare || !compare[0].isObject()) return true;
    ...
    if (newObj->is<PlainObject>()) {
        ...
        for (size_t slot = 0; slot < newObj->as<PlainObject>().slotSpan(); slot++) {
            ...
            if (!newValue.isObject() || !oldValue.isObject()) continue;

            JSObject* newInnerObj = &newValue.toObject();
            JSObject* oldInnerObj = &oldValue.toObject();

            if (SameGroup(oldInnerObj, newInnerObj)) continue;

            if (!GiveObjectGroup(cx, newInnerObj, oldInnerObj)) return false;

            if (SameGroup(oldInnerObj, newInnerObj)) continue;

            if (!GiveObjectGroup(cx, oldInnerObj, newInnerObj)) return false;
        ...
        for (; *traceList != -1; traceList++) {
            ...
            if (!newInnerObj || !oldInnerObj || SameGroup(oldInnerObj, newInnerObj)) continue;
                ...
                for (size_t i = 1; i < ncompare; i++) {
                    if (compare[i].isObject() && SameGroup(&compare[i].toObject(), newObj)) {
                        ...
                        if (otherInnerObj && !SameGroup(otherInnerObj, newInnerObj)) {
                            if (!GiveObjectGroup(cx, otherInnerObj, newInnerObj)) return false;
                        }
                    }
                }
            }
...
struct ObjectGroupCompartment::PlainObjectKey {
    ...
    static inline bool match(const PlainObjectKey& v, const Lookup& lookup) {
        if (lookup.nproperties != v.nproperties) return false;
        for (size_t i = 0; i < lookup.nproperties; i++) {
            if (lookup.properties[i].id != v.properties[i]) return false;
        }
        return true;
    }
    ...
    bool needsSweep() {
        for (unsigned i = 0; i < nproperties; i++) {
            if (gc::IsAboutToBeFinalizedUnbarriered(&properties[i])) return true;
        }
        return false;
    }
};
...
struct ObjectGroupCompartment::PlainObjectEntry {
    ...
    bool needsSweep(unsigned nproperties) {
        if (IsAboutToBeFinalized(&group)) return true;
        if (IsAboutToBeFinalized(&shape)) return true;
        for (unsigned i = 0; i < nproperties; i++) {
            MOZ_ASSERT(!types[i].isSingleton());
            if (types[i].isGroup()) {
                ObjectGroup* group = types[i].groupNoBarrier();
                if (IsAboutToBeFinalizedUnbarriered(&group)) return true;
                if (group != types[i].groupNoBarrier()) types[i] = TypeSet::ObjectType(group);
            }
        }
        return false;
    }
...
static bool CanShareObjectGroup(IdValuePair* properties, size_t nproperties) {
    ...
    for (size_t i = 0; i < nproperties; i++) {
        uint32_t index;
        if (IdIsIndex(properties[i].id, &index)) return false;
    }
    return true;
}

static bool AddPlainObjectProperties(JSContext* cx, HandlePlainObject obj, IdValuePair* properties,
                                     size_t nproperties) {
    RootedId propid(cx);
    ...
    for (size_t i = 0; i < nproperties; i++) {
        ...
        if (!NativeDefineDataProperty(cx, obj, propid, value, JSPROP_ENUMERATE)) return false;
    }

    return true;
}

PlainObject* js::NewPlainObjectWithProperties(JSContext* cx, IdValuePair* properties,
    ...
    if (!obj || !AddPlainObjectProperties(cx, obj, properties, nproperties)) return nullptr;
    return obj;
}

/* static */ JSObject* ObjectGroup::newPlainObject(JSContext* cx, IdValuePair* properties,
                                                   size_t nproperties, NewObjectKind newKind) {
    ...
    if (!p) {
        ...
        RootedObject proto(cx);
        if (!GetBuiltinPrototype(cx, JSProto_Object, &proto)) return nullptr;

        Rooted<TaggedProto> tagged(cx, TaggedProto(proto));
        RootedObjectGroup group(
            cx, ObjectGroupCompartment::makeGroup(cx, &PlainObject::class_, tagged));
        if (!group) return nullptr;

        gc::AllocKind allocKind = gc::GetGCObjectKind(nproperties);
        RootedPlainObject obj(
            cx, NewObjectWithGroup<PlainObject>(cx, group, allocKind, TenuredObject));
        if (!obj || !AddPlainObjectProperties(cx, obj, properties, nproperties)) return nullptr;
        ...
        if (!preliminaryObjects) return nullptr;
    ...
    if (!group->unknownProperties()) {
        for (size_t i = 0; i < nproperties; i++) {
            ...
            if (ntype.isPrimitive(JSVAL_TYPE_INT32) && type.isPrimitive(JSVAL_TYPE_DOUBLE)) {
                // The property types already reflect 'int32'.
            } else {
                if (ntype.isPrimitive(JSVAL_TYPE_DOUBLE) && type.isPrimitive(JSVAL_TYPE_INT32)) {
    ...
    if (group->maybePreliminaryObjects()) newKind = TenuredObject;

    gc::AllocKind allocKind = gc::GetGCObjectKind(nproperties);
    RootedPlainObject obj(cx, NewObjectWithGroup<PlainObject>(cx, group, allocKind, newKind));

    if (!obj || !obj->setLastProperty(cx, shape)) return nullptr;

    for (size_t i = 0; i < nproperties; i++) obj->setSlot(i, properties[i].value);
...
struct ObjectGroupCompartment::AllocationSiteKey : public DefaultHasher<AllocationSiteKey> {
    ...
    AllocationSiteKey(const AllocationSiteKey& key)
        : script(key.script), offset(key.offset), kind(key.kind), proto(key.proto) {}
    ...
    bool operator==(const AllocationSiteKey& other) const {
        return script == other.script && offset == other.offset && kind == other.kind &&
               proto == other.proto;
    }
};
...
class ObjectGroupCompartment::AllocationSiteTable
    ...
    using Table = js::GCHashMap<AllocationSiteKey, ReadBarrieredObjectGroup, AllocationSiteKey,
                                SystemAllocPolicy>;
    using Base = JS::WeakCache<Table>;
...
/* static */ ObjectGroup* ObjectGroup::allocationSiteGroup(JSContext* cx, JSScript* scriptArg,
                                                           jsbytecode* pc, JSProtoKey kind,
    ...
    if (offset >= ObjectGroupCompartment::AllocationSiteKey::OFFSET_LIMIT) {
        if (protoArg) return defaultNewGroup(cx, GetClassForProtoKey(kind), TaggedProto(protoArg));
    ...
    if (!proto && kind != JSProto_Null && !GetBuiltinPrototype(cx, kind, &proto)) return nullptr;

    Rooted<ObjectGroupCompartment::AllocationSiteKey> key(
        cx, ObjectGroupCompartment::AllocationSiteKey(script, offset, kind, proto));
...
void ObjectGroupCompartment::replaceAllocationSiteGroup(JSScript* script, jsbytecode* pc,
    ...
    allocationSiteTable->remove(p);
    {
        ...
        if (!allocationSiteTable->putNew(key, group)) oomUnsafe.crash("Inconsistent object table");
    }
}

/* static */ ObjectGroup* ObjectGroup::callingAllocationSiteGroup(JSContext* cx, JSProtoKey key,
                                                                  HandleObject proto) {
    MOZ_ASSERT_IF(proto, key == JSProto_Array);

    jsbytecode* pc;
    RootedScript script(cx, cx->currentScript(&pc));
    if (script) return allocationSiteGroup(cx, script, pc, key, proto);
    if (proto) return defaultNewGroup(cx, GetClassForProtoKey(key), TaggedProto(proto));
    return defaultNewGroup(cx, key);
}

/* static */ bool ObjectGroup::setAllocationSiteObjectGroup(JSContext* cx, HandleScript script,
...
/* static */ ArrayObject* ObjectGroup::getOrFixupCopyOnWriteObject(JSContext* cx,
                                                                   HandleScript script,
                                                                   jsbytecode* pc) {
...
/* static */ ArrayObject* ObjectGroup::getCopyOnWriteObject(JSScript* script, jsbytecode* pc) {
...
/* static */ bool ObjectGroup::findAllocationSite(JSContext* cx, ObjectGroup* group,
    ...
    *offset = 0;
    ...
    for (ObjectGroupCompartment::AllocationSiteTable::Range r = table->all(); !r.empty();
         r.popFront()) {
...
ObjectGroupCompartment::ObjectGroupCompartment() { PodZero(this); }
...
void ObjectGroupCompartment::removeDefaultNewGroup(const Class* clasp, TaggedProto proto,
                                                   JSObject* associated) {
...
void ObjectGroupCompartment::replaceDefaultNewGroup(const Class* clasp, TaggedProto proto,
...
/* static */
ObjectGroup* ObjectGroupCompartment::makeGroup(JSContext* cx, const Class* clasp,
                                               Handle<TaggedProto> proto,
                                               ObjectGroupFlags initialFlags /* = 0 */) {
    MOZ_ASSERT_IF(proto.isObject(), cx->isInsideCurrentCompartment(proto.toObject()));
...
/* static */
ObjectGroup* ObjectGroupCompartment::getStringSplitStringGroup(JSContext* cx) {
    ObjectGroupCompartment& groups = cx->compartment()->objectGroups;
...
void ObjectGroupCompartment::addSizeOfExcludingThis(mozilla::MallocSizeOf mallocSizeOf,
    ...
    if (defaultNewTable) *compartmentTables += defaultNewTable->sizeOfIncludingThis(mallocSizeOf);

    if (lazyTable) *compartmentTables += lazyTable->sizeOfIncludingThis(mallocSizeOf);
}

void ObjectGroupCompartment::clearTables() {
    if (allocationSiteTable && allocationSiteTable->initialized()) allocationSiteTable->clear();
    if (arrayObjectTable && arrayObjectTable->initialized()) arrayObjectTable->clear();
    ...
    if (defaultNewTable && defaultNewTable->initialized()) defaultNewTable->clear();
    if (lazyTable && lazyTable->initialized()) lazyTable->clear();
    defaultNewGroupCache.purge();
}

/* static */ bool ObjectGroupCompartment::PlainObjectTableSweepPolicy::needsSweep(
...
void ObjectGroupCompartment::sweep(FreeOp* fop) {
    /*
     * Iterate through the array/object group tables and remove all entries
     * referencing collected data. These tables only hold weak references.
     */

    if (arrayObjectTable) arrayObjectTable->sweep();
    if (plainObjectTable) plainObjectTable->sweep();
...
void ObjectGroupCompartment::fixupNewTableAfterMovingGC(NewTable* table) {
    /*
...
void ObjectGroupCompartment::checkNewTableAfterMovingGC(NewTable* table) {
    /*
