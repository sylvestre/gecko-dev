...
RegExpObject* js::RegExpAlloc(JSContext* cx, NewObjectKind newKind,
    ...
    if (!EmptyShape::ensureInitialCustomShape<RegExpObject>(cx, regexp)) return nullptr;

    MOZ_ASSERT(regexp->lookupPure(cx->names().lastIndex)->slot() == RegExpObject::lastIndexSlot());
...
bool MatchPairs::initArrayFrom(MatchPairs& copyFrom) {
    MOZ_ASSERT(copyFrom.pairCount() > 0);

    if (!allocOrExpandArray(copyFrom.pairCount())) return false;
...
bool ScopedMatchPairs::allocOrExpandArray(size_t pairCount) {
...
bool VectorMatchPairs::allocOrExpandArray(size_t pairCount) {
    if (!vec_.resizeUninitialized(sizeof(MatchPair) * pairCount)) return false;
...
/* static */ RegExpShared* RegExpObject::getShared(JSContext* cx, Handle<RegExpObject*> regexp) {
    if (regexp->hasShared()) return regexp->sharedRef();

    return createShared(cx, regexp);
}

/* static */ bool RegExpObject::isOriginalFlagGetter(JSNative native, RegExpFlag* mask) {
...
static JSObject* CreateRegExpPrototype(JSContext* cx, JSProtoKey key) {
...
const Class RegExpObject::class_ = {js_RegExp_str,
                                    JSCLASS_HAS_PRIVATE |
                                        JSCLASS_HAS_RESERVED_SLOTS(RegExpObject::RESERVED_SLOTS) |
                                        JSCLASS_HAS_CACHED_PROTO(JSProto_RegExp),
                                    &RegExpObjectClassOps, &RegExpObjectClassSpec};

const Class RegExpObject::protoClass_ = {js_Object_str, JSCLASS_HAS_CACHED_PROTO(JSProto_RegExp),
                                         JS_NULL_CLASS_OPS, &RegExpObjectClassSpec};

RegExpObject* RegExpObject::create(JSContext* cx, const char16_t* chars, size_t length,
                                   RegExpFlag flags, const ReadOnlyCompileOptions* options,
...
RegExpObject* RegExpObject::create(JSContext* cx, HandleAtom source, RegExpFlag flags,
    ...
    if (!tokenStream) {
        dummyTokenStream.emplace(cx, *options, (const char16_t*)nullptr, 0,
...
/* static */ RegExpShared* RegExpObject::createShared(JSContext* cx,
...
Shape* RegExpObject::assignInitialShape(JSContext* cx, Handle<RegExpObject*> self) {
...
void RegExpObject::initAndZeroLastIndex(JSAtom* source, RegExpFlag flags, JSContext* cx) {
    initIgnoringLastIndex(source, flags);
    zeroLastIndex(cx);
}

static MOZ_ALWAYS_INLINE bool IsLineTerminator(const JS::Latin1Char c) {
    return c == '\n' || c == '\r';
}

static MOZ_ALWAYS_INLINE bool IsLineTerminator(const char16_t c) {
...
static MOZ_ALWAYS_INLINE bool AppendEscapedLineTerminator(StringBuffer& sb,
    ...
    switch (c) {
        case '\n':
            if (!sb.append('n')) return false;
            break;
        case '\r':
            if (!sb.append('r')) return false;
            break;
        default:
...
static MOZ_ALWAYS_INLINE bool AppendEscapedLineTerminator(StringBuffer& sb, const char16_t c) {
    switch (c) {
        case '\n':
            if (!sb.append('n')) return false;
            break;
        case '\r':
            ...
            break;
        case 0x2028:
            if (!sb.append("u2028")) return false;
            break;
        case 0x2029:
            if (!sb.append("u2029")) return false;
            break;
        default:
            MOZ_CRASH("Bad LineTerminator");
    }
...
static MOZ_ALWAYS_INLINE bool SetupBuffer(StringBuffer& sb, const CharT* oldChars, size_t oldLen,
                                          const CharT* it) {
    if (mozilla::IsSame<CharT, char16_t>::value && !sb.ensureTwoByteChars()) return false;

    if (!sb.reserve(oldLen + 1)) return false;
...
template <typename CharT>
static bool EscapeRegExpPattern(StringBuffer& sb, const CharT* oldChars, size_t oldLen) {
    ...
    for (const CharT* it = oldChars; it < oldChars + oldLen; ++it) {
        ...
        if (!previousCharacterWasBackslash) {
            if (inBrackets) {
                ...
                if (sb.empty()) {
                    ...
                    if (!SetupBuffer(sb, oldChars, oldLen, it)) return false;
                }
        ...
        if (IsLineTerminator(ch)) {
            ...
            if (!AppendEscapedLineTerminator(sb, ch)) return false;
        } else if (!sb.empty()) {
            if (!sb.append(ch)) return false;
        }
...
JSAtom* js::EscapeRegExpPattern(JSContext* cx, HandleAtom src) {
    // Step 2.
    if (src->length() == 0) return cx->names().emptyRegExp;
    ...
    if (src->hasLatin1Chars()) {
        ...
        if (!::EscapeRegExpPattern(sb, src->latin1Chars(nogc), src->length())) return nullptr;
    } else {
        JS::AutoCheckCannotGC nogc;
        if (!::EscapeRegExpPattern(sb, src->twoByteChars(nogc), src->length())) return nullptr;
    }
...
JSFlatString* RegExpObject::toString(JSContext* cx) const {
    ...
    if (!sb.reserve(len + 2)) return nullptr;
    sb.infallibleAppend('/');
    if (!sb.append(escapedSrc)) return nullptr;
    sb.infallibleAppend('/');

    // Steps 5-7.
    if (global() && !sb.append('g')) return nullptr;
    if (ignoreCase() && !sb.append('i')) return nullptr;
    if (multiline() && !sb.append('m')) return nullptr;
    if (unicode() && !sb.append('u')) return nullptr;
    if (sticky() && !sb.append('y')) return nullptr;

    return sb.finishString();
}

#ifdef DEBUG
/* static */ bool RegExpShared::dumpBytecode(JSContext* cx, MutableHandleRegExpShared re,
                                             bool match_only, HandleLinearString input) {
    CompilationMode mode = match_only ? MatchOnly : Normal;
    if (!RegExpShared::compileIfNecessary(cx, re, input, mode, ForceByteCode)) return false;
...
#define BRANCH(NAME, OFFSET)                                \
    ...
    while (pc <= maxPc) {
        ...
        switch (insn & irregexp::BYTECODE_MASK) {
            ...
            BYTECODE(LOAD_4_CURRENT_CHARS_UNCHECKED) { ADVANCE(LOAD_4_CURRENT_CHARS_UNCHECKED); }
            ...
            BYTECODE(AND_CHECK_4_CHARS) {
                ...
                fprintf(stderr, " %d, %d, %d", Load32Aligned(pc + 4), Load32Aligned(pc + 8),
                        offset);
            ...
            BYTECODE(CHECK_CHAR_IN_RANGE) {
                int32_t offset = Load32Aligned(pc + 8);
                fprintf(stderr, " %d, %d, %d", Load16Aligned(pc + 4), Load16Aligned(pc + 6),
                        offset);
            ...
            BYTECODE(CHECK_BIT_IN_TABLE) {
                ...
                fprintf(stderr,
                        ...
                        "%02x %02x %02x %02x %02x %02x %02x %02x",
                        offset, pc[8], pc[9], pc[10], pc[11], pc[12], pc[13], pc[14], pc[15],
            ...
            BYTECODE(CHECK_REGISTER_EQ_POS) {
                int32_t offset = Load32Aligned(pc + 4);
                fprintf(stderr, " reg[%d], %d", insn >> irregexp::BYTECODE_SHIFT, offset);
            ...
            BYTECODE(SET_CURRENT_POSITION_FROM_END) {
                fprintf(stderr, " %u", static_cast<uint32_t>(insn) >> irregexp::BYTECODE_SHIFT);
...
/* static */ bool RegExpObject::dumpBytecode(JSContext* cx, Handle<RegExpObject*> regexp,
                                             bool match_only, HandleLinearString input) {
...
bool js::HasRegExpMetaChars(const CharT* chars, size_t length) {
    for (size_t i = 0; i < length; ++i) {
        if (IsRegExpMetaChar<CharT>(chars[i])) return true;
    }
    return false;
}

template bool js::HasRegExpMetaChars<Latin1Char>(const Latin1Char* chars, size_t length);

template bool js::HasRegExpMetaChars<char16_t>(const char16_t* chars, size_t length);

bool js::StringHasRegExpMetaChars(JSLinearString* str) {
    AutoCheckCannotGC nogc;
    if (str->hasLatin1Chars()) return HasRegExpMetaChars(str->latin1Chars(nogc), str->length());
...
void RegExpShared::traceChildren(JSTracer* trc) {
    ...
    if (IsMarkingTrace(trc) && trc->runtime()->gc.isShrinkingGC()) discardJitCode();

    TraceNullableEdge(trc, &source, "RegExpShared source");
    for (auto& comp : compilationArray) TraceNullableEdge(trc, &comp.jitCode, "RegExpShared code");
}

void RegExpShared::discardJitCode() {
    for (auto& comp : compilationArray) comp.jitCode = nullptr;
...
void RegExpShared::finalize(FreeOp* fop) {
    for (auto& comp : compilationArray) js_free(comp.byteCode);
    tables.~JitCodeTables();
}

/* static */ bool RegExpShared::compile(JSContext* cx, MutableHandleRegExpShared re,
...
/* static */ bool RegExpShared::compile(JSContext* cx, MutableHandleRegExpShared re,
    ...
    if (!re->ignoreCase() && !StringHasRegExpMetaChars(pattern)) re->canStringMatch = true;
    ...
    if (!irregexp::ParsePattern(dummyTokenStream, cx->tempLifoAlloc(), pattern, re->multiline(),
                                mode == MatchOnly, re->unicode(), re->ignoreCase(), re->global(),
                                re->sticky(), &data)) {
        return false;
    }
    ...
    irregexp::RegExpCode code = irregexp::CompilePattern(
        cx, re, &data, input, false /* global() */, re->ignoreCase(), input->hasLatin1Chars(),
        mode == MatchOnly, force == ForceByteCode, re->sticky(), re->unicode(), tables);
    ...
    if (code.jitCode) {
        ...
        for (size_t i = 0; i < tables.length(); i++) {
            if (!re->addTable(Move(tables[i]))) return false;
        }
...
/* static */ bool RegExpShared::compileIfNecessary(JSContext* cx, MutableHandleRegExpShared re,
                                                   HandleLinearString input, CompilationMode mode,
                                                   ForceByteCodeEnum force) {
    if (re->isCompiled(mode, input->hasLatin1Chars(), force)) return true;
    return compile(cx, re, input, mode, force);
}

/* static */ RegExpRunStatus RegExpShared::execute(JSContext* cx, MutableHandleRegExpShared re,
    ...
    if (!compileIfNecessary(cx, re, input, mode, DontForceByteCode)) return RegExpRunStatus_Error;
    ...
    if (re->canStringMatch) {
        ...
        if (re->sticky()) {
            ...
            if (!HasSubstringAt(input, re->source, start)) return RegExpRunStatus_Success_NotFound;
        ...
        if (res == -1) return RegExpRunStatus_Success_NotFound;
    ...
    do {
        ...
        if (result == RegExpRunStatus_Error) {
            ...
            if (!jit::CheckOverRecursed(cx)) return RegExpRunStatus_Error;
            break;
        }

        if (result == RegExpRunStatus_Success_NotFound) return RegExpRunStatus_Success_NotFound;

        MOZ_ASSERT(result == RegExpRunStatus_Success);

        if (matches) matches->checkAgainst(length);
        return RegExpRunStatus_Success;
    } while (false);

    // Compile bytecode for the RegExp if necessary.
    if (!compileIfNecessary(cx, re, input, mode, ForceByteCode)) return RegExpRunStatus_Error;
    ...
    if (!inputChars.init(cx, input)) return RegExpRunStatus_Error;
    ...
    if (result == RegExpRunStatus_Success && matches) matches->checkAgainst(length);
    return result;
}
...
size_t RegExpShared::sizeOfExcludingThis(mozilla::MallocSizeOf mallocSizeOf) {
    ...
    for (size_t i = 0; i < ArrayLength(compilationArray); i++) {
        ...
        if (compilation.byteCode) n += mallocSizeOf(compilation.byteCode);
    }

    n += tables.sizeOfExcludingThis(mallocSizeOf);
    for (size_t i = 0; i < tables.length(); i++) n += mallocSizeOf(tables[i].get());
...
ArrayObject* RegExpCompartment::createMatchResultTemplateObject(JSContext* cx) {
    MOZ_ASSERT(!matchResultTemplateObject_);

    /* Create template array object */
    RootedArrayObject templateObject(
        cx, NewDenseUnallocatedArray(cx, RegExpObject::MaxPairCount, nullptr, TenuredObject));
    if (!templateObject) return matchResultTemplateObject_;  // = nullptr

    // Create a new group for the template.
    Rooted<TaggedProto> proto(cx, templateObject->taggedProto());
    ObjectGroup* group = ObjectGroupCompartment::makeGroup(cx, templateObject->getClass(), proto);
    if (!group) return matchResultTemplateObject_;  // = nullptr
    ...
    MOZ_ASSERT(shape->slot() == 1 && shape->propidRef() == NameToId(cx->names().input));
...
bool RegExpZone::init() {
    if (!set_.init(0)) return false;

    return true;
}

void RegExpCompartment::sweep(JSRuntime* rt) {
    if (matchResultTemplateObject_ && IsAboutToBeFinalized(&matchResultTemplateObject_)) {
...
RegExpShared* RegExpZone::get(JSContext* cx, HandleAtom source, RegExpFlag flags) {
...
RegExpShared* RegExpZone::get(JSContext* cx, HandleAtom atom, JSString* opt) {
    ...
    if (opt && !ParseRegExpFlags(cx, opt, &flags)) return nullptr;
...
JSObject* js::CloneRegExpObject(JSContext* cx, Handle<RegExpObject*> regex) {
    ...
    if (!EmptyShape::ensureInitialCustomShape<RegExpObject>(cx, clone)) return nullptr;
...
template <typename CharT>
static bool ParseRegExpFlags(const CharT* chars, size_t length, RegExpFlag* flagsOut,
...
bool js::ParseRegExpFlags(JSContext* cx, JSString* flagStr, RegExpFlag* flagsOut) {
...
bool js::XDRScriptRegExpObject(XDRState<mode>* xdr, MutableHandle<RegExpObject*> objp) {
    ...
    if (!XDRAtom(xdr, &source) || !xdr->codeUint32(&flagsword)) return false;
    if (mode == XDR_DECODE) {
        RegExpFlag flags = RegExpFlag(flagsword);
        const ReadOnlyCompileOptions* options = nullptr;
        if (xdr->hasOptions()) options = &xdr->options();
        RegExpObject* reobj = RegExpObject::create(xdr->cx(), source, flags, options, nullptr,
...
template bool js::XDRScriptRegExpObject(XDRState<XDR_ENCODE>* xdr,
                                        MutableHandle<RegExpObject*> objp);

template bool js::XDRScriptRegExpObject(XDRState<XDR_DECODE>* xdr,
                                        MutableHandle<RegExpObject*> objp);
...
JSObject* js::CloneScriptRegExpObject(JSContext* cx, RegExpObject& reobj) {
    ...
    return RegExpObject::create(cx, source, reobj.getFlags(), nullptr, nullptr,
                                cx->tempLifoAlloc(), TenuredObject);
}

JS_FRIEND_API(RegExpShared*)
js::RegExpToSharedNonInline(JSContext* cx, HandleObject obj) { return RegExpToShared(cx, obj); }
