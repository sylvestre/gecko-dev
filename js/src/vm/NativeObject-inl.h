...
inline void NativeObject::initDenseElementWithType(JSContext* cx, uint32_t index,
                                                   const Value& val) {
...
/* static */ inline void NativeObject::removeDenseElementForSparseIndex(JSContext* cx,
...
inline void NativeObject::elementsRangeWriteBarrierPost(uint32_t start, uint32_t count) {
    for (size_t i = 0; i < count; i++) {
        const Value& v = elements_[start + i];
        if (v.isObject() && IsInsideNursery(&v.toObject())) {
            zone()->group()->storeBuffer().putSlot(this, HeapSlot::Element,
                                                   unshiftedIndex(start + i), count - i);
            return;
        }
    }
}

inline void NativeObject::copyDenseElements(uint32_t dstStart, const Value* src, uint32_t count) {
    ...
    if (JS::shadow::Zone::asShadowZone(zone())->needsIncrementalBarrier()) {
        ...
        for (uint32_t i = 0; i < count; ++i) {
            elements_[dstStart + i].set(this, HeapSlot::Element, dstStart + i + numShifted,
                                        src[i]);
        }
    } else {
...
inline void NativeObject::initDenseElements(NativeObject* src, uint32_t srcStart, uint32_t count) {
...
inline bool NativeObject::tryShiftDenseElements(uint32_t count) {
    ...
    if (header->initializedLength == count || count > ObjectElements::MaxShiftedElements ||
        header->isCopyOnWrite() || header->isFrozen() || header->hasNonwritableArrayLength()) {
        return false;
    }
...
inline void NativeObject::moveDenseElements(uint32_t dstStart, uint32_t srcStart, uint32_t count) {
    MOZ_ASSERT(dstStart + count <= getDenseCapacity());
...
inline void NativeObject::moveDenseElementsNoPreBarrier(uint32_t dstStart, uint32_t srcStart,
                                                        uint32_t count) {
...
inline void NativeObject::ensureDenseInitializedLengthNoPackedCheck(JSContext* cx, uint32_t index,
                                                                    uint32_t extra) {
    MOZ_ASSERT(!denseElementsAreCopyOnWrite());
    MOZ_ASSERT(!denseElementsAreFrozen());
...
inline void NativeObject::ensureDenseInitializedLength(JSContext* cx, uint32_t index,
                                                       uint32_t extra) {
...
DenseElementResult NativeObject::extendDenseElements(JSContext* cx, uint32_t requiredCapacity,
                                                     uint32_t extra) {
    MOZ_ASSERT(!denseElementsAreCopyOnWrite());
    MOZ_ASSERT(!denseElementsAreFrozen());
     ...
     */
    if (requiredCapacity > MIN_SPARSE_INDEX && willBeSparseElements(requiredCapacity, extra)) {
        return DenseElementResult::Incomplete;
    }
...
inline DenseElementResult NativeObject::ensureDenseElements(JSContext* cx, uint32_t index,
                                                            uint32_t extra) {
    MOZ_ASSERT(isNative());
...
inline DenseElementResult NativeObject::setOrExtendDenseElements(JSContext* cx, uint32_t start,
                                                                 const Value* vp, uint32_t count,
    ...
    if (updateTypes == ShouldUpdateTypes::DontUpdate && !shouldConvertDoubleElements()) {
        ...
        for (size_t i = 0; i < count; i++) setDenseElementWithType(cx, start + i, vp[i]);
    }
...
/* static */ inline NativeObject* NativeObject::copy(JSContext* cx, gc::AllocKind kind,
                                                     gc::InitialHeap heap,
...
MOZ_ALWAYS_INLINE void NativeObject::setSlotWithType(JSContext* cx, Shape* shape,
...
inline void NativeObject::updateShapeAfterMovingGC() {
    Shape* shape = shape_;
    if (IsForwarded(shape)) shape_.unsafeSet(Forwarded(shape));
}
...
/* static */ inline JS::Result<NativeObject*, JS::OOM&> NativeObject::create(
    JSContext* cx, js::gc::AllocKind kind, js::gc::InitialHeap heap, js::HandleShape shape,
    ...
    if (clasp->hasPrivate()) nobj->privateRef(shape->numFixedSlots()) = nullptr;
    ...
    if (clasp->isJSFunction()) {
        ...
        size_t size = kind == js::gc::AllocKind::FUNCTION ? sizeof(JSFunction)
...
/* static */ inline JS::Result<NativeObject*, JS::OOM&> NativeObject::createWithTemplate(
...
/* static */ MOZ_ALWAYS_INLINE uint32_t NativeObject::dynamicSlotsCount(uint32_t nfixed,
                                                                        uint32_t span,
                                                                        const Class* clasp) {
    ...
    if (clasp != &ArrayObject::class_ && span <= SLOT_CAPACITY_MIN) return SLOT_CAPACITY_MIN;
...
/* static */ MOZ_ALWAYS_INLINE uint32_t NativeObject::dynamicSlotsCount(Shape* shape) {
    return dynamicSlotsCount(shape->numFixedSlots(), shape->slotSpan(), shape->getObjectClass());
}

MOZ_ALWAYS_INLINE bool NativeObject::updateSlotsForSpan(JSContext* cx, size_t oldSpan,
                                                        size_t newSpan) {
...
MOZ_ALWAYS_INLINE bool NativeObject::setLastProperty(JSContext* cx, Shape* shape) {
    MOZ_ASSERT(!inDictionaryMode());
...
static inline PlainObject* CopyInitializerObject(JSContext* cx, HandlePlainObject baseobj,
...
inline NativeObject* NewNativeObjectWithGivenTaggedProto(JSContext* cx, const Class* clasp,
                                                         Handle<TaggedProto> proto,
                                                         gc::AllocKind allocKind,
                                                         NewObjectKind newKind) {
    return MaybeNativeObject(NewObjectWithGivenTaggedProto(cx, clasp, proto, allocKind, newKind));
}
...
inline NativeObject* NewNativeObjectWithGivenProto(JSContext* cx, const Class* clasp,
...
inline NativeObject* NewNativeObjectWithClassProto(JSContext* cx, const Class* clasp,
                                                   HandleObject proto, gc::AllocKind allocKind,
                                                   NewObjectKind newKind = GenericObject) {
 ...
 */
static MOZ_ALWAYS_INLINE bool CallResolveOp(JSContext* cx, HandleNativeObject obj, HandleId id,
...
static MOZ_ALWAYS_INLINE bool LookupOwnPropertyInline(
    ...
    typename MaybeRooted<PropertyResult, allowGC>::MutableHandleType propp, bool* donep) {
    ...
    if (obj->getClass()->getResolve()) {
        ...
        bool recursed;
        if (!CallResolveOp(cx, MaybeRooted<NativeObject*, allowGC>::toHandle(obj),
 ...
 * hooks.
 */
static inline void NativeLookupOwnPropertyNoResolve(JSContext* cx, HandleNativeObject obj,
                                                    HandleId id,
...
static MOZ_ALWAYS_INLINE bool LookupPropertyInline(
    ...
    while (true) {
        bool done;
        if (!LookupOwnPropertyInline<allowGC>(cx, current, id, propp, &done)) return false;
        if (done) {
            if (propp)
                objp.set(current);
            else
        ...
        if (!proto->isNative()) {
            MOZ_ASSERT(!cx->helperThread());
            if (!allowGC) return false;
            return LookupProperty(cx, MaybeRooted<JSObject*, allowGC>::toHandle(proto),
                                  MaybeRooted<jsid, allowGC>::toHandle(id),
