...
class SavedFrame : public NativeObject {
   ...
   public:
    ...
    class ConstIterator {
       ...
       public:
        ...
        const SavedFrame& operator*() const {
            MOZ_ASSERT(frame_);
            return *frame_;
        }
    ...
    typedef JS::GCHashSet<ReadBarriered<SavedFrame*>, HashPolicy, SystemAllocPolicy> Set;
   ...
   private:
    ...
    static MOZ_MUST_USE bool finishSavedFrameInit(JSContext* cx, HandleObject ctor,
                                                  HandleObject proto);
...
struct ReconstructedSavedFramePrincipals : public JSPrincipals {
    ...
    MOZ_MUST_USE bool write(JSContext* cx, JSStructuredCloneWriter* writer) override {
        MOZ_ASSERT(false,
                   "ReconstructedSavedFramePrincipals should never be exposed to embedders");
        return false;
    }
...
inline void SavedFrame::Iterator::operator++() { frame_ = frame_->getParent(); }

inline void SavedFrame::ConstIterator::operator++() { frame_ = frame_->getParent(); }
...
class ConcreteStackFrame<SavedFrame> : public BaseStackFrame {
   ...
   public:
    ...
    bool isSelfHosted(JSContext* cx) const override { return get().isSelfHosted(cx); }
