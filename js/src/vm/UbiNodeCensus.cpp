...
class SimpleCount : public CountType {
   ...
   public:
    explicit SimpleCount(UniqueTwoByteChars& label, bool reportCount = true,
        ...
        : CountType(), label(Move(label)), reportCount(reportCount), reportBytes(reportBytes) {}

    explicit SimpleCount() : CountType(), label(nullptr), reportCount(true), reportBytes(true) {}
...
bool SimpleCount::count(CountBase& countBase, mozilla::MallocSizeOf mallocSizeOf,
                        const Node& node) {
    Count& count = static_cast<Count&>(countBase);
...
bool SimpleCount::report(JSContext* cx, CountBase& countBase, MutableHandleValue report) {
    Count& count = static_cast<Count&>(countBase);
    ...
    if (reportCount && !DefineDataProperty(cx, obj, cx->names().count, countValue)) return false;

    RootedValue bytesValue(cx, NumberValue(count.totalBytes_));
    if (reportBytes && !DefineDataProperty(cx, obj, cx->names().bytes, bytesValue)) return false;
    ...
    if (label) {
        ...
        if (!DefineDataProperty(cx, obj, cx->names().label, labelValue)) return false;
    }
...
bool BucketCount::count(CountBase& countBase, mozilla::MallocSizeOf mallocSizeOf,
                        const Node& node) {
    Count& count = static_cast<Count&>(countBase);
    return count.ids_.append(node.identifier());
}

bool BucketCount::report(JSContext* cx, CountBase& countBase, MutableHandleValue report) {
    Count& count = static_cast<Count&>(countBase);
    ...
    for (size_t i = 0; i < length; i++) arr->setDenseElement(i, NumberValue(count.ids_[i]));
...
class ByCoarseType : public CountType {
    ...
    struct Count : CountBase {
        Count(CountType& type, CountBasePtr& objects, CountBasePtr& scripts, CountBasePtr& strings,
              CountBasePtr& other)
            : CountBase(type),
   ...
   public:
    ByCoarseType(CountTypePtr& objects, CountTypePtr& scripts, CountTypePtr& strings,
                 CountTypePtr& other)
        : CountType(),
          objects(Move(objects)),
          scripts(Move(scripts)),
...
CountBasePtr ByCoarseType::makeCount() {
    ...
    return CountBasePtr(
        js_new<Count>(*this, objectsCount, scriptsCount, stringsCount, otherCount));
}
...
bool ByCoarseType::count(CountBase& countBase, mozilla::MallocSizeOf mallocSizeOf,
                         const Node& node) {
    Count& count = static_cast<Count&>(countBase);
...
bool ByCoarseType::report(JSContext* cx, CountBase& countBase, MutableHandleValue report) {
    Count& count = static_cast<Count&>(countBase);
...
static PlainObject* countMapToObject(JSContext* cx, Map& map, GetName getName) {
    ...
    for (auto r = map.all(); !r.empty(); r.popFront()) entries.infallibleAppend(&r.front());
...
class ByObjectClass : public CountType {
    ...
    struct Count : public CountBase {
        ...
        Count(CountType& type, CountBasePtr& other) : CountBase(type), other(Move(other)) {}
   ...
   public:
    ByObjectClass(CountTypePtr& classesType, CountTypePtr& otherType)
        : CountType(), classesType(Move(classesType)), otherType(Move(otherType)) {}
...
bool ByObjectClass::count(CountBase& countBase, mozilla::MallocSizeOf mallocSizeOf,
                          const Node& node) {
    Count& count = static_cast<Count&>(countBase);
    ...
    if (!p) {
        ...
        if (!classCount || !count.table.add(p, className, Move(classCount))) return false;
    }
    return p->value()->count(mallocSizeOf, node);
}

bool ByObjectClass::report(JSContext* cx, CountBase& countBase, MutableHandleValue report) {
    Count& count = static_cast<Count&>(countBase);
...
class ByUbinodeType : public CountType {
    ...
    using Table =
        HashMap<const char16_t*, CountBasePtr, DefaultHasher<const char16_t*>, SystemAllocPolicy>;
    using Entry = Table::Entry;
   ...
   public:
    explicit ByUbinodeType(CountTypePtr& entryType) : CountType(), entryType(Move(entryType)) {}
...
bool ByUbinodeType::count(CountBase& countBase, mozilla::MallocSizeOf mallocSizeOf,
                          const Node& node) {
    Count& count = static_cast<Count&>(countBase);
...
bool ByUbinodeType::report(JSContext* cx, CountBase& countBase, MutableHandleValue report) {
    Count& count = static_cast<Count&>(countBase);
...
class ByAllocationStack : public CountType {
    using Table = HashMap<StackFrame, CountBasePtr, DefaultHasher<StackFrame>, SystemAllocPolicy>;
    using Entry = Table::Entry;
    ...
    struct Count : public CountBase {
        ...
        Count(CountType& type, CountBasePtr& noStack) : CountBase(type), noStack(Move(noStack)) {}
        bool init() { return table.init(); }
    };
   ...
   public:
    ByAllocationStack(CountTypePtr& entryType, CountTypePtr& noStackType)
        : CountType(), entryType(Move(entryType)), noStackType(Move(noStackType)) {}
...
bool ByAllocationStack::count(CountBase& countBase, mozilla::MallocSizeOf mallocSizeOf,
                              const Node& node) {
    Count& count = static_cast<Count&>(countBase);
...
bool ByAllocationStack::report(JSContext* cx, CountBase& countBase, MutableHandleValue report) {
    Count& count = static_cast<Count&>(countBase);
...
class ByFilename : public CountType {
    ...
    using Table = HashMap<UniqueCString, CountBasePtr, UniqueCStringHasher, SystemAllocPolicy>;
    using Entry = Table::Entry;
   ...
   public:
    ByFilename(CountTypePtr&& thenType, CountTypePtr&& noFilenameType)
        : CountType(), thenType(Move(thenType)), noFilenameType(Move(noFilenameType)) {}
...
bool ByFilename::count(CountBase& countBase, mozilla::MallocSizeOf mallocSizeOf,
    ...
    if (!p) {
        CountBasePtr thenCount(thenType->makeCount());
        if (!thenCount || !count.table.add(p, Move(myFilename), Move(thenCount))) return false;
    }
    return p->value()->count(mallocSizeOf, node);
}

bool ByFilename::report(JSContext* cx, CountBase& countBase, MutableHandleValue report) {
    Count& count = static_cast<Count&>(countBase);

    RootedPlainObject obj(
        cx, countMapToObject(cx, count.table, [](const UniqueCString& key) { return key.get(); }));
    if (!obj) return false;
...
JS_PUBLIC_API(bool)
CensusHandler::operator()(BreadthFirst<CensusHandler>& traversal, Node origin, const Edge& edge,
...
ParseBreakdown(JSContext* cx, HandleValue breakdownValue) {
    ...
    if (!GetProperty(cx, breakdown, breakdown, cx->names().by, &byValue)) return nullptr;
    ...
    if (StringEqualsAscii(by, "count")) {
        ...
        if (!GetProperty(cx, breakdown, breakdown, cx->names().label, &label)) return nullptr;
        ...
        CountTypePtr simple(
            cx->new_<SimpleCount>(labelUnique, ToBoolean(countValue), ToBoolean(bytesValue)));
        return simple;
    }

    if (StringEqualsAscii(by, "bucket")) return CountTypePtr(cx->new_<BucketCount>());
    ...
    if (StringEqualsAscii(by, "coarseType")) {
        ...
        return CountTypePtr(
            cx->new_<ByCoarseType>(objectsType, scriptsType, stringsType, otherType));
    }
...
ParseCensusOptions(JSContext* cx, Census& census, HandleObject options, CountTypePtr& outResult) {
    ...
    outResult = breakdown.isUndefined() ? GetDefaultBreakdown(cx) : ParseBreakdown(cx, breakdown);
    return !!outResult;
}
