 ...
 */
extern Shape* EnvironmentCoordinateToEnvironmentShape(JSScript* script, jsbytecode* pc);

/* Return the name being accessed by the given ALIASEDVAR op. */
extern PropertyName* EnvironmentCoordinateName(EnvironmentCoordinateNameCache& cache,
                                               JSScript* script, jsbytecode* pc);

/* Return the function script accessed by the given ALIASEDVAR op, or nullptr. */
extern JSScript* EnvironmentCoordinateFunctionScript(JSScript* script, jsbytecode* pc);
...
class EnvironmentObject : public NativeObject {
   ...
   public:
    ...
    const Value& aliasedBinding(EnvironmentCoordinate ec) { return getSlot(ec.slot()); }
    ...
    // For JITs.
    static size_t offsetOfEnclosingEnvironment() { return getFixedSlotOffset(ENCLOSING_ENV_SLOT); }
...
class CallObject : public EnvironmentObject {
   ...
   public:
    ...
    JSFunction& callee() const { return getReservedSlot(CALLEE_SLOT).toObject().as<JSFunction>(); }
...
class VarEnvironmentObject : public EnvironmentObject {
    ...
    void initScope(Scope* scope) { initReservedSlot(SCOPE_SLOT, PrivateGCThingValue(scope)); }
...
class LexicalEnvironmentObject : public EnvironmentObject {
   ...
   public:
    ...
    static LexicalEnvironmentObject* recreate(JSContext* cx,
    ...
    GlobalObject& global() const { return enclosingEnvironment().as<GlobalObject>(); }
...
class NamedLambdaObject : public LexicalEnvironmentObject {
    static NamedLambdaObject* create(JSContext* cx, HandleFunction callee,
                                     HandleFunction replacement, HandleObject enclosing,
                                     gc::InitialHeap heap);
...
class WithEnvironmentObject : public EnvironmentObject {
   ...
   public:
    ...
    static WithEnvironmentObject* create(JSContext* cx, HandleObject object,
                                         HandleObject enclosing, Handle<WithScope*> scope);
...
class MOZ_RAII EnvironmentIter {
   ...
   public:
    ...
    EnvironmentIter(JSContext* cx, const EnvironmentIter& ei MOZ_GUARD_OBJECT_NOTIFIER_PARAM);
    ...
    EnvironmentIter(JSContext* cx, JSObject* env, Scope* scope MOZ_GUARD_OBJECT_NOTIFIER_PARAM);
...
class MissingEnvironmentKey {
   ...
   public:
    ...
    MissingEnvironmentKey(AbstractFramePtr frame, Scope* scope) : frame_(frame), scope_(scope) {}
...
extern JSObject* GetDebugEnvironmentForFunction(JSContext* cx, HandleFunction fun);

extern JSObject* GetDebugEnvironmentForFrame(JSContext* cx, AbstractFramePtr frame,
                                             jsbytecode* pc);
...
class DebugEnvironments {
    ...
    typedef GCHashMap<ReadBarriered<JSObject*>, LiveEnvironmentVal,
                      MovableCellHasher<ReadBarriered<JSObject*>>, ZoneAllocPolicy>
        LiveEnvironmentMap;
...
inline bool JSObject::is<js::EnvironmentObject>() const {
    return is<js::CallObject>() || is<js::VarEnvironmentObject>() ||
           is<js::ModuleEnvironmentObject>() || is<js::WasmInstanceEnvironmentObject>() ||
           is<js::WasmFunctionCallObject>() || is<js::LexicalEnvironmentObject>() ||
           is<js::WithEnvironmentObject>() || is<js::NonSyntacticVariablesObject>() ||
...
inline bool IsSyntacticEnvironment(JSObject* env) {
    if (!env->is<EnvironmentObject>()) return false;

    if (env->is<WithEnvironmentObject>()) return env->as<WithEnvironmentObject>().isSyntactic();

    if (env->is<LexicalEnvironmentObject>())
        return env->as<LexicalEnvironmentObject>().isSyntactic();

    if (env->is<NonSyntacticVariablesObject>()) return false;
...
inline bool IsGlobalLexicalEnvironment(JSObject* env) {
    return env->is<LexicalEnvironmentObject>() && env->as<LexicalEnvironmentObject>().isGlobal();
}

inline bool IsNSVOLexicalEnvironment(JSObject* env) {
    return env->is<LexicalEnvironmentObject>() && env->as<LexicalEnvironmentObject>()
                                                      .enclosingEnvironment()
                                                      .is<NonSyntacticVariablesObject>();
}

inline JSObject* MaybeUnwrapWithEnvironment(JSObject* env) {
    if (env->is<WithEnvironmentObject>()) return &env->as<WithEnvironmentObject>().object();
    return env;
}

template <typename SpecificEnvironment>
inline bool IsFrameInitialEnvironment(AbstractFramePtr frame, SpecificEnvironment& env) {
    ...
    // environment.
    if (mozilla::IsSame<SpecificEnvironment, CallObject>::value) return true;

    // For an eval frame, the VarEnvironmentObject, if present, is always the
    // initial environment.
    if (mozilla::IsSame<SpecificEnvironment, VarEnvironmentObject>::value && frame.isEvalFrame()) {
        return true;
    }
    ...
    if (mozilla::IsSame<SpecificEnvironment, NamedLambdaObject>::value &&
        frame.isFunctionFrame() && frame.callee()->needsNamedLambdaEnvironment() &&
...
extern bool CreateObjectsForEnvironmentChain(JSContext* cx, AutoObjectVector& chain,
...
MOZ_MUST_USE bool CheckCanDeclareGlobalBinding(JSContext* cx, Handle<GlobalObject*> global,
...
MOZ_MUST_USE bool CheckEvalDeclarationConflicts(JSContext* cx, HandleScript script,
                                                HandleObject envChain, HandleObject varObj);
...
MOZ_MUST_USE bool GetFrameEnvironmentAndScope(JSContext* cx, AbstractFramePtr frame,
