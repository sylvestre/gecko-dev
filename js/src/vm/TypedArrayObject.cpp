...
void TypedArrayObject::notifyBufferDetached(JSContext* cx, void* newData) {
    ...
    if (isTenured() && !hasBuffer() && !hasInlineElements() && !nursery.isInside(elements())) {
        js_free(elements());
    }
...
/* static */ bool TypedArrayObject::ensureHasBuffer(JSContext* cx,
...
/* static */ size_t TypedArrayObject::objectMoved(JSObject* obj, JSObject* old) {
...
#undef OBJECT_MOVED_TYPED_ARRAY
    ...
    if (headerSize + nbytes <= GetGCKindBytes(newAllocKind)) {
        ...
        if (!data) oomUnsafe.crash("Failed to allocate typed array elements while tenuring.");
...
class TypedArrayObjectTemplate : public TypedArrayObject {
   ...
   public:
    ...
    static JSObject* createConstructor(JSContext* cx, JSProtoKey key) {
        ...
        JSFunction* fun = NewFunctionWithProto(cx, class_constructor, 3, JSFunction::NATIVE_CTOR,
    ...
    static void setIndexValue(TypedArrayObject& tarray, uint32_t index, double d) {
    ...
    static TypedArrayObject* makeTypedInstance(JSContext* cx, uint32_t len,
    ...
    static TypedArrayObject* makeInstance(JSContext* cx,
        ...
        gc::AllocKind allocKind = buffer ? GetGCObjectKind(instanceClass())
        ...
        if (proto &&
            !GetBuiltinPrototype(cx, JSCLASS_CACHED_PROTO_KEY(instanceClass()), &checkProto))
            return nullptr;
    ...
    static void initTypedArrayData(JSContext* cx, TypedArrayObject* tarray, int32_t len, void* buf,
                                   AllocKind allocKind) {
        if (buf) {
#ifdef DEBUG
            Nursery& nursery = cx->nursery();
    ...
    static TypedArrayObject* makeTypedArrayWithTemplate(JSContext* cx,
   ...
   private:
    static JSObject* create(JSContext* cx, const CallArgs& args) {
        ...
        if (args.length() == 0 || !args[0].isObject()) {
            // 22.2.4.2, step 3.
            uint64_t len;
            if (!ToIndex(cx, args.get(0), JSMSG_BAD_ARRAY_LENGTH, &len)) return nullptr;
    ...
    // 22.2.4.5 TypedArray ( buffer [ , byteOffset [ , length ] ] )
    // Steps 9-12.
    static bool computeAndCheckLength(JSContext* cx,
    ...
    // 22.2.4.5 TypedArray ( buffer [ , byteOffset [ , length ] ] )
    // Steps 9-17.
    static JSObject* fromBufferSameCompartment(JSContext* cx,
                                               HandleArrayBufferObjectMaybeShared buffer,
                                               uint64_t byteOffset, uint64_t lengthIndex,
                                               HandleObject proto) {
        // Steps 9-12.
        uint32_t length;
        if (!computeAndCheckLength(cx, buffer, byteOffset, lengthIndex, &length)) return nullptr;
    ...
    static JSObject* fromBufferWrapped(JSContext* cx, HandleObject bufobj, uint64_t byteOffset,
   ...
   public:
    static JSObject* fromBuffer(JSContext* cx, HandleObject bufobj, uint32_t byteOffset,
                                int32_t lengthInt) {
        if (byteOffset % sizeof(NativeType) != 0) {
            JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr,
                                      JSMSG_TYPED_ARRAY_CONSTRUCT_BOUNDS);
    ...
    static bool maybeCreateArrayBuffer(JSContext* cx, uint32_t count, uint32_t unit,
    ...
    // 22.2.4.1 TypedArray ( )
    // 22.2.4.2 TypedArray ( length )
    static JSObject* fromLength(JSContext* cx, uint64_t nelements, HandleObject proto = nullptr) {
    ...
    static bool AllocateArrayBuffer(JSContext* cx, HandleObject ctor, uint32_t count,
                                    uint32_t unit, MutableHandle<ArrayBufferObject*> buffer);

    static JSObject* fromArray(JSContext* cx, HandleObject other, HandleObject proto = nullptr);

    static JSObject* fromTypedArray(JSContext* cx, HandleObject other, bool isWrapped,
                                    HandleObject proto);

    static JSObject* fromObject(JSContext* cx, HandleObject other, HandleObject proto);
    ...
    static void setIndex(TypedArrayObject& tarray, uint32_t index, NativeType val) {
        MOZ_ASSERT(index < tarray.length());
...
#define CREATE_TYPE_FOR_TYPED_ARRAY(T, N) typedef TypedArrayObjectTemplate<T> N##Array;
JS_FOR_EACH_TYPED_ARRAY(CREATE_TYPE_FOR_TYPED_ARRAY)
#undef CREATE_TYPE_FOR_TYPED_ARRAY

} /* anonymous namespace */

TypedArrayObject* js::TypedArrayCreateWithTemplate(JSContext* cx, HandleObject templateObj,
                                                   int32_t len) {
...
static JSObject* GetBufferSpeciesConstructor(JSContext* cx, Handle<TypedArrayObject*> typedArray,
...
template <typename T>
/* static */ JSObject* TypedArrayObjectTemplate<T>::fromArray(JSContext* cx, HandleObject other,
...
// 22.2.4.3 TypedArray ( typedArray )
template <typename T>
/* static */ JSObject* TypedArrayObjectTemplate<T>::fromTypedArray(JSContext* cx,
                                                                   HandleObject other,
                                                                   bool isWrapped,
                                                                   HandleObject proto) {
    // Step 1.
    ...
    MOZ_ASSERT_IF(isWrapped,
                  other->is<WrapperObject>() && UncheckedUnwrap(other)->is<TypedArrayObject>());
    ...
    SpeciesConstructorOverride override =
        isShared ? SpeciesConstructorOverride::ArrayBuffer : SpeciesConstructorOverride::None;
    ...
    if (ArrayTypeID() == srcType) {
        ...
        if (!AllocateArrayBuffer(cx, bufferCtor, byteLength, 1, &buffer)) return nullptr;
    } else {
        // Steps 14-15, 19.a.
    ...
    if (isShared) {
        if (!ElementSpecific<T, SharedOps>::setFromTypedArray(obj, srcArray, 0)) return nullptr;
    } else {
        if (!ElementSpecific<T, UnsharedOps>::setFromTypedArray(obj, srcArray, 0)) return nullptr;
    }
...
static MOZ_ALWAYS_INLINE bool IsOptimizableInit(JSContext* cx, HandleObject iterable,
                                                bool* optimized) {
...
// 22.2.4.4 TypedArray ( object )
template <typename T>
/* static */ JSObject* TypedArrayObjectTemplate<T>::fromObject(JSContext* cx, HandleObject other,
                                                               HandleObject proto) {
    ...
    if (optimized) {
        ...
        // Step 6.c.
        Rooted<ArrayBufferObject*> buffer(cx);
        if (!maybeCreateArrayBuffer(cx, len, BYTES_PER_ELEMENT, nullptr, &buffer)) return nullptr;
...
/* static */ bool TypedArrayObject::GetTemplateObjectForNative(JSContext* cx, Native native,
                                                               uint32_t len,
...
static bool TypedArray_byteLengthGetter(JSContext* cx, unsigned argc, Value* vp) {
    return TypedArrayObject::Getter<TypedArrayObject::byteLengthValue>(cx, argc, vp);
}

static bool TypedArray_byteOffsetGetter(JSContext* cx, unsigned argc, Value* vp) {
...
/*static*/ bool js::TypedArray_bufferGetter(JSContext* cx, unsigned argc, Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);
...
template <typename T>
static inline bool SetFromNonTypedArray(JSContext* cx, Handle<TypedArrayObject*> target,
                                        HandleObject source, uint32_t len, uint32_t offset) {
    MOZ_ASSERT(!source->is<TypedArrayObject>(), "use SetFromTypedArray");

    if (target->isSharedMemory())
        return ElementSpecific<T, SharedOps>::setFromNonTypedArray(cx, target, source, len,
                                                                   offset);
...
/* static */ bool TypedArrayObject::set_impl(JSContext* cx, const CallArgs& args) {
    MOZ_ASSERT(TypedArrayObject::is(args.thisv()));
    ...
    Rooted<TypedArrayObject*> srcTypedArray(cx);
    {
        ...
        if (obj->is<TypedArrayObject>()) srcTypedArray = &obj->as<TypedArrayObject>();
    }
...
#define SET_FROM_NON_TYPED_ARRAY(T, N)                                                  \
    case Scalar::N:                                                                     \
        if (!SetFromNonTypedArray<T>(cx, target, src, srcLength, offset)) return false; \
        break;
...
/* static */ bool TypedArrayObject::set(JSContext* cx, unsigned argc, Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);
...
template <typename NativeType>
Value TypedArrayObjectTemplate<NativeType>::getIndexValue(JSObject* tarray, uint32_t index) {
...
template <>
Value TypedArrayObjectTemplate<int32_t>::getIndexValue(JSObject* tarray, uint32_t index) {
    return Int32Value(getIndex(tarray, index));
}

template <>
Value TypedArrayObjectTemplate<uint32_t>::getIndexValue(JSObject* tarray, uint32_t index) {
    uint32_t val = getIndex(tarray, index);
    return NumberValue(val);
}

template <>
Value TypedArrayObjectTemplate<float>::getIndexValue(JSObject* tarray, uint32_t index) {
...
template <>
Value TypedArrayObjectTemplate<double>::getIndexValue(JSObject* tarray, uint32_t index) {
...
void TypedArrayObject::setElement(TypedArrayObject& obj, uint32_t index, double d) {
...
#define GET_ELEMENTS(T, N)                                                                  \
    case Scalar::N:                                                                         \
        for (uint32_t i = 0; i < length; ++i, ++vp) *vp = N##Array::getIndexValue(this, i); \
        break;
...
#define IMPL_TYPED_ARRAY_JSAPI_CONSTRUCTORS(Name, NativeType)                                   \
    ...
    JS_New##Name##ArrayWithBuffer(JSContext* cx, HandleObject arrayBuffer, uint32_t byteOffset, \
                                  int32_t length) {                                             \
    ...
    JS_FRIEND_API(JSObject*) js::Unwrap##Name##Array(JSObject* obj) {                           \
        ...
        if (!obj) return nullptr;                                                               \
        const Class* clasp = obj->getClass();                                                   \
        if (clasp == TypedArrayObjectTemplate<NativeType>::instanceClass()) return obj;         \
        return nullptr;                                                                         \
    }                                                                                           \
...
#define IMPL_TYPED_ARRAY_COMBINED_UNWRAPPERS(Name, ExternalType, InternalType)                \
    JS_FRIEND_API(JSObject*)                                                                  \
    JS_GetObjectAs##Name##Array(JSObject* obj, uint32_t* length, bool* isShared,              \
                                ExternalType** data) {                                        \
        if (!(obj = CheckedUnwrap(obj))) return nullptr;                                      \
                                                                                              \
        const Class* clasp = obj->getClass();                                                 \
        if (clasp != TypedArrayObjectTemplate<InternalType>::instanceClass()) return nullptr; \
                                                                                              \
...
static const JSPropertySpec static_prototype_properties[Scalar::MaxTypedArrayViewType][2] = {
    IMPL_TYPED_ARRAY_PROPERTIES(Int8),        IMPL_TYPED_ARRAY_PROPERTIES(Uint8),
    IMPL_TYPED_ARRAY_PROPERTIES(Int16),       IMPL_TYPED_ARRAY_PROPERTIES(Uint16),
    IMPL_TYPED_ARRAY_PROPERTIES(Int32),       IMPL_TYPED_ARRAY_PROPERTIES(Uint32),
    IMPL_TYPED_ARRAY_PROPERTIES(Float32),     IMPL_TYPED_ARRAY_PROPERTIES(Float64),
...
#define IMPL_TYPED_ARRAY_CLASS_SPEC(_type)                                                \
    {                                                                                     \
        _type##Array::createConstructor, _type##Array::createPrototype, nullptr,          \
            ...
            static_prototype_properties[Scalar::Type::_type], nullptr, JSProto_TypedArray \
    }

static const ClassSpec TypedArrayObjectClassSpecs[Scalar::MaxTypedArrayViewType] = {
    IMPL_TYPED_ARRAY_CLASS_SPEC(Int8),        IMPL_TYPED_ARRAY_CLASS_SPEC(Uint8),
    IMPL_TYPED_ARRAY_CLASS_SPEC(Int16),       IMPL_TYPED_ARRAY_CLASS_SPEC(Uint16),
    IMPL_TYPED_ARRAY_CLASS_SPEC(Int32),       IMPL_TYPED_ARRAY_CLASS_SPEC(Uint32),
    IMPL_TYPED_ARRAY_CLASS_SPEC(Float32),     IMPL_TYPED_ARRAY_CLASS_SPEC(Float64),
    IMPL_TYPED_ARRAY_CLASS_SPEC(Uint8Clamped)};

#define IMPL_TYPED_ARRAY_CLASS(_type)                                                            \
    {                                                                                            \
        #_type "Array",                                                                          \
            JSCLASS_HAS_RESERVED_SLOTS(TypedArrayObject::RESERVED_SLOTS) | JSCLASS_HAS_PRIVATE | \
                JSCLASS_HAS_CACHED_PROTO(JSProto_##_type##Array) |                               \
                JSCLASS_DELAY_METADATA_BUILDER | JSCLASS_SKIP_NURSERY_FINALIZE |                 \
                JSCLASS_BACKGROUND_FINALIZE,                                                     \
            &TypedArrayClassOps,                                                                 \
            &TypedArrayObjectClassSpecs[Scalar::Type::_type], &TypedArrayClassExtension          \
    }
...
const Class TypedArrayObject::protoClasses[Scalar::MaxTypedArrayViewType] = {
    IMPL_TYPED_ARRAY_PROTO_CLASS(Int8),        IMPL_TYPED_ARRAY_PROTO_CLASS(Uint8),
    IMPL_TYPED_ARRAY_PROTO_CLASS(Int16),       IMPL_TYPED_ARRAY_PROTO_CLASS(Uint16),
    IMPL_TYPED_ARRAY_PROTO_CLASS(Int32),       IMPL_TYPED_ARRAY_PROTO_CLASS(Uint32),
    IMPL_TYPED_ARRAY_PROTO_CLASS(Float32),     IMPL_TYPED_ARRAY_PROTO_CLASS(Float64),
...
bool js::IsBufferSource(JSObject* object, SharedMem<uint8_t*>* dataPointer, size_t* byteLength) {
...
bool js::StringIsTypedArrayIndex(const CharT* s, size_t length, uint64_t* indexp) {
...
template bool js::StringIsTypedArrayIndex(const char16_t* s, size_t length, uint64_t* indexp);

template bool js::StringIsTypedArrayIndex(const Latin1Char* s, size_t length, uint64_t* indexp);

/* ES6 draft rev 34 (2015 Feb 20) 9.4.5.3 [[DefineOwnProperty]] step 3.c. */
bool js::DefineTypedArrayElement(JSContext* cx, HandleObject obj, uint64_t index,
    ...
    // Step viii.
    if (desc.hasEnumerable() && !desc.enumerable()) return result.fail(JSMSG_CANT_REDEFINE_PROP);

    // Step ix.
    if (desc.hasWritable() && !desc.writable()) return result.fail(JSMSG_CANT_REDEFINE_PROP);
