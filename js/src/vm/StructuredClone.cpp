...
struct BufferIterator {
    ...
    explicit BufferIterator(BufferList& buffer) : mBuffer(buffer), mIter(buffer.Iter()) {
        JS_STATIC_ASSERT(8 % sizeof(T) == 0);
    }

    BufferIterator(const BufferIterator& other) : mBuffer(other.mBuffer), mIter(other.mIter) {}
    ...
    bool readBytes(char* outData, size_t size) { return mBuffer.ReadBytes(mIter, outData, size); }
...
SharedArrayRawBufferRefs& SharedArrayRawBufferRefs::operator=(SharedArrayRawBufferRefs&& other) {
...
bool SharedArrayRawBufferRefs::acquire(JSContext* cx, SharedArrayRawBuffer* rawbuf) {
...
bool SharedArrayRawBufferRefs::acquireAll(JSContext* cx, const SharedArrayRawBufferRefs& that) {
...
struct SCOutput {
   public:
    ...
    Iter iter() { return BufferIterator<uint64_t, TempAllocPolicy>(buf); }
...
struct JSStructuredCloneWriter {
   public:
    explicit JSStructuredCloneWriter(JSContext* cx, JS::StructuredCloneScope scope,
   ...
   private:
    ...
    using CloneMemory =
        GCHashMap<JSObject*, uint32_t, MovableCellHasher<JSObject*>, SystemAllocPolicy>;
...
static void ReportDataCloneError(JSContext* cx, const JSStructuredCloneCallbacks* callbacks,
                                 uint32_t errorId) {
...
bool WriteStructuredClone(JSContext* cx, HandleValue v, JSStructuredCloneData* bufp,
...
static void DiscardTransferables(mozilla::BufferList<AllocPolicy>& buffer,
                                 const JSStructuredCloneCallbacks* cb, void* cbClosure) {
    ...
    while (numTransferables--) {
        ...
        if (ownership == JS::SCTAG_TMO_ALLOC_DATA) {
            ...
            cb->freeTransfer(tag, JS::TransferableOwnership(ownership), content, extraData,
                             cbClosure);
        } else {
...
static bool StructuredCloneHasTransferObjects(const JSStructuredCloneData& data) {
    auto iter = data.Iter();
...
SCInput::SCInput(JSContext* cx, JSStructuredCloneData& data) : cx(cx), point(data) {
...
bool SCInput::readBytes(void* p, size_t nbytes) { return readArray((uint8_t*)p, nbytes); }
...
SCOutput::SCOutput(JSContext* cx) : cx(cx), buf(0, 0, 4096, cx) {}
...
bool SCOutput::writeDouble(double d) { return write(BitwiseCast<uint64_t>(CanonicalizeNaN(d))); }
...
bool SCOutput::writeArray(const T* p, size_t nelems) {
    ...
    for (size_t i = 0; i < nelems; i++) {
        ...
        if (!buf.WriteBytes(reinterpret_cast<char*>(&value), sizeof(value))) return false;
    }

    // Zero-pad to 8 bytes boundary.
    size_t padbytes = ComputePadding(nelems, sizeof(T));
    char zeroes[sizeof(uint64_t)] = {0};
    if (!buf.WriteBytes(zeroes, padbytes)) return false;
...
bool SCOutput::writeArray<uint8_t>(const uint8_t* p, size_t nelems) {
    ...
    if (!buf.WriteBytes(reinterpret_cast<const char*>(p), nelems)) return false;
...
bool SCOutput::writePtr(const void* p) { return write(reinterpret_cast<uint64_t>(p)); }

bool SCOutput::extractBuffer(JSStructuredCloneData* data) {
    bool success;
    mozilla::BufferList<SystemAllocPolicy> out = buf.MoveFallible<SystemAllocPolicy>(&success);
    if (!success) {
        ReportOutOfMemory(cx);
        return false;
    }
...
void SCOutput::discardTransferables(const JSStructuredCloneCallbacks* cb, void* cbClosure) {
...
bool JSStructuredCloneWriter::parseTransferable() {
    ...
    MOZ_ASSERT(!transferableObjects.initialized(), "parseTransferable called with stale data");

    if (transferable.isNull() || transferable.isUndefined()) return transferableObjects.init(0);

    if (!transferable.isObject()) return reportDataCloneError(JS_SCERR_TRANSFERABLE);
...
bool JSStructuredCloneWriter::writeString(uint32_t tag, JSString* str) {
    ...
    return linear->hasLatin1Chars() ? out.writeChars(linear->latin1Chars(nogc), length)
...
bool JSStructuredCloneWriter::writeSharedArrayBuffer(HandleObject obj) {
    ...
    if (!refsHeld.acquire(context(), rawbuf)) return false;
...
bool JSStructuredCloneWriter::startObject(HandleObject obj, bool* backref) {
    ...
    if ((*backref = p.found())) return out.writePair(SCTAG_BACK_REFERENCE_OBJECT, p->value());
...
bool JSStructuredCloneWriter::traverseObject(HandleObject obj) {
    ...
    if (!GetPropertyKeys(context(), obj, JSITER_OWNONLY, &properties)) return false;
    ...
    if (!objs.append(ObjectValue(*obj)) || !counts.append(properties.length())) return false;
...
bool JSStructuredCloneWriter::traverseMap(HandleObject obj) {
    ...
    /* Push obj and count to the stack. */
    if (!objs.append(ObjectValue(*obj)) || !counts.append(newEntries.length())) return false;
...
bool JSStructuredCloneWriter::traverseSet(HandleObject obj) {
    ...
    /* Push obj and count to the stack. */
    if (!objs.append(ObjectValue(*obj)) || !counts.append(keys.length())) return false;
...
bool JSStructuredCloneWriter::traverseSavedFrame(HandleObject obj) {
    ...
    if (!objs.append(ObjectValue(*obj)) ||
        !entries.append(parent ? ObjectValue(*parent) : NullValue()) || !counts.append(1)) {
        return false;
    }
    ...
    if (savedFrame->getPrincipals() == &ReconstructedSavedFramePrincipals::IsSystem) {
        ...
        if (auto principals = savedFrame->getPrincipals()) {
            ...
            if (!out.writePair(SCTAG_SAVED_FRAME_OBJECT, SCTAG_NULL_JSPRINCIPALS)) return false;
        }
    }
...
bool JSStructuredCloneWriter::startWrite(HandleValue v) {
    ...
    if (v.isString()) {
        ...
        if (callbacks && callbacks->write) return callbacks->write(context(), this, obj, closure);
        /* else fall through */
    }
...
bool JSStructuredCloneWriter::writeTransferMap() {
    ...
    if (!out.writePair(SCTAG_TRANSFER_MAP_HEADER, (uint32_t)SCTAG_TM_UNREAD)) return false;
    ...
    for (auto tr = transferableObjects.all(); !tr.empty(); tr.popFront()) {
        ...
        if (!out.writePair(SCTAG_TRANSFER_MAP_PENDING_ENTRY, JS::SCTAG_TMO_UNFILLED)) return false;
    ...
    for (auto tr = transferableObjects.all(); !tr.empty(); tr.popFront()) {
        ...
        if (cls == ESClass::ArrayBuffer) {
            ...
            Rooted<ArrayBufferObject*> arrayBuffer(cx,
                                                   &CheckedUnwrap(obj)->as<ArrayBufferObject>());
            ...
            if (!callbacks->writeTransfer(cx, obj, closure, &tag, &ownership, &content,
                                          &extraData))
                return false;
            MOZ_ASSERT(tag > SCTAG_TRANSFER_MAP_PENDING_ENTRY);
        }
...
JSString* JSStructuredCloneReader::readStringImpl(uint32_t nchars) {
    ...
    if (!chars.allocate(nchars) || !in.readChars(chars.get(), nchars)) return nullptr;
...
bool JSStructuredCloneReader::readTypedArray(uint32_t arrayType, uint32_t nelems,
                                             MutableHandleValue vp, bool v1Read) {
...
bool JSStructuredCloneReader::readDataView(uint32_t byteLength, MutableHandleValue vp) {
...
bool JSStructuredCloneReader::readArrayBuffer(uint32_t nbytes, MutableHandleValue vp) {
 ...
 */
bool JSStructuredCloneReader::readV1ArrayBuffer(uint32_t arrayType, uint32_t nelems,
...
bool JSStructuredCloneReader::startRead(MutableHandleValue vp) {
    ...
    switch (tag) {
        ...
        case SCTAG_BOOLEAN_OBJECT:
            vp.setBoolean(!!data);
            if (tag == SCTAG_BOOLEAN_OBJECT && !PrimitiveToObject(context(), vp)) return false;
            break;
        ...
        case SCTAG_STRING_OBJECT: {
            ...
            if (!str) return false;
            vp.setString(str);
            if (tag == SCTAG_STRING_OBJECT && !PrimitiveToObject(context(), vp)) return false;
            break;
        }
        ...
        case SCTAG_SHARED_ARRAY_BUFFER_OBJECT:
            if (!readSharedArrayBuffer(data, vp)) return false;
            break;
...
bool JSStructuredCloneReader::readTransferMap() {
    ...
    for (uint64_t i = 0; i < numTransferables; i++) {
        ...
        if (tag == SCTAG_TRANSFER_MAP_ARRAY_BUFFER) {
            ...
            MOZ_ASSERT(data == JS::SCTAG_TMO_ALLOC_DATA || data == JS::SCTAG_TMO_MAPPED_DATA);
...
JSObject* JSStructuredCloneReader::readSavedFrame(uint32_t principalsTag) {
    ...
    if (principalsTag == SCTAG_JSPRINCIPALS) {
        ...
        if (!context()->runtime()->readPrincipals(context(), this, &principals)) return nullptr;
    ...
    if (!startRead(&cause) || !(cause.isString() || cause.isNull())) return nullptr;
...
JS_PUBLIC_API(bool)
JS_ReadStructuredClone(JSContext* cx, JSStructuredCloneData& buf, uint32_t version,
                       JS::StructuredCloneScope scope, MutableHandleValue vp,
...
JS_StructuredClone(JSContext* cx, HandleValue value, MutableHandleValue vp,
    ...
    JSAutoStructuredCloneBuffer buf(JS::StructuredCloneScope::SameProcessSameThread, callbacks,
                                    closure);
    {
...
void JSAutoStructuredCloneBuffer::clear(const JSStructuredCloneCallbacks* optionalCallbacks,
...
bool JSAutoStructuredCloneBuffer::copy(JSContext* cx, const JSStructuredCloneData& srcData,
                                       uint32_t version,
                                       const JSStructuredCloneCallbacks* callbacks,
                                       void* closure) {
    // transferable objects cannot be copied
    if (StructuredCloneHasTransferObjects(srcData)) return false;

    clear();

    auto iter = srcData.Iter();
    while (!iter.Done()) {
        if (!data_.WriteBytes(iter.Data(), iter.RemainingInSegment())) return false;
        iter.Advance(srcData, iter.RemainingInSegment());
    }

    version_ = version;

    if (!data_.refsHeld_.acquireAll(cx, srcData.refsHeld_)) return false;

    data_.setOptionalCallbacks(callbacks, closure, OwnTransferablePolicy::NoTransferables);
    return true;
}

void JSAutoStructuredCloneBuffer::adopt(JSStructuredCloneData&& data, uint32_t version,
                                        const JSStructuredCloneCallbacks* callbacks,
                                        void* closure) {
    clear();
    data_ = Move(data);
...
void JSAutoStructuredCloneBuffer::steal(JSStructuredCloneData* data, uint32_t* versionp,
                                        const JSStructuredCloneCallbacks** callbacks,
                                        void** closure) {
...
bool JSAutoStructuredCloneBuffer::read(JSContext* cx, MutableHandleValue vp,
                                       const JSStructuredCloneCallbacks* optionalCallbacks,
                                       void* closure) {
    MOZ_ASSERT(cx);
    return !!JS_ReadStructuredClone(cx, data_, version_, scope_, vp, optionalCallbacks, closure);
}

bool JSAutoStructuredCloneBuffer::write(JSContext* cx, HandleValue value,
                                        const JSStructuredCloneCallbacks* optionalCallbacks,
                                        void* closure) {
...
bool JSAutoStructuredCloneBuffer::write(JSContext* cx, HandleValue value, HandleValue transferable,
                                        JS::CloneDataPolicy cloneDataPolicy,
                                        const JSStructuredCloneCallbacks* optionalCallbacks,
                                        void* closure) {
    clear();
...
JS_GetStructuredCloneScope(JSStructuredCloneWriter* w) { return w->cloneScope(); }
