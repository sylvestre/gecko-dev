...
void UnboxedLayout::setNewScript(TypeNewScript* newScript, bool writeBarrier /* = true */) {
...
/* static */ bool UnboxedLayout::makeConstructorCode(JSContext* cx, HandleObjectGroup group) {
    ...
    for (size_t i = 0; i < layout.properties().length(); i++) {
        ...
        if (property.type == JSVAL_TYPE_OBJECT) {
            Address valueAddress(propertiesReg,
                                 i * sizeof(IdValuePair) + offsetof(IdValuePair, value));
            Label notObject;
    ...
    for (size_t i = 0; i < layout.properties().length(); i++) {
        ...
        if (property.type == JSVAL_TYPE_OBJECT) {
            ...
            Label notObject;
            masm.branchTestObject(
                Assembler::NotEqual, valueOperand,
            ...
            masm.storeUnboxedProperty(
                ...
                TypedOrValueRegister(MIRType::Object, AnyRegister(payloadReg)), nullptr);
...
bool UnboxedPlainObject::setValue(JSContext* cx, const UnboxedLayout::Property& property,
                                  const Value& v) {
...
void UnboxedPlainObject::trace(JSTracer* trc, JSObject* obj) {
    if (obj->as<UnboxedPlainObject>().expando_) {
        TraceManuallyBarrieredEdge(
            trc, reinterpret_cast<NativeObject**>(&obj->as<UnboxedPlainObject>().expando_),
            "unboxed_expando");
    }
...
/* static */ UnboxedExpandoObject* UnboxedPlainObject::ensureExpando(
    ...
    if (obj->expando_) return obj->expando_;
...
bool UnboxedPlainObject::containsUnboxedOrExpandoProperty(JSContext* cx, jsid id) const {
    if (layoutDontCheckGeneration().lookup(id)) return true;

    if (maybeExpando() && maybeExpando()->containsShapeOrElement(cx, id)) return true;

    return false;
}

static bool PropagatePropertyTypes(JSContext* cx, jsid id, ObjectGroup* oldGroup,
...
static PlainObject* MakeReplacementTemplateObject(JSContext* cx, HandleObjectGroup group,
                                                  const UnboxedLayout& layout) {
    Rooted<PlainObject*> obj(
        cx, NewObjectWithGroup<PlainObject>(cx, group, layout.getAllocKind(), TenuredObject));
...
/* static */ bool UnboxedLayout::makeNativeGroup(JSContext* cx, ObjectGroup* group) {
    ...
    ObjectGroup* nativeGroup = ObjectGroupCompartment::makeGroup(
        cx, &PlainObject::class_, proto, group->flags() & OBJECT_FLAG_DYNAMIC_MASK);
...
/* static */ bool UnboxedPlainObject::convertToNative(JSContext* cx, JSObject* obj) {
    const UnboxedLayout& layout = obj->as<UnboxedPlainObject>().layout();
    UnboxedExpandoObject* expando = obj->as<UnboxedPlainObject>().maybeExpando();

    if (!layout.nativeGroup()) {
        if (!UnboxedLayout::makeNativeGroup(cx, obj->group())) return false;

        // makeNativeGroup can reentrantly invoke this method.
        if (obj->is<PlainObject>()) return true;
    }
    ...
    if (expando) {
        ...
        for (Shape::Range<NoGC> r(expando->lastProperty()); !r.empty(); r.popFront()) {
            if (!ids.append(r.front().propid())) return false;
        }
        ...
        for (size_t i = 0; i < ids.length(); i++) {
            ...
            if (!DefineProperty(cx, nobj, id, desc, result)) return false;
...
/* static */ JS::Result<UnboxedObject*, JS::OOM&> UnboxedObject::createInternal(
    JSContext* cx, js::gc::AllocKind kind, js::gc::InitialHeap heap, js::HandleObjectGroup group) {
...
/* static */
UnboxedPlainObject* UnboxedPlainObject::create(JSContext* cx, HandleObjectGroup group,
...
/* static */ JSObject* UnboxedPlainObject::createWithProperties(JSContext* cx,
    ...
    if (layout.constructorCode()) {
        ...
        if (obj > reinterpret_cast<JSObject*>(CLEAR_CONSTRUCTOR_CODE_TOKEN)) return obj;
    ...
    for (size_t i = 0; i < layout.properties().length(); i++) {
        if (!obj->setValue(cx, layout.properties()[i], properties[i].value))
            return NewPlainObjectWithProperties(cx, properties, layout.properties().length(),
                                                newKind);
    }

#ifndef JS_CODEGEN_NONE
    if (!cx->helperThread() && !group->unknownProperties() && !layout.constructorCode() &&
        cx->runtime()->jitSupportsFloatingPoint && jit::CanLikelyAllocateMoreExecutableMemory()) {
...
/* static */ bool UnboxedPlainObject::obj_lookupProperty(JSContext* cx, HandleObject obj,
...
/* static */ bool UnboxedPlainObject::obj_defineProperty(JSContext* cx, HandleObject obj,
                                                         HandleId id,
...
/* static */ bool UnboxedPlainObject::obj_hasProperty(JSContext* cx, HandleObject obj, HandleId id,
                                                      bool* foundp) {
    if (obj->as<UnboxedPlainObject>().containsUnboxedOrExpandoProperty(cx, id)) {
        *foundp = true;
        return true;
...
/* static */ bool UnboxedPlainObject::obj_getProperty(JSContext* cx, HandleObject obj,
...
/* static */ bool UnboxedPlainObject::obj_setProperty(JSContext* cx, HandleObject obj, HandleId id,
    ...
    if (const UnboxedLayout::Property* property = layout.lookup(id)) {
        if (receiver.isObject() && obj == &receiver.toObject()) {
            if (obj->as<UnboxedPlainObject>().setValue(cx, *property, v)) return result.succeed();
...
/* static */ bool UnboxedPlainObject::obj_deleteProperty(JSContext* cx, HandleObject obj,
                                                         HandleId id, ObjectOpResult& result) {
    if (!convertToNative(cx, obj)) return false;
    return DeleteProperty(cx, obj, id, result);
}

/* static */ bool UnboxedPlainObject::obj_watch(JSContext* cx, HandleObject obj, HandleId id,
                                                HandleObject callable) {
    if (!convertToNative(cx, obj)) return false;
    return WatchProperty(cx, obj, id, callable);
}

/* static */ bool UnboxedPlainObject::newEnumerate(JSContext* cx, HandleObject obj,
...
const Class UnboxedPlainObject::class_ = {
    js_Object_str,
    Class::NON_NATIVE | JSCLASS_HAS_CACHED_PROTO(JSProto_Object) | JSCLASS_DELAY_METADATA_BUILDER,
...
static bool UnboxedTypeIncludes(JSValueType supertype, JSValueType subtype) {
    if (supertype == JSVAL_TYPE_DOUBLE && subtype == JSVAL_TYPE_INT32) return true;
...
// specified vector.
static bool PropertiesAreSuperset(const UnboxedLayout::PropertyVector& properties,
                                  UnboxedLayout* layout) {
    for (size_t i = 0; i < layout->properties().length(); i++) {
...
static bool CombinePlainObjectProperties(PlainObject* obj, Shape* templateShape,
...
static size_t ComputePlainObjectLayout(JSContext* cx, Shape* templateShape,
                                       UnboxedLayout::PropertyVector& properties) {
...
static bool SetLayoutTraceList(JSContext* cx, UnboxedLayout* layout) {
    ...
    if (!objectOffsets.empty() || !stringOffsets.empty()) {
        ...
        if (!entries.appendAll(stringOffsets) || !entries.append(-1) ||
            !entries.appendAll(objectOffsets) || !entries.append(-1) || !entries.append(-1)) {
            return false;
        }
...
static inline Value NextValue(Handle<GCVector<Value>> values, size_t* valueCursor) {
    return values[(*valueCursor)++];
}

static bool GetValuesFromPreliminaryPlainObject(PlainObject* obj,
...
void UnboxedPlainObject::fillAfterConvert(JSContext* cx, Handle<GCVector<Value>> values,
                                          size_t* valueCursor) {
    initExpando();
...
bool js::TryConvertToUnboxedLayout(JSContext* cx, AutoEnterAnalysis& enter, Shape* templateShape,
    ...
    if (group->runtimeFromAnyThread()->isSelfHostingGlobal(cx->global())) return true;

    if (templateShape->slotSpan() == 0) return true;

    UnboxedLayout::PropertyVector properties;
    if (!properties.appendN(UnboxedLayout::Property(), templateShape->slotSpan())) return false;
    ...
    if (UnboxedPlainObject::offsetOfData() + layoutSize > JSObject::MAX_BYTE_SIZE) return true;
    ...
    if (TypeNewScript* newScript = group->newScript()) layout->setNewScript(newScript);
