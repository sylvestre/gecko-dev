...
JSObject* WrapAsyncGeneratorWithProto(JSContext* cx, HandleFunction unwrapped, HandleObject proto);
...
MOZ_MUST_USE bool AsyncGeneratorAwaitedFulfilled(JSContext* cx,
                                                 Handle<AsyncGeneratorObject*> asyncGenObj,
                                                 HandleValue value);
MOZ_MUST_USE bool AsyncGeneratorAwaitedRejected(JSContext* cx,
                                                Handle<AsyncGeneratorObject*> asyncGenObj,
                                                HandleValue reason);
...
class AsyncGeneratorRequest : public NativeObject {
   private:
    ...
    void init(CompletionKind completionKind, HandleValue completionValue, HandleObject promise) {
   ...
   public:
    ...
    JS::Value completionValue() const { return getFixedSlot(Slot_CompletionValue); }
...
class AsyncGeneratorObject : public NativeObject {
   private:
    ...
    State state() const { return static_cast<State>(getFixedSlot(Slot_State).toInt32()); }
    ...
    bool isSingleQueueEmpty() const { return getFixedSlot(Slot_QueueOrRequest).isNull(); }
    void setSingleQueueRequest(AsyncGeneratorRequest* request) {
        setFixedSlot(Slot_QueueOrRequest, ObjectValue(*request));
    }
    void clearSingleQueueRequest() { setFixedSlot(Slot_QueueOrRequest, NullValue()); }
    ...
    void setQueue(JSObject* queue_) { setFixedSlot(Slot_QueueOrRequest, ObjectValue(*queue_)); }
   ...
   public:
    ...
    bool isAwaitingYieldReturn() const { return state() == State_AwaitingYieldReturn; }
    ...
    static MOZ_MUST_USE bool enqueueRequest(JSContext* cx,
                                            Handle<AsyncGeneratorObject*> asyncGenObj,
                                            Handle<AsyncGeneratorRequest*> request);

    static AsyncGeneratorRequest* dequeueRequest(JSContext* cx,
                                                 Handle<AsyncGeneratorObject*> asyncGenObj);

    static AsyncGeneratorRequest* peekRequest(JSContext* cx,
                                              Handle<AsyncGeneratorObject*> asyncGenObj);
...
class AsyncFromSyncIteratorObject : public NativeObject {
   private:
    enum AsyncFromSyncIteratorObjectSlots { Slot_Iterator = 0, Slots };
...
MOZ_MUST_USE bool AsyncGeneratorResume(JSContext* cx, Handle<AsyncGeneratorObject*> asyncGenObj,
