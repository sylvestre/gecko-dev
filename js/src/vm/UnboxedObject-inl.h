...
static inline Value GetUnboxedValue(uint8_t* p, JSValueType type, bool maybeUninitialized) {
    switch (type) {
        ...
        case JSVAL_TYPE_DOUBLE: {
            ...
            if (maybeUninitialized) return DoubleValue(JS::CanonicalizeNaN(d));
...
static inline void SetUnboxedValueNoTypeChange(JSObject* unboxedObject, uint8_t* p,
                                               JSValueType type, const Value& v, bool preBarrier) {
    switch (type) {
        case JSVAL_TYPE_BOOLEAN:
            *p = v.toBoolean();
            return;
        ...
        case JSVAL_TYPE_STRING: {
            ...
            if (preBarrier) JSString::writeBarrierPre(*np);
            *np = v.toString();
            return;
        }
        ...
        case JSVAL_TYPE_OBJECT: {
            ...
            if (preBarrier) JSObject::writeBarrierPre(*np);
            *np = obj;
            return;
        }
...
static inline bool SetUnboxedValue(JSContext* cx, JSObject* unboxedObject, jsid id, uint8_t* p,
                                   JSValueType type, const Value& v, bool preBarrier) {
    switch (type) {
        case JSVAL_TYPE_BOOLEAN:
            if (v.isBoolean()) {
...
inline const UnboxedLayout& UnboxedPlainObject::layout() const { return group()->unboxedLayout(); }
