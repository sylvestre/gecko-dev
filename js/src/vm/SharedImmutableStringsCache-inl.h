...
MOZ_MUST_USE mozilla::Maybe<SharedImmutableString> SharedImmutableStringsCache::getOrCreate(
    ...
    if (!locked->set.initialized() && !locked->set.init()) return mozilla::Nothing();

    auto entry = locked->set.lookupForAdd(lookup);
    if (!entry) {
        OwnedChars ownedChars(intoOwnedChars());
        if (!ownedChars) return mozilla::Nothing();
        MOZ_ASSERT(ownedChars.get() == chars || memcmp(ownedChars.get(), chars, length) == 0);
        auto box = StringBox::Create(mozilla::Move(ownedChars), length);
        if (!box || !locked->set.add(entry, mozilla::Move(box))) return mozilla::Nothing();
    }
...
MOZ_MUST_USE mozilla::Maybe<SharedImmutableTwoByteString> SharedImmutableStringsCache::getOrCreate(
    const char16_t* chars, size_t length, IntoOwnedTwoByteChars intoOwnedTwoByteChars) {
    MOZ_ASSERT(inner_);
    MOZ_ASSERT(chars);
    auto hash =
        Hasher::hashLongString(reinterpret_cast<const char*>(chars), length * sizeof(char16_t));
