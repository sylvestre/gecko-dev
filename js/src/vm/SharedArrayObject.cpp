...
static inline void* MapMemory(size_t length, bool commit) {
#ifdef XP_WIN
...
static inline void UnmapMemory(void* addr, size_t len) {
#ifdef XP_WIN
...
static inline bool MarkValidRegion(void* addr, size_t len) {
#ifdef XP_WIN
    if (!VirtualAlloc(addr, len, MEM_COMMIT, PAGE_READWRITE)) return false;
    return true;
#else
    if (mprotect(addr, len, PROT_READ | PROT_WRITE)) return false;
    return true;
#endif
}
...
static uint64_t SharedArrayMappedSize(uint32_t allocSize) {
...
static uint32_t SharedArrayAllocSize(uint32_t length) {
    return AlignBytes(length + gc::SystemPageSize(), gc::SystemPageSize());
}

int32_t SharedArrayRawBuffer::liveBuffers() { return liveBufferCount; }

SharedArrayRawBuffer* SharedArrayRawBuffer::New(JSContext* cx, uint32_t length) {
    ...
    if (allocSize <= length) return nullptr;
    ...
    SharedArrayRawBuffer* rawbuf =
        new (base) SharedArrayRawBuffer(buffer, length, preparedForAsmJS);
...
bool SharedArrayRawBuffer::addReference() {
    ...
    for (;;) {
        ...
        if (this->refcount_.compareExchange(old_refcount, new_refcount)) return true;
    }
}
...
MOZ_ALWAYS_INLINE bool SharedArrayBufferObject::byteLengthGetterImpl(JSContext* cx,
                                                                     const CallArgs& args) {
...
bool SharedArrayBufferObject::byteLengthGetter(JSContext* cx, unsigned argc, Value* vp) {
...
bool SharedArrayBufferObject::class_constructor(JSContext* cx, unsigned argc, Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);
...
SharedArrayBufferObject* SharedArrayBufferObject::New(JSContext* cx, uint32_t length,
                                                      HandleObject proto) {
...
SharedArrayBufferObject* SharedArrayBufferObject::New(JSContext* cx, SharedArrayRawBuffer* buffer,
                                                      HandleObject proto) {
...
void SharedArrayBufferObject::dropRawBuffer() { setReservedSlot(RAWBUF_SLOT, UndefinedValue()); }
...
/* static */ void SharedArrayBufferObject::addSizeOfExcludingThis(
    ...
    info->objectsNonHeapElementsShared += buf.byteLength() / buf.rawBufferObject()->refcount();
}

/* static */ void SharedArrayBufferObject::copyData(Handle<SharedArrayBufferObject*> toBuffer,
                                                    uint32_t toIndex,
...
static const JSPropertySpec prototype_properties[] = {
    ...
    JS_STRING_SYM_PS(toStringTag, "SharedArrayBuffer", JSPROP_READONLY), JS_PS_END};
...
const Class SharedArrayBufferObject::class_ = {
    ...
    JSCLASS_DELAY_METADATA_BUILDER |
        ...
        JSCLASS_HAS_CACHED_PROTO(JSProto_SharedArrayBuffer) | JSCLASS_BACKGROUND_FINALIZE,
    &SharedArrayBufferObjectClassOps, &SharedArrayBufferObjectClassSpec, JS_NULL_CLASS_EXT};

const Class SharedArrayBufferObject::protoClass_ = {
    "SharedArrayBufferPrototype", JSCLASS_HAS_CACHED_PROTO(JSProto_SharedArrayBuffer),
    JS_NULL_CLASS_OPS, &SharedArrayBufferObjectClassSpec};

bool js::IsSharedArrayBuffer(HandleValue v) {
    return v.isObject() && v.toObject().is<SharedArrayBufferObject>();
}

bool js::IsSharedArrayBuffer(HandleObject o) { return o->is<SharedArrayBufferObject>(); }

bool js::IsSharedArrayBuffer(JSObject* o) { return o->is<SharedArrayBufferObject>(); }
...
JS_FRIEND_API(void)
js::GetSharedArrayBufferLengthAndData(JSObject* obj, uint32_t* length, bool* isSharedMemory,
                                      uint8_t** data) {
