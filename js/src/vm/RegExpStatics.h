...
class RegExpStatics {
   ...
   public:
    /* Mutators. */
    inline void updateLazily(JSContext* cx, JSLinearString* input, RegExpShared* shared,
                             size_t lastIndex);
   ...
   public:
    ...
    static size_t offsetOfPendingInput() { return offsetof(RegExpStatics, pendingInput); }

    static size_t offsetOfMatchesInput() { return offsetof(RegExpStatics, matchesInput); }

    static size_t offsetOfLazySource() { return offsetof(RegExpStatics, lazySource); }

    static size_t offsetOfLazyFlags() { return offsetof(RegExpStatics, lazyFlags); }

    static size_t offsetOfLazyIndex() { return offsetof(RegExpStatics, lazyIndex); }
...
inline bool RegExpStatics::createDependent(JSContext* cx, size_t start, size_t end,
...
inline bool RegExpStatics::createPendingInput(JSContext* cx, MutableHandleValue out) {
...
inline bool RegExpStatics::makeMatch(JSContext* cx, size_t pairNum, MutableHandleValue out) {
    /* Private function: caller must perform lazy evaluation. */
    MOZ_ASSERT(!pendingLazyEvaluation);
...
inline bool RegExpStatics::createLastMatch(JSContext* cx, MutableHandleValue out) {
    if (!executeLazy(cx)) return false;
    return makeMatch(cx, 0, out);
}

inline bool RegExpStatics::createLastParen(JSContext* cx, MutableHandleValue out) {
    if (!executeLazy(cx)) return false;
...
inline bool RegExpStatics::createParen(JSContext* cx, size_t pairNum, MutableHandleValue out) {
...
inline bool RegExpStatics::createLeftContext(JSContext* cx, MutableHandleValue out) {
    if (!executeLazy(cx)) return false;
...
inline bool RegExpStatics::createRightContext(JSContext* cx, MutableHandleValue out) {
    if (!executeLazy(cx)) return false;
...
inline void RegExpStatics::updateLazily(JSContext* cx, JSLinearString* input, RegExpShared* shared,
                                        size_t lastIndex) {
    MOZ_ASSERT(input && shared);

    BarrieredSetPair<JSString, JSLinearString>(cx->zone(), pendingInput, input, matchesInput,
                                               input);
...
inline bool RegExpStatics::updateFromMatchPairs(JSContext* cx, JSLinearString* input,
                                                MatchPairs& newPairs) {
    MOZ_ASSERT(input);
...
inline void RegExpStatics::setPendingInput(JSString* newInput) { pendingInput = newInput; }
