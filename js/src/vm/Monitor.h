...
class AutoLockMonitor : public LockGuard<Mutex> {
   ...
   public:
    explicit AutoLockMonitor(Monitor& monitor) : Base(monitor.lock_), monitor(monitor) {}
...
class AutoUnlockMonitor {
   ...
   public:
    explicit AutoUnlockMonitor(Monitor& monitor) : monitor(monitor) { monitor.lock_.unlock(); }
