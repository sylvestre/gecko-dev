...
inline GlobalObject& InterpreterFrame::global() const { return environmentChain()->global(); }
...
inline LexicalEnvironmentObject& InterpreterFrame::extensibleLexicalEnvironment() const {
    return NearestEnclosingExtensibleLexicalEnvironment(environmentChain());
}

inline void InterpreterFrame::initCallFrame(JSContext* cx, InterpreterFrame* prev,
                                            jsbytecode* prevpc, Value* prevsp, JSFunction& callee,
                                            JSScript* script, Value* argv, uint32_t nactual,
...
inline Value& InterpreterFrame::unaliasedFormal(unsigned i, MaybeCheckAliasing checkAliasing) {
...
inline Value& InterpreterFrame::unaliasedActual(unsigned i, MaybeCheckAliasing checkAliasing) {
...
inline EnvironmentObject& InterpreterFrame::aliasedEnvironment(EnvironmentCoordinate ec) const {
...
inline void InterpreterFrame::replaceInnermostEnvironment(EnvironmentObject& env) {
...
inline CallObject& InterpreterFrame::callObj() const {
    ...
    while (MOZ_UNLIKELY(!pobj->is<CallObject>())) pobj = pobj->enclosingEnvironment();
...
MOZ_ALWAYS_INLINE InterpreterFrame* InterpreterStack::getCallFrame(JSContext* cx,
                                                                   const CallArgs& args,
                                                                   HandleScript script,
                                                                   MaybeConstruct constructing,
                                                                   Value** pargv) {
...
MOZ_ALWAYS_INLINE bool InterpreterStack::pushInlineFrame(JSContext* cx, InterpreterRegs& regs,
...
MOZ_ALWAYS_INLINE bool InterpreterStack::resumeGeneratorCallFrame(JSContext* cx,
...
inline bool AbstractFramePtr::pushVarEnvironment(JSContext* cx, HandleScope scope) {
...
inline Value& AbstractFramePtr::unaliasedFormal(unsigned i, MaybeCheckAliasing checkAliasing) {
    if (isInterpreterFrame()) return asInterpreterFrame()->unaliasedFormal(i, checkAliasing);
    if (isBaselineFrame()) return asBaselineFrame()->unaliasedFormal(i, checkAliasing);
    return asRematerializedFrame()->unaliasedFormal(i, checkAliasing);
}

inline Value& AbstractFramePtr::unaliasedActual(unsigned i, MaybeCheckAliasing checkAliasing) {
    if (isInterpreterFrame()) return asInterpreterFrame()->unaliasedActual(i, checkAliasing);
    if (isBaselineFrame()) return asBaselineFrame()->unaliasedActual(i, checkAliasing);
    return asRematerializedFrame()->unaliasedActual(i, checkAliasing);
}

inline bool AbstractFramePtr::hasInitialEnvironment() const {
    if (isInterpreterFrame()) return asInterpreterFrame()->hasInitialEnvironment();
...
inline bool AbstractFramePtr::hasCachedSavedFrame() const {
    if (isInterpreterFrame()) return asInterpreterFrame()->hasCachedSavedFrame();
    if (isBaselineFrame()) return asBaselineFrame()->hasCachedSavedFrame();
    if (isWasmDebugFrame()) return asWasmDebugFrame()->hasCachedSavedFrame();
    return asRematerializedFrame()->hasCachedSavedFrame();
}

inline void AbstractFramePtr::setHasCachedSavedFrame() {
    if (isInterpreterFrame())
...
inline bool AbstractFramePtr::isNonStrictDirectEvalFrame() const {
    if (isInterpreterFrame()) return asInterpreterFrame()->isNonStrictDirectEvalFrame();
...
inline LiveSavedFrameCache* Activation::getLiveSavedFrameCache(JSContext* cx) {
    if (!frameCache_.get().initialized() && !frameCache_.get().init(cx)) return nullptr;
...
inline bool InterpreterActivation::pushInlineFrame(const CallArgs& args, HandleScript script,
                                                   MaybeConstruct constructing) {
...
inline bool InterpreterActivation::resumeGeneratorFrame(HandleFunction callee,
    ...
    if (!stack.resumeGeneratorCallFrame(cx_, regs_, callee, newTarget, envChain)) return false;

    MOZ_ASSERT(regs_.fp()->script()->compartment() == compartment_);
    return true;
}

inline bool FrameIter::hasCachedSavedFrame() const {
    if (isWasm()) return false;

    if (hasUsableAbstractFramePtr()) return abstractFramePtr().hasCachedSavedFrame();
...
inline void FrameIter::setHasCachedSavedFrame() {
    MOZ_ASSERT(!isWasm());
