...
static inline jsid id___proto__(JSContext* cx) { return NameToId(cx->names().proto); }

static inline jsid id_constructor(JSContext* cx) { return NameToId(cx->names().constructor); }

static inline jsid id_caller(JSContext* cx) { return NameToId(cx->names().caller); }
...
/* static */ UniqueChars TypeSet::TypeString(TypeSet::Type type) {
    ...
    if (type.isSingleton()) {
        ...
        SprintfLiteral(buf, "[%s * %#" PRIxPTR "]", type.groupNoBarrier()->clasp()->name,
...
const char* js::InferSpewColor(TypeConstraint* constraint) {
    ...
    static const char* const colors[] = {"\x1b[31m", "\x1b[32m", "\x1b[33m", "\x1b[34m",
...
const char* js::InferSpewColor(TypeSet* types) {
    ...
    static const char* const colors[] = {"\x1b[1;31m", "\x1b[1;32m", "\x1b[1;33m", "\x1b[1;34m",
...
bool js::ObjectGroupHasProperty(JSContext* cx, ObjectGroup* group, jsid id, const Value& value) {
    /*
    ...
    if (!group->unknownProperties() && !value.isUndefined()) {
        ...
        if (id == id___proto__(cx) || id == id_constructor(cx) || id == id_caller(cx)) return true;
        ...
        if (value.isObject()) {
            ...
            if (!obj->hasLazyGroup() && obj->group()->maybeOriginalUnboxedGroup()) return true;
        }
...
bool TypeSet::mightBeMIRType(jit::MIRType type) const {
    ...
    if (type == jit::MIRType::Object) return unknownObject() || baseObjectCount() != 0;
...
inline bool TypeSet::addTypesToConstraint(JSContext* cx, TypeConstraint* constraint) {
    /*
    ...
    for (unsigned i = 0; i < types.length(); i++) constraint->newType(cx, this, types[i]);
...
bool ConstraintTypeSet::addConstraint(JSContext* cx, TypeConstraint* constraint,
    ...
    InferSpew(ISpewOps, "addConstraint: %sT%p%s %sC%p%s %s", InferSpewColor(this), this,
              InferSpewColorReset(), InferSpewColor(constraint), constraint, InferSpewColorReset(),
              constraint->kind());
...
void ConstraintTypeSet::addType(JSContext* cx, Type type) {
    ...
    InferSpew(ISpewOps, "addType: %sT%p%s %s", InferSpewColor(this), this, InferSpewColorReset(),
              TypeString(type).get());
...
/* static */ TemporaryTypeSet* TypeSet::unionSets(TypeSet* a, TypeSet* b, LifoAlloc* alloc) {
    ...
    if (!res->unknownObject()) {
        for (size_t i = 0; i < a->getObjectCount() && !res->unknownObject(); i++) {
            if (ObjectKey* key = a->getObject(i)) res->addType(ObjectType(key), alloc);
        }
        for (size_t i = 0; i < b->getObjectCount() && !res->unknownObject(); i++) {
            if (ObjectKey* key = b->getObject(i)) res->addType(ObjectType(key), alloc);
        }
    }
...
/* static */ TemporaryTypeSet* TypeSet::removeSet(TemporaryTypeSet* input,
                                                  TemporaryTypeSet* removal, LifoAlloc* alloc) {
...
/* static */ TemporaryTypeSet* TypeSet::intersectSets(TemporaryTypeSet* a, TemporaryTypeSet* b,
                                                      LifoAlloc* alloc) {
    TemporaryTypeSet* res;
...
class js::CompilerConstraintList {
   ...
   public:
    explicit CompilerConstraintList(jit::TempAllocator& alloc)
        : failed_(false), alloc_(alloc.lifoAlloc()), constraints(alloc), frozenScripts(alloc) {}

    void add(CompilerConstraint* constraint) {
        if (!constraint || !constraints.append(constraint)) setFailed();
    }

    void freezeScript(JSScript* script, TemporaryTypeSet* thisTypes, TemporaryTypeSet* argTypes,
                      TemporaryTypeSet* bytecodeTypes) {
        FrozenScript entry;
        entry.script = script;
...
CompilerConstraintList* js::NewCompilerConstraintList(jit::TempAllocator& alloc) {
    return alloc.lifoAlloc()->new_<CompilerConstraintList>(alloc);
}

/* static */ bool TypeScript::FreezeTypeSets(CompilerConstraintList* constraints, JSScript* script,
...
class TypeCompilerConstraint : public TypeConstraint {
   ...
   public:
    ...
    void newType(JSContext* cx, TypeSet* source, TypeSet::Type type) {
        if (data.invalidateOnNewType(type)) cx->zone()->types.addPendingRecompile(cx, compilation);
    }
    ...
    JSCompartment* maybeCompartment() { return data.maybeCompartment(); }
};

template <typename T>
bool CompilerConstraintInstance<T>::generateTypeConstraint(JSContext* cx,
    ...
    return property.maybeTypes()->addConstraint(
        cx, cx->typeLifoAlloc().new_<TypeCompilerConstraint<T> >(recompileInfo, data),
        /* callExisting = */ false);
}
...
HeapTypeSetKey TypeSet::ObjectKey::property(jsid id) {
    ...
    property.id_ = id;
    if (ObjectGroup* group = maybeGroup()) property.maybeTypes_ = group->maybeGetProperty(id);
...
static bool CheckFrozenTypeSet(JSContext* cx, TemporaryTypeSet* frozen, StackTypeSet* actual) {
...
class TypeConstraintFreezeStack : public TypeConstraint {
   ...
   public:
    ...
    JSCompartment* maybeCompartment() { return script_->compartment(); }
};

} /* anonymous namespace */

bool js::FinishCompilation(JSContext* cx, HandleScript script, CompilerConstraintList* constraints,
    ...
    *precompileInfo = RecompileInfo(index, types.generation);
    ...
    for (size_t i = 0; i < constraints->length(); i++) {
        ...
        if (!constraint->generateTypeConstraint(cx, *precompileInfo)) succeeded = false;
    }
    ...
    for (size_t i = 0; i < constraints->numFrozenScripts(); i++) {
        ...
        if (entry.script != script) {
            if (!entry.script->types()->addInlinedCompilation(*precompileInfo)) succeeded = false;
        }
        ...
        for (size_t i = 0; i < count; i++) {
            if (!array[i].addConstraint(
                    cx, cx->typeLifoAlloc().new_<TypeConstraintFreezeStack>(entry.script), false))
                succeeded = false;
        }
...
static void CheckDefinitePropertiesTypeSet(JSContext* cx, TemporaryTypeSet* frozen,
                                           StackTypeSet* actual) {
...
void js::FinishDefinitePropertiesAnalysis(JSContext* cx, CompilerConstraintList* constraints) {
#ifdef DEBUG
...
bool HeapTypeSetKey::isOwnProperty(CompilerConstraintList* constraints,
    ...
    if (maybeTypes() && (!maybeTypes()->empty() || maybeTypes()->nonDataProperty())) return true;
    if (object()->isSingleton()) {
...
bool HeapTypeSetKey::knownSubset(CompilerConstraintList* constraints,
    ...
    if (!other.maybeTypes() || !maybeTypes()->isSubset(other.maybeTypes())) return false;
    freeze(constraints);
    return true;
}
...
class ConstraintDataFreezeObjectFlags {
   public:
    ...
    explicit ConstraintDataFreezeObjectFlags(ObjectGroupFlags flags) : flags(flags) {
        MOZ_ASSERT(flags);
    }
    ...
    bool invalidateOnNewObjectState(ObjectGroup* group) { return group->hasAnyFlags(flags); }
...
bool TypeSet::ObjectKey::hasFlags(CompilerConstraintList* constraints, ObjectGroupFlags flags) {
    MOZ_ASSERT(flags);
    ...
    constraints->add(
        alloc->new_<T>(alloc, objectProperty, ConstraintDataFreezeObjectFlags(flags)));
    return false;
}

bool TypeSet::ObjectKey::hasStableClassAndProto(CompilerConstraintList* constraints) {
...
class ConstraintDataFreezeObjectForTypedArrayData {
   ...
   public:
    explicit ConstraintDataFreezeObjectForTypedArrayData(TypedArrayObject& tarray)
        : obj(&tarray), viewData(tarray.viewDataEither().unwrapValue()), length(tarray.length()) {
...
static void ObjectStateChange(JSContext* cx, ObjectGroup* group, bool markingUnknown) {
    ...
    if (markingUnknown) group->addFlags(OBJECT_FLAG_DYNAMIC_MASK | OBJECT_FLAG_UNKNOWN_PROPERTIES);
...
class ConstraintDataFreezePropertyState {
   public:
    ...
    bool invalidateOnNewPropertyState(TypeSet* property) {
        return (which == NON_DATA) ? property->nonDataProperty() : property->nonWritableProperty();
    }
    bool invalidateOnNewObjectState(ObjectGroup* group) { return false; }
...
bool HeapTypeSetKey::constant(CompilerConstraintList* constraints, Value* valOut) {
...
bool TemporaryTypeSet::filtersType(const TemporaryTypeSet* other, Type filteredType) const {
    if (other->unknown()) return unknown();

    for (TypeFlags flag = 1; flag < TYPE_FLAG_ANYOBJECT; flag <<= 1) {
        Type type = PrimitiveType(TypeFlagPrimitive(flag));
        if (type != filteredType && other->hasType(type) && !hasType(type)) return false;
    }
...
TemporaryTypeSet::DoubleConversion TemporaryTypeSet::convertDoubleElements(
    ...
    for (unsigned i = 0; i < getObjectCount(); i++) {
        ...
        // cannot be converted).
        if (!property.maybeTypes() || !property.maybeTypes()->hasType(DoubleType()) ||
...
const Class* TemporaryTypeSet::getKnownClass(CompilerConstraintList* constraints) {
    if (unknownObject()) return nullptr;
...
void TemporaryTypeSet::getTypedArraySharedness(CompilerConstraintList* constraints,
                                               TypedArraySharedness* sharedness) {
    // In the future this will inspect the object set.
    *sharedness = UnknownSharedness;
}

TemporaryTypeSet::ForAllResult TemporaryTypeSet::forAllClasses(CompilerConstraintList* constraints,
    ...
    for (unsigned i = 0; i < count; i++) {
        const Class* clasp = getObjectClass(i);
        if (!clasp) continue;
        if (!getObject(i)->hasStableClassAndProto(constraints)) return ForAllResult::MIXED;
        if (func(clasp)) {
            true_results = true;
...
Scalar::Type TemporaryTypeSet::getTypedArrayType(CompilerConstraintList* constraints,
                                                 TypedArraySharedness* sharedness) {
...
bool TemporaryTypeSet::maybeProxy(CompilerConstraintList* constraints) {
    if (!maybeObject()) return false;
    ...
    for (unsigned i = 0; i < count; i++) {
        const Class* clasp = getObjectClass(i);
        if (!clasp) continue;
        if (clasp->isProxy() || !getObject(i)->hasStableClassAndProto(constraints)) return true;
    }

    return false;
}

bool TemporaryTypeSet::maybeEmulatesUndefined(CompilerConstraintList* constraints) {
    if (!maybeObject()) return false;
...
bool TemporaryTypeSet::getCommonPrototype(CompilerConstraintList* constraints, JSObject** proto) {
    if (unknownObject()) return false;
...
bool TemporaryTypeSet::propertyNeedsBarrier(CompilerConstraintList* constraints, jsid id) {
    if (unknownObject()) return true;
...
bool js::ClassCanHaveExtraProperties(const Class* clasp) {
    ...
    return clasp->getResolve() || clasp->getOpsLookupProperty() || clasp->getOpsGetProperty() ||
           IsTypedArrayClass(clasp);
}

void TypeZone::processPendingRecompiles(FreeOp* fop, RecompileInfoVector& recompiles) {
    ...
    for (size_t i = 0; i < recompiles.length(); i++) {
        AutoEnterOOMUnsafeRegion oomUnsafe;
        if (!pending.append(recompiles[i])) oomUnsafe.crash("processPendingRecompiles");
    }
    recompiles.clear();
...
void TypeZone::addPendingRecompile(JSContext* cx, const RecompileInfo& info) {
    ...
    InferSpew(ISpewOps, "addPendingRecompile: %p:%s:%zu", co->script(), co->script()->filename(),
              co->script()->lineno());
...
void TypeZone::addPendingRecompile(JSContext* cx, JSScript* script) {
    ...
    if (script->hasIonScript()) addPendingRecompile(cx, script->ionScript()->recompileInfo());

    // Trigger recompilation of any callers inlining this script.
    if (TypeScript* types = script->types()) {
        for (RecompileInfo info : types->inlinedCompilations()) addPendingRecompile(cx, info);
...
void js::ReportMagicWordFailure(uintptr_t actual, uintptr_t expected) {
    MOZ_CRASH_UNSAFE_PRINTF("Got 0x%" PRIxPTR " expected magic word 0x%" PRIxPTR, actual,
                            expected);
}

void js::ReportMagicWordFailure(uintptr_t actual, uintptr_t expected, uintptr_t flags,
                                uintptr_t objectSet) {
...
void js::PrintTypes(JSContext* cx, JSCompartment* comp, bool force) {
    ...
    for (auto group = zone->cellIter<ObjectGroup>(); !group.done(); group.next()) group->print();
#endif
}
...
static inline void UpdatePropertyType(JSContext* cx, HeapTypeSet* types, NativeObject* obj,
                                      Shape* shape, bool indexed) {
    ...
    if (shape->hasGetterValue() || shape->hasSetterValue()) {
        ...
        if (!indexed && types->canSetDefinite(shape->slot())) types->setDefinite(shape->slot());
...
void ObjectGroup::updateNewPropertyTypes(JSContext* cx, JSObject* objArg, jsid id,
                                         HeapTypeSet* types) {
    InferSpew(ISpewOps, "typeSet: %sT%p%s property %s %s", InferSpewColor(types), types,
              InferSpewColorReset(), TypeSet::ObjectGroupString(this).get(), TypeIdString(id));
...
void ObjectGroup::addDefiniteProperties(JSContext* cx, Shape* shape) {
    ...
    while (!shape->isEmptyShape()) {
        ...
        if (!JSID_IS_VOID(id)) {
            ...
            if (types->canSetDefinite(shape->slot())) types->setDefinite(shape->slot());
        }
...
void js::AddTypePropertyId(JSContext* cx, ObjectGroup* group, JSObject* obj, jsid id,
                           TypeSet::Type type) {
    ...
    if (!types->empty() && !types->nonConstantProperty()) {
        InferSpew(ISpewOps, "constantMutated: %sT%p%s %s", InferSpewColor(types), types,
                  InferSpewColorReset(), TypeSet::TypeString(type).get());
        types->setNonConstantProperty(cx);
    }
    ...
    // propagated to below.
    if (type.isObjectUnchecked() && types->unknownObject()) type = TypeSet::AnyObjectType();
...
void ObjectGroup::markPropertyNonWritable(JSContext* cx, JSObject* obj, jsid id) {
    AutoEnterAnalysis enter(cx);
...
void ObjectGroup::setFlags(JSContext* cx, ObjectGroupFlags flags) {
    ...
    if (ObjectGroup* unboxedGroup = maybeOriginalUnboxedGroup()) unboxedGroup->setFlags(cx, flags);
}
...
void ObjectGroup::clearNewScript(JSContext* cx, ObjectGroup* replacement /* = nullptr*/) {
    TypeNewScript* newScript = anyNewScript();
    if (!newScript) return;
    ...
    if (!replacement) {
        ...
        if (!NativeObject::setNewScriptCleared(cx, fun)) cx->recoverFromOutOfMemory();
    }
...
void ObjectGroup::print() {
    ...
    fprintf(stderr, "%s : %s", TypeSet::ObjectGroupString(this).get(),
            tagged.isObject() ? TypeSet::TypeString(TypeSet::ObjectType(tagged.toObject())).get()
    ...
    if (unknownProperties()) {
        ...
        if (!hasAnyFlags(OBJECT_FLAG_LENGTH_OVERFLOW)) fprintf(stderr, " noLengthOverflow");
...
class TypeConstraintClearDefiniteGetterSetter : public TypeConstraint {
   public:
    ...
    explicit TypeConstraintClearDefiniteGetterSetter(ObjectGroup* group) : group(group) {}
    ...
    void newPropertyState(JSContext* cx, TypeSet* source) {
         ...
         * non-writable.
         */
        if (source->nonDataProperty() || source->nonWritableProperty()) group->clearNewScript(cx);
    }
    ...
    JSCompartment* maybeCompartment() { return group->compartment(); }
};

bool js::AddClearDefiniteGetterSetterForPrototypeChain(JSContext* cx, ObjectGroup* group,
                                                       HandleId id) {
    /*
    ...
    while (proto) {
        ...
        if (!protoTypes->addConstraint(
                cx, cx->typeLifoAlloc().new_<TypeConstraintClearDefiniteGetterSetter>(group)))
            return false;
...
class TypeConstraintClearDefiniteSingle : public TypeConstraint {
   public:
    ...
    explicit TypeConstraintClearDefiniteSingle(ObjectGroup* group) : group(group) {}

    const char* kind() { return "clearDefiniteSingle"; }

    void newType(JSContext* cx, TypeSet* source, TypeSet::Type type) {
        if (source->baseFlags() || source->getObjectCount() > 1) group->clearNewScript(cx);
    }
...
bool js::AddClearDefiniteFunctionUsesInScript(JSContext* cx, ObjectGroup* group, JSScript* script,
    ...
    for (unsigned i = 0; i < count; i++) {
        ...
        if (!types->unknownObject() && types->getObjectCount() == 1) {
            ...
            if (!types->addConstraint(
                    cx, cx->typeLifoAlloc().new_<TypeConstraintClearDefiniteSingle>(group)))
                return false;
        }
    }
...
void js::TypeMonitorCallSlow(JSContext* cx, JSObject* callee, const CallArgs& args,
                             bool constructing) {
    ...
    for (; arg < nargs; arg++) TypeScript::SetArgument(cx, script, arg, UndefinedValue());
}
...
void js::TypeMonitorResult(JSContext* cx, JSScript* script, jsbytecode* pc, TypeSet::Type type) {
    ...
    InferSpew(ISpewOps, "bytecodeType: %p %05zu: %s", script, script->pcToOffset(pc),
              TypeSet::TypeString(type).get());
    types->addType(cx, type);
}

void js::TypeMonitorResult(JSContext* cx, JSScript* script, jsbytecode* pc, StackTypeSet* types,
                           TypeSet::Type type) {
    assertSameCompartment(cx, script, type);
...
bool JSScript::makeTypes(JSContext* cx) {
    ...
    TypeScript* typeScript =
        (TypeScript*)zone()->pod_calloc<uint8_t>(TypeScript::SizeIncludingTypeArray(count));
    if (!typeScript) {
        ReportOutOfMemory(cx);
        return false;
    }
    ...
    for (unsigned i = 0; i < nTypeSets(); i++) {
        InferSpew(ISpewOps, "typeSet: %sT%p%s bytecode%u %p", InferSpewColor(&typeArray[i]),
                  &typeArray[i], InferSpewColorReset(), i, this);
    }
    TypeSet* thisTypes = TypeScript::ThisTypes(this);
    InferSpew(ISpewOps, "typeSet: %sT%p%s this %p", InferSpewColor(thisTypes), thisTypes,
              InferSpewColorReset(), this);
    unsigned nargs = functionNonDelazifying() ? functionNonDelazifying()->nargs() : 0;
    for (unsigned i = 0; i < nargs; i++) {
        TypeSet* types = TypeScript::ArgTypes(this, i);
        InferSpew(ISpewOps, "typeSet: %sT%p%s arg%u %p", InferSpewColor(types), types,
...
/* static */ bool JSFunction::setTypeForScriptedFunction(JSContext* cx, HandleFunction fun,
...
static bool OnlyHasDataProperties(Shape* shape) {
    ...
    while (!shape->isEmptyShape()) {
        if (!shape->isDataProperty() || !shape->configurable() || !shape->enumerable() ||
            !shape->writable()) {
            return false;
        }
        shape = shape->previous();
...
void PreliminaryObjectArrayWithTemplate::maybeAnalyze(JSContext* cx, ObjectGroup* group,
                                                      bool force) {
    ...
    for (size_t i = 0; i < PreliminaryObjectArray::COUNT; i++) {
        ...
        if (obj->inDictionaryMode() || !OnlyHasDataProperties(obj->lastProperty())) return;
...
/* static */ bool TypeNewScript::make(JSContext* cx, ObjectGroup* group, JSFunction* fun) {
    MOZ_ASSERT(cx->zone()->types.activeAnalysis);
...
/* static */ TypeNewScript* TypeNewScript::makeNativeVersion(JSContext* cx,
...
size_t TypeNewScript::sizeOfIncludingThis(mozilla::MallocSizeOf mallocSizeOf) const {
...
static bool ChangeObjectFixedSlotCount(JSContext* cx, PlainObject* obj, gc::AllocKind allocKind) {
...
struct DestroyTypeNewScript {
    ...
    DestroyTypeNewScript(JSContext* cx, ObjectGroup* group) : cx(cx), group(group) {}
...
bool TypeNewScript::maybeAnalyze(JSContext* cx, ObjectGroup* group, bool* regenerate, bool force) {
    ...
    if (templateObject()->slotSpan() != 0) {
        ...
        if (templateObject()->slotSpan() > prefixShape->slotSpan()) return true;
        {
            ...
            while (shape->slotSpan() != templateObject()->slotSpan()) shape = shape->previous();
    ...
    if (!TryConvertToUnboxedLayout(cx, enter, templateObject()->lastProperty(), group,
                                   preliminaryObjects))
        return false;
    ...
    ObjectGroup* initialGroup =
        ObjectGroupCompartment::makeGroup(cx, group->clasp(), protoRoot, initialFlags);
...
bool TypeNewScript::rollbackPartiallyInitializedObjects(JSContext* cx, ObjectGroup* group) {
    ...
    for (const CooperatingContext& target : cx->runtime()->cooperatingContexts()) {
        for (AllScriptFramesIter iter(cx, target); !iter.done(); ++iter) {
            ...
            if (!iter.isConstructing() || !iter.matchCallee(cx, function)) continue;
...
void ConstraintTypeSet::sweep(Zone* zone, AutoClearTypeInferenceStateOnOOM& oom) {
...
void ObjectGroup::sweep(AutoClearTypeInferenceStateOnOOM* oom) {
    ...
    if (maybeUnboxedLayout()) {
        ...
        if (IsAboutToBeFinalizedUnbarriered(&group)) unboxedLayout().detachFromCompartment();
...
/* static */ void JSScript::maybeSweepTypes(AutoClearTypeInferenceStateOnOOM* oom) {
    ...
    if (types.sweepReleaseTypes && !hasBaselineScript() && !hasIonScript()) {
        types_->destroy();
        types_ = nullptr;
...
void Zone::addSizeOfIncludingThis(mozilla::MallocSizeOf mallocSizeOf, size_t* typePool,
                                  size_t* regexpZone, size_t* jitZone,
                                  size_t* baselineStubsOptimized, size_t* cachedCFG,
                                  size_t* uniqueIdMap, size_t* shapeTables,
...
void TypeZone::beginSweep(FreeOp* fop, bool releaseTypes, AutoClearTypeInferenceStateOnOOM& oom) {
    MOZ_ASSERT(zone()->isGCSweepingOrCompacting());
        ...
        for (size_t i = 0; i < compilerOutputs->length(); i++) {
            ...
            if (output.isValid()) {
                ...
                if (IsAboutToBeFinalizedUnbarriered(&script)) {
                    ...
                    if (!newCompilerOutputs) newCompilerOutputs = js_new<CompilerOutputVector>();
...
void TypeZone::clearAllNewScriptsOnOOM() {
    for (auto iter = zone()->cellIter<ObjectGroup>(); !iter.done(); iter.next()) {
        ...
        if (!IsAboutToBeFinalizedUnbarriered(&group)) group->maybeClearNewScriptOnOOM();
    }
}
...
void TypeScript::printTypes(JSContext* cx, HandleScript script) const {
    ...
    if (script->functionNonDelazifying())
        ...
        fprintf(stderr, "Main");
    fprintf(stderr, " %#" PRIxPTR " %s:%zu ", uintptr_t(script.get()), script->filename(),
            script->lineno());
    ...
    for (unsigned i = 0;
         script->functionNonDelazifying() && i < script->functionNonDelazifying()->nargs(); i++) {
        fprintf(stderr, "\n    arg%u:", i);
