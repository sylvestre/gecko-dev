...
class GeneratorObject : public NativeObject {
   ...
   public:
    ...
    static bool resume(JSContext* cx, InterpreterActivation& activation, HandleObject obj,
                       HandleValue arg, ResumeKind resumeKind);

    static bool initialSuspend(JSContext* cx, HandleObject obj, AbstractFramePtr frame,
                               jsbytecode* pc) {
        return suspend(cx, obj, frame, pc, nullptr, 0);
    }

    static bool normalSuspend(JSContext* cx, HandleObject obj, AbstractFramePtr frame,
    ...
    void setCallee(JSFunction& callee) { setFixedSlot(CALLEE_SLOT, ObjectValue(callee)); }

    JSObject& environmentChain() const { return getFixedSlot(ENV_CHAIN_SLOT).toObject(); }
    ...
    bool hasExpressionStack() const { return getFixedSlot(EXPRESSION_STACK_SLOT).isObject(); }
    ...
    void clearExpressionStack() { setFixedSlot(EXPRESSION_STACK_SLOT, NullValue()); }

    bool isConstructing() const { return getFixedSlot(NEWTARGET_SLOT).isObject(); }
    const Value& newTarget() const { return getFixedSlot(NEWTARGET_SLOT); }
    void setNewTarget(const Value& newTarget) { setFixedSlot(NEWTARGET_SLOT, newTarget); }
   ...
   public:
    ...
    static size_t offsetOfEnvironmentChainSlot() { return getFixedSlotOffset(ENV_CHAIN_SLOT); }
    static size_t offsetOfArgsObjSlot() { return getFixedSlotOffset(ARGS_OBJ_SLOT); }
    ...
    static size_t offsetOfNewTargetSlot() { return getFixedSlotOffset(NEWTARGET_SLOT); }
};
...
MOZ_MUST_USE bool CheckStarGeneratorResumptionValue(JSContext* cx, HandleValue v);

}  // namespace js

template <>
inline bool JSObject::is<js::GeneratorObject>() const {
