...
class GenericPrinterPrintfTarget : public mozilla::PrintfTarget {
   public:
    ...
    bool append(const char* sp, size_t len) { return printer.put(sp, len); }
...
bool Sprinter::put(const char* s, size_t len) {
    ...
    if (s >= oldBase && s < oldEnd) {
        ...
        if (base != oldBase) s = stringAt(s - oldBase); /* this is where it lives now */
        memmove(bp, s, len);
    } else {
        js_memcpy(bp, s, len);
    }
...
const char js_EscapeMap[] = {'\b', 'b', '\f', 'f', '\n', 'n',  '\r', 'r',  '\t', 't',
                             '\v', 'v', '"',  '"', '\'', '\'', '\\', '\\', '\0'};

template <typename CharT>
static char* QuoteString(Sprinter* sp, const mozilla::Range<const CharT> chars, char16_t quote) {
    ...
    for (CharPtr t = chars.begin(); t < end; ++t) {
        ...
        if (!(c >> 8) && c != 0 && (escape = strchr(js_EscapeMap, int(c))) != nullptr) {
             ...
             * (see bug 621814).
             */
            if (!sp->jsprintf((quote && !(c >> 8)) ? "\\x%02X" : "\\u%04X", c)) return nullptr;
        }
    }
...
char* QuoteString(Sprinter* sp, JSString* str, char16_t quote) {
    ...
    JS::AutoCheckCannotGC nogc;
    return linear->hasLatin1Chars() ? QuoteString(sp, linear->latin1Range(nogc), quote)
...
void LSprinter::exportInto(GenericPrinter& out) const {
    ...
    for (Chunk* it = head_; it != tail_; it = it->next) out.put(it->chars(), it->length);
