...
static inline void AssertScopeMatchesEnvironment(Scope* scope, JSObject* originalEnv) {
#ifdef DEBUG
    JSObject* env = originalEnv;
    for (ScopeIter si(scope); si; si++) {
        if (si.kind() == ScopeKind::NonSyntactic) {
            while (env->is<WithEnvironmentObject>() || env->is<NonSyntacticVariablesObject>() ||
...
static inline void AssertScopeMatchesEnvironment(InterpreterFrame* fp, jsbytecode* pc) {
#ifdef DEBUG
...
bool InterpreterFrame::prologue(JSContext* cx) {
    ...
    if (isGlobalFrame()) {
        ...
        if (!CheckGlobalDeclarationConflicts(cx, script, lexicalEnv, varObjRoot)) return false;
        return probes::EnterScript(cx, script, nullptr, this);
    }
...
void InterpreterFrame::epilogue(JSContext* cx, jsbytecode* pc) {
    ...
    if (isFunctionFrame()) {
        if (!callee().isStarGenerator() && !callee().isLegacyGenerator() && !callee().isAsync() &&
            isConstructing() && thisArgument().isObject() && returnValue().isPrimitive()) {
...
bool InterpreterFrame::checkReturn(JSContext* cx, HandleValue thisv) {
    ...
    if (thisv.isMagic(JS_UNINITIALIZED_LEXICAL)) return ThrowUninitializedThis(cx, this);
...
bool InterpreterFrame::pushLexicalEnvironment(JSContext* cx, Handle<LexicalScope*> scope) {
...
void InterpreterFrame::traceValues(JSTracer* trc, unsigned start, unsigned end) {
    if (start < end) TraceRootRange(trc, end - start, slots() + start, "vm_stack");
}

static void TraceInterpreterActivation(JSTracer* trc, InterpreterActivation* act) {
...
void js::TraceInterpreterActivations(JSContext* cx, const CooperatingContext& target,
                                     JSTracer* trc) {
    for (ActivationIterator iter(cx, target); !iter.done(); ++iter) {
        Activation* act = iter.activation();
        if (act->isInterpreter()) TraceInterpreterActivation(trc, act->asInterpreter());
    }
}
...
InterpreterFrame* InterpreterStack::pushInvokeFrame(JSContext* cx, const CallArgs& args,
...
InterpreterFrame* InterpreterStack::pushExecuteFrame(JSContext* cx, HandleScript script,
...
OnlyJSJitFrameIter::OnlyJSJitFrameIter(jit::JitActivation* act) : JitFrameIter(act) { settle(); }
...
FrameIter::FrameIter(const Data& data)
    : data_(data), ionInlineFrames_(data.cx_, isIonScripted() ? &jsJitFrame() : nullptr) {
    MOZ_ASSERT(data.cx_);
    if (isIonScripted()) {
        while (ionInlineFrames_.frameNo() != data.ionInlineFrameNo_) ++ionInlineFrames_;
    }
}
...
FrameIter::Data* FrameIter::copyData() const {
    ...
    if (data && isIonScripted()) data->ionInlineFrameNo_ = ionInlineFrames_.frameNo();
    return data;
}
...
bool FrameIter::isConstructing() const {
    switch (data_.state_) {
        ...
        case JIT:
            MOZ_ASSERT(isJSJit());
            if (jsJitFrame().isIonScripted()) return ionInlineFrames_.isConstructing();
            MOZ_ASSERT(jsJitFrame().isBaselineJS());
    ...
    switch (data_.state_) {
        ...
        case JIT:
            if (jsJitFrame().isBaselineJS()) {
                ...
                while (data_.activations_.activation() != activation) ++data_.activations_;
...
bool FrameIter::matchCallee(JSContext* cx, HandleFunction fun) const {
    ...
    if (useSameScript && (currentCallee->hasScript() != fun->hasScript() ||
...
unsigned FrameIter::numFormalArgs() const { return script()->functionNonDelazifying()->nargs(); }

Value FrameIter::unaliasedActual(unsigned i, MaybeCheckAliasing checkAliasing) const {
...
Value FrameIter::returnValue() const {
    switch (data_.state_) {
        case DONE:
            break;
        case JIT:
            if (jsJitFrame().isBaselineJS()) return jsJitFrame().baselineFrame()->returnValue();
            break;
        case INTERP:
...
void NonBuiltinFrameIter::settle() {
    if (!SelfHostedFramesVisible()) {
        while (!done() && hasScript() && script()->selfHosted()) FrameIter::operator++();
    }
}
...
jit::RematerializedFrame* jit::JitActivation::getRematerializedFrame(JSContext* cx,
...
jit::RematerializedFrame* jit::JitActivation::lookupRematerializedFrame(uint8_t* top,
                                                                        size_t inlineDepth) {
    if (!rematerializedFrames_) return nullptr;
    if (RematerializedFrameTable::Ptr p = rematerializedFrames_->lookup(top))
...
void jit::JitActivation::removeRematerializedFramesFromDebugger(JSContext* cx, uint8_t* top) {
...
bool jit::JitActivation::registerIonFrameRecovery(RInstructionResults&& results) {
...
jit::RInstructionResults* jit::JitActivation::maybeIonFrameRecovery(JitFrameLayout* fp) {
...
void jit::JitActivation::startWasmInterrupt(
...
void* jit::JitActivation::wasmUnwindPC() const {
    MOZ_ASSERT(hasWasmExitFP());
    MOZ_ASSERT(isWasmInterrupted());
    return cx_->runtime()->wasmUnwindPC();
}

void* jit::JitActivation::wasmResumePC() const {
    MOZ_ASSERT(hasWasmExitFP());
    MOZ_ASSERT(isWasmInterrupted());
...
ActivationIterator::ActivationIterator(JSContext* cx) : activation_(cx->activation_) {
...
JS::ProfilingFrameIterator::ProfilingFrameIterator(JSContext* cx, const RegisterState& state,
    ...
    : cx_(cx), sampleBufferGen_(sampleBufferGen), activation_(nullptr) {
...
Maybe<JS::ProfilingFrameIterator::Frame> JS::ProfilingFrameIterator::getPhysicalFrameAndEntry(
...
uint32_t JS::ProfilingFrameIterator::extractStack(Frame* frames, uint32_t offset,
                                                  uint32_t end) const {
    ...
    uint32_t depth = entry.callStackAtAddr(cx_->runtime(), jsJitIter().returnAddressToFp(), labels,
                                           ArrayLength(labels));
...
Maybe<JS::ProfilingFrameIterator::Frame> JS::ProfilingFrameIterator::getPhysicalFrameWithoutLabel()
    const {
...
bool JS::ProfilingFrameIterator::isJSJit() const { return kind_ == Kind::JSJit; }
