...
/* static */ Shape* js::ErrorObject::assignInitialShape(JSContext* cx, Handle<ErrorObject*> obj) {
...
/* static */ bool js::ErrorObject::init(JSContext* cx, Handle<ErrorObject*> obj, JSExnType type,
    ...
    if (!EmptyShape::ensureInitialCustomShape<ErrorObject>(cx, obj)) return false;
    ...
    if (message) {
        messageShape =
            NativeObject::addDataProperty(cx, obj, cx->names().message, MESSAGE_SLOT, 0);
        if (!messageShape) return false;
    ...
    MOZ_ASSERT(obj->lookupPure(NameToId(cx->names().columnNumber))->slot() == COLUMNNUMBER_SLOT);
    MOZ_ASSERT_IF(message, obj->lookupPure(NameToId(cx->names().message))->slot() == MESSAGE_SLOT);
    ...
    if (message) obj->setSlotWithType(cx, messageShape, StringValue(message));

    return true;
}

/* static */ ErrorObject* js::ErrorObject::create(JSContext* cx, JSExnType errorType,
    ...
    if (!ErrorObject::init(cx, errObject, errorType, report, fileName, stack, lineNumber,
...
JSErrorReport* js::ErrorObject::getOrCreateErrorReport(JSContext* cx) {
    if (JSErrorReport* r = getErrorReport()) return r;
    ...
    if (!filenameStr.encodeLatin1(cx, fileName(cx))) return nullptr;
    ...
    if (!message) message = cx->runtime()->emptyString;
    if (!message->ensureFlat(cx)) return nullptr;
...
/* static */ bool js::ErrorObject::getStack(JSContext* cx, unsigned argc, Value* vp) {
...
/* static */ bool js::ErrorObject::getStack_impl(JSContext* cx, const CallArgs& args) {
    RootedObject thisObj(cx, &args.thisv().toObject());

    RootedObject obj(cx);
    if (!FindErrorInstanceOrPrototype(cx, thisObj, &obj)) return false;
    ...
    if (!BuildStackString(cx, savedFrameObj, &stackString)) return false;
    ...
    if (cx->runtime()->stackFormat() == js::StackFormat::V8) {
        ...
        RootedValue rval(cx);
        if (!js::Call(cx, val, args.thisv(), &rval)) return false;

        if (!rval.isString()) return false;
...
/* static */ bool js::ErrorObject::setStack(JSContext* cx, unsigned argc, Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);
    // We accept any object here, because of poor-man's subclassing of Error.
    return CallNonGenericMethod<IsObject, setStack_impl>(cx, args);
}

/* static */ bool js::ErrorObject::setStack_impl(JSContext* cx, const CallArgs& args) {
    RootedObject thisObj(cx, &args.thisv().toObject());

    if (!args.requireAtLeast(cx, "(set stack)", 1)) return false;
