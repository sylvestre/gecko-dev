...
inline jit::IonScript* CompilerOutput::ion() const {
...
inline CompilerOutput* RecompileInfo::compilerOutput(TypeZone& types) const {
    if (generation != types.generation) {
        ...
        if (!output->isValid()) return nullptr;
        output = &(*types.compilerOutputs)[output->sweepIndex()];
        return output->isValid() ? output : nullptr;
    }

    if (!types.compilerOutputs || outputIndex >= types.compilerOutputs->length()) return nullptr;
...
inline bool RecompileInfo::shouldSweep(TypeZone& types) {
    CompilerOutput* output = compilerOutput(types);
    if (!output || !output->isValid()) return true;
...
/* static */ inline TypeSet::ObjectKey* TypeSet::ObjectKey::get(JSObject* obj) {
    MOZ_ASSERT(obj);
    if (obj->isSingleton()) return (ObjectKey*)(uintptr_t(obj) | 1);
    return (ObjectKey*)obj->group();
}

/* static */ inline TypeSet::ObjectKey* TypeSet::ObjectKey::get(ObjectGroup* group) {
    MOZ_ASSERT(group);
    if (group->singleton()) return (ObjectKey*)(uintptr_t(group->singleton()) | 1);
...
inline JSCompartment* TypeSet::ObjectKey::maybeCompartment() {
    if (isSingleton()) return singleton()->compartment();

    return group()->compartment();
}

/* static */ inline TypeSet::Type TypeSet::ObjectType(JSObject* obj) {
    if (obj->isSingleton()) return Type(uintptr_t(obj) | 1);
    return Type(uintptr_t(obj->group()));
}

/* static */ inline TypeSet::Type TypeSet::ObjectType(ObjectGroup* group) {
    if (group->singleton()) return Type(uintptr_t(group->singleton()) | 1);
    return Type(uintptr_t(group));
}

/* static */ inline TypeSet::Type TypeSet::ObjectType(ObjectKey* obj) {
    return Type(uintptr_t(obj));
}

inline TypeSet::Type TypeSet::GetValueType(const Value& val) {
    if (val.isDouble()) return TypeSet::DoubleType();
...
inline bool TypeSet::IsUntrackedValue(const Value& val) {
    return val.isMagic() &&
           (val.whyMagic() == JS_OPTIMIZED_OUT || val.whyMagic() == JS_UNINITIALIZED_LEXICAL);
}
...
struct AutoEnterAnalysis {
    ...
    explicit AutoEnterAnalysis(JSContext* cx) : suppressGC(cx), suppressMetadata(cx) {
    ...
    ~AutoEnterAnalysis() {
        if (this != zone->types.activeAnalysis) return;
...
void TypeMonitorCallSlow(JSContext* cx, JSObject* callee, const CallArgs& args, bool constructing);

/*
 * Monitor a javascript call, either on entry to the interpreter or made
 * from within the interpreter.
 */
inline void TypeMonitorCall(JSContext* cx, const js::CallArgs& args, bool constructing) {
...
MOZ_ALWAYS_INLINE bool TrackPropertyTypes(JSObject* obj, jsid id) {
    if (obj->hasLazyGroup() || obj->group()->unknownPropertiesDontCheckGeneration()) return false;

    if (obj->isSingleton() && !obj->group()->maybeGetPropertyDontCheckGeneration(id)) return false;
...
inline bool PropertyHasBeenMarkedNonConstant(JSObject* obj, jsid id) {
    ...
    if (!obj->isSingleton()) return true;

    // EnsureTrackPropertyTypes must have been called on this object.
    if (obj->group()->unknownProperties()) return true;
    HeapTypeSet* types = obj->group()->maybeGetProperty(IdToTypeId(id));
    return types->nonConstantProperty();
}

MOZ_ALWAYS_INLINE bool HasTrackedPropertyType(JSObject* obj, jsid id, TypeSet::Type type) {
    ...
    if (HeapTypeSet* types = obj->group()->maybeGetPropertyDontCheckGeneration(id)) {
        if (!types->hasType(type)) return false;
        // Non-constant properties are only relevant for singleton objects.
        if (obj->isSingleton() && !types->nonConstantProperty()) return false;
        return true;
    }
...
MOZ_ALWAYS_INLINE bool HasTypePropertyId(JSObject* obj, jsid id, TypeSet::Type type) {
    id = IdToTypeId(id);
    if (!TrackPropertyTypes(obj, id)) return true;

    return HasTrackedPropertyType(obj, id, type);
}

MOZ_ALWAYS_INLINE bool HasTypePropertyId(JSObject* obj, jsid id, const Value& value) {
    return HasTypePropertyId(obj, id, TypeSet::GetValueType(value));
}

void AddTypePropertyId(JSContext* cx, ObjectGroup* group, JSObject* obj, jsid id,
                       TypeSet::Type type);
void AddTypePropertyId(JSContext* cx, ObjectGroup* group, JSObject* obj, jsid id,
                       const Value& value);
...
inline void MarkObjectGroupFlags(JSContext* cx, JSObject* obj, ObjectGroupFlags flags) {
...
inline void MarkObjectGroupUnknownProperties(JSContext* cx, ObjectGroup* obj) {
    if (!obj->unknownProperties()) obj->markUnknown(cx);
}

inline void MarkTypePropertyNonData(JSContext* cx, JSObject* obj, jsid id) {
    id = IdToTypeId(id);
    if (TrackPropertyTypes(obj, id)) obj->group()->markPropertyNonData(cx, obj, id);
}

inline void MarkTypePropertyNonWritable(JSContext* cx, JSObject* obj, jsid id) {
    id = IdToTypeId(id);
    if (TrackPropertyTypes(obj, id)) obj->group()->markPropertyNonWritable(cx, obj, id);
}
...
/* static */ inline StackTypeSet* TypeScript::ArgTypes(JSScript* script, unsigned i) {
...
/* static */ inline TYPESET* TypeScript::BytecodeTypes(JSScript* script, jsbytecode* pc,
    ...
    if (bytecodeMap[*hint] == offset) return typeArray + *hint;
...
/* static */ inline StackTypeSet* TypeScript::BytecodeTypes(JSScript* script, jsbytecode* pc) {
    ...
    return BytecodeTypes(script, pc, script->baselineScript()->bytecodeTypeMap(), hint,
                         types->typeArray());
}

/* static */ inline void TypeScript::Monitor(JSContext* cx, JSScript* script, jsbytecode* pc,
                                             const js::Value& rval) {
...
/* static */ inline void TypeScript::Monitor(JSContext* cx, const js::Value& rval) {
    jsbytecode* pc;
...
/* static */ inline void TypeScript::Monitor(JSContext* cx, JSScript* script, jsbytecode* pc,
                                             StackTypeSet* types, const js::Value& rval) {
    TypeSet::Type type = TypeSet::GetValueType(rval);
    if (!types->hasType(type)) TypeMonitorResult(cx, script, pc, types, type);
}

/* static */ inline void TypeScript::MonitorAssign(JSContext* cx, HandleObject obj, jsid id) {
    if (!obj->isSingleton()) {
        /*
        ...
        if (group->basePropertyCount() < 128) return;
        MarkObjectGroupUnknownProperties(cx, group);
    }
}

/* static */ inline void TypeScript::SetThis(JSContext* cx, JSScript* script, TypeSet::Type type) {
...
/* static */ inline void TypeScript::SetArgument(JSContext* cx, JSScript* script, unsigned arg,
                                                 TypeSet::Type type) {
    assertSameCompartment(cx, script, type);
...
struct TypeHashSet {
    ...
    static inline unsigned Capacity(unsigned count) {
        ...
        if (count <= SET_ARRAY_SIZE) return SET_ARRAY_SIZE;
    ...
    static U** InsertTry(LifoAlloc& alloc, U**& values, unsigned& count, T key) {
        ...
        if (!converting) {
            while (values[insertpos] != nullptr) {
                if (KEY::getKey(values[insertpos]) == key) return &values[insertpos];
                insertpos = (insertpos + 1) & (capacity - 1);
            }
        }

        if (count >= SET_CAPACITY_OVERFLOW) return nullptr;
        ...
        for (unsigned i = 0; i < capacity; i++) {
            if (values[i]) {
                ...
                while (newValues[pos] != nullptr) pos = (pos + 1) & (newCapacity - 1);
        ...
        while (values[insertpos] != nullptr) insertpos = (insertpos + 1) & (newCapacity - 1);
        return &values[insertpos];
    }
    ...
    template <class T, class U, class KEY>
    static inline U** Insert(LifoAlloc& alloc, U**& values, unsigned& count, T key) {
        if (count == 0) {
        ...
        if (count == 1) {
            U* oldData = (U*)values;
            if (KEY::getKey(oldData) == key) return (U**)&values;
        ...
        if (count <= SET_ARRAY_SIZE) {
            ...
            for (unsigned i = 0; i < count; i++) {
                if (KEY::getKey(values[i]) == key) return &values[i];
            }
    ...
    static MOZ_ALWAYS_INLINE U* Lookup(U** values, unsigned count, T key) {
        ...
        if (count == 1) return (KEY::getKey((U*)values) == key) ? (U*)values : nullptr;
        ...
        while (values[pos] != nullptr) {
            if (KEY::getKey(values[pos]) == key) return values[pos];
...
inline JSObject* TypeSet::Type::singleton() const { return objectKey()->singleton(); }

inline ObjectGroup* TypeSet::Type::group() const { return objectKey()->group(); }

inline JSObject* TypeSet::Type::singletonNoBarrier() const {
    return objectKey()->singletonNoBarrier();
}

inline ObjectGroup* TypeSet::Type::groupNoBarrier() const { return objectKey()->groupNoBarrier(); }
...
inline JSCompartment* TypeSet::Type::maybeCompartment() {
    if (isSingletonUnchecked()) return singletonNoBarrier()->compartment();

    if (isGroupUnchecked()) return groupNoBarrier()->compartment();
...
MOZ_ALWAYS_INLINE bool TypeSet::hasType(Type type) const {
    ...
    if (type.isUnknown()) {
        ...
        return !!(flags & TYPE_FLAG_ANYOBJECT) ||
               TypeHashSet::Lookup<ObjectKey*, ObjectKey, ObjectKey>(objectSet, baseObjectCount(),
                                                                     type.objectKey()) != nullptr;
    }
}

inline void TypeSet::setBaseObjectCount(uint32_t count) {
    MOZ_ASSERT(count <= TYPE_FLAG_DOMOBJECT_COUNT_LIMIT);
    flags = (flags & ~TYPE_FLAG_OBJECT_COUNT_MASK) | (count << TYPE_FLAG_OBJECT_COUNT_SHIFT);
}

inline void HeapTypeSet::newPropertyState(JSContext* cx) {
    checkMagic();
...
inline void HeapTypeSet::setNonDataProperty(JSContext* cx) {
    checkMagic();

    if (flags & TYPE_FLAG_NON_DATA_PROPERTY) return;

    flags |= TYPE_FLAG_NON_DATA_PROPERTY;
    newPropertyState(cx);
}

inline void HeapTypeSet::setNonWritableProperty(JSContext* cx) {
    checkMagic();

    if (flags & TYPE_FLAG_NON_WRITABLE_PROPERTY) return;

    flags |= TYPE_FLAG_NON_WRITABLE_PROPERTY;
    newPropertyState(cx);
}

inline void HeapTypeSet::setNonConstantProperty(JSContext* cx) {
    checkMagic();

    if (flags & TYPE_FLAG_NON_CONSTANT_PROPERTY) return;
...
inline unsigned TypeSet::getObjectCount() const {
    ...
    if (count > TypeHashSet::SET_ARRAY_SIZE) return TypeHashSet::Capacity(count);
    return count;
}
...
inline const Class* TypeSet::getObjectClass(unsigned i) const {
    if (JSObject* object = getSingleton(i)) return object->getClass();
    if (ObjectGroup* group = getGroup(i)) return group->clasp();
    return nullptr;
}
...
inline void ObjectGroup::setBasePropertyCount(uint32_t count) {
    // Note: Callers must ensure they are performing threadsafe operations.
    MOZ_ASSERT(count <= OBJECT_FLAG_PROPERTY_COUNT_LIMIT);
    flags_ =
        (flags() & ~OBJECT_FLAG_PROPERTY_COUNT_MASK) | (count << OBJECT_FLAG_PROPERTY_COUNT_SHIFT);
}

inline HeapTypeSet* ObjectGroup::getProperty(JSContext* cx, JSObject* obj, jsid id) {
    ...
    if (HeapTypeSet* types = maybeGetProperty(id)) return types;
...
MOZ_ALWAYS_INLINE HeapTypeSet* ObjectGroup::maybeGetPropertyDontCheckGeneration(jsid id) {
    MOZ_ASSERT(JSID_IS_VOID(id) || JSID_IS_EMPTY(id) || JSID_IS_STRING(id) || JSID_IS_SYMBOL(id));
    MOZ_ASSERT_IF(!JSID_IS_EMPTY(id), id == IdToTypeId(id));
...
MOZ_ALWAYS_INLINE HeapTypeSet* ObjectGroup::maybeGetProperty(jsid id) {
    maybeSweep(nullptr);
...
inline ObjectGroup::Property* ObjectGroup::getProperty(unsigned i) {
    ...
    if (result) result->types.checkMagic();
    return result;
}

}  // namespace js

inline js::TypeScript* JSScript::types() {
    maybeSweepTypes(nullptr);
    return types_;
}

inline bool JSScript::ensureHasTypes(JSContext* cx) { return types() || makeTypes(cx); }
