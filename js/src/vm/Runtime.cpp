...
static const JSWrapObjectCallbacks DefaultWrapObjectCallbacks = {TransparentObjectWrapper,
                                                                 nullptr};
...
JSRuntime::JSRuntime(JSRuntime* parentRuntime)
    : parentRuntime(parentRuntime),
      ...
      stackFormat_(parentRuntime ? js::StackFormat::Default : js::StackFormat::SpiderMonkey) {
    liveRuntimesCount++;
...
bool JSRuntime::init(JSContext* cx, uint32_t maxbytes, uint32_t maxNurseryBytes) {
#ifdef DEBUG
    MOZ_ASSERT(!initialized_);
    initialized_ = true;
#endif
...
void JSRuntime::setNewbornActiveContext(JSContext* cx) {
    ...
    if (!cooperatingContexts().append(cx)) oomUnsafe.crash("Add cooperating context");
}
...
bool JSRuntime::beginSingleThreadedExecution(JSContext* cx) {
    if (singleThreadedExecutionRequired_ == 0) {
        ...
        if (beginSingleThreadedExecutionCallback) beginSingleThreadedExecutionCallback(cx);
...
void JSRuntime::endSingleThreadedExecution(JSContext* cx) {
    ...
    if (--singleThreadedExecutionRequired_ == 0) {
        if (endSingleThreadedExecutionCallback) endSingleThreadedExecutionCallback(cx);
    }
}
...
void JSRuntime::setTelemetryCallback(JSRuntime* rt, JSAccumulateTelemetryDataCallback callback) {
...
void JSRuntime::setUseCounterCallback(JSRuntime* rt, JSSetUseCounterCallback callback) {
...
void JSRuntime::traceSharedIntlData(JSTracer* trc) { sharedIntlData.ref().trace(trc); }
...
FreeOp::~FreeOp() {
    ...
    if (!jitPoisonRanges.empty()) jit::ExecutableAllocator::poisonCode(runtime(), jitPoisonRanges);
}

bool FreeOp::isDefaultFreeOp() const { return runtime_ && runtime_->defaultFreeOp() == this; }
...
bool JSRuntime::enqueuePromiseJob(JSContext* cx, HandleFunction job, HandleObject promise,
...
void JSRuntime::addUnhandledRejectedPromise(JSContext* cx, js::HandleObject promise) {
    MOZ_ASSERT(promise->is<PromiseObject>());
    if (!cx->promiseRejectionTrackerCallback) return;

    void* data = cx->promiseRejectionTrackerCallbackData;
    cx->promiseRejectionTrackerCallback(cx, promise, PromiseRejectionHandlingState::Unhandled,
                                        data);
}

void JSRuntime::removeUnhandledRejectedPromise(JSContext* cx, js::HandleObject promise) {
    MOZ_ASSERT(promise->is<PromiseObject>());
    if (!cx->promiseRejectionTrackerCallback) return;

    void* data = cx->promiseRejectionTrackerCallbackData;
    cx->promiseRejectionTrackerCallback(cx, promise, PromiseRejectionHandlingState::Handled, data);
}
...
void JSRuntime::updateMallocCounter(size_t nbytes) { gc.updateMallocCounter(nbytes); }
...
void* JSRuntime::onOutOfMemoryCanGC(AllocFunction allocFunc, size_t bytes, void* reallocPtr) {
    if (OnLargeAllocationFailure && bytes >= LARGE_ALLOCATION) OnLargeAllocationFailure();
...
void JSRuntime::clearUsedByHelperThread(Zone* zone) {
    ...
    numActiveHelperThreadZones--;
    JSContext* cx = TlsContext.get();
    if (gc.fullGCForAtomsRequested() && cx->canCollectAtoms()) gc.triggerFullGCForAtoms(cx);
}
...
#ifdef DEBUG
bool js::CurrentThreadIsPerformingGC() { return TlsContext.get()->performingGC; }
#endif

JS_FRIEND_API(void)
JS::UpdateJSContextProfilerSampleBufferGen(JSContext* cx, uint32_t generation, uint32_t lapCount) {
