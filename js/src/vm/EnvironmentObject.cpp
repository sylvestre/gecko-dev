...
Shape* js::EnvironmentCoordinateToEnvironmentShape(JSScript* script, jsbytecode* pc) {
...
PropertyName* js::EnvironmentCoordinateName(EnvironmentCoordinateNameCache& cache,
    ...
    if (shape == cache.shape) {
        ...
        Shape::Range<NoGC> r(shape);
        while (r.front().slot() != ec.slot()) r.popFront();
        id = r.front().propidRaw();
    }

    /* Beware nameless destructuring formal. */
    if (!JSID_IS_ATOM(id)) return script->runtimeFromAnyThread()->commonNames->empty;
    return JSID_TO_ATOM(id)->asPropertyName();
}

JSScript* js::EnvironmentCoordinateFunctionScript(JSScript* script, jsbytecode* pc) {
    MOZ_ASSERT(JOF_OPTYPE(JSOp(*pc)) == JOF_ENVCOORD);
    ScopeIter si(script->innermostScope(pc));
    uint32_t hops = EnvironmentCoordinate(pc).hops();
    while (true) {
    ...
    if (si.kind() != ScopeKind::Function) return nullptr;
    return si.scope()->as<FunctionScope>().script();
}

/*****************************************************************************/

CallObject* CallObject::create(JSContext* cx, HandleShape shape, HandleObjectGroup group) {
    ...
    JS_TRY_VAR_OR_RETURN_NULL(cx, obj,
                              NativeObject::create(cx, kind, gc::DefaultHeap, shape, group));
...
CallObject* CallObject::createSingleton(JSContext* cx, HandleShape shape) {
    ...
    RootedObjectGroup group(cx,
                            ObjectGroup::lazySingletonGroup(cx, &class_, TaggedProto(nullptr)));
    if (!group) return nullptr;

    JSObject* obj;
    JS_TRY_VAR_OR_RETURN_NULL(cx, obj,
                              NativeObject::create(cx, kind, gc::TenuredHeap, shape, group));

    MOZ_ASSERT(obj->isSingleton(), "group created inline above must be a singleton");
 ...
 * must be null.
 */
CallObject* CallObject::create(JSContext* cx, HandleFunction callee, HandleObject enclosing) {
    ...
    if (!frame.script()->bodyScope()->as<FunctionScope>().hasParameterExprs()) {
        ...
        for (PositionalFormalParameterIter fi(frame.script()); fi; fi++) {
            ...
            callobj->setAliasedBinding(
                cx, fi, frame.unaliasedFormal(fi.argumentSlot(), DONT_CHECK_ALIASING));
        }
    }
...
CallObject* CallObject::createHollowForDebug(JSContext* cx, HandleFunction callee) {
    ...
    for (Rooted<BindingIter> bi(cx, BindingIter(script)); bi; bi++) {
        ...
        if (!SetProperty(cx, callobj, id, optimizedOut)) return nullptr;
    }
...
const Class CallObject::class_ = {
    "Call", JSCLASS_IS_ANONYMOUS | JSCLASS_HAS_RESERVED_SLOTS(CallObject::RESERVED_SLOTS)};

/*****************************************************************************/

/* static */ VarEnvironmentObject* VarEnvironmentObject::create(JSContext* cx, HandleShape shape,
...
/* static */ VarEnvironmentObject* VarEnvironmentObject::create(JSContext* cx, HandleScope scope,
...
/* static */ VarEnvironmentObject* VarEnvironmentObject::createHollowForDebug(
    ...
    for (Rooted<BindingIter> bi(cx, BindingIter(scope)); bi; bi++) {
        id = NameToId(bi.name()->asPropertyName());
        if (!SetProperty(cx, env, id, optimizedOut)) return nullptr;
    }
...
const ObjectOps ModuleEnvironmentObject::objectOps_ = {
    ...
    nullptr,
    nullptr, /* watch/unwatch */
    nullptr, /* getElements */
    nullptr};

const ClassOps ModuleEnvironmentObject::classOps_ = {nullptr, /* addProperty */
                                                     nullptr, /* delProperty */
                                                     nullptr, /* enumerate */
...
const Class ModuleEnvironmentObject::class_ = {
    ...
    JSCLASS_HAS_RESERVED_SLOTS(ModuleEnvironmentObject::RESERVED_SLOTS) | JSCLASS_IS_ANONYMOUS,
    &ModuleEnvironmentObject::classOps_,
    JS_NULL_CLASS_SPEC,
    JS_NULL_CLASS_EXT,
    &ModuleEnvironmentObject::objectOps_};

/* static */ ModuleEnvironmentObject* ModuleEnvironmentObject::create(JSContext* cx,
    ...
    if (!JSObject::setSingleton(cx, env)) return nullptr;
...
IndirectBindingMap& ModuleEnvironmentObject::importBindings() { return module().importBindings(); }

bool ModuleEnvironmentObject::createImportBinding(JSContext* cx, HandleAtom importName,
                                                  HandleModuleObject module,
...
bool ModuleEnvironmentObject::lookupImport(jsid name, ModuleEnvironmentObject** envOut,
                                           Shape** shapeOut) {
    return importBindings().lookup(name, envOut, shapeOut);
}

void ModuleEnvironmentObject::fixEnclosingEnvironmentAfterCompartmentMerge(GlobalObject& global) {
    setEnclosingEnvironment(&global.lexicalEnvironment());
}

/* static */ bool ModuleEnvironmentObject::lookupProperty(JSContext* cx, HandleObject obj,
    ...
    if (!NativeLookupOwnProperty<CanGC>(cx, target, id, propp)) return false;

    objp.set(obj);
    return true;
}

/* static */ bool ModuleEnvironmentObject::hasProperty(JSContext* cx, HandleObject obj,
...
/* static */ bool ModuleEnvironmentObject::getProperty(JSContext* cx, HandleObject obj,
...
/* static */ bool ModuleEnvironmentObject::setProperty(JSContext* cx, HandleObject obj,
                                                       HandleId id, HandleValue v,
                                                       HandleValue receiver,
                                                       JS::ObjectOpResult& result) {
    RootedModuleEnvironmentObject self(cx, &obj->as<ModuleEnvironmentObject>());
    if (self->importBindings().has(id)) return result.failReadOnly();
...
/* static */ bool ModuleEnvironmentObject::getOwnPropertyDescriptor(
    JSContext* cx, HandleObject obj, HandleId id, MutableHandle<PropertyDescriptor> desc) {
    const IndirectBindingMap& bindings = obj->as<ModuleEnvironmentObject>().importBindings();
    Shape* shape;
    ModuleEnvironmentObject* env;
    if (bindings.lookup(id, &env, &shape)) {
...
/* static */ bool ModuleEnvironmentObject::deleteProperty(JSContext* cx, HandleObject obj,
                                                          HandleId id, ObjectOpResult& result) {
    return result.failCantDelete();
}

/* static */ bool ModuleEnvironmentObject::newEnumerate(JSContext* cx, HandleObject obj,
...
const Class WasmInstanceEnvironmentObject::class_ = {
    "WasmInstance", JSCLASS_IS_ANONYMOUS |
                        JSCLASS_HAS_RESERVED_SLOTS(WasmInstanceEnvironmentObject::RESERVED_SLOTS)};

/* static */ WasmInstanceEnvironmentObject* WasmInstanceEnvironmentObject::createHollowForDebug(
...
/* static */ WasmFunctionCallObject* WasmFunctionCallObject::createHollowForDebug(
...
WithEnvironmentObject* WithEnvironmentObject::create(JSContext* cx, HandleObject object,
                                                     HandleObject enclosing,
...
WithEnvironmentObject* WithEnvironmentObject::createNonSyntactic(JSContext* cx,
                                                                 HandleObject object,
                                                                 HandleObject enclosing) {
...
static bool CheckUnscopables(JSContext* cx, HandleObject obj, HandleId id, bool* scopable) {
    RootedId unscopablesId(
        cx, SYMBOL_TO_JSID(cx->wellKnownSymbols().get(JS::SymbolCode::unscopables)));
    RootedValue v(cx);
    if (!GetProperty(cx, obj, obj, unscopablesId, &v)) return false;
    if (v.isObject()) {
        RootedObject unscopablesObj(cx, &v.toObject());
        if (!GetProperty(cx, unscopablesObj, unscopablesObj, id, &v)) return false;
...
static bool with_LookupProperty(JSContext* cx, HandleObject obj, HandleId id,
    ...
    if (!LookupProperty(cx, actual, id, objp, propp)) return false;

    if (propp) {
        bool scopable;
        if (!CheckUnscopables(cx, actual, id, &scopable)) return false;
        if (!scopable) {
            objp.set(nullptr);
            propp.setNotFound();
        }
...
static bool with_HasProperty(JSContext* cx, HandleObject obj, HandleId id, bool* foundp) {
    MOZ_ASSERT(!IsUnscopableDotName(cx, id));
    RootedObject actual(cx, &obj->as<WithEnvironmentObject>().object());

    // ES 8.1.1.2.1 step 3-5.
    if (!HasProperty(cx, actual, id, foundp)) return false;
...
static bool with_GetProperty(JSContext* cx, HandleObject obj, HandleValue receiver, HandleId id,
                             MutableHandleValue vp) {
    MOZ_ASSERT(!IsUnscopableDotName(cx, id));
    RootedObject actual(cx, &obj->as<WithEnvironmentObject>().object());
    RootedValue actualReceiver(cx, receiver);
    if (receiver.isObject() && &receiver.toObject() == obj) actualReceiver.setObject(*actual);
    return GetProperty(cx, actual, actualReceiver, id, vp);
}

static bool with_SetProperty(JSContext* cx, HandleObject obj, HandleId id, HandleValue v,
...
static bool with_GetOwnPropertyDescriptor(JSContext* cx, HandleObject obj, HandleId id,
...
const Class WithEnvironmentObject::class_ = {
    "With",
    JSCLASS_HAS_RESERVED_SLOTS(WithEnvironmentObject::RESERVED_SLOTS) | JSCLASS_IS_ANONYMOUS,
    JS_NULL_CLASS_OPS,
    JS_NULL_CLASS_SPEC,
    JS_NULL_CLASS_EXT,
    &WithEnvironmentObjectOps};

/* static */ NonSyntacticVariablesObject* NonSyntacticVariablesObject::create(JSContext* cx) {
    Rooted<NonSyntacticVariablesObject*> obj(
        cx, NewObjectWithNullTaggedProto<NonSyntacticVariablesObject>(cx, TenuredObject,
                                                                      BaseShape::DELEGATE));
    if (!obj) return nullptr;

    MOZ_ASSERT(obj->isUnqualifiedVarObj());
    if (!JSObject::setQualifiedVarObj(cx, obj)) return nullptr;
...
/* static */ LexicalEnvironmentObject* LexicalEnvironmentObject::createTemplateObject(
    JSContext* cx, HandleShape shape, HandleObject enclosing, gc::InitialHeap heap) {
    ...
    if (enclosing) env->initEnclosingEnvironment(enclosing);

    return env;
}

/* static */ LexicalEnvironmentObject* LexicalEnvironmentObject::create(
    JSContext* cx, Handle<LexicalScope*> scope, HandleObject enclosing, gc::InitialHeap heap) {
...
/* static */ LexicalEnvironmentObject* LexicalEnvironmentObject::createGlobal(
    ...
    Rooted<LexicalEnvironmentObject*> env(
        cx, LexicalEnvironmentObject::createTemplateObject(cx, shape, global, gc::TenuredHeap));
    if (!env) return nullptr;
...
/* static */ LexicalEnvironmentObject* LexicalEnvironmentObject::createNonSyntactic(
...
/* static */ LexicalEnvironmentObject* LexicalEnvironmentObject::createHollowForDebug(
...
bool LexicalEnvironmentObject::isExtensible() const { return nonProxyIsExtensible(); }
...
const Class LexicalEnvironmentObject::class_ = {
    "LexicalEnvironment",
    JSCLASS_HAS_RESERVED_SLOTS(LexicalEnvironmentObject::RESERVED_SLOTS) | JSCLASS_IS_ANONYMOUS,
    JS_NULL_CLASS_OPS,
    JS_NULL_CLASS_SPEC,
    JS_NULL_CLASS_EXT,
    JS_NULL_OBJECT_OPS};

/* static */ NamedLambdaObject* NamedLambdaObject::create(JSContext* cx, HandleFunction callee,
                                                          HandleFunction func,
                                                          HandleObject enclosing,
                                                          gc::InitialHeap heap) {
...
/* static */ NamedLambdaObject* NamedLambdaObject::createTemplateObject(JSContext* cx,
...
/* static */ RuntimeLexicalErrorObject* RuntimeLexicalErrorObject::create(JSContext* cx,
                                                                          HandleObject enclosing,
                                                                          unsigned errorNumber) {
    RuntimeLexicalErrorObject* obj = NewObjectWithNullTaggedProto<RuntimeLexicalErrorObject>(
...
static void ReportRuntimeLexicalErrorId(JSContext* cx, unsigned errorNumber, HandleId id) {
    if (JSID_IS_ATOM(id)) {
...
static bool lexicalError_LookupProperty(JSContext* cx, HandleObject obj, HandleId id,
...
static bool lexicalError_HasProperty(JSContext* cx, HandleObject obj, HandleId id, bool* foundp) {
    ReportRuntimeLexicalErrorId(cx, obj->as<RuntimeLexicalErrorObject>().errorNumber(), id);
    return false;
}

static bool lexicalError_GetProperty(JSContext* cx, HandleObject obj, HandleValue receiver,
                                     HandleId id, MutableHandleValue vp) {
    ReportRuntimeLexicalErrorId(cx, obj->as<RuntimeLexicalErrorObject>().errorNumber(), id);
    return false;
}

static bool lexicalError_SetProperty(JSContext* cx, HandleObject obj, HandleId id, HandleValue v,
                                     HandleValue receiver, ObjectOpResult& result) {
    ReportRuntimeLexicalErrorId(cx, obj->as<RuntimeLexicalErrorObject>().errorNumber(), id);
    return false;
}

static bool lexicalError_GetOwnPropertyDescriptor(JSContext* cx, HandleObject obj, HandleId id,
                                                  MutableHandle<PropertyDescriptor> desc) {
    ReportRuntimeLexicalErrorId(cx, obj->as<RuntimeLexicalErrorObject>().errorNumber(), id);
    return false;
}

static bool lexicalError_DeleteProperty(JSContext* cx, HandleObject obj, HandleId id,
                                        ObjectOpResult& result) {
    ReportRuntimeLexicalErrorId(cx, obj->as<RuntimeLexicalErrorObject>().errorNumber(), id);
    return false;
}
...
const Class RuntimeLexicalErrorObject::class_ = {
    "RuntimeLexicalError",
    JSCLASS_HAS_RESERVED_SLOTS(RuntimeLexicalErrorObject::RESERVED_SLOTS) | JSCLASS_IS_ANONYMOUS,
    JS_NULL_CLASS_OPS,
    JS_NULL_CLASS_SPEC,
    JS_NULL_CLASS_EXT,
...
EnvironmentIter::EnvironmentIter(JSContext* cx,
                                 const EnvironmentIter& ei MOZ_GUARD_OBJECT_NOTIFIER_PARAM_IN_IMPL)
    : si_(cx, ei.si_.get()), env_(cx, ei.env_), frame_(ei.frame_) {
    MOZ_GUARD_OBJECT_NOTIFIER_INIT;
}

EnvironmentIter::EnvironmentIter(JSContext* cx, JSObject* env,
                                 Scope* scope MOZ_GUARD_OBJECT_NOTIFIER_PARAM_IN_IMPL)
...
EnvironmentIter::EnvironmentIter(JSContext* cx, AbstractFramePtr frame,
                                 jsbytecode* pc MOZ_GUARD_OBJECT_NOTIFIER_PARAM_IN_IMPL)
...
EnvironmentIter::EnvironmentIter(JSContext* cx, JSObject* env, Scope* scope,
                                 AbstractFramePtr frame MOZ_GUARD_OBJECT_NOTIFIER_PARAM_IN_IMPL)
...
void EnvironmentIter::incrementScopeIter() {
    if (si_.scope()->is<GlobalScope>()) {
        ...
        if (!env_->is<EnvironmentObject>()) si_++;
    } else {
        si_++;
    }
}
...
void EnvironmentIter::settle() {
    ...
    if (frame_ && frame_.hasScript() && frame_.script()->initialEnvironmentShape() &&
    ...
    if (frame_ &&
        (!si_ || (frame_.hasScript() && si_.scope() == frame_.script()->enclosingScope()) ||
...
bool LiveEnvironmentVal::needsSweep() {
    if (scope_) MOZ_ALWAYS_FALSE(IsAboutToBeFinalized(&scope_));
    return false;
}
...
class DebugEnvironmentProxyHandler : public BaseProxyHandler {
    ...
    bool handleUnaliasedAccess(JSContext* cx, Handle<DebugEnvironmentProxy*> debugEnv,
        ...
        *accessResult = ACCESS_GENERIC;
        ...
        if (env->is<CallObject>()) {
            ...
            while (bi && NameToId(bi.name()->asPropertyName()) != id) bi++;
            if (!bi) return true;

            if (!bi.hasArgumentSlot()) {
                if (bi.closedOver()) return true;
                ...
                if (action == SET) TypeScript::SetArgument(cx, script, i, vp);
            }
        ...
        if (env->is<LexicalEnvironmentObject>() || env->is<VarEnvironmentObject>()) {
            ...
            if (maybeLiveEnv) {
                ...
                if (action == GET) {
                    ...
                    if (!GetProperty(cx, env, env, id, vp)) return false;
                } else {
                    if (!SetProperty(cx, env, id, vp)) return false;
                }
            }
        ...
        if (env->is<WasmFunctionCallObject>()) {
            if (maybeLiveEnv) {
                ...
                for (BindingIter bi(scope); bi; bi++) {
                    if (JSID_IS_ATOM(id, bi.name())) break;
        ...
        if (env->is<WasmInstanceEnvironmentObject>()) {
            ...
            if (action == GET) {
                ...
                if (instanceScope->globalsStart() <= index) {
                    ...
                    if (!instance.debug().getGlobal(instance,
                                                    index - instanceScope->globalsStart(), vp)) {
                        ReportOutOfMemory(cx);
                        return false;
                    }
                }
        ...
        MOZ_ASSERT(!IsSyntacticEnvironment(env) || env->is<WithEnvironmentObject>());
        return true;
    }
    ...
    static bool isThis(JSContext* cx, jsid id) { return id == NameToId(cx->names().dotThis); }

    static bool isFunctionEnvironment(const JSObject& env) { return env.is<CallObject>(); }
    ...
    static Scope* getEnvironmentScope(const JSObject& env) {
        ...
        if (env.is<VarEnvironmentObject>()) return &env.as<VarEnvironmentObject>().scope();
        if (env.is<WasmInstanceEnvironmentObject>())
            return &env.as<WasmInstanceEnvironmentObject>().scope();
        if (env.is<WasmFunctionCallObject>()) return &env.as<WasmFunctionCallObject>().scope();
        return nullptr;
    }
     ...
     */
    static bool isMagicMissingArgumentsValue(JSContext* cx, EnvironmentObject& env,
                                             HandleValue v) {
     ...
     */
    static bool createMissingThis(JSContext* cx, EnvironmentObject& env, MutableHandleValue thisv,
                                  bool* success) {
        *success = false;
        ...
        if (!GetFunctionThis(cx, maybeEnv->frame(), thisv)) return false;
   ...
   public:
    ...
    bool getMissingArgumentsPropertyDescriptor(JSContext* cx,
        ...
        if (!createMissingArguments(cx, env, &argsObj)) return false;
    ...
    bool getMissingThisPropertyDescriptor(JSContext* cx, Handle<DebugEnvironmentProxy*> debugEnv,
                                          EnvironmentObject& env,
                                          MutableHandle<PropertyDescriptor> desc) const {
        RootedValue thisv(cx);
        bool success;
        if (!createMissingThis(cx, env, &thisv, &success)) return false;
    ...
    bool getOwnPropertyDescriptor(JSContext* cx, HandleObject proxy, HandleId id,
        ...
        RootedValue v(cx);
        AccessResult access;
        if (!handleUnaliasedAccess(cx, debugEnv, env, id, GET, &v, &access)) return false;
    ...
    bool get(JSContext* cx, HandleObject proxy, HandleValue receiver, HandleId id,
        ...
        if (isMissingArguments(cx, id, *env)) return getMissingArguments(cx, *env, vp);

        if (isMissingThis(cx, id, *env)) return getMissingThis(cx, *env, vp);

        AccessResult access;
        if (!handleUnaliasedAccess(cx, debugEnv, env, id, GET, vp, &access)) return false;

        switch (access) {
            case ACCESS_UNALIASED:
                if (isMagicMissingArgumentsValue(cx, *env, vp))
                    return getMissingArguments(cx, *env, vp);
                if (isMaybeUninitializedThisValue(cx, id, vp)) return getMissingThis(cx, *env, vp);
                return true;
            case ACCESS_GENERIC:
                if (!GetProperty(cx, env, env, id, vp)) return false;
                if (isMaybeUninitializedThisValue(cx, id, vp)) return getMissingThis(cx, *env, vp);
     ...
     * exceptional cases.
     */
    bool getMaybeSentinelValue(JSContext* cx, Handle<DebugEnvironmentProxy*> debugEnv, HandleId id,
                               MutableHandleValue vp) const {
        Rooted<EnvironmentObject*> env(cx, &debugEnv->environment());

        if (isMissingArguments(cx, id, *env))
            return getMissingArgumentsMaybeSentinelValue(cx, *env, vp);
        if (isMissingThis(cx, id, *env)) return getMissingThisMaybeSentinelValue(cx, *env, vp);
    ...
    bool set(JSContext* cx, HandleObject proxy, HandleId id, HandleValue v, HandleValue receiver,
        ...
        if (debugEnv->isOptimizedOut()) return Throw(cx, id, JSMSG_DEBUG_CANT_SET_OPT_ENV);

        AccessResult access;
        RootedValue valCopy(cx, v);
        if (!handleUnaliasedAccess(cx, debugEnv, env, id, SET, &valCopy, &access)) return false;
    ...
    bool defineProperty(JSContext* cx, HandleObject proxy, HandleId id,
                        Handle<PropertyDescriptor> desc, ObjectOpResult& result) const override {
        Rooted<EnvironmentObject*> env(cx, &proxy->as<DebugEnvironmentProxy>().environment());

        bool found;
        if (!has(cx, proxy, id, &found)) return false;
        if (found) return Throw(cx, id, JSMSG_CANT_REDEFINE_PROP);
    ...
    bool ownPropertyKeys(JSContext* cx, HandleObject proxy, AutoIdVector& props) const override {
        ...
        if (isMissingArgumentsBinding(*env)) {
            if (!props.append(NameToId(cx->names().arguments))) return false;
        }
        if (isMissingThisBinding(*env)) {
            if (!props.append(NameToId(cx->names().dotThis))) return false;
        }
        ...
        if (isWith)
            ...
            target = env;
        if (!GetPropertyKeys(cx, target, JSITER_OWNONLY, &props)) return false;

        if (isWith) {
            size_t j = 0;
            for (size_t i = 0; i < props.length(); i++) {
                bool inScope;
                if (!CheckUnscopables(cx, env, props[i], &inScope)) return false;
                if (inScope) props[j++].set(props[i]);
            }
            if (!props.resize(j)) return false;
        }
    ...
    bool has(JSContext* cx, HandleObject proxy, HandleId id_, bool* bp) const override {
        ...
        if (!JS_HasPropertyById(cx, env, id, &found)) return false;
...
/* static */ DebugEnvironmentProxy* DebugEnvironmentProxy::create(JSContext* cx,
                                                                  EnvironmentObject& env,
                                                                  HandleObject enclosing) {
    ...
    JSObject* obj =
        NewProxyObject(cx, &DebugEnvironmentProxyHandler::singleton, priv, nullptr /* proto */);
    if (!obj) return nullptr;
...
bool DebugEnvironmentProxy::isForDeclarative() const {
    ...
    return e.is<CallObject>() || e.is<VarEnvironmentObject>() || e.is<ModuleEnvironmentObject>() ||
           e.is<WasmInstanceEnvironmentObject>() || e.is<WasmFunctionCallObject>() ||
           e.is<LexicalEnvironmentObject>();
}

/* static */ bool DebugEnvironmentProxy::getMaybeSentinelValue(JSContext* cx,
...
bool DebugEnvironmentProxy::isOptimizedOut() const {
    ...
    if (DebugEnvironments::hasLiveEnvironment(e)) return false;
...
void DebugEnvironments::checkHashTablesAfterMovingGC(JSRuntime* runtime) {
    /*
 ...
 */
static bool CanUseDebugEnvironmentMaps(JSContext* cx) { return cx->compartment()->isDebuggee(); }
...
/* static */ bool DebugEnvironments::addDebugEnvironment(JSContext* cx,
                                                         Handle<EnvironmentObject*> env,
                                                         Handle<DebugEnvironmentProxy*> debugEnv) {
    MOZ_ASSERT(cx->compartment() == env->compartment());
    MOZ_ASSERT(cx->compartment() == debugEnv->compartment());

    if (!CanUseDebugEnvironmentMaps(cx)) return true;
...
/* static */ bool DebugEnvironments::addDebugEnvironment(JSContext* cx, const EnvironmentIter& ei,
                                                         Handle<DebugEnvironmentProxy*> debugEnv) {
    MOZ_ASSERT(!ei.hasSyntacticEnvironment());
    MOZ_ASSERT(cx->compartment() == debugEnv->compartment());
...
/* static */ void DebugEnvironments::takeFrameSnapshot(JSContext* cx,
                                                       Handle<DebugEnvironmentProxy*> debugEnv,
                                                       AbstractFramePtr frame) {
    /*
    ...
    if (debugEnv->environment().is<CallObject>()) {
        ...
        if (script->analyzedArgsUsage() && script->needsArgsObj() && frame.hasArgsObj()) {
            for (unsigned i = 0; i < frame.numFormalArgs(); ++i) {
                if (script->formalLivesInArgumentsObject(i)) vec[i].set(frame.argsObj().arg(i));
            }
        }
    } else {
...
/* static */ void DebugEnvironments::onPopCall(JSContext* cx, AbstractFramePtr frame) {
    assertSameCompartment(cx, frame);
    ...
    if (funScope->hasEnvironment()) {
         ...
         */
        if (!frame.environmentChain()->is<CallObject>()) return;
...
void DebugEnvironments::onPopLexical(JSContext* cx, AbstractFramePtr frame, jsbytecode* pc) {
    assertSameCompartment(cx, frame);
...
void DebugEnvironments::onPopVar(JSContext* cx, AbstractFramePtr frame, jsbytecode* pc) {
    assertSameCompartment(cx, frame);
...
bool DebugEnvironments::updateLiveEnvironments(JSContext* cx) {
    ...
    for (AllFramesIter i(cx); !i.done(); ++i) {
        if (!i.hasUsableAbstractFramePtr()) continue;

        AbstractFramePtr frame = i.abstractFramePtr();
        if (frame.environmentChain()->compartment() != cx->compartment()) continue;
        ...
        if (!frame.isDebuggee()) continue;

        RootedObject env(cx);
        RootedScope scope(cx);
        if (!GetFrameEnvironmentAndScope(cx, frame, i.pc(), &env, &scope)) return false;
        ...
        for (EnvironmentIter ei(cx, env, scope, frame); ei.withinInitialFrame(); ei++) {
            if (ei.hasSyntacticEnvironment() && !ei.scope().is<GlobalScope>()) {
                ...
                DebugEnvironments* envs = ensureCompartmentData(cx);
                if (!envs) return false;
                if (!envs->liveEnvs.put(&ei.environment(), LiveEnvironmentVal(ei))) return false;
            }
        }

        if (frame.prevUpToDate()) return true;
...
LiveEnvironmentVal* DebugEnvironments::hasLiveEnvironment(EnvironmentObject& env) {
    DebugEnvironments* envs = env.compartment()->debugEnvs;
    if (!envs) return nullptr;

    if (LiveEnvironmentMap::Ptr p = envs->liveEnvs.lookup(&env)) return &p->value();

    return nullptr;
}

/* static */ void DebugEnvironments::unsetPrevUpToDateUntil(JSContext* cx,
...
/* static */ void DebugEnvironments::forwardLiveFrame(JSContext* cx, AbstractFramePtr from,
                                                      AbstractFramePtr to) {
    DebugEnvironments* envs = cx->compartment()->debugEnvs;
    if (!envs) return;
    ...
    for (LiveEnvironmentMap::Enum e(envs->liveEnvs); !e.empty(); e.popFront()) {
        ...
        if (val.frame() == from) val.updateFrame(to);
    }
}

/* static */ void DebugEnvironments::traceLiveFrame(JSTracer* trc, AbstractFramePtr frame) {
    for (MissingEnvironmentMap::Enum e(missingEnvs); !e.empty(); e.popFront()) {
...
static DebugEnvironmentProxy* GetDebugEnvironmentForEnvironmentObject(JSContext* cx,
    ...
    if (!enclosingDebug) return nullptr;

    Rooted<DebugEnvironmentProxy*> debugEnv(
        cx, DebugEnvironmentProxy::create(cx, *env, enclosingDebug));
    if (!debugEnv) return nullptr;

    if (!DebugEnvironments::addDebugEnvironment(cx, env, debugEnv)) return nullptr;

    return debugEnv;
}

static DebugEnvironmentProxy* GetDebugEnvironmentForMissing(JSContext* cx,
                                                            const EnvironmentIter& ei) {
    MOZ_ASSERT(!ei.hasSyntacticEnvironment() &&
               (ei.scope().is<FunctionScope>() || ei.scope().is<LexicalScope>() ||
                ei.scope().is<WasmInstanceScope>() || ei.scope().is<WasmFunctionScope>() ||
    ...
    if (ei.scope().is<FunctionScope>()) {
        ...
        Rooted<WasmInstanceEnvironmentObject*> env(
            cx, WasmInstanceEnvironmentObject::createHollowForDebug(cx, wasmInstanceScope));
        if (!env) return nullptr;
        ...
        Rooted<WasmFunctionCallObject*> callobj(
            cx, WasmFunctionCallObject::createHollowForDebug(cx, enclosing, wasmFunctionScope));
        if (!callobj) return nullptr;
    ...
    if (!DebugEnvironments::addDebugEnvironment(cx, ei, debugEnv)) return nullptr;

    return debugEnv;
}

static JSObject* GetDebugEnvironmentForNonEnvironmentObject(const EnvironmentIter& ei) {
    JSObject& enclosing = ei.enclosingEnvironment();
#ifdef DEBUG
    JSObject* o = &enclosing;
    while ((o = o->enclosingEnvironment())) MOZ_ASSERT(!o->is<EnvironmentObject>());
#endif
    return &enclosing;
}
...
static JSObject* GetDebugEnvironment(JSContext* cx, const EnvironmentIter& ei) {
    ...
    if (ei.done()) return GetDebugEnvironmentForNonEnvironmentObject(ei);

    if (ei.hasAnyEnvironmentObject()) return GetDebugEnvironmentForEnvironmentObject(cx, ei);
...
JSObject* js::GetDebugEnvironmentForFunction(JSContext* cx, HandleFunction fun) {
    assertSameCompartment(cx, fun);
    MOZ_ASSERT(CanUseDebugEnvironmentMaps(cx));
    if (!DebugEnvironments::updateLiveEnvironments(cx)) return nullptr;
...
JSObject* js::GetDebugEnvironmentForFrame(JSContext* cx, AbstractFramePtr frame, jsbytecode* pc) {
    assertSameCompartment(cx, frame);
    ...
    RootedObject env(cx);
    RootedScope scope(cx);
    if (!GetFrameEnvironmentAndScope(cx, frame, pc, &env, &scope)) return nullptr;
...
bool js::CreateObjectsForEnvironmentChain(JSContext* cx, AutoObjectVector& chain,
                                          HandleObject terminatingEnv,
                                          MutableHandleObject envObj) {
#ifdef DEBUG
    for (size_t i = 0; i < chain.length(); ++i) {
        assertSameCompartment(cx, chain[i]);
        MOZ_ASSERT(!chain[i]->is<GlobalObject>() && !chain[i]->is<NonSyntacticVariablesObject>());
    }
#endif
...
JSObject& WithEnvironmentObject::object() const { return getReservedSlot(OBJECT_SLOT).toObject(); }
...
bool js::GetThisValueForDebuggerMaybeOptimizedOut(JSContext* cx, AbstractFramePtr frame,
                                                  jsbytecode* pc, MutableHandleValue res) {
    RootedObject scopeChain(cx);
    RootedScope scope(cx);
    if (!GetFrameEnvironmentAndScope(cx, frame, pc, &scopeChain, &scope)) return false;
    ...
    for (EnvironmentIter ei(cx, scopeChain, scope, frame); ei; ei++) {
        ...
        if (ei.withinInitialFrame() && !executedInitThisOp) {
            ...
            if (!GetFunctionThis(cx, frame, res)) return false;
        ...
        for (Rooted<BindingIter> bi(cx, BindingIter(script)); bi; bi++) {
            if (bi.name() != cx->names().dotThis) continue;
...
bool js::CheckLexicalNameConflict(JSContext* cx, Handle<LexicalEnvironmentObject*> lexicalEnv,
    ...
    if (varObj->is<GlobalObject>() && varObj->compartment()->isInVarNames(name)) {
        ...
        if (!shape->configurable()) redeclKind = "non-configurable global property";
    } else {
        // ES 15.1.11 step 5.c-d
        Rooted<PropertyDescriptor> desc(cx);
        if (!GetOwnPropertyDescriptor(cx, varObj, id, &desc)) return false;
...
bool js::CheckVarNameConflict(JSContext* cx, Handle<LexicalEnvironmentObject*> lexicalEnv,
...
static void ReportCannotDeclareGlobalBinding(JSContext* cx, HandlePropertyName name,
                                             const char* reason) {
    JSAutoByteString printable;
    if (AtomToPrintableString(cx, name, &printable)) {
        JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, JSMSG_CANT_DECLARE_GLOBAL_BINDING,
                                   printable.ptr(), reason);
    }
}

bool js::CheckCanDeclareGlobalBinding(JSContext* cx, Handle<GlobalObject*> global,
                                      HandlePropertyName name, bool isFunction) {
    RootedId id(cx, NameToId(name));
    Rooted<PropertyDescriptor> desc(cx);
    if (!GetOwnPropertyDescriptor(cx, global, id, &desc)) return false;
    ...
    if (!desc.object()) {
        // 8.1.14.15 step 6.
        // 8.1.14.16 step 5.
        if (global->nonProxyIsExtensible()) return true;
    ...
    if (isFunction) {
        ...
        // 8.1.14.16 step 7.
        if (desc.isDataDescriptor() && desc.writable() && desc.enumerable()) return true;
...
bool js::CheckGlobalDeclarationConflicts(JSContext* cx, HandleScript script,
    ...
    for (; bi; bi++) {
        if (bi.kind() != BindingKind::Var) break;
        name = bi.name()->asPropertyName();
        if (!CheckVarNameConflict(cx, lexicalEnv, name)) return false;
    ...
    for (; bi; bi++) {
        name = bi.name()->asPropertyName();
        if (!CheckLexicalNameConflict(cx, lexicalEnv, varObj, name)) return false;
    }

    return true;
}

static bool CheckVarNameConflictsInEnv(JSContext* cx, HandleScript script, HandleObject obj) {
    ...
    for (BindingIter bi(script); bi; bi++) {
        name = bi.name()->asPropertyName();
        if (!CheckVarNameConflict(cx, env, name)) return false;
    }

    return true;
}

bool js::CheckEvalDeclarationConflicts(JSContext* cx, HandleScript script, HandleObject scopeChain,
                                       HandleObject varObj) {
    if (!script->bodyScope()->as<EvalScope>().hasBindings()) return true;
    ...
    while (obj != varObj) {
        if (!CheckVarNameConflictsInEnv(cx, script, obj)) return false;
...
bool js::PushVarEnvironmentObject(JSContext* cx, HandleScope scope, AbstractFramePtr frame) {
...
bool js::GetFrameEnvironmentAndScope(JSContext* cx, AbstractFramePtr frame, jsbytecode* pc,
...
static bool RemoveReferencedNames(JSContext* cx, HandleScript script,
    ...
    for (jsbytecode* pc = script->code(); pc != script->codeEnd(); pc += GetBytecodeLength(pc)) {
        ...
        if (name) remainingNames.remove(name);
    }
    ...
    if (script->hasObjects()) {
        ...
        for (size_t i = 0; i < objects->length; i++) {
            ...
            if (obj->is<JSFunction>() && obj->as<JSFunction>().isInterpreted()) {
                ...
                if (!innerScript) return false;

                if (!RemoveReferencedNames(cx, innerScript, remainingNames)) return false;
            }
        }
    }
...
static bool AnalyzeEntrainedVariablesInScript(JSContext* cx, HandleScript script,
                                              HandleScript innerScript) {
    PropertyNameSet remainingNames(cx);
    if (!remainingNames.init()) return false;

    for (BindingIter bi(script); bi; bi++) {
        if (bi.closedOver()) {
            PropertyName* name = bi.name()->asPropertyName();
            PropertyNameSet::AddPtr p = remainingNames.lookupForAdd(name);
            if (!p && !remainingNames.add(p, name)) return false;
        }
    }
    ...
    for (size_t i = 0; i < objects->length; i++) {
        ...
        if (obj->is<JSFunction>() && obj->as<JSFunction>().isInterpreted()) {
            ...
            if (script->functionDelazifying() &&
                ...
                if (!AnalyzeEntrainedVariablesInScript(cx, script, innerScript)) return false;
            }

            if (!AnalyzeEntrainedVariables(cx, innerScript)) return false;
        }
    }
