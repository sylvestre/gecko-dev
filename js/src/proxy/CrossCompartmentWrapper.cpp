...
bool CrossCompartmentWrapper::getPropertyDescriptor(JSContext* cx, HandleObject wrapper,
                                                    HandleId id,
                                                    MutableHandle<PropertyDescriptor> desc) const {
    PIERCE(cx, wrapper, MarkAtoms(cx, id), Wrapper::getPropertyDescriptor(cx, wrapper, id, desc),
...
bool CrossCompartmentWrapper::defineProperty(JSContext* cx, HandleObject wrapper, HandleId id,
...
bool CrossCompartmentWrapper::ownPropertyKeys(JSContext* cx, HandleObject wrapper,
                                              AutoIdVector& props) const {
    PIERCE(cx, wrapper, NOTHING, Wrapper::ownPropertyKeys(cx, wrapper, props),
           MarkAtoms(cx, props));
}

bool CrossCompartmentWrapper::delete_(JSContext* cx, HandleObject wrapper, HandleId id,
                                      ObjectOpResult& result) const {
    PIERCE(cx, wrapper, MarkAtoms(cx, id), Wrapper::delete_(cx, wrapper, id, result), NOTHING);
}
...
bool CrossCompartmentWrapper::getPrototype(JSContext* cx, HandleObject wrapper,
                                           MutableHandleObject protop) const {
    {
        ...
        if (!GetPrototype(cx, wrapped, protop)) return false;
        if (protop) {
            if (!JSObject::setDelegate(cx, protop)) return false;
        }
    }
...
bool CrossCompartmentWrapper::setPrototype(JSContext* cx, HandleObject wrapper, HandleObject proto,
                                           ObjectOpResult& result) const {
...
bool CrossCompartmentWrapper::getPrototypeIfOrdinary(JSContext* cx, HandleObject wrapper,
                                                     bool* isOrdinary,
                                                     MutableHandleObject protop) const {
    {
        RootedObject wrapped(cx, wrappedObject(wrapper));
        AutoCompartment call(cx, wrapped);
        if (!GetPrototypeIfOrdinary(cx, wrapped, isOrdinary, protop)) return false;
...
bool CrossCompartmentWrapper::setImmutablePrototype(JSContext* cx, HandleObject wrapper,
                                                    bool* succeeded) const {
    PIERCE(cx, wrapper, NOTHING, Wrapper::setImmutablePrototype(cx, wrapper, succeeded), NOTHING);
}

bool CrossCompartmentWrapper::preventExtensions(JSContext* cx, HandleObject wrapper,
                                                ObjectOpResult& result) const {
    PIERCE(cx, wrapper, NOTHING, Wrapper::preventExtensions(cx, wrapper, result), NOTHING);
}

bool CrossCompartmentWrapper::isExtensible(JSContext* cx, HandleObject wrapper,
                                           bool* extensible) const {
    PIERCE(cx, wrapper, NOTHING, Wrapper::isExtensible(cx, wrapper, extensible), NOTHING);
}

bool CrossCompartmentWrapper::has(JSContext* cx, HandleObject wrapper, HandleId id,
                                  bool* bp) const {
    PIERCE(cx, wrapper, MarkAtoms(cx, id), Wrapper::has(cx, wrapper, id, bp), NOTHING);
}

bool CrossCompartmentWrapper::hasOwn(JSContext* cx, HandleObject wrapper, HandleId id,
                                     bool* bp) const {
    PIERCE(cx, wrapper, MarkAtoms(cx, id), Wrapper::hasOwn(cx, wrapper, id, bp), NOTHING);
}

static bool WrapReceiver(JSContext* cx, HandleObject wrapper, MutableHandleValue receiver) {
...
bool CrossCompartmentWrapper::get(JSContext* cx, HandleObject wrapper, HandleValue receiver,
                                  HandleId id, MutableHandleValue vp) const {
    RootedValue receiverCopy(cx, receiver);
    {
        AutoCompartment call(cx, wrappedObject(wrapper));
        if (!MarkAtoms(cx, id) || !WrapReceiver(cx, wrapper, &receiverCopy)) return false;

        if (!Wrapper::get(cx, wrapper, receiverCopy, id, vp)) return false;
    }
    return cx->compartment()->wrap(cx, vp);
}

bool CrossCompartmentWrapper::set(JSContext* cx, HandleObject wrapper, HandleId id, HandleValue v,
...
bool CrossCompartmentWrapper::getOwnEnumerablePropertyKeys(JSContext* cx, HandleObject wrapper,
                                                           AutoIdVector& props) const {
    PIERCE(cx, wrapper, NOTHING, Wrapper::getOwnEnumerablePropertyKeys(cx, wrapper, props),
           MarkAtoms(cx, props));
}
...
struct AutoCloseIterator {
    ...
    ~AutoCloseIterator() {
        if (obj) MOZ_ALWAYS_TRUE(CloseIterator(cx, obj));
    }
...
static JSObject* Reify(JSContext* cx, JSCompartment* origin, HandleObject objp) {
    ...
    RootedObject obj(cx, ni->obj);
    {
        ...
        /* Wrap the iteratee. */
        if (!origin->wrap(cx, &obj)) return nullptr;
        ...
        if (length > 0) {
            if (!keys.reserve(length)) return nullptr;
            for (size_t i = 0; i < length; ++i) {
                RootedId id(cx);
                RootedValue v(cx, StringValue(ni->begin()[i]));
                if (!ValueToId<CanGC>(cx, v, &id)) return nullptr;
...
JSObject* CrossCompartmentWrapper::enumerate(JSContext* cx, HandleObject wrapper) const {
    RootedObject res(cx);
    {
        AutoCompartment call(cx, wrappedObject(wrapper));
    ...
    if (CanReify(res)) return Reify(cx, cx->compartment(), res);
    if (!cx->compartment()->wrap(cx, &res)) return nullptr;
    return res;
}
...
bool CrossCompartmentWrapper::call(JSContext* cx, HandleObject wrapper,
...

    {
        ...
        if (!cx->compartment()->wrap(cx, args.mutableThisv())) return false;

        for (size_t n = 0; n < args.length(); ++n) {
            if (!cx->compartment()->wrap(cx, args[n])) return false;
        }

        if (!Wrapper::call(cx, wrapper, args)) return false;
    }
...
bool CrossCompartmentWrapper::construct(JSContext* cx, HandleObject wrapper,
    ...
    RootedObject wrapped(cx, wrappedObject(wrapper));
    {
        ...
        if (!cx->compartment()->wrap(cx, args.newTarget())) return false;
        if (!Wrapper::construct(cx, wrapper, args)) return false;
    }
    return cx->compartment()->wrap(cx, args.rval());
}

bool CrossCompartmentWrapper::nativeCall(JSContext* cx, IsAcceptableThis test, NativeImpl impl,
    ...
    RootedObject wrapped(cx, wrappedObject(wrapper));
    {
        ...
        if (!dstArgs.init(cx, srcArgs.length())) return false;
        ...
        if (!CallNonGenericMethod(cx, test, impl, dstArgs)) return false;
...
const char* CrossCompartmentWrapper::className(JSContext* cx, HandleObject wrapper) const {
    AutoCompartment call(cx, wrappedObject(wrapper));
    return Wrapper::className(cx, wrapper);
}

JSString* CrossCompartmentWrapper::fun_toString(JSContext* cx, HandleObject wrapper,
    ...
    if (!cx->compartment()->wrap(cx, &str)) return nullptr;
    return str;
}

RegExpShared* CrossCompartmentWrapper::regexp_toShared(JSContext* cx, HandleObject wrapper) const {
...
bool CrossCompartmentWrapper::boxedValue_unbox(JSContext* cx, HandleObject wrapper,
...
bool js::IsCrossCompartmentWrapper(JSObject* obj) {
    return IsWrapper(obj) &&
           !!(Wrapper::wrapperHandler(obj)->flags() & Wrapper::CROSS_COMPARTMENT);
}

static void NukeRemovedCrossCompartmentWrapper(JSContext* cx, JSObject* wrapper) {
...
js::NukeCrossCompartmentWrappers(JSContext* cx, const CompartmentFilter& sourceFilter,
    ...
    for (CompartmentsIter c(rt, SkipAtoms); !c.done(); c.next()) {
        ...
        for (; !e->empty(); e->popFront()) {
            ...
            if (nukeReferencesToWindow == DontNukeWindowReferences && MOZ_LIKELY(!nukeAll) &&
...
// inconsistent state.
void js::RemapWrapper(JSContext* cx, JSObject* wobjArg, JSObject* newTargetArg) {
    ...
    MOZ_ASSERT(!JS_IsDeadWrapper(origTarget), "We don't want a dead proxy in the wrapper map");
    ...
    MOZ_ASSERT_IF(origTarget != newTarget, !wcompartment->lookupWrapper(ObjectValue(*newTarget)));
    ...
    if (!wcompartment->rewrap(cx, &tobj, wobj)) MOZ_CRASH();
    ...
    if (tobj != wobj) {
        ...
        // new one.
        if (!JSObject::swap(cx, wobj, tobj)) MOZ_CRASH();
    }
...
JS_FRIEND_API(bool)
js::RemapAllWrappersForObject(JSContext* cx, JSObject* oldTargetArg, JSObject* newTargetArg) {
    ...
    if (!toTransplant.reserve(cx->runtime()->numCompartments)) return false;
    ...
    for (const WrapperValue& v : toTransplant) RemapWrapper(cx, &v.toObject(), newTarget);
...
js::RecomputeWrappers(JSContext* cx, const CompartmentFilter& sourceFilter,
    ...
    for (CompartmentsIter c(cx->runtime(), SkipAtoms); !c.done(); c.next()) {
        ...
        for (JSCompartment::NonStringWrapperEnum e(c, targetFilter); !e.empty(); e.popFront()) {
            ...
            // Add it to the list.
            if (!toRecompute.append(WrapperValue(e))) return false;
        }
    }
