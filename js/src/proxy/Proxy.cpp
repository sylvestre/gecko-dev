...
void js::AutoEnterPolicy::reportErrorIfExceptionIsNotPending(JSContext* cx, jsid id) {
    ...
    if (JSID_IS_VOID(id)) {
        ...
        if (str->ensureFlat(cx) && chars.initTwoByte(cx, str)) prop = chars.twoByteChars();

        JS_ReportErrorNumberUC(cx, GetErrorMessage, nullptr, JSMSG_PROPERTY_ACCESS_DENIED, prop);
    }
}

#ifdef DEBUG
void js::AutoEnterPolicy::recordEnter(JSContext* cx, HandleObject proxy, HandleId id, Action act) {
    if (allowed()) {
        context = cx;
...
JS_FRIEND_API(void)
js::assertEnteredPolicy(JSContext* cx, JSObject* proxy, jsid id, BaseProxyHandler::Action act) {
...
bool Proxy::getPropertyDescriptor(JSContext* cx, HandleObject proxy, HandleId id,
    ...
    AutoEnterPolicy policy(cx, handler, proxy, id, BaseProxyHandler::GET_PROPERTY_DESCRIPTOR,
                           true);
...
bool Proxy::getOwnPropertyDescriptor(JSContext* cx, HandleObject proxy, HandleId id,
                                     MutableHandle<PropertyDescriptor> desc) {
    if (!CheckRecursionLimit(cx)) return false;
    const BaseProxyHandler* handler = proxy->as<ProxyObject>().handler();
    desc.object().set(nullptr);  // default result if we refuse to perform this action
...
bool Proxy::ownPropertyKeys(JSContext* cx, HandleObject proxy, AutoIdVector& props) {
    if (!CheckRecursionLimit(cx)) return false;
    const BaseProxyHandler* handler = proxy->as<ProxyObject>().handler();
...
bool Proxy::delete_(JSContext* cx, HandleObject proxy, HandleId id, ObjectOpResult& result) {
    if (!CheckRecursionLimit(cx)) return false;
    const BaseProxyHandler* handler = proxy->as<ProxyObject>().handler();
    AutoEnterPolicy policy(cx, handler, proxy, id, BaseProxyHandler::SET, true);
    if (!policy.allowed()) {
...
/* static */ bool Proxy::setPrototype(JSContext* cx, HandleObject proxy, HandleObject proto,
...
/* static */ bool Proxy::getPrototypeIfOrdinary(JSContext* cx, HandleObject proxy,
                                                bool* isOrdinary, MutableHandleObject proto) {
    if (!CheckRecursionLimit(cx)) return false;
    return proxy->as<ProxyObject>().handler()->getPrototypeIfOrdinary(cx, proxy, isOrdinary,
                                                                      proto);
}

/* static */ bool Proxy::setImmutablePrototype(JSContext* cx, HandleObject proxy,
                                               bool* succeeded) {
    if (!CheckRecursionLimit(cx)) return false;
    const BaseProxyHandler* handler = proxy->as<ProxyObject>().handler();
...
/* static */ bool Proxy::isExtensible(JSContext* cx, HandleObject proxy, bool* extensible) {
    if (!CheckRecursionLimit(cx)) return false;
...
static MOZ_ALWAYS_INLINE Value ValueToWindowProxyIfWindow(const Value& v, JSObject* proxy) {
    if (v.isObject() && v != ObjectValue(*proxy))
        return ObjectValue(*ToWindowProxyIfWindow(&v.toObject()));
    return v;
}

MOZ_ALWAYS_INLINE bool Proxy::getInternal(JSContext* cx, HandleObject proxy, HandleValue receiver,
...
bool Proxy::get(JSContext* cx, HandleObject proxy, HandleValue receiver_, HandleId id,
...
bool js::ProxyGetProperty(JSContext* cx, HandleObject proxy, HandleId id, MutableHandleValue vp) {
    RootedValue receiver(cx, ObjectValue(*proxy));
    return Proxy::getInternal(cx, proxy, receiver, id, vp);
}

bool js::ProxyGetPropertyByValue(JSContext* cx, HandleObject proxy, HandleValue idVal,
                                 MutableHandleValue vp) {
    RootedId id(cx);
    if (!ValueToId<CanGC>(cx, idVal, &id)) return false;
...
MOZ_ALWAYS_INLINE bool Proxy::setInternal(JSContext* cx, HandleObject proxy, HandleId id,
...
bool js::ProxySetProperty(JSContext* cx, HandleObject proxy, HandleId id, HandleValue val,
                          bool strict) {
    ObjectOpResult result;
    RootedValue receiver(cx, ObjectValue(*proxy));
...
bool js::ProxySetPropertyByValue(JSContext* cx, HandleObject proxy, HandleValue idVal,
...
bool Proxy::getOwnEnumerablePropertyKeys(JSContext* cx, HandleObject proxy, AutoIdVector& props) {
    if (!CheckRecursionLimit(cx)) return false;
    const BaseProxyHandler* handler = proxy->as<ProxyObject>().handler();
    AutoEnterPolicy policy(cx, handler, proxy, JSID_VOIDHANDLE, BaseProxyHandler::ENUMERATE, true);
    if (!policy.allowed()) return policy.returnValue();
...
JSObject* Proxy::enumerate(JSContext* cx, HandleObject proxy) {
    ...
    if (handler->hasPrototype()) {
        ...
        if (!proto) return EnumeratedIdVectorToIterator(cx, proxy, 0, props);
...
bool Proxy::call(JSContext* cx, HandleObject proxy, const CallArgs& args) {
    ...
    // trap.
    AutoEnterPolicy policy(cx, handler, proxy, JSID_VOIDHANDLE, BaseProxyHandler::CALL, true);
    if (!policy.allowed()) {
...
bool Proxy::hasInstance(JSContext* cx, HandleObject proxy, MutableHandleValue v, bool* bp) {
    if (!CheckRecursionLimit(cx)) return false;
    const BaseProxyHandler* handler = proxy->as<ProxyObject>().handler();
    *bp = false;  // default result if we refuse to perform this action
...
bool Proxy::isArray(JSContext* cx, HandleObject proxy, JS::IsArrayAnswer* answer) {
    if (!CheckRecursionLimit(cx)) return false;
...
const char* Proxy::className(JSContext* cx, HandleObject proxy) {
    ...
    int stackDummy;
    if (!JS_CHECK_STACK_SIZE(GetNativeStackLimit(cx), &stackDummy)) return "too much recursion";
...
JSString* Proxy::fun_toString(JSContext* cx, HandleObject proxy, bool isToSource) {
    ...
    AutoEnterPolicy policy(cx, handler, proxy, JSID_VOIDHANDLE, BaseProxyHandler::GET,
                           /* mayThrow = */ false);
    // Do the safe thing if the policy rejects.
    if (!policy.allowed()) return handler->BaseProxyHandler::fun_toString(cx, proxy, isToSource);
...
bool Proxy::boxedValue_unbox(JSContext* cx, HandleObject proxy, MutableHandleValue vp) {
    if (!CheckRecursionLimit(cx)) return false;
...
/* static */ bool Proxy::watch(JSContext* cx, JS::HandleObject proxy, JS::HandleId id,
                               JS::HandleObject callable) {
    if (!CheckRecursionLimit(cx)) return false;
    return proxy->as<ProxyObject>().handler()->watch(cx, proxy, id, callable);
}

/* static */ bool Proxy::unwatch(JSContext* cx, JS::HandleObject proxy, JS::HandleId id) {
    if (!CheckRecursionLimit(cx)) return false;
    return proxy->as<ProxyObject>().handler()->unwatch(cx, proxy, id);
}

/* static */ bool Proxy::getElements(JSContext* cx, HandleObject proxy, uint32_t begin,
...
/* static */ void ProxyObject::traceEdgeToTarget(JSTracer* trc, ProxyObject* obj) {
...
const ClassExtension js::ProxyClassExtension = {proxy_WeakmapKeyDelegate, proxy_ObjectMoved};

const ObjectOps js::ProxyObjectOps = {proxy_LookupProperty, Proxy::defineProperty,
                                      Proxy::has,           Proxy::get,
                                      Proxy::set,           Proxy::getOwnPropertyDescriptor,
                                      proxy_DeleteProperty, Proxy::watch,
                                      Proxy::unwatch,       Proxy::getElements,
                                      Proxy::fun_toString};

const Class js::ProxyObject::proxyClass = PROXY_CLASS_DEF(
    "Proxy", JSCLASS_HAS_CACHED_PROTO(JSProto_Proxy) | JSCLASS_HAS_RESERVED_SLOTS(2));

const Class* const js::ProxyClassPtr = &js::ProxyObject::proxyClass;

JS_FRIEND_API(JSObject*)
js::NewProxyObject(JSContext* cx, const BaseProxyHandler* handler, HandleValue priv,
...
void ProxyObject::renew(const BaseProxyHandler* handler, const Value& priv) {
    ...
    for (size_t i = 0; i < numReservedSlots(); i++) setReservedSlot(i, UndefinedValue());
}

JS_FRIEND_API(JSObject*)
js::InitProxyClass(JSContext* cx, HandleObject obj) {
    static const JSFunctionSpec static_methods[] = {JS_FN("revocable", proxy_revocable, 2, 0),
                                                    JS_FS_END};
    ...
    if (!JS_DefineProperty(cx, obj, "Proxy", ctor, JSPROP_RESOLVING)) return nullptr;
