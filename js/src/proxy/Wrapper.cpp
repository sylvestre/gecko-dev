...
bool ForwardingProxyHandler::defineProperty(JSContext* cx, HandleObject proxy, HandleId id,
...
bool ForwardingProxyHandler::delete_(JSContext* cx, HandleObject proxy, HandleId id,
                                     ObjectOpResult& result) const {
    assertEnteredPolicy(cx, proxy, id, SET);
    RootedObject target(cx, proxy->as<ProxyObject>().target());
...
JSObject* ForwardingProxyHandler::enumerate(JSContext* cx, HandleObject proxy) const {
    assertEnteredPolicy(cx, proxy, JSID_VOID, ENUMERATE);
...
bool ForwardingProxyHandler::setPrototype(JSContext* cx, HandleObject proxy, HandleObject proto,
                                          ObjectOpResult& result) const {
    RootedObject target(cx, proxy->as<ProxyObject>().target());
    return SetPrototype(cx, target, proto, result);
}

bool ForwardingProxyHandler::getPrototypeIfOrdinary(JSContext* cx, HandleObject proxy,
                                                    bool* isOrdinary,
                                                    MutableHandleObject protop) const {
    RootedObject target(cx, proxy->as<ProxyObject>().target());
...
bool ForwardingProxyHandler::setImmutablePrototype(JSContext* cx, HandleObject proxy,
                                                   bool* succeeded) const {
    RootedObject target(cx, proxy->as<ProxyObject>().target());
    return SetImmutablePrototype(cx, target, succeeded);
}

bool ForwardingProxyHandler::preventExtensions(JSContext* cx, HandleObject proxy,
                                               ObjectOpResult& result) const {
    RootedObject target(cx, proxy->as<ProxyObject>().target());
...
bool ForwardingProxyHandler::has(JSContext* cx, HandleObject proxy, HandleId id, bool* bp) const {
...
bool ForwardingProxyHandler::get(JSContext* cx, HandleObject proxy, HandleValue receiver,
...
bool ForwardingProxyHandler::set(JSContext* cx, HandleObject proxy, HandleId id, HandleValue v,
...
bool ForwardingProxyHandler::call(JSContext* cx, HandleObject proxy, const CallArgs& args) const {
...
bool ForwardingProxyHandler::construct(JSContext* cx, HandleObject proxy,
    ...
    RootedObject obj(cx);
    if (!Construct(cx, target, cargs, args.newTarget(), &obj)) return false;

    args.rval().setObject(*obj);
    return true;
}

bool ForwardingProxyHandler::getPropertyDescriptor(JSContext* cx, HandleObject proxy, HandleId id,
...
bool ForwardingProxyHandler::hasOwn(JSContext* cx, HandleObject proxy, HandleId id,
                                    bool* bp) const {
    assertEnteredPolicy(cx, proxy, id, GET);
    RootedObject target(cx, proxy->as<ProxyObject>().target());
...
bool ForwardingProxyHandler::getOwnEnumerablePropertyKeys(JSContext* cx, HandleObject proxy,
                                                          AutoIdVector& props) const {
    assertEnteredPolicy(cx, proxy, JSID_VOID, ENUMERATE);
    RootedObject target(cx, proxy->as<ProxyObject>().target());
...
bool ForwardingProxyHandler::nativeCall(JSContext* cx, IsAcceptableThis test, NativeImpl impl,
                                        const CallArgs& args) const {
...
bool ForwardingProxyHandler::hasInstance(JSContext* cx, HandleObject proxy, MutableHandleValue v,
                                         bool* bp) const {
...
const char* ForwardingProxyHandler::className(JSContext* cx, HandleObject proxy) const {
    assertEnteredPolicy(cx, proxy, JSID_VOID, GET);
    RootedObject target(cx, proxy->as<ProxyObject>().target());
    return GetObjectClassName(cx, target);
}

JSString* ForwardingProxyHandler::fun_toString(JSContext* cx, HandleObject proxy,
...
RegExpShared* ForwardingProxyHandler::regexp_toShared(JSContext* cx, HandleObject proxy) const {
    RootedObject target(cx, proxy->as<ProxyObject>().target());
...
JSObject* Wrapper::Renew(JSObject* existing, JSObject* obj, const Wrapper* handler) {
    existing->as<ProxyObject>().renew(handler, ObjectValue(*obj));
    return existing;
}

const Wrapper* Wrapper::wrapperHandler(JSObject* wrapper) {
...
JSObject* Wrapper::wrappedObject(JSObject* wrapper) {
    ...
    if (target) {
        if (wrapper->isMarkedBlack()) MOZ_ASSERT(JS::ObjectIsNotGray(target));
        if (!wrapper->isMarkedGray()) JS::ExposeObjectToActiveJS(target);
    }
...
js::UncheckedUnwrapWithoutExpose(JSObject* wrapped) {
    while (true) {
        if (!wrapped->is<WrapperObject>() || MOZ_UNLIKELY(IsWindowProxy(wrapped))) break;
        ...
        if (wrapped) wrapped = MaybeForwarded(wrapped);
    }
    return wrapped;
}
...
js::UnwrapOneChecked(JSObject* obj, bool stopAtWindowProxy) {
    ...
    if (!obj->is<WrapperObject>() || MOZ_UNLIKELY(stopAtWindowProxy && IsWindowProxy(obj))) {
        return obj;
    }
...
JSObject* js::TransparentObjectWrapper(JSContext* cx, HandleObject existing, HandleObject obj) {
...
ErrorCopier::~ErrorCopier() {
    ...
    if (ac->origin() != cx->compartment() && cx->isExceptionPending() &&
        ...
        if (cx->getPendingException(&exc) && exc.isObject() && exc.toObject().is<ErrorObject>()) {
            ...
            if (copyobj) cx->setPendingException(ObjectValue(*copyobj));
        }
    }
}
