...
JS_NewObjectWithUniqueType(JSContext* cx, const JSClass* clasp, HandleObject proto) {
     ...
     */
    RootedObject obj(
        cx, NewObjectWithGivenProto(cx, (const js::Class*)clasp, nullptr, SingletonObject));
...
JS_GetCompartmentPrincipals(JSCompartment* compartment) { return compartment->principals(); }
...
JS_FRIEND_API(JSPrincipals*)
JS_GetScriptPrincipals(JSScript* script) { return script->principals(); }

JS_FRIEND_API(bool)
JS_ScriptHasMutedErrors(JSScript* script) { return script->mutedErrors(); }
...
static bool DefineHelpProperty(JSContext* cx, HandleObject obj, const char* prop,
                               const char* value) {
...
JS_DefineFunctionsWithHelp(JSContext* cx, HandleObject obj, const JSFunctionSpecWithHelp* fs) {
    ...
    for (; fs->name; fs++) {
        ...
        RootedFunction fun(
            cx, DefineFunction(cx, obj, id, fs->call, fs->nargs, fs->flags | JSPROP_RESOLVING));
        if (!fun) return false;
...
js::GetBuiltinClass(JSContext* cx, HandleObject obj, ESClass* cls) {
    if (MOZ_UNLIKELY(obj->is<ProxyObject>())) return Proxy::getBuiltinClass(cx, obj, cls);
...
js::GetCompartmentZone(JSCompartment* comp) { return comp->zone(); }

JS_FRIEND_API(bool)
js::IsSystemCompartment(JSCompartment* comp) { return comp->isSystem(); }

JS_FRIEND_API(bool)
js::IsSystemZone(Zone* zone) { return zone->isSystem; }
...
JS_FRIEND_API(bool)
js::IsAtomsZone(JS::Zone* zone) { return zone->runtimeFromAnyThread()->isAtomsZone(zone); }

JS_FRIEND_API(bool)
js::IsFunctionObject(JSObject* obj) { return obj->is<JSFunction>(); }

JS_FRIEND_API(JSObject*)
js::GetGlobalForObjectCrossCompartment(JSObject* obj) { return &obj->global(); }
...
JS_FRIEND_API(void)
js::AssertSameCompartment(JSContext* cx, JSObject* obj) { assertSameCompartment(cx, obj); }

JS_FRIEND_API(void)
js::AssertSameCompartment(JSContext* cx, JS::HandleValue v) { assertSameCompartment(cx, v); }
...
js::GetObjectSlotSpan(JSObject* obj) { return obj->as<NativeObject>().slotSpan(); }
...
JS_FRIEND_API(bool)
js::RunningWithTrustedPrincipals(JSContext* cx) { return cx->runningWithTrustedPrincipals(); }
...
void js::SetPreserveWrapperCallback(JSContext* cx, PreserveWrapperCallback callback) {
...
JS_SetProtoCalled(JSContext*) { return sSetProtoCalled; }
...
JS_FRIEND_API(bool)
JS_IsDeadWrapper(JSObject* obj) { return IsDeadProxyObject(obj); }

JS_FRIEND_API(JSObject*)
JS_NewDeadWrapper(JSContext* cx, JSObject* origObj) { return NewDeadProxyObject(cx, origObj); }

JS_FRIEND_API(bool)
JS_IsScriptSourceObject(JSObject* obj) { return obj->is<ScriptSourceObject>(); }
...
js::GetWeakmapKeyDelegate(JSObject* key) {
    if (JSWeakmapKeyDelegateOp op = key->getClass()->extWeakmapKeyDelegateOp()) return op(key);
    return nullptr;
}

JS_FRIEND_API(JSLinearString*)
js::StringToLinearStringSlow(JSContext* cx, JSString* str) { return str->ensureLinear(cx); }
...
extern JS_FRIEND_API(void) DumpChars(const char16_t* s, size_t n, js::GenericPrinter& out);

extern JS_FRIEND_API(void) DumpValue(const JS::Value& val, js::GenericPrinter& out);

extern JS_FRIEND_API(void) DumpId(jsid id, js::GenericPrinter& out);

extern JS_FRIEND_API(void) DumpInterpreterFrame(JSContext* cx, js::GenericPrinter& out,
                                                InterpreterFrame* start = nullptr);

}  // namespace js

JS_FRIEND_API(void)
js::DumpString(JSString* str, js::GenericPrinter& out) { str->dump(out); }

JS_FRIEND_API(void)
js::DumpAtom(JSAtom* atom, js::GenericPrinter& out) { atom->dump(out); }
...
JS_FRIEND_API(void)
js::DumpString(JSString* str) { DumpString(str, stderr); }
JS_FRIEND_API(void)
js::DumpAtom(JSAtom* atom) { DumpAtom(atom, stderr); }
JS_FRIEND_API(void)
js::DumpObject(JSObject* obj) { DumpObject(obj, stderr); }
JS_FRIEND_API(void)
js::DumpChars(const char16_t* s, size_t n) { DumpChars(s, n, stderr); }
JS_FRIEND_API(void)
js::DumpValue(const JS::Value& val) { DumpValue(val, stderr); }
JS_FRIEND_API(void)
js::DumpId(jsid id) { DumpId(id, stderr); }
JS_FRIEND_API(void)
...
JS_FRIEND_API(bool)
js::DumpPC(JSContext* cx) { return DumpPC(cx, stdout); }
JS_FRIEND_API(bool)
js::DumpScript(JSContext* cx, JSScript* scriptArg) { return DumpScript(cx, scriptArg, stdout); }

#endif

static const char* FormatValue(JSContext* cx, const Value& vArg, JSAutoByteString& bytes) {
...
static JS::UniqueChars FormatFrame(JSContext* cx, const FrameIter& iter, JS::UniqueChars&& inBuf,
                                   int num, bool showArgs, bool showLocals, bool showThisProps) {
    ...
    if (iter.hasUsableAbstractFramePtr() && iter.isFunctionFrame() && fun && !fun->isArrow() &&
        !fun->isDerivedClassConstructor() && !(fun->isBoundFunction() && iter.isConstructing())) {
    ...
    if (showArgs && iter.hasArgs()) {
        ...
        for (unsigned i = 0; i < iter.numActualArgs(); i++) {
            ...
            if (value) {
                buf =
                    sprintf_append(cx, Move(buf), "%s%s%s%s%s%s", !first ? ", " : "",
                                   name ? name : "", name ? " = " : "", arg.isString() ? "\"" : "",
                                   value, arg.isString() ? "\"" : "");
                if (!buf) return nullptr;
                ...
                buf = sprintf_append(
                    cx, Move(buf), "    <Failed to get argument while inspecting stack frame>\n");
                if (!buf) return nullptr;
            }
        }
    }
        ...
        for (size_t i = 0; i < keys.length(); i++) {
            ...
            if (name && value) {
                buf = sprintf_append(cx, Move(buf), "    this.%s = %s%s%s\n", name,
                                     v.isString() ? "\"" : "", value, v.isString() ? "\"" : "");
            } else {
                buf = sprintf_append(
                    cx, Move(buf), "    <Failed to format values while inspecting stack frame>\n");
            }
            if (!buf) return nullptr;
        }
    }
...
static JS::UniqueChars FormatWasmFrame(JSContext* cx, const FrameIter& iter,
                                       JS::UniqueChars&& inBuf, int num, bool showArgs) {
    JSAtom* functionDisplayAtom = iter.functionDisplayAtom();
    UniqueChars nameStr;
    if (functionDisplayAtom) nameStr = StringToNewUTF8CharsZ(cx, *functionDisplayAtom);

    JS::UniqueChars buf = sprintf_append(cx, Move(inBuf), "%d %s()", num,
                                         nameStr ? nameStr.get() : "<wasm-function>");
    if (!buf) return nullptr;
    const char* filename = iter.filename();
    uint32_t lineno = iter.computeLine();
    buf =
        sprintf_append(cx, Move(buf), " [\"%s\":%d]\n", filename ? filename : "<unknown>", lineno);
...
extern JS_FRIEND_API(bool) JS::ForceLexicalInitialization(JSContext* cx, HandleObject obj) {
    AssertHeapIsIdle();
    CHECK_REQUEST(cx);
    assertSameCompartment(cx, obj);
...
struct DumpHeapTracer : public JS::CallbackTracer, public WeakMapTracer {
   ...
   private:
    void trace(JSObject* map, JS::GCCellPtr key, JS::GCCellPtr value) override {
        ...
        if (key.is<JSObject>()) kdelegate = js::GetWeakmapKeyDelegate(&key.as<JSObject>());

        fprintf(output, "WeakMapEntry map=%p key=%p keyDelegate=%p value=%p\n", map, key.asCell(),
                kdelegate, value.asCell());
    }
...
static void DumpHeapVisitCompartment(JSContext* cx, void* data, JSCompartment* comp) {
    char name[1024];
...
static void DumpHeapVisitArena(JSRuntime* rt, void* data, gc::Arena* arena,
    ...
    DumpHeapTracer* dtrc = static_cast<DumpHeapTracer*>(data);
    fprintf(dtrc->output, "# arena allockind=%u size=%u\n", unsigned(arena->getAllocKind()),
            unsigned(thingSize));
}

static void DumpHeapVisitCell(JSRuntime* rt, void* data, void* thing, JS::TraceKind traceKind,
                              size_t thingSize) {
    DumpHeapTracer* dtrc = static_cast<DumpHeapTracer*>(data);
...
void DumpHeapTracer::onChild(const JS::GCCellPtr& thing) {
    ...
    fprintf(output, "%s%p %c %s\n", prefix, thing.asCell(), MarkDescriptor(thing.asCell()),
            buffer);
}

void js::DumpHeap(JSContext* cx, FILE* fp, js::DumpHeapNurseryBehaviour nurseryBehaviour) {
...
JS_FRIEND_API(void)
JS::NotifyDidPaint(JSContext* cx) { cx->runtime()->gc.notifyDidPaint(); }

JS_FRIEND_API(void)
JS::NotifyGCRootsRemoved(JSContext* cx) { cx->runtime()->gc.notifyRootsRemoved(); }
...
void JS::ObjectPtr::updateWeakPointerAfterGC() {
    if (js::gc::IsAboutToBeFinalizedUnbarriered(value.unsafeGet())) value = nullptr;
}

void JS::ObjectPtr::trace(JSTracer* trc, const char* name) { JS::TraceEdge(trc, &value, name); }
...
#ifdef DEBUG
JS_FRIEND_API(unsigned)
js::GetEnterCompartmentDepth(JSContext* cx) { return cx->getEnterCompartmentDepth(); }
#endif
...
js::GetDOMCallbacks(JSContext* cx) { return cx->runtime()->DOMcallbacks; }
...
DOMProxyShadowsCheck js::GetDOMProxyShadowsCheck() { return gDOMProxyShadowsCheck; }

static XrayJitInfo* gXrayJitInfo = nullptr;

JS_FRIEND_API(void)
js::SetXrayJitInfo(XrayJitInfo* info) { gXrayJitInfo = info; }
...
JS_FRIEND_API(void)
js::PrepareScriptEnvironmentAndInvoke(JSContext* cx, HandleObject scope,
...
#ifdef DEBUG
bool js::HasObjectMovedOp(JSObject* obj) { return !!GetObjectClass(obj)->extObjectMovedOp(); }
#endif
...
js::detail::ToWindowProxyIfWindowSlow(JSObject* obj) {
    if (JSObject* windowProxy = obj->as<GlobalObject>().maybeWindowProxy()) return windowProxy;
    return obj;
}
...
JS_FRIEND_API(void)
js::EnableAccessValidation(JSContext* cx, bool enabled) { cx->enableAccessValidation = enabled; }
