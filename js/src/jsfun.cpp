...
bool IsFunction(HandleValue v) { return v.isObject() && v.toObject().is<JSFunction>(); }

static bool AdvanceToActiveCallLinear(JSContext* cx, NonBuiltinScriptFrameIter& iter,
                                      HandleFunction fun) {
...
static void ThrowTypeErrorBehavior(JSContext* cx) {
...
static bool IsSloppyNormalFunction(JSFunction* fun) {
    ...
    if (fun->kind() == JSFunction::NormalFunction) {
        ...
        if (fun->isStarGenerator() || fun->isLegacyGenerator() || fun->isAsync()) return false;
    ...
    if (fun->kind() == JSFunction::AsmJS) return !IsAsmJSStrictModeModuleOrFunction(fun);
...
static bool ResolveInterpretedFunctionPrototype(JSContext* cx, HandleFunction fun, HandleId id) {
    ...
    if (!isStarGenerator && !isAsyncGenerator) {
        ...
        if (!DefineDataProperty(cx, proto, cx->names().constructor, objVal, 0)) return false;
    }
...
static bool fun_resolve(JSContext* cx, HandleObject obj, HandleId id, bool* resolvedp) {
...
bool js::XDRInterpretedFunction(XDRState<mode>* xdr, HandleScope enclosingScope,
    ...
    if (mode == XDR_ENCODE) {
        fun = objp;
        if (!fun->isInterpreted()) return xdr->fail(JS::TranscodeResult_Failure_NotInterpretedFun);

        if (fun->explicitName() || fun->hasCompileTimeName() || fun->hasGuessedAtom())
            firstword |= HasAtom;

        if (fun->isStarGenerator() || fun->isAsync()) firstword |= HasStarGeneratorProto;
        ...
        flagsword = (fun->nargs() << 16) | (fun->flags() & ~JSFunction::NO_XDR_FLAGS);
    ...
    if (firstword & IsLazy) {
        if (!XDRLazyScript(xdr, enclosingScope, sourceObject, fun, &lazy)) return false;
    } else {
        if (!XDRScript(xdr, enclosingScope, sourceObject, fun, &script)) return false;
    }
...
template bool js::XDRInterpretedFunction(XDRState<XDR_ENCODE>*, HandleScope, HandleScriptSource,
                                         MutableHandleFunction);

template bool js::XDRInterpretedFunction(XDRState<XDR_DECODE>*, HandleScope, HandleScriptSource,
                                         MutableHandleFunction);
 ...
 */
bool JS::OrdinaryHasInstance(JSContext* cx, HandleObject objArg, HandleValue v, bool* bp) {
    AssertHeapIsIdle();
...
inline void JSFunction::trace(JSTracer* trc) {
    ...
    if (isInterpreted()) {
        ...
        if (u.i.env_) TraceManuallyBarrieredEdge(trc, &u.i.env_, "fun_environment");
    }
}

static void fun_trace(JSTracer* trc, JSObject* obj) { obj->as<JSFunction>().trace(trc); }
...
static JSObject* CreateFunctionConstructor(JSContext* cx, JSProtoKey key) {
    ...
    RootedObject functionCtor(
        cx, NewFunctionWithProto(cx, Function, 1, JSFunction::NATIVE_CTOR, nullptr,
...
static JSObject* CreateFunctionPrototype(JSContext* cx, JSProtoKey key) {
    ...
    JSObject* functionProto_ =
        NewFunctionWithProto(cx, nullptr, 0, JSFunction::INTERPRETED, enclosingEnv, nullptr,
    ...
    RootedScript script(
        cx, JSScript::Create(cx, options, sourceObject, begin, ss->length(), 0, ss->length()));
    if (!script || !JSScript::initFunctionPrototype(cx, script, functionProto)) return nullptr;
     ...
     * NewFunctionClone.
     */
    if (!JSObject::setNewGroupUnknown(cx, &JSFunction::class_, functionProto)) return nullptr;
    ...
    RootedFunction throwTypeError(
        cx, NewFunctionWithProto(cx, ThrowTypeError, 0, JSFunction::NATIVE_FUN, nullptr, nullptr,
...
const Class JSFunction::class_ = {js_Function_str, JSCLASS_HAS_CACHED_PROTO(JSProto_Function),
...
JSString* js::FunctionToString(JSContext* cx, HandleFunction fun, bool isToSource) {
    if (fun->isInterpretedLazy() && !JSFunction::getOrCreateScript(cx, fun)) return nullptr;
    ...
    bool haveSource =
        fun->isInterpreted() && (fun->isClassConstructor() || !fun->isSelfHostedBuiltin());
    ...
    if (haveSource) {
        ...
        if (fun->isInterpreted() &&
            (!fun->isSelfHostedBuiltin() || fun->infallibleIsDefaultClassConstructor(cx))) {
...
JSString* fun_toStringHelper(JSContext* cx, HandleObject obj, bool isToSource) {
    if (!obj->is<JSFunction>()) {
        if (JSFunToStringOp op = obj->getOpsFunToString()) return op(cx, obj, isToSource);
...
bool js::FunctionHasDefaultHasInstance(JSFunction* fun, const WellKnownSymbols& symbols) {
...
bool JSFunction::isDerivedClassConstructor() {
    ...
    if (isInterpretedLazy()) {
        ...
        if (isSelfHostedBuiltin()) {
            ...
            derived =
                name ==
                compartment()->runtimeFromAnyThread()->commonNames->DefaultDerivedClassConstructor;
        } else {
...
/* static */ bool JSFunction::getLength(JSContext* cx, HandleFunction fun, uint16_t* length) {
    MOZ_ASSERT(!fun->isBoundFunction());
...
/* static */ bool JSFunction::getUnresolvedLength(JSContext* cx, HandleFunction fun,
...
/* static */ bool JSFunction::getUnresolvedName(JSContext* cx, HandleFunction fun,
                                                MutableHandleAtom v) {
    MOZ_ASSERT(!IsInternalFunctionObject(*fun));
    ...
    if (fun->isBoundFunction() && !fun->hasBoundFunctionNamePrefix()) {
        ...
        if (name->length() > 0) {
            StringBuffer sb(cx);
            if (!sb.append(cx->names().boundWithSpace) || !sb.append(name)) return false;
...
/* static */ bool JSFunction::finishBoundFunctionInit(JSContext* cx, HandleFunction bound,
                                                      HandleObject targetObj, int32_t argCount) {
    ...
    if (targetObj->is<JSFunction>() && !targetObj->as<JSFunction>().hasResolvedLength()) {
        ...
        if (hasLength) {
            RootedValue targetLength(cx);
            if (!GetProperty(cx, targetObj, targetObj, idRoot, &targetLength)) return false;
    ...
    if (targetObj->is<JSFunction>() && !targetObj->as<JSFunction>().hasResolvedName()) {
        if (!JSFunction::getUnresolvedName(cx, targetObj.as<JSFunction>(), &name)) return false;
    }
    ...
    if (!name) {
        ...
        if (!GetProperty(cx, targetObj, targetObj, cx->names().name, &targetName)) return false;
...
/* static */ bool JSFunction::createScriptForLazilyInterpretedFunction(JSContext* cx,
                                                                       HandleFunction fun) {
    ...
    if (lazy) {
        ...
        if (script) {
            ...
            RootedScript clonedScript(cx,
                                      CloneScriptIntoFunction(cx, enclosingScope, fun, script));
            if (!clonedScript) return false;

            clonedScript->setSourceObject(lazy->sourceObject());

            fun->initAtom(script->functionNonDelazifying()->displayAtom());

            if (!lazy->maybeScript()) lazy->initScript(clonedScript);
            return true;
        }
        ...
        ScriptSource::PinnedChars chars(cx, lazy->scriptSource(), holder, lazy->begin(),
                                        lazyLength);
        ...
        if (script->scriptSource()->hasEncoder()) {
            ...
            if (!script->scriptSource()->xdrEncodeFunction(cx, fun, sourceObject)) return false;
        }
...
void JSFunction::maybeRelazify(JSRuntime* rt) {
    ...
    // parsing.
    if (!hasScript() || !u.i.s.script_) return;
    ...
    if (!u.i.s.script_->isRelazifiable()) return;
...
static bool fun_isGenerator(JSContext* cx, unsigned argc, Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);
    JSFunction* fun;
...
static bool CreateDynamicFunction(JSContext* cx, const CallArgs& args, GeneratorKind generatorKind,
    ...
    RootedFunction fun(
        cx, NewFunctionWithProto(cx, nullptr, 0, flags, globalLexical, anonymousAtom, defaultProto,
                                 allocKind, TenuredObject));
    if (!fun) return false;
    ...
    if (isAsync) {
        ...
        if (isStarGenerator) {
            wrapped =
                proto ? WrapAsyncGeneratorWithProto(cx, fun, proto) : WrapAsyncGenerator(cx, fun);
        } else {
            // Step 9.d, use %AsyncFunctionPrototype% as the fallback prototype.
            wrapped =
                proto ? WrapAsyncFunctionWithProto(cx, fun, proto) : WrapAsyncFunction(cx, fun);
        }
...
JSFunction* js::NewScriptedFunction(JSContext* cx, unsigned nargs, JSFunction::Flags flags,
    ...
    return NewFunctionWithProto(cx, nullptr, nargs, flags, enclosingEnv, atom, proto, allocKind,
                                newKind);
}

#ifdef DEBUG
static bool NewFunctionEnvironmentIsWellFormed(JSContext* cx, HandleObject env) {
...
JSFunction* js::NewFunctionWithProto(JSContext* cx, Native native, unsigned nargs,
...
static inline JSFunction* NewFunctionClone(JSContext* cx, HandleFunction fun,
                                           NewObjectKind newKind, gc::AllocKind allocKind,
                                           HandleObject proto) {
    ...
    JSObject* cloneobj =
        NewObjectWithClassProto(cx, &JSFunction::class_, cloneProto, allocKind, newKind);
...
JSFunction* js::CloneFunctionReuseScript(JSContext* cx, HandleFunction fun,
     ...
     */
    if (fun->staticPrototype() == clone->staticPrototype()) clone->setGroup(fun->group());
    return clone;
}

JSFunction* js::CloneFunctionAndScript(JSContext* cx, HandleFunction fun,
                                       HandleObject enclosingEnv, HandleScope newScope,
                                       gc::AllocKind allocKind /* = FUNCTION */,
                                       HandleObject proto /* = nullptr */) {
    MOZ_ASSERT(NewFunctionEnvironmentIsWellFormed(cx, enclosingEnv));
    MOZ_ASSERT(!fun->isBoundFunction());
 ...
 */
JSAtom* js::IdToFunctionName(JSContext* cx, HandleId id,
                             FunctionPrefixKind prefixKind /* = FunctionPrefixKind::None */) {
    // No prefix fastpath.
    if (JSID_IS_ATOM(id) && prefixKind == FunctionPrefixKind::None) return JSID_TO_ATOM(id);
    ...
    if (JSID_IS_SYMBOL(id)) {
        ...
        if (!desc && prefixKind == FunctionPrefixKind::None) return cx->names().empty;
        ...
        if (desc) {
            // Step 4.c.
            if (!sb.append('[') || !sb.append(desc) || !sb.append(']')) return nullptr;
        }
...
JSAtom* js::NameToFunctionName(JSContext* cx, HandleAtom name,
                               FunctionPrefixKind prefixKind /* = FunctionPrefixKind::None */) {
...
bool js::SetFunctionNameIfNoOwnName(JSContext* cx, HandleFunction fun, HandleValue name,
...
JSFunction* js::DefineFunction(JSContext* cx, HandleObject obj, HandleId id, Native native,
                               unsigned nargs, unsigned flags,
                               AllocKind allocKind /* = AllocKind::FUNCTION */) {
    ...
    if (!native)
        fun = NewScriptedFunction(cx, nargs, JSFunction::INTERPRETED_LAZY, atom,
                                  /* proto = */ nullptr, allocKind, GenericObject, obj);
    ...
    if (!DefineDataProperty(cx, obj, id, funVal, flags & ~JSFUN_FLAGS_MASK)) return nullptr;

    return fun;
}

void js::ReportIncompatibleMethod(JSContext* cx, const CallArgs& args, const Class* clasp) {
    RootedValue thisv(cx, args.thisv());

#ifdef DEBUG
    if (thisv.isObject()) {
        MOZ_ASSERT(thisv.toObject().getClass() != clasp || !thisv.toObject().isNative() ||
