...
extern JSString* ConcatStrings(JSContext* cx,
...
inline int32_t CompareChars(const Char1* s1, size_t len1, const Char2* s2, size_t len2) {
...
extern int32_t CompareChars(const char16_t* s1, size_t len1, JSLinearString* s2);
...
template <typename CharT>
extern const CharT* js_strchr_limit(const CharT* s, char16_t c, const CharT* limit);
 ...
 */
extern const char* ValueToPrintable(JSContext* cx, const Value&, JSAutoByteString* bytes,
                                    bool asSource = false);
 ...
 */
template <AllowGC allowGC>
extern JSString* ToStringSlow(JSContext* cx, typename MaybeRooted<Value, allowGC>::HandleType arg);
 ...
 */
extern bool EqualStrings(JSContext* cx, JSString* str1, JSString* str2, bool* result);

/* Use the infallible method instead! */
extern bool EqualStrings(JSContext* cx, JSLinearString* str1, JSLinearString* str2,
                         bool* result) = delete;
 ...
 */
extern bool CompareStrings(JSContext* cx, JSString* str1, JSString* str2, int32_t* result);
...
extern int StringFindPattern(JSLinearString* text, JSLinearString* pat, size_t start);

/* Return true if the string contains a pattern at |start|. */
extern bool HasSubstringAt(JSLinearString* text, JSLinearString* pat, size_t start);
 ...
 */
JSString* SubstringKernel(JSContext* cx, HandleString str, int32_t beginInt, int32_t lengthInt);
...
inline void CopyAndInflateChars(char16_t* dst, const JS::Latin1Char* src, size_t srclen) {
 ...
 */
template <typename CharT>
extern bool DeflateStringToBuffer(JSContext* maybecx, const CharT* chars, size_t charsLength,
                                  char* bytes, size_t* length);

extern bool str_fromCharCode(JSContext* cx, unsigned argc, Value* vp);

extern bool str_fromCharCode_one_arg(JSContext* cx, HandleValue code, MutableHandleValue rval);

extern bool str_fromCodePoint(JSContext* cx, unsigned argc, Value* vp);

extern bool str_fromCodePoint_one_arg(JSContext* cx, HandleValue code, MutableHandleValue rval);
...
extern bool str_charCodeAt_impl(JSContext* cx, HandleString string, HandleValue index,
...
extern bool str_trimLeft(JSContext* cx, unsigned argc, Value* vp);

extern bool str_trimRight(JSContext* cx, unsigned argc, Value* vp);
...
extern size_t PutEscapedStringImpl(char* buffer, size_t size, GenericPrinter* out,
                                   JSLinearString* str, uint32_t quote);

template <typename CharT>
extern size_t PutEscapedStringImpl(char* buffer, size_t bufferSize, GenericPrinter* out,
 ...
 */
inline size_t PutEscapedString(char* buffer, size_t size, JSLinearString* str, uint32_t quote) {
...
template <typename CharT>
inline size_t PutEscapedString(char* buffer, size_t bufferSize, const CharT* chars, size_t length,
                               uint32_t quote) {
...
inline bool EscapedStringPrinter(GenericPrinter& out, JSLinearString* str, uint32_t quote) {
    return PutEscapedStringImpl(nullptr, 0, &out, str, quote) != size_t(-1);
}

inline bool EscapedStringPrinter(GenericPrinter& out, const char* chars, size_t length,
                                 uint32_t quote) {
...
inline bool FileEscapedString(FILE* fp, const char* chars, size_t length, uint32_t quote) {
    Fprinter out(fp);
...
bool EncodeURI(JSContext* cx, StringBuffer& sb, const char* chars, size_t length);

ArrayObject* str_split_string(JSContext* cx, HandleObjectGroup group, HandleString str,
                              HandleString sep, uint32_t limit);

JSString* str_flat_replace_string(JSContext* cx, HandleString string, HandleString pattern,
                                  HandleString replacement);

JSString* str_replace_string_raw(JSContext* cx, HandleString string, HandleString pattern,
                                 HandleString replacement);
