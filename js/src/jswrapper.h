...
class MOZ_STACK_CLASS WrapperOptions : public ProxyOptions {
   public:
    ...
    explicit WrapperOptions(JSContext* cx) : ProxyOptions(false), proto_() { proto_.emplace(cx); }
...
class JS_FRIEND_API(Wrapper) : public ForwardingProxyHandler {
   ...
   public:
    explicit constexpr Wrapper(unsigned aFlags, bool aHasPrototype = false,
        ...
        : ForwardingProxyHandler(&family, aHasPrototype, aHasSecurityPolicy), mFlags(aFlags) {}
...
inline JSObject* WrapperOptions::proto() const { return proto_ ? *proto_ : Wrapper::defaultProto; }
...
class JS_FRIEND_API(CrossCompartmentWrapper) : public Wrapper {
   public:
    ...
    virtual bool get(JSContext* cx, HandleObject wrapper, HandleValue receiver, HandleId id,
                     MutableHandleValue vp) const override;
...
extern JSObject* TransparentObjectWrapper(JSContext* cx, HandleObject existing, HandleObject obj);

inline bool IsWrapper(JSObject* obj) {
...
JS_FRIEND_API(bool)
RemapAllWrappersForObject(JSContext* cx, JSObject* oldTarget, JSObject* newTarget);
