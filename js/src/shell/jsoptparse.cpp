...
const ValuedOption* Option::asValued() const { return const_cast<Option*>(this)->asValued(); }
...
static void PrintParagraph(const char* text, unsigned startColno, const unsigned limitColno,
                           bool padFirstLine) {
...
static const char* OptionFlagsToFormatInfo(char shortflag, bool isValued, size_t* length) {
    static const char* const fmt[4] = {"  -%c --%s ", "  --%s ", "  -%c --%s=%s ", "  --%s=%s "};

    /* How mny chars w/o longflag? */
    size_t lengths[4] = {strlen(fmt[0]) - 3, strlen(fmt[1]) - 3, strlen(fmt[2]) - 5,
                         strlen(fmt[3]) - 5};
...
OptionParser::Result OptionParser::printHelp(const char* progname) {
    ...
    if (!arguments.empty()) {
        ...
        size_t lhsLen = 0;
        for (Option* arg : arguments) lhsLen = Max(lhsLen, strlen(arg->longflag) + fmtChars);
...
OptionParser::Result OptionParser::extractValue(size_t argc, char** argv, size_t* i,
                                                char** value) {
    MOZ_ASSERT(*i < argc);
...
OptionParser::Result OptionParser::handleOption(Option* opt, size_t argc, char** argv, size_t* i,
                                                bool* optionsAllowed) {
...
OptionParser::Result OptionParser::handleArg(size_t argc, char** argv, size_t* i,
                                             bool* optionsAllowed) {
    if (nextArgument >= arguments.length()) return error("Too many arguments provided");
    ...
    for (size_t i = 1; i < argc; ++i) {
        ...
        if (arg[0] == '-' && arg[1] != '\0' && optionsAllowed) {
            ...
            if (arg[1] == '-') {
                ...
                /* Short option */
                if (arg[2] != '\0') return error("Short option followed by junk: %s", arg);
...
void OptionParser::setHelpOption(char shortflag, const char* longflag, const char* help) {
...
MultiStringRange OptionParser::getMultiStringOption(const char* longflag) const {
...
bool OptionParser::addIntOption(char shortflag, const char* longflag, const char* metavar,
...
bool OptionParser::addBoolOption(char shortflag, const char* longflag, const char* help) {
    if (!options.reserve(options.length() + 1)) return false;
...
bool OptionParser::addStringOption(char shortflag, const char* longflag, const char* metavar,
                                   const char* help) {
    if (!options.reserve(options.length() + 1)) return false;
...
bool OptionParser::addMultiStringOption(char shortflag, const char* longflag, const char* metavar,
                                        const char* help) {
    if (!options.reserve(options.length() + 1)) return false;
...
bool OptionParser::addOptionalMultiStringArg(const char* name, const char* help) {
