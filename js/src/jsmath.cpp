...
#if defined(ANDROID) || defined(XP_DARWIN) || defined(__DragonFly__) || defined(__FreeBSD__) || \
...
static const JSConstDoubleSpec math_constants[] = {
    {"E", M_E},   {"LOG2E", M_LOG2E}, {"LOG10E", M_LOG10E},   {"LN2", M_LN2}, {"LN10", M_LN10},
    {"PI", M_PI}, {"SQRT2", M_SQRT2}, {"SQRT1_2", M_SQRT1_2}, {nullptr, 0}};
...
const Class js::MathClass = {js_Math_str, JSCLASS_HAS_CACHED_PROTO(JSProto_Math)};

bool js::math_abs_handle(JSContext* cx, js::HandleValue v, js::MutableHandleValue r) {
    double x;
...
bool js::math_atan2_handle(JSContext* cx, HandleValue y, HandleValue x, MutableHandleValue res) {
    double dy;
...
bool js::math_ceil_handle(JSContext* cx, HandleValue v, MutableHandleValue res) {
    double d;
...
bool js::math_imul_handle(JSContext* cx, HandleValue lhs, HandleValue rhs,
    ...
    uint32_t a = 0, b = 0;
    if (!lhs.isUndefined() && !ToUint32(cx, lhs, &a)) return false;
    if (!rhs.isUndefined() && !ToUint32(cx, rhs, &b)) return false;

    uint32_t product = a * b;
    res.setInt32(product > INT32_MAX ? int32_t(INT32_MIN + (product - INT32_MAX - 1))
                                     : int32_t(product));
    return true;
}
...
bool js::math_log_handle(JSContext* cx, HandleValue val, MutableHandleValue res) {
    double in;
...
bool js::math_round_handle(JSContext* cx, HandleValue arg, MutableHandleValue res) {
    double d;
...
double js::math_round_impl(double x) {
    ...
    if (ExponentComponent(x) >= int_fast16_t(FloatingPoint<double>::kExponentShift)) return x;
...
float js::math_roundf_impl(float x) {
    ...
    /* Some numbers are so big that adding 0.5 would give the wrong number. */
    if (ExponentComponent(x) >= int_fast16_t(FloatingPoint<float>::kExponentShift)) return x;
...
bool js::math_sin_handle(JSContext* cx, HandleValue val, MutableHandleValue res) {
    double in;
    if (!ToNumber(cx, val, &in)) return false;
...
void js::math_sincos_impl(MathCache* mathCache, double x, double* sin, double* cos) {
    AutoUnsafeCallWithABI unsafe;
    unsigned indexSin;
    unsigned indexCos;
...
bool js::math_sqrt_handle(JSContext* cx, HandleValue number, MutableHandleValue result) {
    double x;
...
bool js::math_hypot_handle(JSContext* cx, HandleValueArray args, MutableHandleValue res) {
    ...
    double result =
        isInfinite ? PositiveInfinity<double>() : isNaN ? GenericNaN() : scale * sqrt(sumsq);
    res.setNumber(result);
    return true;
}
...
JSObject* js::InitMathClass(JSContext* cx, HandleObject obj) {
    ...
    if (!JS_DefineProperty(cx, obj, js_Math_str, Math, JSPROP_RESOLVING)) return nullptr;
