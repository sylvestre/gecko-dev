...
static MOZ_ALWAYS_INLINE void* js_memcpy(void* dst_, const void* src_, size_t len) {
    char* dst = (char*)dst_;
...
static inline typename Container::ElementType* Find(Container& c,
                                                    const typename Container::ElementType& v) {
...
static inline bool EqualContainers(const Container1& lhs, const Container2& rhs) {
...
template <class Container>
static inline HashNumber AddContainerToHash(const Container& c, HashNumber hn = 0) {
    for (size_t i = 0; i < c.length(); i++) hn = mozilla::AddToHash(hn, HashNumber(c[i]));
    return hn;
}
...
static inline U ComputeByteAlignment(T bytes, U alignment) {
    static_assert(mozilla::IsUnsigned<U>::value, "alignment amount must be unsigned");
...
static inline unsigned BitArrayIndexToWordIndex(size_t length, size_t bitIndex) {
...
static inline bool IsBitArrayElementSet(const size_t* array, size_t length, size_t i) {
...
static inline void ClearBitArrayElement(size_t* array, size_t length, size_t i) {
    array[BitArrayIndexToWordIndex(length, i)] &= ~BitArrayIndexToWordMask(i);
}
...
#define JS_MeanAndStdDevBS(bs, sigma) JS_MeanAndStdDev((bs)->num, (bs)->sum, (bs)->sqsum, sigma)
extern void JS_BasicStatsAccum(JSBasicStats* bs, uint32_t val);
extern double JS_MeanAndStdDev(uint32_t num, double sum, double sqsum, double* sigma);
...
#define JS_TEST_BIT(_map, _bit) \
    ((_map)[(_bit) / JS_BITMAP_NBITS] & (jsbitmap(1) << ((_bit) % JS_BITMAP_NBITS)))
...
#define JS_SILENCE_UNUSED_VALUE_IN_EXPR(expr)                                                          \
    JS_BEGIN_MACRO                                                                                     \
    _Pragma(                                                                                           \
        "clang diagnostic push") /* If these _Pragmas cause warnings for you, try disabling ccache. */ \
        _Pragma("clang diagnostic ignored \"-Wunused-value\"") {                                       \
        expr;                                                                                          \
    }                                                                                                  \
    _Pragma("clang diagnostic pop") JS_END_MACRO
#elif MOZ_IS_GCC
...
#define JS_SILENCE_UNUSED_VALUE_IN_EXPR(expr)                                 \
    ...
    _Pragma("GCC diagnostic pop") JS_END_MACRO
#endif
