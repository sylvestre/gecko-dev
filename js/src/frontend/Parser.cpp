...
#define MUST_MATCH_TOKEN(tt, errorNumber) MUST_MATCH_TOKEN_MOD(tt, TokenStream::None, errorNumber)
...
static inline void PropagateTransitiveParseFlags(const T* inner, U* outer) {
    if (inner->bindingsAccessedDynamically()) outer->setBindingsAccessedDynamically();
...
void ParseContext::Scope::dump(ParseContext* pc) {
    ...
    for (DeclaredNameMap::Range r = declared_->all(); !r.empty(); r.popFront()) {
        ...
        fprintf(stdout, "    %s %s%s\n", DeclarationKindString(info.kind()), bytes.ptr(),
...
bool ParseContext::Scope::addPossibleAnnexBFunctionBox(ParseContext* pc, FunctionBox* funbox) {
...
bool ParseContext::Scope::propagateAndMarkAnnexBFunctionBoxes(ParseContext* pc) {
    ...
    if (this == &pc->varScope()) {
        ...
        for (FunctionBox* funbox : *possibleAnnexBFunctionBoxes_) {
            if (pc->annexBAppliesToLexicalFunctionInInnermostScope(funbox)) {
                ...
                if (!pc->tryDeclareVar(name, DeclarationKind::VarForAnnexBLexicalFunction,
        ...
        for (FunctionBox* funbox : *possibleAnnexBFunctionBoxes_) {
            if (pc->annexBAppliesToLexicalFunctionInInnermostScope(funbox)) {
                if (!enclosing()->addPossibleAnnexBFunctionBox(pc, funbox)) return false;
            }
        }
    }
...
bool ParseContext::Scope::addCatchParameters(ParseContext* pc, Scope& catchParamScope) {
...
void ParseContext::Scope::removeCatchParameters(ParseContext* pc, Scope& catchParamScope) {
    ...
    for (DeclaredNameMap::Range r = catchParamScope.declared_->all(); !r.empty(); r.popFront()) {
        ...
        if (DeclarationKindIsCatchParameter(r.front().value()->kind())) declared_->remove(p);
    }
}
...
void SharedContext::computeThisBinding(Scope* scope) {
    for (ScopeIter si(scope); si; si++) {
        ...
        if (si.kind() == ScopeKind::Function) {
            ...
            if (fun->isArrow() || fun->nonLazyScript()->isGeneratorExp()) continue;
...
EvalSharedContext::EvalSharedContext(JSContext* cx, JSObject* enclosingEnv, Scope* enclosingScope,
...
bool UsedNameTracker::noteUse(JSContext* cx, JSAtom* name, uint32_t scriptId, uint32_t scopeId) {
...
void UsedNameTracker::UsedNameInfo::resetToScope(uint32_t scriptId, uint32_t scopeId) {
    while (!uses_.empty()) {
...
FunctionBox::FunctionBox(JSContext* cx, LifoAlloc& alloc, ObjectBox* traceListHead,
                         JSFunction* fun, uint32_t toStringStart, Directives directives,
...
void FunctionBox::initFromLazyFunction() {
    ...
    if (fun->lazyScript()->isDerivedClassConstructor()) setDerivedClassConstructor();
...
void FunctionBox::initWithEnclosingParseContext(ParseContext* enclosing, FunctionSyntaxKind kind) {
...
void ParserBase::error(unsigned errorNumber, ...) {
    va_list args;
...
void ParserBase::errorWithNotes(UniquePtr<JSErrorNotes> notes, unsigned errorNumber, ...) {
    va_list args;
    va_start(args, errorNumber);
...
void ParserBase::errorAt(uint32_t offset, unsigned errorNumber, ...) {
    va_list args;
    va_start(args, errorNumber);
...
void ParserBase::errorWithNotesAt(UniquePtr<JSErrorNotes> notes, uint32_t offset,
...
bool ParserBase::warning(unsigned errorNumber, ...) {
    va_list args;
    va_start(args, errorNumber);
...
bool ParserBase::warningAt(uint32_t offset, unsigned errorNumber, ...) {
    va_list args;
    va_start(args, errorNumber);
    ...
    if (result) {
        result = tokenStream.compileWarning(Move(metadata), nullptr, JSREPORT_WARNING, errorNumber,
                                            args);
    }
...
bool ParserBase::extraWarning(unsigned errorNumber, ...) {
    va_list args;
    va_start(args, errorNumber);
...
bool ParserBase::extraWarningAt(uint32_t offset, unsigned errorNumber, ...) {
    va_list args;
    va_start(args, errorNumber);

    bool result = tokenStream.reportExtraWarningErrorNumberVA(nullptr, offset, errorNumber, args);

    va_end(args);
    return result;
}

bool ParserBase::strictModeError(unsigned errorNumber, ...) {
    va_list args;
    va_start(args, errorNumber);

    bool res = tokenStream.reportStrictModeErrorNumberVA(nullptr, pos().begin, pc->sc()->strict(),
                                                         errorNumber, args);
...
bool ParserBase::strictModeErrorAt(uint32_t offset, unsigned errorNumber, ...) {
    va_list args;
    va_start(args, errorNumber);

    bool res = tokenStream.reportStrictModeErrorNumberVA(nullptr, offset, pc->sc()->strict(),
                                                         errorNumber, args);
...
bool ParserBase::reportNoOffset(ParseReportKind kind, bool strict, unsigned errorNumber, ...) {
    va_list args;
    va_start(args, errorNumber);
    ...
    switch (kind) {
        ...
        case ParseWarning: {
            ...
            if (kind == ParseError) {
                ...
                result = tokenStream.compileWarning(Move(metadata), nullptr, JSREPORT_WARNING,
                                                    errorNumber, args);
            }
        ...
        case ParseStrictError:
            result = tokenStream.reportStrictModeErrorNumberVA(nullptr, TokenStream::NoOffset,
                                                               strict, errorNumber, args);
            break;
    }
...
template <>
bool Parser<FullParseHandler, char16_t>::hadAbortedSyntaxParse() {
    return false;
}

template <>
bool Parser<SyntaxParseHandler, char16_t>::hadAbortedSyntaxParse() {
    return syntaxParser_ == ABORTED_SYNTAX_PARSE_SENTINEL;
}

template <>
void Parser<FullParseHandler, char16_t>::clearAbortedSyntaxParse() {}

template <>
void Parser<SyntaxParseHandler, char16_t>::clearAbortedSyntaxParse() {
    syntaxParser_ = nullptr;
}

template <>
void Parser<FullParseHandler, char16_t>::disableSyntaxParser() {
    syntaxParser_ = nullptr;
}

template <>
void Parser<SyntaxParseHandler, char16_t>::disableSyntaxParser() {}

template <>
inline bool Parser<FullParseHandler, char16_t>::abortIfSyntaxParser() {
    disableSyntaxParser();
    return true;
}

template <>
inline bool Parser<SyntaxParseHandler, char16_t>::abortIfSyntaxParser() {
...
ParserBase::ParserBase(JSContext* cx, LifoAlloc& alloc, const ReadOnlyCompileOptions& options,
...
Parser<ParseHandler, CharT>::Parser(JSContext* cx, LifoAlloc& alloc,
    ...
    if (options.extraWarningsOption) disableSyntaxParser();
}

template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::checkOptions() {
#ifdef DEBUG
...
template <>
void Parser<SyntaxParseHandler, char16_t>::setAwaitHandling(AwaitHandling awaitHandling) {
    awaitHandling_ = awaitHandling;
}

template <>
void Parser<FullParseHandler, char16_t>::setAwaitHandling(AwaitHandling awaitHandling) {
    awaitHandling_ = awaitHandling;
    if (syntaxParser_) syntaxParser_->setAwaitHandling(awaitHandling);
}
...
template <class ParseHandler, typename CharT>
FunctionBox* Parser<ParseHandler, CharT>::newFunctionBox(Node fn, JSFunction* fun,
    ...
    FunctionBox* funbox = alloc.new_<FunctionBox>(
        context, alloc, traceListHead, fun, toStringStart, inheritedDirectives,
...
template <class ParseHandler, typename CharT>
void Parser<ParseHandler, CharT>::trace(JSTracer* trc) {
 ...
 */
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::parse() {
    MOZ_ASSERT(checkOptionsCalled);
...
bool ParserBase::isValidStrictBinding(PropertyName* name) {
    ...
    return tt != TOK_LET && tt != TOK_STATIC && tt != TOK_YIELD &&
...
template <class ParseHandler, typename CharT>
void Parser<ParseHandler, CharT>::reportMissingClosing(unsigned errorNumber, unsigned noteNumber,
                                                       uint32_t openedPos) {
    ...
    if (!notes->addNoteASCII(pc->sc()->context, getFilename(), line, column, GetErrorMessage,
...
template <class ParseHandler, typename CharT>
void Parser<ParseHandler, CharT>::reportRedeclaration(HandlePropertyName name,
    ...
    if (!notes->addNoteASCII(pc->sc()->context, getFilename(), line, column, GetErrorMessage,
                             nullptr, JSMSG_REDECLARED_PREV, lineNumber, columnNumber)) {
        return;
    }

    errorWithNotesAt(Move(notes), pos.begin, JSMSG_REDECLARED_VAR, DeclarationKindString(prevKind),
                     bytes.ptr());
}
...
// forbid duplicates.)
template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::notePositionalFormalParameter(Node fn, HandlePropertyName name,
                                                                uint32_t beginPos,
    ...
    if (AddDeclaredNamePtr p = pc->functionScope().lookupDeclaredNameForAdd(name)) {
        ...
        *duplicatedParam = true;
        ...
        if (!pc->functionScope().addDeclaredName(pc, p, name, kind, beginPos)) return false;
    }
...
template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::noteDestructuredPositionalFormalParameter(Node fn,
                                                                            Node destruct) {
...
static bool DeclarationKindIsVar(DeclarationKind kind) {
    return kind == DeclarationKind::Var || kind == DeclarationKind::BodyLevelFunction ||
           kind == DeclarationKind::VarForAnnexBLexicalFunction ||
           kind == DeclarationKind::ForOfVar;
}

Maybe<DeclarationKind> ParseContext::isVarRedeclaredInEval(HandlePropertyName name,
            ...
            switch (bi.kind()) {
                case BindingKind::Let: {
                    ...
                    bool annexB35Allowance =
                        si.kind() == ScopeKind::SimpleCatch && kind != DeclarationKind::ForOfVar;
                    if (!annexB35Allowance) {
                        return Some(ScopeKindIsCatch(si.kind()) ? DeclarationKind::CatchParameter
                                                                : DeclarationKind::Let);
                    }
                    break;
                }
...
Maybe<DeclarationKind> ParseContext::isVarRedeclaredInInnermostScope(HandlePropertyName name,
                                                                     DeclarationKind kind) {
    Maybe<DeclarationKind> redeclaredKind;
    uint32_t unused;
...
bool ParseContext::tryDeclareVar(HandlePropertyName name, DeclarationKind kind, uint32_t beginPos,
...
bool ParseContext::tryDeclareVarHelper(HandlePropertyName name, DeclarationKind kind,
    ...
    for (ParseContext::Scope* scope = innermostScope(); scope != varScope().enclosing();
...
bool ParseContext::annexBAppliesToLexicalFunctionInInnermostScope(FunctionBox* funbox) {
...
template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::checkLexicalDeclarationDirectlyWithinBlock(
    ParseContext::Statement& stmt, DeclarationKind kind, TokenPos pos) {
    MOZ_ASSERT(DeclarationKindIsLexical(kind));

    // It is an early error to declare a lexical binding not directly
    // within a block.
    if (!StatementKindIsBraced(stmt.kind()) && stmt.kind() != StatementKind::ForLoopLexicalHead) {
        errorAt(pos.begin,
                stmt.kind() == StatementKind::Label ? JSMSG_LEXICAL_DECL_LABEL
...
template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::noteDeclaredName(HandlePropertyName name, DeclarationKind kind,
                                                   TokenPos pos) {
    ...
    switch (kind) {
        ...
        case DeclarationKind::ForOfVar: {
            Maybe<DeclarationKind> redeclaredKind;
            uint32_t prevPos;
            if (!pc->tryDeclareVar(name, kind, pos.begin, &redeclaredKind, &prevPos)) return false;
        ...
        case DeclarationKind::ModuleBodyLevelFunction: {
            ...
            if (!pc->varScope().addDeclaredName(pc, p, name, kind, pos.begin)) return false;
        ...
        case DeclarationKind::FormalParameter: {
            ...
            if (!pc->functionScope().addDeclaredName(pc, p, name, kind, pos.begin)) return false;
        ...
        case DeclarationKind::CatchParameter: {
            if (ParseContext::Statement* stmt = pc->innermostStatement()) {
                if (!checkLexicalDeclarationDirectlyWithinBlock(*stmt, kind, pos)) return false;
            }
...
template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::noteUsedName(HandlePropertyName name) {
...
template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::hasUsedName(HandlePropertyName name) {
    if (UsedNamePtr p = usedNames.lookup(name)) return p->value().isUsedInScript(pc->scriptId());
    return false;
}

template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::propagateFreeNamesAndMarkClosedOverBindings(
...
template <>
bool Parser<FullParseHandler, char16_t>::checkStatementsEOF() {
...
static typename Scope::Data* NewEmptyBindingData(JSContext* cx, LifoAlloc& alloc,
...
Maybe<GlobalScope::Data*> ParserBase::newGlobalScopeData(ParseContext::Scope& scope) {
...
Maybe<ModuleScope::Data*> ParserBase::newModuleScopeData(ParseContext::Scope& scope) {
...
Maybe<EvalScope::Data*> ParserBase::newEvalScopeData(ParseContext::Scope& scope) {
    Vector<BindingName> funs(context);
    Vector<BindingName> vars(context);
...
Maybe<FunctionScope::Data*> ParserBase::newFunctionScopeData(ParseContext::Scope& scope,
    ...
    for (size_t i = 0; i < pc->positionalFormalParameterNames().length(); i++) {
        ...
        if (name) {
            ...
            bool closedOver = allBindingsClosedOver || (p && p->value()->closedOver());
...
Maybe<LexicalScope::Data*> ParserBase::newLexicalScopeData(ParseContext::Scope& scope) {
    Vector<BindingName> lets(context);
    Vector<BindingName> consts(context);
...
template <>
SyntaxParseHandler::Node Parser<SyntaxParseHandler, char16_t>::finishLexicalScope(
...
template <>
ParseNode* Parser<FullParseHandler, char16_t>::finishLexicalScope(ParseContext::Scope& scope,
                                                                  ParseNode* body) {
...
static bool IsArgumentsUsedInLegacyGenerator(JSContext* cx, Scope* scope) {
    ...
    for (ScopeIter si(scope); si; si++) {
        if (si.scope()->is<LexicalScope>()) {
            ...
            for (::BindingIter bi(si.scope()); bi; bi++) {
                if (bi.name() == argumentsName) return false;
            }
...
template <>
ParseNode* Parser<FullParseHandler, char16_t>::evalBody(EvalSharedContext* evalsc) {
...
template <>
ParseNode* Parser<FullParseHandler, char16_t>::globalBody(GlobalSharedContext* globalsc) {
    SourceParseContext globalpc(this, globalsc, /* newDirectives = */ nullptr);
...
template <>
ParseNode* Parser<FullParseHandler, char16_t>::moduleBody(ModuleSharedContext* modulesc) {
    MOZ_ASSERT(checkOptionsCalled);
    ...
    if (!propagateFreeNamesAndMarkClosedOverBindings(modulepc.varScope())) return null();
...
template <>
SyntaxParseHandler::Node Parser<SyntaxParseHandler, char16_t>::moduleBody(
...
template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::hasUsedFunctionSpecialName(HandlePropertyName name) {
...
template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::declareFunctionThis() {
    // The asm.js validator does all its own symbol-table management so, as an
    // optimization, avoid doing any work here.
    if (pc->useAsmOrInsideUseAsm()) return true;
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::newInternalDotName(
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::newThisName() {
    return newInternalDotName(context->names().dotThis);
}

template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::newDotGeneratorName() {
    return newInternalDotName(context->names().dotGenerator);
}

template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::declareDotGeneratorName() {
...
template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::finishFunctionScopes(bool isStandaloneFunction) {
    FunctionBox* funbox = pc->functionBox();

    if (funbox->hasParameterExprs) {
        if (!propagateFreeNamesAndMarkClosedOverBindings(pc->functionScope())) return false;
    }

    if (funbox->function()->isNamedLambda() && !isStandaloneFunction) {
        if (!propagateFreeNamesAndMarkClosedOverBindings(pc->namedLambdaScope())) return false;
    }
...
template <>
bool Parser<FullParseHandler, char16_t>::finishFunction(bool isStandaloneFunction /* = false */) {
...
template <>
bool Parser<SyntaxParseHandler, char16_t>::finishFunction(
...
static YieldHandling GetYieldHandling(GeneratorKind generatorKind) {
    if (generatorKind == NotGenerator) return YieldIsName;
    return YieldIsKeyword;
}

static AwaitHandling GetAwaitHandling(FunctionAsyncKind asyncKind) {
    if (asyncKind == SyncFunction) return AwaitIsName;
    return AwaitIsKeyword;
}

template <>
ParseNode* Parser<FullParseHandler, char16_t>::standaloneFunction(
    HandleFunction fun, HandleScope enclosingScope, const Maybe<uint32_t>& parameterListEnd,
    ...
    if (!functionFormalParametersAndBody(InAllowed, yieldHandling, fn, Statement, parameterListEnd,
...
template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::declareFunctionArgumentsObject() {
    FunctionBox* funbox = pc->functionBox();
    ParseContext::Scope& funScope = pc->functionScope();
    ...
    if (funbox->usesArguments) {
        ...
        if (pc->sc()->bindingsAccessedDynamically()) funbox->setDefinitelyNeedsArgsObj();
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::functionBody(InHandling inHandling,
...
JSFunction* ParserBase::newFunction(HandleAtom atom, FunctionSyntaxKind kind,
    ...
    if (asyncKind == AsyncFunction) allocKind = gc::AllocKind::FUNCTION_EXTENDED;

    fun = NewFunctionWithProto(context, nullptr, 0, flags, nullptr, atom, proto, allocKind,
                               TenuredObject);
 ...
 */
template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::matchOrInsertSemicolonHelper(TokenStream::Modifier modifier) {
    TokenKind tt = TOK_EOF;
    if (!tokenStream.peekTokenSameLine(&tt, modifier)) return false;
    ...
    bool matched;
    if (!tokenStream.matchToken(&matched, TOK_SEMI, modifier)) return false;
...
template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::matchOrInsertSemicolonAfterExpression() {
    return matchOrInsertSemicolonHelper(TokenStream::None);
}

template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::matchOrInsertSemicolonAfterNonExpression() {
    return matchOrInsertSemicolonHelper(TokenStream::Operand);
}

template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::leaveInnerFunction(ParseContext* outerpc) {
    ...
    if (!outerpc->innerFunctionsForLazy.append(pc->functionBox()->function())) return false;
...
template <class ParseHandler, typename CharT>
JSAtom* Parser<ParseHandler, CharT>::prefixAccessorName(PropertyType propType,
                                                        HandleAtom propAtom) {
...
template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::functionArguments(YieldHandling yieldHandling,
                                                    FunctionSyntaxKind kind, Node funcpn) {
    FunctionBox* funbox = pc->functionBox();
    ...
    if (!parenFreeArrow) {
        ...
        if (!tokenStream.peekTokenPos(&firstTokenPos, firstTokenModifier)) return false;
    }
    ...
    if (parenFreeArrow) {
        ...
        bool matched;
        if (!tokenStream.matchToken(&matched, TOK_RP, TokenStream::Operand)) return false;
        ...
        while (true) {
            ...
            switch (tt) {
                ...
                case TOK_LC: {
                    ...
                    if (!noteDestructuredPositionalFormalParameter(funcpn, destruct)) return false;
                ...
                default: {
                    ...
                    if (parenFreeArrow) funbox->setStart(tokenStream);
            ...
            bool matched;
            if (!tokenStream.matchToken(&matched, TOK_ASSIGN)) return false;
            if (matched) {
                ...
                if (!handler.setLastFunctionFormalParameterDefault(funcpn, def_expr)) return false;
            }

            if (parenFreeArrow || IsSetterKind(kind)) break;

            if (!tokenStream.matchToken(&matched, TOK_COMMA)) return false;
            if (!matched) break;
...
template <>
bool Parser<FullParseHandler, char16_t>::skipLazyInnerFunction(ParseNode* pn,
                                                               uint32_t toStringStart,
                                                               FunctionSyntaxKind kind,
                                                               bool tryAnnexB) {
    ...
    if (kind == Statement && funbox->isExprBody()) {
        if (!matchOrInsertSemicolonAfterExpression()) return false;
    }
#endif

    // Append possible Annex B function box only upon successfully parsing.
    if (tryAnnexB && !pc->innermostScope()->addPossibleAnnexBFunctionBox(pc, funbox)) return false;

    return true;
}

template <>
bool Parser<SyntaxParseHandler, char16_t>::skipLazyInnerFunction(Node pn, uint32_t toStringStart,
                                                                 FunctionSyntaxKind kind,
                                                                 bool tryAnnexB) {
...
template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::addExprAndGetNextTemplStrToken(YieldHandling yieldHandling,
...
template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::taggedTemplate(YieldHandling yieldHandling, Node nodeList,
                                                 TokenKind tt) {
    ...
    while (true) {
        ...
        if (tt != TOK_TEMPLATE_HEAD) break;

        if (!addExprAndGetNextTemplStrToken(yieldHandling, nodeList, &tt)) return false;
    }
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::templateLiteral(
    YieldHandling yieldHandling) {
    ...
    TokenKind tt;
    do {
        if (!addExprAndGetNextTemplStrToken(yieldHandling, nodeList, &tt)) return null();
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::functionDefinition(
    Node pn, uint32_t toStringStart, InHandling inHandling, YieldHandling yieldHandling,
    HandleAtom funName, FunctionSyntaxKind kind, GeneratorKind generatorKind,
    ...
    if (handler.canSkipLazyInnerFunctions()) {
        if (!skipLazyInnerFunction(pn, toStringStart, kind, tryAnnexB)) return null();
        return pn;
    }
    ...
    while (true) {
        ...
        // Return on error.
        if (tokenStream.hadError() || directives == newDirectives) return null();
...
template <>
bool Parser<FullParseHandler, char16_t>::trySyntaxParseInnerFunction(
    ParseNode* pn, HandleFunction fun, uint32_t toStringStart, InHandling inHandling,
    YieldHandling yieldHandling, FunctionSyntaxKind kind, GeneratorKind generatorKind,
    ...
    do {
        ...
        if (!syntaxParser_->tokenStream.seek(position, tokenStream)) return false;

        // Make a FunctionBox before we enter the syntax parser, because |pn|
        // still expects a FunctionBox to be attached to it during BCE, and
        // the syntax parser cannot attach one to it.
        FunctionBox* funbox =
            newFunctionBox(pn, fun, toStringStart, inheritedDirectives, generatorKind, asyncKind);
        if (!funbox) return false;
        funbox->initWithEnclosingParseContext(pc, kind);

        if (!syntaxParser_->innerFunction(SyntaxParseHandler::NodeGeneric, pc, funbox,
                                          toStringStart, inHandling, yieldHandling, kind,
        ...
        if (!tokenStream.seek(position, syntaxParser_->tokenStream)) return false;
...
template <>
bool Parser<SyntaxParseHandler, char16_t>::trySyntaxParseInnerFunction(
    Node pn, HandleFunction fun, uint32_t toStringStart, InHandling inHandling,
    YieldHandling yieldHandling, FunctionSyntaxKind kind, GeneratorKind generatorKind,
    FunctionAsyncKind asyncKind, bool tryAnnexB, Directives inheritedDirectives,
    Directives* newDirectives) {
...
template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::innerFunction(Node pn, ParseContext* outerpc,
                                                FunctionBox* funbox, uint32_t toStringStart,
    ...
    if (!functionFormalParametersAndBody(inHandling, yieldHandling, pn, kind)) return false;

    return leaveInnerFunction(outerpc);
}

template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::innerFunction(
    Node pn, ParseContext* outerpc, HandleFunction fun, uint32_t toStringStart,
...
template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::appendToCallSiteObj(Node callSiteObj) {
...
template <>
ParseNode* Parser<FullParseHandler, char16_t>::standaloneLazyFunction(
    HandleFunction fun, uint32_t toStringStart, bool strict, GeneratorKind generatorKind,
    FunctionAsyncKind asyncKind) {
    MOZ_ASSERT(checkOptionsCalled);
    ...
    FunctionBox* funbox =
        newFunctionBox(pn, fun, toStringStart, directives, generatorKind, asyncKind);
    if (!funbox) return null();
    ...
    TokenStream::Modifier modifier =
        (fun->isArrow() && asyncKind == SyncFunction) ? TokenStream::Operand : TokenStream::None;
...
template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::functionFormalParametersAndBody(
    InHandling inHandling, YieldHandling yieldHandling, Node pn, FunctionSyntaxKind kind,
    ...
    if ((kind == Statement || kind == Expression) && fun->explicitName() && !inheritedStrict &&
        pc->sc()->strict()) {
        MOZ_ASSERT(pc->sc()->hasExplicitUseStrict(),
                   "strict mode should only change when a 'use strict' directive is present");
        ...
        if (!checkBindingIdentifier(propertyName, nameOffset, nameYieldHandling)) return false;
    }

    if (bodyType == StatementListBody) {
        MUST_MATCH_TOKEN_MOD_WITH_REPORT(
            TOK_RC, TokenStream::Operand,
        ...
        MOZ_ASSERT(kind == Arrow);
#endif
        if (tokenStream.hadError()) return false;
        funbox->setEnd(tokenStream);
        if (kind == Statement && !matchOrInsertSemicolonAfterExpression()) return false;
    }
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::functionStmt(
    uint32_t toStringStart, YieldHandling yieldHandling, DefaultHandling defaultHandling,
    FunctionAsyncKind asyncKind) {
    ...
    if (declaredInStmt && declaredInStmt->kind() == StatementKind::Label) {
        MOZ_ASSERT(!pc->sc()->strict(), "labeled functions shouldn't be parsed in strict mode");
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::functionExpr(
    uint32_t toStringStart, InvokedPrediction invoked, FunctionAsyncKind asyncKind) {
    MOZ_ASSERT(tokenStream.isCurrentTokenType(TOK_FUNCTION));
...
template <>
bool Parser<SyntaxParseHandler, char16_t>::asmJS(Node list) {
...
template <>
bool Parser<FullParseHandler, char16_t>::asmJS(Node list) {
 ...
 */
template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::maybeParseDirective(Node list, Node possibleDirective,
                                                      bool* cont) {
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::statementList(
    YieldHandling yieldHandling) {
    ...
    if (canHaveDirectives) tokenStream.clearSawOctalEscape();
    ...
    for (;;) {
        ...
        if (!tokenStream.peekToken(&tt, TokenStream::Operand)) {
            if (tokenStream.isEOF()) isUnexpectedEOF_ = true;
            return null();
        }
        ...
        if (afterReturn) {
            if (!tokenStream.peekOffset(&statementBegin, TokenStream::Operand)) return null();
        }
        ...
        if (!warnedAboutStatementsAfterReturn) {
            if (afterReturn) {
                if (!handler.isStatementPermittedAfterReturnStatement(next)) {
                    if (!warningAt(statementBegin, JSMSG_STMT_AFTER_RETURN)) return null();
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::condition(InHandling inHandling,
                                                                   YieldHandling yieldHandling) {
...
template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::matchLabel(YieldHandling yieldHandling,
...
template <class ParseHandler, typename CharT>
void Parser<ParseHandler, CharT>::PossibleError::setResolved(ErrorKind kind) {
    error(kind).state_ = ErrorState::None;
}

template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::PossibleError::hasError(ErrorKind kind) {
    return error(kind).state_ == ErrorState::Pending;
}

template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::PossibleError::hasPendingDestructuringError() {
    return hasError(ErrorKind::Destructuring);
}

template <class ParseHandler, typename CharT>
void Parser<ParseHandler, CharT>::PossibleError::setPending(ErrorKind kind, const TokenPos& pos,
                                                            unsigned errorNumber) {
...
template <class ParseHandler, typename CharT>
void Parser<ParseHandler, CharT>::PossibleError::setPendingDestructuringErrorAt(
    const TokenPos& pos, unsigned errorNumber) {
    setPending(ErrorKind::Destructuring, pos, errorNumber);
}

template <class ParseHandler, typename CharT>
void Parser<ParseHandler, CharT>::PossibleError::setPendingDestructuringWarningAt(
    const TokenPos& pos, unsigned errorNumber) {
    setPending(ErrorKind::DestructuringWarning, pos, errorNumber);
}

template <class ParseHandler, typename CharT>
void Parser<ParseHandler, CharT>::PossibleError::setPendingExpressionErrorAt(
    const TokenPos& pos, unsigned errorNumber) {
    setPending(ErrorKind::Expression, pos, errorNumber);
}

template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::PossibleError::checkForError(ErrorKind kind) {
...
template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::PossibleError::checkForWarning(ErrorKind kind) {
    if (!hasError(kind)) return true;
...
template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::PossibleError::checkForDestructuringErrorOrWarning() {
...
template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::PossibleError::checkForExpressionError() {
...
template <class ParseHandler, typename CharT>
void Parser<ParseHandler, CharT>::PossibleError::transferErrorTo(ErrorKind kind,
                                                                 PossibleError* other) {
...
template <class ParseHandler, typename CharT>
void Parser<ParseHandler, CharT>::PossibleError::transferErrorsTo(PossibleError* other) {
    MOZ_ASSERT(other);
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::bindingInitializer(
    Node lhs, DeclarationKind kind, YieldHandling yieldHandling) {
    MOZ_ASSERT(tokenStream.isCurrentTokenType(TOK_ASSIGN));

    if (kind == DeclarationKind::FormalParameter) pc->functionBox()->hasParameterExprs = true;
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::bindingIdentifier(
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::bindingIdentifierOrPattern(
    DeclarationKind kind, YieldHandling yieldHandling, TokenKind tt) {
    if (tt == TOK_LB) return arrayBindingPattern(kind, yieldHandling);

    if (tt == TOK_LC) return objectBindingPattern(kind, yieldHandling);
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::objectBindingPattern(
    DeclarationKind kind, YieldHandling yieldHandling) {
    ...
    for (;;) {
        ...
        if (tt == TOK_TRIPLEDOT) {
            ...
            if (propType == PropertyType::Normal) {
                ...
                bool hasInitializer;
                if (!tokenStream.matchToken(&hasInitializer, TOK_ASSIGN)) return null();

                Node bindingExpr =
                    hasInitializer ? bindingInitializer(binding, kind, yieldHandling) : binding;
                if (!bindingExpr) return null();

                if (!handler.addPropertyDefinition(literal, propName, bindingExpr)) return null();
        ...
        bool matched;
        if (!tokenStream.matchToken(&matched, TOK_COMMA)) return null();
        if (!matched) break;
        if (tt == TOK_TRIPLEDOT) {
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::arrayBindingPattern(
    DeclarationKind kind, YieldHandling yieldHandling) {
    ...
    MUST_MATCH_TOKEN_MOD_WITH_REPORT(
        TOK_RB, modifier,
        reportMissingClosing(JSMSG_BRACKET_AFTER_LIST, JSMSG_BRACKET_OPENED, begin));
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::destructuringDeclaration(
    DeclarationKind kind, YieldHandling yieldHandling, TokenKind tt) {
    MOZ_ASSERT(tokenStream.isCurrentTokenType(tt));
    MOZ_ASSERT(tt == TOK_LB || tt == TOK_LC);

    return tt == TOK_LB ? arrayBindingPattern(kind, yieldHandling)
                        : objectBindingPattern(kind, yieldHandling);
}

template <class ParseHandler, typename CharT>
typename ParseHandler::Node
Parser<ParseHandler, CharT>::destructuringDeclarationWithoutYieldOrAwait(
    DeclarationKind kind, YieldHandling yieldHandling, TokenKind tt) {
    uint32_t startYieldOffset = pc->lastYieldOffset;
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::blockStatement(
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::expressionAfterForInOrOf(
    ParseNodeKind forHeadKind, YieldHandling yieldHandling) {
    MOZ_ASSERT(forHeadKind == PNK_FORIN || forHeadKind == PNK_FOROF);
    Node pn = forHeadKind == PNK_FOROF ? assignExpr(InAllowed, yieldHandling, TripledotProhibited)
                                       : expr(InAllowed, yieldHandling, TripledotProhibited);
    return pn;
}

template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::declarationPattern(
    Node decl, DeclarationKind declKind, TokenKind tt, bool initialDeclaration,
    YieldHandling yieldHandling, ParseNodeKind* forHeadKind, Node* forInOrOfExpression) {
    MOZ_ASSERT(tokenStream.isCurrentTokenType(TOK_LB) || tokenStream.isCurrentTokenType(TOK_LC));
    ...
    if (initialDeclaration && forHeadKind) {
        ...
        if (isForIn) {
            ...
            *forHeadKind = PNK_FOROF;
            ...
            if (declKind == DeclarationKind::Var) declKind = DeclarationKind::ForOfVar;
        } else {
    ...
    Node init =
        assignExpr(forHeadKind ? InProhibited : InAllowed, yieldHandling, TripledotProhibited);
    if (!init) return null();
    ...
    if (forHeadKind) {
        ...
        // function.
        TokenKind ignored;
        if (!tokenStream.peekToken(&ignored)) return null();
        tokenStream.addModifierException(TokenStream::OperandIsNone);
    }
...
template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::initializerInNameDeclaration(
    Node decl, Node binding, Handle<PropertyName*> name, DeclarationKind declKind,
    bool initialDeclaration, YieldHandling yieldHandling, ParseNodeKind* forHeadKind,
    Node* forInOrOfExpression) {
    MOZ_ASSERT(tokenStream.isCurrentTokenType(TOK_ASSIGN));

    uint32_t initializerOffset;
    if (!tokenStream.peekOffset(&initializerOffset, TokenStream::Operand)) return false;
    ...
    if (forHeadKind) {
        if (initialDeclaration) {
            ...
            // certainly correct.
            TokenKind ignored;
            if (!tokenStream.peekToken(&ignored)) return false;
        }
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::declarationName(
    Node decl, DeclarationKind declKind, TokenKind tt, bool initialDeclaration,
    YieldHandling yieldHandling, ParseNodeKind* forHeadKind, Node* forInOrOfExpression) {
    ...
    bool matched;
    if (!tokenStream.matchToken(&matched, TOK_ASSIGN, TokenStream::Operand)) return null();
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::declarationList(
    YieldHandling yieldHandling, ParseNodeKind kind, ParseNodeKind* forHeadKind /* = nullptr */,
    ...
    do {
        MOZ_ASSERT_IF(!initialDeclaration && forHeadKind, *forHeadKind == PNK_FORHEAD);

        TokenKind tt;
        if (!tokenStream.getToken(&tt)) return null();

        Node binding = (tt == TOK_LB || tt == TOK_LC)
                           ? declarationPattern(decl, declKind, tt, initialDeclaration,
                                                yieldHandling, forHeadKind, forInOrOfExpression)
        ...
        if (forHeadKind && *forHeadKind != PNK_FORHEAD) break;
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::lexicalDeclaration(
     ...
     */
    Node decl =
        declarationList(yieldHandling, kind == DeclarationKind::Const ? PNK_CONST : PNK_LET);
    if (!decl || !matchOrInsertSemicolonAfterExpression()) return null();

    return decl;
}

template <>
bool Parser<FullParseHandler, char16_t>::namedImportsOrNamespaceImport(TokenKind tt,
                                                                       Node importSpecSet) {
    if (tt == TOK_LC) {
        while (true) {
            ...
            bool matched;
            if (!tokenStream.matchToken(&matched, TOK_AS)) return null();
            ...
            if (!noteDeclaredName(bindingAtom, DeclarationKind::Import, pos())) return false;
        ...
        if (!noteDeclaredName(bindingName, DeclarationKind::Const, pos())) return false;
...
template <>
bool Parser<SyntaxParseHandler, char16_t>::namedImportsOrNamespaceImport(TokenKind tt,
                                                                         Node importSpecSet) {
    MOZ_ALWAYS_FALSE(abortIfSyntaxParser());
    return false;
}
...
template <>
ParseNode* Parser<FullParseHandler, char16_t>::importDeclaration() {
    ...
    if (tt == TOK_STRING) {
        ...
        if (tt == TOK_LC || tt == TOK_MUL) {
            ...
            if (!noteDeclaredName(bindingAtom, DeclarationKind::Import, pos())) return null();
    ...
    if (!matchOrInsertSemicolonAfterNonExpression()) return null();

    ParseNode* node =
        handler.newImportDeclaration(importSpecSet, moduleSpec, TokenPos(begin, pos().end));
    if (!node || !pc->sc()->asModuleContext()->builder.processImport(node)) return null();

    return node;
}

template <>
SyntaxParseHandler::Node Parser<SyntaxParseHandler, char16_t>::importDeclaration() {
    MOZ_ALWAYS_FALSE(abortIfSyntaxParser());
    return SyntaxParseHandler::NodeFailure;
}

template <>
bool Parser<FullParseHandler, char16_t>::checkExportedName(JSAtom* exportName) {
    if (!pc->sc()->asModuleContext()->builder.hasExportedName(exportName)) return true;
...
template <>
bool Parser<SyntaxParseHandler, char16_t>::checkExportedName(JSAtom* exportName) {
    MOZ_ALWAYS_FALSE(abortIfSyntaxParser());
    return false;
}

template <>
bool Parser<FullParseHandler, char16_t>::checkExportedNamesForDeclaration(ParseNode* node) {
    MOZ_ASSERT(node->isArity(PN_LIST));
    for (ParseNode* binding = node->pn_head; binding; binding = binding->pn_next) {
        if (binding->isKind(PNK_ASSIGN)) binding = binding->pn_left;
        MOZ_ASSERT(binding->isKind(PNK_NAME));
        if (!checkExportedName(binding->pn_atom)) return false;
    }
...
template <>
bool Parser<SyntaxParseHandler, char16_t>::checkExportedNamesForDeclaration(Node node) {
    MOZ_ALWAYS_FALSE(abortIfSyntaxParser());
    return false;
}

template <>
bool Parser<FullParseHandler, char16_t>::checkExportedNameForClause(ParseNode* node) {
    return checkExportedName(node->pn_atom);
}

template <>
bool Parser<SyntaxParseHandler, char16_t>::checkExportedNameForClause(Node node) {
    MOZ_ALWAYS_FALSE(abortIfSyntaxParser());
    return false;
}

template <>
bool Parser<FullParseHandler, char16_t>::checkExportedNameForFunction(ParseNode* node) {
    return checkExportedName(node->pn_funbox->function()->explicitName());
}

template <>
bool Parser<SyntaxParseHandler, char16_t>::checkExportedNameForFunction(Node node) {
    MOZ_ALWAYS_FALSE(abortIfSyntaxParser());
    return false;
}

template <>
bool Parser<FullParseHandler, char16_t>::checkExportedNameForClass(ParseNode* node) {
...
template <>
bool Parser<SyntaxParseHandler, char16_t>::checkExportedNameForClass(Node node) {
    MOZ_ALWAYS_FALSE(abortIfSyntaxParser());
    return false;
}

template <>
bool Parser<FullParseHandler, char16_t>::processExport(ParseNode* node) {
    return pc->sc()->asModuleContext()->builder.processExport(node);
}

template <>
bool Parser<SyntaxParseHandler, char16_t>::processExport(Node node) {
    MOZ_ALWAYS_FALSE(abortIfSyntaxParser());
    return false;
}

template <>
bool Parser<FullParseHandler, char16_t>::processExportFrom(ParseNode* node) {
    return pc->sc()->asModuleContext()->builder.processExportFrom(node);
}

template <>
bool Parser<SyntaxParseHandler, char16_t>::processExportFrom(Node node) {
    MOZ_ALWAYS_FALSE(abortIfSyntaxParser());
    return false;
}

template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::exportFrom(uint32_t begin,
                                                                    Node specList) {
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::exportBatch(uint32_t begin) {
    if (!abortIfSyntaxParser()) return null();
...
template <>
bool Parser<FullParseHandler, char16_t>::checkLocalExportNames(ParseNode* node) {
...
template <>
bool Parser<SyntaxParseHandler, char16_t>::checkLocalExportNames(Node node) {
    MOZ_ALWAYS_FALSE(abortIfSyntaxParser());
    return false;
}

template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::exportClause(uint32_t begin) {
    if (!abortIfSyntaxParser()) return null();
    ...
    while (true) {
        ...
        bool foundAs;
        if (!tokenStream.matchToken(&foundAs, TOK_AS)) return null();
        if (foundAs)
    ...
    bool matched;
    if (!tokenStream.matchToken(&matched, TOK_FROM, TokenStream::Operand)) return null();
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::exportVariableStatement(uint32_t begin) {
    if (!abortIfSyntaxParser()) return null();
    ...
    if (!kid) return null();
    if (!matchOrInsertSemicolonAfterExpression()) return null();
    if (!checkExportedNamesForDeclaration(kid)) return null();
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::exportFunctionDeclaration(
    uint32_t begin, uint32_t toStringStart, FunctionAsyncKind asyncKind /* = SyncFunction */) {
    if (!abortIfSyntaxParser()) return null();
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::exportClassDeclaration(uint32_t begin) {
    if (!abortIfSyntaxParser()) return null();
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::exportLexicalDeclaration(
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::exportDefaultFunctionDeclaration(
    uint32_t begin, uint32_t toStringStart, FunctionAsyncKind asyncKind /* = SyncFunction */) {
    if (!abortIfSyntaxParser()) return null();
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::exportDefaultClassDeclaration(
    uint32_t begin) {
    if (!abortIfSyntaxParser()) return null();
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::exportDefaultAssignExpr(uint32_t begin) {
    if (!abortIfSyntaxParser()) return null();
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::exportDefault(uint32_t begin) {
    if (!abortIfSyntaxParser()) return null();
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::exportDeclaration() {
    if (!abortIfSyntaxParser()) return null();
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::expressionStatement(
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::consequentOrAlternative(
    YieldHandling yieldHandling) {
    TokenKind next;
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::ifStatement(YieldHandling yieldHandling) {
    ...
    while (true) {
        ...
        bool matched;
        if (!tokenStream.matchToken(&matched, TOK_ELSE, TokenStream::Operand)) return null();
        if (matched) {
            if (!tokenStream.matchToken(&matched, TOK_IF, TokenStream::Operand)) return null();
            if (matched) continue;
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::doWhileStatement(
    YieldHandling yieldHandling) {
    uint32_t begin = pos().begin;
    ...
    bool ignored;
    if (!tokenStream.matchToken(&ignored, TOK_SEMI, TokenStream::Operand)) return null();
    return handler.newDoWhileStatement(body, cond, TokenPos(begin, pos().end));
}

template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::whileStatement(
    YieldHandling yieldHandling) {
    uint32_t begin = pos().begin;
...
template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::matchInOrOf(bool* isForInp, bool* isForOfp) {
    TokenKind tt;
    if (!tokenStream.getToken(&tt)) return false;

    *isForInp = tt == TOK_IN;
    *isForOfp = tt == TOK_OF;
    if (!*isForInp && !*isForOfp) tokenStream.ungetToken();
...
template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::forHeadStart(YieldHandling yieldHandling, IteratorKind iterKind,
    ...
    if (tt == TOK_VAR) {
        ...
        *forInitialPart =
            declarationList(yieldHandling, PNK_VAR, forHeadKind, forInOrOfExpression);
    ...
    *forHeadKind = isForIn ? PNK_FORIN : PNK_FOROF;
    ...
    if (handler.isUnparenthesizedDestructuringPattern(*forInitialPart)) {
        ...
        if (chars) {
            ...
            if (!strictModeErrorAt(exprOffset, JSMSG_BAD_STRICT_ASSIGN, chars)) return false;
        }
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::forStatement(
    YieldHandling yieldHandling) {
    ...
    if (allowsForEachIn()) {
        bool matched;
        if (!tokenStream.matchToken(&matched, TOK_EACH)) return null();
        if (matched) {
            iflags = JSITER_FOREACH;
            isForEach = true;
            addTelemetry(DeprecatedLanguageExtension::ForEach);
            if (!warnOnceAboutForEach()) return null();
        }
    }

    if (pc->isAsync()) {
        bool matched;
        if (!tokenStream.matchToken(&matched, TOK_AWAIT)) return null();
    ...
    MUST_MATCH_TOKEN_MOD_WITH_REPORT(
        TOK_LP, TokenStream::None,
        error((token == TOK_AWAIT && !pc->isAsync()) ? JSMSG_FOR_AWAIT_OUTSIDE_ASYNC
    ...
    if (forLoopLexicalScope) return finishLexicalScope(*forLoopLexicalScope, forLoop);

    return forLoop;
}

template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::switchStatement(
    YieldHandling yieldHandling) {
    ...
    while (true) {
        ...
        while (true) {
            if (!tokenStream.peekToken(&tt, TokenStream::Operand)) return null();
            if (tt == TOK_RC || tt == TOK_CASE || tt == TOK_DEFAULT) break;
            if (afterReturn) {
                if (!tokenStream.peekOffset(&statementBegin, TokenStream::Operand)) return null();
            }
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::continueStatement(
    YieldHandling yieldHandling) {
    ...
    auto isLoop = [](ParseContext::Statement* stmt) { return StatementKindIsLoop(stmt->kind()); };
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::breakStatement(
    YieldHandling yieldHandling) {
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::returnStatement(
    YieldHandling yieldHandling) {
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::yieldExpression(InHandling inHandling) {
    ...
    switch (pc->generatorKind()) {
        case StarGenerator: {
            ...
            if (kind == PNK_YIELD_STAR) return handler.newYieldStarExpression(begin, exprNode);
        ...
        case NotGenerator:
            ...
            if (!warnOnceAboutLegacyGenerator()) return null();
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::withStatement(
    YieldHandling yieldHandling) {
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::labeledItem(YieldHandling yieldHandling) {
    TokenKind tt;
    if (!tokenStream.getToken(&tt, TokenStream::Operand)) return null();
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::labeledStatement(
    YieldHandling yieldHandling) {
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::throwStatement(
    YieldHandling yieldHandling) {
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::tryStatement(
    YieldHandling yieldHandling) {
    ...
    Node innerBlock;
    {
        ...
        MUST_MATCH_TOKEN_MOD_WITH_REPORT(
            TOK_RC, TokenStream::Operand,
            reportMissingClosing(JSMSG_CURLY_AFTER_TRY, JSMSG_CURLY_OPENED, openedPos));
    }
    ...
    if (tt == TOK_CATCH) {
        ...
        do {
             ...
             */
            bool matched;
            if (!tokenStream.matchToken(&matched, TOK_IF)) return null();
            if (matched) {
                catchGuard = expr(InAllowed, yieldHandling, TripledotProhibited);
                if (!catchGuard) return null();
            }
#endif
            ...
            if (!catchGuard) hasUnconditionalCatch = true;

            pnblock = finishLexicalScope(scope, catchBody);
            if (!pnblock) return null();
    ...
    if (tt == TOK_FINALLY) {
        ...
        MUST_MATCH_TOKEN_MOD_WITH_REPORT(
            TOK_RC, TokenStream::Operand,
            reportMissingClosing(JSMSG_CURLY_AFTER_FINALLY, JSMSG_CURLY_OPENED, openedPos));
    } else {
        tokenStream.ungetToken();
    }
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::catchBlockStatement(
    ...
    MUST_MATCH_TOKEN_MOD_WITH_REPORT(
        TOK_RC, TokenStream::Operand,
        reportMissingClosing(JSMSG_CURLY_AFTER_CATCH, JSMSG_CURLY_OPENED, openedPos));
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::debuggerStatement() {
    TokenPos p;
    p.begin = pos().begin;
    if (!matchOrInsertSemicolonAfterNonExpression()) return null();
    p.end = pos().end;
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::classDefinition(
    YieldHandling yieldHandling, ClassContext classContext, DefaultHandling defaultHandling) {
    MOZ_ASSERT(tokenStream.isCurrentTokenType(TOK_CLASS));
    ...
    bool hasHeritage;
    if (!tokenStream.matchToken(&hasHeritage, TOK_EXTENDS)) return null();
    if (hasHeritage) {
        if (!tokenStream.getToken(&tt)) return null();
    ...
    for (;;) {
        ...
        if (propType == PropertyType::Getter) propType = PropertyType::GetterNoExpressionClosure;
        if (propType == PropertyType::Setter) propType = PropertyType::SetterNoExpressionClosure;
        ...
        switch (propType) {
            ...
            case PropertyType::DerivedConstructor:
                ...
                break;
            default:
                if (!tokenStream.isCurrentTokenType(TOK_RB)) funName = propAtom;
        }
        ...
        Node fn =
            methodDefinition(isConstructor ? classStartOffset : nameOffset, propType, funName);
        if (!fn) return null();
    ...
    if (name) {
        ...
        if (classContext == ClassStatement) {
            ...
            if (!noteDeclaredName(name, DeclarationKind::Let, namePos)) return null();
...
template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::nextTokenContinuesLetDeclaration(TokenKind next) {
    ...
    if (next == TOK_LB || next == TOK_LC) return true;
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::variableStatement(
    YieldHandling yieldHandling) {
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::statement(YieldHandling yieldHandling) {
    MOZ_ASSERT(checkOptionsCalled);
    ...
    switch (tt) {
        ...
        case TOK_YIELD: {
            ...
            if (next == TOK_COLON) return labeledStatement(yieldHandling);
        ...
        default: {
            ...
            if (tt == TOK_AWAIT && pc->isAsync()) return expressionStatement(yieldHandling);

            if (!TokenKindIsPossibleIdentifier(tt)) return expressionStatement(yieldHandling);
            ...
            if (tt == TOK_LET) {
                ...
                if (next == TOK_LB) {
                    ...
                    MOZ_ASSERT(TokenKindIsPossibleIdentifier(nextSameLine) ||
                               nextSameLine == TOK_LC || nextSameLine == TOK_EOL);
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::statementListItem(
    ...
    switch (tt) {
        ...
        case TOK_STRING:
            if (!canHaveDirectives &&
                tokenStream.currentToken().atom() == context->names().useAsm) {
                if (!abortIfSyntaxParser()) return null();
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::expr(
    InHandling inHandling, YieldHandling yieldHandling, TripledotHandling tripledotHandling,
    PossibleError* possibleError /* = nullptr */,
    InvokedPrediction invoked /* = PredictUninvoked */) {
    Node pn = assignExpr(inHandling, yieldHandling, tripledotHandling, possibleError, invoked);
    if (!pn) return null();
    ...
    while (true) {
        ...
        pn = assignExpr(inHandling, yieldHandling, tripledotHandling, &possibleErrorInner);
        if (!pn) return null();
...
template <class ParseHandler, typename CharT>
MOZ_ALWAYS_INLINE typename ParseHandler::Node Parser<ParseHandler, CharT>::orExpr1(
    InHandling inHandling, YieldHandling yieldHandling, TripledotHandling tripledotHandling,
    ...
    for (;;) {
        ...
        if (tok == TOK_IN ? inHandling == InAllowed : TokenKindIsBinaryOp(tok)) {
            ...
            if (possibleError && !possibleError->checkForExpressionError()) return null();
...
template <class ParseHandler, typename CharT>
MOZ_ALWAYS_INLINE typename ParseHandler::Node Parser<ParseHandler, CharT>::condExpr1(
    InHandling inHandling, YieldHandling yieldHandling, TripledotHandling tripledotHandling,
    PossibleError* possibleError, InvokedPrediction invoked /* = PredictUninvoked */) {
    Node condition = orExpr1(inHandling, yieldHandling, tripledotHandling, possibleError, invoked);

    if (!condition || !tokenStream.isCurrentTokenType(TOK_HOOK)) return condition;
    ...
    TokenKind ignored;
    if (!tokenStream.getToken(&ignored)) return null();
    return handler.newConditional(condition, thenExpr, elseExpr);
}

template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::assignExpr(
    InHandling inHandling, YieldHandling yieldHandling, TripledotHandling tripledotHandling,
    PossibleError* possibleError /* = nullptr */,
    InvokedPrediction invoked /* = PredictUninvoked */) {
    if (!CheckRecursionLimit(context)) return null();
    ...
    if (tt == TOK_NUMBER) {
        if (!tokenStream.nextTokenEndsExpr(&endsExpr)) return null();
        if (endsExpr) return newNumber(tokenStream.currentToken());
    }
    ...
    if (tt == TOK_YIELD && yieldExpressionsSupported()) return yieldExpression(inHandling);
    ...
    if (maybeAsyncArrow) {
        ...
        lhs =
            condExpr1(inHandling, yieldHandling, tripledotHandling, &possibleErrorInner, invoked);
        if (!lhs) {
            return null();
        }
    }
    ...
    switch (tokenStream.currentToken().type) {
        ...
        case TOK_BITORASSIGN:
            kind = PNK_BITORASSIGN;
            break;
        case TOK_BITXORASSIGN:
            kind = PNK_BITXORASSIGN;
            break;
        case TOK_BITANDASSIGN:
            kind = PNK_BITANDASSIGN;
            break;
        case TOK_LSHASSIGN:
            kind = PNK_LSHASSIGN;
            break;
        ...
        case TOK_ARROW: {
            ...
            tokenStream.ungetToken();
            TokenKind next;
            if (!tokenStream.peekTokenSameLine(&next)) return null();
            ...
            Node arrowFunc = functionDefinition(pn, toStringStart, inHandling, yieldHandling,
                                                nullptr, Arrow, NotGenerator, asyncKind);
            ...
            if (isBlock) {
                ...
                // peeked consistently.
                TokenKind ignored;
                if (!tokenStream.peekToken(&ignored, TokenStream::Operand)) return null();
                tokenStream.addModifierException(TokenStream::NoneIsOperand);
            }
            return arrowFunc;
        }
    ...
    if (handler.isUnparenthesizedDestructuringPattern(lhs)) {
        ...
        if (!possibleErrorInner.checkForDestructuringErrorOrWarning()) return null();
    } else if (handler.isNameAnyParentheses(lhs)) {
        if (const char* chars = handler.nameIsArgumentsEvalAnyParentheses(lhs, context)) {
            // |chars| is "arguments" or "eval" here.
            if (!strictModeErrorAt(exprPos.begin, JSMSG_BAD_STRICT_ASSIGN, chars)) return null();
        }
        ...
        if (!strictModeErrorAt(exprPos.begin, JSMSG_BAD_LEFTSIDE_OF_ASS)) return null();
    ...
    if (kind == PNK_ASSIGN) handler.checkAndSetIsDirectRHSAnonFunction(rhs);

    return handler.newAssignment(kind, lhs, rhs);
}

template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::isValidSimpleAssignmentTarget(
...
template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::checkIncDecOperand(Node operand, uint32_t operandOffset) {
    if (handler.isNameAnyParentheses(operand)) {
        if (const char* chars = handler.nameIsArgumentsEvalAnyParentheses(operand, context)) {
            if (!strictModeErrorAt(operandOffset, JSMSG_BAD_STRICT_ASSIGN, chars)) return false;
        }
        ...
        // otherwise warn.
        if (!strictModeErrorAt(operandOffset, JSMSG_BAD_INCOP_OPERAND)) return false;
    } else {
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::unaryOpExpr(YieldHandling yieldHandling,
                                                                     ParseNodeKind kind,
                                                                     uint32_t begin) {
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::unaryExpr(
    ...
    switch (tt) {
        ...
        case TOK_DEC: {
            ...
            if (!operand || !checkIncDecOperand(operand, operandOffset)) return null();

            return handler.newUpdate((tt == TOK_INC) ? PNK_PREINCREMENT : PNK_PREDECREMENT, begin,
                                     operand);
        }

        case TOK_DELETE: {
            uint32_t exprOffset;
            if (!tokenStream.peekOffset(&exprOffset, TokenStream::Operand)) return null();
            ...
            if (handler.isNameAnyParentheses(expr)) {
                if (!strictModeErrorAt(exprOffset, JSMSG_DEPRECATED_DELETE_OPERAND)) return null();
        ...
        default: {
            ...
            if (tt != TOK_INC && tt != TOK_DEC) return expr;
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::generatorComprehensionLambda(
    unsigned begin) {
    ...
    RootedFunction fun(context, newFunction(/* atom = */ nullptr, Expression, StarGenerator,
                                            SyncFunction, proto));
    if (!fun) return null();
    ...
    Node generator = newDotGeneratorName();
    if (!generator) return null();
    if (!handler.prependInitialYield(body, generator)) return null();

    if (!propagateFreeNamesAndMarkClosedOverBindings(pc->varScope())) return null();
    if (!finishFunction()) return null();
    ...
    // lazyscript.
    if (!handler.setComprehensionLambdaBody(genfn, body)) return null();

    return genfn;
}

template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::comprehensionFor(
    ...
    if (!lhs) return null();
    bool matched;
    if (!tokenStream.matchToken(&matched, TOK_OF)) return null();
    if (!matched) {
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::comprehensionIf(
    GeneratorKind comprehensionKind) {
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::comprehensionTail(
    GeneratorKind comprehensionKind) {
    if (!CheckRecursionLimit(context)) return null();

    bool matched;
    if (!tokenStream.matchToken(&matched, TOK_FOR, TokenStream::Operand)) return null();
    if (matched) return comprehensionFor(comprehensionKind);

    if (!tokenStream.matchToken(&matched, TOK_IF, TokenStream::Operand)) return null();
    if (matched) return comprehensionIf(comprehensionKind);
    ...
    if (comprehensionKind == NotGenerator) return handler.newArrayPush(begin, bodyExpr);

    MOZ_ASSERT(comprehensionKind == StarGenerator);
    Node yieldExpr = handler.newYieldExpression(begin, bodyExpr);
    if (!yieldExpr) return null();
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::comprehension(
    GeneratorKind comprehensionKind) {
    MOZ_ASSERT(tokenStream.isCurrentTokenType(TOK_FOR));
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::arrayComprehension(uint32_t begin) {
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::generatorComprehension(uint32_t begin) {
    MOZ_ASSERT(tokenStream.isCurrentTokenType(TOK_FOR));
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::assignExprWithoutYieldOrAwait(
    YieldHandling yieldHandling) {
    uint32_t startYieldOffset = pc->lastYieldOffset;
    uint32_t startAwaitOffset = pc->lastAwaitOffset;
...
template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::argumentList(YieldHandling yieldHandling, Node listNode,
                                               bool* isSpread,
                                               PossibleError* possibleError /* = nullptr */) {
    bool matched;
    if (!tokenStream.matchToken(&matched, TOK_RP, TokenStream::Operand)) return false;
    ...
    while (true) {
        bool spread = false;
        uint32_t begin = 0;
        if (!tokenStream.matchToken(&matched, TOK_TRIPLEDOT, TokenStream::Operand)) return false;
        if (matched) {
            spread = true;
            begin = pos().begin;
            *isSpread = true;
...
template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::checkAndMarkSuperScope() {
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::memberExpr(
    YieldHandling yieldHandling, TripledotHandling tripledotHandling, TokenKind tt,
    ...
    if (tt == TOK_NEW) {
        ...
        if (newTarget) {
            ...
            bool matched;
            if (!tokenStream.matchToken(&matched, TOK_LP)) return null();
            if (matched) {
                bool isSpread = false;
    ...
    while (true) {
        ...
        if (tt == TOK_DOT) {
            ...
            if (!nextMember) return null();
        } else if ((allowCallSyntax && tt == TOK_LP) || tt == TOK_TEMPLATE_HEAD ||
            ...
            if (handler.isSuperBase(lhs)) {
                ...
                nextMember = tt == TOK_LP ? handler.newCall(nextMemberPos)
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::newName(PropertyName* name) {
...
template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::checkLabelOrIdentifierReference(
    ...
    if (TokenKindIsContextualKeyword(tt)) {
        if (tt == TOK_YIELD) {
            ...
            if (pc->sc()->needStrictChecks()) {
                if (!strictModeErrorAt(offset, JSMSG_RESERVED_ID, "yield")) return false;
            }
            return true;
        }
        if (tt == TOK_AWAIT) {
        ...
        if (pc->sc()->needStrictChecks()) {
            ...
            if (tt == TOK_STATIC) {
                if (!strictModeErrorAt(offset, JSMSG_RESERVED_ID, "static")) return false;
                return true;
            }
        }
        return true;
...
template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::checkBindingIdentifier(PropertyName* ident, uint32_t offset,
                                                         YieldHandling yieldHandling,
                                                         TokenKind hint /* = TOK_LIMIT */) {
    if (pc->sc()->needStrictChecks()) {
        if (ident == context->names().arguments) {
            if (!strictModeErrorAt(offset, JSMSG_BAD_STRICT_ASSIGN, "arguments")) return false;
            return true;
        }

        if (ident == context->names().eval) {
            if (!strictModeErrorAt(offset, JSMSG_BAD_STRICT_ASSIGN, "eval")) return false;
            return true;
        }
    }
...
template <class ParseHandler, typename CharT>
PropertyName* Parser<ParseHandler, CharT>::labelOrIdentifierReference(
    YieldHandling yieldHandling) {
    ...
    TokenKind hint =
        !tokenStream.currentNameHasEscapes() ? tokenStream.currentToken().type : TOK_LIMIT;
    RootedPropertyName ident(context, tokenStream.currentName());
    if (!checkLabelOrIdentifierReference(ident, pos().begin, yieldHandling, hint)) return nullptr;
    return ident;
}

template <class ParseHandler, typename CharT>
PropertyName* Parser<ParseHandler, CharT>::bindingIdentifier(YieldHandling yieldHandling) {
    TokenKind hint =
        !tokenStream.currentNameHasEscapes() ? tokenStream.currentToken().type : TOK_LIMIT;
    RootedPropertyName ident(context, tokenStream.currentName());
    if (!checkBindingIdentifier(ident, pos().begin, yieldHandling, hint)) return nullptr;
    return ident;
}

template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::identifierReference(
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::stringLiteral() {
    return handler.newStringLiteral(tokenStream.currentToken().atom(), pos());
}

template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::noSubstitutionTaggedTemplate() {
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::noSubstitutionUntaggedTemplate() {
...
template <>
ParseNode* Parser<FullParseHandler, char16_t>::newRegExp() {
...
template <>
SyntaxParseHandler::Node Parser<SyntaxParseHandler, char16_t>::newRegExp() {
    MOZ_ASSERT(!options().selfHostingMode);
...
template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::checkDestructuringAssignmentTarget(
    Node expr, TokenPos exprPos, PossibleError* exprPossibleError, PossibleError* possibleError,
...
template <class ParseHandler, typename CharT>
void Parser<ParseHandler, CharT>::checkDestructuringAssignmentName(Node name, TokenPos namePos,
...
template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::checkDestructuringAssignmentElement(
    Node expr, TokenPos exprPos, PossibleError* exprPossibleError, PossibleError* possibleError) {
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::arrayInitializer(
    ...
    if (tt == TOK_FOR) return arrayComprehension(begin);
    ...
    if (tt == TOK_RB) {
        ...
        for (;; index++) {
            ...
            if (tt == TOK_COMMA) {
                ...
                TokenPos innerPos;
                if (!tokenStream.peekTokenPos(&innerPos, TokenStream::Operand)) return null();
                ...
                TokenPos elementPos;
                if (!tokenStream.peekTokenPos(&elementPos, TokenStream::Operand)) return null();
                ...
                if (foldConstants && !FoldConstants(context, &element, this)) return null();
                handler.addArrayElement(literal, element);
            }
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::propertyName(
    YieldHandling yieldHandling, const Maybe<DeclarationKind>& maybeDecl, Node propList,
    ...
    if (TokenKindIsPossibleIdentifierName(ltok) &&
        ...
        tokenStream.ungetToken();
        *propType =
            tt == TOK_ASSIGN ? PropertyType::CoverInitializedName : PropertyType::Shorthand;
        return propName;
    }
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::computedPropertyName(
    YieldHandling yieldHandling, const Maybe<DeclarationKind>& maybeDecl, Node literal) {
    uint32_t begin = pos().begin;
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::objectLiteral(
    YieldHandling yieldHandling, PossibleError* possibleError) {
    MOZ_ASSERT(tokenStream.isCurrentTokenType(TOK_LC));
    ...
    for (;;) {
        ...
        if (tt == TOK_TRIPLEDOT) {
            ...
            if (propType == PropertyType::Normal) {
                TokenPos exprPos;
                if (!tokenStream.peekTokenPos(&exprPos, TokenStream::Operand)) return null();
                ...
                if (foldConstants && !FoldConstants(context, &propExpr, this)) return null();
                ...
                if (propAtom == context->names().proto) {
                    ...
                    if (!handler.isConstant(propExpr)) handler.setListFlag(literal, PNX_NONCONST);

                    if (!handler.addPropertyDefinition(literal, propName, propExpr)) return null();
                }
            } else if (propType == PropertyType::Shorthand) {
                /*
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::methodDefinition(uint32_t toStringStart,
                                                                          PropertyType propType,
                                                                          HandleAtom funName) {
    FunctionSyntaxKind kind;
    switch (propType) {
...
template <class ParseHandler, typename CharT>
bool Parser<ParseHandler, CharT>::tryNewTarget(Node& newTarget) {
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::primaryExpr(
    YieldHandling yieldHandling, TripledotHandling tripledotHandling, TokenKind tt,
    PossibleError* possibleError, InvokedPrediction invoked /* = PredictUninvoked */) {
    MOZ_ASSERT(tokenStream.isCurrentTokenType(tt));
    if (!CheckRecursionLimit(context)) return null();
...
template <class ParseHandler, typename CharT>
typename ParseHandler::Node Parser<ParseHandler, CharT>::exprInParens(
    InHandling inHandling, YieldHandling yieldHandling, TripledotHandling tripledotHandling,
    PossibleError* possibleError /* = nullptr */) {
    MOZ_ASSERT(tokenStream.isCurrentTokenType(TOK_LP));
...
bool ParserBase::warnOnceAboutExprClosure() {
...
bool ParserBase::warnOnceAboutForEach() {
    if (context->helperThread()) return true;

    if (!context->compartment()->warnedAboutForEach) {
        if (!warning(JSMSG_DEPRECATED_FOR_EACH)) return false;
        context->compartment()->warnedAboutForEach = true;
    }
    return true;
}

bool ParserBase::warnOnceAboutLegacyGenerator() {
    if (context->helperThread()) return true;

    if (!context->compartment()->warnedAboutLegacyGenerator) {
        if (!warning(JSMSG_DEPRECATED_LEGACY_GENERATOR)) return false;
