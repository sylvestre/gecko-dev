...
#define RESERVED_WORD_INFO(word, name, type) {js_##word##_str, js::frontend::type},
...
static const ReservedWordInfo* FindReservedWord(JSLinearString* str) {
    ...
    return str->hasLatin1Chars() ? FindReservedWord(str->latin1Chars(nogc), str->length())
...
bool IsIdentifier(const char* chars, size_t length) { return ::IsIdentifier(chars, length); }

bool IsIdentifier(const char16_t* chars, size_t length) { return ::IsIdentifier(chars, length); }
...
PropertyName* TokenStreamAnyChars::reservedWordToPropertyName(TokenKind tt) const {
    MOZ_ASSERT(tt != TOK_NAME);
    switch (tt) {
#define EMIT_CASE(word, name, type) \
    case type:                      \
...
MOZ_ALWAYS_INLINE bool TokenStream::SourceCoords::add(uint32_t lineNum, uint32_t lineStartOffset) {
...
MOZ_ALWAYS_INLINE bool TokenStreamAnyChars::SourceCoords::fill(
    ...
    if (lineStartOffsets_.length() >= other.lineStartOffsets_.length()) return true;
...
MOZ_ALWAYS_INLINE uint32_t TokenStreamAnyChars::SourceCoords::lineIndexOf(uint32_t offset) const {
...
void TokenStreamAnyChars::SourceCoords::lineNumAndColumnIndex(uint32_t offset, uint32_t* lineNum,
...
TokenStream::TokenStream(JSContext* cx, const ReadOnlyCompileOptions& options, const CharT* base,
...
MOZ_MUST_USE MOZ_ALWAYS_INLINE bool TokenStream::updateLineInfoForEOL() {
...
MOZ_ALWAYS_INLINE void TokenStreamAnyChars::updateFlagsForEOL() { flags.isDirtyLine = false; }

// This gets the next char, normalizing all EOL sequences to '\n' as it goes.
bool TokenStream::getChar(int32_t* cp) {
    ...
    do {
        ...
        if (MOZ_UNLIKELY(c == unicode::LINE_SEPARATOR || c == unicode::PARA_SEPARATOR)) break;
...
int32_t TokenStream::getCharIgnoreEOL() {
...
void TokenStream::ungetChar(int32_t c) {
    if (c == EOF) return;
...
void TokenStream::ungetCharIgnoreEOL(int32_t c) {
    if (c == EOF) return;
    MOZ_ASSERT(!userbuf.atStart());
    userbuf.ungetRawChar();
}
...
bool TokenStream::peekChars(int n, char16_t* cp) {
    int i, j;
    int32_t c;
    ...
    for (j = i - 1; j >= 0; j--) ungetCharIgnoreEOL(cp[j]);
    return i == n;
}

size_t TokenStream::TokenBuf::findEOLMax(size_t start, size_t max) {
...
bool TokenStream::advance(size_t position) {
...
void TokenStream::tell(Position* pos) {
...
void TokenStream::seek(const Position& pos) {
...
bool TokenStream::seek(const Position& pos, const TokenStream& other) {
    if (!srcCoords.fill(other.srcCoords)) return false;
    seek(pos);
    return true;
}

bool TokenStream::reportStrictModeErrorNumberVA(UniquePtr<JSErrorNotes> notes, uint32_t offset,
                                                bool strictMode, unsigned errorNumber,
                                                va_list args) {
    if (!strictMode && !options().extraWarningsOption) return true;
...
bool TokenStreamAnyChars::compileWarning(ErrorMetadata&& metadata, UniquePtr<JSErrorNotes> notes,
...
bool TokenStreamAnyChars::fillExcludingContext(ErrorMetadata* err, uint32_t offset) {
    err->isMuted = mutedErrors;
...
void TokenStreamAnyChars::lineNumAndColumnIndex(size_t offset, uint32_t* line,
                                                uint32_t* column) const {
    srcCoords.lineNumAndColumnIndex(offset, line, column);
}

size_t TokenStreamAnyChars::offset() const {
    // Default implementation. Overridden by `TokenStream`.
    return 0;
}

bool TokenStream::computeErrorMetadata(ErrorMetadata* err, uint32_t offset) {
    if (offset == NoOffset) {
    ...
    if (!fillExcludingContext(err, offset)) return true;

    // Add a line of context from this TokenStream to help with debugging.
    return computeLineOfContext(err, offset);
}

bool TokenStream::computeLineOfContext(ErrorMetadata* err, uint32_t offset) {
    ...
    if (err->lineNumber != lineno) return true;
    ...
    size_t windowStart = (offset - linebase > windowRadius) ? offset - windowRadius : linebase;
...
size_t TokenStream::offset() const { return userbuf.offset(); }

bool TokenStream::reportStrictModeError(unsigned errorNumber, ...) {
    va_list args;
...
void TokenStream::reportError(unsigned errorNumber, ...) {
    va_list args;
    va_start(args, errorNumber);
...
bool TokenStream::warning(unsigned errorNumber, ...) {
    va_list args;
    va_start(args, errorNumber);
...
bool TokenStream::reportExtraWarningErrorNumberVA(UniquePtr<JSErrorNotes> notes, uint32_t offset,
                                                  unsigned errorNumber, va_list args) {
    if (!options().extraWarningsOption) return true;
...
void TokenStream::error(unsigned errorNumber, ...) {
    va_list args;
    va_start(args, errorNumber);
...
void TokenStream::errorAt(uint32_t offset, unsigned errorNumber, ...) {
    va_list args;
    va_start(args, errorNumber);
...
uint32_t TokenStream::peekUnicodeEscape(uint32_t* codePoint) {
    ...
    uint32_t length;
    c = getCharIgnoreEOL();
    if (JS7_ISHEX(c) && peekChars(3, cp) && JS7_ISHEX(cp[0]) && JS7_ISHEX(cp[1]) &&
        JS7_ISHEX(cp[2])) {
        *codePoint = (JS7_UNHEX(c) << 12) | (JS7_UNHEX(cp[0]) << 8) | (JS7_UNHEX(cp[1]) << 4) |
                     JS7_UNHEX(cp[2]);
        length = 5;
    } else if (c == '{') {
...
uint32_t TokenStream::peekExtendedUnicodeEscape(uint32_t* codePoint) {
...
uint32_t TokenStream::matchUnicodeEscapeIdStart(uint32_t* codePoint) {
...
bool TokenStream::matchUnicodeEscapeIdent(uint32_t* codePoint) {
    uint32_t length = peekUnicodeEscape(codePoint);
...
bool TokenStream::getDirectives(bool isMultiline, bool shouldWarnDeprecated) {
    ...
    if (!getDisplayURL(isMultiline, shouldWarnDeprecated)) return false;
    if (!getSourceMappingURL(isMultiline, shouldWarnDeprecated)) return false;

    return true;
}

bool TokenStream::getDirective(bool isMultiline, bool shouldWarnDeprecated, const char* directive,
...
bool TokenStream::getDisplayURL(bool isMultiline, bool shouldWarnDeprecated) {
    ...
    return getDirective(isMultiline, shouldWarnDeprecated, sourceURLDirective,
                        sourceURLDirectiveLength, "sourceURL", &displayURL_);
}

bool TokenStream::getSourceMappingURL(bool isMultiline, bool shouldWarnDeprecated) {
    ...
    return getDirective(isMultiline, shouldWarnDeprecated, sourceMappingURLDirective,
                        sourceMappingURLDirectiveLength, "sourceMappingURL", &sourceMapURL_);
}

MOZ_ALWAYS_INLINE Token* TokenStream::newToken(ptrdiff_t adjust) {
...
MOZ_ALWAYS_INLINE JSAtom* TokenStream::atomize(JSContext* cx, CharBuffer& cb) {
...
static bool IsTokenSane(Token* tp) {
    ...
    if (tp->type >= TOK_LIMIT || tp->type == TOK_EOL) return false;
...
bool TokenStream::matchTrailForLeadSurrogate(char16_t lead, char16_t* trail, uint32_t* codePoint) {
    ...
    if (trail) *trail = maybeTrail;
    *codePoint = unicode::UTF16Decode(lead, maybeTrail);
    return true;
}

bool TokenStream::putIdentInTokenbuf(const char16_t* identStart) {
    int32_t c;
    uint32_t qc;
...
bool TokenStream::getTokenInternal(TokenKind* ttp, Modifier modifier) {
    int c;
    uint32_t qc;
    Token* tp;
...
retry:
    ...
    //
    if (c1kind == String) {
        if (!getStringOrTemplateToken(c, &tp)) goto error;
        goto out;
    }
    ...
    switch (c) {
        ...
        case '/':
            ...
            if (matchChar('*')) {
                ...
                if (linenoBefore != lineno) updateFlagsForEOL();
                cursor = (cursor - 1) & ntokensMask;
                goto retry;
            }
...
bool TokenStream::getStringOrTemplateToken(int untilChar, Token** tp) {
    int c;
    int nc = -1;
                    ...
                    if (c2 == '{') {
                        ...
                        do {
                            ...
                            if (c == '}') {
                                if (first) {
                                    if (parsingTemplate) {
                                        setInvalidTemplateEscape(start,
                            ...
                            if (code > unicode::NonBMPMax) {
                                if (parsingTemplate) {
                                    setInvalidTemplateEscape(start + 3,
                ...
                default:
                    ...
                    if (JS7_ISOCT(c)) {
                        ...
                        if (val != 0 || JS7_ISDEC(c)) {
                            if (parsingTemplate) {
                                setInvalidTemplateEscape(userbuf.offset() - 2,
