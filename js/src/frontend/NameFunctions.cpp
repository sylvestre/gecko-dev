...
class NameResolver {
    ...
    bool call(ParseNode* pn) { return pn && pn->isKind(PNK_CALL); }
    ...
    bool nameExpression(ParseNode* n, bool* foundName) {
        switch (n->getKind()) {
            ...
            case PNK_ELEM:
                ...
                if (!*foundName) return true;
                if (!buf->append('[') || !nameExpression(n->pn_right, foundName)) return false;
                if (!*foundName) return true;
                return buf->append(']');
    ...
    bool resolveFun(ParseNode* pn, HandleAtom prefix, MutableHandleAtom retAtom) {
        ...
        if (fun->displayAtom() != nullptr) {
            ...
            if (!buf.append(prefix) || !buf.append('/') || !buf.append(fun->displayAtom()))
                return false;
        ...
        if (prefix != nullptr && (!buf.append(prefix) || !buf.append('/'))) return false;
         ...
         */
        if (!buf.empty() && buf.getChar(buf.length() - 1) == '/' && !buf.append('<')) return false;
   ...
   public:
    ...
    bool resolve(ParseNode* cur, HandleAtom prefixArg = nullptr) {
        ...
        MOZ_ASSERT((cur->isKind(PNK_FUNCTION) || cur->isKind(PNK_MODULE)) ==
                   cur->isArity(PN_CODE));
        ...
        switch (cur->getKind()) {
            ...
            case PNK_EXPORT_DEFAULT:
                ...
                // it.  So recur anyway.
                if (!resolve(cur->pn_left, prefix)) return false;
                MOZ_ASSERT_IF(!cur->isKind(PNK_EXPORT_DEFAULT), cur->pn_right->isKind(PNK_STRING));
                break;
            ...
            case PNK_TRY:
                ...
                if (ParseNode* catchList = cur->pn_kid2) {
                    ...
                    if (!resolve(catchList, prefix)) return false;
                }
            ...
            case PNK_ARRAYCOMP:
                ...
                if (!resolve(cur->pn_head, prefix)) return false;
                break;
            ...
            case PNK_CATCHLIST: {
                MOZ_ASSERT(cur->isArity(PN_LIST));
                for (ParseNode* catchNode = cur->pn_head; catchNode;
                     catchNode = catchNode->pn_next) {
                    ...
                    if (!resolve(catchNode->scopeBody(), prefix)) return false;
                }
                break;
            }
