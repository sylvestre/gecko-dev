...
class SourceParseContext : public ParseContext {
   public:
    ...
    SourceParseContext(Parser<ParseHandler, CharT>* prs, SharedContext* sc,
...
inline ParseContext::Scope::BindingIter ParseContext::Scope::bindings(ParseContext* pc) {
...
class ParserBase : public StrictModeGetter {
   ...
   public:
    ...
    bool yieldExpressionsSupported() {
        return (versionNumber() >= JSVERSION_1_7 && !pc->isAsync()) || pc->isStarGenerator() ||
    ...
    const ReadOnlyCompileOptions& options() const { return tokenStream.options(); }
    ...
    void errorWithNotesAt(UniquePtr<JSErrorNotes> notes, uint32_t offset, unsigned errorNumber,
                          ...);
...
inline ParseContext::Scope::Scope(JSContext* cx, ParseContext* pc, UsedNameTracker& usedNames)
...
class Parser final : public ParserBase, private JS::AutoGCRooter {
   ...
   private:
     ...
     */
    Node functionStmt(uint32_t toStringStart, YieldHandling yieldHandling,
                      DefaultHandling defaultHandling, FunctionAsyncKind asyncKind = SyncFunction);
     ...
     */
    bool functionArguments(YieldHandling yieldHandling, FunctionSyntaxKind kind, Node funcpn);
    ...
    bool argumentList(YieldHandling yieldHandling, Node listNode, bool* isSpread,
                      PossibleError* possibleError = nullptr);
    Node destructuringDeclaration(DeclarationKind kind, YieldHandling yieldHandling, TokenKind tt);
    ...
    bool checkBindingIdentifier(PropertyName* ident, uint32_t offset, YieldHandling yieldHandling,
   ...
   public:
    enum FunctionCallBehavior { PermitAssignmentToFunctionCalls, ForbidAssignmentToFunctionCalls };
   ...
   private:
    ...
    enum class TargetBehavior { PermitAssignmentPattern, ForbidAssignmentPattern };
...
class MOZ_STACK_CLASS AutoAwaitIsKeyword {
   ...
   public:
    AutoAwaitIsKeyword(Parser* parser, AwaitHandling awaitHandling) {
        ...
        if (oldAwaitHandling_ != AwaitIsModuleKeyword) parser_->setAwaitHandling(awaitHandling);
    }
