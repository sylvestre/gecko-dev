...
class MOZ_STACK_CLASS BytecodeCompiler {
   public:
    ...
    BytecodeCompiler(JSContext* cx, LifoAlloc& alloc, const ReadOnlyCompileOptions& options,
                     SourceBufferHolder& sourceBuffer, HandleScope enclosingScope);
...
AutoFrontendTraceLog::AutoFrontendTraceLog(JSContext* cx, const TraceLoggerTextId id,
                                           const ErrorReporter& errorReporter, FunctionBox* funbox)
#ifdef JS_TRACE_LOGGING
    : logger_(TraceLoggerForCurrentThread(cx)) {
    frontendEvent_.emplace(TraceLogger_Frontend, errorReporter.getFilename(), funbox->startLine,
                           funbox->startColumn);
    frontendLog_.emplace(logger_, *frontendEvent_);
    typeLog_.emplace(logger_, id);
}
...
bool BytecodeCompiler::createScriptSource(const Maybe<uint32_t>& parameterListEnd) {
...
bool BytecodeCompiler::canLazilyParse() {
    return options.canLazilyParse && !cx->compartment()->behaviors().disableLazyParsing() &&
           !cx->compartment()->behaviors().discardSource() && !options.sourceIsLazy &&
           !cx->lcovEnabled();
}
...
bool BytecodeCompiler::createScript() { return createScript(0, sourceBuffer.length()); }

bool BytecodeCompiler::createScript(uint32_t toStringStart, uint32_t toStringEnd) {
...
bool BytecodeCompiler::deoptimizeArgumentsInEnclosingScripts(JSContext* cx,
...
JSScript* BytecodeCompiler::compileScript(HandleObject environment, SharedContext* sc) {
    ...
    for (;;) {
        ...
        if (pn) {
            if (sc->isEvalContext() && sc->hasDebuggerStatement() && !cx->helperThread()) {
                ...
                // scope chain.
                if (!deoptimizeArgumentsInEnclosingScripts(cx, environment)) return nullptr;
            }
...
// constructor.
bool BytecodeCompiler::compileStandaloneFunction(MutableHandleFunction fun,
    ...
    if (fn->pn_funbox->function()->isInterpreted()) {
        ...
        if (!createScript(fn->pn_funbox->toStringStart, fn->pn_funbox->toStringEnd)) return false;
...
ScriptSourceObject* BytecodeCompiler::sourceObjectPtr() const { return sourceObject.get(); }
...
JSScript* frontend::CompileGlobalScript(JSContext* cx, LifoAlloc& alloc, ScopeKind scopeKind,
                                        const ReadOnlyCompileOptions& options,
...
JSScript* frontend::CompileEvalScript(JSContext* cx, LifoAlloc& alloc, HandleObject environment,
                                      HandleScope enclosingScope,
                                      const ReadOnlyCompileOptions& options,
                                      SourceBufferHolder& srcBuf,
                                      ScriptSourceObject** sourceObjectOut) {
...
ModuleObject* frontend::CompileModule(JSContext* cx, const ReadOnlyCompileOptions& optionsInput,
...
ModuleObject* frontend::CompileModule(JSContext* cx, const ReadOnlyCompileOptions& options,
                                      SourceBufferHolder& srcBuf) {
    if (!GlobalObject::ensureModulePrototypesCreated(cx, cx->global())) return nullptr;
...
bool frontend::CompileLazyFunction(JSContext* cx, Handle<LazyScript*> lazy, const char16_t* chars,
                                   size_t length) {
    ...
    ParseNode* pn = parser.standaloneLazyFunction(fun, lazy->toStringStart(), lazy->strict(),
    ...
    Rooted<JSScript*> script(
        cx, JSScript::Create(cx, options, sourceObject, lazy->begin(), lazy->end(),
    ...
    BytecodeEmitter bce(/* parent = */ nullptr, &parser, pn->pn_funbox, script, lazy, pn->pn_pos,
...
bool frontend::CompileStandaloneFunction(JSContext* cx, MutableHandleFunction fun,
                                         const ReadOnlyCompileOptions& options,
...
bool frontend::CompileStandaloneGenerator(JSContext* cx, MutableHandleFunction fun,
                                          const ReadOnlyCompileOptions& options,
                                          JS::SourceBufferHolder& srcBuf,
                                          const Maybe<uint32_t>& parameterListEnd) {
    RootedScope emptyGlobalScope(cx, &cx->global()->emptyGlobalScope());
...
bool frontend::CompileStandaloneAsyncFunction(JSContext* cx, MutableHandleFunction fun,
                                              const ReadOnlyCompileOptions& options,
                                              JS::SourceBufferHolder& srcBuf,
                                              const Maybe<uint32_t>& parameterListEnd) {
    RootedScope emptyGlobalScope(cx, &cx->global()->emptyGlobalScope());
...
bool frontend::CompileStandaloneAsyncGenerator(JSContext* cx, MutableHandleFunction fun,
                                               const ReadOnlyCompileOptions& options,
                                               JS::SourceBufferHolder& srcBuf,
                                               const Maybe<uint32_t>& parameterListEnd) {
    RootedScope emptyGlobalScope(cx, &cx->global()->emptyGlobalScope());
