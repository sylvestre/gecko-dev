...
class EitherParser {
   ...
   public:
    TokenStream& tokenStream() { return parser.match(TokenStreamMatcher()); }

    const TokenStream& tokenStream() const { return parser.match(TokenStreamMatcher()); }
   ...
   public:
    ScriptSource* ss() { return parser.match(ScriptSourceMatcher()); }
   ...
   public:
    const JS::ReadOnlyCompileOptions& options() { return parser.match(OptionsMatcher()); }
   ...
   public:
    ObjectBox* newObjectBox(JSObject* obj) { return parser.match(NewObjectBoxMatcher(obj)); }
   ...
   public:
    bool isDeclarationList(Node node) { return parser.match(IsDeclarationListMatcher(node)); }
   ...
   public:
    bool isSuperBase(Node node) { return parser.match(IsSuperBaseMatcher(node)); }

   private:
    template <typename Function, class This, typename... Args, size_t... Indices>
    static auto CallGenericFunction(Function func, This* obj, mozilla::Tuple<Args...>& args,
    ...
    struct ReportErrorMatcher {
        ...
        void match(Parser* parser) {
            return CallGenericFunction(&TokenStream::reportError, &parser->tokenStream, args,
   ...
   public:
    ...
    void reportError(Args&&... args) {
        ReportErrorMatcher<typename mozilla::Decay<Args>::Type...> matcher{
