 ...
 */
extern MOZ_MUST_USE bool JS_FASTCALL NumberValueToStringBuffer(JSContext* cx, const Value& v,
                                                               StringBuffer& sb);
 ...
 * NumberToCString().
 */
extern char* NumberToCString(JSContext* cx, ToCStringBuf* cbuf, double d, int base = 10);
 ...
 */
template <typename CharT>
extern MOZ_MUST_USE bool GetPrefixInteger(JSContext* cx, const CharT* start, const CharT* end,
...
extern MOZ_MUST_USE bool StringToNumber(JSContext* cx, JSString* str, double* result);

/* ES5 9.3 ToNumber, overwriting *vp with the appropriate number value. */
MOZ_ALWAYS_INLINE MOZ_MUST_USE bool ToNumber(JSContext* cx, JS::MutableHandleValue vp) {
 ...
 */
template <typename CharT>
extern MOZ_MUST_USE bool js_strtod(JSContext* cx, const CharT* begin, const CharT* end,
 ...
 */
static MOZ_ALWAYS_INLINE bool IsDefinitelyIndex(const Value& v, uint32_t* indexp) {
...
/* ES5 9.4 ToInteger. */
static MOZ_MUST_USE inline bool ToInteger(JSContext* cx, HandleValue v, double* dp) {
    if (v.isInt32()) {
        *dp = v.toInt32();
        return true;
    }
 ...
 */
MOZ_MUST_USE bool ToIndex(JSContext* cx, JS::HandleValue v, const unsigned errorNumber,
                          uint64_t* index);

static MOZ_MUST_USE inline bool ToIndex(JSContext* cx, JS::HandleValue v, uint64_t* index) {
