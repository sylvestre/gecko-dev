...
#define CTYPES_FOR_EACH_INT_TYPE(macro)                                                           \
    macro(int8_t, int8_t, ffi_type_sint8) macro(int16_t, int16_t, ffi_type_sint16)                \
        macro(int32_t, int32_t, ffi_type_sint32) macro(uint8_t, uint8_t, ffi_type_uint8)          \
            macro(uint16_t, uint16_t, ffi_type_uint16) macro(uint32_t, uint32_t, ffi_type_uint32) \
...
#define CTYPES_FOR_EACH_WRAPPED_INT_TYPE(macro)                                                  \
    macro(int64_t, int64_t, ffi_type_sint64) macro(uint64_t, uint64_t, ffi_type_uint64)          \
        macro(long, long, CTYPES_FFI_LONG) macro(unsigned_long, unsigned long, CTYPES_FFI_ULONG) \
...
#define CTYPES_FOR_EACH_FLOAT_TYPE(macro)                                             \
    macro(float32_t, float, ffi_type_float) macro(float64_t, double, ffi_type_double) \
        macro(float, float, ffi_type_float) macro(double, double, ffi_type_double)

#define CTYPES_FOR_EACH_CHAR_TYPE(macro)                                              \
    macro(char, char, ffi_type_uint8) macro(signed_char, signed char, ffi_type_sint8) \
        macro(unsigned_char, unsigned char, ffi_type_uint8)

#define CTYPES_FOR_EACH_CHAR16_TYPE(macro) macro(char16_t, char16_t, ffi_type_uint16)
