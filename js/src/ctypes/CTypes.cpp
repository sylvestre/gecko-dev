...
size_t GetDeflatedUTF8StringLength(JSContext* maybecx, const CharT* chars, size_t nchars) {
    size_t nbytes;
    const CharT* end;
    unsigned c, c2;
    ...
    for (end = chars + nchars; chars != end; chars++) {
        ...
        if (0xD800 <= c && c <= 0xDFFF) {
            ...
            nbytes--;
            if (c >= 0xDC00 || chars == end) goto bad_surrogate;
            c2 = *chars;
            if (c2 < 0xDC00 || c2 > 0xDFFF) goto bad_surrogate;
...
bad_surrogate:
    if (maybecx) {
        ...
        JS_ReportErrorFlagsAndNumberASCII(maybecx, JSREPORT_ERROR, GetErrorMessage, nullptr,
                                          JSMSG_BAD_SURROGATE_CHAR, buffer);
    }
    return (size_t)-1;
}

template size_t GetDeflatedUTF8StringLength(JSContext* maybecx, const Latin1Char* chars,
                                            size_t nchars);

template size_t GetDeflatedUTF8StringLength(JSContext* maybecx, const char16_t* chars,
                                            size_t nchars);

static size_t GetDeflatedUTF8StringLength(JSContext* maybecx, JSLinearString* str) {
...
template <typename CharT>
bool DeflateStringToUTF8Buffer(JSContext* maybecx, const CharT* src, size_t srclen, char* dst,
                               size_t* dstlenp) {
    size_t i, utf8Len;
    char16_t c, c2;
    uint32_t v;
    ...
    while (srclen) {
        c = *src++;
        srclen--;
        if (c >= 0xDC00 && c <= 0xDFFF) goto badSurrogate;
        if (c < 0xD800 || c > 0xDBFF) {
            v = c;
        } else {
            if (srclen < 1) goto badSurrogate;
            c2 = *src;
            if ((c2 < 0xDC00) || (c2 > 0xDFFF)) goto badSurrogate;
            src++;
            srclen--;
        ...
        if (v < 0x0080) {
            ...
            if (dstlen == 0) goto bufferTooSmall;
            *dst++ = (char)v;
            utf8Len = 1;
        } else {
            utf8Len = js::OneUcs4ToUtf8Char(utf8buf, v);
            if (utf8Len > dstlen) goto bufferTooSmall;
            for (i = 0; i < utf8Len; i++) *dst++ = (char)utf8buf[i];
        }
        dstlen -= utf8Len;
    }
...
badSurrogate:
    *dstlenp = (origDstlen - dstlen);
    ...
    if (maybecx) GetDeflatedUTF8StringLength(maybecx, src - 1, srclen + 1);
    return false;
...
bufferTooSmall:
    *dstlenp = (origDstlen - dstlen);
    if (maybecx) {
        js::gc::AutoSuppressGC suppress(maybecx);
        JS_ReportErrorNumberASCII(maybecx, GetErrorMessage, nullptr, JSMSG_BUFFER_TOO_SMALL);
    }
    return false;
}

template bool DeflateStringToUTF8Buffer(JSContext* maybecx, const Latin1Char* src, size_t srclen,
                                        char* dst, size_t* dstlenp);

template bool DeflateStringToUTF8Buffer(JSContext* maybecx, const char16_t* src, size_t srclen,
                                        char* dst, size_t* dstlenp);

static bool DeflateStringToUTF8Buffer(JSContext* maybecx, JSLinearString* str, char* dst,
                                      size_t* dstlenp) {
    size_t length = str->length();
...
static bool Create(JSContext* cx, unsigned argc, Value* vp);
static bool ConstructData(JSContext* cx, HandleObject typeObj, HandleObject dataObj,
                          HandleObject fnObj, HandleObject thisObj, HandleValue errVal);
...
static bool ToSource(JSContext* cx, unsigned argc, Value* vp);
static JSString* GetSourceString(JSContext* cx, HandleObject typeObj, void* data);
 ...
 */
static void CallFinalizer(CDataFinalizer::Private* p, int* errnoStatus, int32_t* lastErrorStatus);
...
namespace Int64Base {
JSObject* Construct(JSContext* cx, HandleObject proto, uint64_t data, bool isUnsigned);

uint64_t GetInt(JSObject* obj);

bool ToString(JSContext* cx, JSObject* obj, const CallArgs& args, bool isUnsigned);

bool ToSource(JSContext* cx, JSObject* obj, const CallArgs& args, bool isUnsigned);
...
static const JSClass sCTypesGlobalClass = {"ctypes",
                                           JSCLASS_HAS_RESERVED_SLOTS(CTYPESGLOBAL_SLOTS)};

static const JSClass sCABIClass = {"CABI", JSCLASS_HAS_RESERVED_SLOTS(CABI_SLOTS)};
...
static const JSClass sCTypeClass = {
    "CType", JSCLASS_HAS_RESERVED_SLOTS(CTYPE_SLOTS) | JSCLASS_FOREGROUND_FINALIZE,
    &sCTypeClassOps};
...
static const JSClass sCDataClass = {
    "CData", JSCLASS_HAS_RESERVED_SLOTS(CDATA_SLOTS) | JSCLASS_FOREGROUND_FINALIZE,
    &sCDataClassOps};

static const JSClassOps sCClosureClassOps = {
    nullptr, nullptr, nullptr, nullptr,        nullptr, nullptr, CClosure::Finalize,
    nullptr, nullptr, nullptr, CClosure::Trace};
static const JSClass sCClosureClass = {
    "CClosure", JSCLASS_HAS_RESERVED_SLOTS(CCLOSURE_SLOTS) | JSCLASS_FOREGROUND_FINALIZE,
    &sCClosureClassOps};
...
static const JSClassOps sCDataFinalizerClassOps = {
    nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, CDataFinalizer::Finalize};
static const JSClass sCDataFinalizerClass = {"CDataFinalizer",
                                             JSCLASS_HAS_PRIVATE |
...
static const JSPropertySpec sCTypeProps[] = {
    JS_PSG("name", (Property<CType::IsCType, CType::NameGetter>::Fun), CTYPESACC_FLAGS),
    JS_PSG("size", (Property<CType::IsCType, CType::SizeGetter>::Fun), CTYPESACC_FLAGS),
    JS_PSG("ptr", (Property<CType::IsCType, CType::PtrGetter>::Fun), CTYPESACC_FLAGS),
    JS_PSG("prototype", (Property<CType::IsCTypeOrProto, CType::PrototypeGetter>::Fun),
           CTYPESACC_FLAGS),
    JS_PS_END};
...
static const JSFunctionSpec sCABIFunctions[] = {JS_FN("toSource", ABI::ToSource, 0, CABIFN_FLAGS),
...
static const JSPropertySpec sCDataProps[] = {
    JS_PSGS("value", (Property<CData::IsCData, CData::ValueGetter>::Fun),
            (Property<CData::IsCData, CData::ValueSetter>::Fun), JSPROP_PERMANENT),
    JS_PS_END};
...
static const JSPropertySpec sPointerInstanceProps[] = {
    JS_PSGS("contents", (Property<PointerType::IsPointer, PointerType::ContentsGetter>::Fun),
...
static const JSPropertySpec sArrayProps[] = {
    JS_PSG("elementType", (Property<ArrayType::IsArrayType, ArrayType::ElementTypeGetter>::Fun),
           CTYPESACC_FLAGS),
    JS_PSG("length", (Property<ArrayType::IsArrayOrArrayType, ArrayType::LengthGetter>::Fun),
           CTYPESACC_FLAGS),
    JS_PS_END};

static const JSFunctionSpec sArrayInstanceFunctions[] = {
    JS_FN("addressOfElement", ArrayType::AddressOfElement, 1, CDATAFN_FLAGS), JS_FS_END};
...
static const JSPropertySpec sStructProps[] = {
    JS_PSG("fields", (Property<StructType::IsStruct, StructType::FieldsArrayGetter>::Fun),
           CTYPESACC_FLAGS),
    JS_PS_END};
...
static const JSFunctionSpec sStructInstanceFunctions[] = {
    JS_FN("addressOfField", StructType::AddressOfField, 1, CDATAFN_FLAGS), JS_FS_END};
...
static const JSPropertySpec sFunctionProps[] = {
    JS_PSG("argTypes", (Property<FunctionType::IsFunctionType, FunctionType::ArgTypesGetter>::Fun),
           CTYPESACC_FLAGS),
    JS_PSG("returnType",
           (Property<FunctionType::IsFunctionType, FunctionType::ReturnTypeGetter>::Fun),
           CTYPESACC_FLAGS),
    JS_PSG("abi", (Property<FunctionType::IsFunctionType, FunctionType::ABIGetter>::Fun),
           CTYPESACC_FLAGS),
    JS_PSG("isVariadic",
...
static const JSFunctionSpec sFunctionInstanceFunctions[] = {
    JS_FN("call", js::fun_call, 1, CDATAFN_FLAGS), JS_FN("apply", js::fun_apply, 2, CDATAFN_FLAGS),
    JS_FS_END};
...
static const JSClass sInt64Class = {
    "Int64", JSCLASS_HAS_RESERVED_SLOTS(INT64_SLOTS) | JSCLASS_FOREGROUND_FINALIZE,
    &sInt64ClassOps};

static const JSClass sUInt64Class = {
    "UInt64", JSCLASS_HAS_RESERVED_SLOTS(INT64_SLOTS) | JSCLASS_FOREGROUND_FINALIZE,
    &sInt64ClassOps};

static const JSFunctionSpec sInt64StaticFunctions[] = {
    JS_FN("compare", Int64::Compare, 2, CTYPESFN_FLAGS), JS_FN("lo", Int64::Lo, 1, CTYPESFN_FLAGS),
...
static const JSFunctionSpec sUInt64StaticFunctions[] = {
    ...
    JS_FN("lo", UInt64::Lo, 1, CTYPESFN_FLAGS), JS_FN("hi", UInt64::Hi, 1, CTYPESFN_FLAGS),
    // "join" is defined specially; see InitInt64Class.
    JS_FS_END};
...
static const JSPropertySpec sModuleProps[] = {
    JS_PSG("errno", (Property<IsCTypesGlobal, CData::ErrnoGetter>::Fun), JSPROP_PERMANENT),
#if defined(XP_WIN)
    JS_PSG("winLastError", (Property<IsCTypesGlobal, CData::LastErrorGetter>::Fun),
           JSPROP_PERMANENT),
#endif  // defined(XP_WIN)
    JS_PS_END};
...
bool CDataArrayProxyHandler::get(JSContext* cx, HandleObject proxy, HandleValue receiver,
                                 HandleId id, MutableHandleValue vp) const {
    RootedObject target(cx, proxy->as<ProxyObject>().target());
    bool handled = false;
    if (!ArrayType::Getter(cx, target, id, vp, &handled)) return false;
    if (handled) return true;
    return ForwardingProxyHandler::get(cx, proxy, receiver, id, vp);
}

bool CDataArrayProxyHandler::set(JSContext* cx, HandleObject proxy, HandleId id, HandleValue v,
                                 HandleValue receiver, ObjectOpResult& result) const {
    RootedObject target(cx, proxy->as<ProxyObject>().target());
    bool handled = false;
    if (!ArrayType::Setter(cx, target, id, v, result, &handled)) return false;
    if (handled) return true;
...
static MOZ_ALWAYS_INLINE JSString* NewUCString(JSContext* cx, const AutoString& from) {
...
static ABICode GetABICode(JSObject* obj) {
    ...
    if (JS_GetClass(obj) != &sCABIClass) return INVALID_ABI;
...
static const JSErrorFormatString* GetErrorMessage(void* userRef, const unsigned errorNumber) {
    if (0 < errorNumber && errorNumber < CTYPESERR_LIMIT) return &ErrorFormatString[errorNumber];
    return nullptr;
}

static const char* EncodeLatin1(JSContext* cx, AutoString& str, JSAutoByteString& bytes) {
...
static void BuildCStyleTypeSource(JSContext* cx, JSObject* typeObj_, AutoString& source) {
...
#undef BUILD_SOURCE
        ...
        case TYPE_pointer: {
            ...
            if (type == TYPE_function) {
                BuildCStyleFunctionTypeSource(cx, baseTypeObj, nullptr, ptrCount, source);
                break;
            }
...
static void BuildFunctionTypeSource(JSContext* cx, HandleObject funObj, AutoString& source) {
...
enum class ConversionType { Argument = 0, Construct, Finalizer, Return, Setter };

static void BuildConversionPosition(JSContext* cx, ConversionType convType, HandleObject funObj,
                                    unsigned argIndex, AutoString& source) {
    switch (convType) {
...
static void BuildTypeSource(JSContext* cx, JSObject* typeObj_, bool makeShort, AutoString& result);

static bool ConvError(JSContext* cx, const char* expectedStr, HandleValue actual,
                      ConversionType convType, HandleObject funObj = nullptr,
                      unsigned argIndex = 0, HandleObject arrObj = nullptr,
                      unsigned arrIndex = 0) {
        ...
        switch (CType::GetTypeCode(arrObj)) {
            case TYPE_array: {
                ...
                JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr,
                                           CTYPESMSG_CONV_ERROR_ARRAY, valStr, indexStr, arrStr);
                break;
            }
            case TYPE_struct: {
                ...
                JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr,
                                           CTYPESMSG_CONV_ERROR_STRUCT, valStr, nameStr,
                                           expectedStr, structStr, posStr);
                break;
            }
            default:
    ...
    switch (convType) {
        case ConversionType::Argument: {
            ...
            if (!funStr) return false;

            JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, CTYPESMSG_CONV_ERROR_ARG,
        ...
        case ConversionType::Finalizer: {
            ...
            JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, CTYPESMSG_CONV_ERROR_FIN,
                                       valStr, funStr);
            break;
        }
        case ConversionType::Return: {
            ...
            JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, CTYPESMSG_CONV_ERROR_RET,
                                       valStr, funStr);
            break;
        }
        ...
        case ConversionType::Construct:
            ...
            JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, CTYPESMSG_CONV_ERROR_SET,
                                       valStr, expectedStr);
            break;
    }
...
static bool ConvError(JSContext* cx, HandleObject expectedType, HandleValue actual,
                      ConversionType convType, HandleObject funObj = nullptr,
                      unsigned argIndex = 0, HandleObject arrObj = nullptr,
                      unsigned arrIndex = 0) {
    ...
    return ConvError(cx, expectedStr, actual, convType, funObj, argIndex, arrObj, arrIndex);
}

static bool ArgumentConvError(JSContext* cx, HandleValue actual, const char* funStr,
                              unsigned argIndex) {
    JSAutoByteString valBytes;
    const char* valStr = CTypesToSourceForError(cx, actual, valBytes);
    if (!valStr) return false;
    ...
    JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, CTYPESMSG_CONV_ERROR_ARG, valStr,
                               indexStr, funStr);
    return false;
}

static bool ArgumentLengthError(JSContext* cx, const char* fun, const char* count, const char* s) {
    JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, CTYPESMSG_WRONG_ARG_LENGTH, fun,
                               count, s);
    return false;
}

static bool ArrayLengthMismatch(JSContext* cx, unsigned expectedLength, HandleObject arrObj,
    ...
    JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, CTYPESMSG_ARRAY_MISMATCH, valStr,
                               arrStr, expectedLengthStr, actualLengthStr);
    return false;
}

static bool ArrayLengthOverflow(JSContext* cx, unsigned expectedLength, HandleObject arrObj,
                                unsigned actualLength, HandleValue actual,
                                ConversionType convType) {
    MOZ_ASSERT(arrObj && CType::IsCType(arrObj));
    ...
    JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, CTYPESMSG_ARRAY_OVERFLOW, valStr,
                               arrStr, expectedLengthStr, actualLengthStr);
    return false;
}

static bool ArgumentRangeMismatch(JSContext* cx, const char* func, const char* range) {
    JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr, CTYPESMSG_ARG_RANGE_MISMATCH, func,
                              range);
    return false;
}

static bool ArgumentTypeMismatch(JSContext* cx, const char* arg, const char* func,
                                 const char* type) {
    JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr, CTYPESMSG_ARG_TYPE_MISMATCH, arg, func,
                              type);
    return false;
}

static bool CannotConstructError(JSContext* cx, const char* type) {
    JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr, CTYPESMSG_CANNOT_CONSTRUCT, type);
    return false;
}
...
static bool DuplicateFieldError(JSContext* cx, Handle<JSFlatString*> name) {
    ...
    JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, CTYPESMSG_DUPLICATE_FIELD, nameStr);
    return false;
}

static bool EmptyFinalizerCallError(JSContext* cx, const char* funName) {
    JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr, CTYPESMSG_EMPTY_FIN_CALL, funName);
    return false;
}
...
static bool EmptyFinalizerError(JSContext* cx, ConversionType convType,
    ...
    JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, CTYPESMSG_EMPTY_FIN, posStr);
    return false;
}

static bool FieldCountMismatch(JSContext* cx, unsigned expectedCount, HandleObject structObj,
                               unsigned actualCount, HandleValue actual, ConversionType convType,
                               HandleObject funObj = nullptr, unsigned argIndex = 0) {
    ...
    JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, CTYPESMSG_FIELD_MISMATCH, valStr,
                               structStr, expectedCountStr, actualCountStr, posStr);
    return false;
}

static bool FieldDescriptorCountError(JSContext* cx, HandleValue typeVal, size_t length) {
    JSAutoByteString valBytes;
    ...
    JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, CTYPESMSG_FIELD_DESC_COUNT, valStr,
                               lengthStr);
    return false;
}
...
static bool FieldDescriptorNameError(JSContext* cx, HandleId id) {
    ...
    JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, CTYPESMSG_FIELD_DESC_NAME, propStr);
    return false;
}

static bool FieldDescriptorSizeError(JSContext* cx, HandleObject typeObj, HandleId id) {
    ...
    JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, CTYPESMSG_FIELD_DESC_SIZE, typeStr,
                               propStr);
    return false;
}
...
static bool FieldDescriptorNameTypeError(JSContext* cx, HandleValue typeVal) {
    ...
    JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, CTYPESMSG_FIELD_DESC_NAMETYPE,
                               valStr);
    return false;
}

static bool FieldDescriptorTypeError(JSContext* cx, HandleValue poroVal, HandleId id) {
    JSAutoByteString typeBytes;
    ...
    JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, CTYPESMSG_FIELD_DESC_TYPE, typeStr,
                               propStr);
    return false;
}

static bool FieldMissingError(JSContext* cx, JSObject* typeObj, JSFlatString* name_) {
    JSAutoByteString typeBytes;
    ...
    JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, CTYPESMSG_FIELD_MISSING, typeStr,
                               nameStr);
    return false;
}

static bool FinalizerSizeError(JSContext* cx, HandleObject funObj, HandleValue actual) {
    ...
    JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, CTYPESMSG_FIN_SIZE_ERROR, funStr,
                               valStr);
    return false;
}

static bool FunctionArgumentLengthMismatch(JSContext* cx, unsigned expectedCount,
                                           unsigned actualCount, HandleObject funObj,
                                           HandleObject typeObj, bool isVariadic) {
    AutoString funSource;
    JSAutoByteString funBytes;
    Value slot = JS_GetReservedSlot(funObj, SLOT_REFERENT);
    if (!slot.isUndefined() && Library::IsLibrary(&slot.toObject())) {
    ...
    JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, CTYPESMSG_ARG_COUNT_MISMATCH, funStr,
                               expectedCountStr, variadicStr, actualCountStr);
    return false;
}

static bool FunctionArgumentTypeError(JSContext* cx, uint32_t index, HandleValue typeVal,
                                      const char* reason) {
    JSAutoByteString valBytes;
    const char* valStr = CTypesToSourceForError(cx, typeVal, valBytes);
    if (!valStr) return false;
    ...
    JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, CTYPESMSG_ARG_TYPE_ERROR, indexStr,
                               reason, valStr);
    return false;
}

static bool FunctionReturnTypeError(JSContext* cx, HandleValue type, const char* reason) {
    JSAutoByteString valBytes;
    const char* valStr = CTypesToSourceForError(cx, type, valBytes);
    if (!valStr) return false;

    JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, CTYPESMSG_RET_TYPE_ERROR, reason,
                               valStr);
    return false;
}

static bool IncompatibleCallee(JSContext* cx, const char* funName, HandleObject actualObj) {
    JSAutoByteString valBytes;
    RootedValue val(cx, ObjectValue(*actualObj));
    const char* valStr = CTypesToSourceForError(cx, val, valBytes);
    if (!valStr) return false;

    JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, CTYPESMSG_INCOMPATIBLE_CALLEE,
                               funName, valStr);
    return false;
}

static bool IncompatibleThisProto(JSContext* cx, const char* funName, const char* actualType) {
    JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, CTYPESMSG_INCOMPATIBLE_THIS, funName,
                               actualType);
    return false;
}

static bool IncompatibleThisProto(JSContext* cx, const char* funName, HandleValue actualVal) {
    JSAutoByteString valBytes;
    const char* valStr = CTypesToSourceForError(cx, actualVal, valBytes);
    if (!valStr) return false;

    JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, CTYPESMSG_INCOMPATIBLE_THIS_VAL,
                               funName, "incompatible object", valStr);
    return false;
}

static bool IncompatibleThisType(JSContext* cx, const char* funName, const char* actualType) {
    JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr, CTYPESMSG_INCOMPATIBLE_THIS_TYPE,
                              funName, actualType);
    return false;
}
...
static bool InvalidIndexError(JSContext* cx, HandleValue val) {
    ...
    JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, CTYPESMSG_INVALID_INDEX, indexStr);
    return false;
}
...
static bool InvalidIndexRangeError(JSContext* cx, size_t index, size_t length) {
    ...
    JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr, CTYPESMSG_INVALID_RANGE, indexStr,
                              lengthStr);
    return false;
}
...
static bool NonPrimitiveError(JSContext* cx, HandleObject typeObj) {
    ...
    JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, CTYPESMSG_NON_PRIMITIVE, typeStr);
    return false;
}
...
static bool NonStringBaseError(JSContext* cx, HandleValue thisVal) {
    ...
    JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, CTYPESMSG_NON_STRING_BASE, valStr);
    return false;
}

static bool NullPointerError(JSContext* cx, const char* action, HandleObject obj) {
    JSAutoByteString valBytes;
    RootedValue val(cx, ObjectValue(*obj));
    const char* valStr = CTypesToSourceForError(cx, val, valBytes);
    if (!valStr) return false;

    JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, CTYPESMSG_NULL_POINTER, action,
                               valStr);
    return false;
}

static bool PropNameNonStringError(JSContext* cx, HandleId id, HandleValue actual,
                                   ConversionType convType, HandleObject funObj = nullptr,
                                   unsigned argIndex = 0) {
    JSAutoByteString valBytes;
    const char* valStr = CTypesToSourceForError(cx, actual, valBytes);
    ...
    JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, CTYPESMSG_PROP_NONSTRING, propStr,
                               valStr, posStr);
    return false;
}

static bool SizeOverflow(JSContext* cx, const char* name, const char* limit) {
    JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr, CTYPESMSG_SIZE_OVERFLOW, name, limit);
    return false;
}
...
static bool TypeError(JSContext* cx, const char* expected, HandleValue actual) {
    ...
    JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, CTYPESMSG_TYPE_ERROR, expected, src);
    return false;
}

static bool TypeOverflow(JSContext* cx, const char* expected, HandleValue actual) {
    JSAutoByteString valBytes;
    const char* valStr = CTypesToSourceForError(cx, actual, valBytes);
    if (!valStr) return false;

    JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, CTYPESMSG_TYPE_OVERFLOW, valStr,
                               expected);
    return false;
}
...
static bool UndefinedSizeCastError(JSContext* cx, HandleObject targetTypeObj) {
    ...
    if (!targetTypeStr) return false;

    JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, CTYPESMSG_UNDEFINED_SIZE_CAST,
                               targetTypeStr);
    return false;
}
...
static bool SizeMismatchCastError(JSContext* cx, HandleObject sourceTypeObj,
    ...
    JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, CTYPESMSG_SIZE_MISMATCH_CAST,
                               targetTypeStr, sourceTypeStr, targetSizeStr, sourceSizeStr);
    return false;
}

static bool UndefinedSizePointerError(JSContext* cx, const char* action, HandleObject obj) {
    JSAutoByteString valBytes;
    RootedValue val(cx, ObjectValue(*obj));
    const char* valStr = CTypesToSourceForError(cx, val, valBytes);
    if (!valStr) return false;

    JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, CTYPESMSG_UNDEFINED_SIZE, action,
                               valStr);
    return false;
}

static bool VariadicArgumentTypeError(JSContext* cx, uint32_t index, HandleValue actual) {
    JSAutoByteString valBytes;
    const char* valStr = CTypesToSourceForError(cx, actual, valBytes);
    if (!valStr) return false;
    ...
    JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, CTYPESMSG_VARG_TYPE_ERROR, indexStr,
                               valStr);
    return false;
}
...
static JSObject* InitCTypeClass(JSContext* cx, HandleObject ctypesObj) {
    JSFunction* fun =
        JS_DefineFunction(cx, ctypesObj, "CType", ConstructAbstract, 0, CTYPESCTOR_FLAGS);
    if (!fun) return nullptr;

    RootedObject ctor(cx, JS_GetFunctionObject(fun));
    RootedObject fnproto(cx);
    if (!JS_GetPrototype(cx, ctor, &fnproto)) return nullptr;
    MOZ_ASSERT(ctor);
    MOZ_ASSERT(fnproto);
    ...
    if (!JS_FreezeObject(cx, ctor) || !JS_FreezeObject(cx, prototype)) return nullptr;
...
static JSObject* InitABIClass(JSContext* cx) {
    ...
    if (!JS_DefineFunctions(cx, obj, sCABIFunctions)) return nullptr;

    return obj;
}

static JSObject* InitCDataClass(JSContext* cx, HandleObject parent, HandleObject CTypeProto) {
    JSFunction* fun =
        JS_DefineFunction(cx, parent, "CData", ConstructAbstract, 0, CTYPESCTOR_FLAGS);
    if (!fun) return nullptr;
    ...
    // prototype chain.)
    if (!JS_SetPrototype(cx, ctor, CTypeProto)) return nullptr;
...
static bool DefineABIConstant(JSContext* cx, HandleObject ctypesObj, const char* name,
                              ABICode code, HandleObject prototype) {
...
static bool InitTypeConstructor(JSContext* cx, HandleObject parent, HandleObject CTypeProto,
                                ...
                                const JSPropertySpec* instanceProps, MutableHandleObject typeProto,
    ...
    if (fns && !JS_DefineFunctions(cx, typeProto, fns)) return false;

    if (!JS_DefineProperties(cx, typeProto, props)) return false;
    ...
    if (instanceFns && !JS_DefineFunctions(cx, dataProto, instanceFns)) return false;

    if (instanceProps && !JS_DefineProperties(cx, dataProto, instanceProps)) return false;
...
static JSObject* InitInt64Class(JSContext* cx, HandleObject parent, const JSClass* clasp,
                                JSNative construct, const JSFunctionSpec* fs,
                                const JSFunctionSpec* static_fs) {
    // Init type class and constructor
    RootedObject prototype(cx, JS_InitClass(cx, parent, nullptr, clasp, construct, 0, nullptr, fs,
                                            nullptr, static_fs));
    if (!prototype) return nullptr;
    ...
    JSFunction* fun = js::DefineFunctionWithReserved(cx, ctor, "join", native, 2, CTYPESFN_FLAGS);
    if (!fun) return nullptr;

    js::SetFunctionNativeReserved(fun, SLOT_FN_INT64PROTO, ObjectValue(*prototype));

    if (!JS_FreezeObject(cx, ctor)) return nullptr;
    if (!JS_FreezeObject(cx, prototype)) return nullptr;
...
static bool InitTypeClasses(JSContext* cx, HandleObject ctypesObj) {
    ...
    if (!protos.resize(CTYPEPROTO_SLOTS)) return false;
    if (!InitTypeConstructor(cx, ctypesObj, CTypeProto, CDataProto, sPointerFunction, nullptr,
                             sPointerProps, sPointerInstanceFunctions, sPointerInstanceProps,
                             protos[SLOT_POINTERPROTO], protos[SLOT_POINTERDATAPROTO]))
        return false;

    if (!InitTypeConstructor(cx, ctypesObj, CTypeProto, CDataProto, sArrayFunction, nullptr,
                             sArrayProps, sArrayInstanceFunctions, sArrayInstanceProps,
                             protos[SLOT_ARRAYPROTO], protos[SLOT_ARRAYDATAPROTO]))
        return false;

    if (!InitTypeConstructor(cx, ctypesObj, CTypeProto, CDataProto, sStructFunction,
    ...
    if (!InitTypeConstructor(cx, ctypesObj, CTypeProto, protos[SLOT_POINTERDATAPROTO],
                             sFunctionFunction, nullptr, sFunctionProps,
                             sFunctionInstanceFunctions, nullptr, protos[SLOT_FUNCTIONPROTO],
    ...
    protos[SLOT_INT64PROTO].set(InitInt64Class(cx, ctypesObj, &sInt64ProtoClass, Int64::Construct,
                                               sInt64Functions, sInt64StaticFunctions));
    if (!protos[SLOT_INT64PROTO]) return false;
    protos[SLOT_UINT64PROTO].set(InitInt64Class(cx, ctypesObj, &sUInt64ProtoClass,
                                                UInt64::Construct, sUInt64Functions,
                                                sUInt64StaticFunctions));
    if (!protos[SLOT_UINT64PROTO]) return false;
...
#define DEFINE_TYPE(name, type, ffiType)                                                        \
    ...
    {                                                                                           \
        ...
        typeObj_##name = CType::DefineBuiltin(cx, ctypesObj, #name, CTypeProto, CDataProto,     \
...
#undef DEFINE_TYPE
    ...
    RootedObject typeObj(cx, CType::DefineBuiltin(cx, ctypesObj, "void_t", CTypeProto, CDataProto,
...
bool IsCTypesGlobal(JSObject* obj) { return JS_GetClass(obj) == &sCTypesGlobalClass; }

bool IsCTypesGlobal(HandleValue v) { return v.isObject() && IsCTypesGlobal(&v.toObject()); }
...
const JSCTypesCallbacks* GetCallbacks(JSObject* obj) {
    ...
    if (result.isUndefined()) return nullptr;
...
// or is not an object
static bool GetObjectProperty(JSContext* cx, HandleObject obj, const char* property,
...
JS_InitCTypesClass(JSContext* cx, HandleObject global) {
    ...
    if (!InitTypeClasses(cx, ctypes)) return false;
    ...
    if (!GetObjectProperty(cx, ctypes, "CDataFinalizer", &ctor)) return false;

    RootedObject prototype(cx, JS_NewObject(cx, &sCDataFinalizerProtoClass));
    if (!prototype) return false;

    if (!JS_DefineFunctions(cx, prototype, sCDataFinalizerFunctions)) return false;
...
struct ConvertImpl {
    static MOZ_ALWAYS_INLINE TargetType Convert(FromType d) { return TargetType(d); }
};
...
struct ConvertImpl<uint64_t, double> {
    static MOZ_ALWAYS_INLINE uint64_t Convert(double d) {
        return d >= 0xffffffffffffffff ? 0x8000000000000000 : uint64_t(d);
    }
};
...
struct ConvertImpl<int64_t, double> {
    static MOZ_ALWAYS_INLINE int64_t Convert(double d) {
        return d >= 0x7fffffffffffffff ? 0x8000000000000000 : int64_t(d);
    }
};
#endif
...
static MOZ_ALWAYS_INLINE bool IsAlwaysExact() {
    ...
    if (numeric_limits<TargetType>::digits < numeric_limits<FromType>::digits) return false;

    if (numeric_limits<FromType>::is_signed && !numeric_limits<TargetType>::is_signed)
        return false;

    if (!numeric_limits<FromType>::is_exact && numeric_limits<TargetType>::is_exact) return false;
...
static MOZ_ALWAYS_INLINE bool ConvertExact(FromType i, TargetType* result) {
    ...
    *result = Convert<TargetType>(i);
    ...
    if (IsAlwaysExact<TargetType, FromType>()) return true;

    // Return 'true' if 'i' is exactly representable in 'TargetType'.
    return IsExactImpl<TargetType, FromType, numeric_limits<TargetType>::is_signed,
...
static bool jsvalToInteger(JSContext* cx, HandleValue val, IntegerType* result) {
...
static bool StringToInteger(JSContext* cx, CharT* cp, size_t length, IntegerType* result,
                            bool* overflow) {
    JS_STATIC_ASSERT(numeric_limits<IntegerType>::is_exact);
    ...
    IntegerType sign = 1;
    if (cp[0] == '-') {
        if (!numeric_limits<IntegerType>::is_signed) return false;
...
template <class IntegerType>
static bool StringToInteger(JSContext* cx, JSString* string, IntegerType* result, bool* overflow) {
    JSLinearString* linear = string->ensureLinear(cx);
    if (!linear) return false;

    AutoCheckCannotGC nogc;
    size_t length = linear->length();
    return string->hasLatin1Chars() ? StringToInteger<IntegerType>(cx, linear->latin1Chars(nogc),
...
static bool jsvalToSize(JSContext* cx, HandleValue val, bool allowString, size_t* result) {
    bool dummy;
    if (!jsvalToBigInteger(cx, val, allowString, result, &dummy)) return false;
...
// (This is common code shared by jsvalToSize and the Int64/UInt64 constructors.)
template <class IntegerType>
static bool jsidToBigInteger(JSContext* cx, jsid val, bool allowString, IntegerType* result) {
    JS_STATIC_ASSERT(numeric_limits<IntegerType>::is_exact);
...
// Implicitly convert val to a size value, where the size value is represented
// by size_t but must also fit in a double.
static bool jsidToSize(JSContext* cx, jsid val, bool allowString, size_t* result) {
    if (!jsidToBigInteger(cx, val, allowString, result)) return false;
...
static bool jsvalToPtrExplicit(JSContext* cx, HandleValue val, uintptr_t* result) {
    if (val.isInt32()) {
...
void IntegerToString(IntegerType i, int radix, mozilla::Vector<CharType, N, AP>& result) {
    JS_STATIC_ASSERT(numeric_limits<IntegerType>::is_exact);
...
static bool ConvertToJS(JSContext* cx, HandleObject typeObj, HandleObject parentObj, void* data,
                        bool wantPrimitive, bool ownResult, MutableHandleValue result) {
    ...
    case TYPE_##name: {                                                                           \
        ...
        if (!numeric_limits<type>::is_signed) {                                                   \
            ...
            if (!proto) return false;                                                             \
                                                                                                  ...
                                                                                                  \
        JSObject* obj = Int64Base::Construct(cx, proto, value, !numeric_limits<type>::is_signed); \
        if (!obj) return false;                                                                   \
...
static bool ImplicitConvert(JSContext* cx, HandleValue val, JSObject* targetType_, void* buffer,
    ...
    switch (targetCode) {
        case TYPE_bool: {
            ...
            if (!jsvalToBool(cx, val, &result))
                return ConvError(cx, "boolean", val, convType, funObj, argIndex, arrObj, arrIndex);
...
#define CHAR16_CASE(name, type, ffiType)                                                        \
    case TYPE_##name: {                                                                         \
        ...
        if (val.isString()) {                                                                   \
            ...
            if (str->length() != 1)                                                             \
                return ConvError(cx, #name, val, convType, funObj, argIndex, arrObj, arrIndex); \
...
#undef FLOAT_CASE
        case TYPE_pointer: {
            ...
            if (sourceData) {
                ...
                switch (CType::GetTypeCode(baseType)) {
                    ...
                    case TYPE_unsigned_char: {
                        ...
                        if (nbytes == (size_t)-1) return false;
                        ...
                        *charBuffer = cx->pod_malloc<char>(nbytes + 1);
                        ...
                        ASSERT_OK(
                            DeflateStringToUTF8Buffer(cx, sourceLinear, *charBuffer, &nbytes));
                        (*charBuffer)[nbytes] = 0;
                        *freePointer = true;
                        break;
                    }
                    case TYPE_char16_t: {
                        ...
                        *freePointer = true;
                        if (sourceLinear->hasLatin1Chars()) {
                            AutoCheckCannotGC nogc;
                            CopyAndInflateChars(*char16Buffer, sourceLinear->latin1Chars(nogc),
                                                sourceLength);
                        } else {
                            AutoCheckCannotGC nogc;
                            mozilla::PodCopy(*char16Buffer, sourceLinear->twoByteChars(nogc),
                                             sourceLength);
                        }
            ...
            return ConvError(cx, targetType, val, convType, funObj, argIndex, arrObj, arrIndex);
        }
        case TYPE_array: {
            MOZ_ASSERT(!funObj);
            ...
            if (val.isString()) {
                ...
                switch (CType::GetTypeCode(baseType)) {
                    ...
                    case TYPE_unsigned_char: {
                        ...
                        if (targetLength > nbytes) charBuffer[nbytes] = 0;
                    ...
                    case TYPE_char16_t: {
                        ...
                        if (targetLength < sourceLength) {
                            MOZ_ASSERT(!funObj);
                            return ArrayLengthOverflow(cx, targetLength, targetType, sourceLength,
                                                       val, convType);
                        }
                        ...
                        if (sourceLinear->hasLatin1Chars()) {
                            AutoCheckCannotGC nogc;
                            CopyAndInflateChars(dest, sourceLinear->latin1Chars(nogc),
                                                sourceLength);
                        } else {
                            AutoCheckCannotGC nogc;
                        ...
                        if (targetLength > sourceLength) dest[sourceLength] = 0;
                ...
                ESClass cls;
                if (!GetClassOfValue(cx, val, &cls)) return false;
                ...
                if (cls == ESClass::Array) {
                    ...
                    for (uint32_t i = 0; i < sourceLength; ++i) {
                        if (!JS_GetElement(cx, valObj, i, &item)) return false;

                        char* data = intermediate.get() + elementSize * i;
                        if (!ImplicitConvert(cx, item, baseType, data, convType, nullptr, funObj,
                                             argIndex, targetType, i))
                            return false;
                    }
                    ...
                    uint32_t sourceLength = bufferShared
                                                ? JS_GetSharedArrayBufferByteLength(valObj)
                                                : JS_GetArrayBufferByteLength(valObj);
                    size_t elementSize = CType::GetSize(baseType);
                    size_t arraySize = elementSize * targetLength;
                    if (arraySize != size_t(sourceLength)) {
                        MOZ_ASSERT(!funObj);
                        return ArrayLengthMismatch(cx, arraySize, targetType, size_t(sourceLength),
                                                   val, convType);
                    }
                    ...
                    bool isShared;
                    SharedMem<void*> src =
                        (bufferShared ? SharedMem<void*>::shared(
        ...
        case TYPE_struct: {
            if (val.isObject() && !sourceData) {
                ...
                if (!JS_Enumerate(cx, valObj, &props)) return false;
                ...
                if (props.length() != fields->count()) {
                    return FieldCountMismatch(cx, fields->count(), targetType, props.length(), val,
                                              convType, funObj, argIndex);
                }
                ...
                for (size_t i = 0; i < props.length(); ++i) {
                    ...
                    if (!JSID_IS_STRING(id)) {
                        return PropNameNonStringError(cx, id, val, convType, funObj, argIndex);
                    }
                    ...
                    RootedValue prop(cx);
                    if (!JS_GetPropertyById(cx, valObj, id, &prop)) return false;

                    // Convert the field via ImplicitConvert().
                    char* fieldData = intermediate.get() + field->mOffset;
                    if (!ImplicitConvert(cx, prop, field->mType, fieldData, convType, nullptr,
...
// ImplicitConvert.
static bool ExplicitConvert(JSContext* cx, HandleValue val, HandleObject targetType, void* buffer,
                            ConversionType convType) {
    // If ImplicitConvert succeeds, use that result.
    if (ImplicitConvert(cx, val, targetType, buffer, convType, nullptr)) return true;
    ...
    RootedValue ex(cx);
    if (!JS_GetPendingException(cx, &ex)) return false;
...
#define INTEGRAL_CASE(name, type, ffiType)                                                   \
    case TYPE_##name: {                                                                      \
        ...
        if (!jsvalToIntegerExplicit(val, &result) &&                                         \
            (!val.isString() || !StringToInteger(cx, val.toString(), &result, &overflow))) { \
            if (overflow) {                                                                  \
...
static JSString* BuildTypeName(JSContext* cx, JSObject* typeObj_) {
    ...
    while (true) {
        ...
        switch (currentGrouping) {
            ...
            case TYPE_function: {
                ...
                if (fninfo->mIsVariadic) AppendString(result, "...");
                AppendString(result, ")");
    ...
    if (('a' <= result[0] && result[0] <= 'z') || ('A' <= result[0] && result[0] <= 'Z') ||
        (result[0] == '_'))
        PrependString(result, " ");
...
#undef CASE_FOR_TYPE
        ...
        case TYPE_array: {
            ...
            size_t length;
            if (ArrayType::GetSafeLength(typeObj, &length)) IntegerToString(length, 10, result);
        ...
        case TYPE_struct: {
            ...
            if (!fieldsArray.resize(length)) break;
            ...
            for (size_t i = 0; i < length; ++i) {
                ...
                AppendString(result, " }");
                if (i != length - 1) AppendString(result, ", ");
            }
...
static bool BuildDataSource(JSContext* cx, HandleObject typeObj, void* data, bool isImplicit,
                            AutoString& result) {
...
#undef CHAR_CASE
        ...
        case TYPE_function: {
            ...
            if (isImplicit) AppendString(result, ")");
        ...
        case TYPE_array: {
            ...
            for (size_t i = 0; i < length; ++i) {
                ...
                if (!BuildDataSource(cx, baseType, element, true, result)) return false;

                if (i + 1 < length) AppendString(result, ", ");
            }
            AppendString(result, "]");
            break;
        }
        case TYPE_struct: {
            ...
            size_t length = fields->count();
            Vector<const FieldInfoHash::Entry*, 64, SystemAllocPolicy> fieldsArray;
            if (!fieldsArray.resize(length)) return false;
            ...
            for (size_t i = 0; i < length; ++i) {
                ...
                if (!BuildDataSource(cx, entryType, fieldData, true, result)) return false;
...
bool CType::ConstructData(JSContext* cx, unsigned argc, Value* vp) {
    ...
    switch (GetTypeCode(obj)) {
        ...
        case TYPE_function:
            JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr, CTYPESMSG_FUNCTION_CONSTRUCT);
            return false;
        case TYPE_pointer:
...
bool CType::ConstructBasic(JSContext* cx, HandleObject obj, const CallArgs& args) {
    if (args.length() > 1) {
    ...
    if (args.length() == 1) {
        if (!ExplicitConvert(cx, args[0], obj, CData::GetData(result), ConversionType::Construct))
            return false;
    }
...
JSObject* CType::Create(JSContext* cx, HandleObject typeProto, HandleObject dataProto,
    ...
    if (ffiType) JS_SetReservedSlot(typeObj, SLOT_FFITYPE, PrivateValue(ffiType));
    if (name) JS_SetReservedSlot(typeObj, SLOT_NAME, StringValue(name));
    ...
    if (!JS_FreezeObject(cx, typeObj)) return nullptr;

    // Assert a sanity check on size and alignment: size % alignment should always
    // be zero.
    MOZ_ASSERT_IF(IsSizeDefined(typeObj), GetSize(typeObj) % GetAlignment(typeObj) == 0);

    return typeObj;
}

JSObject* CType::DefineBuiltin(JSContext* cx, HandleObject ctypesObj, const char* propName,
                               JSObject* typeProto_, JSObject* dataProto_, const char* name,
                               TypeCode type, HandleValue size, HandleValue align,
                               ffi_type* ffiType) {
    ...
    RootedObject typeObj(cx,
                         Create(cx, typeProto, dataProto, type, nameStr, size, align, ffiType));
    if (!typeObj) return nullptr;
...
void CType::Trace(JSTracer* trc, JSObject* obj) {
    ...
    switch (TypeCode(slot.toInt32())) {
        ...
        case TYPE_function: {
            ...
            for (auto& argType : fninfo->mArgTypes) JS::TraceEdge(trc, &argType, "argType");
...
bool CType::IsCTypeProto(JSObject* obj) { return JS_GetClass(obj) == &sCTypeProtoClass; }
...
bool CType::TypesEqual(JSObject* t1, JSObject* t2) {
    ...
    switch (c1) {
        ...
        case TYPE_function: {
            ...
            if (f1->mABI != f2->mABI) return false;

            if (!TypesEqual(f1->mReturnType, f2->mReturnType)) return false;

            if (f1->mArgTypes.length() != f2->mArgTypes.length()) return false;

            if (f1->mIsVariadic != f2->mIsVariadic) return false;

            for (size_t i = 0; i < f1->mArgTypes.length(); ++i) {
                if (!TypesEqual(f1->mArgTypes[i], f2->mArgTypes[i])) return false;
            }
        ...
        case TYPE_array: {
            ...
            if (d1 != d2 || (d1 && s1 != s2)) return false;
...
size_t CType::GetSize(JSObject* obj) {
    ...
    // directly.
    if (size.isInt32()) return size.toInt32();
...
JSString* CType::GetName(JSContext* cx, HandleObject obj) {
    ...
    if (!string.isUndefined()) return string.toString();
...
JSObject* CType::GetProtoFromType(JSContext* cx, JSObject* objArg, CTypeProtoSlot slot) {
    MOZ_ASSERT(IsCType(objArg));
    RootedObject obj(cx, objArg);

    // Get the prototype of the type object.
    RootedObject proto(cx);
    if (!JS_GetPrototype(cx, obj, &proto)) return nullptr;
    MOZ_ASSERT(proto);
    MOZ_ASSERT(CType::IsCTypeProto(proto));
...
bool CType::PrototypeGetter(JSContext* cx, const JS::CallArgs& args) {
    ...
    unsigned slot = CType::IsCTypeProto(obj) ? (unsigned)SLOT_OURDATAPROTO : (unsigned)SLOT_PROTO;
    args.rval().set(JS_GetReservedSlot(obj, slot));
    MOZ_ASSERT(args.rval().isObject() || args.rval().isUndefined());
    return true;
}

bool CType::IsCType(HandleValue v) { return v.isObject() && CType::IsCType(&v.toObject()); }
...
bool CType::CreateArray(JSContext* cx, unsigned argc, Value* vp) {
    ...
    if (args.length() == 1 && !jsvalToSize(cx, args[0], false, &length)) {
        return ArgumentTypeMismatch(cx, "", "CType.prototype.array", "a nonnegative integer");
    }
...
bool CType::HasInstance(JSContext* cx, HandleObject obj, MutableHandleValue v, bool* bp) {
    MOZ_ASSERT(CType::IsCType(obj));
    ...
    *bp = false;
    ...
    for (;;) {
        if (!JS_GetPrototype(cx, proto, &proto)) return false;
        if (!proto) break;
        if (proto == prototype) {
            *bp = true;
            break;
...
static JSObject* CType::GetGlobalCTypes(JSContext* cx, JSObject* objArg) {
    ...
    if (!JS_GetPrototype(cx, obj, &objTypeProto)) return nullptr;
...
JSObject* PointerType::CreateInternal(JSContext* cx, HandleObject baseType) {
    ...
    if (!slot.isUndefined()) return &slot.toObject();
...
bool PointerType::ConstructData(JSContext* cx, HandleObject obj, const CallArgs& args) {
    if (!CType::IsCType(obj) || CType::GetTypeCode(obj) != TYPE_pointer) {
        return IncompatibleCallee(cx, "PointerType constructor", obj);
    }

    if (args.length() > 3) {
        return ArgumentLengthError(cx, "PointerType constructor", "0, 1, 2, or 3", "s");
    }
    ...
    bool looksLikeClosure = CType::GetTypeCode(baseObj) == TYPE_function && args[0].isObject() &&
    ...
    if (args.length() == 3) errVal = args[2];
...
bool PointerType::IsNull(JSContext* cx, unsigned argc, Value* vp) {
    ...
    if (!obj) return false;
    if (!CData::IsCDataMaybeUnwrap(&obj)) {
        return IncompatibleThisProto(cx, "PointerType.prototype.isNull", args.thisv());
    }

    // Get pointer type and base type.
    JSObject* typeObj = CData::GetCType(obj);
    if (CType::GetTypeCode(typeObj) != TYPE_pointer) {
        return IncompatibleThisType(cx, "PointerType.prototype.isNull", "non-PointerType CData",
                                    args.thisv());
    }
...
bool PointerType::OffsetBy(JSContext* cx, const CallArgs& args, int offset) {
    ...
    if (!CData::IsCDataMaybeUnwrap(&obj)) {
        if (offset == 1) {
            return IncompatibleThisProto(cx, "PointerType.prototype.increment", args.thisv());
        }
        return IncompatibleThisProto(cx, "PointerType.prototype.decrement", args.thisv());
    }
    ...
    if (CType::GetTypeCode(typeObj) != TYPE_pointer) {
        ...
        return IncompatibleThisType(cx, "PointerType.prototype.decrement", "non-PointerType CData",
                                    args.thisv());
    }
...
bool PointerType::ContentsGetter(JSContext* cx, const JS::CallArgs& args) {
    ...
    if (!ConvertToJS(cx, baseType, nullptr, data, false, false, &result)) return false;
...
bool PointerType::ContentsSetter(JSContext* cx, const JS::CallArgs& args) {
    ...
    return ImplicitConvert(cx, args.get(0), baseType, data, ConversionType::Setter, nullptr);
}
...
bool ArrayType::Create(JSContext* cx, unsigned argc, Value* vp) {
    ...
    if (args.length() == 2 && !jsvalToSize(cx, args[1], false, &length)) {
        return ArgumentTypeMismatch(cx, "second ", "ArrayType", "a nonnegative integer");
    }
...
JSObject* ArrayType::CreateInternal(JSContext* cx, HandleObject baseType, size_t length,
                                    bool lengthDefined) {
    ...
    // Create a new CType object with the common properties and slots.
    JSObject* typeObj =
        CType::Create(cx, typeProto, dataProto, TYPE_array, nullptr, sizeVal, alignVal, nullptr);
    if (!typeObj) return nullptr;
...
bool ArrayType::ConstructData(JSContext* cx, HandleObject obj_, const CallArgs& args) {
    ...
    if (CType::IsSizeDefined(obj)) {
        if (args.length() > 1) {
            return ArgumentLengthError(cx, "size defined ArrayType constructor", "at most one",
                                       "");
        }

    } else {
        if (args.length() != 1) {
            return ArgumentLengthError(cx, "size undefined ArrayType constructor", "one", "");
        }
        ...
        if (jsvalToSize(cx, args[0], false, &length)) {
            ...
            if (!JS_GetProperty(cx, arg, "length", &lengthVal) ||
                ...
                return ArgumentTypeMismatch(cx, "", "size undefined ArrayType constructor",
...
size_t ArrayType::GetLength(JSObject* obj) {
    ...
    // For callers who know it can never be JS::UndefinedValue(), return a size_t
    // directly.
    if (length.isInt32()) return length.toInt32();
...
UniquePtrFFIType ArrayType::BuildFFIType(JSContext* cx, JSObject* obj) {
    ...
    for (size_t i = 0; i < length; ++i) ffiType->elements[i] = ffiBaseType;
...
bool ArrayType::LengthGetter(JSContext* cx, const JS::CallArgs& args) {
    ...
    if (CData::IsCDataMaybeUnwrap(&obj)) obj = CData::GetCType(obj);
...
bool ArrayType::Getter(JSContext* cx, HandleObject obj, HandleId idval, MutableHandleValue vp,
                       bool* handled) {
    *handled = false;
    ...
    JSObject* typeObj = CData::GetCType(obj);
    if (CType::GetTypeCode(typeObj) != TYPE_array) return true;
    ...
    int32_t dummy;
    if (!ok && JSID_IS_SYMBOL(idval)) return true;
    bool dummy2;
...
bool ArrayType::Setter(JSContext* cx, HandleObject obj, HandleId idval, HandleValue vp,
                       ObjectOpResult& result, bool* handled) {
    *handled = false;
    ...
    // CData, regardless of CType.)
    RootedObject typeObj(cx, CData::GetCType(obj));
    if (CType::GetTypeCode(typeObj) != TYPE_array) return result.succeed();
    ...
    *handled = true;
    ...
    size_t elementSize = CType::GetSize(baseType);
    char* data = static_cast<char*>(CData::GetData(obj)) + elementSize * index;
    if (!ImplicitConvert(cx, vp, baseType, data, ConversionType::Setter, nullptr, nullptr, 0,
                         typeObj, index))
        return false;
    return result.succeed();
}
...
bool ArrayType::AddressOfElement(JSContext* cx, unsigned argc, Value* vp) {
    ...
    if (!obj) return false;
    if (!CData::IsCDataMaybeUnwrap(&obj)) {
        return IncompatibleThisProto(cx, "ArrayType.prototype.addressOfElement", args.thisv());
    }
    ...
    if (args.length() != 1) {
        return ArgumentLengthError(cx, "ArrayType.prototype.addressOfElement", "one", "");
    }
...
static JSFlatString* ExtractStructField(JSContext* cx, HandleValue val,
    ...
    RootedObject obj(cx, &val.toObject());
    Rooted<IdVector> props(cx, IdVector(cx));
    if (!JS_Enumerate(cx, obj, &props)) return nullptr;
    ...
    if (!JS_GetPropertyById(cx, obj, nameid, &propVal)) return nullptr;
...
// { name : type }.
static bool AddFieldToArray(JSContext* cx, MutableHandleValue element, JSFlatString* name_,
                            JSObject* typeObj_) {
    RootedObject typeObj(cx, typeObj_);
    ...
    if (!nameChars.initTwoByte(cx, name)) return false;

    if (!JS_DefineUCProperty(cx, fieldObj, nameChars.twoByteChars(), name->length(), typeObj,
                             JSPROP_ENUMERATE | JSPROP_READONLY | JSPROP_PERMANENT))
        return false;
...
bool StructType::Create(JSContext* cx, unsigned argc, Value* vp) {
    ...
    RootedObject result(
        cx, CType::Create(cx, typeProto, nullptr, TYPE_struct, name.toString(),
                          JS::UndefinedHandleValue, JS::UndefinedHandleValue, nullptr));
    if (!result) return false;
    ...
    if (args.length() == 2) {
        ...
        if (!arr) {
            ...
            if (!JS_IsArrayObject(cx, arr, &isArray)) return false;
        }
        if (!isArray) return ArgumentTypeMismatch(cx, "second ", "StructType", "an array");

        // Define the struct fields.
        if (!DefineInternal(cx, result, arr)) return false;
    }
...
bool StructType::DefineInternal(JSContext* cx, JSObject* typeObj_, JSObject* fieldsObj_) {
    RootedObject typeObj(cx, typeObj_);
    ...
    if (len != 0) {
        ...
        for (uint32_t i = 0; i < len; ++i) {
            RootedValue item(cx);
            if (!JS_GetElement(cx, fieldsObj, i, &item)) return false;
            ...
            RootedFunction getter(
                cx, NewFunctionWithReserved(cx, StructType::FieldGetter, 0, 0, nullptr));
            ...
            RootedFunction setter(
                cx, NewFunctionWithReserved(cx, StructType::FieldSetter, 1, 0, nullptr));
            ...
            if (!JS_DefineUCProperty(
                    cx, prototype, nameChars.twoByteChars(), name->length(),
            ...
            if (fieldAlign > structAlign) structAlign = fieldAlign;
        }
...
bool StructType::Define(JSContext* cx, unsigned argc, Value* vp) {
    ...
    if (!obj) return false;
    if (!CType::IsCType(obj)) {
        return IncompatibleThisProto(cx, "StructType.prototype.define", args.thisv());
    }
    if (CType::GetTypeCode(obj) != TYPE_struct) {
        return IncompatibleThisType(cx, "StructType.prototype.define", "non-StructType",
                                    args.thisv());
    }
    ...
    if (arg.isPrimitive()) {
        return ArgumentTypeMismatch(cx, "", "StructType.prototype.define", "an array");
    }
    ...
    if (!arg.isObject()) {
        ...
        if (!JS_IsArrayObject(cx, arg, &isArray)) return false;
    }
...
bool StructType::ConstructData(JSContext* cx, HandleObject obj, const CallArgs& args) {
    ...
    if (args.length() == 1) {
        ...
        // Try option 1) first.
        if (ExplicitConvert(cx, args[0], obj, buffer, ConversionType::Construct)) return true;
    ...
    if (args.length() == fields->count()) {
        for (FieldInfoHash::Range r = fields->all(); !r.empty(); r.popFront()) {
            ...
            if (!ImplicitConvert(cx, args[field.mIndex], field.mType, buffer + field.mOffset,
                                 ConversionType::Construct, nullptr, nullptr, 0, obj,
                                 field.mIndex))
                return false;
        }
    ...
    if (count >= 2) {
        ...
        return ArgumentLengthError(cx, "StructType constructor", fieldLengthStr, "s");
    }
...
const FieldInfo* StructType::LookupField(JSContext* cx, JSObject* obj, JSFlatString* name) {
    MOZ_ASSERT(CType::IsCType(obj));
    MOZ_ASSERT(CType::GetTypeCode(obj) == TYPE_struct);
...
JSObject* StructType::BuildFieldsArray(JSContext* cx, JSObject* obj) {
    ...
    if (!fieldsVec.resize(len)) return nullptr;
    ...
    if (!fieldsProp) return nullptr;

    // Seal the fields array.
    if (!JS_FreezeObject(cx, fieldsProp)) return nullptr;
...
bool StructType::FieldGetter(JSContext* cx, unsigned argc, Value* vp) {
    ...
    if (CType::GetTypeCode(typeObj) != TYPE_struct) {
        return IncompatibleThisType(cx, "StructType property getter", "non-StructType CData",
                                    args.thisv());
    }
...
bool StructType::FieldSetter(JSContext* cx, unsigned argc, Value* vp) {
    ...
    RootedObject typeObj(cx, CData::GetCType(obj));
    if (CType::GetTypeCode(typeObj) != TYPE_struct) {
        return IncompatibleThisType(cx, "StructType property setter", "non-StructType CData",
                                    args.thisv());
    }
...
bool StructType::AddressOfField(JSContext* cx, unsigned argc, Value* vp) {
    ...
    if (!obj) return false;
    if (!CData::IsCDataMaybeUnwrap(&obj)) {
        return IncompatibleThisProto(cx, "StructType.prototype.addressOfField", args.thisv());
    }
    ...
    if (!args[0].isString()) {
        return ArgumentTypeMismatch(cx, "", "StructType.prototype.addressOfField", "a string");
    }
...
static bool GetABI(JSContext* cx, HandleValue abiType, ffi_abi* result) {
    if (abiType.isPrimitive()) return false;
...
static MOZ_ALWAYS_INLINE bool IsEllipsis(JSContext* cx, HandleValue v, bool* isEllipsis) {
    *isEllipsis = false;
    ...
    *isEllipsis = (linear->latin1OrTwoByteChar(0) == dot &&
                   linear->latin1OrTwoByteChar(1) == dot && linear->latin1OrTwoByteChar(2) == dot);
    return true;
}
...
static bool PrepareCIF(JSContext* cx, FunctionInfo* fninfo) {
    ...
    ffi_status status = ffi_prep_cif(&fninfo->mCIF, abi, fninfo->mFFITypes.length(), rtype,
...
void FunctionType::BuildSymbolName(JSString* name, JSObject* typeObj, AutoCString& result) {
...
static bool CreateFunctionInfo(JSContext* cx, HandleObject typeObj, HandleValue abiType,
                               HandleObject returnType, const HandleValueArray& args) {
    ...
    if (!fninfo->mArgTypes.reserve(args.length()) || !fninfo->mFFITypes.reserve(args.length())) {
        JS_ReportOutOfMemory(cx);
        return false;
    }
    ...
    for (uint32_t i = 0; i < args.length(); ++i) {
        bool isEllipsis;
        if (!IsEllipsis(cx, args[i], &isEllipsis)) return false;
        if (isEllipsis) {
    ...
    if (!PrepareCIF(cx, fninfo)) return false;
...
bool FunctionType::Create(JSContext* cx, unsigned argc, Value* vp) {
    ...
    if (args.length() == 3) {
        ...
        if (!args[2].isObject()) {
            ...
            if (!JS_IsArrayObject(cx, args[2], &isArray)) return false;
        }

        if (!isArray) return ArgumentTypeMismatch(cx, "third ", "FunctionType", "an array");
    ...
    for (uint32_t i = 0; i < argTypes.length(); ++i) {
        if (!JS_GetElement(cx, arrayObj, i, argTypes[i])) return false;
    }
...
JSObject* FunctionType::CreateInternal(JSContext* cx, HandleValue abi, HandleValue rtype,
    ...
    if (!CreateFunctionInfo(cx, typeObj, abi, returnType, args)) return nullptr;
...
bool FunctionType::ConstructData(JSContext* cx, HandleObject typeObj, HandleObject dataObj,
                                 HandleObject fnObj, HandleObject thisObj, HandleValue errVal) {
...
static bool ConvertArgument(JSContext* cx, HandleObject funObj, unsigned argIndex, HandleValue arg,
                            JSObject* type, AutoValue* value, AutoValueAutoArray* strings) {
    ...
    if (!ImplicitConvert(cx, arg, type, value->mData, ConversionType::Argument, &freePointer,
                         funObj, argIndex))
        return false;
...
bool FunctionType::Call(JSContext* cx, unsigned argc, Value* vp) {
    ...
    RootedObject obj(cx, &args.callee());
    if (!CData::IsCDataMaybeUnwrap(&obj)) {
        return IncompatibleThisProto(cx, "FunctionType.prototype.call", args.calleev());
    }

    RootedObject typeObj(cx, CData::GetCType(obj));
    if (CType::GetTypeCode(typeObj) != TYPE_pointer) {
        return IncompatibleThisType(cx, "FunctionType.prototype.call", "non-PointerType CData",
                                    args.calleev());
    }

    typeObj = PointerType::GetBaseType(typeObj);
    if (CType::GetTypeCode(typeObj) != TYPE_function) {
        return IncompatibleThisType(cx, "FunctionType.prototype.call", "non-FunctionType pointer",
                                    args.calleev());
    }
    ...
    if ((!fninfo->mIsVariadic && args.length() != argcFixed) ||
        ...
        return FunctionArgumentLengthMismatch(cx, argcFixed, args.length(), obj, typeObj,
                                              fninfo->mIsVariadic);
    }
    ...
    for (unsigned i = 0; i < argcFixed; ++i)
        if (!ConvertArgument(cx, obj, i, args[i], fninfo->mArgTypes[i], &values[i], &strings))
            return false;
    ...
    if (typeCode != TYPE_void_t && !returnValue.SizeToType(cx, fninfo->mReturnType)) {
        JS_ReportAllocationOverflow(cx);
        return false;
    }
...
bool FunctionType::ArgTypesGetter(JSContext* cx, const JS::CallArgs& args) {
    ...
    if (!args.rval().isUndefined()) return true;
    ...
    JS::Rooted<JSObject*> argTypes(cx);
    {
        ...
        if (!vec.resize(len)) return false;

        for (size_t i = 0; i < len; ++i) vec[i].setObject(*fninfo->mArgTypes[i]);

        argTypes = JS_NewArrayObject(cx, vec);
        if (!argTypes) return false;
    }

    // Seal and cache it.
    if (!JS_FreezeObject(cx, argTypes)) return false;
...
JSObject* CClosure::Create(JSContext* cx, HandleObject typeObj, HandleObject fnObj,
    ...
    RootedObject proto(cx);
    if (!JS_GetPrototype(cx, typeObj, &proto)) return nullptr;
    MOZ_ASSERT(proto);
    MOZ_ASSERT(CType::IsCTypeProto(proto));
    ...
    void* code;
    cinfo->closure = static_cast<ffi_closure*>(ffi_closure_alloc(sizeof(ffi_closure), &code));
    ...
    ffi_status status =
        ffi_prep_closure_loc(cinfo->closure, &fninfo->mCIF, CClosure::ClosureStub, cinfo, code);
    if (status != FFI_OK) {
        JS_ReportErrorASCII(cx, "couldn't create closure - libffi error");
        return nullptr;
    }
...
void CClosure::Trace(JSTracer* trc, JSObject* obj) {
    ...
    if (cinfo->thisObj) JS::TraceEdge(trc, &cinfo->thisObj, "thisObj");
}
...
void CClosure::ClosureStub(ffi_cif* cif, void* result, void** args, void* userData) {
    MOZ_ASSERT(cif);
    MOZ_ASSERT(result);
    MOZ_ASSERT(args);
    MOZ_ASSERT(userData);
...
#undef INTEGRAL_CASE
    ...
    for (uint32_t i = 0; i < cif->nargs; ++i) {
        ...
        RootedObject argType(cx, fninfo->mArgTypes[i]);
        if (!ConvertToJS(cx, argType, nullptr, args[i], false, false, argv[i])) return false;
    }
    ...
    if (success && cif->rtype != &ffi_type_void)
        success = ImplicitConvert(cx, rval, fninfo->mReturnType, result, ConversionType::Return,
                                  nullptr, typeObj);
...
JSObject* CData::Create(JSContext* cx, HandleObject typeObj, HandleObject refObj, void* source,
                        bool ownResult) {
    MOZ_ASSERT(typeObj);
    MOZ_ASSERT(CType::IsCType(typeObj));
    ...
    if (refObj) JS_SetReservedSlot(dataObj, SLOT_REFERENT, ObjectValue(*refObj));
    ...
    *buffer = data;
    ...
    if (CType::GetTypeCode(typeObj) != TYPE_array) return dataObj;
...
void CData::Finalize(JSFreeOp* fop, JSObject* obj) {
    ...
    if (owns) FreeOp::get(fop)->free_(*buffer);
...
bool CData::IsCDataProto(JSObject* obj) { return JS_GetClass(obj) == &sCDataProtoClass; }
...
bool CData::ValueSetter(JSContext* cx, const JS::CallArgs& args) {
    RootedObject obj(cx, &args.thisv().toObject());
    args.rval().setUndefined();
    return ImplicitConvert(cx, args.get(0), GetCType(obj), GetData(obj), ConversionType::Setter,
                           nullptr);
}
...
bool CData::Cast(JSContext* cx, unsigned argc, Value* vp) {
    ...
    if (targetSize > CType::GetSize(sourceType)) {
        return SizeMismatchCastError(cx, sourceType, targetType, CType::GetSize(sourceType),
                                     targetSize);
    }
...
bool CData::GetRuntime(JSContext* cx, unsigned argc, Value* vp) {
    ...
    size_t targetSize;
    if (!CType::GetSafeSize(targetType, &targetSize) || targetSize != sizeof(void*)) {
...
static bool ReadStringCommon(JSContext* cx, InflateUTF8Method inflateUTF8, unsigned argc,
...
bool CDataFinalizer::Methods::ReadString(JSContext* cx, unsigned argc, Value* vp) {
    return ReadStringCommon(cx, JS::UTF8CharsToNewTwoByteCharsZ, argc, vp,
                            "CDataFinalizer.prototype.readString");
}

bool CData::ReadStringReplaceMalformed(JSContext* cx, unsigned argc, Value* vp) {
    return ReadStringCommon(cx, JS::LossyUTF8CharsToNewTwoByteCharsZ, argc, vp,
                            "CData.prototype.readStringReplaceMalformed");
}

JSString* CData::GetSourceString(JSContext* cx, HandleObject typeObj, void* data) {
    // Walk the types, building up the toSource() string.
    ...
    AppendString(source, "(");
    if (!BuildDataSource(cx, typeObj, data, false, source)) return nullptr;
...
bool CDataFinalizer::Methods::ToSource(JSContext* cx, unsigned argc, Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);
    ...
    if (!p) {
        ...
        AppendString(source, ", ");
        Value valCodePtrType = JS_GetReservedSlot(objThis, SLOT_DATAFINALIZER_CODETYPE);
...
bool CDataFinalizer::Methods::ToString(JSContext* cx, unsigned argc, Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);
...
bool CDataFinalizer::GetValue(JSContext* cx, JSObject* obj, MutableHandleValue aResult) {
    MOZ_ASSERT(IsCDataFinalizer(obj));
...
bool CDataFinalizer::Construct(JSContext* cx, unsigned argc, Value* vp) {
    ...
    if (!valCodePtr.isObject()) {
        return TypeError(cx, "_a CData object_ of a function pointer type", valCodePtr);
    }
    ...
    if (!CData::IsCDataMaybeUnwrap(&objCodePtr)) {
        return TypeError(cx, "a _CData_ object of a function pointer type", valCodePtr);
    }
    ...
    if (typCodePtr != TYPE_pointer) {
        return TypeError(cx, "a CData object of a function _pointer_ type", valCodePtr);
    }
    ...
    if (typCode != TYPE_function) {
        return TypeError(cx, "a CData object of a _function_ pointer type", valCodePtr);
    }
    uintptr_t code = *reinterpret_cast<uintptr_t*>(CData::GetData(objCodePtr));
    if (!code) {
        return TypeError(cx, "a CData object of a _non-NULL_ function pointer type", valCodePtr);
    }

    FunctionInfo* funInfoFinalizer = FunctionType::GetFunctionInfo(objCodeType);
    MOZ_ASSERT(funInfoFinalizer);

    if ((funInfoFinalizer->mArgTypes.length() != 1) || (funInfoFinalizer->mIsVariadic)) {
        RootedValue valCodeType(cx, ObjectValue(*objCodeType));
        return TypeError(cx, "a function accepting exactly one argument", valCodeType);
    }
    ...
    if (!CType::GetSafeSize(objArgType, &sizeArg)) {
        RootedValue valCodeType(cx, ObjectValue(*objCodeType));
        return TypeError(cx, "a function with one known size argument", valCodeType);
    }

    ScopedJSFreePtr<void> cargs(malloc(sizeArg));

    if (!ImplicitConvert(cx, valData, objArgType, cargs.get(), ConversionType::Finalizer,
                         &freePointer, objCodePtrType, 0)) {
        return false;
    }
    if (freePointer) {
    ...
    if (CType::GetTypeCode(returnType) != TYPE_void_t) {
        rvalue = malloc(Align(CType::GetSize(returnType), sizeof(ffi_arg)));
    ...
    // Used by GetCType
    JS_SetReservedSlot(objResult, SLOT_DATAFINALIZER_VALTYPE, ObjectOrNullValue(objBestArgType));

    // Used by ToSource
    JS_SetReservedSlot(objResult, SLOT_DATAFINALIZER_CODETYPE, ObjectValue(*objCodePtrType));
    ...
    ScopedJSFreePtr<CDataFinalizer::Private> p(
...
bool CDataFinalizer::Methods::Forget(JSContext* cx, unsigned argc, Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);
    if (args.length() != 0) {
        return ArgumentLengthError(cx, "CDataFinalizer.prototype.forget", "no", "s");
    }

    RootedObject obj(cx, JS_THIS_OBJECT(cx, vp));
    if (!obj) return false;
    if (!CDataFinalizer::IsCDataFinalizer(obj)) {
        return IncompatibleThisProto(cx, "CDataFinalizer.prototype.forget", args.thisv());
    }
...
bool CDataFinalizer::Methods::Dispose(JSContext* cx, unsigned argc, Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);
    if (args.length() != 0) {
        return ArgumentLengthError(cx, "CDataFinalizer.prototype.dispose", "no", "s");
    }

    RootedObject obj(cx, JS_THIS_OBJECT(cx, vp));
    if (!obj) return false;
    if (!CDataFinalizer::IsCDataFinalizer(obj)) {
        return IncompatibleThisProto(cx, "CDataFinalizer.prototype.dispose", args.thisv());
    }
...
JSObject* Int64Base::Construct(JSContext* cx, HandleObject proto, uint64_t data, bool isUnsigned) {
    ...
    if (!JS_FreezeObject(cx, result)) return nullptr;
...
bool Int64Base::ToString(JSContext* cx, JSObject* obj, const CallArgs& args, bool isUnsigned) {
    if (args.length() > 1) {
        if (isUnsigned) {
    ...
    if (args.length() == 1) {
        Value arg = args[0];
        if (arg.isInt32()) radix = arg.toInt32();
        if (!arg.isInt32() || radix < 2 || radix > 36) {
            if (isUnsigned) {
                return ArgumentRangeMismatch(cx, "UInt64.prototype.toString",
                                             "an integer at least 2 and no greater than 36");
            }
            return ArgumentRangeMismatch(cx, "Int64.prototype.toString",
                                         "an integer at least 2 and no greater than 36");
        }
    }
...
bool Int64Base::ToSource(JSContext* cx, JSObject* obj, const CallArgs& args, bool isUnsigned) {
    if (args.length() != 0) {
        if (isUnsigned) {
...
bool Int64::ToString(JSContext* cx, unsigned argc, Value* vp) {
    ...
    if (!Int64::IsInt64(obj)) {
        ...
        return IncompatibleThisType(cx, "Int64.prototype.toString", "non-Int64 CData");
    }
...
bool Int64::ToSource(JSContext* cx, unsigned argc, Value* vp) {
    ...
    if (!Int64::IsInt64(obj)) {
        ...
        return IncompatibleThisType(cx, "Int64.prototype.toSource", "non-Int64 CData");
    }
...
bool Int64::Join(JSContext* cx, unsigned argc, Value* vp) {
    ...
    int32_t hi;
    uint32_t lo;
    if (!jsvalToInteger(cx, args[0], &hi)) return ArgumentConvError(cx, args[0], "Int64.join", 0);
    if (!jsvalToInteger(cx, args[1], &lo)) return ArgumentConvError(cx, args[1], "Int64.join", 1);
...
bool UInt64::IsUInt64(JSObject* obj) { return JS_GetClass(obj) == &sUInt64Class; }
...
bool UInt64::ToString(JSContext* cx, unsigned argc, Value* vp) {
    ...
    if (!UInt64::IsUInt64(obj)) {
        ...
        return IncompatibleThisType(cx, "UInt64.prototype.toString", "non-UInt64 CData");
    }
...
bool UInt64::ToSource(JSContext* cx, unsigned argc, Value* vp) {
    ...
    if (!UInt64::IsUInt64(obj)) {
        ...
        return IncompatibleThisType(cx, "UInt64.prototype.toSource", "non-UInt64 CData");
    }
...
bool UInt64::Join(JSContext* cx, unsigned argc, Value* vp) {
    ...
    uint32_t hi;
    uint32_t lo;
    if (!jsvalToInteger(cx, args[0], &hi)) return ArgumentConvError(cx, args[0], "UInt64.join", 0);
    if (!jsvalToInteger(cx, args[1], &lo)) return ArgumentConvError(cx, args[1], "UInt64.join", 1);
