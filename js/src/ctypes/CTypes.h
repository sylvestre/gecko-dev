...
void AppendString(mozilla::Vector<T, N, AP>& v, const char (&array)[ArrayLength]) {
...
template <class T, size_t N, class AP, size_t ArrayLength>
void PrependString(mozilla::Vector<T, N, AP>& v, const char (&array)[ArrayLength]) {
    // Don't include the trailing '\0'.
    size_t alen = ArrayLength - 1;
    size_t vlen = v.length();
    if (!v.resize(vlen + alen)) return;
...
extern size_t GetDeflatedUTF8StringLength(JSContext* maybecx, const CharT* chars,
                                          size_t charsLength);

template <typename CharT>
MOZ_MUST_USE bool DeflateStringToUTF8Buffer(JSContext* maybecx, const CharT* src, size_t srclen,
 ...
 */
enum ABICode { ABI_DEFAULT, ABI_STDCALL, ABI_THISCALL, ABI_WINAPI, INVALID_ABI };
...
using FieldInfoHash =
    GCHashMap<js::HeapPtr<JSFlatString*>, FieldInfo, FieldHashPolicy, SystemAllocPolicy>;
...
struct ClosureInfo {
    ...
    // nullptr here.
    explicit ClosureInfo(JSContext* context) : cx(context), errResult(nullptr), closure(nullptr) {}
...
namespace CType {
JSObject* Create(JSContext* cx, HandleObject typeProto, HandleObject dataProto, TypeCode type,
                 JSString* name, HandleValue size, HandleValue align, ffi_type* ffiType);
...
namespace ArrayType {
JSObject* CreateInternal(JSContext* cx, HandleObject baseType, size_t length, bool lengthDefined);
...
JSObject* ConstructWithObject(JSContext* cx, JSObject* typeObj, JSObject* refObj, PRFuncPtr fnptr,
                              JSObject* result);
...
namespace CClosure {
JSObject* Create(JSContext* cx, HandleObject typeObj, HandleObject fnObj, HandleObject thisObj,
                 HandleValue errVal, PRFuncPtr* fnptr);
}  // namespace CClosure

namespace CData {
JSObject* Create(JSContext* cx, HandleObject typeObj, HandleObject refObj, void* data,
                 bool ownResult);
