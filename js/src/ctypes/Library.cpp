...
static const JSClassOps sLibraryClassOps = {nullptr, nullptr, nullptr,          nullptr,
                                            nullptr, nullptr, Library::Finalize};

static const JSClass sLibraryClass = {
    "Library", JSCLASS_HAS_RESERVED_SLOTS(LIBRARY_SLOTS) | JSCLASS_FOREGROUND_FINALIZE,
    &sLibraryClassOps};
...
bool Library::Name(JSContext* cx, unsigned argc, Value* vp) {
    ...
    JSString* result = JS_NewUCStringCopyN(cx, resultString.begin(), resultString.length());
...
JSObject* Library::Create(JSContext* cx, HandleValue path, const JSCTypesCallbacks* callbacks) {
    ...
    if (callbacks && callbacks->unicodeToNative) {
        pathBytes = callbacks->unicodeToNative(cx, pathStrChars.twoByteChars(), pathStr->length());
        ...
        ASSERT_OK(DeflateStringToUTF8Buffer(cx, pathStrChars.twoByteChars(), pathStr->length(),
                                            pathBytes, &nbytes));
        pathBytes[nbytes] = 0;
    }
...
#undef MAX_ERROR_LEN
        ...
        if (JS::StringIsASCII(error)) {
            ...
            if (pathCharsLatin1.encodeLatin1(cx, pathStr))
                JS_ReportErrorLatin1(cx, "couldn't open library %s: %s", pathCharsLatin1.ptr(),
                                     error);
        }
        return nullptr;
    }
...
bool Library::IsLibrary(JSObject* obj) { return JS_GetClass(obj) == &sLibraryClass; }
...
bool Library::Declare(JSContext* cx, unsigned argc, Value* vp) {
    ...
    if (isFunction) {
        ...
        fnObj = FunctionType::CreateInternal(
            cx, args[1], args[2], HandleValueArray::subarray(args, 3, args.length() - 3));
    ...
    if (isFunction) JS_SetReservedSlot(result, SLOT_FUNNAME, StringValue(nameStr));
