...
static MOZ_ALWAYS_INLINE uint8_t GET_UINT8(jsbytecode* pc) { return uint8_t(pc[1]); }

static MOZ_ALWAYS_INLINE void SET_UINT8(jsbytecode* pc, uint8_t u) { pc[1] = jsbytecode(u); }
...
static MOZ_ALWAYS_INLINE int8_t GET_INT8(const jsbytecode* pc) { return int8_t(pc[1]); }
...
static MOZ_ALWAYS_INLINE uint32_t GET_UINT32(const jsbytecode* pc) {
    ...
    return result;
#else
    return (uint32_t(pc[4]) << 24) | (uint32_t(pc[3]) << 16) | (uint32_t(pc[2]) << 8) |
           uint32_t(pc[1]);
#endif
}
...
static MOZ_ALWAYS_INLINE int32_t GET_JUMP_OFFSET(jsbytecode* pc) { return GET_INT32(pc); }

static MOZ_ALWAYS_INLINE void SET_JUMP_OFFSET(jsbytecode* pc, int32_t off) { SET_INT32(pc, off); }

static const unsigned UINT32_INDEX_LEN = 4;

static MOZ_ALWAYS_INLINE uint32_t GET_UINT32_INDEX(const jsbytecode* pc) { return GET_UINT32(pc); }
...
static inline uint16_t GET_ARGNO(const jsbytecode* pc) { return GET_UINT16(pc); }

static inline void SET_ARGNO(jsbytecode* pc, uint16_t argno) { SET_UINT16(pc, argno); }

static const unsigned ARGNO_LEN = 2;
static const unsigned ARGNO_LIMIT = UINT16_LIMIT;

static inline uint32_t GET_LOCALNO(const jsbytecode* pc) { return GET_UINT24(pc); }

static inline void SET_LOCALNO(jsbytecode* pc, uint32_t varno) { SET_UINT24(pc, varno); }
...
static inline uint8_t PackLoopEntryDepthHintAndFlags(unsigned loopDepth, bool canIonOsr) {
...
static inline uint8_t GET_ENVCOORD_HOPS(jsbytecode* pc) { return GET_UINT8(pc); }

static inline void SET_ENVCOORD_HOPS(jsbytecode* pc, uint8_t hops) { SET_UINT8(pc, hops); }
...
static inline uint32_t GET_ENVCOORD_SLOT(const jsbytecode* pc) { return GET_UINT24(pc); }

static inline void SET_ENVCOORD_SLOT(jsbytecode* pc, uint32_t slot) { SET_UINT24(pc, slot); }
...
static inline uint32_t JOF_OPTYPE(JSOp op) { return JOF_TYPE(CodeSpec[op].format); }
...
class SrcNoteLineScanner {
   ...
   public:
    SrcNoteLineScanner(jssrcnote* sn, uint32_t lineno) : offset(0), sn(sn), lineno(lineno) {}
    ...
    switch (op) {
        ...
        default:
            ...
            MOZ_ASSERT(op == JSOP_CALL || op == JSOP_CALL_IGNORES_RV || op == JSOP_EVAL ||
                       op == JSOP_CALLITER || op == JSOP_STRICTEVAL || op == JSOP_FUNCALL ||
                       op == JSOP_FUNAPPLY);
 ...
 * false.
 */
extern bool ReconstructStackDepth(JSContext* cx, JSScript* script, jsbytecode* pc, uint32_t* depth,
                                  bool* reachablePC);
#endif
...
extern bool IsValidBytecodeOffset(JSContext* cx, JSScript* script, size_t offset);
...
inline bool IsPropertySetOp(JSOp op) { return CodeSpec[op].format & JOF_PROPSET; }

inline bool IsPropertyInitOp(JSOp op) { return CodeSpec[op].format & JOF_PROPINIT; }

inline bool IsEqualityOp(JSOp op) {
    return op == JSOP_EQ || op == JSOP_NE || op == JSOP_STRICTEQ || op == JSOP_STRICTNE;
}

inline bool IsCheckStrictOp(JSOp op) { return CodeSpec[op].format & JOF_CHECKSTRICT; }

#ifdef DEBUG
inline bool IsCheckSloppyOp(JSOp op) { return CodeSpec[op].format & JOF_CHECKSLOPPY; }
#endif
...
inline bool IsStrictSetPC(jsbytecode* pc) {
    JSOp op = JSOp(*pc);
    return op == JSOP_STRICTSETPROP || op == JSOP_STRICTSETNAME || op == JSOP_STRICTSETGNAME ||
...
inline bool IsCallPC(jsbytecode* pc) { return CodeSpec[*pc].format & JOF_INVOKE; }
...
inline bool IsSpreadCallPC(jsbytecode* pc) {
    JSOp op = JSOp(*pc);
    return op == JSOP_SPREADCALL || op == JSOP_SPREADNEW || op == JSOP_SPREADSUPERCALL ||
...
class PCCounts {
   ...
   public:
    ...
    bool operator<(const PCCounts& rhs) const { return pcOffset_ < rhs.pcOffset_; }
...
static inline jsbytecode* GetNextPc(jsbytecode* pc) { return pc + GetBytecodeLength(pc); }

#if defined(DEBUG)
/*
 * Disassemblers, for debugging only.
 */
extern MOZ_MUST_USE bool Disassemble(JSContext* cx, JS::Handle<JSScript*> script, bool lines,
                                     Sprinter* sp);

unsigned Disassemble1(JSContext* cx, JS::Handle<JSScript*> script, jsbytecode* pc, unsigned loc,
