...
static unsigned binLimit(unsigned bin) { return unsigned(pow(BinLog, bin) * FirstBinSize); }
...
static const char* allocKindName(AllocKind kind) {
    static const char* AllocKindNames[] = {
        "Object0", "Object0Bg",      "Object2",  "Object2Bg",  "Object4",    "Object4Bg",
        "Object8", "Object8Bg",      "Object12", "Object12Bg", "Object16",   "Object16Bg",
        "Script",  "LazyScript",     "Shape",    "BaseShape",  "TypeObject", "FatInlineString",
        "String",  "ExternalString", "Symbol",   "JitCode",    "Total"};
...
static void outputThingCounts(FILE* file) {
    ...
    for (unsigned i = 0; i < AllocKinds; ++i) {
        fprintf(file, "%15s, %8" PRIu64 ", %8" PRIu64 "\n", allocKindName(i),
                allocCountByHeapAndKind[Nursery][i], allocCountByHeapAndKind[TenuredHeap][i]);
    }
}
...
static void outputObjectCounts(FILE* file) {
    ...
    for (unsigned i = 0; i < classes.size(); ++i) {
        fprintf(file, "%30s, %8" PRIu64 ", %8" PRIu64 ", %8" PRIu64 "\n", classes[i].name,
                objectCountByHeapAndClass[Nursery][i], objectCountByHeapAndClass[TenuredHeap][i],
...
static void outputFinalizedHeapObjectLifetimeByClass(FILE* file) {
    ...
    fprintf(file, "# Total allocations: %" PRIu64 "\n", allocCount);
    fprintf(file, "Lifetime");
    for (unsigned i = 0; i < classes.size(); ++i) fprintf(file, ", %15s", classes[i].name);
    fprintf(file, "\n");

    for (unsigned i = 0; i < lifetimeBins; ++i) {
        fprintf(file, "%8d", binLimit(i));
        for (unsigned j = 0; j < classes.size(); ++j) {
            fprintf(file, ", %8" PRIu64, finalizedHeapObjectCountByClassAndLifetime[j][i]);
        }
        fprintf(file, "\n");
    }
}
...
static void outputLifetimeByKind(FILE* file, unsigned initialHeap) {
    ...
    fprintf(file, "# Total allocations: %" PRIu64 "\n", allocCount);
    fprintf(file, "Lifetime");
    for (unsigned i = 0; i < AllocKinds; ++i) fprintf(file, ", %15s", allocKindName(i));
    fprintf(file, "\n");

    for (unsigned i = 0; i < lifetimeBins; ++i) {
        fprintf(file, "%8d", binLimit(i));
        for (unsigned j = 0; j < AllocKinds; ++j)
            fprintf(file, ", %8" PRIu64, allocCountByHeapKindAndLifetime[initialHeap][j][i]);
        fprintf(file, "\n");
    }
}
...
static void outputLifetimeByClass(FILE* file, unsigned initialHeap) {
    ...
    for (unsigned i = 0; i < lifetimeBins; ++i) {
        fprintf(file, "%8d", binLimit(i));
        for (unsigned j = 0; j < classes.size(); ++j)
            fprintf(file, ", %8" PRIu64, objectCountByHeapClassAndLifetime[initialHeap][j][i]);
        fprintf(file, "\n");
    }
}
...
static void outputLifetimeByType(FILE* file, unsigned initialHeap) {
    ...
    fprintf(file, "Lifetime");
    for (unsigned i = 0; i < count; ++i) fprintf(file, ", %15s", types[topTypes[i]].getName());
    fprintf(file, "\n");
...
static uint64_t getTracePayload(uint64_t trace) { return trace & ((1lu << TracePayloadBits) - 1); }
...
static uint64_t expectTrace(FILE* file, GCTraceEvent event) {
    uint64_t trace;
    if (!readTrace(file, trace)) die("End of file while expecting trace %d", event);
    if (getTraceEvent(trace) != event)
        die("Expected trace %d but got trace %d", event, getTraceEvent(trace));
    return getTracePayload(trace);
}

static uint64_t expectDataAddress(FILE* file) { return expectTrace(file, TraceDataAddress); }

static uint32_t expectDataInt(FILE* file) { return (uint32_t)expectTrace(file, TraceDataInt); }
...
static void createClassInfo(const char* name, uint32_t flags, FinalizerKind hasFinalizer,
                            address clasp = 0) {
...
static void createTypeInfo(ClassId classId, uint32_t flags, address typeObject = 0) {
...
static void setObjectType(address obj, address typeObject) {
    ...
    if (j == nurseryThings.end()) {
        ...
        if (j == tenuredThings.end()) die("Can't find allocation for object %p", obj);
    }
...
static void finalizeThing(const AllocInfo& info) { processAlloc(info, allocCount); }
...
static void updateTimeslices(std::vector<uint64_t>& data, uint64_t lastTime, uint64_t currentTime,
                             uint64_t value) {
...
int main(int argc, const char* argv[]) {
    ...
    withOutputFile(outputBase, "lifetimeByHasFinalizer", outputLifetimeByHasFinalizer);
