...
#define Lock(lock)                                                                            \
    while (_InterlockedCompareExchange(lock, LOCK_IS_TAKEN, LOCK_IS_FREE) == LOCK_IS_TAKEN) { \
    };
...
static void dumpProfile(void) {
    ...
    for (e = entries; e; e = e->next) {
        ...
        vprof_printf(" %s [%lld : %lld] %lld %lld ", f(((double)e->sum) / ((double)e->count)),
                     (long long int)e->min, (long long int)e->max, (long long int)e->sum,
                     (long long int)e->count);
        if (e->h) {
            int j = MAXINT;
