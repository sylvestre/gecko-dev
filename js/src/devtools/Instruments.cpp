...
#define OldDTPerformanceLibraryPath                                                          \
    "/Library/Developer/4.0/Instruments/Frameworks/DTPerformanceSession.framework/Versions/" \
    "Current/DTPerformanceSession"
...
typedef DTPerformanceSessionRef (*DTPerformanceSessionCreateFunction)(CFStringRef, CFStringRef,
                                                                      CFDictionaryRef,
                                                                      CFErrorRef*);
typedef bool (*DTPerformanceSessionAddInstrumentFunction)(DTPerformanceSessionRef, CFStringRef,
                                                          CFDictionaryRef, dtps_errorcallback_t,
                                                          CFErrorRef*);
typedef bool (*DTPerformanceSessionIsRecordingFunction)(DTPerformanceSessionRef);
typedef bool (*DTPerformanceSessionStartFunction)(DTPerformanceSessionRef, CFArrayRef,
                                                  CFErrorRef*);
typedef bool (*DTPerformanceSessionStopFunction)(DTPerformanceSessionRef, CFArrayRef, CFErrorRef*);
typedef bool (*DTPerformanceSessionSaveFunction)(DTPerformanceSessionRef, CFStringRef,
                                                 CFErrorRef*);
...
bool Start(pid_t pid) {
    ...
    AutoReleased<CFNumberRef> interval = CFNumberCreate(0, kCFNumberIntType, &kSamplingInterval);
    if (!interval) {
        return false;
    }
    ...
    if (!DTPerformanceSessionAddInstrument(gSession, CFSTR(DTPerformanceSession_TimeProfiler),
...
void Stop(const char* profileName) {
    ...
    AutoReleased<CFStringRef> name =
        CFStringCreateWithFormat(kCFAllocatorDefault, nullptr, CFSTR("%s%s"), "/tmp/",
