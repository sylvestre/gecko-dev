...
MOZ_ALWAYS_INLINE bool IdIsIndex(jsid id, uint32_t* indexp) {
    ...
    if (atom->length() == 0 || !JS7_ISDEC(atom->latin1OrTwoByteChar(0))) return false;
...
extern ArrayObject* JS_FASTCALL NewDenseEmptyArray(JSContext* cx, HandleObject proto = nullptr,
 ...
 */
extern ArrayObject* JS_FASTCALL NewDenseUnallocatedArray(JSContext* cx, uint32_t length,
...
/* Create a dense array with length and capacity == 'length', initialized length set to 0. */
extern ArrayObject* JS_FASTCALL NewDenseFullyAllocatedArray(JSContext* cx, uint32_t length,
                                                            HandleObject proto = nullptr,
                                                            NewObjectKind newKind = GenericObject);

/* Create a dense array from the given array values, which must be rooted */
extern ArrayObject* NewDenseCopiedArray(JSContext* cx, uint32_t length, const Value* values,
                                        HandleObject proto = nullptr,
                                        NewObjectKind newKind = GenericObject);

/* Create a dense array based on templateObject with the given length. */
extern ArrayObject* NewDenseFullyAllocatedArrayWithTemplate(JSContext* cx, uint32_t length,
                                                            JSObject* templateObject);

/* Create a dense array with the same copy-on-write elements as another object. */
extern ArrayObject* NewDenseCopyOnWriteArray(JSContext* cx, HandleArrayObject templateObject,
                                             gc::InitialHeap heap);

extern ArrayObject* NewFullyAllocatedArrayTryUseGroup(JSContext* cx, HandleObjectGroup group,
                                                      size_t length,
                                                      NewObjectKind newKind = GenericObject);

extern ArrayObject* NewPartlyAllocatedArrayTryUseGroup(JSContext* cx, HandleObjectGroup group,
                                                       size_t length);

extern ArrayObject* NewFullyAllocatedArrayTryReuseGroup(JSContext* cx, HandleObject obj,
                                                        size_t length,
                                                        NewObjectKind newKind = GenericObject);

extern ArrayObject* NewPartlyAllocatedArrayTryReuseGroup(JSContext* cx, HandleObject obj,
                                                         size_t length);
...
extern ArrayObject* NewPartlyAllocatedArrayForCallingAllocationSite(JSContext* cx, size_t length,
                                                                    HandleObject proto);
...
extern ArrayObject* NewCopiedArrayForCallingAllocationSite(JSContext* cx, const Value* vp,
                                                           size_t length,
                                                           HandleObject proto = nullptr);

extern bool NewValuePair(JSContext* cx, const Value& val1, const Value& val2,
 ...
 */
extern bool WouldDefinePastNonwritableLength(HandleNativeObject obj, uint32_t index);

extern bool GetLengthProperty(JSContext* cx, HandleObject obj, uint32_t* lengthp);
 ...
 */
extern bool GetElements(JSContext* cx, HandleObject aobj, uint32_t length, js::Value* vp);

/* Natives exposed for optimization by the interpreter and JITs. */

extern bool intrinsic_ArrayNativeSort(JSContext* cx, unsigned argc, js::Value* vp);
...
extern JSObject* array_slice_dense(JSContext* cx, HandleObject obj, int32_t begin, int32_t end,
                                   HandleObject result);
...
extern ArrayObject* ArrayConstructorOneArg(JSContext* cx, HandleObjectGroup group,
                                           int32_t lengthInt);
...
extern bool IsWrappedArrayConstructor(JSContext* cx, const Value& v, bool* result);
