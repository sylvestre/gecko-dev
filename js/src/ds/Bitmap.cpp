...
SparseBitmap::~SparseBitmap() {
    if (data.initialized()) {
        for (Data::Range r(data.all()); !r.empty(); r.popFront()) js_delete(r.front().value());
    }
}
...
SparseBitmap::BitBlock& SparseBitmap::createBlock(Data::AddPtr p, size_t blockId) {
    MOZ_ASSERT(!p);
    AutoEnterOOMUnsafeRegion oomUnsafe;
    BitBlock* block = js_new<BitBlock>();
    if (!block || !data.add(p, blockId, block)) oomUnsafe.crash("Bitmap OOM");
    PodZero(block);
    return *block;
}
...
bool SparseBitmap::getBit(size_t bit) const {
    ...
    if (block) return (*block)[word - blockWord] & (uintptr_t(1) << (bit % JS_BITS_PER_WORD));
    return false;
}
...
void SparseBitmap::bitwiseOrRangeInto(size_t wordStart, size_t numWords, uintptr_t* target) const {
    ...
    BitBlock* block = getBlock(blockWord / WordsInBlock);
    if (block) {
        for (size_t i = 0; i < numWords; i++) target[i] |= (*block)[wordStart - blockWord + i];
    }
}
