 ...
 * the rug.
 */
extern void ErrorToException(JSContext* cx, JSErrorReport* reportp, JSErrorCallback callback,
                             void* userRef);
 ...
 */
extern JSObject* CopyErrorObject(JSContext* cx, JS::Handle<ErrorObject*> errobj);

static_assert(JSEXN_ERR == 0 && JSProto_Error + JSEXN_INTERNALERR == JSProto_InternalError &&
                  ...
                  JSEXN_WASMRUNTIMEERROR + 1 == JSEXN_WARN && JSEXN_WARN + 1 == JSEXN_NOTE &&
...
extern const char* ValueToSourceForError(JSContext* cx, HandleValue val, JSAutoByteString& bytes);

bool GetInternalError(JSContext* cx, unsigned errorNumber, MutableHandleValue error);
bool GetTypeError(JSContext* cx, unsigned errorNumber, MutableHandleValue error);
