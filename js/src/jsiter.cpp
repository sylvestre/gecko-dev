...
void NativeIterator::trace(JSTracer* trc) {
    for (GCPtrFlatString* str = begin(); str < end(); str++) TraceNullableEdge(trc, str, "prop");
    ...
    if (iterObj_) TraceManuallyBarrieredEdge(trc, &iterObj_, "iterObj");
}

typedef HashSet<jsid, DefaultHasher<jsid>> IdSet;

template <bool CheckForDuplicates>
static inline bool Enumerate(JSContext* cx, HandleObject pobj, jsid id, bool enumerable,
                             unsigned flags, Maybe<IdSet>& ht, AutoIdVector* props) {
    if (CheckForDuplicates) {
        if (!ht) {
            ht.emplace(cx);
            // Most of the time there are only a handful of entries.
            if (!ht->init(5)) return false;
        }

        // If we've already seen this, we definitely won't add it.
        IdSet::AddPtr p = ht->lookupForAdd(id);
        if (MOZ_UNLIKELY(!!p)) return true;
    ...
    if (!enumerable && !(flags & JSITER_HIDDEN)) return true;
    ...
    if (JSID_IS_SYMBOL(id) ? !(flags & JSITER_SYMBOLS) : (flags & JSITER_SYMBOLSONLY)) return true;

    return props->append(id);
}

template <bool CheckForDuplicates>
static bool EnumerateExtraProperties(JSContext* cx, HandleObject obj, unsigned flags,
                                     Maybe<IdSet>& ht, AutoIdVector* props) {
    MOZ_ASSERT(obj->getClass()->getNewEnumerate());

    AutoIdVector properties(cx);
    bool enumerableOnly = !(flags & JSITER_HIDDEN);
    if (!obj->getClass()->getNewEnumerate()(cx, obj, properties, enumerableOnly)) return false;
...
template <bool CheckForDuplicates>
static bool EnumerateNativeProperties(JSContext* cx, HandleNativeObject pobj, unsigned flags,
                                      Maybe<IdSet>& ht, AutoIdVector* props,
    ...
    if (flags & JSITER_SYMBOLSONLY) {
        ...
        if (isIndexed) {
            ...
            // them in the sort.
            if (!hasHoles) firstElemIndex = props->length();
            ...
            if (!MergeSort(ids, n, tmp.begin(), SortComparatorIntegerIds)) return false;
        }
        ...
        for (; !r.empty(); r.popFront()) {
            ...
            uint32_t dummy;
            if (isIndexed && IdIsIndex(id, &dummy)) continue;
...
template <bool CheckForDuplicates>
static bool EnumerateProxyProperties(JSContext* cx, HandleObject pobj, unsigned flags,
                                     Maybe<IdSet>& ht, AutoIdVector* props) {
    ...
    if (flags & JSITER_HIDDEN || flags & JSITER_SYMBOLS) {
        ...
        // flags.
        if (!Proxy::ownPropertyKeys(cx, pobj, proxyProps)) return false;
        ...
        for (size_t n = 0, len = proxyProps.length(); n < len; n++) {
            ...
            if (!(flags & JSITER_HIDDEN)) {
                if (!Proxy::getOwnPropertyDescriptor(cx, pobj, proxyProps[n], &desc)) return false;
    ...
    if (!Proxy::getOwnEnumerablePropertyKeys(cx, pobj, proxyProps)) return false;
...
static bool Snapshot(JSContext* cx, HandleObject pobj_, unsigned flags, AutoIdVector* props) {
    ...
    do {
        if (pobj->getClass()->getNewEnumerate()) {
            if (pobj->is<UnboxedPlainObject>() && pobj->as<UnboxedPlainObject>().maybeExpando()) {
                ...
                if (!pobj->is<UnboxedPlainObject>()) checkForDuplicates = true;

                if (checkForDuplicates) {
                    if (!EnumerateExtraProperties<true>(cx, pobj, flags, ht, props)) return false;
                } else {
                    if (!EnumerateExtraProperties<false>(cx, pobj, flags, ht, props)) return false;
                }
            ...
            if (JSEnumerateOp enumerate = pobj->getClass()->getEnumerate()) {
                if (!enumerate(cx, pobj.as<NativeObject>())) return false;
            }
            if (!EnumerateNativeProperties(cx, pobj.as<NativeObject>(), flags, ht, props,
                                           checkForDuplicates)) {
                return false;
            ...
            if (checkForDuplicates) {
                if (!EnumerateProxyProperties<true>(cx, pobj, flags, ht, props)) return false;
            } else {
                if (!EnumerateProxyProperties<false>(cx, pobj, flags, ht, props)) return false;
            }
        } else {
            MOZ_CRASH("non-native objects must have an enumerate op");
        }

        if (flags & JSITER_OWNONLY) break;
    ...
    if (!MergeSort(ids, n, tmp.begin(), SortComparatorIds(cx))) return false;
...
static inline PropertyIteratorObject* NewPropertyIteratorObject(JSContext* cx, unsigned flags) {
    if (flags & JSITER_ENUMERATE) {
        RootedObjectGroup group(cx,
                                ObjectGroup::defaultNewGroup(cx, &PropertyIteratorObject::class_,
                                                             TaggedProto(nullptr)));
        ...
        JS_TRY_VAR_OR_RETURN_NULL(
            cx, obj,
            NativeObject::create(cx, ITERATOR_FINALIZE_KIND, GetInitialHeap(GenericObject, clasp),
                                 shape, group));
    ...
    if (flags == 0) {
        ...
        if (!DefineDataProperty(cx, res, cx->names().next, value)) return nullptr;
    }

    return res;
}

NativeIterator* NativeIterator::allocateIterator(JSContext* cx, uint32_t numGuards,
                                                 uint32_t plength) {
...
NativeIterator* NativeIterator::allocateSentinel(JSContext* maybecx) {
...
inline void NativeIterator::init(JSObject* obj, JSObject* iterObj, unsigned flags,
...
static inline void RegisterEnumerator(JSContext* cx, PropertyIteratorObject* iterobj,
                                      NativeIterator* ni) {
...
static inline PropertyIteratorObject* VectorToKeyIterator(JSContext* cx, HandleObject obj,
                                                          unsigned flags, AutoIdVector& keys,
                                                          uint32_t numGuards) {
    MOZ_ASSERT(!(flags & JSITER_FOREACH));

    if (obj->isSingleton() && !JSObject::setIteratedSingleton(cx, obj)) return nullptr;
    ...
    if (!ni->initProperties(cx, iterobj, keys)) return nullptr;
...
static PropertyIteratorObject* VectorToValueIterator(JSContext* cx, HandleObject obj,
                                                     unsigned flags, AutoIdVector& keys) {
    MOZ_ASSERT(flags & JSITER_FOREACH);
...
JSObject* js::EnumeratedIdVectorToIterator(JSContext* cx, HandleObject obj, unsigned flags,
                                           AutoIdVector& props) {
    if (!(flags & JSITER_FOREACH)) return VectorToKeyIterator(cx, obj, flags, props, 0);

    return VectorToValueIterator(cx, obj, flags, props);
}

// Mainly used for .. in over null/undefined
JSObject* js::NewEmptyPropertyIterator(JSContext* cx, unsigned flags) {
    Rooted<PropertyIteratorObject*> iterobj(cx, NewPropertyIteratorObject(cx, flags));
    if (!iterobj) return nullptr;
...
/* static */ bool IteratorHashPolicy::match(PropertyIteratorObject* obj, const Lookup& lookup) {
    NativeIterator* ni = obj->getNativeIterator();
    if (ni->guard_key != lookup.key || ni->guard_length != lookup.numGuards) return false;

    return PodEqual(reinterpret_cast<ReceiverGuard*>(ni->guard_array), lookup.guards,
                    ni->guard_length);
}

static inline void UpdateNativeIterator(NativeIterator* ni, JSObject* obj) {
...
static inline bool CanCompareIterableObjectToCache(JSObject* obj) {
    if (obj->isNative()) return obj->as<NativeObject>().hasEmptyElements();
...
static MOZ_ALWAYS_INLINE PropertyIteratorObject* LookupInIteratorCache(JSContext* cx,
                                                                       JSObject* obj,
                                                                       uint32_t* numGuards) {
    ...
    JSObject* pobj = obj;
    do {
        if (!CanCompareIterableObjectToCache(pobj)) return nullptr;
    ...
    *numGuards = guards.length();
    ...
    if (ni->flags & (JSITER_ACTIVE | JSITER_UNREUSABLE)) return nullptr;

    return iterobj;
}

static bool CanStoreInIteratorCache(JSContext* cx, JSObject* obj) {
    do {
        if (obj->isNative()) {
            ...
            if (MOZ_UNLIKELY(IsTypedArrayClass(clasp))) return false;
            if (MOZ_UNLIKELY(clasp->getNewEnumerate() || clasp->getEnumerate())) return false;
        } else {
...
JSObject* js::GetIterator(JSContext* cx, HandleObject obj, unsigned flags) {
    ...
    if (flags == JSITER_ENUMERATE) {
        ...
        if (numGuards > 0 && !CanStoreInIteratorCache(cx, obj)) numGuards = 0;
    }
    ...
    if (flags == 0 || flags == JSITER_ENUMERATE) {
        if (MOZ_UNLIKELY(obj->is<ProxyObject>())) return Proxy::enumerate(cx, obj);
    }

    AutoIdVector keys(cx);
    if (!Snapshot(cx, obj, flags, &keys)) return nullptr;
    ...
    if (numGuards > 0) {
        if (!StoreInIteratorCache(cx, obj, iterobj)) return nullptr;
    }

    return iterobj;
}

PropertyIteratorObject* js::LookupInIteratorCache(JSContext* cx, HandleObject obj) {
    uint32_t numGuards = 0;
    return LookupInIteratorCache(cx, obj, &numGuards);
}

// ES 2017 draft 7.4.7.
JSObject* js::CreateIterResultObject(JSContext* cx, HandleValue value, bool done) {
    // Step 1 (implicit).
...
NativeObject* JSCompartment::getOrCreateIterResultTemplateObject(JSContext* cx) {
    if (iterResultTemplate_) return iterResultTemplate_;

    // Create template plain object
    RootedNativeObject templateObject(cx, NewBuiltinClassInstance<PlainObject>(cx, TenuredObject));
    if (!templateObject) return iterResultTemplate_;  // = nullptr

    // Create a new group for the template.
    Rooted<TaggedProto> proto(cx, templateObject->taggedProto());
    RootedObjectGroup group(
        cx, ObjectGroupCompartment::makeGroup(cx, templateObject->getClass(), proto));
    if (!group) return iterResultTemplate_;  // = nullptr
...
bool js::ThrowStopIteration(JSContext* cx) {
...
MOZ_ALWAYS_INLINE bool NativeIteratorNext(JSContext* cx, NativeIterator* ni,
                                          MutableHandleValue rval, bool* done) {
    *done = false;
    ...
    if (!ValueToId<CanGC>(cx, current, &id)) return false;
    ni->incCursor();
    RootedObject obj(cx, ni->obj);
    if (!GetProperty(cx, obj, obj, id, rval)) return false;

    return true;
}

bool js::IsPropertyIterator(HandleValue v) {
    return v.isObject() && v.toObject().is<PropertyIteratorObject>();
}

MOZ_ALWAYS_INLINE bool property_iterator_next_impl(JSContext* cx, const CallArgs& args) {
    ...
    bool done;
    if (!NativeIteratorNext(cx, ni, &value, &done)) return false;
...
size_t PropertyIteratorObject::sizeOfMisc(mozilla::MallocSizeOf mallocSizeOf) const {
    return mallocSizeOf(getPrivate());
}

void PropertyIteratorObject::trace(JSTracer* trc, JSObject* obj) {
    if (NativeIterator* ni = obj->as<PropertyIteratorObject>().getNativeIterator()) ni->trace(trc);
}

void PropertyIteratorObject::finalize(FreeOp* fop, JSObject* obj) {
    if (NativeIterator* ni = obj->as<PropertyIteratorObject>().getNativeIterator()) fop->free_(ni);
}
...
const Class PropertyIteratorObject::class_ = {"Iterator",
...
const Class ArrayIteratorObject::class_ = {"Array Iterator",
                                           JSCLASS_HAS_RESERVED_SLOTS(ArrayIteratorSlotCount)};

ArrayIteratorObject* js::NewArrayIteratorObject(JSContext* cx, NewObjectKind newKind) {
...
enum { StringIteratorSlotIteratedObject, StringIteratorSlotNextIndex, StringIteratorSlotCount };

const Class StringIteratorObject::class_ = {"String Iterator",
                                            JSCLASS_HAS_RESERVED_SLOTS(StringIteratorSlotCount)};

static const JSFunctionSpec string_iterator_methods[] = {
    JS_SELF_HOSTED_FN("next", "StringIteratorNext", 0, 0), JS_FS_END};

StringIteratorObject* js::NewStringIteratorObject(JSContext* cx, NewObjectKind newKind) {
...
JSObject* js::ValueToIterator(JSContext* cx, unsigned flags, HandleValue vp) {
    RootedObject obj(cx);
    if (vp.isObject()) {
        /* Common case. */
        obj = &vp.toObject();
...
bool js::CloseIterator(JSContext* cx, HandleObject obj) {
    if (obj->is<PropertyIteratorObject>()) {
        ...
        if (genObj->isClosed()) return true;
...
bool js::UnwindIteratorForException(JSContext* cx, HandleObject obj) {
    RootedValue v(cx);
    bool getOk = cx->getPendingException(&v);
    cx->clearPendingException();
    if (!CloseIterator(cx, obj)) return false;
...
bool js::IteratorCloseForException(JSContext* cx, HandleObject obj) {
    ...
    if (!GetProperty(cx, obj, obj, cx->names().return_, &returnMethod)) return false;

    // Step 4.
    //
    // Do nothing if "return" is null or undefined. Throw a TypeError if the
    // method is not IsCallable.
    if (returnMethod.isNullOrUndefined()) return true;
    if (!IsCallable(returnMethod)) return ReportIsNotFunction(cx, returnMethod);
    ...
    if (isClosingGenerator) {
        ...
        if (!ok) return false;
        if (!rval.isObject()) return ThrowCheckIsObject(cx, CheckIsObjectKind::IteratorReturn);
    } else {
...
void js::UnwindIteratorForUncatchableException(JSContext* cx, JSObject* obj) {
    if (obj->is<PropertyIteratorObject>()) {
        NativeIterator* ni = obj->as<PropertyIteratorObject>().getNativeIterator();
        if (ni->flags & JSITER_ENUMERATE) ni->unlink();
    }
}
            ...
            for (GCPtrFlatString* idp = props_cursor; idp < props_end; ++idp) {
                if (predicate(*idp)) {
                    ...
                    if (proto) {
                        ...
                        if (!ValueToId<CanGC>(cx, idv, &id)) return false;

                        Rooted<PropertyDescriptor> desc(cx);
                        if (!GetPropertyDescriptor(cx, proto, id, &desc)) return false;
                     ...
                     * ni, start over.
                     */
                    if (props_end != ni->props_end || props_cursor != ni->props_cursor) goto again;
                    ...
                    if (idp == props_cursor) {
                        ...
                        for (GCPtrFlatString* p = idp; p + 1 != props_end; p++) *p = *(p + 1);
                    ...
                    if (predicate.matchesAtMostOne()) break;
                }
            }
        }
        ni = ni->next();
...
bool js::SuppressDeletedProperty(JSContext* cx, HandleObject obj, jsid id) {
    if (MOZ_LIKELY(!cx->compartment()->objectMaybeInIteration(obj))) return true;
...
bool js::SuppressDeletedElement(JSContext* cx, HandleObject obj, uint32_t index) {
    if (MOZ_LIKELY(!cx->compartment()->objectMaybeInIteration(obj))) return true;
...
bool js::IteratorMore(JSContext* cx, HandleObject iterobj, MutableHandleValue rval) {
    // Fast path for native iterators.
    if (iterobj->is<PropertyIteratorObject>()) {
        NativeIterator* ni = iterobj->as<PropertyIteratorObject>().getNativeIterator();
        bool done;
        if (!NativeIteratorNext(cx, ni, rval, &done)) return false;

        if (done) rval.setMagic(JS_NO_ITER_VALUE);
        return true;
    }
    ...
    if (!GetProperty(cx, iterobj, iterobj, cx->names().next, rval)) return false;
    ...
    if (!js::Call(cx, rval, iterval, args, rval)) {
        ...
        if (!cx->getPendingException(&exception)) return false;
        if (!JS_IsStopIteration(exception)) return false;
...
static bool stopiter_hasInstance(JSContext* cx, HandleObject obj, MutableHandleValue v, bool* bp) {
    *bp = JS_IsStopIteration(v);
    return true;
}

static const ClassOps StopIterationObjectClassOps = {nullptr, /* addProperty */
                                                     nullptr, /* delProperty */
                                                     nullptr, /* enumerate */
                                                     nullptr, /* enumerate */
                                                     nullptr, /* resolve */
...
const Class StopIterationObject::class_ = {"StopIteration",
...
/* static */ bool GlobalObject::initIteratorProto(JSContext* cx, Handle<GlobalObject*> global) {
    if (global->getReservedSlot(ITERATOR_PROTO).isObject()) return true;
...
/* static */ bool GlobalObject::initArrayIteratorProto(JSContext* cx,
                                                       Handle<GlobalObject*> global) {
    if (global->getReservedSlot(ARRAY_ITERATOR_PROTO).isObject()) return true;
    ...
    RootedObject proto(
        cx, GlobalObject::createBlankPrototypeInheriting(cx, global, cls, iteratorProto));
...
/* static */ bool GlobalObject::initStringIteratorProto(JSContext* cx,
                                                        Handle<GlobalObject*> global) {
    if (global->getReservedSlot(STRING_ITERATOR_PROTO).isObject()) return true;
...
JSObject* js::InitStopIterationClass(JSContext* cx, HandleObject obj) {
    Handle<GlobalObject*> global = obj.as<GlobalObject>();
    if (!global->getPrototype(JSProto_StopIteration).isObject()) {
        RootedObject proto(
            cx, GlobalObject::createBlankPrototype(cx, global, &StopIterationObject::class_));
        if (!proto || !FreezeObject(cx, proto)) return nullptr;
