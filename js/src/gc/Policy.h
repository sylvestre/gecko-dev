...
#define FOR_EACH_INTERNAL_TAGGED_GC_POINTER_TYPE(D) D(js::TaggedProto)
...
struct InternalGCPointerPolicy {
    ...
    static bool isValid(T v) { return gc::IsCellPointerValidOrNull(v); }
};
...
#define DEFINE_INTERNAL_GC_POLICY(type) \
    template <>                         \
    struct GCPolicy<type> : public js::InternalGCPointerPolicy<type> {};
...
struct GCPolicy<js::HeapPtr<T>> {
    ...
    static bool needsSweep(js::HeapPtr<T>* thingp) { return js::gc::IsAboutToBeFinalized(thingp); }
};
