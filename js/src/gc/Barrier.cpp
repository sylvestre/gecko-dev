...
bool RuntimeFromActiveCooperatingThreadIsHeapMajorCollecting(JS::shadow::Zone* shadowZone) {
...
bool HeapSlot::preconditionForSet(NativeObject* owner, Kind kind, uint32_t slot) const {
...
void HeapSlot::assertPreconditionForWriteBarrierPost(NativeObject* obj, Kind kind, uint32_t slot,
...
/* static */ HashNumber MovableCellHasher<T>::hash(const Lookup& l) {
    ...
    MOZ_ASSERT(CurrentThreadCanAccessZone(l->zoneFromAnyThread()) ||
               l->zoneFromAnyThread()->isSelfHostingZone() || CurrentThreadIsPerformingGC());
