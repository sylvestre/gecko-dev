...
static inline bool IsThingPoisoned(T* thing) {
    const uint8_t poisonBytes[] = {JS_FRESH_NURSERY_PATTERN,     JS_SWEPT_NURSERY_PATTERN,
...
bool ThingIsPermanentAtomOrWellKnownSymbol(JSString* str) { return str->isPermanentAtom(); }
bool ThingIsPermanentAtomOrWellKnownSymbol(JSFlatString* str) { return str->isPermanentAtom(); }
bool ThingIsPermanentAtomOrWellKnownSymbol(JSLinearString* str) { return str->isPermanentAtom(); }
bool ThingIsPermanentAtomOrWellKnownSymbol(JSAtom* atom) { return atom->isPermanent(); }
bool ThingIsPermanentAtomOrWellKnownSymbol(PropertyName* name) { return name->isPermanent(); }
bool ThingIsPermanentAtomOrWellKnownSymbol(JS::Symbol* sym) { return sym->isWellKnownSymbol(); }
...
void js::CheckTracedThing(JSTracer* trc, T* thing) {
    ...
    if (!trc->checkEdges()) return;

    if (IsForwarded(thing)) thing = Forwarded(thing);

    /* This function uses data that's not available in the nursery. */
    if (IsInsideNursery(thing)) return;
     ...
     */
    if (IsOwnedByOtherRuntime(trc->runtime(), thing)) return;
     ...
     */
    MOZ_ASSERT_IF(
        JS::CurrentThreadIsHeapBusy() && !zone->isGCCompacting() && !rt->gc.isBackgroundSweeping(),
...
static bool ShouldTraceCrossCompartment(JSTracer* trc, JSObject* src, Cell* cell) {
    if (!trc->isMarkingTracer()) return true;
    ...
    if (!src->zone()->isGCMarking() && !zone->isGCMarking()) return false;
    ...
    if (color == MarkColor::Black) {
        ...
        if (zone->isGCMarkingBlack()) {
             ...
             */
            if (!tenured.isMarkedAny()) DelayCrossCompartmentGrayMarking(src);
            return false;
        }
...
static void AssertRootMarkingPhase(JSTracer* trc) {
    MOZ_ASSERT_IF(trc->isMarkingTracer(), trc->runtime()->gc.state() == State::NotActive ||
...
#define INSTANTIATE_ALL_VALID_TRACE_FUNCTIONS(type)                                               \
    ...
    template void js::TraceRange<type>(JSTracer*, size_t, WriteBarrieredBase<type>*,              \
                                       const char*);                                              \
...
#define INSTANTIATE_PUBLIC_TRACE_FUNCTIONS(type)                                           \
    ...
    template JS_PUBLIC_API(void)                                                           \
        js::UnsafeTraceManuallyBarrieredEdge<type>(JSTracer*, type*, const char*);         \
...
template <typename T>
void js::TraceManuallyBarrieredCrossCompartmentEdge(JSTracer* trc, JSObject* src, T* dst,
                                                    const char* name) {
    if (ShouldTraceCrossCompartment(trc, src, *dst)) DispatchToTracer(trc, dst, name);
}
...
template <typename T>
void js::TraceCrossCompartmentEdge(JSTracer* trc, JSObject* src, WriteBarrieredBase<T>* dst,
                                   const char* name) {
...
#undef IS_SAME_TYPE_OR
    if (trc->isMarkingTracer()) return DoMarking(GCMarker::fromTracer(trc), *thingp);
    if (trc->isTenuringTracer()) return static_cast<TenuringTracer*>(trc)->traverse(thingp);
...
static inline bool ShouldMark(GCMarker* gcmarker, T thing) {
    ...
    if (IsOwnedByOtherRuntime(gcmarker->runtime(), thing)) return false;
...
bool ShouldMark<JSObject*>(GCMarker* gcmarker, JSObject* obj) {
    // Don't trace things that are owned by another runtime.
    if (IsOwnedByOtherRuntime(gcmarker->runtime(), obj)) return false;
    ...
    // incremental slice.
    if (IsInsideNursery(obj)) return false;
...
void DoMarking(GCMarker* gcmarker, T* thing) {
    ...
    if (!ShouldMark(gcmarker, thing)) return;
...
void js::GCMarker::markAndTraceChildren(T* thing) {
    if (ThingIsPermanentAtomOrWellKnownSymbol(thing)) return;
    if (mark(thing)) thing->traceChildren(this);
}
namespace js {
template <>
...
void js::GCMarker::markAndScan(T* thing) {
    if (ThingIsPermanentAtomOrWellKnownSymbol(thing)) return;
    if (mark(thing)) eagerlyMarkChildren(thing);
}
namespace js {
template <>
...
void LazyScript::traceChildren(JSTracer* trc) {
    if (script_) TraceWeakEdge(trc, &script_, "script");

    if (function_) TraceEdge(trc, &function_, "function");

    if (sourceObject_) TraceEdge(trc, &sourceObject_, "sourceObject");

    if (enclosingScope_) TraceEdge(trc, &enclosingScope_, "enclosingScope");
...
inline void js::GCMarker::eagerlyMarkChildren(LazyScript* thing) {
    if (thing->script_) noteWeakEdge(thing->script_.unsafeUnbarrieredForTracing());

    if (thing->function_) traverseEdge(thing, static_cast<JSObject*>(thing->function_));

    if (thing->sourceObject_) traverseEdge(thing, static_cast<JSObject*>(thing->sourceObject_));

    if (thing->enclosingScope_) traverseEdge(thing, static_cast<Scope*>(thing->enclosingScope_));
...
void Shape::traceChildren(JSTracer* trc) {
    ...
    if (parent) TraceEdge(trc, &parent, "parent");

    if (hasGetterObject()) TraceManuallyBarrieredEdge(trc, &asAccessorShape().getterObj, "getter");
    if (hasSetterObject()) TraceManuallyBarrieredEdge(trc, &asAccessorShape().setterObj, "setter");
}
...
inline void js::GCMarker::eagerlyMarkChildren(JSLinearString* linearStr) {
    ...
    while (linearStr->hasBase()) {
        ...
        MOZ_ASSERT(linearStr->JSString::isLinear());
        if (linearStr->isPermanentAtom()) break;
        AssertShouldMarkInZone(linearStr);
        if (!mark(static_cast<JSString*>(linearStr))) break;
    }
}
    ...
    while (true) {
        ...
        if (!left->isPermanentAtom() && mark(left)) {
            if (left->isLinear()) {
                ...
                // scan it later.
                if (next && !stack.pushTempRope(next)) delayMarkingChildren(next);
                next = &left->asRope();
            }
        }
        if (next) {
...
static inline void TraceNullableBindingNames(JSTracer* trc, BindingName* names, uint32_t length) {
    for (uint32_t i = 0; i < length; i++) {
        if (JSAtom* name = names[i].name()) TraceManuallyBarrieredEdge(trc, &name, "scope name");
    }
};
void BindingName::trace(JSTracer* trc) {
    if (JSAtom* atom = name()) TraceManuallyBarrieredEdge(trc, &atom, "binding name");
}
void BindingIter::trace(JSTracer* trc) { TraceNullableBindingNames(trc, names_, length_); }
void LexicalScope::Data::trace(JSTracer* trc) { TraceBindingNames(trc, names, length); }
...
void VarScope::Data::trace(JSTracer* trc) { TraceBindingNames(trc, names, length); }
void GlobalScope::Data::trace(JSTracer* trc) { TraceBindingNames(trc, names, length); }
void EvalScope::Data::trace(JSTracer* trc) { TraceBindingNames(trc, names, length); }
...
void WasmFunctionScope::Data::trace(JSTracer* trc) { TraceBindingNames(trc, names, length); }
...
inline void js::GCMarker::eagerlyMarkChildren(Scope* scope) {
    if (scope->enclosing_) traverseEdge(scope, static_cast<Scope*>(scope->enclosing_));
    ...
    switch (scope->kind_) {
        ...
        case ScopeKind::WasmInstance: {
            WasmInstanceScope::Data* data =
                reinterpret_cast<WasmInstanceScope::Data*>(scope->data_);
        ...
        case ScopeKind::WasmFunction: {
            WasmFunctionScope::Data* data =
                reinterpret_cast<WasmFunctionScope::Data*>(scope->data_);
            names = data->names;
            length = data->length;
            break;
        }
    ...
    if (scope->kind_ == ScopeKind::Function) {
        for (uint32_t i = 0; i < length; i++) {
            if (JSAtom* name = names[i].name()) traverseEdge(scope, static_cast<JSString*>(name));
        }
    } else {
        for (uint32_t i = 0; i < length; i++)
...
void js::ObjectGroup::traceChildren(JSTracer* trc) {
    ...
    if (proto().isObject()) TraceEdge(trc, &proto(), "group_proto");
    ...
    if (maybeUnboxedLayout()) unboxedLayout().trace(trc);
...
void js::GCMarker::lazilyMarkChildren(ObjectGroup* group) {
    ...
    if (group->proto().isObject()) traverseEdge(group, group->proto().toObject());
    ...
    if (group->newScript()) group->newScript()->trace(this);

    if (group->maybePreliminaryObjects()) group->maybePreliminaryObjects()->trace(this);

    if (group->maybeUnboxedLayout()) group->unboxedLayout().trace(this);
...
static inline NativeObject* CallTraceHook(Functor f, JSTracer* trc, JSObject* obj,
                                          CheckGeneration check, Args&&... args) {
    const Class* clasp = obj->getClass();
    MOZ_ASSERT(clasp);
    MOZ_ASSERT(obj->isNative() == clasp->isNative());

    if (!clasp->hasTrace()) return &obj->as<NativeObject>();
    ...
    if (clasp == &UnboxedPlainObject::class_) {
        ...
        if (*pexpando) f(pexpando, mozilla::Forward<Args>(args)...);
    ...
    if (!clasp->isNative()) return nullptr;
...
static void VisitTraceList(F f, const int32_t* traceList, uint8_t* memory, Args&&... args) {
    ...
    while (*traceList != -1) {
        ...
        if (*objp) f(objp, mozilla::Forward<Args>(args)...);
        traceList++;
    }
    traceList++;
    while (*traceList != -1) {
...
inline static bool ObjectDenseElementsMayBeMarkable(NativeObject* nobj) {
    ...
    if (nobj->getDenseInitializedLength() < MinElementsLength || nobj->isSingleton()) return true;

    ObjectGroup* group = nobj->group();
    if (group->needsSweep() || group->unknownProperties()) return true;
...
scan_obj : {
    ...
    NativeObject* nobj =
        CallTraceHook(TraverseObjectFunctor(), this, obj, CheckGeneration::DoChecks, this, obj);
    if (!nobj) return;
    ...
    do {
        if (nobj->hasEmptyElements()) break;
        ...
        if (!ObjectDenseElementsMayBeMarkable(nobj)) break;

        vp = nobj->getDenseElementsAllowCopyOnWrite();
        end = vp + nobj->getDenseInitializedLength();

        if (!nslots) goto scan_value_array;
...
bool GCMarker::restoreValueArray(const MarkStack::SavedValueArray& array, HeapSlot** vpp,
                                 HeapSlot** endp) {
    JSObject* objArg = array.ptr.asSavedValueArrayObject();
    if (!objArg->isNative()) return false;
...
inline Cell* MarkStack::TaggedPtr::ptr() const { return reinterpret_cast<Cell*>(bits & ~TagMask); }
...
inline MarkStack::SavedValueArray::SavedValueArray(JSObject* obj, size_t indexArg,
...
void MarkStack::setBaseCapacity(JSGCMode mode) {
    ...
    if (baseCapacity_ > maxCapacity_) baseCapacity_ = maxCapacity_;
}
...
inline bool MarkStack::pushTempRope(JSRope* rope) { return pushTaggedPtr(TempRopeTag, rope); }
...
inline bool MarkStack::push(const ValueArray& array) {
    ...
    if (!ensureSpace(ValueArrayWords)) return false;
...
inline bool MarkStack::ensureSpace(size_t count) {
    if ((tos_ + count) <= end_) return true;
...
bool MarkStack::enlarge(size_t count) {
    ...
    if (newCapacity < capacity() + count) return false;
...
inline size_t MarkStackIter::position() const { return pos_ - stack_.stack_; }
...
void GCMarker::stop() {
    ...
    for (GCZonesIter zone(runtime()); !zone.done(); zone.next()) {
        if (!zone->gcWeakKeys().clear()) oomUnsafe.crash("clearing weak keys in GCMarker::stop()");
    }
}
...
void GCMarker::pushTaggedPtr(T* ptr) {
    checkZone(ptr);
    if (!stack.push(ptr)) delayMarkingChildren(ptr);
}

void GCMarker::pushValueArray(JSObject* obj, HeapSlot* start, HeapSlot* end) {
    checkZone(obj);
    if (!stack.push(obj, start, end)) delayMarkingChildren(obj);
}
...
void GCMarker::enterWeakMarkingMode() {
    ...
    if (linearWeakMarkingDisabled_) return;
    ...
    if (weakMapAction() == ExpandWeakMaps) {
        ...
        for (SweepGroupZonesIter zone(runtime()); !zone.done(); zone.next()) {
            for (WeakMapBase* m : zone->gcWeakMapList()) {
                if (m->marked) (void)m->markIteratively(this);
            }
        }
    }
}
...
static void PushArenaTyped(GCMarker* gcmarker, Arena* arena) {
    for (ArenaCellIterUnderGC i(arena); !i.done(); i.next()) gcmarker->traverse(i.get<T>());
}
...
void gc::PushArena(GCMarker* gcmarker, Arena* arena) {
    DispatchTraceKindTyped(PushArenaFunctor(), MapAllocToTraceKind(arena->getAllocKind()),
                           gcmarker, arena);
}
...
Zone* GCMarker::stackContainsCrossZonePointerTo(const Cell* target) const {
    ...
    for (MarkStackIter iter(stack); !iter.done(); iter.next()) {
        if (iter.peekTag() != MarkStack::ObjectTag) continue;
        ...
        if (source->is<ProxyObject>()) {
            ...
            if (value.isObject() && &value.toObject() == target) return sourceZone;
        }

        if (Debugger::isDebuggerCrossCompartmentEdge(source, target)) return sourceZone;
    }
...
void TenuringTracer::traverse(JSObject** objp) {
    ...
    JSObject* obj = *objp;
    if (!IsInsideNursery(obj) || nursery().getForwardedPointer(objp)) return;
...
namespace js {
namespace gc {
template void StoreBuffer::MonoTypeBuffer<StoreBuffer::ValueEdge>::trace(StoreBuffer*,
                                                                         TenuringTracer&);
template void StoreBuffer::MonoTypeBuffer<StoreBuffer::SlotsEdge>::trace(StoreBuffer*,
                                                                         TenuringTracer&);
template void StoreBuffer::MonoTypeBuffer<StoreBuffer::CellPtrEdge>::trace(StoreBuffer*,
                                                                           TenuringTracer&);
}  // namespace gc
}  // namespace js
...
void js::TenuringTracer::traceObject(JSObject* obj) {
    NativeObject* nobj =
        CallTraceHook(TenuringFunctor(), this, obj, CheckGeneration::NoChecks, *this);
    if (!nobj) return;
    ...
    if (!nobj->hasEmptyElements() && !nobj->denseElementsAreCopyOnWrite() &&
        ...
        Value* elems =
            static_cast<HeapSlot*>(nobj->getDenseElements())->unsafeUnbarrieredForTracing();
...
inline void js::TenuringTracer::insertIntoFixupList(RelocationOverlay* entry) {
    *tail = entry;
    tail = &entry->nextRef();
    *tail = nullptr;
}
...
inline T* js::TenuringTracer::allocTenured(Zone* zone, AllocKind kind) {
    ...
    if (!t) {
        ...
        if (!t) oomUnsafe.crash(ChunkSize, "Failed to allocate object while tenuring.");
    }
...
size_t js::TenuringTracer::moveSlotsToTenured(NativeObject* dst, NativeObject* src,
    ...
    if (!src->hasDynamicSlots()) return 0;
...
size_t js::TenuringTracer::moveElementsToTenured(NativeObject* dst, NativeObject* src,
                                                 AllocKind dstKind) {
    if (src->hasEmptyElements() || src->denseElementsAreCopyOnWrite()) return 0;
...
static bool IsMarkedInternalCommon(T* thingp) {
    ...
    Zone* zone = thing.zoneFromAnyThread();
    if (!zone->isCollectingFromAnyThread() || zone->isGCFinished()) return true;
...
static bool IsMarkedInternal(JSRuntime* rt, T** thingp) {
    if (IsOwnedByOtherRuntime(rt, *thingp)) return true;
...
bool js::gc::IsAboutToBeFinalizedDuringSweep(TenuredCell& tenured) {
    ...
    if (tenured.arena()->allocatedDuringIncremental) return false;
...
void UnmarkGrayTracer::onChild(const JS::GCCellPtr& thing) {
    ...
    TenuredCell& tenured = cell->asTenured();
    if (!tenured.isMarkedGray()) return;

    tenured.markBlack();
    unmarkedAny = true;

    if (!stack.append(thing)) oom = true;
}
...
void UnmarkGrayTracer::unmark(JS::GCCellPtr cell) {
    ...
    while (!stack.empty() && !oom) TraceChildren(this, stack.popCopy());
...
MarkInfo GetMarkInfo(Cell* rawCell) {
    if (!rawCell->isTenured()) return MarkInfo::NURSERY;

    TenuredCell* cell = &rawCell->asTenured();
    if (cell->isMarkedGray()) return MarkInfo::GRAY;
    if (cell->isMarkedBlack()) return MarkInfo::BLACK;
    return MarkInfo::UNMARKED;
}

uintptr_t* GetMarkWordAddress(Cell* cell) {
    if (!cell->isTenured()) return nullptr;
