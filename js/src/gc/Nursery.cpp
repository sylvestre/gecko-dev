...
inline void js::Nursery::NurseryChunk::poisonAndInit(JSRuntime* rt, uint8_t poison) {
    JS_POISON(this, poison, ChunkSize);
    init(rt);
}

inline void js::Nursery::NurseryChunk::init(JSRuntime* rt) {
    new (&trailer) gc::ChunkTrailer(rt, &rt->gc.storeBuffer());
}

/* static */ inline js::Nursery::NurseryChunk* js::Nursery::NurseryChunk::fromChunk(Chunk* chunk) {
    return reinterpret_cast<NurseryChunk*>(chunk);
}

inline Chunk* js::Nursery::NurseryChunk::toChunk(JSRuntime* rt) {
...
bool js::Nursery::init(uint32_t maxNurseryBytes, AutoLockGCBgAlloc& lock) {
    ...
    if (!freeMallocedBuffersTask || !freeMallocedBuffersTask->init()) return false;
    ...
    if (maxNurseryChunks_ == 0) return true;
    ...
    if (env) {
        if (0 == strcmp(env, "help")) {
            fprintf(stderr,
                    ...
                    "\tAfter a minor GC, report any ObjectGroups with at least N instances "
                    "tenured.\n");
            exit(0);
        }
...
void js::Nursery::enterZealMode() {
    if (isEnabled()) updateNumChunks(maxNurseryChunks_);
}
...
JSObject* js::Nursery::allocateObject(JSContext* cx, size_t size, size_t numDynamic,
...
void* js::Nursery::allocate(size_t size) {
    ...
    if (currentEnd() < position() + size) {
        if (currentChunk_ + 1 == numChunks()) return nullptr;
...
void* js::Nursery::reallocateBuffer(JSObject* obj, void* oldBuffer, size_t oldBytes,
                                    size_t newBytes) {
    ...
    if (!isInside(oldBuffer)) {
        void* newBuffer =
            obj->zone()->pod_realloc<uint8_t>((uint8_t*)oldBuffer, oldBytes, newBytes);
...
#endif
    if (!forwardedBuffers.put(oldData, newData)) oomUnsafe.crash("Nursery::setForwardingPointer");
}
...
#undef EXTRACT_NAME
    ...
    size_t i = 0;
    for (auto time : profileDurations_) json.property(names[i++], time, json.MICROSECONDS);
...
/* static */ void js::Nursery::printProfileDurations(const ProfileDurations& times) {
    for (auto time : times)
...
inline void js::Nursery::startProfile(ProfileKey key) { startTimes_[key] = TimeStamp::Now(); }
...
static inline bool IsFullStoreBufferReason(JS::gcreason::Reason reason) {
    return reason == JS::gcreason::FULL_WHOLE_CELL_BUFFER ||
           ...
           reason == JS::gcreason::FULL_SLOT_BUFFER || reason == JS::gcreason::FULL_SHAPE_BUFFER;
}
...
void js::Nursery::collect(JS::gcreason::Reason reason) {
    ...
    // restarting firefox.
    if (maxNurseryChunks_ == 0) disable();
...
void js::Nursery::doCollection(JS::gcreason::Reason reason, TenureCountCache& tenureCounts) {
    JSRuntime* rt = runtime();
    ...
    if (sb.cancelIonCompilations()) js::CancelOffThreadIonCompilesUsingNurseryPointers(rt);
...
#ifdef JS_GC_ZEAL
    if (rt->hasZealMode(ZealMode::CheckHashTablesOnMinorGC)) CheckHashTablesAfterMovingGC(rt);
#endif
...
void js::Nursery::freeMallocedBuffers() {
    ...
    if (!started) freeMallocedBuffersTask->runFromActiveCooperatingThread(runtime());
...
void js::Nursery::sweep(JSTracer* trc) {
    ...
    for (CompartmentsIter c(runtime(), SkipAtoms); !c.done(); c.next()) c->sweepAfterMinorGC(trc);
...
void js::Nursery::clear() {
    ...
    if (runtime()->hasZealMode(ZealMode::GenerationalGC)) {
        ...
        if (currentChunk_ + 1 == numChunks()) setCurrentChunk(0);
    } else
#endif
    {
...
size_t js::Nursery::spaceToEnd() const {
...
void js::Nursery::maybeResizeNursery(JS::gcreason::Reason reason) {
     ...
     */
    const float promotionRate = float(previousGC.tenuredBytes) / float(previousGC.nurseryCapacity);
...
void js::Nursery::shrinkAllocableSpace(unsigned removeNumChunks) {
#ifdef JS_GC_ZEAL
    if (runtime()->hasZealMode(ZealMode::GenerationalGC)) return;
#endif
...
void js::Nursery::updateNumChunks(unsigned newCount) {
...
void js::Nursery::updateNumChunksLocked(unsigned newCount, AutoLockGCBgAlloc& lock) {
    ...
    if (!chunks_.resize(newCount)) return;
