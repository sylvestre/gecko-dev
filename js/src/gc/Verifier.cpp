...
static VerifyNode* MakeNode(VerifyPreTracer* trc, void* thing, JS::TraceKind kind) {
...
void gc::GCRuntime::startVerifyPreBarriers() {
    ...
    if (IsIncrementalGCUnsafe(rt) != AbortReason::None || TlsContext.get()->keepAtoms ||
        rt->hasHelperThreadZones() || rt->cooperatingContexts().length() != 1) {
        return;
    }
...
static bool IsMarkedOrAllocated(const EdgeValue& edge) {
    if (!edge.thing || IsMarkedOrAllocated(TenuredCell::fromPointer(edge.thing))) return true;
            ...
            if (node->count <= MAX_VERIFIER_EDGES) {
                for (uint32_t i = 0; i < node->count; i++) {
                    ...
                    if (!IsMarkedOrAllocated(edge)) {
                        char msgbuf[1024];
                        SprintfLiteral(
                            msgbuf, "[barrier verifier] Unmarked edge: %s %p '%s' edge to %s %p",
...
class HeapCheckTracerBase : public JS::CallbackTracer {
   ...
   protected:
    ...
    Cell* parentCell() { return parentIndex == -1 ? nullptr : stack[parentIndex].thing.asCell(); }
...
HeapCheckTracerBase::HeapCheckTracerBase(JSRuntime* rt, WeakMapTraceKind weakTraceKind)
    : CallbackTracer(rt, weakTraceKind), failures(0), rt(rt), oom(false), parentIndex(-1) {
#ifdef DEBUG
    setCheckEdges(false);
#endif
}
...
bool HeapCheckTracerBase::traceHeap(AutoLockForExclusiveAccess& lock) {
    // The analysis thinks that traceRuntime might GC by calling a GC callback.
    JS::AutoSuppressGCAnalysis nogc;
    if (!rt->isBeingDestroyed()) rt->gc.traceRuntime(this, lock);
...
void CheckHeapTracer::check(AutoLockForExclusiveAccess& lock) {
    if (!traceHeap(lock)) return;
...
bool CheckGrayMarkingTracer::check(AutoLockForExclusiveAccess& lock) {
    if (!traceHeap(lock)) return true;  // Ignore failure.
