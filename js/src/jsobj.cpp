...
void js::ReportNotObjectArg(JSContext* cx, const char* nth, const char* fun, HandleValue v) {
    MOZ_ASSERT(!v.isObject());

    JSAutoByteString bytes;
    if (const char* chars = ValueToSourceForError(cx, v, bytes)) {
        JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, JSMSG_NOT_NONNULL_OBJECT_ARG, nth,
                                   fun, chars);
    }
}

void js::ReportNotObjectWithName(JSContext* cx, const char* name, HandleValue v) {
    MOZ_ASSERT(!v.isObject());
...
bool js::FromPropertyDescriptorToObject(JSContext* cx, Handle<PropertyDescriptor> desc,
...
bool js::GetFirstArgumentAsObject(JSContext* cx, const CallArgs& args, const char* method,
                                  MutableHandleObject objp) {
    if (args.length() == 0) {
        JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr, JSMSG_MORE_ARGS_NEEDED, method,
                                  "0", "s");
        return false;
    }
...
bool js::Throw(JSContext* cx, jsid id, unsigned errorNumber, const char* details) {
...
static Result<> CheckCallable(JSContext* cx, JSObject* obj, const char* fieldName) {
...
bool js::ToPropertyDescriptor(JSContext* cx, HandleValue descval, bool checkAccessors,
...
Result<> js::CheckPropertyDescriptorAccessors(JSContext* cx, Handle<PropertyDescriptor> desc) {
    if (desc.hasGetterObject()) MOZ_TRY(CheckCallable(cx, desc.getterObject(), js_getter_str));

    if (desc.hasSetterObject()) MOZ_TRY(CheckCallable(cx, desc.setterObject(), js_setter_str));
...
bool js::ReadPropertyDescriptors(JSContext* cx, HandleObject props, bool checkAccessors,
                                 AutoIdVector* ids,
                                 MutableHandle<PropertyDescriptorVector> descs) {
    if (!GetPropertyKeys(cx, props, JSITER_OWNONLY | JSITER_SYMBOLS, ids)) return false;
    ...
    for (size_t i = 0, len = ids->length(); i < len; i++) {
        ...
        if (!GetProperty(cx, props, props, id, &v) ||
            !ToPropertyDescriptor(cx, v, checkAccessors, &desc) || !descs.append(desc)) {
            return false;
        }
    }
    return true;
...
static unsigned GetSealedOrFrozenAttributes(unsigned attrs, IntegrityLevel level) {
...
bool js::SetIntegrityLevel(JSContext* cx, HandleObject obj, IntegrityLevel level) {
    ...
    if (obj->isNative() && !obj->as<NativeObject>().inDictionaryMode() &&
        ...
        // dictionary mode.
        RootedShape last(cx, EmptyShape::getInitialShape(cx, nobj->getClass(), nobj->taggedProto(),
                                                         nobj->numFixedSlots(),
    ...
    if (level == IntegrityLevel::Frozen && obj->isNative()) {
        if (!ObjectElements::FreezeElements(cx, obj.as<NativeObject>())) return false;
    }
...
bool js::TestIntegrityLevel(JSContext* cx, HandleObject obj, IntegrityLevel level, bool* result) {
    ...
    if (obj->isNative()) {
        ...
        for (Shape::Range<NoGC> r(nobj->lastProperty()); !r.empty(); r.popFront()) {
            ...
            // Steps 9.c.i-ii.
            if (shape->configurable() || (level == IntegrityLevel::Frozen &&
...
static inline JSObject* NewObject(JSContext* cx, HandleObjectGroup group, gc::AllocKind kind,
    ...
    RootedShape shape(
        cx, EmptyShape::getInitialShape(cx, clasp, group->proto(), nfixed, initialShapeFlags));
...
void NewObjectCache::fillProto(EntryIndex entry, const Class* clasp, js::TaggedProto proto,
...
bool js::NewObjectWithTaggedProtoIsCachable(JSContext* cx, Handle<TaggedProto> proto,
...
JSObject* js::NewObjectWithGivenTaggedProto(JSContext* cx, const Class* clasp,
                                            Handle<TaggedProto> proto, gc::AllocKind allocKind,
                                            NewObjectKind newKind, uint32_t initialShapeFlags) {
...
static bool NewObjectIsCachable(JSContext* cx, NewObjectKind newKind, const Class* clasp) {
...
JSObject* js::NewObjectWithClassProtoCommon(JSContext* cx, const Class* clasp,
                                            HandleObject protoArg, gc::AllocKind allocKind,
                                            NewObjectKind newKind) {
    if (protoArg)
        return NewObjectWithGivenTaggedProto(cx, clasp, AsTaggedProto(protoArg), allocKind,
                                             newKind);
    ...
    if (!GetBuiltinPrototype(cx, protoKey, &proto)) return nullptr;
...
static bool NewObjectWithGroupIsCachable(JSContext* cx, HandleObjectGroup group,
                                         NewObjectKind newKind) {
    return group->proto().isObject() && newKind == GenericObject && group->clasp()->isNative() &&
           (!group->newScript() || group->newScript()->analyzed()) && !cx->helperThread();
}
...
JSObject* js::NewObjectWithGroupCommon(JSContext* cx, HandleObjectGroup group,
    ...
    if (isCachable) {
        ...
        if (cache.lookupGroup(group, allocKind, &entry)) {
            JSObject* obj =
                cache.newObjectFromHit(cx, entry, GetInitialHeap(newKind, group->clasp()));
            if (obj) return obj;
        }
    }
...
JSObject* js::CreateThis(JSContext* cx, const Class* newclasp, HandleObject callee) {
    RootedObject proto(cx);
...
static inline JSObject* CreateThisForFunctionWithGroup(JSContext* cx, HandleObjectGroup group,
                                                       NewObjectKind newKind) {
    ...
    if (newKind == SingletonObject) {
        ...
        return NewObjectWithGivenTaggedProto(cx, &PlainObject::class_, protoRoot, allocKind,
                                             newKind);
    }
    return NewObjectWithGroup<PlainObject>(cx, group, allocKind, newKind);
}

JSObject* js::CreateThisForFunctionWithProto(JSContext* cx, HandleObject callee,
    ...
    if (proto) {
        RootedObjectGroup group(
            cx, ObjectGroup::defaultNewGroup(cx, nullptr, TaggedProto(proto), newTarget));
        if (!group) return nullptr;

        if (group->newScript() && !group->newScript()->analyzed()) {
            bool regenerate;
            if (!group->newScript()->maybeAnalyze(cx, group, &regenerate)) return nullptr;
            if (regenerate) {
                // The script was analyzed successfully and may have changed
                // the new type table, so refetch the group.
                group = ObjectGroup::defaultNewGroup(cx, nullptr, TaggedProto(proto), newTarget);
...
bool js::GetPrototypeFromConstructor(JSContext* cx, HandleObject newTarget,
    ...
    if (!GetProperty(cx, newTarget, newTarget, cx->names().prototype, &protov)) return false;
    proto.set(protov.isObject() ? &protov.toObject() : nullptr);
    return true;
}

JSObject* js::CreateThisForFunction(JSContext* cx, HandleObject callee, HandleObject newTarget,
                                    NewObjectKind newKind) {
    RootedObject proto(cx);
...
/* static */ bool JSObject::nonNativeSetProperty(JSContext* cx, HandleObject obj, HandleId id,
                                                 HandleValue v, HandleValue receiver,
                                                 ObjectOpResult& result) {
    RootedValue value(cx, v);
    if (MOZ_UNLIKELY(obj->watched())) {
        WatchpointMap* wpmap = cx->compartment()->watchpointMap;
        if (wpmap && !wpmap->triggerWatchpoint(cx, obj, id, &value)) return false;
    }
    return obj->getOpsSetProperty()(cx, obj, id, value, receiver, result);
}

/* static */ bool JSObject::nonNativeSetElement(JSContext* cx, HandleObject obj, uint32_t index,
                                                HandleValue v, HandleValue receiver,
                                                ObjectOpResult& result) {
    RootedId id(cx);
...
JS_FRIEND_API(bool)
JS_CopyPropertyFrom(JSContext* cx, HandleId id, HandleObject target, HandleObject obj,
...
static bool CopyProxyObject(JSContext* cx, Handle<ProxyObject*> from, Handle<ProxyObject*> to) {
...
JSObject* js::CloneObject(JSContext* cx, HandleObject obj, Handle<js::TaggedProto> proto) {
    ...
    if (obj->isNative()) {
        ...
        if (!CopyProxyObject(cx, obj.as<ProxyObject>(), clone.as<ProxyObject>())) return nullptr;
    }

    return clone;
}

static bool GetScriptArrayObjectElements(JSContext* cx, HandleArrayObject arr,
...
static bool GetScriptPlainObjectProperties(JSContext* cx, HandleObject obj,
    ...
    if (obj->is<UnboxedPlainObject>()) {
        ...
        if (!properties.appendN(IdValuePair(), layout.properties().length())) return false;
...
JSObject* js::DeepCloneObjectLiteral(JSContext* cx, HandleObject obj, NewObjectKind newKind) {
    /* NB: Keep this in sync with XDRObjectLiteral. */
    MOZ_ASSERT_IF(obj->isSingleton(), cx->compartment()->behaviors().getSingletonsAsTemplates());
    MOZ_ASSERT(obj->is<PlainObject>() || obj->is<UnboxedPlainObject>() || obj->is<ArrayObject>());
    MOZ_ASSERT(newKind != SingletonObject);

    if (obj->is<ArrayObject>()) {
        Rooted<GCVector<Value>> values(cx, GCVector<Value>(cx));
        if (!GetScriptArrayObjectElements(cx, obj.as<ArrayObject>(), &values)) return nullptr;
    ...
    if (!GetScriptPlainObjectProperties(cx, obj, &properties)) return nullptr;

    for (size_t i = 0; i < properties.length(); i++) {
        cx->markId(properties[i].get().id);
        if (!DeepCloneValue(cx, &properties[i].get().value, newKind)) return nullptr;
    }
...
static bool InitializePropertiesFromCompatibleNativeObject(JSContext* cx, HandleNativeObject dst,
...
JS_FRIEND_API(bool)
JS_InitializePropertiesFromCompatibleNativeObject(JSContext* cx, HandleObject dst,
                                                  HandleObject src) {
    return InitializePropertiesFromCompatibleNativeObject(cx, dst.as<NativeObject>(),
                                                          src.as<NativeObject>());
}
...
bool js::XDRObjectLiteral(XDRState<mode>* xdr, MutableHandleObject obj) {
    ...
    if (isArray) {
        ...
        if (mode == XDR_ENCODE) {
            ...
            if (!GetScriptArrayObjectElements(cx, arr, &values)) return false;
        }
        ...
        if (mode == XDR_ENCODE)
            copyOnWrite =
                obj->is<ArrayObject>() && obj->as<ArrayObject>().denseElementsAreCopyOnWrite();
        ...
        if (mode == XDR_DECODE) {
            ...
            obj.set(ObjectGroup::newArrayObject(cx, values.begin(), values.length(), TenuredObject,
                                                arrayKind));
            if (!obj) return false;
        }
    ...
    Rooted<IdValueVector> properties(cx, IdValueVector(cx));
    if (mode == XDR_ENCODE && !GetScriptPlainObjectProperties(cx, obj, &properties)) return false;

    uint32_t nproperties = properties.length();
    if (!xdr->codeUint32(&nproperties)) return false;

    if (mode == XDR_DECODE && !properties.appendN(IdValuePair(), nproperties)) return false;
    ...
    for (size_t i = 0; i < nproperties; i++) {
        ...
        if (!xdr->codeConstValue(&tmpIdValue) || !xdr->codeConstValue(&tmpValue)) return false;
...
template bool js::XDRObjectLiteral(XDRState<XDR_ENCODE>* xdr, MutableHandleObject obj);

template bool js::XDRObjectLiteral(XDRState<XDR_DECODE>* xdr, MutableHandleObject obj);

/* static */ bool NativeObject::fillInAfterSwap(JSContext* cx, HandleNativeObject obj,
...
static MOZ_MUST_USE bool CopyProxyValuesBeforeSwap(ProxyObject* proxy, Vector<Value>& values) {
...
bool ProxyObject::initExternalValueArrayAfterSwap(JSContext* cx, const Vector<Value>& values) {
    ...
    for (size_t i = 0; i < nreserved; i++) valArray->reservedSlots.slots[i] = values[i + 1];
...
static bool DefineStandardSlot(JSContext* cx, HandleObject obj, JSProtoKey key, JSAtom* atom,
...
static void SetClassObject(JSObject* obj, JSProtoKey key, JSObject* cobj, JSObject* proto) {
...
static NativeObject* DefineConstructorAndPrototype(
    JSContext* cx, HandleObject obj, JSProtoKey key, HandleAtom atom, HandleObject protoProto,
    const Class* clasp, Native constructor, unsigned nargs, const JSPropertySpec* ps,
    const JSFunctionSpec* fs, const JSPropertySpec* static_ps, const JSFunctionSpec* static_fs,
    ...
    RootedNativeObject proto(
        cx, NewNativeObjectWithClassProto(cx, clasp, protoProto, SingletonObject));
    ...
    if (!constructor) {
        ...
        if (!(clasp->flags & JSCLASS_IS_ANONYMOUS) || !obj->is<GlobalObject>() ||
            key == JSProto_Null) {
            uint32_t attrs =
                (clasp->flags & JSCLASS_IS_ANONYMOUS) ? JSPROP_READONLY | JSPROP_PERMANENT : 0;
            RootedValue value(cx, ObjectValue(*proto));
            if (!DefineStandardSlot(cx, obj, key, atom, value, attrs, named)) goto bad;
        }
        ...
        if (!DefineStandardSlot(cx, obj, key, atom, value, 0, named)) goto bad;
...
NativeObject* js::InitClass(JSContext* cx, HandleObject obj, HandleObject protoProto_,
...
static bool SetClassAndProto(JSContext* cx, HandleObject obj, const Class* clasp,
    ...
    while (oldproto && oldproto->isNative()) {
        if (oldproto->isSingleton()) {
            ...
            // this.
            if (!NativeObject::generateOwnShape(cx, oldproto.as<NativeObject>())) return false;
        } else {
            if (!JSObject::setUncacheableProto(cx, oldproto)) return false;
        }
        if (!obj->isDelegate()) {
...
/* static */ bool JSObject::changeToSingleton(JSContext* cx, HandleObject obj) {
    ...
    ObjectGroup* group = ObjectGroup::lazySingletonGroup(cx, obj->getClass(), obj->taggedProto());
...
static bool MaybeResolveConstructor(JSContext* cx, Handle<GlobalObject*> global, JSProtoKey key) {
...
bool js::GetBuiltinConstructor(JSContext* cx, JSProtoKey key, MutableHandleObject objp) {
    MOZ_ASSERT(key != JSProto_Null);
    Rooted<GlobalObject*> global(cx, cx->global());
    if (!MaybeResolveConstructor(cx, global, key)) return false;

    objp.set(&global->getConstructor(key).toObject());
    return true;
}

bool js::GetBuiltinPrototype(JSContext* cx, JSProtoKey key, MutableHandleObject protop) {
    MOZ_ASSERT(key != JSProto_Null);
    Rooted<GlobalObject*> global(cx, cx->global());
    if (!MaybeResolveConstructor(cx, global, key)) return false;
...
bool js::IsStandardPrototype(JSObject* obj, JSProtoKey key) {
...
bool js::LookupProperty(JSContext* cx, HandleObject obj, js::HandleId id, MutableHandleObject objp,
                        MutableHandle<PropertyResult> propp) {
    if (LookupPropertyOp op = obj->getOpsLookupProperty()) return op(cx, obj, id, objp, propp);
    return LookupPropertyInline<CanGC>(cx, obj.as<NativeObject>(), id, objp, propp);
}

bool js::LookupName(JSContext* cx, HandlePropertyName name, HandleObject envChain,
...
bool js::LookupNameNoGC(JSContext* cx, PropertyName* name, JSObject* envChain, JSObject** objp,
    ...
    for (JSObject* env = envChain; env; env = env->enclosingEnvironment()) {
        ...
        if (!LookupPropertyInline<NoGC>(cx, &env->as<NativeObject>(), NameToId(name), pobjp,
                                        propp))
            return false;
        if (*propp) {
            *objp = env;
...
bool js::LookupNameWithGlobalDefault(JSContext* cx, HandlePropertyName name, HandleObject envChain,
                                     MutableHandleObject objp) {
    RootedId id(cx, NameToId(name));
...
bool js::LookupNameUnqualified(JSContext* cx, HandlePropertyName name, HandleObject envChain,
                               MutableHandleObject objp) {
    RootedId id(cx, NameToId(name));
...
bool js::HasOwnProperty(JSContext* cx, HandleObject obj, HandleId id, bool* result) {
    ...
    Rooted<PropertyResult> prop(cx);
    if (!NativeLookupOwnProperty<CanGC>(cx, obj.as<NativeObject>(), id, &prop)) return false;
    *result = prop.isFound();
    return true;
}

bool js::LookupPropertyPure(JSContext* cx, JSObject* obj, jsid id, JSObject** objp,
                            PropertyResult* propp) {
    bool isTypedArrayOutOfRange = false;
    do {
        if (!LookupOwnPropertyPure(cx, obj, id, propp, &isTypedArrayOutOfRange)) return false;
...
bool js::LookupOwnPropertyPure(JSContext* cx, JSObject* obj, jsid id, PropertyResult* propp,
...
static inline bool NativeGetPureInline(NativeObject* pobj, jsid id, PropertyResult prop,
                                       Value* vp) {
...
static inline bool NativeGetGetterPureInline(PropertyResult prop, JSFunction** fp) {
...
bool js::GetOwnGetterPure(JSContext* cx, JSObject* obj, jsid id, JSFunction** fp) {
    JS::AutoCheckCannotGC nogc;
    PropertyResult prop;
    if (!LookupOwnPropertyPure(cx, obj, id, &prop)) return false;
...
bool js::GetOwnNativeGetterPure(JSContext* cx, JSObject* obj, jsid id, JSNative* native) {
    JS::AutoCheckCannotGC nogc;
    *native = nullptr;
    PropertyResult prop;
    if (!LookupOwnPropertyPure(cx, obj, id, &prop)) return false;
...
bool js::HasOwnDataPropertyPure(JSContext* cx, JSObject* obj, jsid id, bool* result) {
    PropertyResult prop;
    if (!LookupOwnPropertyPure(cx, obj, id, &prop)) return false;

    *result = prop && !prop.isDenseOrTypedArrayElement() && prop.shape()->isDataProperty();
    return true;
}

/* static */ bool JSObject::reportReadOnly(JSContext* cx, jsid id, unsigned report) {
    RootedValue val(cx, IdToValue(id));
    return ReportValueErrorFlags(cx, report, JSMSG_READ_ONLY, JSDVG_IGNORE_STACK, val, nullptr,
                                 nullptr, nullptr);
}

/* static */ bool JSObject::reportNotConfigurable(JSContext* cx, jsid id, unsigned report) {
    RootedValue val(cx, IdToValue(id));
    return ReportValueErrorFlags(cx, report, JSMSG_CANT_DELETE, JSDVG_IGNORE_STACK, val, nullptr,
                                 nullptr, nullptr);
}

/* static */ bool JSObject::reportNotExtensible(JSContext* cx, HandleObject obj, unsigned report) {
    RootedValue val(cx, ObjectValue(*obj));
    return ReportValueErrorFlags(cx, report, JSMSG_OBJECT_NOT_EXTENSIBLE, JSDVG_IGNORE_STACK, val,
                                 nullptr, nullptr, nullptr);
}

bool js::GetPrototypeIfOrdinary(JSContext* cx, HandleObject obj, bool* isOrdinary,
...
bool js::SetPrototype(JSContext* cx, HandleObject obj, HandleObject proto,
    ...
    if (obj->staticPrototypeIsImmutable()) return result.fail(JSMSG_CANT_SET_PROTO);
    ...
    if (obj->is<GlobalObject>()) {
        ...
        if (!GlobalObject::ensureConstructor(cx, global, JSProto_Object)) return false;
    }
    ...
    while (obj2) {
        ...
        if (obj2 == objMaybeWindowProxy) return result.fail(JSMSG_CANT_SET_PROTO_CYCLE);
...
bool js::PreventExtensions(JSContext* cx, HandleObject obj, ObjectOpResult& result,
                           IntegrityLevel level) {
    if (obj->is<ProxyObject>()) return js::Proxy::preventExtensions(cx, obj, result);
    ...
    // Force lazy properties to be resolved.
    if (obj->isNative() && !ResolveLazyProperties(cx, obj.as<NativeObject>())) return false;
    ...
    if (obj->isNative() && level != IntegrityLevel::Frozen) {
        if (!NativeObject::sparsifyDenseElements(cx, obj.as<NativeObject>())) return false;
    }
...
bool js::PreventExtensions(JSContext* cx, HandleObject obj, IntegrityLevel level) {
    ObjectOpResult result;
...
bool js::DefineProperty(JSContext* cx, HandleObject obj, HandleId id,
    ...
    ObjectOpResult result;
    return DefineProperty(cx, obj, id, desc, result) && result.checkStrict(cx, obj, id);
}

bool js::DefineProperty(JSContext* cx, HandleObject obj, HandleId id,
                        Handle<PropertyDescriptor> desc, ObjectOpResult& result) {
    desc.assertValid();
    if (DefinePropertyOp op = obj->getOpsDefineProperty()) return op(cx, obj, id, desc, result);
    return NativeDefineProperty(cx, obj.as<NativeObject>(), id, desc, result);
}

bool js::DefineAccessorProperty(JSContext* cx, HandleObject obj, HandleId id, JSGetterOp getter,
                                JSSetterOp setter, unsigned attrs, ObjectOpResult& result) {
...
bool js::DefineDataProperty(JSContext* cx, HandleObject obj, HandleId id, HandleValue value,
...
bool js::DefineAccessorProperty(JSContext* cx, HandleObject obj, PropertyName* name,
...
bool js::DefineDataProperty(JSContext* cx, HandleObject obj, PropertyName* name, HandleValue value,
                            unsigned attrs, ObjectOpResult& result) {
    RootedId id(cx, NameToId(name));
    return DefineDataProperty(cx, obj, id, value, attrs, result);
}

bool js::DefineAccessorElement(JSContext* cx, HandleObject obj, uint32_t index, JSGetterOp getter,
                               JSSetterOp setter, unsigned attrs, ObjectOpResult& result) {
    RootedId id(cx);
    if (!IndexToId(cx, index, &id)) return false;
    return DefineAccessorProperty(cx, obj, id, getter, setter, attrs, result);
...
bool js::DefineDataElement(JSContext* cx, HandleObject obj, uint32_t index, HandleValue value,
                           unsigned attrs, ObjectOpResult& result) {
    RootedId id(cx);
    if (!IndexToId(cx, index, &id)) return false;
    return DefineDataProperty(cx, obj, id, value, attrs, result);
...
bool js::DefineAccessorProperty(JSContext* cx, HandleObject obj, HandleId id, JSGetterOp getter,
    ...
    ObjectOpResult result;
    if (!DefineAccessorProperty(cx, obj, id, getter, setter, attrs, result)) return false;
    if (!result) {
        MOZ_ASSERT(!cx->helperThread());
...
bool js::SetImmutablePrototype(JSContext* cx, HandleObject obj, bool* succeeded) {
    if (obj->hasDynamicPrototype()) {
        MOZ_ASSERT(!cx->helperThread());
        return Proxy::setImmutablePrototype(cx, obj, succeeded);
    }

    if (!JSObject::setFlags(cx, obj, BaseShape::IMMUTABLE_PROTOTYPE)) return false;
    *succeeded = true;
    return true;
}
...
bool js::WatchGuts(JSContext* cx, JS::HandleObject origObj, JS::HandleId id,
...
bool js::UnwatchGuts(JSContext* cx, JS::HandleObject origObj, JS::HandleId id) {
    ...
    RootedObject obj(cx, ToWindowIfWindowProxy(origObj));
    if (WatchpointMap* wpmap = cx->compartment()->watchpointMap) wpmap->unwatch(obj, id);
    return true;
}

bool js::WatchProperty(JSContext* cx, HandleObject obj, HandleId id, HandleObject callable) {
    if (WatchOp op = obj->getOpsWatch()) return op(cx, obj, id, callable);
...
bool js::UnwatchProperty(JSContext* cx, HandleObject obj, HandleId id) {
    if (UnwatchOp op = obj->getOpsUnwatch()) return op(cx, obj, id);
...
extern bool PropertySpecNameToId(JSContext* cx, const char* name, MutableHandleId id,
                                 js::PinningBehavior pin = js::DoNotPinAtom);

static bool DefineFunctionFromSpec(JSContext* cx, HandleObject obj, const JSFunctionSpec* fs,
                                   unsigned flags, DefineAsIntrinsic intrinsic) {
    RootedId id(cx);
...
bool js::DefineFunctions(JSContext* cx, HandleObject obj, const JSFunctionSpec* fs,
                         DefineAsIntrinsic intrinsic) {
    for (; fs->name; fs++) {
        if (!DefineFunctionFromSpec(cx, obj, fs, fs->flags, intrinsic)) return false;
    }
    return true;
}
 ...
 */
static bool MaybeCallMethod(JSContext* cx, HandleObject obj, HandleId id, MutableHandleValue vp) {
...
static bool ReportCantConvert(JSContext* cx, unsigned errorNumber, HandleObject obj, JSType hint) {
    const Class* clasp = obj->getClass();
    ...
    RootedValue val(cx, ObjectValue(*obj));
    ReportValueError2(
        cx, errorNumber, JSDVG_SEARCH_STACK, val, str,
        hint == JSTYPE_UNDEFINED ? "primitive type" : hint == JSTYPE_STRING ? "string" : "number");
    return false;
}

bool JS::OrdinaryToPrimitive(JSContext* cx, HandleObject obj, JSType hint, MutableHandleValue vp) {
...
bool js::ToPrimitiveSlow(JSContext* cx, JSType preferredType, MutableHandleValue vp) {
    // Step numbers refer to the first algorithm listed in ES6 draft rev 36
    // (2015 Mar 17) 7.1.1 ToPrimitive.
    MOZ_ASSERT(preferredType == JSTYPE_UNDEFINED || preferredType == JSTYPE_STRING ||
    ...
    if (!method.isNullOrUndefined()) {
        ...
        // Steps 1-3, 6.a-b.
        RootedValue arg0(cx,
                         StringValue(preferredType == JSTYPE_STRING
                                         ? cx->names().string
...
bool js::ToPropertyKeySlow(JSContext* cx, HandleValue argument, MutableHandleId result) {
...
bool js::IsDelegate(JSContext* cx, HandleObject obj, const js::Value& v, bool* result) {
    if (v.isPrimitive()) {
        *result = false;
        return true;
    }
...
bool js::IsDelegateOfObject(JSContext* cx, HandleObject protoObj, JSObject* obj, bool* result) {
...
JSObject* js::GetBuiltinPrototypePure(GlobalObject* global, JSProtoKey protoKey) {
 ...
 */
JSObject* js::ToObjectSlow(JSContext* cx, JS::HandleValue val, bool reportScanStack) {
...
void GetObjectSlotNameFunctor::operator()(JS::CallbackTracer* trc, char* buf, size_t bufsize) {
    ...
    if (obj->isNative()) {
        ...
        while (shape &&
               (shape->isEmptyShape() || !shape->isDataProperty() || shape->slot() != slot)) {
...
static void dumpValue(const Value& v, js::GenericPrinter& out) {
    if (v.isNull())
        ...
        const Class* clasp = obj->getClass();
        out.printf("<%s%s at %p>", clasp->name, (clasp == &PlainObject::class_) ? "" : " object",
                   (void*)obj);
    } else if (v.isBoolean()) {
        if (v.toBoolean())
            out.put("true");
...
static void DumpProperty(const NativeObject* obj, Shape& shape, js::GenericPrinter& out) {
...
void JSObject::dump(js::GenericPrinter& out) const {
    ...
    if (!obj->isNative()) out.put("not native\n");
    ...
    if (stop > 0) out.printf(obj->isNative() ? "reserved slots:\n" : "slots:\n");
    for (uint32_t i = 0; i < stop; i++) {
        out.printf(" %3d ", i);
        if (i < reservedEnd) out.printf("(reserved) ");
        out.put("= ");
    ...
    if (obj->isNative()) {
        ...
        for (Shape::Range<NoGC> r(obj->as<NativeObject>().lastProperty()); !r.empty();
             r.popFront()) {
...
static void MaybeDumpValue(const char* name, const Value& v, js::GenericPrinter& out) {
    if (!v.isNull()) {
        out.printf("  %s: ", name);
        dumpValue(v, out);
        out.putChar('\n');
...
js::DumpInterpreterFrame(JSContext* cx, js::GenericPrinter& out, InterpreterFrame* start) {
    ...
    for (; !i.done(); ++i) {
        ...
        if (!i.isJSJit() && i.interpFrame()->isDebuggerEvalFrame()) out.put(" debugger eval");
...
js::DumpBacktrace(JSContext* cx, js::GenericPrinter& out) {
    ...
    for (AllFramesIter i(cx); !i.done(); ++i, ++depth) {
        ...
        char frameType =
            i.isInterp() ? 'i' : i.isBaseline() ? 'b' : i.isIon() ? 'I' : i.isWasm() ? 'W' : '?';

        out.printf("#%zu %14p %c   %s:%d", depth, i.rawFramePtr(), frameType, filename, line);
...
JS_FRIEND_API(void)
js::DumpBacktrace(JSContext* cx) { DumpBacktrace(cx, stdout); }

/* * */

js::gc::AllocKind JSObject::allocKindForTenure(const js::Nursery& nursery) const {
    ...
    if (is<ArrayObject>()) {
        ...
        if (!nursery.isInside(aobj.getElementsHeader())) return AllocKind::OBJECT0_BACKGROUND;
...
void JSObject::addSizeOfExcludingThis(mozilla::MallocSizeOf mallocSizeOf, JS::ClassInfo* info) {
    ...
    if (is<JSFunction>() || is<PlainObject>() || is<ArrayObject>() || is<CallObject>() ||
        is<RegExpObject>() || is<ProxyObject>()) {
...
JS::ubi::Node::Size JS::ubi::Concrete<JSObject>::size(mozilla::MallocSizeOf mallocSizeOf) const {
    JSObject& obj = get();
...
/* static */ bool JSObject::constructorDisplayAtom(JSContext* cx, js::HandleObject obj,
...
// ES 2016 7.3.20.
MOZ_MUST_USE JSObject* js::SpeciesConstructor(JSContext* cx, HandleObject obj,
    ...
    //   getter.
    RootedValue ctor(cx);
    bool ctorGetSucceeded =
        GetPropertyPure(cx, obj, NameToId(cx->names().constructor), ctor.address());
...
MOZ_MUST_USE JSObject* js::SpeciesConstructor(JSContext* cx, HandleObject obj, JSProtoKey ctorKey,
                                              bool (*isDefaultSpecies)(JSContext*, JSFunction*)) {
    if (!GlobalObject::ensureConstructor(cx, cx->global(), ctorKey)) return nullptr;
...
bool js::Unbox(JSContext* cx, HandleObject obj, MutableHandleValue vp) {
    if (MOZ_UNLIKELY(obj->is<ProxyObject>())) return Proxy::boxedValue_unbox(cx, obj, vp);
...
#ifdef DEBUG
/* static */ void JSObject::debugCheckNewObject(ObjectGroup* group, Shape* shape,
