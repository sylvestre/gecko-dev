...
BEGIN_TEST(testIntlAvailableLocales) {
    ...
    EXEC(
        "if (Intl.Collator().resolvedOptions().locale !== 'de-ZA-abcde-x-private') \n"
    ...
    EXEC(
        ...
        "}");
    EXEC(
        "if (Intl.Collator('de-ZA-abcde').resolvedOptions().locale !== 'de-ZA-abcde') \n"
    ...
    EXEC(
        ...
        "    throw 'unexpected default locale where proposed default included a Unicode "
        "extension';");
