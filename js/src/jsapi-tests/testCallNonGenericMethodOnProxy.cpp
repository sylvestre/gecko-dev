...
static const JSClass CustomClass = {"CustomClass", JSCLASS_HAS_RESERVED_SLOTS(1)};
...
BEGIN_TEST(test_CallNonGenericMethodOnProxy) {
    ...
    JS::RootedFunction customMethodA(cx, JS_NewFunction(cx, CustomMethod, 0, 0, "customMethodA"));
    CHECK(customMethodA);

    JS::RootedValue rval(cx);
    CHECK(JS_CallFunction(cx, customA, customMethodA, JS::HandleValueArray::empty(), &rval));
    ...
    // Now create the second global object and compartment...
    {
        ...
        JS::RootedFunction customMethodB(cx,
                                         JS_NewFunction(cx, CustomMethod, 0, 0, "customMethodB"));
        CHECK(customMethodB);

        JS::RootedValue rval(cx);
        CHECK(JS_CallFunction(cx, customB, customMethodB, JS::HandleValueArray::empty(), &rval));
