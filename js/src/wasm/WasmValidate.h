...
struct ModuleEnvironment {
    ...
    explicit ModuleEnvironment(CompileMode mode = CompileMode::Once, Tier tier = Tier::Ion,
    ...
    size_t numFuncDefs() const { return funcSigs.length() - funcImportGlobalDataOffsets.length(); }
...
class Encoder {
   ...
   public:
    ...
    MOZ_MUST_USE bool writeFixedI8x16(const I8x16& i8x16) { return write<I8x16>(i8x16); }
    MOZ_MUST_USE bool writeFixedI16x8(const I16x8& i16x8) { return write<I16x8>(i16x8); }
    MOZ_MUST_USE bool writeFixedI32x4(const I32x4& i32x4) { return write<I32x4>(i32x4); }
    MOZ_MUST_USE bool writeFixedF32x4(const F32x4& f32x4) { return write<F32x4>(f32x4); }
...
class Decoder {
    ...
    MOZ_MUST_USE bool readVarU(UInt* out) {
        ...
        if (!readFixedU8(&byte) || (byte & (unsigned(-1) << remainderBits))) return false;
    ...
    MOZ_MUST_USE bool readVarS(SInt* out) {
        ...
        if ((byte & mask) != ((byte & (1 << (remainderBits - 1))) ? mask : 0)) return false;
   ...
   public:
    ...
    MOZ_MUST_USE bool readVarU32(uint32_t* out) { return readVarU<uint32_t>(out); }
    MOZ_MUST_USE bool readVarS32(int32_t* out) { return readVarS<int32_t>(out); }
    MOZ_MUST_USE bool readVarU64(uint64_t* out) { return readVarU<uint64_t>(out); }
    ...
    MOZ_MUST_USE bool startSection(SectionId id, ModuleEnvironment* env, MaybeSectionRange* range,
                                   const char* sectionName);
    MOZ_MUST_USE bool finishSection(const SectionRange& range, const char* sectionName);

    // Custom sections do not cause validation errors unless the error is in
    // the section header itself.

    MOZ_MUST_USE bool startCustomSection(const char* expected, size_t expectedLength,
                                         ModuleEnvironment* env, MaybeSectionRange* range);
    template <size_t NameSizeWith0>
    MOZ_MUST_USE bool startCustomSection(const char (&name)[NameSizeWith0], ModuleEnvironment* env,
...
MOZ_MUST_USE bool DecodeLocalEntries(Decoder& d, ModuleKind kind, ValTypeVector* locals);
...
MOZ_MUST_USE bool StartsCodeSection(const uint8_t* begin, const uint8_t* end, SectionRange* range);
...
MOZ_MUST_USE bool ValidateFunctionBody(const ModuleEnvironment& env, uint32_t funcIndex,
...
MOZ_MUST_USE bool Validate(const ShareableBytes& bytecode, UniqueChars* error);
