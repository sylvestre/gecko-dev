...
class FunctionCompiler {
    struct ControlFlowPatch {
        ...
        uint32_t index;
        ControlFlowPatch(MControlInstruction* ins, uint32_t index) : ins(ins), index(index) {}
    };
   ...
   public:
    FunctionCompiler(const ModuleEnvironment& env, Decoder& decoder, const FuncCompileInput& func,
                     const ValTypeVector& locals, MIRGenerator& mirGen)
        : env_(env),
          iter_(env, decoder),
          func_(func),
          locals_(locals),
        ...
        for (size_t i = args.length(); i < locals_.length(); i++) {
            ...
            switch (locals_[i]) {
                ...
                case ValType::B8x16:
                    ...
                    ins =
                        MSimdConstant::New(alloc(), SimdConstant::SplatX16(0), MIRType::Bool8x16);
                    break;
                case ValType::B16x8:
...
#ifdef DEBUG
        for (ControlFlowPatchVector& patches : blockPatches_) MOZ_ASSERT(patches.empty());
#endif
    ...
    bool mustPreserveNaN(MIRType type) { return IsFloatingPointType(type) && !env().isAsmJS(); }
    ...
    MDefinition* insertElementSimd(MDefinition* vec, MDefinition* val, unsigned lane,
                                   MIRType type) {
        if (inDeadCode()) return nullptr;
   ...
   private:
    ...
    void checkOffsetAndBounds(MemoryAccessDesc* access, MDefinition** base) {
        ...
        if (boundsCheckLimit)
            curBlock_->add(
                MWasmBoundsCheck::New(alloc(), *base, boundsCheckLimit, bytecodeOffset()));
    }
   ...
   public:
    ...
    MDefinition* atomicBinopHeap(js::jit::AtomicOp op, MDefinition* base, MemoryAccessDesc* access,
                                 MDefinition* v) {
        if (inDeadCode()) return nullptr;
    ...
    MDefinition* loadGlobalVar(unsigned globalDataOffset, bool isConst, MIRType type) {
        ...
        auto* load =
            MWasmLoadGlobalVar::New(alloc(), type, globalDataOffset, isConst, tlsPointer_);
        curBlock_->add(load);
        return load;
    }
    ...
    MDefinition* extractSimdElement(unsigned lane, MDefinition* base, MIRType type,
                                    SimdSign sign) {
        if (inDeadCode()) return nullptr;
   ...
   public:
    ...
    bool startLoop(MBasicBlock** loopHeader) {
        ...
        *loopHeader =
            MBasicBlock::New(mirGraph(), info(), curBlock_, MBasicBlock::PENDING_LOOP_HEADER);
    ...
    bool setLoopBackedge(MBasicBlock* loopEntry, MBasicBlock* loopBody, MBasicBlock* backedge) {
        ...
        for (ControlFlowPatchVector& patches : blockPatches_) {
            for (ControlFlowPatch& p : patches) {
                ...
                if (block->loopDepth() >= loopEntry->loopDepth()) fixupRedundantPhis(block);
            }
        }
   ...
   public:
    ...
    bool br(uint32_t relativeDepth, MDefinition* maybeValue) {
        ...
        if (!addControlFlowPatch(jump, relativeDepth, MGoto::TargetIndex)) return false;
    ...
    bool brIf(uint32_t relativeDepth, MDefinition* maybeValue, MDefinition* condition) {
        ...
        if (!addControlFlowPatch(test, relativeDepth, MTest::TrueBranchIndex)) return false;
    ...
    bool brTable(MDefinition* operand, uint32_t defaultDepth, const Uint32Vector& depths,
        ...
        if (!indexToCase.init() || !indexToCase.put(defaultDepth, defaultIndex)) return false;
   ...
   private:
    ...
    bool bindBranches(uint32_t absolute, MDefinition** def) {
        ...
        for (uint32_t i = 0; i < join->numPredecessors(); i++) join->getPredecessor(i)->unmark();
...
static bool EmitBlock(FunctionCompiler& f) { return f.iter().readBlock() && f.startBlock(); }
...
static bool EmitElse(FunctionCompiler& f) {
    ...
    if (!f.switchToElse(f.iter().controlItem(), &f.iter().controlItem())) return false;
...
static bool EmitBrIf(FunctionCompiler& f) {
    ...
    if (!f.iter().readBrIf(&relativeDepth, &type, &value, &condition)) return false;
...
static bool EmitCallArgs(FunctionCompiler& f, const Sig& sig, const DefVector& args,
...
static bool EmitCall(FunctionCompiler& f, bool asmJSFuncDef) {
    ...
    DefVector args;
    if (asmJSFuncDef) {
        if (!f.iter().readOldCallDirect(f.env().numFuncImports(), &funcIndex, &args)) return false;
    } else {
...
static bool EmitGetGlobal(FunctionCompiler& f) {
    ...
    if (!global.isConstant()) {
        f.iter().setResult(
            f.loadGlobalVar(global.offset(), !global.isMutable(), ToMIRType(global.type())));
        return true;
    }
...
template <typename MIRClass>
static bool EmitConversion(FunctionCompiler& f, ValType operandType, ValType resultType) {
    MDefinition* input;
...
template <typename MIRClass>
static bool EmitUnaryWithType(FunctionCompiler& f, ValType operandType, MIRType mirType) {
    MDefinition* input;
    if (!f.iter().readUnary(operandType, &input)) return false;
...
template <typename MIRClass>
static bool EmitConversionWithType(FunctionCompiler& f, ValType operandType, ValType resultType,
                                   MIRType mirType) {
    MDefinition* input;
    if (!f.iter().readConversion(operandType, resultType, &input)) return false;
...
static bool EmitTruncate(FunctionCompiler& f, ValType operandType, ValType resultType,
                         bool isUnsigned) {
    MDefinition* input;
    if (!f.iter().readConversion(operandType, resultType, &input)) return false;
...
static bool EmitSignExtend(FunctionCompiler& f, uint32_t srcSize, uint32_t targetSize) {
    MDefinition* input;
...
static bool EmitExtendI32(FunctionCompiler& f, bool isUnsigned) {
    MDefinition* input;
    if (!f.iter().readConversion(ValType::I32, ValType::I64, &input)) return false;

    f.iter().setResult(f.extendI32(input, isUnsigned));
    return true;
}

static bool EmitConvertI64ToFloatingPoint(FunctionCompiler& f, ValType resultType, MIRType mirType,
                                          bool isUnsigned) {
    MDefinition* input;
...
static bool EmitReinterpret(FunctionCompiler& f, ValType resultType, ValType operandType,
                            MIRType mirType) {
    MDefinition* input;
    if (!f.iter().readConversion(operandType, resultType, &input)) return false;
...
template <typename MIRClass>
static bool EmitBitwise(FunctionCompiler& f, ValType operandType, MIRType mirType) {
    MDefinition* lhs;
    MDefinition* rhs;
...
static bool EmitMul(FunctionCompiler& f, ValType operandType, MIRType mirType) {
    ...
    f.iter().setResult(
        f.mul(lhs, rhs, mirType, mirType == MIRType::Int32 ? MMul::Integer : MMul::Normal));
    return true;
}

static bool EmitDiv(FunctionCompiler& f, ValType operandType, MIRType mirType, bool isUnsigned) {
    MDefinition* lhs;
    MDefinition* rhs;
    if (!f.iter().readBinary(operandType, &lhs, &rhs)) return false;
...
static bool EmitRem(FunctionCompiler& f, ValType operandType, MIRType mirType, bool isUnsigned) {
    MDefinition* lhs;
    MDefinition* rhs;
    if (!f.iter().readBinary(operandType, &lhs, &rhs)) return false;
...
static bool EmitMinMax(FunctionCompiler& f, ValType operandType, MIRType mirType, bool isMax) {
    MDefinition* lhs;
    MDefinition* rhs;
    if (!f.iter().readBinary(operandType, &lhs, &rhs)) return false;
...
static bool EmitComparison(FunctionCompiler& f, ValType operandType, JSOp compareOp,
...
static bool EmitSelect(FunctionCompiler& f) {
    ...
    MDefinition* condition;
    if (!f.iter().readSelect(&type, &trueValue, &falseValue, &condition)) return false;
...
static bool EmitStore(FunctionCompiler& f, ValType resultType, Scalar::Type viewType) {
    LinearMemoryAddress<MDefinition*> addr;
    MDefinition* value;
    if (!f.iter().readStore(resultType, Scalar::byteSize(viewType), &addr, &value)) return false;
...
static bool EmitTeeStore(FunctionCompiler& f, ValType resultType, Scalar::Type viewType) {
    LinearMemoryAddress<MDefinition*> addr;
    MDefinition* value;
...
static bool EmitUnaryMathBuiltinCall(FunctionCompiler& f, SymbolicAddress callee,
                                     ValType operandType) {
    uint32_t lineOrBytecode = f.readCallSiteLineOrBytecode();
...
static bool EmitBinaryMathBuiltinCall(FunctionCompiler& f, SymbolicAddress callee,
                                      ValType operandType) {
    uint32_t lineOrBytecode = f.readCallSiteLineOrBytecode();
...
static bool EmitAtomicsLoad(FunctionCompiler& f) {
    LinearMemoryAddress<MDefinition*> addr;
    Scalar::Type viewType;
    if (!f.iter().readAtomicLoad(&addr, &viewType)) return false;
...
static bool EmitAtomicsStore(FunctionCompiler& f) {
    LinearMemoryAddress<MDefinition*> addr;
    Scalar::Type viewType;
    MDefinition* value;
    if (!f.iter().readAtomicStore(&addr, &viewType, &value)) return false;

    MemoryAccessDesc access(viewType, addr.align, addr.offset, Some(f.bytecodeOffset()), 0,
                            MembarBeforeStore, MembarAfterStore);

    if (!f.store(addr.base, &access, value)) return false;

    f.iter().setResult(value);
    return true;
}

static bool EmitAtomicsBinOp(FunctionCompiler& f) {
    LinearMemoryAddress<MDefinition*> addr;
    Scalar::Type viewType;
    jit::AtomicOp op;
    MDefinition* value;
    if (!f.iter().readAtomicBinOp(&addr, &viewType, &op, &value)) return false;
...
static bool EmitAtomicsCompareExchange(FunctionCompiler& f) {
    LinearMemoryAddress<MDefinition*> addr;
    Scalar::Type viewType;
    MDefinition* oldValue;
    MDefinition* newValue;
    if (!f.iter().readAtomicCompareExchange(&addr, &viewType, &oldValue, &newValue)) return false;
...
static bool EmitAtomicsExchange(FunctionCompiler& f) {
    LinearMemoryAddress<MDefinition*> addr;
    Scalar::Type viewType;
    MDefinition* value;
    if (!f.iter().readAtomicExchange(&addr, &viewType, &value)) return false;
...
static bool EmitSimdUnary(FunctionCompiler& f, ValType type, SimdOperation simdOp) {
...
static bool EmitSimdShift(FunctionCompiler& f, ValType operandType, MSimdShift::Operation op) {
    MDefinition* lhs;
    MDefinition* rhs;
...
static bool EmitExtractLane(FunctionCompiler& f, ValType operandType, SimdSign sign) {
    uint8_t lane;
    MDefinition* vector;
    if (!f.iter().readExtractLane(operandType, &lane, &vector)) return false;

    f.iter().setResult(
        f.extractSimdElement(lane, vector, ToMIRType(SimdToLaneType(operandType)), sign));
    return true;
}

// Emit an I32 expression and then convert it to a boolean SIMD lane value, i.e. -1 or 0.
static MDefinition* EmitSimdBooleanLaneExpr(FunctionCompiler& f, MDefinition* i32) {
...
static bool EmitSimdReplaceLane(FunctionCompiler& f, ValType simdType) {
    ...
    MDefinition* vector;
    MDefinition* scalar;
    if (!f.iter().readReplaceLane(simdType, &lane, &vector, &scalar)) return false;

    f.iter().setResult(f.insertElementSimd(vector, scalar, lane, ToMIRType(simdType)));
    return true;
}

inline bool EmitSimdBitcast(FunctionCompiler& f, ValType fromType, ValType toType) {
    MDefinition* input;
...
inline bool EmitSimdConvert(FunctionCompiler& f, ValType fromType, ValType toType, SimdSign sign) {
    MDefinition* input;
    if (!f.iter().readConversion(fromType, toType, &input)) return false;
...
static inline Scalar::Type SimdExprTypeToViewType(ValType type, unsigned* defaultNumElems) {
    switch (type) {
        case ValType::I8x16:
            *defaultNumElems = 16;
...
static bool EmitSimdLoad(FunctionCompiler& f, ValType resultType, unsigned numElems) {
    ...
    LinearMemoryAddress<MDefinition*> addr;
    if (!f.iter().readLoad(resultType, Scalar::byteSize(viewType), &addr)) return false;
...
static bool EmitSimdStore(FunctionCompiler& f, ValType resultType, unsigned numElems) {
    unsigned defaultNumElems;
    Scalar::Type viewType = SimdExprTypeToViewType(resultType, &defaultNumElems);
...
static bool EmitSimdSelect(FunctionCompiler& f, ValType simdType) {
    ...
    MDefinition* falseValue;
    MDefinition* condition;
    if (!f.iter().readSimdSelect(simdType, &trueValue, &falseValue, &condition)) return false;

    f.iter().setResult(f.selectSimd(condition, trueValue, falseValue, ToMIRType(simdType)));
    return true;
}
...
static bool EmitSimdChainedCtor(FunctionCompiler& f, ValType valType, MIRType type,
                                const SimdConstant& init) {
    const unsigned length = SimdTypeToLength(type);

    DefVector args;
    if (!f.iter().readSimdCtor(ValType::I32, length, valType, &args)) return false;

    MDefinition* val = f.constant(init, type);
    for (unsigned i = 0; i < length; i++) val = f.insertElementSimd(val, args[i], i, type);
...
static bool EmitSimdBooleanChainedCtor(FunctionCompiler& f, ValType valType, MIRType type,
                                       const SimdConstant& init) {
    const unsigned length = SimdTypeToLength(type);
...
static bool EmitSimdCtor(FunctionCompiler& f, ValType type) {
    switch (type) {
        ...
        case ValType::B32x4: {
            ...
            for (unsigned i = 0; i < 4; i++) args[i] = EmitSimdBooleanLaneExpr(f, args[i]);
...
static bool EmitSimdOp(FunctionCompiler& f, ValType type, SimdOperation op, SimdSign sign) {
    switch (op) {
            ...
            case uint16_t(Op::MozPrefix): {
                switch (op.b1) {
                    ...
                    case uint16_t(MozOp::I32Max):
                        CHECK_ASMJS(EmitMinMax(f, ValType::I32, MIRType::Int32,
                    ...
                    case uint16_t(MozOp::F64Mod):
                        CHECK_ASMJS(
                            EmitRem(f, ValType::F64, MIRType::Double, /* isUnsigned = */ false));
                    case uint16_t(MozOp::F64Sin):
                        CHECK_ASMJS(
                            EmitUnaryMathBuiltinCall(f, SymbolicAddress::SinD, ValType::F64));
                    case uint16_t(MozOp::F64Cos):
                        CHECK_ASMJS(
                            EmitUnaryMathBuiltinCall(f, SymbolicAddress::CosD, ValType::F64));
                    case uint16_t(MozOp::F64Tan):
                        CHECK_ASMJS(
                            EmitUnaryMathBuiltinCall(f, SymbolicAddress::TanD, ValType::F64));
                    case uint16_t(MozOp::F64Asin):
                        CHECK_ASMJS(
                            EmitUnaryMathBuiltinCall(f, SymbolicAddress::ASinD, ValType::F64));
                    case uint16_t(MozOp::F64Acos):
                        CHECK_ASMJS(
                            EmitUnaryMathBuiltinCall(f, SymbolicAddress::ACosD, ValType::F64));
                    case uint16_t(MozOp::F64Atan):
                        CHECK_ASMJS(
                            EmitUnaryMathBuiltinCall(f, SymbolicAddress::ATanD, ValType::F64));
                    case uint16_t(MozOp::F64Exp):
                        CHECK_ASMJS(
                            EmitUnaryMathBuiltinCall(f, SymbolicAddress::ExpD, ValType::F64));
                    case uint16_t(MozOp::F64Log):
                        CHECK_ASMJS(
                            EmitUnaryMathBuiltinCall(f, SymbolicAddress::LogD, ValType::F64));
                    case uint16_t(MozOp::F64Pow):
                        CHECK_ASMJS(
                            EmitBinaryMathBuiltinCall(f, SymbolicAddress::PowD, ValType::F64));
                    case uint16_t(MozOp::F64Atan2):
                        CHECK_ASMJS(
                            EmitBinaryMathBuiltinCall(f, SymbolicAddress::ATan2D, ValType::F64));
...
#define ENUMERATE(TYPE, FORALL, DO)                                                             \
    case uint16_t(MozOp::TYPE##Constructor):                                                    \
        CHECK_ASMJS(                                                                            \
            EmitSimdOp(f, ValType::TYPE, SimdOperation::Constructor, SimdSign::NotApplicable)); \
        FORALL(DO)
...
#undef ENUMERATE
                    ...
                    case uint16_t(MozOp::I8x16addSaturateU):
                        CHECK_ASMJS(EmitSimdOp(f, ValType::I8x16, SimdOperation::Fn_addSaturate,
                                               SimdSign::Unsigned));
                    case uint16_t(MozOp::I8x16subSaturateU):
                        CHECK_ASMJS(EmitSimdOp(f, ValType::I8x16, SimdOperation::Fn_subSaturate,
                                               SimdSign::Unsigned));
                    case uint16_t(MozOp::I8x16shiftRightByScalarU):
                        CHECK_ASMJS(EmitSimdOp(f, ValType::I8x16,
                                               SimdOperation::Fn_shiftRightByScalar,
                                               SimdSign::Unsigned));
                    case uint16_t(MozOp::I8x16lessThanU):
                        CHECK_ASMJS(EmitSimdOp(f, ValType::I8x16, SimdOperation::Fn_lessThan,
                                               SimdSign::Unsigned));
                    ...
                    case uint16_t(MozOp::I8x16greaterThanU):
                        CHECK_ASMJS(EmitSimdOp(f, ValType::I8x16, SimdOperation::Fn_greaterThan,
                                               SimdSign::Unsigned));
                    ...
                    case uint16_t(MozOp::I8x16extractLaneU):
                        CHECK_ASMJS(EmitSimdOp(f, ValType::I8x16, SimdOperation::Fn_extractLane,
                                               SimdSign::Unsigned));

                    case uint16_t(MozOp::I16x8addSaturateU):
                        CHECK_ASMJS(EmitSimdOp(f, ValType::I16x8, SimdOperation::Fn_addSaturate,
                                               SimdSign::Unsigned));
                    case uint16_t(MozOp::I16x8subSaturateU):
                        CHECK_ASMJS(EmitSimdOp(f, ValType::I16x8, SimdOperation::Fn_subSaturate,
                                               SimdSign::Unsigned));
                    case uint16_t(MozOp::I16x8shiftRightByScalarU):
                        CHECK_ASMJS(EmitSimdOp(f, ValType::I16x8,
                                               SimdOperation::Fn_shiftRightByScalar,
                                               SimdSign::Unsigned));
                    case uint16_t(MozOp::I16x8lessThanU):
                        CHECK_ASMJS(EmitSimdOp(f, ValType::I16x8, SimdOperation::Fn_lessThan,
                                               SimdSign::Unsigned));
                    ...
                    case uint16_t(MozOp::I16x8greaterThanU):
                        CHECK_ASMJS(EmitSimdOp(f, ValType::I16x8, SimdOperation::Fn_greaterThan,
                                               SimdSign::Unsigned));
                    ...
                    case uint16_t(MozOp::I16x8extractLaneU):
                        CHECK_ASMJS(EmitSimdOp(f, ValType::I16x8, SimdOperation::Fn_extractLane,
                                               SimdSign::Unsigned));

                    case uint16_t(MozOp::I32x4shiftRightByScalarU):
                        CHECK_ASMJS(EmitSimdOp(f, ValType::I32x4,
                                               SimdOperation::Fn_shiftRightByScalar,
                                               SimdSign::Unsigned));
                    case uint16_t(MozOp::I32x4lessThanU):
                        CHECK_ASMJS(EmitSimdOp(f, ValType::I32x4, SimdOperation::Fn_lessThan,
                                               SimdSign::Unsigned));
                    ...
                    case uint16_t(MozOp::I32x4greaterThanU):
                        CHECK_ASMJS(EmitSimdOp(f, ValType::I32x4, SimdOperation::Fn_greaterThan,
                                               SimdSign::Unsigned));
                    ...
                    case uint16_t(MozOp::I32x4fromFloat32x4U):
                        CHECK_ASMJS(EmitSimdOp(f, ValType::I32x4, SimdOperation::Fn_fromFloat32x4,
                                               SimdSign::Unsigned));
...
bool wasm::IonCompileFunctions(const ModuleEnvironment& env, LifoAlloc& lifo,
    ...
    for (const FuncCompileInput& func : inputs) {
        ...
        // Compile MIR graph
        {
            ...
            FuncOffsets offsets;
            if (!codegen.generateWasm(sigId, prologueTrapOffset, &offsets)) return false;
