...
struct js::AsmJSMetadata : Metadata, AsmJSMetadataCacheablePod {
    ...
    uint32_t srcEndAfterCurly() const { return srcStart + srcLengthWithRightBrace; }
    ...
    bool mutedErrors() const override { return scriptSource.get()->mutedErrors(); }
    const char16_t* displayURL() const override {
        return scriptSource.get()->hasDisplayURL() ? scriptSource.get()->displayURL() : nullptr;
    }
    ScriptSource* maybeScriptSource() const override { return scriptSource.get(); }
...
static inline bool IsDefaultCase(ParseNode* pn) { return pn->as<CaseClause>().isDefault(); }

static inline ParseNode* CaseExpr(ParseNode* pn) { return pn->as<CaseClause>().caseExpression(); }

static inline ParseNode* CaseBody(ParseNode* pn) { return pn->as<CaseClause>().statementList(); }
...
static inline ParseNode* BitwiseRight(ParseNode* pn) { return BinaryOpRight(pn); }
...
static inline ParseNode* ComparisonLeft(ParseNode* pn) { return BinaryOpLeft(pn); }

static inline ParseNode* ComparisonRight(ParseNode* pn) { return BinaryOpRight(pn); }

static inline bool IsExpressionStatement(ParseNode* pn) { return pn->isKind(PNK_SEMI); }
...
static inline PropertyName* FunctionName(ParseNode* fn) {
    if (JSAtom* name = FunctionObject(fn)->explicitName()) return name->asPropertyName();
    return nullptr;
}
...
static inline bool IsNormalObjectField(JSContext* cx, ParseNode* pn) {
    return pn->isKind(PNK_COLON) && pn->getOp() == JSOP_INITPROP &&
           BinaryLeft(pn)->isKind(PNK_OBJECT_PROPERTY_NAME);
}

static inline PropertyName* ObjectNormalFieldName(JSContext* cx, ParseNode* pn) {
...
static inline ParseNode* ObjectNormalFieldInitializer(JSContext* cx, ParseNode* pn) {
    MOZ_ASSERT(IsNormalObjectField(cx, pn));
    return BinaryRight(pn);
}
...
static inline bool IsIgnoredDirective(JSContext* cx, ParseNode* pn) {
    return pn->isKind(PNK_SEMI) && UnaryKid(pn) && UnaryKid(pn)->isKind(PNK_STRING) &&
...
static bool GetToken(AsmJSParser& parser, TokenKind* tkp) {
    ...
    TokenKind tk;
    while (true) {
        if (!ts.getToken(&tk, TokenStream::Operand)) return false;
        if (tk != TOK_SEMI) break;
    }
    *tkp = tk;
    return true;
}
...
class Type {
   ...
   public:
    ...
    bool isUnsignedSimd() const { return isUint8x16() || isUint16x8() || isUint32x4(); }
    ...
    ValType canonicalToValType() const { return NonVoidToValType(canonicalToExprType()); }
...
class MOZ_STACK_CLASS ModuleValidator {
   public:
    ...
    class Global {
       ...
       public:
        ...
        bool isConst() const { return which_ == ConstantLiteral || which_ == ConstantImport; }
        ...
        bool isAnyArrayView() const { return which_ == ArrayView || which_ == ArrayViewCtor; }
        Scalar::Type viewType() const {
    ...
    struct MathBuiltin {
        ...
        explicit MathBuiltin(AsmJSMathBuiltinFunction func) : kind(Function) { u.func = func; }
    };
   ...
   private:
    ...
    class NamedSig : public HashableSig {
       ...
       public:
        ...
        static HashNumber hash(Lookup l) { return HashGeneric(l.name, l.sig.hash()); }
    ...
    bool newSig(Sig&& sig, uint32_t* sigIndex) {
        if (env_.sigs.length() >= MaxTypes) return failCurrentOffset("too many signatures");
    ...
    bool declareSig(Sig&& sig, uint32_t* sigIndex) {
        ...
        return newSig(Move(sig), sigIndex) && sigSet_.add(p, HashableSig(*sigIndex, env_.sigs));
    }
   ...
   public:
    bool init() {
        ...
        if (!globalMap_.init() || !sigSet_.init() || !funcImportMap_.init()) return false;
        ...
        if (!standardLibraryMathNames_.init() ||
            ...
            !addStandardLibraryMathName("E", M_E) || !addStandardLibraryMathName("LN10", M_LN10) ||
...
#define ADDSTDLIBSIMDOPNAME(op) || !addStandardLibrarySimdOpName(#op, SimdOperation::Fn_##op)
        if (!standardLibrarySimdOpNames_.init() FORALL_SIMD_ASMJS_OP(ADDSTDLIBSIMDOPNAME)) {
            return false;
        }
#undef ADDSTDLIBSIMDOPNAME
        ...
        dummyFunction_ =
            NewScriptedFunction(cx_, 0, JSFunction::INTERPRETED, nullptr,
    ...
    bool addGlobalVarInit(PropertyName* var, const NumLit& lit, Type type, bool isConst) {
        ...
        if (!env_.globals.emplaceBack(type.canonicalToValType(), !isConst, index)) return false;
   ...
   public:
    ...
    bool addFuncDef(PropertyName* name, uint32_t firstUse, Sig&& sig, Func** func) {
        ...
        if (funcDefIndex >= MaxFuncs) return failCurrentOffset("too many functions");
        ...
        if (!funcDefs_.emplaceBack(name, sigIndex, firstUse, funcDefIndex)) return false;
        *func = &funcDefs_.back();
        return true;
    }
    bool declareFuncPtrTable(Sig&& sig, PropertyName* name, uint32_t firstUse, uint32_t mask,
        ...
        *tableIndex = env_.tables.length();
        ...
        if (!env_.tables.emplaceBack(TableKind::TypedFunction, Limits(mask + 1))) return false;
    ...
    bool declareImport(PropertyName* name, Sig&& sig, unsigned ffiIndex, uint32_t* importIndex) {
        ...
        *importIndex = funcImportMap_.count();
        ...
        if (*importIndex >= MaxImports) return failCurrentOffset("too many imports");
    ...
    bool fail(ParseNode* pn, const char* str) { return failOffset(pn->pn_pos.begin, str); }
    ...
    bool failNameOffset(uint32_t offset, const char* fmt, PropertyName* name) {
        ...
        if (AtomToPrintableString(cx_, name, &bytes)) failfOffset(offset, fmt, bytes.ptr());
        return false;
    }
    ...
    Func* lookupFuncDef(PropertyName* name) {
        if (GlobalMap::Ptr p = globalMap_.lookup(name)) {
            ...
            if (value->which() == Global::Function) return &funcDefs_[value->funcDefIndex()];
        }
        return nullptr;
    }
    ...
    bool lookupStandardLibraryAtomicsName(PropertyName* name,
                                          AsmJSAtomicsBuiltinFunction* atomicsBuiltin) const {
    ...
    SharedModule finish() {
        ...
        if (!env_.funcSigs.resize(funcImportMap_.count() + funcDefs_.length())) return nullptr;
        ...
        if (!env_.funcImportGlobalDataOffsets.resize(funcImportMap_.count())) return nullptr;

        asmJSMetadata_->usesSimd = simdPresent_;

        MOZ_ASSERT(asmJSMetadata_->asmJSFuncNames.empty());
        if (!asmJSMetadata_->asmJSFuncNames.resize(funcImportMap_.count())) return nullptr;
        for (const Func& func : funcDefs_) {
        ...
        if (!args || !args->initFromContext(cx_, Move(scriptedCaller))) return nullptr;
...
static bool IsNumericNonFloatLiteral(ParseNode* pn) {
    ...
    return pn->isKind(PNK_NUMBER) || (pn->isKind(PNK_NEG) && UnaryKid(pn)->isKind(PNK_NUMBER));
}

static bool IsCallToGlobal(ModuleValidator& m, ParseNode* pn,
                           const ModuleValidator::Global** global) {
    if (!pn->isKind(PNK_CALL)) return false;

    ParseNode* callee = CallCallee(pn);
    if (!callee->isKind(PNK_NAME)) return false;
...
static bool IsSimdTuple(ModuleValidator& m, ParseNode* pn, SimdType* type) {
    ...
    if (CallArgListLength(pn) != GetSimdLanes(global->simdCtorType())) return false;
...
static inline bool IsLiteralInt(ModuleValidator& m, ParseNode* pn, uint32_t* u32);
...
static double ExtractNumericNonFloatValue(ParseNode* pn, ParseNode** out = nullptr) {
...
static NumLit ExtractNumericLiteral(ModuleValidator& m, ParseNode* pn) {
    ...
    if (NumberNodeHasFrac(pn) || IsNegativeZero(d)) return NumLit(NumLit::Double, DoubleValue(d));
...
static inline bool IsLiteralInt(ModuleValidator& m, ParseNode* pn, uint32_t* u32) {
    return IsNumericLiteral(m, pn) && IsLiteralInt(ExtractNumericLiteral(m, pn), u32);
}
...
class MOZ_STACK_CLASS FunctionValidator {
   ...
   public:
    ...
    bool init() { return locals_.init() && breakLabels_.init() && continueLabels_.init(); }
   ...
   private:
    bool writeBr(uint32_t absolute, Op op = Op::Br) {
        ...
        return encoder().writeOp(op) && encoder().writeVarU32(blockDepth_ - 1 - absolute);
    }
   ...
   public:
    bool pushBreakableBlock() {
        return encoder().writeOp(Op::Block) && encoder().writeFixedU8(uint8_t(ExprType::Void)) &&
    ...
    bool pushLoop() {
        return encoder().writeOp(Op::Block) && encoder().writeFixedU8(uint8_t(ExprType::Void)) &&
               encoder().writeOp(Op::Loop) && encoder().writeFixedU8(uint8_t(ExprType::Void)) &&
               breakableStack_.append(blockDepth_++) && continuableStack_.append(blockDepth_++);
    }
    bool popLoop() {
        JS_ALWAYS_TRUE(continuableStack_.popCopy() == --blockDepth_);
        JS_ALWAYS_TRUE(breakableStack_.popCopy() == --blockDepth_);
    ...
    void setIfType(size_t typeAt, ExprType type) { encoder().patchFixedU7(typeAt, uint8_t(type)); }
    bool popIf() {
        MOZ_ASSERT(blockDepth_ > 0);
        --blockDepth_;
        return encoder().writeOp(Op::End);
    ...
    bool addLabels(const NameVector& labels, uint32_t relativeBreakDepth,
        ...
        for (PropertyName* label : labels) {
            if (!breakLabels_.putNew(label, blockDepth_ + relativeBreakDepth)) return false;
            if (!continueLabels_.putNew(label, blockDepth_ + relativeContinueDepth)) return false;
        }
        return true;
    }
    ...
    MOZ_MUST_USE bool writeConstExpr(const NumLit& lit) {
        switch (lit.which()) {
            ...
            case NumLit::Float:
                return encoder().writeOp(Op::F32Const) && encoder().writeFixedF32(lit.toFloat());
            case NumLit::Double:
                return encoder().writeOp(Op::F64Const) && encoder().writeFixedF64(lit.toDouble());
            case NumLit::Int8x16:
            case NumLit::Uint8x16:
...
static bool CheckIdentifier(ModuleValidator& m, ParseNode* usepn, PropertyName* name) {
    if (name == m.cx()->names().arguments || name == m.cx()->names().eval)
        return m.failName(usepn, "'%s' is not an allowed identifier", name);
    return true;
}

static bool CheckModuleLevelName(ModuleValidator& m, ParseNode* usepn, PropertyName* name) {
    if (!CheckIdentifier(m, usepn, name)) return false;

    if (name == m.moduleFunctionName() || name == m.globalArgumentName() ||
        name == m.importArgumentName() || name == m.bufferArgumentName() || m.lookupGlobal(name)) {
...
static bool CheckFunctionHead(ModuleValidator& m, ParseNode* fn) {
    ...
    if (fn->pn_funbox->isExprBody()) return m.fail(fn, "expression closures not allowed");
    if (fn->pn_funbox->hasDestructuringArgs) return m.fail(fn, "destructuring args not allowed");
    return true;
}

static bool CheckArgument(ModuleValidator& m, ParseNode* arg, PropertyName** name) {
    *name = nullptr;

    if (!arg->isKind(PNK_NAME)) return m.fail(arg, "argument is not a plain name");
...
static bool CheckModuleArgument(ModuleValidator& m, ParseNode* arg, PropertyName** name) {
...
static bool CheckModuleArguments(ModuleValidator& m, ParseNode* fn) {
    ...
    if (numFormals > 3) return m.fail(fn, "asm.js modules takes at most 3 argument");
...
static bool CheckPrecedingStatements(ModuleValidator& m, ParseNode* stmtList) {
    ...
    for (unsigned i = 0, n = ListLength(stmtList); i < n; i++) {
        if (!IsIgnoredDirective(m.cx(), stmt)) return m.fail(stmt, "invalid asm.js statement");
    }

    return true;
}

static bool CheckGlobalVariableInitConstant(ModuleValidator& m, PropertyName* varName,
...
static bool CheckTypeAnnotation(ModuleValidator& m, ParseNode* coercionNode, Type* coerceTo,
...
static bool CheckGlobalVariableInitImport(ModuleValidator& m, PropertyName* varName,
                                          ParseNode* initNode, bool isConst) {
    Type coerceTo;
    ParseNode* coercedExpr;
...
static bool IsArrayViewCtorName(ModuleValidator& m, PropertyName* name, Scalar::Type* type) {
...
static bool CheckNewArrayView(ModuleValidator& m, PropertyName* varName, ParseNode* newExpr) {
    ...
    if (ctorExpr->isKind(PNK_DOT)) {
        ...
        if (global->which() != ModuleValidator::Global::ArrayViewCtor)
            return m.failName(ctorExpr, "%s must be an imported array view constructor",
                              globalName);
...
static bool CheckGlobalMathImport(ModuleValidator& m, ParseNode* initNode, PropertyName* varName,
                                  PropertyName* field) {
...
static bool CheckGlobalSimdImport(ModuleValidator& m, ParseNode* initNode, PropertyName* varName,
                                  PropertyName* field) {
    if (!m.supportsSimd()) return m.fail(initNode, "SIMD is not supported on this platform");
...
static bool CheckGlobalSimdOperationImport(ModuleValidator& m,
                                           const ModuleValidator::Global* global,
                                           ParseNode* initNode, PropertyName* varName,
                                           PropertyName* opName) {
...
static bool CheckGlobalDotImport(ModuleValidator& m, PropertyName* varName, ParseNode* initNode) {
    ...
    if (!base->isKind(PNK_NAME)) return m.fail(base, "expected name of variable or parameter");

    if (base->name() == m.globalArgumentName()) {
        if (field == m.cx()->names().NaN) return m.addGlobalConstant(varName, GenericNaN(), field);
        if (field == m.cx()->names().Infinity)
            return m.addGlobalConstant(varName, PositiveInfinity<double>(), field);

        Scalar::Type type;
        if (IsArrayViewCtorName(m, field, &type)) return m.addArrayViewCtor(varName, type, field);
    ...
    if (!global) return m.failName(initNode, "%s not found in module global scope", base->name());
...
static bool CheckModuleGlobal(ModuleValidator& m, ParseNode* var, bool isConst) {
    if (!var->isKind(PNK_NAME)) return m.fail(var, "import variable is not a plain name");
    ...
    if (initNode->isKind(PNK_NEW)) return CheckNewArrayView(m, var->name(), initNode);

    if (initNode->isKind(PNK_DOT)) return CheckGlobalDotImport(m, var->name(), initNode);
...
static bool CheckModuleProcessingDirectives(ModuleValidator& m) {
    ...
    while (true) {
        bool matched;
        if (!ts.matchToken(&matched, TOK_STRING, TokenStream::Operand)) return false;
        ...
        if (tt != TOK_SEMI) return m.failCurrentOffset("expected semicolon after string literal");
    }
}
...
static bool CheckModuleGlobals(ModuleValidator& m) {
    while (true) {
        ...
        for (ParseNode* var = VarListHead(varStmt); var; var = NextNode(var)) {
            if (!CheckModuleGlobal(m, var, varStmt->isKind(PNK_CONST))) return false;
        }
    }
...
static bool ArgFail(FunctionValidator& f, PropertyName* argName, ParseNode* stmt) {
...
static bool CheckArgumentType(FunctionValidator& f, ParseNode* stmt, PropertyName* name,
                              Type* type) {
    if (!stmt || !IsExpressionStatement(stmt)) return ArgFail(f, name, stmt ? stmt : f.fn());

    ParseNode* initNode = ExpressionStatementExpr(stmt);
    if (!initNode || !initNode->isKind(PNK_ASSIGN)) return ArgFail(f, name, stmt);
    ...
    ParseNode* coercedExpr;
    if (!CheckTypeAnnotation(f.m(), coercionNode, type, &coercedExpr)) return false;

    if (!type->isArgType()) return f.failName(stmt, "invalid type for argument '%s'", name);
...
static bool CheckProcessingDirectives(ModuleValidator& m, ParseNode** stmtIter) {
...
static bool CheckArguments(FunctionValidator& f, ParseNode** stmtIter, ValTypeVector* argTypes) {
    ParseNode* stmt = *stmtIter;
...
static bool IsLiteralOrConst(FunctionValidator& f, ParseNode* pn, NumLit* lit) {
    if (pn->isKind(PNK_NAME)) {
        ...
        if (!global || global->which() != ModuleValidator::Global::ConstantLiteral) return false;
...
static bool CheckFinalReturn(FunctionValidator& f, ParseNode* lastNonEmptyStmt) {
...
static bool CheckVariable(FunctionValidator& f, ParseNode* var, ValTypeVector* types,
                          Vector<NumLit>* inits) {
    if (!var->isKind(PNK_NAME)) return f.fail(var, "local variable is not a plain name");
    ...
    ParseNode* initNode = MaybeInitializer(var);
    if (!initNode)
        return f.failName(var, "var '%s' needs explicit type declaration via an initial value",
                          name);
    ...
    if (!lit.valid()) return f.failName(var, "var '%s' initializer out of range", name);

    Type type = Type::canonicalize(Type::lit(lit));

    return f.addLocal(var, name, type) && types->append(type.canonicalToValType()) &&
           inits->append(lit);
}
...
static bool CheckNumericLiteral(FunctionValidator& f, ParseNode* num, Type* type) {
    NumLit lit = ExtractNumericLiteral(f.m(), num);
    if (!lit.valid()) return f.fail(num, "numeric literal out of representable integer range");
    *type = Type::lit(lit);
...
static inline bool IsLiteralOrConstInt(FunctionValidator& f, ParseNode* pn, uint32_t* u32) {
    NumLit lit;
...
static bool CheckArrayAccess(FunctionValidator& f, ParseNode* viewName, ParseNode* indexExpr,
...
static bool CheckAndPrepareArrayAccess(FunctionValidator& f, ParseNode* viewName,
                                       ParseNode* indexExpr, bool isSimd, Scalar::Type* viewType) {
...
static bool CheckStoreArray(FunctionValidator& f, ParseNode* lhs, ParseNode* rhs, Type* type) {
    Scalar::Type viewType;
...
static bool CheckAssignName(FunctionValidator& f, ParseNode* lhs, ParseNode* rhs, Type* type) {
...
static bool CheckAssign(FunctionValidator& f, ParseNode* assign, Type* type) {
    ...
    if (lhs->getKind() == PNK_ELEM) return CheckStoreArray(f, lhs, rhs, type);

    if (lhs->getKind() == PNK_NAME) return CheckAssignName(f, lhs, rhs, type);
...
static bool CheckMathIMul(FunctionValidator& f, ParseNode* call, Type* type) {
    if (CallArgListLength(call) != 2) return f.fail(call, "Math.imul must be passed 2 arguments");
...
static bool CheckMathClz32(FunctionValidator& f, ParseNode* call, Type* type) {
    if (CallArgListLength(call) != 1) return f.fail(call, "Math.clz32 must be passed 1 argument");
...
static bool CheckMathAbs(FunctionValidator& f, ParseNode* call, Type* type) {
    if (CallArgListLength(call) != 1) return f.fail(call, "Math.abs must be passed 1 argument");
...
static bool CheckMathSqrt(FunctionValidator& f, ParseNode* call, Type* type) {
    if (CallArgListLength(call) != 1) return f.fail(call, "Math.sqrt must be passed 1 argument");
...
static bool CheckMathMinMax(FunctionValidator& f, ParseNode* callNode, bool isMax, Type* type) {
...
static bool CheckSharedArrayAtomicAccess(FunctionValidator& f, ParseNode* viewName,
                                         ParseNode* indexExpr, Scalar::Type* viewType) {
    if (!CheckAndPrepareArrayAccess(f, viewName, indexExpr, NoSimd, viewType)) return false;
...
static bool WriteAtomicOperator(FunctionValidator& f, MozOp opcode, Scalar::Type viewType) {
    return f.encoder().writeOp(opcode) && f.encoder().writeFixedU8(viewType);
}

static bool CheckAtomicsLoad(FunctionValidator& f, ParseNode* call, Type* type) {
    ...
    Scalar::Type viewType;
    if (!CheckSharedArrayAtomicAccess(f, arrayArg, indexArg, &viewType)) return false;
...
static bool CheckAtomicsStore(FunctionValidator& f, ParseNode* call, Type* type) {
...
static bool CheckAtomicsBinop(FunctionValidator& f, ParseNode* call, Type* type, AtomicOp op) {
    if (CallArgListLength(call) != 3)
...
static bool CheckAtomicsIsLockFree(FunctionValidator& f, ParseNode* call, Type* type) {
...
static bool CheckAtomicsCompareExchange(FunctionValidator& f, ParseNode* call, Type* type) {
    ...
    if (!WriteAtomicOperator(f, MozOp::I32AtomicsCompareExchange, viewType)) return false;
...
static bool CheckAtomicsExchange(FunctionValidator& f, ParseNode* call, Type* type) {
    if (CallArgListLength(call) != 3)
    ...
    if (!WriteAtomicOperator(f, MozOp::I32AtomicsExchange, viewType)) return false;
...
static bool CheckAtomicsBuiltinCall(FunctionValidator& f, ParseNode* callNode,
                                    AsmJSAtomicsBuiltinFunction func, Type* type) {
    switch (func) {
...
static bool CheckCallArgs(FunctionValidator& f, ParseNode* callNode, ValTypeVector* args) {
    ...
    for (unsigned i = 0; i < CallArgListLength(callNode); i++, argNode = NextNode(argNode)) {
        ...
        if (!args->append(Type::canonicalize(type).canonicalToValType())) return false;
    }
    return true;
}

static bool CheckSignatureAgainstExisting(ModuleValidator& m, ParseNode* usepn, const Sig& sig,
                                          const Sig& existing) {
...
static bool CheckFunctionSignature(ModuleValidator& m, ParseNode* usepn, Sig&& sig,
...
static bool CheckIsArgType(FunctionValidator& f, ParseNode* argNode, Type type) {
    if (!type.isArgType())
...
static bool CheckInternalCall(FunctionValidator& f, ParseNode* callNode, PropertyName* calleeName,
                              Type ret, Type* type) {
    ...
    if (!CheckFunctionSignature(f.m(), callNode, Move(sig), calleeName, &callee)) return false;
...
static bool CheckFuncPtrTableAgainstExisting(ModuleValidator& m, ParseNode* usepn,
                                             PropertyName* name, Sig&& sig, unsigned mask,
                                             uint32_t* tableIndex) {
...
static bool CheckFuncPtrCall(FunctionValidator& f, ParseNode* callNode, Type ret, Type* type) {
    MOZ_ASSERT(ret.isCanonical());
    ...
    if (!IsLiteralInt(f.m(), maskNode, &mask) || mask == UINT32_MAX || !IsPowerOfTwo(mask + 1))
        return f.fail(maskNode,
                      "function-pointer table index mask value must be a power of two minus 1");
    ...
    // Call signature
    if (!f.encoder().writeVarU32(f.m().table(tableIndex).sigIndex())) return false;

    *type = Type::ret(ret);
    return true;
}

static bool CheckIsExternType(FunctionValidator& f, ParseNode* argNode, Type type) {
    if (!type.isExtern()) return f.failf(argNode, "%s is not a subtype of extern", type.toChars());
    return true;
}

static bool CheckFFICall(FunctionValidator& f, ParseNode* callNode, unsigned ffiIndex, Type ret,
                         Type* type) {
    MOZ_ASSERT(ret.isCanonical());
    ...
    if (ret.isSimd()) return f.fail(callNode, "FFI calls can't return SIMD values");
    ...
    if (!f.m().declareImport(calleeName, Move(sig), ffiIndex, &importIndex)) return false;
...
static bool CheckFloatCoercionArg(FunctionValidator& f, ParseNode* inputNode, Type inputType) {
...
static bool CheckCoercedCall(FunctionValidator& f, ParseNode* call, Type ret, Type* type);

static bool CheckCoercionArg(FunctionValidator& f, ParseNode* arg, Type expected, Type* type) {
    MOZ_ASSERT(expected.isCanonicalValType());

    if (arg->isKind(PNK_CALL)) return CheckCoercedCall(f, arg, expected, type);
...
static bool CheckMathFRound(FunctionValidator& f, ParseNode* callNode, Type* type) {
...
static bool CheckSimdCallArgs(FunctionValidator& f, ParseNode* call, unsigned expectedArity,
...
static bool CheckSimdUnary(FunctionValidator& f, ParseNode* call, SimdType opType,
...
static bool CheckSimdBinaryShift(FunctionValidator& f, ParseNode* call, SimdType opType,
                                 SimdOperation op, Type* type) {
    if (!CheckSimdCallArgs(f, call, 2, CheckSimdVectorScalarArgs(opType))) return false;
    if (!f.writeSimdOp(opType, op)) return false;
    *type = opType;
    return true;
}

static bool CheckSimdBinaryComp(FunctionValidator& f, ParseNode* call, SimdType opType,
                                SimdOperation op, Type* type) {
...
static bool CheckSimdBinary(FunctionValidator& f, ParseNode* call, SimdType opType,
                            SimdOperation op, Type* type) {
    if (!CheckSimdCallArgs(f, call, 2, CheckArgIsSubtypeOf(opType))) return false;
    if (!f.writeSimdOp(opType, op)) return false;
    *type = opType;
...
static bool CheckSimdExtractLane(FunctionValidator& f, ParseNode* call, SimdType opType,
                                 Type* type) {
    switch (opType) {
        case SimdType::Int8x16:
        case SimdType::Int16x8:
    ...
    if (lane >= GetSimdLanes(opType)) return f.failf(arg, "lane selector should be in bounds");
...
static bool CheckSimdReplaceLane(FunctionValidator& f, ParseNode* call, SimdType opType,
                                 Type* type) {
    unsigned numArgs = CallArgListLength(call);
    if (numArgs != 3)
...
static bool CheckSimdCast(FunctionValidator& f, ParseNode* call, SimdType fromType,
                          SimdType toType, SimdOperation op, Type* type) {
    if (!CheckSimdCallArgs(f, call, 1, CheckArgIsSubtypeOf(fromType))) return false;
...
static bool CheckSimdShuffleSelectors(FunctionValidator& f, ParseNode* lane,
                                      mozilla::Array<uint8_t, 16>& lanes, unsigned numLanes,
                                      unsigned maxLane) {
    for (unsigned i = 0; i < numLanes; i++, lane = NextNode(lane)) {
        ...
        if (u32 >= maxLane) return f.failf(lane, "lane selector should be less than %u", maxLane);
        lanes[i] = uint8_t(u32);
    }
    return true;
}

static bool CheckSimdSwizzle(FunctionValidator& f, ParseNode* call, SimdType opType, Type* type) {
    ...
    if (!CheckSimdShuffleSelectors(f, NextNode(vec), lanes, numLanes, numLanes)) return false;
...
static bool CheckSimdShuffle(FunctionValidator& f, ParseNode* call, SimdType opType, Type* type) {
    const unsigned numLanes = GetSimdLanes(opType);
    unsigned numArgs = CallArgListLength(call);
    ...
    mozilla::Array<uint8_t, 16> lanes;
    if (!CheckSimdShuffleSelectors(f, arg, lanes, numLanes, 2 * numLanes)) return false;
...
static bool CheckSimdLoadStoreArgs(FunctionValidator& f, ParseNode* call, Scalar::Type* viewType) {
    ...
    if (!CheckAndPrepareArrayAccess(f, view, indexExpr, YesSimd, viewType)) return false;
...
static bool CheckSimdLoad(FunctionValidator& f, ParseNode* call, SimdType opType, SimdOperation op,
                          Type* type) {
    unsigned numArgs = CallArgListLength(call);
    if (numArgs != 2) return f.failf(call, "expected 2 arguments to SIMD load, got %u", numArgs);
...
static bool CheckSimdStore(FunctionValidator& f, ParseNode* call, SimdType opType,
                           SimdOperation op, Type* type) {
    unsigned numArgs = CallArgListLength(call);
    if (numArgs != 3) return f.failf(call, "expected 3 arguments to SIMD store, got %u", numArgs);
...
static bool CheckSimdSelect(FunctionValidator& f, ParseNode* call, SimdType opType, Type* type) {
...
static bool CheckSimdAllTrue(FunctionValidator& f, ParseNode* call, SimdType opType, Type* type) {
    if (!CheckSimdCallArgs(f, call, 1, CheckArgIsSubtypeOf(opType))) return false;
...
static bool CheckSimdAnyTrue(FunctionValidator& f, ParseNode* call, SimdType opType, Type* type) {
    if (!CheckSimdCallArgs(f, call, 1, CheckArgIsSubtypeOf(opType))) return false;
...
static bool CheckSimdCheck(FunctionValidator& f, ParseNode* call, SimdType opType, Type* type) {
    Type coerceTo;
    ParseNode* argNode;
...
static bool CheckSimdSplat(FunctionValidator& f, ParseNode* call, SimdType opType, Type* type) {
...
static bool CheckSimdOperationCall(FunctionValidator& f, ParseNode* call,
                                   const ModuleValidator::Global* global, Type* type) {
...
static bool CheckSimdCtorCall(FunctionValidator& f, ParseNode* call,
    ...
    if (!CheckSimdCallArgs(f, call, length, CheckSimdScalarArgs(simdType))) return false;
...
static bool CheckUncoercedCall(FunctionValidator& f, ParseNode* expr, Type* type) {
    ...
    if (IsCallToGlobal(f.m(), expr, &global)) {
        ...
        if (global->isSimdOperation()) return CheckSimdOperationCall(f, expr, global, type);
    }
...
static bool CoerceResult(FunctionValidator& f, ParseNode* expr, Type expected, Type actual,
                         Type* type) {
...
static bool CheckCoercedMathBuiltinCall(FunctionValidator& f, ParseNode* callNode,
                                        AsmJSMathBuiltinFunction func, Type ret, Type* type) {
    Type actual;
...
static bool CheckCoercedSimdCall(FunctionValidator& f, ParseNode* call,
                                 const ModuleValidator::Global* global, Type ret, Type* type) {
    MOZ_ASSERT(ret.isCanonical());
...
static bool CheckCoercedAtomicsBuiltinCall(FunctionValidator& f, ParseNode* callNode,
                                           AsmJSAtomicsBuiltinFunction func, Type ret,
                                           Type* type) {
    MOZ_ASSERT(ret.isCanonical());
...
static bool CheckCoercedCall(FunctionValidator& f, ParseNode* call, Type ret, Type* type) {
    ...
    if (callee->isKind(PNK_ELEM)) return CheckFuncPtrCall(f, call, ret, type);

    if (!callee->isKind(PNK_NAME)) return f.fail(callee, "unexpected callee expression type");
    ...
    if (const ModuleValidator::Global* global = f.lookupGlobal(calleeName)) {
        switch (global->which()) {
            ...
            case ModuleValidator::Global::MathBuiltinFunction:
                return CheckCoercedMathBuiltinCall(f, call, global->mathBuiltinFunction(), ret,
                                                   type);
            case ModuleValidator::Global::AtomicsBuiltinFunction:
                return CheckCoercedAtomicsBuiltinCall(f, call, global->atomicsBuiltinFunction(),
                                                      ret, type);
            case ModuleValidator::Global::ConstantLiteral:
            case ModuleValidator::Global::ConstantImport:
            case ModuleValidator::Global::Variable:
...
static bool CheckPos(FunctionValidator& f, ParseNode* pos, Type* type) {
    ...
    if (operand->isKind(PNK_CALL)) return CheckCoercedCall(f, operand, Type::Double, type);
...
static bool CheckCoerceToInt(FunctionValidator& f, ParseNode* expr, Type* type) {
...
static bool CheckBitNot(FunctionValidator& f, ParseNode* neg, Type* type) {
    ...
    if (operand->isKind(PNK_BITNOT)) return CheckCoerceToInt(f, operand, type);
...
static bool CheckConditional(FunctionValidator& f, ParseNode* ternary, Type* type) {
...
static bool CheckAddOrSub(FunctionValidator& f, ParseNode* expr, Type* type,
    ...
    if (lhsType.isInt() && rhsType.isInt()) {
        if (!f.encoder().writeOp(expr->isKind(PNK_ADD) ? Op::I32Add : Op::I32Sub)) return false;
        *type = Type::Intish;
    } else if (lhsType.isMaybeDouble() && rhsType.isMaybeDouble()) {
        if (!f.encoder().writeOp(expr->isKind(PNK_ADD) ? Op::F64Add : Op::F64Sub)) return false;
        *type = Type::Double;
    } else if (lhsType.isMaybeFloat() && rhsType.isMaybeFloat()) {
        if (!f.encoder().writeOp(expr->isKind(PNK_ADD) ? Op::F32Add : Op::F32Sub)) return false;
        *type = Type::Floatish;
    } else {
        return f.failf(expr,
...
static bool CheckDivOrMod(FunctionValidator& f, ParseNode* expr, Type* type) {
    ...
    if (lhsType.isMaybeDouble() && rhsType.isMaybeDouble()) {
        *type = Type::Double;
        if (expr->isKind(PNK_DIV)) return f.encoder().writeOp(Op::F64Div);
...
static bool CheckComparison(FunctionValidator& f, ParseNode* comp, Type* type) {
    ...
    if (!(lhsType.isSigned() && rhsType.isSigned()) &&
        ...
        !(lhsType.isDouble() && rhsType.isDouble()) && !(lhsType.isFloat() && rhsType.isFloat())) {
        return f.failf(
            comp,
            "arguments to a comparison must both be signed, unsigned, floats or doubles; "
            "%s and %s are given",
            lhsType.toChars(), rhsType.toChars());
    }
...
static bool CheckFor(FunctionValidator& f, ParseNode* forStmt,
    ...
    if (!forHead->isKind(PNK_FORHEAD)) return f.fail(forHead, "unsupported for-loop statement");
...
static bool CheckCaseExpr(FunctionValidator& f, ParseNode* caseExpr, int32_t* value) {
...
static bool CheckSwitchRange(FunctionValidator& f, ParseNode* stmt, int32_t* low, int32_t* high,
...
static bool CheckSwitch(FunctionValidator& f, ParseNode* switchStmt) {
    ...
    for (ParseNode* s = stmt; s && !IsDefaultCase(s); s = NextNode(s)) {
        ...
        if (caseDepths[i] != CASE_NOT_DEFINED) return f.fail(s, "no duplicate case labels");
...
static bool CheckBreakOrContinue(FunctionValidator& f, bool isBreak, ParseNode* stmt) {
...
static bool ParseFunction(ModuleValidator& m, ParseNode** fnOut, unsigned* line) {
    ...
    *line = tokenStream.srcCoords.lineNum(tokenStream.currentToken().pos.end);
    ...
    TokenKind tk;
    if (!tokenStream.getToken(&tk, TokenStream::Operand)) return false;
    ...
    FunctionBox* funbox =
        m.parser().newFunctionBox(fn, fun, toStringStart, directives, NotGenerator, SyncFunction);
    ...
    if (!m.parser().functionFormalParametersAndBody(InAllowed, YieldIsName, fn, Statement)) {
        if (tokenStream.hadError() || directives == newDirectives) return false;
...
static bool CheckFunction(ModuleValidator& m) {
    ...
    if (!f.init()) return m.fail(fn, "internal compiler failure (probably out of memory)");
    ...
    if (func->defined()) return m.failName(fn, "function '%s' already defined", FunctionName(fn));
...
static bool CheckFuncPtrTable(ModuleValidator& m, ParseNode* var) {
    ...
    if (!IsPowerOfTwo(length))
        return m.failf(arrayLiteral, "function-pointer table length must be a power of 2 (is %u)",
                       length);
...
static bool CheckFuncPtrTables(ModuleValidator& m) {
    ...
    for (unsigned i = 0; i < m.numFuncPtrTables(); i++) {
        ...
        if (!table.defined()) {
            return m.failNameOffset(table.firstUse(), "function-pointer table %s wasn't defined",
                                    table.name());
        }
    }
...
static bool CheckModuleExportFunction(ModuleValidator& m, ParseNode* pn,
    ...
    if (!pn->isKind(PNK_NAME)) return m.fail(pn, "expected name of exported function");
...
static bool CheckModuleExportObject(ModuleValidator& m, ParseNode* object) {
    ...
    for (ParseNode* pn = ListHead(object); pn; pn = NextNode(pn)) {
        if (!IsNormalObjectField(m.cx(), pn))
            return m.fail(
                pn, "only normal object properties may be used in the export object literal");
...
static bool CheckModuleReturn(ModuleValidator& m) {
    ...
    TokenStream& ts = m.parser().tokenStream;
    if (tk != TOK_RETURN) {
        return m.failCurrentOffset((tk == TOK_RC || tk == TOK_EOF) ? "expecting return statement"
    ...
    if (!returnExpr) return m.fail(returnStmt, "export statement must return something");
...
static SharedModule CheckModule(JSContext* cx, AsmJSParser& parser, ParseNode* stmtList,
                                unsigned* time) {
...
static bool GetDataProperty(JSContext* cx, HandleValue objVal, HandleAtom field,
    ...
    if (!objVal.isObject()) return LinkFail(cx, "accessing property of non-object");

    RootedObject obj(cx, &objVal.toObject());
    if (IsMaybeWrappedScriptedProxy(obj)) return LinkFail(cx, "accessing property of a Proxy");
    ...
    if (!desc.isDataDescriptor()) return LinkFail(cx, "property is not a data property");

    v.set(desc.value());
    return true;
}

static bool GetDataProperty(JSContext* cx, HandleValue objVal, const char* fieldChars,
                            MutableHandleValue v) {
...
static bool GetDataProperty(JSContext* cx, HandleValue objVal, ImmutablePropertyNamePtr field,
                            MutableHandleValue v) {
...
static bool HasObjectValueOfMethodPure(JSObject* obj, JSContext* cx) {
    Value v;
    if (!GetPropertyPure(cx, obj, NameToId(cx->names().valueOf), &v)) return false;
...
static bool HasPureCoercion(JSContext* cx, HandleValue v) {
    ...
    if (v.toObject().is<JSFunction>() && HasNoToPrimitiveMethodPure(&v.toObject(), cx) &&
...
static bool ValidateGlobalVariable(JSContext* cx, const AsmJSGlobal& global, HandleValue importVal,
                                   Val* val) {
...
static bool ValidateFFI(JSContext* cx, const AsmJSGlobal& global, HandleValue importVal,
                        MutableHandle<FunctionVector> ffis) {
    RootedValue v(cx);
    if (!GetDataProperty(cx, importVal, global.field(), &v)) return false;

    if (!IsFunctionObject(v)) return LinkFail(cx, "FFI imports must be functions");

    ffis[global.ffiIndex()].set(&v.toObject().as<JSFunction>());
    return true;
}

static bool ValidateArrayView(JSContext* cx, const AsmJSGlobal& global, HandleValue globalVal) {
...
static bool ValidateMathBuiltinFunction(JSContext* cx, const AsmJSGlobal& global,
    ...
    if (!IsNativeFunction(v, native)) return LinkFail(cx, "bad Math.* builtin function");

    return true;
}

static bool ValidateSimdType(JSContext* cx, const AsmJSGlobal& global, HandleValue globalVal,
    ...
    if (type != simdDesc->as<SimdTypeDescr>().type()) return LinkFail(cx, "bad SIMD type");
...
#undef SET_NATIVE
    ...
    if (!native || !IsNativeFunction(v, native)) return LinkFail(cx, "bad SIMD.type.* operation");
    return true;
}

static bool ValidateAtomicsBuiltinFunction(JSContext* cx, const AsmJSGlobal& global,
                                           HandleValue globalVal) {
    RootedValue v(cx);
    ...
    if (!IsNativeFunction(v, native)) return LinkFail(cx, "bad Atomics.* builtin function");

    return true;
}

static bool ValidateConstant(JSContext* cx, const AsmJSGlobal& global, HandleValue globalVal) {
    ...
    if (IsNaN(global.constantValue())) {
        if (!IsNaN(v.toNumber())) return LinkFail(cx, "global constant value needs to be NaN");
    } else {
...
static bool CheckBuffer(JSContext* cx, const AsmJSMetadata& metadata, HandleValue bufferVal,
...
static bool GetImports(JSContext* cx, const AsmJSMetadata& metadata, HandleValue globalVal,
    ...
    for (const AsmJSGlobal& global : metadata.asmJSGlobals) {
        switch (global.which()) {
            ...
            case AsmJSGlobal::AtomicsBuiltinFunction:
                if (!ValidateAtomicsBuiltinFunction(cx, global, globalVal)) return false;
                break;
...
static bool TryInstantiate(JSContext* cx, CallArgs args, Module& module,
    ...
    if (!GetImports(cx, metadata, globalVal, importVal, &funcs, &valImports)) return false;
...
static bool HandleInstantiationFailure(JSContext* cx, CallArgs args,
    ...
    if (!haveSource && !JSScript::loadSource(cx, source, &haveSource)) return false;
    if (!haveSource) {
    ...
    RootedFunction fun(
        cx, NewScriptedFunction(cx, 0, JSFunction::INTERPRETED_NORMAL, name, /* proto = */ nullptr,
    ...
    if (!frontend::CompileStandaloneFunction(cx, &fun, options, srcBuf, Nothing())) return false;
...
static bool InstantiateAsmJS(JSContext* cx, unsigned argc, JS::Value* vp) {
...
static JSFunction* NewAsmJSModuleFunction(JSContext* cx, JSFunction* origFun,
    ...
    JSFunction::Flags flags =
        origFun->isLambda() ? JSFunction::ASMJS_LAMBDA_CTOR : JSFunction::ASMJS_CTOR;
...
size_t AsmJSGlobal::serializedSize() const { return sizeof(pod) + field_.serializedSize(); }
...
const uint8_t* AsmJSGlobal::deserialize(const uint8_t* cursor) {
    (cursor = ReadBytes(cursor, &pod, sizeof(pod))) && (cursor = field_.deserialize(cursor));
    return cursor;
}
...
size_t AsmJSMetadata::serializedSize() const {
    return Metadata::serializedSize() + sizeof(pod()) + SerializedVectorSize(asmJSGlobals) +
           SerializedPodVectorSize(asmJSImports) + SerializedPodVectorSize(asmJSExports) +
           SerializedVectorSize(asmJSFuncNames) + globalArgumentName.serializedSize() +
           importArgumentName.serializedSize() + bufferArgumentName.serializedSize();
}
...
class ModuleCharsForStore : ModuleChars {
   ...
   public:
    ...
    size_t serializedSize() const {
        return sizeof(uint32_t) + sizeof(uint32_t) + compressedSize_ + sizeof(uint32_t) +
...
struct ScopedCacheEntryOpenedForWrite {
    ...
    ~ScopedCacheEntryOpenedForWrite() {
        if (memory) cx->asmJSCacheOps().closeEntryForWrite(serializedSize, memory, handle);
    }
};
...
struct ScopedCacheEntryOpenedForRead {
    ...
    ~ScopedCacheEntryOpenedForRead() {
        if (memory) cx->asmJSCacheOps().closeEntryForRead(serializedSize, memory, handle);
    }
};

}  // unnamed namespace

static JS::AsmJSCacheResult StoreAsmJSModuleInCache(AsmJSParser& parser, Module& module,
                                                    JSContext* cx) {
    ...
    size_t serializedSize = sizeof(uint32_t) + compiledSize + moduleChars.serializedSize();
...
static bool LookupAsmJSModuleInCache(JSContext* cx, AsmJSParser& parser, bool* loadedFromCache,
    ...
    *module = Module::deserialize(/* bytecodeBegin = */ nullptr, /* bytecodeSize = */ 0, cursor,
                                  compiledSize, asmJSMetadata.get());
    if (!*module) {
    ...
    if (!parser.tokenStream.advance(asmJSMetadata->srcEndBeforeCurly())) return false;
...
static bool Warn(AsmJSParser& parser, int errorNumber, const char* str) {
...
static bool EstablishPreconditions(JSContext* cx, AsmJSParser& parser) {
    ...
    if (parser.pc->isMethod() || parser.pc->isGetterOrSetter())
        return Warn(parser, JSMSG_USE_ASM_TYPE_FAIL,
                    "Disabled by class constructor or method context");

    return true;
}

static UniqueChars BuildConsoleMessage(JSContext* cx, unsigned time,
...
bool js::CompileAsmJS(JSContext* cx, AsmJSParser& parser, ParseNode* stmtList, bool* validated) {
    *validated = false;
    ...
    SharedModule module;
    UniqueChars message;
    if (!LookupAsmJSModuleInCache(cx, parser, &loadedFromCache, &module, &message)) return false;
...
bool js::IsAsmJSModuleNative(Native native) { return native == InstantiateAsmJS; }

bool js::IsAsmJSModule(JSFunction* fun) { return fun->maybeNative() == InstantiateAsmJS; }

bool js::IsAsmJSFunction(JSFunction* fun) {
    if (IsExportedFunction(fun)) return ExportedFunctionToInstance(fun).metadata().isAsmJS();
    return false;
}

bool js::IsAsmJSStrictModeModuleOrFunction(JSFunction* fun) {
    if (IsAsmJSModule(fun)) return AsmJSModuleFunctionToModule(fun).metadata().asAsmJS().strict;

    if (IsAsmJSFunction(fun)) return ExportedFunctionToInstance(fun).metadata().asAsmJS().strict;
...
bool js::IsAsmJSModule(JSContext* cx, unsigned argc, Value* vp) {
    ...
    bool rval = false;
    if (JSFunction* fun = MaybeWrappedNativeFunction(args.get(0))) rval = IsAsmJSModule(fun);
...
bool js::IsAsmJSFunction(JSContext* cx, unsigned argc, Value* vp) {
    ...
    bool rval = false;
    if (JSFunction* fun = MaybeWrappedNativeFunction(args.get(0))) rval = IsAsmJSFunction(fun);
...
JSString* js::AsmJSModuleToString(JSContext* cx, HandleFunction fun, bool isToSource) {
    MOZ_ASSERT(IsAsmJSModule(fun));
    ...
    bool haveSource = source->hasSourceData();
    if (!haveSource && !JSScript::loadSource(cx, source, &haveSource)) return nullptr;
