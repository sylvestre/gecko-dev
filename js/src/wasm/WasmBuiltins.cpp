...
static bool WasmHandleDebugTrap() {
    ...
    if (site->kind() == CallSite::EnterFrame) {
        if (!frame.instance()->enterFrameTrapsEnabled()) return true;
...
static void WasmReportTrap(int32_t trapIndex) {
...
static int64_t DivI64(uint32_t x_hi, uint32_t x_lo, uint32_t y_hi, uint32_t y_lo) {
...
static int64_t UDivI64(uint32_t x_hi, uint32_t x_lo, uint32_t y_hi, uint32_t y_lo) {
...
static int64_t ModI64(uint32_t x_hi, uint32_t x_lo, uint32_t y_hi, uint32_t y_lo) {
    int64_t x = ((uint64_t)x_hi << 32) + x_lo;
    int64_t y = ((uint64_t)y_hi << 32) + y_lo;
    MOZ_ASSERT(x != INT64_MIN || y != -1);
    MOZ_ASSERT(y != 0);
...
static int64_t UModI64(uint32_t x_hi, uint32_t x_lo, uint32_t y_hi, uint32_t y_lo) {
    uint64_t x = ((uint64_t)x_hi << 32) + x_lo;
    uint64_t y = ((uint64_t)y_hi << 32) + y_lo;
    MOZ_ASSERT(y != 0);
    return x % y;
...
static uint64_t TruncateDoubleToUint64(double input) {
    ...
    if (input >= double(UINT64_MAX) || input <= -1.0 || IsNaN(input)) return 0x8000000000000000;
...
#define DEFINE_UNARY_FLOAT_WRAPPER(func, _) \
    static float func##_uncached_f32(float x) { return float(func##_uncached(double(x))); }

#define DEFINE_BINARY_FLOAT_WRAPPER(func, _) \
    static float func##_f32(float x, float y) { return float(func(double(x), double(y))); }
...
using TypedNativeToFuncPtrMap = HashMap<TypedNative, void*, TypedNative, SystemAllocPolicy>;
...
using TypedNativeToCodeRangeMap = HashMap<TypedNative, uint32_t, TypedNative, SystemAllocPolicy>;
...
bool wasm::EnsureBuiltinThunksInitialized() {
    ...
    for (auto sym : MakeEnumeratedRange(SymbolicAddress::Limit)) {
        ...
        if (!GenerateBuiltinThunk(masm, abiType, exitReason, funcPtr, &offsets)) return false;
        if (!thunks->codeRanges.emplaceBack(CodeRange::BuiltinThunk, offsets)) return false;
    }
    ...
    for (TypedNativeToFuncPtrMap::Range r = typedNatives.all(); !r.empty(); r.popFront()) {
        ...
        uint32_t codeRangeIndex = thunks->codeRanges.length();
        if (!thunks->typedNativeToCodeRange.putNew(typedNative, codeRangeIndex)) return false;
    ...
    if (!ExecutableAllocator::makeExecutable(thunks->codeBase, thunks->codeSize)) return false;
...
static Maybe<ABIFunctionType> ToBuiltinABIFunctionType(const Sig& sig) {
    ...
    if ((args.length() + 1) > (sizeof(uint32_t) * 8 / ArgType_Shift)) return Nothing();
...
void* wasm::MaybeGetBuiltinThunk(HandleFunction f, const Sig& sig, JSContext* cx) {
    MOZ_ASSERT(builtinThunks);
...
bool wasm::LookupBuiltinThunk(void* pc, const CodeRange** codeRange, uint8_t** codeBase) {
    if (!builtinThunks) return false;

    const BuiltinThunks& thunks = *builtinThunks;
    if (pc < thunks.codeBase || pc >= thunks.codeBase + thunks.codeSize) return false;
