...
struct AstDecodeStackItem {
    ...
    explicit AstDecodeStackItem(AstExpr* expr)
        : expr(expr), terminationKind(AstDecodeTerminationKind::Unknown), type(ExprType::Limit) {}
};
...
static bool GenerateName(AstDecodeContext& c, const AstName& prefix, uint32_t index,
                         AstName* name) {
    if (!c.generateNames) {
        *name = AstName();
        return true;
...
static bool GenerateRef(AstDecodeContext& c, const AstName& prefix, uint32_t index, AstRef* ref) {
...
static bool GenerateFuncRef(AstDecodeContext& c, uint32_t funcIndex, AstRef* ref) {
...
static bool AstDecodeCallArgs(AstDecodeContext& c, const SigWithId& sig, AstExprVector* funcArgs) {
...
static bool AstDecodeGetBlockRef(AstDecodeContext& c, uint32_t depth, AstRef* ref) {
...
static bool AstDecodeBrTable(AstDecodeContext& c) {
    ...
    ExprType type;
    if (!c.iter().readBrTable(&depths, &defaultDepth, &type, nullptr, nullptr)) return false;
...
static bool AstDecodeEnd(AstDecodeContext& c) {
    ...
    if (!c.push(AstDecodeStackItem(AstDecodeTerminationKind::End, type))) return false;
...
static bool AstDecodeElse(AstDecodeContext& c) {
    ...
    if (!c.push(AstDecodeStackItem(AstDecodeTerminationKind::Else, type))) return false;
...
static bool AstDecodeSelect(AstDecodeContext& c) {
    ...
    AstTernaryOperator* ternary =
        new (c.lifo) AstTernaryOperator(Op::Select, cond.expr, selectTrue.expr, selectFalse.expr);
    if (!ternary) return false;

    if (!c.push(AstDecodeStackItem(ternary))) return false;
...
static bool AstDecodeConversion(AstDecodeContext& c, ValType fromType, ValType toType, Op op) {
...
static AstLoadStoreAddress AstDecodeLoadStoreAddress(const LinearMemoryAddress<Nothing>& addr,
                                                     const AstDecodeStackItem& item) {
    uint32_t flags = FloorLog2(addr.align);
    return AstLoadStoreAddress(item.expr, flags, addr.offset);
}

static bool AstDecodeLoad(AstDecodeContext& c, ValType type, uint32_t byteSize, Op op) {
...
static bool AstDecodeStore(AstDecodeContext& c, ValType type, uint32_t byteSize, Op op) {
    LinearMemoryAddress<Nothing> addr;
...
static bool AstDecodeExpr(AstDecodeContext& c) {
    ...
    switch (op.b0) {
        ...
        case uint16_t(Op::I32Eqz):
            if (!AstDecodeConversion(c, ValType::I32, ValType::I32, Op(op.b0))) return false;
            break;
        case uint16_t(Op::I64Eqz):
        case uint16_t(Op::I32WrapI64):
            if (!AstDecodeConversion(c, ValType::I64, ValType::I32, Op(op.b0))) return false;
            break;
        case uint16_t(Op::I32TruncSF32):
        case uint16_t(Op::I32TruncUF32):
        case uint16_t(Op::I32ReinterpretF32):
            if (!AstDecodeConversion(c, ValType::F32, ValType::I32, Op(op.b0))) return false;
            break;
        case uint16_t(Op::I32TruncSF64):
        case uint16_t(Op::I32TruncUF64):
            if (!AstDecodeConversion(c, ValType::F64, ValType::I32, Op(op.b0))) return false;
            break;
        case uint16_t(Op::I64ExtendSI32):
        case uint16_t(Op::I64ExtendUI32):
            if (!AstDecodeConversion(c, ValType::I32, ValType::I64, Op(op.b0))) return false;
            break;
        case uint16_t(Op::I64TruncSF32):
        case uint16_t(Op::I64TruncUF32):
            if (!AstDecodeConversion(c, ValType::F32, ValType::I64, Op(op.b0))) return false;
            break;
        case uint16_t(Op::I64TruncSF64):
        case uint16_t(Op::I64TruncUF64):
        case uint16_t(Op::I64ReinterpretF64):
            if (!AstDecodeConversion(c, ValType::F64, ValType::I64, Op(op.b0))) return false;
            break;
        case uint16_t(Op::F32ConvertSI32):
        case uint16_t(Op::F32ConvertUI32):
        case uint16_t(Op::F32ReinterpretI32):
            if (!AstDecodeConversion(c, ValType::I32, ValType::F32, Op(op.b0))) return false;
            break;
        case uint16_t(Op::F32ConvertSI64):
        case uint16_t(Op::F32ConvertUI64):
            if (!AstDecodeConversion(c, ValType::I64, ValType::F32, Op(op.b0))) return false;
            break;
        case uint16_t(Op::F32DemoteF64):
            if (!AstDecodeConversion(c, ValType::F64, ValType::F32, Op(op.b0))) return false;
            break;
        case uint16_t(Op::F64ConvertSI32):
        case uint16_t(Op::F64ConvertUI32):
            if (!AstDecodeConversion(c, ValType::I32, ValType::F64, Op(op.b0))) return false;
            break;
        case uint16_t(Op::F64ConvertSI64):
        case uint16_t(Op::F64ConvertUI64):
        case uint16_t(Op::F64ReinterpretI64):
            if (!AstDecodeConversion(c, ValType::I64, ValType::F64, Op(op.b0))) return false;
            break;
        case uint16_t(Op::F64PromoteF32):
            if (!AstDecodeConversion(c, ValType::F32, ValType::F64, Op(op.b0))) return false;
            break;
        ...
        case uint16_t(Op::I64Extend32S):
            if (!AstDecodeConversion(c, ValType::I64, ValType::I64, Op(op.b0))) return false;
            break;
#endif
...
static bool AstDecodeFunctionBody(AstDecodeContext& c, uint32_t funcIndex, AstFunc** func) {
    uint32_t offset = c.d.currentOffset();
    uint32_t bodySize;
    if (!c.d.readVarU32(&bodySize)) return c.d.fail("expected number of function body bytes");

    if (c.d.bytesRemain() < bodySize) return c.d.fail("function body length too big");
    ...
    if (c.d.currentPosition() != bodyEnd) return c.d.fail("function body length mismatch");
    ...
    for (size_t importIndex = 0; importIndex < c.env().imports.length(); importIndex++) {
        ...
        switch (import.kind) {
            case DefinitionKind::Function: {
                AstName importName;
                if (!GenerateName(c, AstName(u"import"), lastFunc, &importName)) return false;
            ...
            case DefinitionKind::Global: {
                AstName importName;
                if (!GenerateName(c, AstName(u"global"), lastGlobal, &importName)) return false;
            ...
            case DefinitionKind::Table: {
                AstName importName;
                if (!GenerateName(c, AstName(u"table"), lastTable, &importName)) return false;
            ...
            case DefinitionKind::Memory: {
                AstName importName;
                if (!GenerateName(c, AstName(u"memory"), lastMemory, &importName)) return false;

                ast = new (c.lifo)
                    AstImport(importName, moduleName, fieldName, DefinitionKind::Memory, *memory);
                lastMemory++;
                break;
            }
        }
...
static bool AstCreateMemory(AstDecodeContext& c) {
    ...
    AstName name;
    if (!GenerateName(c, AstName(u"memory"), c.module().memories().length(), &name)) return false;
...
static bool AstCreateGlobals(AstDecodeContext& c) {
    for (uint32_t i = 0; i < c.env().globals.length(); i++) {
        ...
        AstExpr* init = global.isConstant() ? new (c.lifo) AstConst(global.constantValue())
...
static bool AstDecodeCodeSection(AstDecodeContext& c) {
    ...
    if (!c.d.readVarU32(&numFuncBodies)) return c.d.fail("expected function body count");
...
bool wasm::BinaryToAst(JSContext* cx, const uint8_t* bytes, uint32_t length, LifoAlloc& lifo,
                       AstModule** module) {
    ...
    if (!AstDecodeEnvironment(c) || !AstDecodeCodeSection(c) || !AstDecodeModuleTail(c)) {
        if (error) {
