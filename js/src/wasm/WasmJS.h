...
template <typename T>
JSObject* CreateCustomNaNObject(JSContext* cx, T* addr);

// Converts a testing-only NaN JS object with a nan_low field to a float32 NaN
// with nan_low as the payload.

bool ReadCustomFloat32NaNObject(JSContext* cx, HandleValue v, uint32_t* ret);
...
bool ReadCustomDoubleNaNObject(JSContext* cx, HandleValue v, uint64_t* ret);
...
MOZ_MUST_USE bool Eval(JSContext* cx, Handle<TypedArrayObject*> code, HandleObject importObj,
...
class WasmInstanceObject : public NativeObject {
    ...
    using ExportMap =
        GCHashMap<uint32_t, HeapPtr<JSFunction*>, DefaultHasher<uint32_t>, SystemAllocPolicy>;
    ...
    // during debugging.
    using ScopeMap = JS::WeakCache<GCHashMap<uint32_t, ReadBarriered<WasmFunctionScope*>,
   ...
   public:
    ...
    static WasmInstanceObject* create(JSContext* cx, RefPtr<const wasm::Code> code,
                                      UniquePtr<wasm::DebugState> debug,
                                      UniquePtr<wasm::GlobalSegment> globals,
                                      HandleWasmMemoryObject memory,
                                      Vector<RefPtr<wasm::Table>, 0, SystemAllocPolicy>&& tables,
                                      Handle<FunctionVector> funcImports,
                                      const wasm::ValVector& globalImports, HandleObject proto);
    void initExportsObj(JSObject& exportsObj);

    wasm::Instance& instance() const;
    JSObject& exportsObj() const;

    static bool getExportedFunction(JSContext* cx, HandleWasmInstanceObject instanceObj,
    ...
    static WasmFunctionScope* getFunctionScope(JSContext* cx, HandleWasmInstanceObject instanceObj,
                                               uint32_t funcIndex);
};
...
class WasmMemoryObject : public NativeObject {
   ...
   public:
    ...
    static WasmMemoryObject* create(JSContext* cx, Handle<ArrayBufferObjectMaybeShared*> buffer,
                                    HandleObject proto);
