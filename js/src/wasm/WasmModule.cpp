...
size_t LinkDataTier::SymbolicLinkArray::serializedSize() const {
    size_t size = 0;
    for (const Uint32Vector& offsets : *this) size += SerializedPodVectorSize(offsets);
    return size;
}

uint8_t* LinkDataTier::SymbolicLinkArray::serialize(uint8_t* cursor) const {
    for (const Uint32Vector& offsets : *this) cursor = SerializePodVector(cursor, offsets);
    return cursor;
}

const uint8_t* LinkDataTier::SymbolicLinkArray::deserialize(const uint8_t* cursor) {
...
size_t LinkDataTier::SymbolicLinkArray::sizeOfExcludingThis(MallocSizeOf mallocSizeOf) const {
    size_t size = 0;
    for (const Uint32Vector& offsets : *this) size += offsets.sizeOfExcludingThis(mallocSizeOf);
    return size;
}

size_t LinkDataTier::serializedSize() const {
    return sizeof(pod()) + SerializedPodVectorSize(internalLinks) + symbolicLinks.serializedSize();
}
...
Tiers LinkData::tiers() const {
    if (hasTier2()) return Tiers(linkData1_->tier, linkData2_->tier);
...
LinkDataTier& LinkData::linkData(Tier tier) {
    switch (tier) {
        case Tier::Baseline:
            if (linkData1_->tier == Tier::Baseline) return *linkData1_;
            MOZ_CRASH("No linkData at this tier");
...
bool LinkData::initTier1(Tier tier, const Metadata& metadata) {
...
size_t LinkData::sizeOfExcludingThis(MallocSizeOf mallocSizeOf) const {
    size_t sum = 0;
    for (auto t : tiers()) sum += linkData(t).sizeOfExcludingThis(mallocSizeOf);
    return sum;
}
...
class Module::Tier2GeneratorTaskImpl : public Tier2GeneratorTask {
   ...
   public:
    Tier2GeneratorTaskImpl(Module& module, const CompileArgs& compileArgs)
        : module_(&module), compileArgs_(&compileArgs), cancelled_(false), finished_(false) {}
...
void Module::notifyCompilationListeners() {
    ...
    for (RefPtr<JS::WasmModuleListener>& listener : listeners) listener->onCompilationComplete();
}

void Module::finishTier2(UniqueLinkDataTier linkData2, UniqueMetadataTier metadata2,
...
/* virtual */ size_t Module::bytecodeSerializedSize() const { return bytecode_->bytes.length(); }

/* virtual */ void Module::bytecodeSerialize(uint8_t* bytecodeBegin, size_t bytecodeSize) const {
...
/* virtual */ bool Module::notifyWhenCompilationComplete(JS::WasmModuleListener* listener) {
    {
        auto tiering = tiering_.lock();
...
/* virtual */ size_t Module::compiledSerializedSize() const {
    ...
    return assumptions_.serializedSize() + linkData_.serializedSize() +
           ...
           SerializedPodVectorSize(dataSegments_) + SerializedVectorSize(elemSegments_) +
           code_->serializedSize();
}

/* virtual */ void Module::compiledSerialize(uint8_t* compiledBegin, size_t compiledSize) const {
    MOZ_ASSERT(!tiering_.lock()->active);
...
/* static */ SharedModule Module::deserialize(const uint8_t* bytecodeBegin, size_t bytecodeSize,
                                              const uint8_t* compiledBegin, size_t compiledSize,
                                              Metadata* maybeMetadata) {
    MutableBytes bytecode = js_new<ShareableBytes>();
    if (!bytecode || !bytecode->bytes.initLengthUninitialized(bytecodeSize)) return nullptr;
    ...
    if (!metadata) {
        ...
        if (!tierMetadata) return nullptr;
    ...
    LinkData linkData;
    if (!linkData.initTier1(Tier::Serialized, *metadata)) return nullptr;
    ...
    return js_new<Module>(Move(assumptions), *code,
                          ...
                          Move(linkData), Move(imports), Move(exports), Move(dataSegments),
                          Move(elemSegments), *bytecode);
}

/* virtual */ JSObject* Module::createObject(JSContext* cx) {
    if (!GlobalObject::ensureConstructor(cx, cx->global(), JSProto_WebAssembly)) return nullptr;
...
bool wasm::CompiledModuleAssumptionsMatch(PRFileDesc* compiled, JS::BuildIdCharVector&& buildId) {
    PRFileInfo info;
...
SharedModule wasm::DeserializeModule(PRFileDesc* bytecodeFile, PRFileDesc* maybeCompiledFile,
    ...
    if (PRFileDesc* compiledFile = maybeCompiledFile) {
        ...
        return Module::deserialize(bytecodeMapping.get(), bytecodeInfo.size, compiledMapping.get(),
                                   compiledInfo.size);
    }
    ...
    MutableBytes bytecode = js_new<ShareableBytes>();
    if (!bytecode || !bytecode->bytes.initLengthUninitialized(bytecodeInfo.size)) return nullptr;
...
bool Module::extractCode(JSContext* cx, Tier tier, MutableHandleValue vp) const {
    ...
    memcpy(code->as<TypedArrayObject>().viewDataUnshared(), codeSegment.base(),
           codeSegment.length());

    RootedValue value(cx, ObjectValue(*code));
    if (!JS_DefineProperty(cx, result, "code", value, JSPROP_ENUMERATE)) return false;
    ...
    for (const CodeRange& p : metadata(tier).codeRanges) {
        ...
        if (!JS_DefineProperty(cx, segment, "begin", value, JSPROP_ENUMERATE)) return false;

        value.setNumber((uint32_t)p.end());
        if (!JS_DefineProperty(cx, segment, "end", value, JSPROP_ENUMERATE)) return false;

        value.setNumber((uint32_t)p.kind());
        if (!JS_DefineProperty(cx, segment, "kind", value, JSPROP_ENUMERATE)) return false;
    ...
    if (!JS_DefineProperty(cx, result, "segments", value, JSPROP_ENUMERATE)) return false;

    vp.setObject(*result);
    return true;
}

static uint32_t EvaluateInitExpr(const ValVector& globalImports, InitExpr initExpr) {
...
bool Module::initSegments(JSContext* cx, HandleWasmInstanceObject instanceObj,
                          Handle<FunctionVector> funcImports, HandleWasmMemoryObject memoryObj,
    ...
    for (const ElemSegment& seg : elemSegments_) {
        ...
        if (offset > tableLength || tableLength - offset < numElems) {
            JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr, JSMSG_WASM_BAD_FIT, "elem",
                                      "table");
            return false;
        }
    }
    ...
    if (memoryObj) {
        for (const DataSegment& seg : dataSegments_) {
            ...
            if (offset > memoryLength || memoryLength - offset < seg.length) {
                JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr, JSMSG_WASM_BAD_FIT, "data",
                                          "memory");
                return false;
            }
        }
    ...
    for (const ElemSegment& seg : elemSegments_) {
        ...
        for (uint32_t i = 0; i < seg.elemCodeRangeIndices(tier).length(); i++) {
            ...
            if (funcIndex < funcImports.length() &&
                ...
                table.set(offset + i, exportInstance.codeBase(exportTier) + cr.funcTableEntry(),
                          exportInstance);
            } else {
                const CodeRange& cr = codeRanges[seg.elemCodeRangeIndices(tier)[i]];
                uint32_t entryOffset =
                    table.isTypedFunction() ? cr.funcNormalEntry() : cr.funcTableEntry();
...
bool Module::instantiateFunctions(JSContext* cx, Handle<FunctionVector> funcImports) const {
#ifdef DEBUG
    ...
    for (size_t i = 0; i < metadata(tier).funcImports.length(); i++) {
        ...
        if (!IsExportedFunction(f) || ExportedFunctionToInstance(f).isAsmJS()) continue;
...
static bool CheckLimits(JSContext* cx, uint32_t declaredMin, const Maybe<uint32_t>& declaredMax,
                        uint32_t actualLength, const Maybe<uint32_t>& actualMax, bool isAsmJS,
                        const char* kind) {
    if (isAsmJS) {
...
bool Module::instantiateMemory(JSContext* cx, MutableHandleWasmMemoryObject memory) const {
    ...
    if (memory) {
        ...
        RootedArrayBufferObjectMaybeShared buffer(
            cx, ArrayBufferObject::createForWasm(cx, declaredMin, declaredMax));
...
bool Module::instantiateTable(JSContext* cx, MutableHandleWasmTableObject tableObj,
...
static bool GetFunctionExport(JSContext* cx, HandleWasmInstanceObject instanceObj,
                              Handle<FunctionVector> funcImports, const Export& exp,
    ...
    if (!instanceObj->getExportedFunction(cx, instanceObj, exp.funcIndex(), &fun)) return false;

    val.setObject(*fun);
    return true;
}

static bool GetGlobalExport(JSContext* cx, const GlobalDescVector& globals, uint32_t globalIndex,
    ...
    switch (global.kind()) {
        ...
        case GlobalKind::Variable:
            MOZ_CRASH("mutable variables can't be exported");
...
static bool CreateExportObject(JSContext* cx, HandleWasmInstanceObject instanceObj,
                               Handle<FunctionVector> funcImports, HandleWasmTableObject tableObj,
                               HandleWasmMemoryObject memoryObj, const ValVector& globalImports,
    ...
    if (metadata.isAsmJS() && exports.length() == 1 && strlen(exports[0].fieldName()) == 0) {
        RootedValue val(cx);
        if (!GetFunctionExport(cx, instanceObj, funcImports, exports[0], &val)) return false;
    ...
    for (const Export& exp : exports) {
        ...
        switch (exp.kind()) {
            case DefinitionKind::Function:
                if (!GetFunctionExport(cx, instanceObj, funcImports, exp, &val)) return false;
                break;
        ...
        if (!JS_DefinePropertyById(cx, exportObj, id, val, JSPROP_ENUMERATE)) return false;
    }
...
bool Module::instantiate(JSContext* cx, Handle<FunctionVector> funcImports,
                         HandleWasmTableObject tableImport, HandleWasmMemoryObject memoryImport,
    ...
    if (metadata().debugEnabled) {
        ...
        if (!codeIsBusy_.compareExchange(false, true)) {
            auto codeSegment =
                CodeSegment::create(Tier::Baseline, *unlinkedCodeForDebugging_, *bytecode_,
                                    linkData_.linkData(Tier::Baseline), metadata());
            if (!codeSegment) {
                ReportOutOfMemory(cx);
                return false;
            }
    ...
    instance.set(WasmInstanceObject::create(cx, code, Move(debug), Move(globalSegment), memory,
    ...
    if (!initSegments(cx, instance, funcImports, memory, globalImports)) return false;
    ...
    if (metadata().startFuncIndex) {
        ...
        if (!instance->instance().callExport(cx, *metadata().startFuncIndex, args)) return false;
    }
