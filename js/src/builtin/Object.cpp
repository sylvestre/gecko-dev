 ...
 */
template <typename CharT>
static bool ArgsAndBodySubstring(mozilla::Range<const CharT> chars, size_t* outOffset,
                                 size_t* outLen) {
...
JSString* js::ObjectToSource(JSContext* cx, HandleObject obj) {
    ...
    AutoIdVector idv(cx);
    if (!GetPropertyKeys(cx, obj, JSITER_OWNONLY | JSITER_SYMBOLS, &idv)) return nullptr;
    ...
    auto AddProperty = [cx, &comma, &buf](HandleId id, HandleValue val,
        ...
        if (JSID_IS_SYMBOL(id)) {
             ...
             */
            if (JSID_IS_ATOM(id) ? !IsIdentifier(JSID_TO_ATOM(id)) : JSID_TO_INT(id) < 0) {
        ...
        if (kind == PropertyKind::Getter || kind == PropertyKind::Setter ||
            ...
            if (val.toObject().is<JSFunction>()) {
                ...
                if (((fun->isGetter() && kind == PropertyKind::Getter) ||
                    ...
                    fun->explicitName()) {
                    bool result;
                    if (!EqualStrings(cx, fun->explicitName(), idstr, &result)) return false;
...
static bool GetBuiltinTagSlow(JSContext* cx, HandleObject obj, MutableHandleString builtinTag) {
    // Step 4.
    bool isArray;
...
static MOZ_ALWAYS_INLINE JSString* GetBuiltinTagFast(JSObject* obj, const Class* clasp,
                                                     JSContext* cx) {
...
bool js::obj_toString(JSContext* cx, unsigned argc, Value* vp) {
    ...
    // Step 17.
    StringBuffer sb(cx);
    if (!sb.append("[object ") || !sb.append(tag.toString()) || !sb.append(']')) return false;
...
JSString* js::ObjectClassToString(JSContext* cx, HandleObject obj) {
    ...
    StringBuffer sb(cx);
    if (!sb.append("[object ") || !sb.append(className, strlen(className)) || !sb.append(']')) {
        return nullptr;
    }
...
static bool PropertyIsEnumerable(JSContext* cx, HandleObject obj, HandleId id, bool* enumerable) {
    PropertyResult prop;
    if (obj->isNative() &&
        NativeLookupOwnProperty<NoGC>(cx, &obj->as<NativeObject>(), id, &prop)) {
        if (!prop) {
...
static bool TryAssignNative(JSContext* cx, HandleObject to, HandleObject from, bool* optimized) {
    *optimized = false;
    ...
    if (fromNative->getDenseInitializedLength() > 0 || fromNative->isIndexed() ||
        fromNative->is<TypedArrayObject>() || fromNative->getClass()->getNewEnumerate() ||
    ...
    *optimized = true;
    ...
    for (size_t i = shapes.length(); i > 0; i--) {
        ...
        if (MOZ_LIKELY(from->isNative() && from->as<NativeObject>().lastProperty() == fromShape &&
...
static bool AssignSlow(JSContext* cx, HandleObject to, HandleObject from) {
    ...
    for (size_t i = 0, len = keys.length(); i < len; i++) {
        ...
        // Step 4.c.i.
        bool enumerable;
        if (MOZ_UNLIKELY(!PropertyIsEnumerable(cx, from, nextKey, &enumerable))) return false;
        if (!enumerable) continue;

        // Step 4.c.ii.1.
        if (MOZ_UNLIKELY(!GetProperty(cx, from, from, nextKey, &propValue))) return false;
...
bool js::WatchHandler(JSContext* cx, JSObject* obj_, jsid id_, const JS::Value& old,
    ...
    if (resolving.alreadyStarted()) return true;
    ...
    RootedValue rv(cx);
    if (!Call(cx, callable, thisv, args, &rv)) return false;

    *nvp = rv;
    return true;
}

static bool obj_watch(JSContext* cx, unsigned argc, Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);
    ...
    RootedId propid(cx);
    if (!ValueToId<CanGC>(cx, args[0], &propid)) return false;

    if (!WatchProperty(cx, obj, propid, callable)) return false;

    args.rval().setUndefined();
    return true;
}

static bool obj_unwatch(JSContext* cx, unsigned argc, Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);
    ...
    if (!UnwatchProperty(cx, obj, id)) return false;
...
PlainObject* js::ObjectCreateImpl(JSContext* cx, HandleObject proto, NewObjectKind newKind,
...
PlainObject* js::ObjectCreateWithTemplate(JSContext* cx, HandlePlainObject templateObj) {
...
static bool ObjectDefineProperties(JSContext* cx, HandleObject obj, HandleValue properties) {
    // Step 1. implicit
    // Step 2.
    ...
    for (size_t i = 0, len = keys.length(); i < len; i++) {
        ...
        if (desc.object() && desc.enumerable()) {
            if (!GetProperty(cx, props, props, nextKey, &descObj) ||
                !ToPropertyDescriptor(cx, descObj, true, &desc) || !descriptors.append(desc) ||
    ...
    for (size_t i = 0, len = descriptors.length(); i < len; i++) {
        if (!DefineProperty(cx, obj, descriptorKeys[i], descriptors[i])) return false;
    }
...
static bool FromPropertyDescriptorToArray(JSContext* cx, Handle<PropertyDescriptor> desc,
...
bool js::GetOwnPropertyDescriptorToArray(JSContext* cx, unsigned argc, Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);
...
enum EnumerableOwnPropertiesKind { Keys, Values, KeysAndValues };
...
static bool EnumerableOwnProperties(JSContext* cx, const JS::CallArgs& args,
    ...
    // Step 2.
    AutoIdVector ids(cx);
    if (!GetPropertyKeys(cx, obj, JSITER_OWNONLY | JSITER_HIDDEN, &ids)) return false;
    ...
    if (obj->is<NativeObject>()) nobj = &obj->as<NativeObject>();
    RootedShape shape(cx);
    Rooted<PropertyDescriptor> desc(cx);
    // Step 4.
    size_t out = 0;
    for (size_t i = 0; i < len; i++) {
        ...
        if (nobj) {
            if (JSID_IS_INT(id) && nobj->containsDenseElement(JSID_TO_INT(id))) {
                ...
                if (!shape || !(shape->attributes() & JSPROP_ENUMERATE)) continue;
                if (!shape->isAccessorShape()) {
                    if (!NativeGetExistingProperty(cx, nobj, nobj, shape, &value)) return false;
...
bool js::IdToStringOrSymbol(JSContext* cx, HandleId id, MutableHandleValue result) {
    if (JSID_IS_INT(id)) {
...
bool js::GetOwnPropertyKeys(JSContext* cx, const JS::CallArgs& args, unsigned flags) {
    // Step 1.
    RootedObject obj(cx, ToObject(cx, args.get(0)));
    if (!obj) return false;
...
static bool obj_getOwnPropertySymbols(JSContext* cx, unsigned argc, Value* vp) {
    ...
    return GetOwnPropertyKeys(
        cx, args, JSITER_OWNONLY | JSITER_HIDDEN | JSITER_SYMBOLS | JSITER_SYMBOLSONLY);
}
...
bool js::obj_defineProperty(JSContext* cx, unsigned argc, Value* vp) {
    ...
    // Steps 1-3.
    RootedObject obj(cx);
    if (!GetFirstArgumentAsObject(cx, args, "Object.defineProperty", &obj)) return false;
    RootedId id(cx);
...
static bool obj_defineProperties(JSContext* cx, unsigned argc, Value* vp) {
    ...
    /* Steps 1 and 7. */
    RootedObject obj(cx);
    if (!GetFirstArgumentAsObject(cx, args, "Object.defineProperties", &obj)) return false;
    args.rval().setObject(*obj);
...
static bool obj_isFrozen(JSContext* cx, unsigned argc, Value* vp) {
    ...
    if (args.get(0).isObject()) {
        RootedObject obj(cx, &args.get(0).toObject());
        if (!TestIntegrityLevel(cx, obj, IntegrityLevel::Frozen, &frozen)) return false;
    }
...
static bool obj_isSealed(JSContext* cx, unsigned argc, Value* vp) {
    ...
    if (args.get(0).isObject()) {
        RootedObject obj(cx, &args.get(0).toObject());
        if (!TestIntegrityLevel(cx, obj, IntegrityLevel::Sealed, &sealed)) return false;
    }
...
static bool ProtoSetter(JSContext* cx, unsigned argc, Value* vp) {
    ...
    if (!cx->runningWithTrustedPrincipals()) ++sSetProtoCalled;
...
static JSObject* CreateObjectConstructor(JSContext* cx, JSProtoKey key) {
    ...
    if (!GlobalObject::ensureConstructor(cx, self, JSProto_Function)) return nullptr;

    /* Create the Object function now that we have a [[Prototype]] for it. */
    JSFunction* fun =
        NewNativeConstructor(cx, obj_construct, 1, HandlePropertyName(cx->names().Object),
...
static JSObject* CreateObjectPrototype(JSContext* cx, JSProtoKey key) {
     ...
     */
    if (!JSObject::setNewGroupUnknown(cx, &PlainObject::class_, objectProto)) return nullptr;

    return objectProto;
}

static bool FinishObjectClassInit(JSContext* cx, JS::HandleObject ctor, JS::HandleObject proto) {
    ...
    if (global->shouldSplicePrototype()) {
        if (!JSObject::splicePrototype(cx, global, global->getClass(), tagged)) return false;
    }
    return true;
}

static const ClassSpec PlainObjectClassSpec = {
    CreateObjectConstructor, CreateObjectPrototype, object_static_methods, nullptr,
    object_methods,          object_properties,     FinishObjectClassInit};

const Class PlainObject::class_ = {js_Object_str, JSCLASS_HAS_CACHED_PROTO(JSProto_Object),
