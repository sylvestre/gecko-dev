 ...
 */
extern MOZ_MUST_USE JS_PUBLIC_API(bool) JS_StartProfiling(const char* profileName, pid_t pid);

/**
 * Stop any profilers that were previously started with JS_StartProfiling.
 * Returns true if no profilers fail to stop.
 */
extern MOZ_MUST_USE JS_PUBLIC_API(bool) JS_StopProfiling(const char* profileName);
 ...
 */
extern MOZ_MUST_USE JS_PUBLIC_API(bool) JS_PauseProfilers(const char* profileName);

/**
 * Resume suspended profilers
 */
extern MOZ_MUST_USE JS_PUBLIC_API(bool) JS_ResumeProfilers(const char* profileName);
