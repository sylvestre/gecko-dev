...
const Class js::TypedObjectModuleObject::class_ = {
    "TypedObject",
    JSCLASS_HAS_RESERVED_SLOTS(SlotCount) | JSCLASS_HAS_CACHED_PROTO(JSProto_TypedObject)};

static const JSFunctionSpec TypedObjectMethods[] = {
    JS_SELF_HOSTED_FN("objectType", "TypeOfTypedObject", 1, 0),
    JS_SELF_HOSTED_FN("storage", "StorageOfTypedObject", 1, 0), JS_FS_END};

static void ReportCannotConvertTo(JSContext* cx, HandleValue fromValue, const char* toType) {
...
static inline CheckedInt32 RoundUpToAlignment(CheckedInt32 address, uint32_t align) {
 ...
 */
static bool ConvertAndCopyTo(JSContext* cx, HandleTypeDescr typeObj, HandleTypedObject typedObj,
                             int32_t offset, HandleAtom name, HandleValue val) {
...
static bool ConvertAndCopyTo(JSContext* cx, HandleTypedObject typedObj, HandleValue val) {
 ...
 */
static bool Reify(JSContext* cx, HandleTypeDescr type, HandleTypedObject typedObj, size_t offset,
...
const Class js::TypedProto::class_ = {"TypedProto", JSCLASS_HAS_RESERVED_SLOTS(JS_TYPROTO_SLOTS)};
...
const Class js::ScalarTypeDescr::class_ = {
    "Scalar", JSCLASS_HAS_RESERVED_SLOTS(JS_DESCR_SLOTS) | JSCLASS_BACKGROUND_FINALIZE,
...
uint32_t ScalarTypeDescr::size(Type t) { return AssertedCast<uint32_t>(Scalar::byteSize(t)); }

uint32_t ScalarTypeDescr::alignment(Type t) { return AssertedCast<uint32_t>(Scalar::byteSize(t)); }
...
const Class js::ReferenceTypeDescr::class_ = {
    "Reference", JSCLASS_HAS_RESERVED_SLOTS(JS_DESCR_SLOTS) | JSCLASS_BACKGROUND_FINALIZE,
 ...
 */
static TypedProto* CreatePrototypeObjectForComplexTypeInstance(JSContext* cx,
...
const Class ArrayTypeDescr::class_ = {
    "ArrayType", JSCLASS_HAS_RESERVED_SLOTS(JS_DESCR_SLOTS) | JSCLASS_BACKGROUND_FINALIZE,
...
bool js::CreateUserSizeAndAlignmentProperties(JSContext* cx, HandleTypeDescr descr) {
...
ArrayTypeDescr* ArrayMetaTypeDescr::create(JSContext* cx, HandleObject arrayTypePrototype,
                                           HandleTypeDescr elementType, HandleAtom stringRepr,
    ...
    if (elementType->getReservedSlot(JS_DESCR_SLOT_ARRAYPROTO).isObject()) {
        prototypeObj =
            &elementType->getReservedSlot(JS_DESCR_SLOT_ARRAYPROTO).toObject().as<TypedProto>();
    } else {
...
bool ArrayMetaTypeDescr::construct(JSContext* cx, unsigned argc, Value* vp) {
    ...
    if (!NumberValueToStringBuffer(cx, NumberValue(length), contents)) return false;
    if (!contents.append(")")) return false;
...
const Class StructTypeDescr::class_ = {
    "StructType", JSCLASS_HAS_RESERVED_SLOTS(JS_DESCR_SLOTS) | JSCLASS_BACKGROUND_FINALIZE,
...
JSObject* StructMetaTypeDescr::create(JSContext* cx, HandleObject metaTypeDescr,
    ...
    AutoIdVector ids(cx);
    if (!GetPropertyKeys(cx, fields, JSITER_OWNONLY | JSITER_SYMBOLS, &ids)) return nullptr;
    ...
    // Construct for internal use an array with the name for each field.
    {
        ...
        fieldNamesVec = NewDenseCopiedArray(cx, fieldNames.length(), fieldNames.begin(), nullptr,
                                            TenuredObject);
    ...
    fieldTypeVec = NewDenseCopiedArray(cx, fieldTypeObjs.length(), fieldTypeObjs.begin(), nullptr,
                                       TenuredObject);
    ...
    // Construct for internal use an array with the offset for each field.
    {
        ...
        fieldOffsetsVec = NewDenseCopiedArray(cx, fieldOffsets.length(), fieldOffsets.begin(),
...
bool StructMetaTypeDescr::construct(JSContext* cx, unsigned int argc, Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);
...
template <typename T>
static bool DefineSimpleTypeDescr(JSContext* cx, Handle<GlobalObject*> global, HandleObject module,
                                  typename T::Type type, HandlePropertyName className) {
...
template <typename T>
static JSObject* DefineMetaTypeDescr(JSContext* cx, const char* name, Handle<GlobalObject*> global,
    ...
    if (!ctor || !LinkConstructorAndPrototype(cx, ctor, proto) ||
        !DefinePropertiesAndFunctions(cx, proto, T::typeObjectProperties, T::typeObjectMethods) ||
 ...
 */
/* static */ bool GlobalObject::initTypedObjectModule(JSContext* cx,
...
#undef BINARYDATA_REFERENCE_DEFINE
    ...
    if (!DefineDataProperty(cx, global, cx->names().TypedObject, moduleValue, JSPROP_RESOLVING)) {
        return false;
    }
...
uint32_t TypedObject::length() const { return typeDescr().as<ArrayTypeDescr>().length(); }
...
uint8_t* TypedObject::typedMemBase() const {
    ...
    if (owner.is<ArrayBufferObject>()) return owner.as<ArrayBufferObject>().dataPointer();
...
bool TypedObject::isAttached() const {
    ...
    JSObject& owner = as<OutlineTypedObject>().owner();
    if (owner.is<ArrayBufferObject>() && owner.as<ArrayBufferObject>().isDetached()) return false;
    return true;
}

/* static */ bool TypedObject::GetBuffer(JSContext* cx, unsigned argc, Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);
...
/* static */ bool TypedObject::GetByteOffset(JSContext* cx, unsigned argc, Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);
...
/*static*/ OutlineTypedObject* OutlineTypedObject::createUnattached(JSContext* cx,
...
/*static*/ OutlineTypedObject* OutlineTypedObject::createUnattachedWithClass(
    ...
    RootedObjectGroup group(
        cx, ObjectGroup::defaultNewGroup(cx, clasp, TaggedProto(&descr->typedProto()), descr));
    if (!group) return nullptr;

    NewObjectKind newKind = (heap == gc::TenuredHeap) ? TenuredObject : GenericObject;
    OutlineTypedObject* obj =
        NewObjectWithGroup<OutlineTypedObject>(cx, group, gc::AllocKind::OBJECT0, newKind);
    if (!obj) return nullptr;
...
void OutlineTypedObject::attach(JSContext* cx, ArrayBufferObject& buffer, uint32_t offset) {
    MOZ_ASSERT(!isAttached());
...
void OutlineTypedObject::attach(JSContext* cx, TypedObject& typedObj, uint32_t offset) {
    MOZ_ASSERT(!isAttached());
...
/*static*/ OutlineTypedObject* OutlineTypedObject::createDerived(JSContext* cx,
    ...
    const js::Class* clasp = typedObj->opaque() ? &OutlineOpaqueTypedObject::class_
...
/*static*/ TypedObject* TypedObject::createZeroed(JSContext* cx, HandleTypeDescr descr,
                                                  int32_t length, gc::InitialHeap heap) {
    ...
    Rooted<OutlineTypedObject*> obj(cx,
                                    OutlineTypedObject::createUnattached(cx, descr, length, heap));
    if (!obj) return nullptr;
...
/* static */ void OutlineTypedObject::obj_trace(JSTracer* trc, JSObject* object) {
    ...
    MOZ_ASSERT_IF(owner->is<ArrayBufferObject>(),
                  !owner->as<ArrayBufferObject>().forInlineTypedObject());
    if (owner != oldOwner &&
        (owner->is<InlineTypedObject>() || owner->as<ArrayBufferObject>().hasInlineData())) {
...
/* static */ bool TypedObject::obj_lookupProperty(JSContext* cx, HandleObject obj, HandleId id,
...
static bool ReportPropertyError(JSContext* cx, const unsigned errorNumber, HandleId id) {
...
bool TypedObject::obj_defineProperty(JSContext* cx, HandleObject obj, HandleId id,
                                     Handle<PropertyDescriptor> desc, ObjectOpResult& result) {
    Rooted<TypedObject*> typedObj(cx, &obj->as<TypedObject>());
    return ReportTypedObjTypeError(cx, JSMSG_OBJECT_NOT_EXTENSIBLE, typedObj);
}

bool TypedObject::obj_hasProperty(JSContext* cx, HandleObject obj, HandleId id, bool* foundp) {
    Rooted<TypedObject*> typedObj(cx, &obj->as<TypedObject>());
...
bool TypedObject::obj_getProperty(JSContext* cx, HandleObject obj, HandleValue receiver,
                                  HandleId id, MutableHandleValue vp) {
    Rooted<TypedObject*> typedObj(cx, &obj->as<TypedObject>());

    // Dispatch elements to obj_getElement:
    uint32_t index;
    if (IdIsIndex(id, &index)) return obj_getElement(cx, obj, receiver, index, vp);
...
bool TypedObject::obj_getElement(JSContext* cx, HandleObject obj, HandleValue receiver,
...
/*static*/ bool TypedObject::obj_getArrayElement(JSContext* cx, Handle<TypedObject*> typedObj,
                                                 Handle<TypeDescr*> typeDescr, uint32_t index,
...
bool TypedObject::obj_setProperty(JSContext* cx, HandleObject obj, HandleId id, HandleValue v,
    ...
    switch (typedObj->typeDescr().kind()) {
        ...
        case type::Array: {
            ...
            if (IdIsIndex(id, &index)) {
                ...
                size_t offset = elementType->size() * index;
                if (!ConvertAndCopyTo(cx, elementType, typedObj, offset, nullptr, v)) return false;
        ...
        case type::Struct: {
            ...
            if (!ConvertAndCopyTo(cx, fieldType, typedObj, offset, fieldName, v)) return false;
            return result.succeed();
        }
    }
...
bool TypedObject::obj_getOwnPropertyDescriptor(JSContext* cx, HandleObject obj, HandleId id,
    ...
    switch (descr->kind()) {
        ...
        case type::Array: {
            uint32_t index;
            if (IdIsIndex(id, &index)) {
                if (!obj_getArrayElement(cx, typedObj, descr, index, desc.value())) return false;
...
static bool IsOwnId(JSContext* cx, HandleObject obj, HandleId id) {
    ...
    switch (typedObj->typeDescr().kind()) {
        ...
        case type::Struct:
            size_t index;
            if (typedObj->typeDescr().as<StructTypeDescr>().fieldIndex(id, &index)) return true;
    }

    return false;
}

bool TypedObject::obj_deleteProperty(JSContext* cx, HandleObject obj, HandleId id,
                                     ObjectOpResult& result) {
    if (IsOwnId(cx, obj, id)) return ReportPropertyError(cx, JSMSG_CANT_DELETE, id);
...
bool TypedObject::obj_newEnumerate(JSContext* cx, HandleObject obj, AutoIdVector& properties,
                                   bool enumerableOnly) {
    MOZ_ASSERT(obj->is<TypedObject>());
    Rooted<TypedObject*> typedObj(cx, &obj->as<TypedObject>());
    Rooted<TypeDescr*> descr(cx, &typedObj->typeDescr());
...
/* static */ InlineTypedObject* InlineTypedObject::create(JSContext* cx, HandleTypeDescr descr,
                                                          gc::InitialHeap heap) {
    gc::AllocKind allocKind = allocKindForTypeDescriptor(descr);

    const Class* clasp =
        descr->opaque() ? &InlineOpaqueTypedObject::class_ : &InlineTransparentTypedObject::class_;
...
/* static */ InlineTypedObject* InlineTypedObject::createCopy(
    JSContext* cx, Handle<InlineTypedObject*> templateObject, gc::InitialHeap heap) {
    AutoSetNewObjectMetadata metadata(cx);
...
/* static */ void InlineTypedObject::obj_trace(JSTracer* trc, JSObject* object) {
...
ArrayBufferObject* InlineTransparentTypedObject::getOrCreateBuffer(JSContext* cx) {
...
ArrayBufferObject* OutlineTransparentTypedObject::getOrCreateBuffer(JSContext* cx) {
...
#define DEFINE_TYPEDOBJ_CLASS(Name, Trace, Moved, flag)                    \
    ...
    const Class Name::class_ = {#Name,           Class::NON_NATIVE | flag, \
                                &Name##ClassOps, JS_NULL_CLASS_SPEC,       \
                                &Name##ClassExt, &TypedObject::objectOps_}

DEFINE_TYPEDOBJ_CLASS(OutlineTransparentTypedObject, OutlineTypedObject::obj_trace, nullptr, 0);
DEFINE_TYPEDOBJ_CLASS(OutlineOpaqueTypedObject, OutlineTypedObject::obj_trace, nullptr, 0);
DEFINE_TYPEDOBJ_CLASS(InlineTransparentTypedObject, InlineTypedObject::obj_trace,
                      InlineTypedObject::obj_moved, JSCLASS_DELAY_METADATA_BUILDER);
...
/*static*/ bool TypedObject::construct(JSContext* cx, unsigned int argc, Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);
    ...
    if (args[0].isObject() && args[0].toObject().is<ArrayBufferObject>()) {
        ...
        if (!CheckOffset(offset, callee->size(), callee->alignment(), buffer->byteLength())) {
            JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr, JSMSG_TYPEDOBJECT_BAD_ARGS);
            return false;
        }
...
/* static */ JS::Result<TypedObject*, JS::OOM&> TypedObject::create(JSContext* cx,
...
bool js::NewOpaqueTypedObject(JSContext* cx, unsigned argc, Value* vp) {
    ...
    Rooted<OutlineTypedObject*> obj(cx);
    obj = OutlineTypedObject::createUnattachedWithClass(cx, &OutlineOpaqueTypedObject::class_,
                                                        descr, length);
    if (!obj) return false;
    args.rval().setObject(*obj);
    return true;
...
#define JS_STORE_REFERENCE_CLASS_IMPL(_constant, T, _name)                                        \
    bool js::StoreReference##_name::Func(JSContext* cx, unsigned argc, Value* vp) {               \
        CallArgs args = CallArgsFromVp(argc, vp);                                                 \
        ...
        int32_t offset = args[1].toInt32();                                                       \
                                                                                                  \
        jsid id =                                                                                 \
            args[2].isString() ? IdToTypeId(AtomToId(&args[2].toString()->asAtom())) : JSID_VOID; \
                                                                                                  \
        /* Should be guaranteed by the typed objects API: */                                      \
        MOZ_ASSERT(offset % MOZ_ALIGNOF(T) == 0);                                                 \
                                                                                                  \
...
#define JS_LOAD_REFERENCE_CLASS_IMPL(_constant, T, _name)                          \
    bool js::LoadReference##_name::Func(JSContext* cx, unsigned argc, Value* vp) { \
        CallArgs args = CallArgsFromVp(argc, vp);                                  \
        MOZ_ASSERT(args.length() == 2);                                            \
        MOZ_ASSERT(args[0].isObject() && args[0].toObject().is<TypedObject>());    \
        MOZ_ASSERT(args[1].isInt32());                                             \
...
bool StoreReferenceAny::store(JSContext* cx, GCPtrValue* heap, const Value& v, TypedObject* obj,
                              jsid id) {
...
bool StoreReferenceObject::store(JSContext* cx, GCPtrObject* heap, const Value& v,
...
bool StoreReferencestring::store(JSContext* cx, GCPtrString* heap, const Value& v,
                                 TypedObject* obj, jsid id) {
...
void LoadReferenceAny::load(GCPtrValue* heap, MutableHandleValue v) { v.set(*heap); }
...
void LoadReferencestring::load(GCPtrString* heap, MutableHandleValue v) { v.setString(*heap); }
...
void MemoryInitVisitor::visitReference(ReferenceTypeDescr& descr, uint8_t* mem) {
    switch (descr.type()) {
        case ReferenceTypeDescr::TYPE_ANY: {
...
void TypeDescr::initInstances(const JSRuntime* rt, uint8_t* mem, size_t length) {
    MOZ_ASSERT(length >= 1);
...
void MemoryTracingVisitor::visitReference(ReferenceTypeDescr& descr, uint8_t* mem) {
    switch (descr.type()) {
        case ReferenceTypeDescr::TYPE_ANY: {
...
void TraceListVisitor::visitReference(ReferenceTypeDescr& descr, uint8_t* mem) {
    ...
    AutoEnterOOMUnsafeRegion oomUnsafe;
    if (!offsets->append((uintptr_t)mem)) oomUnsafe.crash("TraceListVisitor::visitReference");
}
...
static bool CreateTraceList(JSContext* cx, HandleTypeDescr descr) {
    ...
    // than inline ones.
    if (descr->size() > InlineTypedObject::MaximumSize || descr->transparent()) return true;
