...
static bool CheckVectorObject(HandleValue v, SimdType expectedType) {
    ...
    if (typeRepr.kind() != type::Simd) return false;

    return typeRepr.as<SimdTypeDescr>().type() == expectedType;
}

template <class V>
bool js::IsVectorObject(HandleValue v) {
    return CheckVectorObject(v, V::type);
}

#define FOR_EACH_SIMD(macro)                                                                     \
    macro(Int8x16) macro(Int16x8) macro(Int32x4) macro(Uint8x16) macro(Uint16x8) macro(Uint32x4) \
        macro(Float32x4) macro(Float64x2) macro(Bool8x16) macro(Bool16x8) macro(Bool32x4)        \
            macro(Bool64x2)

#define InstantiateIsVectorObject_(T) template bool js::IsVectorObject<T>(HandleValue v);
FOR_EACH_SIMD(InstantiateIsVectorObject_)
#undef InstantiateIsVectorObject_

const char* js::SimdTypeToString(SimdType type) {
    switch (type) {
#define RETSTR_(TYPE)    \
    case SimdType::TYPE: \
        return #TYPE;
...
PropertyName* js::SimdTypeToName(const JSAtomState& atoms, SimdType type) {
    switch (type) {
#define CASE_(TypeName)      \
    case SimdType::TypeName: \
        return atoms.TypeName;
        FOR_EACH_SIMD(CASE_)
#undef CASE_
        case SimdType::Count:
            break;
...
bool js::IsSimdTypeName(const JSAtomState& atoms, const PropertyName* name, SimdType* type) {
#define CHECK_(TypeName)            \
    if (name == atoms.TypeName) {   \
...
static inline bool ErrorBadArgs(JSContext* cx) {
    JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr, JSMSG_TYPED_ARRAY_BAD_ARGS);
    return false;
}

static inline bool ErrorWrongTypeArg(JSContext* cx, unsigned argIndex,
    ...
    if (!typeNameStr) return false;

    JS_ReportErrorNumberLatin1(cx, GetErrorMessage, nullptr, JSMSG_SIMD_NOT_A_VECTOR, typeNameStr,
                               charArgIndex);
    JS_free(cx, typeNameStr);
    return false;
}

static inline bool ErrorBadIndex(JSContext* cx) {
 ...
 */
static bool NonStandardToIndex(JSContext* cx, HandleValue v, uint64_t* index) {
    // Fast common case.
    if (v.isInt32()) {
        int32_t i = v.toInt32();
        if (i >= 0) {
...
template <typename T>
static SimdTypeDescr* GetTypeDescr(JSContext* cx) {
    RootedGlobalObject global(cx, cx->global());
    return GlobalObject::getOrCreateSimdTypeDescr(cx, global, T::type);
}

template <typename V>
bool js::ToSimdConstant(JSContext* cx, HandleValue v, jit::SimdConstant* out) {
    typedef typename V::Elem Elem;
    Rooted<TypeDescr*> typeDescr(cx, GetTypeDescr<V>(cx));
    if (!typeDescr) return false;
    if (!IsVectorObject<V>(v)) return ErrorWrongTypeArg(cx, 1, typeDescr);
...
static Elem TypedObjectMemory(HandleValue v, const JS::AutoRequireNoGC& nogc) {
    TypedObject& obj = v.toObject().as<TypedObject>();
    return reinterpret_cast<Elem>(obj.typedMem(nogc));
}

static const ClassOps SimdTypeDescrClassOps = {nullptr, /* addProperty */
                                               nullptr, /* delProperty */
                                               nullptr, /* enumerate */
...
const Class SimdTypeDescr::class_ = {
    "SIMD", JSCLASS_HAS_RESERVED_SLOTS(JS_DESCR_SLOTS) | JSCLASS_BACKGROUND_FINALIZE,
...
static const JSFunctionSpec SimdTypedObjectMethods[] = {
    ...
    JS_SELF_HOSTED_FN("toSource", "SimdToSource", 0, 0), JS_FS_END};
...
#define DEFN(TYPE, OP)                                                                                            \
    const JSJitInfo JitInfo_Simd##TYPE##_##OP = {                                                                 \
        /* .getter, unused for inlinable natives. */                                                              \
        {nullptr}, /* .inlinableNative, but we have to init first union member: .protoID. */                      \
        {uint16_t(                                                                                                \
            InlinableNative::                                                                                     \
                Simd##TYPE)}, /* .nativeOp. Actually initializing first union member .depth. */                   \
        {uint16_t(SimdOperation::                                                                                 \
                      Fn_##OP)}, /* .type_ bitfield says this in an inlinable native function. */                 \
        JSJitInfo::                                                                                               \
            InlinableNative /* Remaining fields are not used for inlinable natives. They are zero-initialized. */ \
    };
...
template <typename T>
static bool FillLanes(JSContext* cx, Handle<TypedObject*> result, const CallArgs& args) {
    typedef typename T::Elem Elem;
    Elem tmp;
    for (unsigned i = 0; i < T::lanes; i++) {
        if (!T::Cast(cx, args.get(i), &tmp)) return false;
...
bool SimdTypeDescr::call(JSContext* cx, unsigned argc, Value* vp) {
...
#define CASE_CALL_(Type) \
    case SimdType::Type: \
        return FillLanes< ::Type>(cx, result, args);
...
static const ClassOps SimdObjectClassOps = {nullptr, /* addProperty */
                                            nullptr, /* delProperty */
                                            nullptr, /* enumerate */
                                            nullptr, /* newEnumerate */
                                            SimdObject::resolve};

const Class SimdObject::class_ = {"SIMD", JSCLASS_HAS_RESERVED_SLOTS(uint32_t(SimdType::Count)),
                                  &SimdObjectClassOps};

/* static */ bool GlobalObject::initSimdObject(JSContext* cx, Handle<GlobalObject*> global) {
    ...
    if (!GlobalObject::getOrCreateTypedObjectModule(cx, global)) return false;
    ...
    if (!globalSimdObject) return false;
...
static bool CreateSimdType(JSContext* cx, Handle<GlobalObject*> global,
                           HandlePropertyName stringRepr, SimdType simdType,
    ...
    typeDescr->initReservedSlot(JS_DESCR_SLOT_ALIGNMENT,
                                Int32Value(SimdTypeDescr::alignment(simdType)));
    ...
    if (!CreateUserSizeAndAlignmentProperties(cx, typeDescr)) return false;
    ...
    if (!JS_DefineFunctions(cx, typeDescr, TypeDescriptorMethods)) return false;
...
/* static */ bool GlobalObject::initSimdType(JSContext* cx, Handle<GlobalObject*> global,
                                             SimdType simdType) {
#define CREATE_(Type)    \
    case SimdType::Type: \
...
/* static */ SimdTypeDescr* GlobalObject::getOrCreateSimdTypeDescr(JSContext* cx,
                                                                   Handle<GlobalObject*> global,
                                                                   SimdType simdType) {
    MOZ_ASSERT(unsigned(simdType) < unsigned(SimdType::Count), "Invalid SIMD type");

    RootedObject globalSimdObject(cx, GlobalObject::getOrCreateSimdGlobalObject(cx, global));
    if (!globalSimdObject) return nullptr;
...
bool SimdObject::resolve(JSContext* cx, JS::HandleObject obj, JS::HandleId id, bool* resolved) {
    *resolved = false;
...
#define TRY_RESOLVE_(Type)                                                                     \
    if (str == cx->names().Type) {                                                             \
        *resolved =                                                                            \
            CreateSimdType(cx, global, cx->names().Type, SimdType::Type, Type##Defn::Methods); \
        return *resolved;                                                                      \
    }
...
template <typename V>
JSObject* js::CreateSimd(JSContext* cx, const typename V::Elem* data) {
    typedef typename V::Elem Elem;
    Rooted<TypeDescr*> typeDescr(cx, GetTypeDescr<V>(cx));
    if (!typeDescr) return nullptr;
...
static T Saturate(int x) {
    ...
    if (x > int(upper)) return upper;
    if (x < int(lower)) return lower;
    return T(x);
}
...
static bool StoreResult(JSContext* cx, CallArgs& args, typename Out::Elem* result) {
...
static bool CoercedUnaryFunc(JSContext* cx, unsigned argc, Value* vp) {
    typedef typename Coercion::Elem CoercionElem;
    typedef typename Out::Elem RetElem;

    CallArgs args = CallArgsFromVp(argc, vp);
    if (args.length() != 1 || !IsVectorObject<In>(args[0])) return ErrorBadArgs(cx);

    CoercionElem result[Coercion::lanes];
    TypedObjectElemArray<CoercionElem> val(args[0]);
    for (unsigned i = 0; i < Coercion::lanes; i++) result[i] = Op<CoercionElem>::apply(val[i]);
...
// and converts the result to the type Out.
template <typename In, typename Coercion, template <typename C> class Op, typename Out>
static bool CoercedBinaryFunc(JSContext* cx, unsigned argc, Value* vp) {
    typedef typename Coercion::Elem CoercionElem;
    typedef typename Out::Elem RetElem;
...
static bool UnaryFunc(JSContext* cx, unsigned argc, Value* vp) {
    return CoercedUnaryFunc<In, Out, Op, Out>(cx, argc, vp);
}

template <typename In, template <typename C> class Op, typename Out>
static bool BinaryFunc(JSContext* cx, unsigned argc, Value* vp) {
    return CoercedBinaryFunc<In, Out, Op, Out>(cx, argc, vp);
}

template <typename V>
static bool ExtractLane(JSContext* cx, unsigned argc, Value* vp) {
    typedef typename V::Elem Elem;

    CallArgs args = CallArgsFromVp(argc, vp);
    if (args.length() < 2 || !IsVectorObject<V>(args[0])) return ErrorBadArgs(cx);

    unsigned lane;
    if (!ArgumentToLaneIndex(cx, args[1], V::lanes, &lane)) return false;
...
template <typename V>
static bool AllTrue(JSContext* cx, unsigned argc, Value* vp) {
    typedef typename V::Elem Elem;

    CallArgs args = CallArgsFromVp(argc, vp);
    if (args.length() < 1 || !IsVectorObject<V>(args[0])) return ErrorBadArgs(cx);

    JS::AutoCheckCannotGC nogc(cx);
    Elem* vec = TypedObjectMemory<Elem*>(args[0], nogc);
    bool allTrue = true;
    for (unsigned i = 0; allTrue && i < V::lanes; i++) allTrue = vec[i];
...
template <typename V>
static bool AnyTrue(JSContext* cx, unsigned argc, Value* vp) {
    typedef typename V::Elem Elem;
    ...
    Elem* vec = TypedObjectMemory<Elem*>(args[0], nogc);
    bool anyTrue = false;
    for (unsigned i = 0; !anyTrue && i < V::lanes; i++) anyTrue = vec[i];
...
template <typename V>
static bool ReplaceLane(JSContext* cx, unsigned argc, Value* vp) {
    typedef typename V::Elem Elem;
    ...
    Elem value;
    if (!V::Cast(cx, args.get(2), &value)) return false;

    TypedObjectElemArray<Elem> vec(args[0]);
    Elem result[V::lanes];
    for (unsigned i = 0; i < V::lanes; i++) result[i] = i == lane ? value : vec[i];

    return StoreResult<V>(cx, args, result);
}

template <typename V>
static bool Swizzle(JSContext* cx, unsigned argc, Value* vp) {
    typedef typename V::Elem Elem;

    CallArgs args = CallArgsFromVp(argc, vp);
    if (args.length() != (V::lanes + 1) || !IsVectorObject<V>(args[0])) return ErrorBadArgs(cx);

    unsigned lanes[V::lanes];
    for (unsigned i = 0; i < V::lanes; i++) {
        if (!ArgumentToLaneIndex(cx, args[i + 1], V::lanes, &lanes[i])) return false;
    }

    TypedObjectElemArray<Elem> val(args[0]);
    Elem result[V::lanes];
    for (unsigned i = 0; i < V::lanes; i++) result[i] = val[lanes[i]];

    return StoreResult<V>(cx, args, result);
}

template <typename V>
static bool Shuffle(JSContext* cx, unsigned argc, Value* vp) {
    typedef typename V::Elem Elem;

    CallArgs args = CallArgsFromVp(argc, vp);
    if (args.length() != (V::lanes + 2) || !IsVectorObject<V>(args[0]) ||
        !IsVectorObject<V>(args[1]))
        return ErrorBadArgs(cx);

    unsigned lanes[V::lanes];
    for (unsigned i = 0; i < V::lanes; i++) {
        if (!ArgumentToLaneIndex(cx, args[i + 2], 2 * V::lanes, &lanes[i])) return false;
    }
...
static bool BinaryScalar(JSContext* cx, unsigned argc, Value* vp) {
    typedef typename V::Elem Elem;

    CallArgs args = CallArgsFromVp(argc, vp);
    if (args.length() != 2) return ErrorBadArgs(cx);

    if (!IsVectorObject<V>(args[0])) return ErrorBadArgs(cx);

    int32_t bits;
    if (!ToInt32(cx, args[1], &bits)) return false;

    TypedObjectElemArray<Elem> val(args[0]);
    Elem result[V::lanes];
    for (unsigned i = 0; i < V::lanes; i++) result[i] = Op<Elem>::apply(val[i], bits);

    return StoreResult<V>(cx, args, result);
}

template <typename In, template <typename C> class Op, typename Out>
static bool CompareFunc(JSContext* cx, unsigned argc, Value* vp) {
...
struct NeverThrow {
    static bool value(int32_t v) { return false; }
};
...
static bool FuncConvert(JSContext* cx, unsigned argc, Value* vp) {
    typedef typename V::Elem Elem;
...
template <typename V, typename Vret>
static bool FuncConvertBits(JSContext* cx, unsigned argc, Value* vp) {
    typedef typename V::Elem Elem;
    typedef typename Vret::Elem RetElem;
...
static bool FuncSplat(JSContext* cx, unsigned argc, Value* vp) {
    typedef typename Vret::Elem RetElem;

    CallArgs args = CallArgsFromVp(argc, vp);
    RetElem arg;
    if (!Vret::Cast(cx, args.get(0), &arg)) return false;

    RetElem result[Vret::lanes];
    for (unsigned i = 0; i < Vret::lanes; i++) result[i] = arg;
    return StoreResult<Vret>(cx, args, result);
}

template <typename V>
static bool Bool(JSContext* cx, unsigned argc, Value* vp) {
    typedef typename V::Elem Elem;

    CallArgs args = CallArgsFromVp(argc, vp);

    Elem result[V::lanes];
    for (unsigned i = 0; i < V::lanes; i++) result[i] = ToBoolean(args.get(i)) ? -1 : 0;
    return StoreResult<V>(cx, args, result);
}

template <typename V, typename MaskType>
static bool SelectBits(JSContext* cx, unsigned argc, Value* vp) {
    typedef typename V::Elem Elem;
    typedef typename MaskType::Elem MaskTypeElem;

    CallArgs args = CallArgsFromVp(argc, vp);
    if (args.length() != 3 || !IsVectorObject<MaskType>(args[0]) || !IsVectorObject<V>(args[1]) ||
        !IsVectorObject<V>(args[2])) {
        return ErrorBadArgs(cx);
    }
    ...
    for (unsigned i = 0; i < MaskType::lanes; i++) tr[i] = And<MaskTypeElem>::apply(val[i], tv[i]);
...
template <typename V, typename MaskType>
static bool Select(JSContext* cx, unsigned argc, Value* vp) {
    typedef typename V::Elem Elem;
    typedef typename MaskType::Elem MaskTypeElem;
    ...
    Elem result[V::lanes];
    for (unsigned i = 0; i < V::lanes; i++) result[i] = mask[i] ? tv[i] : fv[i];
...
static bool ArgumentToLaneIndex(JSContext* cx, JS::HandleValue v, unsigned limit, unsigned* lane) {
    uint64_t arg;
    if (!NonStandardToIndex(cx, v, &arg)) return false;
    if (arg >= limit) return ErrorBadIndex(cx);
...
static bool TypedArrayFromArgs(JSContext* cx, const CallArgs& args, uint32_t accessBytes,
                               MutableHandleObject typedArray, size_t* byteStart) {
    if (!args[0].isObject()) return ErrorBadArgs(cx);

    JSObject& argobj = args[0].toObject();
    if (!argobj.is<TypedArrayObject>()) return ErrorBadArgs(cx);

    typedArray.set(&argobj);

    uint64_t index;
    if (!NonStandardToIndex(cx, args[1], &index)) return false;
...
template <class V, unsigned NumElem>
static bool Store(JSContext* cx, unsigned argc, Value* vp) {
    typedef typename V::Elem Elem;

    CallArgs args = CallArgsFromVp(argc, vp);
    if (args.length() != 3) return ErrorBadArgs(cx);
    ...
    if (!IsVectorObject<V>(args[2])) return ErrorBadArgs(cx);
...
#define DEFINE_SIMD_FLOAT32X4_FUNCTION(Name, Func, Operands)                  \
    bool js::simd_float32x4_##Name(JSContext* cx, unsigned argc, Value* vp) { \
...
#define DEFINE_SIMD_FLOAT64X2_FUNCTION(Name, Func, Operands)                  \
    bool js::simd_float64x2_##Name(JSContext* cx, unsigned argc, Value* vp) { \
        return Func(cx, argc, vp);                                            \
    }
...
#define DEFINE_SIMD_INT8X16_FUNCTION(Name, Func, Operands)                  \
    bool js::simd_int8x16_##Name(JSContext* cx, unsigned argc, Value* vp) { \
        return Func(cx, argc, vp);                                          \
    }
...
#define DEFINE_SIMD_INT16X8_FUNCTION(Name, Func, Operands)                  \
    bool js::simd_int16x8_##Name(JSContext* cx, unsigned argc, Value* vp) { \
        return Func(cx, argc, vp);                                          \
    }
...
#define DEFINE_SIMD_INT32X4_FUNCTION(Name, Func, Operands)                  \
    bool js::simd_int32x4_##Name(JSContext* cx, unsigned argc, Value* vp) { \
        return Func(cx, argc, vp);                                          \
    }
...
#define DEFINE_SIMD_UINT8X16_FUNCTION(Name, Func, Operands)                  \
    bool js::simd_uint8x16_##Name(JSContext* cx, unsigned argc, Value* vp) { \
        return Func(cx, argc, vp);                                           \
    }
...
#define DEFINE_SIMD_UINT16X8_FUNCTION(Name, Func, Operands)                  \
    bool js::simd_uint16x8_##Name(JSContext* cx, unsigned argc, Value* vp) { \
        return Func(cx, argc, vp);                                           \
    }
...
#define DEFINE_SIMD_UINT32X4_FUNCTION(Name, Func, Operands)                  \
    bool js::simd_uint32x4_##Name(JSContext* cx, unsigned argc, Value* vp) { \
        return Func(cx, argc, vp);                                           \
    }
...
#define DEFINE_SIMD_BOOL8X16_FUNCTION(Name, Func, Operands)                  \
    bool js::simd_bool8x16_##Name(JSContext* cx, unsigned argc, Value* vp) { \
        return Func(cx, argc, vp);                                           \
    }
...
#define DEFINE_SIMD_BOOL16X8_FUNCTION(Name, Func, Operands)                  \
    bool js::simd_bool16x8_##Name(JSContext* cx, unsigned argc, Value* vp) { \
        return Func(cx, argc, vp);                                           \
    }
...
#define DEFINE_SIMD_BOOL32X4_FUNCTION(Name, Func, Operands)                  \
    bool js::simd_bool32x4_##Name(JSContext* cx, unsigned argc, Value* vp) { \
        return Func(cx, argc, vp);                                           \
    }
...
#define DEFINE_SIMD_BOOL64X2_FUNCTION(Name, Func, Operands)                  \
    bool js::simd_bool64x2_##Name(JSContext* cx, unsigned argc, Value* vp) { \
        return Func(cx, argc, vp);                                           \
    }
