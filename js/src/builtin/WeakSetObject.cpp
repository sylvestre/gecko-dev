...
MOZ_ALWAYS_INLINE bool WeakSet_add_impl(JSContext* cx, const CallArgs& args) {
    ...
    if (!WeakCollectionPutEntryInternal(cx, map, value, TrueHandleValue)) return false;
...
MOZ_ALWAYS_INLINE bool WeakSet_delete_impl(JSContext* cx, const CallArgs& args) {
    MOZ_ASSERT(IsWeakSet(args.thisv()));
...
const Class WeakSetObject::class_ = {
    "WeakSet",
    JSCLASS_HAS_PRIVATE | JSCLASS_HAS_CACHED_PROTO(JSProto_WeakSet) | JSCLASS_BACKGROUND_FINALIZE,
    &WeakCollectionObject::classOps_};

const JSPropertySpec WeakSetObject::properties[] = {JS_PS_END};

const JSFunctionSpec WeakSetObject::methods[] = {JS_FN("add", WeakSet_add, 1, 0),
...
JSObject* WeakSetObject::initClass(JSContext* cx, HandleObject obj) {
    ...
    Rooted<JSFunction*> ctor(
        cx, GlobalObject::createConstructor(cx, construct, ClassName(JSProto_WeakSet, cx), 0));
...
WeakSetObject* WeakSetObject::create(JSContext* cx, HandleObject proto /* = nullptr */) {
    return NewObjectWithClassProto<WeakSetObject>(cx, proto);
}

bool WeakSetObject::isBuiltinAdd(HandleValue add, JSContext* cx) {
...
bool WeakSetObject::construct(JSContext* cx, unsigned argc, Value* vp) {
    ...
    if (!args.get(0).isNullOrUndefined()) {
        ...
        if (!IsOptimizableInitForSet<GlobalObject::getOrCreateWeakSetPrototype, isBuiltinAdd>(
        ...
        if (optimized) {
            ...
            if (!CallSelfHostedFunction(cx, cx->names().WeakSetConstructorInit, thisv, args2,
                                        args2.rval()))
                return false;
        }
    }
