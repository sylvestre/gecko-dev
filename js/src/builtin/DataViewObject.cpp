...
static NewObjectKind DataViewNewObjectKind(JSContext* cx, uint32_t byteLength, JSObject* proto) {
    if (!proto && byteLength >= TypedArrayObject::SINGLETON_BYTE_LENGTH) return SingletonObject;
    jsbytecode* pc;
...
DataViewObject* DataViewObject::create(JSContext* cx, uint32_t byteOffset, uint32_t byteLength,
    ...
    if (isSharedMemory) dvobj.setIsSharedMemory();
    ...
    if (arrayBuffer->is<ArrayBufferObject>()) {
        if (!arrayBuffer->as<ArrayBufferObject>().addView(cx, &dvobj)) return nullptr;
    }
...
bool DataViewObject::getAndCheckConstructorArgs(JSContext* cx, HandleObject bufobj,
                                                const CallArgs& args, uint32_t* byteOffsetPtr,
    ...
    // Step 4.
    uint64_t offset;
    if (!ToIndex(cx, args.get(1), &offset)) return false;
    ...
    if (args.hasDefined(2)) {
        // Step 9.a.
        if (!ToIndex(cx, args.get(2), &viewByteLength)) return false;
...
bool DataViewObject::constructSameCompartment(JSContext* cx, HandleObject bufobj,
                                              const CallArgs& args) {
    MOZ_ASSERT(args.isConstructing());
    assertSameCompartment(cx, bufobj);

    uint32_t byteOffset, byteLength;
    if (!getAndCheckConstructorArgs(cx, bufobj, args, &byteOffset, &byteLength)) return false;
...
bool DataViewObject::constructWrapped(JSContext* cx, HandleObject bufobj, const CallArgs& args) {
    MOZ_ASSERT(args.isConstructing());
    ...
    uint32_t byteOffset, byteLength;
    if (!getAndCheckConstructorArgs(cx, unwrapped, args, &byteOffset, &byteLength)) return false;
    ...
    RootedObject dv(cx);
    {
        ...
        if (!cx->compartment()->wrap(cx, &wrappedProto)) return false;

        dv = DataViewObject::create(cx, byteOffset, byteLength, buffer, wrappedProto);
        if (!dv) return false;
    }

    if (!cx->compartment()->wrap(cx, &dv)) return false;
...
bool DataViewObject::construct(JSContext* cx, unsigned argc, Value* vp) {
    ...
    if (!ThrowIfNotConstructing(cx, args, "DataView")) return false;

    RootedObject bufobj(cx);
    if (!GetFirstArgumentAsObject(cx, args, "DataView constructor", &bufobj)) return false;

    if (bufobj->is<WrapperObject>()) return constructWrapped(cx, bufobj, args);
    return constructSameCompartment(cx, bufobj, args);
}

template <typename NativeType>
/* static */ SharedMem<uint8_t*> DataViewObject::getDataPointer(JSContext* cx,
...
static inline uint16_t swapBytes(uint16_t x) { return ((x & 0xff) << 8) | (x >> 8); }
...
static inline void Memcpy(uint8_t* dest, SharedMem<uint8_t*> src, size_t nbytes) {
    jit::AtomicOperations::memcpySafeWhenRacy(dest, src, nbytes);
}

static inline void Memcpy(SharedMem<uint8_t*> dest, uint8_t* src, size_t nbytes) {
    jit::AtomicOperations::memcpySafeWhenRacy(dest, src, nbytes);
}
...
struct DataViewIO {
    ...
    static void toBuffer(BufferPtrType unalignedBuffer, const DataType* src, bool wantSwap) {
        ...
        if (wantSwap) temp = swapBytes(temp);
        Memcpy(unalignedBuffer, (uint8_t*)&temp, sizeof(ReadWriteType));
    }
};

template <typename NativeType>
/* static */ bool DataViewObject::read(JSContext* cx, Handle<DataViewObject*> obj,
    ...
    // Step 4.
    uint64_t getIndex;
    if (!ToIndex(cx, args.get(0), &getIndex)) return false;
...
template <typename NativeType>
static inline bool WebIDLCast(JSContext* cx, HandleValue value, NativeType* out) {
    int32_t temp;
    if (!ToInt32(cx, value, &temp)) return false;
...
inline bool WebIDLCast<float>(JSContext* cx, HandleValue value, float* out) {
    double temp;
    if (!ToNumber(cx, value, &temp)) return false;
...
template <typename NativeType>
/* static */ bool DataViewObject::write(JSContext* cx, Handle<DataViewObject*> obj,
                                        const CallArgs& args) {
    // Steps 1-2. done by the caller
    // Step 3. unnecessary assert
    ...
    NativeType value;
    if (!WebIDLCast(cx, args.get(1), &value)) return false;

#ifdef JS_MORE_DETERMINISTIC
    // See the comment in ElementSpecific::doubleToNative.
    if (TypeIsFloatingPoint<NativeType>()) value = JS::CanonicalizeNaN(value);
#endif
...
bool DataViewObject::getInt8Impl(JSContext* cx, const CallArgs& args) {
    ...
    int8_t val;
    if (!read(cx, thisView, args, &val)) return false;
...
bool DataViewObject::setInt8Impl(JSContext* cx, const CallArgs& args) {
    ...
    if (!write<int8_t>(cx, thisView, args)) return false;
...
bool DataViewObject::setUint8Impl(JSContext* cx, const CallArgs& args) {
    ...
    if (!write<uint8_t>(cx, thisView, args)) return false;
    args.rval().setUndefined();
    return true;
}
...
bool DataViewObject::setInt16Impl(JSContext* cx, const CallArgs& args) {
    ...
    if (!write<int16_t>(cx, thisView, args)) return false;
    args.rval().setUndefined();
    return true;
}
...
bool DataViewObject::setUint16Impl(JSContext* cx, const CallArgs& args) {
    ...
    if (!write<uint16_t>(cx, thisView, args)) return false;
    args.rval().setUndefined();
    return true;
}
...
bool DataViewObject::setInt32Impl(JSContext* cx, const CallArgs& args) {
    ...
    if (!write<int32_t>(cx, thisView, args)) return false;
    args.rval().setUndefined();
    return true;
}
...
bool DataViewObject::setUint32Impl(JSContext* cx, const CallArgs& args) {
    ...
    if (!write<uint32_t>(cx, thisView, args)) return false;
    args.rval().setUndefined();
    return true;
}
...
bool DataViewObject::setFloat32Impl(JSContext* cx, const CallArgs& args) {
    ...
    if (!write<float>(cx, thisView, args)) return false;
    args.rval().setUndefined();
    return true;
}
...
bool DataViewObject::setFloat64Impl(JSContext* cx, const CallArgs& args) {
    ...
    if (!write<double>(cx, thisView, args)) return false;
    args.rval().setUndefined();
    return true;
}
...
void DataViewObject::notifyBufferDetached(void* newData) {
...
JS_NewDataView(JSContext* cx, HandleObject buffer, uint32_t byteOffset, int32_t byteLength) {
    ...
    if (!GetBuiltinConstructor(cx, key, &constructor)) return nullptr;
    ...
    RootedObject obj(cx);
    if (!Construct(cx, fun, cargs, fun, &obj)) return nullptr;
    return obj;
}
