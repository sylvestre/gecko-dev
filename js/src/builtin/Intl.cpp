...
inline UBool U_FAILURE(UErrorCode code) { MOZ_CRASH("U_FAILURE: Intl API disabled"); }

const char* uloc_getAvailable(int32_t n) { MOZ_CRASH("uloc_getAvailable: Intl API disabled"); }

int32_t uloc_countAvailable() { MOZ_CRASH("uloc_countAvailable: Intl API disabled"); }
...
void uenum_close(UEnumeration* en) { MOZ_CRASH("uenum_close: Intl API disabled"); }
...
int32_t ucol_countAvailable() { MOZ_CRASH("ucol_countAvailable: Intl API disabled"); }
...
UColAttributeValue ucol_getAttribute(const UCollator* coll, UColAttribute attr,
                                     UErrorCode* status) {
    MOZ_CRASH("ucol_getAttribute: Intl API disabled");
}

void ucol_setAttribute(UCollator* coll, UColAttribute attr, UColAttributeValue value,
                       UErrorCode* status) {
    MOZ_CRASH("ucol_setAttribute: Intl API disabled");
}

UCollationResult ucol_strcoll(const UCollator* coll, const UChar* source, int32_t sourceLength,
                              const UChar* target, int32_t targetLength) {
    MOZ_CRASH("ucol_strcoll: Intl API disabled");
}

void ucol_close(UCollator* coll) { MOZ_CRASH("ucol_close: Intl API disabled"); }

UEnumeration* ucol_getKeywordValuesForLocale(const char* key, const char* locale,
...
int32_t unum_countAvailable() { MOZ_CRASH("unum_countAvailable: Intl API disabled"); }

const char* unum_getAvailable(int32_t localeIndex) {
    MOZ_CRASH("unum_getAvailable: Intl API disabled");
}

UNumberFormat* unum_open(UNumberFormatStyle style, const UChar* pattern, int32_t patternLength,
...
int32_t unum_formatDoubleForFields(const UNumberFormat* fmt, double number, UChar* result,
...
void unum_close(UNumberFormat* fmt) { MOZ_CRASH("unum_close: Intl API disabled"); }

void unum_setTextAttribute(UNumberFormat* fmt, UNumberFormatTextAttribute tag,
                           const UChar* newValue, int32_t newValueLength, UErrorCode* status) {
...
void unumsys_close(UNumberingSystem* unumsys) { MOZ_CRASH("unumsys_close: Intl API disabled"); }

typedef void* UCalendar;

enum UCalendarType { UCAL_TRADITIONAL, UCAL_DEFAULT = UCAL_TRADITIONAL, UCAL_GREGORIAN };

enum UCalendarAttribute { UCAL_FIRST_DAY_OF_WEEK, UCAL_MINIMAL_DAYS_IN_FIRST_WEEK };
...
UCalendar* ucal_open(const UChar* zoneID, int32_t len, const char* locale, UCalendarType type,
                     UErrorCode* status) {
...
UEnumeration* ucal_getKeywordValuesForLocale(const char* key, const char* locale,
                                             UBool commonlyUsed, UErrorCode* status) {
    MOZ_CRASH("ucal_getKeywordValuesForLocale: Intl API disabled");
}

void ucal_close(UCalendar* cal) { MOZ_CRASH("ucal_close: Intl API disabled"); }

UCalendarWeekdayType ucal_getDayOfWeekType(const UCalendar* cal, UCalendarDaysOfWeek dayOfWeek,
                                           UErrorCode* status) {
...
int32_t ucal_getCanonicalTimeZoneID(const UChar* id, int32_t len, UChar* result,
                                    int32_t resultCapacity, UBool* isSystemID,
                                    UErrorCode* status) {
    MOZ_CRASH("ucal_getCanonicalTimeZoneID: Intl API disabled");
}

int32_t ucal_getDefaultTimeZone(UChar* result, int32_t resultCapacity, UErrorCode* status) {
...
int32_t udatpg_getBestPattern(UDateTimePatternGenerator* dtpg, const UChar* skeleton,
                              int32_t length, UChar* bestPattern, int32_t capacity,
                              UErrorCode* pErrorCode) {
    MOZ_CRASH("udatpg_getBestPattern: Intl API disabled");
}

static const UChar* udatpg_getAppendItemName(const UDateTimePatternGenerator* dtpg,
...
int32_t udat_countAvailable() { MOZ_CRASH("udat_countAvailable: Intl API disabled"); }

int32_t udat_toPattern(const UDateFormat* fmt, UBool localized, UChar* result,
...
UDateFormat* udat_open(UDateFormatStyle timeStyle, UDateFormatStyle dateStyle, const char* locale,
...
int32_t udat_format(const UDateFormat* format, UDate dateToFormat, UChar* result,
                    int32_t resultLength, UFieldPosition* position, UErrorCode* status) {
    MOZ_CRASH("udat_format: Intl API disabled");
}

int32_t udat_formatForFields(const UDateFormat* format, UDate dateToFormat, UChar* result,
                             int32_t resultLength, UFieldPositionIterator* fpositer,
                             UErrorCode* status) {
...
int32_t ufieldpositer_next(UFieldPositionIterator* fpositer, int32_t* beginIndex,
                           int32_t* endIndex) {
    MOZ_CRASH("ufieldpositer_next: Intl API disabled");
}

void udat_close(UDateFormat* format) { MOZ_CRASH("udat_close: Intl API disabled"); }
...
void uplrules_close(UPluralRules* uplrules) { MOZ_CRASH("uplrules_close: Intl API disabled"); }
...
int32_t uplrules_selectWithFormat(const UPluralRules* uplrules, double number,
                                  const UNumberFormat* fmt, UChar* keyword, int32_t capacity,
                                  UErrorCode* status) {
...
enum UDateRelativeDateTimeFormatterStyle { UDAT_STYLE_LONG, UDAT_STYLE_SHORT, UDAT_STYLE_NARROW };
...
URelativeDateTimeFormatter* ureldatefmt_open(const char* locale, UNumberFormat* nfToAdopt,
...
int32_t ureldatefmt_format(const URelativeDateTimeFormatter* reldatefmt, double offset,
...
static bool IntlInitialize(JSContext* cx, HandleObject obj, Handle<PropertyName*> initializer,
...
static bool LegacyIntlInitialize(JSContext* cx, HandleObject obj,
                                 Handle<PropertyName*> initializer, HandleValue thisValue,
                                 HandleValue locales, HandleValue options,
    ...
    if (!js::CallSelfHostedFunction(cx, initializer, NullHandleValue, args, result)) return false;
...
static bool intl_availableLocales(JSContext* cx, CountAvailable countAvailable,
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (!a) return false;
        if (!DefineDataProperty(cx, locales, a->asPropertyName(), TrueHandleValue)) return false;
    }
#endif
 ...
 */
static JSObject* GetInternals(JSContext* cx, HandleObject obj) {
...
static const char* icuLocale(const char* locale) {
    if (equal(locale, "und")) return "";  // ICU root locale
    return locale;
}
...
static JSString* Call(JSContext* cx, const ICUStringFunction& strFn) {
    ...
    if (status == U_BUFFER_OVERFLOW_ERROR) {
        MOZ_ASSERT(size >= 0);
        if (!chars.resize(size_t(size))) return nullptr;
        status = U_ZERO_ERROR;
...
const Class CollatorObject::class_ = {
    js_Object_str,
    JSCLASS_HAS_RESERVED_SLOTS(CollatorObject::SLOT_COUNT) | JSCLASS_FOREGROUND_FINALIZE,
...
static const JSFunctionSpec collator_static_methods[] = {
    JS_SELF_HOSTED_FN("supportedLocalesOf", "Intl_Collator_supportedLocalesOf", 1, 0), JS_FS_END};
...
void CollatorObject::finalize(FreeOp* fop, JSObject* obj) {
    MOZ_ASSERT(fop->onActiveCooperatingThread());

    const Value& slot = obj->as<CollatorObject>().getReservedSlot(CollatorObject::UCOLLATOR_SLOT);
    if (UCollator* coll = static_cast<UCollator*>(slot.toPrivate())) ucol_close(coll);
}

static JSObject* CreateCollatorPrototype(JSContext* cx, HandleObject Intl,
                                         Handle<GlobalObject*> global) {
    RootedFunction ctor(cx,
                        GlobalObject::createConstructor(cx, &Collator, cx->names().Collator, 0));
    ...
    if (!DefineDataProperty(cx, Intl, cx->names().Collator, ctorValue, 0)) return nullptr;

    return proto;
}

bool js::intl_Collator_availableLocales(JSContext* cx, unsigned argc, Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);
    MOZ_ASSERT(args.length() == 0);

    RootedValue result(cx);
    if (!intl_availableLocales(cx, ucol_countAvailable, ucol_getAvailable, &result)) return false;
...
bool js::intl_availableCollations(JSContext* cx, unsigned argc, Value* vp) {
    ...
    if (!DefineDataElement(cx, collations, index++, NullHandleValue)) return false;
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        // array."
        if (equal(collation, "standard") || equal(collation, "search")) continue;
 ...
 */
static UCollator* NewUCollator(JSContext* cx, Handle<CollatorObject*> collator) {
    RootedValue value(cx);

    RootedObject internals(cx, GetInternals(cx, collator));
    if (!internals) return nullptr;

    if (!GetProperty(cx, internals, internals, cx->names().locale, &value)) return nullptr;
    ...
    if (!GetProperty(cx, internals, internals, cx->names().usage, &value)) return nullptr;
    ...
    if (!GetProperty(cx, internals, internals, cx->names().sensitivity, &value)) return nullptr;
    ...
    if (!GetProperty(cx, internals, internals, cx->names().numeric, &value)) return nullptr;
    if (!value.isUndefined() && value.toBoolean()) uNumeric = UCOL_ON;

    if (!GetProperty(cx, internals, internals, cx->names().caseFirst, &value)) return nullptr;
...
static bool intl_CompareStrings(JSContext* cx, UCollator* coll, HandleString str1,
                                HandleString str2, MutableHandleValue result) {
    MOZ_ASSERT(str1);
    MOZ_ASSERT(str2);
    ...
    UCollationResult uresult = ucol_strcoll(coll, chars1.begin().get(), chars1.length(),
...
bool js::SharedIntlData::LocaleHasher::match(Locale key, const Lookup& lookup) {
    if (key->length() != lookup.length) return false;

    if (key->hasLatin1Chars()) {
        const Latin1Char* keyChars = key->latin1Chars(lookup.nogc);
        if (lookup.isLatin1) return EqualChars(keyChars, lookup.latin1Chars, lookup.length);
        return EqualChars(keyChars, lookup.twoByteChars, lookup.length);
    }

    const char16_t* keyChars = key->twoByteChars(lookup.nogc);
    if (lookup.isLatin1) return EqualChars(lookup.latin1Chars, keyChars, lookup.length);
    return EqualChars(keyChars, lookup.twoByteChars, lookup.length);
}

bool js::SharedIntlData::ensureUpperCaseFirstLocales(JSContext* cx) {
...
bool js::SharedIntlData::isUpperCaseFirst(JSContext* cx, HandleString locale, bool* isUpperFirst) {
...
const Class NumberFormatObject::class_ = {
    js_Object_str,
    JSCLASS_HAS_RESERVED_SLOTS(NumberFormatObject::SLOT_COUNT) | JSCLASS_FOREGROUND_FINALIZE,
...
void NumberFormatObject::finalize(FreeOp* fop, JSObject* obj) {
    MOZ_ASSERT(fop->onActiveCooperatingThread());

    const Value& slot =
        obj->as<NumberFormatObject>().getReservedSlot(NumberFormatObject::UNUMBER_FORMAT_SLOT);
    if (UNumberFormat* nf = static_cast<UNumberFormat*>(slot.toPrivate())) unum_close(nf);
}

static JSObject* CreateNumberFormatPrototype(JSContext* cx, HandleObject Intl,
                                             Handle<GlobalObject*> global,
    ...
    // 11.3.2
    if (!JS_DefineFunctions(cx, ctor, numberFormat_static_methods)) return nullptr;

    // 11.4.4
    if (!JS_DefineFunctions(cx, proto, numberFormat_methods)) return nullptr;

    // 11.4.2 and 11.4.3
    if (!JS_DefineProperties(cx, proto, numberFormat_properties)) return nullptr;

    // 8.1
    RootedValue ctorValue(cx, ObjectValue(*ctor));
    if (!DefineDataProperty(cx, Intl, cx->names().NumberFormat, ctorValue, 0)) return nullptr;

    constructor.set(ctor);
    return proto;
}

bool js::intl_NumberFormat_availableLocales(JSContext* cx, unsigned argc, Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);
    MOZ_ASSERT(args.length() == 0);

    RootedValue result(cx);
    if (!intl_availableLocales(cx, unum_countAvailable, unum_getAvailable, &result)) return false;
    args.rval().set(result);
    return true;
}
 ...
 * percent types.
 */
static UNumberFormat* NewUNumberFormatForPluralRules(JSContext* cx,
    ...
    bool hasP;
    if (!HasProperty(cx, internals, cx->names().minimumSignificantDigits, &hasP)) return nullptr;
 ...
 */
static UNumberFormat* NewUNumberFormat(JSContext* cx, Handle<NumberFormatObject*> numberFormat) {
    RootedValue value(cx);
    ...
    if (!GetProperty(cx, internals, internals, cx->names().style, &value)) return nullptr;

    {
        ...
        if (StringEqualsAscii(style, "currency")) {
            ...
            if (!stableChars.initTwoByte(cx, currency)) return nullptr;
            ...
            if (!currencyDisplay) return nullptr;
    ...
    if (!GetProperty(cx, internals, internals, cx->names().useGrouping, &value)) return nullptr;
...
static JSString* PartitionNumberPattern(JSContext* cx, UNumberFormat* nf, double* x,
                                        UFieldPositionIterator* fpositer) {
    // PartitionNumberPattern doesn't consider -0.0 to be negative.
    if (IsNegativeZero(*x)) *x = 0.0;
...
static FieldType GetFieldTypeForNumberField(UNumberFormatFields fieldName, double d) {
    ...
    switch (fieldName) {
        case UNUM_INTEGER_FIELD:
            if (IsNaN(d)) return &JSAtomState::nan;
            if (!IsFinite(d)) return &JSAtomState::infinity;
...
static bool intl_FormatNumberToParts(JSContext* cx, UNumberFormat* nf, double x,
    ...
    struct Field {
        ...
        Field(uint32_t begin, uint32_t end, FieldType type) : begin(begin), end(end), type(type) {}
    };
    ...
    while ((fieldInt = ufieldpositer_next(fpositer, &beginIndexInt, &endIndexInt)) >= 0) {
        ...
        MOZ_ASSERT(beginIndexInt < endIndexInt, "erm, aren't fields always non-empty?");
    ...
    if (!fields.resizeUninitialized(fieldsLen * 2)) return false;
    ...
    if (!fields.resize(fieldsLen)) return false;
    ...
    class PartGenerator {
        ...
        void popEnclosingFieldsEndingAt(uint32_t end) {
            MOZ_ASSERT_IF(enclosingFields.length() > 0, fields[enclosingFields.back()].end >= end);
        ...
        bool nextPartInternal(Part* part) {
            ...
            do {
                ...
                if (current->end > next->begin) {
                    if (!enclosingFields.append(index - 1)) return false;
                }
       ...
       public:
        ...
        bool nextPart(bool* hasPart, Part* part) {
            ...
            if (!nextPartInternal(part)) return false;
    ...
    do {
        bool hasPart;
        Part part;
        if (!gen.nextPart(&hasPart, &part)) return false;
        ...
        if (!DefineDataProperty(cx, singlePart, cx->names().type, propVal)) return false;
        ...
        if (!DefineDataProperty(cx, singlePart, cx->names().value, propVal)) return false;

        propVal.setObject(*singlePart);
        if (!DefineDataElement(cx, partsArray, partIndex, propVal)) return false;
...
const ClassOps DateTimeFormatObject::classOps_ = {nullptr, /* addProperty */
                                                  nullptr, /* delProperty */
                                                  nullptr, /* enumerate */
                                                  nullptr, /* newEnumerate */
                                                  nullptr, /* resolve */
...
const Class DateTimeFormatObject::class_ = {
    js_Object_str,
    JSCLASS_HAS_RESERVED_SLOTS(DateTimeFormatObject::SLOT_COUNT) | JSCLASS_FOREGROUND_FINALIZE,
...
void DateTimeFormatObject::finalize(FreeOp* fop, JSObject* obj) {
    MOZ_ASSERT(fop->onActiveCooperatingThread());

    const Value& slot =
        obj->as<DateTimeFormatObject>().getReservedSlot(DateTimeFormatObject::UDATE_FORMAT_SLOT);
    if (UDateFormat* df = static_cast<UDateFormat*>(slot.toPrivate())) udat_close(df);
}

static JSObject* CreateDateTimeFormatPrototype(JSContext* cx, HandleObject Intl,
                                               Handle<GlobalObject*> global,
                                               MutableHandleObject constructor,
                                               DateTimeFormatOptions dtfOptions) {
    RootedFunction ctor(cx);
    ctor =
        dtfOptions == DateTimeFormatOptions::EnableMozExtensions
    ...
    // 12.3.2
    if (!JS_DefineFunctions(cx, ctor, dateTimeFormat_static_methods)) return nullptr;

    // 12.4.4 and 12.4.5
    if (!JS_DefineFunctions(cx, proto, dateTimeFormat_methods)) return nullptr;

    // 12.4.2 and 12.4.3
    if (!JS_DefineProperties(cx, proto, dateTimeFormat_properties)) return nullptr;

    // 8.1
    RootedValue ctorValue(cx, ObjectValue(*ctor));
    if (!DefineDataProperty(cx, Intl, cx->names().DateTimeFormat, ctorValue, 0)) return nullptr;

    constructor.set(ctor);
    return proto;
}

bool js::AddMozDateTimeFormatConstructor(JSContext* cx, JS::Handle<JSObject*> intl) {
    Handle<GlobalObject*> global = cx->global();

    RootedObject mozDateTimeFormat(cx);
    JSObject* mozDateTimeFormatProto = CreateDateTimeFormatPrototype(
        cx, intl, global, &mozDateTimeFormat, DateTimeFormatOptions::EnableMozExtensions);
    return mozDateTimeFormatProto != nullptr;
}

bool js::intl_DateTimeFormat_availableLocales(JSContext* cx, unsigned argc, Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);
    MOZ_ASSERT(args.length() == 0);

    RootedValue result(cx);
    if (!intl_availableLocales(cx, udat_countAvailable, udat_getAvailable, &result)) return false;
    args.rval().set(result);
    return true;
}
...
static bool EqualCharsIgnoreCaseASCII(const Char1* s1, const Char2* s2, size_t len) {
...
template <typename Char>
static js::HashNumber HashStringIgnoreCaseASCII(const Char* s, size_t length) {
    uint32_t hash = 0;
    for (size_t i = 0; i < length; i++) hash = mozilla::AddToHash(hash, ToUpperASCII(s[i]));
    return hash;
}
...
bool js::SharedIntlData::TimeZoneHasher::match(TimeZoneName key, const Lookup& lookup) {
    if (key->length() != lookup.length) return false;
...
static bool IsLegacyICUTimeZone(const char* timeZone) {
    for (const auto& legacyTimeZone : js::timezone::legacyICUTimeZones) {
        if (equal(timeZone, legacyTimeZone)) return true;
    }
    return false;
}

bool js::SharedIntlData::ensureTimeZones(JSContext* cx) {
    ...
    if (ianaZonesTreatedAsLinksByICU.initialized()) ianaZonesTreatedAsLinksByICU.finish();
...
bool js::SharedIntlData::validateTimeZoneName(JSContext* cx, HandleString timeZone,
...
bool js::SharedIntlData::tryCanonicalizeTimeZoneConsistentWithIANA(JSContext* cx,
...
void js::SharedIntlData::destroyInstance() {
...
void js::SharedIntlData::trace(JSTracer* trc) {
...
size_t js::SharedIntlData::sizeOfExcludingThis(mozilla::MallocSizeOf mallocSizeOf) const {
...
bool js::intl_IsValidTimeZoneName(JSContext* cx, unsigned argc, Value* vp) {
    ...
    if (!sharedIntlData.validateTimeZoneName(cx, timeZone, &validatedTimeZone)) return false;
...
bool js::intl_canonicalizeTimeZone(JSContext* cx, unsigned argc, Value* vp) {
    ...
    JSString* str = Call(cx, [&tzchars](UChar* chars, uint32_t size, UErrorCode* status) {
        return ucal_getCanonicalTimeZoneID(tzchars.begin().get(), tzchars.length(), chars, size,
                                           nullptr, status);
    });
    if (!str) return false;
...
bool js::intl_patternForSkeleton(JSContext* cx, unsigned argc, Value* vp) {
    ...
    JSString* str = Call(cx, [gen, &skelChars](UChar* chars, uint32_t size, UErrorCode* status) {
        return udatpg_getBestPattern(gen, skelChars.begin().get(), skelChars.length(), chars, size,
                                     status);
    });
    if (!str) return false;
...
bool js::intl_patternForStyle(JSContext* cx, unsigned argc, Value* vp) {
    ...
    UErrorCode status = U_ZERO_ERROR;
    UDateFormat* df =
        udat_open(timeStyle, dateStyle, icuLocale(locale.ptr()), timeZoneChars.begin().get(),
 ...
 */
static UDateFormat* NewUDateFormat(JSContext* cx, Handle<DateTimeFormatObject*> dateTimeFormat) {
    RootedValue value(cx);
    ...
    if (!GetProperty(cx, internals, internals, cx->names().timeZone, &value)) return nullptr;
    ...
    if (!GetProperty(cx, internals, internals, cx->names().pattern, &value)) return nullptr;
    ...
    UErrorCode status = U_ZERO_ERROR;
    UDateFormat* df = udat_open(UDAT_PATTERN, UDAT_PATTERN, icuLocale(locale.ptr()),
...
static bool intl_FormatDateTime(JSContext* cx, UDateFormat* df, double x,
                                MutableHandleValue result) {
    if (!IsFinite(x)) {
...
static bool intl_FormatToPartsDateTime(JSContext* cx, UDateFormat* df, double x,
                                       MutableHandleValue result) {
    if (!IsFinite(x)) {
        JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr, JSMSG_DATE_NOT_FINITE);
        return false;
    ...
    auto AppendPart = [&](FieldType type, size_t beginIndex, size_t endIndex) {
        ...
        if (!DefineDataProperty(cx, singlePart, cx->names().type, partType)) return false;
        ...
        if (!DefineDataProperty(cx, singlePart, cx->names().value, val)) return false;
    ...
    while ((fieldInt = ufieldpositer_next(fpositer, &beginIndexInt, &endIndexInt)) >= 0) {
        MOZ_ASSERT(beginIndexInt >= 0);
        MOZ_ASSERT(endIndexInt >= 0);
        MOZ_ASSERT(beginIndexInt <= endIndexInt, "field iterator returning invalid range");
        ...
        if (FieldType type = GetFieldTypeForFormatField(static_cast<UDateFormatField>(fieldInt))) {
            if (lastEndIndex < beginIndex) {
                if (!AppendPart(&JSAtomState::literal, lastEndIndex, beginIndex)) return false;
            }
...
const Class PluralRulesObject::class_ = {
    js_Object_str,
    JSCLASS_HAS_RESERVED_SLOTS(PluralRulesObject::SLOT_COUNT) | JSCLASS_FOREGROUND_FINALIZE,
...
void PluralRulesObject::finalize(FreeOp* fop, JSObject* obj) {
    MOZ_ASSERT(fop->onActiveCooperatingThread());

    const Value& slot =
        obj->as<PluralRulesObject>().getReservedSlot(PluralRulesObject::UPLURAL_RULES_SLOT);
    if (UPluralRules* pr = static_cast<UPluralRules*>(slot.toPrivate())) uplrules_close(pr);
}

static JSObject* CreatePluralRulesPrototype(JSContext* cx, HandleObject Intl,
                                            Handle<GlobalObject*> global) {
    RootedFunction ctor(cx);
    ...
    RootedValue ctorValue(cx, ObjectValue(*ctor));
    if (!DefineDataProperty(cx, Intl, cx->names().PluralRules, ctorValue, 0)) return nullptr;

    return proto;
}

bool js::intl_PluralRules_availableLocales(JSContext* cx, unsigned argc, Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);
    MOZ_ASSERT(args.length() == 0);
    ...
    if (!intl_availableLocales(cx, uloc_countAvailable, uloc_getAvailable, &result)) return false;
    args.rval().set(result);
    return true;
}
...
bool js::intl_SelectPluralRule(JSContext* cx, unsigned argc, Value* vp) {
    ...
    if (!GetProperty(cx, internals, internals, cx->names().locale, &value)) return false;
    JSAutoByteString locale(cx, value.toString());
    if (!locale) return false;

    if (!GetProperty(cx, internals, internals, cx->names().type, &value)) return false;
...
const ClassOps RelativeTimeFormatObject::classOps_ = {nullptr, /* addProperty */
                                                      nullptr, /* delProperty */
                                                      nullptr, /* enumerate */
                                                      nullptr, /* newEnumerate */
                                                      nullptr, /* resolve */
...
const Class RelativeTimeFormatObject::class_ = {
    js_Object_str,
    JSCLASS_HAS_RESERVED_SLOTS(RelativeTimeFormatObject::SLOT_COUNT) | JSCLASS_FOREGROUND_FINALIZE,
    &RelativeTimeFormatObject::classOps_};

#if JS_HAS_TOSOURCE
static bool relativeTimeFormat_toSource(JSContext* cx, unsigned argc, Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);
...
static bool RelativeTimeFormat(JSContext* cx, unsigned argc, Value* vp) {
    ...
    // Step 1.
    if (!ThrowIfNotConstructing(cx, args, "Intl.RelativeTimeFormat")) return false;
    ...
    if (!relativeTimeFormat) return false;
    ...
    // Step 3.
    if (!IntlInitialize(cx, relativeTimeFormat, cx->names().InitializeRelativeTimeFormat, locales,
                        options))
        return false;
...
void RelativeTimeFormatObject::finalize(FreeOp* fop, JSObject* obj) {
    MOZ_ASSERT(fop->onActiveCooperatingThread());

    const Value& slot = obj->as<RelativeTimeFormatObject>().getReservedSlot(
...
static JSObject* CreateRelativeTimeFormatPrototype(JSContext* cx, HandleObject Intl,
                                                   Handle<GlobalObject*> global) {
    RootedFunction ctor(cx);
    ...
    if (!JS_DefineFunctions(cx, ctor, relativeTimeFormat_static_methods)) return nullptr;

    if (!JS_DefineFunctions(cx, proto, relativeTimeFormat_methods)) return nullptr;
...
/* static */ bool js::GlobalObject::addRelativeTimeFormatConstructor(JSContext* cx,
                                                                     HandleObject intl) {
    Handle<GlobalObject*> global = cx->global();
    ...
    if (!relativeTimeFormatProto) return false;

    global->setReservedSlot(RELATIVE_TIME_FORMAT_PROTO, ObjectValue(*relativeTimeFormatProto));
    return true;
}

bool js::AddRelativeTimeFormatConstructor(JSContext* cx, JS::Handle<JSObject*> intl) {
    return GlobalObject::addRelativeTimeFormatConstructor(cx, intl);
}

bool js::intl_RelativeTimeFormat_availableLocales(JSContext* cx, unsigned argc, Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);
    MOZ_ASSERT(args.length() == 0);
...
bool js::intl_FormatRelativeTime(JSContext* cx, unsigned argc, Value* vp) {
    ...
    if (!GetProperty(cx, internals, internals, cx->names().style, &value)) return false;
    ...
    JSString* str =
        Call(cx, [rtf, t, relDateTimeUnit](UChar* chars, int32_t size, UErrorCode* status) {
...
bool js::intl_GetCalendarInfo(JSContext* cx, unsigned argc, Value* vp) {
    ...
    if (!DefineDataProperty(cx, info, cx->names().firstDayOfWeek, v)) return false;
    ...
    if (!DefineDataProperty(cx, info, cx->names().weekendStart, weekendStart)) return false;

    if (!DefineDataProperty(cx, info, cx->names().weekendEnd, weekendEnd)) return false;

    args.rval().setObject(*info);
    return true;
}

static void ReportBadKey(JSContext* cx, const Range<const JS::Latin1Char>& range) {
...
static void ReportBadKey(JSContext* cx, const Range<const char16_t>& range) {
    JS_ReportErrorNumberUC(cx, GetErrorMessage, nullptr, JSMSG_INVALID_KEY, range.begin().get());
}

template <typename ConstChar>
static bool MatchPart(RangedPtr<ConstChar> iter, const RangedPtr<ConstChar> end, const char* part,
                      size_t partlen) {
...
inline bool MatchPart(RangedPtr<ConstChar>* iter, const RangedPtr<ConstChar> end,
                      const char (&part)[N]) {
...
static JSString* ComputeSingleDisplayName(JSContext* cx, UDateFormat* fmt,
                                          UDateTimePatternGenerator* dtpg, DisplayNameStyle style,
...
bool js::intl_ComputeDisplayNames(JSContext* cx, unsigned argc, Value* vp) {
    ...
    UDateFormat* fmt = udat_open(UDAT_DEFAULT, UDAT_DEFAULT, icuLocale(locale.ptr()), nullptr, 0,
                                 nullptr, 0, &status);
    if (U_FAILURE(status)) {
        JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr, JSMSG_INTERNAL_INTL_ERROR);
        return false;
    ...
    Vector<char16_t, INITIAL_CHAR_BUFFER_SIZE> chars(cx);
    if (!chars.resize(INITIAL_CHAR_BUFFER_SIZE)) return false;
    ...
    for (uint32_t i = 0; i < keys->length(); i++) {
        ...
        JSString* displayName = stablePatternChars.isLatin1()
...
const Class js::IntlClass = {js_Object_str, JSCLASS_HAS_CACHED_PROTO(JSProto_Intl)};
...
#endif
    JS_SELF_HOSTED_FN("getCanonicalLocales", "Intl_getCanonicalLocales", 1, 0), JS_FS_END};

/**
 * Initializes the Intl Object and its standard built-in properties.
 * Spec: ECMAScript Internationalization API Specification, 8.0, 8.1
 */
/* static */ bool GlobalObject::initIntlObject(JSContext* cx, Handle<GlobalObject*> global) {
