...
static HashNumber HashValue(const Value& v, const mozilla::HashCodeScrambler& hcs) {
...
static const ClassExtension MapIteratorObjectClassExtension = {nullptr, /* weakmapKeyDelegateOp */
                                                               MapIteratorObject::objectMoved};

const Class MapIteratorObject::class_ = {
    "Map Iterator",
    JSCLASS_HAS_RESERVED_SLOTS(MapIteratorObject::SlotCount) | JSCLASS_FOREGROUND_FINALIZE |
        JSCLASS_SKIP_NURSERY_FINALIZE,
    &MapIteratorObjectClassOps, JS_NULL_CLASS_SPEC, &MapIteratorObjectClassExtension};
...
/* static */ bool GlobalObject::initMapIteratorProto(JSContext* cx, Handle<GlobalObject*> global) {
...
MapIteratorObject* MapIteratorObject::create(JSContext* cx, HandleObject obj, ValueMap* data,
...
/* static */ JSObject* MapIteratorObject::createResultPair(JSContext* cx) {
    RootedArrayObject resultPairObj(cx,
                                    NewDenseFullyAllocatedArray(cx, 2, nullptr, TenuredObject));
...
const Class MapObject::class_ = {"Map",
                                 JSCLASS_HAS_PRIVATE |
...
const Class MapObject::protoClass_ = {js_Object_str, JSCLASS_HAS_CACHED_PROTO(JSProto_Map),
                                      JS_NULL_CLASS_OPS, &MapObject::classSpec_};

const JSPropertySpec MapObject::properties[] = {
    JS_PSG("size", size, 0), JS_STRING_SYM_PS(toStringTag, "Map", JSPROP_READONLY), JS_PS_END};

const JSFunctionSpec MapObject::methods[] = {
    JS_FN("get", get, 1, 0), JS_FN("has", has, 1, 0), JS_FN("set", set, 2, 0),
    JS_FN("delete", delete_, 1, 0), JS_FN("keys", keys, 0, 0), JS_FN("values", values, 0, 0),
    JS_FN("clear", clear, 0, 0), JS_SELF_HOSTED_FN("forEach", "MapForEach", 2, 0),
...
template <typename ObjectT>
inline static MOZ_MUST_USE bool WriteBarrierPostImpl(JSRuntime* rt, ObjectT* obj,
...
inline static MOZ_MUST_USE bool WriteBarrierPost(JSRuntime* rt, MapObject* map, const Value& key) {
    return WriteBarrierPostImpl(rt, map, key);
}

inline static MOZ_MUST_USE bool WriteBarrierPost(JSRuntime* rt, SetObject* set, const Value& key) {
    return WriteBarrierPostImpl(rt, set, key);
}

bool MapObject::getKeysAndValuesInterleaved(JSContext* cx, HandleObject obj,
                                            JS::MutableHandle<GCVector<JS::Value>> entries) {
    ValueMap* map = obj->as<MapObject>().getData();
    if (!map) return false;

    for (ValueMap::Range r = map->all(); !r.empty(); r.popFront()) {
        if (!entries.append(r.front().key.get()) || !entries.append(r.front().value)) {
            return false;
        }
    }
...
bool MapObject::set(JSContext* cx, HandleObject obj, HandleValue k, HandleValue v) {
    ValueMap* map = obj->as<MapObject>().getData();
    if (!map) return false;
...
MapObject* MapObject::create(JSContext* cx, HandleObject proto /* = nullptr */) {
    auto map = cx->make_unique<ValueMap>(cx->zone(), cx->compartment()->randomHashCodeScrambler());
    if (!map || !map->init()) {
        ReportOutOfMemory(cx);
        return nullptr;
    }
...
bool MapObject::construct(JSContext* cx, unsigned argc, Value* vp) {
    ...
    if (!args.get(0).isNullOrUndefined()) {
        ...
        if (!CallSelfHostedFunction(cx, cx->names().MapConstructorInit, thisv, args2,
                                    args2.rval()))
            return false;
    }
...
bool MapObject::get(JSContext* cx, HandleObject obj, HandleValue key, MutableHandleValue rval) {
    ValueMap& map = extract(obj);
...
bool MapObject::has(JSContext* cx, HandleObject obj, HandleValue key, bool* rval) {
    ValueMap& map = extract(obj);
    Rooted<HashableValue> k(cx);
...
bool MapObject::delete_(JSContext* cx, HandleObject obj, HandleValue key, bool* rval) {
    ValueMap& map = extract(obj);
    Rooted<HashableValue> k(cx);
...
bool MapObject::iterator_impl(JSContext* cx, const CallArgs& args, IteratorKind kind) {
    RootedObject obj(cx, &args.thisv().toObject());
...
static const ClassExtension SetIteratorObjectClassExtension = {nullptr, /* weakmapKeyDelegateOp */
                                                               SetIteratorObject::objectMoved};

const Class SetIteratorObject::class_ = {
    "Set Iterator",
    JSCLASS_HAS_RESERVED_SLOTS(SetIteratorObject::SlotCount) | JSCLASS_FOREGROUND_FINALIZE |
        JSCLASS_SKIP_NURSERY_FINALIZE,
    &SetIteratorObjectClassOps, JS_NULL_CLASS_SPEC, &SetIteratorObjectClassExtension};
...
/* static */ bool GlobalObject::initSetIteratorProto(JSContext* cx, Handle<GlobalObject*> global) {
    Rooted<JSObject*> base(cx, GlobalObject::getOrCreateIteratorPrototype(cx, global));
    if (!base) return false;
    RootedPlainObject proto(cx, NewObjectWithGivenProto<PlainObject>(cx, base));
    if (!proto) return false;
...
SetIteratorObject* SetIteratorObject::create(JSContext* cx, HandleObject obj, ValueSet* data,
...
bool SetIteratorObject::next(Handle<SetIteratorObject*> setIterator, HandleArrayObject resultObj,
                             JSContext* cx) {
...
const Class SetObject::protoClass_ = {js_Object_str, JSCLASS_HAS_CACHED_PROTO(JSProto_Set),
                                      JS_NULL_CLASS_OPS, &SetObject::classSpec_};

const JSPropertySpec SetObject::properties[] = {
    JS_PSG("size", size, 0), JS_STRING_SYM_PS(toStringTag, "Set", JSPROP_READONLY), JS_PS_END};

const JSFunctionSpec SetObject::methods[] = {
    JS_FN("has", has, 1, 0), JS_FN("add", add, 1, 0), JS_FN("delete", delete_, 1, 0),
    JS_FN("entries", entries, 0, 0), JS_FN("clear", clear, 0, 0),
    JS_SELF_HOSTED_FN("forEach", "SetForEach", 2, 0),
    // SetValues only exists to preseve the equal identity of
    // values, keys and @@iterator.
    JS_SELF_HOSTED_FN("values", "SetValues", 0, 0), JS_SELF_HOSTED_FN("keys", "SetValues", 0, 0),
...
bool SetObject::add(JSContext* cx, HandleObject obj, HandleValue k) {
    ...
    if (!WriteBarrierPost(cx->runtime(), &obj->as<SetObject>(), key.value()) || !set->put(key)) {
        ReportOutOfMemory(cx);
        return false;
    }
    return true;
...
SetObject* SetObject::create(JSContext* cx, HandleObject proto /* = nullptr */) {
    auto set = cx->make_unique<ValueSet>(cx->zone(), cx->compartment()->randomHashCodeScrambler());
    if (!set || !set->init()) {
        ReportOutOfMemory(cx);
        return nullptr;
    }
...
void SetObject::trace(JSTracer* trc, JSObject* obj) {
    ...
    if (ValueSet* set = setobj->getData()) {
        for (ValueSet::Range r = set->all(); !r.empty(); r.popFront()) TraceKey(r, r.front(), trc);
    }
}
...
bool SetObject::isBuiltinAdd(HandleValue add, JSContext* cx) {
...
bool SetObject::construct(JSContext* cx, unsigned argc, Value* vp) {
    ...
    if (!args.get(0).isNullOrUndefined()) {
        ...
        if (!IsOptimizableInitForSet<GlobalObject::getOrCreateSetPrototype, isBuiltinAdd>(
                cx, obj, iterable, &optimized))
            return false;
        ...
        if (optimized) {
            ...
            for (uint32_t index = 0; index < array->getDenseInitializedLength(); ++index) {
                ...
                if (!WriteBarrierPost(cx->runtime(), obj, keyVal) || !set->put(key)) {
                    ReportOutOfMemory(cx);
                    return false;
                }
            }
            ...
            RootedValue thisv(cx, ObjectValue(*obj));
            if (!CallSelfHostedFunction(cx, cx->names().SetConstructorInit, thisv, args2,
                                        args2.rval()))
                return false;
        }
    }
...
bool SetObject::has(JSContext* cx, HandleObject obj, HandleValue key, bool* rval) {
    MOZ_ASSERT(SetObject::is(obj));
...
bool SetObject::delete_(JSContext* cx, HandleObject obj, HandleValue key, bool* rval) {
    MOZ_ASSERT(SetObject::is(obj));
...
bool SetObject::iterator_impl(JSContext* cx, const CallArgs& args, IteratorKind kind) {
...
static bool forEach(const char* funcName, JSContext* cx, HandleObject obj, HandleValue callbackFn,
                    HandleValue thisArg) {
    CHECK_REQUEST(cx);
...
RetT CallObjFunc(RetT (*ObjFunc)(JSContext*, HandleObject), JSContext* cx, HandleObject obj) {
    CHECK_REQUEST(cx);
...
bool CallObjFunc(bool (*ObjFunc)(JSContext* cx, HandleObject obj, HandleValue key, bool* rval),
...
JS::NewMapObject(JSContext* cx) { return MapObject::create(cx); }
...
JS_PUBLIC_API(bool)
JS::MapClear(JSContext* cx, HandleObject obj) { return CallObjFunc(&MapObject::clear, cx, obj); }
...
JS_PUBLIC_API(JSObject*)
JS::NewSetObject(JSContext* cx) { return SetObject::create(cx); }
...
JS_PUBLIC_API(bool)
JS::SetClear(JSContext* cx, HandleObject obj) { return CallObjFunc(&SetObject::clear, cx, obj); }
