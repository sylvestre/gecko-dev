...
enum BYOBRequestSlots { BYOBRequestSlot_Controller, BYOBRequestSlot_View, BYOBRequestSlotCount };
...
static inline void AddControllerFlags(NativeObject* controller, uint32_t flags) {
    MOZ_ASSERT(IsReadableStreamController(controller));
    controller->setFixedSlot(ControllerSlot_Flags,
                             Int32Value(ControllerFlags(controller) | flags));
}

static inline void RemoveControllerFlags(NativeObject* controller, uint32_t flags) {
    MOZ_ASSERT(IsReadableStreamController(controller));
    controller->setFixedSlot(ControllerSlot_Flags,
                             Int32Value(ControllerFlags(controller) & ~flags));
}
...
inline static MOZ_MUST_USE ReadableStream* StreamFromController(const NativeObject* controller) {
    MOZ_ASSERT(IsReadableStreamController(controller));
    return &controller->getFixedSlot(ControllerSlot_Stream).toObject().as<ReadableStream>();
}

inline static MOZ_MUST_USE NativeObject* ControllerFromStream(const ReadableStream* stream) {
...
JS::ReadableStreamMode ReadableStream::mode() const {
    ...
    if (controller->is<ReadableStreamDefaultController>()) return JS::ReadableStreamMode::Default;
    return controller->as<ReadableByteStreamController>().hasExternalSource()
               ? JS::ReadableStreamMode::ExternalSource
               : JS::ReadableStreamMode::Byte;
}

inline static MOZ_MUST_USE ReadableStream* StreamFromReader(const NativeObject* reader) {
    MOZ_ASSERT(ReaderHasStream(reader));
    return &reader->getFixedSlot(ReaderSlot_Stream).toObject().as<ReadableStream>();
}

inline static MOZ_MUST_USE NativeObject* ReaderFromStream(const NativeObject* stream) {
...
inline static MOZ_MUST_USE JSFunction* NewHandler(JSContext* cx, Native handler,
    ...
    RootedFunction handlerFun(
        cx, NewNativeFunction(cx, handler, 0, funName, gc::AllocKind::FUNCTION_EXTENDED,
                              GenericObject));
...
inline static MOZ_MUST_USE bool ResetQueue(JSContext* cx, HandleNativeObject container);

inline static MOZ_MUST_USE bool InvokeOrNoop(JSContext* cx, HandleValue O, HandlePropertyName P,
                                             HandleValue arg, MutableHandleValue rval);

static MOZ_MUST_USE JSObject* PromiseInvokeOrNoop(JSContext* cx, HandleValue O,
                                                  HandlePropertyName P, HandleValue arg);
...
static bool ReportArgTypeError(JSContext* cx, const char* funName, const char* expectedType,
                               HandleValue arg) {
    UniqueChars bytes = DecompileValueGenerator(cx, JSDVG_SEARCH_STACK, arg, nullptr);
    if (!bytes) return false;

    return JS_ReportErrorFlagsAndNumberLatin1(cx, JSREPORT_ERROR, GetErrorMessage, nullptr,
                                              JSMSG_NOT_EXPECTED_TYPE, funName, expectedType,
                                              bytes.get());
}

static MOZ_MUST_USE bool RejectWithPendingError(JSContext* cx, Handle<PromiseObject*> promise) {
    // Not much we can do about uncatchable exceptions, just bail.
    RootedValue exn(cx);
...
static MOZ_MUST_USE bool ReturnPromiseRejectedWithPendingError(JSContext* cx,
                                                               const CallArgs& args) {
...
static MOZ_MUST_USE bool RejectNonGenericMethod(JSContext* cx, const CallArgs& args,
                                                const char* className, const char* methodName) {
    ReportValueError3(cx, JSMSG_INCOMPATIBLE_PROTO, JSDVG_SEARCH_STACK, args.thisv(), nullptr,
                      className, methodName);

    return ReturnPromiseRejectedWithPendingError(cx, args);
}

inline static MOZ_MUST_USE NativeObject* SetNewList(JSContext* cx, HandleNativeObject container,
                                                    uint32_t slot) {
...
const Class ByteStreamChunk::class_ = {"ByteStreamChunk", JSCLASS_HAS_RESERVED_SLOTS(SlotCount)};
   ...
   public:
    ...
    static PullIntoDescriptor* create(JSContext* cx, HandleArrayBufferObject buffer,
        ...
        Rooted<PullIntoDescriptor*> descriptor(cx,
                                               NewObjectWithClassProto<PullIntoDescriptor>(cx));
...
const Class PullIntoDescriptor::class_ = {"PullIntoDescriptor",
...
const Class QueueEntry::class_ = {"QueueEntry", JSCLASS_HAS_RESERVED_SLOTS(SlotCount)};
...
class TeeState : public NativeObject {
   ...
   public:
    ...
    PromiseObject* promise() { return &getFixedSlot(Slot_Promise).toObject().as<PromiseObject>(); }
    ReadableStream* stream() { return &getFixedSlot(Slot_Stream).toObject().as<ReadableStream>(); }
    ...
    ReadableStreamDefaultController* branch1() {
        ReadableStreamDefaultController* controller =
            &getFixedSlot(Slot_Branch1).toObject().as<ReadableStreamDefaultController>();
    ...
    ReadableStreamDefaultController* branch2() {
        ReadableStreamDefaultController* controller =
            &getFixedSlot(Slot_Branch2).toObject().as<ReadableStreamDefaultController>();
        MOZ_ASSERT(ControllerFlags(controller) & ControllerFlag_TeeBranch);
...
const Class TeeState::class_ = {"TeeState", JSCLASS_HAS_RESERVED_SLOTS(SlotCount)};
...
#define CLASS_SPEC(cls, nCtorArgs, nSlots, specFlags, classFlags, classOps)              \
    ...
    const Class cls::class_ = {#cls,                                                     \
                               JSCLASS_HAS_RESERVED_SLOTS(nSlots) |                      \
                                   JSCLASS_HAS_CACHED_PROTO(JSProto_##cls) | classFlags, \
                               classOps, &cls::classSpec_};                              \
                                                                                         \
    const Class cls::protoClass_ = {"object", JSCLASS_HAS_CACHED_PROTO(JSProto_##cls),   \
                                    JS_NULL_CLASS_OPS, &cls::classSpec_};

// Streams spec, 3.2.3., steps 1-4.
ReadableStream* ReadableStream::createStream(JSContext* cx, HandleObject proto /* = nullptr */) {
...
static MOZ_MUST_USE ReadableStreamDefaultController* CreateReadableStreamDefaultController(
    JSContext* cx, Handle<ReadableStream*> stream, HandleValue underlyingSource, HandleValue size,
    HandleValue highWaterMarkVal);

// Streams spec, 3.2.3., steps 1-4, 8.
ReadableStream* ReadableStream::createDefaultStream(JSContext* cx, HandleValue underlyingSource,
    ...
    //                         highWaterMark »).
    RootedObject controller(cx, CreateReadableStreamDefaultController(cx, stream, underlyingSource,
                                                                      size, highWaterMark));
...
static MOZ_MUST_USE ReadableByteStreamController* CreateReadableByteStreamController(
    JSContext* cx, Handle<ReadableStream*> stream, HandleValue underlyingByteSource,
    HandleValue highWaterMarkVal);

// Streams spec, 3.2.3., steps 1-4, 7.
ReadableStream* ReadableStream::createByteStream(JSContext* cx, HandleValue underlyingSource,
    ...
    RootedObject controller(
        cx, CreateReadableByteStreamController(cx, stream, underlyingSource, highWaterMark));
    if (!controller) return nullptr;
...
static MOZ_MUST_USE ReadableByteStreamController* CreateReadableByteStreamController(
    JSContext* cx, Handle<ReadableStream*> stream, void* underlyingSource);

ReadableStream* ReadableStream::createExternalSourceStream(JSContext* cx, void* underlyingSource,
                                                           uint8_t flags,
                                                           HandleObject proto /* = nullptr */) {
    Rooted<ReadableStream*> stream(cx, createStream(cx, proto));
    if (!stream) return nullptr;
...
bool ReadableStream::constructor(JSContext* cx, unsigned argc, Value* vp) {
    ...
    if (!options.isUndefined()) {
        ...
        if (!GetProperty(cx, options, cx->names().highWaterMark, &highWaterMark)) return false;
    }
    ...
    if (!GetProperty(cx, underlyingSource, cx->names().type, &typeVal)) return false;
    ...
    if (!CompareStrings(cx, type, cx->names().bytes, &notByteStream)) return false;

    // Step 7.a & 8.a (reordered): If highWaterMark is undefined, let
    //                             highWaterMark be 1 (or 0 for byte streams).
    if (highWaterMark.isUndefined()) highWaterMark = Int32Value(notByteStream ? 1 : 0);
...
static MOZ_MUST_USE bool ReadableStream_locked_impl(JSContext* cx, const CallArgs& args) {
...
static MOZ_MUST_USE bool ReadableStream_cancel(JSContext* cx, unsigned argc, Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);
    ...
    if (stream->locked()) {
        JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr, JSMSG_READABLESTREAM_NOT_LOCKED,
                                  "cancel");
        return ReturnPromiseRejectedWithPendingError(cx, args);
    }
...
static MOZ_MUST_USE ReadableStreamDefaultReader* CreateReadableStreamDefaultReader(
...
static MOZ_MUST_USE bool ReadableStream_getReader_impl(JSContext* cx, const CallArgs& args) {
    Rooted<ReadableStream*> stream(cx, &args.thisv().toObject().as<ReadableStream>());
...
static MOZ_MUST_USE bool ReadableStream_pipeThrough(JSContext* cx, unsigned argc, Value* vp) {
    JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr,
...
static MOZ_MUST_USE bool ReadableStream_pipeTo(JSContext* cx, unsigned argc, Value* vp) {
    JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr,
                              JSMSG_READABLESTREAM_METHOD_NOT_IMPLEMENTED, "pipeTo");
    return false;
}

static MOZ_MUST_USE bool ReadableStreamTee(JSContext* cx, Handle<ReadableStream*> stream,
                                           bool cloneForBranch2,
                                           MutableHandle<ReadableStream*> branch1,
                                           MutableHandle<ReadableStream*> branch2);

// Streams spec, 3.2.4.6. tee()
static MOZ_MUST_USE bool ReadableStream_tee_impl(JSContext* cx, const CallArgs& args) {
    Rooted<ReadableStream*> stream(cx, &args.thisv().toObject().as<ReadableStream>());
...
static MOZ_MUST_USE bool ReadableStreamDefaultControllerEnqueue(
    JSContext* cx, Handle<ReadableStreamDefaultController*> controller, HandleValue chunk);
...
static bool TeeReaderReadHandler(JSContext* cx, unsigned argc, Value* vp) {
    ...
    if (!GetPropertyPure(cx, result, NameToId(cx->names().value), value.address())) return false;

    // Step c: Let done be ? Get(result, "done").
    RootedValue doneVal(cx);
    if (!GetPropertyPure(cx, result, NameToId(cx->names().done), doneVal.address())) return false;
    ...
    if (!teeState->canceled1()) {
        ...
        if (!ReadableStreamDefaultControllerEnqueue(cx, controller, value1)) return false;
    }
    ...
    if (!teeState->canceled2()) {
        ...
        if (!ReadableStreamDefaultControllerEnqueue(cx, controller, value2)) return false;
    }
...
static MOZ_MUST_USE JSObject* ReadableStreamTee_Pull(JSContext* cx, Handle<TeeState*> teeState,
...
static MOZ_MUST_USE JSObject* ReadableStreamTee_Cancel(
    JSContext* cx, Handle<TeeState*> teeState, Handle<ReadableStreamDefaultController*> branch,
    HandleValue reason) {
...
static MOZ_MUST_USE bool ReadableStreamControllerError(JSContext* cx,
...
static MOZ_MUST_USE bool ReadableStreamTee(JSContext* cx, Handle<ReadableStream*> stream,
    ...
    branch1Stream.set(
        ReadableStream::createDefaultStream(cx, underlyingSource, UndefinedHandleValue, hwmValue));
    ...
    if (!JS::AddPromiseReactions(cx, closedPromise, nullptr, onRejected)) return false;
...
static MOZ_MUST_USE PromiseObject* ReadableStreamAddReadRequest(JSContext* cx,
...
static MOZ_MUST_USE JSObject* ReadableStreamControllerCancelSteps(JSContext* cx,
                                                                  HandleNativeObject controller,
                                                                  HandleValue reason);
...
MOZ_MUST_USE bool ReadableStreamCloseInternal(JSContext* cx, Handle<ReadableStream*> stream);

// Streams spec, 3.4.3. ReadableStreamCancel ( stream, reason )
/* static */ MOZ_MUST_USE JSObject* ReadableStream::cancel(JSContext* cx,
                                                           Handle<ReadableStream*> stream,
                                                           HandleValue reason) {
    ...
    //         with undefined.
    if (stream->closed()) return PromiseObject::unforgeableResolve(cx, UndefinedHandleValue);
...
MOZ_MUST_USE bool ReadableStreamErrorInternal(JSContext* cx, Handle<ReadableStream*> stream,
                                              HandleValue e) {
    // Step 1: Assert: ! IsReadableStream(stream) is true (implicit).
    ...
    if (stream->mode() == JS::ReadableStreamMode::ExternalSource &&
        ...
        NativeObject* controller = ControllerFromStream(stream);
        void* source = controller->getFixedSlot(ControllerSlot_UnderlyingSource).toPrivate();
        cx->runtime()->readableStreamErroredCallback(cx, stream, source, stream->embeddingFlags(),
                                                     e);
    }
...
static MOZ_MUST_USE bool ReadableStreamFulfillReadOrReadIntoRequest(JSContext* cx,
                                                                    Handle<ReadableStream*> stream,
...
static MOZ_MUST_USE bool ReadableStreamHasDefaultReader(ReadableStream* stream) {
    // Step 1: Let reader be stream.[[reader]].
    // Step 2: If reader is undefined, return false.
...
static MOZ_MUST_USE bool ReadableStreamReaderGenericInitialize(JSContext* cx,
...
static MOZ_MUST_USE ReadableStreamDefaultReader* CreateReadableStreamDefaultReader(
    ...
    if (!ReadableStreamReaderGenericInitialize(cx, reader, stream)) return nullptr;
...
// Stream spec, 3.5.3. new ReadableStreamDefaultReader ( stream )
bool ReadableStreamDefaultReader::constructor(JSContext* cx, unsigned argc, Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);

    if (!ThrowIfNotConstructing(cx, args, "ReadableStreamDefaultReader")) return false;
...
static MOZ_MUST_USE bool ReadableStreamDefaultReader_closed(JSContext* cx, unsigned argc,
                                                            Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);
...
static MOZ_MUST_USE JSObject* ReadableStreamReaderGenericCancel(JSContext* cx,
                                                                HandleNativeObject reader,
                                                                HandleValue reason);

// Streams spec, 3.5.4.2. cancel ( reason )
static MOZ_MUST_USE bool ReadableStreamDefaultReader_cancel(JSContext* cx, unsigned argc,
                                                            Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);
...
static MOZ_MUST_USE bool ReadableStreamDefaultReader_read(JSContext* cx, unsigned argc,
                                                          Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);
...
static MOZ_MUST_USE bool ReadableStreamReaderGenericRelease(JSContext* cx,
                                                            HandleNativeObject reader);

// Streams spec, 3.5.4.4. releaseLock ( )
static MOZ_MUST_USE bool ReadableStreamDefaultReader_releaseLock_impl(JSContext* cx,
                                                                      const CallArgs& args) {
    Rooted<ReadableStreamDefaultReader*> reader(cx);
    reader = &args.thisv().toObject().as<ReadableStreamDefaultReader>();
...
static bool ReadableStreamDefaultReader_releaseLock(JSContext* cx, unsigned argc, Value* vp) {
...
static const JSFunctionSpec ReadableStreamDefaultReader_methods[] = {
    ...
    JS_FN("releaseLock", ReadableStreamDefaultReader_releaseLock, 0, 0), JS_FS_END};
...
// Streams spec, 3.6.3 new ReadableStreamBYOBReader ( stream )
bool ReadableStreamBYOBReader::constructor(JSContext* cx, unsigned argc, Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);

    if (!ThrowIfNotConstructing(cx, args, "ReadableStreamBYOBReader")) return false;
...
static MOZ_MUST_USE bool ReadableStreamBYOBReader_closed(JSContext* cx, unsigned argc, Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);
...
static MOZ_MUST_USE bool ReadableStreamBYOBReader_cancel(JSContext* cx, unsigned argc, Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);
...
static MOZ_MUST_USE bool ReadableStreamBYOBReader_read(JSContext* cx, unsigned argc, Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);
...
static MOZ_MUST_USE bool ReadableStreamBYOBReader_releaseLock_impl(JSContext* cx,
                                                                   const CallArgs& args) {
    Rooted<ReadableStreamBYOBReader*> reader(cx);
    reader = &args.thisv().toObject().as<ReadableStreamBYOBReader>();
...
static bool ReadableStreamBYOBReader_releaseLock(JSContext* cx, unsigned argc, Value* vp) {
...
static const JSFunctionSpec ReadableStreamBYOBReader_methods[] = {
    ...
    JS_FN("releaseLock", ReadableStreamBYOBReader_releaseLock, 0, 0), JS_FS_END};
...
/* static */ MOZ_MUST_USE JSObject* ReadableStreamBYOBReader::read(
    JSContext* cx, Handle<ReadableStreamBYOBReader*> reader, Handle<ArrayBufferViewObject*> view) {
...
static MOZ_MUST_USE JSObject* ReadableStreamControllerPullSteps(JSContext* cx,
                                                                HandleNativeObject controller);
...
static bool ControllerStartHandler(JSContext* cx, unsigned argc, Value* vp) {
    ...
    MOZ_ASSERT(
        !(ControllerFlags(controller) & (ControllerFlag_Pulling | ControllerFlag_PullAgain)));
...
static MOZ_MUST_USE bool ReadableStreamDefaultControllerErrorIfNeeded(
    JSContext* cx, Handle<ReadableStreamDefaultController*> controller, HandleValue e);
...
static bool ControllerStartFailedHandler(JSContext* cx, unsigned argc, Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);
...
static MOZ_MUST_USE bool ValidateAndNormalizeHighWaterMark(JSContext* cx,
                                                           HandleValue highWaterMarkVal,
                                                           double* highWaterMark);

static MOZ_MUST_USE bool ValidateAndNormalizeQueuingStrategy(JSContext* cx, HandleValue size,
                                                             HandleValue highWaterMarkVal,
                                                             double* highWaterMark);
...
//                                      highWaterMark )
bool ReadableStreamDefaultController::constructor(JSContext* cx, unsigned argc, Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);

    if (!ThrowIfNotConstructing(cx, args, "ReadableStreamDefaultController")) return false;

    // Step 1: If ! IsReadableStream(stream) is false, throw a TypeError exception.
    HandleValue streamVal = args.get(0);
    if (!Is<ReadableStream>(streamVal)) {
        ReportArgTypeError(cx, "ReadableStreamDefaultController", "ReadableStream", args.get(0));
        return false;
    }
...
static MOZ_MUST_USE bool ReadableStreamController_desiredSize_impl(JSContext* cx,
                                                                   const CallArgs& args) {
...
static bool ReadableStreamDefaultController_desiredSize(JSContext* cx, unsigned argc, Value* vp) {
...
static MOZ_MUST_USE bool VerifyControllerStateForClosing(JSContext* cx,
                                                         HandleNativeObject controller) {
...
static MOZ_MUST_USE bool ReadableStreamDefaultController_close_impl(JSContext* cx,
                                                                    const CallArgs& args) {
    Rooted<ReadableStreamDefaultController*> controller(cx);
...
static bool ReadableStreamDefaultController_close(JSContext* cx, unsigned argc, Value* vp) {
    // Step 1: If ! IsReadableStreamDefaultController(this) is false, throw a
    //         TypeError exception.
...
static MOZ_MUST_USE bool ReadableStreamDefaultController_enqueue_impl(JSContext* cx,
                                                                      const CallArgs& args) {
    Rooted<ReadableStreamDefaultController*> controller(cx);
    controller = &args.thisv().toObject().as<ReadableStreamDefaultController>();
    ...
    if (!ReadableStreamDefaultControllerEnqueue(cx, controller, args.get(0))) return false;
    args.rval().setUndefined();
    return true;
}

static bool ReadableStreamDefaultController_enqueue(JSContext* cx, unsigned argc, Value* vp) {
    // Step 1: If ! IsReadableStreamDefaultController(this) is false, throw a
    //         TypeError exception.
...
static MOZ_MUST_USE bool ReadableStreamDefaultController_error_impl(JSContext* cx,
                                                                    const CallArgs& args) {
    Rooted<ReadableStreamDefaultController*> controller(cx);
    controller = &args.thisv().toObject().as<ReadableStreamDefaultController>();
...
static bool ReadableStreamDefaultController_error(JSContext* cx, unsigned argc, Value* vp) {
    // Step 1: If ! IsReadableStreamDefaultController(this) is false, throw a
    //         TypeError exception.
...
static const JSPropertySpec ReadableStreamDefaultController_properties[] = {
    JS_PSG("desiredSize", ReadableStreamDefaultController_desiredSize, 0), JS_PS_END};
...
inline static MOZ_MUST_USE bool DequeueValue(JSContext* cx, HandleNativeObject container,
                                             MutableHandleValue chunk);

// Streams spec, 3.8.5.2. ReadableStreamDefaultController [[PullSteps]]()
static JSObject* ReadableStreamDefaultControllerPullSteps(JSContext* cx,
                                                          HandleNativeObject controller) {
...
static bool ControllerPullFailedHandler(JSContext* cx, unsigned argc, Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);
    RootedNativeObject controller(cx, TargetFromHandler<NativeObject>(args.callee()));
...
static bool ReadableStreamControllerShouldCallPull(NativeObject* controller) {
    ...
    if (stream->locked() && ReadableStreamGetNumReadRequests(stream) > 0) return true;
...
static MOZ_MUST_USE bool ReadableStreamDefaultControllerClose(
    ...
    if (queue->getDenseInitializedLength() == 0) return ReadableStreamCloseInternal(cx, stream);

    return true;
}

static MOZ_MUST_USE bool EnqueueValueWithSize(JSContext* cx, HandleNativeObject container,
...
static MOZ_MUST_USE bool ReadableStreamDefaultControllerEnqueue(
    ...
    if (stream->locked() && ReadableStreamGetNumReadRequests(stream) > 0) {
        if (!ReadableStreamFulfillReadOrReadIntoRequest(cx, stream, chunk, false)) return false;
    } else {
        // Step 5: Otherwise,
        ...
        if (success) success = EnqueueValueWithSize(cx, controller, chunk, chunkSize);
        ...
        if (!success) {
            ...
            //                                                        chunkSize.[[Value]]).
            if (!ReadableStreamDefaultControllerErrorIfNeeded(cx, controller, exn)) return false;
...
static MOZ_MUST_USE bool ReadableStreamDefaultControllerErrorIfNeeded(
    ...
    Rooted<ReadableStream*> stream(cx, StreamFromController(controller));
    if (stream->readable()) return ReadableStreamControllerError(cx, controller, e);
    return true;
}
...
static MOZ_MUST_USE ReadableByteStreamController* CreateReadableByteStreamController(
    ...
    if (!ReadableByteStreamControllerClearPendingPullIntos(cx, controller)) return nullptr;
    ...
    double highWaterMark;
    if (!ValidateAndNormalizeHighWaterMark(cx, highWaterMarkVal, &highWaterMark)) return nullptr;
    controller->setFixedSlot(ControllerSlot_StrategyHWM, NumberValue(highWaterMark));
    ...
    if (!SetNewList(cx, controller, ByteControllerSlot_PendingPullIntos)) return nullptr;
...
//                                    highWaterMark )
bool ReadableByteStreamController::constructor(JSContext* cx, unsigned argc, Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);

    if (!ThrowIfNotConstructing(cx, args, "ReadableByteStreamController")) return false;
    ...
    RootedObject controller(
        cx, CreateReadableByteStreamController(cx, stream, args.get(1), args.get(2)));
    if (!controller) return false;
...
static MOZ_MUST_USE ReadableStreamBYOBRequest* CreateReadableStreamBYOBRequest(
    JSContext* cx, Handle<ReadableByteStreamController*> controller, HandleObject view);

// Streams spec, 3.10.4.1. get byobRequest
static MOZ_MUST_USE bool ReadableByteStreamController_byobRequest_impl(JSContext* cx,
                                                                       const CallArgs& args) {
    Rooted<ReadableByteStreamController*> controller(cx);
    ...
    if (byobRequest.isUndefined() && pendingPullIntos->getDenseInitializedLength() != 0) {
        ...
        RootedObject view(
            cx, JS_NewUint8ArrayWithBuffer(cx, buffer, firstDescriptor->byteOffset() + bytesFilled,
...
static bool ReadableByteStreamController_byobRequest(JSContext* cx, unsigned argc, Value* vp) {
...
static bool ReadableByteStreamController_desiredSize(JSContext* cx, unsigned argc, Value* vp) {
...
static MOZ_MUST_USE bool ReadableByteStreamController_close_impl(JSContext* cx,
                                                                 const CallArgs& args) {
    Rooted<ReadableByteStreamController*> controller(cx);
    controller = &args.thisv().toObject().as<ReadableByteStreamController>();
...
static bool ReadableByteStreamController_close(JSContext* cx, unsigned argc, Value* vp) {
    // Step 1: If ! IsReadableByteStreamController(this) is false, throw a
    //         TypeError exception.
    CallArgs args = CallArgsFromVp(argc, vp);
    return CallNonGenericMethod<Is<ReadableByteStreamController>,
...
static MOZ_MUST_USE bool ReadableByteStreamControllerEnqueue(
    JSContext* cx, Handle<ReadableByteStreamController*> controller, HandleObject chunk);

// Streams spec, 3.10.4.4. enqueue ( chunk )
static MOZ_MUST_USE bool ReadableByteStreamController_enqueue_impl(JSContext* cx,
                                                                   const CallArgs& args) {
    Rooted<ReadableByteStreamController*> controller(cx);
    controller = &args.thisv().toObject().as<ReadableByteStreamController>();
...
static bool ReadableByteStreamController_enqueue(JSContext* cx, unsigned argc, Value* vp) {
    // Step 1: If ! IsReadableByteStreamController(this) is false, throw a
    //         TypeError exception.
    CallArgs args = CallArgsFromVp(argc, vp);
    return CallNonGenericMethod<Is<ReadableByteStreamController>,
...
static MOZ_MUST_USE bool ReadableByteStreamController_error_impl(JSContext* cx,
                                                                 const CallArgs& args) {
    Rooted<ReadableByteStreamController*> controller(cx);
    controller = &args.thisv().toObject().as<ReadableByteStreamController>();
    HandleValue e = args.get(0);
...
static bool ReadableByteStreamController_error(JSContext* cx, unsigned argc, Value* vp) {
    // Step 1: If ! IsReadableByteStreamController(this) is false, throw a
    //         TypeError exception.
    CallArgs args = CallArgsFromVp(argc, vp);
    return CallNonGenericMethod<Is<ReadableByteStreamController>,
...
static const JSPropertySpec ReadableByteStreamController_properties[] = {
    ...
    JS_PSG("desiredSize", ReadableByteStreamController_desiredSize, 0), JS_PS_END};
...
static JSObject* ReadableByteStreamControllerPullSteps(JSContext* cx,
                                                       HandleNativeObject controller) {
    // Step 1: Let stream be this.[[controlledReadableStream]].
    Rooted<ReadableStream*> stream(cx, StreamFromController(controller));
    ...
    if (queueTotalSize > 0) {
        ...
        if (stream->mode() == JS::ReadableStreamMode::ExternalSource) {
            ...
            size_t bytesWritten;
            {
                ...
                cb(cx, stream, underlyingSource, stream->embeddingFlags(), buffer, queueTotalSize,
                   &bytesWritten);
            }
        ...
        // (reordered)
        if (!ReadableByteStreamControllerHandleQueueDrain(cx, controller)) return nullptr;
    ...
    if (!val.isUndefined()) {
        ...
        pullIntoDescriptor = PullIntoDescriptor::create(cx, buffer, 0, autoAllocateChunkSize, 0, 1,
...
bool ReadableStreamBYOBRequest::constructor(JSContext* cx, unsigned argc, Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);
    HandleValue controllerVal = args.get(0);
    HandleValue viewVal = args.get(1);

    if (!ThrowIfNotConstructing(cx, args, "ReadableStreamBYOBRequest")) return false;
    ...
    if (!Is<ReadableByteStreamController>(controllerVal)) {
        ReportArgTypeError(cx, "ReadableStreamBYOBRequest", "ReadableByteStreamController",
                           args.get(0));
        return false;
    }
    ...
    if (!viewVal.isObject() || !JS_IsArrayBufferViewObject(&viewVal.toObject())) {
        ReportArgTypeError(cx, "ReadableStreamBYOBRequest", "ArrayBuffer view", args.get(1));
        return false;
    }
...
static MOZ_MUST_USE bool ReadableStreamBYOBRequest_view_impl(JSContext* cx, const CallArgs& args) {
...
static bool ReadableStreamBYOBRequest_view(JSContext* cx, unsigned argc, Value* vp) {
...
static MOZ_MUST_USE bool ReadableByteStreamControllerRespond(
    JSContext* cx, Handle<ReadableByteStreamController*> controller, HandleValue bytesWrittenVal);

// Streams spec, 3.11.4.2. respond ( bytesWritten )
static MOZ_MUST_USE bool ReadableStreamBYOBRequest_respond_impl(JSContext* cx,
                                                                const CallArgs& args) {
    Rooted<ReadableStreamBYOBRequest*> request(cx);
    ...
    if (!ReadableByteStreamControllerRespond(cx, controller, bytesWritten)) return false;

    args.rval().setUndefined();
    return true;
}

static bool ReadableStreamBYOBRequest_respond(JSContext* cx, unsigned argc, Value* vp) {
    // Step 1: If ! IsReadableStreamBYOBRequest(this) is false, throw a TypeError
    //         exception.
    CallArgs args = CallArgsFromVp(argc, vp);
    return CallNonGenericMethod<Is<ReadableStreamBYOBRequest>,
...
static MOZ_MUST_USE bool ReadableStreamBYOBRequest_respondWithNewView_impl(JSContext* cx,
                                                                           const CallArgs& args) {
    Rooted<ReadableStreamBYOBRequest*> request(cx);
    request = &args.thisv().toObject().as<ReadableStreamBYOBRequest>();
    HandleValue viewVal = args.get(0);
    ...
    if (!ReadableByteStreamControllerRespondWithNewView(cx, controller, view)) return false;

    args.rval().setUndefined();
    return true;
}

static bool ReadableStreamBYOBRequest_respondWithNewView(JSContext* cx, unsigned argc, Value* vp) {
    // Step 1: If ! IsReadableStreamBYOBRequest(this) is false, throw a TypeError
    //         exception.
    CallArgs args = CallArgsFromVp(argc, vp);
    return CallNonGenericMethod<Is<ReadableStreamBYOBRequest>,
...
static const JSFunctionSpec ReadableStreamBYOBRequest_methods[] = {
    ...
    JS_FN("respondWithNewView", ReadableStreamBYOBRequest_respondWithNewView, 1, 0), JS_FS_END};
...
static void ReadableByteStreamControllerInvalidateBYOBRequest(NativeObject* controller);
...
static MOZ_MUST_USE JSObject* ReadableByteStreamControllerConvertPullIntoDescriptor(
...
static MOZ_MUST_USE JSObject* ReadableByteStreamControllerConvertPullIntoDescriptor(
    ...
    if (!ctor) {
        if (!GetBuiltinConstructor(cx, JSProto_Uint8Array, &ctor)) return nullptr;
    }
...
static MOZ_MUST_USE bool ReadableByteStreamControllerEnqueueChunkToQueue(
    JSContext* cx, Handle<ReadableByteStreamController*> controller, HandleObject buffer,
    uint32_t byteOffset, uint32_t byteLength);

static MOZ_MUST_USE ArrayBufferObject* TransferArrayBuffer(JSContext* cx, HandleObject buffer);

static MOZ_MUST_USE bool ReadableByteStreamControllerProcessPullIntoDescriptorsUsingQueue(
    JSContext* cx, Handle<ReadableByteStreamController*> controller);
    ...
    if (ReadableStreamHasDefaultReader(stream)) {
        ...
        // Step i: Assert: controller.[[queue]] is empty.
            ...
            RootedObject transferredView(
                cx, JS_NewUint8ArrayWithBuffer(cx, transferredBuffer, byteOffset, byteLength));
...
static MOZ_MUST_USE bool ReadableByteStreamControllerFillPullIntoDescriptorFromQueue(
    JSContext* cx, Handle<ReadableByteStreamController*> controller,
...
static MOZ_MUST_USE PullIntoDescriptor* ReadableByteStreamControllerShiftPendingPullInto(
    JSContext* cx, HandleNativeObject controller);
...
static MOZ_MUST_USE bool ReadableByteStreamControllerProcessPullIntoDescriptorsUsingQueue(
    ...
    while (pendingPullIntos->getDenseInitializedLength() != 0) {
        ...
        if (ready) {
            ...
            if (!ReadableByteStreamControllerShiftPendingPullInto(cx, controller)) return false;
...
static MOZ_MUST_USE JSObject* ReadableByteStreamControllerPullInto(
    ...
    if (view->is<TypedArrayObject>()) {
        ...
        if (!GetBuiltinConstructor(cx, protoKey, &ctor)) return nullptr;
        elementSize = 1 << TypedArrayShift(view->as<TypedArrayObject>().type());
    } else {
        // Step 3: Let ctor be %DataView% (reordered).
        if (!GetBuiltinConstructor(cx, JSProto_DataView, &ctor)) return nullptr;
    }
    ...
    RootedArrayBufferObject buffer(
        cx, &JS_GetArrayBufferViewBuffer(cx, view, &dummy)->as<ArrayBufferObject>());
    ...
    if (queueTotalSize > 0) {
        ...
        if (ready) {
            ...
            RootedObject filledView(cx);
            filledView =
                ReadableByteStreamControllerConvertPullIntoDescriptor(cx, pullIntoDescriptor);
...
static MOZ_MUST_USE bool ReadableByteStreamControllerRespondInReadableState(
    JSContext* cx, Handle<ReadableByteStreamController*> controller, uint32_t bytesWritten,
    Handle<PullIntoDescriptor*> pullIntoDescriptor) {
...
bool js::ByteLengthQueuingStrategy::constructor(JSContext* cx, unsigned argc, Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);
    ...
    RootedValue highWaterMark(cx);
    if (!GetProperty(cx, argObj, argObj, cx->names().highWaterMark, &highWaterMark)) return false;

    if (!SetProperty(cx, strategy, cx->names().highWaterMark, highWaterMark)) return false;
...
static const JSPropertySpec ByteLengthQueuingStrategy_properties[] = {JS_PS_END};
...
bool js::CountQueuingStrategy::constructor(JSContext* cx, unsigned argc, Value* vp) {
    CallArgs args = CallArgsFromVp(argc, vp);
...
static MOZ_MUST_USE JSObject* PromiseInvokeOrNoop(JSContext* cx, HandleValue O,
    ...
    if (!InvokeOrNoop(cx, O, P, arg, &returnValue)) return PromiseRejectedWithPendingError(cx);
...
static MOZ_MUST_USE bool ValidateAndNormalizeQueuingStrategy(JSContext* cx, HandleValue size,
    ...
    if (!ValidateAndNormalizeHighWaterMark(cx, highWaterMarkVal, highWaterMark)) return false;

    // Step 3: Return Record {[[size]]: size, [[highWaterMark]]: highWaterMark}.
    return true;
}

MOZ_MUST_USE bool js::ReadableStreamReaderCancel(JSContext* cx, HandleObject readerObj,
                                                 HandleValue reason) {
...
MOZ_MUST_USE bool js::ReadableStreamReaderReleaseLock(JSContext* cx, HandleObject readerObj) {
    MOZ_ASSERT(IsReadableStreamReader(readerObj));
    RootedNativeObject reader(cx, &readerObj->as<NativeObject>());
...
MOZ_MUST_USE bool ReadableStream::enqueue(JSContext* cx, Handle<ReadableStream*> stream,
                                          HandleValue chunk) {
    Rooted<ReadableStreamDefaultController*> controller(cx);
...
MOZ_MUST_USE bool ReadableStream::enqueueBuffer(JSContext* cx, Handle<ReadableStream*> stream,
...
/*static */ bool ReadableStream::getExternalSource(JSContext* cx, Handle<ReadableStream*> stream,
                                                   void** source) {
...
MOZ_MUST_USE bool ReadableStream::updateDataAvailableFromSource(JSContext* cx,
                                                                Handle<ReadableStream*> stream,
    ...
    if (ReadableStreamHasDefaultReader(stream)) {
        ...
        size_t bytesWritten;
        {
            ...
            MOZ_ASSERT(cb);
            // TODO: use bytesWritten to correctly update the request's state.
            cb(cx, stream, underlyingSource, stream->embeddingFlags(), buffer, availableData,
               &bytesWritten);
        }
...
MOZ_MUST_USE bool ReadableStream::close(JSContext* cx, Handle<ReadableStream*> stream) {
...
MOZ_MUST_USE bool ReadableStream::error(JSContext* cx, Handle<ReadableStream*> stream,
                                        HandleValue reason) {
    // Step 3: If stream.[[state]] is not "readable", throw a TypeError exception.
    if (!stream->readable()) {
        JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr,
...
MOZ_MUST_USE bool ReadableStream::tee(JSContext* cx, Handle<ReadableStream*> stream,
                                      bool cloneForBranch2,
                                      MutableHandle<ReadableStream*> branch1Stream,
                                      MutableHandle<ReadableStream*> branch2Stream) {
...
MOZ_MUST_USE NativeObject* ReadableStream::getReader(JSContext* cx, Handle<ReadableStream*> stream,
