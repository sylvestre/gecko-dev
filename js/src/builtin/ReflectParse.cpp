...
enum VarDeclKind { VARDECL_ERR = -1, VARDECL_VAR = 0, VARDECL_CONST, VARDECL_LET, VARDECL_LIMIT };
...
static bool GetPropertyDefault(JSContext* cx, HandleObject obj, HandleId id,
                               HandleValue defaultValue, MutableHandleValue result) {
    bool found;
...
enum class GeneratorStyle { None, Legacy, ES6 };
...
class NodeBuilder {
   ...
   public:
    NodeBuilder(JSContext* c, bool l, char const* s)
        : cx(c), tokenStream(nullptr), saveLoc(l), src(s), srcval(c), callbacks(cx), userv(c) {}
    ...
    MOZ_MUST_USE bool init(HandleObject userobj = nullptr) {
        ...
        for (unsigned i = 0; i < AST_LIMIT; i++) {
            ...
            if (!funv.isObject() || !funv.toObject().is<JSFunction>()) {
                ReportValueErrorFlags(cx, JSREPORT_ERROR, JSMSG_NOT_FUNCTION, JSDVG_SEARCH_STACK,
                                      funv, nullptr, nullptr, nullptr);
                return false;
            }
   ...
   private:
    ...
    MOZ_MUST_USE bool newNodeHelper(HandleObject obj, const char* name, HandleValue value,
        ...
        return defineProperty(obj, name, value) && newNodeHelper(obj, Forward<Arguments>(rest)...);
    }
    ...
    MOZ_MUST_USE bool newNode(ASTType type, TokenPos* pos, Arguments&&... args) {
        ...
        return createNode(type, pos, &node) && newNodeHelper(node, Forward<Arguments>(args)...);
    }
   ...
   public:
    ...
    MOZ_MUST_USE bool function(ASTType type, TokenPos* pos, HandleValue id, NodeVector& args,
                               NodeVector& defaults, HandleValue body, HandleValue rest,
    ...
    MOZ_MUST_USE bool switchCase(HandleValue expr, NodeVector& elts, TokenPos* pos,
                                 MutableHandleValue dst);

    MOZ_MUST_USE bool catchClause(HandleValue var, HandleValue guard, HandleValue body,
                                  TokenPos* pos, MutableHandleValue dst);
    ...
    MOZ_MUST_USE bool ifStatement(HandleValue test, HandleValue cons, HandleValue alt,
                                  TokenPos* pos, MutableHandleValue dst);
    ...
    MOZ_MUST_USE bool forStatement(HandleValue init, HandleValue test, HandleValue update,
                                   HandleValue stmt, TokenPos* pos, MutableHandleValue dst);
    ...
    MOZ_MUST_USE bool forOfStatement(HandleValue var, HandleValue expr, HandleValue stmt,
                                     TokenPos* pos, MutableHandleValue dst);

    MOZ_MUST_USE bool withStatement(HandleValue expr, HandleValue stmt, TokenPos* pos,
                                    MutableHandleValue dst);

    MOZ_MUST_USE bool whileStatement(HandleValue test, HandleValue stmt, TokenPos* pos,
                                     MutableHandleValue dst);

    MOZ_MUST_USE bool doWhileStatement(HandleValue stmt, HandleValue test, TokenPos* pos,
                                       MutableHandleValue dst);

    MOZ_MUST_USE bool switchStatement(HandleValue disc, NodeVector& elts, bool lexical,
                                      TokenPos* pos, MutableHandleValue dst);
    ...
    MOZ_MUST_USE bool importDeclaration(NodeVector& elts, HandleValue moduleSpec, TokenPos* pos,
                                        MutableHandleValue dst);

    MOZ_MUST_USE bool importSpecifier(HandleValue importName, HandleValue bindingName,
                                      TokenPos* pos, MutableHandleValue dst);
    ...
    MOZ_MUST_USE bool exportSpecifier(HandleValue bindingName, HandleValue exportName,
                                      TokenPos* pos, MutableHandleValue dst);
    ...
    MOZ_MUST_USE bool logicalExpression(bool lor, HandleValue left, HandleValue right,
                                        TokenPos* pos, MutableHandleValue dst);
...
bool NodeBuilder::createNode(ASTType type, TokenPos* pos, MutableHandleObject dst) {
...
bool NodeBuilder::program(NodeVector& elts, TokenPos* pos, MutableHandleValue dst) {
    return listNode(AST_PROGRAM, "body", elts, pos, dst);
}

bool NodeBuilder::blockStatement(NodeVector& elts, TokenPos* pos, MutableHandleValue dst) {
    return listNode(AST_BLOCK_STMT, "body", elts, pos, dst);
}

bool NodeBuilder::expressionStatement(HandleValue expr, TokenPos* pos, MutableHandleValue dst) {
...
bool NodeBuilder::ifStatement(HandleValue test, HandleValue cons, HandleValue alt, TokenPos* pos,
                              MutableHandleValue dst) {
    RootedValue cb(cx, callbacks[AST_IF_STMT]);
    if (!cb.isNull()) return callback(cb, test, cons, opt(alt), pos, dst);

    return newNode(AST_IF_STMT, pos, "test", test, "consequent", cons, "alternate", alt, dst);
}

bool NodeBuilder::breakStatement(HandleValue label, TokenPos* pos, MutableHandleValue dst) {
...
bool NodeBuilder::continueStatement(HandleValue label, TokenPos* pos, MutableHandleValue dst) {
...
bool NodeBuilder::labeledStatement(HandleValue label, HandleValue stmt, TokenPos* pos,
                                   MutableHandleValue dst) {
...
bool NodeBuilder::throwStatement(HandleValue arg, TokenPos* pos, MutableHandleValue dst) {
...
bool NodeBuilder::returnStatement(HandleValue arg, TokenPos* pos, MutableHandleValue dst) {
...
bool NodeBuilder::forStatement(HandleValue init, HandleValue test, HandleValue update,
                               HandleValue stmt, TokenPos* pos, MutableHandleValue dst) {
    RootedValue cb(cx, callbacks[AST_FOR_STMT]);
    if (!cb.isNull()) return callback(cb, opt(init), opt(test), opt(update), stmt, pos, dst);

    return newNode(AST_FOR_STMT, pos, "init", init, "test", test, "update", update, "body", stmt,
                   dst);
}

bool NodeBuilder::forInStatement(HandleValue var, HandleValue expr, HandleValue stmt,
                                 bool isForEach, TokenPos* pos, MutableHandleValue dst) {
    RootedValue isForEachVal(cx, BooleanValue(isForEach));

    RootedValue cb(cx, callbacks[AST_FOR_IN_STMT]);
    if (!cb.isNull()) return callback(cb, var, expr, stmt, isForEachVal, pos, dst);

    return newNode(AST_FOR_IN_STMT, pos, "left", var, "right", expr, "body", stmt, "each",
                   isForEachVal, dst);
}

bool NodeBuilder::forOfStatement(HandleValue var, HandleValue expr, HandleValue stmt,
                                 TokenPos* pos, MutableHandleValue dst) {
    RootedValue cb(cx, callbacks[AST_FOR_OF_STMT]);
    if (!cb.isNull()) return callback(cb, var, expr, stmt, pos, dst);

    return newNode(AST_FOR_OF_STMT, pos, "left", var, "right", expr, "body", stmt, dst);
}

bool NodeBuilder::withStatement(HandleValue expr, HandleValue stmt, TokenPos* pos,
                                MutableHandleValue dst) {
...
bool NodeBuilder::whileStatement(HandleValue test, HandleValue stmt, TokenPos* pos,
                                 MutableHandleValue dst) {
...
bool NodeBuilder::doWhileStatement(HandleValue stmt, HandleValue test, TokenPos* pos,
                                   MutableHandleValue dst) {
...
bool NodeBuilder::switchStatement(HandleValue disc, NodeVector& elts, bool lexical, TokenPos* pos,
                                  MutableHandleValue dst) {
    RootedValue array(cx);
    if (!newArray(elts, &array)) return false;
    ...
    return newNode(AST_SWITCH_STMT, pos, "discriminant", disc, "cases", array, "lexical",
                   lexicalVal, dst);
}

bool NodeBuilder::tryStatement(HandleValue body, NodeVector& guarded, HandleValue unguarded,
                               HandleValue finally, TokenPos* pos, MutableHandleValue dst) {
    RootedValue guardedHandlers(cx);
    if (!newArray(guarded, &guardedHandlers)) return false;

    RootedValue cb(cx, callbacks[AST_TRY_STMT]);
    if (!cb.isNull())
        return callback(cb, body, guardedHandlers, unguarded, opt(finally), pos, dst);

    return newNode(AST_TRY_STMT, pos, "block", body, "guardedHandlers", guardedHandlers, "handler",
                   unguarded, "finalizer", finally, dst);
}
...
bool NodeBuilder::binaryExpression(BinaryOperator op, HandleValue left, HandleValue right,
                                   TokenPos* pos, MutableHandleValue dst) {
    ...
    return newNode(AST_BINARY_EXPR, pos, "operator", opName, "left", left, "right", right, dst);
}

bool NodeBuilder::unaryExpression(UnaryOperator unop, HandleValue expr, TokenPos* pos,
                                  MutableHandleValue dst) {
    ...
    return newNode(AST_UNARY_EXPR, pos, "operator", opName, "argument", expr, "prefix", trueVal,
                   dst);
}

bool NodeBuilder::assignmentExpression(AssignmentOperator aop, HandleValue lhs, HandleValue rhs,
                                       TokenPos* pos, MutableHandleValue dst) {
    ...
    return newNode(AST_ASSIGN_EXPR, pos, "operator", opName, "left", lhs, "right", rhs, dst);
}

bool NodeBuilder::updateExpression(HandleValue expr, bool incr, bool prefix, TokenPos* pos,
                                   MutableHandleValue dst) {
    RootedValue opName(cx);
    ...
    return newNode(AST_UPDATE_EXPR, pos, "operator", opName, "argument", expr, "prefix", prefixVal,
                   dst);
}

bool NodeBuilder::logicalExpression(bool lor, HandleValue left, HandleValue right, TokenPos* pos,
                                    MutableHandleValue dst) {
    RootedValue opName(cx);
    ...
    return newNode(AST_LOGICAL_EXPR, pos, "operator", opName, "left", left, "right", right, dst);
}

bool NodeBuilder::conditionalExpression(HandleValue test, HandleValue cons, HandleValue alt,
                                        TokenPos* pos, MutableHandleValue dst) {
    RootedValue cb(cx, callbacks[AST_COND_EXPR]);
    if (!cb.isNull()) return callback(cb, test, cons, alt, pos, dst);

    return newNode(AST_COND_EXPR, pos, "test", test, "consequent", cons, "alternate", alt, dst);
}

bool NodeBuilder::sequenceExpression(NodeVector& elts, TokenPos* pos, MutableHandleValue dst) {
    return listNode(AST_LIST_EXPR, "expressions", elts, pos, dst);
}

bool NodeBuilder::callExpression(HandleValue callee, NodeVector& args, TokenPos* pos,
                                 MutableHandleValue dst) {
    RootedValue array(cx);
...
bool NodeBuilder::newExpression(HandleValue callee, NodeVector& args, TokenPos* pos,
                                MutableHandleValue dst) {
    RootedValue array(cx);
    if (!newArray(args, &array)) return false;
...
bool NodeBuilder::memberExpression(bool computed, HandleValue expr, HandleValue member,
                                   TokenPos* pos, MutableHandleValue dst) {
    ...
    return newNode(AST_MEMBER_EXPR, pos, "object", expr, "property", member, "computed",
                   computedVal, dst);
}

bool NodeBuilder::arrayExpression(NodeVector& elts, TokenPos* pos, MutableHandleValue dst) {
    return listNode(AST_ARRAY_EXPR, "elements", elts, pos, dst);
}

bool NodeBuilder::callSiteObj(NodeVector& raw, NodeVector& cooked, TokenPos* pos,
                              MutableHandleValue dst) {
    ...
    return newNode(AST_CALL_SITE_OBJ, pos, "raw", rawVal, "cooked", cookedVal, dst);
}

bool NodeBuilder::taggedTemplate(HandleValue callee, NodeVector& args, TokenPos* pos,
                                 MutableHandleValue dst) {
    RootedValue array(cx);
    if (!newArray(args, &array)) return false;

    return newNode(AST_TAGGED_TEMPLATE, pos, "callee", callee, "arguments", array, dst);
}

bool NodeBuilder::templateLiteral(NodeVector& elts, TokenPos* pos, MutableHandleValue dst) {
    return listNode(AST_TEMPLATE_LITERAL, "elements", elts, pos, dst);
}

bool NodeBuilder::computedName(HandleValue name, TokenPos* pos, MutableHandleValue dst) {
    return newNode(AST_COMPUTED_NAME, pos, "name", name, dst);
}

bool NodeBuilder::spreadExpression(HandleValue expr, TokenPos* pos, MutableHandleValue dst) {
    return newNode(AST_SPREAD_EXPR, pos, "expression", expr, dst);
}

bool NodeBuilder::propertyPattern(HandleValue key, HandleValue patt, bool isShorthand,
                                  TokenPos* pos, MutableHandleValue dst) {
    ...
    return newNode(AST_PROP_PATT, pos, "key", key, "value", patt, "kind", kindName, "shorthand",
                   isShorthandVal, dst);
}

bool NodeBuilder::prototypeMutation(HandleValue val, TokenPos* pos, MutableHandleValue dst) {
...
bool NodeBuilder::propertyInitializer(HandleValue key, HandleValue val, PropKind kind,
                                      bool isShorthand, bool isMethod, TokenPos* pos,
                                      MutableHandleValue dst) {
    RootedValue kindName(cx);
    if (!atomValue(kind == PROP_INIT ? "init" : kind == PROP_GETTER ? "get" : "set", &kindName)) {
        return false;
    }
    ...
    return newNode(AST_PROPERTY, pos, "key", key, "value", val, "kind", kindName, "method",
                   isMethodVal, "shorthand", isShorthandVal, dst);
}

bool NodeBuilder::objectExpression(NodeVector& elts, TokenPos* pos, MutableHandleValue dst) {
...
bool NodeBuilder::yieldExpression(HandleValue arg, YieldKind kind, TokenPos* pos,
                                  MutableHandleValue dst) {
...
bool NodeBuilder::comprehensionBlock(HandleValue patt, HandleValue src, bool isForEach,
                                     bool isForOf, TokenPos* pos, MutableHandleValue dst) {
    RootedValue isForEachVal(cx, BooleanValue(isForEach));
    RootedValue isForOfVal(cx, BooleanValue(isForOf));

    RootedValue cb(cx, callbacks[AST_COMP_BLOCK]);
    if (!cb.isNull()) return callback(cb, patt, src, isForEachVal, isForOfVal, pos, dst);

    return newNode(AST_COMP_BLOCK, pos, "left", patt, "right", src, "each", isForEachVal, "of",
                   isForOfVal, dst);
}

bool NodeBuilder::comprehensionIf(HandleValue test, TokenPos* pos, MutableHandleValue dst) {
    RootedValue cb(cx, callbacks[AST_COMP_IF]);
    if (!cb.isNull()) return callback(cb, test, pos, dst);

    return newNode(AST_COMP_IF, pos, "test", test, dst);
}

bool NodeBuilder::comprehensionExpression(HandleValue body, NodeVector& blocks, HandleValue filter,
                                          bool isLegacy, TokenPos* pos, MutableHandleValue dst) {
    RootedValue array(cx);
    if (!newArray(blocks, &array)) return false;

    RootedValue style(cx);
    if (!atomValue(isLegacy ? "legacy" : "modern", &style)) return false;

    RootedValue cb(cx, callbacks[AST_COMP_EXPR]);
    if (!cb.isNull()) return callback(cb, body, array, opt(filter), style, pos, dst);

    return newNode(AST_COMP_EXPR, pos, "body", body, "blocks", array, "filter", filter, "style",
                   style, dst);
}

bool NodeBuilder::generatorExpression(HandleValue body, NodeVector& blocks, HandleValue filter,
                                      bool isLegacy, TokenPos* pos, MutableHandleValue dst) {
    RootedValue array(cx);
    if (!newArray(blocks, &array)) return false;
    ...
    return newNode(AST_GENERATOR_EXPR, pos, "body", body, "blocks", array, "filter", filter,
                   "style", style, dst);
}

bool NodeBuilder::importDeclaration(NodeVector& elts, HandleValue moduleSpec, TokenPos* pos,
                                    MutableHandleValue dst) {
    RootedValue array(cx);
    if (!newArray(elts, &array)) return false;
    ...
    return newNode(AST_IMPORT_DECL, pos, "specifiers", array, "source", moduleSpec, dst);
}

bool NodeBuilder::importSpecifier(HandleValue importName, HandleValue bindingName, TokenPos* pos,
                                  MutableHandleValue dst) {
    RootedValue cb(cx, callbacks[AST_IMPORT_SPEC]);
    if (!cb.isNull()) return callback(cb, importName, bindingName, pos, dst);

    return newNode(AST_IMPORT_SPEC, pos, "id", importName, "name", bindingName, dst);
}

bool NodeBuilder::exportDeclaration(HandleValue decl, NodeVector& elts, HandleValue moduleSpec,
    ...
    return newNode(AST_EXPORT_DECL, pos, "declaration", decl, "specifiers", array, "source",
                   moduleSpec, "isDefault", isDefault, dst);
}

bool NodeBuilder::exportSpecifier(HandleValue bindingName, HandleValue exportName, TokenPos* pos,
                                  MutableHandleValue dst) {
    RootedValue cb(cx, callbacks[AST_EXPORT_SPEC]);
    if (!cb.isNull()) return callback(cb, bindingName, exportName, pos, dst);

    return newNode(AST_EXPORT_SPEC, pos, "id", bindingName, "name", exportName, dst);
}
...
bool NodeBuilder::variableDeclaration(NodeVector& elts, VarDeclKind kind, TokenPos* pos,
                                      MutableHandleValue dst) {
    MOZ_ASSERT(kind > VARDECL_ERR && kind < VARDECL_LIMIT);

    RootedValue array(cx), kindName(cx);
    if (!newArray(elts, &array) ||
        !atomValue(kind == VARDECL_CONST ? "const" : kind == VARDECL_LET ? "let" : "var",
                   &kindName)) {
        return false;
    }
    ...
    return newNode(AST_VAR_DECL, pos, "kind", kindName, "declarations", array, dst);
}

bool NodeBuilder::variableDeclarator(HandleValue id, HandleValue init, TokenPos* pos,
                                     MutableHandleValue dst) {
...
bool NodeBuilder::catchClause(HandleValue var, HandleValue guard, HandleValue body, TokenPos* pos,
                              MutableHandleValue dst) {
    RootedValue cb(cx, callbacks[AST_CATCH]);
    if (!cb.isNull()) return callback(cb, var, opt(guard), body, pos, dst);

    return newNode(AST_CATCH, pos, "param", var, "guard", guard, "body", body, dst);
}

bool NodeBuilder::literal(HandleValue val, TokenPos* pos, MutableHandleValue dst) {
...
bool NodeBuilder::identifier(HandleValue name, TokenPos* pos, MutableHandleValue dst) {
...
bool NodeBuilder::objectPattern(NodeVector& elts, TokenPos* pos, MutableHandleValue dst) {
    return listNode(AST_OBJECT_PATT, "properties", elts, pos, dst);
}

bool NodeBuilder::arrayPattern(NodeVector& elts, TokenPos* pos, MutableHandleValue dst) {
    return listNode(AST_ARRAY_PATT, "elements", elts, pos, dst);
}

bool NodeBuilder::function(ASTType type, TokenPos* pos, HandleValue id, NodeVector& args,
                           NodeVector& defaults, HandleValue body, HandleValue rest,
                           GeneratorStyle generatorStyle, bool isAsync, bool isExpression,
                           MutableHandleValue dst) {
    ...
    if (isGenerator) {
        ...
        JSAtom* styleStr = generatorStyle == GeneratorStyle::ES6 ? Atomize(cx, "es6", 3)
                                                                 : Atomize(cx, "legacy", 6);
        if (!styleStr) return false;
        styleVal.setString(styleStr);
        return newNode(type, pos, "id", id, "params", array, "defaults", defarray, "body", body,
                       "rest", rest, "generator", isGeneratorVal, "async", isAsyncVal, "style",
                       styleVal, "expression", isExpressionVal, dst);
    }

    return newNode(type, pos, "id", id, "params", array, "defaults", defarray, "body", body,
                   "rest", rest, "generator", isGeneratorVal, "async", isAsyncVal, "expression",
                   isExpressionVal, dst);
}

bool NodeBuilder::classMethod(HandleValue name, HandleValue body, PropKind kind, bool isStatic,
                              TokenPos* pos, MutableHandleValue dst) {
    RootedValue kindName(cx);
    if (!atomValue(kind == PROP_INIT ? "method" : kind == PROP_GETTER ? "get" : "set",
                   &kindName)) {
        return false;
    }
    ...
    if (!cb.isNull()) return callback(cb, kindName, name, body, isStaticVal, pos, dst);

    return newNode(AST_CLASS_METHOD, pos, "name", name, "body", body, "kind", kindName, "static",
                   isStaticVal, dst);
}
...
bool NodeBuilder::classDefinition(bool expr, HandleValue name, HandleValue heritage,
                                  HandleValue block, TokenPos* pos, MutableHandleValue dst) {
    ASTType type = expr ? AST_CLASS_EXPR : AST_CLASS_STMT;
    RootedValue cb(cx, callbacks[type]);
    if (!cb.isNull()) return callback(cb, name, heritage, block, pos, dst);

    return newNode(type, pos, "id", name, "superClass", heritage, "body", block, dst);
}

bool NodeBuilder::metaProperty(HandleValue meta, HandleValue property, TokenPos* pos,
                               MutableHandleValue dst) {
    RootedValue cb(cx, callbacks[AST_METAPROPERTY]);
    if (!cb.isNull()) return callback(cb, meta, property, pos, dst);

    return newNode(AST_METAPROPERTY, pos, "meta", meta, "property", property, dst);
}
...
class ASTSerializer {
    ...
    bool functionArgs(ParseNode* pn, ParseNode* pnargs, NodeVector& args, NodeVector& defaults,
    ...
    bool functionArgsAndBody(ParseNode* pn, NodeVector& args, NodeVector& defaults, bool isAsync,
                             bool isExpression, MutableHandleValue body, MutableHandleValue rest);
...
bool ASTSerializer::blockStatement(ParseNode* pn, MutableHandleValue dst) {
    ...
    NodeVector stmts(cx);
    return statements(pn, stmts) && builder.blockStatement(stmts, &pn->pn_pos, dst);
}
...
bool ASTSerializer::declaration(ParseNode* pn, MutableHandleValue dst) {
    MOZ_ASSERT(pn->isKind(PNK_FUNCTION) || pn->isKind(PNK_VAR) || pn->isKind(PNK_LET) ||
               pn->isKind(PNK_CONST));
...
bool ASTSerializer::variableDeclaration(ParseNode* pn, bool lexical, MutableHandleValue dst) {
...
bool ASTSerializer::importSpecifier(ParseNode* pn, MutableHandleValue dst) {
    ...
    return identifier(pn->pn_left, &importName) && identifier(pn->pn_right, &bindingName) &&
...
bool ASTSerializer::exportDeclaration(ParseNode* pn, MutableHandleValue dst) {
    ...
    switch (ParseNodeKind kind = kid->getKind()) {
        ...
        case PNK_CONST:
        case PNK_LET:
            if (!variableDeclaration(kid, kind != PNK_VAR, &decl)) return false;
            break;
    ...
    if (pn->isKind(PNK_EXPORT_FROM) && !literal(pn->pn_right, &moduleSpec)) return false;

    RootedValue isDefault(cx, BooleanValue(false));
    if (pn->isKind(PNK_EXPORT_DEFAULT)) isDefault.setBoolean(true);
...
bool ASTSerializer::exportSpecifier(ParseNode* pn, MutableHandleValue dst) {
    ...
    return identifier(pn->pn_left, &bindingName) && identifier(pn->pn_right, &exportName) &&
...
bool ASTSerializer::catchClause(ParseNode* pn, bool* isGuarded, MutableHandleValue dst) {
    ...
    if (!pattern(pn->pn_kid1, &var) || !optExpression(pn->pn_kid2, &guard)) {
        return false;
    }
...
bool ASTSerializer::tryStatement(ParseNode* pn, MutableHandleValue dst) {
    ...
    if (ParseNode* catchList = pn->pn_kid2) {
        if (!guarded.reserve(catchList->pn_count)) return false;

        for (ParseNode* next = catchList->pn_head; next; next = next->pn_next) {
            RootedValue clause(cx);
            bool isGuarded;
            if (!catchClause(next->pn_expr, &isGuarded, &clause)) return false;
            if (isGuarded)
...
bool ASTSerializer::forInit(ParseNode* pn, MutableHandleValue dst) {
    ...
    return (lexical || pn->isKind(PNK_VAR)) ? variableDeclaration(pn, lexical, dst)
                                            : expression(pn, dst);
}

bool ASTSerializer::forOf(ParseNode* loop, ParseNode* head, HandleValue var, HandleValue stmt,
                          MutableHandleValue dst) {
    RootedValue expr(cx);
...
bool ASTSerializer::forIn(ParseNode* loop, ParseNode* head, HandleValue var, HandleValue stmt,
                          MutableHandleValue dst) {
    RootedValue expr(cx);
...
bool ASTSerializer::classDefinition(ParseNode* pn, bool expr, MutableHandleValue dst) {
    RootedValue className(cx, MagicValue(JS_SERIALIZE_NO_NODE));
    RootedValue heritage(cx);
    RootedValue classBody(cx);

    if (pn->pn_kid1) {
        if (!identifier(pn->pn_kid1->as<ClassNames>().innerBinding(), &className)) return false;
    }

    return optExpression(pn->pn_kid2, &heritage) && statement(pn->pn_kid3, &classBody) &&
...
bool ASTSerializer::statement(ParseNode* pn, MutableHandleValue dst) {
    ...
    switch (pn->getKind()) {
        ...
        case PNK_LEXICALSCOPE:
            pn = pn->pn_expr;
            if (!pn->isKind(PNK_STATEMENTLIST)) return statement(pn, dst);
            MOZ_FALLTHROUGH;
        ...
        case PNK_WHILE: {
            ...
            return expression(pn->pn_left, &expr) && statement(pn->pn_right, &stmt) &&
                   (pn->isKind(PNK_WITH) ? builder.withStatement(expr, stmt, &pn->pn_pos, dst)
        ...
        case PNK_COMPREHENSIONFOR: {
            ...
            if (head->isKind(PNK_FORIN) || head->isKind(PNK_FOROF)) {
                ...
                if (head->pn_kid1->isKind(PNK_LEXICALSCOPE)) {
                    if (!variableDeclaration(head->pn_kid1->pn_expr, true, &var)) return false;
                } else if (!head->pn_kid1->isKind(PNK_VAR) && !head->pn_kid1->isKind(PNK_LET) &&
                           !head->pn_kid1->isKind(PNK_CONST)) {
                    if (!pattern(head->pn_kid1, &var)) return false;
                } else {
                    if (!variableDeclaration(
                            head->pn_kid1,
                            head->pn_kid1->isKind(PNK_LET) || head->pn_kid1->isKind(PNK_CONST),
                            &var)) {
                        return false;
                    }
                }
                if (head->isKind(PNK_FORIN)) return forIn(pn, head, var, stmt, dst);
                return forOf(pn, head, var, stmt, dst);
            }

            RootedValue init(cx), test(cx), update(cx);

            return forInit(head->pn_kid1, &init) && optExpression(head->pn_kid2, &test) &&
        ...
        case PNK_CONTINUE: {
            ...
            return optIdentifier(pnAtom, nullptr, &label) &&
                   (pn->isKind(PNK_BREAK) ? builder.breakStatement(label, &pn->pn_pos, dst)
        ...
        case PNK_LABEL: {
            ...
            return identifier(pnAtom, nullptr, &label) && statement(pn->pn_expr, &stmt) &&
...
bool ASTSerializer::leftAssociate(ParseNode* pn, MutableHandleValue dst) {
    ...
    for (ParseNode* next = head->pn_next; next; next = next->pn_next) {
        ...
        if (logop) {
            if (!builder.logicalExpression(lor, left, right, &subpos, &left)) return false;
        } else {
            BinaryOperator op = binop(pn->getKind());
            LOCAL_ASSERT(op > BINOP_ERR && op < BINOP_LIMIT);

            if (!builder.binaryExpression(op, left, right, &subpos, &left)) return false;
        }
    }
...
bool ASTSerializer::rightAssociate(ParseNode* pn, MutableHandleValue dst) {
    ...
    for (ParseNode* next = head->pn_next; next; next = next->pn_next) {
        ...
        if (!builder.binaryExpression(op, left, right, &subpos, &right)) return false;
    }
...
bool ASTSerializer::comprehensionBlock(ParseNode* pn, MutableHandleValue dst) {
    ...
    if (decl->isKind(PNK_LEXICALSCOPE)) decl = decl->pn_expr;
    MOZ_ASSERT(decl->pn_count == 1);

    RootedValue patt(cx), src(cx);
    return pattern(decl->pn_head, &patt) && expression(in->pn_kid3, &src) &&
           builder.comprehensionBlock(patt, src, isForEach, isForOf, &in->pn_pos, dst);
}

bool ASTSerializer::comprehensionIf(ParseNode* pn, MutableHandleValue dst) {
    LOCAL_ASSERT(pn->isKind(PNK_IF));
    LOCAL_ASSERT(!pn->pn_kid3);

    RootedValue patt(cx);
    return pattern(pn->pn_kid1, &patt) && builder.comprehensionIf(patt, &pn->pn_pos, dst);
}

bool ASTSerializer::comprehension(ParseNode* pn, MutableHandleValue dst) {
    ...
    while (true) {
        if (next->isKind(PNK_COMPREHENSIONFOR)) {
            ...
            if (!comprehensionBlock(next, &block) || !blocks.append(block)) return false;
            next = next->pn_right;
        } else if (next->isKind(PNK_IF)) {
            if (isLegacy) {
                MOZ_ASSERT(filter.isMagic(JS_SERIALIZE_NO_NODE));
                if (!optExpression(next->pn_kid1, &filter)) return false;
            } else {
                // ES7 comprehension can contain multiple ComprehensionIfs.
                RootedValue compif(cx);
                if (!comprehensionIf(next, &compif) || !blocks.append(compif)) return false;
            }
            next = next->pn_kid2;
        } else {
            break;
...
bool ASTSerializer::generatorExpression(ParseNode* pn, MutableHandleValue dst) {
    ...
    LOCAL_ASSERT(next->isKind(PNK_SEMI) && next->pn_kid->isKind(PNK_YIELD) &&
                 next->pn_kid->pn_kid);
...
bool ASTSerializer::expression(ParseNode* pn, MutableHandleValue dst) {
    ...
    switch (pn->getKind()) {
        ...
        case PNK_COMMA: {
            NodeVector exprs(cx);
            return expressions(pn, exprs) && builder.sequenceExpression(exprs, &pn->pn_pos, dst);
        }
        ...
        case PNK_SUPERCALL: {
            ...
            return pn->isKind(PNK_NEW) ? builder.newExpression(callee, args, &pn->pn_pos, dst)
        ...
        case PNK_COMPUTED_NAME: {
            RootedValue name(cx);
            return expression(pn->pn_kid, &name) && builder.computedName(name, &pn->pn_pos, dst);
        }
...
bool ASTSerializer::propertyName(ParseNode* pn, MutableHandleValue dst) {
    if (pn->isKind(PNK_COMPUTED_NAME)) return expression(pn, dst);
    if (pn->isKind(PNK_OBJECT_PROPERTY_NAME)) return identifier(pn, dst);
...
bool ASTSerializer::property(ParseNode* pn, MutableHandleValue dst) {
    if (pn->isKind(PNK_MUTATEPROTO)) {
        RootedValue val(cx);
        return expression(pn->pn_kid, &val) && builder.prototypeMutation(val, &pn->pn_pos, dst);
    }
    if (pn->isKind(PNK_SPREAD)) return expression(pn, dst);
    ...
    bool isMethod = pn->pn_right->isKind(PNK_FUNCTION) &&
...
bool ASTSerializer::arrayPattern(ParseNode* pn, MutableHandleValue dst) {
    ...
    for (ParseNode* next = pn->pn_head; next; next = next->pn_next) {
        if (next->isKind(PNK_ELISION)) {
            ...
            if (!builder.spreadExpression(target, &next->pn_pos, &spread)) return false;
...
bool ASTSerializer::objectPattern(ParseNode* pn, MutableHandleValue dst) {
    ...
    for (ParseNode* propdef = pn->pn_head; propdef; propdef = propdef->pn_next) {
        if (propdef->isKind(PNK_SPREAD)) {
            ...
            if (!builder.spreadExpression(target, &propdef->pn_pos, &spread)) return false;
            elts.infallibleAppend(spread);
            continue;
        }
...
bool ASTSerializer::identifier(HandleAtom atom, TokenPos* pos, MutableHandleValue dst) {
...
bool ASTSerializer::function(ParseNode* pn, ASTType type, MutableHandleValue dst) {
    RootedFunction func(cx, pn->pn_funbox->function());

    GeneratorStyle generatorStyle =
        pn->pn_funbox->isStarGenerator()
            ? GeneratorStyle::ES6
            : pn->pn_funbox->isLegacyGenerator() ? GeneratorStyle::Legacy : GeneratorStyle::None;
    ...
    return functionArgsAndBody(pn->pn_body, args, defaults, isAsync, isExpression, &body, &rest) &&
           builder.function(type, &pn->pn_pos, id, args, defaults, body, rest, generatorStyle,
                            isAsync, isExpression, dst);
}

bool ASTSerializer::functionArgsAndBody(ParseNode* pn, NodeVector& args, NodeVector& defaults,
                                        bool isAsync, bool isExpression, MutableHandleValue body,
                                        MutableHandleValue rest) {
    ParseNode* pnargs;
    ParseNode* pnbody;
    ...
    if (pnbody->isKind(PNK_LEXICALSCOPE)) pnbody = pnbody->scopeBody();
    ...
    switch (pnbody->getKind()) {
        ...
        case PNK_STATEMENTLIST: /* statement closure */
        {
            ...
            if (pnstart && pnstart->isKind(PNK_INITIALYIELD)) pnstart = pnstart->pn_next;
...
bool ASTSerializer::functionArgs(ParseNode* pn, ParseNode* pnargs, NodeVector& args,
...
bool ASTSerializer::functionBody(ParseNode* pn, TokenPos* pos, MutableHandleValue dst) {
    NodeVector elts(cx);
...
static bool reflect_parse(JSContext* cx, uint32_t argc, Value* vp) {
    ...
    if (!arg.isNullOrUndefined()) {
        if (!arg.isObject()) {
            ReportValueErrorFlags(cx, JSREPORT_ERROR, JSMSG_UNEXPECTED_TYPE, JSDVG_SEARCH_STACK,
                                  arg, nullptr, "not an object", nullptr);
            return false;
        }
        ...
        if (loc) {
            ...
            RootedValue nullVal(cx, NullValue());
            if (!GetPropertyDefault(cx, config, sourceId, nullVal, &prop)) return false;
        ...
        RootedValue nullVal(cx, NullValue());
        if (!GetPropertyDefault(cx, config, builderId, nullVal, &prop)) return false;

        if (!prop.isNullOrUndefined()) {
            if (!prop.isObject()) {
                ReportValueErrorFlags(cx, JSREPORT_ERROR, JSMSG_UNEXPECTED_TYPE,
                                      JSDVG_SEARCH_STACK, prop, nullptr, "not an object", nullptr);
                return false;
            }
        ...
        if (!GetPropertyDefault(cx, config, targetId, scriptVal, &prop)) return false;
        ...
        bool isScript = false;
        bool isModule = false;
        if (!EqualStrings(cx, stringProp, cx->names().script, &isScript)) return false;

        if (!EqualStrings(cx, stringProp, cx->names().module, &isModule)) return false;
    ...
    if (target == ParseTarget::Script) {
        ...
        if (!GlobalObject::ensureModulePrototypesCreated(cx, cx->global())) return false;
...
JS_InitReflectParse(JSContext* cx, HandleObject global) {
    ...
    if (!GetProperty(cx, global, global, cx->names().Reflect, &reflectVal)) return false;
