...
class TypeDescr : public NativeObject {
   public:
    ...
    type::Kind kind() const { return (type::Kind)getReservedSlot(JS_DESCR_SLOT_KIND).toInt32(); }

    bool opaque() const { return getReservedSlot(JS_DESCR_SLOT_OPAQUE).toBoolean(); }
...
#define JS_FOR_EACH_UNIQUE_SCALAR_TYPE_REPR_CTYPE(macro_)                                  \
    macro_(Scalar::Int8, int8_t, int8) macro_(Scalar::Uint8, uint8_t, uint8)               \
        macro_(Scalar::Int16, int16_t, int16) macro_(Scalar::Uint16, uint16_t, uint16)     \
            macro_(Scalar::Int32, int32_t, int32) macro_(Scalar::Uint32, uint32_t, uint32) \
                macro_(Scalar::Float32, float, float32) macro_(Scalar::Float64, double, float64)
...
class ArrayMetaTypeDescr : public NativeObject {
   private:
    ...
    static ArrayTypeDescr* create(JSContext* cx, HandleObject arrayTypePrototype,
                                  HandleTypeDescr elementType, HandleAtom stringRepr, int32_t size,
                                  int32_t length);
...
class ArrayTypeDescr : public ComplexTypeDescr {
   public:
    ...
    static int32_t offsetOfLength() { return getFixedSlotOffset(JS_DESCR_SLOT_ARRAY_LENGTH); }
};
...
class StructMetaTypeDescr : public NativeObject {
   private:
    static JSObject* create(JSContext* cx, HandleObject structTypeGlobal, HandleObject fields);
...
class TypedObject : public ShapedObject {
    ...
    static MOZ_MUST_USE bool obj_getArrayElement(JSContext* cx, Handle<TypedObject*> typedObj,
   ...
   protected:
    ...
    static MOZ_MUST_USE bool obj_lookupProperty(JSContext* cx, HandleObject obj, HandleId id,
   ...
   public:
    ...
    static JS::Result<TypedObject*, JS::OOM&> create(JSContext* cx, js::gc::AllocKind kind,
...
class OutlineTypedObject : public TypedObject {
   ...
   public:
    ...
    static OutlineTypedObject* createUnattachedWithClass(JSContext* cx, const Class* clasp,
                                                         HandleTypeDescr type, int32_t length,
                                                         gc::InitialHeap heap = gc::DefaultHeap);
    ...
    static OutlineTypedObject* createDerived(JSContext* cx, HandleTypeDescr type,
                                             Handle<TypedObject*> typedContents, uint32_t offset);
...
class InlineTypedObject : public TypedObject {
   ...
   public:
    ...
    uint8_t* inlineTypedMem(const JS::AutoRequireNoGC&) const { return inlineTypedMem(); }
    ...
    static size_t offsetOfDataStart() { return offsetof(InlineTypedObject, data_); }
...
class InlineTransparentTypedObject : public InlineTypedObject {
   public:
    ...
    uint8_t* inlineTypedMem() const { return InlineTypedObject::inlineTypedMem(); }
};
...
#define JS_STORE_REFERENCE_CLASS_DEFN(_constant, T, _name)                                       \
    class StoreReference##_name {                                                                \
       private:                                                                                  \
        static MOZ_MUST_USE bool store(JSContext* cx, T* heap, const Value& v, TypedObject* obj, \
                                       jsid id);                                                 \
                                                                                                 \
       public:                                                                                   \
        static MOZ_MUST_USE bool Func(JSContext* cx, unsigned argc, Value* vp);                  \
...
inline bool IsSimpleTypeDescrClass(const Class* clasp) {
    return clasp == &ScalarTypeDescr::class_ || clasp == &ReferenceTypeDescr::class_;
}
...
inline bool TypedObject::opaque() const { return IsOpaqueTypedObjectClass(getClass()); }
