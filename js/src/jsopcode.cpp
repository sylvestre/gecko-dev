...
static bool DecompileArgumentFromStack(JSContext* cx, int formalIndex, char** res);
...
static MOZ_MUST_USE bool DumpIonScriptCounts(Sprinter* sp, HandleScript script,
    ...
    for (size_t i = 0; i < ionCounts->numBlocks(); i++) {
        ...
        if (!sp->jsprintf("BB #%" PRIu32 " [%05u,%u,%u]", block.id(), block.offset(), lineNumber,
                          columnNumber)) {
            return false;
        }
...
static MOZ_MUST_USE bool DumpPCCounts(JSContext* cx, HandleScript script, Sprinter* sp) {
    ...
    while (pc < script->codeEnd()) {
        ...
        if (!Disassemble1(cx, script, pc, script->pcToOffset(pc), true, sp)) return false;

        if (!sp->put("                  {")) return false;

        PCCounts* counts = script->maybeGetPCCounts(pc);
        if (double val = counts ? counts->numExec() : 0.0) {
            if (!sp->jsprintf("\"%s\": %.0f", PCCounts::numExecName, val)) return false;
        }
        if (!sp->put("}\n")) return false;
...
struct OffsetAndDefIndex {
    ...
    bool operator!=(const OffsetAndDefIndex& rhs) const { return !(*this == rhs); }
};
...
class BytecodeParser {
   public:
    enum class JumpKind { Simple, SwitchCase, SwitchDefault, TryCatch, TryFinally };
    ...
    class Bytecode {
       public:
        ...
        bool captureOffsetStackAfter(LifoAlloc& alloc, const OffsetAndDefIndex* stack,
            ...
            if (stackDepthAfter) {
                for (uint32_t n = 0; n < stackDepthAfter; n++) offsetStackAfter[n] = stack[n];
            }
            return true;
        }
...
#ifdef DEBUG
    uint32_t stackDepthAfterPC(uint32_t offset) { return getCode(offset).stackDepthAfter; }
   ...
   private:
    ...
    inline bool addJump(uint32_t offset, uint32_t* currentOffset, uint32_t stackDepth,
                        const OffsetAndDefIndex* offsetStack, jsbytecode* pc, JumpKind kind);
};

}  // anonymous namespace

uint32_t BytecodeParser::simulateOp(JSOp op, uint32_t offset, OffsetAndDefIndex* offsetStack,
                                    uint32_t stackDepth) {
    ...
    switch (op) {
        default:
            for (uint32_t n = 0; n != ndefs; ++n) offsetStack[stackDepth + n].set(offset, n);
            break;
...
bool BytecodeParser::recordBytecode(uint32_t offset, const OffsetAndDefIndex* offsetStack,
                                    uint32_t stackDepth) {
    MOZ_ASSERT(offset < script_->length());
...
bool BytecodeParser::addJump(uint32_t offset, uint32_t* currentOffset, uint32_t stackDepth,
                             const OffsetAndDefIndex* offsetStack, jsbytecode* pc, JumpKind kind) {
...
bool BytecodeParser::parse() {
    ...
    while (nextOffset < length) {
        ...
        if (BytecodeIsJumpTarget(op)) {
            for (uint32_t n = 0; n < code->stackDepth; ++n) offsetStack[n] = code->offsetStack[n];
        }
                ...
                for (; tn < tnlimit; tn++) {
                    ...
                    if (startOffset == offset + 1) {
                        ...
                        if (tn->kind == JSTRY_CATCH) {
                            if (!addJump(catchOffset, &nextOffset, stackDepth, offsetStack, pc,
                                         JumpKind::TryCatch)) {
                                return false;
                            }
        ...
        if (BytecodeFallsThrough(op)) {
            if (!recordBytecode(successorOffset, offsetStack, stackDepth)) return false;
        }
    }
...
bool js::ReconstructStackDepth(JSContext* cx, JSScript* script, jsbytecode* pc, uint32_t* depth,
                               bool* reachablePC) {
...
static unsigned Disassemble1(JSContext* cx, HandleScript script, jsbytecode* pc, unsigned loc,
                             bool lines, BytecodeParser* parser, Sprinter* sp);
 ...
 * entry stack depth.
 */
static MOZ_MUST_USE bool DisassembleAtPC(JSContext* cx, JSScript* scriptArg, bool lines,
    ...
    if (showAll) {
        if (!sp->jsprintf("%s:%u\n", script->filename(), unsigned(script->lineno()))) return false;
    }
    ...
    while (next < end) {
        ...
        unsigned len =
            Disassemble1(cx, script, next, script->pcToOffset(next), lines, &parser, sp);
        if (!len) return false;
...
bool js::Disassemble(JSContext* cx, HandleScript script, bool lines, Sprinter* sp) {
...
static bool ToDisassemblySource(JSContext* cx, HandleValue v, JSAutoByteString* bytes) {
    if (v.isString()) {
        Sprinter sprinter(cx);
        if (!sprinter.init()) return false;
...
static bool ToDisassemblySource(JSContext* cx, HandleScope scope, JSAutoByteString* bytes) {
...
static bool DumpJumpOrigins(JSContext* cx, HandleScript script, jsbytecode* pc,
...
static bool DecompileAtPCForStackDump(JSContext* cx, HandleScript script,
                                      const OffsetAndDefIndex& offsetAndDefIndex, Sprinter* sp);

static unsigned Disassemble1(JSContext* cx, HandleScript script, jsbytecode* pc, unsigned loc,
                             bool lines, BytecodeParser* parser, Sprinter* sp) {
    if (parser && parser->isReachable(pc)) {
        if (!DumpJumpOrigins(cx, script, pc, parser, sp)) return 0;
    }
    ...
    auto dumpStack = [&cx, &script, &pc, &parser, &sp, &before, &stackDumped]() {
        ...
        if (!parser->isReachable(pc)) {
            ...
            for (uint32_t i = 0; i < depth; i++) {
                ...
                if (!DecompileAtPCForStackDump(cx, script, offsetAndDefIndex, sp)) return false;
            }
        }
    ...
    if (op >= JSOP_LIMIT) {
        ...
        JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr, JSMSG_BYTECODE_TOO_BIG, numBuf1,
                                  numBuf2);
        return 0;
    }
    ...
    switch (JOF_TYPE(cs->format)) {
        ...
        case JOF_JUMP: {
            ...
            if (!sp->jsprintf(" %u (%+d)", unsigned(loc + int(off)), int(off))) return 0;
            break;
        }
        ...
        case JOF_TABLESWITCH: {
            ...
            pc2 += JUMP_OFFSET_LEN;
            if (!sp->jsprintf(" defaultOffset %d low %d high %d", int(off), low, high)) return 0;
...
unsigned js::Disassemble1(JSContext* cx, JS::Handle<JSScript*> script, jsbytecode* pc,
...
bool ExpressionDecompiler::decompilePC(jsbytecode* pc, uint8_t defIndex) {
            ...
            case 2: {
                ...
                if (!sn || SN_TYPE(sn) != SRC_ASSIGNOP)
                    return write("(") && decompilePCForStackOperand(pc, -2) && write(" ") &&
                           write(token) && write(" ") && decompilePCForStackOperand(pc, -1) &&
                           write(")");
                break;
            }
            case 1:
                return write("(") && write(token) && decompilePCForStackOperand(pc, -1) &&
                       write(")");
            default:
                break;
        }
    ...
    switch (op) {
        ...
        case JSOP_CALLPROP: {
            ...
            return (hasDelete ? write("(delete ") : true) && decompilePCForStackOperand(pc, -1) &&
                   (IsIdentifier(prop) ? write(".") && quote(prop, '\0')
        ...
        case JSOP_CALLELEM: {
            ...
            return (hasDelete ? write("(delete ") : true) && decompilePCForStackOperand(pc, -2) &&
                   write("[") && decompilePCForStackOperand(pc, -1) && write("]") &&
                   (hasDelete ? write(")") : true);
        }

        case JSOP_GETELEM_SUPER:
            return write("super[") && decompilePCForStackOperand(pc, -3) && write("]");
        case JSOP_NULL:
        ...
        case JSOP_FUNCALL:
        case JSOP_FUNAPPLY:
            return decompilePCForStackOperand(pc, -int32_t(GET_ARGC(pc) + 2)) && write("(...)");
        ...
        case JSOP_VOID:
            return write("(void ") && decompilePCForStackOperand(pc, -1) && write(")");
        ...
        case JSOP_NEW:
            return write("(new ") && decompilePCForStackOperand(pc, -int32_t(GET_ARGC(pc) + 3)) &&
                   write("(...))");

        case JSOP_SPREADNEW:
            return write("(new ") && decompilePCForStackOperand(pc, -4) && write("(...))");

        case JSOP_TYPEOF:
        case JSOP_TYPEOFEXPR:
            return write("(typeof ") && decompilePCForStackOperand(pc, -1) && write(")");
    ...
    if (isStackDump) {
        ...
        switch (op) {
            ...
            case JSOP_TOID:
                return write("TOID(") && decompilePCForStackOperand(pc, -1) && write(")");
            case JSOP_TOSTRING:
                return write("TOSTRING(") && decompilePCForStackOperand(pc, -1) && write(")");
...
bool ExpressionDecompiler::decompilePC(const OffsetAndDefIndex& offsetAndDefIndex) {
...
static bool FindStartPC(JSContext* cx, const FrameIter& iter, int spindex, int skipStackHits,
                        const Value& v, jsbytecode** valuepc, uint8_t* defIndex) {
...
static bool DecompileExpressionFromStack(JSContext* cx, int spindex, int skipStackHits,
                                         HandleValue v, char** res) {
    MOZ_ASSERT(spindex < 0 || spindex == JSDVG_IGNORE_STACK || spindex == JSDVG_SEARCH_STACK);
    ...
    *res = nullptr;
    ...
    uint8_t defIndex;
    if (!FindStartPC(cx, frameIter, spindex, skipStackHits, v, &valuepc, &defIndex)) return false;
...
UniqueChars js::DecompileValueGenerator(JSContext* cx, int spindex, HandleValue v,
                                        HandleString fallbackArg, int skipStackHits) {
    RootedString fallback(cx, fallbackArg);
    {
        char* result;
        if (!DecompileExpressionFromStack(cx, spindex, skipStackHits, v, &result)) return nullptr;
        if (result) {
    ...
    if (!fallback) {
        if (v.isUndefined())
            return UniqueChars(
                JS_strdup(cx, js_undefined_str));  // Prevent users from seeing "(void 0)"
...
static bool DecompileArgumentFromStack(JSContext* cx, int formalIndex, char** res) {
    ...
    *res = nullptr;
     ...
     */
    ++frameIter;
    if (frameIter.done() || !frameIter.hasScript() || frameIter.script()->selfHosted() ||
    ...
    if (op != JSOP_CALL && op != JSOP_CALL_IGNORES_RV && op != JSOP_NEW) return true;
    ...
    int formalStackIndex =
        parser.stackDepthAtPC(current) - GET_ARGC(current) - pushedNewTarget + formalIndex;
...
extern bool js::IsValidBytecodeOffset(JSContext* cx, JSScript* script, size_t offset) {
...
static MOZ_MUST_USE bool AppendJSONProperty(StringBuffer& buf, const char* name,
    ...
    return buf.append('\"') && buf.append(name, strlen(name)) && buf.append("\":", 2);
}
...
js::GetPCCountScriptSummary(JSContext* cx, size_t index) {
    ...
    if (ionActivity) {
        ...
        if (!NumberValueToStringBuffer(cx, DoubleValue(ionActivity), buf)) return nullptr;
    }
...
static bool GetPCCountJSON(JSContext* cx, const ScriptAndCounts& sac, StringBuffer& buf) {
    RootedScript script(cx, sac.script);
    ...
    if (!NumberValueToStringBuffer(cx, Int32Value(script->lineno()), buf)) return false;
    ...
    for (jsbytecode* pc = script->code(); pc < end; pc = GetNextPc(pc)) {
        ...
        if (!AppendJSONProperty(buf, "line")) return false;
        if (!NumberValueToStringBuffer(cx, Int32Value(scanner.getLine()), buf)) return false;
        ...
        if (hits > 0) {
            if (!AppendJSONProperty(buf, PCCounts::numExecName, NO_COMMA)) return false;
    ...
    if (ionCounts) {
        ...
        while (ionCounts) {
            ...
            for (size_t i = 0; i < ionCounts->numBlocks(); i++) {
                ...
                if (!buf.append('{')) return false;
                if (!AppendJSONProperty(buf, "id", NO_COMMA)) return false;
                if (!NumberValueToStringBuffer(cx, Int32Value(block.id()), buf)) return false;
                if (!AppendJSONProperty(buf, "offset")) return false;
                if (!NumberValueToStringBuffer(cx, Int32Value(block.offset()), buf)) return false;
...
static bool GenerateLcovInfo(JSContext* cx, JSCompartment* comp, GenericPrinter& out) {
    JSRuntime* rt = cx->runtime();
    ...
    for (ZonesIter zone(rt, SkipAtoms); !zone.done(); zone.next()) {
        for (auto script = zone->cellIter<JSScript>(); !script.done(); script.next()) {
            if (script->compartment() != comp || !script->isTopLevel() || !script->filename()) {
                continue;
            }
