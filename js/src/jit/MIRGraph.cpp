...
bool MIRGenerator::usesSimd() {
    ...
    for (ReversePostorderIterator block = graph_->rpoBegin(), end = graph_->rpoEnd(); block != end;
         block++) {
...
mozilla::GenericErrorResult<AbortReason> MIRGenerator::abortFmt(AbortReason r, const char* message,
                                                                va_list ap) {
    JitSpewVA(JitSpew_IonAbort, message, ap);
    return Err(mozilla::Move(r));
}

mozilla::GenericErrorResult<AbortReason> MIRGenerator::abort(AbortReason r, const char* message,
                                                             ...) {
    va_list ap;
    va_start(ap, message);
...
void MIRGenerator::addAbortedPreliminaryGroup(ObjectGroup* group) {
    ...
    if (!abortedPreliminaryGroups_.append(group)) oomUnsafe.crash("addAbortedPreliminaryGroup");
}
...
void MIRGraph::unmarkBlocks() {
    for (MBasicBlockIterator i(blocks_.begin()); i != blocks_.end(); i++) i->unmark();
}

MBasicBlock* MBasicBlock::New(MIRGraph& graph, size_t stackDepth, const CompileInfo& info,
...
MBasicBlock* MBasicBlock::NewPopN(MIRGraph& graph, const CompileInfo& info, MBasicBlock* pred,
                                  BytecodeSite* site, Kind kind, uint32_t popped) {
    MBasicBlock* block = new (graph.alloc()) MBasicBlock(graph, info, site, kind);
    if (!block->init()) return nullptr;

    if (!block->inherit(graph.alloc(), pred->stackDepth(), pred, popped)) return nullptr;

    return block;
}

MBasicBlock* MBasicBlock::NewWithResumePoint(MIRGraph& graph, const CompileInfo& info,
...
MBasicBlock* MBasicBlock::NewPendingLoopHeader(MIRGraph& graph, const CompileInfo& info,
                                               MBasicBlock* pred, BytecodeSite* site,
    ...
    if (!block->inherit(graph.alloc(), pred->stackDepth(), pred, 0, stackPhiCount)) return nullptr;

    return block;
}

MBasicBlock* MBasicBlock::NewSplitEdge(MIRGraph& graph, MBasicBlock* pred, size_t predEdgeIdx,
                                       MBasicBlock* succ) {
    ...
    if (!succ->pc()) {
        ...
        MResumePoint* splitEntry =
            new (graph.alloc()) MResumePoint(split, succEntry->pc(), MResumePoint::ResumeAt);
...
MBasicBlock* MBasicBlock::New(MIRGraph& graph, const CompileInfo& info, MBasicBlock* pred,
                              Kind kind) {
...
bool MBasicBlock::increaseSlots(size_t num) { return slots_.growBy(graph_.alloc(), num); }
...
void MBasicBlock::copySlots(MBasicBlock* from) {
    ...
    for (size_t i = 0, e = stackPosition_; i < e; ++i) thisSlots[i] = fromSlots[i];
}

bool MBasicBlock::inherit(TempAllocator& alloc, size_t stackDepth, MBasicBlock* maybePred,
    ...
    if (maybePred) {
        ...
        if (kind_ == PENDING_LOOP_HEADER) {
            ...
            for (size_t i = 0; i < stackDepth; i++) entryResumePoint()->initOperand(i, getSlot(i));
        }
    } else {
        /*
...
bool MBasicBlock::inheritResumePoint(MBasicBlock* pred) {
    ...
    for (uint32_t i = 0; i < stackPosition_; i++) slots_[i] = entryResumePoint_->getOperand(i);
...
MDefinition* MBasicBlock::environmentChain() { return getSlot(info().environmentChainSlot()); }

MDefinition* MBasicBlock::argumentsObject() { return getSlot(info().argsObjSlot()); }
...
MInstruction* MBasicBlock::safeInsertTop(MDefinition* ins, IgnoreTop ignore) {
    ...
    // such instructions.
    MInstructionIterator insertIter = !ins || ins->isPhi() ? begin() : begin(ins->toInstruction());
    while (insertIter->isBeta() || insertIter->isInterruptCheck() || insertIter->isConstant() ||
...
void MBasicBlock::discardResumePoint(MResumePoint* rp,
...
void MBasicBlock::prepareForDiscard(MInstruction* ins,
                                    ReferencesType refType /* = RefType_Default */) {
    ...
    if ((refType & RefType_DiscardResumePoint) && rp) discardResumePoint(rp, refType);
    ...
    if ((refType & InstructionOperands) == InstructionOperands) {
        for (size_t i = 0, e = ins->numOperands(); i < e; i++) ins->releaseOperand(i);
    }
...
#ifdef DEBUG
    for (size_t i = 0, e = ins->numOperands(); i < e; i++) MOZ_ASSERT(!ins->hasOperand(i));
#endif
...
void MBasicBlock::discardAllPhiOperands() {
    for (MPhiIterator iter = phisBegin(); iter != phisEnd(); iter++) iter->removeAllOperands();
...
bool MBasicBlock::addPredecessorPopN(TempAllocator& alloc, MBasicBlock* pred, uint32_t popped) {
    MOZ_ASSERT(pred);
...
void MBasicBlock::addPredecessorSameInputsAs(MBasicBlock* pred, MBasicBlock* existingPred) {
    MOZ_ASSERT(pred);
    MOZ_ASSERT(predecessors_.length() > 0);
    ...
    if (!predecessors_.append(pred)) oomUnsafe.crash("MBasicBlock::addPredecessorAdjustPhis");
}
...
void MBasicBlock::removeImmediatelyDominatedBlock(MBasicBlock* child) {
    for (size_t i = 0;; ++i) {
        MOZ_ASSERT(i < immediatelyDominated_.length(), "Dominated block to remove not present");
...
AbortReason MBasicBlock::setBackedge(TempAllocator& alloc, MBasicBlock* pred) {
    ...
    if (!inheritPhisFromBackedge(alloc, pred, &hadTypeChange)) return AbortReason::Alloc;
...
void MBasicBlock::replacePredecessor(MBasicBlock* old, MBasicBlock* split) {
    for (size_t i = 0; i < numPredecessors(); i++) {
        if (getPredecessor(i) == old) {
            ...
            for (size_t j = i; j < numPredecessors(); j++) MOZ_ASSERT(predecessors_[j] != old);
#endif
...
void MBasicBlock::removePredecessorWithoutPhiOperands(MBasicBlock* pred, size_t predIndex) {
    // If we're removing the last backedge, this is no longer a loop.
    if (isLoopHeader() && hasUniqueBackedge() && backedge() == pred) clearLoopHeader();
...
bool MBasicBlock::inheritPhisFromBackedge(TempAllocator& alloc, MBasicBlock* backedge,
                                          bool* hadTypeChange) {
    // We must be a pending loop header
    MOZ_ASSERT(kind_ == PENDING_LOOP_HEADER);
    ...
    for (size_t slot = 0; slot < stackDepth; slot++) {
        ...
        if (!entryDef->checkForTypeChange(alloc, exitDef, &typeChange)) return false;
...
bool MBasicBlock::BackupPoint::init(TempAllocator& alloc) {
    ...
    for (size_t i = 0, e = stackPosition_; i < e; ++i) slots_[i] = current_->slots_[i];
    return true;
}

#ifdef DEBUG
uintptr_t MBasicBlock::BackupPoint::computePredecessorsCheckSum(MBasicBlock* block) {
    uintptr_t hash = 0;
    for (size_t i = 0; i < block->numPredecessors(); i++) {
...
HashNumber MBasicBlock::BackupPoint::computeInstructionsCheckSum(MBasicBlock* block) {
    HashNumber h = 0;
...
MBasicBlock* MBasicBlock::BackupPoint::restore() {
    ...
    if (current_->stackPosition_ != stackPosition_) current_->setStackDepth(stackPosition_);
    for (size_t i = 0, e = stackPosition_; i < e; ++i) current_->slots_[i] = slots_[i];
...
void MBasicBlock::dump(GenericPrinter& out) {
    ...
    out.printf("block%u:%s%s%s\n", id(), isLoopHeader() ? " (loop header)" : "",
               unreachable() ? " (unreachable)" : "", isMarked() ? " (marked)" : "");
    if (MResumePoint* resume = entryResumePoint()) resume->dump(out);
    for (MPhiIterator iter(phisBegin()); iter != phisEnd(); iter++) iter->dump(out);
    for (MInstructionIterator iter(begin()); iter != end(); iter++) iter->dump(out);
#endif
}
