...
void AliasAnalysisShared::spewDependencyList() {
    ...
    if (JitSpewEnabled(JitSpew_AliasSummaries)) {
        ...
        for (ReversePostorderIterator block(graph_.rpoBegin()); block != graph_.rpoEnd();
             block++) {
...
static inline const MDefinition* GetObject(const MDefinition* ins) {
    if (!ins->getAliasSet().isStore() && !ins->getAliasSet().isLoad()) return nullptr;
...
MDefinition::AliasType AliasAnalysisShared::genericMightAlias(const MDefinition* load,
