...
class RValueAllocation {
   ...
   private:
    ...
    struct FloatRegisterBits {
        uint32_t data;
        bool operator==(const FloatRegisterBits& other) const { return data == other.data; }
    ...
    union Payload {
        ...
        Payload() : index(0) {
            static_assert(sizeof(index) == sizeof(Payload), "All Payload bits are initialized.");
        }
    };
    ...
    static void readPayload(CompactBufferReader& reader, PayloadType t, uint8_t* mode, Payload* p);
    static void writePayload(CompactBufferWriter& writer, PayloadType t, Payload p);
    ...
    RValueAllocation(Mode mode, Payload a1, Payload a2) : mode_(mode), arg1_(a1), arg2_(a2) {}

    RValueAllocation(Mode mode, Payload a1) : mode_(mode), arg1_(a1) { arg2_.index = 0; }
   ...
   public:
    ...
    static RValueAllocation Typed(JSValueType type, Register reg) {
        ...
        return RValueAllocation(TYPED_REG, payloadOfValueType(type), payloadOfRegister(reg));
    }
    ...
    static RValueAllocation Untyped(int32_t typeStackOffset, Register payload) {
        return RValueAllocation(UNTYPED_STACK_REG, payloadOfStackOffset(typeStackOffset),
                                payloadOfRegister(payload));
    }

    static RValueAllocation Untyped(int32_t typeStackOffset, int32_t payloadStackOffset) {
        return RValueAllocation(UNTYPED_STACK_STACK, payloadOfStackOffset(typeStackOffset),
                                payloadOfStackOffset(payloadStackOffset));
    }
    ...
    // common constants.
    static RValueAllocation Undefined() { return RValueAllocation(CST_UNDEFINED); }
   ...
   public:
    bool operator==(const RValueAllocation& rhs) const {
        ...
        static_assert(sizeof(int32_t) == sizeof(Payload), "All Payload bits are compared.");
...
class SnapshotWriter {
   ...
   public:
    ...
    void trackSnapshot(uint32_t pcOpcode, uint32_t mirOpcode, uint32_t mirId, uint32_t lirOpcode,
                       uint32_t lirId);
#endif
    ...
    bool oom() const {
        return writer_.oom() || writer_.length() >= MAX_BUFFER_SIZE || allocWriter_.oom() ||
               allocWriter_.length() >= MAX_BUFFER_SIZE;
    }
...
class RecoverWriter {
   ...
   public:
    ...
    bool oom() const { return writer_.oom() || writer_.length() >= MAX_BUFFER_SIZE; }
};
...
class SnapshotReader {
   ...
   public:
    SnapshotReader(const uint8_t* snapshots, uint32_t offset, uint32_t RVATableSize,
                   uint32_t listSize);
...
class RecoverReader {
   ...
   public:
    ...
    bool moreInstructions() const { return numInstructionsRead_ < numInstructions_; }
