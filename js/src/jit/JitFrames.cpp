...
static inline void WriteFrameSlot(JitFrameLayout* fp, int32_t slot, uintptr_t value) {
...
static void CloseLiveIteratorIon(JSContext* cx, const InlineFrameIterator& frame, JSTryNote* tn) {
    MOZ_ASSERT(tn->kind == JSTRY_FOR_IN || tn->kind == JSTRY_DESTRUCTURING_ITERCLOSE);
...
static void OnLeaveBaselineFrame(JSContext* cx, const JSJitFrameIter& frame, jsbytecode* pc,
...
static inline void ForcedReturn(JSContext* cx, const JSJitFrameIter& frame, jsbytecode* pc,
                                ResumeFromException* rfe) {
    OnLeaveBaselineFrame(cx, frame, pc, rfe, true);
}

static inline void BaselineFrameAndStackPointersFromTryNote(JSTryNote* tn,
...
static void SettleOnTryNote(JSContext* cx, JSTryNote* tn, const JSJitFrameIter& frame,
                            EnvironmentIter& ei, ResumeFromException* rfe, jsbytecode** pc) {
    RootedScript script(cx, frame.baselineFrame()->script());

    // Unwind environment chain (pop block objects).
    if (cx->isExceptionPending()) UnwindEnvironment(cx, ei, UnwindEnvironmentToTryPc(script, tn));
...
struct AutoBaselineHandlingException {
    BaselineFrame* frame;
    AutoBaselineHandlingException(BaselineFrame* frame, jsbytecode* pc) : frame(frame) {
...
static void CloseLiveIteratorsBaselineForUncatchableException(JSContext* cx,
                                                              const JSJitFrameIter& frame,
                                                              jsbytecode* pc) {
...
static bool ProcessTryNotesBaseline(JSContext* cx, const JSJitFrameIter& frame,
                                    EnvironmentIter& ei, ResumeFromException* rfe,
                                    jsbytecode** pc) {
    RootedScript script(cx, frame.baselineFrame()->script());
    bool inForOfIterClose = false;
...
struct AutoResetLastProfilerFrameOnReturnFromException {
    ...
    ~AutoResetLastProfilerFrameOnReturnFromException() {
        if (!cx->runtime()->jitRuntime()->isProfilerInstrumentationEnabled(cx->runtime())) return;
...
void HandleExceptionWasm(JSContext* cx, wasm::WasmFrameIter* iter, ResumeFromException* rfe) {
...
void HandleException(ResumeFromException* rfe) {
    ...
    while (!iter.done()) {
        ...
        if (frame.isIonJS()) {
            ...
            if (invalidated) ionScript->decrementInvalidationCount(cx->runtime()->defaultFreeOp());
...
#ifdef JS_NUNBOX32
static inline uintptr_t ReadAllocation(const JSJitFrameIter& frame, const LAllocation* a) {
    if (a->isGeneralReg()) {
...
static void TraceThisAndArguments(JSTracer* trc, const JSJitFrameIter& frame) {
    ...
    for (size_t i = nformals + 1; i < nargs + 1; i++) TraceRoot(trc, &argv[i], "ion-argv");
...
#ifdef JS_NUNBOX32
static inline void WriteAllocation(const JSJitFrameIter& frame, const LAllocation* a,
                                   uintptr_t value) {
    if (a->isGeneralReg()) {
        Register reg = a->toGeneralReg()->reg();
...
void UpdateIonJSFrameForMinorGC(JSTracer* trc, const JSJitFrameIter& frame) {
...
void GetPcScript(JSContext* cx, JSScript** scriptRes, jsbytecode** pcRes) {
    ...
    if (retAddr) {
        ...
        if (MOZ_UNLIKELY(cx->ionPcScriptCache == nullptr)) {
            ...
            if (cx->ionPcScriptCache) cx->ionPcScriptCache->clear(cx->runtime()->gc.gcNumber());
        }
    ...
    // Add entry to cache.
    if (retAddr && cx->ionPcScriptCache) cx->ionPcScriptCache->add(hash, retAddr, pc, *scriptRes);
}
...
RInstructionResults::RInstructionResults(RInstructionResults&& src)
    : results_(mozilla::Move(src.results_)), fp_(src.fp_), initialized_(src.initialized_) {
...
HeapPtr<Value>& RInstructionResults::operator[](size_t index) { return (*results_)[index]; }
...
SnapshotIterator::SnapshotIterator(const JSJitFrameIter& iter, const MachineState* machineState)
    : snapshot_(iter.ionScript()->snapshots(), iter.snapshotOffset(),
                iter.ionScript()->snapshotsRVATableSize(), iter.ionScript()->snapshotsListSize()),
      recover_(snapshot_, iter.ionScript()->recovers(), iter.ionScript()->recoversSize()),
      fp_(iter.jsFrame()),
...
int32_t SnapshotIterator::readOuterNumActualArgs() const { return fp_->numActualArgs(); }

uintptr_t SnapshotIterator::fromStack(int32_t offset) const { return ReadFrameSlot(fp_, offset); }
...
bool SnapshotIterator::allocationReadable(const RValueAllocation& alloc, ReadMethod rm) {
...
Value SnapshotIterator::allocationValue(const RValueAllocation& alloc, ReadMethod rm) {
    switch (alloc.mode()) {
...
Value SnapshotIterator::maybeRead(const RValueAllocation& a, MaybeReadFallback& fallback) {
...
void SnapshotIterator::writeAllocationValuePayload(const RValueAllocation& alloc, const Value& v) {
...
uint32_t SnapshotIterator::numAllocations() const { return instruction()->numOperands(); }

uint32_t SnapshotIterator::pcOffset() const { return resumePoint()->pcOffset(); }
...
bool SnapshotIterator::initInstructionResults(MaybeReadFallback& fallback) {
    ...
    if (!results) {
        ...
        if (!fallback.activation->registerIonFrameRecovery(mozilla::Move(tmp))) return false;
...
bool SnapshotIterator::computeInstructionResults(JSContext* cx,
                                                 RInstructionResults* results) const {
    MOZ_ASSERT(!results->isInitialized());
...
JSObject* InlineFrameIterator::computeEnvironmentChain(const Value& envChainValue,
    ...
    if (envChainValue.isObject()) {
        if (hasInitialEnv) {
            if (fallback.canRecoverResults()) {
                ...
                *hasInitialEnv =
                    isFunctionFrame() && callee(fallback)->needsFunctionEnvironmentObjects();
                return obj;
            } else {
            ...
            for (; !frames.done(); ++frames) {
                ...
                if (frames.isIonJS()) {
                    ...
                    MOZ_RELEASE_ASSERT(frames.ionScript()->frameSize() % JitStackAlignment == 0,
                                       "Ensure that if the Ion frame is aligned, then the spill "
                                       "base is also aligned");
