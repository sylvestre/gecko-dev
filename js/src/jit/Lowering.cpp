...
LBoxAllocation LIRGenerator::useBoxFixedAtStart(MDefinition* mir, ValueOperand op) {
...
LBoxAllocation LIRGenerator::useBoxAtStart(MDefinition* mir, LUse::Policy policy) {
...
void LIRGenerator::visitCallee(MCallee* ins) { define(new (alloc()) LCallee(), ins); }

void LIRGenerator::visitIsConstructing(MIsConstructing* ins) {
    define(new (alloc()) LIsConstructing(), ins);
}

static void TryToUseImplicitInterruptCheck(MIRGraph& graph, MBasicBlock* backedge) {
    // Implicit interrupt checks require wasm signal handlers to be installed.
    if (!wasm::HaveSignalHandlers() || JitOptions.ionInterruptWithoutSignals) return;
...
void LIRGenerator::visitDefFun(MDefFun* ins) {
    ...
    LDefFun* lir = new (alloc())
        LDefFun(useRegisterAtStart(fun), useRegisterAtStart(ins->environmentChain()));
    add(lir, ins);
    assignSafepoint(lir, ins);
}
...
void LIRGenerator::visitNewTypedArrayDynamicLength(MNewTypedArrayDynamicLength* ins) {
    MDefinition* length = ins->length();
    MOZ_ASSERT(length->type() == MIRType::Int32);
...
void LIRGenerator::visitNewDerivedTypedObject(MNewDerivedTypedObject* ins) {
    LNewDerivedTypedObject* lir = new (alloc())
        LNewDerivedTypedObject(useRegisterAtStart(ins->type()), useRegisterAtStart(ins->owner()),
...
void LIRGenerator::visitInitElem(MInitElem* ins) {
    LInitElem* lir =
        new (alloc()) LInitElem(useRegisterAtStart(ins->getObject()), useBoxAtStart(ins->getId()),
...
void LIRGenerator::visitInitElemGetterSetter(MInitElemGetterSetter* ins) {
    LInitElemGetterSetter* lir = new (alloc())
        LInitElemGetterSetter(useRegisterAtStart(ins->object()), useBoxAtStart(ins->idValue()),
...
void LIRGenerator::visitMutateProto(MMutateProto* ins) {
    LMutateProto* lir = new (alloc())
        LMutateProto(useRegisterAtStart(ins->getObject()), useBoxAtStart(ins->getValue()));
    add(lir, ins);
    assignSafepoint(lir, ins);
}

void LIRGenerator::visitInitPropGetterSetter(MInitPropGetterSetter* ins) {
    LInitPropGetterSetter* lir = new (alloc())
        LInitPropGetterSetter(useRegisterAtStart(ins->object()), useRegisterAtStart(ins->value()));
    add(lir, ins);
    assignSafepoint(lir, ins);
}
...
void LIRGenerator::visitCreateThis(MCreateThis* ins) {
    LCreateThis* lir =
        new (alloc()) LCreateThis(useRegisterOrConstantAtStart(ins->getCallee()),
                                  useRegisterOrConstantAtStart(ins->getNewTarget()));
    defineReturn(lir, ins);
    assignSafepoint(lir, ins);
}
...
void LIRGenerator::visitCreateArgumentsObject(MCreateArgumentsObject* ins) {
    ...
    LCreateArgumentsObject* lir = new (alloc()) LCreateArgumentsObject(
        callObj, tempFixed(CallTempReg1), tempFixed(CallTempReg2), tempFixed(CallTempReg3));
    defineReturn(lir, ins);
    assignSafepoint(lir, ins);
}
...
void LIRGenerator::visitReturnFromCtor(MReturnFromCtor* ins) {
    LReturnFromCtor* lir =
        new (alloc()) LReturnFromCtor(useBox(ins->getValue()), useRegister(ins->getObject()));
    define(lir, ins);
}
...
bool LIRGenerator::lowerCallArguments(MCall* call) {
    ...
    for (size_t i = 0; i < argc; i++) {
        ...
        if (arg->type() == MIRType::Value) {
            ...
            LStackArgT* stack =
                new (alloc()) LStackArgT(argslot, arg->type(), useRegisterOrConstant(arg));
            add(stack);
        }
...
void LIRGenerator::visitCall(MCall* call) {
    ...
    if (call->isCallDOMNative()) {
        ...
        lir = new (alloc()) LCallDOMNative(tempFixed(cxReg), tempFixed(objReg), tempFixed(privReg),
                                           tempFixed(argsReg));
    } else if (target) {
        ...
        if (target->isNative()) {
            ...
            lir = new (alloc()) LCallNative(tempFixed(cxReg), tempFixed(numReg), tempFixed(vpReg),
                                            tempFixed(tmpReg));
        } else {
...
void LIRGenerator::visitApplyArgs(MApplyArgs* apply) {
    ...
    LApplyArgsGeneric* lir = new (alloc())
        LApplyArgsGeneric(useFixedAtStart(apply->getFunction(), CallTempReg3),
...
void LIRGenerator::visitApplyArray(MApplyArray* apply) {
    ...
    LApplyArrayGeneric* lir = new (alloc())
        LApplyArrayGeneric(useFixedAtStart(apply->getFunction(), CallTempReg3),
...
void LIRGenerator::visitAssertRecoveredOnBailout(MAssertRecoveredOnBailout* assertion) {
...
void LIRGenerator::visitGetDynamicName(MGetDynamicName* ins) {
    ...
    LGetDynamicName* lir = new (alloc()) LGetDynamicName(
        useFixedAtStart(envChain, CallTempReg0), useFixedAtStart(name, CallTempReg1),
        tempFixed(CallTempReg2), tempFixed(CallTempReg3), tempFixed(CallTempReg4));
...
void LIRGenerator::visitCallDirectEval(MCallDirectEval* ins) {
    ...
    LInstruction* lir = new (alloc()) LCallDirectEval(
        useRegisterAtStart(envChain), useRegisterAtStart(string), useBoxAtStart(newTargetValue));
    defineReturn(lir, ins);
    assignSafepoint(lir, ins);
}
...
void LIRGenerator::visitTest(MTest* test) {
    ...
    if (opd->type() == MIRType::Value) {
        ...
        LTestVAndBranch* lir = new (alloc())
            LTestVAndBranch(ifTrue, ifFalse, useBox(opd), tempDouble(), temp0, temp1);
        add(lir, test);
        return;
    }
    ...
    if (opd->type() == MIRType::ObjectOrNull) {
        LDefinition temp0 =
            test->operandMightEmulateUndefined() ? temp() : LDefinition::BogusTemp();
    ...
    if (opd->isCompare() && opd->isEmittedAtUses()) {
        ...
        if ((comp->isInt32Comparison() || comp->compareType() == MCompare::Compare_UInt32) &&
            comp->getOperand(1)->isConstant() && comp->getOperand(1)->toConstant()->isInt32(0) &&
            comp->getOperand(0)->isBitAnd() && comp->getOperand(0)->isEmittedAtUses()) {
            MDefinition* bitAnd = opd->getOperand(0);
            MDefinition* lhs = bitAnd->getOperand(0);
            MDefinition* rhs = bitAnd->getOperand(1);

            Assembler::Condition cond = JSOpToCondition(comp->compareType(), comp->jsop());
    ...
    if (opd->isCompare() && opd->isEmittedAtUses()) {
        ...
        if (comp->compareType() == MCompare::Compare_Null ||
            ...
            if (left->type() == MIRType::Object || left->type() == MIRType::ObjectOrNull) {
                MOZ_ASSERT(
                    left->type() == MIRType::ObjectOrNull || comp->operandMightEmulateUndefined(),
                ...
                LIsNullOrLikeUndefinedAndBranchT* lir =
                    new (alloc()) LIsNullOrLikeUndefinedAndBranchT(comp, useRegister(left), ifTrue,
                                                                   ifFalse, tmp);
                add(lir, test);
                return;
            }
            ...
            LIsNullOrLikeUndefinedAndBranchV* lir = new (alloc()) LIsNullOrLikeUndefinedAndBranchV(
        ...
        if (comp->compareType() == MCompare::Compare_Int64 ||
            ...
            JSOp op = ReorderComparison(comp->jsop(), &left, &right);
            LCompareI64AndBranch* lir = new (alloc()) LCompareI64AndBranch(
                comp, op, useInt64Register(left), useInt64OrConstant(right), ifTrue, ifFalse);
            add(lir, test);
            return;
        }
    ...
    if (opd->isBitAnd() && opd->isEmittedAtUses()) {
        ...
        if (lhs->type() == MIRType::Int32 && rhs->type() == MIRType::Int32) {
            ReorderCommutative(&lhs, &rhs, test);
            lowerForBitAndAndBranch(new (alloc()) LBitAndAndBranch(ifTrue, ifFalse), test, lhs,
                                    rhs);
            return;
        }
    }
    ...
    if (opd->isIsNoIter()) {
        ...
        LIsNoIterAndBranch* lir = new (alloc()) LIsNoIterAndBranch(ifTrue, ifFalse, useBox(input));
        add(lir, test);
        return;
    }
...
void LIRGenerator::visitCompare(MCompare* comp) {
    ...
    if (comp->compareType() == MCompare::Compare_StrictString) {
        ...
        LCompareStrictS* lir =
            new (alloc()) LCompareStrictS(useBox(left), useRegister(right), tempToUnbox());
        define(lir, comp);
        assignSafepoint(lir, comp);
        return;
    }
    ...
    if (comp->compareType() == MCompare::Compare_Bitwise) {
        LCompareBitwise* lir =
            new (alloc()) LCompareBitwise(useBoxAtStart(left), useBoxAtStart(right));
        define(lir, comp);
        return;
    }
...
static bool CanEmitBitAndAtUses(MInstruction* ins) {
    ...
    MNode* node = iter->consumer();
    if (!node->isDefinition() || !node->toDefinition()->isInstruction()) return false;

    MInstruction* use = node->toDefinition()->toInstruction();
    if (!use->isTest() && !(use->isCompare() && CanEmitCompareAtUses(use))) return false;
...
void LIRGenerator::visitAtan2(MAtan2* ins) {
    ...
    LAtan2D* lir = new (alloc())
        LAtan2D(useRegisterAtStart(y), useRegisterAtStart(x), tempFixed(CallTempReg0));
    defineReturn(lir, ins);
}
...
void LIRGenerator::visitHypot(MHypot* ins) {
    ...
    switch (length) {
        case 2:
            lir = new (alloc())
                LHypot(useRegisterAtStart(ins->getOperand(0)),
                       useRegisterAtStart(ins->getOperand(1)), tempFixed(CallTempReg0));
            break;
        case 3:
            lir = new (alloc()) LHypot(
                useRegisterAtStart(ins->getOperand(0)), useRegisterAtStart(ins->getOperand(1)),
                useRegisterAtStart(ins->getOperand(2)), tempFixed(CallTempReg0));
            break;
        case 4:
            lir = new (alloc()) LHypot(
                useRegisterAtStart(ins->getOperand(0)), useRegisterAtStart(ins->getOperand(1)),
                useRegisterAtStart(ins->getOperand(2)), useRegisterAtStart(ins->getOperand(3)),
...
void LIRGenerator::visitPow(MPow* ins) {
    ...
    if (power->type() == MIRType::Int32) {
        ...
        lir = new (alloc())
            LPowD(useRegisterAtStart(input), useRegisterAtStart(power), tempFixed(CallTempReg0));
    }
    defineReturn(lir, ins);
}
...
void LIRGenerator::visitMathFunction(MMathFunction* ins) {
    ...
    if (ins->type() == MIRType::Double) {
        ...
        lir = new (alloc())
            LMathFunctionD(useRegisterAtStart(ins->input()), tempFixed(CallTempReg0));
    } else {
        lir = new (alloc())
            LMathFunctionF(useRegisterAtStart(ins->input()), tempFixed(CallTempReg0));
    }
    defineReturn(lir, ins);
}
...
void LIRGenerator::visitMod(MMod* ins) {
    ...
    if (ins->specialization() == MIRType::Double) {
        ...
        LDefinition maybeTemp =
            gen->compilingWasm() ? LDefinition::BogusTemp() : tempFixed(CallTempReg0);

        // Note: useRegisterAtStart is safe here, the temp is not a FP register.
        LModD* lir = new (alloc())
            LModD(useRegisterAtStart(ins->lhs()), useRegisterAtStart(ins->rhs()), maybeTemp);
        defineReturn(lir, ins);
        return;
    }
...
void LIRGenerator::visitConcat(MConcat* ins) {
    ...
    LConcat* lir = new (alloc())
        LConcat(useFixedAtStart(lhs, CallTempReg0), useFixedAtStart(rhs, CallTempReg1),
...
void LIRGenerator::visitLimitedTruncate(MLimitedTruncate* nop) { redefine(nop, nop->input()); }
...
void LIRGenerator::visitToInt32(MToInt32* convert) {
    MDefinition* opd = convert->input();

    switch (opd->type()) {
        case MIRType::Value: {
            LValueToInt32* lir = new (alloc())
                LValueToInt32(useBox(opd), tempDouble(), temp(), LValueToInt32::NORMAL);
            assignSnapshot(lir, Bailout_NonPrimitiveInput);
            define(lir, convert);
...
void LIRGenerator::visitTruncateToInt32(MTruncateToInt32* truncate) {
    ...
    switch (opd->type()) {
        case MIRType::Value: {
            LValueToInt32* lir = new (alloc())
                LValueToInt32(useBox(opd), tempDouble(), temp(), LValueToInt32::TRUNCATE);
            assignSnapshot(lir, Bailout_NonPrimitiveInput);
            define(lir, truncate);
...
void LIRGenerator::visitRegExpMatcher(MRegExpMatcher* ins) {
    ...
    LRegExpMatcher* lir =
        new (alloc()) LRegExpMatcher(useFixedAtStart(ins->regexp(), RegExpMatcherRegExpReg),
...
void LIRGenerator::visitRegExpTester(MRegExpTester* ins) {
    ...
    LRegExpTester* lir =
        new (alloc()) LRegExpTester(useFixedAtStart(ins->regexp(), RegExpTesterRegExpReg),
                                    useFixedAtStart(ins->string(), RegExpTesterStringReg),
                                    useFixedAtStart(ins->lastIndex(), RegExpTesterLastIndexReg));
    defineReturn(lir, ins);
    assignSafepoint(lir, ins);
...
void LIRGenerator::visitRegExpPrototypeOptimizable(MRegExpPrototypeOptimizable* ins) {
    MOZ_ASSERT(ins->object()->type() == MIRType::Object);
    MOZ_ASSERT(ins->type() == MIRType::Boolean);
    LRegExpPrototypeOptimizable* lir =
        new (alloc()) LRegExpPrototypeOptimizable(useRegister(ins->object()), temp());
    define(lir, ins);
}

void LIRGenerator::visitRegExpInstanceOptimizable(MRegExpInstanceOptimizable* ins) {
    MOZ_ASSERT(ins->object()->type() == MIRType::Object);
    MOZ_ASSERT(ins->proto()->type() == MIRType::Object);
    MOZ_ASSERT(ins->type() == MIRType::Boolean);
    LRegExpInstanceOptimizable* lir = new (alloc())
        LRegExpInstanceOptimizable(useRegister(ins->object()), useRegister(ins->proto()), temp());
    define(lir, ins);
}
...
void LIRGenerator::visitGetFirstDollarIndex(MGetFirstDollarIndex* ins) {
    ...
    MOZ_ASSERT(ins->type() == MIRType::Int32);
    LGetFirstDollarIndex* lir =
        new (alloc()) LGetFirstDollarIndex(useRegister(ins->str()), temp(), temp(), temp());
    define(lir, ins);
    assignSafepoint(lir, ins);
}
...
void LIRGenerator::visitStringReplace(MStringReplace* ins) {
    ...
    LStringReplace* lir = new (alloc()) LStringReplace(
        useRegisterOrConstantAtStart(ins->string()), useRegisterAtStart(ins->pattern()),
...
void LIRGenerator::visitBinarySharedStub(MBinarySharedStub* ins) {
    ...
    LBinarySharedStub* lir =
        new (alloc()) LBinarySharedStub(useBoxFixedAtStart(lhs, R0), useBoxFixedAtStart(rhs, R1));
...
void LIRGenerator::visitLambda(MLambda* ins) {
    if (ins->info().singletonType || ins->info().useSingletonForClone) {
        ...
        LLambdaForSingleton* lir =
            new (alloc()) LLambdaForSingleton(useRegisterAtStart(ins->environmentChain()));
        defineReturn(lir, ins);
        assignSafepoint(lir, ins);
    } else {
...
void LIRGenerator::visitLambdaArrow(MLambdaArrow* ins) {
    ...
    LLambdaArrow* lir = new (alloc())
        LLambdaArrow(useRegister(ins->environmentChain()), useBox(ins->newTargetDef()));
    define(lir, ins);
    assignSafepoint(lir, ins);
}
...
void LIRGenerator::visitSetFunName(MSetFunName* ins) {
    ...
    LSetFunName* lir =
        new (alloc()) LSetFunName(useRegisterAtStart(ins->fun()), useBoxAtStart(ins->name()));
    add(lir, ins);
    assignSafepoint(lir, ins);
}

void LIRGenerator::visitNewLexicalEnvironmentObject(MNewLexicalEnvironmentObject* ins) {
...
void LIRGenerator::visitCopyLexicalEnvironmentObject(MCopyLexicalEnvironmentObject* ins) {
...
void LIRGenerator::visitConstantElements(MConstantElements* ins) {
    define(new (alloc())
               LPointer(ins->value().unwrap(/*safe - pointer does not flow back to C++*/),
                        LPointer::NON_GC_THING),
           ins);
}

void LIRGenerator::visitConvertElementsToDoubles(MConvertElementsToDoubles* ins) {
...
void LIRGenerator::visitMaybeToDoubleElement(MMaybeToDoubleElement* ins) {
    ...
    LMaybeToDoubleElement* lir = new (alloc()) LMaybeToDoubleElement(
        useRegisterAtStart(ins->elements()), useRegisterAtStart(ins->value()), tempDouble());
    defineBox(lir, ins);
}

void LIRGenerator::visitMaybeCopyElementsForWrite(MMaybeCopyElementsForWrite* ins) {
    LInstruction* check =
        new (alloc()) LMaybeCopyElementsForWrite(useRegister(ins->object()), temp());
    add(check, ins);
    assignSafepoint(check, ins);
}
...
void LIRGenerator::visitLoadSlot(MLoadSlot* ins) {
    switch (ins->type()) {
        ...
        default:
            define(new (alloc()) LLoadSlotT(useRegisterForTypedLoad(ins->slots(), ins->type())),
                   ins);
            break;
    }
}
...
void LIRGenerator::visitWasmTrap(MWasmTrap* ins) { add(new (alloc()) LWasmTrap, ins); }
...
void LIRGenerator::visitStoreSlot(MStoreSlot* ins) {
    ...
    switch (ins->value()->type()) {
        ...
        case MIRType::Double:
            add(new (alloc()) LStoreSlotT(useRegister(ins->slots()), useRegister(ins->value())),
                ins);
            break;
        ...
        default:
            add(new (alloc())
                    LStoreSlotT(useRegister(ins->slots()), useRegisterOrConstant(ins->value())),
                ins);
            break;
    }
}

void LIRGenerator::visitFilterTypeSet(MFilterTypeSet* ins) { redefine(ins, ins->input()); }
...
void LIRGenerator::visitMonitorTypes(MMonitorTypes* ins) {
    // Requesting a non-GC pointer is safe here since we never re-enter C++
    ...
    switch (ins->value()->type()) {
        ...
        case MIRType::ObjectOrNull: {
            ...
            LPostWriteBarrierO* lir = new (alloc()) LPostWriteBarrierO(
                useConstantObject ? useOrConstant(ins->object()) : useRegister(ins->object()),
...
void LIRGenerator::visitPostWriteElementBarrier(MPostWriteElementBarrier* ins) {
    ...
    bool useConstantObject =
        ins->object()->isConstant() && !IsInsideNursery(&ins->object()->toConstant()->toObject());
...
void LIRGenerator::visitSetArrayLength(MSetArrayLength* ins) {
    ...
    add(new (alloc())
            LSetArrayLength(useRegister(ins->elements()), useRegisterOrConstant(ins->index())),
        ins);
}
...
void LIRGenerator::visitGetNextEntryForIterator(MGetNextEntryForIterator* ins) {
    ...
    auto lir = new (alloc()) LGetNextEntryForIterator(
        useRegister(ins->iter()), useRegister(ins->result()), temp(), temp(), temp());
    define(lir, ins);
    assignSafepoint(lir, ins);
}
...
void LIRGenerator::visitSetDisjointTypedElements(MSetDisjointTypedElements* ins) {
    ...
    auto lir = new (alloc()) LSetDisjointTypedElements(
        useRegister(target), useRegister(targetOffset), useRegister(source), temp());
    add(lir, ins);
}
...
void LIRGenerator::visitSetTypedObjectOffset(MSetTypedObjectOffset* ins) {
    add(new (alloc()) LSetTypedObjectOffset(useRegister(ins->object()), useRegister(ins->offset()),
                                            temp(), temp()),
        ins);
}
...
void LIRGenerator::visitBoundsCheck(MBoundsCheck* ins) {
    ...
    if (ins->minimum() || ins->maximum()) {
        check = new (alloc())
            LBoundsCheckRange(useRegisterOrConstant(ins->index()), useAny(ins->length()), temp());
    } else {
        check = new (alloc())
            LBoundsCheck(useRegisterOrConstant(ins->index()), useAnyOrConstant(ins->length()));
    }
...
void LIRGenerator::visitInArray(MInArray* ins) {
    ...
    LInArray* lir =
        new (alloc()) LInArray(useRegister(ins->elements()), useRegisterOrConstant(ins->index()),
...
void LIRGenerator::visitLoadElement(MLoadElement* ins) {
    ...
    switch (ins->type()) {
        case MIRType::Value: {
            LLoadElementV* lir = new (alloc())
                LLoadElementV(useRegister(ins->elements()), useRegisterOrConstant(ins->index()));
        ...
        default: {
            LLoadElementT* lir = new (alloc())
                LLoadElementT(useRegister(ins->elements()), useRegisterOrConstant(ins->index()));
            if (ins->fallible()) assignSnapshot(lir, Bailout_Hole);
            define(lir, ins);
            break;
        }
...
void LIRGenerator::visitLoadElementHole(MLoadElementHole* ins) {
    ...
    LLoadElementHole* lir = new (alloc())
        LLoadElementHole(useRegister(ins->elements()), useRegisterOrConstant(ins->index()),
...
void LIRGenerator::visitLoadUnboxedString(MLoadUnboxedString* ins) {
    ...
    LLoadUnboxedPointerT* lir = new (alloc())
        LLoadUnboxedPointerT(useRegister(ins->elements()), useRegisterOrConstant(ins->index()));
    define(lir, ins);
}
...
void LIRGenerator::visitLoadElementFromState(MLoadElementFromState* ins) {
    ...
    temp1 = temp();
#endif
    LLoadElementFromStateV* lir =
        new (alloc()) LLoadElementFromStateV(temp(), temp1, tempDouble());
...
void LIRGenerator::visitFallibleStoreElement(MFallibleStoreElement* ins) {
    ...
    switch (ins->value()->type()) {
        case MIRType::Value:
            lir = new (alloc())
                LFallibleStoreElementV(object, elements, index, useBox(ins->value()));
            break;
        default:
            const LAllocation value = useRegisterOrNonDoubleConstant(ins->value());
...
void LIRGenerator::visitStoreUnboxedObjectOrNull(MStoreUnboxedObjectOrNull* ins) {
    MOZ_ASSERT(IsValidElementsType(ins->elements(), ins->offsetAdjustment()));
    MOZ_ASSERT(ins->index()->type() == MIRType::Int32);
    MOZ_ASSERT(ins->value()->type() == MIRType::Object || ins->value()->type() == MIRType::Null ||
...
void LIRGenerator::visitConvertUnboxedObjectToNative(MConvertUnboxedObjectToNative* ins) {
...
void LIRGenerator::visitEffectiveAddress(MEffectiveAddress* ins) {
    define(new (alloc()) LEffectiveAddress(useRegister(ins->base()), useRegister(ins->index())),
           ins);
}
...
void LIRGenerator::visitArrayJoin(MArrayJoin* ins) {
    ...
    LArrayJoin* lir = new (alloc())
        LArrayJoin(useRegisterAtStart(ins->array()), useRegisterAtStart(ins->sep()), tempDef);
    defineReturn(lir, ins);
    assignSafepoint(lir, ins);
}
...
void LIRGenerator::visitSinCos(MSinCos* ins) {
    ...
    MOZ_ASSERT(ins->input()->type() == MIRType::Double ||
               ins->input()->type() == MIRType::Float32 || ins->input()->type() == MIRType::Int32);

    LSinCos* lir =
        new (alloc()) LSinCos(useRegisterAtStart(ins->input()), tempFixed(CallTempReg0), temp());
...
void LIRGenerator::visitStringSplit(MStringSplit* ins) {
    ...
    LStringSplit* lir = new (alloc())
        LStringSplit(useRegisterAtStart(ins->string()), useRegisterAtStart(ins->separator()));
    defineReturn(lir, ins);
    assignSafepoint(lir, ins);
}
...
void LIRGenerator::visitLoadUnboxedScalar(MLoadUnboxedScalar* ins) {
    ...
    LDefinition tempDef = LDefinition::BogusTemp();
    if (ins->readType() == Scalar::Uint32 && IsFloatingPointType(ins->type())) tempDef = temp();
...
void LIRGenerator::visitLoadTypedArrayElementHole(MLoadTypedArrayElementHole* ins) {
    MOZ_ASSERT(ins->object()->type() == MIRType::Object);
    MOZ_ASSERT(ins->index()->type() == MIRType::Int32);
...
void LIRGenerator::visitLoadTypedArrayElementStatic(MLoadTypedArrayElementStatic* ins) {
...
void LIRGenerator::visitStoreTypedArrayElementHole(MStoreTypedArrayElementHole* ins) {
    MOZ_ASSERT(ins->elements()->type() == MIRType::Elements);
    MOZ_ASSERT(ins->index()->type() == MIRType::Int32);
    MOZ_ASSERT(ins->length()->type() == MIRType::Int32);
...
void LIRGenerator::visitStoreFixedSlot(MStoreFixedSlot* ins) {
    ...
    if (ins->value()->type() == MIRType::Value) {
        LStoreFixedSlotV* lir =
            new (alloc()) LStoreFixedSlotV(useRegister(ins->object()), useBox(ins->value()));
        add(lir, ins);
    } else {
        LStoreFixedSlotT* lir = new (alloc())
            LStoreFixedSlotT(useRegister(ins->object()), useRegisterOrConstant(ins->value()));
        add(lir, ins);
    }
}
...
void LIRGenerator::visitGetPropSuperCache(MGetPropSuperCache* ins) {
    ...
    auto* lir = new (alloc()) LGetPropSuperCacheV(useRegister(obj), useBoxOrTyped(receiver),
...
void LIRGenerator::visitGetPropertyCache(MGetPropertyCache* ins) {
    ...
    if (ins->type() == MIRType::Value) {
        LGetPropertyCacheV* lir = new (alloc()) LGetPropertyCacheV(
            useBoxOrTyped(value), useBoxOrTypedOrConstant(id, useConstId), maybeTemp);
        defineBox(lir, ins);
        assignSafepoint(lir, ins);
    } else {
        LGetPropertyCacheT* lir = new (alloc()) LGetPropertyCacheT(
            useBoxOrTyped(value), useBoxOrTypedOrConstant(id, useConstId), maybeTemp);
        define(lir, ins);
        assignSafepoint(lir, ins);
    }
...
void LIRGenerator::visitSetPropertyPolymorphic(MSetPropertyPolymorphic* ins) {
    ...
    if (ins->value()->type() == MIRType::Value) {
        LSetPropertyPolymorphicV* lir = new (alloc())
            LSetPropertyPolymorphicV(useRegister(ins->object()), useBox(ins->value()), temp());
        assignSnapshot(lir, Bailout_ShapeGuard);
        add(lir, ins);
    } else {
...
void LIRGenerator::visitBindNameCache(MBindNameCache* ins) {
    ...
    LBindNameCache* lir =
        new (alloc()) LBindNameCache(useRegister(ins->environmentChain()), temp());
    define(lir, ins);
    assignSafepoint(lir, ins);
}
...
void LIRGenerator::visitGuardObjectIdentity(MGuardObjectIdentity* ins) {
    LGuardObjectIdentity* guard = new (alloc())
        LGuardObjectIdentity(useRegister(ins->object()), useRegister(ins->expected()));
    assignSnapshot(guard, Bailout_ObjectIdentityOrTypeGuard);
    add(guard, ins);
    redefine(ins, ins->object());
}

void LIRGenerator::visitGuardClass(MGuardClass* ins) {
    LDefinition t = temp();
...
void LIRGenerator::visitGuardReceiverPolymorphic(MGuardReceiverPolymorphic* ins) {
    MOZ_ASSERT(ins->object()->type() == MIRType::Object);
    MOZ_ASSERT(ins->type() == MIRType::Object);
...
void LIRGenerator::visitGuardUnboxedExpando(MGuardUnboxedExpando* ins) {
    LGuardUnboxedExpando* guard = new (alloc()) LGuardUnboxedExpando(useRegister(ins->object()));
...
void LIRGenerator::visitAssertRange(MAssertRange* ins) {
    ...
    switch (input->type()) {
        ...
        case MIRType::Value:
            lir = new (alloc())
                LAssertRangeV(useBox(input), tempToUnbox(), tempDouble(), tempDouble());
            break;
...
void LIRGenerator::visitCallGetElement(MCallGetElement* ins) {
    ...
    LCallGetElement* lir =
        new (alloc()) LCallGetElement(useBoxAtStart(ins->lhs()), useBoxAtStart(ins->rhs()));
    defineReturn(lir, ins);
    assignSafepoint(lir, ins);
}
...
void LIRGenerator::visitCallSetProperty(MCallSetProperty* ins) {
    LInstruction* lir = new (alloc())
        LCallSetProperty(useRegisterAtStart(ins->object()), useBoxAtStart(ins->value()));
    add(lir, ins);
    assignSafepoint(lir, ins);
}
...
void LIRGenerator::visitDeleteElement(MDeleteElement* ins) {
    LCallDeleteElement* lir =
        new (alloc()) LCallDeleteElement(useBoxAtStart(ins->value()), useBoxAtStart(ins->index()));
    defineReturn(lir, ins);
    assignSafepoint(lir, ins);
}
...
void LIRGenerator::visitSetPropertyCache(MSetPropertyCache* ins) {
    ...
    LInstruction* lir = new (alloc()) LSetPropertyCache(
        ...
        useBoxOrTypedOrConstant(ins->value(), useConstValue), temp(), tempD, tempF32);
    add(lir, ins);
    assignSafepoint(lir, ins);
}
...
void LIRGenerator::visitCallSetElement(MCallSetElement* ins) {
    ...
    LCallSetElement* lir =
        new (alloc()) LCallSetElement(useRegisterAtStart(ins->object()),
...
void LIRGenerator::visitCallInitElementArray(MCallInitElementArray* ins) {
    LCallInitElementArray* lir = new (alloc()) LCallInitElementArray(
        useRegisterAtStart(ins->object()), useRegisterOrConstantAtStart(ins->index()),
...
void LIRGenerator::visitIteratorEnd(MIteratorEnd* ins) {
    LIteratorEnd* lir =
        new (alloc()) LIteratorEnd(useRegister(ins->iterator()), temp(), temp(), temp());
    add(lir, ins);
    assignSafepoint(lir, ins);
}
...
void LIRGenerator::visitRest(MRest* ins) {
    ...
    LRest* lir = new (alloc())
        LRest(useFixedAtStart(ins->numActuals(), CallTempReg0), tempFixed(CallTempReg1),
...
void LIRGenerator::visitCallInstanceOf(MCallInstanceOf* ins) {
    MDefinition* lhs = ins->lhs();
    MDefinition* rhs = ins->rhs();
    ...
    LCallInstanceOf* lir =
        new (alloc()) LCallInstanceOf(useBoxAtStart(lhs), useRegisterAtStart(rhs));
    defineReturn(lir, ins);
    assignSafepoint(lir, ins);
}
...
void LIRGenerator::visitWasmBoundsCheck(MWasmBoundsCheck* ins) {
    ...
    auto* lir = new (alloc())
        LWasmBoundsCheck(useRegisterAtStart(index), useRegisterAtStart(boundsCheckLimit));
    add(lir, ins);
#endif
}
...
void LIRGenerator::visitSetDOMProperty(MSetDOMProperty* ins) {
    ...
    LSetDOMProperty* lir = new (alloc()) LSetDOMProperty(
        tempFixed(cxReg), useFixedAtStart(ins->object(), objReg),
        useBoxFixedAtStart(val, tempReg1, tempReg2), tempFixed(privReg), tempFixed(valueReg));
    add(lir, ins);
    assignSafepoint(lir, ins);
}
...
void LIRGenerator::visitGetDOMProperty(MGetDOMProperty* ins) {
    ...
    MOZ_ASSERT(ok, "How can we not have four temp registers?");
    LGetDOMProperty* lir =
        new (alloc()) LGetDOMProperty(tempFixed(cxReg), useFixedAtStart(ins->object(), objReg),
...
void LIRGenerator::visitThrowRuntimeLexicalError(MThrowRuntimeLexicalError* ins) {
    LThrowRuntimeLexicalError* lir = new (alloc()) LThrowRuntimeLexicalError();
    add(lir, ins);
    assignSafepoint(lir, ins);
}

void LIRGenerator::visitGlobalNameConflictsCheck(MGlobalNameConflictsCheck* ins) {
...
void LIRGenerator::visitCheckIsCallable(MCheckIsCallable* ins) {
    ...
    LCheckIsCallable* lir = new (alloc()) LCheckIsCallable(useBox(checkVal), temp());
    redefine(ins, checkVal);
    add(lir, ins);
    assignSafepoint(lir, ins);
}
...
void LIRGenerator::visitFinishBoundFunctionInit(MFinishBoundFunctionInit* ins) {
    auto lir = new (alloc())
        LFinishBoundFunctionInit(useRegister(ins->bound()), useRegister(ins->target()),
...
static void SpewResumePoint(MBasicBlock* block, MInstruction* ins, MResumePoint* resumePoint) {
...
bool LIRGenerator::visitBlock(MBasicBlock* block) {
    ...
    MOZ_ASSERT_IF(block->unreachable(),
                  *block->begin() == block->lastIns() || !mir()->optimizationInfo().gvnEnabled());
    MOZ_ASSERT_IF(block->unreachable(),
                  block->graph().osrBlock() || !mir()->optimizationInfo().gvnEnabled());
