...
LBoxAllocation LIRGeneratorX86::useBoxFixed(MDefinition* mir, Register reg1, Register reg2,
                                            bool useAtStart) {
...
LAllocation LIRGeneratorX86::useByteOpRegister(MDefinition* mir) { return useFixed(mir, eax); }

LAllocation LIRGeneratorX86::useByteOpRegisterAtStart(MDefinition* mir) {
    return useFixedAtStart(mir, eax);
}

LAllocation LIRGeneratorX86::useByteOpRegisterOrNonDoubleConstant(MDefinition* mir) {
...
void LIRGeneratorX86::lowerUntypedPhiInput(MPhi* phi, uint32_t inputPosition, LBlock* block,
                                           size_t lirIndex) {
...
void LIRGeneratorX86::lowerInt64PhiInput(MPhi* phi, uint32_t inputPosition, LBlock* block,
                                         size_t lirIndex) {
    MDefinition* operand = phi->getOperand(inputPosition);
...
void LIRGeneratorX86::lowerForALUInt64(LInstructionHelper<INT64_PIECES, 2 * INT64_PIECES, 0>* ins,
...
void LIRGeneratorX86::lowerForMulInt64(LMulI64* ins, MMul* mir, MDefinition* lhs,
                                       MDefinition* rhs) {
    bool needsTemp = true;
    ...
    defineInt64Fixed(
        ins, mir, LInt64Allocation(LAllocation(AnyRegister(edx)), LAllocation(AnyRegister(eax))));
}
...
void LIRGeneratorX86::visitAtomicExchangeTypedArrayElement(MAtomicExchangeTypedArrayElement* ins) {
    lowerAtomicExchangeTypedArrayElement(ins, /*useI386ByteRegisters=*/true);
}

void LIRGeneratorX86::visitAtomicTypedArrayElementBinop(MAtomicTypedArrayElementBinop* ins) {
...
void LIRGeneratorX86::visitWasmUnsignedToDouble(MWasmUnsignedToDouble* ins) {
    ...
    LWasmUint32ToDouble* lir =
        new (alloc()) LWasmUint32ToDouble(useRegisterAtStart(ins->input()), temp());
    define(lir, ins);
}
...
void LIRGeneratorX86::visitWasmUnsignedToFloat32(MWasmUnsignedToFloat32* ins) {
    ...
    LWasmUint32ToFloat32* lir =
        new (alloc()) LWasmUint32ToFloat32(useRegisterAtStart(ins->input()), temp());
    define(lir, ins);
}
...
void LIRGeneratorX86::visitWasmLoad(MWasmLoad* ins) {
    ...
    if (accessType == Scalar::Int8 || accessType == Scalar::Int16 || accessType == Scalar::Int32) {
        ...
        defineInt64Fixed(
            lir, ins,
            LInt64Allocation(LAllocation(AnyRegister(edx)), LAllocation(AnyRegister(eax))));
        return;
    }
...
void LIRGeneratorX86::visitWasmStore(MWasmStore* ins) {
    ...
    switch (ins->access().type()) {
        ...
        case Scalar::Int64: {
            ...
            auto* lir =
                new (alloc()) LWasmStoreI64(baseAlloc, valueAlloc, useRegisterAtStart(memoryBase));
            add(lir, ins);
            return;
        }
...
void LIRGeneratorX86::visitAsmJSLoadHeap(MAsmJSLoadHeap* ins) {
    ...
    LAllocation baseAlloc =
        ins->needsBoundsCheck() ? useRegisterAtStart(base) : useRegisterOrZeroAtStart(base);
    LAllocation limitAlloc =
        ins->needsBoundsCheck() ? useRegisterAtStart(boundsCheckLimit) : LAllocation();

    auto* lir =
        new (alloc()) LAsmJSLoadHeap(baseAlloc, limitAlloc, useRegisterAtStart(memoryBase));
    define(lir, ins);
}
...
void LIRGeneratorX86::visitAsmJSStoreHeap(MAsmJSStoreHeap* ins) {
    ...
    switch (ins->access().type()) {
        ...
        case Scalar::Uint8:
            // See comment for LIRGeneratorX86::useByteOpRegister.
            lir = new (alloc()) LAsmJSStoreHeap(baseAlloc, useFixed(ins->value(), eax), limitAlloc,
...
void LIRGeneratorX86::visitStoreTypedArrayElementStatic(MStoreTypedArrayElementStatic* ins) {
...
void LIRGeneratorX86::visitAsmJSCompareExchangeHeap(MAsmJSCompareExchangeHeap* ins) {
    ...
    LAsmJSCompareExchangeHeap* lir = new (alloc())
        LAsmJSCompareExchangeHeap(useRegister(base), oldval, newval, useRegister(memoryBase));

    lir->setAddrTemp(temp());
    defineFixed(lir, ins, LAllocation(AnyRegister(eax)));
}

void LIRGeneratorX86::visitAsmJSAtomicExchangeHeap(MAsmJSAtomicExchangeHeap* ins) {
...
void LIRGeneratorX86::visitAsmJSAtomicBinopHeap(MAsmJSAtomicBinopHeap* ins) {
    MOZ_ASSERT(ins->access().type() < Scalar::Float32);
    ...
    if (!ins->hasUses()) {
        ...
        LAsmJSAtomicBinopHeapForEffect* lir = new (alloc()) LAsmJSAtomicBinopHeapForEffect(
            useRegister(base), value, LDefinition::BogusTemp(), useRegister(memoryBase));
        lir->setAddrTemp(temp());
        add(lir, ins);
        return;
    }
    ...
    LAsmJSAtomicBinopHeap* lir = new (alloc()) LAsmJSAtomicBinopHeap(
        useRegister(base), value, tempDef, LDefinition::BogusTemp(), useRegister(memoryBase));
...
void LIRGeneratorX86::visitSubstr(MSubstr* ins) {
    ...
    LSubstr* lir = new (alloc())
        LSubstr(useRegister(ins->string()), useRegister(ins->begin()), useRegister(ins->length()),
                temp(), LDefinition::BogusTemp(), tempByteOpRegister());
    define(lir, ins);
...
void LIRGeneratorX86::visitInt64ToFloatingPoint(MInt64ToFloatingPoint* ins) {
    ...
    LDefinition maybeTemp =
        (ins->isUnsigned() && ((ins->type() == MIRType::Double && AssemblerX86Shared::HasSSE3()) ||
