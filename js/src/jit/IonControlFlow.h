...
class CFGAryControlInstruction : public CFGControlInstruction {
   ...
   public:
    ...
    CFGBlock* getSuccessor(size_t i) const final override { return successors_[i]; }
    void replaceSuccessor(size_t i, CFGBlock* succ) final override { successors_[i] = succ; }
};
...
class CFGTry : public CFGControlInstruction {
    ...
    CFGTry(CFGBlock* successor, jsbytecode* catchStartPc, CFGBlock* mergePoint)
        : tryBlock_(successor), catchStartPc_(catchStartPc), mergePoint_(mergePoint) {}

   public:
    CFG_CONTROL_HEADER(Try)
    TRIVIAL_CFG_NEW_WRAPPERS

    static CFGTry* CopyWithNewTargets(TempAllocator& alloc, CFGTry* old, CFGBlock* tryBlock,
                                      CFGBlock* merge) {
        return new (alloc) CFGTry(tryBlock, old->catchStartPc(), merge);
    }

    size_t numSuccessors() const final override { return 2; }
...
class CFGTableSwitch : public CFGControlInstruction {
   ...
   public:
    ...
    size_t numSuccessors() const final override { return successors_.length(); }
    CFGBlock* getSuccessor(size_t i) const final override {
        MOZ_ASSERT(i < numSuccessors());
...
class CFGCompare : public CFGAryControlInstruction<2> {
   ...
   public:
    ...
    static CFGCompare* CopyWithNewTargets(TempAllocator& alloc, CFGCompare* old, CFGBlock* succ1,
                                          CFGBlock* succ2) {
...
class CFGTest : public CFGAryControlInstruction<2> {
    ...
    CFGTest(CFGBlock* succ1, CFGBlock* succ2, bool keepCondition) : keepCondition_(keepCondition) {
        replaceSuccessor(0, succ1);
        replaceSuccessor(1, succ2);
    }
   ...
   public:
    ...
    static CFGTest* CopyWithNewTargets(TempAllocator& alloc, CFGTest* old, CFGBlock* succ1,
                                       CFGBlock* succ2) {
...
class CFGGoto : public CFGUnaryControlInstruction {
    ...
    explicit CFGGoto(CFGBlock* block) : CFGUnaryControlInstruction(block), popAmount_(0) {}
...
class ControlFlowGenerator {
   ...
   public:
    ...
    struct DeferredEdge : public TempObject {
        ...
        DeferredEdge(CFGBlock* block, DeferredEdge* next) : block(block), next(next) {}
    };
   ...
   private:
    ...
    MOZ_MUST_USE bool pushLoop(CFGState::State state, jsbytecode* stopAt, CFGBlock* entry,
                               jsbytecode* loopHead, jsbytecode* initialPc, jsbytecode* bodyStart,
                               jsbytecode* bodyEnd, jsbytecode* exitpc, jsbytecode* continuepc);
