...
static bool LoopContainsPossibleCall(MIRGraph& graph, MBasicBlock* header, MBasicBlock* backedge) {
...
static bool RequiresHoistedUse(const MDefinition* ins, bool hasCalls) {
    if (ins->isConstantElements()) return true;
    ...
    if (ins->isConstant() && (!IsFloatingPointType(ins->type()) || hasCalls)) return true;
...
static bool HasOperandInLoop(MInstruction* ins, bool hasCalls) {
    ...
    for (size_t i = 0, e = ins->numOperands(); i != e; ++i) {
        ...
        if (RequiresHoistedUse(op, hasCalls)) {
            ...
            // level.
            if (!HasOperandInLoop(op->toInstruction(), hasCalls)) continue;
        }
...
static bool HasDependencyInLoop(MInstruction* ins, MBasicBlock* header) {
    ...
    if (MDefinition* dep = ins->dependency()) return !IsBeforeLoop(dep, header);
    return false;
}
...
static void MoveDeferredOperands(MInstruction* ins, MInstruction* hoistPoint, bool hasCalls) {
...
static void VisitLoopBlock(MBasicBlock* block, MBasicBlock* header, MInstruction* hoistPoint,
                           bool hasCalls) {
...
#ifdef JS_JITSPEW
            JitSpew(JitSpew_LICM, "    %s%u will be hoisted only if its users are", ins->opName(),
                    ins->id());
#endif
            continue;
        }
...
#ifdef JS_JITSPEW
    JitSpew(JitSpew_LICM, "  Visiting loop with header block%u, hoisting to %s%u", header->id(),
            hoistPoint->opName(), hoistPoint->id());
#endif
...
bool jit::LICM(MIRGenerator* mir, MIRGraph& graph) {
    ...
    for (auto i(graph.rpoBegin()), e(graph.rpoEnd()); i != e; ++i) {
        ...
        if (mir->shouldCancel("LICM (main loop)")) return false;
    }
