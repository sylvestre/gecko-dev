...
void TrackedOptimizations::amendAttempt(uint32_t index) { currentAttempt_ = index; }
...
bool TrackedOptimizations::matchTypes(const TempOptimizationTypeInfoVector& other) const {
    return VectorContentsMatch(&types_, &other);
}

bool TrackedOptimizations::matchAttempts(const TempOptimizationAttemptsVector& other) const {
...
void SpewTempOptimizationTypeInfoVector(JitSpewChannel channel,
...
void SpewTempOptimizationAttemptsVector(JitSpewChannel channel,
                                        const TempOptimizationAttemptsVector* attempts,
                                        const char* indent = nullptr) {
#ifdef JS_JITSPEW
    for (const OptimizationAttempt* a = attempts->begin(); a != attempts->end(); a++) {
        JitSpew(channel, "   %s%s: %s", indent ? indent : "", TrackedStrategyString(a->strategy()),
...
bool OptimizationTypeInfo::trackType(TypeSet::Type type) { return types_.append(type); }
...
static inline HashNumber HashType(TypeSet::Type ty) {
    if (ty.isObjectUnchecked()) return PointerHasher<TypeSet::ObjectKey*>::hash(ty.objectKey());
    return mozilla::HashGeneric(ty.raw());
}

static HashNumber HashTypeList(const TempTypeList& types) {
    HashNumber h = 0;
    for (uint32_t i = 0; i < types.length(); i++) h = CombineHash(h, HashType(types[i]));
    return h;
}
...
/* static */ HashNumber UniqueTrackedOptimizations::Key::hash(const Lookup& lookup) {
...
/* static */ bool UniqueTrackedOptimizations::Key::match(const Key& key, const Lookup& lookup) {
    return VectorContentsMatch(key.attempts, lookup.attempts) &&
           VectorContentsMatch(key.types, lookup.types);
}

bool UniqueTrackedOptimizations::add(const TrackedOptimizations* optimizations) {
    MOZ_ASSERT(!sorted());
    Key key;
...
struct FrequencyComparator {
    bool operator()(const UniqueTrackedOptimizations::SortEntry& a,
                    const UniqueTrackedOptimizations::SortEntry& b, bool* lessOrEqualp) {
...
uint8_t UniqueTrackedOptimizations::indexOf(const TrackedOptimizations* optimizations) const {
    MOZ_ASSERT(sorted());
    Key key;
    key.types = &optimizations->types_;
    key.attempts = &optimizations->attempts_;
...
bool UniqueTrackedTypes::getIndexOf(JSContext* cx, TypeSet::Type ty, uint8_t* indexp) {
...
void IonTrackedOptimizationsRegion::RangeIterator::readNext(uint32_t* startOffset,
                                                            uint32_t* endOffset, uint8_t* index) {
    MOZ_ASSERT(more());
...
void JitcodeGlobalEntry::IonEntry::forEachOptimizationAttempt(
    JSRuntime* rt, uint8_t index, ForEachTrackedOptimizationAttemptOp& op) {
    trackedOptimizationAttempts(index).forEach(op);
}

void JitcodeGlobalEntry::IonEntry::forEachOptimizationTypeInfo(
    JSRuntime* rt, uint8_t index, IonTrackedOptimizationsTypeInfo::ForEachOpAdapter& op) {
    trackedOptimizationTypeInfo(index).forEach(op, allTrackedTypes());
}

void IonTrackedOptimizationsAttempts::forEach(ForEachTrackedOptimizationAttemptOp& op) {
    CompactBufferReader reader(start_, end_);
...
void IonTrackedOptimizationsTypeInfo::forEach(ForEachOp& op,
    ...
    while (cur != end_) {
        ...
        for (uint32_t i = 0; i < length; i++) op.readType((*allTypes)[reader.readByte()]);
        op(site, mirType);
        cur = reader.currentPosition();
        MOZ_ASSERT(cur <= end_);
    }
...
Maybe<uint8_t> IonTrackedOptimizationsRegion::findIndex(uint32_t offset,
...
Maybe<IonTrackedOptimizationsRegion> IonTrackedOptimizationsRegionTable::findRegion(
...
/* static */ uint32_t IonTrackedOptimizationsRegion::ExpectedRunLength(
    const NativeToTrackedOptimizations* start, const NativeToTrackedOptimizations* end) {
...
bool OptimizationTypeInfo::writeCompact(JSContext* cx, CompactBufferWriter& writer,
    ...
    for (uint32_t i = 0; i < types_.length(); i++) {
        uint8_t index;
        if (!uniqueTypes.getIndexOf(cx, types_[i], &index)) return false;
        writer.writeByte(index);
    }
    return true;
}

/* static */ void IonTrackedOptimizationsRegion::ReadDelta(CompactBufferReader& reader,
    ...
    uint64_t encVal =
        firstByte | secondByte << 8 | thirdByte << 16 | fourthByte << 24 | fifthByte << 32;
...
/* static */ void IonTrackedOptimizationsRegion::WriteDelta(CompactBufferWriter& writer,
                                                            uint32_t startDelta, uint32_t length,
                                                            uint8_t index) {
    // 2 bytes
    // SSSS-SSSL LLLL-LII0
    ...
    if (startDelta <= ENC4_START_DELTA_MAX && length <= ENC4_LENGTH_MAX) {
        ...
        uint64_t val = ENC4_MASK_VAL | (((uint64_t)startDelta) << ENC4_START_DELTA_SHIFT) |
...
/* static */ bool IonTrackedOptimizationsRegion::WriteRun(
    ...
    const NativeToTrackedOptimizations* end, const UniqueTrackedOptimizations& unique) {
    ...
    for (const NativeToTrackedOptimizations* entry = start + 1; entry != end; entry++) {
        ...
        JitSpew(JitSpew_OptimizationTrackingExtended,
                "     [%6u, %6u] delta [+%5u, +%5u] vector %3u, offset %4zu", startOffset,
                endOffset, startDelta, length, index, writer.length());
...
static bool WriteOffsetsTable(CompactBufferWriter& writer, const Vector<uint32_t, 16>& offsets,
    ...
    for (size_t i = 0; i < offsets.length(); i++) {
        JitSpew(JitSpew_OptimizationTrackingExtended, "   Entry %zu reverse offset %u", i,
...
static JSFunction* MaybeConstructorFromType(TypeSet::Type ty) {
    ...
    if (!newScript && obj->maybeUnboxedLayout()) newScript = obj->unboxedLayout().newScript();
    return newScript ? newScript->function() : nullptr;
}

static void InterpretedFunctionFilenameAndLineNumber(JSFunction* fun, const char** filename,
...
static void SpewAllocationSite(TypeSet::Type ty, JSScript* script, uint32_t offset) {
#ifdef JS_JITSPEW
...
bool jit::WriteIonTrackedOptimizationsTable(
    JSContext* cx, CompactBufferWriter& writer, const NativeToTrackedOptimizations* start,
    const NativeToTrackedOptimizations* end, const UniqueTrackedOptimizations& unique,
    uint32_t* numRegions, uint32_t* regionTableOffsetp, uint32_t* typesTableOffsetp,
    ...
    while (entry != end) {
        ...
        JitSpew(JitSpew_OptimizationTrackingExtended,
                "   Run at entry %zu, length %" PRIu32 ", offset %zu", size_t(entry - start),
                runLength, writer.length());
    ...
    *numRegions = offsets.length();
    ...
    for (const UniqueTrackedOptimizations::SortEntry* p = vec.begin(); p != vec.end(); p++) {
        ...
        JitSpew(JitSpew_OptimizationTrackingExtended,
                "   Type info entry %zu of length %zu, offset %zu", size_t(p - vec.begin()),
                v->length(), writer.length());
        ...
        for (const OptimizationTypeInfo* t = v->begin(); t != v->end(); t++) {
            if (!t->writeCompact(cx, writer, uniqueTypes)) return false;
        }
    }
    ...
    for (uint32_t i = 0; i < uniqueTypeList.length(); i++) {
        ...
        if (JSFunction* constructor = MaybeConstructorFromType(ty)) {
            if (!allTypes->append(IonTrackedTypeWithAddendum(ty, constructor))) return false;
    ...
    for (const UniqueTrackedOptimizations::SortEntry* p = vec.begin(); p != vec.end(); p++) {
        ...
        if (JitSpewEnabled(JitSpew_OptimizationTrackingExtended)) {
            JitSpew(JitSpew_OptimizationTrackingExtended,
                    "   Attempts entry %zu of length %zu, offset %zu", size_t(p - vec.begin()),
                    v->length(), writer.length());
...
void IonBuilder::trackOptimizationAttemptUnchecked(TrackedStrategy strategy) {
    BytecodeSite* site = current->trackedSite();
    // OOMs are handled as if optimization tracking were turned off.
    if (!site->optimizations()->trackAttempt(strategy)) site->setOptimizations(nullptr);
}
...
void IonBuilder::trackInlineSuccessUnchecked(InliningStatus status) {
    if (status == InliningStatus_Inlined) trackOptimizationOutcome(TrackedOutcome::Inlined);
}

static JSFunction* FunctionFromTrackedType(const IonTrackedTypeWithAddendum& tracked) {
...
void IonTrackedOptimizationsTypeInfo::ForEachOpAdapter::readType(
    ...
    if (JSFunction* fun = FunctionFromTrackedType(tracked)) {
        ...
        op_.readType(tracked.hasConstructor() ? "constructor" : "function", name, filename,
                     lineno);
        return;
    }
    ...
    if (tracked.hasAllocationSite()) {
        ...
        op_.readType("alloc site", buf, script->maybeForwardedScriptSource()->filename(),
...
void IonTrackedOptimizationsTypeInfo::ForEachOpAdapter::operator()(JS::TrackedTypeSite site,
                                                                   MIRType mirType) {
