...
uint32_t MacroAssembler::framePushed() const { return framePushed_; }

void MacroAssembler::setFramePushed(uint32_t framePushed) { framePushed_ = framePushed; }

void MacroAssembler::adjustFrame(int32_t value) {
    MOZ_ASSERT_IF(value < 0, framePushed_ >= uint32_t(-value));
    setFramePushed(framePushed_ + value);
}

void MacroAssembler::implicitPop(uint32_t bytes) {
...
void MacroAssembler::call(const wasm::CallSiteDesc& desc, wasm::SymbolicAddress imm) {
...
void MacroAssembler::passABIArg(Register reg) { passABIArg(MoveOperand(reg), MoveOp::GENERAL); }

void MacroAssembler::passABIArg(FloatRegister reg, MoveOp::Type type) {
    passABIArg(MoveOperand(reg), type);
}

void MacroAssembler::callWithABI(void* fun, MoveOp::Type result, CheckUnsafeCallWithABI check) {
...
void MacroAssembler::pushStaticFrameDescriptor(FrameType type, uint32_t headerSize) {
...
void MacroAssembler::PushStubCode() {
...
void MacroAssembler::enterExitFrame(Register cxreg, Register scratch, const VMFunction* f) {
...
void MacroAssembler::enterFakeExitFrame(Register cxreg, Register scratch, ExitFrameToken token) {
    linkExitFrame(cxreg, scratch);
    Push(Imm32(int32_t(token)));
    Push(ImmPtr(nullptr));
}

void MacroAssembler::enterFakeExitFrameForNative(Register cxreg, Register scratch,
                                                 bool isConstructing) {
    enterFakeExitFrame(
        cxreg, scratch,
        isConstructing ? ExitFrameToken::ConstructNative : ExitFrameToken::CallNative);
}
...
bool MacroAssembler::hasSelfReference() const { return selfReferencePatch_.bound(); }

// ===============================================================
// Move instructions

void MacroAssembler::moveValue(const ConstantOrRegister& src, const ValueOperand& dest) {
    if (src.constant()) {
...
void MacroAssembler::inc32(RegisterOrInt32Constant* key) {
    if (key->isRegister())
...
void MacroAssembler::dec32(RegisterOrInt32Constant* key) {
    if (key->isRegister())
        add32(Imm32(-1), key->reg());
    else
        key->bumpConstant(-1);
...
void MacroAssembler::branch32(Condition cond, Register length, const RegisterOrInt32Constant& key,
                              Label* label) {
    branch32Impl(cond, length, key, label);
}

void MacroAssembler::branch32(Condition cond, const Address& length,
                              const RegisterOrInt32Constant& key, Label* label) {
    branch32Impl(cond, length, key, label);
}

template <typename T>
void MacroAssembler::branch32Impl(Condition cond, const T& length,
                                  const RegisterOrInt32Constant& key, Label* label) {
    if (key.isRegister())
...
void MacroAssembler::branchIfRopeOrExternal(Register str, Register temp, Label* label) {
    Address flags(str, JSString::offsetOfFlags());
...
void MacroAssembler::branchIfObjectEmulatesUndefined(Register objReg, Register scratch,
...
void MacroAssembler::branchFunctionKind(Condition cond, JSFunction::FunctionKind kind,
...
void MacroAssembler::branchTestObjClass(Condition cond, Register obj, Register scratch,
...
void MacroAssembler::branchTestObjShape(Condition cond, Register obj, const Shape* shape,
                                        Label* label) {
    branchPtr(cond, Address(obj, ShapedObject::offsetOfShape()), ImmGCPtr(shape), label);
}

void MacroAssembler::branchTestObjShape(Condition cond, Register obj, Register shape,
                                        Label* label) {
    branchPtr(cond, Address(obj, ShapedObject::offsetOfShape()), shape, label);
}

void MacroAssembler::branchTestObjGroup(Condition cond, Register obj, ObjectGroup* group,
                                        Label* label) {
    branchPtr(cond, Address(obj, JSObject::offsetOfGroup()), ImmGCPtr(group), label);
}

void MacroAssembler::branchTestObjGroup(Condition cond, Register obj, Register group,
                                        Label* label) {
    branchPtr(cond, Address(obj, JSObject::offsetOfGroup()), group, label);
}

void MacroAssembler::branchTestClassIsProxy(bool proxy, Register clasp, Label* label) {
    branchTest32(proxy ? Assembler::NonZero : Assembler::Zero,
                 Address(clasp, Class::offsetOfFlags()), Imm32(JSCLASS_IS_PROXY), label);
}

void MacroAssembler::branchTestObjectIsProxy(bool proxy, Register object, Register scratch,
                                             Label* label) {
    loadObjClass(object, scratch);
    branchTestClassIsProxy(proxy, scratch, label);
}

void MacroAssembler::branchTestProxyHandlerFamily(Condition cond, Register proxy, Register scratch,
...
void MacroAssembler::branchTestMIRType(Condition cond, const Value& val, MIRType type,
                                       Label* label) {
    switch (type) {
        case MIRType::Null:
        ...
        case MIRType::Symbol:
            return branchTestSymbol(cond, val, label);
        case MIRType::Object:
        ...
        case MIRType::MagicHole:
            return branchTestMagic(cond, val, label);
        default:
            MOZ_CRASH("Bad MIRType");
    }
}

void MacroAssembler::branchTestNeedsIncrementalBarrier(Condition cond, Label* label) {
...
void MacroAssembler::branchTestMagicValue(Condition cond, const ValueOperand& val, JSWhyMagic why,
                                          Label* label) {
    MOZ_ASSERT(cond == Equal || cond == NotEqual);
    branchTestValue(cond, val, MagicValue(why), label);
}

void MacroAssembler::branchDoubleNotInInt64Range(Address src, Register temp, Label* fail) {
...
void MacroAssembler::branchDoubleNotInUInt64Range(Address src, Register temp, Label* fail) {
...
void MacroAssembler::branchFloat32NotInInt64Range(Address src, Register temp, Label* fail) {
...
void MacroAssembler::branchFloat32NotInUInt64Range(Address src, Register temp, Label* fail) {
    // Note: returns failure on -0.0
...
void MacroAssembler::assertStackAlignment(uint32_t alignment, int32_t offset /* = 0 */) {
#ifdef DEBUG
    Label ok, bad;
...
void MacroAssembler::storeCallResultValue(AnyRegister dest) { unboxValue(JSReturnOperand, dest); }
