...
static void AnalyzeLsh(TempAllocator& alloc, MLsh* lsh) {
    ...
    while (true) {
        ...
        if (!use->consumer()->isDefinition() || !use->consumer()->toDefinition()->isAdd()) break;
...
static void AnalyzeLoadUnboxedScalar(TempAllocator& alloc, MLoadUnboxedScalar* load) {
    ...
    int32_t c2 = 0;
    if (!SafeMul(constant->maybeConstantValue()->toInt32(), storageSize, &c2)) return;

    int32_t offset = 0;
    if (!SafeAdd(c1, c2, &offset)) return;

    JitSpew(JitSpew_EAA, "set offset: %d + %d = %d on: %s%u", c1, c2, offset, load->opName(),
            load->id());
...
bool EffectiveAddressAnalysis::tryAddDisplacement(AsmJSMemoryAccess* ins, int32_t o) {
...
void EffectiveAddressAnalysis::analyzeAsmJSHeapAccess(AsmJSMemoryAccess* ins) {
    ...
    if (base->isConstant()) {
        ...
        if (imm >= 0) {
            ...
            if (end >= imm && (uint32_t)end <= mir_->minWasmHeapLength()) ins->removeBoundsCheck();
        }
    } else if (base->isAdd()) {
