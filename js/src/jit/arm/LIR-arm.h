...
class LBoxFloatingPoint : public LInstructionHelper<2, 1, 1> {
   ...
   public:
    ...
    LBoxFloatingPoint(const LAllocation& in, const LDefinition& temp, MIRType type) : type_(type) {
        setOperand(0, in);
        setTemp(0, temp);
    }
...
class LSoftModI : public LBinaryCallInstructionHelper<1, 1> {
   public:
    ...
    LSoftModI(const LAllocation& lhs, const LAllocation& rhs, const LDefinition& temp) {
        setOperand(0, lhs);
        setOperand(1, rhs);
        setTemp(0, temp);
    }
...
class LAsmJSCompareExchangeCallout : public LCallInstructionHelper<1, 4, 2> {
   public:
    ...
    const LAllocation* oldval() { return getOperand(1); }
    const LAllocation* newval() { return getOperand(2); }
    const LAllocation* tls() { return getOperand(3); }

    const MAsmJSCompareExchangeHeap* mir() const { return mir_->toAsmJSCompareExchangeHeap(); }
};
...
class LAsmJSAtomicExchangeCallout : public LCallInstructionHelper<1, 3, 2> {
   public:
    ...
    const LAllocation* tls() { return getOperand(2); }

    const MAsmJSAtomicExchangeHeap* mir() const { return mir_->toAsmJSAtomicExchangeHeap(); }
};
...
class LAsmJSAtomicBinopCallout : public LCallInstructionHelper<1, 3, 2> {
   public:
    ...
    const MAsmJSAtomicBinopHeap* mir() const { return mir_->toAsmJSAtomicBinopHeap(); }
};
...
class LWasmTruncateToInt64 : public LCallInstructionHelper<INT64_PIECES, 1, 0> {
   public:
    ...
    LWasmTruncateToInt64(const LAllocation& in) { setOperand(0, in); }
