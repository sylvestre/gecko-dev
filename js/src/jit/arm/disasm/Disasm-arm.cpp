...
static int MOZ_FORMAT_PRINTF(2, 3) SNPrintF(V8Vector<char> str, const char* format, ...) {
    va_list args;
...
void Decoder::PrintShiftRm(Instruction* instr) {
    ...
    if (instr->RegShiftValue() == 0) {
        ...
        out_buffer_pos_ +=
            SNPrintF(out_buffer_ + out_buffer_pos_, ", %s ", shift_names[shift_index]);
        PrintRegister(rs);
    }
}
...
int Decoder::FormatVFPRegister(Instruction* instr, const char* format) {
    ...
    if (format[1] == 'n') {
        ...
        if ((instr->TypeValue() == 7) && (instr->Bit(24) == 0x0) && (instr->Bits(11, 9) == 0x5) &&
            (instr->Bit(4) == 0x1)) {
...
void Decoder::FormatNeonList(int Vd, int type) {
    if (type == nlt_1) {
        ...
        out_buffer_pos_ += SNPrintF(out_buffer_ + out_buffer_pos_, "{d%d, d%d}", Vd, Vd + 1);
    } else if (type == nlt_3) {
        out_buffer_pos_ +=
            SNPrintF(out_buffer_ + out_buffer_pos_, "{d%d, d%d, d%d}", Vd, Vd + 1, Vd + 2);
    } else if (type == nlt_4) {
        out_buffer_pos_ += SNPrintF(out_buffer_ + out_buffer_pos_, "{d%d, d%d, d%d, d%d}", Vd,
                                    Vd + 1, Vd + 2, Vd + 3);
    }
}
...
void Decoder::FormatNeonMemory(int Rn, int align, int Rm) {
    ...
    if (align != 0) {
        out_buffer_pos_ += SNPrintF(out_buffer_ + out_buffer_pos_, ":%d", (1 << align) << 6);
    }
    if (Rm == 15) {
        Print("]");
    } else if (Rm == 13) {
...
int Decoder::FormatOption(Instruction* instr, const char* format) {
    switch (format[0]) {
        ...
        case 'f': {  // 'f: bitfield instructions - v7 and above.
            ...
            out_buffer_pos_ += SNPrintF(out_buffer_ + out_buffer_pos_, "#%d, #%d", lsbit, width);
            return 1;
        }
        ...
        case 'i': {  // 'i: immediate value from adjacent bits.
            ...
            out_buffer_pos_ +=
                SNPrintF(out_buffer_ + out_buffer_pos_, "%d", instr->Bits(width + lsb - 1, lsb));
            return 8;
        }
        ...
        case 'm': {
            ...
            out_buffer_pos_ +=
                SNPrintF(out_buffer_ + out_buffer_pos_, "%s", converter_.NameInCode(str));
            return 3;
        }
        case 'o': {
            if ((format[3] == '1') && (format[4] == '2')) {
                ...
                out_buffer_pos_ +=
                    SNPrintF(out_buffer_ + out_buffer_pos_, "%d", instr->Offset12Value());
                return 5;
            } else if (format[3] == '0') {
        ...
        case 't': {  // 'target: target of branch instructions
            ...
            out_buffer_pos_ +=
                SNPrintF(out_buffer_ + out_buffer_pos_, "%+d -> %s", off,
    ...
    if (instr->Bit(4) == 0) {
        if (instr->Opc1Value() == 0x7) {
            ...
            if ((instr->Opc2Value() == 0x0) && (instr->Opc3Value() == 0x1)) {
                ...
                out_buffer_pos_ += SNPrintF(out_buffer_ + out_buffer_pos_, ", #%d", fraction_bits);
    ...
    switch (instr->SpecialValue()) {
        case 5:
            if ((instr->Bits(18, 16) == 0) && (instr->Bits(11, 6) == 0x28) &&
                ...
                out_buffer_pos_ += SNPrintF(out_buffer_ + out_buffer_pos_, "vmovl.s%d q%d, d%d",
                                            imm3 * 8, Vd, Vm);
            } else {
                Unknown(instr);
            }
        ...
        case 7:
            if ((instr->Bits(18, 16) == 0) && (instr->Bits(11, 6) == 0x28) &&
                ...
                int Vm = (instr->Bit(5) << 4) | instr->VmValue();
                int imm3 = instr->Bits(21, 19);
                out_buffer_pos_ += SNPrintF(out_buffer_ + out_buffer_pos_, "vmovl.u%d q%d, d%d",
                                            imm3 * 8, Vd, Vm);
            } else {
                Unknown(instr);
            }
        ...
        case 8:
            if (instr->Bits(21, 20) == 0) {
                ...
                out_buffer_pos_ +=
                    SNPrintF(out_buffer_ + out_buffer_pos_, "vst1.%d ", (1 << size) << 3);
        ...
        case 0xB:
            if ((instr->Bits(22, 20) == 5) && (instr->Bits(15, 12) == 0xf)) {
                ...
                if (offset == 0) {
                    out_buffer_pos_ += SNPrintF(out_buffer_ + out_buffer_pos_, "pld [r%d]", Rn);
                } else if (instr->Bit(23) == 0) {
                    out_buffer_pos_ +=
                        SNPrintF(out_buffer_ + out_buffer_pos_, "pld [r%d, #-%d]", Rn, offset);
                } else {
                    out_buffer_pos_ +=
                        SNPrintF(out_buffer_ + out_buffer_pos_, "pld [r%d, #+%d]", Rn, offset);
                }
        ...
        case 0x1D:
            if (instr->Opc1Value() == 0x7 && instr->Bits(19, 18) == 0x2 &&
                instr->Bits(11, 9) == 0x5 && instr->Bits(7, 6) == 0x1 && instr->Bit(4) == 0x0) {
...
int Decoder::InstructionDecode(uint8_t* instr_ptr) {
    ...
    out_buffer_pos_ +=
        SNPrintF(out_buffer_ + out_buffer_pos_, "%08x       ", instr->InstructionBits());
    ...
    if ((instruction_bits & kConstantPoolMarkerMask) == kConstantPoolMarker) {
        out_buffer_pos_ +=
            SNPrintF(out_buffer_ + out_buffer_pos_, "constant pool begin (length %d)",
        ...
        DecodeType2(instr);
        SNPrintF(out_buffer_ + out_buffer_pos_, " (0x%08x)", target->InstructionBits());
        return 2 * Instruction::kInstrSize;
    }
...
const char* NameConverter::NameOfConstant(uint8_t* addr) const { return NameOfAddress(addr); }
...
Disassembler::Disassembler(const NameConverter& converter) : converter_(converter) {}

Disassembler::~Disassembler() {}

int Disassembler::InstructionDecode(V8Vector<char> buffer, uint8_t* instruction) {
...
void Disassembler::Disassemble(FILE* f, uint8_t* begin, uint8_t* end) {
    ...
    for (uint8_t* pc = begin; pc < end;) {
        ...
        uint8_t* prev_pc = pc;
        pc += d.InstructionDecode(buffer, pc);
        fprintf(f, "%p    %08x      %s\n", prev_pc, *reinterpret_cast<int32_t*>(prev_pc),
                buffer.start());
    }
}
