...
void MacroAssembler::alignFrameForICArguments(MacroAssembler::AfterICSaveLive& aic) {
    // Exists for MIPS compatibility.
}

void MacroAssembler::restoreFrameAlignmentForICArguments(MacroAssembler::AfterICSaveLive& aic) {
    // Exists for MIPS compatibility.
}
...
BufferOffset MacroAssemblerCompat::movePatchablePtr(ImmPtr ptr, Register dest) {
    ...
    return allocEntry(numInst, numPoolEntries, (uint8_t*)&instructionScratch, literalAddr);
}

BufferOffset MacroAssemblerCompat::movePatchablePtr(ImmWord ptr, Register dest) {
    const size_t numInst = 1;           // Inserting one load instruction.
    const unsigned numPoolEntries = 2;  // Every pool entry is 4 bytes.
...
void MacroAssemblerCompat::handleFailureWithHandlerTail(void* handler) {
    ...
    Ldr(GetStackPointer64(),
        MemOperand(GetStackPointer64(), offsetof(ResumeFromException, stackPointer)));
    syncStackPtr();
...
void MacroAssemblerCompat::profilerEnterFrame(Register framePtr, Register scratch) {
...
template <typename T>
void MacroAssemblerCompat::compareExchangeToTypedIntArray(Scalar::Type arrayType, const T& mem,
...
template void MacroAssemblerCompat::compareExchangeToTypedIntArray(Scalar::Type arrayType,
                                                                   const Address& mem,
                                                                   Register oldval,
...
template <typename T>
void MacroAssemblerCompat::atomicExchangeToTypedIntArray(Scalar::Type arrayType, const T& mem,
...
template void MacroAssemblerCompat::atomicExchangeToTypedIntArray(Scalar::Type arrayType,
                                                                  const Address& mem,
                                                                  Register value, Register temp,
                                                                  AnyRegister output);
template void MacroAssemblerCompat::atomicExchangeToTypedIntArray(Scalar::Type arrayType,
...
void MacroAssembler::Push(Register reg1, Register reg2, Register reg3, Register reg4) {
...
void MacroAssembler::Pop(FloatRegister f) { MOZ_CRASH("NYI: Pop(FloatRegister)"); }
...
void MacroAssembler::patchCall(uint32_t callerOffset, uint32_t calleeOffset) { MOZ_CRASH("NYI"); }

CodeOffset MacroAssembler::farJumpWithPatch() { MOZ_CRASH("NYI"); }

void MacroAssembler::patchFarJump(CodeOffset farJump, uint32_t targetOffset) { MOZ_CRASH("NYI"); }

void MacroAssembler::repatchFarJump(uint8_t* code, uint32_t farJumpOffset, uint32_t targetOffset) {
    MOZ_CRASH("NYI");
}

CodeOffset MacroAssembler::nopPatchableToNearJump() { MOZ_CRASH("NYI"); }

void MacroAssembler::patchNopToNearJump(uint8_t* jump, uint8_t* target) { MOZ_CRASH("NYI"); }

void MacroAssembler::patchNearJumpToNop(uint8_t* jump) { MOZ_CRASH("NYI"); }
...
void MacroAssembler::callWithABIPost(uint32_t stackAdjust, MoveOp::Type result,
    ...
    if (dynamicAlignment_) Ldr(GetStackPointer64(), MemOperand(GetStackPointer64(), 0));
