...
struct CatchFinallyRange {
    ...
    bool contains(uint32_t offset) const { return start <= offset && offset < end; }
};
...
bool BytecodeAnalysis::init(TempAllocator& alloc, GSNCache& gsn) {
    ...
    for (jsbytecode* pc = script_->code(); pc < end; pc = nextpc) {
        ...
        JitSpew(JitSpew_BaselineOp, "Analyzing op @ %d (end=%d): %s", int(script_->pcToOffset(pc)),
                int(script_->length()), CodeName[op]);
