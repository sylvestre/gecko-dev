...
Register js::jit::toR(Instruction& i) { return Register::FromCode(i.encode() & RegMask); }
...
uint8_t* AssemblerMIPSShared::PatchableJumpAddress(JitCode* code, uint32_t pe_) {
    return code->raw() + pe_;
}
...
AssemblerMIPSShared::Condition AssemblerMIPSShared::InvertCondition(Condition cond) {
    switch (cond) {
        case Equal:
            return NotEqual;
        case NotEqual:
...
AssemblerMIPSShared::DoubleCondition AssemblerMIPSShared::InvertCondition(DoubleCondition cond) {
    switch (cond) {
        case DoubleOrdered:
...
size_t AssemblerMIPSShared::jumpRelocationTableBytes() const { return jumpRelocations_.length(); }

size_t AssemblerMIPSShared::dataRelocationTableBytes() const { return dataRelocations_.length(); }
...
// Logical operations.
BufferOffset AssemblerMIPSShared::as_and(Register rd, Register rs, Register rt) {
...
BufferOffset AssemblerMIPSShared::as_xor(Register rd, Register rs, Register rt) {
    spew("xor    %3s,%3s,%3s", rd.name(), rs.name(), rt.name());
    return writeInst(InstReg(op_special, rs, rt, rd, ff_xor).encode());
}

BufferOffset AssemblerMIPSShared::as_nor(Register rd, Register rs, Register rt) {
...
InstImm AssemblerMIPSShared::getBranchCode(JumpOrCall jumpOrCall) {
    if (jumpOrCall == BranchIsCall) return InstImm(op_regimm, zero, rt_bgezal, BOffImm16(0));

    return InstImm(op_beq, zero, zero, BOffImm16(0));
}

InstImm AssemblerMIPSShared::getBranchCode(Register s, Register t, Condition c) {
...
InstImm AssemblerMIPSShared::getBranchCode(FloatTestKind testKind, FPConditionBit fcc) {
...
BufferOffset AssemblerMIPSShared::as_addu(Register rd, Register rs, Register rt) {
    spew("addu   %3s,%3s,%3s", rd.name(), rs.name(), rt.name());
    return writeInst(InstReg(op_special, rs, rt, rd, ff_addu).encode());
}

BufferOffset AssemblerMIPSShared::as_addiu(Register rd, Register rs, int32_t j) {
...
BufferOffset AssemblerMIPSShared::as_daddu(Register rd, Register rs, Register rt) {
    spew("daddu  %3s,%3s,%3s", rd.name(), rs.name(), rt.name());
    return writeInst(InstReg(op_special, rs, rt, rd, ff_daddu).encode());
}

BufferOffset AssemblerMIPSShared::as_daddiu(Register rd, Register rs, int32_t j) {
    MOZ_ASSERT(Imm16::IsInSignedRange(j));
    spew("daddiu %3s,%3s,0x%x", rd.name(), rs.name(), j);
    return writeInst(InstImm(op_daddiu, rs, rd, Imm16(j)).encode());
}

BufferOffset AssemblerMIPSShared::as_subu(Register rd, Register rs, Register rt) {
    spew("subu   %3s,%3s,%3s", rd.name(), rs.name(), rt.name());
    return writeInst(InstReg(op_special, rs, rt, rd, ff_subu).encode());
}

BufferOffset AssemblerMIPSShared::as_dsubu(Register rd, Register rs, Register rt) {
...
BufferOffset AssemblerMIPSShared::as_drotr(Register rd, Register rt, uint16_t sa) {
    MOZ_ASSERT(sa < 32);
...
BufferOffset AssemblerMIPSShared::as_drotr32(Register rd, Register rt, uint16_t sa) {
    MOZ_ASSERT(31 < sa && sa < 64);
...
BufferOffset AssemblerMIPSShared::as_rotrv(Register rd, Register rt, Register rs) {
    spew("rotrv  %3s,%3s,%3s", rd.name(), rt.name(), rs.name());
    MOZ_ASSERT(hasR2());
    return writeInst(InstReg(op_special, rs, rt, rd, 1, ff_srlv).encode());
}

BufferOffset AssemblerMIPSShared::as_drotrv(Register rd, Register rt, Register rs) {
...
BufferOffset AssemblerMIPSShared::as_lbu(Register rd, Register rs, int16_t off) {
...
BufferOffset AssemblerMIPSShared::as_lhu(Register rd, Register rs, int16_t off) {
...
BufferOffset AssemblerMIPSShared::as_lwu(Register rd, Register rs, int16_t off) {
    spew("lwu    %3s, (0x%x)%2s", rd.name(), off, rs.name());
    return writeInst(InstImm(op_lwu, rs, rd, Imm16(off)).encode());
}

BufferOffset AssemblerMIPSShared::as_lwl(Register rd, Register rs, int16_t off) {
    spew("lwl    %3s, (0x%x)%2s", rd.name(), off, rs.name());
    return writeInst(InstImm(op_lwl, rs, rd, Imm16(off)).encode());
}

BufferOffset AssemblerMIPSShared::as_lwr(Register rd, Register rs, int16_t off) {
...
BufferOffset AssemblerMIPSShared::as_ldl(Register rd, Register rs, int16_t off) {
    spew("ldl    %3s, (0x%x)%2s", rd.name(), off, rs.name());
    return writeInst(InstImm(op_ldl, rs, rd, Imm16(off)).encode());
}

BufferOffset AssemblerMIPSShared::as_ldr(Register rd, Register rs, int16_t off) {
...
BufferOffset AssemblerMIPSShared::as_swl(Register rd, Register rs, int16_t off) {
    spew("swl    %3s, (0x%x)%2s", rd.name(), off, rs.name());
    return writeInst(InstImm(op_swl, rs, rd, Imm16(off)).encode());
}

BufferOffset AssemblerMIPSShared::as_swr(Register rd, Register rs, int16_t off) {
...
BufferOffset AssemblerMIPSShared::as_sdl(Register rd, Register rs, int16_t off) {
    spew("sdl    %3s, (0x%x)%2s", rd.name(), off, rs.name());
    return writeInst(InstImm(op_sdl, rs, rd, Imm16(off)).encode());
}

BufferOffset AssemblerMIPSShared::as_sdr(Register rd, Register rs, int16_t off) {
    spew("sdr    %3s, (0x%x)%2s", rd.name(), off, rs.name());
    return writeInst(InstImm(op_sdr, rs, rd, Imm16(off)).encode());
}

BufferOffset AssemblerMIPSShared::as_gslbx(Register rd, Register rs, Register ri, int16_t off) {
...
BufferOffset AssemblerMIPSShared::as_gssq(Register rh, Register rl, Register rs, int16_t off) {
    MOZ_ASSERT(GSImm13::IsInRange(off));
...
// Set on less than.
BufferOffset AssemblerMIPSShared::as_slt(Register rd, Register rs, Register rt) {
    spew("slt    %3s,%3s,%3s", rd.name(), rs.name(), rt.name());
    return writeInst(InstReg(op_special, rs, rt, rd, ff_slt).encode());
}

BufferOffset AssemblerMIPSShared::as_sltu(Register rd, Register rs, Register rt) {
...
BufferOffset AssemblerMIPSShared::as_sltiu(Register rd, Register rs, uint32_t j) {
    MOZ_ASSERT(Imm16::IsInUnsignedRange(j));
...
// Conditional move.
BufferOffset AssemblerMIPSShared::as_movz(Register rd, Register rs, Register rt) {
    spew("movz   %3s,%3s,%3s", rd.name(), rs.name(), rt.name());
    return writeInst(InstReg(op_special, rs, rt, rd, ff_movz).encode());
}

BufferOffset AssemblerMIPSShared::as_movn(Register rd, Register rs, Register rt) {
    spew("movn   %3s,%3s,%3s", rd.name(), rs.name(), rt.name());
    return writeInst(InstReg(op_special, rs, rt, rd, ff_movn).encode());
}

BufferOffset AssemblerMIPSShared::as_movt(Register rd, Register rs, uint16_t cc) {
    Register rt;
...
BufferOffset AssemblerMIPSShared::as_movf(Register rd, Register rs, uint16_t cc) {
    Register rt;
...
BufferOffset AssemblerMIPSShared::as_ins(Register rt, Register rs, uint16_t pos, uint16_t size) {
...
BufferOffset AssemblerMIPSShared::as_dins(Register rt, Register rs, uint16_t pos, uint16_t size) {
    MOZ_ASSERT(pos < 32 && size != 0 && size <= 32 && pos + size != 0 && pos + size <= 32);
    Register rd;
    rd = Register::FromCode(pos + size - 1);
...
BufferOffset AssemblerMIPSShared::as_dinsm(Register rt, Register rs, uint16_t pos, uint16_t size) {
...
BufferOffset AssemblerMIPSShared::as_dinsu(Register rt, Register rs, uint16_t pos, uint16_t size) {
    MOZ_ASSERT(pos >= 32 && pos < 64 && size >= 1 && size <= 32 && pos + size > 32 &&
               pos + size <= 64);
    Register rd;
    rd = Register::FromCode(pos + size - 1 - 32);
...
BufferOffset AssemblerMIPSShared::as_ext(Register rt, Register rs, uint16_t pos, uint16_t size) {
    MOZ_ASSERT(pos < 32 && size != 0 && size <= 32 && pos + size != 0 && pos + size <= 32);
    Register rd;
...
BufferOffset AssemblerMIPSShared::as_dext(Register rt, Register rs, uint16_t pos, uint16_t size) {
...
BufferOffset AssemblerMIPSShared::as_dextm(Register rt, Register rs, uint16_t pos, uint16_t size) {
...
BufferOffset AssemblerMIPSShared::as_dextu(Register rt, Register rs, uint16_t pos, uint16_t size) {
    MOZ_ASSERT(pos >= 32 && pos < 64 && size != 0 && size <= 32 && pos + size > 32 &&
               pos + size <= 64);
    Register rd;
    rd = Register::FromCode(size - 1);
...
// FP instructions
BufferOffset AssemblerMIPSShared::as_ld(FloatRegister fd, Register base, int32_t off) {
...
BufferOffset AssemblerMIPSShared::as_gsssx(FloatRegister fd, Register rs, Register ri,
                                           int16_t off) {
    MOZ_ASSERT(Imm8::IsInSignedRange(off));
...
BufferOffset AssemblerMIPSShared::as_gsldx(FloatRegister fd, Register rs, Register ri,
                                           int16_t off) {
    MOZ_ASSERT(Imm8::IsInSignedRange(off));
...
BufferOffset AssemblerMIPSShared::as_gssdx(FloatRegister fd, Register rs, Register ri,
                                           int16_t off) {
    MOZ_ASSERT(Imm8::IsInSignedRange(off));
...
BufferOffset AssemblerMIPSShared::as_gslq(FloatRegister rh, FloatRegister rl, Register rs,
                                          int16_t off) {
    MOZ_ASSERT(GSImm13::IsInRange(off));
    spew("gslq   %3s,%3s, (0x%x)%2s", rh.name(), rl.name(), off, rs.name());
...
BufferOffset AssemblerMIPSShared::as_gssq(FloatRegister rh, FloatRegister rl, Register rs,
                                          int16_t off) {
    MOZ_ASSERT(GSImm13::IsInRange(off));
    spew("gssq   %3s,%3s, (0x%x)%2s", rh.name(), rl.name(), off, rs.name());
...
BufferOffset AssemblerMIPSShared::as_adds(FloatRegister fd, FloatRegister fs, FloatRegister ft) {
    spew("add.s  %3s,%3s,%3s", fd.name(), fs.name(), ft.name());
    return writeInst(InstReg(op_cop1, rs_s, ft, fs, fd, ff_add_fmt).encode());
}

BufferOffset AssemblerMIPSShared::as_addd(FloatRegister fd, FloatRegister fs, FloatRegister ft) {
    spew("add.d  %3s,%3s,%3s", fd.name(), fs.name(), ft.name());
    return writeInst(InstReg(op_cop1, rs_d, ft, fs, fd, ff_add_fmt).encode());
}

BufferOffset AssemblerMIPSShared::as_subs(FloatRegister fd, FloatRegister fs, FloatRegister ft) {
    spew("sub.s  %3s,%3s,%3s", fd.name(), fs.name(), ft.name());
    return writeInst(InstReg(op_cop1, rs_s, ft, fs, fd, ff_sub_fmt).encode());
}

BufferOffset AssemblerMIPSShared::as_subd(FloatRegister fd, FloatRegister fs, FloatRegister ft) {
...
BufferOffset AssemblerMIPSShared::as_muls(FloatRegister fd, FloatRegister fs, FloatRegister ft) {
    spew("mul.s  %3s,%3s,%3s", fd.name(), fs.name(), ft.name());
    return writeInst(InstReg(op_cop1, rs_s, ft, fs, fd, ff_mul_fmt).encode());
}

BufferOffset AssemblerMIPSShared::as_muld(FloatRegister fd, FloatRegister fs, FloatRegister ft) {
    spew("mul.d  %3s,%3s,%3s", fd.name(), fs.name(), ft.name());
    return writeInst(InstReg(op_cop1, rs_d, ft, fs, fd, ff_mul_fmt).encode());
}

BufferOffset AssemblerMIPSShared::as_divs(FloatRegister fd, FloatRegister fs, FloatRegister ft) {
    spew("div.s  %3s,%3s,%3s", fd.name(), fs.name(), ft.name());
    return writeInst(InstReg(op_cop1, rs_s, ft, fs, fd, ff_div_fmt).encode());
}

BufferOffset AssemblerMIPSShared::as_divd(FloatRegister fd, FloatRegister fs, FloatRegister ft) {
...
BufferOffset AssemblerMIPSShared::as_cf(FloatFormat fmt, FloatRegister fs, FloatRegister ft,
                                        FPConditionBit fcc) {
    if (fmt == DoubleFloat) {
...
BufferOffset AssemblerMIPSShared::as_cun(FloatFormat fmt, FloatRegister fs, FloatRegister ft,
                                         FPConditionBit fcc) {
    if (fmt == DoubleFloat) {
...
BufferOffset AssemblerMIPSShared::as_ceq(FloatFormat fmt, FloatRegister fs, FloatRegister ft,
                                         FPConditionBit fcc) {
    if (fmt == DoubleFloat) {
...
BufferOffset AssemblerMIPSShared::as_cueq(FloatFormat fmt, FloatRegister fs, FloatRegister ft,
                                          FPConditionBit fcc) {
    if (fmt == DoubleFloat) {
...
BufferOffset AssemblerMIPSShared::as_colt(FloatFormat fmt, FloatRegister fs, FloatRegister ft,
                                          FPConditionBit fcc) {
    if (fmt == DoubleFloat) {
...
BufferOffset AssemblerMIPSShared::as_cult(FloatFormat fmt, FloatRegister fs, FloatRegister ft,
                                          FPConditionBit fcc) {
    if (fmt == DoubleFloat) {
...
BufferOffset AssemblerMIPSShared::as_cole(FloatFormat fmt, FloatRegister fs, FloatRegister ft,
                                          FPConditionBit fcc) {
    if (fmt == DoubleFloat) {
...
BufferOffset AssemblerMIPSShared::as_cule(FloatFormat fmt, FloatRegister fs, FloatRegister ft,
                                          FPConditionBit fcc) {
    if (fmt == DoubleFloat) {
...
// FP conditional move.
BufferOffset AssemblerMIPSShared::as_movt(FloatFormat fmt, FloatRegister fd, FloatRegister fs,
                                          FPConditionBit fcc) {
...
BufferOffset AssemblerMIPSShared::as_movf(FloatFormat fmt, FloatRegister fd, FloatRegister fs,
                                          FPConditionBit fcc) {
...
BufferOffset AssemblerMIPSShared::as_movz(FloatFormat fmt, FloatRegister fd, FloatRegister fs,
                                          Register rt) {
    if (fmt == DoubleFloat) {
...
BufferOffset AssemblerMIPSShared::as_movn(FloatFormat fmt, FloatRegister fd, FloatRegister fs,
                                          Register rt) {
    if (fmt == DoubleFloat) {
...
void AssemblerMIPSShared::bindLater(Label* label, wasm::TrapDesc target) {
    if (label->used()) {
        int32_t next;
...
void AssemblerMIPSShared::UpdateLuiOriValue(Instruction* inst0, Instruction* inst1,
                                            uint32_t value) {
...
void AssemblerMIPSShared::decodeBranchInstAndSpew(InstImm branch) {
    ...
    switch (op) {
        case op_beq:
            ...
            spew("beq    %3s,%3s,0x%x", Registers::GetName(rs_id), Registers::GetName(rt_id),
                 (int32_t(immi << 18) >> 16) + 4);
            break;
        case op_bne:
            rt_id = branch.extractRT();
            rs_id = branch.extractRS();
            spew("bne    %3s,%3s,0x%x", Registers::GetName(rs_id), Registers::GetName(rt_id),
                 (int32_t(immi << 18) >> 16) + 4);
            break;
        case op_bgtz:
            rs_id = branch.extractRS();
