...
class BaseAssemblerX64 : public BaseAssembler {
   ...
   private:
    MOZ_MUST_USE JmpSrc twoByteRipOpSimd(const char* name, VexOperandType ty,
                                         TwoByteOpcodeID opcode, XMMRegisterID src0,
                                         XMMRegisterID dst) {
        if (useLegacySSEEncoding(src0, dst)) {
            ...
            if (IsXMMReversedOperands(opcode))
                ...
                spew("%-11s" MEM_o32r ", %s", legacySSEOpName(name), ADDR_o32r(label.offset()),
                     XMMRegName(dst));
            return label;
        }
        ...
        if (src0 == invalid_xmm) {
            ...
            spew("%-11s" MEM_o32r ", %s, %s", name, ADDR_o32r(label.offset()), XMMRegName(src0),
                 XMMRegName(dst));
        }
        return label;
    }
