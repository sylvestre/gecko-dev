...
static MConstant* EvaluateConstantOperands(TempAllocator& alloc, MBinaryInstruction* ins,
    ...
    if (ins->type() == MIRType::Float32) return MConstant::NewFloat32(alloc, float(ret));
    if (ins->type() == MIRType::Double) return MConstant::New(alloc, DoubleValue(ret));
...
static MMul* EvaluateExactReciprocal(TempAllocator& alloc, MDiv* ins) {
    ...
    int32_t num;
    if (!mozilla::NumberIsInt32(right->toConstant()->numberToDouble(), &num)) return nullptr;
...
HashNumber MDefinition::valueHash() const {
    ...
    for (size_t i = 0, e = numOperands(); i < e; i++) out = addU32ToHash(out, getOperand(i)->id());
...
static bool MaybeCallable(CompilerConstraintList* constraints, MDefinition* op) {
...
void MTest::cacheOperandMightEmulateUndefined(CompilerConstraintList* constraints) {
    MOZ_ASSERT(operandMightEmulateUndefined());

    if (!getOperand(0)->maybeEmulatesUndefined(constraints)) markNoOperandEmulatesUndefined();
}
...
MDefinition* MTest::foldsDoubleNegation(TempAllocator& alloc) {
    ...
    if (op->isNot()) {
        ...
        if (opop->isNot()) return MTest::New(alloc, opop->toNot()->input(), ifTrue(), ifFalse());
...
MDefinition* MTest::foldsConstant(TempAllocator& alloc) {
    ...
    if (MConstant* opConst = op->maybeConstantValue()) {
        bool b;
        if (opConst->valueToBoolean(&b)) return MGoto::New(alloc, b ? ifTrue() : ifFalse());
    }
    return nullptr;
}
...
MDefinition* MTest::foldsNeedlessControlFlow(TempAllocator& alloc) {
    ...
    if (ifTrue()->numSuccessors() != 1 || ifFalse()->numSuccessors() != 1) return nullptr;
...
void MTest::filtersUndefinedOrNull(bool trueBranch, MDefinition** subject, bool* filtersUndefined,
                                   bool* filtersNull) {
    MDefinition* ins = getOperand(0);
    if (ins->isCompare()) {
        ins->toCompare()->filtersUndefinedOrNull(trueBranch, subject, filtersUndefined,
                                                 filtersNull);
        return;
    }
...
void MDefinition::dump(GenericPrinter& out) const {
    ...
    if (isInstruction()) {
        if (MResumePoint* resume = toInstruction()->resumePoint()) resume->dump(out);
    }
}
...
void MDefinition::replaceAllUsesWith(MDefinition* dom) {
    for (size_t i = 0, e = numOperands(); i < e; ++i) getOperand(i)->setUseRemovedUnchecked();
...
void MDefinition::justReplaceAllUsesWith(MDefinition* dom) {
    ...
    for (MUseIterator i(usesBegin()), e(usesEnd()); i != e; ++i) i->setProducerUnchecked(dom);
...
void MDefinition::replaceAllLiveUsesWith(MDefinition* dom) {
    for (MUseIterator i(usesBegin()), e(usesEnd()); i != e;) {
        ...
        if (consumer->isDefinition() && consumer->toDefinition()->isRecoveredOnBailout()) continue;
...
MConstant* MConstant::NewConstraintlessObject(TempAllocator& alloc, JSObject* v) {
    return new (alloc) MConstant(v);
}

static TemporaryTypeSet* MakeSingletonTypeSetFromKey(TempAllocator& tempAlloc,
                                                     CompilerConstraintList* constraints,
...
TemporaryTypeSet* jit::MakeSingletonTypeSet(TempAllocator& alloc,
                                            CompilerConstraintList* constraints, JSObject* obj) {
...
bool jit::IonCompilationCanUseNurseryPointers() {
    ...
    if (CurrentThreadIsIonCompiling()) return !CurrentThreadIsIonCompilingSafeForMinorGC();
...
bool MConstant::canProduceFloat32() const {
    ...
    if (type() == MIRType::Int32) return IsFloat32Representable(static_cast<double>(toInt32()));
...
HashNumber MWasmFloatConstant::valueHash() const { return ConstantValueHash(type(), u.bits_); }
    ...
    if (allConstants) {
        ...
        switch (type()) {
            ...
            case MIRType::Int32x4: {
                int32_t a[4];
                for (size_t i = 0; i < 4; ++i) a[i] = getOperand(i)->toConstant()->toInt32();
                cst = SimdConstant::CreateX4(a);
                break;
            }
...
MDefinition* MSimdGeneralShuffle::foldsTo(TempAllocator& alloc) {
    ...
    for (size_t i = 0; i < numLanes(); i++) {
        if (!lane(i)->isConstant() || lane(i)->type() != MIRType::Int32) return this;
        int32_t temp = lane(i)->toConstant()->toInt32();
        if (temp < 0 || unsigned(temp) >= numLanes() * numVectors()) return this;
        lanes[i] = uint8_t(temp);
    }

    if (numVectors() == 1) return MSimdSwizzle::New(alloc, vector(0), lanes.data());

    MOZ_ASSERT(numVectors() == 2);
    return MSimdShuffle::New(alloc, vector(0), vector(1), lanes.data());
}

MInstruction* MSimdConvert::AddLegalized(TempAllocator& alloc, MBasicBlock* addTo,
                                         MDefinition* obj, MIRType toType, SimdSign sign,
...
MInstruction* MSimdBinaryComp::AddLegalized(TempAllocator& alloc, MBasicBlock* addTo,
                                            MDefinition* left, MDefinition* right, Operation op,
                                            SimdSign sign) {
    ...
    if (sign == SimdSign::Unsigned && !IsEquality) {
        ...
        if (bias) {
            ...
            MInstruction* bright = MSimdBinaryArith::AddLegalized(alloc, addTo, right, bias,
                                                                  MSimdBinaryArith::Op_add);
...
MInstruction* MSimdBinaryArith::AddLegalized(TempAllocator& alloc, MBasicBlock* addTo,
                                             MDefinition* left, MDefinition* right, Operation op) {
    MOZ_ASSERT(left->type() == right->type());
    MIRType opType = left->type();
    MOZ_ASSERT(IsSimdType(opType));
...
MInstruction* MSimdShift::AddLegalized(TempAllocator& alloc, MBasicBlock* addTo, MDefinition* left,
...
void MSimdBinaryArith::printOpcode(GenericPrinter& out) const { PrintOpcodeOperation(this, out); }
...
void MSimdUnaryArith::printOpcode(GenericPrinter& out) const { PrintOpcodeOperation(this, out); }
void MSimdBinaryComp::printOpcode(GenericPrinter& out) const { PrintOpcodeOperation(this, out); }
void MSimdShift::printOpcode(GenericPrinter& out) const { PrintOpcodeOperation(this, out); }
...
MDefinition* MMathFunction::foldsTo(TempAllocator& alloc) {
    ...
    if (!input->isConstant() || !input->toConstant()->isTypeRepresentableAsDouble()) return this;
    ...
    if (input->type() == MIRType::Float32) return MConstant::NewFloat32(alloc, out);
...
MCall* MCall::New(TempAllocator& alloc, JSFunction* target, size_t maxArgc, size_t numActualArgs,
...
void MCallDOMNative::computeMovable() {
    ...
    MOZ_ASSERT_IF(jitInfo->isMovable, jitInfo->aliasSet() != JSJitInfo::AliasEverything);
...
MDefinition* MConcat::foldsTo(TempAllocator& alloc) {
    if (lhs()->isConstant() && lhs()->toConstant()->toString()->empty()) return rhs();

    if (rhs()->isConstant() && rhs()->toConstant()->toString()->empty()) return lhs();

    return this;
}

static bool EnsureFloatInputOrConvert(MUnaryInstruction* owner, TempAllocator& alloc) {
    MDefinition* input = owner->input();
    if (!input->canProduceFloat32()) {
        if (input->type() == MIRType::Float32) ConvertDefinitionToDouble<0>(alloc, input, owner);
        return false;
    }
    return true;
}
...
void MFloor::trySpecializeFloat32(TempAllocator& alloc) {
    ...
    if (EnsureFloatInputOrConvert(this, alloc)) specialization_ = MIRType::Float32;
}
...
MTableSwitch* MTableSwitch::New(TempAllocator& alloc, MDefinition* ins, int32_t low,
                                int32_t high) {
...
bool MTypeBarrier::congruentTo(const MDefinition* def) const {
    ...
    if (barrierKind() != other->barrierKind() || isGuard() != other->isGuard()) return false;
...
MDefinition* MPhi::foldsTernary(TempAllocator& alloc) {
    ...
    if (!trueDef->block()->dominates(truePred) || !falseDef->block()->dominates(falsePred)) {
        return nullptr;
    }
...
MDefinition* MPhi::foldsFilterTypeSet() {
    ...
    for (size_t i = 1, e = numOperands(); i < e; i++) {
        ...
        if (op->isFilterTypeSet() && op->toFilterTypeSet()->input() == subject) continue;
...
static inline TemporaryTypeSet* MakeMIRTypeSet(TempAllocator& alloc, MIRType type) {
...
bool jit::MergeTypes(TempAllocator& alloc, MIRType* ptype, TemporaryTypeSet** ptypeSet,
...
bool jit::CanStoreUnboxedType(TempAllocator& alloc, JSValueType unboxedType, MIRType input,
                              TypeSet* inputTypes) {
...
bool MPhi::addBackedgeType(TempAllocator& alloc, MIRType type, TemporaryTypeSet* typeSet) {
    MOZ_ASSERT(!specialized_);
    ...
    if (hasBackedgeType_) {
        ...
        if (!MergeTypes(alloc, &resultType, &resultTypeSet, type, typeSet)) return false;
...
bool MPhi::typeIncludes(MDefinition* def) {
    if (def->type() == MIRType::Int32 && this->type() == MIRType::Double) return true;
...
bool MPhi::checkForTypeChange(TempAllocator& alloc, MDefinition* ins, bool* ptypeChange) {
    MIRType resultType = this->type();
    TemporaryTypeSet* resultTypeSet = this->resultTypeSet();
...
MDefinition* MBinaryBitwiseInstruction::foldsTo(TempAllocator& alloc) {
    ...
    if (MDefinition* folded = EvaluateConstantOperands(alloc, this)) return folded;
...
MDefinition* MBinaryBitwiseInstruction::foldUnnecessaryBitop() {
    ...
    if (isUrsh() && hasOneDefUse() && IsUint32Type(this)) {
        ...
        if (use.def()->isMod() && use.def()->toMod()->isUnsigned()) return getOperand(0);
        MOZ_ASSERT(!(++use));
    }
...
MBinaryArithInstruction* MBinaryArithInstruction::New(TempAllocator& alloc, Opcode op,
...
void MBinaryArithInstruction::setNumberSpecialization(TempAllocator& alloc,
...
MDefinition* MRsh::foldsTo(TempAllocator& alloc) {
    ...
    if (!lhs->isLsh() || !rhs->isConstant() || rhs->type() != MIRType::Int32) return this;
...
MDefinition* MBinaryArithInstruction::foldsTo(TempAllocator& alloc) {
    ...
    if (MConstant* folded = EvaluateConstantOperands(alloc, this)) {
        if (isTruncated()) {
            ...
            if (folded->type() != MIRType::Int32) return MTruncateToInt32::New(alloc, folded);
        }
        return folded;
    }
...
void MBinaryArithInstruction::trySpecializeFloat32(TempAllocator& alloc) {
    ...
    if (!left->canProduceFloat32() || !right->canProduceFloat32() ||
        ...
        if (left->type() == MIRType::Float32) ConvertDefinitionToDouble<0>(alloc, left, this);
        if (right->type() == MIRType::Float32) ConvertDefinitionToDouble<1>(alloc, right, this);
        return;
    }
...
void MDiv::analyzeEdgeCasesForward() {
    ...
    if (rhs()->isConstant() && !rhs()->toConstant()->isInt32(0)) canBeDivideByZero_ = false;
    ...
    if (rhs()->isConstant() && !rhs()->toConstant()->isInt32(-1)) canBeNegativeOverflow_ = false;

    // If lhs is != 0, then negative zero check can be skipped.
    if (lhs()->isConstant() && !lhs()->toConstant()->isInt32(0)) setCanBeNegativeZero(false);
...
void MDiv::analyzeEdgeCasesBackward() {
    if (canBeNegativeZero() && !NeedNegativeZeroCheck(this)) setCanBeNegativeZero(false);
}
...
bool MMod::fallible() const {
    return !isTruncated() && (isUnsigned() || canBeDivideByZero() || canBeNegativeDividend());
}
...
static inline bool SimpleArithOperand(MDefinition* op) {
    return !op->mightBeType(MIRType::Object) && !op->mightBeType(MIRType::String) &&
...
/* static */ bool MBinaryInstruction::unsignedOperands(MDefinition* left, MDefinition* right) {
    MDefinition* replace;
...
MCompare::CompareType MCompare::determineCompareType(JSOp op, MDefinition* left,
    ...
    if (!relationalEq && lhs == MIRType::Object && rhs == MIRType::Object) return Compare_Object;

    // Handle string comparisons. (Relational string compares are still unsupported).
    if (!relationalEq && lhs == MIRType::String && rhs == MIRType::String) return Compare_String;

    // Handle symbol comparisons. (Relaational compare will throw)
    if (!relationalEq && lhs == MIRType::Symbol && rhs == MIRType::Symbol) return Compare_Symbol;
...
void MCompare::cacheOperandMightEmulateUndefined(CompilerConstraintList* constraints) {
    MOZ_ASSERT(operandMightEmulateUndefined());
...
void MTypeOf::cacheInputMaybeCallableOrEmulatesUndefined(CompilerConstraintList* constraints) {
...
MBitAnd* MBitAnd::New(TempAllocator& alloc, MDefinition* left, MDefinition* right) {
    return new (alloc) MBitAnd(left, right, MIRType::Int32);
}

MBitAnd* MBitAnd::New(TempAllocator& alloc, MDefinition* left, MDefinition* right, MIRType type) {
...
MBitOr* MBitOr::New(TempAllocator& alloc, MDefinition* left, MDefinition* right, MIRType type) {
    MBitOr* ins = new (alloc) MBitOr(left, right, type);
    ins->specializeAs(type);
    return ins;
}

MBitXor* MBitXor::New(TempAllocator& alloc, MDefinition* left, MDefinition* right) {
    return new (alloc) MBitXor(left, right, MIRType::Int32);
}

MBitXor* MBitXor::New(TempAllocator& alloc, MDefinition* left, MDefinition* right, MIRType type) {
...
MLsh* MLsh::New(TempAllocator& alloc, MDefinition* left, MDefinition* right, MIRType type) {
...
MRsh* MRsh::New(TempAllocator& alloc, MDefinition* left, MDefinition* right, MIRType type) {
...
MUrsh* MUrsh::New(TempAllocator& alloc, MDefinition* left, MDefinition* right, MIRType type) {
...
MResumePoint* MResumePoint::New(TempAllocator& alloc, MBasicBlock* block, jsbytecode* pc,
                                Mode mode) {
...
MResumePoint* MResumePoint::New(TempAllocator& alloc, MBasicBlock* block, MResumePoint* model,
    ...
    // Copy the operands.
    for (size_t i = 0; i < operands.length(); i++) resume->initOperand(i, operands[i]);

    return resume;
}

MResumePoint* MResumePoint::Copy(TempAllocator& alloc, MResumePoint* src) {
    MResumePoint* resume = new (alloc) MResumePoint(src->block(), src->pc(), src->mode());
    ...
    // Copy the operands.
    for (size_t i = 0; i < resume->numOperands(); i++) resume->initOperand(i, src->getOperand(i));
    return resume;
}

MResumePoint::MResumePoint(MBasicBlock* block, jsbytecode* pc, Mode mode)
    : MNode(block, Kind::ResumePoint), pc_(pc), instruction_(nullptr), mode_(mode) {
...
MResumePoint* MResumePoint::caller() const { return block()->callerResumePoint(); }
...
void MResumePoint::addStore(TempAllocator& alloc, MDefinition* store, const MResumePoint* cache) {
...
void MResumePoint::dump(GenericPrinter& out) const {
    ...
    if (MResumePoint* c = caller()) out.printf(" (caller in block%u)", c->block()->id());
...
bool MResumePoint::isObservableOperand(MUse* u) const { return isObservableOperand(indexOf(u)); }
...
MDefinition* MToInt32::foldsTo(TempAllocator& alloc) {
    MDefinition* input = getOperand(0);
...
void MToInt32::analyzeEdgeCasesBackward() {
...
bool MCompare::tryFold(bool* result) {
    ...
    if (compareType_ == Compare_Null || compareType_ == Compare_Undefined) {
        ...
        if (op == JSOP_STRICTEQ || op == JSOP_STRICTNE) {
            ...
            if (!lhs()->mightBeType(MIRType::Null) && !lhs()->mightBeType(MIRType::Undefined) &&
...
MDefinition* MCompare::foldsTo(TempAllocator& alloc) {
    ...
    if (tryFold(&result) || evaluateConstantOperands(alloc, &result)) {
        if (type() == MIRType::Int32) return MConstant::New(alloc, Int32Value(result));
...
void MCompare::trySpecializeFloat32(TempAllocator& alloc) {
    ...
    if (lhs->canProduceFloat32() && rhs->canProduceFloat32() && compareType_ == Compare_Double) {
        ...
        if (lhs->type() == MIRType::Float32) ConvertDefinitionToDouble<0>(alloc, lhs, this);
        if (rhs->type() == MIRType::Float32) ConvertDefinitionToDouble<1>(alloc, rhs, this);
    }
}
...
void MCompare::filtersUndefinedOrNull(bool trueBranch, MDefinition** subject,
    ...
    *subject = nullptr;
    ...
    if (compareType() != Compare_Undefined && compareType() != Compare_Null) return;

    MOZ_ASSERT(jsop() == JSOP_STRICTNE || jsop() == JSOP_NE || jsop() == JSOP_STRICTEQ ||
               jsop() == JSOP_EQ);
...
void MNot::cacheOperandMightEmulateUndefined(CompilerConstraintList* constraints) {
    MOZ_ASSERT(operandMightEmulateUndefined());
...
MDefinition* MNot::foldsTo(TempAllocator& alloc) {
    ...
    if (input()->type() == MIRType::Symbol) return MConstant::New(alloc, BooleanValue(false));
...
bool OperandIndexMap::init(TempAllocator& alloc, JSObject* templateObject) {
    ...
    uint8_t index = 1;
    for (size_t i = 0; i < properties.length(); i++, index++) map[properties[i].offset] = index;
...
bool MObjectState::initFromTemplateObject(TempAllocator& alloc, MDefinition* undefinedVal) {
...
MObjectState* MObjectState::New(TempAllocator& alloc, MDefinition* obj) {
    ...
    if (templateObject->is<UnboxedPlainObject>()) {
        ...
        if (!operandIndex || !operandIndex->init(alloc, templateObject)) return nullptr;
    }
...
MObjectState* MObjectState::Copy(TempAllocator& alloc, MObjectState* state) {
    ...
    for (size_t i = 0; i < res->numSlots(); i++) res->initSlot(i, state->getSlot(i));
    return res;
}
...
bool MArrayState::init(TempAllocator& alloc, MDefinition* obj, MDefinition* len) {
...
MArrayState* MArrayState::New(TempAllocator& alloc, MDefinition* arr, MDefinition* undefinedVal,
                              MDefinition* initLength) {
    MArrayState* res = new (alloc) MArrayState(arr);
    if (!res || !res->init(alloc, arr, initLength)) return nullptr;
    for (size_t i = 0; i < res->numElements(); i++) res->initElement(i, undefinedVal);
    return res;
}
...
MArrayState* MArrayState::Copy(TempAllocator& alloc, MArrayState* state) {
    ...
    for (size_t i = 0; i < res->numElements(); i++) res->initElement(i, state->getElement(i));
    return res;
}

MArgumentState* MArgumentState::New(TempAllocator::Fallible view, const MDefinitionVector& args) {
    MArgumentState* res = new (view.alloc) MArgumentState();
    if (!res || !res->init(view.alloc, args.length())) return nullptr;
    for (size_t i = 0, e = args.length(); i < e; i++) res->initOperand(i, args[i]);
    return res;
}

MArgumentState* MArgumentState::Copy(TempAllocator& alloc, MArgumentState* state) {
...
MDefinition::AliasType MLoadFixedSlot::mightAlias(const MDefinition* def) const {
...
MDefinition::AliasType MLoadFixedSlotAndUnbox::mightAlias(const MDefinition* def) const {
    if (def->isStoreFixedSlot()) {
        const MStoreFixedSlot* store = def->toStoreFixedSlot();
        if (store->slot() != slot()) return AliasType::NoAlias;
        if (store->object() != object()) return AliasType::MayAlias;
...
MDefinition::AliasType MAsmJSLoadHeap::mightAlias(const MDefinition* def) const {
    if (def->isAsmJSStoreHeap()) {
        ...
        if (!base()->isConstant() || !store->base()->isConstant()) return AliasType::MayAlias;
...
MDefinition::AliasType MWasmLoadGlobalVar::mightAlias(const MDefinition* def) const {
...
static bool DefinitelyDifferentValue(MDefinition* ins1, MDefinition* ins2) {
    ...
    if (ins1->isToInt32()) return DefinitelyDifferentValue(ins1->toToInt32()->input(), ins2);
    if (ins2->isToInt32()) return DefinitelyDifferentValue(ins2->toToInt32()->input(), ins1);
...
MDefinition::AliasType MLoadElement::mightAlias(const MDefinition* def) const {
    if (def->isStoreElement()) {
        ...
        if (store->index() != index()) {
            if (DefinitelyDifferentValue(store->index(), index())) return AliasType::NoAlias;
            return AliasType::MayAlias;
        }
...
MDefinition::AliasType MLoadUnboxedObjectOrNull::mightAlias(const MDefinition* def) const {
    if (def->isStoreUnboxedObjectOrNull()) {
        ...
        if (store->offsetAdjustment() != offsetAdjustment()) return AliasType::MayAlias;
...
void InlinePropertyTable::trimTo(const InliningTargets& targets, const BoolVector& choiceSet) {
...
void InlinePropertyTable::trimToTargets(const InliningTargets& targets) {
    JitSpew(JitSpew_Inlining, "Got inlineable property cache with %d cases", (int)numEntries());
...
TemporaryTypeSet* InlinePropertyTable::buildTypeSetForFunction(TempAllocator& tempAlloc,
...
SharedMem<void*> MLoadTypedArrayElementStatic::base() const {
    return someTypedArray_->as<TypedArrayObject>().viewDataEither();
}

size_t MLoadTypedArrayElementStatic::length() const {
    return someTypedArray_->as<TypedArrayObject>().byteLength();
}

bool MLoadTypedArrayElementStatic::congruentTo(const MDefinition* ins) const {
    if (!ins->isLoadTypedArrayElementStatic()) return false;
    const MLoadTypedArrayElementStatic* other = ins->toLoadTypedArrayElementStatic();
    if (offset() != other->offset()) return false;
    if (needsBoundsCheck() != other->needsBoundsCheck()) return false;
    if (accessType() != other->accessType()) return false;
    if (base() != other->base()) return false;
    return congruentIfOperandsEqual(other);
}

SharedMem<void*> MStoreTypedArrayElementStatic::base() const {
    return someTypedArray_->as<TypedArrayObject>().viewDataEither();
}
...
size_t MStoreTypedArrayElementStatic::length() const {
    return someTypedArray_->as<TypedArrayObject>().byteLength();
}

MDefinition::AliasType MGetPropertyPolymorphic::mightAlias(const MDefinition* store) const {
    // Allow hoisting this instruction if the store does not write to a
    // slot read by this instruction.

    if (!store->isStoreFixedSlot() && !store->isStoreSlot()) return AliasType::MayAlias;
    ...
    for (size_t i = 0; i < numReceivers(); i++) {
        ...
        if (shape->slot() < shape->numFixedSlots()) {
            ...
            if (store->isStoreFixedSlot() && store->toStoreFixedSlot()->slot() != slot) continue;
            if (store->isStoreSlot()) continue;
        } else {
            // Dynamic slot.
            uint32_t slot = shape->slot() - shape->numFixedSlots();
            if (store->isStoreSlot() && store->toStoreSlot()->slot() != slot) continue;
...
bool MObjectGroupDispatch::appendRoots(MRootList& roots) const {
    if (inlinePropertyTable_ && !inlinePropertyTable_->appendRoots(roots)) return false;
...
MDefinition* MWasmUnsignedToFloat32::foldsTo(TempAllocator& alloc) {
    if (input()->isConstant() && input()->type() == MIRType::Int32) {
        ...
        if (IsFloat32Representable(dval)) return MConstant::NewFloat32(alloc, float(dval));
    }
...
MWasmCall* MWasmCall::New(TempAllocator& alloc, const wasm::CallSiteDesc& desc,
                          ...
                          uint32_t spIncrement, MDefinition* tableIndex) {
    ...
    for (size_t i = 0; i < call->argRegs_.length(); i++) call->argRegs_[i] = args[i].reg;

    if (!call->init(alloc, call->argRegs_.length() + (callee.isTable() ? 1 : 0))) return nullptr;
    // FixedList doesn't initialize its elements, so do an unchecked init.
    for (size_t i = 0; i < call->argRegs_.length(); i++) call->initOperand(i, args[i].def);
    if (callee.isTable()) call->initOperand(call->argRegs_.length(), tableIndex);

    return call;
}

MWasmCall* MWasmCall::NewBuiltinInstanceMethodCall(TempAllocator& alloc,
...
MDefinition* MBoundsCheck::foldsTo(TempAllocator& alloc) {
    if (index()->isConstant() && length()->isConstant()) {
        ...
        if (idx + uint32_t(minimum()) < len && idx + uint32_t(maximum()) < len) return index();
    }
...
MConvertUnboxedObjectToNative* MConvertUnboxedObjectToNative::New(TempAllocator& alloc,
                                                                  MDefinition* obj,
                                                                  ObjectGroup* group) {
...
bool jit::ElementAccessIsDenseNative(CompilerConstraintList* constraints, MDefinition* obj,
                                     MDefinition* id) {
    if (obj->mightBeType(MIRType::String)) return false;

    if (id->type() != MIRType::Int32 && id->type() != MIRType::Double) return false;
...
bool jit::ElementAccessIsTypedArray(CompilerConstraintList* constraints, MDefinition* obj,
...
bool jit::ElementAccessIsPacked(CompilerConstraintList* constraints, MDefinition* obj) {
    TemporaryTypeSet* types = obj->resultTypeSet();
    return types && !types->hasObjectFlags(constraints, OBJECT_FLAG_NON_PACKED);
}

bool jit::ElementAccessMightBeCopyOnWrite(CompilerConstraintList* constraints, MDefinition* obj) {
    TemporaryTypeSet* types = obj->resultTypeSet();
    return !types || types->hasObjectFlags(constraints, OBJECT_FLAG_COPY_ON_WRITE);
}

bool jit::ElementAccessMightBeFrozen(CompilerConstraintList* constraints, MDefinition* obj) {
    TemporaryTypeSet* types = obj->resultTypeSet();
    return !types || types->hasObjectFlags(constraints, OBJECT_FLAG_FROZEN_ELEMENTS);
}

AbortReasonOr<bool> jit::ElementAccessHasExtraIndexedProperty(IonBuilder* builder,
                                                              MDefinition* obj) {
    TemporaryTypeSet* types = obj->resultTypeSet();
...
MIRType jit::DenseNativeElementType(CompilerConstraintList* constraints, MDefinition* obj) {
    TemporaryTypeSet* types = obj->resultTypeSet();
...
static BarrierKind PropertyReadNeedsTypeBarrier(CompilerConstraintList* constraints,
    ...
    if (key->unknownProperties() || observed->empty() || key->clasp()->isProxy()) {
...
BarrierKind jit::PropertyReadNeedsTypeBarrier(JSContext* propertycx, TempAllocator& alloc,
                                              CompilerConstraintList* constraints,
                                              TypeSet::ObjectKey* key, PropertyName* name,
                                              TemporaryTypeSet* observed, bool updateObserved) {
    if (!updateObserved) return PropertyReadNeedsTypeBarrier(constraints, key, name, observed);
...
AbortReasonOr<BarrierKind> jit::PropertyReadOnPrototypeNeedsTypeBarrier(
    IonBuilder* builder, MDefinition* obj, PropertyName* name, TemporaryTypeSet* observed) {
    if (observed->unknown()) return BarrierKind::NoBarrier;
    ...
    for (size_t i = 0; i < types->getObjectCount(); i++) {
        ...
        if (!key) continue;
        while (true) {
            if (!builder->alloc().ensureBallast()) return builder->abort(AbortReason::Alloc);
            if (!key->hasStableClassAndProto(builder->constraints())) return BarrierKind::TypeSet;
            if (!key->proto().isObject()) break;
            JSObject* proto = builder->checkNurseryObject(key->proto().toObject());
            key = TypeSet::ObjectKey::get(proto);
            BarrierKind kind =
                PropertyReadNeedsTypeBarrier(builder->constraints(), key, name, observed);
            if (kind == BarrierKind::TypeSet) return BarrierKind::TypeSet;
...
bool jit::PropertyReadIsIdempotent(CompilerConstraintList* constraints, MDefinition* obj,
                                   PropertyName* name) {
...
void jit::AddObjectsForPropertyRead(TempAllocator& tempAlloc, MDefinition* obj, PropertyName* name,
                                    TemporaryTypeSet* observed) {
...
AbortReasonOr<bool> PrototypeHasIndexedProperty(IonBuilder* builder, JSObject* obj) {
    do {
...
AbortReasonOr<bool> jit::ArrayPrototypeHasIndexedProperty(IonBuilder* builder, JSScript* script) {
...
AbortReasonOr<bool> jit::TypeCanHaveExtraIndexedProperties(IonBuilder* builder,
                                                           TemporaryTypeSet* types) {
    ...
    // by JIT paths.
    if (!clasp || (ClassCanHaveExtraProperties(clasp) && !IsTypedArrayClass(clasp))) return true;

    if (types->hasObjectFlags(builder->constraints(), OBJECT_FLAG_SPARSE_INDEXES)) return true;
...
static bool PropertyTypeIncludes(TempAllocator& alloc, HeapTypeSetKey property, MDefinition* value,
...
static bool TryAddTypeBarrierForWrite(TempAllocator& alloc, CompilerConstraintList* constraints,
    ...
    for (size_t i = 0; i < objTypes->getObjectCount(); i++) {
        ...
        jsid id = name ? NameToId(name) : JSID_VOID;
        HeapTypeSetKey property = key->property(id);
        if (!property.maybeTypes() || property.couldBeConstant(constraints)) return false;

        if (PropertyTypeIncludes(alloc, property, *pvalue, implicitType)) return false;
        ...
        if (!aggregateProperty) {
            ...
            if (!aggregateProperty->maybeTypes()->equals(property.maybeTypes())) return false;
        }
    }
...
static MInstruction* AddGroupGuard(TempAllocator& alloc, MBasicBlock* current, MDefinition* obj,
...
bool jit::CanWriteProperty(TempAllocator& alloc, CompilerConstraintList* constraints,
...
bool jit::PropertyWriteNeedsTypeBarrier(TempAllocator& alloc, CompilerConstraintList* constraints,
    ...
    for (size_t i = 0; i < types->getObjectCount(); i++) {
        ...
        if (key && key->isGroup() && key->group()->maybeUnboxedLayout()) {
            ...
            if (name) {
                ...
                if (property && !CanStoreUnboxedType(alloc, property->type, *pvalue)) return true;
            }
        }
    }
    ...
    for (size_t i = 0; i < types->getObjectCount(); i++) {
        ...
        jsid id = name ? NameToId(name) : JSID_VOID;
        HeapTypeSetKey property = key->property(id);
        if (CanWriteProperty(alloc, constraints, property, *pvalue, implicitType)) continue;

        if ((property.maybeTypes() && !property.maybeTypes()->empty()) || excluded) return true;
        excluded = key;
    }
