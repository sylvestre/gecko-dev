...
ABIArgGenerator::ABIArgGenerator() : usedArgSlots_(0), firstArgFloat(false), current_() {}
...
uint64_t Assembler::ExtractLoad64Value(Instruction* inst0) {
    ...
    uint64_t value = (uint64_t(i0->extractImm16Value()) << 48) |
                     ...
                     (uint64_t(i3->extractImm16Value()) << 16) | uint64_t(i5->extractImm16Value());
    return value;
}
...
void Assembler::WriteLoad64Instructions(Instruction* inst0, Register reg, uint64_t value) {
