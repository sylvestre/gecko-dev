...
class LUnboxFloatingPoint : public LUnbox {
   ...
   public:
    ...
    LUnboxFloatingPoint(const LAllocation& input, MIRType type) : LUnbox(input), type_(type) {}
...
class LInt64ToFloatingPoint : public LInstructionHelper<1, 1, 0> {
   public:
    ...
    explicit LInt64ToFloatingPoint(const LInt64Allocation& in) { setInt64Operand(0, in); }
