...
static inline MOZ_MUST_USE bool MaybeSimdTypeToMIRType(SimdType type, MIRType* mirType) {
    switch (type) {
...
class MUse : public TempObject, public InlineListNode<MUse> {
   ...
   public:
    ...
    MUse(MDefinition* producer, MNode* consumer) { initUnchecked(producer, consumer); }
...
class MNode : public TempObject {
   ...
   public:
    ...
    bool hasOperand(size_t index) const { return getUseFor(index)->hasProducer(); }
...
#undef DEFINE_FLAG
   ...
   protected:
    void setBlock(MBasicBlock* block) { setBlockAndKind(block, Kind::Definition); }
   ...
   public:
    ...
    jsbytecode* trackedPc() const { return trackedSite_ ? trackedSite_->pc() : nullptr; }
    InlineScriptTree* trackedTree() const { return trackedSite_ ? trackedSite_->tree() : nullptr; }
    TrackedOptimizations* trackedOptimizations() const {
        return trackedSite_ && trackedSite_->hasOptimizations() ? trackedSite_->optimizations()
                                                                : nullptr;
    }

    JSScript* profilerLeaveScript() const { return trackedTree()->outermostCaller()->script(); }
...
#undef FLAG_ACCESSOR
    ...
    virtual MOZ_MUST_USE bool updateForReplacement(MDefinition* ins) { return true; }
...
#undef OPCODE_CASTS
    ...
    virtual AliasType mightAlias(const MDefinition* store) const {
        ...
        if (!(getAliasSet().flags() & store->getAliasSet().flags())) return AliasType::NoAlias;
...
class MInstruction : public MDefinition, public InlineListNode<MInstruction> {
   ...
   public:
    ...
    explicit MInstruction(const MInstruction& other) : MDefinition(other), resumePoint_(nullptr) {}
...
#define INSTRUCTION_HEADER_WITHOUT_TYPEPOLICY(opcode)       \
    ...
    void accept(MDefinitionVisitor* visitor) override { visitor->visit##opcode(this); }
...
#define ALLOW_CLONE(typename)                                                                   \
    ...
    MInstruction* clone(TempAllocator& alloc, const MDefinitionVector& inputs) const override { \
        MInstruction* res = new (alloc) typename(*this);                                        \
        for (size_t i = 0; i < numOperands(); i++) res->replaceOperand(i, inputs[i]);           \
        return res;                                                                             \
    }
...
#define NAMED_OPERANDS(...) MOZ_FOR_EACH(NAMED_OPERAND_ACCESSOR_APPLY, (), (__VA_ARGS__))
...
class MAryInstruction : public MInstruction {
   ...
   protected:
    MUse* getUseFor(size_t index) final override { return &operands_[index]; }
    const MUse* getUseFor(size_t index) const final override { return &operands_[index]; }
    void initOperand(size_t index, MDefinition* operand) { operands_[index].init(operand, this); }

   public:
    MDefinition* getOperand(size_t index) const final override {
        return operands_[index].producer();
    }
    size_t numOperands() const final override { return Arity; }
#ifdef DEBUG
    ...
    explicit MAryInstruction(const MAryInstruction<Arity>& other) : MInstruction(other) {
        for (int i = 0; i < (int)Arity; i++)  // N.B. use |int| to avoid warnings when Arity == 0
            operands_[i].init(other.operands_[i].producer(), this);
    }
};

class MNullaryInstruction : public MAryInstruction<0>, public NoTypePolicy::Data {
   protected:
...
class MUnaryInstruction : public MAryInstruction<1> {
   protected:
    MUnaryInstruction(Opcode op, MDefinition* ins) : MAryInstruction(op) { initOperand(0, ins); }
...
class MBinaryInstruction : public MAryInstruction<2> {
   protected:
    MBinaryInstruction(Opcode op, MDefinition* left, MDefinition* right) : MAryInstruction(op) {
        initOperand(0, left);
        initOperand(1, right);
    }
...
class MQuaternaryInstruction : public MAryInstruction<4> {
   protected:
    MQuaternaryInstruction(Opcode op, MDefinition* first, MDefinition* second, MDefinition* third,
                           MDefinition* fourth)
        : MAryInstruction(op) {
        initOperand(0, first);
        initOperand(1, second);
...
class MVariadicT : public T {
   ...
   public:
    ...
    size_t numOperands() const final override { return operands_.length(); }
    size_t indexOf(const MUse* u) const final override {
        MOZ_ASSERT(u >= &operands_[0]);
        MOZ_ASSERT(u <= &operands_[numOperands() - 1]);
        return u - &operands_[0];
...
class MOsrEntry : public MNullaryInstruction {
   protected:
    MOsrEntry() : MNullaryInstruction(classOpcode) { setResultType(MIRType::Pointer); }
...
class MLimitedTruncate : public MUnaryInstruction, public ConvertToInt32Policy<0>::Data {
   public:
   ...
   protected:
    MLimitedTruncate(MDefinition* input, TruncateKind limit)
        : MUnaryInstruction(classOpcode, input), truncate_(NoTruncate), truncateLimit_(limit) {
...
class MConstant : public MNullaryInstruction {
   ...
   public:
    ...
    bool isInt32(int32_t i) const { return type() == MIRType::Int32 && payload_.i32 == i; }
    ...
    bool isTypeRepresentableAsDouble() const { return IsTypeRepresentableAsDouble(type()); }
...
class MSimdConvert : public MUnaryInstruction, public SimdPolicy<0>::Data {
   ...
   public:
    ...
    static MInstruction* AddLegalized(
        TempAllocator& alloc, MBasicBlock* addTo, MDefinition* obj, MIRType toType, SimdSign sign,
...
class MSimdReinterpretCast : public MUnaryInstruction, public SimdPolicy<0>::Data {
    MSimdReinterpretCast(MDefinition* obj, MIRType toType) : MUnaryInstruction(classOpcode, obj) {
        MIRType fromType = obj->type();
        MOZ_ASSERT(IsSimdType(fromType));
        MOZ_ASSERT(IsSimdType(toType));
        setMovable();
...
class MSimdExtractElement : public MUnaryInstruction, public SimdPolicy<0>::Data {
   protected:
    unsigned lane_;
    SimdSign sign_;
...
class MSimdAllTrue : public MUnaryInstruction, public SimdPolicy<0>::Data {
   protected:
    explicit MSimdAllTrue(MDefinition* obj, MIRType result) : MUnaryInstruction(classOpcode, obj) {
...
class MSimdAnyTrue : public MUnaryInstruction, public SimdPolicy<0>::Data {
   protected:
    explicit MSimdAnyTrue(MDefinition* obj, MIRType result) : MUnaryInstruction(classOpcode, obj) {
        MIRType simdType = obj->type();
        MOZ_ASSERT(IsBooleanSimdType(simdType));
        MOZ_ASSERT(result == MIRType::Boolean || result == MIRType::Int32);
        setResultType(result);
...
class MSimdShuffleBase {
   protected:
    ...
    bool sameLanes(const MSimdShuffleBase* other) const {
        return arity_ == other->arity_ && memcmp(&lane_[0], &other->lane_[0], arity_) == 0;
    }
   ...
   public:
    ...
    bool lanesMatch(uint32_t x, uint32_t y, uint32_t z, uint32_t w) const {
        return arity_ == 4 && lane(0) == x && lane(1) == y && lane(2) == z && lane(3) == w;
    }
};
...
class MSimdSwizzle : public MUnaryInstruction, public MSimdShuffleBase, public NoTypePolicy::Data {
   protected:
...
class MSimdGeneralShuffle : public MVariadicInstruction, public SimdShufflePolicy::Data {
   ...
   public:
    ...
    bool congruentTo(const MDefinition* ins) const override {
        ...
        return numVectors_ == other->numVectors() && numLanes_ == other->numLanes() &&
   ...
   public:
    ...
    static MInstruction* New(TempAllocator& alloc, MDefinition* lhs, MDefinition* rhs,
        ...
        if (lanesFromLHS < arity / 2 ||
            ...
            for (unsigned i = 0; i < arity; i++) newLanes[i] = (lanes[i] + arity) % (2 * arity);
...
class MSimdUnaryArith : public MUnaryInstruction, public SimdSameAsReturnedTypePolicy<0>::Data {
   public:
    enum Operation {
...
#undef NAME_
   ...
   public:
    ...
    bool congruentTo(const MDefinition* ins) const override {
        ...
        return specialization_ == other->specialization() && operation_ == other->operation() &&
...
#undef OP_CASE_LIST_
   ...
   private:
    ...
    MSimdBinaryArith(MDefinition* left, MDefinition* right, Operation op)
        ...
        setResultType(type);
        setMovable();
        if (op == Op_add || op == Op_mul || op == Op_min || op == Op_max) setCommutative();
    }
...
class MSimdShift : public MBinaryInstruction,
   ...
   public:
    ...
    static Operation rshForSign(SimdSign sign) { return sign == SimdSign::Unsigned ? ursh : rsh; }
...
class MCloneLiteral : public MUnaryInstruction, public ObjectPolicy<0>::Data {
   protected:
    explicit MCloneLiteral(MDefinition* obj) : MUnaryInstruction(classOpcode, obj) {
...
class MTableSwitch final : public MControlInstruction, public NoFloatPolicy<0>::Data {
    ...
    MTableSwitch(TempAllocator& alloc, MDefinition* ins, int32_t low, int32_t high)
   ...
   public:
    ...
    MOZ_MUST_USE bool addCase(size_t successorIndex) { return cases_.append(successorIndex); }
...
class MAryControlInstruction : public MControlInstruction {
   ...
   protected:
    ...
    void setSuccessor(size_t index, MBasicBlock* successor) { successors_[index] = successor; }
   ...
   public:
    ...
    size_t numSuccessors() const final override { return Successors; }
    MBasicBlock* getSuccessor(size_t i) const final override { return successors_[i]; }
    void replaceSuccessor(size_t i, MBasicBlock* succ) final override { successors_[i] = succ; }
};
...
class MTest : public MAryControlInstruction<1, 2>, public TestPolicy::Data {
    ...
    MTest(MDefinition* ins, MBasicBlock* trueBranch, MBasicBlock* falseBranch)
        : MAryControlInstruction(classOpcode), operandMightEmulateUndefined_(true) {
        initOperand(0, ins);
    ...
    MTest(MDefinition* ins, MBasicBlock* falseBranch) : MTest(ins, nullptr, falseBranch) {}
   ...
   public:
    ...
    void markNoOperandEmulatesUndefined() { operandMightEmulateUndefined_ = false; }
    bool operandMightEmulateUndefined() const { return operandMightEmulateUndefined_; }
#ifdef DEBUG
...
class MGotoWithFake : public MAryControlInstruction<0, 2>, public NoTypePolicy::Data {
...
class MReturn : public MAryControlInstruction<1, 0>, public BoxInputsPolicy::Data {
...
class MThrow : public MAryControlInstruction<1, 0>, public BoxInputsPolicy::Data {
...
TemporaryTypeSet* MakeSingletonTypeSet(TempAllocator& alloc, CompilerConstraintList* constraints,
                                       JSObject* obj);

TemporaryTypeSet* MakeSingletonTypeSet(TempAllocator& alloc, CompilerConstraintList* constraints,
                                       ObjectGroup* obj);

MOZ_MUST_USE bool MergeTypes(TempAllocator& alloc, MIRType* ptype, TemporaryTypeSet** ptypeSet,
...
bool CanStoreUnboxedType(TempAllocator& alloc, JSValueType unboxedType, MIRType input,
                         TypeSet* inputTypes);
...
class MNewArray : public MUnaryInstruction, public NoTypePolicy::Data {
   ...
   public:
    ...
    static MNewArray* NewVM(TempAllocator& alloc, CompilerConstraintList* constraints,
                            uint32_t length, MConstant* templateConst, gc::InitialHeap initialHeap,
                            jsbytecode* pc) {
        return new (alloc)
            MNewArray(alloc, constraints, length, templateConst, initialHeap, pc, true);
    }

    uint32_t length() const { return length_; }

    JSObject* templateObject() const { return getOperand(0)->toConstant()->toObjectOrNull(); }
...
class MNewArrayCopyOnWrite : public MNullaryInstruction {
   ...
   public:
    ...
    bool appendRoots(MRootList& roots) const override { return roots.append(templateObject_); }
};

class MNewArrayDynamicLength : public MUnaryInstruction, public IntPolicy<0>::Data {
...
class MNewTypedArray : public MUnaryInstruction, public NoTypePolicy::Data {
    ...
    MNewTypedArray(TempAllocator& alloc, CompilerConstraintList* constraints,
        ...
        : MUnaryInstruction(classOpcode, templateConst), initialHeap_(initialHeap) {
        MOZ_ASSERT(!templateObject()->isSingleton());
        setResultType(MIRType::Object);
        setResultTypeSet(MakeSingletonTypeSet(alloc, constraints, templateObject()));
    }
...
class MNewTypedArrayDynamicLength : public MUnaryInstruction, public IntPolicy<0>::Data {
    CompilerObject templateObject_;
    gc::InitialHeap initialHeap_;
...
class MNewIterator : public MUnaryInstruction, public NoTypePolicy::Data {
   ...
   public:
    ...
    JSObject* templateObject() { return getOperand(0)->toConstant()->toObjectOrNull(); }
...
class MNewTypedObject : public MNullaryInstruction {
    ...
    MNewTypedObject(TempAllocator& alloc, CompilerConstraintList* constraints,
                    InlineTypedObject* templateObject, gc::InitialHeap initialHeap)
        : MNullaryInstruction(classOpcode),
          templateObject_(templateObject),
          initialHeap_(initialHeap) {
        setResultType(MIRType::Object);
...
class MTypedObjectDescr : public MUnaryInstruction, public SingleObjectPolicy::Data {
   private:
    explicit MTypedObjectDescr(MDefinition* object) : MUnaryInstruction(classOpcode, object) {
        setResultType(MIRType::Object);
        setMovable();
    }
   ...
   public:
    ...
    AliasSet getAliasSet() const override { return AliasSet::Load(AliasSet::ObjectFields); }
};
...
class MSimdBox : public MUnaryInstruction, public NoTypePolicy::Data {
   protected:
    ...
    MSimdBox(TempAllocator& alloc, CompilerConstraintList* constraints, MDefinition* op,
             InlineTypedObject* templateObject, SimdType simdType, gc::InitialHeap initialHeap)
...
class MNewDerivedTypedObject
   ...
   private:
    ...
    MNewDerivedTypedObject(TypedObjectPrediction prediction, MDefinition* type, MDefinition* owner,
                           MDefinition* offset)
        : MTernaryInstruction(classOpcode, type, owner, offset), prediction_(prediction) {
        setMovable();
        setResultType(MIRType::Object);
    }
...
class MObjectState : public MVariadicInstruction, public NoFloatPolicyAfter<1>::Data {
   private:
    uint32_t numSlots_;
   ...
   public:
    ...
    void setSlot(uint32_t slot, MDefinition* def) { replaceOperand(slot + 1, def); }

    bool hasFixedSlot(uint32_t slot) const { return slot < numSlots() && slot < numFixedSlots(); }
    ...
    MDefinition* getDynamicSlot(uint32_t slot) const { return getSlot(slot + numFixedSlots()); }
    void setDynamicSlot(uint32_t slot, MDefinition* def) { setSlot(slot + numFixedSlots(), def); }
...
// lowered and is not used to generate code.
class MArrayState : public MVariadicInstruction, public NoFloatPolicyAfter<2>::Data {
   private:
    ...
    void initElement(uint32_t index, MDefinition* def) { initOperand(index + 2, def); }
   ...
   public:
    ...
    MDefinition* getElement(uint32_t index) const { return getOperand(index + 2); }
    void setElement(uint32_t index, MDefinition* def) { replaceOperand(index + 2, def); }
...
class MArgumentState : public MVariadicInstruction, public NoFloatPolicyAfter<0>::Data {
   private:
...
class MInitPropGetterSetter : public MBinaryInstruction,
   ...
   public:
    ...
    bool appendRoots(MRootList& roots) const override { return roots.append(name_); }
};

class MInitElem : public MTernaryInstruction,
...
class MCall : public MVariadicInstruction, public CallPolicy::Data {
   ...
   public:
    ...
    void initFunction(MDefinition* func) { initOperand(FunctionOperandIndex, func); }
    ...
    void replaceFunction(MInstruction* newfunc) { replaceOperand(FunctionOperandIndex, newfunc); }
    ...
    static size_t IndexOfStackArg(size_t index) { return NumNonArgumentOperands + index; }
    ...
    // Includes |this|.
    uint32_t numStackArgs() const { return numOperands() - NumNonArgumentOperands; }
...
class MBail : public MNullaryInstruction {
   ...
   public:
    ...
    static MBail* New(TempAllocator& alloc, BailoutKind kind) { return new (alloc) MBail(kind); }
    static MBail* New(TempAllocator& alloc) { return new (alloc) MBail(Bailout_Inevitable); }
...
class MUnreachable : public MAryControlInstruction<0, 0>, public NoTypePolicy::Data {
...
class MEncodeSnapshot : public MNullaryInstruction {
   ...
   public:
    ...
    static MEncodeSnapshot* New(TempAllocator& alloc) { return new (alloc) MEncodeSnapshot(); }
};

class MAssertRecoveredOnBailout : public MUnaryInstruction, public NoTypePolicy::Data {
   protected:
...
class MGetDynamicName : public MBinaryInstruction,
   ...
   public:
    ...
    static MGetDynamicName* New(TempAllocator& alloc, MDefinition* envChain, MDefinition* name) {
...
class MCallDirectEval : public MTernaryInstruction,
                        public MixPolicy<ObjectPolicy<0>, StringPolicy<1>, BoxPolicy<2>>::Data {
   protected:
    MCallDirectEval(MDefinition* envChain, MDefinition* string, MDefinition* newTargetValue,
                    jsbytecode* pc)
        : MTernaryInstruction(classOpcode, envChain, string, newTargetValue), pc_(pc) {
        setResultType(MIRType::Value);
    }
   ...
   public:
    ...
    static MCallDirectEval* New(TempAllocator& alloc, MDefinition* envChain, MDefinition* string,
...
class MCompare : public MBinaryInstruction, public ComparePolicy::Data {
   ...
   public:
    ...
    bool isInt32Comparison() const {
        return compareType() == Compare_Int32 || compareType() == Compare_Int32MaybeCoerceBoth ||
               compareType() == Compare_Int32MaybeCoerceLHS ||
               compareType() == Compare_Int32MaybeCoerceRHS;
    }
    bool isDoubleComparison() const {
        return compareType() == Compare_Double || compareType() == Compare_DoubleMaybeCoerceLHS ||
    ...
    AliasSet getAliasSet() const override {
        ...
        if (jsop_ == JSOP_STRICTEQ || jsop_ == JSOP_STRICTNE) return AliasSet::None();
...
class MBox : public MUnaryInstruction, public NoTypePolicy::Data {
    MBox(TempAllocator& alloc, MDefinition* ins) : MUnaryInstruction(classOpcode, ins) {
        setResultType(MIRType::Value);
...
static inline Assembler::Condition JSOpToCondition(MCompare::CompareType compareType, JSOp op) {
...
class MUnbox final : public MUnaryInstruction, public BoxInputsPolicy::Data {
   ...
   private:
    ...
    MUnbox(MDefinition* ins, MIRType type, Mode mode, BailoutKind kind, TempAllocator& alloc)
        ...
        MOZ_ASSERT(type == MIRType::Boolean || type == MIRType::Int32 || type == MIRType::Double ||
                   type == MIRType::String || type == MIRType::Symbol || type == MIRType::Object);
...
class MGuardObject : public MUnaryInstruction, public SingleObjectPolicy::Data {
    explicit MGuardObject(MDefinition* ins) : MUnaryInstruction(classOpcode, ins) {
        setGuard();
        setMovable();
        setResultType(MIRType::Object);
        setResultTypeSet(ins->resultTypeSet());
...
class MGuardString : public MUnaryInstruction, public StringPolicy<0>::Data {
    explicit MGuardString(MDefinition* ins) : MUnaryInstruction(classOpcode, ins) {
        setGuard();
        setMovable();
...
class MPolyInlineGuard : public MUnaryInstruction, public SingleObjectPolicy::Data {
    explicit MPolyInlineGuard(MDefinition* ins) : MUnaryInstruction(classOpcode, ins) {
        setGuard();
        setResultType(MIRType::Object);
        setResultTypeSet(ins->resultTypeSet());
    }
...
class MCreateThisWithTemplate : public MUnaryInstruction, public NoTypePolicy::Data {
    gc::InitialHeap initialHeap_;
   ...
   public:
    ...
    JSObject* templateObject() const { return &getOperand(0)->toConstant()->toObject(); }
...
class MCreateArgumentsObject : public MUnaryInstruction, public ObjectPolicy<0>::Data {
   ...
   public:
    ...
    bool appendRoots(MRootList& roots) const override { return roots.append(templateObj_); }
};
...
class MGetArgumentsObjectArg : public MUnaryInstruction, public ObjectPolicy<0>::Data {
   ...
   public:
    ...
    AliasSet getAliasSet() const override { return AliasSet::Load(AliasSet::Any); }
};

class MSetArgumentsObjectArg : public MBinaryInstruction,
                               public MixPolicy<ObjectPolicy<0>, BoxPolicy<1>>::Data {
    size_t argno_;
   ...
   public:
    ...
    AliasSet getAliasSet() const override { return AliasSet::Store(AliasSet::Any); }
};
...
class MToFPInstruction : public MUnaryInstruction, public ToDoublePolicy::Data {
   public:
    ...
    enum ConversionKind { NonStringPrimitives, NonNullNonStringPrimitives, NumbersOnly };
...
class MToDouble : public MToFPInstruction {
   private:
    ...
    explicit MToDouble(MDefinition* def, ConversionKind conversion = NonStringPrimitives)
        ...
        if (def->mightBeType(MIRType::Object) || def->mightBeType(MIRType::Symbol)) setGuard();
    }
   ...
   public:
    ...
    MDefinition* foldsTo(TempAllocator& alloc) override;
    bool congruentTo(const MDefinition* ins) const override {
        if (!ins->isToDouble() || ins->toToDouble()->conversion() != conversion()) return false;
        return congruentIfOperandsEqual(ins);
    }
    AliasSet getAliasSet() const override { return AliasSet::None(); }
    ...
    void setTruncateKind(TruncateKind kind) { implicitTruncate_ = Max(implicitTruncate_, kind); }
...
class MToFloat32 : public MToFPInstruction {
   protected:
    ...
    explicit MToFloat32(MDefinition* def, ConversionKind conversion = NonStringPrimitives)
        : MToFPInstruction(classOpcode, def, conversion), mustPreserveNaN_(false) {
    ...
    explicit MToFloat32(MDefinition* def, bool mustPreserveNaN) : MToFloat32(def) {
   ...
   public:
    ...
    bool congruentTo(const MDefinition* ins) const override {
        ...
        return other->conversion() == conversion() && other->mustPreserveNaN_ == mustPreserveNaN_;
    }
    AliasSet getAliasSet() const override { return AliasSet::None(); }
...
class MWasmUnsignedToFloat32 : public MUnaryInstruction, public NoTypePolicy::Data {
    explicit MWasmUnsignedToFloat32(MDefinition* def) : MUnaryInstruction(classOpcode, def) {
        setResultType(MIRType::Float32);
        setMovable();
    }
...
class MExtendInt32ToInt64 : public MUnaryInstruction, public NoTypePolicy::Data {
    bool isUnsigned_;
...
class MWasmTruncateToInt64 : public MUnaryInstruction, public NoTypePolicy::Data {
    bool isUnsigned_;
    wasm::BytecodeOffset bytecodeOffset_;
...
class MWasmTruncateToInt32 : public MUnaryInstruction, public NoTypePolicy::Data {
    bool isUnsigned_;
    wasm::BytecodeOffset bytecodeOffset_;
...
class MInt64ToFloatingPoint : public MUnaryInstruction, public NoTypePolicy::Data {
    bool isUnsigned_;
    wasm::BytecodeOffset bytecodeOffset_;
...
class MToInt32 : public MUnaryInstruction, public ToInt32Policy::Data {
    ...
    explicit MToInt32(MDefinition* def, MacroAssembler::IntConversionInputKind conversion =
        ...
        : MUnaryInstruction(classOpcode, def), canBeNegativeZero_(true), conversion_(conversion) {
        setResultType(MIRType::Int32);
        setMovable();
   ...
   public:
    ...
    void setCanBeNegativeZero(bool negativeZero) { canBeNegativeZero_ = negativeZero; }

    MacroAssembler::IntConversionInputKind conversion() const { return conversion_; }

    bool congruentTo(const MDefinition* ins) const override {
        if (!ins->isToInt32() || ins->toToInt32()->conversion() != conversion()) return false;
        return congruentIfOperandsEqual(ins);
    }
...
class MTruncateToInt32 : public MUnaryInstruction, public ToInt32Policy::Data {
   ...
   public:
    ...
    MOZ_MUST_USE bool writeRecoverData(CompactBufferWriter& writer) const override;
    bool canRecoverOnBailout() const override { return input()->type() < MIRType::Symbol; }
...
class MToString : public MUnaryInstruction, public ToStringPolicy::Data {
   ...
   public:
    ...
    bool fallible() const {
        return input()->mightBeType(MIRType::Object) || input()->mightBeType(MIRType::Symbol);
    }
...
class MToObjectOrNull : public MUnaryInstruction, public BoxInputsPolicy::Data {
    explicit MToObjectOrNull(MDefinition* def) : MUnaryInstruction(classOpcode, def) {
...
class MBitNot : public MUnaryInstruction, public BitwisePolicy::Data {
   ...
   public:
    ...
    MOZ_MUST_USE bool writeRecoverData(CompactBufferWriter& writer) const override;
    bool canRecoverOnBailout() const override { return specialization_ != MIRType::None; }
...
class MToAsync : public MUnaryInstruction, public SingleObjectPolicy::Data {
    explicit MToAsync(MDefinition* unwrapped) : MUnaryInstruction(classOpcode, unwrapped) {
        setResultType(MIRType::Object);
    }
...
class MToAsyncGen : public MUnaryInstruction, public SingleObjectPolicy::Data {
    explicit MToAsyncGen(MDefinition* unwrapped) : MUnaryInstruction(classOpcode, unwrapped) {
        setResultType(MIRType::Object);
    }
...
class MToAsyncIter : public MUnaryInstruction, public SingleObjectPolicy::Data {
    explicit MToAsyncIter(MDefinition* unwrapped) : MUnaryInstruction(classOpcode, unwrapped) {
        setResultType(MIRType::Object);
    }
...
class MBinaryBitwiseInstruction : public MBinaryInstruction, public BitwisePolicy::Data {
   protected:
   ...
   public:
    ...
    bool congruentTo(const MDefinition* ins) const override { return binaryCongruentTo(ins); }
    AliasSet getAliasSet() const override {
        if (specialization_ >= MIRType::Object) return AliasSet::Store(AliasSet::Any);
        return AliasSet::None();
    }
...
class MBitXor : public MBinaryBitwiseInstruction {
   ...
   public:
    ...
    MOZ_MUST_USE bool writeRecoverData(CompactBufferWriter& writer) const override;
    bool canRecoverOnBailout() const override { return specialization_ < MIRType::Object; }
...
class MUrsh : public MShiftInstruction {
    ...
    MUrsh(MDefinition* left, MDefinition* right, MIRType type)
        : MShiftInstruction(classOpcode, left, right, type), bailoutsDisabled_(false) {}
...
class MBinaryArithInstruction : public MBinaryInstruction, public ArithPolicy::Data {
   ...
   public:
    ...
    static MBinaryArithInstruction* New(TempAllocator& alloc, Opcode op, MDefinition* left,
                                        MDefinition* right);
    ...
    void setNumberSpecialization(TempAllocator& alloc, BaselineInspector* inspector,
                                 jsbytecode* pc);
...
class MPopcnt : public MUnaryInstruction, public BitwisePolicy::Data {
    explicit MPopcnt(MDefinition* num, MIRType type) : MUnaryInstruction(classOpcode, num) {
        MOZ_ASSERT(IsNumberType(num->type()));
        MOZ_ASSERT(IsIntType(type));
        specialization_ = type;
        setResultType(type);
...
class MAtan2 : public MBinaryInstruction,
    ...
    MAtan2(MDefinition* y, MDefinition* x) : MBinaryInstruction(classOpcode, y, x) {
        setResultType(MIRType::Double);
        setMovable();
    }
...
class MPowHalf : public MUnaryInstruction, public DoublePolicy<0>::Data {
   ...
   public:
    ...
    bool operandIsNeverNegativeInfinity() const { return operandIsNeverNegativeInfinity_; }
    bool operandIsNeverNegativeZero() const { return operandIsNeverNegativeZero_; }
...
class MRandom : public MNullaryInstruction {
    MRandom() : MNullaryInstruction(classOpcode) { setResultType(MIRType::Double); }
...
class MMathFunction : public MUnaryInstruction, public FloatingPointPolicy<0>::Data {
   public:
    enum Function {
        Log,
        Sin,
...
class MDiv : public MBinaryArithInstruction {
   ...
   public:
    ...
    bool isTruncatedIndirectly() const { return truncateKind() >= IndirectTruncate; }

    bool canTruncateInfinities() const { return isTruncated(); }
    bool canTruncateRemainder() const { return isTruncated(); }
    bool canTruncateOverflow() const { return isTruncated() || isTruncatedIndirectly(); }
    bool canTruncateNegativeZero() const { return isTruncated() || isTruncatedIndirectly(); }
...
class MMod : public MBinaryArithInstruction {
   ...
   public:
    ...
    static MMod* New(TempAllocator& alloc, MDefinition* left, MDefinition* right, MIRType type,
                     bool unsignd, bool trapOnError = false,
                     wasm::BytecodeOffset bytecodeOffset = wasm::BytecodeOffset()) {
...
class MConcat : public MBinaryInstruction,
    ...
    MConcat(MDefinition* left, MDefinition* right) : MBinaryInstruction(classOpcode, left, right) {
...
class MCharCodeAt : public MBinaryInstruction,
...
class MFromCharCode : public MUnaryInstruction, public IntPolicy<0>::Data {
...
class MFromCodePoint : public MUnaryInstruction, public IntPolicy<0>::Data {
...
class MStringConvertCase : public MUnaryInstruction, public StringPolicy<0>::Data {
   public:
...
class MStringSplit : public MBinaryInstruction,
   ...
   public:
    ...
    MOZ_MUST_USE bool writeRecoverData(CompactBufferWriter& writer) const override;
    bool canRecoverOnBailout() const override { return true; }
    bool appendRoots(MRootList& roots) const override { return roots.append(group_); }
};
...
class MComputeThis : public MUnaryInstruction, public BoxPolicy<0>::Data {
    explicit MComputeThis(MDefinition* def) : MUnaryInstruction(classOpcode, def) {
        setResultType(MIRType::Value);
    }
...
class MArrowNewTarget : public MUnaryInstruction, public SingleObjectPolicy::Data {
...
class MPhi final : public MDefinition, public InlineListNode<MPhi>, public NoTypePolicy::Data {
   ...
   public:
    ...
    MDefinition* getOperand(size_t index) const override { return inputs_[index].producer(); }
    ...
    MOZ_MUST_USE bool reserveLength(size_t length) { return inputs_.reserve(length); }
    ...
    MOZ_MUST_USE bool addInputSlow(MDefinition* ins) { return inputs_.emplaceBack(ins, this); }
    ...
    bool canConsumeFloat32(MUse* use) const override { return canConsumeFloat32_; }
...
class MOsrEnvironmentChain : public MUnaryInstruction, public NoTypePolicy::Data {
   private:
    explicit MOsrEnvironmentChain(MOsrEntry* entry) : MUnaryInstruction(classOpcode, entry) {
        setResultType(MIRType::Object);
    }
...
// The pointer is indexed off of OsrFrameReg.
class MOsrArgumentsObject : public MUnaryInstruction, public NoTypePolicy::Data {
   private:
    explicit MOsrArgumentsObject(MOsrEntry* entry) : MUnaryInstruction(classOpcode, entry) {
        setResultType(MIRType::Object);
    }
...
class MOsrReturnValue : public MUnaryInstruction, public NoTypePolicy::Data {
   private:
    explicit MOsrReturnValue(MOsrEntry* entry) : MUnaryInstruction(classOpcode, entry) {
        setResultType(MIRType::Value);
    }
...
class MUnarySharedStub : public MUnaryInstruction, public BoxPolicy<0>::Data {
    explicit MUnarySharedStub(MDefinition* input) : MUnaryInstruction(classOpcode, input) {
        setResultType(MIRType::Value);
    }
...
class MWasmTrap : public MAryControlInstruction<0, 0>, public NoTypePolicy::Data {
    wasm::Trap trap_;
    wasm::BytecodeOffset bytecodeOffset_;

    explicit MWasmTrap(wasm::Trap trap, wasm::BytecodeOffset bytecodeOffset)
        : MAryControlInstruction(classOpcode), trap_(trap), bytecodeOffset_(bytecodeOffset) {}
...
class MRegExp : public MNullaryInstruction {
   ...
   public:
    ...
    AliasSet getAliasSet() const override { return AliasSet::None(); }
    bool possiblyCalls() const override { return true; }
    bool appendRoots(MRootList& roots) const override { return roots.append(source_); }
};

class MRegExpMatcher : public MTernaryInstruction,
                       public MixPolicy<ObjectPolicy<0>, StringPolicy<1>, IntPolicy<2>>::Data {
   private:
...
class MRegExpPrototypeOptimizable : public MUnaryInstruction, public SingleObjectPolicy::Data {
...
class MGetFirstDollarIndex : public MUnaryInstruction, public StringPolicy<0>::Data {
    explicit MGetFirstDollarIndex(MDefinition* str) : MUnaryInstruction(classOpcode, str) {
        setResultType(MIRType::Int32);
        setMovable();
    }
...
class MStringReplace : public MTernaryInstruction,
   ...
   public:
    ...
    bool congruentTo(const MDefinition* ins) const override {
        ...
        if (isFlatReplacement_ != ins->toStringReplace()->isFlatReplacement()) return false;
        return congruentIfOperandsEqual(ins);
    }
...
class MClassConstructor : public MNullaryInstruction {
    ...
    explicit MClassConstructor(jsbytecode* pc) : MNullaryInstruction(classOpcode), pc_(pc) {
        setResultType(MIRType::Object);
    }
...
struct LambdaFunctionInfo {
    ...
    bool appendRoots(MRootList& roots) const {
        if (!roots.append(fun)) return false;
        if (fun->hasScript()) return roots.append(fun->nonLazyScript());
...
class MLambda : public MBinaryInstruction, public SingleObjectPolicy::Data {
   ...
   public:
    ...
    MOZ_MUST_USE bool writeRecoverData(CompactBufferWriter& writer) const override;
    bool canRecoverOnBailout() const override { return true; }
    bool appendRoots(MRootList& roots) const override { return info_.appendRoots(roots); }
};

class MLambdaArrow : public MTernaryInstruction,
                     public MixPolicy<ObjectPolicy<0>, BoxPolicy<1>, ObjectPolicy<2>>::Data {
    const LambdaFunctionInfo info_;
...
class MElements : public MUnaryInstruction, public SingleObjectPolicy::Data {
   ...
   public:
    ...
    bool congruentTo(const MDefinition* ins) const override {
        return congruentIfOperandsEqual(ins) && ins->toElements()->unboxed() == unboxed();
    }
    AliasSet getAliasSet() const override { return AliasSet::Load(AliasSet::ObjectFields); }
...
class MConstantElements : public MNullaryInstruction {
   ...
   protected:
    explicit MConstantElements(SharedMem<void*> v) : MNullaryInstruction(classOpcode), value_(v) {
        setResultType(MIRType::Elements);
        setMovable();
    }
   ...
   public:
    ...
    HashNumber valueHash() const override { return (HashNumber)(size_t)value_.asValue(); }
...
class MConvertElementsToDoubles : public MUnaryInstruction, public NoTypePolicy::Data {
...
class MMaybeToDoubleElement : public MBinaryInstruction, public IntPolicy<1>::Data {
...
class MMaybeCopyElementsForWrite : public MUnaryInstruction, public SingleObjectPolicy::Data {
    bool checkNative_;
   ...
   public:
    ...
    AliasSet getAliasSet() const override { return AliasSet::Store(AliasSet::ObjectFields); }
#ifdef DEBUG
...
class MGetNextEntryForIterator : public MBinaryInstruction,
                                 public MixPolicy<ObjectPolicy<0>, ObjectPolicy<1>>::Data {
   public:
    enum Mode { Map, Set };
...
class MTypedArrayLength : public MUnaryInstruction, public SingleObjectPolicy::Data {
    explicit MTypedArrayLength(MDefinition* obj) : MUnaryInstruction(classOpcode, obj) {
        setResultType(MIRType::Int32);
        setMovable();
    }
   ...
   public:
    ...
    AliasSet getAliasSet() const override { return AliasSet::Load(AliasSet::TypedArrayLength); }

    void computeRange(TempAllocator& alloc) override;
};

// Load a typed array's elements vector.
class MTypedArrayElements : public MUnaryInstruction, public SingleObjectPolicy::Data {
    explicit MTypedArrayElements(MDefinition* object) : MUnaryInstruction(classOpcode, object) {
        setResultType(MIRType::Elements);
        setMovable();
    }
...
class MSetDisjointTypedElements : public MTernaryInstruction, public NoTypePolicy::Data {
   ...
   public:
    ...
    static MSetDisjointTypedElements* New(TempAllocator& alloc, MDefinition* target,
                                          MDefinition* targetOffset, MDefinition* source) {
        return new (alloc) MSetDisjointTypedElements(target, targetOffset, source);
    }

    AliasSet getAliasSet() const override { return AliasSet::Store(AliasSet::UnboxedElement); }
...
class MTypedObjectElements : public MUnaryInstruction, public SingleObjectPolicy::Data {
    bool definitelyOutline_;

   private:
    explicit MTypedObjectElements(MDefinition* object, bool definitelyOutline)
        : MUnaryInstruction(classOpcode, object), definitelyOutline_(definitelyOutline) {
        setResultType(MIRType::Elements);
        setMovable();
    }
...
class MSetTypedObjectOffset : public MBinaryInstruction, public NoTypePolicy::Data {
   private:
...
class MKeepAliveObject : public MUnaryInstruction, public SingleObjectPolicy::Data {
...
class MBoundsCheck : public MBinaryInstruction,
   ...
   public:
    ...
    bool congruentTo(const MDefinition* ins) const override {
        ...
        if (minimum() != other->minimum() || maximum() != other->maximum()) return false;
...
class MBoundsCheckLower : public MUnaryInstruction, public IntPolicy<0>::Data {
    int32_t minimum_;
    bool fallible_;
...
// with the access.
static inline bool IsValidElementsType(MDefinition* elements, int32_t offsetAdjustment) {
...
class MLoadElement : public MBinaryInstruction, public SingleObjectPolicy::Data {
...
class MLoadElementHole : public MTernaryInstruction, public SingleObjectPolicy::Data {
...
class MLoadUnboxedObjectOrNull : public MBinaryInstruction, public SingleObjectPolicy::Data {
   public:
   ...
   private:
    ...
    MLoadUnboxedObjectOrNull(MDefinition* elements, MDefinition* index, NullBehavior nullBehavior,
   ...
   public:
    ...
    AliasSet getAliasSet() const override { return AliasSet::Load(AliasSet::UnboxedElement); }
    AliasType mightAlias(const MDefinition* store) const override;
    MDefinition* foldsTo(TempAllocator& alloc) override;
...
class MLoadUnboxedString : public MBinaryInstruction, public SingleObjectPolicy::Data {
    int32_t offsetAdjustment_;

    MLoadUnboxedString(MDefinition* elements, MDefinition* index, int32_t offsetAdjustment = 0)
        : MBinaryInstruction(classOpcode, elements, index), offsetAdjustment_(offsetAdjustment) {
        setResultType(MIRType::String);
        setMovable();
        MOZ_ASSERT(IsValidElementsType(elements, offsetAdjustment));
        MOZ_ASSERT(index->type() == MIRType::Int32);
...
class MLoadElementFromState : public MBinaryInstruction, public SingleObjectPolicy::Data {
...
class MStoreElement : public MTernaryInstruction,
   ...
   public:
    ...
    AliasSet getAliasSet() const override { return AliasSet::Store(AliasSet::Element); }
    bool needsHoleCheck() const { return needsHoleCheck_; }
    int32_t offsetAdjustment() const { return offsetAdjustment_; }
    bool fallible() const { return needsHoleCheck(); }
...
// vector.
class MStoreElementHole : public MQuaternaryInstruction,
                          public MStoreElementCommon,
                          public MixPolicy<SingleObjectPolicy, NoFloatPolicy<3>>::Data {
    MStoreElementHole(MDefinition* object, MDefinition* elements, MDefinition* index,
                      MDefinition* value)
...
class MFallibleStoreElement : public MQuaternaryInstruction,
                              public MStoreElementCommon,
                              public MixPolicy<SingleObjectPolicy, NoFloatPolicy<3>>::Data {
    bool strict_;

    MFallibleStoreElement(MDefinition* object, MDefinition* elements, MDefinition* index,
                          MDefinition* value, bool strict)
        : MQuaternaryInstruction(classOpcode, object, elements, index, value), strict_(strict) {
        MOZ_ASSERT(elements->type() == MIRType::Elements);
        MOZ_ASSERT(index->type() == MIRType::Int32);
    }
...
class MStoreUnboxedObjectOrNull : public MQuaternaryInstruction,
    ...
    MStoreUnboxedObjectOrNull(MDefinition* elements, MDefinition* index, MDefinition* value,
                              MDefinition* typedObj, int32_t offsetAdjustment = 0,
...
class MStoreUnboxedString : public MTernaryInstruction,
...
class MConvertUnboxedObjectToNative : public MUnaryInstruction, public SingleObjectPolicy::Data {
    CompilerObjectGroup group_;
...
class MArrayPopShift : public MUnaryInstruction, public SingleObjectPolicy::Data {
   public:
    enum Mode { Pop, Shift };
...
class MArraySlice : public MTernaryInstruction,
    ...
    MArraySlice(CompilerConstraintList* constraints, MDefinition* obj, MDefinition* begin,
                MDefinition* end, JSObject* templateObj, gc::InitialHeap initialHeap)
...
class MArrayJoin : public MBinaryInstruction,
    ...
    MArrayJoin(MDefinition* array, MDefinition* sep, bool optimizeForArray)
        : MBinaryInstruction(classOpcode, array, sep), optimizeForArray_(optimizeForArray) {
        setResultType(MIRType::String);
    }
...
enum MemoryBarrierRequirement { DoesNotRequireMemoryBarrier, DoesRequireMemoryBarrier };

// Also see comments at MMemoryBarrierRequirement, above.

// Load an unboxed scalar value from a typed array or other object.
class MLoadUnboxedScalar : public MBinaryInstruction, public SingleObjectPolicy::Data {
   ...
   public:
    ...
    int32_t offsetAdjustment() const { return offsetAdjustment_; }
    void setOffsetAdjustment(int32_t offsetAdjustment) { offsetAdjustment_ = offsetAdjustment; }
    AliasSet getAliasSet() const override {
...
class MLoadTypedArrayElementHole : public MBinaryInstruction, public SingleObjectPolicy::Data {
    Scalar::Type arrayType_;
    bool allowDouble_;

    MLoadTypedArrayElementHole(MDefinition* object, MDefinition* index, Scalar::Type arrayType,
                               bool allowDouble)
   ...
   public:
    ...
    bool fallible() const { return arrayType_ == Scalar::Uint32 && !allowDouble_; }
    bool congruentTo(const MDefinition* ins) const override {
...
class MLoadTypedArrayElementStatic : public MUnaryInstruction,
    ...
    MLoadTypedArrayElementStatic(JSObject* someTypedArray, MDefinition* ptr, int32_t offset = 0,
                                 bool needsBoundsCheck = true)
        : MUnaryInstruction(classOpcode, ptr),
   ...
   public:
    ...
    Scalar::Type accessType() const { return someTypedArray_->as<TypedArrayObject>().type(); }
    ...
    void setInfallible() { fallible_ = false; }
    ...
    bool appendRoots(MRootList& roots) const override { return roots.append(someTypedArray_); }
};
...
class StoreUnboxedScalarBase {
   ...
   protected:
    explicit StoreUnboxedScalarBase(Scalar::Type writeType) : writeType_(writeType) {
   ...
   public:
    ...
    bool isIntegerWrite() const {
        return isByteWrite() || writeType_ == Scalar::Int16 || writeType_ == Scalar::Uint16 ||
               writeType_ == Scalar::Int32 || writeType_ == Scalar::Uint32;
    }
...
class MStoreTypedArrayElementStatic : public MBinaryInstruction,
                                      public StoreUnboxedScalarBase,
   ...
   public:
    ...
    Scalar::Type accessType() const { return writeType(); }
...
class MClampToUint8 : public MUnaryInstruction, public ClampPolicy::Data {
    explicit MClampToUint8(MDefinition* input) : MUnaryInstruction(classOpcode, input) {
        setResultType(MIRType::Int32);
        setMovable();
    }
...
class MLoadFixedSlot : public MUnaryInstruction, public SingleObjectPolicy::Data {
    size_t slot_;
   ...
   public:
    ...
    AliasSet getAliasSet() const override { return AliasSet::Load(AliasSet::FixedSlot); }
...
class MLoadFixedSlotAndUnbox : public MUnaryInstruction, public SingleObjectPolicy::Data {
    size_t slot_;
    MUnbox::Mode mode_;
    BailoutKind bailoutKind_;
   ...
   public:
    ...
    bool congruentTo(const MDefinition* ins) const override {
        if (!ins->isLoadFixedSlotAndUnbox() || slot() != ins->toLoadFixedSlotAndUnbox()->slot() ||
...
class MStoreFixedSlot : public MBinaryInstruction,
                        public MixPolicy<SingleObjectPolicy, NoFloatPolicy<1>>::Data {
    bool needsBarrier_;
    size_t slot_;

    MStoreFixedSlot(MDefinition* obj, MDefinition* rval, size_t slot, bool barrier)
        : MBinaryInstruction(classOpcode, obj, rval), needsBarrier_(barrier), slot_(slot) {}
   ...
   public:
    ...
    AliasSet getAliasSet() const override { return AliasSet::Store(AliasSet::FixedSlot); }
    bool needsBarrier() const { return needsBarrier_; }
    void setNeedsBarrier(bool needsBarrier = true) { needsBarrier_ = needsBarrier; }
...
struct InliningTarget {
    ...
    InliningTarget(JSObject* target, ObjectGroup* group) : target(target), group(group) {
...
class MHomeObjectSuperBase : public MUnaryInstruction, public SingleObjectPolicy::Data {
...
class MGetPropSuperCache : public MTernaryInstruction,
...
class MGetPropertyPolymorphic : public MUnaryInstruction, public SingleObjectPolicy::Data {
   ...
   public:
    ...
    const ReceiverGuard receiver(size_t i) const { return receivers_[i].receiver; }
...
class MSetPropertyPolymorphic : public MBinaryInstruction,
                                public MixPolicy<SingleObjectPolicy, NoFloatPolicy<1>>::Data {
    Vector<PolymorphicEntry, 4, JitAllocPolicy> receivers_;
    CompilerPropertyName name_;
    bool needsBarrier_;
...
class MDispatchInstruction : public MControlInstruction, public SingleObjectPolicy::Data {
   ...
   protected:
    ...
    size_t numOperands() const final override { return 1; }
    size_t indexOf(const MUse* u) const final override {
        MOZ_ASSERT(u == getUseFor(0));
        return 0;
    }
   ...
   public:
    ...
    size_t numSuccessors() const final override { return map_.length() + (fallback_ ? 1 : 0); }
   ...
   public:
    ...
    ObjectGroup* getCaseObjectGroup(uint32_t i) const { return map_[i].funcGroup; }
...
class MObjectGroupDispatch : public MDispatchInstruction {
    ...
    MObjectGroupDispatch(TempAllocator& alloc, MDefinition* input, InlinePropertyTable* table)
        : MDispatchInstruction(alloc, classOpcode, input), inlinePropertyTable_(table) {}
...
class MBindNameCache : public MUnaryInstruction, public SingleObjectPolicy::Data {
    CompilerPropertyName name_;
...
class MCallBindVar : public MUnaryInstruction, public SingleObjectPolicy::Data {
    explicit MCallBindVar(MDefinition* envChain) : MUnaryInstruction(classOpcode, envChain) {
        setResultType(MIRType::Object);
        setMovable();
    }
...
class MGuardShape : public MUnaryInstruction, public SingleObjectPolicy::Data {
    ...
    MGuardShape(MDefinition* obj, Shape* shape, BailoutKind bailoutKind)
        : MUnaryInstruction(classOpcode, obj), shape_(shape), bailoutKind_(bailoutKind) {
        setGuard();
        setMovable();
        setResultType(MIRType::Object);
   ...
   public:
    ...
    AliasSet getAliasSet() const override { return AliasSet::Load(AliasSet::ObjectFields); }
    bool appendRoots(MRootList& roots) const override { return roots.append(shape_); }
};

// Bail if the object's shape or unboxed group is not in the input list.
class MGuardReceiverPolymorphic : public MUnaryInstruction, public SingleObjectPolicy::Data {
...
class MGuardObjectGroup : public MUnaryInstruction, public SingleObjectPolicy::Data {
    CompilerObjectGroup group_;
   ...
   public:
    ...
    bool congruentTo(const MDefinition* ins) const override {
        ...
        if (bailOnEquality() != ins->toGuardObjectGroup()->bailOnEquality()) return false;
...
class MGuardObjectIdentity : public MBinaryInstruction, public SingleObjectPolicy::Data {
    bool bailOnEquality_;

    MGuardObjectIdentity(MDefinition* obj, MDefinition* expected, bool bailOnEquality)
        : MBinaryInstruction(classOpcode, obj, expected), bailOnEquality_(bailOnEquality) {
        setGuard();
        setMovable();
        setResultType(MIRType::Object);
    }
   ...
   public:
    ...
    bool congruentTo(const MDefinition* ins) const override {
        ...
        if (bailOnEquality() != ins->toGuardObjectIdentity()->bailOnEquality()) return false;
        return congruentIfOperandsEqual(ins);
    }
    AliasSet getAliasSet() const override { return AliasSet::Load(AliasSet::ObjectFields); }
};
...
class MGuardClass : public MUnaryInstruction, public SingleObjectPolicy::Data {
    const Class* class_;
   ...
   public:
    ...
    bool congruentTo(const MDefinition* ins) const override {
        if (!ins->isGuardClass()) return false;
        if (getClass() != ins->toGuardClass()->getClass()) return false;
        return congruentIfOperandsEqual(ins);
    }
    AliasSet getAliasSet() const override { return AliasSet::Load(AliasSet::ObjectFields); }
...
class MGuardUnboxedExpando : public MUnaryInstruction, public SingleObjectPolicy::Data {
   ...
   public:
    ...
    bool congruentTo(const MDefinition* ins) const override {
        if (!congruentIfOperandsEqual(ins)) return false;
        if (requireExpando() != ins->toGuardUnboxedExpando()->requireExpando()) return false;
        return true;
    }
    AliasSet getAliasSet() const override { return AliasSet::Load(AliasSet::ObjectFields); }
};
...
class MLoadUnboxedExpando : public MUnaryInstruction, public SingleObjectPolicy::Data {
   private:
    explicit MLoadUnboxedExpando(MDefinition* object) : MUnaryInstruction(classOpcode, object) {
        setResultType(MIRType::Object);
        setMovable();
    }
...
class MFunctionEnvironment : public MUnaryInstruction, public SingleObjectPolicy::Data {
...
class MNewLexicalEnvironmentObject : public MUnaryInstruction, public SingleObjectPolicy::Data {
   ...
   public:
    ...
    bool possiblyCalls() const override { return true; }
    bool appendRoots(MRootList& roots) const override { return roots.append(scope_); }
    AliasSet getAliasSet() const override { return AliasSet::None(); }
};

// Allocate a new LexicalEnvironmentObject from existing one
class MCopyLexicalEnvironmentObject : public MUnaryInstruction, public SingleObjectPolicy::Data {
    bool copySlots_;
...
class MStoreSlot : public MBinaryInstruction,
   ...
   public:
    ...
    bool needsBarrier() const { return needsBarrier_; }
    void setNeedsBarrier() { needsBarrier_ = true; }
    AliasSet getAliasSet() const override { return AliasSet::Store(AliasSet::DynamicSlot); }
    void printOpcode(GenericPrinter& out) const override;

    ALLOW_CLONE(MStoreSlot)
};

class MGetNameCache : public MUnaryInstruction, public SingleObjectPolicy::Data {
   private:
    explicit MGetNameCache(MDefinition* obj) : MUnaryInstruction(classOpcode, obj) {
        setResultType(MIRType::Value);
    }
...
class MCallSetProperty : public MSetPropertyInstruction, public CallSetElementPolicy::Data {
...
class MCallGetProperty : public MUnaryInstruction, public BoxInputsPolicy::Data {
    CompilerPropertyName name_;
    bool idempotent_;
...
class MCallGetElement : public MBinaryInstruction, public BoxInputsPolicy::Data {
...
class MCallSetElement : public MSetElementInstruction, public CallSetElementPolicy::Data {
...
class MCallInitElementArray : public MTernaryInstruction,
                              public MixPolicy<ObjectPolicy<0>, IntPolicy<1>, BoxPolicy<2>>::Data {
...
class MSetDOMProperty : public MBinaryInstruction,
    ...
    MSetDOMProperty(const JSJitSetterOp func, MDefinition* obj, MDefinition* val)
        : MBinaryInstruction(classOpcode, obj, val), func_(func) {}
...
class MGetDOMProperty : public MVariadicInstruction, public ObjectPolicy<0>::Data {
   ...
   public:
    ...
    AliasSet getAliasSet() const override {
        ...
        if (aliasSet == JSJitInfo::AliasDOMSets) return AliasSet::Load(AliasSet::DOMProperty);
...
class MGetDOMMember : public MGetDOMProperty {
    ...
    explicit MGetDOMMember(const JSJitInfo* jitinfo) : MGetDOMProperty(classOpcode, jitinfo) {
...
// to the RoundingMode.
class MNearbyInt : public MUnaryInstruction, public FloatingPointPolicy<0>::Data {
...
class MGetIteratorCache : public MUnaryInstruction,
    ...
    explicit MGetIteratorCache(MDefinition* val) : MUnaryInstruction(classOpcode, val) {
        setResultType(MIRType::Object);
    }
...
class MIteratorMore : public MUnaryInstruction, public SingleObjectPolicy::Data {
    explicit MIteratorMore(MDefinition* iter) : MUnaryInstruction(classOpcode, iter) {
        setResultType(MIRType::Value);
    }
...
class MIteratorEnd : public MUnaryInstruction, public SingleObjectPolicy::Data {
    explicit MIteratorEnd(MDefinition* iter) : MUnaryInstruction(classOpcode, iter) {}
...
class MInCache : public MBinaryInstruction,
    ...
    MInCache(MDefinition* key, MDefinition* obj) : MBinaryInstruction(classOpcode, key, obj) {
        setResultType(MIRType::Boolean);
    }
...
class MHasOwnCache
    : public MBinaryInstruction,
      public MixPolicy<BoxExceptPolicy<0, MIRType::Object>, CacheIdPolicy<1>>::Data {
    MHasOwnCache(MDefinition* obj, MDefinition* id) : MBinaryInstruction(classOpcode, obj, id) {
        setResultType(MIRType::Boolean);
    }
...
class MInstanceOf : public MUnaryInstruction, public InstanceOfPolicy::Data {
   ...
   public:
    ...
    bool appendRoots(MRootList& roots) const override { return roots.append(protoObj_); }
};

// Implementation for instanceof operator with unknown rhs.
class MCallInstanceOf : public MBinaryInstruction,
                        public MixPolicy<BoxPolicy<0>, ObjectPolicy<1>>::Data {
...
class MGetFrameArgument : public MUnaryInstruction, public IntPolicy<0>::Data {
...
class MSetFrameArgument : public MUnaryInstruction, public NoFloatPolicy<0>::Data {
    uint32_t argno_;
   ...
   public:
    ...
    AliasSet getAliasSet() const override { return AliasSet::Store(AliasSet::FrameArgument); }
};
...
class MRest : public MUnaryInstruction, public MRestCommon, public IntPolicy<0>::Data {
    MRest(TempAllocator& alloc, CompilerConstraintList* constraints, MDefinition* numActuals,
        ...
        : MUnaryInstruction(classOpcode, numActuals), MRestCommon(numFormals, templateObject) {
        setResultType(MIRType::Object);
        setResultTypeSet(MakeSingletonTypeSet(alloc, constraints, templateObject));
    }
   ...
   public:
    ...
    AliasSet getAliasSet() const override { return AliasSet::None(); }
    bool possiblyCalls() const override { return true; }
    bool appendRoots(MRootList& roots) const override { return roots.append(templateObject()); }
};

class MFilterTypeSet : public MUnaryInstruction, public FilterTypeSetPolicy::Data {
   ...
   public:
    ...
    bool isFloat32Commutative() const override { return IsFloatingPointType(type()); }
...
class MMonitorTypes : public MUnaryInstruction, public BoxInputsPolicy::Data {
    ...
    MMonitorTypes(MDefinition* def, const TemporaryTypeSet* types, BarrierKind kind)
        : MUnaryInstruction(classOpcode, def), typeSet_(types), barrierKind_(kind) {
        MOZ_ASSERT(kind == BarrierKind::TypeTagOnly || kind == BarrierKind::TypeSet);
   ...
   public:
    ...
    const TemporaryTypeSet* typeSet() const { return typeSet_; }
    BarrierKind barrierKind() const { return barrierKind_; }
...
class MNewCallObjectBase : public MUnaryInstruction, public SingleObjectPolicy::Data {
   protected:
    MNewCallObjectBase(Opcode op, MConstant* templateObj) : MUnaryInstruction(op, templateObj) {
        setResultType(MIRType::Object);
    }
...
class MNewStringObject : public MUnaryInstruction, public ConvertToStringPolicy<0>::Data {
    CompilerObject templateObj_;
...
#endif
{
   ...
   private:
    ...
    void setBlock(MBasicBlock* block) { setBlockAndKind(block, Kind::ResumePoint); }
   ...
   protected:
    ...
    const MUse* getUseFor(size_t index) const override { return &operands_[index]; }

   public:
    static MResumePoint* New(TempAllocator& alloc, MBasicBlock* block, jsbytecode* pc, Mode mode);
    ...
    MDefinition* getOperand(size_t index) const override { return operands_[index].producer(); }
    jsbytecode* pc() const { return pc_; }
...
class MIsCallable : public MUnaryInstruction, public BoxExceptPolicy<0, MIRType::Object>::Data {
    explicit MIsCallable(MDefinition* object) : MUnaryInstruction(classOpcode, object) {
...
class MIsConstructor : public MUnaryInstruction, public SingleObjectPolicy::Data {
   public:
...
class MIsArray : public MUnaryInstruction, public BoxExceptPolicy<0, MIRType::Object>::Data {
    explicit MIsArray(MDefinition* value) : MUnaryInstruction(classOpcode, value) {
        setResultType(MIRType::Boolean);
    }
...
class MIsTypedArray : public MUnaryInstruction, public SingleObjectPolicy::Data {
    explicit MIsTypedArray(MDefinition* value) : MUnaryInstruction(classOpcode, value) {
        setResultType(MIRType::Boolean);
        setMovable();
    }
...
class MObjectClassToString : public MUnaryInstruction, public SingleObjectPolicy::Data {
    explicit MObjectClassToString(MDefinition* obj) : MUnaryInstruction(classOpcode, obj) {
        setMovable();
        setResultType(MIRType::String);
    }
...
class MRecompileCheck : public MNullaryInstruction {
   public:
    enum RecompileCheckType { RecompileCheck_OptimizationLevel, RecompileCheck_Inlining };
...
class MAtomicIsLockFree : public MUnaryInstruction, public ConvertToInt32Policy<0>::Data {
    explicit MAtomicIsLockFree(MDefinition* value) : MUnaryInstruction(classOpcode, value) {
        setResultType(MIRType::Boolean);
        setMovable();
    }
...
class MGuardSharedTypedArray : public MUnaryInstruction, public SingleObjectPolicy::Data {
    explicit MGuardSharedTypedArray(MDefinition* obj) : MUnaryInstruction(classOpcode, obj) {
        setGuard();
        setMovable();
    }
...
class MCompareExchangeTypedArrayElement
    : public MQuaternaryInstruction,
      public MixPolicy<ObjectPolicy<0>, IntPolicy<1>, TruncateToInt32Policy<2>,
...
class MAtomicExchangeTypedArrayElement
    ...
    MAtomicExchangeTypedArrayElement(MDefinition* elements, MDefinition* index, MDefinition* value,
        ...
        : MTernaryInstruction(classOpcode, elements, index, value), arrayType_(arrayType) {
...
class MCheckIsCallable : public MUnaryInstruction, public BoxInputsPolicy::Data {
    uint8_t checkKind_;
...
class MCheckObjCoercible : public MUnaryInstruction, public BoxInputsPolicy::Data {
    explicit MCheckObjCoercible(MDefinition* toCheck) : MUnaryInstruction(classOpcode, toCheck) {
        setGuard();
        setResultType(MIRType::Value);
        setResultTypeSet(toCheck->resultTypeSet());
    }
...
class MDebugCheckSelfHosted : public MUnaryInstruction, public BoxInputsPolicy::Data {
...
class MIsPackedArray : public MUnaryInstruction, public SingleObjectPolicy::Data {
    explicit MIsPackedArray(MDefinition* array) : MUnaryInstruction(classOpcode, array) {
        setResultType(MIRType::Boolean);
        setMovable();
    }
...
class MGetPrototypeOf : public MUnaryInstruction, public SingleObjectPolicy::Data {
...
class MWasmLoadTls : public MUnaryInstruction, public NoTypePolicy::Data {
    ...
    explicit MWasmLoadTls(MDefinition* tlsPointer, uint32_t offset, MIRType type, AliasSet aliases)
        : MUnaryInstruction(classOpcode, tlsPointer), offset_(offset), aliases_(aliases) {
   ...
   public:
    ...
    HashNumber valueHash() const override { return addU32ToHash(HashNumber(op()), offset()); }
...
class MWasmAddOffset : public MUnaryInstruction, public NoTypePolicy::Data {
    ...
    MWasmAddOffset(MDefinition* base, uint32_t offset, wasm::BytecodeOffset bytecodeOffset)
        : MUnaryInstruction(classOpcode, base), offset_(offset), bytecodeOffset_(bytecodeOffset) {
        setGuard();
        setResultType(MIRType::Int32);
    }
...
class MWasmLoad : public MVariadicInstruction,  // memoryBase is nullptr on some platforms
                  public NoTypePolicy::Data {
   ...
   public:
    ...
    static MWasmLoad* New(TempAllocator& alloc, MDefinition* memoryBase, MDefinition* base,
                          const wasm::MemoryAccessDesc& access, MIRType resultType) {
...
class MWasmStore : public MVariadicInstruction, public NoTypePolicy::Data {
   ...
   public:
    ...
    static MWasmStore* New(TempAllocator& alloc, MDefinition* memoryBase, MDefinition* base,
                           const wasm::MemoryAccessDesc& access, MDefinition* value) {
    ...
    AliasSet getAliasSet() const override { return AliasSet::Store(AliasSet::WasmHeap); }
};
...
class MAsmJSLoadHeap
   ...
   public:
    ...
    static MAsmJSLoadHeap* New(TempAllocator& alloc, MDefinition* memoryBase, MDefinition* base,
                               MDefinition* boundsCheckLimit, Scalar::Type accessType) {
        ...
        MAsmJSLoadHeap* load =
            new (alloc) MAsmJSLoadHeap(memoryBaseIndex, boundsCheckIndex, accessType);
    ...
    bool congruentTo(const MDefinition* ins) const override;
    AliasSet getAliasSet() const override { return AliasSet::Load(AliasSet::WasmHeap); }
...
class MAsmJSStoreHeap
   ...
   public:
    ...
    static MAsmJSStoreHeap* New(TempAllocator& alloc, MDefinition* memoryBase, MDefinition* base,
                                MDefinition* boundsCheckLimit, Scalar::Type accessType,
                                MDefinition* v) {
        ...
        MAsmJSStoreHeap* store =
            new (alloc) MAsmJSStoreHeap(memoryBaseIndex, boundsCheckIndex, accessType);
        ...
        if (boundsCheckLimit) store->initOperand(boundsCheckIndex, boundsCheckLimit);
...
class MAsmJSCompareExchangeHeap : public MVariadicInstruction, public NoTypePolicy::Data {
    wasm::MemoryAccessDesc access_;
    wasm::BytecodeOffset bytecodeOffset_;

    explicit MAsmJSCompareExchangeHeap(const wasm::MemoryAccessDesc& access,
                                       wasm::BytecodeOffset bytecodeOffset)
        : MVariadicInstruction(classOpcode), access_(access), bytecodeOffset_(bytecodeOffset) {
        setGuard();  // Not removable
        setResultType(MIRType::Int32);
    }
   ...
   public:
    ...
    static MAsmJSCompareExchangeHeap* New(TempAllocator& alloc,
                                          ...
                                          const wasm::MemoryAccessDesc& access, MDefinition* oldv,
                                          MDefinition* newv, MDefinition* tls) {
        MAsmJSCompareExchangeHeap* cas =
            new (alloc) MAsmJSCompareExchangeHeap(access, bytecodeOffset);
...
class MAsmJSAtomicExchangeHeap : public MVariadicInstruction, public NoTypePolicy::Data {
    wasm::MemoryAccessDesc access_;
    wasm::BytecodeOffset bytecodeOffset_;
   ...
   public:
    ...
    static MAsmJSAtomicExchangeHeap* New(TempAllocator& alloc, wasm::BytecodeOffset bytecodeOffset,
                                         MDefinition* memoryBase, MDefinition* base,
                                         const wasm::MemoryAccessDesc& access, MDefinition* value,
                                         MDefinition* tls) {
        MAsmJSAtomicExchangeHeap* xchg =
            new (alloc) MAsmJSAtomicExchangeHeap(access, bytecodeOffset);
...
class MAsmJSAtomicBinopHeap : public MVariadicInstruction, public NoTypePolicy::Data {
    AtomicOp op_;
    wasm::MemoryAccessDesc access_;
    wasm::BytecodeOffset bytecodeOffset_;
   ...
   public:
    ...
    static MAsmJSAtomicBinopHeap* New(TempAllocator& alloc, wasm::BytecodeOffset bytecodeOffset,
                                      AtomicOp op, MDefinition* memoryBase, MDefinition* base,
                                      const wasm::MemoryAccessDesc& access, MDefinition* v,
                                      MDefinition* tls) {
        MAsmJSAtomicBinopHeap* binop =
            new (alloc) MAsmJSAtomicBinopHeap(op, access, bytecodeOffset);
...
class MWasmStoreGlobalVar : public MBinaryInstruction, public NoTypePolicy::Data {
    MWasmStoreGlobalVar(unsigned globalDataOffset, MDefinition* value, MDefinition* tlsPtr)
        : MBinaryInstruction(classOpcode, value, tlsPtr), globalDataOffset_(globalDataOffset) {}
   ...
   public:
    ...
    AliasSet getAliasSet() const override { return AliasSet::Store(AliasSet::WasmGlobalVar); }
};

class MWasmParameter : public MNullaryInstruction {
    ABIArg abi_;

    MWasmParameter(ABIArg abi, MIRType mirType) : MNullaryInstruction(classOpcode), abi_(abi) {
...
class MWasmReturn : public MAryControlInstruction<1, 0>, public NoTypePolicy::Data {
...
class MWasmReturnVoid : public MAryControlInstruction<0, 0>, public NoTypePolicy::Data {
...
class MWasmCall final : public MVariadicInstruction, public NoTypePolicy::Data {
   ...
   public:
    ...
    static MWasmCall* New(TempAllocator& alloc, const wasm::CallSiteDesc& desc,
                          ...
                          uint32_t spIncrement, MDefinition* tableIndex = nullptr);
...
class MWasmReinterpret : public MUnaryInstruction, public NoTypePolicy::Data {
    MWasmReinterpret(MDefinition* val, MIRType toType) : MUnaryInstruction(classOpcode, val) {
        switch (val->type()) {
            case MIRType::Int32:
...
class MUnknownValue : public MNullaryInstruction {
   protected:
    MUnknownValue() : MNullaryInstruction(classOpcode) { setResultType(MIRType::Value); }
...
bool ElementAccessIsDenseNative(CompilerConstraintList* constraints, MDefinition* obj,
                                MDefinition* id);
bool ElementAccessIsTypedArray(CompilerConstraintList* constraints, MDefinition* obj,
                               MDefinition* id, Scalar::Type* arrayType);
...
AbortReasonOr<bool> ElementAccessHasExtraIndexedProperty(IonBuilder* builder, MDefinition* obj);
MIRType DenseNativeElementType(CompilerConstraintList* constraints, MDefinition* obj);
BarrierKind PropertyReadNeedsTypeBarrier(JSContext* propertycx, TempAllocator& alloc,
                                         CompilerConstraintList* constraints,
                                         TypeSet::ObjectKey* key, PropertyName* name,
                                         TemporaryTypeSet* observed, bool updateObserved);
BarrierKind PropertyReadNeedsTypeBarrier(JSContext* propertycx, TempAllocator& alloc,
                                         CompilerConstraintList* constraints, MDefinition* obj,
                                         PropertyName* name, TemporaryTypeSet* observed);
AbortReasonOr<BarrierKind> PropertyReadOnPrototypeNeedsTypeBarrier(IonBuilder* builder,
                                                                   MDefinition* obj,
                                                                   PropertyName* name,
                                                                   TemporaryTypeSet* observed);
bool PropertyReadIsIdempotent(CompilerConstraintList* constraints, MDefinition* obj,
                              PropertyName* name);
...
bool PropertyWriteNeedsTypeBarrier(TempAllocator& alloc, CompilerConstraintList* constraints,
                                   MBasicBlock* current, MDefinition** pobj, PropertyName* name,
                                   MDefinition** pvalue, bool canModify,
                                   MIRType implicitType = MIRType::None);
AbortReasonOr<bool> ArrayPrototypeHasIndexedProperty(IonBuilder* builder, JSScript* script);
