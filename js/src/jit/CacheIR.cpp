...
IRGenerator::IRGenerator(JSContext* cx, HandleScript script, jsbytecode* pc, CacheKind cacheKind,
                         ICState::Mode mode)
    : writer(cx), cx_(cx), script_(script), pc_(pc), cacheKind_(cacheKind), mode_(mode) {}
...
static void EmitLoadSlotResult(CacheIRWriter& writer, ObjOperandId holderOp, NativeObject* holder,
                               Shape* shape) {
...
enum class ProxyStubType { None, DOMExpando, DOMShadowed, DOMUnshadowed, Generic };

static ProxyStubType GetProxyStubType(JSContext* cx, HandleObject obj, HandleId id) {
...
static bool ValueToNameOrSymbolId(JSContext* cx, HandleValue idval, MutableHandleId id,
                                  bool* nameOrSymbol) {
    *nameOrSymbol = false;
...
bool GetPropIRGenerator::tryAttachStub() {
    ...
    if (val_.isObject()) {
        ...
        if (maybeGuardInt32Index(idVal_, getElemKeyValueId(), &index, &indexId)) {
            ...
            if (tryAttachArgumentsObjectArg(obj, objId, index, indexId)) return true;
...
bool GetPropIRGenerator::tryAttachIdempotentStub() {
    ...
    if ((resultFlags_ & GetPropertyResultFlags::AllowInt32) &&
        tryAttachObjectLength(obj, objId, id))
        return true;
...
static bool IsCacheableGetPropReadSlot(JSObject* obj, JSObject* holder, PropertyResult prop) {
...
static bool IsCacheableGetPropCallNative(JSObject* obj, JSObject* holder, Shape* shape) {
    if (!shape || !IsCacheableProtoChain(obj, holder)) return false;

    if (!shape->hasGetterValue() || !shape->getterValue().isObject()) return false;

    if (!shape->getterValue().toObject().is<JSFunction>()) return false;

    JSFunction& getter = shape->getterValue().toObject().as<JSFunction>();
    if (!getter.isNative()) return false;
    ...
    if (getter.jitInfo() && !getter.jitInfo()->needsOuterizedThisObject()) return true;
...
static bool IsCacheableGetPropCallScripted(JSObject* obj, JSObject* holder, Shape* shape,
...
static bool CheckHasNoSuchOwnProperty(JSContext* cx, JSObject* obj, jsid id) {
    if (obj->isNative()) {
        ...
        if (obj->as<UnboxedPlainObject>().containsUnboxedOrExpandoProperty(cx, id)) return false;
    } else if (obj->is<TypedObject>()) {
        if (obj->as<TypedObject>().typeDescr().hasProperty(cx->names(), id)) return false;
    } else {
        return false;
    }
...
static bool IsCacheableNoProperty(JSContext* cx, JSObject* obj, JSObject* holder, Shape* shape,
                                  jsid id, jsbytecode* pc, GetPropertyResultFlags resultFlags) {
    ...
    // cache result.
    if (!pc && !(resultFlags & GetPropertyResultFlags::AllowUndefined)) return false;
...
static NativeGetPropCacheability CanAttachNativeGetProp(JSContext* cx, HandleObject obj,
                                                        HandleId id,
                                                        MutableHandleNativeObject holder,
                                                        MutableHandleShape shape, jsbytecode* pc,
                                                        GetPropertyResultFlags resultFlags,
    ...
    PropertyResult prop;
    if (!LookupPropertyPure(cx, obj, id, &baseHolder, &prop)) return CanAttachNone;
    ...
    if (pc && (resultFlags & GetPropertyResultFlags::Monitored)) {
        ...
        if (IsCacheableGetPropCallNative(obj, holder, shape)) return CanAttachCallGetter;
    }

    return CanAttachNone;
}

static void GeneratePrototypeGuards(CacheIRWriter& writer, JSObject* obj, JSObject* holder,
                                    ObjOperandId objId) {
...
static void GeneratePrototypeHoleGuards(CacheIRWriter& writer, JSObject* obj, ObjOperandId objId) {
    if (obj->hasUncacheableProto()) {
        // If the shape does not imply the proto, emit an explicit proto guard.
        writer.guardProto(objId, obj->staticPrototype());
    }
...
static void TestMatchingReceiver(CacheIRWriter& writer, JSObject* obj, ObjOperandId objId,
...
static void EmitReadSlotGuard(CacheIRWriter& writer, JSObject* obj, JSObject* holder,
...
static void EmitReadSlotResult(CacheIRWriter& writer, JSObject* obj, JSObject* holder,
...
static void EmitReadSlotReturn(CacheIRWriter& writer, JSObject*, JSObject* holder, Shape* shape,
...
static void EmitCallGetterResultNoGuards(CacheIRWriter& writer, JSObject* obj, JSObject* holder,
...
static void EmitCallGetterResult(CacheIRWriter& writer, JSObject* obj, JSObject* holder,
                                 Shape* shape, ObjOperandId objId, ObjOperandId receiverId,
                                 ICState::Mode mode) {
...
void GetPropIRGenerator::attachMegamorphicNativeSlot(ObjOperandId objId, jsid id,
                                                     bool handleMissing) {
    ...
    if (cacheKind_ == CacheKind::GetProp || cacheKind_ == CacheKind::GetPropSuper) {
        writer.megamorphicLoadSlotResult(objId, JSID_TO_ATOM(id)->asPropertyName(), handleMissing);
    } else {
        MOZ_ASSERT(cacheKind_ == CacheKind::GetElem || cacheKind_ == CacheKind::GetElemSuper);
...
bool GetPropIRGenerator::tryAttachNative(HandleObject obj, ObjOperandId objId, HandleId id) {
    RootedShape shape(cx_);
    RootedNativeObject holder(cx_);

    NativeGetPropCacheability type = CanAttachNativeGetProp(
        cx_, obj, id, &holder, &shape, pc_, resultFlags_, isTemporarilyUnoptimizable_);
    switch (type) {
        case CanAttachNone:
            return false;
...
bool GetPropIRGenerator::tryAttachWindowProxy(HandleObject obj, ObjOperandId objId, HandleId id) {
    ...
    RootedNativeObject holder(cx_);
    NativeGetPropCacheability type = CanAttachNativeGetProp(
        cx_, windowObj, id, &holder, &shape, pc_, resultFlags_, isTemporarilyUnoptimizable_);
    switch (type) {
        case CanAttachNone:
            return false;
        ...
        case CanAttachCallGetter: {
            ...
            if (!callee->jitInfo() || callee->jitInfo()->needsOuterizedThisObject()) return false;
...
bool GetPropIRGenerator::tryAttachCrossCompartmentWrapper(HandleObject obj, ObjOperandId objId,
                                                          HandleId id) {
    ...
    if (unwrapped->compartment()->zone() != cx_->compartment()->zone()) return false;

    RootedObject wrappedGlobal(cx_, &obj->global());
    if (!cx_->compartment()->wrap(cx_, &wrappedGlobal)) return false;
    ...
    NativeGetPropCacheability canCache = CanAttachNativeGetProp(
        cx_, unwrapped, id, &holder, &shape, pc_, resultFlags_, isTemporarilyUnoptimizable_);
...
bool GetPropIRGenerator::tryAttachXrayCrossCompartmentWrapper(HandleObject obj, ObjOperandId objId,
                                                              HandleId id) {
    if (!IsProxy(obj)) return false;

    XrayJitInfo* info = GetXrayJitInfo();
    if (!info || !info->isCrossCompartmentXray(GetProxyHandler(obj))) return false;
    ...
    if (!getter || !getter->is<JSFunction>() || !getter->as<JSFunction>().isNative()) return false;
...
bool GetPropIRGenerator::tryAttachGenericProxy(HandleObject obj, ObjOperandId objId, HandleId id,
...
ObjOperandId IRGenerator::guardDOMProxyExpandoObjectAndShape(JSObject* obj, ObjOperandId objId,
...
bool GetPropIRGenerator::tryAttachDOMProxyExpando(HandleObject obj, ObjOperandId objId,
                                                  HandleId id) {
    MOZ_ASSERT(IsCacheableDOMProxy(obj));
    ...
    NativeGetPropCacheability canCache = CanAttachNativeGetProp(
        cx_, expandoObj, id, &holder, &propShape, pc_, resultFlags_, isTemporarilyUnoptimizable_);
    if (canCache != CanAttachReadSlot && canCache != CanAttachCallGetter) return false;
...
bool GetPropIRGenerator::tryAttachDOMProxyShadowed(HandleObject obj, ObjOperandId objId,
                                                   HandleId id) {
    MOZ_ASSERT(IsCacheableDOMProxy(obj));
...
static void CheckDOMProxyExpandoDoesNotShadow(CacheIRWriter& writer, JSObject* obj, jsid id,
                                              ObjOperandId objId) {
    MOZ_ASSERT(IsCacheableDOMProxy(obj));
...
bool GetPropIRGenerator::tryAttachDOMProxyUnshadowed(HandleObject obj, ObjOperandId objId,
                                                     HandleId id) {
    MOZ_ASSERT(IsCacheableDOMProxy(obj));
    ...
    RootedShape shape(cx_);
    NativeGetPropCacheability canCache = CanAttachNativeGetProp(
        cx_, checkObj, id, &holder, &shape, pc_, resultFlags_, isTemporarilyUnoptimizable_);
...
bool GetPropIRGenerator::tryAttachProxy(HandleObject obj, ObjOperandId objId, HandleId id) {
...
bool GetPropIRGenerator::tryAttachUnboxed(HandleObject obj, ObjOperandId objId, HandleId id) {
...
bool GetPropIRGenerator::tryAttachUnboxedExpando(HandleObject obj, ObjOperandId objId,
                                                 HandleId id) {
    if (!obj->is<UnboxedPlainObject>()) return false;
...
bool GetPropIRGenerator::tryAttachTypedObject(HandleObject obj, ObjOperandId objId, HandleId id) {
...
bool GetPropIRGenerator::tryAttachObjectLength(HandleObject obj, ObjOperandId objId, HandleId id) {
...
bool GetPropIRGenerator::tryAttachFunction(HandleObject obj, ObjOperandId objId, HandleId id) {
...
bool GetPropIRGenerator::tryAttachModuleNamespace(HandleObject obj, ObjOperandId objId,
                                                  HandleId id) {
    ...
    if (env->getSlot(shape->slot()).isMagic(JS_UNINITIALIZED_LEXICAL)) return false;
...
bool GetPropIRGenerator::tryAttachPrimitive(ValOperandId valId, HandleId id) {
    ...
    RootedNativeObject holder(cx_);
    NativeGetPropCacheability type = CanAttachNativeGetProp(
        cx_, proto, id, &holder, &shape, pc_, resultFlags_, isTemporarilyUnoptimizable_);
...
bool GetPropIRGenerator::tryAttachStringLength(ValOperandId valId, HandleId id) {
...
bool GetPropIRGenerator::tryAttachStringChar(ValOperandId valId, ValOperandId indexId) {
...
bool GetPropIRGenerator::tryAttachMagicArgumentsName(ValOperandId valId, HandleId id) {
...
bool GetPropIRGenerator::tryAttachMagicArgument(ValOperandId valId, ValOperandId indexId) {
    MOZ_ASSERT(idVal_.isInt32());
...
bool GetPropIRGenerator::tryAttachArgumentsObjectArg(HandleObject obj, ObjOperandId objId,
...
bool GetPropIRGenerator::tryAttachDenseElement(HandleObject obj, ObjOperandId objId,
                                               uint32_t index, Int32OperandId indexId) {
    if (!obj->isNative()) return false;

    if (!obj->as<NativeObject>().containsDenseElement(index)) return false;
...
static bool CanAttachDenseElementHole(JSObject* obj, bool ownProp) {
    ...
    do {
        ...
        if (proto->as<NativeObject>().getDenseInitializedLength() != 0) return false;
...
bool GetPropIRGenerator::tryAttachDenseElementHole(HandleObject obj, ObjOperandId objId,
                                                   uint32_t index, Int32OperandId indexId) {
    if (!obj->isNative()) return false;

    if (obj->as<NativeObject>().containsDenseElement(index)) return false;

    if (!CanAttachDenseElementHole(obj, false)) return false;
...
static bool TypedThingRequiresFloatingPoint(JSObject* obj) {
    ...
    return type == Scalar::Uint32 || type == Scalar::Float32 || type == Scalar::Float64;
}

bool GetPropIRGenerator::tryAttachTypedElement(HandleObject obj, ObjOperandId objId,
                                               uint32_t index, Int32OperandId indexId) {
    if (!obj->is<TypedArrayObject>() && !IsPrimitiveArrayTypedObject(obj)) return false;
    ...
    if (obj->is<TypedArrayObject>() && index >= obj->as<TypedArrayObject>().length()) return false;

    // Don't attach typed object stubs if the underlying storage could be
    // detached, as the stub will always bail out.
    if (IsPrimitiveArrayTypedObject(obj) && cx_->compartment()->detachedTypedObjects) return false;

    TypedThingLayout layout = GetTypedThingLayout(obj->getClass());
    if (layout != Layout_TypedArray) writer.guardNoDetachedTypedObjects();
...
bool GetPropIRGenerator::tryAttachProxyElement(HandleObject obj, ObjOperandId objId) {
...
void GetPropIRGenerator::trackNotAttached() {
#ifdef JS_CACHEIR_SPEW
    CacheIRSpewer& sp = CacheIRSpewer::singleton();
    if (sp.enabled()) {
        LockGuard<Mutex> guard(sp.lock());
...
GetNameIRGenerator::GetNameIRGenerator(JSContext* cx, HandleScript script, jsbytecode* pc,
    ...
    : IRGenerator(cx, script, pc, CacheKind::GetName, mode), env_(env), name_(name) {}
...
bool CanAttachGlobalName(JSContext* cx, Handle<LexicalEnvironmentObject*> globalLexical,
...
bool GetNameIRGenerator::tryAttachGlobalNameValue(ObjOperandId objId, HandleId id) {
    ...
    RootedNativeObject holder(cx_);
    RootedShape shape(cx_);
    if (!CanAttachGlobalName(cx_, globalLexical, id, &holder, &shape)) return false;
...
bool GetNameIRGenerator::tryAttachGlobalNameGetter(ObjOperandId objId, HandleId id) {
    if (!IsGlobalOp(JSOp(*pc_)) || script_->hasNonSyntacticScope()) return false;
    ...
    if (!IsCacheableGetPropCallNative(&globalLexical->global(), holder, shape)) return false;
...
static bool NeedEnvironmentShapeGuard(JSObject* envObj) {
    ...
    if (!fun->hasScript() || fun->nonLazyScript()->funHasExtensibleScope()) return true;

    return false;
}

bool GetNameIRGenerator::tryAttachEnvironmentName(ObjOperandId objId, HandleId id) {
    if (IsGlobalOp(JSOp(*pc_)) || script_->hasNonSyntacticScope()) return false;
    ...
    while (env) {
        ...
        if (!env->is<EnvironmentObject>() || env->is<WithEnvironmentObject>()) return false;
    ...
    if (!IsCacheableGetPropReadSlot(holder, holder, PropertyResult(shape))) return false;
    if (holder->getSlot(shape->slot()).isMagic()) return false;

    ObjOperandId lastObjId = objId;
    env = env_;
    while (env) {
        if (NeedEnvironmentShapeGuard(env)) writer.guardShape(lastObjId, env->maybeShape());
    ...
    if (holder->isFixedSlot(shape->slot())) {
        writer.loadEnvironmentFixedSlotResult(lastObjId,
...
void GetNameIRGenerator::trackNotAttached() {
#ifdef JS_CACHEIR_SPEW
    CacheIRSpewer& sp = CacheIRSpewer::singleton();
    if (sp.enabled()) {
        LockGuard<Mutex> guard(sp.lock());
...
BindNameIRGenerator::BindNameIRGenerator(JSContext* cx, HandleScript script, jsbytecode* pc,
                                         ICState::Mode mode, HandleObject env,
                                         HandlePropertyName name)
    : IRGenerator(cx, script, pc, CacheKind::BindName, mode), env_(env), name_(name) {}
...
bool BindNameIRGenerator::tryAttachGlobalName(ObjOperandId objId, HandleId id) {
    ...
    if (Shape* shape = globalLexical->lookup(cx_, id)) {
        ...
        if (globalLexical->getSlot(shape->slot()).isMagic() || !shape->writable()) return false;
...
bool BindNameIRGenerator::tryAttachEnvironmentName(ObjOperandId objId, HandleId id) {
    if (IsGlobalOp(JSOp(*pc_)) || script_->hasNonSyntacticScope()) return false;
...
void BindNameIRGenerator::trackNotAttached() {
#ifdef JS_CACHEIR_SPEW
    CacheIRSpewer& sp = CacheIRSpewer::singleton();
    if (sp.enabled()) {
        LockGuard<Mutex> guard(sp.lock());
...
bool HasPropIRGenerator::tryAttachDense(HandleObject obj, ObjOperandId objId, uint32_t index,
                                        Int32OperandId indexId) {
    if (!obj->isNative()) return false;
    if (!obj->as<NativeObject>().containsDenseElement(index)) return false;
...
bool HasPropIRGenerator::tryAttachDenseHole(HandleObject obj, ObjOperandId objId, uint32_t index,
                                            Int32OperandId indexId) {
    bool hasOwn = (cacheKind_ == CacheKind::HasOwn);

    if (!obj->isNative()) return false;
    if (obj->as<NativeObject>().containsDenseElement(index)) return false;
    if (!CanAttachDenseElementHole(obj, hasOwn)) return false;
    ...
    if (!hasOwn) GeneratePrototypeHoleGuards(writer, obj, objId);
...
bool HasPropIRGenerator::tryAttachNamedProp(HandleObject obj, ObjOperandId objId, HandleId key,
                                            ValOperandId keyId) {
    bool hasOwn = (cacheKind_ == CacheKind::HasOwn);
...
bool HasPropIRGenerator::tryAttachMegamorphic(ObjOperandId objId, ValOperandId keyId) {
    bool hasOwn = (cacheKind_ == CacheKind::HasOwn);
...
bool HasPropIRGenerator::tryAttachNative(JSObject* obj, ObjOperandId objId, jsid key,
...
bool HasPropIRGenerator::tryAttachUnboxed(JSObject* obj, ObjOperandId objId, jsid key,
                                          ValOperandId keyId) {
    if (!obj->is<UnboxedPlainObject>()) return false;
...
bool HasPropIRGenerator::tryAttachUnboxedExpando(JSObject* obj, ObjOperandId objId, jsid key,
                                                 ValOperandId keyId) {
    if (!obj->is<UnboxedPlainObject>()) return false;
...
bool HasPropIRGenerator::tryAttachTypedObject(JSObject* obj, ObjOperandId objId, jsid key,
                                              ValOperandId keyId) {
    if (!obj->is<TypedObject>()) return false;

    if (!obj->as<TypedObject>().typeDescr().hasProperty(cx_->names(), key)) return false;
...
bool HasPropIRGenerator::tryAttachSlotDoesNotExist(JSObject* obj, ObjOperandId objId, jsid key,
                                                   ValOperandId keyId) {
    bool hasOwn = (cacheKind_ == CacheKind::HasOwn);
...
bool HasPropIRGenerator::tryAttachDoesNotExist(HandleObject obj, ObjOperandId objId, HandleId key,
                                               ValOperandId keyId) {
    bool hasOwn = (cacheKind_ == CacheKind::HasOwn);
...
bool HasPropIRGenerator::tryAttachProxyElement(HandleObject obj, ObjOperandId objId,
                                               ValOperandId keyId) {
    bool hasOwn = (cacheKind_ == CacheKind::HasOwn);
...
void HasPropIRGenerator::trackNotAttached() {
#ifdef JS_CACHEIR_SPEW
    CacheIRSpewer& sp = CacheIRSpewer::singleton();
    if (sp.enabled()) {
        LockGuard<Mutex> guard(sp.lock());
...
bool IRGenerator::maybeGuardInt32Index(const Value& index, ValOperandId indexId,
    ...
    if (index.isNumber()) {
        ...
        if (index.isInt32()) {
            ...
            if (!mozilla::NumberEqualsInt32(index.toDouble(), &indexSigned)) return false;
            if (!cx_->runtime()->jitSupportsFloatingPoint) return false;
        }
...
bool SetPropIRGenerator::tryAttachStub() {
    ...
    if (lhsVal_.isObject()) {
        ...
        if (maybeGuardInt32Index(idVal_, setElemKeyValueId(), &index, &indexId)) {
            if (tryAttachSetDenseElement(obj, objId, index, indexId, rhsValId)) return true;
            if (tryAttachSetDenseElementHole(obj, objId, index, indexId, rhsValId)) return true;
            if (tryAttachSetTypedElement(obj, objId, index, indexId, rhsValId)) return true;
            return false;
        }
        return false;
    }
...
static void EmitStoreSlotAndReturn(CacheIRWriter& writer, ObjOperandId objId, NativeObject* nobj,
                                   Shape* shape, ValOperandId rhsId) {
...
static Shape* LookupShapeForSetSlot(JSOp op, NativeObject* obj, jsid id) {
    ...
    if (IsPropertyInitOp(op) && (!shape->configurable() || !shape->enumerable())) return nullptr;

    return shape;
}

static bool CanAttachNativeSetSlot(JSContext* cx, JSOp op, HandleObject obj, HandleId id,
                                   bool* isTemporarilyUnoptimizable,
...
bool SetPropIRGenerator::tryAttachNativeSetSlot(HandleObject obj, ObjOperandId objId, HandleId id,
                                                ValOperandId rhsId) {
    RootedShape propShape(cx_);
    ...
    if (typeCheckInfo_.needsTypeBarrier()) writer.guardGroup(objId, nobj->group());
...
bool SetPropIRGenerator::tryAttachUnboxedExpandoSetSlot(HandleObject obj, ObjOperandId objId,
...
static void EmitGuardUnboxedPropertyType(CacheIRWriter& writer, JSValueType propType,
                                         ValOperandId valId) {
...
bool SetPropIRGenerator::tryAttachUnboxedProperty(HandleObject obj, ObjOperandId objId,
                                                  HandleId id, ValOperandId rhsId) {
    if (!obj->is<UnboxedPlainObject>() || !cx_->runtime()->jitSupportsFloatingPoint) return false;
...
bool SetPropIRGenerator::tryAttachTypedObjectProperty(HandleObject obj, ObjOperandId objId,
                                                      HandleId id, ValOperandId rhsId) {
    if (!obj->is<TypedObject>() || !cx_->runtime()->jitSupportsFloatingPoint) return false;
...
void SetPropIRGenerator::trackNotAttached() {
#ifdef JS_CACHEIR_SPEW
    CacheIRSpewer& sp = CacheIRSpewer::singleton();
    if (sp.enabled()) {
        LockGuard<Mutex> guard(sp.lock());
...
static bool IsCacheableSetPropCallNative(JSObject* obj, JSObject* holder, Shape* shape) {
    if (!shape || !IsCacheableProtoChain(obj, holder)) return false;

    if (!shape->hasSetterValue()) return false;

    if (!shape->setterObject() || !shape->setterObject()->is<JSFunction>()) return false;

    JSFunction& setter = shape->setterObject()->as<JSFunction>();
    if (!setter.isNative()) return false;

    if (setter.isClassConstructor()) return false;

    if (setter.jitInfo() && !setter.jitInfo()->needsOuterizedThisObject()) return true;

    return !IsWindow(obj);
}

static bool IsCacheableSetPropCallScripted(JSObject* obj, JSObject* holder, Shape* shape,
                                           bool* isTemporarilyUnoptimizable = nullptr) {
    if (!shape || !IsCacheableProtoChain(obj, holder)) return false;
...
static bool CanAttachSetter(JSContext* cx, jsbytecode* pc, HandleObject obj, HandleId id,
...
static void EmitCallSetterNoGuards(CacheIRWriter& writer, JSObject* obj, JSObject* holder,
...
bool SetPropIRGenerator::tryAttachSetter(HandleObject obj, ObjOperandId objId, HandleId id,
                                         ValOperandId rhsId) {
...
bool SetPropIRGenerator::tryAttachSetArrayLength(HandleObject obj, ObjOperandId objId, HandleId id,
                                                 ValOperandId rhsId) {
...
bool SetPropIRGenerator::tryAttachSetDenseElement(HandleObject obj, ObjOperandId objId,
                                                  uint32_t index, Int32OperandId indexId,
                                                  ValOperandId rhsId) {
    if (!obj->isNative()) return false;

    NativeObject* nobj = &obj->as<NativeObject>();
    if (!nobj->containsDenseElement(index) || nobj->getElementsHeader()->isFrozen()) return false;
...
static bool CanAttachAddElement(JSObject* obj, bool isInit) {
    ...
    do {
        ...
        if (clasp != &ArrayObject::class_ &&
            (clasp->getAddProperty() || clasp->getResolve() || clasp->getOpsLookupProperty() ||
...
static void ShapeGuardProtoChain(CacheIRWriter& writer, JSObject* obj, ObjOperandId objId) {
    while (true) {
...
bool SetPropIRGenerator::tryAttachSetDenseElementHole(HandleObject obj, ObjOperandId objId,
                                                      uint32_t index, Int32OperandId indexId,
                                                      ValOperandId rhsId) {
...
bool SetPropIRGenerator::tryAttachSetTypedElement(HandleObject obj, ObjOperandId objId,
                                                  uint32_t index, Int32OperandId indexId,
                                                  ValOperandId rhsId) {
    if (!obj->is<TypedArrayObject>() && !IsPrimitiveArrayTypedObject(obj)) return false;
    ...
    if (!obj->is<TypedArrayObject>()) writer.guardNoDetachedTypedObjects();
...
bool SetPropIRGenerator::tryAttachGenericProxy(HandleObject obj, ObjOperandId objId, HandleId id,
...
bool SetPropIRGenerator::tryAttachDOMProxyShadowed(HandleObject obj, ObjOperandId objId,
                                                   HandleId id, ValOperandId rhsId) {
    MOZ_ASSERT(IsCacheableDOMProxy(obj));
...
bool SetPropIRGenerator::tryAttachDOMProxyUnshadowed(HandleObject obj, ObjOperandId objId,
                                                     HandleId id, ValOperandId rhsId) {
    MOZ_ASSERT(IsCacheableDOMProxy(obj));
...
bool SetPropIRGenerator::tryAttachDOMProxyExpando(HandleObject obj, ObjOperandId objId,
                                                  HandleId id, ValOperandId rhsId) {
    MOZ_ASSERT(IsCacheableDOMProxy(obj));
...
bool SetPropIRGenerator::tryAttachProxy(HandleObject obj, ObjOperandId objId, HandleId id,
                                        ValOperandId rhsId) {
...
bool SetPropIRGenerator::tryAttachProxyElement(HandleObject obj, ObjOperandId objId,
                                               ValOperandId rhsId) {
    // Don't attach a proxy stub for ops like JSOP_INITELEM.
    MOZ_ASSERT(IsPropertySetOp(JSOp(*pc_)));
...
bool SetPropIRGenerator::tryAttachMegamorphicSetElement(HandleObject obj, ObjOperandId objId,
                                                        ValOperandId rhsId) {
    MOZ_ASSERT(IsPropertySetOp(JSOp(*pc_)));

    if (mode_ != ICState::Mode::Megamorphic || cacheKind_ != CacheKind::SetElem) return false;
...
bool SetPropIRGenerator::tryAttachWindowProxy(HandleObject obj, ObjOperandId objId, HandleId id,
                                              ValOperandId rhsId) {
...
bool SetPropIRGenerator::tryAttachAddSlotStub(HandleObjectGroup oldGroup, HandleShape oldShape) {
    AutoAssertNoPendingException aanpe(cx_);
    ...
    // current shape.
    if (!obj->nonProxyIsExtensible() || propShape->previous() != oldShape) return false;

    // Basic shape checks.
    if (propShape->inDictionary() || !propShape->isDataProperty() || !propShape->writable()) {
        return false;
    }
    ...
    if (!obj->is<ArrayObject>() && obj->getClass()->getAddProperty()) return false;
    ...
    if (holderOrExpando->isFixedSlot(propShape->slot())) {
        ...
        writer.addAndStoreFixedSlot(holderId, offset, rhsValId, propShape, changeGroup, newGroup);
        ...
        if (numOldSlots == numNewSlots) {
            writer.addAndStoreDynamicSlot(holderId, offset, rhsValId, propShape, changeGroup,
                                          newGroup);
            trackAttached("AddSlot");
        } else {
            MOZ_ASSERT(numNewSlots > numOldSlots);
            writer.allocateAndStoreDynamicSlot(holderId, offset, rhsValId, propShape, changeGroup,
                                               newGroup, numNewSlots);
...
bool GetIteratorIRGenerator::tryAttachNativeIterator(ObjOperandId objId, HandleObject obj) {
    MOZ_ASSERT(JSOp(*pc_) == JSOP_ITER);

    if (GET_UINT8(pc_) != JSITER_ENUMERATE) return false;
...
bool CallIRGenerator::tryAttachStringSplit() {
    ...
    // to proceed.
    if (args_[0].toString()->isAtom() && args_[1].toString()->isAtom()) return false;
...
bool CallIRGenerator::tryAttachArrayPush() {
    ...
    if (typeCheckInfo_.needsTypeBarrier()) writer.guardGroup(thisObjId, thisobj->group());
...
bool CallIRGenerator::tryAttachArrayJoin() {
    ...
    if (thisarray->getDenseInitializedLength() != thisarray->length()) return false;
...
static const Class shapeContainerClass = {"ShapeContainer", JSCLASS_HAS_RESERVED_SLOTS(1)};

static const size_t SHAPE_CONTAINER_SLOT = 0;

JSObject* jit::NewWrapperWithObjectShape(JSContext* cx, HandleNativeObject obj) {
    ...
    RootedObject wrapper(cx);
    {
        ...
        wrapper->as<NativeObject>().setSlot(SHAPE_CONTAINER_SLOT,
                                            PrivateGCThingValue(obj->lastProperty()));
    }
...
void jit::LoadShapeWrapperContents(MacroAssembler& masm, Register obj, Register dst,
                                   Label* failure) {
...
void CallIRGenerator::trackNotAttached() {
#ifdef JS_CACHEIR_SPEW
    CacheIRSpewer& sp = CacheIRSpewer::singleton();
    if (sp.enabled()) {
        LockGuard<Mutex> guard(sp.lock());
...
bool CompareIRGenerator::tryAttachString(ValOperandId lhsId, ValOperandId rhsId) {
...
bool CompareIRGenerator::tryAttachObject(ValOperandId lhsId, ValOperandId rhsId) {
    MOZ_ASSERT(IsEqualityOp(op_));
...
bool CompareIRGenerator::tryAttachSymbol(ValOperandId lhsId, ValOperandId rhsId) {
    MOZ_ASSERT(IsEqualityOp(op_));
...
bool CompareIRGenerator::tryAttachStub() {
    ...
    MOZ_ASSERT(IsEqualityOp(op_) || op_ == JSOP_LE || op_ == JSOP_LT || op_ == JSOP_GE ||
               op_ == JSOP_GT);
...
void CompareIRGenerator::trackNotAttached() {
#ifdef JS_CACHEIR_SPEW
    CacheIRSpewer& sp = CacheIRSpewer::singleton();
    if (sp.enabled()) {
        LockGuard<Mutex> guard(sp.lock());
