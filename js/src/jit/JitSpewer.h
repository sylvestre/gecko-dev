...
static inline Fprinter& JitSpewPrinter() { MOZ_CRASH("No empty backend for JitSpewPrinter"); }
...
static inline MOZ_FORMAT_PRINTF(2, 0) void JitSpewVA(JitSpewChannel channel, const char* fmt,
                                                     va_list ap) {}
...
class AutoDisableSpew {
   ...
   public:
    AutoDisableSpew() : enabled_(JitSpewEnabled(Channel)) { DisableChannel(Channel); }
