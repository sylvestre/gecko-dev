...
static void EnsureOperandNotFloat32(TempAllocator& alloc, MInstruction* def, unsigned op) {
...
MDefinition* js::jit::AlwaysBoxAt(TempAllocator& alloc, MInstruction* at, MDefinition* operand) {
...
bool BoxInputsPolicy::staticAdjustInputs(TempAllocator& alloc, MInstruction* ins) {
...
bool ArithPolicy::adjustInputs(TempAllocator& alloc, MInstruction* ins) {
    ...
    if (specialization == MIRType::None) return BoxInputsPolicy::staticAdjustInputs(alloc, ins);
...
bool AllDoublePolicy::adjustInputs(TempAllocator& alloc, MInstruction* ins) {
    for (size_t i = 0, e = ins->numOperands(); i < e; i++) {
        MDefinition* in = ins->getOperand(i);
    ...
    for (size_t i = 0; i < 2; i++) {
        ...
        switch (type) {
            case MIRType::Double: {
                ...
                else if (compare->compareType() == MCompare::Compare_DoubleMaybeCoerceRHS &&
                         i == 1)
                    convert = MToFPInstruction::NonNullNonStringPrimitives;
            ...
            case MIRType::Int32: {
                MacroAssembler::IntConversionInputKind convert =
                    MacroAssembler::IntConversion_NumbersOnly;
...
bool PowPolicy::adjustInputs(TempAllocator& alloc, MInstruction* ins) {
    ...
    if (specialization == MIRType::Double) return DoublePolicy<1>::staticAdjustInputs(alloc, ins);
    return IntPolicy<1>::staticAdjustInputs(alloc, ins);
}

template <unsigned Op>
bool StringPolicy<Op>::staticAdjustInputs(TempAllocator& alloc, MInstruction* ins) {
...
template <unsigned Op>
bool ConvertToStringPolicy<Op>::staticAdjustInputs(TempAllocator& alloc, MInstruction* ins) {
    MDefinition* in = ins->getOperand(Op);
    if (in->type() == MIRType::String) return true;
...
template <unsigned Op>
bool BooleanPolicy<Op>::staticAdjustInputs(TempAllocator& alloc, MInstruction* def) {
    MDefinition* in = def->getOperand(Op);
...
template <unsigned Op>
bool IntPolicy<Op>::staticAdjustInputs(TempAllocator& alloc, MInstruction* def) {
    MDefinition* in = def->getOperand(Op);
...
template <unsigned Op>
bool ConvertToInt32Policy<Op>::staticAdjustInputs(TempAllocator& alloc, MInstruction* def) {
    MDefinition* in = def->getOperand(Op);
    if (in->type() == MIRType::Int32) return true;
...
template <unsigned Op>
bool TruncateToInt32Policy<Op>::staticAdjustInputs(TempAllocator& alloc, MInstruction* def) {
    MDefinition* in = def->getOperand(Op);
    if (in->type() == MIRType::Int32) return true;
...
template <unsigned Op>
bool DoublePolicy<Op>::staticAdjustInputs(TempAllocator& alloc, MInstruction* def) {
    MDefinition* in = def->getOperand(Op);
    if (in->type() == MIRType::Double || in->type() == MIRType::SinCosDouble) return true;
...
template <unsigned Op>
bool Float32Policy<Op>::staticAdjustInputs(TempAllocator& alloc, MInstruction* def) {
    MDefinition* in = def->getOperand(Op);
...
template <unsigned Op>
bool FloatingPointPolicy<Op>::adjustInputs(TempAllocator& alloc, MInstruction* def) {
    MIRType policyType = def->typePolicySpecialization();
    if (policyType == MIRType::Double) return DoublePolicy<Op>::staticAdjustInputs(alloc, def);
...
template <unsigned Op>
bool NoFloatPolicy<Op>::staticAdjustInputs(TempAllocator& alloc, MInstruction* def) {
...
bool NoFloatPolicyAfter<FirstOp>::adjustInputs(TempAllocator& alloc, MInstruction* def) {
...
template <unsigned Op>
bool SimdScalarPolicy<Op>::staticAdjustInputs(TempAllocator& alloc, MInstruction* ins) {
...
template <unsigned Op>
bool BoxPolicy<Op>::staticAdjustInputs(TempAllocator& alloc, MInstruction* ins) {
    MDefinition* in = ins->getOperand(Op);
...
bool BoxExceptPolicy<Op, Type>::staticAdjustInputs(TempAllocator& alloc, MInstruction* ins) {
    MDefinition* in = ins->getOperand(Op);
...
template <unsigned Op>
bool CacheIdPolicy<Op>::staticAdjustInputs(TempAllocator& alloc, MInstruction* ins) {
    MDefinition* in = ins->getOperand(Op);
    switch (in->type()) {
        case MIRType::Int32:
        case MIRType::String:
...
bool ToDoublePolicy::staticAdjustInputs(TempAllocator& alloc, MInstruction* ins) {
    ...
    switch (in->type()) {
        ...
        case MIRType::Boolean:
            // No need for boxing, when we will convert.
            if (conversion == MToFPInstruction::NonStringPrimitives) return true;
            if (conversion == MToFPInstruction::NonNullNonStringPrimitives) return true;
            break;
        case MIRType::Object:
        case MIRType::String:
        case MIRType::Symbol:
...
bool ToInt32Policy::staticAdjustInputs(TempAllocator& alloc, MInstruction* ins) {
    ...
    if (ins->isToInt32()) conversion = ins->toToInt32()->conversion();
    ...
    switch (in->type()) {
        ...
        case MIRType::Null:
            // No need for boxing, when we will convert.
            if (conversion == MacroAssembler::IntConversion_Any) return true;
            break;
        case MIRType::Boolean:
            // No need for boxing, when we will convert.
            if (conversion == MacroAssembler::IntConversion_Any) return true;
            if (conversion == MacroAssembler::IntConversion_NumbersOrBoolsOnly) return true;
            break;
        case MIRType::Object:
        case MIRType::String:
        case MIRType::Symbol:
...
bool ToStringPolicy::staticAdjustInputs(TempAllocator& alloc, MInstruction* ins) {
...
template <unsigned Op>
bool ObjectPolicy<Op>::staticAdjustInputs(TempAllocator& alloc, MInstruction* ins) {
    MDefinition* in = ins->getOperand(Op);
...
template bool SimdSameAsReturnedTypePolicy<0>::staticAdjustInputs(TempAllocator& alloc,
                                                                  MInstruction* ins);
template bool SimdSameAsReturnedTypePolicy<1>::staticAdjustInputs(TempAllocator& alloc,
                                                                  MInstruction* ins);
...
template bool SimdPolicy<0>::adjustInputs(TempAllocator& alloc, MInstruction* ins);
...
bool SimdShufflePolicy::adjustInputs(TempAllocator& alloc, MInstruction* ins) {
    ...
    for (unsigned i = 0; i < s->numLanes(); i++) {
        ...
        MInstruction* replace =
            MToInt32::New(alloc, in, MacroAssembler::IntConversion_NumbersOnly);
        ins->block()->insertBefore(ins, replace);
...
bool CallSetElementPolicy::adjustInputs(TempAllocator& alloc, MInstruction* ins) {
...
bool StoreUnboxedScalarPolicy::adjustValueInput(TempAllocator& alloc, MInstruction* ins,
    ...
    MOZ_ASSERT(value->type() == MIRType::Int32 || value->type() == MIRType::Boolean ||
...
bool StoreUnboxedScalarPolicy::adjustInputs(TempAllocator& alloc, MInstruction* ins) {
    if (!SingleObjectPolicy::staticAdjustInputs(alloc, ins)) return false;
...
bool StoreTypedArrayHolePolicy::adjustInputs(TempAllocator& alloc, MInstruction* ins) {
    ...
    return StoreUnboxedScalarPolicy::adjustValueInput(alloc, ins, store->arrayType(),
                                                      store->value(), 3);
}

bool StoreTypedArrayElementStaticPolicy::adjustInputs(TempAllocator& alloc, MInstruction* ins) {
    ...
    return ConvertToInt32Policy<0>::staticAdjustInputs(alloc, ins) &&
           StoreUnboxedScalarPolicy::adjustValueInput(alloc, ins, store->accessType(),
                                                      store->value(), 1);
}

bool StoreUnboxedObjectOrNullPolicy::adjustInputs(TempAllocator& alloc, MInstruction* ins) {
...
bool FilterTypeSetPolicy::adjustInputs(TempAllocator& alloc, MInstruction* ins) {
...
#define DEFINE_MIR_TYPEPOLICY_MEMBERS_(op)                                       \
    TypePolicy* js::jit::M##op::typePolicy() { return M##op::thisTypePolicy(); } \
                                                                                 \
    MIRType js::jit::M##op::typePolicySpecialization() { return thisTypeSpecialization(); }
