...
void RInstruction::readRecoverData(CompactBufferReader& reader, RInstructionStorage* raw) {
...
RResumePoint::RResumePoint(CompactBufferReader& reader) {
    ...
    JitSpew(JitSpew_IonSnapshots, "Read RResumePoint (pc offset %u, nslots %u)", pcOffset_,
            numOperands_);
}
...
RSignExtendInt32::RSignExtendInt32(CompactBufferReader& reader) { mode_ = reader.readByte(); }
...
RAdd::RAdd(CompactBufferReader& reader) { isFloatOperation_ = reader.readByte(); }
...
RSub::RSub(CompactBufferReader& reader) { isFloatOperation_ = reader.readByte(); }
...
bool RRegExpMatcher::recover(JSContext* cx, SnapshotIterator& iter) const {
    ...
    RootedValue result(cx);
    if (!RegExpMatcherRaw(cx, regexp, input, lastIndex, nullptr, &result)) return false;
...
bool RRegExpSearcher::recover(JSContext* cx, SnapshotIterator& iter) const {
    ...
    int32_t result;
    if (!RegExpSearcherRaw(cx, regexp, input, lastIndex, nullptr, &result)) return false;
...
bool RRegExpTester::recover(JSContext* cx, SnapshotIterator& iter) const {
    ...
    if (!js::RegExpTesterRaw(cx, regexp, string, lastIndex, &endIndex)) return false;
...
RNewIterator::RNewIterator(CompactBufferReader& reader) { type_ = reader.readByte(); }
...
bool MNewDerivedTypedObject::writeRecoverData(CompactBufferWriter& writer) const {
    MOZ_ASSERT(canRecoverOnBailout());
...
bool RNewDerivedTypedObject::recover(JSContext* cx, SnapshotIterator& iter) const {
...
bool MCreateThisWithTemplate::writeRecoverData(CompactBufferWriter& writer) const {
    MOZ_ASSERT(canRecoverOnBailout());
...
bool RCreateThisWithTemplate::recover(JSContext* cx, SnapshotIterator& iter) const {
    RootedObject templateObject(cx, &iter.read().toObject());
...
bool MAssertRecoveredOnBailout::writeRecoverData(CompactBufferWriter& writer) const {
    MOZ_ASSERT(canRecoverOnBailout());
...
bool RStringReplace::recover(JSContext* cx, SnapshotIterator& iter) const {
    ...
    JSString* result = isFlatReplacement_
