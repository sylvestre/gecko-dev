...
static inline double ToDouble(const LAllocation* a) { return a->toConstant()->numberToDouble(); }
...
static inline Register ToRegister(const LAllocation* a) { return ToRegister(*a); }

static inline Register ToRegister(const LDefinition* def) { return ToRegister(*def->output()); }
...
static inline FloatRegister ToFloatRegister(const LAllocation* a) { return ToFloatRegister(*a); }

static inline FloatRegister ToFloatRegister(const LDefinition* def) {
    return ToFloatRegister(*def->output());
}

static inline FloatRegister ToTempFloatRegisterOrInvalid(const LDefinition* def) {
...
static inline AnyRegister ToAnyRegister(const LAllocation* a) { return ToAnyRegister(*a); }

static inline AnyRegister ToAnyRegister(const LDefinition* def) {
    return ToAnyRegister(def->output());
}

static inline RegisterOrInt32Constant ToRegisterOrInt32Constant(const LAllocation* a) {
    if (a->isConstant()) return RegisterOrInt32Constant(ToInt32(a));
    return RegisterOrInt32Constant(ToRegister(a));
}

static inline ValueOperand GetValueOutput(LInstruction* ins) {
...
int32_t CodeGeneratorShared::ArgToStackOffset(int32_t slot) const {
    return masm.framePushed() +
           (gen->compilingWasm() ? sizeof(wasm::Frame) : sizeof(JitFrameLayout)) + slot;
}
...
int32_t CodeGeneratorShared::StackOffsetOfPassedArg(int32_t slot) const {
    ...
    int32_t offset = masm.framePushed() - graph.paddedLocalSlotsSize() - (slot * sizeof(Value));
...
Address CodeGeneratorShared::ToAddress(const LAllocation* a) { return ToAddress(*a); }
...
void CodeGeneratorShared::restoreLiveIgnore(LInstruction* ins, LiveRegisterSet ignore) {
    MOZ_ASSERT(!ins->isCall());
    LSafepoint* safepoint = ins->safepoint();
...
void CodeGeneratorShared::verifyHeapAccessDisassembly(uint32_t begin, uint32_t end, bool isLoad,
...
void CodeGeneratorShared::verifyLoadDisassembly(uint32_t begin, uint32_t end, Scalar::Type type,
                                                Operand mem, LAllocation alloc) {
    verifyHeapAccessDisassembly(begin, end, true, type, mem, alloc);
}

void CodeGeneratorShared::verifyStoreDisassembly(uint32_t begin, uint32_t end, Scalar::Type type,
                                                 Operand mem, LAllocation alloc) {
