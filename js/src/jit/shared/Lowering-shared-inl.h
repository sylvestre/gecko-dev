...
template <size_t X>
void LIRGeneratorShared::define(details::LInstructionFixedDefsTempsHelper<1, X>* lir,
...
void LIRGeneratorShared::defineFixed(LInstructionHelper<1, X, Y>* lir, MDefinition* mir,
...
void LIRGeneratorShared::defineInt64Fixed(LInstructionHelper<INT64_PIECES, Ops, Temps>* lir,
...
template <size_t Ops, size_t Temps>
void LIRGeneratorShared::defineReuseInput(LInstructionHelper<1, Ops, Temps>* lir, MDefinition* mir,
                                          uint32_t operand) {
...
template <size_t Ops, size_t Temps>
void LIRGeneratorShared::defineInt64ReuseInput(LInstructionHelper<INT64_PIECES, Ops, Temps>* lir,
...
void LIRGeneratorShared::defineBox(
    details::LInstructionFixedDefsTempsHelper<BOX_PIECES, Temps>* lir, MDefinition* mir,
...
template <size_t Ops, size_t Temps>
void LIRGeneratorShared::defineInt64(LInstructionHelper<INT64_PIECES, Ops, Temps>* lir,
                                     MDefinition* mir, LDefinition::Policy policy) {
    // Call instructions should use defineReturn.
    MOZ_ASSERT(!lir->isCall());
    MOZ_ASSERT(mir->type() == MIRType::Int64);
...
void LIRGeneratorShared::defineSharedStubReturn(LInstruction* lir, MDefinition* mir) {
    lir->setMir(mir);
...
void LIRGeneratorShared::defineReturn(LInstruction* lir, MDefinition* mir) {
    ...
    switch (mir->type()) {
        ...
        case MIRType::Bool32x4:
            lir->setDef(0,
                        LDefinition(vreg, LDefinition::SIMD128INT, LFloatReg(ReturnSimd128Reg)));
            break;
        case MIRType::Float32x4:
            lir->setDef(0,
                        LDefinition(vreg, LDefinition::SIMD128FLOAT, LFloatReg(ReturnSimd128Reg)));
            break;
        default:
            LDefinition::Type type = LDefinition::TypeFrom(mir->type());
...
template <size_t Ops, size_t Temps>
void LIRGeneratorShared::defineSinCos(LInstructionHelper<2, Ops, Temps>* lir, MDefinition* mir,
                                      LDefinition::Policy policy) {
    MOZ_ASSERT(lir->isCall());
    ...
    lir->setDef(1,
                LDefinition(vreg + VREG_INCREMENT, LDefinition::DOUBLE,
...
void LIRGeneratorShared::redefine(MDefinition* def, MDefinition* as,
    ...
    MOZ_ASSERT(math->function() == MMathFunction::Cos || math->function() == MMathFunction::Sin);
...
void LIRGeneratorShared::redefine(MDefinition* def, MDefinition* as) {
    ...
    if (as->isEmittedAtUses() &&
        (def->type() == as->type() ||
         (as->isConstant() && (def->type() == MIRType::Int32 || def->type() == MIRType::Boolean) &&
...
#ifdef DEBUG
        if (JitOptions.runExtraChecks && def->resultTypeSet() && as->resultTypeSet() &&
...
LUse LIRGeneratorShared::useRegister(MDefinition* mir) { return use(mir, LUse(LUse::REGISTER)); }

LUse LIRGeneratorShared::useRegisterAtStart(MDefinition* mir) {
    return use(mir, LUse(LUse::REGISTER, true));
}

LUse LIRGeneratorShared::use(MDefinition* mir) { return use(mir, LUse(LUse::ANY)); }

LUse LIRGeneratorShared::useAtStart(MDefinition* mir) { return use(mir, LUse(LUse::ANY, true)); }
...
LAllocation LIRGeneratorShared::useRegisterOrNonDoubleConstant(MDefinition* mir) {
...
LAllocation LIRGeneratorShared::useStorable(MDefinition* mir) { return useRegister(mir); }
LAllocation LIRGeneratorShared::useStorableAtStart(MDefinition* mir) {
    return useRegisterAtStart(mir);
}

LAllocation LIRGeneratorShared::useAny(MDefinition* mir) { return useRegister(mir); }
#else
LAllocation LIRGeneratorShared::useAnyOrConstant(MDefinition* mir) { return useOrConstant(mir); }
...
LUse LIRGeneratorShared::useFixed(MDefinition* mir, Register reg) { return use(mir, LUse(reg)); }
...
LDefinition LIRGeneratorShared::temp(LDefinition::Type type, LDefinition::Policy policy) {
...
LDefinition LIRGeneratorShared::tempFloat32() { return temp(LDefinition::FLOAT32); }

LDefinition LIRGeneratorShared::tempDouble() { return temp(LDefinition::DOUBLE); }

LDefinition LIRGeneratorShared::tempCopy(MDefinition* input, uint32_t reusedInput) {
...
static inline uint32_t VirtualRegisterOfPayload(MDefinition* mir) {
    ...
    if (mir->isTypeBarrier()) return VirtualRegisterOfPayload(mir->getOperand(0));
...
LUse LIRGeneratorShared::usePayloadAtStart(MDefinition* mir, LUse::Policy policy) {
    MOZ_ASSERT(mir->type() == MIRType::Value);
...
void LIRGeneratorShared::fillBoxUses(LInstruction* lir, size_t n, MDefinition* mir) {
    ensureDefined(mir);
...
LUse LIRGeneratorShared::useRegisterForTypedLoad(MDefinition* mir, MIRType type) {
...
LBoxAllocation LIRGeneratorShared::useBox(MDefinition* mir, LUse::Policy policy, bool useAtStart) {
    MOZ_ASSERT(mir->type() == MIRType::Value);
...
LBoxAllocation LIRGeneratorShared::useBoxOrTypedOrConstant(MDefinition* mir, bool useConstant) {
...
LInt64Allocation LIRGeneratorShared::useInt64(MDefinition* mir, LUse::Policy policy,
                                              bool useAtStart) {
    MOZ_ASSERT(mir->type() == MIRType::Int64);
...
LInt64Allocation LIRGeneratorShared::useInt64Fixed(MDefinition* mir, Register64 regs,
                                                   bool useAtStart) {
    MOZ_ASSERT(mir->type() == MIRType::Int64);
...
LInt64Allocation LIRGeneratorShared::useInt64FixedAtStart(MDefinition* mir, Register64 regs) {
    return useInt64Fixed(mir, regs, true);
}

LInt64Allocation LIRGeneratorShared::useInt64(MDefinition* mir, bool useAtStart) {
...
LInt64Allocation LIRGeneratorShared::useInt64Register(MDefinition* mir, bool useAtStart) {
    return useInt64(mir, LUse::REGISTER, useAtStart);
}

LInt64Allocation LIRGeneratorShared::useInt64OrConstant(MDefinition* mir, bool useAtStart) {
    if (mir->isConstant()) {
#if defined(JS_NUNBOX32)
...
LInt64Allocation LIRGeneratorShared::useInt64RegisterOrConstant(MDefinition* mir,
                                                                bool useAtStart) {
    if (mir->isConstant()) {
#if defined(JS_NUNBOX32)
        return LInt64Allocation(LAllocation(mir->toConstant()), LAllocation());
