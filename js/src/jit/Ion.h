...
enum MethodStatus { Method_Error, Method_CantCompile, Method_Skipped, Method_Compiled };

enum class AbortReason : uint8_t { Alloc, Inlining, PreliminaryObjects, Disable, Error, NoAbort };
...
MethodStatus Recompile(JSContext* cx, HandleScript script, BaselineFrame* osrFrame,
...
void Invalidate(TypeZone& types, FreeOp* fop, const RecompileInfoVector& invalid,
...
inline bool IsIonInlinablePC(jsbytecode* pc) {
    ...
    return (IsCallPC(pc) && !IsSpreadCallPC(pc)) || IsGetPropPC(pc) || IsSetPropPC(pc);
}

inline bool TooManyActualArguments(unsigned nargs) { return nargs > JitOptions.maxStackArgs; }
