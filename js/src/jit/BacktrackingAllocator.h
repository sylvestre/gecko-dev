...
class Requirement {
   public:
    ...
    Requirement(uint32_t vreg, CodePosition at)
        : kind_(MUST_REUSE_INPUT), allocation_(LUse(vreg, LUse::ANY)), position_(at) {}
    ...
    MOZ_MUST_USE bool merge(const Requirement& newRequirement) {
        ...
        if (newRequirement.kind() == Requirement::FIXED) {
            if (kind() == Requirement::FIXED) return newRequirement.allocation() == allocation();
            *this = newRequirement;
            return true;
        }
...
struct UsePosition : public TempObject, public InlineForwardListNode<UsePosition> {
   private:
   ...
   public:
    ...
    UsePosition(LUse* use, CodePosition pos) : pos(pos) {
        ...
        MOZ_ASSERT_IF(
            !use->isFixedRegister(),
            pos.subpos() == (use->usedAtStart() ? CodePosition::INPUT : CodePosition::OUTPUT));
        setUse(use);
    }
};
...
class LiveRange : public TempObject {
   public:
    ...
    struct Range {
        ...
        Range(CodePosition from, CodePosition to) : from(from), to(to) { MOZ_ASSERT(!empty()); }
   ...
   public:
    static LiveRange* FallibleNew(TempAllocator& alloc, uint32_t vreg, CodePosition from,
                                  CodePosition to) {
...
class SpillSet : public TempObject {
   ...
   public:
    static SpillSet* New(TempAllocator& alloc) { return new (alloc) SpillSet(alloc); }

    MOZ_MUST_USE bool addSpilledBundle(LiveBundle* bundle) { return list_.append(bundle); }
...
class LiveBundle : public TempObject {
   ...
   public:
    ...
    MOZ_MUST_USE bool addRange(TempAllocator& alloc, uint32_t vreg, CodePosition from,
                               CodePosition to);
...
class VirtualRegister {
   ...
   public:
    ...
    bool isCompatible(const AnyRegister& r) const { return def_->isCompatibleReg(r); }
    bool isCompatible(const VirtualRegister& vr) const { return def_->isCompatibleDef(*vr.def_); }
    ...
    LiveRange::RegisterLinkIterator rangesBegin() const { return ranges_.begin(); }
...
class BacktrackingAllocator : protected RegisterAllocator {
    ...
    struct QueueItem {
        ...
        QueueItem(LiveBundle* bundle, size_t priority) : bundle(bundle), priority_(priority) {}
    ...
    struct SpillSlot : public TempObject, public InlineForwardListNode<SpillSlot> {
        ...
        SpillSlot(uint32_t slot, LifoAlloc* alloc) : alloc(slot), allocated(alloc) {}
    };
   ...
   private:
    ...
    VirtualRegister& vreg(const LDefinition* def) { return vregs[def->virtualRegister()]; }
    ...
    MOZ_MUST_USE bool tryAllocateFixed(LiveBundle* bundle, Requirement requirement, bool* success,
