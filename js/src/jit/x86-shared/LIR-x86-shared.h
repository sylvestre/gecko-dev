    ...
    const char* extraName() const {
        if (mir()->isTruncated()) {
            if (mir()->canBeNegativeZero()) {
                return mir()->canBeNegativeOverflow() ? "Truncate_NegativeZero_NegativeOverflow"
...
class LModI : public LBinaryMath<1> {
   public:
    ...
    const char* extraName() const { return mir()->isTruncated() ? "Truncated" : nullptr; }
...
class LGuardShape : public LInstructionHelper<0, 1, 0> {
   public:
    ...
    explicit LGuardShape(const LAllocation& in) { setOperand(0, in); }
...
class LGuardObjectGroup : public LInstructionHelper<0, 1, 0> {
   public:
    ...
    explicit LGuardObjectGroup(const LAllocation& in) { setOperand(0, in); }
...
class LSimdValueInt32x4 : public LInstructionHelper<1, 4, 0> {
   public:
    ...
    LSimdValueInt32x4(const LAllocation& x, const LAllocation& y, const LAllocation& z,
                      const LAllocation& w) {
        setOperand(0, x);
        setOperand(1, y);
        setOperand(2, z);
...
class LSimdValueFloat32x4 : public LInstructionHelper<1, 4, 1> {
   public:
    ...
    LSimdValueFloat32x4(const LAllocation& x, const LAllocation& y, const LAllocation& z,
                        const LAllocation& w, const LDefinition& copyY) {
        setOperand(0, x);
        setOperand(1, y);
        setOperand(2, z);
        setOperand(3, w);
