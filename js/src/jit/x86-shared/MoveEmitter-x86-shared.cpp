...
size_t MoveEmitterX86::characterizeCycle(const MoveResolver& moves, size_t i, bool* allGeneralRegs,
                                         bool* allFloatRegs) {
    size_t swapCount = 0;
...
bool MoveEmitterX86::maybeEmitOptimizedCycle(const MoveResolver& moves, size_t i,
...
void MoveEmitterX86::emit(const MoveResolver& moves) {
    ...
    if (scratchRegister_.isSome()) masm.mov(ImmWord(0xdeadbeef), scratchRegister_.value());
#endif
...
Address MoveEmitterX86::toAddress(const MoveOperand& operand) const {
    if (operand.base() != StackPointer) return Address(operand.base(), operand.disp());
...
Operand MoveEmitterX86::toPopOperand(const MoveOperand& operand) const {
    if (operand.isMemory()) {
        if (operand.base() != StackPointer) return Operand(operand.base(), operand.disp());
...
void MoveEmitterX86::emitSimd128IntMove(const MoveOperand& from, const MoveOperand& to) {
...
void MoveEmitterX86::emitSimd128FloatMove(const MoveOperand& from, const MoveOperand& to) {
    MOZ_ASSERT_IF(from.isFloatReg(), from.floatReg().isSimd128());
    MOZ_ASSERT_IF(to.isFloatReg(), to.floatReg().isSimd128());
...
Maybe<Register> MoveEmitterX86::findScratchRegister(const MoveResolver& moves, size_t initial) {
