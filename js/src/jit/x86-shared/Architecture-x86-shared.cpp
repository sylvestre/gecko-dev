...
#define FLOAT_REGS_(TYPE)                                                                     \
    "%xmm0" TYPE, "%xmm1" TYPE, "%xmm2" TYPE, "%xmm3" TYPE, "%xmm4" TYPE, "%xmm5" TYPE,       \
        "%xmm6" TYPE, "%xmm7" TYPE, "%xmm8" TYPE, "%xmm9" TYPE, "%xmm10" TYPE, "%xmm11" TYPE, \
...
#define FLOAT_REGS_(TYPE)                                                               \
    "%xmm0" TYPE, "%xmm1" TYPE, "%xmm2" TYPE, "%xmm3" TYPE, "%xmm4" TYPE, "%xmm5" TYPE, \
        ...
        FLOAT_REGS_(".s"), FLOAT_REGS_(".d"), FLOAT_REGS_(".i4"), FLOAT_REGS_(".s4")
#undef FLOAT_REGS_
...
js::jit::FloatRegisterSet js::jit::FloatRegister::ReduceSetForPush(const FloatRegisterSet& s) {
    SetType bits = s.bits();
...
uint32_t js::jit::FloatRegister::GetPushSizeInBytes(const FloatRegisterSet& s) {
    SetType all = s.bits();
    SetType set128b = (all >> (uint32_t(Codes::Simd128) * Codes::TotalPhys)) & Codes::AllPhysMask;
    SetType doubleSet = (all >> (uint32_t(Codes::Double) * Codes::TotalPhys)) & Codes::AllPhysMask;
    SetType singleSet = (all >> (uint32_t(Codes::Single) * Codes::TotalPhys)) & Codes::AllPhysMask;
    ...
    return mozilla::CountPopulation32(set128b) * (4 * sizeof(int32_t)) +
           mozilla::CountPopulation32(set64b) * sizeof(double) + count32b * sizeof(float);
}
