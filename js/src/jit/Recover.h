...
#define RINSTRUCTION_HEADER_(op)                                          \
   ...
   public:                                                                \
    ...
    void cloneInto(RInstructionStorage* raw) const override { new (raw->addr()) R##op(*this); }
...
#define RINSTRUCTION_HEADER_NUM_OP_(op, numOp) \
    ...
    static_assert(                             \
        ...
        "The recover instructions's numOperands should equal to the MIR's numOperands");
#else
#define RINSTRUCTION_HEADER_NUM_OP_(op, numOp) RINSTRUCTION_HEADER_NUM_OP_MAIN(op, numOp)
#endif
