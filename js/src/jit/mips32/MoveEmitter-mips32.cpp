...
void MoveEmitterMIPS::breakCycle(const MoveOperand& from, const MoveOperand& to, MoveOp::Type type,
                                 uint32_t slotId) {
...
void MoveEmitterMIPS::completeCycle(const MoveOperand& from, const MoveOperand& to,
...
void MoveEmitterMIPS::emitDoubleMove(const MoveOperand& from, const MoveOperand& to) {
    if (from.isFloatReg()) {
        ...
        // MacroAssembler::passABIArg
        MOZ_ASSERT(from.isMemory());
        MOZ_ASSERT(to.evenReg() == a2 && to.oddReg() == a3, "Invalid emitDoubleMove arguments.");
