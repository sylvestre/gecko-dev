...
const Registers::SetType Registers::JSCallMask = (1 << Registers::a2) | (1 << Registers::a3);

const Registers::SetType Registers::CallMask =
    (1 << Registers::v0) | (1 << Registers::v1);  // used for double-size returns
...
FloatRegister FloatRegister::doubleOverlay(unsigned int which) const {
    MOZ_ASSERT(!isInvalid());
    if (kind_ != Double) return FloatRegister(code_ & ~1, Double);
    return *this;
}

FloatRegister FloatRegister::singleOverlay(unsigned int which) const {
    MOZ_ASSERT(!isInvalid());
    if (kind_ == Double) {
