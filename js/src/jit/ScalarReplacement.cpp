...
class EmulateStateOf {
   ...
   public:
    EmulateStateOf(MIRGenerator* mir, MIRGraph& graph) : mir_(mir), graph_(graph) {}
...
bool EmulateStateOf<MemoryView>::run(MemoryView& view) {
    ...
    if (!states_.appendN(nullptr, graph_.numBlocks())) return false;

    // Initialize the first block which needs to be traversed in RPO.
    MBasicBlock* startBlock = view.startingBlock();
    if (!view.initStartingState(&states_[startBlock->id()])) return false;

    // Iterate over each basic block which has a valid entry state, and merge
    // the state in the successor blocks.
    for (ReversePostorderIterator block = graph_.rpoBegin(startBlock); block != graph_.rpoEnd();
         block++) {
        if (mir_->shouldCancel(MemoryView::phaseName)) return false;
        ...
        for (size_t s = 0; s < block->numSuccessors(); s++) {
            ...
            if (!view.mergeIntoSuccessorState(*block, succ, &states_[succ->id()])) return false;
        }
    }
...
static bool IsObjectEscaped(MInstruction* ins, JSObject* objDefault) {
    ...
    if (!obj) obj = MObjectState::templateObjectOf(ins);
...
bool ObjectMemoryView::initStartingState(BlockState** pState) {
    ...
    if (!state->initFromTemplateObject(alloc_, undefinedVal_)) return false;
...
bool ObjectMemoryView::mergeIntoSuccessorState(MBasicBlock* curr, MBasicBlock* succ,
    ...
    if (!succState) {
        ...
        if (!startBlock_->dominates(succ)) return true;
...
void ObjectMemoryView::visitFunctionEnvironment(MFunctionEnvironment* ins) {
    ...
    if (input->isLambda()) {
        if (input->toLambda()->environmentChain() != obj_) return;
    } else if (input->isLambdaArrow()) {
        if (input->toLambdaArrow()->environmentChain() != obj_) return;
    } else {
        return;
    }
...
void ObjectMemoryView::visitLambda(MLambda* ins) {
    if (ins->environmentChain() != obj_) return;
...
static size_t GetOffsetOf(MDefinition* index, size_t width, int32_t baseOffset) {
...
static size_t GetOffsetOf(MDefinition* index, Scalar::Type type, int32_t baseOffset) {
    return GetOffsetOf(index, Scalar::byteSize(type), baseOffset);
}

void ObjectMemoryView::storeOffset(MInstruction* ins, size_t offset, MDefinition* value) {
    // Clone the state and update the slot value.
...
void ObjectMemoryView::visitStoreUnboxedObjectOrNull(MStoreUnboxedObjectOrNull* ins) {
    // Skip stores made on other objects.
    if (ins->elements() != obj_) return;
...
void ObjectMemoryView::visitLoadUnboxedObjectOrNull(MLoadUnboxedObjectOrNull* ins) {
    // Skip loads made on other objects.
    if (ins->elements() != obj_) return;
...
static bool IndexOf(MDefinition* ins, int32_t* res) {
    ...
    if (indexDef->isBoundsCheck()) indexDef = indexDef->toBoundsCheck()->index();
    if (indexDef->isToInt32()) indexDef = indexDef->toToInt32()->getOperand(0);
    MConstant* indexDefConst = indexDef->maybeConstantValue();
    if (!indexDefConst || indexDefConst->type() != MIRType::Int32) return false;
...
static bool IsElementEscaped(MElements* def, uint32_t arraySize) {
        ...
        switch (access->op()) {
            case MDefinition::Opcode::LoadElement: {
                ...
                if (!IndexOf(access, &index)) {
                    JitSpewDef(JitSpew_Escape, "has a load element with a non-trivial index\n",
                               access);
                    return true;
                }
                if (index < 0 || arraySize <= uint32_t(index)) {
                    JitSpewDef(JitSpew_Escape, "has a load element with an out-of-bound index\n",
                               access);
                    return true;
                }
                break;
...
static bool IsArrayEscaped(MInstruction* ins) {
    MOZ_ASSERT(ins->type() == MIRType::Object);
...
bool ArrayMemoryView::mergeIntoSuccessorState(MBasicBlock* curr, MBasicBlock* succ,
                                              BlockState** pSuccState) {
    BlockState* succState = *pSuccState;
...
void ArrayMemoryView::discardInstruction(MInstruction* ins, MDefinition* elements) {
    MOZ_ASSERT(elements->isElements());
    ins->block()->discard(ins);
    if (!elements->hasLiveDefUses()) elements->block()->discard(elements->toInstruction());
}

void ArrayMemoryView::visitStoreElement(MStoreElement* ins) {
    // Skip other array objects.
    MDefinition* elements = ins->elements();
    if (!isArrayStateElements(elements)) return;
...
bool ScalarReplacement(MIRGenerator* mir, MIRGraph& graph) {
    ...
    for (ReversePostorderIterator block = graph.rpoBegin(); block != graph.rpoEnd(); block++) {
        ...
        for (MInstructionIterator ins = block->begin(); ins != block->end(); ins++) {
            ...
            if (ins->isNewArray() && !IsArrayEscaped(*ins)) {
                ...
                if (!replaceArray.run(view)) return false;
                view.assertSuccess();
                addedPhi = true;
                continue;
            }
    ...
    if (addedPhi) {
        ...
        if (!EliminatePhis(mir, graph, ConservativeObservability)) return false;
    }
