...
AutoDetectInvalidation::AutoDetectInvalidation(JSContext* cx, MutableHandleValue rval)
    : cx_(cx), ionScript_(GetTopJitJSScript(cx)->ionScript()), rval_(rval), disabled_(false) {}
...
bool InvokeFunction(JSContext* cx, HandleObject obj, bool constructing, bool ignoresReturnValue,
...
#ifdef JS_SIMULATOR
static bool CheckSimulatorRecursionLimitWithExtra(JSContext* cx, uint32_t extra) {
...
#ifdef JS_SIMULATOR
        (void)checkSp;
        if (!CheckSimulatorRecursionLimitWithExtra(cx, extra)) frame->setOverRecursed();
#else
        if (!CheckRecursionLimitWithStackPointer(cx, checkSp)) frame->setOverRecursed();
#endif
        return true;
    }
...
bool DefVar(JSContext* cx, HandlePropertyName dn, unsigned attrs, HandleObject envChain) {
    // Given the ScopeChain, extract the VarObj.
    RootedObject obj(cx, BindVar(cx, envChain));
    return DefVarOperation(cx, obj, dn, attrs);
}

bool DefLexical(JSContext* cx, HandlePropertyName dn, unsigned attrs, HandleObject envChain) {
...
template bool LooselyEqual<true>(JSContext* cx, MutableHandleValue lhs, MutableHandleValue rhs,
                                 bool* res);
template bool LooselyEqual<false>(JSContext* cx, MutableHandleValue lhs, MutableHandleValue rhs,
                                  bool* res);
...
template bool StrictlyEqual<true>(JSContext* cx, MutableHandleValue lhs, MutableHandleValue rhs,
                                  bool* res);
template bool StrictlyEqual<false>(JSContext* cx, MutableHandleValue lhs, MutableHandleValue rhs,
                                   bool* res);
...
bool StringSplitHelper(JSContext* cx, HandleString str, HandleString sep, HandleObjectGroup group,
...
bool ArrayPushDense(JSContext* cx, HandleArrayObject arr, HandleValue v, uint32_t* length) {
    *length = arr->length();
    DenseElementResult result =
        arr->setOrExtendDenseElements(cx, *length, v.address(), 1, ShouldUpdateTypes::DontUpdate);
...
bool SetArrayLength(JSContext* cx, HandleObject obj, HandleValue value, bool strict) {
    ...
    if (array->lengthIsWritable()) {
        if (!ArraySetLength(cx, array, id, JSPROP_PERMANENT, value, result)) return false;
    } else {
...
JSObject* NewCallObject(JSContext* cx, HandleShape shape, HandleObjectGroup group) {
    ...
    if (!IsInsideNursery(obj)) cx->zone()->group()->storeBuffer().putWholeCell(obj);
...
JSObject* NewSingletonCallObject(JSContext* cx, HandleShape shape) {
    ...
    // the initializing writes. The interpreter, however, may have allocated
    // the call object tenured, so barrier as needed before re-entering.
    MOZ_ASSERT(!IsInsideNursery(obj), "singletons are created in the tenured heap");
...
bool GetIntrinsicValue(JSContext* cx, HandlePropertyName name, MutableHandleValue rval) {
    if (!GlobalObject::getIntrinsicValue(cx, cx->global(), name, rval)) return false;
...
bool CreateThis(JSContext* cx, HandleObject callee, HandleObject newTarget,
    ...
    if (callee->is<JSFunction>()) {
        ...
        if (fun->isInterpreted() && fun->isConstructor()) {
            ...
            if (!script || !script->ensureHasTypes(cx)) return false;
            if (!js::CreateThis(cx, fun, script, newTarget, GenericObject, rval)) return false;
        }
    }
...
void GetDynamicName(JSContext* cx, JSObject* envChain, JSString* str, Value* vp) {
    ...
    if (LookupNameNoGC(cx, atom->asPropertyName(), envChain, &scope, &pobj, &prop)) {
        if (FetchNameNoGC(pobj, prop, MutableHandleValue::fromMarkedLocation(vp))) return;
    }
...
void PostWriteElementBarrier(JSRuntime* rt, JSObject* obj, int32_t index) {
    ...
    if (nobj->getDenseInitializedLength() > MAX_WHOLE_CELL_BUFFER_SIZE
        ...
        rt->gc.storeBuffer().putSlot(nobj, HeapSlot::Element, nobj->unshiftedIndex(index), 1);
        return;
    }
...
template void PostWriteElementBarrier<IndexInBounds::Yes>(JSRuntime* rt, JSObject* obj,
                                                          int32_t index);

template void PostWriteElementBarrier<IndexInBounds::Maybe>(JSRuntime* rt, JSObject* obj,
                                                            int32_t index);
...
bool CheckGlobalOrEvalDeclarationConflicts(JSContext* cx, BaselineFrame* frame) {
    ...
    if (script->isForEval()) {
        ...
        if (!script->bodyScope()->hasEnvironment()) {
            ...
            if (!CheckEvalDeclarationConflicts(cx, script, envChain, varObj)) return false;
        }
    } else {
        Rooted<LexicalEnvironmentObject*> lexicalEnv(
            cx, &NearestEnclosingExtensibleLexicalEnvironment(envChain));
        if (!CheckGlobalDeclarationConflicts(cx, script, lexicalEnv, varObj)) return false;
    }
...
bool PushLexicalEnv(JSContext* cx, BaselineFrame* frame, Handle<LexicalScope*> scope) {
...
bool DebugLeaveThenPopLexicalEnv(JSContext* cx, BaselineFrame* frame, jsbytecode* pc) {
...
bool DebugLeaveThenFreshenLexicalEnv(JSContext* cx, BaselineFrame* frame, jsbytecode* pc) {
    MOZ_ALWAYS_TRUE(DebugLeaveLexicalEnv(cx, frame, pc));
    return frame->freshenLexicalEnvironment(cx);
}
...
bool DebugLeaveThenRecreateLexicalEnv(JSContext* cx, BaselineFrame* frame, jsbytecode* pc) {
    MOZ_ALWAYS_TRUE(DebugLeaveLexicalEnv(cx, frame, pc));
    return frame->recreateLexicalEnvironment(cx);
}
...
bool DebugLeaveLexicalEnv(JSContext* cx, BaselineFrame* frame, jsbytecode* pc) {
    ...
    if (cx->compartment()->isDebuggee()) DebugEnvironments::onPopLexical(cx, frame, pc);
    return true;
}
...
bool ForcedRecompile(JSContext* cx) { return RecompileImpl(cx, /* force = */ true); }
...
void AutoDetectInvalidation::setReturnOverride() { cx_->setIonReturnOverride(rval_.get()); }
...
bool BaselineThrowInitializedThis(JSContext* cx, BaselineFrame* frame) {
...
bool CallNativeSetter(JSContext* cx, HandleFunction callee, HandleObject obj, HandleValue rhs) {
    MOZ_ASSERT(callee->isNative());
    JSNative natfun = callee->native();
...
static MOZ_ALWAYS_INLINE bool GetNativeDataProperty(JSContext* cx, NativeObject* obj, jsid id,
                                                    Value* vp) {
...
template bool GetNativeDataProperty<true>(JSContext* cx, JSObject* obj, PropertyName* name,
                                          Value* vp);

template bool GetNativeDataProperty<false>(JSContext* cx, JSObject* obj, PropertyName* name,
                                           Value* vp);

static MOZ_ALWAYS_INLINE bool ValueToAtomOrSymbol(JSContext* cx, Value& idVal, jsid* id) {
    AutoUnsafeCallWithABI unsafe;
...
template bool GetNativeDataPropertyByValue<true>(JSContext* cx, JSObject* obj, Value* vp);

template bool GetNativeDataPropertyByValue<false>(JSContext* cx, JSObject* obj, Value* vp);
...
bool SetNativeDataProperty(JSContext* cx, JSObject* obj, PropertyName* name, Value* val) {
    ...
    if (!shape || !shape->isDataProperty() || !shape->writable() || nobj->watched()) {
        return false;
    }

    if (NeedsTypeBarrier && !HasTypePropertyId(nobj, NameToId(name), *val)) return false;

    nobj->setSlot(shape->slot(), *val);
    return true;
}

template bool SetNativeDataProperty<true>(JSContext* cx, JSObject* obj, PropertyName* name,
                                          Value* val);

template bool SetNativeDataProperty<false>(JSContext* cx, JSObject* obj, PropertyName* name,
                                           Value* val);
...
bool ObjectHasGetterSetter(JSContext* cx, JSObject* objArg, Shape* propShape) {
    ...
    while (true) {
        ...
        if (!nobj->is<PlainObject>()) {
            if (ClassMayResolveId(cx->names(), nobj->getClass(), id, nobj)) return false;
        }
...
template bool HasNativeDataProperty<true>(JSContext* cx, JSObject* obj, Value* vp);

template bool HasNativeDataProperty<false>(JSContext* cx, JSObject* obj, Value* vp);
...
typedef bool (*SetObjectElementFn)(JSContext*, HandleObject, HandleValue, HandleValue, HandleValue,
                                   bool);
