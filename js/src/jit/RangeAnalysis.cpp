...
static inline void SpewTruncate(MDefinition* def, MDefinition::TruncateKind kind,
                                bool shouldClone) {
#ifdef JS_JITSPEW
...
void RangeAnalysis::replaceDominatedUsesWith(MDefinition* orig, MDefinition* dom,
                                             MBasicBlock* block) {
    for (MUseIterator i(orig->usesBegin()); i != orig->usesEnd();) {
        MUse* use = *i++;
        if (use->consumer() != dom && IsDominatedUse(block, use)) use->replaceProducer(dom);
    }
}
    ...
    for (PostorderIterator i(graph_.poBegin()); i != graph_.poEnd(); i++) {
        ...
        for (MDefinitionIterator iter(*i); iter;) {
            ...
            if (def->isBeta()) {
                ...
                JitSpew(JitSpew_Range, "Removing beta node %d for %d", def->id(), op->id());
...
Range* Range::intersect(TempAllocator& alloc, const Range* lhs, const Range* rhs,
                        bool* emptyRange) {
    *emptyRange = false;
    ...
    FractionalPartFlag newCanHaveFractionalPart =
        FractionalPartFlag(lhs->canHaveFractionalPart_ && rhs->canHaveFractionalPart_);
    ...
    if (lhs->canHaveFractionalPart() != rhs->canHaveFractionalPart() ||
        (lhs->canHaveFractionalPart() && newHasInt32LowerBound && newHasInt32UpperBound &&
         newLower == newUpper)) {
        refineInt32BoundsByExponent(newExponent, &newLower, &newHasInt32LowerBound, &newUpper,
...
void Range::unionWith(const Range* other) {
    ...
    FractionalPartFlag newCanHaveFractionalPart =
        FractionalPartFlag(canHaveFractionalPart_ || other->canHaveFractionalPart_);
    NegativeZeroFlag newMayIncludeNegativeZero =
...
Range::Range(const MDefinition* def) : symbolicLower_(nullptr), symbolicUpper_(nullptr) {
...
Range* Range::add(TempAllocator& alloc, const Range* lhs, const Range* rhs) {
    ...
    if (!lhs->hasInt32LowerBound() || !rhs->hasInt32LowerBound()) l = NoInt32LowerBound;

    int64_t h = (int64_t)lhs->upper_ + (int64_t)rhs->upper_;
    if (!lhs->hasInt32UpperBound() || !rhs->hasInt32UpperBound()) h = NoInt32UpperBound;
    ...
    if (lhs->canBeInfiniteOrNaN() && rhs->canBeInfiniteOrNaN()) e = Range::IncludesInfinityAndNaN;

    return new (alloc) Range(
        l, h, FractionalPartFlag(lhs->canHaveFractionalPart() || rhs->canHaveFractionalPart()),
        NegativeZeroFlag(lhs->canBeNegativeZero() && rhs->canBeNegativeZero()), e);
}

Range* Range::sub(TempAllocator& alloc, const Range* lhs, const Range* rhs) {
    int64_t l = (int64_t)lhs->lower_ - (int64_t)rhs->upper_;
    if (!lhs->hasInt32LowerBound() || !rhs->hasInt32UpperBound()) l = NoInt32LowerBound;

    int64_t h = (int64_t)lhs->upper_ - (int64_t)rhs->lower_;
    if (!lhs->hasInt32UpperBound() || !rhs->hasInt32LowerBound()) h = NoInt32UpperBound;
...
Range* Range::mul(TempAllocator& alloc, const Range* lhs, const Range* rhs) {
    ...
    if (MissingAnyInt32Bounds(lhs, rhs))
        return new (alloc) Range(NoInt32LowerBound, NoInt32UpperBound, newCanHaveFractionalPart,
    ...
    return new (alloc) Range(Min(Min(a, b), Min(c, d)), Max(Max(a, b), Max(c, d)),
...
Range* Range::rsh(TempAllocator& alloc, const Range* lhs, int32_t c) {
    MOZ_ASSERT(lhs->isInt32());
    int32_t shift = c & 0x1f;
    return Range::NewInt32Range(alloc, lhs->lower() >> shift, lhs->upper() >> shift);
}
...
Range* Range::abs(TempAllocator& alloc, const Range* op) {
    ...
    return new (alloc) Range(Max(Max(int32_t(0), l), u == INT32_MIN ? INT32_MAX : -u), true,
...
Range* Range::min(TempAllocator& alloc, const Range* lhs, const Range* rhs) {
    ...
    return new (alloc)
        Range(Min(lhs->lower_, rhs->lower_), lhs->hasInt32LowerBound_ && rhs->hasInt32LowerBound_,
              Min(lhs->upper_, rhs->upper_), lhs->hasInt32UpperBound_ || rhs->hasInt32UpperBound_,
...
Range* Range::max(TempAllocator& alloc, const Range* lhs, const Range* rhs) {
    ...
    return new (alloc)
        Range(Max(lhs->lower_, rhs->lower_), lhs->hasInt32LowerBound_ || rhs->hasInt32LowerBound_,
              Max(lhs->upper_, rhs->upper_), lhs->hasInt32UpperBound_ && rhs->hasInt32UpperBound_,
              newCanHaveFractionalPart, newMayIncludeNegativeZero,
              Max(lhs->max_exponent_, rhs->max_exponent_));
}
...
Range* Range::sign(TempAllocator& alloc, const Range* op) {
    ...
    return new (alloc)
        Range(Max(Min(op->lower_, 1), -1), Max(Min(op->upper_, 1), -1),
              Range::ExcludesFractionalParts, NegativeZeroFlag(op->canBeNegativeZero()), 0);
}
...
bool Range::update(const Range* other) {
    bool changed = lower_ != other->lower_ || hasInt32LowerBound_ != other->hasInt32LowerBound_ ||
                   upper_ != other->upper_ || hasInt32UpperBound_ != other->hasInt32UpperBound_ ||
...
void MAbs::computeRange(TempAllocator& alloc) {
    if (specialization_ != MIRType::Int32 && specialization_ != MIRType::Double) return;
...
void MAdd::computeRange(TempAllocator& alloc) {
    if (specialization() != MIRType::Int32 && specialization() != MIRType::Double) return;
    Range left(getOperand(0));
    Range right(getOperand(1));
...
void MMul::computeRange(TempAllocator& alloc) {
    ...
    Range left(getOperand(0));
    Range right(getOperand(1));
    if (canBeNegativeZero()) canBeNegativeZero_ = Range::negativeZeroMul(&left, &right);
...
void MMod::computeRange(TempAllocator& alloc) {
    ...
    if (specialization() == MIRType::Int32 && rhs.lower() > 0) {
        bool hasDoubles =
            lhs.lower() < 0 || lhs.canHaveFractionalPart() || rhs.canHaveFractionalPart();
        ...
        bool hasUint32s = IsUint32Type(getOperand(0)) && getOperand(1)->type() == MIRType::Int32 &&
    ...
    if (!lhs.canHaveFractionalPart() && !rhs.canHaveFractionalPart()) --rhsAbsBound;
    ...
    Range::FractionalPartFlag newCanHaveFractionalPart =
        Range::FractionalPartFlag(lhs.canHaveFractionalPart() || rhs.canHaveFractionalPart());
    ...
    setRange(new (alloc) Range(lower, upper, newCanHaveFractionalPart, newMayIncludeNegativeZero,
...
void MToDouble::computeRange(TempAllocator& alloc) { setRange(new (alloc) Range(getOperand(0))); }
...
void MToInt32::computeRange(TempAllocator& alloc) {
...
void MLoadTypedArrayElementStatic::computeRange(TempAllocator& alloc) {
...
void MStringLength::computeRange(TempAllocator& alloc) {
    static_assert(JSString::MAX_LENGTH <= UINT32_MAX, "NewUInt32Range requires a uint32 value");
...
void MArgumentsLength::computeRange(TempAllocator& alloc) {
    ...
    MOZ_ASSERT(JitOptions.maxStackArgs <= UINT32_MAX, "NewUInt32Range requires a uint32 value");
...
void MMathFunction::computeRange(TempAllocator& alloc) {
    ...
    switch (function()) {
        case Sin:
        case Cos:
            if (!opRange.canBeInfiniteOrNaN()) setRange(Range::NewDoubleRange(alloc, -1.0, 1.0));
            break;
        case Sign:
...
bool RangeAnalysis::analyzeLoop(MBasicBlock* header) {
    ...
    if (JitSpewEnabled(JitSpew_Range)) {
        ...
        JitSpew(JitSpew_Range, "computed symbolic bound on backedges: %s", sp.string());
    }
#endif
    ...
    if (!mir->compilingWasm()) {
        ...
        for (ReversePostorderIterator iter(graph_.rpoBegin(header)); iter != graph_.rpoEnd();
             iter++) {
...
LoopIterationBound* RangeAnalysis::analyzeLoopIterationCount(MBasicBlock* header, MTest* test,
                                                             BranchDirection direction) {
    SimpleLinearSum lhs(nullptr, 0);
    MDefinition* rhs;
    bool lessEqual;
    if (!ExtractLinearInequality(test, direction, &lhs, &rhs, &lessEqual)) return nullptr;
    ...
    if (!lhs.term || !lhs.term->isPhi() || lhs.term->block() != header) return nullptr;
...
void RangeAnalysis::analyzeLoopPhi(MBasicBlock* header, LoopIterationBound* loopBound, MPhi* phi) {
    ...
    if (!limitSum.multiply(modified.constant) || !limitSum.add(initialSum)) return;
...
static inline bool SymbolicBoundIsValid(MBasicBlock* header, MBoundsCheck* ins,
    ...
    while (bb != header && bb != bound->loop->test->block()) bb = bb->immediateDominator();
    return bb == bound->loop->test->block();
}

bool RangeAnalysis::tryHoistBoundsCheck(MBasicBlock* header, MBoundsCheck* ins) {
    ...
    if (!SafeSub(lowerConstant, lower->sum.constant(), &lowerConstant)) return false;
    ...
    if (!SafeAdd(upper->sum.constant(), upperConstant, &upperConstant)) return false;
...
bool RangeAnalysis::addRangeAssertions() {
    ...
    for (ReversePostorderIterator iter(graph_.rpoBegin()); iter != graph_.rpoEnd(); iter++) {
        ...
        for (MDefinitionIterator iter(block); iter; iter++) {
            ...
            if (r.isUnknown() || (ins->type() == MIRType::Int32 && r.isUnknownInt32())) continue;
...
void Range::wrapAroundToInt32() {
    if (!hasInt32Bounds()) {
        ...
        canHaveFractionalPart_ = ExcludesFractionalParts;
        canBeNegativeZero_ = ExcludesNegativeZero;
        refineInt32BoundsByExponent(max_exponent_, &lower_, &hasInt32LowerBound_, &upper_,
                                    &hasInt32UpperBound_);
...
bool MConstant::needTruncation(TruncateKind kind) { return IsFloatingPointType(type()); }
...
void MPhi::truncate() {
    setResultType(MIRType::Int32);
    if (truncateKind_ >= IndirectTruncate && range()) range()->wrapAroundToInt32();
}
...
void MAdd::truncate() {
    ...
    setResultType(MIRType::Int32);
    if (truncateKind() >= IndirectTruncate && range()) range()->wrapAroundToInt32();
}
...
bool MLoadTypedArrayElementStatic::needTruncation(TruncateKind kind) {
    ...
    if (kind == Truncate) setInfallible();
...
MDefinition::TruncateKind MTruncateToInt32::operandTruncateKind(size_t index) const {
    // This operator is an explicit truncate to int32.
    return Truncate;
}

MDefinition::TruncateKind MBinaryBitwiseInstruction::operandTruncateKind(size_t index) const {
    // The bitwise operators truncate to int32.
    return Truncate;
}

MDefinition::TruncateKind MLimitedTruncate::operandTruncateKind(size_t index) const {
...
MDefinition::TruncateKind MStoreUnboxedScalar::operandTruncateKind(size_t index) const {
...
MDefinition::TruncateKind MStoreTypedArrayElementHole::operandTruncateKind(size_t index) const {
    // An integer store truncates the stored value.
    return index == 3 && isIntegerWrite() ? Truncate : NoTruncate;
}

MDefinition::TruncateKind MStoreTypedArrayElementStatic::operandTruncateKind(size_t index) const {
    // An integer store truncates the stored value.
...
static bool TruncateTest(TempAllocator& alloc, MTest* test) {
    ...
    for (size_t i = 0; i < phi->numOperands(); i++) {
        ...
        if (inner->type() != MIRType::Boolean && inner->type() != MIRType::Int32) return true;
    }
...
// flowing into.
static bool CloneForDeadBranches(TempAllocator& alloc, MInstruction* candidate) {
    ...
    for (size_t i = 0; i < end; ++i) operands.infallibleAppend(candidate->getOperand(i));
    ...
    for (MUseIterator i(candidate->usesBegin()); i != candidate->usesEnd();) {
        ...
        if (ins->isDefinition() && !ins->toDefinition()->isRecoveredOnBailout()) continue;
...
static MDefinition::TruncateKind ComputeRequestedTruncateKind(MDefinition* candidate,
                                                              bool* shouldClone) {
    ...
    for (MUseIterator use(candidate->usesBegin()); use != candidate->usesEnd(); use++) {
        if (use->consumer()->isResumePoint()) {
            ...
            isObservableResult =
                isObservableResult || use->consumer()->toResumePoint()->isObservableOperand(*use);
...
static MDefinition::TruncateKind ComputeTruncateKind(MDefinition* candidate, bool* shouldClone) {
    ...
    if ((candidate->isDiv() || candidate->isMod()) &&
        static_cast<const MBinaryArithInstruction*>(candidate)->specialization() ==
            MIRType::Int32) {
...
static void RemoveTruncatesOnOutput(MDefinition* truncated) {
    ...
    for (MUseDefIterator use(truncated); use; use++) {
        ...
        if (!def->isTruncateToInt32() || !def->isToInt32()) continue;

        def->replaceAllUsesWith(truncated);
    }
}

static void AdjustTruncatedInputs(TempAllocator& alloc, MDefinition* truncated) {
...
void MLoadTypedArrayElementStatic::collectRangeInfoPreTrunc() {
    Range range(ptr());
    ...
    if (range.hasInt32LowerBound() && range.hasInt32UpperBound()) {
        ...
        if (lower >= 0 && upper < length) setNeedsBoundsCheck(false);
    }
}

void MStoreTypedArrayElementStatic::collectRangeInfoPreTrunc() {
    Range range(ptr());
...
void MMul::collectRangeInfoPreTrunc() {
    ...
    if (lhsRange.isFiniteNonNegative() && !lhsRange.canBeZero()) setCanBeNegativeZero(false);

    // Likewise rhsRange.
    if (rhsRange.isFiniteNonNegative() && !rhsRange.canBeZero()) setCanBeNegativeZero(false);
    ...
    if (rhsRange.isFiniteNegative() && lhsRange.isFiniteNegative()) setCanBeNegativeZero(false);
}
...
void MToInt32::collectRangeInfoPreTrunc() {
    Range inputRange(input());
...
void MBoundsCheck::collectRangeInfoPreTrunc() {
    ...
    if (!indexRange.hasInt32LowerBound() || !indexRange.hasInt32UpperBound()) return;
    ...
    if (indexLower + min >= 0 && indexUpper + max < lengthLower) fallible_ = false;
}

void MBoundsCheckLower::collectRangeInfoPreTrunc() {
    Range indexRange(index());
    if (indexRange.hasInt32LowerBound() && indexRange.lower() >= minimum_) fallible_ = false;
}

void MCompare::collectRangeInfoPreTrunc() {
    if (!Range(lhs()).canBeNaN() && !Range(rhs()).canBeNaN()) operandsAreNeverNaN_ = true;
}
