...
class ComplexAddress {
   ...
   public:
    ...
    ComplexAddress(int32_t disp, Register::Encoding base)
        : disp_(disp), base_(base), index_(Registers::Invalid), scale_(0), isPCRelative_(false) {
        MOZ_ASSERT(*this == *this);
    ...
    ComplexAddress(int32_t disp, Register::Encoding base, Register::Encoding index, int scale)
        : disp_(disp), base_(base), index_(index), scale_(scale), isPCRelative_(false) {
...
class HeapAccess {
   ...
   public:
    ...
    HeapAccess(Kind kind, size_t size, const ComplexAddress& address,
        ...
        : kind_(kind), size_(size), address_(address), otherOperand_(otherOperand) {
...
inline void VerifyHeapAccess(uint8_t* begin, uint8_t* end, const HeapAccess& expected) {
