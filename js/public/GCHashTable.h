...
class GCRekeyableHashMap
    : public JS::GCHashMap<Key, Value, HashPolicy, AllocPolicy, MapSweepPolicy>
{
    ...
    : public WrappedPtrOperations<JS::GCHashMap<Args...>, Wrapper>
{
 ...
 public:
  ...
  {
    return map().add(
        p, mozilla::Forward<KeyInput>(k), mozilla::Forward<ValueInput>(v));
  }
  ...
  {
    return map().relookupOrAdd(
        p, k, mozilla::Forward<KeyInput>(k), mozilla::Forward<ValueInput>(v));
  }
