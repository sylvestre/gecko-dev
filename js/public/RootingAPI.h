...
JS_FRIEND_API(void)
HeapObjectPostBarrier(JSObject** objp, JSObject* prev, JSObject* next);
...
class MOZ_NONHEAP_CLASS Handle : public js::HandleBase<T, Handle<T>>
{
 ...
 private:
  ...
  enum Disambiguator
  {
    DeliberatelyChoosingThisOverload = 42
  };
  enum CallerIdentity
  {
    ImUsingThisOnlyInFromFromMarkedLocation = 17
  };
...
struct BarrierMethods<T*>
{
  ...
  {
    if (next)
      JS::AssertGCThingIsNotAnObjectSubclass(
...
class alignas(8) DispatchWrapper
{
 ...
 public:
  ...
  MOZ_IMPLICIT DispatchWrapper(U&& initial)
      : tracer(&JS::GCPolicy<T>::trace), storage(mozilla::Forward<U>(initial))
  {
  }
 ...
 */
template<typename T>
using MaybeWrapped = typename mozilla::Conditional<MapTypeToRootKind<T>::kind ==
...
JS_PUBLIC_API(void)
AddPersistentRoot(RootingContext* cx,
                  RootKind kind,
                         ...
                         private mozilla::LinkedListElement<PersistentRooted<T>>
{
  ...
  void registerWithRootLists(RootingContext* cx)
  {
    ...
    AddPersistentRoot(
        cx, kind, reinterpret_cast<JS::PersistentRooted<void*>*>(this));
  }
  ...
  void registerWithRootLists(JSRuntime* rt)
  {
    ...
    JS::RootKind kind = JS::MapTypeToRootKind<T>::kind;
    AddPersistentRoot(
        rt, kind, reinterpret_cast<JS::PersistentRooted<void*>*>(this));
  }
...
void
CallTraceCallbackOnNonHeap(T* v,
