...
JS_ENUM_HEADER(JSValueShiftedTag, uint64_t){
    ...
    JSVAL_SHIFTED_TAG_UNDEFINED = (((uint64_t)JSVAL_TAG_UNDEFINED)
                                   << JSVAL_TAG_SHIFT),
    JSVAL_SHIFTED_TAG_NULL = (((uint64_t)JSVAL_TAG_NULL) << JSVAL_TAG_SHIFT),
    JSVAL_SHIFTED_TAG_BOOLEAN = (((uint64_t)JSVAL_TAG_BOOLEAN)
                                 << JSVAL_TAG_SHIFT),
    JSVAL_SHIFTED_TAG_MAGIC = (((uint64_t)JSVAL_TAG_MAGIC) << JSVAL_TAG_SHIFT),
    JSVAL_SHIFTED_TAG_STRING = (((uint64_t)JSVAL_TAG_STRING)
                                << JSVAL_TAG_SHIFT),
    JSVAL_SHIFTED_TAG_SYMBOL = (((uint64_t)JSVAL_TAG_SYMBOL)
                                << JSVAL_TAG_SHIFT),
    JSVAL_SHIFTED_TAG_PRIVATE_GCTHING = (((uint64_t)JSVAL_TAG_PRIVATE_GCTHING)
                                         << JSVAL_TAG_SHIFT),
...
static inline constexpr JS::Value
UndefinedValue();
...
class MOZ_NON_PARAM alignas(8) Value
{
 public:
  ...
  void setNull() { data.asBits = bitsFromTagAndPayload(JSVAL_TAG_NULL, 0); }
 ...
 public:
  ...
  JSValueTag toNunboxTag() const { return data.s.tag; }
#elif defined(JS_PUNBOX64)
...
#elif defined(JS_PUNBOX64)
    return (data.asBits | mozilla::DoubleTypeTraits::kSignBit) <=
           JSVAL_SHIFTED_TAG_MAX_DOUBLE;
#endif
  }
  ...
  uint64_t asRawBits() const { return data.asBits; }
 ...
 public:
  ...
  static constexpr Value fromTagAndPayload(JSValueTag tag, PayloadType payload)
  {
...
static_assert(sizeof(Value) == 8,
              "Value size must leave three tag bits, be a binary power, and is "
              "ubiquitously depended upon everywhere");
...
#endif
  if (v.isGCThing()) js::gc::ExposeGCThingToActiveJS(GCCellPtr(v));
}
...
{
  if (mozilla::IsInfinite(d) || mozilla::IsFinite(d)) return true;

  uint64_t bits;
  mozilla::BitwiseCast<uint64_t>(d, &bits);
  return (bits & ~mozilla::DoubleTypeTraits::kSignBit) ==
...
namespace JS {
JS_PUBLIC_API(void)
HeapValuePostBarrier(Value* valuep, const Value& prev, const Value& next);
...
struct BarrierMethods<JS::Value>
{
  ...
  static void postBarrier(JS::Value* v,
                          const JS::Value& prev,
                          const JS::Value& next)
  {
...
{
  if (!value.isGCThing()) return true;
