...
template<typename T>
JS_PUBLIC_API(void)
TraceExternalEdge(JSTracer* trc, T* thingp, const char* name);
}  // namespace gc
}  // namespace js
...
using CompartmentSet = js::HashSet<JSCompartment*,
...
extern JS_PUBLIC_API(void) JS_GetTraceThingInfo(char* buf,
                                                size_t bufsize,
                                                JSTracer* trc,
                                                void* thing,
                                                JS::TraceKind kind,
