...
class JS_PUBLIC_API(DominatorTree)
{
 private:
  ...
  using PredecessorSets = js::
      HashMap<Node, NodeSetPtr, js::DefaultHasher<Node>, js::SystemAllocPolicy>;
  using NodeToIndexMap = js::
      HashMap<Node, uint32_t, js::DefaultHasher<Node>, js::SystemAllocPolicy>;
  class DominatedSets;
 ...
 public:
  ...
  class DominatedSetRange
  {
    ...
    DominatedSetRange(JS::ubi::Vector<Node>& postOrder,
 ...
 private:
  ...
  class DominatedSets
  {
    ...
    DominatedSets(JS::ubi::Vector<uint32_t>&& dominated,
                  JS::ubi::Vector<uint32_t>&& indices)
        : dominated(mozilla::Move(dominated)), indices(mozilla::Move(indices))
    {
    }
 ...
 private:
  ...
  static uint32_t intersect(JS::ubi::Vector<uint32_t>& doms,
                            uint32_t finger1,
                            uint32_t finger2)
  {
  ...
  // predecessor sets.
  static MOZ_MUST_USE bool doTraversal(JSContext* cx,
 ...
 public:
                                              ...
                                              const Node& root)
  {
    ...
    JS::ubi::Vector<JS::ubi::Vector<uint32_t>> predecessorVectors;
    if (!convertPredecessorSetsToVectors(root,
                                         postOrder,
                                         predecessorSets,
