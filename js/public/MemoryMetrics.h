      ...
      macro(Objects, MallocHeap, objectsMallocHeapSlots)                      \
          macro(Objects, MallocHeap, objectsMallocHeapElementsNormal)         \
              macro(Objects, MallocHeap, objectsMallocHeapElementsAsmJS)      \
                  macro(Objects, MallocHeap, objectsMallocHeapMisc) macro(    \
                      Objects, NonHeap, objectsNonHeapElementsNormal)         \
...
#define FOR_EACH_SIZE(macro)                                           \
  ...
  FOR_EACH_SIZE(
      DECL_SIZE) int dummy;  // present just to absorb the trailing comma from FOR_EACH_SIZE(ZERO_SIZE)
...
{
#define FOR_EACH_SIZE(macro)                                                   \
  macro(_, NonHeap, ion) macro(_, NonHeap, baseline) macro(_, NonHeap, regexp) \
      macro(_, NonHeap, other) macro(_, NonHeap, unused)
...
#define FOR_EACH_SIZE(macro)                                       \
  macro(_, MallocHeap, marker) macro(_, NonHeap, nurseryCommitted) \
...
{
#define FOR_EACH_SIZE(macro)                                                \
  macro(_, MallocHeap, object) macro(_, MallocHeap, atomsTable)             \
      macro(_, MallocHeap, atomsMarkBitmaps) macro(_, MallocHeap, contexts) \
...
{
#define FOR_EACH_SIZE(macro)                                                   \
  macro(Other, GCHeapUnused, object) macro(Other, GCHeapUnused, script)        \
      macro(Other, GCHeapUnused, lazyScript) macro(Other, GCHeapUnused, shape) \
...
#define FOR_EACH_SIZE(macro)                                                 \
  macro(Other, GCHeapUsed, symbolsGCHeap)                                    \
      macro(Other, GCHeapAdmin, gcHeapArenaAdmin) macro(                     \
          Other, GCHeapUsed, lazyScriptsGCHeap)                              \
          macro(Other, MallocHeap, lazyScriptsMallocHeap) macro(             \
              Other, GCHeapUsed, jitCodesGCHeap)                             \
              macro(Other, GCHeapUsed, objectGroupsGCHeap) macro(            \
                  Other, MallocHeap, objectGroupsMallocHeap)                 \
                  macro(Other, GCHeapUsed, scopesGCHeap) macro(              \
                      Other, MallocHeap, scopesMallocHeap)                   \
                      macro(Other, GCHeapUsed, regExpSharedsGCHeap) macro(   \
                          Other, MallocHeap, regExpSharedsMallocHeap)        \
                          macro(Other, MallocHeap, typePool) macro(          \
                              Other, MallocHeap, regexpZone)                 \
                              macro(Other, MallocHeap, jitZone) macro(       \
                                  Other, MallocHeap, baselineStubsOptimized) \
                                  macro(Other, MallocHeap, cachedCFG) macro( \
                                      Other, MallocHeap, uniqueIdMap)        \
...
// zero.
#define FOR_EACH_SIZE(macro)                                                   \
  macro(Private, MallocHeap, objectsPrivate) macro(                            \
      Other, GCHeapUsed, scriptsGCHeap) macro(Other,                           \
                                              MallocHeap,                      \
                                              scriptsMallocHeapData)           \
      macro(Other, MallocHeap, baselineData) macro(                            \
          Other, MallocHeap, baselineStubsFallback) macro(Other,               \
                                                          MallocHeap,          \
                                                          ionData)             \
          macro(Other, MallocHeap, typeInferenceTypeScripts) macro(            \
              Other, MallocHeap, typeInferenceAllocationSiteTables)            \
              macro(Other, MallocHeap, typeInferenceArrayTypeTables) macro(    \
                  Other, MallocHeap, typeInferenceObjectTypeTables)            \
                  macro(Other, MallocHeap, compartmentObject) macro(           \
                      Other, MallocHeap, compartmentTables)                    \
                      macro(Other, MallocHeap, innerViewsTable) macro(         \
                          Other, MallocHeap, lazyArrayBuffersTable)            \
                          macro(Other, MallocHeap, objectMetadataTable) macro( \
                              Other,                                           \
                              MallocHeap,                                      \
...
extern JS_PUBLIC_API(bool) AddSizeOfTab(JSContext* cx,
                                        JS::HandleObject obj,
