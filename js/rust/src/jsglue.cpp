...
struct ProxyTraps
{
  bool (*enter)(JSContext* cx,
                ...
                JS::HandleId id,
                js::BaseProxyHandler::Action action,
                bool* bp);
  ...
  bool (*getOwnEnumerablePropertyKeys)(JSContext* cx,
                                       JS::HandleObject proxy,
                                       JS::AutoIdVector& props);
  bool (*nativeCall)(JSContext* cx,
                     JS::IsAcceptableThis test,
                     JS::NativeImpl impl,
                     JS::CallArgs args);
  bool (*hasInstance)(JSContext* cx,
                      JS::HandleObject proxy,
                      JS::MutableHandleValue v,
                      bool* bp);
  bool (*objectClassIs)(JS::HandleObject obj,
                        js::ESClass classValue,
                        JSContext* cx);
  const char* (*className)(JSContext* cx, JS::HandleObject proxy);
  JSString* (*fun_toString)(JSContext* cx,
                            JS::HandleObject proxy,
                            bool isToString);
  //bool (*regexp_toShared)(JSContext *cx, JS::HandleObject proxy, RegExpGuard *g);
  bool (*boxedValue_unbox)(JSContext* cx,
                           JS::HandleObject proxy,
                           JS::MutableHandleValue vp);
  bool (*defaultValue)(JSContext* cx,
                       JS::HandleObject obj,
                       JSType hint,
                       JS::MutableHandleValue vp);
  ...
  {                                                                          \
    return mTraps.enumerate ? mTraps.enumerate(cx, proxy)                    \
                            : _base::enumerate(cx, proxy);                   \
                                                                             ...
                                                                             \
  virtual bool has(                                                          \
      JSContext* cx, JS::HandleObject proxy, JS::HandleId id, bool* bp)      \
      const override                                                         \
  {                                                                          \
  ...
  {                                                                          \
    return mTraps.set ? mTraps.set(cx, proxy, id, v, receiver, result)       \
                      : _base::set(cx, proxy, id, v, receiver, result);      \
                                                                             ...
                                                                             \
  virtual bool call(                                                         \
      JSContext* cx, JS::HandleObject proxy, const JS::CallArgs& args)       \
      const override                                                         \
  {                                                                          \
  ...
  {                                                                          \
    return mTraps.className ? mTraps.className(cx, proxy)                    \
                            : _base::className(cx, proxy);                   \
                                                                             ...
                                                                             \
  virtual JSString* fun_toString(                                            \
      JSContext* cx, JS::HandleObject proxy, bool isToString) const override \
  {                                                                          \
    return mTraps.fun_toString ? mTraps.fun_toString(cx, proxy, isToString)  \
                               : _base::fun_toString(cx, proxy, isToString); \
  }                                                                          \
                                                                             \
  virtual bool boxedValue_unbox(                                             \
      JSContext* cx, JS::HandleObject proxy, JS::MutableHandleValue vp)      \
      const override                                                         \
  {                                                                          \
...
class WrapperProxyHandler : public js::Wrapper
{
 ...
 public:
  WrapperProxyHandler(const ProxyTraps& aTraps) : js::Wrapper(0), mTraps(aTraps)
  {
  }
...
bool
InvokeGetOwnPropertyDescriptor(const void* handler,
                               JSContext* cx,
                               JS::HandleObject proxy,
                               JS::HandleId id,
                               JS::MutableHandle<JS::PropertyDescriptor> desc)
...
bool
InvokeHasOwn(const void* handler,
             JSContext* cx,
             JS::HandleObject proxy,
             JS::HandleId id,
             bool* bp)
{
  return static_cast<const js::BaseProxyHandler*>(handler)->hasOwn(
      cx, proxy, id, bp);
}
...
bool
CallJitGetterOp(const JSJitInfo* info,
                JSContext* cx,
...
bool
CallJitSetterOp(const JSJitInfo* info,
                JSContext* cx,
                JS::HandleObject thisObj,
                void* specializedThis,
                unsigned argc,
...
bool
CallJitMethodOp(const JSJitInfo* info,
                JSContext* cx,
                JS::HandleObject thisObj,
                void* specializedThis,
                uint32_t argc,
...
JSObject*
NewProxyObject(JSContext* aCx,
               const void* aHandler,
               JS::HandleValue aPriv,
               JSObject* proto,
               JSObject* parent,
               JSObject* call,
               JSObject* construct)
{
  ...
  return js::NewProxyObject(
      aCx, (js::BaseProxyHandler*)aHandler, aPriv, proto, options);
}

JSObject*
WrapperNew(JSContext* aCx,
...
CollectServoSizes(JSContext* cx, JS::ServoSizes* sizes)
{
  ...
  return JS::AddServoSizeOf(cx,
                            MallocSizeOf,
                            /* ObjectPrivateVisitor = */ nullptr,
                            sizes);
}
...
#define JS_DEFINE_DATA_AND_LENGTH_ACCESSOR(Type, type)                    \
  void Get##Type##ArrayLengthAndData(                                     \
...
bool
WriteBytesToJSStructuredCloneData(const uint8_t* src,
                                  size_t len,
