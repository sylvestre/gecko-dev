...
// static
bool
XPCConvert::NativeData2JS(MutableHandleValue d,
                          const void* s,
...
// static
bool
XPCConvert::JSData2Native(void* d,
                          HandleValue s,
                          const nsXPTType& type,
                          const nsID* iid,
                          nsresult* pErr)
                                     ...
                                     nsresult* pErr)
{
  ...
  nsresult rv = XPCWrappedNative::GetNewOrUsed(
      aHelper, xpcscope, iface, getter_AddRefs(wrapper));
...
// static
bool
XPCConvert::JSObject2NativeInterface(void** dest,
                                     HandleObject src,
                                     const nsID* iid,
                                     nsISupports* aOuter,
                                     nsresult* pErr)
...
// static
nsresult
XPCConvert::ConstructException(nsresult rv,
                               const char* message,
  ...
  if (!s.isPrimitive()) {
    ...
    if (nsCOMPtr<nsISupports> supports =
      ...
      if (nullptr != (report = JS_ErrorFromException(cx, obj))) {
        ...
        return JSErrorToXPCException(
            toStringResult.ptr(), ifaceName, methodName, report, exceptn);
      }
  ...
  if (s.isNumber()) {
    ...
    if (isResult)
      ...
      if (NS_FAILED(NS_GetComponentManager(getter_AddRefs(cm))) || !cm ||
          NS_FAILED(
              cm->CreateInstanceByContractID(NS_SUPPORTS_DOUBLE_CONTRACTID,
                                             nullptr,
                                             NS_GET_IID(nsISupportsDouble),
                                             getter_AddRefs(data))))
        return NS_ERROR_FAILURE;
      data->SetData(number);
      rv = ConstructException(NS_ERROR_XPC_JS_THREW_NUMBER,
                              nullptr,
                              ifaceName,
                              methodName,
                              data,
...
// static
bool
XPCConvert::NativeArray2JS(MutableHandleValue d,
                           const void** s,
                           const nsXPTType& type,
                           const nsID* iid,
                           uint32_t count,
                                ...
                                nsresult* pErr)
{
  ...
  switch (JS_GetArrayBufferViewType(jsArray)) {
    case js::Scalar::Int8:
      output = CheckTargetAndPopulate(
          nsXPTType::T_I8, type, sizeof(int8_t), count, jsArray, pErr);
      if (!output) {
        return false;
      }
      break;
    ...
    case js::Scalar::Uint8Clamped:
      output = CheckTargetAndPopulate(
          nsXPTType::T_U8, type, sizeof(uint8_t), count, jsArray, pErr);
      if (!output) {
        return false;
      }
      break;
    ...
    case js::Scalar::Int16:
      output = CheckTargetAndPopulate(
          nsXPTType::T_I16, type, sizeof(int16_t), count, jsArray, pErr);
      if (!output) {
        return false;
      }
      break;
    ...
    case js::Scalar::Uint16:
      output = CheckTargetAndPopulate(
          nsXPTType::T_U16, type, sizeof(uint16_t), count, jsArray, pErr);
      if (!output) {
        return false;
      }
      break;
    ...
    case js::Scalar::Int32:
      output = CheckTargetAndPopulate(
          nsXPTType::T_I32, type, sizeof(int32_t), count, jsArray, pErr);
      if (!output) {
        return false;
      }
      break;
    ...
    case js::Scalar::Uint32:
      output = CheckTargetAndPopulate(
          nsXPTType::T_U32, type, sizeof(uint32_t), count, jsArray, pErr);
      if (!output) {
        return false;
      }
      break;
    ...
    case js::Scalar::Float32:
      output = CheckTargetAndPopulate(
          nsXPTType::T_FLOAT, type, sizeof(float), count, jsArray, pErr);
      if (!output) {
        return false;
      }
      break;
    ...
    case js::Scalar::Float64:
      output = CheckTargetAndPopulate(
          nsXPTType::T_DOUBLE, type, sizeof(double), count, jsArray, pErr);
      if (!output) {
        return false;
      }
      break;
...
// static
bool
XPCConvert::JSArray2Native(void** d,
                           HandleValue s,
                           uint32_t count,
                           const nsXPTType& type,
                           const nsID* iid,
...
// static
bool
XPCConvert::NativeStringWithSize2JS(MutableHandleValue d,
                                    const void* s,
                                    const nsXPTType& type,
                                    uint32_t count,
                                    nsresult* pErr)
...
// static
bool
XPCConvert::JSStringWithSize2Native(void* d,
                                    HandleValue s,
                                    uint32_t count,
                                    const nsXPTType& type,
                                    nsresult* pErr)
