...
class Watchdog
{
 public:
  ...
  void Init()
  {
    ...
    mLock = PR_NewLock();
    if (!mLock) NS_RUNTIMEABORT("PR_NewLock failed.");
    mWakeup = PR_NewCondVar(mLock);
    if (!mWakeup) NS_RUNTIMEABORT("PR_NewCondVar failed.");

    {
      ...
      mThread = PR_CreateThread(PR_USER_THREAD,
                                ...
                                PR_JOINABLE_THREAD,
                                0);
      if (!mThread) NS_RUNTIMEABORT("PR_CreateThread failed!");
...
WatchdogMain(void* arg)
{
  ...
  while (!self->ShuttingDown()) {
    ...
    if (!self->ShuttingDown() && manager->IsAnyContextActive()) {
      ...
      manager->ForAllActiveContexts([usecs, manager, &lock](
                                        XPCJSContext* aContext) -> bool {
...
XPCJSContext::InterruptCallback(JSContext* cx)
{
  ...
  if (response == nsGlobalWindow::KillScriptGlobal) {
    ...
    if (!wrapper ||
        !JS_DefineProperty(
            cx, wrapper, "wrappedJSObject", global, JSPROP_ENUMERATE) ||
        NS_FAILED(xpc->WrapJS(
            cx, wrapper, NS_GET_IID(nsISupports), getter_AddRefs(supports)))) {
      return false;
    }
...
{
  return sExtraWarningsForSystemJS;
}
#else
bool
xpc::ExtraWarningsForSystemJS()
...
bool
xpc::SharedMemoryEnabled()
{
  return sSharedMemoryEnabled;
}
...
ReloadPrefsCallback(const char* pref, void* data)
{
  ...
  JS_SetGlobalJitCompilerOption(
      cx, JSJITCOMPILER_ION_WARMUP_TRIGGER, useIonEager ? 0 : ionThreshold);
#ifdef DEBUG
  JS_SetGlobalJitCompilerOption(
      cx, JSJITCOMPILER_FULL_DEBUG_CHECKS, fullJitDebugChecks);
#endif
}
...
XPCJSContext::~XPCJSContext()
{
  ...
  Preferences::UnregisterPrefixCallback(
      ReloadPrefsCallback, JS_OPTIONS_DOT_STR, this);
...
XPCJSContext::Initialize(XPCJSContext* aPrimaryContext)
{
  ...
  if (aPrimaryContext) {
    ...
    rv = CycleCollectedJSContext::Initialize(
        nullptr, JS::DefaultHeapMaxBytes, JS::DefaultNurseryBytes);
  }
