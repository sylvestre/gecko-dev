...
static void
DEBUG_CheckClassInfoClaims(XPCWrappedNative* wrapper);
#else
                                ...
                                XPCWrappedNative** wrappedGlobal)
{
  ...
  GatherScriptable(identity,
                   nativeHelper.GetClassInfo(),
                   getter_AddRefs(scrProto),
  ...
  // cache.
  nsresult rv = FinishCreate(
      scope, iface, nativeHelper.GetWrapperCache(), wrapper, wrappedGlobal);
                               ...
                               XPCWrappedNative** resultWrapper)
{
  ...
  if (!isClassInfoSingleton)
    GatherScriptable(
        identity, info, getter_AddRefs(scrProto), getter_AddRefs(scrWrapper));
                              ...
                              XPCWrappedNative** resultWrapper)
{
  AutoJSContext cx;
  MOZ_ASSERT(Object,
             "XPCWrappedNative::GetUsedOnly was called with a null Object");
  MOZ_ASSERT(Interface);
...
XPCWrappedNative::Init(nsIXPCScriptable* aScriptable)
{
  ...
  MOZ_ASSERT_IF(mScriptable,
                !!mScriptable->IsGlobalObject() ==
  ...
  RootedObject protoJSObject(cx,
                             HasProto() ? GetProto()->GetJSProtoObject()
...
XPCWrappedNative::InitTearOffJSObject(XPCWrappedNativeTearOff* to)
{
  ...
  js::SetReservedSlot(
      obj, XPC_WN_TEAROFF_FLAT_OBJECT_SLOT, JS::ObjectValue(*mFlatJSObject));
  return true;
}
...
static bool
Throw(nsresult errNum, XPCCallContext& ccx)
{
                                        ...
                                        uint32_t* result)
{
  ...
  rv = mIFaceInfo->GetSizeIsArgNumberForParam(
      mVTableIndex, &paramInfo, 0, &paramIndex);
  if (NS_FAILED(rv))
  ...
  if (paramIndex >= mArgc && maybeArray.isObject()) {
    ...
    if (!JS_IsArrayObject(mCallContext, maybeArray, &isArray) || !isArray ||
        !JS_GetArrayLength(mCallContext,
                           arrayOrNull,
                                            ...
                                            nsID* result) const
{
  ...
  if (tag == nsXPTType::T_INTERFACE) {
    ...
    if (NS_FAILED(rv))
      return ThrowBadParam(
          NS_ERROR_XPC_CANT_GET_PARAM_IFACE_INFO, paramIndex, mCallContext);
  } else if (tag == nsXPTType::T_INTERFACE_IS) {
    rv = mIFaceInfo->GetInterfaceIsArgNumberForParam(
        mVTableIndex, &paramInfo, &paramIndex);
    if (NS_FAILED(rv))
      return Throw(NS_ERROR_XPC_CANT_GET_ARRAY_INFO, mCallContext);
...
bool
CallMethodHelper::GetOutParamSource(uint8_t paramIndex,
                                    MutableHandleValue srcp) const
{
  const nsXPTParamInfo& paramInfo = mMethodInfo->GetParam(paramIndex);
...
CallMethodHelper::GatherAndConvertResults()
{
  ...
  for (uint8_t i = 0; i < paramCount; i++) {
    ...
    if (isArray) {
      if (NS_FAILED(mIFaceInfo->GetTypeForParam(
              mVTableIndex, &paramInfo, 1, &datum_type))) {
    ...
    nsresult err;
    if (isArray) {
      if (!XPCConvert::NativeArray2JS(&v,
                                      (const void**)&dp->val,
                                      datum_type,
                                      &param_iid,
                                      array_count,
      ...
      if (!XPCConvert::NativeStringWithSize2JS(
              &v, (const void*)&dp->val, datum_type, array_count, &err)) {
        ThrowBadParam(err, i, mCallContext);
        return false;
      }
    } else {
      if (!XPCConvert::NativeData2JS(
              &v, &dp->val, datum_type, &param_iid, &err)) {
        ThrowBadParam(err, i, mCallContext);
        return false;
      }
    }
...
CallMethodHelper::ConvertIndependentParam(uint8_t i)
{
  ...
  if (type_tag == nsXPTType::T_JSVAL) {
    ...
    if (!js::AddRawValueRoot(
            mCallContext, &dp->val.j, "XPCWrappedNative::CallMethod param"))
      return false;
  }
  ...
  if (type_tag == nsXPTType::T_INTERFACE &&
      NS_FAILED(mIFaceInfo->GetIIDForParamNoAlloc(
          mVTableIndex, &paramInfo, &param_iid))) {
...
CallMethodHelper::ConvertDependentParam(uint8_t i)
{
  ...
  if (isArray || isSizedString) {
    ...
    if (isArray) {
      if (array_count && !XPCConvert::JSArray2Native((void**)&dp->val,
                                                     src,
                                                     array_count,
                                                     datum_type,
                                                     &param_iid,
    ...
    } else  // if (isSizedString)
    {
      if (!XPCConvert::JSStringWithSize2Native(
              (void*)&dp->val, src, array_count, datum_type, &err)) {
        ThrowBadParam(err, i, mCallContext);
        return false;
      }
    }
...
NS_IMETHODIMP
XPCWrappedNative::GetNative(nsISupports** aNative)
{
...
NS_IMETHODIMP
XPCWrappedNative::GetJSObjectPrototype(JSObject** aJSObjectPrototype)
{
  *aJSObjectPrototype =
      HasProto() ? GetProto()->GetJSProtoObject() : GetFlatJSObject();
  return NS_OK;
}
...
NS_IMETHODIMP
XPCWrappedNative::FindInterfaceWithMember(HandleId name,
...
NS_IMETHODIMP
XPCWrappedNative::FindInterfaceWithName(HandleId name,
                                        nsIInterfaceInfo** _retval)
{
...
NS_IMETHODIMP
XPCWrappedNative::DebugDump(int16_t depth)
{
#ifdef DEBUG
  depth--;
  XPC_LOG_ALWAYS(
