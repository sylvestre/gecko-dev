 ...
 public:
                              ...
                              uint32_t aData)
  {
    ...
    if (aTag == SCTAG_FUNCTION) {
      ...
      if (!xpc::NewFunctionForwarder(
              aCx, JSID_VOIDHANDLE, obj, forwarderOptions, &functionValue)) {
        return nullptr;
      }
                          ...
                          JS::Handle<JSObject*> aObj)
  {
    ...
    if (JS::IsCallable(aObj)) {
      if (mOptions->cloneFunctions) {
        ...
        return JS_WriteUint32Pair(
            aWriter, SCTAG_FUNCTION, mFunctions.length() - 1);
      } else {
        JS_ReportErrorASCII(
...
static bool
CheckSameOriginArg(JSContext* cx,
                     ...
                     MutableHandleValue vp)
{
  ...
  JSFunction* fun = js::NewFunctionByIdWithReserved(
      cx, FunctionForwarder, 0, JSFUN_CONSTRUCTOR, id);
  if (!fun) return false;
  ...
  RootedObject obj(cx);
  {
    ...
    if (define) {
      if (!JS_DefinePropertyById(
              cx, scope, options.defineAs, obj, JSPROP_ENUMERATE))
        return false;
    }
  }
