...
GetLocationProperty(JSContext* cx, unsigned argc, Value* vp)
{
  ...
  if (JS::DescribeScriptedCaller(cx, &filename) && filename.get()) {
    ...
    if (location) {
      ...
      rv = nsXPConnect::XPConnect()->WrapNative(cx,
                                                &args.thisv().toObject(),
                                                location,
                                                NS_GET_IID(nsIFile),
                                                locationObj.address());
...
XPCShellInterruptCallback(JSContext* cx)
{
  ...
  if (!JS_CallFunctionValue(
          cx, nullptr, callback, JS::HandleValueArray::empty(), &rv) ||
      !rv.isBoolean()) {
...
RegisterAppManifest(JSContext* cx, unsigned argc, Value* vp)
{
  ...
  nsresult rv = nsXPConnect::XPConnect()->WrapJS(
      cx, arg1, NS_GET_IID(nsIFile), getter_AddRefs(file));
  if (NS_FAILED(rv)) {
  ...
  if (forceTTY || !filename || strcmp(filename, "-") == 0) {
    ...
    if (!file) {
      /*
             ...
             */
      JS_ReportErrorNumberLatin1(jsapi.cx(),
                                 my_GetErrorMessage,
                                 nullptr,
                                 JSSMSG_CANT_OPEN,
                                 filename,
...
usage()
{
  ...
  fprintf(gErrFile,
          ...
          "[-WwxiCSsmIp] [-v version] [-f scriptfile] [-e script] [scriptfile] "
          "[scriptarg...]\n");
  return 2;
}
                 ...
                 const XREShellData* aShellData)
{
  ...
  {  // Start scoping nsCOMPtrs
    ...
    if (NS_FAILED(rv)) {
      fprintf(gErrFile,
              "+++ Failed to create BackstagePass: %8x\n",
...

    {
      ...
      JS_DefineProperty(
          cx, glob, "__LOCATION__", GetLocationProperty, nullptr, 0);
  ...
  if (CrashReporter::GetEnabled()) CrashReporter::UnsetExceptionHandler();
#endif
