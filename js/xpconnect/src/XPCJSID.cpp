...
static nsresult
GetSharedScriptableHelperForJSIID(nsIXPCScriptable** helper)
{
  EnsureClassObjectsInitialized();
...
NS_IMETHODIMP
nsJSIID::Equals(nsIJSID* other, bool* _retval)
{
  if (!_retval) return NS_ERROR_NULL_POINTER;
...
NS_IMETHODIMP
nsJSIID::Initialize(const char* idString)
{
  return NS_ERROR_FAILURE;
}
                 ...
                 bool* _retval)
{
  ...
  if (member && member->IsConstant()) {
    ...
    *resolvedp = true;
    *_retval = JS_DefinePropertyById(cx,
                                     obj,
                                     id,
                                     val,
 ...
 */
static nsresult
FindObjectForHasInstance(JSContext* cx,
                         HandleObject objArg,
...
NS_IMETHODIMP
nsJSCID::GetValid(bool* aValid)
{
  return mDetails->GetValid(aValid);
}

NS_IMETHODIMP
nsJSCID::Equals(nsIJSID* other, bool* _retval)
{
...
NS_IMETHODIMP
nsJSCID::CreateInstance(HandleValue iidval,
                        JSContext* cx,
                        ...
                        MutableHandleValue retval)
{
  ...
  rv = compMgr->CreateInstance(
      mDetails->ID(), nullptr, *iid, getter_AddRefs(inst));
...
NS_IMETHODIMP
nsJSCID::GetService(HandleValue iidval,
                    JSContext* cx,
                    uint8_t optionalArgc,
                    MutableHandleValue retval)
{
