...
XPCWrappedNativeScope::InterpositionMap*
    XPCWrappedNativeScope::gInterpositionMap = nullptr;
  ...
  if (gInterpositionMap) {
    ...
    if (!mInterposition && addonId && isSystem) {
      ...
      if (interpositionEnabled) {
        mInterposition =
            do_GetService("@mozilla.org/addons/default-addon-shims;1");
...
XPCWrappedNativeScope::GetComponentsJSObject(JS::MutableHandleObject obj)
{
  ...
  bool ok = XPCConvert::NativeInterface2JSObject(
      &val, helper, nullptr, false, nullptr);
...
JSObject*
xpc::GetAddonScope(JSContext* cx,
                   JS::HandleObject contentScope,
                   JSAddonId* addonId)
{
  MOZ_RELEASE_ASSERT(!IsInAddonScope(contentScope));
...
bool
XPCWrappedNativeScope::SetExpandoChain(JSContext* cx,
                                       HandleObject target,
                                       HandleObject chain)
{
  MOZ_ASSERT(ObjectScope(target) == this);
...
XPCWrappedNativeScope::GetInterpositionWhitelist(
    nsIAddonInterposition* interposition)
{
  if (!gInterpositionWhitelists) return nullptr;

  InterpositionWhitelistArray& wls = *gInterpositionWhitelists;
  for (size_t i = 0; i < wls.Length(); i++) {
    if (wls[i].interposition == interposition) return &wls[i].whitelist;
  }
...
/* static */ bool
XPCWrappedNativeScope::UpdateInterpositionWhitelist(
    JSContext* cx, nsIAddonInterposition* interposition)
{
  ...
  if (!gInterpositionWhitelists)
    gInterpositionWhitelists =
        new InterpositionWhitelistArray(MAX_INTERPOSITION);

  MOZ_RELEASE_ASSERT(MAX_INTERPOSITION >
                     gInterpositionWhitelists->Length() + 1);
  InterpositionWhitelistPair* newPair =
      gInterpositionWhitelists->AppendElement();
  ...
  if (NS_FAILED(rv)) {
    JS_ReportErrorASCII(cx,
                        "Could not get the whitelist from the interposition.");
    return false;
  }
...

  {
    ...
    bool isArray;
    if (!JS_IsArrayObject(cx, whitelistObj, &isArray)) return false;
    ...
    uint32_t length;
    if (!JS_GetArrayLength(cx, whitelistObj, &length)) return false;

    for (uint32_t i = 0; i < length; i++) {
      RootedValue idval(cx);
      if (!JS_GetElement(cx, whitelistObj, i, &idval)) return false;
...
void
XPCWrappedNativeScope::AddSizeOfAllScopesIncludingThis(
