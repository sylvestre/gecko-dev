    ...
    CompartmentPrivate::LocationHint aLocationHint, nsIURI** aURI)
{
  ...
  if (TryParseLocationURICandidate(
          Substring(location, 0, idx), aLocationHint, aURI))
    return true;
  ...
  for (;;) {
    ...
    if (TryParseLocationURICandidate(
            Substring(chain, idx + arrowLength), aLocationHint, aURI))
      return true;
...
EnableUniversalXPConnect(JSContext* cx)
{
  ...
  // compartment.
  bool ok = js::RecomputeWrappers(
      cx, js::SingleCompartment(compartment), js::AllCompartments());
...
// ways.
void
NukeAllWrappersForCompartment(JSContext* cx,
...
static size_t
CompartmentSizeOfIncludingThisCallback(MallocSizeOf mallocSizeOf,
                                       JSCompartment* compartment)
{
  CompartmentPrivate* priv = CompartmentPrivate::Get(compartment);
 ...
 */
bool
XPCJSRuntime::UsefulToMergeZones() const
{
...
void
XPCJSRuntime::TraceNativeBlackRoots(JSTracer* trc)
{
...
void
XPCJSRuntime::TraceAdditionalNativeGrayRoots(JSTracer* trc)
{
...
void
XPCJSRuntime::TraverseAdditionalNativeRoots(
...
/* static */ void
XPCJSRuntime::WeakPointerCompartmentCallback(JSContext* cx,
                                             JSCompartment* comp,
                                             void* data)
{
...
// be anonymized.
static void
GetCompartmentName(JSCompartment* c,
                   nsCString& name,
                   int* anonymizeID,
                   bool replaceSlashes)
{
...
#define REPORT(_path, _kind, _units, _amount, _desc) \
  handleReport->Callback(EmptyCString(),             \
                         _path,                      \
...
#define STRING_LENGTH "string(length="
    ...
    nsCString path =
        pathPrefix + nsPrintfCString("strings/" STRING_LENGTH
                                     "%zu, copies=%d, \"%s\"%s)/",
                                     info.length,
                                     info.numCopies,
                                     escapedString.get(),
    ...
    if (info.mallocHeapTwoByte > 0) {
      REPORT_BYTES(
          path + NS_LITERAL_CSTRING("malloc-heap/two-byte"),
          KIND_HEAP,
  ...
  if (shapeInfo.shapesMallocHeapTreeKids > 0) {
    REPORT_BYTES(
        ...
        KIND_HEAP,
        shapeInfo.shapesMallocHeapTreeKids,
  ...
  if (sundriesGCHeap > 0) {
    ...
    REPORT_GC_BYTES(
        pathPrefix + NS_LITERAL_CSTRING("sundries/gc-heap"),
        sundriesGCHeap,
  ...
  if (sundriesMallocHeap > 0) {
    ...
    REPORT_BYTES(
        pathPrefix + NS_LITERAL_CSTRING("sundries/malloc-heap"),
        KIND_HEAP,
        sundriesMallocHeap,
...
static void
ReportClassStats(const ClassInfo& classInfo,
                 ...
                 size_t& gcTotal)
{
  ...
  if (classInfo.objectsMallocHeapSlots > 0) {
    REPORT_BYTES(path + NS_LITERAL_CSTRING("objects/malloc-heap/slots"),
                 KIND_HEAP,
                 classInfo.objectsMallocHeapSlots,
  ...
  if (classInfo.objectsMallocHeapMisc > 0) {
    REPORT_BYTES(path + NS_LITERAL_CSTRING("objects/malloc-heap/misc"),
                 KIND_HEAP,
                 classInfo.objectsMallocHeapMisc,
  ...
  if (classInfo.wasmGuardPages > 0) {
    REPORT_BYTES(
        NS_LITERAL_CSTRING("wasm-guard-pages"),
        KIND_OTHER,
                       ...
                       size_t* gcTotalOut = nullptr)
{
  ...
  ReportClassStats(
      cStats.classInfo, nonNotablePath, handleReport, data, gcTotal);
  ...
  ZCREPORT_BYTES(
      cJSPathPrefix + NS_LITERAL_CSTRING("private-data"),
      cStats.privateData,
  ...
  if (sundriesGCHeap > 0) {
    // We deliberately don't use ZCREPORT_GC_BYTES here.
    REPORT_GC_BYTES(
        cJSPathPrefix + NS_LITERAL_CSTRING("sundries/gc-heap"),
        sundriesGCHeap,
        "The sum of all 'gc-heap' measurements that are too small to be "
        "worth showing individually.");
  }
  ...
  if (sundriesMallocHeap > 0) {
    // We deliberately don't use ZCREPORT_BYTES here.
    REPORT_BYTES(
        cJSPathPrefix + NS_LITERAL_CSTRING("sundries/malloc-heap"),
        KIND_HEAP,
        sundriesMallocHeap,
        "The sum of all 'malloc-heap' measurements that are too small to "
        "be worth showing individually.");
                                 ...
                                 size_t* rtTotalOut)
{
  ...
  for (size_t i = 0; i < rtStats.compartmentStatsVector.length(); i++) {
    ...
    ReportCompartmentStats(
        cStats, *extras, addonManager, handleReport, data, &gcTotal);
  }
  ...
  RREPORT_BYTES(
      rtPath + NS_LITERAL_CSTRING("runtime/temporary"),
      KIND_HEAP,
  ...
  RREPORT_BYTES(rtPath + NS_LITERAL_CSTRING("runtime/interpreter-stack"),
                KIND_HEAP,
                rtStats.runtime.interpreterStack,
  ...
  for (size_t i = 0; i < rtStats.runtime.notableScriptSources.length(); i++) {
    ...
    nsCString notablePath =
        ...
        nsPrintfCString("runtime/script-sources/source(scripts=%d, %s)/",
                        scriptSourceInfo.numScripts,
                        escapedFilename.get());

    ReportScriptSourceStats(
        scriptSourceInfo, notablePath, handleReport, data, rtTotal);
  }
  ...
  RREPORT_BYTES(
      rtPath + NS_LITERAL_CSTRING("runtime/code/unused"),
      KIND_NONHEAP,
                                 ...
                                 size_t* rtTotalOut)
{
  ...
  ReportJSRuntimeExplicitTreeStats(
      rtStats, rtPath, am.get(), handleReport, data, anonymize, rtTotalOut);
}
...
class JSMainRuntimeCompartmentsReporter final : public nsIMemoryReporter
{
 ...
 public:
  ...
  static void CompartmentCallback(JSContext* cx, void* vdata, JSCompartment* c)
  {
    ...
    nsCString path;
    GetCompartmentName(
        c, path, &data->anonymizeID, /* replaceSlashes = */ true);
                            ...
                            bool anonymize) override
  {
    ...
    JS_IterateCompartments(
        XPCJSContext::Get()->Context(), &d, CompartmentCallback);

    for (size_t i = 0; i < d.paths.length(); i++)
      REPORT(nsCString(d.paths[i]),
             KIND_OTHER,
             UNITS_COUNT,
             1,
                           ...
                           bool anonymize)
{
  ...
  XPCJSRuntimeStats rtStats(
      windowPaths, topWindowPaths, getLocations, anonymize);
  ...
  ReportCompartmentStats(
      rtStats.cTotals, cExtrasTotal, addonManager, handleReport, data);
  ...
  REPORT_BYTES(
      NS_LITERAL_CSTRING("js-main-runtime/runtime"),
      KIND_OTHER,
      rtTotal,
  ...
  REPORT_BYTES(
      NS_LITERAL_CSTRING("tracelogger"),
      KIND_OTHER,
  ...
  REPORT_BYTES(
      ...
      KIND_OTHER,
      rtStats.zTotals.unusedGCThings.object,
  ...
  REPORT_BYTES(
      ...
      KIND_OTHER,
      rtStats.zTotals.unusedGCThings.string,
  ...
  REPORT_BYTES(
      ...
      KIND_OTHER,
      rtStats.zTotals.unusedGCThings.symbol,
  ...
  REPORT_BYTES(NS_LITERAL_CSTRING(
               ...
               KIND_OTHER,
               rtStats.zTotals.unusedGCThings.shape,
  ...
  REPORT_BYTES(
      ...
      KIND_OTHER,
      rtStats.zTotals.unusedGCThings.baseShape,
  ...
  REPORT_BYTES(
      ...
      KIND_OTHER,
      rtStats.zTotals.unusedGCThings.script,
  ...
  REPORT_BYTES(
      ...
      KIND_OTHER,
      rtStats.zTotals.unusedGCThings.lazyScript,
  ...
  REPORT_BYTES(
      ...
      KIND_OTHER,
      rtStats.zTotals.unusedGCThings.jitcode,
  ...
  REPORT_BYTES(
      ...
      KIND_OTHER,
      rtStats.zTotals.unusedGCThings.regExpShared,
  ...
  MREPORT_BYTES(NS_LITERAL_CSTRING(
                ...
                KIND_OTHER,
                rtStats.cTotals.classInfo.objectsGCHeap,
                "Used object cells.");

  MREPORT_BYTES(NS_LITERAL_CSTRING(
                    "js-main-runtime-gc-heap-committed/used/gc-things/strings"),
                KIND_OTHER,
                rtStats.zTotals.stringInfo.sizeOfLiveGCThings(),
                "Used string cells.");

  MREPORT_BYTES(NS_LITERAL_CSTRING(
                    "js-main-runtime-gc-heap-committed/used/gc-things/symbols"),
                KIND_OTHER,
                rtStats.zTotals.symbolsGCHeap,
                "Used symbol cells.");
  ...
  MREPORT_BYTES(
      ...
      KIND_OTHER,
      rtStats.zTotals.shapeInfo.shapesGCHeapBase,
  ...
  MREPORT_BYTES(
      ...
      KIND_OTHER,
      rtStats.zTotals.regExpSharedsGCHeap,
...
static nsresult
JSSizeOfTab(JSObject* objArg,
            size_t* jsObjectsSize,
            size_t* jsStringsSize,
            size_t* jsPrivateSize,
            size_t* jsOtherSize)
{
  JSContext* cx = XPCJSContext::Get()->Context();
...
AccumulateTelemetryCallback(int id, uint32_t sample, const char* key)
{
  switch (id) {
    ...
    case JS_TELEMETRY_DEPRECATED_LANGUAGE_EXTENSIONS_IN_ADDONS:
      Telemetry::Accumulate(
          Telemetry::JS_DEPRECATED_LANGUAGE_EXTENSIONS_IN_ADDONS, sample);
      break;
...
static void
CompartmentNameCallback(JSContext* cx,
                        JSCompartment* comp,
                        char* buf,
                        size_t bufsize)
{
...
static nsresult
ReadSourceFromFilename(JSContext* cx,
                       const char* filename,
                       char16_t** src,
                       size_t* len)
{
  ...
  rv = ScriptLoader::ConvertToUTF16(
      scriptChannel, buf.get(), rawLen, EmptyString(), nullptr, *src, *len);
  NS_ENSURE_SUCCESS(rv, rv);
...
XPCJSRuntime::Initialize(JSContext* cx)
{
  ...
  JS_AddWeakPointerCompartmentCallback(
      cx, WeakPointerCompartmentCallback, this);
...
bool
XPCJSRuntime::DescribeCustomObjects(JSObject* obj,
                                    ...
                                    char (&name)[72]) const
{
  ...
  SprintfLiteral(
      name, "JS Object (%s - %s)", clasp->name, scr->GetJSClass()->name);
  return true;
}

bool
XPCJSRuntime::NoteCustomGCThingXPCOMChildren(
    const js::Class* clasp,
    JSObject* obj,
...
XPCJSRuntime::InitSingletonScopes()
{
  ...
  rv = CreateSandboxObject(
      cx, &v, nsXPConnect::SystemPrincipal(), privilegedJunkScopeOptions);
  MOZ_RELEASE_ASSERT(NS_SUCCEEDED(rv));
  ...
  rv = CreateSandboxObject(
      cx, &v, /* principal = */ nullptr, compilationScopeOptions);
  MOZ_RELEASE_ASSERT(NS_SUCCEEDED(rv));
