...
XPCTraceableVariant::~XPCTraceableVariant()
{
  ...
  if (!val.isNull()) RemoveFromRootSet();
}

void
XPCTraceableVariant::TraceJS(JSTracer* trc)
{
...
XPCVariant::newVariant(JSContext* cx, const Value& aJSVal)
{
  ...
  if (!variant->InitializeData(cx)) return nullptr;
...
class XPCArrayHomogenizer
{
 ...
 public:
  static bool GetTypeForArray(JSContext* cx,
                              HandleObject array,
                              uint32_t length,
...
// static
bool
XPCArrayHomogenizer::GetTypeForArray(JSContext* cx,
                                     HandleObject array,
                                     uint32_t length,
                                     nsXPTType* resultType,
                                     nsID* resultID)
{
  ...
  for (uint32_t i = 0; i < length; i++) {
    if (!JS_GetElement(cx, array, i, &val)) return false;
    ...
    if (val.isInt32()) {
      ...
      bool isArray;
      if (!JS_IsArrayObject(cx, jsobj, &isArray)) return false;
...
bool
XPCVariant::InitializeData(JSContext* cx)
{
  if (!js::CheckRecursionLimit(cx)) return false;
  ...
  if (isArray) {
    ...
    if (!XPCConvert::JSArray2Native(
            &mData.u.array.mArrayValue, val, len, type, &id, nullptr))
      return false;

    mData.mType = nsIDataType::VTYPE_ARRAY;
    if (type.IsInterfacePointer()) mData.u.array.mArrayInterfaceID = id;
                            ...
                            MutableHandleValue pJSVal)
{
  ...
  uint16_t type;
  if (NS_FAILED(variant->GetDataType(&type))) return false;
  ...
  if (NS_SUCCEEDED(rv) &&
    ...
    if (!JS_WrapValue(cx, &realVal)) return false;
    pJSVal.set(realVal);
    return true;
  }
  ...
  switch (type) {
    ...
    case nsIDataType::VTYPE_DOUBLE: {
      double d;
      if (NS_FAILED(variant->GetAsDouble(&d))) return false;
      pJSVal.setNumber(d);
      return true;
    }
    case nsIDataType::VTYPE_BOOL: {
      bool b;
      if (NS_FAILED(variant->GetAsBool(&b))) return false;
      pJSVal.setBoolean(b);
      return true;
    }
    case nsIDataType::VTYPE_CHAR: {
      char c;
      if (NS_FAILED(variant->GetAsChar(&c))) return false;
      return XPCConvert::NativeData2JS(
          pJSVal, (const void*)&c, TD_CHAR, &iid, pErr);
    }
    case nsIDataType::VTYPE_WCHAR: {
      char16_t wc;
      if (NS_FAILED(variant->GetAsWChar(&wc))) return false;
      return XPCConvert::NativeData2JS(
          pJSVal, (const void*)&wc, TD_WCHAR, &iid, pErr);
    }
    case nsIDataType::VTYPE_ID: {
      if (NS_FAILED(variant->GetAsID(&iid))) return false;
      nsID* v = &iid;
      return XPCConvert::NativeData2JS(
          pJSVal, (const void*)&v, TD_PNSIID, &iid, pErr);
    }
    case nsIDataType::VTYPE_ASTRING: {
      nsAutoString astring;
      if (NS_FAILED(variant->GetAsAString(astring))) return false;
      nsAutoString* v = &astring;
      return XPCConvert::NativeData2JS(
          pJSVal, (const void*)&v, TD_ASTRING, &iid, pErr);
    }
    case nsIDataType::VTYPE_DOMSTRING: {
      ...
      nsAutoString* v = &astring;
      return XPCConvert::NativeData2JS(
          pJSVal, (const void*)&v, TD_DOMSTRING, &iid, pErr);
    }
    case nsIDataType::VTYPE_CSTRING: {
      nsAutoCString cString;
      if (NS_FAILED(variant->GetAsACString(cString))) return false;
      nsAutoCString* v = &cString;
      return XPCConvert::NativeData2JS(
          pJSVal, (const void*)&v, TD_CSTRING, &iid, pErr);
    }
    case nsIDataType::VTYPE_UTF8STRING: {
      nsUTF8String utf8String;
      if (NS_FAILED(variant->GetAsAUTF8String(utf8String))) return false;
      nsUTF8String* v = &utf8String;
      return XPCConvert::NativeData2JS(
          pJSVal, (const void*)&v, TD_UTF8STRING, &iid, pErr);
    }
    case nsIDataType::VTYPE_CHAR_STR: {
      char* pc;
      if (NS_FAILED(variant->GetAsString(&pc))) return false;
      bool success = XPCConvert::NativeData2JS(
          pJSVal, (const void*)&pc, TD_PSTRING, &iid, pErr);
      free(pc);
      return success;
    }
    case nsIDataType::VTYPE_STRING_SIZE_IS: {
      char* pc;
      uint32_t size;
      if (NS_FAILED(variant->GetAsStringWithSize(&size, &pc))) return false;
      bool success = XPCConvert::NativeStringWithSize2JS(
          pJSVal, (const void*)&pc, TD_PSTRING_SIZE_IS, size, pErr);
      free(pc);
      return success;
    }
    case nsIDataType::VTYPE_WCHAR_STR: {
      char16_t* pwc;
      if (NS_FAILED(variant->GetAsWString(&pwc))) return false;
      bool success = XPCConvert::NativeData2JS(
          pJSVal, (const void*)&pwc, TD_PSTRING, &iid, pErr);
      free(pwc);
      return success;
    }
    case nsIDataType::VTYPE_WSTRING_SIZE_IS: {
      char16_t* pwc;
      uint32_t size;
      if (NS_FAILED(variant->GetAsWStringWithSize(&size, &pwc))) return false;
      bool success = XPCConvert::NativeStringWithSize2JS(
          pJSVal, (const void*)&pwc, TD_PWSTRING_SIZE_IS, size, pErr);
      free(pwc);
      return success;
    }
    ...
    case nsIDataType::VTYPE_INTERFACE_IS: {
      nsISupports* pi;
      nsID* piid;
      if (NS_FAILED(variant->GetAsInterface(&piid, (void**)&pi))) return false;

      iid = *piid;
      free((char*)piid);

      bool success = XPCConvert::NativeData2JS(
          pJSVal, (const void*)&pi, TD_INTERFACE_IS_TYPE, &iid, pErr);
      if (pi) pi->Release();
      return success;
    }
...
NS_IMETHODIMP
XPCVariant::GetDataType(uint16_t* aDataType)
{
...
NS_IMETHODIMP
XPCVariant::GetAsWChar(char16_t* _retval)
{
...
NS_IMETHODIMP
XPCVariant::GetAsAString(nsAString& _retval)
{
  return mData.ConvertToAString(_retval);
}

NS_IMETHODIMP
XPCVariant::GetAsDOMString(nsAString& _retval)
{
...
NS_IMETHODIMP
XPCVariant::GetAsACString(nsACString& _retval)
{
  return mData.ConvertToACString(_retval);
}

NS_IMETHODIMP
XPCVariant::GetAsAUTF8String(nsAUTF8String& _retval)
{
...
NS_IMETHODIMP
XPCVariant::GetAsWString(char16_t** _retval)
{
  return mData.ConvertToWString(_retval);
}

NS_IMETHODIMP
XPCVariant::GetAsISupports(nsISupports** _retval)
{
  return mData.ConvertToISupports(_retval);
}

NS_IMETHODIMP
XPCVariant::GetAsInterface(nsIID** iid, void** iface)
{
  return mData.ConvertToInterface(iid, iface);
}

NS_IMETHODIMP_(nsresult)
XPCVariant::GetAsArray(uint16_t* type, nsIID* iid, uint32_t* count, void** ptr)
{
  return mData.ConvertToArray(type, iid, count, ptr);
}

NS_IMETHODIMP
XPCVariant::GetAsStringWithSize(uint32_t* size, char** str)
{
  return mData.ConvertToStringWithSize(size, str);
}

NS_IMETHODIMP
XPCVariant::GetAsWStringWithSize(uint32_t* size, char16_t** str)
{
