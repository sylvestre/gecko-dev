...
bool
AutoScriptEvaluate::StartEvaluating(HandleObject scope)
{
...
bool
xpc_IsReportableErrorCode(nsresult code)
{
...
nsXPCWrappedJSClass::GetNewOrUsed(JSContext* cx,
                                  REFNSIID aIID,
                                  bool allowNonScriptable)
{
...
nsXPCWrappedJSClass::nsXPCWrappedJSClass(JSContext* cx,
                                         REFNSIID aIID,
    ...
    if (!success && JS_IsExceptionPending(cx)) {
      ...
      if (JS_GetPendingException(cx, &jsexception)) {
        ...
        if (jsexception.isObject()) {
          ...
          if (e && NS_SUCCEEDED(e->GetResult(&rv)) && rv == NS_NOINTERFACE) {
                             ...
                             nsresult* pErr)
{
  ...
  return JS_GetPropertyById(ccx, aJSObj, aName, &val) &&
         XPCConvert::JSData2Native(
             aResult, val, type, &NS_GET_IID(nsIVariant), pErr);
}
                                             ...
                                             nsISimpleEnumerator** aEnumerate)
{
  ...
  for (size_t i = 0; i < idArray.length(); i++) {
    ...
    nsresult rv;
    if (!GetNamedPropertyAsVariantRaw(
            ccx, aJSObj, idName, getter_AddRefs(value), &rv)) {
      if (NS_FAILED(rv)) return rv;
      return NS_ERROR_FAILURE;
    }
...
xpcProperty::xpcProperty(const char16_t* aName,
                         uint32_t aNameLen,
                         nsIVariant* aValue)
...
NS_IMETHODIMP
xpcProperty::GetName(nsAString& aName)
{
  aName.Assign(mName);
  return NS_OK;
}
...
NS_IMETHODIMP
xpcProperty::GetValue(nsIVariant** aValue)
{
...
class WrappedJSIdentity
{
 ...
 public:
  ...
  static void* GetSingleton()
  {
    ...
    if (!singleton) singleton = new WrappedJSIdentity();
...
GetFunctionName(JSContext* cx, HandleObject obj)
{
  ...
  if (funName) {
    ...
    if (!XPCConvert::JSData2Native(&displayNamePtr,
                                   funNameVal,
...
nsXPCWrappedJSClass::IsWrappedJS(nsISupports* aPtr)
{
  ...
  return aPtr &&
         NS_OK ==
             aPtr->QueryInterface(NS_GET_IID(WrappedJSIdentity), &result) &&
...
/* static */ void
nsXPCWrappedJSClass::CleanupPointerTypeObject(const nsXPTType& type, void** pp)
{
...
void
nsXPCWrappedJSClass::CleanupOutparams(JSContext* cx,
                                      uint16_t methodIndex,
                                      const nsXPTMethodInfo* info,
                                      nsXPTCMiniVariant* nativeParams,
                                      bool inOutOnly,
    ...
    if (!inOutOnly || param.IsIn()) {
      if (type.IsArray()) {
        ...
        if (pp) {
          ...
          if (NS_SUCCEEDED(mInfo->GetTypeForParam(
                  methodIndex, &param, 1, &datum_type)) &&
              datum_type.deprecated_IsPointer() &&
              GetArraySizeFromParam(cx,
                                    info,
                                    param,
                                    methodIndex,
                                    i,
                                       ...
                                       nsIException* aSyntheticException)
{
  ...
  if (is_js_exception) {
    if (!xpc_exception)
      XPCConvert::JSValToXPCException(&js_exception,
                                      anInterfaceName,
                                      aPropertyName,
    ...
    if (NS_SUCCEEDED(xpc_exception->GetResult(&e_result))) {
      ...
      if (reportable) {
        ...
        if (nullptr != consoleService) {
          ...
          if (NS_SUCCEEDED(rv)) scriptError = do_QueryInterface(errorData);
          ...
          if (nullptr == scriptError) {
            ...
            if (nullptr != scriptError) {
              ...
              if (NS_SUCCEEDED(rv)) {
                ...
                xpc_exception->GetLocation(getter_AddRefs(location));
                if (location) {
                ...
                rv = scriptError->InitWithWindowID(
                    NS_ConvertUTF8toUTF16(newMessage),
                    sourceName,
                    EmptyString(),
                    lineNumber,
                    0,
                                ...
                                nsXPTCMiniVariant* nativeParams)
{
  ...
  uint8_t argc =
      paramCount -
      (paramCount && XPT_PD_IS_RETVAL(info->params[paramCount - 1].flags) ? 1
                                                                          : 0);
  ...
  for (i = 0; i < argc; i++) {
    ...
    if (param.IsIn()) {
      ...
      if (isArray || isSizedString) {
        if (!GetArraySizeFromParam(
                cx, info, param, methodIndex, i, nativeParams, &array_count))
          goto pre_call_clean_up;
      }
      ...
      if (isArray) {
        ...
        if (!XPCConvert::NativeData2JS(
                &val, &pv->val, type, &param_iid, nullptr))
          goto pre_call_clean_up;
      }
    }
...
pre_call_clean_up:
  // clean up any 'out' params handed in
  CleanupOutparams(
      cx, methodIndex, info, nativeParams, /* inOutOnly = */ true, paramCount);
  ...
  if (XPT_MD_IS_GETTER(info->flags)) {
    ...
    if (!fval.isPrimitive()) {
      ...
      XPCConvert::ConstructException(code,
                                     sz.get(),
                                     GetInterfaceName(),
                                     name,
                                     nullptr,
                                     getter_AddRefs(syntheticException),
                                     nullptr,
                                     nullptr);
      success = false;
    }
  ...
  if (!success)
    return CheckForException(
        ccx, aes, name, GetInterfaceName(), syntheticException);
  ...
  if (foundDependentParam && i == paramCount) {
    for (i = 0; i < paramCount; i++) {
      ...
      if (isArray) {
        if (array_count && !XPCConvert::JSArray2Native((void**)&pv->val,
                                                       val,
                                                       array_count,
                                                       datum_type,
                                                       &param_iid,
        ...
        if (!XPCConvert::JSData2Native(
                &pv->val, val, type, &param_iid, nullptr))
          break;
      }
    }
  }
  ...
  if (i != paramCount) {
    ...
    CleanupOutparams(
        cx, methodIndex, info, nativeParams, /* inOutOnly = */ false, i);
  } else {
...
#ifdef DEBUG
  depth--;
  XPC_LOG_ALWAYS(("nsXPCWrappedJSClass @ %p with mRefCnt = %" PRIuPTR,
                  this,
                  mRefCnt.get()));
  XPC_LOG_INDENT();
  char* name;
