...
nsresult
xpcJSWeakReference::Init(JSContext* cx, const JS::Value& object)
{
  ...
  nsresult rv = nsXPCWrappedJS::GetNewOrUsed(
      obj, NS_GET_IID(nsISupports), getter_AddRefs(wrapped));
  if (!wrapped) {
...
xpcJSWeakReference::Get(JSContext* aCx, MutableHandleValue aRetval)
{
  ...
  if (!wrappedObj) {
    ...
    return nsContentUtils::WrapNative(
        aCx, supports, &NS_GET_IID(nsISupports), aRetval);
  }
