...
SandboxCreateRTCIdentityProvider(JSContext* cx, JS::HandleObject obj)
{
  ...
  return JS_DefineProperty(
      cx, obj, "rtcIdentityProvider", wrapped, JSPROP_ENUMERATE);
}
#endif

static bool
SetFetchRequestFromValue(JSContext* cx,
...
SandboxFetch(JSContext* cx, JS::HandleObject scope, const CallArgs& args)
{
  ...
  if (!options.Init(cx,
                    args.hasDefined(1) ? args[1] : JS::NullHandleValue,
                    "Argument 2 of fetch",
                    false)) {
    return false;
  }
...
static bool
SandboxFetchPromise(JSContext* cx, unsigned argc, Value* vp)
{
  CallArgs args = CallArgsFromVp(argc, vp);
...
static bool
writeToProto_setProperty(JSContext* cx,
                         JS::HandleObject obj,
                         JS::HandleId id,
                         JS::HandleValue v,
...
static bool
writeToProto_getProperty(JSContext* cx,
                         JS::HandleObject obj,
                         JS::HandleId id,
...
sandbox_addProperty(JSContext* cx, HandleObject obj, HandleId id, HandleValue v)
{
  ...
  if (pd.object() && !pd.configurable()) {
    ...
    if (!JS_CopyPropertyFrom(
            cx, id, unwrappedProto, obj, MakeNonConfigurableIntoConfigurable))
      return false;
  }
...
bool
xpc::SandboxCallableProxyHandler::call(JSContext* cx,
             ...
             HandleObject sandboxProtoProxy)
{
  ...
  JSObject* obj = js::NewProxyObject(
      cx, &xpc::sandboxCallableProxyHandler, priv, nullptr, options);
  if (obj) {
    js::SetProxyReservedSlot(obj,
                             SandboxCallableProxyHandler::SandboxProxySlot,
...
bool
WrapAccessorFunction(JSContext* cx,
                     Op& op,
...
bool
xpc::SandboxProxyHandler::getPropertyDescriptor(
    JSContext* cx,
    JS::Handle<JSObject*> proxy,
    JS::Handle<jsid> id,
    JS::MutableHandle<PropertyDescriptor> desc) const
{
  ...
  if (!WrapAccessorFunction(
          cx, desc.getter(), desc.address(), JSPROP_GETTER, proxy))
    return false;
  if (!WrapAccessorFunction(
          cx, desc.setter(), desc.address(), JSPROP_SETTER, proxy))
    return false;
...
bool
xpc::SandboxProxyHandler::has(JSContext* cx,
                              JS::Handle<JSObject*> proxy,
                              JS::Handle<jsid> id,
                              bool* bp) const
{
...
bool
xpc::SandboxProxyHandler::hasOwn(JSContext* cx,
                                 JS::Handle<JSObject*> proxy,
                                 JS::Handle<jsid> id,
                                 bool* bp) const
{
...
bool
xpc::SandboxProxyHandler::get(JSContext* cx,
                              JS::Handle<JSObject*> proxy,
...
bool
xpc::SandboxProxyHandler::set(JSContext* cx,
                              JS::Handle<JSObject*> proxy,
                              JS::Handle<jsid> id,
                              JS::Handle<Value> v,
...
bool
xpc::SandboxProxyHandler::getOwnEnumerablePropertyKeys(
...
JSObject*
xpc::SandboxProxyHandler::enumerate(JSContext* cx,
...
bool
xpc::GlobalProperties::DefineInXPCComponents(JSContext* cx,
                                             JS::HandleObject obj)
{
...
nsresult
xpc::CreateSandboxObject(JSContext* cx,
                         ...
                         SandboxOptions& options)
{
  ...
  RootedObject sandbox(
      ...
      xpc::CreateGlobalObject(
          cx, js::Jsvalify(clasp), principal, compartmentOptions));
...

  {
    ...
    if (options.wantExportHelpers &&
        (!JS_DefineFunction(
             cx, sandbox, "exportFunction", SandboxExportFunction, 3, 0) ||
         !JS_DefineFunction(
             cx, sandbox, "createObjectIn", SandboxCreateObjectIn, 2, 0) ||
 ...
 */
bool
ParsePrincipal(JSContext* cx,
               HandleString codebase,
 ...
 */
static bool
GetExpandedPrincipal(JSContext* cx,
                     HandleObject arrayObj,
...
nsresult
xpc::EvalInSandbox(JSContext* cx,
                   HandleObject sandboxArg,
                   ...
                   MutableHandleValue rval)
{
  ...
  bool ok = true;
  {
    ...
    options.setFileAndLine(filenameBuf.get(), lineNo).setVersion(jsVersion);
    MOZ_ASSERT(JS_IsGlobalObject(sandbox));
    ok = JS::Evaluate(
        sandcx, options, PromiseFlatString(source).get(), source.Length(), &v);
...
nsresult
xpc::GetSandboxAddonId(JSContext* cx,
                       HandleObject sandbox,
                       MutableHandleValue rval)
{
...
nsresult
xpc::GetSandboxMetadata(JSContext* cx,
                        HandleObject sandbox,
                        MutableHandleValue rval)
{
  MOZ_ASSERT(NS_IsMainThread());
...
nsresult
xpc::SetSandboxMetadata(JSContext* cx,
                        HandleObject sandbox,
