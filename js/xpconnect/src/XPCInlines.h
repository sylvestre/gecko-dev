...
XPCNativeSet::FindNamedInterface(jsid name) const
{
  ...
  for (int i = (int)mInterfaceCount; i > 0; i--, pp++) {
    ...
    if (name == iface->GetName()) return iface;
  }
  return nullptr;
}
...
inline JSObject*
XPCWrappedNativeTearOff::GetJSObjectPreserveColor() const
{
  return mJSObject.unbarrieredGetPtr();
}

inline JSObject*
XPCWrappedNativeTearOff::GetJSObject()
{
  return mJSObject;
}

inline void
XPCWrappedNativeTearOff::SetJSObject(JSObject* JSObj)
{
  MOZ_ASSERT(!IsMarked());
  mJSObject = JSObj;
}
...
inline void
XPCWrappedNativeTearOff::JSObjectMoved(JSObject* obj, const JSObject* old)
{
  MOZ_ASSERT(!IsMarked());
...
inline bool
ThrowBadParam(nsresult rv, unsigned paramNum, XPCCallContext& ccx)
{
...
inline void
ThrowBadResult(nsresult result, XPCCallContext& ccx)
{
