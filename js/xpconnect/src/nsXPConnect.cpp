...
nsXPConnect::InitStatics()
{
  ...
  if (!JS::InitSelfHostedCode(cx)) MOZ_CRASH("InitSelfHostedCode failed");
...
{
  bool found = false;
  if (info) info->HasAncestor(&NS_GET_IID(nsISupports), &found);
  return found;
}
                       ...
                       uint64_t aWindowID)
{
  ...
  if (aReport->notes) {
    if (!mNotes.SetLength(aReport->notes->length(), fallible)) return;
...
void
xpc::ErrorReport::Init(JSContext* aCx,
...
{
  if (!nsContentUtils::DOMWindowDumpEnabled()) return;
...
xpc::ErrorReport::LogToStderr()
{
  ...
  if (JSREPORT_IS_STRICT(mFlags)) error.AppendLiteral("strict ");
...
xpc::ErrorReport::LogToConsoleWithStack(JS::HandleObject aStack)
{
  ...
  MOZ_LOG(gJSDiagnostics,
          ...
          ("file %s, line %u\n%s",
           NS_LossyConvertUTF16toASCII(mFileName).get(),
           mLineNumber,
  ...
  nsresult rv = errorObject->InitWithWindowID(mErrorMsg,
                                              mFileName,
                                              mSourceLine,
                                              mLineNumber,
                                              mColumn,
  ...
  for (size_t i = 0, len = mNotes.Length(); i < len; i++) {
    ...
    noteObject->Init(
        note.mErrorMsg, note.mFileName, note.mLineNumber, note.mColumn);
...
{
  nsresult rv =
      XPTInterfaceInfoManager::GetSingleton()->GetInfoForName(name, info);
...
JSObject*
CreateGlobalObject(JSContext* cx,
                   const JSClass* clasp,
                        ...
                        nsIPrincipal* aPrincipal)
{
  ...
  if (extraWarningsForSystemJS) {
    if (isSystem) aOptions.behaviors().extraWarningsOverride().set(true);
  }
}

bool
InitGlobalObject(JSContext* aJSContext,
                                ...
                                MutableHandleObject aNewGlobal)
{
  ...
  nsresult rv =
      XPCWrappedNative::WrapNewGlobal(helper,
                                      aPrincipal,
                         ...
                         MutableHandleValue aVal)
{
  ...
  if (!XPCConvert::NativeInterface2JSObject(
          aVal, helper, aIID, aAllowWrapping, &rv))
    return rv;
                        ...
                        JSObject** aRetVal)
{
  ...
  if (!v.isObjectOrNull()) return NS_ERROR_FAILURE;
                               ...
                               MutableHandleValue aVal)
{
  ...
  return NativeInterface2JSObject(
      aScope, aCOMObj, aCache, aIID, aAllowWrapping, aVal);
}
                                      ...
                                      void** result)
{
  ...
  *result = nullptr;
  ...
  RootedObject aJSObj(aJSContext, aJSObjArg);
  nsresult rv;
  if (!XPCConvert::JSObject2NativeInterface(result, aJSObj, &aIID, aOuter, &rv))
    return rv;
  return NS_OK;
}
                                            ...
                                            nsIXPConnectWrappedNative** _retval)
{
  ...
  *_retval = nullptr;
  ...
  if (!scope) return UnexpectedFailure(NS_ERROR_FAILURE);

  RefPtr<XPCNativeInterface> iface = XPCNativeInterface::GetNewOrUsed(&aIID);
  if (!iface) return NS_ERROR_FAILURE;
...
NS_IMETHODIMP
nsXPConnect::SetFunctionThisTranslator(
    const nsIID& aIID, nsIXPCFunctionThisTranslator* aTranslator)
{
...
NS_IMETHODIMP
nsXPConnect::CreateSandbox(JSContext* cx,
                           nsIPrincipal* principal,
                           JSObject** _retval)
{
  *_retval = nullptr;
...
NS_IMETHODIMP
nsXPConnect::EvalInSandboxObject(const nsAString& source,
                                 const char* filename,
                                 JSContext* cx,
                                 JSObject* sandboxArg,
                                 MutableHandleValue rval)
{
  if (!sandboxArg) return NS_ERROR_INVALID_ARG;
  ...
  return EvalInSandbox(
      cx, sandbox, source, filenameStr, 1, JSVERSION_DEFAULT, rval);
}
                                       ...
                                       JSObject** aRetVal)
{
  ...
  if (!proto) return UnexpectedFailure(NS_ERROR_FAILURE);

  JSObject* protoObj = proto->GetJSProtoObject();
  if (!protoObj) return UnexpectedFailure(NS_ERROR_FAILURE);
...
NS_IMETHODIMP
nsXPConnect::VariantToJS(JSContext* ctx,
                         JSObject* scopeArg,
                         nsIVariant* value,
...
NS_IMETHODIMP
nsXPConnect::JSToVariant(JSContext* ctx,
                         HandleValue value,
                         nsIVariant** _retval)
{
  NS_PRECONDITION(ctx, "bad param");
...
static nsresult
WriteScriptOrFunction(nsIObjectOutputStream* stream,
                      JSContext* cx,
                      JSScript* scriptArg,
                      HandleObject functionObj)
{
  ...
  if (code != TranscodeResult_Ok) {
    if ((code & TranscodeResult_Failure) != 0) return NS_ERROR_FAILURE;
...
static nsresult
ReadScriptOrFunction(nsIObjectInputStream* stream,
                     JSContext* cx,
                     JSScript** scriptp,
                     JSObject** functionObjp)
{
...

  {
    ...
    if (scriptp) {
      ...
      if (code == TranscodeResult_Ok) *scriptp = script.get();
    } else {
...
NS_IMETHODIMP
nsXPConnect::WriteScript(nsIObjectOutputStream* stream,
                         JSContext* cx,
                         JSScript* script)
{
...
NS_IMETHODIMP
nsXPConnect::ReadScript(nsIObjectInputStream* stream,
                        JSContext* cx,
                        JSScript** scriptp)
{
...
NS_IMETHODIMP
nsXPConnect::WriteFunction(nsIObjectOutputStream* stream,
                           JSContext* cx,
...
NS_IMETHODIMP
nsXPConnect::ReadFunction(nsIObjectInputStream* stream,
                          JSContext* cx,
                          JSObject** functionObjp)
{
...
extern "C" {
JS_EXPORT_API(void) DumpJSStack() { xpc_DumpJSStack(true, true, false); }

JS_EXPORT_API(char*) PrintJSStack()
{
  nsresult rv;
  nsCOMPtr<nsIXPConnect> xpc(do_GetService(nsIXPConnect::GetCID(), &rv));
  return (NS_SUCCEEDED(rv) && xpc) ? xpc->DebugPrintJSStack(true, true, false)
                                   : nullptr;
}
...
NewAddonId(JSContext* cx, const nsACString& id)
{
  JS::RootedString str(cx,
                       JS_NewStringCopyN(cx, id.BeginReading(), id.Length()));
  if (!str) return nullptr;
  return JS::NewAddonId(cx, str);
}

bool
SetAddonInterposition(const nsACString& addonIdStr,
                      nsIAddonInterposition* interposition)
{
  ...
  return XPCWrappedNativeScope::SetAddonInterposition(
      jsapi.cx(), addonId, interposition);
}
