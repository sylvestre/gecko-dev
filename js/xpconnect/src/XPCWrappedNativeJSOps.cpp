...
static bool
Throw(nsresult errNum, JSContext* cx)
{
  ...
  if (!found) {
    if (reflectToStringAndToSource) {
      ...
      if (call) {
        ...
        return JS_DefinePropertyById(
            ccx, obj, id, value, propFlags & ~JSPROP_ENUMERATE);
      }
    }
    ...
    if (wrapperToReflectInterfaceNames) {
...

      {
        ...
        if (resolved) *resolved = true;
        return JS_DefinePropertyById(
            ccx, obj, id, jso, propFlags & ~JSPROP_ENUMERATE);
    ...
    if (wrapperToReflectDoubleWrap &&
      ...
      AutoResolveName arn(ccx, id);
      if (resolved) *resolved = true;
      return JS_DefinePropertyById(ccx,
                                   obj,
                                   id,
...
static bool
XPC_WN_OnlyIWrite_AddPropertyStub(JSContext* cx,
                                  HandleObject obj,
                                  HandleId id,
                                  HandleValue v)
{
...
bool
XPC_WN_CannotDeletePropertyStub(JSContext* cx,
                                HandleObject obj,
                                HandleId id,
...
bool
XPC_WN_MaybeResolvingDeletePropertyStub(JSContext* cx,
                                        HandleObject obj,
                                        HandleId id,
                                        ObjectOpResult& result)
{
...
bool
XPC_WN_Helper_Resolve(JSContext* cx,
                      HandleObject obj,
                      HandleId id,
                      bool* resolvedp)
{
  ...
  if (resolved) {
    *resolvedp = true;
    ...
    if (set->FindMember(id, &member, &iface, protoSet, &IsLocal) && IsLocal) {
      ...
      retval = DefinePropertyIfFound(ccx,
                                     obj,
                                     id,
                                     set,
                                     iface,
...
bool
XPC_WN_NewEnumerate(JSContext* cx,
                    HandleObject obj,
...
XPC_WN_CallMethod(JSContext* cx, unsigned argc, Value* vp)
{
  ...
  XPCCallContext ccx(
      cx, obj, funobj, JSID_VOIDHANDLE, args.length(), args.array(), vp);
  XPCWrappedNative* wrapper = ccx.GetWrapper();
  THROW_AND_RETURN_IF_BAD_WRAPPER(cx, wrapper);
                                 ...
                                 bool* resolvep)
{
  ...
  return DefinePropertyIfFound(ccx,
                               obj,
                               id,
                               self->GetSet(),
                               nullptr,
...
static bool
XPC_WN_OnlyIWrite_Proto_AddPropertyStub(JSContext* cx,
                                        HandleObject obj,
                                        HandleId id,
                                        HandleValue v)
{
