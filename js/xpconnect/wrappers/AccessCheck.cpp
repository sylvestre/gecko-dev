...
IdentifyCrossOriginObject(JSObject* obj)
{
  ...
  MOZ_ASSERT(!XrayUtils::IsXPCWNHolderClass(Jsvalify(clasp)),
...
bool
AccessCheck::isCrossOriginAccessPermitted(JSContext* cx,
                                          HandleObject wrapper,
                                          HandleId id,
                                          Wrapper::Action act)
{
  if (act == Wrapper::CALL) return false;

  if (act == Wrapper::ENUMERATE) return true;
  ...
  if (act != Wrapper::GET) return false;
...
bool
AccessCheck::checkPassToPrivilegedCode(JSContext* cx,
                                       HandleObject wrapper,
                                       HandleValue v)
{
  // Primitives are fine.
...
AccessCheck::checkPassToPrivilegedCode(JSContext* cx,
                                       ...
                                       const CallArgs& args)
{
  if (!checkPassToPrivilegedCode(cx, wrapper, args.thisv())) return false;
  for (size_t i = 0; i < args.length(); ++i) {
    if (!checkPassToPrivilegedCode(cx, wrapper, args[i])) return false;
  }
  return true;
}

void
AccessCheck::reportCrossOriginDenial(JSContext* cx,
                                     JS::HandleId id,
...
bool
OpaqueWithSilentFailing::deny(JSContext* cx,
