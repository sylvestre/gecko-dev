...
#define AssertErrorObjectKeyInBounds(key)                      \
  static_assert(Between(key, JSProto_Error, JSProto_URIError), \
                "We depend on jsprototypes.h ordering here");
...
bool
OpaqueXrayTraits::resolveOwnProperty(JSContext* cx,
                                     ...
                                     MutableHandle<PropertyDescriptor> desc)
{
  ...
  return ReportWrapperDenial(
      cx, id, WrapperDenialForXray, "object is not safely Xrayable");
}

bool
ReportWrapperDenial(JSContext* cx,
                    HandleId id,
                    WrapperDenialType type,
                    const char* reason)
{
  ...
  nsresult rv =
      errorObject->InitWithWindowID(NS_ConvertASCIItoUTF16(errorMessage.ref()),
                                    filenameStr,
                                    EmptyString(),
                                    line,
                                    column,
...
bool
JSXrayTraits::getOwnPropertyFromTargetIfSafe(
    JSContext* cx,
    HandleObject target,
    HandleObject wrapper,
    HandleId id,
    MutableHandle<PropertyDescriptor> outDesc)
{
  ...
  if (desc.hasGetterOrSetter()) {
    ...
    JS_MarkCrossZoneId(cx, id);
    return ReportWrapperDenial(
        cx, id, WrapperDenialForXray, "property has accessor");
  }
  ...
  if (desc.value().isObject()) {
    ...
    if (!AccessCheck::subsumes(target, propObj)) {
      ...
      JS_MarkCrossZoneId(cx, id);
      return ReportWrapperDenial(
          cx, id, WrapperDenialForXray, "value not same-origin with target");
    }
    ...
    if (xrayType == NotXray || xrayType == XrayForOpaqueObject) {
      ...
      JS_MarkCrossZoneId(cx, id);
      return ReportWrapperDenial(
          cx, id, WrapperDenialForXray, "value not Xrayable");
    }
    ...
    if (JS::IsCallable(propObj)) {
      ...
      JS_MarkCrossZoneId(cx, id);
      return ReportWrapperDenial(
          cx, id, WrapperDenialForXray, "value is callable");
    }
  }
...
static bool
TryResolvePropertyFromSpecs(JSContext* cx,
                            HandleId id,
                            HandleObject holder,
  ...
  if (psMatch) {
    ...
    if (psMatch->isAccessor()) {
      ...
      if (!JS_DefinePropertyById(
              cx, holder, id, v, flags & ~JSPROP_INTERNAL_USE_BIT))
        return false;
    }
...
bool
JSXrayTraits::resolveOwnProperty(JSContext* cx,
                                 HandleObject wrapper,
                                 HandleObject target,
                                 HandleObject holder,
                                 HandleId id,
      ...
      if (id == GetJSIDByIndex(cx, XPCJSContext::IDX_NAME)) {
        ...
        if (standardConstructor != JSProto_Null) {
          ...
          if (ShouldResolveStaticProperties(standardConstructor)) {
            ...
            if (!TryResolvePropertyFromSpecs(cx,
                                             id,
                                             holder,
...
bool
JSXrayTraits::delete_(JSContext* cx,
                      HandleObject wrapper,
                      HandleId id,
...
bool
JSXrayTraits::defineProperty(JSContext* cx,
                             HandleObject wrapper,
                             HandleId id,
...
bool
JSXrayTraits::enumerateNames(JSContext* cx,
                             HandleObject wrapper,
                             unsigned flags,
                             AutoIdVector& props)
{
    ...
    if (key == JSProto_Object || key == JSProto_Array) {
      MOZ_ASSERT(props.empty());
      {
        ...
        if (!js::GetPropertyKeys(
                cx, target, flags | JSITER_OWNONLY, &targetProps))
          return false;
...
bool
JSXrayTraits::construct(JSContext* cx,
                        HandleObject wrapper,
...
bool
XrayTraits::getExpandoObjectInternal(JSContext* cx,
                                     JSObject* expandoChain,
...
bool
XrayTraits::getExpandoObject(JSContext* cx,
                             HandleObject target,
                             ...
                             MutableHandleObject expandoObject)
{
  ...
  return getExpandoObjectInternal(cx,
                                  chain,
                                  isExclusive ? consumer : nullptr,
                                  ObjectPrincipal(consumer),
                                  expandoObject);
}
...
JSObject*
XrayTraits::attachExpandoObject(JSContext* cx,
                                HandleObject target,
                                HandleObject exclusiveWrapper,
                                nsIPrincipal* origin)
{
...
#ifdef DEBUG
  {
    ...
    if (chain) {
      ...
      if (getExpandoObjectInternal(
              cx, chain, exclusiveWrapper, origin, &existingExpandoObject))
  ...
  NS_ADDREF(origin);
  JS_SetReservedSlot(
      expandoObject, JSSLOT_EXPANDO_ORIGIN, JS::PrivateValue(origin));
  ...
  JS_SetReservedSlot(
      expandoObject, JSSLOT_EXPANDO_NEXT, ObjectOrNullValue(chain));
...
JSObject*
XrayTraits::ensureExpandoObject(JSContext* cx,
                                HandleObject wrapper,
                                HandleObject target)
{
...
bool
XrayTraits::cloneExpandoChain(JSContext* cx,
                              HandleObject dst,
                              HandleObject srcChain)
{
  ...
  while (oldHead) {
    ...
    RootedObject wrapperHolder(
        cx,
        JS_GetReservedSlot(oldHead, JSSLOT_EXPANDO_EXCLUSIVE_WRAPPER_HOLDER)
            .toObjectOrNull());
    if (wrapperHolder) {
    ...
    RootedObject newHead(
        cx,
        attachExpandoObject(
            cx, dst, exclusiveWrapper, GetExpandoObjectPrincipal(oldHead)));
...
SetCachedXrayExpando(JSObject* holder, JSObject* expandoWrapper)
{
  ...
  JS_SetReservedSlot(
      holder, XrayTraits::HOLDER_SLOT_EXPANDO, ObjectValue(*expandoWrapper));
}
...
bool
XPCWrappedNativeXrayTraits::resolveNativeProperty(
    JSContext* cx,
    HandleObject wrapper,
    HandleObject holder,
    HandleId id,
    MutableHandle<PropertyDescriptor> desc)
{
  ...
  if (!(iface = ccx.GetInterface()) || !(member = ccx.GetMember())) {
    ...
    FillPropertyDescriptor(
        desc, wrapper, 0, ObjectValue(*JS_GetFunctionObject(toString)));
  ...
  if (member->IsConstant()) {
    ...
    if (!member->NewFunctionObject(
            ccx, iface, wrapper, desc.value().address())) {
      JS_ReportErrorASCII(
...
bool
XrayTraits::resolveOwnProperty(JSContext* cx,
                               HandleObject wrapper,
                               HandleObject target,
                               HandleObject holder,
                               HandleId id,
                               MutableHandle<PropertyDescriptor> desc)
{
  ...
  if (id == GetJSIDByIndex(cx, XPCJSContext::IDX_WRAPPED_JSOBJECT) &&
    ...
    if (!found && !JS_DefinePropertyById(cx,
                                         holder,
                                         id,
...
bool
XPCWrappedNativeXrayTraits::enumerateNames(JSContext* cx,
                                           HandleObject wrapper,
                                           unsigned flags,
                                           AutoIdVector& props)
{
...
bool
XPCWrappedNativeXrayTraits::call(JSContext* cx,
                                 HandleObject wrapper,
                                 const JS::CallArgs& args,
                                 const js::Wrapper& baseInstance)
{
...
bool
XPCWrappedNativeXrayTraits::construct(JSContext* cx,
                                      HandleObject wrapper,
                                      const JS::CallArgs& args,
                                      const js::Wrapper& baseInstance)
{
...
bool
DOMXrayTraits::resolveOwnProperty(JSContext* cx,
                                  HandleObject wrapper,
                                  HandleObject target,
                                  HandleObject holder,
                                  HandleId id,
...
bool
DOMXrayTraits::delete_(JSContext* cx,
...
bool
DOMXrayTraits::defineProperty(JSContext* cx,
                              HandleObject wrapper,
                              HandleId id,
                              Handle<PropertyDescriptor> desc,
                              Handle<PropertyDescriptor> existingDesc,
...
bool
DOMXrayTraits::enumerateNames(JSContext* cx,
                              HandleObject wrapper,
                              unsigned flags,
                              AutoIdVector& props)
{
...
bool
DOMXrayTraits::call(JSContext* cx,
                    HandleObject wrapper,
                    const JS::CallArgs& args,
                    const js::Wrapper& baseInstance)
{
...
bool
DOMXrayTraits::construct(JSContext* cx,
                         HandleObject wrapper,
                         const JS::CallArgs& args,
                         const js::Wrapper& baseInstance)
{
...
bool
DOMXrayTraits::getPrototype(JSContext* cx,
                            JS::HandleObject wrapper,
...
bool
HasNativeProperty(JSContext* cx,
                  HandleObject wrapper,
                  HandleId id,
                  bool* hasProp)
{
...
bool
XrayWrapper<Base, Traits>::preventExtensions(JSContext* cx,
                                             HandleObject wrapper,
...
template<typename Base, typename Traits>
bool
XrayWrapper<Base, Traits>::isExtensible(JSContext* cx,
...
template<typename Base, typename Traits>
bool
XrayWrapper<Base, Traits>::getPropertyDescriptor(
    JSContext* cx,
    HandleObject wrapper,
    HandleId id,
    JS::MutableHandle<PropertyDescriptor> desc) const
{
  ...
  if (!Traits::singleton.resolveOwnProperty(
          cx, wrapper, target, holder, id, desc))
    return false;
...
template<typename Base, typename Traits>
bool
XrayWrapper<Base, Traits>::getOwnPropertyDescriptor(
    JSContext* cx,
    HandleObject wrapper,
    HandleId id,
    JS::MutableHandle<PropertyDescriptor> desc) const
...
static bool
RecreateLostWaivers(JSContext* cx,
                    const PropertyDescriptor* orig,
...
template<typename Base, typename Traits>
bool
XrayWrapper<Base, Traits>::defineProperty(JSContext* cx,
                                          HandleObject wrapper,
                                          HandleId id,
                                          Handle<PropertyDescriptor> desc,
                                          ObjectOpResult& result) const
{
  ...
  bool defined = false;
  if (!Traits::singleton.defineProperty(
          cx, wrapper, id, desc, existing_desc, result, &defined))
    return false;
...
template<typename Base, typename Traits>
bool
XrayWrapper<Base, Traits>::ownPropertyKeys(JSContext* cx,
                                           HandleObject wrapper,
...
template<typename Base, typename Traits>
bool
XrayWrapper<Base, Traits>::delete_(JSContext* cx,
                                   HandleObject wrapper,
                                   HandleId id,
                                   ObjectOpResult& result) const
{
...
template<typename Base, typename Traits>
bool
XrayWrapper<Base, Traits>::get(JSContext* cx,
                               HandleObject wrapper,
...
template<typename Base, typename Traits>
bool
XrayWrapper<Base, Traits>::set(JSContext* cx,
                               HandleObject wrapper,
                               HandleId id,
                               HandleValue v,
                               HandleValue receiver,
...
template<typename Base, typename Traits>
bool
XrayWrapper<Base, Traits>::has(JSContext* cx,
                               HandleObject wrapper,
                               HandleId id,
                               bool* bp) const
{
...
template<typename Base, typename Traits>
bool
XrayWrapper<Base, Traits>::hasOwn(JSContext* cx,
                                  HandleObject wrapper,
                                  HandleId id,
                                  bool* bp) const
{
...
template<typename Base, typename Traits>
bool
XrayWrapper<Base, Traits>::getOwnEnumerablePropertyKeys(
...
template<typename Base, typename Traits>
bool
XrayWrapper<Base, Traits>::call(JSContext* cx,
                                HandleObject wrapper,
...
template<typename Base, typename Traits>
bool
XrayWrapper<Base, Traits>::construct(JSContext* cx,
                                     HandleObject wrapper,
                                     const JS::CallArgs& args) const
{
  assertEnteredPolicy(cx, wrapper, JSID_VOID, BaseProxyHandler::CALL);
...
template<typename Base, typename Traits>
bool
XrayWrapper<Base, Traits>::getBuiltinClass(JSContext* cx,
                                           JS::HandleObject wrapper,
...
template<typename Base, typename Traits>
bool
XrayWrapper<Base, Traits>::getPrototype(JSContext* cx,
                                        JS::HandleObject wrapper,
                                        JS::MutableHandleObject protop) const
{
  ...
  if (cached.isUndefined()) {
    ...
    js::SetReservedSlot(
        holder, Traits::HOLDER_SLOT_CACHED_PROTO, ObjectOrNullValue(protop));
  } else {
...
template<typename Base, typename Traits>
bool
XrayWrapper<Base, Traits>::setPrototype(JSContext* cx,
                                        JS::HandleObject wrapper,
...
template<typename Base, typename Traits>
bool
XrayWrapper<Base, Traits>::getPrototypeIfOrdinary(
    JSContext* cx,
    JS::HandleObject wrapper,
    bool* isOrdinary,
    JS::MutableHandleObject protop) const
...
template<typename Base, typename Traits>
bool
XrayWrapper<Base, Traits>::setImmutablePrototype(JSContext* cx,
                                                 JS::HandleObject wrapper,
...
template<typename Base, typename Traits>
bool
XrayWrapper<Base, Traits>::getPropertyKeys(JSContext* cx,
                                           HandleObject wrapper,
                                           unsigned flags,
                                           AutoIdVector& props) const
{
