...
{
  RootedValue idv(cx);
  if (!JS_IdToValue(cx, id, &idv)) return;
  RootedString idstr(cx, JS::ToString(cx, idv));
  if (!idstr) return;
  JSAutoByteString bytes;
  if (!bytes.encodeUtf8(cx, idstr)) return;
  JS_ReportErrorUTF8(cx, msg, bytes.ptr());
}

bool
InterposeProperty(JSContext* cx,
                  HandleObject target,
                  const nsIID* iid,
                  HandleId id,
                  MutableHandle<PropertyDescriptor> descriptor)
{
  ...
  if (!mozilla::dom::IsDOMClass(clasp) && !IS_WN_CLASS(clasp) &&
      !IS_PROTO_CLASS(clasp) && clasp != &OuterWindowProxyClass && !isCPOW) {
    return true;
  }
  ...
  InterpositionWhitelist* wl =
      XPCWrappedNativeScope::GetInterpositionWhitelist(interp);
  // We do InterposeProperty only if the id is on the whitelist of the interpostion
  // or if the target is a CPOW.
  if ((!wl || !wl->has(JSID_BITS(id.get()))) && !isCPOW) return true;
  ...
  nsresult rv = interp->InterposeProperty(
      addonIdValue, targetValue, iid, prop, &descriptorVal);
  if (NS_FAILED(rv)) {
    xpc::Throw(cx, rv);
    return false;
  }

  if (!descriptorVal.isObject()) return true;
...

  {
    ...
    if (!JS::ObjectToCompletePropertyDescriptor(
            cx, target, descriptorVal, descriptor))
      return false;
  }
  ...
  if (!JS_WrapPropertyDescriptor(cx, descriptor)) return false;

  return true;
}

bool
InterposeCall(JSContext* cx,
              ...
              bool* done)
{
  *done = false;
  ...
  bool hasInterpostion =
      xpc::CompartmentPrivate::Get(unwrappedTarget)->addonCallInterposition;

  if (!hasInterpostion) return true;
  ...
  *done = true;
  ...
  nsresult rv = interp->InterposeCall(
      addonIdValue, targetValue, thisValue, argsVal, args.rval());
  if (NS_FAILED(rv)) {
    xpc::Throw(cx, rv);
    return false;
  }
...
bool
AddonWrapper<Base>::call(JSContext* cx,
                         JS::Handle<JSObject*> wrapper,
                         const JS::CallArgs& args) const
{
  bool done = false;
  if (!InterposeCall(cx, wrapper, args, &done)) return false;
...
template<typename Base>
bool
AddonWrapper<Base>::getPropertyDescriptor(
    JSContext* cx,
    HandleObject wrapper,
    HandleId id,
    MutableHandle<PropertyDescriptor> desc) const
{
  if (!InterposeProperty(cx, wrapper, nullptr, id, desc)) return false;
...
template<typename Base>
bool
AddonWrapper<Base>::getOwnPropertyDescriptor(
    JSContext* cx,
    HandleObject wrapper,
    HandleId id,
    MutableHandle<PropertyDescriptor> desc) const
...
template<typename Base>
bool
AddonWrapper<Base>::get(JSContext* cx,
                        JS::Handle<JSObject*> wrapper,
                        ...
                        JS::MutableHandle<JS::Value> vp) const
{
  ...
  if (!desc.object()) return Base::get(cx, wrapper, receiver, id, vp);

  if (desc.getter()) {
    return Call(
        cx, receiver, desc.getterObject(), HandleValueArray::empty(), vp);
  } else {
    vp.set(desc.value());
    return true;
  }
...
template<typename Base>
bool
AddonWrapper<Base>::set(JSContext* cx,
                        ...
                        JS::ObjectOpResult& result) const
{
  ...
  if (!desc.object()) return Base::set(cx, wrapper, id, v, receiver, result);
  ...
  if (desc.setter()) {
    ...
    if (!JS::Call(cx, receiver, fval, args, &ignored)) return false;
...
template<typename Base>
bool
AddonWrapper<Base>::defineProperty(JSContext* cx,
                                   HandleObject wrapper,
                                   HandleId id,
                                   ...
                                   ObjectOpResult& result) const
{
  ...
  if (!InterposeProperty(cx, wrapper, nullptr, id, &interpDesc)) return false;
...
template<typename Base>
bool
AddonWrapper<Base>::delete_(JSContext* cx,
                            HandleObject wrapper,
                            HandleId id,
                            ObjectOpResult& result) const
{
  ...
  if (!desc.object()) return Base::delete_(cx, wrapper, id, result);
...
template<>
const AddonWrapperCC AddonWrapperCC::singleton(0);
template<>
const AddonWrapperXrayXPCWN AddonWrapperXrayXPCWN::singleton(0);
template<>
const AddonWrapperXrayDOM AddonWrapperXrayDOM::singleton(0);
