...
{
  unsigned flags;
  (void)js::UncheckedUnwrap(
      originalObj, /* stopAtWindowProxy = */ true, &flags);
...
void
WrapperFactory::PrepareForWrapping(JSContext* cx,
                                   HandleObject scope,
                                   HandleObject objArg,
                                   ...
                                   MutableHandleObject retObj)
{
...

  {
    if (ccx.GetScriptable() && ccx.GetScriptable()->WantPreCreate()) {
      ...
      // wrapper?"
      nsresult rv = wn->GetScriptable()->PreCreate(
          wn->Native(), cx, scope, wrapScope.address());
      if (NS_FAILED(rv)) {
        retObj.set(waive ? WaiveXray(cx, obj) : obj);
        return;
      }
      ...
      if (MOZ_UNLIKELY(wrapScope != currentScope)) {
        ...
        rv = wn->GetScriptable()->PreCreate(
            wn->Native(), cx, currentScope, probe.address());
  ...
  RootedValue v(cx);
  nsresult rv =
      nsXPConnect::XPConnect()->WrapNativeToJSVal(cx,
                                                  wrapScope,
                                                  wn->Native(),
                                                  nullptr,
...
#ifdef DEBUG
static void
DEBUG_CheckUnwrapSafety(HandleObject obj,
...
SelectWrapper(bool securityWrapper,
              XrayType xrayType,
              bool waiveXrays,
              JSObject* obj)
{
...
SelectAddonWrapper(JSContext* cx, HandleObject obj, const Wrapper* wrapper)
{
  ...
  if (targetAddon == originAddon) return wrapper;
...
static bool
FixWaiverAfterTransplant(JSContext* cx,
