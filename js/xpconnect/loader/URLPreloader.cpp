...
URLPreloader::CollectReports(nsIHandleReportCallback* aHandleReport,
                             ...
                             bool aAnonymize)
{
  MOZ_COLLECT_REPORT("explicit/url-preloader/other",
                     KIND_HEAP,
                     UNITS_BYTES,
...
URLPreloader::BackgroundReadFiles()
{
...

  {
    ...
    for (auto entry : pendingURLs) {
      ...
      cursors.infallibleEmplaceBack(
          item, zip, reinterpret_cast<uint8_t*>(data), size, true);
    }
...
/* static */ Result<const nsCString, nsresult>
URLPreloader::ReadZip(nsZipArchive* zip,
...
nsresult
URLPreloader::Observe(nsISupports* subject,
                      const char* topic,
                      const char16_t* data)
{
  nsCOMPtr<nsIObserverService> obs = services::GetObserverService();
