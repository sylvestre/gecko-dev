...
static const JSFunctionSpec gGlobalFun[] = {JS_FN("dump", Dump, 1, 0),
...
static nsresult MOZ_FORMAT_PRINTF(2, 3)
    ReportOnCallerUTF8(JSCLContextHelper& helper, const char* format, ...)
{
  va_list ap;
  va_start(ap, format);
...
class MOZ_STACK_CLASS ComponentLoaderInfo
{
 public:
  ...
  nsresult EnsureURI()
  {
    ...
    return mIOService->NewURI(
        mLocation, nullptr, nullptr, getter_AddRefs(mURI));
  }
...
// symbols.
static JSObject*
ResolveModuleObjectProperty(JSContext* aCx,
...
mozJSComponentLoader::LoadModule(FileLocation& aFile)
{
  ...
  AUTO_PROFILER_LABEL_DYNAMIC(
      "mozJSComponentLoader::LoadModule", OTHER, spec.get());
  ...
  if (!NSGetFactoryHolder ||
      !JS_GetProperty(
          cx, NSGetFactoryHolder, "NSGetFactory", &NSGetFactory_val) ||
  ...
  rv = nsXPConnect::XPConnect()->WrapJS(cx,
                                        jsGetFactoryObj,
...
mozJSComponentLoader::GetSharedGlobal(JSContext* aCx)
{
  if (!mLoaderGlobal) {
    ...
    CreateLoaderGlobal(
        aCx, NS_LITERAL_CSTRING("shared JSM global"), nullptr, &globalObj);
  ...
  if (testFile) {
    *aRealFile = true;
    ...
    if (XRE_IsParentProcess()) {
      ...
      rv = nsXPConnect::XPConnect()->WrapNative(aCx,
                                                thisObj,
                                                aComponentFile,
                                                NS_GET_IID(nsIFile),
                                                locationObj.address());
                                        ...
                                        MutableHandleValue aException)
{
  ...
  RootedObject obj(
      ...
      PrepareObjectForLocation(
          cx, aComponentFile, aInfo.URI(), &reuseGlobal, &realFile));
    ...
    if (realFile) {
      ...
      if (reuseGlobal)
        CompileForNonSyntacticScope(
            cx, options, buf.get(), map.size(), &script);
      else
      ...
      if (!len64) return NS_ERROR_FAILURE;
      ...
      if (bytesRead != len) return NS_BASE_STREAM_OSERROR;
                             ...
                             MutableHandleValue retval)
{
  ...
  if (optionalArgc) {
    ...
    if (targetVal.isObject()) {
      ...
      return ReportOnCallerUTF8(
          cx, ERROR_SCOPE_OBJ, PromiseFlatCString(registryLocation).get());
    }
  } else {
...
nsresult
mozJSComponentLoader::IsModuleLoaded(const nsACString& aLocation, bool* retval)
{
  MOZ_ASSERT(nsContentUtils::IsCallerChrome());
...
NS_IMETHODIMP
mozJSComponentLoader::LoadedModules(uint32_t* length, char*** aModules)
{
...
NS_IMETHODIMP
mozJSComponentLoader::LoadedComponents(uint32_t* length, char*** aComponents)
{
...
static JSObject*
ResolveModuleObjectPropertyById(JSContext* aCx,
                                HandleObject aModObj,
                                HandleId id)
{
  if (JS_HasExtensibleLexicalEnvironment(aModObj)) {
                                 ...
                                 MutableHandleObject vp)
{
  ...
  if (!mImports.Get(info.Key(), &mod) &&
    ...
    if (NS_FAILED(rv)) {
      if (!exception.isUndefined()) {
        ...
        if (!JS_WrapValue(callercx, &exception)) return NS_ERROR_OUT_OF_MEMORY;
  ...
  if (targetObj) {
    ...
    RootedObject exportedSymbolsHolder(
        cx, ResolveModuleObjectProperty(cx, mod->obj, "EXPORTED_SYMBOLS"));
    if (!exportedSymbolsHolder ||
        !JS_GetProperty(
            cx, exportedSymbolsHolder, "EXPORTED_SYMBOLS", &symbols)) {
      nsCString location;
      rv = info.GetLocation(location);
      NS_ENSURE_SUCCESS(rv, rv);
      return ReportOnCallerUTF8(cxhelper, ERROR_NOT_PRESENT, location.get());
    }
    ...
    if (!isArray) {
      ...
      rv = info.GetLocation(location);
      NS_ENSURE_SUCCESS(rv, rv);
      return ReportOnCallerUTF8(cxhelper, ERROR_NOT_AN_ARRAY, location.get());
    }
    ...
    if (!JS_GetArrayLength(cx, symbolsObj, &symbolCount)) {
      ...
      NS_ENSURE_SUCCESS(rv, rv);
      return ReportOnCallerUTF8(
          cxhelper, ERROR_GETTING_ARRAY_LENGTH, location.get());
    }
    ...
    for (uint32_t i = 0; i < symbolCount; ++i) {
      if (!JS_GetElement(cx, symbolsObj, i, &value) || !value.isString() ||
        ...
        NS_ENSURE_SUCCESS(rv, rv);
        return ReportOnCallerUTF8(
            cxhelper, ERROR_ARRAY_ELEMENT, location.get(), i);
      }
      ...
      if (!symbolHolder ||
        ...
        NS_ENSURE_SUCCESS(rv, rv);
        return ReportOnCallerUTF8(
            cxhelper, ERROR_GETTING_SYMBOL, location.get(), bytes.ptr());
      }
      ...
      if (!JS_WrapValue(cx, &value) ||
        ...
        NS_ENSURE_SUCCESS(rv, rv);
        return ReportOnCallerUTF8(
            cxhelper, ERROR_SETTING_SYMBOL, location.get(), bytes.ptr());
      }
#ifdef DEBUG
      if (i == 0) {
...
NS_IMETHODIMP
mozJSComponentLoader::Observe(nsISupports* subject,
                              const char* topic,
                              const char16_t* data)
{
...
size_t
mozJSComponentLoader::ModuleEntry::SizeOfIncludingThis(
