...
{
  MOZ_COLLECT_REPORT(
      "explicit/script-preloader/heap/saved-scripts",
      KIND_HEAP,
      UNITS_BYTES,
  ...
  MOZ_COLLECT_REPORT(
      "explicit/script-preloader/heap/restored-scripts",
      KIND_HEAP,
      UNITS_BYTES,
  ...
  MOZ_COLLECT_REPORT("explicit/script-preloader/heap/other",
                     KIND_HEAP,
                     UNITS_BYTES,
...
ScriptPreloader::Trace(JSTracer* trc)
{
  for (auto& script : IterHash(mScripts)) {
    JS::TraceEdge(
        trc, &script->mScript, "ScriptPreloader::CachedScript.mScript");
  }
}
...
ScriptPreloader::InvalidateCache()
{
  ...
  if (mSaveComplete && mChildCache) {
    ...
    Unused << NS_NewNamedThread(
        "SaveScripts", getter_AddRefs(mSaveThread), this);
  }
}

nsresult
ScriptPreloader::Observe(nsISupports* subject,
                         const char* topic,
                         const char16_t* data)
{
  nsCOMPtr<nsIObserverService> obs = services::GetObserverService();
...
Result<Ok, nsresult>
ScriptPreloader::InitCache(const Maybe<ipc::FileDescriptor>& cacheFile,
...
ScriptPreloader::WriteCache()
{
  ...
  {
    AutoFDClose fd;
    MOZ_TRY(cacheFile->OpenNSPRFileDesc(
        PR_WRONLY | PR_CREATE_FILE, 0644, &fd.rwget()));
...
void
ScriptPreloader::NoteScript(const nsCString& url,
...
ScriptPreloader::DecodeNextBatch(size_t chunkSize, JS::HandleObject scope)
{
  ...
  if (!JS::CanCompileOffThread(cx, options, size) ||
      !JS::DecodeMultiOffThreadScripts(cx,
                                       options,
                                       mParsingSources,
  ...
  LOG(Debug,
      ...
      (unsigned)mParsingSources.length(),
      (unsigned)size,
      (TimeStamp::Now() - start).ToMilliseconds());
}
