...
static void
SubscriptCachePath(JSContext* cx,
                   nsIURI* uri,
              ...
              MutableHandleScript script)
{
  ...
  if (!charset.IsVoid()) {
    ...
    nsresult rv =
        ScriptLoader::ConvertToUTF16(nullptr,
    ...
    JS::SourceBufferHolder srcBuf(
        scriptBuf, scriptLength, JS::SourceBufferHolder::GiveOwnership);
...
class AsyncScriptLoader : public nsIIncrementalStreamLoaderObserver
{
 public:
  ...
  AsyncScriptLoader(nsIChannel* aChannel,
                    bool aWantReturnValue,
                    JSObject* aTargetObj,
                    JSObject* aLoadScope,
                                                 ...
                                                 MutableHandleValue retval)
{
  ...
  if (script) {
    ...
    // |back there.
    cache = nullptr;
  } else if (!ReadScript(uri,
                         cx,
                         targetObj,
                         options.charset,
                         static_cast<const char*>(uriStr.get()),
                         serv,
                         options.wantReturnValue,
                         &script)) {
    return NS_OK;
