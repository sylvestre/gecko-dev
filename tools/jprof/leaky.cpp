...
void
leaky::initialize(int argc, char** argv)
{
  ...
  while (
      ((arg = getopt_long(
            argc, argv, "adEe:gh:i:r:Rs:tT:qvx:ln:", longopts, &longindex)) !=
       -1)) {
    switch (arg) {
      case '?':
      default:
...
static void*
mapFile(int fd, u_int flags, off_t* sz)
{
  struct stat sb;
  if (fstat(fd, &sb) < 0) {
    perror("fstat");
...
static int
symbolOrder(void const* a, void const* b)
{
...
void
leaky::ReadSharedLibrarySymbols()
{
...
void
leaky::setupSymbols(const char* fileName)
{
  if (usefulSymbols == 0) {
    // only read once!
...
// address.
int
leaky::findSymbolIndex(u_long addr)
{
  u_int base = 0;
...
bool
leaky::excluded(malloc_log_entry* lep)
{
...
bool
leaky::included(malloc_log_entry* lep)
{
  if (includes.IsEmpty()) {
    return true;
  }
...
void
leaky::displayStackTrace(FILE* out, malloc_log_entry* lep)
{
  char** pcp = &lep->pcs[0];
...
void
leaky::dumpEntryToLog(malloc_log_entry* lep)
{
...
void
leaky::generateReportHTML(FILE* fp, int* countArray, int count, int thread)
{
  fprintf(fp, "<center>");
  if (showThreads) {
    fprintf(
        fp, "<hr><A NAME=thread_%d><b>Thread: %d</b></A><p>", thread, thread);
  }
  fprintf(
      fp,
  ...
  if (totalTimerHits != count)
    fprintf(stderr,
            "Hit count mismatch: count=%d; totalTimerHits=%d",
            count,
            totalTimerHits);
  ...
  for (i = 0;
    ...
    fprintf(fp,
            ...
            rankingTable[i],
            (*sp)->timerHit,
            ((float)(*sp)->timerHit / (float)totalTimerHits) * 100.0,
            symname);
    delete[] symname;
  }
}
...
void
FunctionCount::printReport(FILE* fp, leaky* lk, int parent, int total)
{
  const char* fmt =
