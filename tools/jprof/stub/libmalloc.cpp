...
static void
startSignalCounter(unsigned long millisec);
static int
enableRTCSignals(bool enable);
...
JPROF_STATIC void
CrawlStack(malloc_log_entry* me, void* stack_top, void* top_instr_ptr)
{
...
#if defined(linux)
static int
setupRTCSignals(int hz, struct sigaction* sap)
{
...
JPROF_STATIC void
StackHook(int signum, siginfo_t* info, void* ucontext)
{
	...
	*/
    ...
    if (tst) {
      ...
      if (circular_op) {
        ...
        fprintf(
            stderr, "JP_CIRCULAR buffer of %lu bytes\n", (unsigned long)size);
        circular = true;
      }
