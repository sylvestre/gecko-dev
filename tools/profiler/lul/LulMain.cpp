...
class PriMap
{
 public:
  ...
  void AddSecMap(mozilla::UniquePtr<SecMap>&& aSecMap)
  {
    ...
    char buf[100];
    SprintfLiteral(
        buf, "AddSecMap: now have %d SecMaps\n", (int)mSecMaps.size());
    buf[sizeof(buf) - 1] = 0;
    mLog(buf);
  }
...
void
LUL::NotifyAfterMap(uintptr_t aRXavma,
                    size_t aSize,
                    ...
                    const void* aMappedImage)
{
  ...
  if (aSize > 0) {
    ...
    if (!aMappedImage) {
      (void)lul::ReadSymbolData(string(aFileName),
                                std::vector<string>(),
                                smap.get(),
                                (void*)aRXavma,
                                aSize,
      ...
      (void)lul::ReadSymbolDataInternal((const uint8_t*)aMappedImage,
                                        string(aFileName),
                                        std::vector<string>(),
                                        smap.get(),
                                        (void*)aRXavma,
...
// RUNS IN NO-MALLOC CONTEXT
static TaggedUWord
DerefTUW(TaggedUWord aAddr, const StackImage* aStackImg)
{
  if (!aAddr.Valid()) {
    return TaggedUWord();
  }
...
// RUNS IN NO-MALLOC CONTEXT
static TaggedUWord
EvaluateReg(int16_t aReg, const UnwindRegs* aOldRegs, TaggedUWord aCFA)
{
  switch (aReg) {
    case DW_REG_CFA:
      return aCFA;
...
// RUNS IN NO-MALLOC CONTEXT
TaggedUWord
LExpr::EvaluateExpr(const UnwindRegs* aOldRegs,
                    TaggedUWord aCFA,
                    const StackImage* aStackImg,
...
// RUNS IN NO-MALLOC CONTEXT
static void
UseRuleSet(/*MOD*/ UnwindRegs* aRegs,
           const StackImage* aStackImg,
           const RuleSet* aRS,
           const vector<PfxInstr>* aPfxInstrs)
{
...
#error "Unsupported arch"
  ...
  TaggedUWord cfa = aRS->mCfaExpr.EvaluateExpr(
      &old_regs, inval /*old cfa*/, aStackImg, aPfxInstrs);
  ...
  while (true) {
    if (DEBUG_MAIN) {
      ...
      SprintfLiteral(buf,
                     ...
                     (int)regs.xip.Valid(),
                     (unsigned long long int)regs.xip.Value(),
                     (int)regs.xsp.Valid(),
                     (unsigned long long int)regs.xsp.Value(),
                     (int)regs.xbp.Valid(),
                     (unsigned long long int)regs.xbp.Value());
      buf[sizeof(buf) - 1] = 0;
      mLog(buf);
#elif defined(GP_ARCH_arm)
      SprintfLiteral(buf,
                     ...
                     (int)regs.r15.Valid(),
                     (unsigned long long int)regs.r15.Value(),
                     (int)regs.r7.Valid(),
                     (unsigned long long int)regs.r7.Value(),
                     (int)regs.r11.Valid(),
                     (unsigned long long int)regs.r11.Value(),
                     (int)regs.r12.Valid(),
                     (unsigned long long int)regs.r12.Value(),
                     (int)regs.r13.Valid(),
                     (unsigned long long int)regs.r13.Value(),
                     (int)regs.r14.Valid(),
                     (unsigned long long int)regs.r14.Value());
      buf[sizeof(buf) - 1] = 0;
      mLog(buf);
#else
#error "Unsupported arch"
...
#error "Unsupported arch"
    ...
    if (DEBUG_MAIN) {
      ...
      SprintfLiteral(buf,
                     ...
                     (unsigned long long int)ia.Value(),
                     ruleset);
      buf[sizeof(buf) - 1] = 0;
      mLog(buf);
    }
...
GetAndCheckStackTrace(LUL* aLUL, const char* dstring)
{
