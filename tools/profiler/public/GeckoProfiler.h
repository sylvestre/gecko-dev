...
#define PROFILER_FOR_EACH_FEATURE(macro)                                                                                  \
  ...
  macro(0, "java", Java)                                                                                                  \
      ...
      macro(1, "js", JS)                                                                                                  \
                                                                                                                          \
      /* Include the C++ leaf node if not stackwalking. */ /* The DevTools profiler doesn't want the native addresses. */ \
      macro(2, "leaf", Leaf)                                                                                              \
                                                                                                                          \
...
static inline void
profiler_get_buffer_info(uint32_t* aCurrentPosition,
                         uint32_t* aEntries,
                         uint32_t* aGeneration)
{
  *aCurrentPosition = 0;
...
profiler_get_profile(double aSinceTime = 0, bool aIsShuttingDown = false);
...
class MOZ_RAII AutoProfilerLabel
{
 public:
                        ...
                        MOZ_GUARD_OBJECT_NOTIFIER_PARAM)
  {
    ...
    if (mPseudoStack) {
      mPseudoStack->pushCppFrame(aLabel,
                                 aDynamicString,
                                 this,
                                 aLine,
...
class MOZ_RAII AutoProfilerTracing
{
 public:
      ...
      : mCategory(aCategory), mMarkerName(aMarkerName)
  {
    MOZ_GUARD_OBJECT_NOTIFIER_INIT;
    profiler_tracing(
        mCategory, mMarkerName, TRACING_INTERVAL_START, Move(aBacktrace));
  }
