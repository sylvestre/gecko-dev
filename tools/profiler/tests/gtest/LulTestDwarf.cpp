...
TEST_F(LulDwarfCFI, IncompleteLength32)
{
  ...
  CallFrameInfo parser(
      contents.data(), contents.size() - 2, &reader, &handler, &reporter);
...
TEST_F(LulDwarfCFI, IncompleteLength64)
{
  ...
  CallFrameInfo parser(
      contents.data(), contents.size() - 4, &reader, &handler, &reporter);
  EXPECT_FALSE(parser.Start());
}
...
TEST_F(LulDwarfCFI, IncompleteId32)
{
  ...
  reader.SetAddressSize(8);
  CallFrameInfo parser(
      contents.data(), contents.size(), &reader, &handler, &reporter);
  EXPECT_FALSE(parser.Start());
}
...
TEST_F(LulDwarfCFI, TwoFDEsTwoCIEs)
{
...

  {
    ...
    EXPECT_CALL(
        handler,
        Entry(
            _, 0x778b27dfe5871f05ULL, 0x324ace3448070926ULL, 2, "", 0x61d2c581))
        .WillOnce(Return(true));
    EXPECT_CALL(handler, End()).WillOnce(Return(true));
  }
  {
    ...
    EXPECT_CALL(
        handler,
        Entry(
            _, 0xf6054ca18b10bf5fULL, 0x45fdb970d8bca342ULL, 3, "", 0xbf45e65a))
        .WillOnce(Return(true));
    EXPECT_CALL(handler, End()).WillOnce(Return(true));
  }
...
struct CFIInsnFixture : public CFIFixture
{
  ...
  void StockCIEAndFDE(CFISection* section)
  {
    ...
    EXPECT_CALL(
        handler,
        ValOffsetRule(fde_start, kCFARegister, cfa_base_register, cfa_offset))
        .InSequence(s)
        .WillOnce(Return(true));
  }
                    ...
                    ByteReader* reader = nullptr)
  {
    ...
    CallFrameInfo parser(
        contents.data(), contents.size(), reader_to_use, &handler, &reporter);
    if (succeeds)
      EXPECT_TRUE(parser.Start());
    else
      EXPECT_FALSE(parser.Start());
...
TEST_F(LulDwarfCFIInsn, DW_CFA_advance_loc)
{
  ...
  EXPECT_CALL(
      handler,
      ValOffsetRule(
          fde_start + 0x2a * code_factor, kCFARegister, 0x5bbb3715, 0x0186c7bf))
      .InSequence(s)
      .WillOnce(Return(true));
  EXPECT_CALL(handler, End()).InSequence(s).WillOnce(Return(true));
...
TEST_F(LulDwarfCFIInsn, DW_CFA_def_cfa_sf)
{
  ...
  EXPECT_CALL(
      handler,
      ValOffsetRule(fde_start, kCFARegister, 0x8ccb32b7, 0x9ea * data_factor))
      .InSequence(s)
      .WillOnce(Return(true));
  EXPECT_CALL(
      handler,
      ValOffsetRule(fde_start, kCFARegister, 0x9b40f5da, -0x40a2 * data_factor))
      .InSequence(s)
      .WillOnce(Return(true));
  EXPECT_CALL(handler, End()).InSequence(s).WillOnce(Return(true));
...
TEST_F(LulDwarfCFIInsn, DW_CFA_def_cfa_registerBadRule)
{
  ...
  EXPECT_CALL(
      handler,
      ValExpressionRule(fde_start, kCFARegister, "needle in a haystack"))
...
TEST_F(LulDwarfCFIInsn, DW_CFA_def_cfa_offset)
{
  ...
  EXPECT_CALL(
      handler,
      ValOffsetRule(fde_start, kCFARegister, cfa_base_register, 0x1e8e3b9b))
      .InSequence(s)
      .WillOnce(Return(true));
  EXPECT_CALL(handler, End()).InSequence(s).WillOnce(Return(true));
...
TEST_F(LulDwarfCFIInsn, DW_CFA_def_cfa_offset_sf)
{
  ...
  EXPECT_CALL(
      handler,
      ValOffsetRule(
          fde_start, kCFARegister, cfa_base_register, 0x970 * data_factor))
      .InSequence(s)
      .WillOnce(Return(true));
  EXPECT_CALL(
      handler,
      ValOffsetRule(
          fde_start, kCFARegister, cfa_base_register, -0x2cd * data_factor))
      .InSequence(s)
      .WillOnce(Return(true));
  EXPECT_CALL(handler, End()).InSequence(s).WillOnce(Return(true));
...
TEST_F(LulDwarfCFIInsn, DW_CFA_offset_extended_sf)
{
  ...
  EXPECT_CALL(
      handler,
      OffsetRule(fde_start, 0x997c23ee, kCFARegister, 0x2d00 * data_factor))
      .InSequence(s)
      .WillOnce(Return(true));
  EXPECT_CALL(
      handler,
      OffsetRule(fde_start, 0x9519eb82, kCFARegister, -0xa77 * data_factor))
      .InSequence(s)
      .WillOnce(Return(true));
  EXPECT_CALL(handler, End()).InSequence(s).WillOnce(Return(true));
...
TEST_F(LulDwarfCFIInsn, DW_CFA_val_offset)
{
  ...
  EXPECT_CALL(
      handler,
      ValOffsetRule(fde_start, 0x623562fe, kCFARegister, 0x673 * data_factor))
      .InSequence(s)
      .WillOnce(Return(true));
  EXPECT_CALL(handler, End()).InSequence(s).WillOnce(Return(true));
...
TEST_F(LulDwarfCFIInsn, DW_CFA_val_offset_sf)
{
  ...
  EXPECT_CALL(
      handler,
      ValOffsetRule(fde_start, 0x6f4f, kCFARegister, 0xaab * data_factor))
      .InSequence(s)
      .WillOnce(Return(true));
  EXPECT_CALL(
      handler,
      ValOffsetRule(fde_start, 0x2483, kCFARegister, -0x8a2 * data_factor))
      .InSequence(s)
      .WillOnce(Return(true));
  EXPECT_CALL(handler, End()).InSequence(s).WillOnce(Return(true));
...
TEST_F(LulDwarfCFIInsn, DW_CFA_expression)
{
  ...
  EXPECT_CALL(
      handler,
      ExpressionRule(
          fde_start, 0xa1619fb2, "plus ça change, plus c'est la même chose"))
      .InSequence(s)
      .WillOnce(Return(true));
  EXPECT_CALL(handler, End()).InSequence(s).WillOnce(Return(true));
...
TEST_F(LulDwarfCFIInsn, DW_CFA_val_expression)
{
  ...
  EXPECT_CALL(handler,
              ValExpressionRule(
                  fde_start, 0xc5e4a9e3, "he who has the gold makes the rules"))
      .InSequence(s)
      .WillOnce(Return(true));
  EXPECT_CALL(handler, End()).InSequence(s).WillOnce(Return(true));
...
TEST_F(LulDwarfCFIInsn, DW_CFA_restore)
{
...

  {
    ...
    EXPECT_CALL(handler,
                OffsetRule(fde_start + 0x13 * code_factor,
                           0x3c,
                           kCFARegister,
                           0x9a50 * data_factor))
        .WillOnce(Return(true));
    ...
    EXPECT_CALL(handler,
                OffsetRule(fde_start + (0x13 + 0x01) * code_factor,
                           0x3c,
                           kCFARegister,
                           0xb348 * data_factor))
        .WillOnce(Return(true));
...
TEST_F(LulDwarfCFIInsn, DW_CFA_restoreNoRule)
{
...

  {
    ...
    EXPECT_CALL(handler,
                OffsetRule(fde_start + 0x7 * code_factor,
                           0x2c,
                           kCFARegister,
                           0x1f47 * data_factor))
        .WillOnce(Return(true));
...
TEST_F(LulDwarfCFIInsn, DW_CFA_restore_extended)
{
...

  {
    ...
    EXPECT_CALL(
        handler,
        OffsetRule(fde_start, 0x0f9b8a1c, kCFARegister, 0xc979 * data_factor))
        .WillOnce(Return(true));
    // FDE's rule for register 0x0f9b8a1c.
    EXPECT_CALL(handler,
                OffsetRule(fde_start + 0x3 * code_factor,
                           0x0f9b8a1c,
                           kCFARegister,
                           0x3b7b * data_factor))
        .WillOnce(Return(true));
    ...
    EXPECT_CALL(handler,
                OffsetRule(fde_start + (0x3 + 0x4) * code_factor,
                           0x0f9b8a1c,
                           kCFARegister,
                           0xc979 * data_factor))
        .WillOnce(Return(true));
...
TEST_F(LulDwarfCFIInsn, DW_CFA_remember_and_restore_stateCFA)
{
  ...
  EXPECT_CALL(
      handler,
      ValOffsetRule(
          fde_start + code_factor, kCFARegister, cfa_base_register, 0x90481102))
      .InSequence(s)
      .WillOnce(Return(true));
  EXPECT_CALL(handler,
              ValOffsetRule(fde_start + code_factor * 2,
                            kCFARegister,
                            cfa_base_register,
                            cfa_offset))
      .InSequence(s)
...
TEST_F(LulDwarfCFIInsn, DW_CFA_GNU_negative_offset_extended)
{
  ...
  EXPECT_CALL(
      handler,
      OffsetRule(fde_start, 0x430cc87a, kCFARegister, -0x613 * data_factor))
      .InSequence(s)
      .WillOnce(Return(true));
  EXPECT_CALL(handler, End()).InSequence(s).WillOnce(Return(true));
...
TEST_F(LulDwarfCFIRestore, RestoreOffsetRuleChanged)
{
  ...
  EXPECT_CALL(
      handler,
      OffsetRule(
          fde_start + 2 * code_factor, 0x21, kCFARegister, 0xeb7 * data_factor))
      .InSequence(s)
      .WillOnce(Return(true));
  EXPECT_CALL(handler, End()).WillOnce(Return(true));
...
TEST_F(LulDwarfCFIRestore, RestoreOffsetRuleChangedOffset)
{
  ...
  EXPECT_CALL(
      handler,
      OffsetRule(
          fde_start + code_factor, 0x21, kCFARegister, 0xf4f * data_factor))
      .InSequence(s)
      .WillOnce(Return(true));
  EXPECT_CALL(
      handler,
...
TEST_F(LulDwarfCFIRestore, RestoreValOffsetRuleUnchanged)
{
  ...
  EXPECT_CALL(
      handler,
      ValOffsetRule(fde_start, 0x829caee6, kCFARegister, 0xe4c * data_factor))
      .InSequence(s)
      .WillOnce(Return(true));
  EXPECT_CALL(handler, End()).WillOnce(Return(true));
...
TEST_F(LulDwarfCFIRestore, RestoreValOffsetRuleChanged)
{
  ...
  EXPECT_CALL(
      handler,
      ValOffsetRule(fde_start, 0xf17c36d6, kCFARegister, 0xeb7 * data_factor))
      .InSequence(s)
      .WillOnce(Return(true));
  EXPECT_CALL(handler, UndefinedRule(fde_start + code_factor, 0xf17c36d6))
      ...
      .WillOnce(Return(true));
  EXPECT_CALL(handler,
              ValOffsetRule(fde_start + 2 * code_factor,
                            0xf17c36d6,
                            kCFARegister,
                            0xeb7 * data_factor))
      .InSequence(s)
...
TEST_F(LulDwarfCFIRestore, RestoreValOffsetRuleChangedValOffset)
{
  ...
  EXPECT_CALL(
      handler,
      ValOffsetRule(fde_start, 0x2cf0ab1b, kCFARegister, 0x562 * data_factor))
      .InSequence(s)
      .WillOnce(Return(true));
  EXPECT_CALL(handler,
              ValOffsetRule(fde_start + code_factor,
                            0x2cf0ab1b,
                            kCFARegister,
                            0xe88 * data_factor))
      .InSequence(s)
...
TEST_F(LulDwarfCFIRestore, RestoreValExpressionRuleChanged)
{
  ...
  EXPECT_CALL(
      handler,
      ValExpressionRule(fde_start + 2 * code_factor, 0xb5ca5c46, "revolting"))
      .InSequence(s)
      .WillOnce(Return(true));
  EXPECT_CALL(handler, End()).WillOnce(Return(true));
...
TEST_F(LulDwarfCFIRestore, RestoreValExpressionRuleChangedValExpression)
{
  ...
  EXPECT_CALL(
      handler,
      ValExpressionRule(fde_start + 2 * code_factor, 0x500f5739, "repulsive"))
      .InSequence(s)
      .WillOnce(Return(true));
  EXPECT_CALL(handler, End()).WillOnce(Return(true));
...
struct EHFrameFixture : public CFIInsnFixture
{
  ...
  void ParseEHFrameSection(CFISection* section, bool succeeds = true)
  {
    ...
    CallFrameInfo parser(
        contents.data(), contents.size(), &reader, &handler, &reporter, true);
    if (succeeds)
      EXPECT_TRUE(parser.Start());
    else
      EXPECT_FALSE(parser.Start());
...
TEST_F(LulDwarfCFIReporter, BadInstruction)
{
  reporter.BadInstruction(
      0x0123456789abcdefULL, CallFrameInfo::kFDE, 0xfedcba9876543210ULL);
}

TEST_F(LulDwarfCFIReporter, NoCFARule)
{
  reporter.NoCFARule(
      0x0123456789abcdefULL, CallFrameInfo::kCIE, 0xfedcba9876543210ULL);
}

TEST_F(LulDwarfCFIReporter, EmptyStateStack)
{
  reporter.EmptyStateStack(
      0x0123456789abcdefULL, CallFrameInfo::kTerminator, 0xfedcba9876543210ULL);
}

TEST_F(LulDwarfCFIReporter, ClearingCFARule)
{
  reporter.ClearingCFARule(
      0x0123456789abcdefULL, CallFrameInfo::kFDE, 0xfedcba9876543210ULL);
}
...
TEST_F(LulDwarfEvaluatePfxExpr, NormalEvaluation)
{
  ...
  TaggedUWord res = EvaluatePfxExpr(
      2 /*offset of start insn*/, &regs, initialCFA, &si, instrs);
