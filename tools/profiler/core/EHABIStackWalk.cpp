...
bool
operator<(const EHEntryHandle& lhs, const EHEntryHandle& rhs)
{
...
size_t
EHABIStackWalk(const mcontext_t& aContext,
               void* stackBase,
               void** aSPs,
               void** aPCs,
...
class EHInterp
{
 public:
  ...
  EHInterp(EHState& aState,
           const EHEntry* aEntry,
           uint32_t aStackLimit,
           uint32_t aStackBase)
      : mState(aState),
...
bool
EHState::unwind(const EHEntry* aEntry, const void* stackBasePtr)
{
...
// Async signal unsafe.
EHAddrSpace::EHAddrSpace(const std::vector<EHTable>& aTables) : mTables(aTables)
{
...
const EHTable*
EHAddrSpace::lookup(uint32_t aPC) const
{
...
const EHEntry*
EHTable::lookup(uint32_t aPC) const
{
      ...
      mName(aName)
{
  ...
  for (unsigned i = 0; i < file.e_phnum; ++i) {
    const Elf32_Phdr& phdr = *(reinterpret_cast<Elf32_Phdr*>(
        base + file.e_phoff + i * file.e_phentsize));
  ...
  const EHEntry* endTable = reinterpret_cast<const EHEntry*>(
      mLoadOffset + exidxHdr->p_vaddr + exidxHdr->p_memsz);
#ifdef HAVE_UNSORTED_EXIDX
