...
registerFunctions(sqlite3* aDB)
{
  ...
  for (size_t i = 0; SQLITE_OK == rv && i < ArrayLength(functions); ++i) {
    ...
    rv = ::sqlite3_create_function(aDB,
                                   p->zName,
                                   p->nArg,
                                   p->enc,
                                   p->pContext,
...
void
caseFunction(sqlite3_context* aCtx, int aArgc, sqlite3_value** aArgv)
{
 ...
 */
void
likeFunction(sqlite3_context* aCtx, int aArgc, sqlite3_value** aArgv)
{
  ...
  if (::sqlite3_value_bytes(aArgv[0]) > SQLITE_MAX_LIKE_PATTERN_LENGTH) {
    ::sqlite3_result_error(
        aCtx, "LIKE or GLOB pattern too complex", SQLITE_TOOBIG);
    return;
  }
...
void
levenshteinDistanceFunction(sqlite3_context* aCtx,
                            int aArgc,
                            sqlite3_value** aArgv)
{
