...
AsyncStatement::~AsyncStatement()
{
  ...
  if (!onCallingThread) {
    ...
    NS_ProxyRelease(
        "AsyncStatement::mDBConnection", targetThread, mDBConnection.forget());
  }
}
...
AsyncStatement::getAsyncStatement(sqlite3_stmt** _stmt)
{
  ...
  if (!mAsyncStatement) {
    int rc = mDBConnection->prepareStatement(
        mNativeConnection, mSQLString, &mAsyncStatement);
    if (rc != SQLITE_OK) {
      MOZ_LOG(gStorageLog,
              LogLevel::Error,
              ("Sqlite statement prepare error: %d '%s'",
               rc,
