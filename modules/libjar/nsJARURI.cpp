...
nsresult
nsJARURI::FormatSpec(const nsACString& entrySpec,
                     nsACString& result,
                     bool aIncludeScheme)
{
                         ...
                         nsIURL** url)
{
  *url = nullptr;
  ...
  nsresult rv = stdURL->Init(
      nsIStandardURL::URLTYPE_NO_AUTHORITY, -1, spec, charset, nullptr);
  if (NS_FAILED(rv)) {
    return rv;
  }
...
nsJARURI::SetSpecWithBase(const nsACString& aSpec, nsIURI* aBaseURL)
{
  ...
  if (NS_FAILED(rv)) {
    ...
    rv = entry->Init(nsIStandardURL::URLTYPE_NO_AUTHORITY,
                     -1,
                     aSpec,
                     mCharsetHint.get(),
                     otherJAR->mJAREntry);
  ...
  rv = ioServ->NewURI(Substring(begin, delim_begin),
                      mCharsetHint.get(),
                      aBaseURL,
...
NS_IMETHODIMP
nsJARURI::SetUserPass(const nsACString& aUserPass) { return NS_ERROR_FAILURE; }

NS_IMETHODIMP
nsJARURI::GetUsername(nsACString& aUsername) { return NS_ERROR_FAILURE; }

NS_IMETHODIMP
nsJARURI::SetUsername(const nsACString& aUsername) { return NS_ERROR_FAILURE; }

NS_IMETHODIMP
nsJARURI::GetPassword(nsACString& aPassword) { return NS_ERROR_FAILURE; }

NS_IMETHODIMP
nsJARURI::SetPassword(const nsACString& aPassword) { return NS_ERROR_FAILURE; }

NS_IMETHODIMP
nsJARURI::GetHostPort(nsACString& aHostPort) { return NS_ERROR_FAILURE; }

NS_IMETHODIMP
nsJARURI::SetHostPort(const nsACString& aHostPort) { return NS_ERROR_FAILURE; }
...
NS_IMETHODIMP
nsJARURI::SetHost(const nsACString& aHost) { return NS_ERROR_FAILURE; }

NS_IMETHODIMP
nsJARURI::GetPort(int32_t* aPort) { return NS_ERROR_FAILURE; }

NS_IMETHODIMP
nsJARURI::SetPort(int32_t aPort) { return NS_ERROR_FAILURE; }
...
NS_IMETHODIMP
nsJARURI::SetPathQueryRef(const nsACString& aPath) { return NS_ERROR_FAILURE; }
...
nsJARURI::CloneWithNewRef(const nsACString& newRef, nsIURI** result)
{
  ...
  nsCOMPtr<nsIJARURI> uri;
  rv = CloneWithJARFileInternal(
      mJARFile, eReplaceRef, newRef, getter_AddRefs(uri));
  if (NS_FAILED(rv)) return rv;
...
NS_IMETHODIMP
nsJARURI::SetRef(const nsACString& ref) { return mJAREntry->SetRef(ref); }
...
{
  return CreateEntryURL(
      entryPath, mCharsetHint.get(), getter_AddRefs(mJAREntry));
}
...
nsJARURI::CloneWithJARFileInternal(nsIURI* jarFile,
                                   ...
                                   nsIJARURI** result)
{
  return CloneWithJARFileInternal(
      jarFile, refHandlingMode, EmptyCString(), result);
}
