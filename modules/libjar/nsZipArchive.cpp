...
static uint16_t
xtoint(const uint8_t* ii);
static uint32_t
xtolong(const uint8_t* ll);
static uint32_t
HashName(const char* aName, uint16_t nameLen);
#ifdef XP_UNIX
static nsresult
  ...
  void Write(const nsACString& zip, const char* entry) const
  {
    if (!fd) {
      ...
      nsresult rv = NS_NewLocalFile(
          NS_ConvertUTF8toUTF16(env), false, getter_AddRefs(logFile));
      ...
      HANDLE handle = CreateFileW(path.get(),
                                  FILE_APPEND_DATA,
                                  FILE_SHARE_WRITE,
                                  nullptr,
                                  OPEN_ALWAYS,
      ...
      rv = logFile->OpenNSPRFileDesc(
          PR_WRONLY | PR_CREATE_FILE | PR_APPEND, 0644, &file);
      if (NS_FAILED(rv)) return;
#endif
      fd = file;
    }
...
nsresult
nsZipHandle::Init(nsIFile* file, nsZipHandle** ret, PRFileDesc** aFd)
{
...
nsresult
nsZipHandle::Init(nsZipArchive* zip, const char* entry, nsZipHandle** ret)
{
  RefPtr<nsZipHandle> handle = new nsZipHandle();
...
nsresult
nsZipHandle::Init(const uint8_t* aData, uint32_t aLen, nsZipHandle** aRet)
{
  RefPtr<nsZipHandle> handle = new nsZipHandle();
...
int64_t
nsZipHandle::SizeOfMapping()
{
  return mTotalLen;
}

nsresult
nsZipHandle::GetNSPRFileDesc(PRFileDesc** aNSPRFileDesc)
{
  if (!aNSPRFileDesc) {
...
//---------------------------------------------
nsresult
nsZipArchive::OpenArchive(nsZipHandle* aZipHandle, PRFileDesc* aFd)
{
  mFd = aZipHandle;
...
nsresult
nsZipArchive::OpenArchive(nsIFile* aFile)
{
...
//---------------------------------------------
nsresult
nsZipArchive::ExtractFile(nsZipItem* item, const char* outname, PRFileDesc* aFd)
{
...
//---------------------------------------------
nsresult
nsZipFind::FindNext(const char** aResult, uint16_t* aNameLen)
{
...
ResolveSymlink(const char* path)
{
  ...
  if (!fIn) return NS_ERROR_FILE_DISK_FULL;
...
//---------------------------------------------
nsresult
nsZipArchive::BuildFileList(PRFileDesc* aFd)
{
  ...
  if (mFd->mLen > ZIPCENTRAL_SIZE &&
    ...
    if (readaheadLength) {
      ...
      posix_madvise(
          const_cast<uint8_t*>(startp), readaheadLength, POSIX_MADV_WILLNEED);
...
//---------------------------------------------
uint32_t
nsZipArchive::GetDataOffset(nsZipItem* aItem)
{
  MOZ_ASSERT(aItem);
  MOZ_WIN_MEM_TRY_BEGIN
...
//---------------------------------------------
const uint8_t*
nsZipArchive::GetData(nsZipItem* aItem)
{
  MOZ_ASSERT(aItem);
  MOZ_WIN_MEM_TRY_BEGIN
...
nsZipArchive::nsZipArchive()
    : mRefCnt(0), mCommentPtr(nullptr), mCommentLen(0), mBuiltSynthetics(false)
{
  zipLog.AddRef();
 ...
 */
static uint32_t
HashName(const char* aName, uint16_t len)
{
  MOZ_ASSERT(aName != 0);
 ...
 */
static PRTime
GetModTime(uint16_t aDate, uint16_t aTime)
{
...
const uint8_t*
nsZipItem::GetExtraField(uint16_t aTag, uint16_t* aBlockSize)
{
...
nsZipCursor::nsZipCursor(nsZipItem* item,
                         nsZipArchive* aZip,
                         uint8_t* aBuf,
                         uint32_t aBufSize,
                         bool doCRC)
...
uint8_t*
nsZipCursor::ReadOrCopy(uint32_t* aBytesRead, bool aCopy)
{
  int zerr;
  uint8_t* buf = nullptr;
  bool verifyCRC = true;
  ...
  switch (mItem->Compression()) {
     ...
     * unsigned int for avail_*. So use temporary stack values. */
      ...
      BrotliDecoderResult result = BrotliDecoderDecompressStream(
          mBrotliState,
          &avail_in,
          const_cast<const unsigned char**>(&mZs.next_in),
          &avail_out,
          &mZs.next_out,
          nullptr);
