...
nsresult
nsZipDataStream::Init(nsZipWriter* aWriter,
                      ...
                      int32_t aCompression)
{
  ...
  if (aCompression > 0) {
    ...
    rv = converter->AsyncConvertData(
        "uncompressed", "rawdeflate", mOutput, nullptr);
    NS_ENSURE_SUCCESS(rv, rv);
...
NS_IMETHODIMP
nsZipDataStream::OnDataAvailable(nsIRequest* aRequest,
...
NS_IMETHODIMP
nsZipDataStream::OnStartRequest(nsIRequest* aRequest, nsISupports* aContext)
{
  if (!mOutput) return NS_ERROR_NOT_INITIALIZED;
...
NS_IMETHODIMP
nsZipDataStream::OnStopRequest(nsIRequest* aRequest,
                               nsISupports* aContext,
...
nsresult
nsZipDataStream::ProcessData(nsIRequest* aRequest,
                             nsISupports* aContext,
                             char* aBuffer,
                             uint64_t aOffset,
                             uint32_t aCount)
...
nsresult
nsZipDataStream::ReadStream(nsIInputStream* aStream)
{
  if (!mOutput) return NS_ERROR_NOT_INITIALIZED;
