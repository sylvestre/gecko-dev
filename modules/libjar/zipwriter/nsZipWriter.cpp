...
NS_IMETHODIMP
nsZipWriter::GetInQueue(bool* aInQueue)
{
  *aInQueue = mInQueue;
  return NS_OK;
}
...
NS_IMETHODIMP
nsZipWriter::GetFile(nsIFile** aFile)
{
 ...
 */
nsresult
nsZipWriter::ReadFile(nsIFile* aFile)
{
  int64_t size;
...
NS_IMETHODIMP
nsZipWriter::Open(nsIFile* aFile, int32_t aIoFlags)
{
...
NS_IMETHODIMP
nsZipWriter::GetEntry(const nsACString& aZipEntry, nsIZipEntry** _retval)
{
  int32_t pos;
...
NS_IMETHODIMP
nsZipWriter::HasEntry(const nsACString& aZipEntry, bool* _retval)
{
...
NS_IMETHODIMP
nsZipWriter::AddEntryDirectory(const nsACString& aZipEntry,
                               PRTime aModTime,
                               bool aQueue)
{
  if (!mStream) return NS_ERROR_NOT_INITIALIZED;
...
NS_IMETHODIMP
nsZipWriter::AddEntryFile(const nsACString& aZipEntry,
                          ...
                          bool aQueue)
{
  ...
  rv = AddEntryStream(
      aZipEntry, modtime, aCompression, inputStream, false, permissions);
  NS_ENSURE_SUCCESS(rv, rv);
...
NS_IMETHODIMP
nsZipWriter::AddEntryChannel(const nsACString& aZipEntry,
                             PRTime aModTime,
                             int32_t aCompression,
                             nsIChannel* aChannel,
                             bool aQueue)
{
  ...
  rv = AddEntryStream(
      aZipEntry, aModTime, aCompression, inputStream, false, PERMISSIONS_FILE);
  NS_ENSURE_SUCCESS(rv, rv);
...
NS_IMETHODIMP
nsZipWriter::AddEntryStream(const nsACString& aZipEntry,
                            PRTime aModTime,
                            int32_t aCompression,
                            nsIInputStream* aStream,
                            bool aQueue)
{
  return AddEntryStream(
      aZipEntry, aModTime, aCompression, aStream, aQueue, PERMISSIONS_FILE);
}
...
NS_IMETHODIMP
nsZipWriter::RemoveEntry(const nsACString& aZipEntry, bool aQueue)
{
  if (!mStream) return NS_ERROR_NOT_INITIALIZED;
...
NS_IMETHODIMP
nsZipWriter::ProcessQueue(nsIRequestObserver* aObserver, nsISupports* aContext)
{
  if (!mStream) return NS_ERROR_NOT_INITIALIZED;
  if (mInQueue) return NS_ERROR_IN_PROGRESS;
...
NS_IMETHODIMP
nsZipWriter::OnStartRequest(nsIRequest* aRequest, nsISupports* aContext)
{
  return NS_OK;
}

NS_IMETHODIMP
nsZipWriter::OnStopRequest(nsIRequest* aRequest,
 ...
 */
NS_IMETHODIMP
nsZipWriter::AlignStoredFiles(uint16_t aAlignSize)
{
  nsresult rv;
...
nsresult
nsZipWriter::InternalAddEntryDirectory(const nsACString& aZipEntry,
                                       PRTime aModTime,
                                       uint32_t aPermissions)
{
  RefPtr<nsZipHeader> header = new nsZipHeader();
 ...
 */
nsresult
nsZipWriter::EntryCompleteCallback(nsZipHeader* aHeader, nsresult aStatus)
{
...
inline nsresult
nsZipWriter::BeginProcessingAddition(nsZipQueueItem* aItem, bool* complete)
{
  if (aItem->mFile) {
    bool exists;
  ...
  if (aItem->mStream || aItem->mChannel) {
    ...
    if (aItem->mStream) {
      ...
      rv = NS_NewInputStreamPump(
          getter_AddRefs(pump), aItem->mStream, 0, 0, true);
      NS_ENSURE_SUCCESS(rv, rv);
  ...
  *complete = true;
  return InternalAddEntryDirectory(
      aItem->mZipEntry, aItem->mModTime, aItem->mPermissions);
}

inline nsresult
nsZipWriter::BeginProcessingRemoval(int32_t aPos)
{
 ...
 */
void
nsZipWriter::BeginProcessingNextItem()
{
 ...
 */
void
nsZipWriter::FinishQueue(nsresult aStatus)
{
