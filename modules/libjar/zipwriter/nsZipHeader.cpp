...
NS_IMETHODIMP
nsZipHeader::GetLastModifiedTime(PRTime* aLastModifiedTime)
{
  NS_ASSERTION(mInited, "Not initalised");
...
void
nsZipHeader::Init(const nsACString& aPath,
                  PRTime aDate,
                  uint32_t aAttr,
                  uint32_t aOffset)
{
...
uint32_t
nsZipHeader::GetFileHeaderLength()
{
  return ZIP_FILE_HEADER_SIZE + mName.Length() + mLocalFieldLength;
}

nsresult
nsZipHeader::WriteFileHeader(nsIOutputStream* aStream)
{
  NS_ASSERTION(mInited, "Not initalised");
  ...
  if (mLocalFieldLength) {
    rv = ZW_WriteData(
        aStream, (const char*)mLocalExtraField.get(), mLocalFieldLength);
    NS_ENSURE_SUCCESS(rv, rv);
  }
...
nsresult
nsZipHeader::WriteCDSHeader(nsIOutputStream* aStream)
{
  NS_ASSERTION(mInited, "Not initalised");
...
nsresult
nsZipHeader::ReadCDSHeader(nsIInputStream* stream)
{
  NS_ASSERTION(!mInited, "Already initalised");
...
const uint8_t*
nsZipHeader::GetExtraField(uint16_t aTag, bool aLocal, uint16_t* aBlockSize)
{
 ...
 */
nsresult
nsZipHeader::PadExtraField(uint32_t aOffset, uint16_t aAlignSize)
{
  uint32_t pad_size;
  uint32_t pa_offset;
  uint32_t pa_end;
