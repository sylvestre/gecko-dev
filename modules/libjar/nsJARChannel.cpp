...
nsJARInputThunk::Init()
{
  ...
  if (ENTRY_IS_DIRECTORY(mJarEntry)) {
    ...
    rv = mJarReader->GetInputStreamWithSpec(
        mJarDirSpec, mJarEntry, getter_AddRefs(mJarStream));
  } else {
...
NS_IMETHODIMP
nsJARInputThunk::ReadSegments(nsWriteSegmentFun writer,
                              void* closure,
                              uint32_t count,
                              uint32_t* countRead)
{
                             ...
                             nsJARInputThunk** resultInput)
{
  ...
  if (mPreCachedJarReader) {
    ...
    if (mInnerJarEntry.IsEmpty())
      ...
      rv = jarCache->GetInnerZip(
          clonedFile, mInnerJarEntry, getter_AddRefs(reader));
  } else {
  ...
  if (mProgressSink && NS_SUCCEEDED(rv)) {
    if (NS_IsMainThread()) {
      ...
      NS_DispatchToMainThread(
          NewRunnableMethod<uint64_t>("nsJARChannel::FireOnProgress",
                                      this,
