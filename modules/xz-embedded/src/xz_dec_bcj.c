  ...
  for (i = 0; i < size; ++i) {
    ...
    if (bcj_x86_test_msbyte(buf[i + 4])) {
      ...
      while (true) {
        ...
        b = (uint8_t)(dest >> (24 - j));
        if (!bcj_x86_test_msbyte(b)) break;
 ...
 */
static void
bcj_apply(struct xz_dec_bcj* s, uint8_t* buf, size_t* pos, size_t size)
{
  size_t filtered;
 ...
 */
static void
bcj_flush(struct xz_dec_bcj* s, struct xz_buf* b)
{
  size_t copy_size;
               ...
               struct xz_buf* b)
{
  ...
  if (s->temp.filtered > 0) {
    bcj_flush(s, b);
    if (s->temp.filtered > 0) return XZ_OK;

    if (s->ret == XZ_STREAM_END) return XZ_STREAM_END;
  }
  ...
  if (s->temp.size < b->out_size - b->out_pos || s->temp.size == 0) {
    ...
    /*
		 ...
		 */
    if (b->out_pos + s->temp.size < b->out_size) return XZ_OK;
  }
  ...
  if (b->out_pos < b->out_size) {
    ...
    if (s->ret != XZ_OK && s->ret != XZ_STREAM_END) return s->ret;
    ...
    /*
		 ...
		 */
    if (s->ret == XZ_STREAM_END) s->temp.filtered = s->temp.size;
...
XZ_EXTERN struct xz_dec_bcj*
xz_dec_bcj_create(bool single_call)
{
  struct xz_dec_bcj* s = kmalloc(sizeof(*s), GFP_KERNEL);
  if (s != NULL) s->single_call = single_call;
